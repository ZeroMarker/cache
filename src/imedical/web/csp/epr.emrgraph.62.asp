<%@ Language=VBScript %>
<% 'LCID=3081 2070
 'Log 58659 YC - took out LCID so that different locale dates can be used (TRC 3224)
 'Translated all messages for this page - defined in ChartFX.Link component
%>
...
if ChartFXVer="6.2" then
	Dim AF_NONE, AS_SINGLELINE, LINES, MK_NONE, Curve
	AF_NONE = AxisFormat_None
	AS_SINGLELINE = AxisStyle_SingleLine
	LINES = Gallery_Lines
	MK_NONE = MarkerShape_None
	Curve = Gallery_Curve
End If
%>
		...
		For i = 0 to 7
			...
			For j = 0 to Days-1
				if j<=Ubound(LineData) Then
					...
					if ChartFXVer="6.2" then
						GraphChart.Value(i+iSeriesCount,j) = CDbl(LineData(j))  'x + (j * 10)					
						GraphChart.XValue(i+iSeriesCount,j) = j*1440
					Else
