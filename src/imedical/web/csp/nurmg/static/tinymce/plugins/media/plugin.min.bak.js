...
(function () {
    ...
    var createPreviewIframeNode = function (editor, node) {
      ...
      if(name=='video'){
        ...
        previewNode.attr({
          src: src,
          controls: 'controls', //显示播放工具就取消这行的注释
		  //autoplay: 'autoplay',
		  //loop: 'loop',
	  ...
	  previewWrapper.attr('style', 'border: none;'); //wangpf 去掉border
	...
	//wangpf
	var register$wpf = function (editor) {
	  editor.ui.registry.addContextMenu('media', {
        update: function (element) {
          return ((element.className=='mce-shim' && (element.parentNode.firstChild.nodeName == 'VIDEO' || element.parentNode.firstChild.nodeName == 'AUDIO')) || (element.nodeName == 'VIDEO' || element.nodeName == 'AUDIO')) ? ['media'] : [];
        }
      });
    };
