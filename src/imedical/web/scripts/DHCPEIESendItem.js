...
function Save_Click()
{
	var Strings="",ID="",CostFeeMin="",CostFeeMax="",BeginDate="",EndDate="",GroupFlag="N"
	var ParRef="",SP_1=""
	...
	if (obj) ID=obj.value;
	var obj=document.getElementById("CostFeeMin");
	if (obj) CostFeeMin=Trim(obj.value);
	if (CostFeeMin=="")
	{
		alert("价格不能为空");
		return false;
	...
	var obj=document.getElementById("CostFeeMax");
	if (obj) CostFeeMax=Trim(obj.value);
	
	Strings=ParRef+"^"+CostFeeMin+"^"+CostFeeMax+"^"+BeginDate+"^"+EndDate+"^"+GroupFlag+"^"+SP_1;
	var encmeth="";
...
{	
	...
	var objtbl=document.getElementById('tDHCPEIESendItem');	//取表格元素?名称
...
function FillData(CurRow)
{
	...
	else
	{
		...
		var DataStr=cspRunServerMethod(encmeth,RowID,"IS");
	}
	var StrArr=DataStr.split("^");
	obj=document.getElementById("RowID");
	if (obj) obj.value=RowID;
	obj=document.getElementById("CostFeeMin");
	if (obj) obj.value=StrArr[0];
	obj=document.getElementById("CostFeeMax");
	...
	obj=document.getElementById("GroupFlag");
	if (obj) obj.value=StrArr[4];
}
//去除字符串两端的空格
