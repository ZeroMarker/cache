...
function initCirCle(){
	 ...
	 context.beginPath();
	 context.arc(21,28,12,0,Math.PI*2,true); //Math.PI*2是JS计算方法，是圆
	 context.closePath();
	 context.fill();
	 var canvas=document.getElementById("topcircle");
	 var context=canvas.getContext("2d");
	 ...
	 context.beginPath();
	 context.arc(21,28,6,0,Math.PI*2,true); //Math.PI*2是JS计算方法，是圆
	 context.closePath();
	 context.fill();	
}
...
function GetPatcurdetail(){
	runClassMethod("web.DHCDISDetail","FindDisDetail",{'taskID':mainRowID,'typeID':typeID},function(data){	
			if(data!=""){
				var list=data.split("$$"); 
