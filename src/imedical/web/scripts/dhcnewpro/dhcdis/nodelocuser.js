...
function save(){
	...
	saveByDataGrid("web.DHCDISNodeLocUser","SaveUpdNodeLocUser","#datagrid",function(data){
			if(data==0){
...
function cancel(){
    ...
    if (r){
	    var row =$("#datagrid").datagrid('getSelected');     
		 runClassMethod("web.DHCDISNodeLocUser","RemoveNodeLocUser",{'Id':row.ID},function(data){ $('#datagrid').datagrid('load'); })
    }    
}); 
}
