...
function initTempPageData(data){
	...
	if(tempDataArr[7]==""){
		...
		$("#p-panel").css(cssObj);
		$("#p-demoArea").is(":hidden")?$("#p-panel").html(tempDataArr[7]):$("#p-demoArea-text").val(tempDataArr[7]);
	}else{
		$("#p-demoArea").is(":hidden")?$("#p-panel").html(tempDataArr[7].saveOrShowDataFormatNew("show")):$("#p-demoArea-text").val(tempDataArr[7].saveOrShowDataFormatNew("show"));	
	}	
...
function addTable(){
	...
	if($("#p-panel").find("table").length){
		...
		if($("#p-panel").find("table").attr("style").indexOf("word-break")==-1){
			...
			$.messager.popover({msg: '成功同步table的break-all属性',type:'success',timeout: 2000});
			return;
		}
