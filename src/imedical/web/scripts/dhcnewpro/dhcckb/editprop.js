...
var parentId = getParam("parentId"); // 药品字典id 
var dialogFlag = getParam("dialogFlag"); // 弹窗标志, 用于修改极简模式下, 弹窗调用时的背景色
var EntLinkId="";					// 属性Id
var property="" ;  //选择属性
/// 页面初始化函数
...
function InitCombobox(){
	...
	var option = {
	    ...
	    loadFilter:function(data){                
			for(var i = 0; i < data.length; i++){
			    if(data[i].text != "西药" && data[i].text != "中成药" && data[i].text != "中药饮片" && data[i].text != "中药方剂"){
			        data.splice(i,1);
	...
	var genNameId = serverCall("web.DHCCKBCommon","GetGeneralFromData",{});
	///通用名
	var uniturl = $URL+"?ClassName=web.DHCCKBDiction&MethodName=QueryDicComboboxByID&id="+genNameId  
  	$HUI.combobox("#genername",{
	    url:uniturl,
	    valueField:'id',
		textField:'text',
		panelHeight:"150",
    ...
    //检索条件
    var uniturl = $URL+"?ClassName=web.DHCCKBEditProp&MethodName=GetAttrComboxIdData" ;
	$HUI.combobox("#queryDic",{
	    url:uniturl,
	    valueField:'value',
		textField:'text',
		panelHeight:"150",
...
function InitDataList(){
	...
	var columns=[[   	 
			...
			{field:'dicCode',title:'代码',width:200,align:'left',editor:texteditor,hidden:true},
			{field:'dicDesc',title:'描述',width:350,align:'left',editor:texteditor},
			{field:'ruleStatus',title:'结构规则',width:60,align:'center',editor:texteditor},
			{field:'tiny',title:'说明书',width:60,align:'center',formatter:setLogCellOpen},  //ld  2022-11-28
			{field:'log',title:'日志',width:40,align:'center',formatter:setEditLogCellOpen},
	...
	var params = "";
	var uniturl = $URL+"?ClassName=web.DHCCKBEditProp&MethodName=GetDicInstanceByParrefNew&extraAttr="+"DataSource"+"&parref="+dicParref+"&params="+params+"&drugType="+InitDrugType()+"&hospID=";
	
	new ListComponent('diclist', columns, uniturl, option).Init();
	
}
///设置日志查看连接  lidong
function setEditLogCellOpen(value, rowData, rowIndex){
	return "<a href='#' class='icon icon-clock-record' style='color:#000;display:inline-block;width:16px;height:16px' onclick=\"showEditLogWin('"+rowData.dicID+"','"+rowData.parref+"')\"></a>";
	 

}
/// 查看日志详情  lidong
function showEditLogWin(ID,parref){
	var dicParref = getParam("dicParref");
	var linkUrl = "dhcckb.editlog.csp"
	var title = "日志详情"
	...
	if ("undefined"!==typeof websys_getMWToken){
		openUrl = '<iframe scrolling="yes" width=100% height=100%  frameborder="0" src="'+linkUrl+'?ID='+ID+'&parref='+parref+'&dicParref='+dicParref+"&MWToken="+websys_getMWToken()+'"></iframe>'; 
	}
		
	if($('#logwin').is(":visible")){return;}  			//窗体处在打开状态,退出
	$('body').append('<div id="logwin"></div>');
	var myWin = $HUI.dialog("#logwin",{
        iconCls:'icon-w-find',
        title:title,
        modal:true,
        width:1000,
...
function ShowMoreBtn()
{
	...
	if($("#showtr").is(':hidden')){ 
	  $("#showtr").show();
	  $("#searchmore .l-btn-text").text("还原");
	}else
	  {
		$("#showtr").hide();	
		$("#searchmore .l-btn-text").text("更多");  
	  }	
}
/// 属性DataGrid初始定义通用名
function InitSubDataList(){
	...
	var columns=[[   
			...
			{field:'log',title:'日志',width:40,align:'center',formatter:setEditDetLogCellOpen},
	...
	var option={	
		...
		onDblClickRow:function(rowIndex,rowData){  
            ...
            for (var i = 0; i < editors.length; i++){  
	           ...
	           //lidong 2022-9-21  combobox无法删除问题       
	           if((e.field == "AttrValue")&&(e.type=="combobox")) {
	           	$("input",$(e.target).next("span")).blur(function(){
					var selItem = $(e.target).combobox('getValue');
					var selIteme = $(e.target).combobox('getText');
					if(selItem==""){
						var ListData = "" +"^"+ parref +"^"+ EntLinkId +"^"+ selItem +"^"+""+"^"+ "" +"^"+ "";
	                	saveTypeData(ListData,e.type)
						}
...
///设置日志查看连接  lidong
function setEditDetLogCellOpen(value, rowData, rowIndex){
	return "<a href='#' class='icon icon-clock-record' style='color:#000;display:inline-block;width:16px;height:16px' onclick=\"showEditDetLogWin('"+rowData.attrID+"')\"></a>";
	 

}
/// 查看日志详情  lidong
function showEditDetLogWin(ID){
	var linkUrl = "dhcckb.editdetlog.csp"
	var title = "药品属性日志详情"
	var openUrl = '<iframe scrolling="yes" width=100% height=100%  frameborder="0" src="'+linkUrl+'?ID='+ID+'&parref='+parref+'"></iframe>';
	if ("undefined"!==typeof websys_getMWToken){
		openUrl = '<iframe scrolling="yes" width=100% height=100%  frameborder="0" src="'+linkUrl+'?ID='+ID+'&parref='+parref+"&MWToken="+websys_getMWToken()+'"></iframe>'; 
	}
...
{
	var params = $HUI.searchbox("#queryCode").getValue() +"^" + "^" + $HUI.combobox("#genername").getText()+ "^^" + $HUI.combobox("#queryDic").getValue();
	var hospId=$("#hospId").combobox('getValue');
	dicParref = $("#dicType").combobox('getValue');
...
function SubQueryDicList(){
	...
	var params=dicParref+"^"+parref+"^"+property +"^"+ $HUI.checkbox("#genMatch").getValue(); 
	//var options={}
	//options.url=$URL+"?ClassName=web.DHCCKBEditProp&MethodName=GetAttrListData&params="+params+"&drugType="+InitDrugType();
	var url = $URL+"?ClassName=web.DHCCKBEditProp&MethodName=GetAttrListData&params="+params+"&drugType="+InitDrugType();
	$('#linkattrlist').datagrid({	
		url:url
	});
}
		...
		$.messager.confirm("提示", "您确定要删除这些数据吗？", function (res) {//提示是否删除
			if (res) {
				runClassMethod("web.DHCCKBDicLinkAttr","DelDicAttr",{"linkRowID":ItmId},function(jsonString){
					if(jsonString!=0){
						$.messager.alert('提示',"ErrMsg:"+jsonString,"error")
					}
					$('#linklist').datagrid('reload'); 
				})
			}
...
function InsertDsource(flag)
{
	...
	runClassMethod("web.DHCCKBRuleMaintain","saveOrUpdateData",{"params":ListData,"LoginInfo":LoginInfo,"ClientIPAddress":ClientIPAdd},function(jsonString){
		if (jsonString == 0){
			...
			$.messager.alert('提示','保存失败,名称重复.','warning');
		}
		else{
    ...
    $HUI.combobox("#propertyComBox",{
		...
		*/
		onChange:function(ret){
			var param=dicParref+"^"+parref+"^"+ret;
			$('#linkattrlist').datagrid('load',{
			params:param
			}); 
		}
...
//url:$URL+'?ClassName=web.DHCCKBDicLinkAttr&MethodName=GetDataCombo&DataSource='+jsonString[j].source+'&filed='+jsonString[j].Filed,
	...
	if((rowsData==null)&&((parentId=="")||(parref==""))){
...
{	
	var rowsData = $("#diclist").datagrid('getSelected');
	var paref = rowsData==null?parentId:rowsData.parref;
	var dicID = rowsData==null?parref:rowsData.dicID;
...
function InitPageClass() {
	if ((dialogFlag == 1) && ((typeof HISUIStyleCode == "string") && (HISUIStyleCode == "lite"))) {
