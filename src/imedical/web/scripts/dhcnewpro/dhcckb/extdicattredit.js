...
function InitCombobox(){
	...
	//初始化字典下拉框
	var url = $URL+"?ClassName=web.DHCCKBDicExtLinkAttr&MethodName=QueryExtDic"+"&drugType="+InitParams();
	$('#treedic').combobox({
		url:url,
		valueField: 'value',
		textField: 'text',
		...
		onSelect:function(option){
	        //var datatype = serverCall("web.DHCCKBEditDiction","GetDicDataType",{'dicID':dicParref})
	        dataType = option.code;
	        var params = $HUI.searchbox("#queryCode").getValue();
	        var hosp = $HUI.combobox("#hosp").getValue();
	        if(hosp==""){hosp = LgHospID;}

	        $("#diclist").datagrid("load",{"HospID":hosp,"Type":option.code,"params":params});
		}
	})
	$('#griddic').combobox({
		url:url,
...
function InitDataList(){
	...
	var columns=[[   	 
			...
			{field:'EDCode',title:'代码',width:200,editor:texteditor},
			{field:'EDDesc',title:'描述',width:350,align:'left',editor:texteditor},
			{field:'EDType',title:'类型',width:250,align:'left',hidden:true},
			{field:'EDHospID',title:'医院',width:200,align:'left',hidden:true}
	...
	var option={	
		...
		pageList:[30,60,90],		
 		onClickRow:function(rowIndex,rowData){
	 		parref=rowData.EDRowID; 
	 		dicParref = $HUI.combobox('#griddic').getValue();
	 		var params = dicParref+"^"+parref;
				var options={}
				options.url=$URL+"?ClassName=web.DHCCKBDicExtLinkAttr&MethodName=QueryExtDicLInkAttrVal&params="+params;
 		...
 		onClickRow:function(rowIndex,rowData){   
 			EntLinkId=rowData.attrID;
 			 if (editsubRow != ""||editsubRow == 0) {    //wangxuejian 2021-05-21  关闭编辑行 
                $("#linkattrlist").datagrid('endEdit', editsubRow); 
            } 
		}, 	
		onDblClickRow:function(rowIndex,rowData){  
...
function InitAddgridDataList(attrCode,attrID,htmlType){
	...
	runClassMethod("web.DHCCKBDicLinkAttr","GetColumnsByDicCode",{"AttrId":"","DicCode":attrCode},function(jsonString){
		...
		var uniturl = $URL+"?ClassName=web.DHCCKBDicExtLinkAttr&MethodName=QueryEntyAttr&params="+params+"&Type="+htmlType;
...
function AddAttrValue(attrID,htmlType,attrCode){
	...
	if (htmlType == "textarea"){			
		...
		var options={}
		var params = parref +"^"+attrID;
		options.url=$URL+"?ClassName=web.DHCCKBDicExtLinkAttr&MethodName=QueryEntyAttr&params="+params+"&Type="+htmlType;
...
{
	runClassMethod("web.DHCCKBDicExtLinkAttr","saveDicAttrByType",{"ListData":params,"Type":htmlType,"LoginInfo":LoginInfo,"ClientIPAddress":ClientIPAdd},function(jsonString){
		if (jsonString < 0){
...
function DelBtn()
{
	...
	if (rowsData != null) {
		$.messager.confirm("提示", "您确定要删除这些数据吗？", function (res) {//提示是否删除
			if (res) {
				runClassMethod("web.DHCCKBDicExtLinkAttr","DelData",{"params":params},function(jsonString){
					if (jsonString!="0")
					{
...
function UpdAttrDiction()
{
	...
	$("#UpdDicWin").show();
	$HUI.combobox("#dicDesc").setValue("");	  //dic
	var option = {
		modal:true,
		collapsible : false,
		minimizable : false,
...
function saveDiction()
{
	...
	var selecItm=$("#diclist").datagrid('getSelected');
	var node=$("#dictree").tree('getSelected');
	var dicId=selecItm==null?(node.EDRowID):(selecItm.EDRowID);
	var dictionId=$HUI.combobox("#dicDesc").getValue();
	if(dictionId==""){
		$.messager.alert("提示","请选择字典！");
		return;
	}
	runClassMethod("web.DHCCKBDiction","UdpDictions",{"DicId":dicId,"DictionId":dictionId},
        	function(data){
            	if(data==0){
