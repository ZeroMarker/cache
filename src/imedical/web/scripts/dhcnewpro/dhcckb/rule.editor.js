...
function initPageDefault()
{
	...
	if(_json[0].ruleId==0){
		...
		html = html + '	<desc style="-webkit-tap-highlight-color: rgba(0, 0, 0, 0);">Created with Raphaël 2.2.0</desc>'; //hxy 2021-03-27 谷歌ok,ie不兼容问题-在无线状态下画线时线不能显示解决 st add
		...
		html=html+"<div id='then' style='padding: 5px;' class='ui-sortable'>"
		html=html+"</div>"			  
		//Part3st			
		...
		html=html+" <div id='else' style='padding: 5px' class='ui-sortable'>"	    
		html=html+" </div>"
		html=html+"</div>"
		$("#line").html(html);
	...
	var htmlTwo="<span><strong class='font-12'>正向审查</strong><span class='rule-add-action font-12' onclick='addelseItm(1)'>添加动作</span></span>"
	...
	htmlThr=htmlThr+" <span><strong class='font-12'>逆向审查</strong><span class='rule-add-action font-12' onclick='addelseItm(2)'>添加动作</span></span>"
...
function addelseItm(other)
{
	...
	html = html +"	<span class='line-darkcyan-zero' par_data='' data_id='' onclick='chooseVar(this)'>请选择变量</span>"  //sufan 新增par_data=''
...
function displayPathCon(xLev, yLev, con,yLevSt) //2020-11-09 add ,yLevSt
{
    ...
    if (con.value.extconstvar!="") { //2022-04-20 qnp 放消息级别
     ...
     htmlOne=htmlOne+' <span style="color: red;">=</span>'
     htmlOne=htmlOne+' <span><span onclick="chooseDatasetlevel(this)" data_id="'+con.value.extconstvar+'" class="lineext-blue">'+ con.value.extconstvarName +'</span></span>)</span>'
		...
		htmlOne=htmlOne+"  <span class='manlevopeator' onclick='chooseManLevCombox(this)'> <img src='../scripts/dhcnewpro/dhcckb/images/ruleindex_editman.png' alt=''/> </span>" 
	}
...
function displayPathConAppend(xLev, yLevSt, yLev)
{
	...
	htmlOne=htmlOne+"	<span style='font-size:12px'>"
	htmlOne=htmlOne+"  <span class='line-darkcyan' par_data='' data_id='' onclick='chooseVar(this)'>请选择变量1</span>"			//sufan 新增 par_data
	...
	htmlOne=htmlOne+"  <span class='manlevopeator' onclick='chooseManLevCombox(this)'>	<img src='../scripts/dhcnewpro/dhcckb/images/ruleindex_editman.png' alt=''/></span>" //2022-04-20 qnp 放消息级别
	htmlOne=htmlOne+"	</span>"
	htmlOne=htmlOne+"	<span class='icon icon-cancel' onclick='deleteCon(this)'>&nbsp;</span>"
	htmlOne=htmlOne+"</div>"
...
function resetHtml(x1,y1){
	...
	$("#line").find("path").each(function(){ //path
	    ...
	    if(seatY>y1){
		 	...
		 	seatDTwoNew=seatDTwo.split(seatDYOld).join(seatDYNew);
		 	//seatDTwoNew=seatDTwo.replace(new RegExp(seatDYOld,'g'),seatDYNew)
...
function saveRule(status){
	...
	if(len>0){
		...
		$.messager.alert("提示","请选择 "+text+" 输入值类型！");
		return;
	}//ed
	...
	if(flagStr!=""){
		$.messager.alert("提示","“变量赋值”动作【"+flagStr+"】下未选择具体属性，该动作只能针对具体变量下的属性进行赋值，不能对变量本身进行赋值!");
		return;
	}
	if(thenJson!=""){
...
function chooseCombox(choose){
    ...
    if(choose=="del"){
	  ...
	  if((number<1)||(numberPath>0)){ //if(number<1){
		  resetHtml(x1,y1);
		  height=$("#line>div:first").height()-30; //st 删除逻辑的同时,调整高度
		  if(height==30)height=40;
			$("#line>div:first").height(height); //ed
	  } //ed	  
	  $(".combo-p").css("display","none");
...
function chooseManLevCombox(option){
	...
	htmlOne=htmlOne+'<span>';
	htmlOne=htmlOne+'(';
	htmlOne=htmlOne+'<span class="lineext-darkcyan" onclick="chooseLevelVar(this)">请选择变量</span>'; //2022-04-20 qnp 放消息级别
	htmlOne=htmlOne+'<span style="color: red;">=</span>'
    htmlOne=htmlOne+'<span><span onclick="chooseDatasetlevel(this)" data_id="1" class="lineext-blue">请选择数据集</span></span>'
    htmlOne=htmlOne+')';
    htmlOne=htmlOne+'</span>';
