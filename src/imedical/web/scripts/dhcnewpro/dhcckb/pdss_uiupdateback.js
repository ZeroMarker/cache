...
(function(window){
	...
	function getFormatHtml(tempHtml, area) {
		...
		var areaObj = $(tempHtml).find('.area[data-area="'+ area +'"]');
		// 标题区域
		if(area == "monHead"){
			htmlstr = htmlstr + getFormatRowsHtml(areaObj[0], area);
		}
		
		// 目录区域
		if (area == "monBody"){
			var areaObj = $(tempHtml).find('.area[data-area="'+ area +'"][data-areatype!="ALL"]');
			if (areaObj.length > 0){
				for(var m=0; m<areaObj.length; m++){
					htmlstr = htmlstr + '<% if ( label == "'+ $(areaObj[m]).attr("data-areatype") +'" ) { %>';
					htmlstr = htmlstr + getFormatRowsHtml(areaObj[m], area);
					htmlstr = htmlstr + '<% isExecFlag = 0; } %>';
				}
			}
			var areaObj = $(tempHtml).find('.area[data-area="'+ area +'"][data-areatype="ALL"]');
			if (areaObj.length > 0){
				htmlstr = htmlstr + '<% if ( isExecFlag ) { %>';
				htmlstr = htmlstr + getFormatRowsHtml(areaObj[0], area);
				htmlstr = htmlstr + '<% } %>';
			}
		}
		...
		if (area == "monBase"){
			for(var m=0; m<areaObj.length; m++){
				htmlstr = htmlstr + '<div class="ckb-view-base-item" >';
				htmlstr = htmlstr + getFormatRowsHtml(areaObj[m], area);
				htmlstr = htmlstr + '</div>';
			}
		}
	...
	function getFormatRowsHtml(areaObj, area) {
		
		var htmlstr = "";
		// var rows = $(tempHtml).find('.area[data-area="'+ area +'"] .row');
		//var rows = $(areaObj).find('.row');		
		var rows = $(areaObj).find('.ckb-row');
		for(var m=0; m<rows.length; m++){
			
			htmlstr = htmlstr +'<div class="ckb-row">';
			/// 默认添加项目序号及目录样式标识
			if (m == 0){
...
//									'</div>'+				
		...
		bin_event : function() {
			...
			$("#bt_audit").bind('click',function(){
				...
				me.controlFlag = "Y";	
				var reasonArr = [];											
				$("textarea[name='ckb-reason']").each(function() {
					var reasonObj={};
				    //if ($(this).val().trim() != '') {
					...
					/*var reasoncomb=$("#reasoncomb"+$(this).attr("data-unique")).combobox("getValue"); //xww 2021-12-01 
					if(reasoncomb!=""){
						reasonObj.reasoncomb =reasoncomb;
					}
					*/
					if (selectReason != ""){
		...
		relocation : function() {
	        ...
	        //El.style.top = (Height - El_Height)/2+"px";	// 居中
	        //El.style.left = (Width - El_Width)/2+"px";
	        //El.style.top = 1+"px";	// 左上角
	        //El.style.left = 1+"px";
	        El.style.top = 1+"px";	// 右上角
	        El.style.left = (Width - El_Width)+ 1 +"px";
	        ...
	        //VR_El.style.top = 1+"px";
	        //VR_El.style.left = 1 +"px";
	     	VR_El.style.top = 1+"px";
	        VR_El.style.left = (Width - El_Width)+ 1 +"px";	        
		...
		temp_html : function(jsonObj) {
			...
			var tempbodyHtml = getFormatHtml(tempHtml, "monBody"); /// 审查目录
			var tempDetaHtml = getFormatHtml(tempHtml, "monDeta"); /// 详情信息
			var tempBaseHtml = getFormatHtml(tempHtml, "monBase"); /// 数据信息
		    ...
		    var tempHtml = "";
				...
				tempHtml = tempHtml + '   	<% for( var j = 0; j < item.warns.length; j ++ ) { %> <% var warn = item.warns[ j ]; var itemCss = j==item.warns.length-1?"":"ckb-view-item-bottom-line"; var label = warn.keyname;%>';
				...
				//tempHtml = tempHtml + '				<%if (rule.manLevel != "forbid")  continue;%>';	// 非禁止规则不显示
				...
				tempHtml = tempHtml + '   				<% for (var m = 0; m<rule.itms.length; m++){ var val = rule.itms[m].val; var isExecFlag = 1;%>';
				...
				tempHtml = tempHtml +     tempDetaHtml;
				tempHtml = tempHtml + '</div>';	// detail结束
				
				tempHtml = tempHtml + '<div class="ckb-view-base" data-id="WL<%-item.unique %>" data-itmid="<%-item.itmID %>">';
				tempHtml = tempHtml +     tempBaseHtml;
				tempHtml = tempHtml + '</div>';
				
				tempHtml = tempHtml + '<div class="ckb-view-education" data-id="WL<%-item.unique %>" data-itmid="<%-item.itmID %>">';
				//tempHtml = tempHtml +     tempBaseHtml;
				tempHtml = tempHtml + '</div>';	// 用药教育结束
				
				
				tempHtml = tempHtml + '<div class="ckb-view-docnotes" id="WL<%-item.unique %>" data-id="WL<%-item.unique %>">';
				...
				tempHtml = tempHtml + '    </div>';
				//tempHtml = tempHtml + '	   <div class="view-reasoncomb">';
				//tempHtml = tempHtml + '			<input id="reasoncomb<%-item.unique %>" data-unique="<%-item.unique %>" class="hisui-combobox" style="width:300px"/  > '
				//tempHtml = tempHtml + '	   </div>';
				tempHtml = tempHtml + '    <div style="margin-top:5px;"><textarea name="ckb-reason" id="reason<%-item.unique %>" data-unique="<%-item.unique %>" data-seqno="<%-item.seqno%>" data-item="<%-item.item %>" data-manlevel="<%-item.manLevel %>" style="width:560px;height:30px;resize:none;margin-left:88px;" required></textarea></div>';
				tempHtml = tempHtml + '</div>';	// detail结束
		...
		formatTemp : function () {
			...
			for (var m=0; m<=itemgroup.length; m++){
				...
				for (var n=0; n<item_group_rows.length; n++){
					...
					var els = $(item_group_rows[n]).find('.ckb-row');
				...
				if (typeof data.ErrorCode == "undefined"){
				 /*
					...
					*/
					...
					//me.initDocNotes();				
					if (data.manLevel != "forbid"){	// 不是禁止级别的默认显示5秒后，直接关闭( 2021-04-19) 非禁止级别不需要强制审核
						//$('#bt_audit').addClass('forbid');		
						$('#bt_audit').unbind("click");	
						$('#bt_audit').hide();	
						$(".ckb-view-control").hide();
						$('#ckb_view_win').delay(5000).hide(0);	
					}	
					if (data.UseType == "Pha"){
						$("textarea[name='ckb-reason'").attr("disabled",true); 
						$("input[type=checkbox][name='reagrp']").attr("disabled",true); 		
					}
														
					/*
					if (data.manLevel == "forbid"){	// 禁止级别不让强制审核(哈医大 2021-04-14)
						$('#bt_audit').addClass('forbid');		
						$('#bt_audit').unbind("click");		
					}*/
...
function InitCombobox(){
	/*$HUI.combobox("#reasoncomb",{
		url: LINK_CSP+"?ClassName=web.DHCCKBReasTemp&MethodName=JsonGetReason&LocID="+LgUserID+"&UserID="+LgUserID,
		valueField: "value", 
		textField: "text",
		editable:true,
	})*/
	
	$("input[id^='reasoncomb']").each(function(){
		
		$HUI.combobox(this,{
			url: LINK_CSP+"?ClassName=web.DHCCKBReasTemp&MethodName=JsonGetReason&LocID="+LgCtLocID+"&UserID="+LgUserID,
			valueField: "value", 
			textField: "text",
			editable:true,
		})
