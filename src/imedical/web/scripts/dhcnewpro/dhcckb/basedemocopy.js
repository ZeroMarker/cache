...
function InsTemp(){
	...
	if (!$.isEmptyObject(TempObj)){
		InvInsTemp(ID, $("#TempTitle").val(), TempObj);
	}
}
...
function TakTempObj(){
	...
	TempObj.itemDis = TrGridToArr("itemDis");      /// 疾病诊断
...
function InvInsTemp(ID, TempTitle, jsTempObj){
	...
	runClassMethod("web.DHCCKBBaseDemo","InsTestCaseTemp",{"ID":ID, "TempTitle":TempTitle, "jsTempObj":JSON.stringify(jsTempObj)},function(ID){
		if (ID < 0){
			$.messager.alert("提示:","保存失败，失败原因:"+jsonString);
		}else{
...
function TrGridToHisOrderArr(id){
	...
	for (var m in rowsData){
		if (id == "itemOrder"){	
			...
			if ((tmpData[m].OrdEndDate =="")){	// 医嘱停止日期不为空,则认为是历史医嘱
				continue;
			}
			TmpArr.push(tmpData[m]);			
		}else{
...
///模板管理
function DelTemp(){
	 var TempID = $HUI.combobox("#TakTemp").getValue();
	 if(TempID == ""){
