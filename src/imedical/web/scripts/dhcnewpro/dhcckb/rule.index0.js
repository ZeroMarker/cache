...
　　　　 	// 在拖动一个节点之前触发。
	...
	$('#commonRuleTree').tree({
    	...
    	url:LINK_CSP+'?ClassName=web.DHCCKBRuleIndex&MethodName=GetSortTreeJsonData&LoginInfo='+LoginInfo,
    	lines:true,
		animate:true,
		onContextMenu: function(e, node){
		...
		onSelect: function(rec){
			...
			runClassMethod("web.DHCCKBRuleIndex","ListModelDataGrid",{"parent":rec.id},function(data){
				...
				$('#modelTable').datagrid({
					...
					url:LINK_CSP+"?ClassName=web.DHCCKBRuleIndex&MethodName=ListModelDataNew&parent="+rec.id+"&drugID="+drugID+"&hospId="+LgHospID,  //xww 2021-08-06 新加drugID ,
					columns:[json],
...
function onClickTreeNode(node){
		 ...
		 var url="dhcckb.rule.editor.csp?ruleId="+ruleId;
		 $('#ruleFrame').attr('src', url);	
}
...
function CreateRule(){
	...
	var url="dhcckb.rule.editor.csp?ruleId="+ruleId+"&relation="+ dicCode+"&dicCode="+relation;
	$('#ruleFrame').attr('src', url);	
}

function DeleteRule(){
	var curNode = $("#ruleTree").tree('getSelected');
	var curNodeDomId = curNode.domId;  //右键时会选中当前的节点 '_hisui_tree_702'
	var curNodeParref = $("#"+curNodeDomId).parent().parent().prev().attr("id");
	var node = $("#ruleTree").tree('getChecked');
	var nodeArr = []
	for (i=0;i<node.length;i++){
		var seNodeDomId = node[i].domId;
		var seNodeParref = $("#"+seNodeDomId).parent().parent().prev().attr("id");
		if (seNodeParref != curNodeParref){	// 只删除当前节点选中的规则
			continue;
		}
...
function AddRule(id){
	...
	var url="dhcckb.rule.editor.csp?ruleId=0&relation="+ dicCode+"&dicCode="+AddRuleFlag;
	$('#ruleFrame').attr('src', url);	
}
...
function getRuleIframe(ruleId){
	$('#ruleFrame').attr('src', "dhcckb.rule.editor.csp?ruleId="+ruleId); 
}
