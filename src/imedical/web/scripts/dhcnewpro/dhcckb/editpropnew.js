...
{
	var params = $HUI.searchbox("#queryCode").getValue() +"^" + "^" + $HUI.combobox("#genername").getText();;
...
function editdivComponent(opt,callback){
		...
		$div = ("<div id='myEditor' style='width:"+option.width+" ;height:"+option.height+";display:none;position:absolute;background: #99FFFF;'></div>")
		...
		if($.trim(option.input)!="")
		{
			editor.addListener('focus',function(){
				editor.setContent(option.input);
			})
		...
		UE.getEditor('myEditor').addListener('blur',
			function(editor){
				//destroy()
				closeDiv();
		});
...
function showDiv() {
    //通过doc得到div赋值给变量div
    var div = document.getElementById("myEditor");
    //设置div的属性为block即显示
