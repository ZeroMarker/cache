   ...
   $("#dicLookUp").lookup({
      ...
      //目录
		...
		$('#micrData').click(function(e)
		  {
		    micrDataImp();      /// 导入数据  wangxin 2020-12-10
		  });
...
function initTree(){
	$('#ruleTree').tree({
    	url:LINK_CSP+"?ClassName=web.DHCCKBRuleIndex&MethodName=GetDrugLibraryTree&dic="+DrugId+"&userInfo="+LoginInfo,
    	lines:true,
		animate:true,
		dnd:true,
		checkbox:true,
...
　　　　 	// 在拖动一个节点之前触发。
	...
	$('#modelTree').combotree({
    	url:LINK_CSP+'?ClassName=web.DHCCKBRuleIndex&MethodName=ListModelTree',
    	lines:true,
		animate:true,
		onSelect: function(rec){
			...
			runClassMethod("web.DHCCKBRuleIndex","ListModelDataGrid",{"parent":rec.id},function(data){
				...
				$('#modelTable').datagrid({
					...
					url:LINK_CSP+"?ClassName=web.DHCCKBRuleIndex&MethodName=ListModelDataNew&parent="+rec.id,
					columns:[json],
...
function initSelectTabs(){
	...
	$HUI.tabs("#ruleTabs",{
		onSelect:function(title){
			...
			if ((title=="子规则定义")||(title=="全局规则")){				 		
				$('#ruleLayout').layout('hidden','west');          
			}else{				
				$('#ruleLayout').layout('show','west');        
			}
		}
	}); 
}
...
function onClickTreeNode(node){
		 ...
		 var url="dhcckb.rule.edit.csp?ruleId="+ruleId;
		 $('#ruleFrame').attr('src', url);	
}
...
function CreateRule(){
	...
	var url="dhcckb.rule.edit.csp?ruleId="+ruleId+"&relation="+ dicCode+"&dicCode="+relation;
	$('#ruleFrame').attr('src', url);	
}
...
function CopyRule()
{
   ...
   var nodes = $('#ruleTree').tree('getChecked');
	...
	var url = LINK_CSP+"?ClassName=web.DHCCKBRuleCopy&MethodName=GetDicComboboxList&extraAttr="+"KnowType" +"&extraAttrValue="+"DrugData";
...
function SaveCopyRuleData()
{
    ...
    var newDicDrug=$("#newDicDrug").combobox('getValue');
	$.messager.confirm("提示", "您确定要复制这些规则？", function (res) {	// 提示是否复制
		if (res) {
			
			//alert(dicID+"@@"+userInfo+"@@"+rule);
			//保存数据
			runClassMethod("web.DHCCKBDrugDetail","SaveSingleRules",{"dicID":dicID,"newDicID":newDicDrug,"LgUserId":LgUserID,"userInfo":userInfo,"rulestr":rule,"loginInfo":loginInfo},function(jsonString){
				reloadTree();
				if (jsonString == 0){
...
function AddRule(id){
	...
	var url="dhcckb.rule.edit.csp?ruleId=0&relation="+ dicCode+"&dicCode="+AddRuleFlag;
	$('#ruleFrame').attr('src', url);	
}
...
function getRuleIframe(ruleId){
	$('#ruleFrame').attr('src', "dhcckb.rule.edit.csp?ruleId="+ruleId); 
}
