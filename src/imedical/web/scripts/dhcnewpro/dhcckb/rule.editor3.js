...
function initPageDefault()
{
	...
	if(_json[0].ruleId==0){
		...
		html = html + '</div>';
   		...
   		//Part2st
		html=html+"<span><strong class='font-12'>那么</strong><span class='rule-add-action font-12' onclick='addelseItm(1)'>添加动作</span></span>"
		html=html+"<div id='then' style='padding: 5px;' class='ui-sortable'>"
		html=html+"</div>"			  
		//Part3st			
		html=html+"<div style=margin-top: 5px;>"
		html=html+" <span><strong class='font-12'>否则</strong><span class='rule-add-action font-12' onclick='addelseItm(2)'>添加动作</span></span>"
...
function saveRule(status){
	...
	$.post(
 		"web.DHCCKBRuleSave.cls",
   		{json:savejson,
		 dicStr:_dicStr, //2021-03-26 dicArr.join("^") -> _dicStr
