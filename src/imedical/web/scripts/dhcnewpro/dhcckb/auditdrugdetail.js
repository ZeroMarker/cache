...
function initDataGrid(){
	...
	var columns= [[
		...
		{field:'Length',title:'审核药品数量',width:90,hidden:false},
	...
	$HUI.datagrid('#datagrid',{
		url:'dhcapp.broker.csp?ClassName=web.DHCCKBRecipeAuditStat&MethodName=AuditDrugDetail',
		fit:true,
		rownumbers:true,
		columns:columns,
		pageSize:30,  
...
function showAuditDetailhtml(retJson){
	...
	for (var i = 0; i < itemsArr.length; i++){
		...
		$row.append('<div class="pha-col"><span class="itemlabel-xh '+ itemsArr[i].manLevel +'-bg"><label>'+ (i+1) +'</label></span></div>');
