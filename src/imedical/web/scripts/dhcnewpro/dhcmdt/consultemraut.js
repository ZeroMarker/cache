...
function initPageMainGrid(){
	...
	var columns=[[
		...
		{field:'TakOrd',title:'权限名称',editor:TakOrdEditor,width:100,hidden:true},
	...
	/// 就诊类型
	var uniturl = $URL+"?ClassName=web.DHCMDTConsultQuery&MethodName=JsCstEmrAutItem&CstID="+CstID+"&TakType="+TakType+"&MWToken="+websys_getMWToken();
...
function OpenAuthorize(rowIndex){
	...
	runClassMethod("web.DHCMDTConsult","InsEmrAutMasNoAff",{"mListData":mListData},function(jsonString){
...
function CloseAuthorize(rowIndex){
	$("#main").datagrid('endEdit', rowIndex);
	var rowData=$("#main").datagrid('getData').rows[rowIndex];
	runClassMethod("web.DHCMDTConsult","ClsCstEmrAut",{"EpisodeID":EpisodeID, "itmID":rowData.itmID,"TakType":rowData.TakOrdFlag},function(jsonString){
		if (jsonString < 0){
...
function initPageItemGrid(){
	...
	/// 就诊类型
	var uniturl = $URL+"?ClassName=web.DHCMDTConsultQuery&MethodName=JsEmrAutDet&CstID="+CstID+"&MWToken="+websys_getMWToken();
...
function OpenEmrAut(){
	...
	runClassMethod("web.DHCMDTConsult","InvEmrAutMas",{"mListData":mListData},function(jsonString){
