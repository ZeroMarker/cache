...
function InitPatEpisodeID(){
	...
	if((HASCENTER!=2)||((IsContact!="Y")&&(NOCENTPLANPAT==1))||((IsConsExperts!="Y")&&(IsContact!="Y"))){
		///会诊中心安排
		if(IsConsCentPlan!=1){
			IsHasPlantAut=false;
		}
...
//	  }
	...
	var itmCodes = "20";
	if (GetIsTakOperFlag(mdtID, itmCodes) != "1"){
...
function updRes(mdtID,mdtMakResID,DisGrpID,EpisodeID){
	var option = {
		collapsible:false,
		minimizable:false,
		maximizable:false,
	...
	new WindowUX("修改资源", 'mdtResWin', 910, 460, option).Init();
}
...
function TakMakResDate(){
	...
	var mdtPreTime = $("#mdtPreTime").val();       /// 预约时间
	var mdtMakResID = $("#mdtMakResID").val();     /// 出诊表ID
	if (!mdtMakResID){//(mdtMakResID == selMdtMakResID){
	...
	$.messager.confirm('确认对话框','您确定要将会诊时间修改为：'+ mdtPreData +" "+ mdtPreTime +'吗？', function(r){
		if (r){
			runClassMethod("web.DHCMDTConsult","TakMakResDate",{"mdtID":mdtID, "LgParams":LgParams, "makResParams":makResParams},function(jsonString){
				if (jsonString == -1){
