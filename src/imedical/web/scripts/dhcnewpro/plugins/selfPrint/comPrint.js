...
String.prototype.prtDataFormatDom = function() {
	...
	if (typeof window.DOMParser != "undefined") {   ///Google或者IE11高版本浏览器
		...
		for (i=0;i<_l;i++){
			...
			var trExtType=$("#"+_id,doc).attr("exttype");
			if(typeof(param[_id])=="object"){
				
				if(trExtType=="split"){
					param[_id]=splitMoreTrData(_nh,param[_id])
				}
				var _pItmArr=param[_id];
				_i=_pItmArr.length;
				for(j=1;j<_i;j++){
					$("#"+_id,doc).after(_nh);
				}
				...
				for (ii in _pItmArr){
					for (j in _pItmArr[ii]){
						if($("td[id='"+j+"']",doc)[ii]){
							$("td[id='"+j+"']",doc)[ii].innerHTML=_pItmArr[ii][j];
						}
					}
				}
			}
...
function splitMoreTrData(trHtml,arrayObject){
	if(!$("#calcuWordLimit").length){
		$("body").append("<div style='display:none' id='calcuWordLimit'></div>");
	}
	var doc = $("#calcuWordLimit");
	$("#calcuWordLimit").append(trHtml);
	var limitNumberObj={};	///记录具体多少字换行
	$("td",doc).each(function(){
		if($(this).attr("id")){
			$(this).html("憌");
			var oneRowHeight=$("#calcuWordLimit").height();
			for(var count=1;count<100;count++){
				$(this).html($(this).html()+"憌");
				if($("#calcuWordLimit").height()!=oneRowHeight){
					limitNumberObj[$(this).attr("id")]=$(this).html().length-1;
					$(this).html("");
					break;
				}
			}
	...
	$("#calcuWordLimit").remove();
	
	
	var retArrayObject=[],cycleIndex=0;
	for (var i in arrayObject){
		var oneObject=arrayObject[i];
		while (true){
			var endCycle=true;
		    cycleIndex++;
		    var retOneObject={};
		    for(indexName in limitNumberObj){
				var thisOneRowNumber = limitNumberObj[indexName];
				var thisAllText = oneObject[indexName];
				var stPosition = (cycleIndex-1)*thisOneRowNumber;
				var endPosition = cycleIndex*thisOneRowNumber;
				var thisText = thisAllText.substring(stPosition,endPosition);
				if(thisText!=="") endCycle=false;
				retOneObject[indexName]=thisText;
			}
			
			///循环次数超过999次或者全部分割完毕
			if(endCycle||cycleIndex>999){
		    	break;
			}
			retArrayObject.push(retOneObject);
		}
