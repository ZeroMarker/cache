...
function InitDetList(){
	...
	var columns=[[
		...
		{field:'PatLev',title:'当前级别',width:90,formatter:setCellLevLabel},
		...
		{field:'mainDoc',title:'主管医生',width:120},
...
function setCellLevLabel(value, rowData, rowIndex){
	...
	var fontColor = "";
	if (value == "1级"){ fontColor = "#F16E57";} //原：((value == "1级")||(value == "2级"))
	if (value == "2级"){ fontColor = "orange";}  //hxy 2020-02-20
	if (value == "3级"){ fontColor = "#FFB746";}
	if ((value == "4级")||(value == "5级")){ fontColor = "#2AB66A";} //原：(value == "4级")
	return "<font color='" + fontColor + "'>"+setCellLabel(value)+"</font>"; //hxy 2020-02-20
}
...
function setCellType(value, row, index){
	...
	}else if (value == "橙区"){ //hxy 2020-02-20 st 目测当前未用到
...
function InitShiftsItem(){
	...
	// 获取显示数据
	runClassMethod("web.DHCEMDicItem","JsGetColumns",{"mCode":"BSN", "HospID":session['LOGON.HOSPID']},function(jsonString){
...
function takShift(){
	...
	var mParams = MedGrpID +"^"+ WardID +"^"+ Schedule +"^"+ WrDate +"^"+ EmType +"^"+ LgUserID +"^"+ "BSN" +"^"+ LgHospID;
...
//	if (bsItemID == ""){
//		$.messager.alert("提示:","请先选择需删除病人记录！","warning");
//		return;
//	}
	if (CompFlag == "Y"){
