...
function initpatient(row,cow){
	...
	for (i = 1; i <= row*cow; i++) {
		$("<div class='sickbed' style='visibility:hidden' id='sickbed" +i+"'>" +
			"<div class='sickbedBorder' id='sickbedBorder"+i+"'>"+
				"<div id='seattitle'>"+
			...
			"</div>"+
			"<div class='ArrangeBtn' id='ArrangeBtn"+i+"'></div>"+       //这个参数只为了存储数据
		 "</div>").appendTo($BedDiv); 
	...
	if(seatObj.seatWidth!=""){
		...
		if(cow<6){
			$('.sickbed').css('width',120);
			$('.sickbed').css('height',60);	
		}else{
			$('.sickbed').css('width',$(window).width()/cow);
			$('.sickbed').css('height',60);			
		}
	}
	initMethod();
...
function initPatientText(row,cow,str,noPerple){
	...
	$('#top_btn_no').html('无人<span class="label label-default">'+noPerple+'</span>');
	$('#top_btn_man').html('男<span class="label label-default">'+ManSeatNum+'</span>');
	$('#top_btn_woman').html('女<span class="label label-default">'+WomanSeatNum+'</span>');
	$('#top_btn_unman').html('未知<span class="label label-default">'+UnknownSeatNum+'</span>');
...
function initSeatClick(){
	...
	$('.sickbed').on('click',function(){    //无人的时候点击座位可以安排
	   ...
	   if(hasPat=="YY"){
		...
		setEprMenuForm(curSelSeat.admId,"");
		///根据列表安排  yyt
...
function transfuse(){
	...
	if(curSelSeat.admId===""){
		...
		window.location.href="dhcem.nur.main.hisui.csp?EpisodeID="+EpisodeID;
	}
	return false;
}
...
function NurExec(){
	...
	var LinkUrl = 'dhcem.nur.main.hisui.csp?EpisodeID='+ EpisodeID;
	setEprMenuForm(EpisodeID,"");
...
function checkOrUnCheckSeat($this)
{
	...
	if(selItems.length>0){
		...
		setEprMenuForm(selItems[0].EpisodeID,"");
	}else{
		setCurSelSeat("");
		setEprMenuForm("","");
	}
