...
function initTable(){
	...
	var uniturl = LINK_CSP+"?ClassName=web.DHCEMPat&MethodName=listEmPat&params="+params;
...
function getParams(){
	...
	var admType="";
	$("[name='EM']:checked").each(function(){
		admType+=$(this).attr("attr-type");
	})
	
	
	return StartDate+'^'+EndDate+'^'+CheckLev+'^'+TmpCondition+'^'+CardNo+'^'+AdmLoc+'^'+EmWardID+'^'+admType;
}

/// 查询
function qryEmPatList(){
	$("#patList").datagrid("load",{"params":getParams()}); 
}
