...
function insRow(){
	...
	if (isEditFlag == 1) return;
	var CstType = $HUI.combobox("#CstType").getText(); 	          /// 会诊类型
	if ((CstType.indexOf($g("单科")) != "-1")||(CstType == "")) return;
...
function SureCstNo(){
	...
	runClassMethod("web.DHCEMConsult","SureCstMas",{"CstID": CstID, "ItmID":ItmID, "UserID":session['LOGON.USERID']},function(jsonString){
		...
		if (jsonString.split("^")[0] == -1){
			$.messager.alert("提示:",jsonString.split("^")[1],"warning");
			return;
		}//ed
		if (jsonString < 0){
...
function PreCompCstNo(){
	...
	runClassMethod("web.DHCEMConsult","PreCompCstMas",{"CstID":CstID,"ItmID": CstItmID,"CstOpinion":ConsOpinion,"LgParam":LgParam},function(jsonString){
		...
		if (jsonString == -1){
			$.messager.alert("提示:","申请单当前状态，不允许进行预完成操作！","warning");
			return;
		}
...
function EvaRCstNo(){
	...
	if ((IsUserSure!=-1)&(GetIsOperFlag("60") != "1")){
		$.messager.alert("提示:","申请单当前状态，不允许进行评价操作！","warning");
		return;
	}
	if ((IsUserSure==-1)&(GetIsOperFlag("50") != "1")&(GetIsOperFlag("55") != "1")){
		$.messager.alert("提示:","申请单当前状态，不允许进行评价操作！","warning");
		return;
	}
...
function HideByConsType(){
	var CstType = $HUI.combobox("#CstType").getText(); 	          /// 会诊类型
	if ((CstType.indexOf($g("单科")) != "-1")||(CstType == "")){
...
function HideAndShowButton(BTFlag){
	...
	if (BTFlag == 2){
		...
		PageEditFlag(2);	   /// 页面编辑
		isEditFlag = 1;	       /// 行编辑标志
		if (ModCsFlag == 1) isEditFlag = 0;	 /// 行编辑标志
	}
...
function InsCsLocItem(){
	...
	if (isExistItem() == 1){
		$.messager.alert("提示:","存在相同记录，不能保存！","warning");
		return;		
	}
