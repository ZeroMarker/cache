...
function disHospWin(){
	...
	if (AbnOrdInfo!=""){
		$.messager.alert("提示","有需关注医嘱！"+AbnOrdInfo);
		return;
	}
...
function addSeatMethod(){
	$(".seat").each(function(){
		$(this).on('click',function(){
			...
			if(($(this).attr("data-adm")!=="")&&(SelData.length)){   //选中等候区病人再选中床位上病人
...
function initCombogrid(data){
	...
	///  定义columns
	var columns=[[
		{field:'BedID',title:'床位ID'},
		{field:'BedCode',title:'床号'},
		{field:'WardDesc',title:'病区'},
		{field:'WardRoomDesc',title:'房间名称'},
		{field:'WardRoomType',title:'床位类型'},
...
function addSeat(data){
	...
	var seatTitleHeight=25;						//Title高度
	...
	var seatBodyIconHeight = seatBodyHeight-20; //图标高度
	...
	var drop="";
	var seadDrop="";
	var seatColor =""; 							//界面床的颜色
	var isMakeFlag="";							//包床标志
	var patAdm ="" ; 							//绑定床位Adm
    var seatTitleBgColor="#FBFBFB"              //界面title的背景色 hxy 2018-04-25
	...
	if (PatCareLevel == "二级"){seatTitleBgColor="#7CCD7C";}
	if (PatCareLevel == "三级"){seatTitleBgColor="#5EA5E8";}
	...
	var htmlStr = htmlStr+				"<div class='seatbodyname' style='height:"+seatBodyNameHeight+"'>"+patInfoHtmlText+"</div>"   //姓名部分
