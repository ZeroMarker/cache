...
function save(){
	saveByDataGrid("web.DHCDISType","SaveDisType","#datagrid",function(data){
			if(data==0){
...
function cancel(){
    ...
    if (r){
	    var row =$("#datagrid").datagrid('getSelected');     
		 runClassMethod("web.DHCDISType","RemoveDisType",{'Id':row.ID},function(data){ $('#datagrid').datagrid('load'); })
    }    
}); 
}
