...
function InitPageComponent(){
	...
	var option = {
        ...
        onSelect:function(option){
	        isEditFlag = 1;  /// 行编辑标志
			$("#dgCstDetList").datagrid("load",{"GrpID":option.value});      /// 会诊科室
	    },
		onShowPanel:function(){
...
function InitPageDataGrid(){
	...
	var columns=[[
		...
		{field:'LocDesc',title:'科室',width:300,editor:LocEditor,align:'center'},
...
function HideAndShowButton(BTFlag){
	...
	if (BTFlag == 3){
		...
		if ((CsStatCode == "发送")||(CsStatCode == "审核")||(CsStatCode == "取消接收")){
			$("#bt_ref").show();/// 拒绝接收
			$("#bt_acc").show();/// 接收
		}else{
			$("#bt_ref").hide();/// 拒绝接收
