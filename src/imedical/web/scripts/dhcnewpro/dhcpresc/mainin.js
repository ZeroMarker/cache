...
function initPageDefault(){
	...
	loadOeordList(auditId);			//初始化处方信息
	
	loopPointer = setInterval("reloadAuditList()",5000);		//刷新审核列表列表-记录轮训id
}
...
function initAuditList()
{
	...
	var uniturl = $URL +"?ClassName=web.DHCPRESCAudit&MethodName=QueryAuditList&params="+params+"&type=I";
...
function setCellLabel(value, rowData, rowIndex){
	...
	var htmlstr = '<div class="celllabel">'
	 	htmlstr = htmlstr + '<h3 style="float:left;background-color:transparent;">'+ rowData.patName +patType+'</h3><span style="float:right;background-color:transparent;font-size:5px;">'+ rowData.prescNo +'</span>'
...
function LoadPatInfo(rowData)
{
	...
	$("#bedNo").html(rowData.bedNo);
	$("#allegry").html(rowData.spalleHis);
	...
	BindTips(rowData.diag,rowData.chndiag,rowData.alleHis);
}
...
function loadOeordList(auditId)
{
	/**
	 * 定义columns
	 */
	var columns=[[
		...
		{field:'priority',title:'医嘱类型',width:100,align:'left'},
		{field:'linkSeqNo',title:'关联',width:60,align:'left'},
		{field:'drugDesc',title:'医嘱名称',width:250,align:'left',formatter:linkInstruct},
		{field:'onceDose',title:'单次剂量',width:100,align:'left'},
		{field:'preMet',title:'用法',width:100,align:'left'},
		{field:'freq',title:'频次',width:80,align:'left'},
		{field:'treatment',title:'疗程',width:60,align:'left'},
		{field:'packQty',title:'数量',width:60,align:'left'},
		{field:'ordPrice',title:'单价',width:50,align:'left'},
		{field:'skinTest',title:'皮试',width:50,align:'left',hidden:true},
		{field:'recDeptDesc',title:'接收科室',width:150,align:'left'},
		{field:'ordDate',title:'开医嘱日期',width:150,align:'left'},
		{field:'ordStDate',title:'医嘱开始日期',width:150,align:'left'},
		{field:'ordEndDate',title:'医嘱结束日期',width:150,align:'left'},
		{field:'drugSpeed',title:'滴速',width:150,align:'left'}
	]];
	...
	var params=auditId+"^^^";
	var uniturl = $URL+"?ClassName=web.DHCPRESCAudit&MethodName=GetOrdList&params="+params;
	new ListComponent('ordInfo', columns, uniturl, option).Init();	
}

function searchOeordList()
{
	var stDate=$HUI.datebox("#sel-stDate").getValue();
	var edDate=$HUI.datebox("#sel-edDate").getValue();
	var params=auditStr+"^"+stDate+"^"+edDate;	
	var uniturl = $URL+"?ClassName=web.DHCPRESCAudit&MethodName=GetPrescNoI&params="+params;
	
		$('#ordInfo').datagrid('load',
		{"params":params});
}
///处理处方的html
function AppendPrescHtml(json)
{
	...
	for(var i=0;i<length;i++){
		...
		$cols.append("<input name='OeOrdExe' type='checkbox' value='' style='margin: 10px;'/>");
...
function initButton()
{
	...
	$("#monitoring").bind('click',auditStart);	//开启监测 shy 2022-04-06
...
function OpenEmrO()
{
	...
	var url="dhcem.consultpatemr.csp?&EpisodeID="+admId+"&PatientID="+patientId+"&targetName=Attitude"+"&TextValue="; //+obj.text;
				...
				runClassMethod(
					...
					function(ret){
						if(ret==1){
							...
							initCompent();  //离开后查询当前监测状态更新界面元素 shy 2022-4-6
							return;
						}else{
							$.messager.alert('提示',"处理失败！"+ret,"error");
							return;
		...
		runClassMethod(
			...
			function(ret){
				if(ret==1){
					...
					initCompent();  //开启检测后查询当前监测状态更新界面元素 shy 2022-4-6
...
function initLisDatagrid()
{
	...
	var columns=[[
          ...
          { field: 'ResultStatus', title: '结果状态', width:100, sortable: false, align: 'left'},
          ...
          { field: 'ReadFlag', title: '阅读', width: 40, sortable: false, hidden:true,align: 'center',
          	formatter: function(value, rowData, rowIndex){
	          	if (rowData.ResultStatus != "3") return "";
	        	if (value == "1") {
	...
	$HUI.datagrid('#lisOrdTable',{
		url:$URL +"?ClassName=web.DHCPRESCQuery&MethodName=GetLisHisList&params="+params,
		fit:true,
		rownumbers:true,
		columns:columns,
		fitColumns:false,
	...
	$HUI.datagrid('#lisOrdDetailTable',{
		url:$URL +"?ClassName=web.DHCPRESCQuery&MethodName=GetLisDetList&reportId=",
		fit:true,
		rownumbers:true,
		columns:columns,
		fitColumns:false,
...
function setReadFlag(auditId)
{
	runClassMethod("web.DHCPRESCAudit","SetReadFlag",{"AuditId":auditId},
		...
		//重新计时 超时自动通过
		//
		("ReadOverTime()", OverTime);   //18000需要改为配置     阅读18s超时自动通过
	},'text',true);	
}
//shy 阅读超时自动通过
...
function panoview()
{
	...
	//var url = 'websys.chartbook.hisui.csp?ChartBookName='+"DHC.Doctor.DHCEMRbrowse"+"&PatientListPage="+ "emr.browse.patientlist.csp"+"&SwitchSysPat=N"+"&EpisodeID="+admId+'';
...
//这里面写在关闭或刷新页面时，要调用的事件 shy 2022-04-07
$(window).bind('beforeunload', function () {
//return '该操作会导致药师离开不再监测，确定离开此页面吗？';
runClassMethod(
				"web.DHCPRESCAudit",
				"updScheStateNew",
				{
				...
				function(ret){
					if(ret==1){
						$.messager.alert('提示',"您未选择审核科室，请先在审核资源选择页面维护！","warning");
						return;
					}else if(ret==0){
						//$.messager.alert('提示',"操作成功！");  
						initCompent();  //离开后查询当前监测状态更新界面元素 shy 2022-4-6
...
function linkInstruct(value, rowData, rowIndex)
{
	var html = "<a href='#' onclick=\"OpenEditWin('"+rowData.drugCode+"','"+rowData.drugDesc+"')\">"+rowData.drugDesc+"</a>";

 	return html; 

}

function OpenEditWin(drugCode,drugDesc)
{  

	var width = (document.documentElement.clientWidth-100);
	var height = (document.documentElement.clientHeight-100);
	var url="dhcckb.pdss.instruction.csp?IncId=&IncCode="+drugCode+"&IncDesc="+encodeURI(drugDesc);
	websys_showModal({
		url: url,
		width:width,
		height:height,
...
function BindTips(diag,chndiag,alleHis)
{
