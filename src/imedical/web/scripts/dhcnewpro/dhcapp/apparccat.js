...
function initItmlist(){
	...
	var uniturl = LINK_CSP+"?ClassName=web.DHCAPPArcCat&MethodName=QueryArcCat";
...
function saveItemRow(){
	...
	for(var i=0;i<rowsData.length;i++){
		...
		if(rowsData[i].hospdesc==""){
			$.messager.alert("提示","第"+(rowsData.length-i)+"行医院为空！"); 
			return false;
		}
		var tmp=rowsData[i].acrowid +"^"+ rowsData[i].catcode +"^"+ rowsData[i].catdesc +"^"+ rowsData[i].hospdr;
		dataList.push(tmp);
	} 
