...
function sendExaReqDetail(){
	...
	$('input[name="Diags"]:checked').each(function(){
		PatDiagsArr.push(this.value +mDel+ $(this).parent().next().text());
	})
	var PatDiags = PatDiagsArr.join("@");
	
	/// 病人生命体征信息
	var mPatSymData = EpisodeID + "^" + itemExaDisHis + "^" + itemExaPhySig + "^" + itemExaSym;
	...
	$.each(selItems, function(index, selItem){
		...
		var ListData = ItemID +"#"+ EpisodeID +"^"+ itemLocID +"^"+ arEmgFlag +"^"+ itemExaPurp +"^"+ LgUserID +"^^^"+ LgCtLocID +"^^"+ "Y" +"^"+ PatDiags;
			ListData = ListData +"#"+ itmmmastid +"^"+ itemExaPosiID +"^"+ itemExaPartID +"^"+ itemExaDispID +"^"+ itemRemark +"^^^"+ itemBillTypeID;
...
function InsMarIndTable(itmArr){
	...
	for (var j=1; j<=itmArr.length; j++){
		if($('input[name="Diags"][value="'+ itmArr[j-1].value +'"]').length == 0){
			itemhtmlArr.push('<tr><td style="width:20px;"><input value="'+ itmArr[j-1].value +'" name="Diags" type="checkbox"></input></td><td>'+ itmArr[j-1].text +'</td></tr>');
		}
	}
			...
			if (isLinkPart != ""){
				if (isLinkPart == 1){
					...
					if (IsRepeatOneday(arExaItmID,"") == 1){
						$.messager.confirm('确认对话框','当天已经有相同医嘱是否继续添加？', function(r){
							if (r){
...
//	}
	...
	ItemEmgFlag:arEmgFlag, ItemRemark:'', ItemBillTypeID:BillTypeID, ItemBillType:BillType}
	$("#ItemSelList").datagrid('appendRow',rowobj);
