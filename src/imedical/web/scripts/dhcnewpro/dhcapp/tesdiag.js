...
function insertMetRow(){
	...
	if(editRow>="0"){
		$("#tesdiaglist").datagrid('endEdit', editRow);//结束编辑，传入之前编辑的行
	}
	 
	$("#tesdiaglist").datagrid('insertRow', {//在指定行添加数据，appendRow是在最后一行添加数据
		index: 0, // 行数从0开始计算
		row: {ATRowID:'',ATCode:'',ATDesc:'',FlagCode:'Y',ATFlag:'Y',HospDr:LgHospID,HospDesc:LgHospID}
	});
    
	$("#tesdiaglist").datagrid('beginEdit', 0);//开启编辑并传入要编辑的行
	editRow=0;
}
///保存申请状态
function saveMetRow(){
	...
	runClassMethod("web.DHCAPPTesDiag","SaveTesDiag",{"params":params},function(jsonString){
		...
		$('#tesdiaglist').datagrid('reload'); //重新加载
	});
}
...
function deleteMetRow(){
	...
	var rowsData = $("#tesdiaglist").datagrid('getSelected'); //选中要删除的行
	if (rowsData != null) {
