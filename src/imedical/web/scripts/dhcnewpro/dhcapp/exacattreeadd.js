...
function initDataGrid(){
	...
	var boxArcEditor = {
		...
		options:{
			...
			textField:'itmDesc',
			mode:'remote',
			url:LINK_CSP+'?ClassName=web.DHCAPPTreeAdd&MethodName=QueryArcItmDetail',
			columns:arcColumns,
...
function initPartDataGrid(){
	...
	var boxPartEditor = {
		...
		options:{
			...
			textField:'PartDesc',
			mode:'remote',
			url:LINK_CSP+'?ClassName=web.DHCAPPExaReportQuery&MethodName=jsonPart',
			columns:PartColumns,
				onSelect:function(rowIndex, rowData) {
...
function initBlButton(){
	...
	$('#PartDesc').bind('keypress',function(event){
		if(event.keyCode == "13"){
			...
			var unitUrl = LINK_CSP+'?ClassName=web.DHCAPPExaReportQuery&MethodName=jsonPart&PartName='+$('#PartDesc').val();
			/// 调用部位列表窗口
	...
	$('#ItmmastDesc').bind('keypress',function(event){
		if(event.keyCode == "13"){
			...
			var unitUrl = LINK_CSP+'?ClassName=web.DHCAPPTreeAdd&MethodName=QueryArcItmDetail&Input='+$('#ItmmastDesc').val();
			/// 调用部位列表窗口
...
function dataArcGridBindEnterEvent(index){
	...
	workRateEditor.target.bind('keydown', function(e){
		if (e.keyCode == 13) {
			...
			if (input == ""){return;}
			var unitUrl = LINK_CSP+'?ClassName=web.DHCAPPTreeAdd&MethodName=QueryArcItmDetail&Input='+$(ed.target).val();
			/// 调用医嘱项列表窗口
...
function saveCat(){
	...
	var mListData =ItemDesc +"^"+ ItemDesc +"^"+ LastItmID +"^"+ ItemAlias +"^"+ LgHospID +"^"+ ItemType;
...
function moveTree(mListData){
	...
	var InsFlag = false;
	runClassMethod("web.DHCAPPTreeAdd","moveTree",{'mListData':mListData},function(jsonString){
		if (jsonString != 0){
