...
function initArcItemList(){
	...
	var columns=[[
		...
		{field:'TarFlag',title:'按部位累加收费',width:180,align:'center',editor:TarFlageditor},
		{field:'ReqCode',title:'ReqCode',width:100,align:'center',hidden:true,editor:textEditor},
		{field:'ReqFlag',title:'自动生成申请',width:180,align:'center',editor:ReqFlageditor},
	...
	var uniturl = LINK_CSP+"?ClassName=web.DHCAPPArcAdd&MethodName=QueryExaArc";
...
function saveExecArc(){
	...
	for(var i=0;i<rowsData.length;i++){
		...
		var tmp=rowsData[i].AraRowId+"^"+rowsData[i].ArcDr+"^"+rowsData[i].AraArcDesc+"^"+rowsData[i].ExecCode+"^"+ rowsData[i].TarCode +"^"+ rowsData[i].ReqCode;
		dataList.push(tmp);
	} 
