...
function CallListUnocc(WardID) {
	...
	window.open(lnk, "ListUnocc", "HEIGHT=300,WIDTH=700,LEFT=50,TOP=50,scrollbars=yes,resizable=yes,status=yes");
}

function TrakIt_onmousedown(evt) {
	if ((displayOnly && displayOnly.value == "Y") && (isEmLoc && isEmLoc.value == "1")) return; //Log 63302
	websys_cancel();
	...
	try {
		...
		winP.SetEpisodeDetails(arrIDs[1], arrIDs[0], arrIDs[2], arrIDs[3], "", "", "", "", currentWardID, "", "", "", "", "", "", "", "", "", "", "", "", arrIDs[21]);
	...
	if ((p.className == "bedNorm") || ((p.className == "bedNorm clsRowTempLoc"))) {
		if (p.className == "bedNorm clsRowTempLoc") isTempLoc = p.id
		else isTempLoc = ""
...
function TrakIt_onmousemove(evt) {
	...
	if ((displayOnly && displayOnly.value == "Y") && (isEmLoc && isEmLoc.value == "1")) return false; //Log 63302
	...
	if (selectedcontainer.getAttribute("moveEnabled") == 1 && dragEnabled > 0) {
		...
		if (websys_getButton(evt) != "L") {
			SnapBack(selectedcontainer, evt);
			return false;
		}
...
function TrakIt_onmouseup(evt) {
	...
	var eSrc = getSelectedRoom(websys_getSrcElement(evt))
		//wanghc 2014-12-16 选中病人后再点右下角消息输入框会提示
	if ((eSrc.className.split(" ")[0] == "bedNorm") && eSrc.id.substring(0, 1) == "x") {
		alert(t['BedInactive']);
		return;
	}
	if (selectedcontainer.getAttribute("moveEnabled")) {
		SnapBack(selectedcontainer, evt);
		return;
	}
...
function OpenPACBedAdmWindow(eSrc) {
	...
	if (selectedcontainer.getAttribute("moveEnabled") == 0) {
		if (selectedcontainer.getAttribute("showemeralert") == 1) {
			alert(t['SelectEmergency']);
			return;
		}
		alert(t['MoveDisabled']);
		return;
	}
	...
	if ((eSrc.className == "room") || (eSrc.className == "clsRoomLink")) {
		...
		myShowModal(lnk, "BEDADM", "width=500,height=650,left=50,top=50,scrollbars=yes,resizable=yes,status=yes");
	}
	if (eSrc.className.split(" ")[0] == "bedNorm") {
		//转床发送转床申请
		var Hosid = session['LOGON.HOSPID'];
		var Ctloc = session['LOGON.CTLOCID'];
		var EpisodeID = arrIDs[0];
		var ApplyBedId = eSrc.id;
		var ret = tkMakeServerCall("Nur.DHCBedManager", "IfCanTransBed", Hosid, Ctloc, ApplyBedId, EpisodeID);
		if (ret == "1") {
			alert("床位被预约,不能分配")
			return;
		}
		if (ret != "0") {
			//先屏蔽
			var lnk = "DHCNurBedApply.csp?&EpisodeID=" + arrIDs[0] + "&ApplyBedId=" + eSrc.id;
		...
		myShowModal(lnk, "BEDADM", "width=470,height=470,left=50,top=50,scrollbars=yes,resizable=yes,status=yes");
	}
}

//分配床位到指定床位
function SetPatinbed(adm, ward, bedsub) {

	var patientId=tkMakeServerCall("patientId","GetPatientIdByAdm",adm)
	var lnk = "websys.default.csp?WEBSYS.TCOMPONENT=PAAdmTransaction.Edit&PatientBanner=1&ID=" + adm + "&EpisodeID=" + adm + "&PatientID=" + patientId+ "&roomQuery=&PAADMType=I" + "&WardID=" + ward+ "&BedID=" + ward+"||"+bedsub;
		//var lnk = "websys.default.csp?WEBSYS.TCOMPONENT=PACBedAdm.Edit&EpisodeID="+arrIDs[0]+"&PARREF="+eSrc.id+"&PatientID="+arrIDs[1] + "&roomQuery=&PAADMType="+admtype;
	myShowModal(lnk, "BEDADM", "width=400,height=650,left=50,top=50,scrollbars=yes,resizable=yes,status=yes");
	
	// var bedid=ward+"||"+bedsub
	// //alert(bedid)
	// var userid=session['LOGON.USERID']
	// var ret=tkMakeServerCall("Nur.DHCBedManager","UpdateBed",adm,bedid,userid);
	// if(ret=="0")
	// {
	// 	alert("分配成功")
...
function BodyOnloadHandler() {
	...
	//DHC WKZ
	var tableObj = document.getElementById("header");
	var str = "";
	if (tableObj) {
		for (var i = 0; i < tableObj.rows.length; i++) {
			for (var j = 0; j < tableObj.rows[i].cells.length; j++) {
				var innerHTMLStr = tableObj.rows[i].cells[j].innerHTML
				var regExpStr = new RegExp(">(\\w+)\-", "g")
				innerHTMLStr = innerHTMLStr.replace(regExpStr, ">")
				tableObj.rows[i].cells[j].innerHTML = innerHTMLStr
			}
		}
	} //by wuqk 2011-07-09
	...
	if (window.parent != window.self) winf = window.parent;
	if ((winf) && (winf.frames['eprmenu'])) {
		//winf.frames['eprmenu'].currentWardId=WardID;
	}
}
