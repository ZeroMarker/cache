...
function InitSMSWinEvent(obj)
{
   ...
   //---------------------------------------
   obj.SMSWin_activate = function(win)
	{//alert('a');
		//BodyLoadHandler();
	}
	obj.SMSWin_hide= function(win)
	{
		//BodyUnLoadHandler();
		//alert('h');
	}
...
function SendMessage_click(){
	...
	var TheOBJ = ExtTool.StaticServerObject("web.DHCHM.MessageSend");
     ...
     // User=1;
      ...
      var MessageID=TheMainobj.MSMMessageDetail.getValue();
      if (MessageID=="") 
          { ExtTool.alert("请填写短信内容!");return;}
      var SendList="";
      var Valuestr=MessageID+"^"+TheMainobj.MSMRemark.getValue()+"^"+User;
   
	//以下为消息发送
	 var Mobile="",iMessage="",flag="";
	 var row;
	 iMessage=TheMainobj.MSMMessageDetail.getValue();
	 if (iMessage=="") {ExtTool.alert("发送失败,短信内容为空!");return;};
