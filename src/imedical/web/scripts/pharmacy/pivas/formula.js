...
function MainTainItm(type) {
    ...
    $.data($('#gridIngIndItmWin')[0], 'operateType', type == 'A' ? 'add' : 'edit');
...
function DeleteIngIndex() {
    ...
    $.messager.confirm('删除提醒', '删除后明细公式将全部消失, 您确定删除吗?', function (r) {
        if (r) {
            if (ingIndexId == '') {
...
function SaveIngIndItm() {
    ...
    var ingIndItmId = '';
    if ($.data($('#gridIngIndItmWin')[0], 'operateType') === 'add') {
    } else {
    ...
    var params =
        ingInexId + '^' + ingIndItmId + '^' + ingIndItmDesc + '^' + ingIndItmMax + '^' + ingIndItmMin + '^' + ingIndItmUom + '^' + ingIndItmFormula + '^' + ingIndItmCode + '^' + ingIndItmRemark;
