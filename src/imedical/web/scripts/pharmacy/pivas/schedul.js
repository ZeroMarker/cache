...
function InitDict() {
    ...
    PIVAS.ComboBox.Init({
	 ...
	 PIVAS.ComboBox.Init({ Id: "cmbLinkSche", Type: "PIVASchedul" }, {
        editable: false,
        placeholder: "班次...",
        width: 200,
        multiple: true,
...
function Save() {
    ...
    for (var i = 0; i < gridChangeLen; i++) {
        ...
        var params = (iData.psId || "") + "^" + pivaLocId + "^" + (iData.psCode || "") + "^" + (iData.psDesc || "") + "^" + (iData.psShortDesc || "") + "^" +
            (iData.psStartTime || "") + "^" + (iData.psEndTime || "") + "^" + (iData.psScheTypeId || "") + "^" + (iData.psCustFlag || "")+"^"+
            (iData.psDuration || "") + "^" + (iData.psInterval|| "") 
