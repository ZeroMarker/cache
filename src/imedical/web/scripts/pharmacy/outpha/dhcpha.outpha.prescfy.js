...
function ExecuteAllFY() {
	...
	if ((IsOnlyOnePatno() == false)){
		...
		var warntitle = "存在多个患者的处方确认全发吗?系统将全部发放查询出的所有处方!"
	}else {
		var warntitle = "确认全发吗?系统将全部发放查询出的所有处方!"
	}
...
function DispensingMonitor(rowid) {
	...
	if (checkprescref == "S") {
		dhcphaMsgBox.confirm($g("该处方医生已提交申诉")+'<br/>'+$g("点击[确认]将同意申诉继续发药，点击[取消]将放弃发药操作。"), function (r) {
			if (r == true) {
...
function ClearConditions() {
	...
	if (NoJudgWinFlag=="Y") $('#noJudgWin').iCheck('uncheck');
...
function InitGirdCheck(){
	var columns=[
		...
		{	name:'TSelect',index:'TSelect',header:'全选',
			width:35,
			align: 'center',
			editable: false,
			edittype: 'checkbox',
		...
		{header:'TPrescNo',index:'TPrescNo',name:'TPrescNo',width:20, hidden:true},
		{header:'TOrditm',index:'TOrditm',name:'TOrditm',width:20, hidden:true},
		{header:'TPrescStat',index:'TPrescStat',name:'TPrescStat',width:20, hidden:true},
	    {header:$g("名称"),index:'TPhDesc',name:'TPhDesc',width:380, align:'left',cellattr:addPrescColor},
		{header:$g("数量"),index:'TPhQty',name:'TPhQty',width:60},
		{header:$g("单位"),index:'TPhUom',name:'TPhUom',width:80},
		{header:$g("频次"),index:'TPC',name:'TPC',width:80},
		{header:$g("剂量"),index:'TJL',name:'TJL',width:100},
		{header:$g("用法"),index:'TYF',name:'TYF'},
		{header:$g("条码"),index:'TBarCode',name:'TBarCode',width:120, hidden:true},
		{header:$g("易混标识"),index:'IecTypeStr',name:'IecTypeStr',width:120,formatter:formatterIecTypeIcon},
		{header:'TInci',index:'TInci',name:'TInci',width:120, hidden:true},
	];
	var jqOptions={
	    colModel: columns,
	    multiselect: false,
...
function SelectAll(){
	...
	for(var i=0; i<len; i++){
		...
		if(TPrescStat=="Yes"){continue;} //整张处方已完成核对不允许修改
		//更新界面勾选状态
		var newdata={
...
function CheckAction(id){
	...
	var TOrditm = selrowdata.TOrditm;
	var TPrescStat = selrowdata.TPrescStat;
	if(TPrescStat=="Yes"){return;} //整张处方已完成核对不允许修改
...
function ShowIncEasyCon(event,Inci,Type){
	...
	var tipwidth=600;
	var tipheight=330;
	var left =200;  // window.event.clientX-(document.body.clientWidth-modalwidth)/2 -tipwidth -30; 
	var top =15;  //window.event.clientY-(document.body.clientHeight-modalhight)/2 -tipheight-110; ;
	if(top<-15){
	...
	var htmlStr=""
	htmlStr += '	<div class=" modal-content" style="height:'+tipheight+'px;width:'+tipwidth+'px;z-index:999;position:fixed;left:0;top:0" align="left" >';  //;overflow-y:auto
	...
	htmlStr += '				</div>';
	htmlStr += '				<div >';
	htmlStr += '					<a style="color:black">'+$g("厂家")+':</a>';
