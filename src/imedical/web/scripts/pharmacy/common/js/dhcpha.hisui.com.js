        ...
        Init: function (_cOptions, _options, _app) {
            ...
            if (_cOptions.data != undefined) {
                ...
                if (_app == 'PIVAS') {
                    ...
                    _queryOpts = HERB.ComboBox[_cOptions.Type];
                } else {
            ...
            if (_options.placeholder != undefined) {
                $('#' + _id)
                    .next()
                    .find('.combo-text')
                    .attr('placeholder', $g(_options.placeholder));
            }
        },
        Default: {
            valueField: 'RowId',
        ...
        Init: function (_cOptions, _options, _app) {
            ...
            if (_options.placeholder != undefined) {
                $('#' + _id)
                    .next()
                    .find('.combo-text')
                    .attr('placeholder', _options.placeholder);
            }
        },
        Default: {
            panelWidth: 500,
    ...
    Grid: {
        ...
        Init: function (_gridId, _options) {
            ...
            if (typeof PHA_SYS_SET == 'object') {
                PHA_SYS_SET.Grid.Init(_gridId, newOptions, function (gridOpts) {
                    $HUI.datagrid('#' + _gridId, gridOpts);
                });
            }
        },
        Default: {
            ...
            onRowContextMenu: function (e, rowIndex, rowData) {
                if ($(this).datagrid('options').exportXls == true) {
                    ...
                    var menuHtml =
                        '<div id="menuCommon" class="hisui-menu" style="display: none;">' + '<div id="menuCommonExport" data-options="' + "iconCls:'icon-export'" + '">导出</div>' + '</div>';
                ...
                return function (value, row, index) {
                    ...
                    try {
                        ...
                        if (value == row[valueField]) {
                            ...
                            if (textData != '') return row[textField];
                        }
                        // 在这循环格式化不好
