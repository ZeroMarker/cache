...
$(function(){
	...
	$.fn.dhcphaDateRange=function(_options){
		...
		$(this).next().children("i").click(function() {
			$(thiscalendar).click();
    	});
	}
	/* dateReange end*/
	...
	$.fn.dhcphaJqGrid=function(_options){
		options={
	        ...
	        pagerpos: 'left',
        	recordpos: 'right',
        	recordtext: '<div style="color:#666;">当前记录{0}--{1}条　共{2}条记录</div>', 
        	pgtext : '<div style="color:#666;">当前页{0} 共{1}页</div>' ,
        	autoScroll: true,
        	shrinkToFit:true,
        	refresh:true,
        	loadtext:'数据加载中...',
        	emptyrecords:'<div style="color:#666">没有数据</div>',
        	sortable:false,
		...
		for(var i=0; i<colModelLen; i++){
			...
			if ((optionsModal[i].header.indexOf("金额")>=0)||((optionsModal[i].header.indexOf("差额")>=0))){
				if (optionsModal[i].formatter==undefined){
					...
					optionsModal[i].formatoptions={decimalPlaces: parseInt(DHCPHA_CONSTANT.PLUGINS.SAFMT)};
				}
			}
		...
		if (parseInt(jqGridWidth)>=parseInt(tblPanelWidth)){
			_options.shrinkToFit=false;
		}else{
			_options.shrinkToFit=true;	
		}
		var opts = $.extend({},options, _options);
	    $(this).jqGrid(opts); //jqgrid初始化
	    if (opts.pager!=undefined){
			...
			$("#refresh_"+this.attr("id")).css({'padding':'0px 0px 0px 5px'})
			$(".ui-icon-seek-first").css({'background':'url(/dthealth/web/scripts/pharmacy/images/pagination_icons.png) 0px center no-repeat'});
			$(".ui-icon-seek-prev").css({'background':'url(/dthealth/web/scripts/pharmacy/images/pagination_icons.png) -16px center no-repeat'});
			$(".ui-icon-seek-next").css({'background':'url(/dthealth/web/scripts/pharmacy/images/pagination_icons.png) -32px center no-repeat'});
			$(".ui-icon-seek-end").css({'background':'url(/dthealth/web/scripts/pharmacy/images/pagination_icons.png) -48px center no-repeat'});
			$(".ui-icon-refresh").css({'background':'url(/dthealth/web/scripts/pharmacy/images/pagination_icons.png) -64px center no-repeat'});
	    }
	    // 覆盖样式
	...
	$.fn.HideJqGridScroll=function(_options){
		...
		if (_options.hideType=="X"){
			$(_thisid).closest(".ui-jqgrid-bdiv").css({ "overflow-x" : "hidden" });
		}else if (_options.hideType=="Y"){
			$(_thisid).closest(".ui-jqgrid-bdiv").css({ "overflow-y" : "hidden" });
		}
	}
	/* jqGrid end*/
	...
	$.fn.dhcphaImageBtn=function(_options){
		...
		$(this).mouseover(function(){
	       $(this).children().attr("src",_btnimagesrc+"-hover.png")
	       $(this).addClass("dhcpha-btn-img-color")
	    }) ;
	    $(this).mouseout(function(){
	       $(this).children().attr("src",_btnimagesrc+"-default.png")
	    }) ;		
	}
...
var dhcphaMsgBox=({
	alert:function(_message,_msgtype,_callback){
		var msgtitle='<i class="fa fa-info-circle"></i> 温馨提示';
		if (_msgtype=="warn"){
			msgtitle='<i class="fa fa-exclamation-triangle" aria-hidden="true"></i> 警告';
		}else if (_msgtype=="error"){
			msgtitle='<i class="fa fa-times-circle" aria-hidden="true"></i>　错误提示';
		}else if (_msgtype=="success"){
			msgtitle='<i class="fa fa-check-circle-o" aria-hidden="true"></i>　提示';
		}
		var options={
			title:msgtitle,
			size: 'small',
		...
		window.setTimeout(function(){btnoktarget.focus(); },500);
	},
	confirm:function(_message,_callback){
		var msgtitle='<i class="fa fa-info-circle"></i> 操作提示';
		var options={
			title:msgtitle,
			size: 'small',
			closeButton: false,
...
var dhcphaReadCard=function(CardCallBack){
	...
	$.ajax({
		...
		'<div class="modal fade" id="modal-readcard" tabindex="-1" role="dialog" aria-hidden="false">'+
			...
			var cardTypeI=0;
			var cardTypeRowId="",cardTypeInfo="",cardTypeObj="";
			for (cardTypeI=0;cardTypeI<cardTypeLen;cardTypeI++){
				...
				var cardTypeBtnCls="";"btn btn-primary  dhcpha-cardtype-icon-normal";
				var cardTypeIconCls="";"fa fa-credit-card dhcpha-cardtype-icon-common dhcpha-cardtype-icon-normal";
				...
				dhcphaReadCardHtml+='<div class="btn-group ">'+  
                   ' <span class="'+cardTypeIconCls+'"></span>'+
		          '  <button class="'+cardTypeBtnCls+'" type="button" value='+cardTypeRowId+'>'+cardTypeInfo+'</button>'+
                '</div></br></br>'
				//拼串组合modal
			}
			dhcphaReadCardHtml+='</div>'+
								'</div>'+
							'</div>'+
						'</div>'+
					'</div>'+
...
$.fn.dhcphaEasyUIGrid=function(_options){
	...
	var options={
	  ...
	  onLoadError: function (XMLHttpRequest) {
	        ...
	        if (retJson!=""){  
	        	dhcphaMsgBox.alert("错误信息</br>"+retJson.ERROR,"error"); 
	        }else{
		       dhcphaMsgBox.alert("数据格式可能存在问题,请联系工程师!","error"); 
		    }
	  },
...
$.fn.clearEasyUIGrid=function(_options){
	...
	$(_thisid).datagrid('loadData', { total: 0, rows: [] ,footer:[]}); 
...
DHCSTEASYUI.GenHospComp=function(_options,sessionStr){
	...
	if (hospAutFlag === 'Y') {
		...
		if ($("#_HospListLabel").length==0){
			$("<label style='color:red;margin:0 10px 0 10px' class='r-label'>医院</label>").prependTo("body");
		}
		// ##class(web.DHCBL.BDP.BDPMappingHOSP).GetDefHospIdByTableName(tableName,hospid,"");
		var defHospId = $cm({dataType:'text',ClassName:"web.DHCBL.BDP.BDPMappingHOSP",MethodName:"GetDefHospIdByTableName",tableName:tableName,HospID:hospid},false);
		var hospObj = $('#_HospList').combogrid({
			...
			queryParams:{ClassName:'web.DHCBL.BDP.BDPMappingHOSP',QueryName:'GetHospDataForCombo',tablename:tableName,SessionStr:sessionStr},
