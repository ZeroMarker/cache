...
function InitDict() {
    ...
    DHCPHA.ComboBox.Init(
        { Id: 'cmbCTLoc', Type: 'DHCPhaLoc' },
        {
    ...
    DHCPHA.ComboBox.Init({ Id: 'cmbPresChkSel', Type: 'PresChkSel' }, { editable: true, width: 150 });
...
function SaveConfig() {
    ...
    if (presChkFlag == 'Y' && presChkSel == '') {
        $.messager.alert('提示', '请选择审方节点!', 'warning');
        return;
    }
    if (presChkFlag !== 'Y' && presChkSel !== '') {
        $.messager.alert('提示', '已选择审方节点，请确认是否需要审方并勾选!', 'warning');
        return;
    }
