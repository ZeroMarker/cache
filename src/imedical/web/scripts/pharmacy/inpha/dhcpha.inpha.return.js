...
$(function () {
    ...
    $("#btn-redir-returnquery").on("click", function () {
        var lnk = ChangeCspPathToAll("dhcpha/dhcpha.inpha.phareturnquery.csp");
        window.open(lnk, "_target", "width=" + (document.body.clientWidth - 6) + ",height=" + (document.body.clientHeight - 30) + ",menubar=no,status=yes,toolbar=no,resizable=yes,top=90,left=3");
    })
    /* 绑定按钮事件 end*/
    ;
...
function ExecuteReturn(returnreason) {
    ...
    var execret = tkMakeServerCall("web.DHCSTPHARETURN", "ExecReturn", reclocdr, userid, "RT", returnstr);
    ...
    if (retstatus == "success") {
        ...
        if (retvalue == -3) {
            dhcphaMsgBox.alert("存在药品退药数量 >  (发药数量 - 已退药数量),请刷新后核实!");
        } else if (retvalue == -12) {
            dhcphaMsgBox.alert("存在执行记录状态不是停止执行或撤销执行的药品，不允许退药!");
        } else if (retvalue == -9) {
            dhcphaMsgBox.alert("存在未退申请单，不允许再使用直接退药!");
        } else if (retvalue == -4) {
            dhcphaMsgBox.alert("该患者已做完最终结算，不允许退药!");
        } else if (retvalue == -11) {
            dhcphaMsgBox.alert("不允许退药:已经中途结算,不能退药!");
        } else if (retvalue == -1) {
            dhcphaMsgBox.alert("执行退药失败,请检查退药数据是否无误，或者退药数量是否为0");
        } else {
            dhcphaMsgBox.alert("执行退药失败,错误代码:" + retvalue);
        }
        return;
    }
}
