...
function GetIPMonitorParams() {
    ...
    var params = daterange + "^" + phaloc + "^" + phaward + "^" + chkout + "^" + chkaudit + "^" + phalocgrp;
    return params;
}
//查询
...
function RefuseIPOrder() {
    ...
    for (var i = 0; i < idslen; i++) {
        ...
        if (tmpmoeori != mainoeori) {
            dhcphaMsgBox.alert("每次只能选择一条记录拒绝!");
            return;
        }
    ...
    if (canrefuse == 2) {
        dhcphaMsgBox.alert("您选择的记录已经拒绝(接受)!");
        return;
    }
    if (canrefuse == 3) {
        dhcphaMsgBox.alert("您选择的记录已经护士拒绝!");
        return;
    }
    if (canrefuse == 4) {
        dhcphaMsgBox.alert("您选择的记录已处理,不能直接操作,请先取消!");
        return;
    }
    var dispedRet = tkMakeServerCall("web.DHCSTCNTSIPMONITOR", "IfOrderDisped", mainoeori);
    if (dispedRet == "Y") {
        dhcphaMsgBox.alert("您选择的记录已经发药!");
        return;
    }
...
function DrugTips() {
    var passType = DHCPHA_CONSTANT.DEFAULT.PASS;
    if (passType == "") {
        dhcphaMsgBox.alert("未设置药典接口，请在参数设置-住院药房-合理用药厂商中添加相应厂商");
        return;
    }
