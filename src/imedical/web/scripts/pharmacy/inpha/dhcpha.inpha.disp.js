 ...
 * 编写日期:    2016-09-02
 * 编写人:     yunhaibao
 * 重大更新:    病区表格的TWardRowID, 已存储为ward#floor, floor指楼层, 20201221 yunhaibao
 */
 ...
 $(function () {
     ...
     $('#btn-disp').on('click', function(){
         CACert('PHAIPDisp', ConfirmDisp);
     });
     ...
     $('#btn-redir-dispquery').on('click', function () {
         var lnk = 'pha.ip.v4.query.disp.csp';
 ...
 function InitGridDispWard() {
     ...
     var jqOptions = {
         ...
         loadComplete: function () {
             ...
             $("#grid-wardlist").setGridWidth($('#gbox_grid-wardlist').width());
 ...
 function getPhaLocDispType(phaLocId) {
     ...
     for (dispcatsi = 0; dispcatsi < dispcatslength; dispcatsi++) {
         ...
         var newdispcatdesc = dispcatsdesc.split('').join('\t');
 ...
 function InitGridOrdTotal() {
     var columns = [
         ...
         { name: 'TCollStat', index: 'TCollStat', header: '状态', width: 100, cellattr: addCollStatCellAttr },
         ...
         { name: 'TManufacture', name: 'TManufacture', header: '生产企业', width: 150, align: 'left' },
         { name: 'TDrugForm', name: 'TDrugForm', header: '剂型', width: 100, align: 'left' },
         { name: 'TIncstk', name: 'TIncstk', header: '货位', width: 100, align: 'left' },
         { name: 'TInsuCode', name: 'TInsuCode', header: '国家医保编码', align: 'left' , width:100},
         { name: 'TInsuDesc', name: 'TInsuDesc', header: '国家医保名称', align: 'left' , width:100}
 ...
 function InitGridOrdDetail() {
     var columns = [
         ...
         { name: 'TCollStat', index: 'TCollStat', header: '状态', width: 100, cellattr: addCollStatCellAttr, align: 'left' },
         ...
         { name: 'TPaName', index: 'TPaName', header: '姓名', width: 100, align: 'left' },
         {
             name: 'TRegNo',
             index: 'TRegNo',
             header: '登记号',
             ...
             formatter: function (cellvalue, options, rowObject) {
                 return '<a onclick="ViewSkinTestInfo()" style=\'cursor:pointer;\' class="pha-grid-link">' + cellvalue + '</a>';
             }
         },
         ...
         { name: 'TDoseQty', index: 'TDoseQty', header: '剂量', width: 100, align: 'left' },
         { name: 'TFreq', index: 'TFreq', header: '频率', width: 100, align: 'left' },
         { name: 'TInstruction', index: 'TInstruction', header: '用法', width: 100, align: 'left' },
         { name: 'TTimeAdd', index: 'TTimeAdd', header: '用药日期', width: 100, align: 'center' },
         { name: 'TTimeDosing', index: 'TTimeDosing', header: '分发时间', width: 125, align: 'left' },
         { name: 'TNurseAuditDate', index: 'TNurseAuditDate', header: '领药审核时间', width: 155, align: 'left' },
         ...
         { name: 'TAge', index: 'TAge', header: '年龄', width: 100, align: 'left' },
         ...
         { name: 'TPrescNo', index: 'TPrescNo', header: '处方号', width: 120, align: 'left' },
         { name: 'TEncryptLevel', index: 'TEncryptLevel', header: '病人密级', width: 80, align: 'left', hidden: true },
         { name: 'TPatLevel', index: 'TPatLevel', header: '病人级别', width: 80, align: 'left' , hidden: true},
     ...
     var jqOptions = {
         ...
         loadComplete: function () {
             ...
             $('#grid-disptotal').clearJqGrid();
             $("#jqgh_grid-dispdetail_TSelect a").text($g('全消'));
             AddjqGridCheckEvent();
         },
     ...
     $("#jqgh_grid-dispdetail_TSelect").html('<a>'+$g('全消')+'</a>')
     $("#jqgh_grid-dispdetail_TSelect a").on('click',function(){SetSelectAll()})
 }
 ...
 function SetSelectAll() {
     var tmpSelectFlag = '';
     if ($("#jqgh_grid-dispdetail_TSelect a").text() == $g('全选')) {
         $("#jqgh_grid-dispdetail_TSelect a").text($g('全消'));
         tmpSelectFlag = 'Y';
     } else {
         $("#jqgh_grid-dispdetail_TSelect a").text($g('全选'));
         tmpSelectFlag = 'N';
     }
     var selDspIdArr = [];
 ...
 function QueryDispWardList() {
     ...
     var dispWay = $('#sel-dispway').select2('data')[0].id;
     var includePreIP = "";
     if ($('#chk-preip').is(':checked')) {
        includePreIP = 'Y';
     }
     var params = startdate + '!!' + starttime + '!!' + enddate + '!!' + endtime + '!!' + phaloc + '!!' + wardloc + '!!' + phalocgrp + '!!' + priority + '!!' + dispTypeStr + '!!' + dispWay + '!!' + includePreIP;
     $('#grid-wardlist')
         .setGridParam({
             datatype: 'json',
             page: 1,
 ...
 function GetQueryDispParams() {
     ...
     if (DHCPHA_CONSTANT.VAR.TAB == '#div-ward-condition') {
         ...
         if (selectid == '' || selectid == null) {
             dhcphaMsgBox.alert($g('请先选择需要发药的就诊记录!'));
             return '';
         }
         ...
         if (dispTypeStr!="") dispCats = dispTypeStr + '#' + pri;
         else  dispCats = DHCPHA_CONSTANT.VAR.PHALOCDISPTYPE + '#' + pri;
         
     } else {
         dhcphaMsgBox.alert($g('请刷新界面后重试!'));
         return '';
     }
     var startdatetime = $('#date-start').val();
     var enddatetime = $('#date-end').val();
 ...
 function DoDisp(result) {
     ...
     if (result == true) {
         ...
         if (DHCPHA_CONSTANT.VAR.PARAMS != '') {
             ...
             var audituserflag = paramsarr[34];
             if (dispuserflag == 'Y' || operaterflag == 'Y' || audituserflag == 'Y') {
                 dispflag = 1;
 ...
 function ExecuteDisp(dispoptions) {
     ...
     dispoptions.audituser = dispoptions.audituser || '';
     var phaloc = $('#sel-phaloc').val();
     ...
     if (admdispflag != '') {
         ...
         if (wardStr == '') {
             dhcphaMsgBox.alert($g('按登记号发放获取不到病区'));
             return;
         }
     ...
     for (var wardi = 0; wardi < wardArr.length; wardi++) {
         ...
         for (var cati = 0; cati < catarr.length; cati++) {
             ...
             var PhacRowid = SaveDispensing(cat, pid, phaloc, wardid, dispoptions.phauser, dispoptions.operateuser, dispoptions.audituser);
             ...
             if (PhacRowid > 0) {
                 ...
                 alert($g('发药类别')+':' + GetDispCatNameByCode(cat) + ',' + PhacRowid);
             } else{
                var phacRetArr = PhacRowid.toString().split('^')
                if (phacRetArr[0] < 0){
                    if (cantDispInfoErr.indexOf(phacRetArr[1]) < 0 && phacRetArr[1] !== '未发出药品'){
                        cantDispInfoErr.push(phacRetArr[1])
                    }
                }
             }
         }
     ...
     if (phacrowidStr == '' || phacrowidStr == 0) {
         if (cantDispInfoErr.length > 0){
            dhcphaMsgBox.alert($g('未发出药品，原因如下')+'：</br>'+cantDispInfoErr.join('</br>')); 
         }else{
            dhcphaMsgBox.alert($g('未发出药品，您可以在发药明细中查具体原因'));
         }
         return;
     } else {
     ...
     dhcphaMsgBox.confirm($g('是否打印？'), function (rtn) {
         $('[data-bb-handler]').attr('disabled', true);
         KillDetailTmp();
         if (rtn == true) {
             $('.bootbox-body').text($g('正在打印')+'...');
 ...
 function SaveDispensing(dispcat, pid, phaLoc, wardid, colluser, operateuser, audituser) {
     if (colluser == '') {
     ...
     var printuser = DHCPHA_CONSTANT.SESSION.GUSER_ROWID;
     var inputStr = [phaLoc, wardid, dispcat, colluser, operateuser, printuser, audituser].join('^');
 ...
 //拒绝发药
 function DoRefuse() {
     if ($('#sp-title').text() == $g('发药汇总')) {
         dhcphaMsgBox.alert($g('请切换到发药明细进行拒绝!'));
         return;
     }
     if (DhcphaGridIsEmpty('#grid-dispdetail') == true) {
         return;
     ...
     if (canrefuse == 0) {
         dhcphaMsgBox.alert($g('请选择需要拒绝发药的明细!'));
         return;
     }
 ...
 function InitDispType(locrowid) {
  ...
  //初始化发药方式
 function InitDispWay() {
     var selectoption = {
        data:[
        { id: "WARDDISP", text: $g("病区发药") },
        { id: "BASEDISP", text: $g("基数药发药") }
        ],
         allowClear: false,
         minimumResultsForSearch: Infinity,
         placeholder: '发药类型...'
     };
     $('#sel-dispway').dhcphaSelect(selectoption);
     $('#sel-dispway').on('select2:select', function (event) { 
        $('#btn-find').click();
     });
 }
 ...
 function SendOrderToMachine(phacStr) {
     if (DHCPHA_CONSTANT.VAR.PARAMS != '') {
         ...
         if (sendflag == 'Y') {
             dhcphaMsgBox.alert($g("如需使用包药机接口，需要有对应的合同模块，请添加合同模块后联系产品组！"))
             return;
 ...
 function SetLocalConfig() {
     ...
     if ($('#chk-localconfig').is(':checked')) {
         if (DHCPHA_CONSTANT.VAR.MAC == '') {
             dhcphaMsgBox.alert($g('获取不到Mac地址,请检查是否添加信任站点'));
             return;
         }
         var phaloc = $('#sel-phaloc').val();
 ...
 function InitRefuseReasonModal() {
     ...
     $('#btn-refusereason-sure').on('click', function () {
         ...
         if (refusereason == '' || refusereason == null) {
             dhcphaMsgBox.alert($g('请选择拒绝发药原因!'));
             return;
         }
 ...
 function InitDispUserModal(params) {
     ...
     if (audituserflag != 'Y') {
         $('#sel-audituser').closest('div').hide();
     }
     $('#modal-inphaphauser').on('show.bs.modal', function () {
         ...
         $('#sel-audituser').dhcphaSelect(option);
         $('#sel-audituser').empty()
     });
     $('#btn-phauser-sure').on('click', function () {
         var phauser = $('#sel-phauser').val();
         var operateuser = $('#sel-operateuser').val();
         var audituser = $('#sel-audituser').val();
         if (dispuserflag == 'Y' && (phauser == '' || phauser == null)) {
             dhcphaMsgBox.alert($g('请选择发药人!'));
             return;
         }
         if (operaterflag == 'Y' && (operateuser == '' || operateuser == null)) {
             dhcphaMsgBox.alert($g('请选择配药人!'));
             return;
         }
         if (audituserflag == 'Y' && (audituser == '' || audituser == null)) {
             dhcphaMsgBox.alert($g('请选择核对人!'));
             return;
         }
 ...
 window.addEventListener('beforeunload', function () {
     if (navigator.sendBeacon) {
         navigator.sendBeacon('websys.Broker.cls?ClassName=web.DHCINPHA.Disp.Global&MethodName=KillQuerySave&Pid=' + DHCPHA_CONSTANT.VAR.QUERYPID, '');
         navigator.sendBeacon('websys.Broker.cls?ClassName=web.DHCINPHA.Disp.Global&MethodName=KillSetTmpWardCats&Pid=' + DHCPHA_CONSTANT.VAR.QUERYPID, '');
     } else {
         tkMakeServerCall('web.DHCINPHA.Disp.Global', 'KillSetTmpWardCats', DHCPHA_CONSTANT.VAR.QUERYPID);
