 ...
 */
 ...
 window.onload = function () {
     //QueryDispDocLocList();
     setTimeout('QueryDispDocLocList()', 1000);
 };
 //初始病区发药类别table
 ...
 function getPhaLocDispType(phaLocId) {
     ...
     if (dispcatsstr === ''){
         dhcphaMsgBox.alert($g('请先维护该药房的发药类别'));
         return        
     }
 ...
 function GetQueryDispParams() {
     ...
     if ($('#div-ward-condition').is(':hidden') == false) {
         ...
         if (selectid == '' || selectid == null) {
             dhcphaMsgBox.alert($g('请先选择需要发药的医生科室!'));
             $('#grid-disptotal').clearJqGrid();
             $('#grid-dispdetail').clearJqGrid();
             return '';
         }
 ...
 function ExecuteDisp(dispoptions) {
    ...
    if (dispoptions.phauser == undefined) {
     ...
     for (var cati = 0; cati < dispcatarr.length; cati++) {
         ...
         var PhacRowid = SaveDispensing(dispcat, pid, doclocrowid, dispoptions.phauser, dispoptions.operateuser, dispoptions.audituser);
         if (PhacRowid > 0) {
             if (phacrowidStr != '') {
             ...
             alert(getDispCatName(cat) + $g('发药失败!'));
         }
     }
     // 调用满整支
 ...
 function SaveDispensing(dispcat, pid, doclocrowid,phauser,operateuser,audituser) {
     if(phauser ==""||phauser == undefined) {
        phauser = DHCPHA_CONSTANT.SESSION.GUSER_ROWID ;
     }
     if(operateuser == undefined){
        operateuser = "";
     }
     var PhacRowid = tkMakeServerCall('web.DHCSTPCHCOLLSDOCLOC', 'SaveDisp', '', '', dispcat, pid, phauser, doclocrowid,operateuser,audituser);
     return PhacRowid;
 }
 //拒绝发药
 function DoRefuse() {
 ...
 function addCollStatCellAttr(rowId, val, rawObject, cm, rdata) {
     if (val.indexOf($g('欠费')) >= 0) {
         return 'class=dhcpha-record-owefee';
     } else if (val.indexOf($g('库存不足')) >= 0) {
