...
function BirthTimeOnBlur() {
    ...
    $.cm({
        ...
        if (parseInt(ageStr) < 0) {
            $.messager.alert("提示", "出生时间不能小于当前时间!", "info", function() {
                $("#Age").val("");
                $("#BirthTime").addClass("newclsInvalid");
                $("#BirthTime").focus();
            });
...
function GetPatDetailByPAPMINo() {
    ...
    if (myPAPMINo != "") {
        ...
        if (myary[0] == "0") {
            ...
            $.messager.alert("提示", "无此登记号的患者!");
        } else if (myary[0] == "-353") {
            $.messager.alert("提示", "此登记号不能重复建立账户!");
        } else if (myary[0] == "2002") {
	        $.messager.confirm("提示", "该登记号已经被合并无法使用，保留的登记号为 <font color=red>" + myary[1]+ "</font>，是否自动使用新登记号进行检索？", function(r) {
                if (r) {
	                $("#PAPMINo").val(myary[1]);
                    GetPatDetailByPAPMINo();
                }
    ...
    if (mykey == 13) {
        ...
        if (FindAppFlag == "1") {
            ...
            if (DifferenceAppInfo != "") {
                $.messager.confirm('确认对话框', "此证件号存在以下有效的公共卡预约预留信息:<br/>姓名:" + LastAppendName + "  电话:" + LastAppenTelH + "(最后一次预留信息)<br/>" + DifferenceAppInfo + "<br/>是否取最后一次的预留信息?", function(r) {
                    if (r) {
                ...
                $.messager.confirm('确认对话框', "此证件号存在有效的公共卡预约预留信息:<br/>姓名:" + LastAppendName + "  电话:" + LastAppenTelH + "<br/>是否带入预约预留信息?", function(r) {
                    if (r) {
                        $("#Name").val(LastAppendName);
                        $("#TelHome").val(LastAppenTelH);
                    }
...
function OtherNameclick(that) {
    ...
    src=('undefined'!==typeof websys_writeMWToken)?websys_writeMWToken(src):src;
    var $code = "<iframe width='100%' height='100%' scrolling='auto' frameborder='0' src='" + src + "'></iframe>";
    createModalDialog("OtherNameInfoManager", "多类型姓名管理", "500", "290", "icon-w-list", "", $code, "");
}
...
function ChangeCardClick() {
    ...
    src=('undefined'!==typeof websys_writeMWToken)?websys_writeMWToken(src):src;
    var $code = "<iframe width='100%' height='100%' scrolling='auto' frameborder='0' src='" + src + "'></iframe>";
    createModalDialog("cardmanagement","卡操作", 1360  , PageLogicObj.dh, "icon-edit", "", $code, "");
}
