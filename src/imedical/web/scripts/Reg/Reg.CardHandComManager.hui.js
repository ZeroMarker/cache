...
function InitComManagerWin(){
	...
	$HUI.combobox("#DLLType", {
		onSelect:function(record){
			var needChgElement=$("#ClassID,#Version,#CodeBase,#JsFunctionName,#EquipPort");
			if (record["ValueList"]=='LS') {
				needChgElement.val('');
				$('#ObjectID').popover({trigger:'hover',title:'提示',content:'必须和插件管理中的插件【调用ID名】一致'}).show();
				needChgElement.attr('disabled','disabled');
			}else{
				$('#ObjectID').popover('destroy');
				needChgElement.removeAttr('disabled');
			}
		}	
	})
