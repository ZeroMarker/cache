...
//document.write("<object ID='ClsIDCode' WIDTH=0 HEIGHT=0 CLASSID='CLSID:D5832C57-9821-4BB9-B500-35EEA0A926F3' CODEBASE='../addins/client/ReadPersonInfo.CAB#version=1,0,0,45' VIEWASTEXT>");
...
function ReadRegInfo_OnClick()
  { 
   ...
   //var ClsIDCode2=new ActiveXObject("ReadPersonInfo.ClsIDCode");
...
function BodyLoadHandler() { 
	...
	obj=document.getElementById("BSelectPreDate");
	if (obj){obj.onclick=BSelectPreDate_click;}
	...
	obj=document.getElementById("BSetDefaultVIP");
    if (obj){ obj.onclick=BSetDefaultVIP_click; }
...
{
	list=value.split("^");
	var obj=document.getElementById("PGADM_DR");
...
function SetTeam(value)
{
	...
	if (obj){obj.value=list[1]}
	var obj=document.getElementById("PGTeam_DR_Name");
	if (obj){obj.value=list[4]}
}
function CleanGroup()
{
	var obj=document.getElementById("PGADM_DR_Name");
	if (obj){obj.value=""}
	var obj=document.getElementById("PGADM_DR");
	if (obj){obj.value=""}
	var obj=document.getElementById("PGTeam_DR");
...
function iniForm()
{
	...
	if (obj && ""!=obj.value) {
		...
		if (parent.frames("PreItemList")){
				...
				var VipFlag=tkMakeServerCall("web.DHCPE.PreIADM","GetIfPccu",VIPLevel);
				
				if (VipFlag!="")
				{
					lnk=VipFlag+"?AdmId="+ID;
				}
				else{lnk="websys.default.csp?WEBSYS.TCOMPONENT=DHCPEPreItemList&TargetFrame=PreItemList.Qry&AdmType=PERSON&AdmId="+ID+"&PreOrAdd=PRE";}
...
function Save() {
	...
	if (obj){
		...
		var flag=cspRunServerMethod(IsClass,PEDateBegin,LocID,VIPLevel,"I",PETime);
		var Arr=flag.split("^");
		if (Arr[0]=="0"){
	...
	if ('0'==flag) {
		...
		if (""==iRowId) {
			...
			if (parent.frames["PreItemList"]){
				...
				var obj=parent.frames["PreItemList"].document.getElementById("NoRefresh");
				if (obj&&obj.checked){
					parent.frames["PreItemList"].CopyItem(iRowId);
				}
				...
				if (VipFlag!="")
				{
					...
					lnk=VipFlag+"?AdmId="+iRowId;
					}
				else
				{
					lnk="websys.default.csp?WEBSYS.TCOMPONENT=DHCPEPreItemList&TargetFrame=PreItemList.Qry&AdmType=PERSON&AdmId="+iRowId+"&PreOrAdd=PRE"+"&NetSetsID="+NetSetsID;
				}
				parent.frames["PreItemList"].location.href=lnk;
			}
		...
		if (""==iRowId) {
			if (parent.frames["PreItemList"]){
				...
				if (VipFlag!="")
				{
					...
					lnk=VipFlag+"?AdmId="+Rets[1];
					}
				else
				{
					lnk="websys.default.csp?WEBSYS.TCOMPONENT=DHCPEPreItemList&TargetFrame=PreItemList.Qry&AdmType=PERSON&AdmId="+Rets[1]+"&PreOrAdd=PRE"+"&NetSetsID="+NetSetsID;
				}
				parent.frames["PreItemList"].location.href=lnk;
			}
			if (parent.frames["PreItemList.Qry"]){
...
function Update_click() {
	...
	if (eSrc.disable) {
		...
		if (PicFileIsExist(FileName)) {
			...
			PEPhoto.DBConnectString = "CN_IPTCP:10.160.16.90[1972]:DHC-APP" //数据库服务器
			if (PhotoFtpInfo==""){
				PEPhoto.FTPFlag = "0" //是否保存到数据库  0  1
			}else{
				var FTPArr=PhotoFtpInfo.split("^");
				PEPhoto.DBFlag = "0" //是否保存到数据库  0  1
...
function Clear_click() {
	...
	if (parent.frames["PreIADM.Edit"]){
		lnk="websys.default.csp?WEBSYS.TCOMPONENT=DHCPEPreIADM.Edit";
		parent.frames["PreIADM.Edit"].location.href=lnk;
	}

	if (parent.frames["PreItemList"]){
		var obj=parent.frames["PreItemList"].document.getElementById("NoRefresh");
		if (obj&&obj.checked) return false;
		lnk="websys.default.csp";
		parent.frames["PreItemList"].location.href=lnk;
	}
	if (parent.frames["PreItemList.Qry"]){
...
function SaveIBIInfo()
{
	...
	*/
	...
	//证件类型
	var iPAPMICardType=""
	var obj=document.getElementById("PAPMICardType_DR_Name");
	if (obj) iPAPMICardType=obj.value;
...
{
	if (NetPreID!="") return fasle;
	var obj=document.getElementById("PEDateBegin");
	if (obj) obj.value=NewDateStr;
	var obj=document.getElementById("PEDateEnd");
	if (obj) obj.value=NewDateStr;
}
function BSelectPreDate_click()
{
	var PreIADMDR="",ExpStr="",Sex="",VIP="";
	var obj=document.getElementById("PIADM_RowId");  
	if (obj){ PreIADMDR=obj.value; }
	if (PreIADMDR==""){
		var obj=document.getElementById("Sex_DR_Name");
	...
	var lnk="websys.default.csp?WEBSYS.TCOMPONENT="+"DHCPEPreDate"
			+"&PreIADMID="+PreIADMDR+"&ExpStr="+ExpStr;
	//var wwidth=800;
	//var wheight=500;
	//var xposition = (screen.width - wwidth) / 2;
	//var yposition = (screen.height - wheight) / 2;
	var nwin='toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=no,resizable=yes,copyhistory=yesheight='
			+'height='+300+',width='+800+',left='+200+',top='+300
			;
	var cwin=window.open(lnk,"_blank",nwin)	
}
...
function UpdatePreAudit()
{
	alert("个人费用请在个人预约查询界面查看");
	return false;
}
function SetDefaultDate()
{
	if (NetPreID!="") return false;
	var PreIADMDR="",ExpStr="",Sex="",VIP="";
	var obj=document.getElementById("PIADM_RowId");  
	if (obj){ PreIADMDR=obj.value; }
	if (PreIADMDR==""){
	...
	var LocID=session["LOGON.CTLOCID"];
	var DefaultDate=tkMakeServerCall("web.DHCPE.PreDate","GetDefaultDate",LocID,PreIADMDR,ExpStr);
	var obj=document.getElementById("PEDateBegin");
	if (obj) obj.value=DefaultDate;
	change();
}
function change()
{
