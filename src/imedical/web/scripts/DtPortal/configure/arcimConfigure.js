...
function formExp()
{
	...
	if ("IE" == mb) {
		...
		xlApp.SheetsInNewWorkbook = 3 //'设置创建几个工作表
		
		var hisNum=2,HisName="老版医嘱项配置"
		if (arcimObj.hisEdition==3){hisNum=3;HisName="iMedical8.3医嘱项配置"}
	...
	if (("FF"==mb)||("Chrome" == mb)) {
        ...
        var hisNum=2,HisName="老版医嘱项配置"
		if (arcimObj.hisEdition==3){hisNum=3;HisName="iMedical8_3医嘱项配置"}
		...
		for (i=0;i<jsonStr.length;i++)
		{
		 ...
		 ctx = {rows: rowsXML1, nameWS:  '医嘱配置'|| 'Sheet' + 1};
         worksheetsXML += format(tmplWorksheetXML, ctx);
         ctx = {rows: rowsXML2, nameWS: '老版医嘱项配置' || 'Sheet' + 2};
         worksheetsXML += format(tmplWorksheetXML, ctx);
         ctx = {rows: rowsXML3, nameWS: 'iMedical8_3医嘱项配置' || 'Sheet' + 3};
         worksheetsXML += format(tmplWorksheetXML, ctx);
