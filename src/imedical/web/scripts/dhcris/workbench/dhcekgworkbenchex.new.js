...
var RegBillEKGBtn = new Ext.Button({
    text:'心电收费',
    width:100,
    iconCls:'btn-ris-register',
    cls:'orderbtn',
  		...
  		if (rows.length > 0) {
			...
			if (isCharge=="N")
			{
  			...
  			var regInfo=orderBodyList+"^"+""+"^"+""+"^"+""+"^"+session['LOGON.USERID']+"^"+""+"^"+""+"^"+""+"^"+""+"^"+LocDR;
   			...
   			var ret = tkMakeServerCall("web.DHCEkgRegisterPatient","register",regInfo);
   			if (ret=="-10001")
			 {
				alert('病人未交费不能登记!');
				return false;
		     ...
		     {
			    tkMakeServerCall("web.DHCEkgService","register",regInfo);
...
//心电登记EKG OBillEKGBtn RegEKGBtn
var RegEKGBtn = new Ext.Button({
    text:'登记',
    width:80,
    iconCls:'btn-ris-register',
    //cls:'orderbtn',
...
//门诊病人收费 OBillEKGBtn
var OBillEKGBtn = new Ext.Button({
    text:'门诊病人卡消费',
    width:100,
    iconCls:'btn-ris-paid',
    cls:'orderbtn',
    //height:50,
    handler:function(){
	  	//alert(CarrierGrid);
	    	...
	    	var LocDR="";
	  		...
	  		for(var i=0;i<rows.length;i++)
	  		{
	  			...
	  			else
	  			{
	  				if (LocDR!=LocDRGet)
	  				{
	  					alert("接收科室不同，不允许一起收费!");
	  					return;
	  				}
	  			}
	  			
	  		var patientType = row.get('TPatientType');
	  		if(patientType!="门诊病人")
	      	{
		      	alert("非门诊病人不能收费 !");
		      	return false;
    ...
    handler:function(){
    	...
    	var rows = gsm.getSelections();//根据选择列获取到所有的行
  		if (rows.length > 0) {
   		...
   		var str="websys.default.csp?TPSID=dhcekg&WEBSYS.TCOMPONENT=DHCRisUnRegisterPatient&EpisodeID="+PaadmDR+"&OEOrdItemID="+orderBodyList+"&PatineStudyNo="+PatineStudyNo;
...
var tb = new Ext.Toolbar({
		id : 'tb',
		items : [
			'日期: ',StartDate,' 至 ',EndDate,'-',' 接收科室:',RecDep, '-', '卡类型:',CardType,' 卡号:',CardNo,ReadCardBtn,'-' ,' 登记号:',PatientNo //,'-',LockFlag
		]
	});
...
var tbar1 = new Ext.Toolbar({
	...
	items:[ 
	 //findBtn, '-', ClearBT, '-', RegBtn, '-', AppBtn, '-', UnRegBtn, '-', UnbookBtn, '-',RejectAppBillBtn,'-',DHCRisBookedItemBtn,'-',ExportBtn,'-',QueryFeeBtn,'-',PrintAppBillBtn,'->', GridColSetBT
	 	...
	 	//{xtype:'tbspacer',width:880},'-', findBtn,'-', ClearBT,'-',RegEKGBtn ,'-' //,RegPrintBtn
	 	
	 	//{xtype:'tbspacer',width:790},'-', findBtn,'-', ClearBT,'-',RegEKGBtn ,'-' ,UnRegBtn,'-' 
	 	
	 	{xtype:'tbspacer',width:705},'-', findBtn,'-', ClearBT,'-',RegEKGBtn ,'-' ,OBillEKGBtn,'-' 
	] 
}); 
