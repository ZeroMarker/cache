...
function GridColSet(gridSelected,AppName) {
	...
	function SaveData() {
		...
		for (var i = 0; i < rowCount; i++) {				
			...
			if(StrData=="")
			{
				StrData=rowid+"^"+SeqNo+"^"+id+"^"+Header+"^"+Width+"^"+Align+"^"+Sort+"^"+Hidden+"^"+Name+"^"+Format+"^"+DataType;
			}
			else
			{
				StrData=StrData+xRowDelim()+rowid+"^"+SeqNo+"^"+id+"^"+Header+"^"+Width+"^"+Align+"^"+Sort+"^"+Hidden+"^"+Name+"^"+Format+"^"+DataType;
			}			
		}
		if(StrData==""){
	...
	function InitSave(records) {
		...
		for (var i = 1; i < ColCount; i++) {
			...
			if(FindRecord(Name,records)==false){
				...
				if(StrData=="")
				{		
					StrData="^"+i+"^"+ColId+"^"+Header+"^"+Width+"^"+Align+"^"+Sortable+"^"+Hide+"^"+Name+"^"+Format+"^"+DataType;
				}
				else
				{
					StrData=StrData+xRowDelim()+"^"+i+"^"+ColId+"^"+Header+"^"+Width+"^"+Align+"^"+Sortable+"^"+Hide+"^"+Name+"^"+Format+"^"+DataType;
				}
			}
		}	
		if(StrData==""){
	...
	// 指定列参数
	var fields = ["rowid","id","header","width","align", "name","sortable", "hidden",{name:'seqno',type: 'float'},"format"];
	// 支持分页显示的读取方式
	...
	var MasterInfoGrid = new Ext.grid.EditorGridPanel({
				...
				stripeRows : true,
				loadMask : true,
				plugins: [ColumnSort,ColumnHidden],
				clicksToEdit : 1
			});
	...
	var window = new Ext.Window({
				...
				layout : 'border',
				modal:true,
				items : [MasterInfoGrid,HisListTab,PanelButton],
