...
function checkSchedule()
{
    ...
    if (status=="预约")
    {
		...
		var infoHint="此医嘱已经预约到"+shcduleDate+" "+shcduleTime+",是否转预约?";
	...
	if( arcItmFieldSet )
	{
		...
		while (m < length)
		{
			...
			if ( index >= 0 )
			{
					...
					if (modalityStore.getAt(m).get('earliest')=="Y")
					{
						nearestResScheduleCmbId=arcItmFieldSet.id+"^"+modalityStore.getAt(m).get('rowid');
					}
					
					/*
					if (modalityStore.getAt(m).get('view')!="N")
					{
						if (nearestResScheduleCmbId=="")
						{
							nearestResScheduleCmbId=arcItmFieldSet.id+"^"+modalityStore.getAt(m).get('rowid');
						
							nearestDate=scheduleDate;
							nearestTime=modalityStore.getAt(m).get('timespan');
							nearestRate=modalityStore.getAt(m).get('number')/modalityStore.getAt(m).get('MaxNumber');
						}
						else
						{
							//判断日期、时间和比率  时间和日期可以用数值来比较
							if ( (parseInt(nearestDate,10))>(parseInt(scheduleDate,10)))
							{
								nearestResScheduleCmbId=arcItmFieldSet.id+"^"+modalityStore.getAt(m).get('rowid');
								nearestDate=scheduleDate;
								nearestTime=modalityStore.getAt(m).get('timespan');
							...
							else if (nearestDate==scheduleDate)
							{
								var nearestTimeInt=nearestTime.replace(/[^0-9]/ig,"");
								var getTime=modalityStore.getAt(m).get('timespan').replace(/[^0-9]/ig,"");
								if ( nearestTimeInt>getTime)
								{
									nearestResScheduleCmbId=arcItmFieldSet.id+"^"+modalityStore.getAt(m).get('rowid');
									nearestDate=scheduleDate;
									nearestTime=modalityStore.getAt(m).get('timespan');
								...
								else if (nearestTimeInt==getTime )
								{
									var getRate=modalityStore.getAt(m).get('number')/modalityStore.getAt(m).get('MaxNumber');
									if (getRate>nearestRate)
									{
										nearestResScheduleCmbId=arcItmFieldSet.id+"^"+modalityStore.getAt(m).get('rowid');
										nearestDate=scheduleDate;
										nearestTime=modalityStore.getAt(m).get('timespan');
		...
		if (checkboxId!="")
		{
			...
			{
				if ( nearestResScheduleCmbId!="" && status!="预约")
				{
					var checkBoxObjNear=Ext.getCmp(nearestResScheduleCmbId);
					if ( checkBoxObjNear != null)
					{
						//jsSet=true;
						checkBoxObjNear.setValue(true);
					}
				}
