...
function initDateArray()
{
	...
	var format=Ext.getCmp("startDate").format;
	//alert(format);
	//for ( var i = 0 ; i <length ; i++)
	while ( ( (dateEnd.getTime()-startDate.getTime())/3600/1000 ) >=0 )
	{
		var year=startDate.getFullYear().toString();
		var month=((startDate.getMonth()+1)>9)?((startDate.getMonth()+1).toString()):('0'+(startDate.getMonth()+1).toString());
		var day=(startDate.getDate()>9)?(startDate.getDate().toString()):('0'+startDate.getDate().toString());
		var dateDesc=format.replace(/Y/g,year);
		dateDesc=dateDesc.replace(/m|n/g,month);
		dateDesc=dateDesc.replace(/d|j/g,day);
