(function($){
    ...
    function isSpecialCol(target,key){
    ...
    function toHtml(target, rows,specialColumn,filtterCol){
        ...
        for(var i=0;i<columns.length;i++){
	       if((columns[i].hidden)&&(columns[i].exportHidden==undefined)){}
			else if (columns[i].exportHidden==true){}
			else if ((columns[i].hidden==true)&&(columns[i].exportHidden==false)){
				fields[j]=columns[i].field;
				j++;    
			}
			else if (columns[i].checkbox){}
			else{
				fields[j]=columns[i].field;
				j++;
			}
        ...
        for(var i=0; i<fields.length; i++){
            ...
            if(filtterCol&&includes(filtterCol,col.field)) continue  //检测是否需要过滤此列
        ...
        $.map(rows, function(row){
            ...
            for(var i=0; i<fields.length; i++){
                var field = fields[i];
                if(filtterCol&&includes(filtterCol,field)) continue
                
                if(specialColumn){
	            	var specialValue=isSpecialCol(specialColumn,field)
                
	                ///特殊字段
	                if(specialValue!=false){
		            	data.push(
	                    	'<td style="'+tdStyle0+'">'+specialValue+'</td>'
	                	);
		            }	
	            }
    ...
    function toExcel(target, param){
        ...
        if (typeof param == 'string'){
            ...
            rows = param['rows'];
            specialCol=param['specialCol']
            filtterCol=param['filtterCol']
        ...
        var table = toHtml(target, rows,specialCol,filtterCol);
