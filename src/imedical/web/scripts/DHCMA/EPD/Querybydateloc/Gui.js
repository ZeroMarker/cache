	...
	obj.chkStatus = function() {
		...
		for (var dicIndex = 0; dicIndex < dicList.length; dicIndex++) {
			...
			$("#chkStatusList").append(
				 "<div style='float:left;width:11%'><input id="+dicSubList[0]+" type='checkbox' class='hisui-checkbox' "+(dicSubList[0]==1? "checked='checked'":"")+" label="+dicSubList[1]+" name='chkStatus' value="+dicSubList[0]+"></div>"
			);
		}
		...
		columns:[[
			...
			{field:'expander',title:'操作',width:45,align:'center',
				formatter: function(value,row,index){
					...
					var btn = '<a href="#" class="btn_detail" onclick="objScreen.OpenEPDReport(\'' + ReportID + '\',\'' + PatientID + '\',\'' + EpisodeID + '\')"></a>';
					return btn;
				}
			}, 
			...
			{field:'CardNo',title:'有效证件号',width:'200'},
