//流行病学->项目定义->Event
function InitESurTypeExtWinEvent(obj) {
    //初始化信息
    obj.LoadEvent = function (args) {
        ...
        $('#btnEdit').on('click', function () {
            var rowData = obj.gridESurTypeExt.getSelected();
            obj.LayerEdit();        //打开编辑框
    ...
    //选中类型
    obj.gridESurTypeExt_onSelect = function () {
        var rowData = obj.gridESurTypeExt.getSelected();
        ...
        if (rowData["ID"] == obj.RecRowID) {    //取消选中
            ...
            obj.RecRowID = "";
            obj.gridESurTypeExt.clearSelections();
        } else {    //选中行
    ...
    obj.btnSave_click = function () {
        ...
        InputStr += "^" + IsActive;
        var flg = $m({
            ClassName: "DHCMed.EPD.ESurTypeExt",
            MethodName: "Update",
            aInputStr: InputStr,
            aSeparete: "^"
        }, false);
        if (parseInt(flg) <= 0) {
            ...
            obj.gridESurTypeExt.reload();//刷新当前页
        }
    }
    //删除
    obj.btnDelete_click = function () {
        var rowDataID = obj.gridESurTypeExt.getSelected()["ID"];
        if (rowDataID == "") {
