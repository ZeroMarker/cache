function InitReportWinEvent(obj) {
	...
	obj.cboDeathReason_select = function(){
		...
		if (DeathReason=='艾滋病相关疾病死亡') {
			$('input[name="cbgDeathReasonHIV"]').checkbox('enable');
			$('input[name="cbgDeathReasonOthers"]').checkbox('disable');
		}else if(DeathReason=='艾滋病无关死亡'){
			$('input[name="cbgDeathReasonHIV"]').checkbox('disable');
			$('input[name="cbgDeathReasonOthers"]').checkbox('enable');
		}else{
			$('input[name="cbgDeathReasonHIV"]').checkbox('disable');
			$('input[name="cbgDeathReasonOthers"]').checkbox('disable');
		}
	...
	obj.cboIsDead_select = function(){
		...
		if (IsDead=='是') {
			...
			$('input[name="cbgDeathReasonSource"]').checkbox('enable');
		}else {
			...
			$('input[name="cbgDeathReasonSource"]').checkbox('disable');
	...
	obj.DisplayRepInfo = function(){
		...
		if(obj.ReportID==""){
			...
			if(arrHIVSurvey9RowID!=""){
					var objDicHIVSurvey9 = $cm({                  
						ClassName:"DHCMed.SS.Dictionary",
						MethodName:"GetObjById",
						id:arrHIVSurvey9RowID
					},false);
					if(objDicHIVSurvey9!=""){
						$('#cboHIVSurvey9').combobox('setValue',arrHIVSurvey9RowID);
						$('#cboHIVSurvey9').combobox('setText',objDicHIVSurvey9.Description);
					}
			}
	...
	obj.GetRepData = function (step) {
		...
		inputStr=inputStr+"^"+$('#cboHIVSurvey9').combobox('getValue');
	...
	obj.btnDelete_click = function(){
		...
		$.messager.confirm("提示","删除操作会直接删除本报告，删除后将不可再查询到，请确认是否删除?",function(r){
			if(r){
				...
				if(parseInt(ret)<=0){
					$.messager.alert("错误","删除失败!"+ret, 'error');
					return;
				}else{
					$.messager.alert("提示","报告删除成功!", 'info');  //报告删除为物理删除
