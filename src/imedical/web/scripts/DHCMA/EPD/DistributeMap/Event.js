function InitDMapChartWinEvent(obj){
	...
	myChart.setOption(option1,true);
   	...
   	obj.LoadRep = function(){
		...
		var dataInput = "ClassName=" + 'DHCMed.EPDService.DistributeMap' + "&QueryName=" + 'QryDisMap' + "&Arg1=" + aHospIDs + "&Arg2=" + aDateFrom + "&Arg3="+ aDateTo + "&Arg4="+ aRepStatus   +"&ArgCnt=" + 4;
		$.ajax({
			url: "./dhcma.query.csp",
			type: "post",
			timeout: 30000, //30秒超时
	...
	obj.echartRatio = function(runQuery){
		...
		for (var indRd = 0; indRd < arrRecord.length; indRd++){
			...
			var str= new Object();
			arrxAxis.push(rd["BTDecs"]);
			arrCount.push(rd["DiseaseNum"]);
			if (rd["DiseaseNum"]>0){
				str.name  = rd["BTDecs"];
				str.value = rd["DiseaseNum"];
				arrCount02.push(str);
			}
		}
