function InitReportWinEvent(obj) {
	...
	obj.CheckReport=function(){
		...
		if (!$.trim(PatCardNo)){errStr += '请填写证件号码!<br>';}
		...
		if ((obj.RepTypeCode=="2")||(obj.RepTypeCode=="4")) {		
			...
			if (Common_CompareDate(Birthday,SickDate)>0){
				errStr = errStr + "初次发病时间不能早于出生时间！<br>"; 
			}
			if (Common_CompareDate(SickDate,FDTreatDate)>0){
				errStr = errStr + "首次抗精神病药治疗时间不能早于初次发病时间！<br>"; 
			}
			if (Common_CompareDate(FDTreatDate,AdmitDate)>0){
				errStr = errStr + "首次抗精神病药治疗时间不能晚于本次入院时间！<br>"; 
			}
			if (Common_CompareDate(AdmitDate,DiagDate)>0){
				errStr = errStr + "本次确诊时间不能早于本次入院时间！<br>"; 
			}
			if (Common_CompareDate(DiagDate,RepDate)>0){
				errStr = errStr + "本次确诊时间不能晚于填卡时间！<br>"; 
			}
		} else {
			...
			if (Behaviors=="") { errStr = errStr + "请选择既往危险行为!<br>"; }
			...
			if ((AgreeText=="同意参加社区服务管理") && (AgreeDate=="")){ errStr = errStr + "请选择“知情同意时间”!<br>"; }
			
			//初次发病时间≤知情同意时间≤当前时间
			if (Common_CompareDate(SickDate,AgreeDate)>0){
				errStr = errStr + "知情同意时间不能晚于初次发病时间！<br>"; 
			}
			if (Common_CompareDate(AgreeDate,Common_GetDate(new Date()))>0){
				errStr = errStr + "知情同意时间不能晚于当前时间！<br>"; 
			}
			if (Common_CompareDate(SickDate,FDTreatDate)>0){
				errStr = errStr + "首次抗精神病药治疗时间不能早于初次发病时间！<br>"; 
			}
			if (Common_CompareDate(Birthday,SickDate)>0){
				errStr = errStr + "出生时间不能晚于初次发病时间！<br>"; 
			}
			if (Common_CompareDate(SickDate,AdmitDate)>0){
				errStr = errStr + "初次发病时间不能晚于本次入院时间！<br>"; 
			}
			if (Common_CompareDate(AdmitDate,DiagDate)>0){
				errStr = errStr + "本次入院时间不能晚于本次确诊时间！<br>"; 
			}
			if (Common_CompareDate(DiagDate,RepDate)>0){
				errStr = errStr + "本次确诊时间不能晚于填卡时间！<br>"; 
			}
			if (Common_CompareDate(RepDate,Common_GetDate(new Date()))>0){
				errStr = errStr + "填卡时间不能晚于当前时间！<br>"; 
			}
		}
		if(errStr != "")
		{
