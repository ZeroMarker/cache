...
function ExportGridByCls(grid1,grid2,strfilename)
{	
	var TemplatePath = $m({                  
	...
	var FileName=TemplatePath+"\\\\"+"DHCMA.HAI.ANT.RepList.xlsx";
    ...
    var columns = grid1.options().columns[0];
	var rows = grid1.getData().originalRows;   //本地所有数据
    ...
    var columns = grid2.options().columns[0];
	var rows = grid2.getData().originalRows;   //本地所有数据
	for (var i = 0; i < columns.length; i++) {
	 	if ((columns[i].hidden)||(!columns[i].title)) {
		 	xlSheet.Columns(i+1).Hidden = true;	
		 	continue;   //隐藏列不写入
	...
	var fname = xls.Application.GetSaveAsFilename(strfilename, "Excel Spreadsheets (*.xlsx), *.xlsx");
	if (fname != false){
