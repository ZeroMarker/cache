function InitS390InfPosfWinEvent(obj){
	obj.numbers = "ALL";
   	...
   	obj.LoadRep = function(){
		...
		false);	
		p_URL = 'dhccpmrunqianreport.csp?reportName=DHCMA.HAI.STATV2.S231OpRiskType.raq&aHospIDs='+aHospID +'&aDateFrom=' + DateFrom +'&aDateTo='+ DateTo +'&aStaType='+ Statunit +'&aQryCon='+ objDic.BTCode;	
		if(!ReportFrame.src){
   	...
   	obj.ShowEChaert1 = function(arrViewLoc,arrInfRatio){
		...
		option1 = {
			 title : {
				text: '不同感染风险指数（手术类型）手术部位感染例数统计表',
				textStyle:{
						fontSize:20
					},
				x:'center',
		...
		var dataInput = "ClassName=" + 'DHCHAI.STATV2.S231OpRiskType' + "&QueryName=" + 'QryOpRiskType' + "&Arg1=" + HospID + "&Arg2=" + DateFrom + "&Arg3=" + DateTo+ "&Arg4=" + StaType+"&ArgCnt=" + 4;
		$.ajax({
	   ...
	   obj.echartLocInfRatio = function(runQuery){
			...
			for (var indRd = 0; indRd < arrRecord.length; indRd++){                     
				...
				if(rd==undefined){   //去掉全院、医院、科室组后,会有数据变为undefined
					...
					arrViewLoc.push(rd["OperJZInfCount"]);
					arrViewLoc.push(rd["OperZQInfCount"]);
					arrInfRatio.push("急诊");
					arrInfRatio.push("择期");
				}
			}
