	...
	obj.numbers = "ALL";
   	...
   	obj.ShowEChaert1 = function(){
		...
		var option1 = {
			...
			legend: {
				data:['应用抗菌药物的手术例次中24小时内停药的手术例次数','停药率'],
				x: 'center',
				y: 30
			},
			grid:{
			...
			series: [
				 {
					name:'应用抗菌药物的手术例次中24小时内停药的手术例次数',
					type:'bar',
					barMaxWidth:50,
					data:[]
				},
	   ...
	   obj.echartLocInfRatio = function(runQuery){
			...
			for (var indRd = 0; indRd < arrRecord.length; indRd++){                     
				...
				if(rd==undefined){   //去掉全院、医院、科室组后,会有数据变为undefined
					...
					arrInfCount.push(rd["OperAntiEndCount"]);
					arrInfRatio.push(parseFloat(rd["EndAntiRatio"]).toFixed(2));
