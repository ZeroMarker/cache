function InitUCADJInfWinEvent(obj){
	obj.numbers = "ALL";
   	...
   	obj.LoadRep = function(){
		...
		p_URL = 'dhccpmrunqianreport.csp?reportName=DHCMA.HAI.STATV2.S321UCADJInf.raq&aHospIDs='+aHospID +'&aDateFrom=' + aDateFrom +'&aDateTo='+ aDateTo+'&aLocType='+aLocType+'&aQryCon='+aQrycon+'&aStatDimens='+aStatDimens+'&aLocIDs='+aLocIDs+'&aPath='+cspPath;	
		if(!ReportFrame.src){
   ...
   obj.up=function(x,y){
		 if(obj.sortName=="调整率"){
			return y.AdjINFRatio-x.AdjINFRatio;
		}else {
   	...
   	obj.ShowEChaert1 = function(){
		...
		$cm({
			ClassName:"DHCHAI.STATV2.S3TubeAdjInf",
	   ...
	   obj.echartLocInfRatio = function(runQuery){
			...
			for (var indRd = 0; indRd < arrRecord.length; indRd++){
				...
				rd["AdjINFRatio"] =parseFloat(rd["AdjINFRatio"].replace('%','').replace('‰','')).toFixed(2);
			}
			...
			for (var indRd = 0; indRd < arrRecord.length; indRd++){                     
				...
				if(rd==undefined){   //去掉全院、医院、科室组后,会有数据变为undefined
					...
					arrUseRatio.push(parseFloat(rd["AdjINFRatio"]).toFixed(2));
