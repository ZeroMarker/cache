function InitICUTotalInfWinEvent(obj){
	obj.numbers = "ALL";
   	...
   	obj.LoadRep = function(){
		...
		p_URL = 'dhccpmrunqianreport.csp?reportName=DHCMA.HAI.STATV2.ICUTotalStat.raq&aHospIDs='+aHospID +'&aDateFrom=' + aDateFrom +'&aDateTo='+ aDateTo+'&aLocType='+aLocType+'&aQryCon='+aQrycon+'&aStatDimens='+aStatDimens+'&aLocIDs='+aLocIDs+'&aIsSplit='+IsSplit+'&aPath='+cspPath;	
		if(!ReportFrame.src){
	...
	obj.up=function(x,y){
        ...
        if(obj.sortName=="呼吸机使用率"){
			return y.VAPRatio-x.VAPRatio;
		}else if(obj.sortName=="导尿管使用率"){
			return y.UCRatio-x.UCRatio;
		}else if(obj.sortName=="导尿管感染发病率"){
			return y.UCINFRatio-x.UCINFRatio;
		}else if(obj.sortName=="中央血管导管使用率"){
			return y.PICCRatio-x.PICCRatio;
		}else if(obj.sortName=="中央血管导管感染发病率"){
