function InitImpSubRateWinEvent(obj){	 
	...
	obj.LoadContent =function (index){
		if (index==0){
	...
	$('#btnMore').on('click', function(){ 	
   	...
   	obj.ShowReport = function() {
		...
		if (obj.Type==1) {
			p_URL = 'dhccpmrunqianreport.csp?reportName=DHCMA.HAI.STATV2.SpeActAntSub.raq&aHospIDs='+aHospID +'&aDateFrom=' + aDateFrom +'&aDateTo='+ aDateTo+'&aLocType='+aLocType+'&aSubDateType='+aSubDateType+'&aSubHourType='+aSubHourType+'&aLabTestSet='+LabTestSet+'&aQryCon='+aQryCon+'&aUseSubDateType='+aUseSubDateType+'&aStatDimens='+aStatDimens+'&aLocIDs='+aLocIDs+'&aPath='+cspPath;				
		}else if(obj.Type==2) {
			p_URL = 'dhccpmrunqianreport.csp?reportName=DHCMA.HAI.STATV2.SpeActInfSub.raq&aHospIDs='+aHospID +'&aDateFrom=' + aDateFrom +'&aDateTo='+ aDateTo+'&aLocType='+aLocType+'&aLabTestSet='+LabTestSet+'&aQryCon='+aQryCon+'&aStatDimens='+aStatDimens+'&aLocIDs='+aLocIDs+'&aPath='+cspPath;			
		}else if(obj.Type==3) {
			p_URL = 'dhccpmrunqianreport.csp?reportName=DHCMA.HAI.STATV2.SpeActAntComb.raq&aHospIDs='+aHospID +'&aDateFrom=' + aDateFrom +'&aDateTo='+ aDateTo+'&aLocType='+aLocType+'&aSubDateType='+aSubDateType+'&aSubHourType='+aSubHourType+'&aLabTestSet='+LabTestSet+'&aQryCon='+aQryCon+'&aStatDimens='+aStatDimens+'&aLocIDs='+aLocIDs+'&aPath='+cspPath;				
		}
		if(!ReportFrame.src){
	...
	obj.option3 = function(arrViewLoc,arrCureUseAntiCnt,arrCombUseCnt,arrBfCureSubCnt,arrBfCureSubRatio,endnumber){
		var option1 = {
			title : {
				text: '住院患者抗菌药物治疗前病原学送检率统计图',
				textStyle:{
			...
			yAxis: [
				{
					...
					name: '使用人数',
					min: 0,
					interval:Math.ceil(arrCureUseAntiCnt[0]/10),
					axisLabel: {
						formatter: '{value} '
					}
				},
   	...
   	obj.ShowEChaert = function(){
		...
		if (obj.Type==1) {
			...
			$cm({
				ClassName:"DHCHAI.STATV2.SpeActAntSub",
			...
			obj.myChart.showLoading();	//隐藏加载动画
			$cm({
				ClassName:"DHCHAI.STATV2.SpeActInfSub",
				QueryName:"QryInfSub",
				aHospIDs:aHospID, 
				aDateFrom:aDateFrom, 
				aDateTo:aDateTo, 
			...
			obj.myChart.showLoading();	//隐藏加载动画	
			$cm({
				ClassName:"DHCHAI.STATV2.SpeActAntComb",
	...
	obj.echartAntCombRatio = function(runQuery){
		...
		var endnumber = (14/arrViewLoc.length)*100;
		// 使用刚指定的配置项和数据显示图表。
		obj.myChart.setOption(obj.option3(arrViewLoc,arrCureUseAntiCnt,arrCombUseCnt,arrBfCureSubCnt,arrBfCureSubRatio,endnumber),true);
	}
