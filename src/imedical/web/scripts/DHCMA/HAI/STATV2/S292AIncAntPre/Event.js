function InitS021DayInfWinEvent(obj){
	...
	obj.up=function(x,y){
        if(obj.sortName=="抗菌药物的给药时间在术前0.5-1小时的手术例次数")
		{
   	...
   	obj.LoadRep = function(){
		...
		p_URL = 'dhccpmrunqianreport.csp?reportName=DHCMA.HAI.STATV2.S292AIncAntPre.raq&aHospIDs='+aHospID +'&aDateFrom=' + DateFrom +'&aDateTo='+ DateTo +'&aStaType='+ Statunit +'&aQryCon='+ Qrycon+'&aOperCat='+OperCat+'&aStatDimens='+aStatDimens+'&aLocIDs='+aLocIDs+'&aPath='+cspPath;	
		if(!ReportFrame.src){
   	...
   	obj.ShowEChaert1 = function(){
		...
		var option1 = {
			title : {
				text: 'Ⅰ类切口手术患者预防使用抗菌药物术前0.5-1小时给药率统计图',
				textStyle:{
						fontSize:20
					},
				x:'center',
			...
			legend: {
				data:['抗菌药物的给药时间在术前0.5-1小时的手术例次数','术前0.5-1小时给药率'],
				x: 'center',
				y: 30
			},
			grid:{
			...
			series: [
				 {
					name:'抗菌药物的给药时间在术前0.5-1小时的手术例次数',
					type:'bar',
					barMaxWidth:50,
					data:[]
				},
		...
		$cm({
			ClassName:'DHCHAI.STATV2.S292AIncAntPre',
			QueryName:'S292AIncAntPre',
			aHospIDs:HospID,
			aDateFrom:DateFrom,
			aDateTo:DateTo,
			aStaType:StaType,
			...
			obj.sortName="术前0.5-1小时给药率"; //初始化排序指标
...
//		var dataInput = "ClassName=" + 'DHCHAI.STATV2.S292AIncAntPre' + "&QueryName=" + 'S292AIncAntPre' + "&Arg1=" + HospID + "&Arg2=" + DateFrom + "&Arg3=" + DateTo+ "&Arg4=" + StaType+ "&Arg5=" + Qrycon+"&Arg6=" + OperCat+"&Arg7=" + aStatDimens+"&Arg8=" + aLocIDs+"&ArgCnt=" + 8;
//		$.ajax({
