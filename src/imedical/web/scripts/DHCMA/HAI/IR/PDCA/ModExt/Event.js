function InitModExtWinEvent(obj) {
    //初始化信息
    obj.LoadEvent = function (args) {
        ...
        $('#btnEdit').on('click', function () {
            var rowData = obj.gridPDCAModExt.getSelected();
            obj.LayerEdit();        //打开编辑框
    ...
    //选中类型
    obj.gridPDCAModExt_onSelect = function () {
        var rowData = obj.gridPDCAModExt.getSelected();
        ...
        if (rowData["ID"] == obj.RecRowID) {    //取消选中
            ...
            obj.RecRowID = "";
            obj.gridPDCAModExt.clearSelections();
        } else {    //选中行
    ...
    obj.btnSave_click = function () {
        var PDCACode = $('#txtCode').val();
        var PDCADesc = $('#txtDesc').val();
        var PDCAExtType = $('#cboExtType').combobox('getValue');
        var PDCADatType = $('#cboDatType').combobox('getValue');
        var PDCADicType = $('#cboDicType').combobox('getValue');
        ...
        InputStr += "^" + PDCACode;
        InputStr += "^" + PDCADesc;
        InputStr += "^" + PDCAExtType;
        InputStr += "^" + PDCADatType;
        InputStr += "^" + PDCADicType;
        InputStr += "^" + IsActive;

        var flg = $m({
            ClassName: "DHCHAI.IR.PDCAModExt",
            MethodName: "Update",
            aInputStr: InputStr,
            aSeparete: "^"
        }, false);
        if (parseInt(flg) <= 0) {
           if(flg=="-2"){
            ...
            obj.gridPDCAModExt.reload();//刷新当前页
        }
    }
    //删除
    obj.btnDelete_click = function () {
        var rowDataID = obj.gridPDCAModExt.getSelected()["ID"];
        if (rowDataID == "") {
