...
function InitExpTypeWinEvent(obj) {
    ...
    obj.LoadEvent = function (args) {
		...
		$('#btnEdit').on('click', function(){
		    var rowData = obj.gridExpType.getSelected();
		    obj.LayerEdit();        //打开编辑框
    ...
    //选中职业暴露类型
    obj.gridExpType_onSelect = function () {
        var rowData = obj.gridExpType.getSelected();
        ...
        if (rowData["ID"] == obj.RecRowID) {    //取消选中
            ...
            obj.RecRowID = "";
            obj.gridExpType.clearSelections();
        } else {    //选中行
	...
	obj.btnSave_click = function(){
		...
		var flg = $m({
		    ClassName: "DHCHAI.IR.OccExpType",
		    MethodName: "Update",
		    aInputStr: InputStr,
			aSeparete:"^"
		},false);
		if (parseInt(flg) <= 0){
			...
			obj.gridExpType.reload();   //刷新当前页
		}
	}
	//删除
	obj.btnDelete_click = function(){
	    var rowDataID = obj.gridExpType.getSelected()["ID"];
		if (rowDataID==""){
	...
	obj.OpenView1 = function (aID) {
	    ...
	    var strUrl = "./dhcma.hai.ir.occexptypeext.csp?Parref=" + aID ;
		if(obj.flg=="1"){
	    websys_showModal({
	        url: strUrl,
	        title: '项目定义',
    ...
    //检验结果对照点击事件
	obj.OpenView2 = function (aID) {
	    var strUrl = "./dhcma.hai.ir.occexptypeepd.csp?Parref=" + aID;
	    websys_showModal({
	        url: strUrl,
	        title: '检验结果对照',
	        iconCls: 'icon-w-paper',
    ...
    //血清检查计划点击事件
	obj.OpenView3 = function (aID) {
	    var strUrl = "./dhcma.hai.ir.occexptypelab.csp?Parref=" + aID;
	    websys_showModal({
	        url: strUrl,
	        title: '血清检查计划',
	        iconCls: 'icon-w-paper',
