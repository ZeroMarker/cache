...
function InitBedChart(obj,WardID,WarnDate,WarnItems,WarnItemsArray,OpenType){
	obj.WardDesc="";
	var objWard="";
	var htmlStr = '<div class="hisui-dialog" id="layerbed" style="display:none;overflow-y:auto;height:100%;">'
				+	'<link rel="stylesheet" href="../scripts/dhcMA/HAI/BT/PACRoomBed/BedChart.css">'
				+	'<div id="LayerWard">'
				+ 	'</div>'
				+ '</div>';
	if ($(".hisui-layout").siblings().is("#layerbed")) {
		$("#LayerWard").html();   // 清除 LayerWard 下面的内容
	}else {
		$(".hisui-layout").after(htmlStr);
	}
	...
	if (OpenType=="2") {   // 疑似筛查床位图
		...
		var runQuery = $m({
			...
			MethodName:"QryScreenPatToBed",
			aTypeFlag:status,
			aDateFrom:start,
			aDateTo:end,
			aLocID:locID,
			aIsMessage:IsMessage,
			aIsFollow:IsFollow,
			aIsFinDel:IsFinDel,
			aIsAttFlg:IsAttFlg
		},false);
	...
	obj.SetBedStyle = function(BedWarnDate,WardID){
		obj.WarnBedArray.length=0;
	   	...
	   	obj.BedEpisodeDr.length=0;
	   	var DataQuery = $m({
			ClassName:"DHCHAI.IRS.CCWarningSrv",
			MethodName:"QryWarnPatList",
			aLocID:WardID,
			aWarnDate:BedWarnDate,
	   ...
	   //	var DataQuery = $.Tool.RunQuery('DHCHAI.IRS.CCWarningSrv','QryWarnPatList',WardID,BedWarnDate,WarnItems,ItemList);
		...
		var DataQuery = $m({
			...
			MethodName:"QryBedDrByDate",
			aDateFrom:BedWarnDate,
			aDateTo:BedWarnDate,
			aHospIDs:"",
			aLocIDs:WardID
		},false);
