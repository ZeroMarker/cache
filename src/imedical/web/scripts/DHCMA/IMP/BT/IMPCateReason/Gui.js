...
function InitviewScreen(){
	...
	obj.BTReason=$HUI.combobox("#BTReason",{
				url:$URL+'?ClassName=DHCMA.IMP.BTS.IMPCateReasonSrv&QueryName=QryIMPReasonDic&aIsActive=1&ResultSetType=Array',
				valueField:'BTID',
				textField:'ReasDesc',
				})
		 
	obj.gridIMPReason = $HUI.datagrid("#gridIMPReason",{
		fit:true,
		title: "重点患者特因维护",
		...
		columns:[[	
			{field:'BTCode',title:'特因代码',width:'150',sortable:true},
			{field:'BTDesc',title:'特因描述',width:'200',sortable:true},
			{field:'BTIsActive',title:'是否<br>有效',align:'center',width:'80',sortable:true}
		]],
		onDblClickRow:function(rowIndex,rowData){
			if(rowIndex>-1){
				obj.gridIMPReason_onDbselect(rowData);
			}
		},
		onSelect:function(rindex,rowData){
			if (rindex>-1) {
				obj.gridIMPReason_onSelect(rowData);
			}
		},
		onLoadSuccess:function(data){
		}
