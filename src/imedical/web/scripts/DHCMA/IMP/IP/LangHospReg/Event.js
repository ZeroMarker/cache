function InitReportWinEvent(obj) {
	...
	obj.btnSave_click = function(){
		...
		var RegDate ="";
		var RegTime ="";
		var diagnosisProcess = $('#diagnosisProcess').val();
		var nowDiagnosis = $('#nowDiagnosis').val();
		var longHospReason  = $('#longHospReason').val();
		var diagnosisPlan  = $('#diagnosisPlan').val();
		var ReportStatus = "Submit";
		
		if(typeof diagnosisProcess=="undefined"||diagnosisProcess==""){
			$.messager.popover({msg: $g('诊疗经过不能为空！'),type:'error'});
			return false;
		}else if(typeof nowDiagnosis=="undefined"||nowDiagnosis==""){
			$.messager.popover({msg: $g('目前诊断不能为空！'),type:'error'});
			return false;
		}else if(typeof longHospReason=="undefined"||longHospReason==""){
			$.messager.popover({msg: $g('长期住院原因分析不能为空！'),type:'error'});
			return false;
		}else if(typeof diagnosisPlan=="undefined"||""==diagnosisPlan){
			$.messager.popover({msg: $g('诊疗计划不能为空！'),type:'error'});
			return false;
		}
		...
		if(IMPRecordID>0){
			...
			if(IMPRegRet>0){
				...
				//更新长期住院病例上报登记表
				var LangHospRegID="";
				if(obj.objLangHospReg){
					//var OperCompJson = JSON.parse(obj.OperCompl);
					LangHospRegID=obj.objLangHospReg.ID;
				}
				var RegisterDr=IMPRegRet;
				var InHospLoc=obj.ActLoc;
				var RegLoc=session['LOGON.CTLOCID'];
				var AdmDiagnosis=obj.AdmDiagnos;
				var DiagnosisProcess=diagnosisProcess;
				var NowDiagnosis=nowDiagnosis;
				var LongHospReason =longHospReason;
				var DiagnosisPlan = diagnosisPlan;
				
				var InputStr = LangHospRegID+"^";
				InputStr=InputStr+RegisterDr+"^";
				InputStr=InputStr+InHospLoc+"^";
				InputStr=InputStr+RegLoc+"^";
				InputStr=InputStr+AdmDiagnosis+"^";
				InputStr=InputStr+DiagnosisProcess+"^";
				InputStr=InputStr+NowDiagnosis+"^";
				InputStr=InputStr+LongHospReason+"^";
				InputStr=InputStr+DiagnosisPlan;
				var ret =$m({
					ClassName:"DHCMA.IMP.IP.LangHospReg",
					MethodName:"Update",
					aInputStr:InputStr
				}, false);
				if(ret>0){
