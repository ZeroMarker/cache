function InitReportWinEvent(obj) {
	...
	obj.DisplayRepInfo = function(){
		if(obj.EpisodeID!=""){
			var PatientAdmCls = $cm({
				...
				paadm:obj.aEpisodeID
			},false);
			
			$('#txtPatientNo').text(PatientAdmCls.MrNo);             //病案号
			$('#txtReLoc').text(PatientAdmCls.Department);		  		//科室
			$('#txtWard').text(PatientAdmCls.Ward);					//病房
			$("#reInHospDate").text(PatientAdmCls.AdmitDate);
			obj.reInHospDate = PatientAdmCls.AdmitDate
			/*$HUI.datebox("#reInHospDate",{
				value:PatientAdmCls.AdmitDate
			});*/
			var IntReInDate=$m({                  
				ClassName:"DHCMA.Util.IO.FromHisSrv",
				MethodName:"DateHtmlToLogical",
				aDate:PatientAdmCls.AdmitDate
			},false);
			...
			var AdmDiagnosStr = $m({
				...
				aEpisodeID:obj.aEpisodeID
			},false);
			if(AdmDiagnosStr!=""){
				var arrDiagnos=AdmDiagnosStr.split("^");
				$('#txtReInDiagnos').text(arrDiagnos[2]);
				obj.AdmDiagnos = arrDiagnos[2];
			}
			var lastEpisodeID = $cm({
				ClassName:"DHCMA.IMP.Task.IMPScreeningSrv",
				MethodName:"getLastHospEpisodeID",
				aEpisodeID:obj.aEpisodeID
			},false);
			if(lastEpisodeID>0){
				obj.lastEpisodeID=lastEpisodeID;
				var lastPatientAdmCls = $cm({
					ClassName:"DHCMed.Base.PatientAdm",
					MethodName:"GetObjById",
					paadm:obj.lastEpisodeID
				},false);
				$('#txtLoc').text(lastPatientAdmCls.Department);
				$('#txtIndih').text(lastPatientAdmCls.AdmitDate);
				$('#txtOutHDate').text(lastPatientAdmCls.DisDate);
				
				var IntOutDate=$m({                  
					ClassName:"DHCMA.Util.IO.FromHisSrv",
					MethodName:"DateHtmlToLogical",
					aDate:lastPatientAdmCls.DisDate
				},false);
				var ReDays=IntReInDate-IntOutDate;
				$('#readmissionWay').text(ReDays);			
				var LastAdmDiagnosStr = $m({
					ClassName:"DHCMA.IMP.IPS.OperCompRegSrv",
					MethodName:"GetAdmDiagnos",
					aEpisodeID:obj.lastEpisodeID
				},false);
				if(LastAdmDiagnosStr!=""){
					var arrDiagnos=LastAdmDiagnosStr.split("^");
					$('#txtAdmDiagnos').text(arrDiagnos[2]);
				}
				var OutDiagnosStr = $m({
					ClassName:"DHCMA.IMP.IPS.OperCompRegSrv",
					MethodName:"GetDisDiagnos",
					aEpisodeID:obj.lastEpisodeID
				},false);
				if(OutDiagnosStr!=""){
					var arrDiagnos=OutDiagnosStr.split("^");
					$('#txtOutHDiagnos').text(arrDiagnos[2]);
				}
			}
	...
	obj.btnSave_click = function(){
		...
		var AdmDiagnosis =obj.AdmDiagnos;
		var aRegId="";
		var RegUser = session['LOGON.USERID'];
		var RegDate ="";
		var RegTime ="";
		var reInHospDate = obj.reInHospDate
		var readmissionWay = $('#readmissionWay').val();
		var firstHospSituation  = $('#firstHospSituation').val();
		var firstDischSituation = $('#firstDischSituation').val();
		var readmissionReason = $('#readmissionReason').val();
		var preventionMeasures = $('#preventionMeasures').val();
		var ReportStatus = "Submit";
		if(typeof firstHospSituation=="undefined"||firstHospSituation==""){
			$.messager.popover({msg: $g('首次住院情况不能为空！'),type:'error'});
			return false;
		}else if(typeof firstDischSituation=="undefined"||firstDischSituation==""){
			$.messager.popover({msg: $g('首次出院后情况不能为空！'),type:'error'});
			return false;
		}else if(typeof readmissionReason=="undefined"||""==readmissionReason){
			$.messager.popover({msg: $g('再次住院原因分析不能为空！'),type:'error'});
			return false;
		}else if(typeof preventionMeasures=="undefined"||""==preventionMeasures){
			$.messager.popover({msg: $g('拟采取预防手段不能为空！'),type:'error'});
			return false;
		}
		...
		if(IMPRecordRet>0){
			...
			if(IMPRegRet>0){
				...
				var URTHospRegID="";
				if(obj.objURTHosp){
					URTHospRegID=obj.objURTHosp.ID;
				}
				var RegisterDr=IMPRegRet;
				var AdmDiagnosis=AdmDiagnosis;
				var ReInHospDate=reInHospDate;
				var ReadmissionWay=readmissionWay;
				var FirstHospSituation=firstHospSituation;
				var FirstDischSituation=firstDischSituation;
				var ReadmissionReason= readmissionReason;
				var PreventionMeasures = preventionMeasures;
				
				var InputStr = URTHospRegID+"^";
				InputStr=InputStr+RegisterDr+"^";
				InputStr=InputStr+AdmDiagnosis+"^";
				InputStr=InputStr+ReInHospDate+"^";
				InputStr=InputStr+ReadmissionWay+"^";
				InputStr=InputStr+FirstHospSituation+"^";
				InputStr=InputStr+FirstDischSituation+"^";
				InputStr=InputStr+ReadmissionReason+"^";
				InputStr=InputStr+PreventionMeasures;
				var ret =$m({
				...
				$.messager.popover({msg: $g('主表登记提交失败！'),type:'error'});
			}
		}else{
			$.messager.popover({msg: $g('筛查登记提交失败！'),type:'error'});
		}
	};
