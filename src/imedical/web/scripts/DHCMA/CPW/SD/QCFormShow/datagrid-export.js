(function($){
    ...
    function toHtml(target, rows){
        ...
        var tdStyle0 = 'vertical-align:middle;text-align:left;padding:0 4px';
        data.push('<tr style="'+trStyle+'">');
        for(var i=0; i<fields.length; i++){
            var col = dg.datagrid('getColumnOption', fields[i]);
            var tdStyle = tdStyle0 + ';width:'+col.boxWidth+'px;';
            if (col.hidden==true)  //隐藏元素不输出
            	{ 
            		fields.splice(i,i);
            		var i=i-1;  //移除元素后，指针前移
            		continue; 
            	}
        ...
        $.map(rows, function(row){
	        if (row.EditPower==1) {  //隐藏行不输出
	            data.push('<tr style="'+trStyle+'">');
	            for(var i=0; i<fields.length; i++){
	                var field = fields[i];
	                if (row.BTTypeDesc.indexOf('字典')>-1) {
		                if (field=='ExecResult') field='ExecResultText'
		                }
	                data.push(
