...
function InitviewScreen(){
    ...
    $.parser.parse(); // 解析整个页面
	obj.gridItemCat =$HUI.datagrid("#gridItemCat",{
		fit: true,
		title: "付费项目分类",
		iconCls:"icon-resort",
	    ...
	    url:$URL,
	    queryParams:{
		    ClassName:"DHCMA.CPW.SDS.PCItemCatSrv",
			QueryName:"QryPCItemCat"		
	    },
		columns:[[
		...
		onSelect:function(rindex,rowData){
			if (rindex>-1) {
				obj.gridItemCat_onSelect();
			}
		},
		onDblClickRow:function(rowIndex,rowData){
			if(rowIndex>-1){
				obj.gridItemCat_onDbselect(rowData);
			}
		},
