...
function InitviewScreenEvent(obj){	
	...
	obj.LoadEvent = function(args){ 
		...
		$('#btnEdit').on('click', function(){
			var rd=obj.gridItemEpis.getSelected();
			obj.layer(rd);
		});
    ...
    //双击编辑事件
	obj.gridItemEpis_onDbselect = function(rd){
		obj.layer(rd);
	}
	//选择
	obj.gridItemEpis_onSelect = function (){
		var rowData = obj.gridItemEpis.getSelected();
		...
		if (rowData["BTID"] == obj.RecRowID) {
			...
			obj.gridItemEpis.clearSelections();
		} else {
	...
	obj.btnSave_click = function(){
		...
		var flg = $m({
			ClassName:"DHCMA.CPW.SD.PCItemEpis",
			MethodName:"Update",
			aInputStr:inputStr,
			aSeparete:CHR_1
		},false);
