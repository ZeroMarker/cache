...
///临床路径出入径查询
function ExportCPWGridByCls(grid,filename)
{
	try {
	...
	var rs=grid.getData().originalRows;    //本地所有数据
    ...
    for (var i = 0; i <rs.length; i++) {
        for (var j = 0; j < columns.length; j++) {	
			//alert(columns[j].title)	
            if (rs[i][columns[j].field] != null) {
				xlSheet.Cells(i + 2, j + 1).NumberFormatLocal = "@";
				xlSheet.Cells(i + 2, j + 1).value = rs[i][columns[j].field].toString();            
            } else {			
...
function ExportGridByCls(grid,filename)
{
	...
	if (fields != '') {
		...
		var flg = ExtTool.RunServerMethod("web.DHCCPW.MR.ExcelSrv","ExportGrid","fillxlSheet",xClass,xQuery,xArguments,fields);
	}
