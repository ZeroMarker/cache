...
function PrintDataToExcel(strMethodGetServer,strMethodGetData,strTemplateName,strArguments)
{
	...
	var flg = tkMakeServerCall("DHCMA.CPW.CPS.PrintSrv","PrintCPWInformedConsert","fillxlSheet",EpisodeID,session['DHCMA.CTLOCID'],session['DHCMA.HOSPID']);
	xlSheet.printout();
...
function PrintCPWToExcel(strMethodGetServer,strMethodGetData,strTemplateName,strArguments)
{
	...
	xlBook=xls.Workbooks.Add(FileName);
	xlSheet=xlBook.Worksheets.Item(1);
	var flg = tkMakeServerCall("DHCMA.CPW.CPS.PrintSrv","PrintCPWImpl","fillxlSheet",EpisodeID);
	xlSheet.printout();
