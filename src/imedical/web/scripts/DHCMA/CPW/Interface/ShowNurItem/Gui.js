...
function InitShowNurItemWin(){
	...
	if (CPWInfo!=""){
		...
		$("input[name='EpType'][value='C']").attr("label",$g("当前阶段")+"<span style='color:#F00'>["+$g(obj.CurEpisDesc)+"]</span>");
	}else{
		$("#layout1").hide();
		$("#layout2").show();
	}
	...
	// 护理项目
	obj.gridNurItem = $HUI.datagrid("#gridNurItem",{
		fit: true,
		title: "主要护理工作",
		iconCls:"icon-resort",
		...
		columns:[[
			...
			{field:'ItemInfo',title:'护理项目',width:'350'},
		...
		onLoadSuccess: function(data){
			$("#gridNurItem").datagrid("uncheckAll");
		}
	});
