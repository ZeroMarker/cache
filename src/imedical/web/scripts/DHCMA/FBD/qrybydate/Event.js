//页面Event
function InitFBDQueryWinEvent(obj){	
    
	obj.LoadEvent = function(args){ 
	    obj.chkStatus();
	    $('#FBDQuery').datagrid('loadData',{ 'total':'0',rows:[] });  //初始加载显示记录为0
	    obj.FBDQueryLoad();
		//查询
	...
	obj.btnExportSel_click = function() {
		...
		if (rows>0) {
			...
			if (length>0) {
				$('#FBDQuery').datagrid('toExcel', {
				    filename: '食源性疾病报告查询表.xls',
	...
	obj.btnExportAll_click = function() {
		var rows = obj.gridFBDQuery.getRows().length;  
		if (rows>0) {
			$('#FBDQuery').datagrid('toExcel','食源性疾病报告查询表.xls');
		}else {
			$.messager.alert("确认", "无数据记录,不允许导出", 'info');
			return;
		}
	...
	//打开链接
	obj.OpenFBDReport = function(aReportID,aEpisodeID) {
		var strUrl = "./dhcma.fbd.report.csp?1=1&EpisodeID=" + aEpisodeID+"&ReportID=" + aReportID+ "&LocFlag=" + LocFlag;
	    websys_showModal({
			url:strUrl,
			title:'食源性疾病报告',
			iconCls:'icon-w-epr',  
	...
	obj.gridReport_rowdbclick = function(rowData) {
		...
		var strUrl = "./dhcma.fbd.report.csp?1=1"
	...
	obj.FBDQueryLoad = function(){
		...
		$cm ({
			...
			$('#FBDQuery').datagrid({loadFilter:pagerFilter}).datagrid('loadData', rs);				
		});
    }
