function InitReportWinEvent(obj) {
	...
	obj.btnSaveTemp_click = function(){
		...
		if(parseInt(ret)<=0){
			...
			$.messager.alert($g("提示"),$g("草稿数据保存成功")+"！<br>"+$g("请及时完善糖尿病卡信息"), 'info');
			obj.ReportID=ret;
			obj.DisplayRepInfo();
			obj.InitRepPowerByStatus(obj.ReportID);
			//追加隐藏元素，用于强制报告保存失败时作废诊断
	...
	obj.btnPrint_click = function(){
		...
		var LODOP=getLodop();
		LODOP.PRINT_INIT("PrintCDTNBReport");		//打印任务的名称
		...
		LodopPrintURL(LODOP,"dhcma.cd.lodoptnb.csp?ReportID="+obj.ReportID);
		LODOP.PRINT();			//直接打印
		/*
		if(obj.ReportID==""){
