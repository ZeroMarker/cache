...
function InitQueryWinEvent(obj){	
	...
	obj.LoadEvent = function(args){ 
		...
		$('#btnEdit').on('click', function(){
			var rd = obj.gridICDDx.getSelected();
			obj.InitDialog(rd);
		});
		//删除
	...
	//单击选中事件
	obj.gridICDDx_onSelect = function (){
		var rowData = obj.gridICDDx.getSelected();
		...
		if (rowData["ID"] == obj.RecRowID) {
			...
			obj.gridICDDx.clearSelections();  //清除选中行
		} else {
    ...
    //双击弹出编辑事件
	obj.gridICDDx_onDbselect = function(rd){
		obj.InitDialog(rd);
	}
	...
	obj.InitDialog = function(rd){
		if(rd){
			...
			$('#txtName').val('').validatebox("validate");
			$('#txtResume').val('');
