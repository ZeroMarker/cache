...
function InitViewportEvent(obj){	
	...
	obj.btnPrint_click = function() {
		...
		if (length>1) {
			...
			if(parseInt(ret)<=0){
				...
				var LODOP=getLodop();
				LODOP.PRINT_INIT("PrintCDGWZSInfo");		//打印任务的名称
				...
				LodopPrintURL(LODOP,"dhcma.cd.lodopgwzs.csp?ReportID="+aReportID);
				LODOP.PRINT();			//直接打印
			}
		}else{
	...
	obj.OpenReport = function(aReportID,aEpisodeID) {
		var strUrl= "./dhcma.cd.reportgwzs.csp?1=1&ReportID=" + aReportID + "&EpisodeID=" + aEpisodeID +"&LocFlag=" + LocFlag;
	    websys_showModal({
			url:strUrl,
			title:'高温中暑报卡',
			iconCls:'icon-w-epr',  
	        ...
	        closable:false,
			width:1320,
			height:'86.8%',  //8.2以上版本支持百分比，8.2以下的用具体像素，如height:window.screen.availHeight-80,
	...
	obj.gridRepInfo_click = function() {
        ...
        var strUrl= "./dhcma.cd.reportgwzs.csp?1=1&ReportID=" + ReportID + "&EpisodeID=" + EpisodeID +"&LocFlag=" + LocFlag;
