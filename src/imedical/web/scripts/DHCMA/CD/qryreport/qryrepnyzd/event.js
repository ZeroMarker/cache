...
function InitViewportEvent(obj){	
	...
	obj.btnExport_click = function() {
		...
		if (rows>0) {
			$('#gridRepInfo').datagrid('toExcel','农药中毒报告查询表.xls');
		}else {
	...
	obj.OpenReport = function(aReportID,aEpisodeID) {
		var strUrl= "./dhcma.cd.reportnyzd.csp?1=1&ReportID=" + aReportID + "&EpisodeID=" + aEpisodeID ;
	    websys_showModal({
			url:strUrl,
			title:'农药中毒报卡',
			iconCls:'icon-w-epr',  
	...
	obj.gridRepInfo_click = function() {
        ...
        var strUrl= "./dhcma.cd.reportnyzd.csp?1=1&ReportID=" + ReportID + "&EpisodeID=" + EpisodeID ;
