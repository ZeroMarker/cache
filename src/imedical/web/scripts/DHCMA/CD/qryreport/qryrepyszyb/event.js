...
function InitViewportEvent(obj){	
	...
	obj.btnPrint_click = function() {
		...
		if (length>1) {
			...
			if(parseInt(ret)<=0){
				...
				var LODOP=getLodop();
				LODOP.PRINT_INIT("PrintCDYSZYBInfo");		//打印任务的名称
				...
				LodopPrintURL(LODOP,"dhcma.cd.lodopyszyb.csp?ReportID="+aReportID);
				LODOP.PRINT();			//直接打印
			}
		}else{
	...
	obj.OpenReport = function(aReportID,aEpisodeID) {
		var strUrl= "./dhcma.cd.reportyszyb.csp?1=1&ReportID=" + aReportID + "&EpisodeID=" + aEpisodeID +"&LocFlag=" + LocFlag;
	    websys_showModal({
			url:strUrl,
			title:'职业病报卡',
			iconCls:'icon-w-epr',  
	...
	obj.gridRepInfo_click = function() {
        ...
        var strUrl= "./dhcma.cd.reportyszyb.csp?1=1&ReportID=" + ReportID + "&EpisodeID=" + EpisodeID +"&LocFlag=" + LocFlag;
