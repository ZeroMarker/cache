...
function InitViewportEvent(obj){	
	...
	obj.btnExport_click = function() {
		...
		if (rows>0) {
			grid2excel($("#gridRepInfo"), {IE11IsLowIE: false,filename:'高血压报告查询表',allPage: true});
			/*var length = obj.gridRepInfo.getChecked().length;
			if (length>0) {
				$('#gridRepInfo').datagrid('toExcel', {
				    filename: '高血压报告查询表.xls',
	...
	obj.btnPrint_click = function() {
		...
		if (length>1) {
			...
			if(parseInt(ret)<=0){
				...
				var LODOP=getLodop();
				LODOP.PRINT_INIT("PrintCDGXYInfo");		//打印任务的名称
	...
	obj.OpenReport = function(aReportID,aEpisodeID) {
	    ...
	    websys_showModal({
	        ...
	        closable:false,
			width:1320,
			height:'638px',  //8.2以上版本支持百分比，8.2以下的用具体像素，如height:window.screen.availHeight-80,
