...
function InitViewportEvent(obj){	
	...
	obj.btnExport_click = function() {
		...
		if (rows>0) {
			$('#gridRepInfo').datagrid('toExcel','心脑血管事报告查询表.xls');
		}else {
	...
	obj.OpenReport = function(aReportID,aEpisodeID) {
		var strUrl= "./dhcma.cd.reportxnxg.csp?1=1&ReportID=" + aReportID + "&EpisodeID=" + aEpisodeID ;
	    websys_showModal({
			url:strUrl,
			title:'心脑血管报卡',
			iconCls:'icon-w-epr',  
	...
	obj.gridRepInfo_click = function() {
        ...
        var strUrl= "./dhcma.cd.reportxnxg.csp?1=1&ReportID=" + ReportID + "&EpisodeID=" + EpisodeID ;
