...
function InitViewportEvent(obj){	
	...
	obj.btnExport_click = function() {
		...
		if (rows>0) {
			grid2excel($("#gridRepInfo"), {IE11IsLowIE: false,filename:'糖尿病报告查询表',allPage: true});
			/*var length = obj.gridRepInfo.getChecked().length;
			if (length>0) {
				$('#gridRepInfo').datagrid('toExcel', {
				    filename: '糖尿病报告查询表.xls',
	...
	obj.btnPrint_click = function() {
		...
		if (length>1) {
			...
			if(parseInt(ret)<=0){
				...
				var LODOP=getLodop();
				LODOP.PRINT_INIT("PrintCDTNBInfo");		//打印任务的名称
				...
				LodopPrintURL(LODOP,"dhcma.cd.lodoptnb.csp?ReportID="+aReportID);
				LODOP.PRINT();			//直接打印
			}
		}else{
	...
	obj.OpenReport = function(aReportID,aEpisodeID) {
		var strUrl= "./dhcma.cd.reporttnb.csp?1=1&ReportID=" + aReportID + "&EpisodeID=" + aEpisodeID +"&LocFlag=" + LocFlag;
	    websys_showModal({
			url:strUrl,
			title:'糖尿病报卡',
			iconCls:'icon-w-epr',  
	...
	obj.gridRepInfo_click = function() {
        ...
        var strUrl= "./dhcma.cd.reporttnb.csp?1=1&ReportID=" + ReportID + "&EpisodeID=" + EpisodeID +"&LocFlag=" + LocFlag;
