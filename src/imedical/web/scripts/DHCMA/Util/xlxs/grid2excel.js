    ...
    function formatDatagridData(rows,columns,cfg){
        ...
        for (var i = 0; i < columns.length; i++) {
            ...
            for (var j = 0; j < cols.length; j++) {
                var col = cols[j];			
                if ((col.hidden && ! cfg.showHidden && col.exportHidden )||(col.checkbox)||!col.field) {  //隐藏列及、复选框无标题列
                    continue;
                ...
                for (var j = 0; j < cols.length; j++) {
                    ...
                    if ((col.hidden && ! cfg.showHidden && col.exportHidden )||(col.checkbox) ||!col.field) {  //隐藏列及、复选框无标题列
                        ...
                        if (typeof col.exportFormatter == 'function') {
                            var content=replaceHtml(col.exportFormatter(row[col.field]||"",row,ind)||'') ;
                            //newrow[key]=content;
                            newrow.push(content);
    ...
    function formatExtjsData(grid,store,cfg){
        ...
        for (var j=0;j<storeItems.length;j++){
            ...
            for (i=0;i<colCfg.length;i++){
                ...
                if (typeof col.exportFormatter == 'function') {
                    val=replaceHtml(col.exportFormatter(val,"",storeItem));
                } else if (col.renderer){  //renderer 只考虑value和record参数
