/**
 * 团体导入  DHCPEImportDailyPersonLisResultInfo.js
 * @Author   wangguoying sxt
 * @DateTime 2022-08-23
 */
...
var actionListObj = $HUI.datagrid("#actionList",{
		...
		columns:[[
			{field:'TName',align:"center",title:'姓名',editor:'text'},
			{field:'TDate',title:'日期',editor:'text'},
			{field:'TPAADMIn',title:'院内就诊ID',editor:'text'},
			{field:'TOEOrdItemIn',title:'院内医嘱ID',editor:'text'},
			{field:'TPAADMOut',title:'院外就诊ID',editor:'text'},
			{field:'TArcItm',title:'医嘱项ID',editor:'text'},
			{field:'TOEOrdItemOut',title:'院外医嘱ID',editor:'text'},
			{field:'TLisResultStr',title:'检验结果串',editor:'text'},
		]],
		data: {"total":0,"rows":[]},
		fit:true,
		rownumbers:true,
...
function setData(excelArr,i,OldData){
	...
	jsonObj.TName=Name;								//TName1
	var Date ="";
	if(obj.日期) Date = StringIsNull(obj.日期);
	jsonObj.TDate=Date;				//RegNo2
	
	var PAADMIn ="";
	if(obj.院内就诊ID) PAADMIn = StringIsNull(obj.院内就诊ID);
	jsonObj.TPAADMIn=PAADMIn;

	var OEOrdItemIn="";
	if(obj.院内医嘱ID) OEOrdItemIn = StringIsNull(obj.院内医嘱ID);
	jsonObj.TOEOrdItemIn=OEOrdItemIn; 				//Name3
	
	var PAADMOut="";
	if(obj.院外就诊ID) PAADMOut = StringIsNull(obj.院外就诊ID);
	jsonObj.TPAADMOut=PAADMOut; 
	
	
	var ArcItm="";
	if(obj.医嘱项ID) ArcItm = StringIsNull(obj.医嘱项ID);
	jsonObj.TArcItm=ArcItm; 				
    
    
    var OEOrdItemOut="";
	if(obj.院外医嘱ID) OEOrdItemOut = StringIsNull(obj.院外医嘱ID);
	jsonObj.TOEOrdItemOut=OEOrdItemOut;  
	
	
	 var LisResultStr="";
	if(obj.检验结果串){
		//alert(2)
		 LisResultStr = StringIsNull(obj.检验结果串);
	}
	jsonObj.TLisResultStr=LisResultStr;  
...
function valid_obj(job,obj,index){
	...
	IInString = Name; 								//TName1
	var Date ="";
	if(obj.TDate) Date = StringIsNull(obj.TDate);
	IInString = IInString + "^" + Date; 				//RegNo2
	
	var PAADMIn ="";
	if(obj.TPAADMIn) PAADMIn = StringIsNull(obj.TPAADMIn);
	IInString = IInString + "^" + PAADMIn; 

	var OEOrdItemIn="";
	if(obj.TOEOrdItemIn) OEOrdItemIn = StringIsNull(obj.TOEOrdItemIn);
	IInString = IInString + "^" + OEOrdItemIn; 			
	//if(PatName==""){
		//obj.TTipMsg="姓名为空";
		//return "";
	//}
	...
	var PAADMOut="";
	if(obj.TPAADMOut) PAADMOut = StringIsNull(obj.TPAADMOut);
	IInString = IInString + "^" + PAADMOut; 		
	
	var ArcItm="";
	if(obj.TArcItm) ArcItm = StringIsNull(obj.TArcItm);
	IInString = IInString + "^" + ArcItm; 	
	
	
	var OEOrdItemOut="";
	if(obj.TOEOrdItemOut) OEOrdItemOut = StringIsNull(obj.TOEOrdItemOut);
	IInString = IInString + "^" + OEOrdItemOut; 
	
	var LisResultStr="";
	if(obj.TLisResultStr) LisResultStr = StringIsNull(obj.TLisResultStr);
	IInString = IInString + "^" + LisResultStr; 
	...
	var ReturnValue = tkMakeServerCall("web.DHCPE.SecretPE", "GetSecretInfoNew", IInString, "Check", job);
	if (ReturnValue != 0) {
...
function import_rowData(job, rowData,index,failNum,successNum,needNum){
	var importRet=tkMakeServerCall("web.DHCPE.SecretPE","MainNew",job);
	return	
