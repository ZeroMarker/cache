...
function BUpdate_Click() //增加
{
	...
	{
		alertShow(t["-3001"]);	//预计结束时间不能在开始时间之前 !
		return;	
	}
    ...
    {
	    alertShow("操作成功!")
	    window.location.href= 'websys.default.csp?WEBSYS.TCOMPONENT=DHCEQRent&RowID='+result+"&Type=Apply";
	}
    else
    {
...
function BDelete_Click() 
{
	...
	{
		alertShow("删除成功!")
	    window.location.href= 'websys.default.csp?WEBSYS.TCOMPONENT=DHCEQRent&Type=Apply';
	}	
    else
    {
...
function BSubmit_Click() 
{
    ...
    {
	    alertShow("操作成功!")
	    window.location.href= 'websys.default.csp?WEBSYS.TCOMPONENT=DHCEQRent&RowID='+rowid+"&Type=Apply";
		//刷新父级窗口
...
function BAudit_Clicked()
{
    ...
    {
	    alertShow("操作成功!")
	    window.location.href= 'websys.default.csp?WEBSYS.TCOMPONENT=DHCEQRent&RowID='+rowid+"&Type=Find&Action="+Action;
	    //刷新父级窗口
	    parent.opener.BFind_Click()
	}
 	else
...
function BCancelSubmit_Click() 
{
    ...
    {
	    alertShow("操作成功!")
	    window.location.href= 'websys.default.csp?WEBSYS.TCOMPONENT=DHCEQRent&RowID='+result+"&Type=Find";
	}
    else
    {
	    alertShow(t[result]+"   "+t["01"]);
...
function FillData()
{
	...
	if (Mode==1)  //按时间收费
	{
		...
		if (UsedHour<=0)
		{
			UsedHour="0"
			var Action=GetElementValue("Action")
			if (Action=="ZL_Return")	UsedHour="1"		//不够1小时按1小时计费
		}
...
function BEquipRentPrint_Clicked()
{
	var RowID=GetElementValue("RowID");
	if (RowID=="") return;
	BEquipRentPrint(RowID);		//DHCEQEquipRentPrint.js
}
