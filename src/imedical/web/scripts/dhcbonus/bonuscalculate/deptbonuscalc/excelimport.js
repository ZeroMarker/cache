...
var importExcel = function() {
	...
	fileTypeDs.on('beforeload', function(ds, o){
		ds.proxy=new Ext.data.HttpProxy({
			url:'../csp/dhc.bonus.bonustargetcollectexe.csp?action=fileType&filetype=2&userCode='+session['LOGON.USERCODE'],method:'POST'})
	});
			...
			function callback(id) {
				if (id == "yes") {
					...
					if (excelName == "") {
						...
						//alert(configFile);
						var uploadUrl = "http://127.0.0.1:8080/dhchxbonus/bonusIssuedExcel";
						if(configFile=="GHZB"){
							uploadUrl = "http://127.0.0.1:8080/dhchxbonus/regexcel";
						}
						if(configFile=="JKSJ"){
							uploadUrl = "http://127.0.0.1:8080/dhchxbonus/interfaceexcel";
						}
						var upUrl = uploadUrl+"?bYear="+bYear+"&bPeriod="+bPeriod+"&configFile="+configFile;
						
						formPanel.getForm().submit({
							url : upUrl,
							//url:'http://127.0.0.1:8080/dhchxbonus/abc',
							method : 'POST',
							waitMsg : '数据导入中, 请稍等...',
