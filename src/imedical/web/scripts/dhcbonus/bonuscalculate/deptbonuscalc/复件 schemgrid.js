...
schemeTypeSt.on('beforeload', function(ds, o) {
	ds.proxy = new Ext.data.HttpProxy({
				url : 'dhc.bonus.bonusschemtypeexe.csp?action=list&start=0&limit=25',
				method : 'GET'
			});
});
...
// ==============2012-3-30===limingzhong====奖金送审===============================================
var DownButton = new Ext.Toolbar.Button({
	text : '奖金送审',
	tooltip : '发送给相关领导进行奖金二次分配！',
	iconCls : 'add',
	handler : function() {
		...
		if (len < 1) {
			...
			if (auditFlag == "已审核") {
				...
				if (rowObj[0].get("IsPay") == "1") {
					...
					Ext.MessageBox.confirm('提示', '数据已经送审，是否要重新送审?', handler1);
