...
var upButton = new Ext.Toolbar.Button({
			...
			handler : function() {
				...
				//alert(rowid);
				Ext.Ajax.request({
							url : '../csp/dhc.bonus.bonusissexe.csp?action=bonusyz&rowid='
									+ rowid + '&total=' + total,
							...
							success : function(result, request) {
								...
								//if (jsonData.success == 'true') {
									Ext.MessageBox.confirm('提示',
											'['+jsonData.info+'],确定要上报数据吗?', callback);
								/*} else {
									Ext.Msg.show({
												title : '上报失败',
												msg : jsonData.info,
...
var scheme1Ds = new Ext.data.Store({
			...
			reader : new Ext.data.JsonReader({
						...
						totalProperty : 'results'
							...
							schemeValue[11].dataIndex,'schemeName','PayBonus','Payye','PayBalance']),
			remoteSort : true
		});
...
scheme1Main.on('rowclick', function(grid, rowIndex, e) {
			...
			rowid = selectedRow.data['rowid']; // 主键ID
			MainID= selectedRow.data['rowid']; // 主键ID
			...
			Ext.Ajax.request({
						...
						success : function(result, request) {
							...
							if (jsonData.success == 'true') {
								...
								msginfoButton.setValue(jsonData.info);
...
var inDeptsCm = new Ext.grid.ColumnModel([new Ext.grid.RowNumberer(), {
			...
			width : 80,
			align : 'right',
			css : 'width:100;background:#d0def0;color:#000000',
			sortable : true,
				...
				Ext.Ajax.request({
							...
							success : function(result, request) {
								var jsonData = Ext.util.JSON
										.decode(result.responseText);
								Ext.MessageBox.confirm('提示',
											'['+jsonData.info+'],确定要验证吗?',callback1);
...
var addButton = new Ext.Toolbar.Button({
	...
	if ((upFlag == "已上报")||(upFlag == "已接收")) {
		...
		var itemName = rowObj[0].get("itemName");
		...
		var fyearField = new Ext.form.TextField({
					id : 'fyearField',
					fieldLabel : '发放年度',
					allowBlank : false,
					width : 180,
		...
		EmpDs.on('beforeload', function(ds, o) {
					ds.proxy = new Ext.data.HttpProxy({
								url : '../csp/dhc.bonus.bonusissexe.csp?action=EmpInfo&str='
		...
		unitDs.on('beforeload', function(ds, o) {
					ds.proxy = new Ext.data.HttpProxy({
								url : '../csp/dhc.bonus.bonusissexe.csp?action=unit&str='
		...
		formPanel = new Ext.form.FormPanel({
					baseCls : 'x-plain',
					labelWidth : 100,
					items : [fyearField, monthField, Item1Field,EmpField, 
							aField]
				});
		// 面板加载 unitField, cField, nField,
		formPanel.on('afterlayout', function(panel, layout) {
					this.getForm().loadRecord(rowObj[0]);
					fyearField.setValue(year);
		...
		addHandler = function() {
			...
			var code = EmpField.getValue();
			var name = EmpField.getRawValue();
...
var msginfoButton = new Ext.form.TextField({
					id : 'msginfoButton',
					fieldLabel : '下发金额-分配金额：',
					allowBlank : false,
					width : 180,
					listWidth : 180,
					anchor : '90%',
...
var scheme02Main = new Ext.grid.EditorGridPanel({
			...
			loadMask : true,
			tbar : [addButton, '-', delButton, '-', yzButton,'-','下发金额-分配金额：',msginfoButton],
	...
	if ((upFlag == "已上报")||(upFlag == "已接收")) {
		...
		Ext.Ajax.request({
					url : '../csp/dhc.bonus.bonusissexe.csp?action=update&rowid='
							...
							+ userCode
							+ '&tzmoeny='
							+ tzmoeny + '&downmoeny=' + downmoeny + '&Remark=' + Remark,
					failure : function(result, request) {
						Ext.Msg.show({
									title : '错误',
									msg : '请检查网络连接!',
