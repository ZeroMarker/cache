...
var itemTypeDs = new Ext.data.Store({
	...
	itemTypeDs.on('beforeload', function(ds, o){
		ds.proxy=new Ext.data.HttpProxy({url:'dhc.bonus.datalevelsetsexe.csp?action=listitemtype&searchValue='+Ext.getCmp('itemType').getRawValue(),method:'GET'});
	});
/*
	itemType.on("select",function(cmb,rec,id ){
		//items.setRawValue("");
		//items.setValue("");
		itemsDs.proxy=new Ext.data.HttpProxy({url:'dhc.bonus.datalevelsetsexe.csp?action=listloc&recname='+Ext.getCmp('items').getRawValue()+'&id='+itemType.getValue()+'&repdr='+repdr,method:'GET'});
		//itemsDs.proxy = new Ext.data.HttpProxy({url:'dhc.bonus.datalevelsetsexe.csp?action=items&id='+cmb.getValue(),method:'GET'});
