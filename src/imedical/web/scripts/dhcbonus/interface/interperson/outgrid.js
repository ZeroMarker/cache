var outUrl = 'dhc.pa.interpersonexe.csp';
var outProxy;
		...
		if(len < 1){
			...
			Ext.MessageBox.confirm('提示','确定要删除选定的行?',
				function(btn) {
					if(btn == 'yes'){
						Ext.Ajax.request({
							url:'dhc.pa.interpersonexe.csp?action=del&rowid='+rowObj[0].get("rowid"),
							waitMsg:'删除中...',
...
var outFilterItem = new Ext.Toolbar.MenuButton({
	...
	menu: {items: [
		...
		new Ext.menu.CheckItem({ text: '名称',value: 'name',checked: true,group: 'outFilter',checkHandler: onCheck }),
	...
	listeners: {
		...
		onTrigger2Click: function() {
			if(this.getValue()) {
				...
				url: outUrl+'?action=outlist&inDr='+InGrid.getSelections()[0].get("userDr")+'&searchField='+outSearchField+'&searchValue='+this.getValue()+'&sort=rowid&dir=asc'});
				outDs.load({params:{start:0, limit:outPagingToolbar.pageSize}});
			}
		}
	});
