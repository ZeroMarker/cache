...
var CalcTypeGroupUrl = '../csp/dhc.bonus.CalculateTypeGroupexe.csp';
function trim(str){
...
var CalcTypeGroupProxy= new Ext.data.HttpProxy({url:CalcTypeGroupUrl + '?action=list'});
var CalcTypeGroupDs = new Ext.data.Store({
	proxy: CalcTypeGroupProxy,
...
//设置默认排序字段和排序方向
CalcTypeGroupDs.setDefaultSort('rowid', 'name');

//数据库数据模型
var CalcTypeGroupCm = new Ext.grid.ColumnModel([
...
//初始化默认排序功能
CalcTypeGroupCm.defaultSortable = true;
...
//添加按钮
var addButtonG = new Ext.Toolbar.Button({
	text: '添加',
    tooltip:'添加',        
    iconCls:'add',
	handler:function(){
		...
		//初始化添加按钮
		addButtonG = new Ext.Toolbar.Button({
			text:'添 加'
		});
		...
		addHandler = function(){
			...
			//encodeURI
			Ext.Ajax.request({
				url: ('../csp/dhc.bonus.CalculateTypeGroupexe.csp?action=add&code='+code+'&name='+name),
				waitMsg:'保存中...',
				...
				success: function(result, request){
					...
					if (jsonData.success=='true'){
						...
						CalcTypeGroupDs.load({params:{start:0, limit:CalcTypeGroupPagingToolbar.pageSize}});
						//addwin.close();
					}
					else
							{
		...
		//添加保存按钮的监听事件
		addButtonG.addListener('click',addHandler,false);
...
//修改按钮
var editButtonG = new Ext.Toolbar.Button({
	text: '修改',
    tooltip:'修改',        
    iconCls: 'option',
	handler:function(){
		//定义并初始化行对象
		var rowObj=CalculateTypeGroupPanel.getSelections();
		//定义并初始化行对象长度变量
		...
		editHandler = function(){
			...
			Ext.Ajax.request({
				url: '../csp/dhc.bonus.CalculateTypeGroupexe.csp?action=edit&rowid='+rowid+'&code='+code+'&name='+name,
				waitMsg:'保存中...',
				failure: function(result, request){
		...
		//添加保存修改按钮的监听事件
		editButtonG.addListener('click',editHandler,false);
...
//删除按钮
var delButtonG = new Ext.Toolbar.Button({
	text: '删除',
    tooltip:'删除',        
    iconCls:'remove',
	handler:function(){
		...
		function handler(id){
			if(id=="yes"){
					...
					Ext.Ajax.request({
						url:'../csp/dhc.bonus.CalculateTypeGroupexe.csp?action=del&rowid='+rowid,
						waitMsg:'删除中...',
						failure: function(result, request){
...
//分页工具栏
var CalcTypeGroupPagingToolbar = new Ext.PagingToolbar({
    store: CalcTypeGroupDs,
	pageSize:25,
    displayInfo: true,
...
//表格
var CalculateTypeGroupPanel = new Ext.grid.EditorGridPanel({
	title: '核算类别组',
	region:'west',
	width:300,
	height : 450,
	...
	loadMask: true,
	tbar:[addButtonG,'-',editButtonG,'-',delButtonG],
	bbar:CalcTypeGroupPagingToolbar
});
CalcTypeGroupDs.load({params:{start:0, limit:CalcTypeGroupPagingToolbar.pageSize}});
//----------------------------
var CalcTypeGroupCurrRowID='';

CalculateTypeGroupPanel.on('rowclick',function(grid,rowIndex,e){
	var selectedRow = CalcTypeGroupDs.data.items[rowIndex];

	//单击接口核算部门后刷新接口核算部门单元
	CalcTypeGroupCurrRowID=selectedRow.data['rowid'];
	CalcTypeGroupCurrName=selectedRow.data['name'];
	modelType=selectedRow.data['name'];
	
	//alert(CalcTypeGroupCurrRowID)
