...
var BonusItemTypeTabUrl = '../csp/dhc.bonus.bonusitemtypeexe.csp';
function trim(str){
...
var BonusItemTypeTabProxy= new Ext.data.HttpProxy({url:BonusItemTypeTabUrl + '?action=list'});
var BonusItemTypeTabDs = new Ext.data.Store({
...
//设置默认排序字段和排序方向
BonusItemTypeTabDs.setDefaultSort('rowid', 'name');

//数据库数据模型
var BonusItemTypeTabCm = new Ext.grid.ColumnModel([
...
//搜索过滤器
var BonusItemTypeFilterItem = new Ext.Toolbar.MenuButton({
	text: '过滤器',
	tooltip: '关键字所属类别',
	menu: {items: [
		new Ext.menu.CheckItem({ 
			...
			value: 'code',
			checked: false ,
			group: 'BonusItemTypeFilter',
			checkHandler: onBonusItemTypeItemCheck 
		}),
		new Ext.menu.CheckItem({ 
			text: '名称',
			value: 'name',
...
//定义搜索响应函数
function onBonusItemTypeItemCheck (item, checked){
	if(checked) {
		BonusItemTypeSearchField = item.value;
		BonusItemTypeFilterItem.setText(item.text + ':');
	}
};

//查找按钮
var BonusItemTypeSearchBox = new Ext.form.TwinTriggerField({
	width: 180,
	...
	onTrigger1Click: function() {
		if(this.getValue()) {
			this.setValue('');    
			BonusItemTypeTabDs.proxy = new Ext.data.HttpProxy({url:  BonusItemTypeTabUrl + '?action=list'});
			BonusItemTypeTabDs.load({params:{start:0, limit:BonusItemTypeTabPagingToolbar.pageSize}});
		}
	},
	onTrigger2Click: function() {
		if(this.getValue()) {
				BonusItemTypeTabDs.proxy = new Ext.data.HttpProxy({
				url: BonusItemTypeTabUrl + '?action=list&searchField=' + BonusItemTypeSearchField + '&searchValue=' + this.getValue()});
	        	BonusItemTypeTabDs.load({params:{start:0, limit:BonusItemTypeTabPagingToolbar.pageSize}});
	    	}
	}
});
...
var addButton = new Ext.Toolbar.Button({
	...
	handler:function(){
		...
		addHandler = function(){
			...
			//encodeURI
			Ext.Ajax.request({
				url: ('../csp/dhc.bonus.bonusitemtypeexe.csp?action=add&code='+code+'&name='+name),
				waitMsg:'保存中...',
...
var editButton = new Ext.Toolbar.Button({
	...
	handler:function(){
		//定义并初始化行对象
		var rowObj=BonusItemTypeTab.getSelections();
		//定义并初始化行对象长度变量
		...
		editHandler = function(){
			...
			Ext.Ajax.request({
				url: '../csp/dhc.bonus.bonusitemtypeexe.csp?action=edit&rowid='+rowid+'&code='+code+'&name='+name,
				waitMsg:'保存中...',
				failure: function(result, request){
	...
	handler:function(){
		...
		function handler(id){
			if(id=="yes"){
					...
					Ext.Ajax.request({
						url:'../csp/dhc.bonus.bonusitemtypeexe.csp?action=del&rowid='+rowid,
						waitMsg:'删除中...',
						failure: function(result, request){
						...
						success: function(result, request){
							...
							if (jsonData.success=='true'){
								...
								Ext.Msg.show({title:'错误',msg:'所选中的项目已被方案引用!',buttons: Ext.Msg.OK,icon:Ext.MessageBox.ERROR});
							}
						},
						scope: this
					});
...
//表格
var BonusItemTypeTab = new Ext.grid.EditorGridPanel({
	title: '奖金项目类别',
	store: BonusItemTypeTabDs,
