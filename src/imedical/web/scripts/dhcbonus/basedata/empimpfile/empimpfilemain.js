...
var BonusEmployeeTabUrl = '../csp/dhc.bonus.empimpfileexe.csp';
function trim(str) {
...
var BonusEmployeeTabDs = new Ext.data.Store({
			...
			reader : new Ext.data.JsonReader({
						...
						totalProperty : 'results'
							...
							'impFileID', 'impFileName', 'FileTypeName']),
			// turn on remote sorting
			remoteSort : true
		});

// 设置默认排序字段和排序方向  "rowid^EmployeeID^EmployeeName^impFileID^impFileName^FileTypeName"
...
// 初始化搜索字段
var BonusEmployeeSearchField = 'EmployeeName';
...
var addButton = new Ext.Toolbar.Button({
	...
	handler : function() {
		...
		unitDs.on('beforeload', function(ds, o) {
			ds.proxy = new Ext.data.HttpProxy({
						url : '../csp/dhc.bonus.empimpfileexe.csp?action=impfile'
								+'&fileType='+unitTypeField.getValue(),
						method : 'POST'
					})
		});
		...
		var dataTypeStore = new Ext.data.SimpleStore({
			fields : ['rowid', 'name'],
			data : [['1', '指标横向扩展'], ['2', '指标纵向扩展'], ['3', '科室横向扩展']]
		});
		...
		addHandler = function() {
			...
			Ext.Ajax.request({
				url : '../csp/dhc.bonus.empimpfileexe.csp?action=add&EmployeeID='
	...
	handler : function() {
		...
		function handler(id) {
			if (id == "yes") {
				...
				Ext.Ajax.request({
					url : '../csp/dhc.bonus.empimpfileexe.csp?action=del&rowid='
							+ rowid,
					waitMsg : '删除中...',
					failure : function(result, request) {
						Ext.Msg.show({
