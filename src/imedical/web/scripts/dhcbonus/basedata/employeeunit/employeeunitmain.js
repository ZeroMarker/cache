...
var BonusEmployeeTabUrl = '../csp/dhc.bonus.employeeunitexe.csp';
function trim(str) {
...
var BonusEmployeeTabDs = new Ext.data.Store({
			...
			reader : new Ext.data.JsonReader({
						...
						totalProperty : 'results'
							...
							'BonusUnitID', 'unitName', 'UnitTypeName']),
...
// 初始化搜索字段
var BonusEmployeeSearchField = 'BonusUnitName';
...
var addButton = new Ext.Toolbar.Button({
	...
	handler : function() {
		...
		unitDs.on('beforeload', function(ds, o) {
			ds.proxy = new Ext.data.HttpProxy({
						url : '../csp/dhc.bonus.bonusemployeeexe.csp?action=unit&str='
								...
								+'&BonusUnitTypeID='+unitTypeField.getValue(),
						method : 'POST'
					})
		});
		...
		bonusUnitTypeDs.on('beforeload', function(ds, o) {
					ds.proxy = new Ext.data.HttpProxy({
								url : 'dhc.bonus.bonusunittypeexe.csp?action=list',
								method : 'POST'
							})
				});
		...
		addHandler = function() {
			...
			Ext.Ajax.request({
				url : '../csp/dhc.bonus.employeeunitexe.csp?action=add&EmployeeID='
						+ EmployeeID + '&UnitID=' + UnitID,
				waitMsg : '保存中...',
	...
	handler : function() {
		...
		function handler(id) {
			if (id == "yes") {
				...
				Ext.Ajax.request({
					url : '../csp/dhc.bonus.employeeunitexe.csp?action=del&rowid='
							+ rowid,
					waitMsg : '删除中...',
					failure : function(result, request) {
						Ext.Msg.show({
