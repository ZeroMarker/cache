...
var calculator = new Ext.Toolbar.Button(
		{
			...
			if (yearmonth!=undefined){
				...
				if (myyearmonth!=""){
				myyearmonth=myyearmonth.getFirstDateOfMonth().format("Y-m-d");
				}else{				Ext.Msg.show({title:'注意',msg:'请选择年月!',buttons: Ext.Msg.OK,icon:Ext.MessageBox.WARING});
				return null;}
				}
...
var itemGrid = new dhc.herp.Grid({
        ...
        fields: [{
			...
			return '<font color="blue" onclick="income(\''+record.data.BonusUnitID+'\',\''+record.data.yearmonth+'\')">'+value+'</blue>';
		     ...
		     dataIndex: 'myout',
		      renderer : function(value,cellmeta, record,rowIndex, columnIndex, store) { 					
			return '<font color="blue" onclick="expend(\''+record.data.BonusUnitID+'\',\''+record.data.yearmonth+'\')">'+value+'</blue>';
