...
var importExcel = function() {
	...
	fileTypeDs.on('beforeload', function(ds, o){
		ds.proxy=new Ext.data.HttpProxy({
			url:'../csp/dhc.bonus.bonustargetcollectexe.csp?action=fileType&modeType=1&userCode='+session['LOGON.USERCODE'],method:'POST'})
	});
     ...
     Ext.Ajax.request({
			...
			var callback=function callback(id) {
				if (id == "yes") {
					...
					//alert(Ext.getCmp('fileType').getRawValue().split("-")[0])
					...
					if (excelName == "") {
						...
						var uploadUrl = dhcbaUrl+"/commonImportExcel";

						var sfileRowID=fileTypeID.split(",")[0]
						var sfileTypeID=fileTypeID.split(",")[1]
						var upUrl = uploadUrl+"?bYear="+bYear+"&bPeriod="+bPeriod+"&configFile="+configFile+"&fileRowID="+sfileRowID+"&fileTypeID="+sfileTypeID;
...
// Ext.MessageBox.confirm('提示', '确定要导入excel数据吗?', callback);
	...
	var handler1 = function(bt) {
		if (bt == "yes") {
			var callback1= function callback(id) {
				if (id == "yes") {
					// 判断是否已选择好所要上传的excel文件
					var excelName = Ext.getCmp('excelUpload').getRawValue();// 上传文件名称的路径
					if (excelName == "") {
