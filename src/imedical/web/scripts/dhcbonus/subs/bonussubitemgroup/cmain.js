...
var cValue = new Array(
	{header:'',dataIndex:'BonusSubItemGroupMapID'},										//0
	{header:'',dataIndex:'BonusSubItemGroupID'}, 		//1
	{header:'项目ID',dataIndex:'BonusSubItemID'},         		//2
	{header:'项目编码',dataIndex:'SubItemCode'},			//3
	{header:'项目名称',dataIndex:'SubItemName'},        	//4
	{header:'',dataIndex:'SubItemType'},			//5
	{header:'',dataIndex:'SubItemTypeName'}        	//6
);

var cUrl = 'dhc.bonus.bonussubitemgroupexe.csp';
...
cDs.setDefaultSort('BonusSubItemGroupMapID', 'Desc');
...
var cCm = new Ext.grid.ColumnModel([
	...
	{
		header: cValue[2].header,
		dataIndex:cValue[2].dataIndex,
		width: 50,
		align: 'left',
		sortable: true
	}
	...
	handler: function(){
		...
		if(len < 1){
			...
			Ext.MessageBox.confirm(
				...
				function(btn){
					if(btn == 'yes'){
						myId = rowObj[0].get("BonusSubItemGroupMapID");
						Ext.Ajax.request({
