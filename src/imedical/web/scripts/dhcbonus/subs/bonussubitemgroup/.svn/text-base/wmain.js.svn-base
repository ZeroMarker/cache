var wValue = new Array({
			header : '',
			dataIndex : 'BonusSubItemGroupID'
		}, // 0
		{
			header : '项目组编码',
			dataIndex : 'SubItemGroupCode'
		}, // 1
		{
			header : '项目组名称',
			dataIndex : 'SubItemGroupName'
		}, // 2
		{
			header : '',
			dataIndex : 'IsValid'
...
var wUrl = 'dhc.bonus.bonussubitemgroupexe.csp';
...
wDs.setDefaultSort('BonusSubItemGroupID', 'Desc');
...
var wDelButton = new Ext.Toolbar.Button({
	...
	handler : function() {
		...
		if (len < 1) {
			...
			tmpRowid = rowObj[0].get("BonusSubItemGroupID");
...
wMain.on('rowclick', function(grid, rowIndex, e) {
			...
			if (num > 0) {
				...
				var tmpWRowid = selectedRow.data['BonusSubItemGroupID'];
				// alert(tmpWRowid);
				cSelected = tmpWRowid;
