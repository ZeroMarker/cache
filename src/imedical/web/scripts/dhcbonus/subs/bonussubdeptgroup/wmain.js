// 1：开单科室、2：执行科室、3：病人科室
var wType = new Array('', '开单科室', '执行科室', '病人科室');

// BonusSubDeptGroupID^code^name^type^isValid
var wValue = new Array({
			header : '',
			dataIndex : 'BonusSubDeptGroupID'
		}, // 0
		{
			header : '科室组编码',
...
var wUrl = 'dhc.bonus.bonussubdeptgroupexe.csp';
...
wDs.setDefaultSort('BonusSubDeptGroupID', 'Desc');
...
var wCm = new Ext.grid.ColumnModel([new Ext.grid.RowNumberer(), {
			...
			header : wValue[3].header,
			dataIndex : wValue[3].dataIndex,
...
wMain.on('rowclick', function(grid, rowIndex, e) {
			...
			if (num > 0) {
				...
				var tmpWRowid = selectedRow.data['BonusSubDeptGroupID'];
				cSelected = tmpWRowid;
