cEditFun = function() {
	...
	if(len < 1)
	{
		...
		tmpRowid = rowObj[0].get("BonusSubDeptGroupMapID");
	...
	bonusUnitTypeCombo.on('select',function(){
		unitDs.proxy=new Ext.data.HttpProxy({url:'dhc.bonus.bonussubdeptgroupexe.csp?action=listunit&type='+bonusUnitTypeCombo.getValue(), method:'GET'});
	...
	unitDs.on('beforeload', function(ds, o){
		ds.proxy=new Ext.data.HttpProxy({url:'dhc.bonus.bonussubdeptgroupexe.csp?action=listunit&type='+tmpType,method:'GET'});
	});
