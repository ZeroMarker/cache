...
var upButton = new Ext.Toolbar.Button({
			...
			handler : function() {
				...
				Ext.MessageBox.confirm('提示','确定要接收上报的数据吗?', upCheck);
...
var CacelCheckButton = new Ext.Toolbar.Button({
			...
			handler : function() {
				...
				var total = rowObj[0].get("downmoeny");
				var rowid = rowObj[0].get("rowid");
				Ext.MessageBox.confirm('提示','确定要拒绝接收上报的数据吗?', upCacelCheck);
...
var scheme1Main = new Ext.grid.GridPanel({
			...
			tbar : ['年度', yearField, '-', '类别', periodTypeField, '-', '期间',
					periodField, '-', '科室名称', unitFileld,'数据状态',dataStatusField, '-', queryButton, '-',
