AddFun = function(dataStore,grid,pagingTool) {
	...
	if(leaf == "true")
	{
		Ext.Msg.show({title:'错误',msg:'所选节点为数据,不能进行此操作!',buttons: Ext.Msg.OK,icon:Ext.MessageBox.ERROR});
		return;
	}
			...
			handler: function() {
				...
				if (formPanel.form.isValid()) {
					...
					if(code==""){
						...
						Ext.Ajax.request({
							url: encodeURI(deptLevelSetsUrl+'?action=add&code='+code+'&name='+name+'&remark='+remark+'&active='+active+'&id='+repdr+'&order='+orderField.getValue()),
							waitMsg:'保存中...',
