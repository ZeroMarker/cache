...
var repdr;
var leaf;
var deptLevelSetsUrl = 'dhc.ca.datalevelsetsexe.csp';
...
var addButton = new Ext.Toolbar.Button({
	...
	handler: function(){
		Ext.Ajax.request({
			url: deptLevelSetsUrl + '?action=check&id='+repdr,
			waitMsg:'增加中...',
...
var delLocButton  = new Ext.Toolbar.Button({
	...
	handler: function(){
		...
		if(end != "")
		{
			Ext.Msg.show({title:'注意',msg:'您选择的数据不是数据!',buttons: Ext.Msg.OK,icon:Ext.MessageBox.WARNING});
			return;
		}
		else
		{	
			...
			function(btn) {
				...
				myId = rowObj[0].get("id");
					Ext.Ajax.request({
					url:'dhc.ca.datalevelsetsexe.csp?action=delloc&id='+myId,
					waitMsg:'删除中...',
					failure: function(result, request) {
						Ext.Msg.show({title:'错误',msg:'请检查网络连接!',buttons: Ext.Msg.OK,icon:Ext.MessageBox.ERROR});
					},
...
var deptLevelSetsSearchBox = new Ext.form.TwinTriggerField({//查找按钮
	...
	onTrigger1Click: function() {
		if(this.getValue()) {
			this.setValue('');
			deptLevelSetsDs.proxy = new Ext.data.HttpProxy({url: deptLevelSetsUrl + '?action=listsub&repdr=' + repdr});
	...
	onTrigger2Click: function() {
		if(this.getValue()) {
			...
			url: deptLevelSetsUrl + '?action=listsub&repdr=' + repdr+'&searchfield=' + deptLevelSetsSearchField + '&searchvalue=' + this.getValue()});
			deptLevelSetsDs.load({params:{start:0, limit:deptLevelSetsPagingToolbar.pageSize,repdr:repdr}});
		}
	}
});
