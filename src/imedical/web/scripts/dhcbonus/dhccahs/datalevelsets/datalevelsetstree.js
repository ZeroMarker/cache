...
locTreeLoader.on('beforeload', function(locTreeLoader,node){
	var url="dhc.ca.datalevelsetsexe.csp?action=list";
	...
	handler: function(){
		...
		{	
			...
			function(btn) {
				...
				{	
					Ext.Ajax.request({
					url:'dhc.ca.datalevelsetsexe.csp?action=delloc&id='+repdr,
					waitMsg:'删除中...',
	...
	handler: function(){
		...
		{	
			...
			function(btn) {
				...
				{	//alert(repdr)
					Ext.Ajax.request({
					url:'dhc.ca.datalevelsetsexe.csp?action=copy&id='+repdr,
					waitMsg:'复制中...',
					failure: function(result, request) {
						Ext.Msg.show({title:'错误',msg:'请检查网络连接!',buttons: Ext.Msg.OK,icon:Ext.MessageBox.ERROR});
					},
