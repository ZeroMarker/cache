...
var delAllDataTypesButton = {
    text: '删除全部项目',
    tooltip: '删除参数数据表',
    iconCls: 'remove',
    //disabled: true,
    handler: function(){
        delAllFun(paramDatasDs, paramDatasMain, paramDatasPagingToolbar);
    }
};
...
var importButton = {
    ...
    handler: function(){
        ...
        paramDataLoad(monthDr, itemDr, userCode,paramDatasDs);
    }
};
    ...
    handler: function(){
        ...
        Ext.MessageBox.confirm('提示', '确定要导入数据么?', function(btn){
            if (btn == 'yes') {
                ...
                loadMask.show();
                Ext.Ajax.request({
                    url: 'dhc.ca.paramdatasexe.csp?action=importDeptSumIncome&userCode=' + userCode + '&intervalDr=' + monthDr + '&itemDr=' + itemDr,
                    waitMsg: '导入中...',
                    failure: function(result, request){loadMask.hide();
                        Ext.Msg.show({
                            title: '错误',
...
var opMenu = new Ext.menu.Menu({
    id: 'opMenu',
    items: [addDataTypesButton, editDataTypesButton, delDataTypesButton, delAllDataTypesButton]
});
...
var importMenu = new Ext.menu.Menu({
    ...
    items: [hisImportButton,DeptSumIncomeImportButton, importButton]
});
...
var _Excel = new Herp.Excel(
    {
	 url:ServletURL,
     sql:'SELECT ParamDatas_intervalDr->AccountMonths_name as 月份,ParamDatas_itemDr->AllDataItems_name as 参数名称,ParamDatas_servedDeptCode as HIS科室代码,ParamDatas_servedDeptName as HIS科室名称,ParamDatas_servedDeptDr->UnitDepts_code as 核算科室代码,ParamDatas_servedDeptDr->UnitDepts_name as 核算科室名称,ParamDatas_value as 参数值,ParamDatas_inPersonDr->UnitPersons_name as 操作人,ParamDatas_inDate as 导入日期  FROM dhc_ca_cache_data.ParamDatas where ParamDatas_intervalDr=? and ParamDatas_itemDr=? ',
     fileName:"参数"     
    });
...
var exportButton = {
    ...
    tooltip: '导出参数数据',
    iconCls: 'remove',
    handler: function() { _Excel.download(monthDr,itemDr); }
};
...
var paramDatasFilterItem = new Ext.Toolbar.MenuButton({
    ...
    menu: {
        items: [new Ext.menu.CheckItem({
            ...
            group: 'DataTypesFilter',
            checkHandler: onDataTypesItemCheck
        }),/*zjw 20160808 new Ext.menu.CheckItem({
            text: '服务部门代码',
...
var paramDatasMain = new Ext.grid.GridPanel({//表格
    ...
    tbar: ['核算区间:', months, '-', '项目:', items, '-', opTool, '-', copeOtherMonthButton, '-', importTool, '-', relationButton, '-',exportButton],
