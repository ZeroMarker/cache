...
unitTypeSelecter.on(
    ...
    unitRowid = "";
		unitDeptsDs.proxy = new Ext.data.HttpProxy({url: unitDeptsUrl + '?action=list'});
...
{
		unitRowid = unitDr;
		unitDeptsDs.proxy = new Ext.data.HttpProxy({url: unitDeptsUrl + '?action=list&unitDr='+unitDr});
...
var unitDeptsSearchBox = new Ext.form.TwinTriggerField({//查找按钮
		...
		onTrigger1Click: function() {
				if(this.getValue()) {
							this.setValue('');
							unitDeptsDs.proxy = new Ext.data.HttpProxy({url: unitDeptsUrl + '?action=list&unitDr='+unitRowid});
							unitDeptsDs.load({params:{start:0, limit:unitDeptsPagingToolbar.pageSize}});
				}
		},
		onTrigger2Click: function() {
				if(this.getValue()) {
						...
						url: unitDeptsUrl + '?action=list&unitDr='+unitRowid+'&searchField=' + unitDeptsSearchField + '&searchValue=' + this.getValue()});
	        	unitDeptsDs.load({params:{start:0, limit:unitDeptsPagingToolbar.pageSize}});
	    	}
		}
});
...
var unitDeptsMain = new Ext.grid.GridPanel({//表格
		...
		loadMask: true,
		tbar: ['单位类别：',unitTypeSelecter,'-','单位：',unitSelecter],
