inDataCardRuleEditFun = function(dataStore,grid,inDataCardingPanel,pagingTool) {
	Ext.QuickTips.init();
  // pre-define fields in the form
	...
	var CardRuleId=selectedRow[0].get("rowid");
	
	var selectedCarding = inDataCardingPanel.getSelections();
	var selectedCode=selectedCarding[0].get("code");
	var selectedRowid=selectedCarding[0].get("rowid");
    ...
    itemsDs.on('beforeload', function(ds, o) {
	    ...
	    if(PatType=="")
      		{
      			Ext.Msg.show({title:'错误',msg:'请先选出收入类型！',buttons: Ext.Msg.OK,icon:Ext.MessageBox.ERROR});
      			return;
      		};
      	*/
      ...
      handler: function() {
			...
			var itemDr=itemsCommbo.getValue();
			 if(srDeptDr==deDeptDr)
      		{
      			Ext.Msg.show({title:'错误',msg:'目标科室与源科室重复！',buttons: Ext.Msg.OK,icon:Ext.MessageBox.ERROR});
      			return;
      		};
			
        	if (formPanel.form.isValid()) {
						Ext.Ajax.request({
							url: busdingUrl+'?action=editCardRule&itemDr='+itemDr+'&deDeptDr='+deDeptDr+'&srDeptDr='+srDeptDr+'&CardRuleId='+CardRuleId+'&PatTypeCode='+PatTypeDr,
							waitMsg:'保存中...',
