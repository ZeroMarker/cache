outdeptschange=function(parRef,rowId,code,name,patType,remark){
	...
	inDeptDs.on('beforeload', function(ds, o){
		ds.proxy=new Ext.data.HttpProxy({url:encodeURI('dhc.ca.indeptsexe.csp?action=listInDepts&inDeptSetsId='+inDeptSetsId+'&str='+Ext.getCmp('inDepts').getRawValue()),method:'GET'});
	});
	var window = new Ext.Window({
        ...
        buttons: [{
            ...
            handler: function(){
                ...
                Ext.Ajax.request({
                    url: encodeURI(outDeptsUrl + '?action=outdeptschange&inDeptId=' + inDepts.getValue() + '&outDeptId=' + rowId+'&code='+code+'&name='+name+'&patType='+patType+'&remark='+remark),
                    waitMsg: '保存中...',
