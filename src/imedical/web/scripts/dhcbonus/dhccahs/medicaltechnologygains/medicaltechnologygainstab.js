			...
			Ext.Msg.alert("提示","请先选择各项参数");
					...
					Ext.Ajax.request({
						...
						success: function(result,request){
							...
							//»򈢓û§³ɹ¦ʱ£¬·¢̍¼ǋ䋽¾ݵŇ숳
							Ext.Ajax.request({
								url:'dhc.ca.medicaltechnologygainsexe.csp?action=depttype&month='+monthCombo.getValue()+'&deptClass='+deptType+'&cost='+disCostSet,
								waitMsg:'正在计算...',
								timeout: 120000,
								success: function(result,request){	
...
var checkButton = new Ext.Toolbar.Button({
	...
	handler: function(){
		if((monthCombo.getValue()=="")){
			...
			Ext.Ajax.request({
				url:'dhc.ca.medicaltechnologygainsexe.csp?action=getUser&userCode='+userCode+'&month='+monthCombo.getValue()+'&deptClass='+deptType+'&cost='+disCostSet,
				waitMsg:'查询中...',
				success: function(result,request){
					...
					center.add(new Ext.Panel({
	    				...
	    				bodyCfg: {
							...
							cn:[{tag: 'param', name: 'type', value: 'application/x-java-applet;version=1.2.2'},
								...
								{tag: 'param', name: 'REPORT_URL', value: '../servlets/medicalTechnologyGains/medicalTechnologyGains?month='+monthCombo.getValue()+'&monName='+monName+'&deptClass='+deptType+'&userInfo='+aaa+'&cost='+disCostSet+'&outType=print'}
							]
						}
              		}));
		...
		if((monthCombo.getValue()=="")){
	    	...
	    	Ext.Ajax.request({
				...
				success: function(result,request){	
					...
					var monName=monthCombo.getRawValue();
					var monName=encodeURI(encodeURI(monName));
					location.href = 'http://172.26.201.66:1969/dhccareport/servlets/medicalTechnologyGains/medicalTechnologyGains?month='+monthCombo.getValue()+'&monName='+monName+'&deptClass='+deptType+'&cost='+disCostSet+'&userInfo='+aaa+'&outType=xls';
				}
	    	});
		}
	}
