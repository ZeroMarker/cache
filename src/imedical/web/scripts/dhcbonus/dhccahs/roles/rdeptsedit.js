editRdeptsFun = function(dataStore,grid,pagingTool) {
	Ext.QuickTips.init();
...
/////////////////////////////////////////////////	
	var deptSelectWindow = function(){
		...
		typeSelecter.on(
			"select",
			function(cmb,rec,id ){
				unitDs.proxy = new Ext.data.HttpProxy({url:mainUrl+'?action=listUnit&active=Y&unitTypeDr='+cmb.getValue(), method:'GET'});		// zhw 去掉注释 20160821
				unitDs.load({params:{start:0, limit:pagingTool.pageSize}});		// zhw 去掉注释 20160821
		...
		unitSelecter.on(
			"select",
			function(cmb,rec,id){
				deptDs.proxy = new Ext.data.HttpProxy({url:mainUrl+'?action=listDept&active=Y&unitDr='+cmb.getValue(), method:'GET'});		// zhw 去掉注释 20160821
