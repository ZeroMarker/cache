...
var addDataTypesButton ={
		...
		tooltip: '添加新的凭证数据表',        
		iconCls: 'add',
		handler: function(){addFun(vouchDatasDs,vouchDatasMain,vouchDatasPagingToolbar);}
};
...
var importButtonP ={
 ...
 tooltip:'导入药品成本数据',
 iconCls:'remove',
 handler:function(){imporFun(vouchDatasDs,vouchDatasPagingToolbar,monthDr,"P");}
};
var importButtonM ={
 text:'物资成本采集',
 tooltip:'导入物资出库数据',
...
var opMenu = new Ext.menu.Menu({
    id: 'opMenu',
    items: [importButtonP, importButtonM,importButtonZ,importButton]
});
...
var vouchDatasFilterItem = new Ext.Toolbar.MenuButton({
		...
		tooltip: '关键字所属类别',
		menu: {items: [
				new Ext.menu.CheckItem({ text: '(核算)部门名称',value: 'tDeptName',checked: false,group: 'DataTypesFilter',checkHandler: onDataTypesItemCheck }),
				new Ext.menu.CheckItem({ text: '(核算)数据项名称',value: 'itemName',checked: true,group: 'DataTypesFilter',checkHandler: onDataTypesItemCheck }),
...
var vouchDatasMain = new Ext.grid.GridPanel({//表格
		...
		loadMask: true,
		tbar: ['核算区间:',months,'-',operation,'-',opTool,'-',refreshButton ],
