function loadVouchData(dataStore)
{
	if(monthDr==""){
					...
					handler: function(){
						if (formPanel.getForm().isValid()) {
							formPanel.getForm().submit({
								url: ServletURL+'/dhccanow/vouchdataup?interval='+monthDr+'&inType=load&user='+userDr,
                                                      
								method : 'POST',
								waitMsg:'正在上传, 请等待...',
								success:function(form, action, o) {
									Ext.Msg.show({title:'注意',msg:'上传成功!',buttons: Ext.Msg.OK,icon:Ext.MessageBox.INFO});
                                  dataStore.load({params:{start:0,limit:50,monthDr:monthDr}})
									win.close();
								},
