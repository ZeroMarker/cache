...
inItemSetsDs.on('beforeload',function(ds, o){
		ds.proxy = new Ext.data.HttpProxy({url:inItemsUrl+'?action=listInItemSets&searchField=shortcut&searchValue='+Ext.getCmp('inItemSetSelecter').getRawValue(), method:'GET'});
	//zjw 20160828 
	}
);
...
var unittypesFilterItem = new Ext.Toolbar.MenuButton({
		...
		menu: {items: [
				...
				new Ext.menu.CheckItem({ text: '核算项目代码',value: 'itemCode',checked: false,group: 'UnittypesFilter',checkHandler: onUnittypesItemCheck }),
				new Ext.menu.CheckItem({ text: '核算项目名称',value: 'itemName',checked: true,group: 'UnittypesFilter',checkHandler: onUnittypesItemCheck })
		]}
});
...
var inItemsGrid = new Ext.grid.GridPanel({
	...
	tbar: ['接口项目套:',inItemSetSelecter,'-',addButton,'-',remainButton,'-',editButton,'-',delButton],
