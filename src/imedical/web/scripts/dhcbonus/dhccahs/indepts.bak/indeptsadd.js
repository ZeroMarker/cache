addInDeptsFun = function(dataStore,grid,pagingTool) {
	if(inDeptSetsId=="")
	{
		Ext.Msg.show({title:'注意',msg:'请选择接口核算部门后再添加!',buttons: Ext.Msg.OK,icon:Ext.MessageBox.INFO});
		return;
	}
	...
	assLocDs.on('beforeload', function(ds, o){
		ds.proxy=new Ext.data.HttpProxy({url:'dhc.ca.indeptsexe.csp?action=listInDept&searchField=shortcut&searchValue='+Ext.getCmp('assLoc').getRawValue()+'&unitDr='+units.getValue()+'&inDeptSetsId='+inDeptSetsId,method:'GET'});
	});
	...
	var window = new Ext.Window({
		...
		},
		{
			...
			handler: function() {
				if (formPanel.form.isValid()) {
					Ext.Ajax.request({
						url: outDeptsUrl+'?action=addAll&order='+orderField.getValue()+'&unit='+units.getValue()+'&inDeptSetsId='+inDeptSetsId,
						waitMsg:'保存中...',
						failure: function(result, request) {
							Ext.Msg.show({title:'错误',msg:'请检查网络连接!',buttons: Ext.Msg.OK,icon:Ext.MessageBox.ERROR});
						},
