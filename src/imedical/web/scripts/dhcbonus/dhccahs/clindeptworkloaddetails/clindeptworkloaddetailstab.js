...
var deptSetComb = new Ext.ux.form.LovCombo({
	...
	store: new Ext.data.Store({
		autoLoad: true,
		proxy: new Ext.data.HttpProxy({url: 'dhc.ca.clindeptworkloaddetailsexe.csp?action=depttype'}),
		reader: new Ext.data.JsonReader({
			root: 'rows',
			totalProperty: 'results',
			id: 'id'
...
var checkButton = new Ext.Toolbar.Button({
	...
	handler: function(){
		if((monthCombo.getValue()=="")||(deptSetComb.getValue()=="")){
			...
			//准备数据
			Ext.Ajax.request({
				url:'dhc.ca.clindeptworkloaddetailsexe.csp?action=readydatas&monthDr='+monthCombo.getValue()+'&DeptTypeDrStr='+deptSetComb.getValue()+'&ItemDrStr='+itemIDStr,
				waitMsg:'正在准备数据...',
							...
							center.add(new Ext.Panel({
								...
								bodyCfg: {
									...
									cn:[{tag: 'param', name: 'type', value: 'application/x-java-applet;version=1.2.2'},
										...
										{tag: 'param', name: 'REPORT_URL', value: '../clindeptworkloaddetails?monthName='+monthCombo.getRawValue()+'&userInfo='+aaa+'&outType=print'}
									]
								}
							}));
				...
				success: function(result,request){
					Ext.Ajax.request({
						...
						success: function(result,request){
							...
							var userInfo=jsonData.info;
							var aaa=encodeURI(encodeURI(userInfo));
							location.href = 'http://172.26.201.66:1969/dhccareport/clindeptworkloaddetails?monthName='+monthCombo.getRawValue()+'&userInfo='+aaa+'&outType=xls';
						}
					});
				}
	    	});
