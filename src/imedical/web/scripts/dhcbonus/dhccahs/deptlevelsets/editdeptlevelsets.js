EditFun = function(dataStore,grid,pagingTool) {
  ...
  // pre-define fields in the form
	...
	var repFlagCBox = new Ext.form.Checkbox({
		id: 'repFlagCBox',
		labelSeparator: '报表输出标志:',
		allowBlank: false,
		checked: (rowObj[0].data['repFlag'])=='Y'?true:false
	});
	// create form panel
		...
		buttons: [{
			...
			handler: function() {
				...
				if (formPanel.form.isValid()) {
					...
					var rep=(repFlagCBox.getValue()==true)?'Y':'N';
					if(code==""){
						...
						return;
							...
							Ext.Ajax.request({
								url:encodeURI(deptLevelSetsUrl+'?action=edit&code='+code+'&name='+name+'&remark='+remark+'&active='+active+'&end='+end+'&id='+rowid+'&order='+orderField.getValue()+'&repFlag='+rep),
								waitMsg:'保存中...',
