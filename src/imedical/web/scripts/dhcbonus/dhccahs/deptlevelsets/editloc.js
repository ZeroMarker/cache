editLocFun = function(dataStore,grid,pagingTool) {
	...
	assLocDs.on('beforeload', function(ds, o){
		ds.proxy=new Ext.data.HttpProxy({url:encodeURI('dhc.ca.deptlevelsetsexe.csp?action=listloc&recname='+Ext.getCmp('assLoc').getRawValue()+'&id='+hospSelecter.getValue()),method:'GET'});
	});
			...
			handler: function() {
				...
				if (formPanel.form.isValid()) {
					var active=(activeFlag.getValue()==true)?'Y':'N';
						Ext.Ajax.request({
							url: encodeURI(deptLevelSetsUrl+'?action=editloc&locDr='+assLoc.getValue()+'&active='+active+'&id='+rowid+"&subjdr="+repdr+'&order='+orderField.getValue()),
							waitMsg:'保存中...',
