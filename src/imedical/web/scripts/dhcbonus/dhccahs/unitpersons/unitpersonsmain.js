...
unitsDs.on('beforeload', function(ds, o){
	ds.proxy=new Ext.data.HttpProxy({url:encodeURI('dhc.ca.unitpersonsexe.csp?action=units&searchValue='+Ext.getCmp('units').getRawValue()+'&id='+unitType.getValue()),method:'GET'});
});
...
var UnitPersonsSearchBox = new Ext.form.TwinTriggerField({//查找按钮
		...
		onTrigger2Click: function() {
			if(this.getValue()) {
				...
				url:encodeURI(UnitPersonsUrl + '?action=list&searchField=' + UnitPersonsSearchField + '&searchValue=' + this.getValue())
...
var UnitPersonsMain = new Ext.grid.GridPanel({//表格
		...
		loadMask: true,
		//tbar: ['单元类别:',unitType,'-','单元:',units,'-',addUnitPersonsButton,'-',editUnitPersonsButton,'-',deptPersonsButton,'-',personsFindButton,'-',personsButton],
		tbar: ['单位类别:',unitType,'-','单位:',units,'-',addUnitPersonsButton,'-',editUnitPersonsButton],
