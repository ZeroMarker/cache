CommFindFun = function(dataStore, grid, pagingTool) {
    ...
    var tmpUId = "";
    var findCommTabProxy = new Ext.data.HttpProxy({ url: inDistDataUrl + '?action=find' });
    var tmpMonth = "";
    var myAct = "";
    ...
    var flagStore = new Ext.data.SimpleStore({//定义组合框中显示的数据源
        fields: ['type', 'rowid'],
        data: [['开单', 'order'], ['执行', 'do'], ['病人', 'pat'], ['指定', 'desig']]
    });
    ...
    var actionButton = new Ext.Toolbar.Button({
        ...
        handler: function() {
            ...
            findCommTabDs.load({ params: { start: 0, limit: findCommTabPagingToolbar.pageSize, intervalDr: tmpMonth, itemTypes: itemTypes, deptDr: deptDr, itemDr: itemDr, flag: flag, inType: inType} });
        }
    });
    ...
    var formPanel = new Ext.grid.GridPanel({
        ...
        loadMask: true,
        tbar: [monName, '-', itemTypesCombo, '-', items, '-', unitType, '-', units, '-', assLoc, '-', flagComm, '-', formComm, '-', actionButton],
