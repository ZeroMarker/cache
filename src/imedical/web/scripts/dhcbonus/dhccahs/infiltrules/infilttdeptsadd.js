inFilttDeptsAddFun = function(dataStore,grid,pagingTool) {
	Ext.QuickTips.init();
  // pre-define fields in the form
    ...
    assLocDs.on('beforeload', function(ds, o) {
        ds.proxy = new Ext.data.HttpProxy({ url: 'dhc.ca.infiltrulesexe.csp?action=tdepts&searchField=shortcut&searchValue=' + Ext.getCmp('assLoc').getRawValue() + '&unitsDr=' + units.getValue()+'&parRef='+selectedRowid, method: 'GET' });
    });
    ...
    buttons: [{
      ...
      handler: function() {
        	...
        	if (formPanel.form.isValid()) {
						Ext.Ajax.request({
							url: 'dhc.ca.infiltrulesexe.csp?action=addtdept&deptDr='+assLoc.getValue()+'&parRef='+selectedRowid,
							waitMsg:'保存中...',
