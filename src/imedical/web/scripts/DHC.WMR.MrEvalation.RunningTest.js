...
function ProcessRequestPatientInfo(NULL)
{
	if(GetMrInfo(strPaadm))
    {
	    DisplayPatientBaseInfo(objCurrVolume.RowID);
    }
    else
    {
...
function DisplayPatientBaseInfo(VolumeID)
{
	...
	if(strInfo == "")
		return;
	EvaluationApp.DisplayPatientBaseInfo(strInfo);
}
...
{	
	...
	strInfo += objCurrPatient.PatientNo + "     " + objCurrPatient.PatientName;
...
//get mr information from paadm no.
function GetMrInfo(paadm)
{
	if(paadm == "")
		return false;
	objCurrAdmit = GetPatientAdmitInfo("MethodGetPatientAdmitInfo", paadm);
	objCurrVolume = GetVolumeByAdm("MethodGetVolumeByAdm", paadm);
	objCurrMain = GetDHCWMRMainByID("MethodGetDHCWMRMainByID", objCurrVolume.Main_Dr);
	objCurrPatient = GetPatientByID("MethodGetPatientByID", objCurrAdmit.PatientID);
	return true;
}

function InitForm()
{
	var strApplet = "<APPLET ID = 'EvaluationApp' Name = 'EvaluationApp' codebase = '../addins/java' code = 'com/dhcc/wmr/qualityctl/evaluation/EvaluationPanelRuningTest.class' width = '900' height	= '600' >";
