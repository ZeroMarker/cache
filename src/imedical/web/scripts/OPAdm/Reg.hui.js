	...
	if (CFNotNullRealAmount == 1) {
		...
		if (EnableInsuBillFlag == false) {
			...
			if ((parseFloat(GetAmountPrice) == "0") & (PayModeCode == "CASH")) {
				var ContinueReg = dhcsys_confirm(('实收金额为零是否继续挂此号?'), false);
	...
	try {
		for (var j = 0; j < Data["rows"].length; j++) {
			...
			if (retarr[0] == "0") {
				...
				if(DYOPMRN!="")BLNo=1;
				if(BLNo==1){
					var BLFlag=dhcsys_confirm('有病历号,是否传病历号?');
					if(BLFlag==true){
				...
				if (retarr[0] == "-304") errmsg = "超过每人每天相同时段同科室同医生限额!";
				if (retarr[0] == "-405") errmsg = "请去挂号设置界面维护免费医嘱!";
...
function GetPannelHTML(jsonData, LinkID) {
	...
	if (ServerObj.ParaRegType == "APP") {
		...
		for (var j = 0; j < Len; j++) {
			...
			if (Status == 0) {
				innerHTML = innerHTML + "<td class='td-seqno-invalid'>" + "<span class='td-seqno'>可用数:" + SeqNo + "</span><br><span class='td-time'>" + Time + "</span></td>";
			} else {
				innerHTML = innerHTML + "<td onmouseover=mouserover(this) onmouseout=mouserout(this) onclick=tdclick(this) ondblclick=dbtdclick(this) id='" + LinkID + "_table_" + Time + "'>" + "<span class='td-seqno'>可用数:" + SeqNo + "</span><br><span class='td-time'>" + Time + "</span></td>";
			}
