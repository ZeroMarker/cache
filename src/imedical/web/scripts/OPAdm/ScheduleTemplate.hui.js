...
function InitEvent(){
	...
	$("#BtnExtendConfig").click(function(){
		ExtendConfigshow();
	});
	...
	var ListColumns=[[ 
		...
		{field:'ResDesc',title:'医生',align:'center',width:200,resizable:true,
			formatter:function(val, row, index){
				if(row.Invalid == "Y") {
					return val+"<font color='red'>(已停用)</font>";
				} 
				return val;
			}
		},
    ...
    var ScheduleTemplateListDataGrid=$("#ScheduleTemplateList").datagrid({
		...
		onClickCell:function(rowIndex, field, value){
			...
			if(ClickType=='1'){
				...
				var Invalid=DataArr.rows[rowIndex].Invalid;
				var ResDesc=DataArr.rows[rowIndex].ResDesc;
				var Week="",WeekIndex="";
				...
				ResClick(DateRowid,LocRowid,DocRowid,Week,TimeRange,Invalid,ResDesc);
			...
			$(this).droppable({
				onDrop:function(e,source){
					...
					if((ResLocID!="")&&(LocRowid!=ResLocID)){
						dhcsys_alert("不同科室不允许复制专业组,专业组数据未复制,请确认并修改!")
					...
					//Calc_TRInfo();
					var ret=SaveClick("CopyAdd",CopyFromTimeRange,SessRowid);
					if(ret){
...
function ResClick(DateRowid,LocRowid,DocRowid,Week,TimeRange,Invalid,ResDesc)
{
	...
	if (PageLogicObj.m_AddFlag=="1"){
		...
		if (DateRowid!=""){
			...
			if(Invalid == "Y") {
				$("#AdmDoc").combobox("disable").combobox("setValue",ResDesc+"(已停止)");
			}else {
				$("#AdmDoc").combobox("disable").combobox("select",DocRowid);
			}
			setTimeout(function(){
				...
				$('#TRFlag,#NoLimitLoadFlag').checkbox('uncheck');
				TRFlag_Click();
				NoLimitLoad_Click();
	...
	setTimeout(function(){
		if (DateRowid!=""){
			...
			$('#SessEditWindow').window('setTitle',WinTitle+" <font color='#FF9999'>"+selLoc+"  "+selDoc+"<font>");
		}
	},1500)
	return;
}
...
function LoadWeekTabs(DateRowid,DefWeek,DefTimeRange){
	...
	PageLogicObj.OldTimeRange=DefTimeRange
}
function SetResSessData(SessRowid){
	...
	if (SessRowid!=""){
		...
		var OldTRValue=$('#TRFlag').checkbox('getValue');
		/*
		...
		//if (((OldTRValue)&&(TRFlag=="Y"))||(!OldTRValue)&&(TRFlag!="Y")) {
			setTimeout(function(){
				TRFlag_Click();
			},500);
		//}
		*/
		var TRFlag=TRArr[0];
		CheckBoxChangeHandle("TRFlag",TRFlag,TRFlag_Click)
		var NoLimitLoadFlag=SessArr[42];
		CheckBoxChangeHandle("NoLimitLoadFlag",NoLimitLoadFlag,""); //NoLimitLoad_Click
		NoLimitLoad_Click("Y");
		/*setTimeout(function(){
			var TRFlag=TRArr[0];
			if(TRFlag=="Y"){
	...
	if($("#TRFlag").checkbox('getValue')){
        ...
        if(!data.length){
			...
			var cbox = $HUI.combobox("#TRTemp", {
					        ...
					        onBeforeLoad:function(param){
					            param.TRRowid=$("#TimeRange").combobox('getValue');
...
function SaveClick(Type,CopyFromTimeRange,CopyFromSessRowid)
{
	...
	if ((Type=="Add")||(Type=="CopyAdd")){
		...
		InsertData=InsertData+"^"+TRFlag+"^"+""+"^"+""+"^"+TRLength+"^"+TRRegNum+"^"+TRRegNumStr+"^"+TRRegInfoStr+"^"+AutoLoad+"^"+ExtLoad+"^"+NoLimitLoadFlag;
		...
		if (retCode == 0){
			...
			if(temparr[2]!=""){
				...
				if ((ClickType==3)&&(Type=="CopyAdd")){
					...
					$.cm({
				        ClassName:'DHCDoc.OPAdm.ScheduleTemp',
				        MethodName:'SaveTempExtendConfig',
				        CopyFromSessRowid:CopyFromSessRowid,
				        NewSessRowId:SessRowId,
				        dataType:"text"
				    },false);
					return SessRowId;
				}
		...
		UpdateData=UpdateData+"^"+TRFlag+"^"+TRStartTime+"^"+TREndTime+"^"+TRLength+"^"+TRRegNum+"^"+TRRegNumStr+"^"+TRRegInfoStr+"^"+AutoLoad+"^"+ExtLoad+"^"+NoLimitLoadFlag;
		...
		if (retcode==0){
			...
			if(NoLimitLoadFlag=="Y"){
                ...
                for(var i=0;i<rows.length;i++){
               	...
               	var TimeRangeText=$("#TimeRange").combobox("getText")
                if ((parseFloat(TRInfoTotal)!=parseFloat(Load))||(PageLogicObj.OldTimeRange!=TimeRangeText)){
	            	var SessRowid=""
				    var tab = $('#tabTimeRange').tabs('getSelected');
					var index = $('#tabTimeRange').tabs('getTabIndex',tab);
	                var tab=$('#tabTimeRange').tabs('getTab',index);
			...
			if (retcode=="-201"){
				...
				$.messager.alert('提示',"分时段专业组里存在模板中未对应的专业组信息，请核实! ");
			}else{
...
function CheckBeforeSave()
{
	...
	if(NoLimitLoadFlag!="Y"){
		...
		if ((parseInt(EPositiveMax))<(parseInt(EApptMax))){
			$.messager.alert('提示', "正号限额不能小于预约限额！","warning");
			return false;
		}
		if (parseInt(EApptMax)!=0){
			if ((parseInt(EPositiveMax)-parseInt(EStartPrefix)+1)<(parseInt(EApptMax))){
				$.messager.alert('提示', "正号限额减去预约起始号要大于等于预约限额！","warning");
				return false;
			}
		}
		if (parseInt(EPositiveMax)>999){
			$.messager.alert('提示', "非便捷排班的排班模板正号限额不能超过999!","warning");
			return false;
		}
		if (parseInt(EAddtionMax)>9999){
			$.messager.alert('提示', "非便捷排班的排班模板加号限额不能超过9999!","warning");
			return false;
		}
...
function DocSelectFun(){
	...
	if(PageLogicObj.m_AddFlag == 1){
		...
		//$("#ClinicGroup").combobox('loadData',[]);
		//LoadClinicGroup(LocSelectedId,"ClinicGroup");
		var sbox = $HUI.combobox("#ClinicGroup");
		if (ClinicGroupDR!="")  sbox.select(ClinicGroupDR);
		///$("#ClinicGroup").combobox("select",ClinicGroupDR);
	}
}
...
function InitTRDataGrid()
{
    $('#tabTRInfo').datagrid({
        ...
        toolbar:[{
            ...
            handler:function(){
			    ...
			    if (retJson=="0"){
					...
					// 维护时，先添加模板，再勾选分时段，弹出分时段信息模板选择保存后，没有点击模板维护的保存,在点分时段保存时同时调用模板信息保存
					SaveClick("Update");
				    LoadtabTRInfo(SessRowid)
				}
            }
        ...
        onBeforeSelect:function(index,row){
            ...
            if(CheckGridEditing('tabTRClinicGroupInfo')){
                $.messager.popover({msg:"请先保存专业组后再进行分时段操作",type:'alert'});
                return false;
            }
            if(CheckGridEditing('tabTRClinicGroupInfo')){
                var UnsaveFlag=dhcsys_confirm('有未保存专业组方式信息,是否继续?');
                if (UnsaveFlag==false){
	               return UnsaveFlag;
	            }else{
		           return CheckBeforClinicGroup();
		        }
            }
        ...
        onSelect:function(index,row){
            ...
            if(!tabTRClinicGroupInfo) tabTRAppMethodInfo=[];
            if(typeof(tabTRClinicGroupInfo)=='string'){
                tabTRClinicGroupInfo=JSON.parse(tabTRClinicGroupInfo);
            }
            $('#tabTRClinicGroupInfo').datagrid("rejectChanges").datagrid("unselectAll");
    ...
    $('#tabTRClinicGroupInfo').datagrid({
                ...
                editor:{
                    ...
                    options:{
                        ...
                        onBeforeLoad:function(param){
							...
							var SessRowid=tab[0].id;
                            param.ClassName='DHCDoc.OPAdm.ScheduleTemp';
                            param.QueryName='QueryClinicGroup';
                            param.SessRowid=SessRowid;
                            param.ArgCnt=1;
                        }
                    }
                }
            },
            {field:'Qty',title:'数量',align:'center',width:110,
                editor:{type:'numberbox',options:{min:0,
                    onChange:function(val,oldVal){
                        //QtyChange('tabTRAppMethodInfo',this);
                    }
        ...
        toolbar:[{
            ...
            handler:function(){
                ...
                tabClinicGroupEditRow=rows.length-1
            }
        },{
            text:'',
            iconCls:'icon-remove',
            handler:function(){
                ...
                if((!Selected)&&(tabClinicGroupEditRow==="")){
                    $.messager.popover({msg:'请选择需要删除的行',type:'alert'});
                    return;
                }
               if (Selected){
	               ...
	               var index=tabClinicGroupEditRow
	               if ($('#tabTRClinicGroupInfo').datagrid('getEditors',tabClinicGroupEditRow-1).length>0){
		               tabClinicGroupEditRow=tabClinicGroupEditRow-1
	               }
	           }
            ...
            iconCls:'icon-save',
            handler:function(){
	            var CheckFlag=CheckBeforClinicGroup()
                if (CheckFlag==false) return;
                var TRInfoSelected=$('#tabTRInfo').datagrid('getSelected');
                var newRows=new Array();
                var AMArr=new Array();
...
function QtyChange(id,that){
    ...
    if(Number(MaxQty)<Number(ReserveQty)) {
	    $.messager.popover({msg:"保留数不能大于最大预约数!",type:'error',style: {
       ...
       var TRAMData=$.extend({},tabTRAppMethodInfo);
        for(var j in TRAMData){
            ...
            if(TRAMArr[AppMethodID]){
               ...
               if (TRAMArr[AppMethodID].TRMaxQty<Number(TRAMData[j].MaxQty)) {
	               TRAMArr[AppMethodID].TRMaxQty=Number(TRAMData[j].MaxQty);
               }
            }else{
...
function AppMethodQtySave()
{
	...
	var tab=$('#tabTimeRange').tabs('getTab',index);
	if(!tab || tab.length==0){
		$.messager.popover({msg:"预约方式限额保存失败:请先保存排班模板",type:'alert'});
        return false;
	}
    var SessRowid=tab[0].id;
    if (SessRowid==""){
	...
	if (NewNoLimitLoadFlag!=NoLimitLoadFlag) {
		$.messager.popover({msg:"预约方式限额保存失败:便捷排班标识勾选变化,请先保存排班模板信息!",type:'alert'});
		return false;
	...
	if (NewAvailAppQty!=AvailAppQty) {
		$.messager.alert("提示","预约方式限额保存失败:请先保存排班模板预约限额!","info",function(){
			$("#ApptMax").focus();
		});
		return false;
	}else if ((NewAvailAppQty=="0")&&(AvailAppQty>0)){
		$.messager.alert("提示","预约方式限额保存失败:预约限额为0，请维护预约限额","info",function(){
			$("#ApptMax").focus();
		});
		return false;
	}
    ...
    for(var i=0;i<rows.length;i++){
        ...
        if(rows[i].AppMethodID){
            ...
            if(TRAMArr[rows[i].AppMethodID]){
                ...
                if (Number(TRAMArr[rows[i].AppMethodID].TRMaxQty)>Number(rows[i].MaxQty)){
	                $.messager.popover({
                        msg:rows[i].AppMethod+'分时段最大预约数:'+TRAMArr[rows[i].AppMethodID].TRMaxQty+"大于预约方式最大预约数:"+rows[i].MaxQty,
                        type:'error'
                    });
                    return false;
                }
    ...
    if (retJson=="0"){
	    $.messager.popover({msg:"预约方式限额保存成功!",type:'success'});
...
function CheckBeforClinicGroup(){
	var TRInfoSelected=$('#tabTRInfo').datagrid('getSelected');
    if(!TRInfoSelected){
        $.messager.popover({msg:'请选择时段后再维护时段预约方式',type:'error'});
        return false;
    ...
    for(var i=0;i<rows.length;i++){
        ...
        if(rows[i].ClinicGroupMethodID){
            ...
            if(rows[i].Qty==""){
                $.messager.popover({msg:rows[i].ClinicGroup+"数不能为空!",type:'error'});
                return false;
            }
    ...
    if(ReserveQtySum>Number(TRLoad)){
        $.messager.popover({msg:"专业组数量合计:"+ReserveQtySum+",不能超过时段总数:"+TRLoad+"!",type:'error'});
        return false;
    }
    return true;
	}
...
function ExtendConfigshow(){
	...
	ExtendConfiginit(SessRowid)
	ExtendConfigload(SessRowid)
	}
var retArry	
var ExtendConfigArry=new Object();
function ExtendConfiginit(SessRowid){
	...
	$("#SaveExtendConfig").click(function(){
		ExtendConfigsave(SessRowid);
	});
}
function ExtendConfigload(SessRowid){
	ExtendConfigArry=[];
	var ret=$.m({ 
		ClassName:"DHCDoc.OPAdm.ScheduleTemplateConfig", 
		MethodName:"ShowTempExtendConfig",
		SessRowid:SessRowid,dataType:"text"
	},false);
...
function ExtendConfigsave(SessRowid){
	var RowIDStr=""
	var TypeIDStr=""
	var ValueStr=""
	for (var i=0; i<retArry.length; i++){
	...
	var ret=$.m({ 
		ClassName:"DHCDoc.OPAdm.ScheduleTemplateConfig", 
		MethodName:"SaveTempExtendConfig",
		SessRowid:SessRowid,RowIDStr:RowIDStr, TypeIDStr:TypeIDStr,ValueStr:ValueStr,dataType:"text"
	},false); 
	if (ret==0){
		$.messager.popover({msg:"保存成功!",type:'success'});
...
function NoLimitLoad_Click(NotResetVal){
	//正号限额、预约限额 、加号限额预约起始号
	var CacheEleAry=["ApptMax","EStartPrefix"]; //"PositiveMax",,"AddtionMax"
	var value=$("#NoLimitLoadFlag").checkbox('getValue')
	if(value){
		var CacheObj={};
...
function CheckBoxChangeHandle(id,chk,callFun){
	var onCheckChange=$("#"+id).checkbox('options').onCheckChange;
	$("#"+id).checkbox('options').onCheckChange=function(){}
	if(chk=="Y"){
		$("#"+id).checkbox('check');
	}else{
		$("#"+id).checkbox('uncheck');
	}
	$("#"+id).checkbox('options').onCheckChange=onCheckChange;
