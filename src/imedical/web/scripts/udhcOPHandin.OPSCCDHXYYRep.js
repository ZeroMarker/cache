...
function BHandin_Click()
{
	...
	///alert(myinfo);
	///alert(m_INVPRTReportsXmlData);
	var myrtn=confirm(t['03']);
	if (myrtn==false){
		return;
	}
	
	////ReportsXmlData, INVPRTPayModeXml, PDXmlData, APIXmlData, CardXmlData
	///var m_INVPRTReportsXmlData="";
	///var m_PDReportsXmlData="";
	///var m_APIReportsXmlData ="";
	///var m_CardReportsXmlData = "";
	if (encmeth!=""){
		///alert(m_CatXmlData);
		///m_INVPRTReportsXmlData, m_PDReportsXmlData, m_APIReportsXmlData, m_CardReportsXmlData,
		var rtn=(cspRunServerMethod(encmeth,Guser,myinfo, m_ReportsXmlData, m_CatXmlData));
	}
	////alert(rtn);
...
function BuildFootInfo(){
	...
	myList[62]=DHCWebD_GetObjValue("RepNote");
...
function RemoveRows()
{
	var myMaxRow=DHCWeb_GetTBRows("tudhcOPHandin_OPSCCDHXYYRep");
	
	var tablistobj=document.getElementById("tudhcOPHandin_OPSCCDHXYYRep");
...
function SetListQ(TARData){
	...
	var myTblObj=document.getElementById("tudhcOPHandin_OPSCCDHXYYRep");
	if (myTblObj){
...
function SetHandsum(value, CatXmlData)
{
