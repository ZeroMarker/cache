    ...
    switch (phatype) {
        ...
        case "DSY":
	        ...
	        PrintPJ(phac,phatype)  ; 	 
          	break;
        case "KFY":
           	PrintHz(phac,phatype)  ; //口服药
            PrintPJ(phac,phatype)  ; //摆药
           	break;
        case "WYY":
           	PrintHz(phac,phatype)  ; //外用药
            PrintPJ(phac,phatype)  ;  //摆药
           	break;
        case "DMY":
...
{    
  ...
  try{
        ...
        if (gTotalCnt>0) 
        {
	        ...
	        xlsheet.Cells(2, 1).Value = getDesc(WardName)+"   "+getDesc(PhaLoc) +DisTypeDesc+"发药单NO."+" "+DispNo
...
function PrintPJ(phac,phatype)
{
 ...
 try{
    ...
    xlsheet.Cells(2, 1).Value = getDesc(WardName)+"  "+ getDesc(PhaLoc)+DisTypeDesc+"摆药单No."+DispNo
    xlsheet.Cells(3, 1).Value ="("+Priority+")"+"  "+t['DISPDATE']+DateRange+reflag
    xlsheet.Cells(4, 1).Value = t['PRINTDATE']+formatDate2(getPrintDate())+" "+getPrintTime(); 
    xlsheet.Cells(4, 4).Value = t['WARD'] + getDesc(WardName) 
     ...
     {    
	    ...
	    {           
	      ...
	      //  alert(row)
	        ...
	        xlsheet.Cells(row, 13).Value=sprice		//单价
	  ...
	  for (var i=1;i<=gDetailCnt;i++)
	  {
         ...
         if ((tmpno==prescno)&&(i>1)&&(tmpid==relateid))
         {
		  ...
		  if ((tmpno!=prescno)&&(tmpid!=relateid))
		  {
            ...
            if (priority=="出院带药"){bt=bt+" "+"(  出院带药  )"}

            setBottomLine(xlsheet,startrow-1,1,12);
 
            xlsheet.Cells(startrow-3, 5).Value="深圳市中医院中药处方"+bt
