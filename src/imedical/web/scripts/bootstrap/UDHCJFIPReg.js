...
function getpatinfo() {
	...
	if (key == 13) {
		//add by wangjian
		//CheckBookMesage(); //+2015-3-4 hujunbin 注释
		//end
		//clear_click()
		getpatinfo1();
	...
	var BookID = IPBookObj.value
		...
		/*
		//2015-3-4 hujunbin 注释
		if ((BookID!="")&&(!Parobj)){
		SetDefAdmValue();
		IPBookObj.value="" //这里赋空,住院证查询结束 保证在院后不再赋值默认就诊信息
		}
		 */
}
...
function regsave_click() {
	...
	var qfstatus = cspRunServerMethod(encmeth, papnoobj.value)
		...
		///判断是否为预约，如果为预约则修改为在院状态  yyx 2012-10-26
		///modify 2014-03-19 测试有预约记录病人住院时预约状态的住院记录未变又重新办住院的问题
		...
		if (obj)
			TestIPBook = obj.value
				...
				if ((admidobj.value != "") && (AdmStatus == "P")) {
					...
					///modify 2014-03-19 测试有预约记录病人住院时预约状态的住院记录未变又重新办住院的问题
					var AdmConfirm = window.confirm("此病人为预住院状态,是否确认住院?")
						if (AdmConfirm) {
							var IPBook = ""
								var obj = document.getElementById("IPBook")
								if (obj)
...
function admupdate_click() {
	...
	var p2 = p2 + "^" + "" + "^" + "" + "^" + admdepidobj.value + "^" + admwardidobj.value + "^" + AdmDocDrObj.value + "^" + AdmSourceIDObj.value + "^" + admdateobj.value + "^" + AdmPriorityID; //入院病情
...
function Prt_WCinctureo() {
	...
	if (admidobj.value == "") {
		...
		if (getadmstatus != "A") {
	     ...
	     str=str+"^"+"HospitalName"+CH2+"东华标准版数字化医院"+"^"+"MedicareNo"+CH2+medicareobj.value+"^"+"RegNoBarCode"+CH2+"*"+papnoobj.value+"*";
	     //alert(str);
...
function readperson_click() {
	...
	if (myary[0] == "0") {
		//<IDRoot><Age>24</Age><Name>王伟</Name><Sex>男</Sex><NationDesc>01</NationDesc><Birth>1988-08-19</Birth><Address>山东省东营市东营区北一路739号</Address><CredNo>37078419880819641X</CredNo></IDRoot>
		//var str=myary[1]
		var str = GetPatInfoByXML(myary[1]);
		var arr = str.split("^");
		var tnameobj = document.getElementById('name');
		if ((tnameobj.value != "") & (tnameobj.value != arr[1]) & (arr[1] != "")) {
			var myrtn = window.confirm("身份证姓名:" + arr[1] + " 与HIS姓名:" + tnameobj.value + " 不一致,是否使用身份证姓名!");
			if (myrtn) {
...
function LinkADDDEP()
{
	var AddDepConfirm = window.confirm("是否进入预交金界面?")
		if (AddDepConfirm) {
