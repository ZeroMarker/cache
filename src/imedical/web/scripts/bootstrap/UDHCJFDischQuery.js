...
function RecordSave_Click() {
	var RecordComment = document.getElementById('Comment').value
		...
		var GetRecordObj = document.getElementById('GetRecord');
	if (GetRecordObj) {
		var encmeth = GetRecordObj.value
	} else {
		var encmeth = ''
	};
...
function GetPCPName(value)
{
	...
	var PCPRowID=RtnInfo[2]
	document.getElementById('DocID').value=PCPRowID;
}
...
function outExp_click()
{
  ...
  //Medval As %String, Finalval As %String, Stdate As %String, EndDate As %String, locid As %String, wardid As %String, payflagvalue As %String, grp As %String, confirmvalue As %String, regno As %String, MasterName As %String, Patzyno As %String, CurAdm, DocID
	...
	if(CurAdmobj.checked==true){CurAdm="on";}
	else{CurAdm="";}
	var fileName="DHCBill_出院患者统计.raq&Medval="+Medval+"&Finalval="+Finalval+"&Stdate="+Stdate+"&EndDate="+EndDate+"&locid="+locid+"&wardid="+wardid+"&payflagvalue="+payflagvalue+"&grp="+grp+"&confirmvalue="+confirmvalue+"&regno="+regno+"&MasterName="+MasterName+"&Patzyno="+Patzyno+"&CurAdm="+CurAdm+"&DocID="+DocID
	alert(fileName);
