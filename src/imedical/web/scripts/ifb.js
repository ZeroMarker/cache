...
function initDocument()
{
    ...
    initLookUp(); //初始化放大镜
    var paramsFrom=[{"name":"Type","type":"2","value":""},{"name":"LocDesc","type":"1","value":"IFBManageLocDRR_CTLOCDesc"},{"name":"vgroupid","type":"2","value":""},{"name":"LocType","type":"2","value":"0101"},{"name":"notUseFlag","type":"2","value":""}];
    ...
    //muilt_Tab()
    //muilttable_Tab()   //modified by wy 2020-7-3 	1405497 下表格回车事件还未完成
...
function BSave_Clicked()
{
	...
	for (var i = 0; i < rows.length; i++) 
	{
			...
			if (rows[i].IFBLVendorDR!="")
		{
			...
			IFBLData["IFBLSourceType"]="3"
			IFBLData["IFBLSourceID"]=ValList.IFBLRowID
...
function GetExtendID(index,data)
{
	...
	$('#DHCEQIFB').datagrid('endEdit',editIndex);
	$('#DHCEQIFB').datagrid('beginEdit',editIndex);  // add by wy 2020-7-3 bugWY0072 重复开始编译
	bindGridEvent();	// MZY0037	1400284		2020-07-06
}
...
function BupdateIFBBList(index)
{
	...
	//modified by ZY0228 20200509
	url=url+"&SourceType=3";
	url=url+"&SourceID="+IFBBRowID;
	url=url+"&ReadOnly="+getElementValue("ReadOnly")  //modfied by wy 2019-12-3 需求1110996 
	showWindow(url,"应标供应商","","","icon-w-paper","modal","","","large");    // modfied by lmm 2020-06-04 UI
