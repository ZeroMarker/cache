/**
 * 医保剂型对照测试JS
 * FileName:insuformdictionarycon.js
 * xubaobao 2019-04-04
 * 版本：V1.0
 * hisui版本:0.1.0
 */
	...
	$('#HISInfoGV').datagrid({
		...
		{field:'delCon',title:'撤销对照',width:70
			,formatter:function(value,rowData,index){
				if(rowData.MedCode!=""){
						return "<a href='#' onclick='DelCon("+index+','+JSON.stringify(rowData)+")'>\
...
function SaveHisDicConDo(HisSelRowData, InsuRowData){
	...
	var ConInfo=HisCode+"^"+HisDesc+"^"+diccode+"^"+dicdesc+"^"+userDr+"^"+PUBLIC_CONSTANT.SESSION.HOSPID    //增加医院编号
	...
	$.post(
		APP_PATH+"/INSUDictionaryContrast/SaveDicConAjax",
		{
			KeyCode:KeyCode
			,InsuType:InsuType
			,HospitalNo:PUBLIC_CONSTANT.SESSION.HOSPID
			,ConInfo:ConInfo
		},
		function(data,textStatus){
			if(textStatus=="success"){
				if(data.status>0){
					...
					$.messager.alert('温馨提醒','对照信息保存失败：' + data,'error');
				}
			}else{
		...
		function(data,textStatus){
			if(textStatus=="success"){
				if(data.status>0){
					...
					$.messager.alert('温馨提醒','对照信息删除失败：' + data,'error');
				}
			}else{
				$.messager.alert('系统错误','系统异常，请稍后重试');
			}
...
function reloadHisDicConGV(loadType){
	...
	//重新加载查询结果
	// rows, page, KeyCode As %String, InsuType As %String, HospitalNo As %String, ExtStr
...
function Import()
{
	var KeyCode=DicCode;
	var InsuType=$('#InsuTypeBox').combobox('getValue');     //医保类型
	importData(KeyCode,InsuType,PUBLIC_CONSTANT.SESSION.HOSPID);		 //insuimportdictionarycon.js
	reloadHisDicConGV('reload');
}
function selectHospCombHandle(){
	$('#InsuTypeBox').combobox('clear');
	var ExtStr = PUBLIC_CONSTANT.SESSION.HOSPID;  //增加院区 20230314 
