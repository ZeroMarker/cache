...
{
	LookUpSourceID("GetEquip","'',UserDR,SourceID");
}

function LookUpSourceID(jsfunction,paras)
{
	LookUp("","web.DHCEQEquipOperator:GetOperatorEquipList",jsfunction,paras);
}
...
function filldata()
{
	...
	else
	{
		...
		SetElement("SourceType",list[1]) ; //来源类型
	  	...
	  	SetChkElement("IsInputFlag",list[18]) ; //手工录入标识
...
function SetEnabled()
{
	...
	if ((GetElementValue("Status")=="2")||(GetElementValue("Status")=="3")||(GetChkElementValue("IsInputFlag")==false))
	{
		DisableBElement("BAdd",true)
		DisableBElement("BUpdate",true)
		DisableBElement("BSubmit",true)
...
function BAudit_Click()
{
	...
	if (result>0)
	{
		parent.DHCEQUseRecord.location.href="websys.default.csp?WEBSYS.TCOMPONENT=DHCEQUseRecord&SourceIDDR="+SourceID+"&ServiceItemDR="+ServiceDR
		parent.DHCEQUseRecordList.location.href="websys.default.csp?WEBSYS.TCOMPONENT=DHCEQUseRecordList&StartDate="+today+"&EndDate="+today+"&UserDR="+userdr+"&QXType=0"
	}
	else
	{
		alertShow(t["01"]);
...
function BCancelSubmit_Click()
{
	...
	if (result>0)
	{
		parent.DHCEQUseRecord.location.href="websys.default.csp?WEBSYS.TCOMPONENT=DHCEQUseRecord&RowID="+result
		parent.DHCEQUseRecordList.location.href="websys.default.csp?WEBSYS.TCOMPONENT=DHCEQUseRecordList&StartDate="+today+"&EndDate="+today+"&UserDR="+userdr+"&QXType=0"
	}
	else
	{
...
function BUpdate_Click() //添加
{
	...
	var plist=CombinData(); //函数调用
  	var flag=GetChkElementValue("IsInputFlag")
  	if (flag==false)
  	{
	  	flag="N"
  	}
	...
	{
		parent.DHCEQUseRecord.location.href="websys.default.csp?WEBSYS.TCOMPONENT=DHCEQUseRecord&RowID="+result
		parent.DHCEQUseRecordList.location.href="websys.default.csp?WEBSYS.TCOMPONENT=DHCEQUseRecordList&StartDate="+today+"&EndDate="+today+"&UserDR="+user+"&QXType=0"
	}	
}
