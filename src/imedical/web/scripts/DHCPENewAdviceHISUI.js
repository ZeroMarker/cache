...
function StatusChange(Type, CompleteFlag) {
    ...
    CompleteFlag = tkMakeServerCall("web.DHCPE.ResultNew", "GetWriteWay", StationIDHISUI, CurLocID, UserID, GroupID)
    ...
    if (flag == "1") {
        $.messager.alert("提示", "住院病人需在体检列表设置为体检类型", "info");
        return false;
    }
    ...
    if (Type == "Submit") {
        ...
        for (var i = 0; i < ButtonLength; i++) {
            ...
            if (ButtonID.split("Update").length > 1) {
                ...
                if ((UObj[i].innerText.indexOf("保存") != -1)) {
                    ...
                    //科室提交时，谢绝检查的项目退出
                    var RefuseFlag = tkMakeServerCall("web.DHCPE.DHCPECommon", "IsRefuseCheck", ResultInfo.split("^")[0])
                    if (RefuseFlag == 1) continue;
    ...
    $PECA.CASign("1", PAADM + "%" + ChartID, "", CAAuditCallback, { Type: Type, CompleteFlag: CompleteFlag });
}
...
* @Date 2022-10-13
*/
function CAAuditCallback(pObj) {
    var PAADM = "", ChartID = "", encmeth = "", Type = pObj.Type, CompleteFlag = pObj.CompleteFlag,
        CurLocID = session['LOGON.CTLOCID'], UserID = session['LOGON.USERID'];
    var obj = document.getElementById("EpisodeID");
    ...
    if (flag == "0") {
        if (Type == "Submit") {
            if (CompleteFlag == "1")  //保存时科室确认
            {
                ...
                if(parent && parent.BComplete) parent.BComplete();
            }
            ...
            if(parent && parent.ShowNextPage) parent.ShowNextPage();
        } else {
            window.location.reload();
        }
        return false;
        ...
        if (Type == "Submit") {
            var ConRet = tkMakeServerCall("web.DHCPE.OtherPatientToHP", "InsertConOrder", PAADM, ChartID, UserID, UserID, CurLocID)
...
function SaveResult() {
    ...
    for (var i = 0; i < ButtonLength; i++) {
        ...
        if ((ButtonID.split("Update").length > 1) && (UObj[i].innerText.indexOf("保存建议") != -1)) UObj[i].click();
    ...
    for (var i = 0; i < ButtonLength; i++) {
        var ButtonID = UObj[i].name;
        if (ButtonID.split("Update").length > 1) {
            if (UObj[i].innerText.indexOf("保存建议") != -1) { UObj[i].click(); }

        }
        if (ButtonID == "BSave") UObj[i].click();
    }
    /** 保存体检结果  超级权限组不保存*/
    // if ($("#H_SuperFlag").val() === "0") {
    //     $(".SaveRltCls").each((i, btn) => {
    //         console.log(btn);
    //     });
    // }
}
...
function EDClick() {
    ...
    if (obj) encmeth = obj.value;
    EDDesc = Desc;
    if (RealDesc) EDDesc = RealDesc;
    //var Info=cspRunServerMethod(encmeth,StationID,Desc,EpisodeID);
    var Info = tkMakeServerCall("web.DHCPE.DHCPEExpertDiagnosis", "GetEDInfoByDesc", StationID, EDDesc, EpisodeID);
