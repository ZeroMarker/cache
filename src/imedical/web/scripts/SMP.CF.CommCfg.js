...
var init=function(){
	var dg=SMP_COMM.createSimpleEditGrid({
		...
		deleteFun:function(ind,row,callback){
			$.ajaxRunServerMethod({ClassName:'CF.BSP.SMP.Comm',MethodName:'Delete',Id:row.TId},function(rtn){
				if (rtn>0){
					callback(true,rtn);
				}else{
		...
		saveFun:function(ind,row,callback){
			$.ajaxRunServerMethod({ClassName:'CF.BSP.SMP.Comm',MethodName:'Save',
				Id:row.TId,
				Type:row.TType,
				Code:row.TCode,
				Value:row.TValue,
		...
		queryParams:{ClassName:'CF.BSP.SMP.Comm',QueryName:'Find',IsPar:1}
		,onSelectChange:function(){
			debounced_onWestSelectChange();
		},deleteConfirmMsg:'确定删除此配置类型么，此操作会删除该配置类型下所有数据'
	...
	function onWestSelectChange(){
		...
		if(dg2){
			var title=GV.selectedDicDesc==""?'配置数据列表':('配置【'+GV.selectedDicDesc.split('配置')[0]+'】数据列表')
	...
	var dg2=SMP_COMM.createSimpleEditGrid({
		...
		columns:[[
			...
			{field:"TValue",title:"配置值",width:100,editor:{
				type:'text'
			}},
		...
		queryParams:{ClassName:'CF.BSP.SMP.Comm',QueryName:'Find',IsPar:0},
		parentGrid:'#t-w',
		...
		importFun:function(row,callback){
			var param={ClassName:'CF.BSP.SMP.Comm',MethodName:'Save',IsPar:0};
