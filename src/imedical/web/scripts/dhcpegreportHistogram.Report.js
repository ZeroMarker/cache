...
function pagesetup_default(PrintFlag)
{   
	PrintActiveX.marginTop=10;
	PrintActiveX.marginRight=8;
	PrintActiveX.marginBottom=12;
	PrintActiveX.marginLeft=13;
	PrintActiveX.isHtmlHeadFoot=true;

	PrintActiveX.paddingTop=176;
	//PrintActiveX.paddingRight=10;
	PrintActiveX.paddingBottom=10;
	//PrintActiveX.paddingLeft=5;
	PrintActiveX.pageName="A4";
	PrintActiveX.orientation=1;
	PrintActiveX.footer="";
	PrintActiveX.header="";
	var obj,Name="",HPNo="",Sex="",Age="",RegNo="";	
	...
	Sex=obj.innerText;
	obj=document.getElementById("PatAge");
	Age=obj.innerText;
	obj=document.getElementById("RegNo");
	RegNo=obj.innerText;
	var footerstr='&b<div style="width:800px;height:2px;margin:0px auto;padding:0px;background-color:black;overflow:hidden;"></div><div style="padding:0px 0px 50px 0px"><font face=华文楷体 size=2px>';
	footerstr=footerstr+"体检编号:"+RegNo;
    footerstr=footerstr+"　姓名:"+Name;
    footerstr=footerstr+"　性别:"+Sex;
    footerstr=footerstr+"　年龄:"+Age;
    footerstr=footerstr+"　　健康档案号:"+HPNo;
	footerstr=footerstr+'　　&p/&P</font></div>&b';
	
	PrintActiveX.footer=escape(footerstr);
	PrintActiveX.footer=unescape(footerstr);
	//alert(PrintActiveX.footer)
	//PrintActiveX.pageWidth=2000;
	//PrintActiveX.pageHeight=800;
	var str=PrintActiveX.footer;
	window.showModalDialog("http://172.19.19.82/dthealth/web/TimeShow.html",str,"dialogWidth=0px;dialogHeight=0px;status=no;help=no;scrollbars=no");
...
function DrawHistogram(oHistogram,iLabels,iList,Width,Heigth){	
	...
	for (var iLLoop=1;iLLoop<=iList.length;iLLoop++) {
		...
		fillcolor=tmdColor1[13]; //   GetColor(); 
		...
		DrawRect(oHistogram, rect_x, rect_y, 1, rect_height, fillcolor,iLLoop);
		//strText=iLabels[(iLLoop-1)];
		//DrawText(oHistogram, "", 15, rect_x, canvas_height+5, 80, 180, 'v', "blue", 0);	// 柱底文字
		//strText=iList[(iLLoop-1)];
		//DrawText(oHistogram, "", 15, rect_x, rect_y-20, 50, 40, 'v', "blue", 0);			// 柱顶文字
