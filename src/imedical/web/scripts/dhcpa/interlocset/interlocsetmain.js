...
var InterLocSetTabProxy= new Ext.data.HttpProxy({url:InterLocSetTabUrl + '?action=list'});
...
//数据库数据模型
var InterLocSetTabCm = new Ext.grid.ColumnModel([
...
var addInterLocSet = new Ext.Toolbar.Button({
	...
	handler:function(){
				...
				specialKey :function(field,e){
					if (e.getKey() == Ext.EventObject.ENTER){
						if(orderField.getValue()!=""){
							...
							Ext.Msg.show({title:'提示',msg:'顺序不能为空!',buttons: Ext.Msg.OK,icon:Ext.MessageBox.INFO,fn:Handler});
						}
					}
				}
			}
		...
		addHandler = function(){
			...
			name = trim(name);
			if(code==""){
				Ext.Msg.show({title:'错误',msg:'接口套代码为空',buttons: Ext.Msg.OK,icon:Ext.MessageBox.ERROR});
				return;
			};
			if(name==""){
				Ext.Msg.show({title:'错误',msg:'接口套名称为空',buttons: Ext.Msg.OK,icon:Ext.MessageBox.ERROR});
				return;
			};
			//alert('code='+code+'&name='+name+'&order='+order+'&desc='+desc);
			Ext.Ajax.request({
				url: '../csp/dhc.pa.interlocsetexe.csp?action=add&code='+code+'&name='+encodeURIComponent(name)+'&order='+order+'&desc='+encodeURIComponent(desc),
				waitMsg:'保存中...',
				...
				success: function(result, request){
					...
					if (jsonData.success=='true'){
						...
						if(jsonData.info=='RepCode'){
							Handler = function(){codeField.focus();}
							Ext.Msg.show({title:'错误',msg:'此接口套代码已经存在!',buttons: Ext.Msg.OK,icon:Ext.MessageBox.ERROR,fn:Handler});
						}
						if(jsonData.info=='RepName'){
							Handler = function(){nameField.focus();}
							Ext.Msg.show({title:'错误',msg:'此接口套名称已经存在!',buttons: Ext.Msg.OK,icon:Ext.MessageBox.ERROR,fn:Handler});
						}
					}
				},
				scope: this
...
edit=function(){
		...
		editHandler = function(){
			...
			Ext.Ajax.request({
				url: '../csp/dhc.pa.interlocsetexe.csp?action=edit&rowid='+rowid+'&code='+code+'&name='+encodeURIComponent(name)+'&order='+order+'&desc='+encodeURIComponent(desc)+'&active='+active,
				waitMsg:'保存中...',
				failure: function(result, request){
					Handler = function(){codeField.focus();}
					Ext.Msg.show({title:'错误',msg:'请检查网络连接!',buttons: Ext.Msg.OK,icon:Ext.MessageBox.ERROR,fn:Handler});
				...
				success: function(result, request){
					...
					if (jsonData.success=='true'){
						...
						if(jsonData.info=='RepCode'){
							Handler = function(){codeField.focus();}
							Ext.Msg.show({title:'错误',msg:'接口套代码已经存在!',buttons: Ext.Msg.OK,icon:Ext.MessageBox.ERROR,fn:Handler});
						}
						if(jsonData.info=='RepName'){
							Handler = function(){nameField.focus();}
							Ext.Msg.show({title:'错误',msg:'接口套名称已经存在!',buttons: Ext.Msg.OK,icon:Ext.MessageBox.ERROR,fn:Handler});
						}
					}
				},
				scope: this
		...
		function handler(id){
			if(id=="yes"){
				...
				if(active=="Y"||active=="Yes"||active=="y"||active=="yes"){
					...
					Ext.Ajax.request({
						url:'../csp/dhc.pa.interlocsetexe.csp?action=del&rowid='+rowid,
						waitMsg:'删除中...',
						failure: function(result, request){
...
var InterLocSetSearchBox = new Ext.form.TwinTriggerField({
	...
	onTrigger1Click: function() {
		if(this.getValue()) {
			this.setValue('');    
			InterLocSetTabDs.proxy = new Ext.data.HttpProxy({url: InterLocSetTabUrl + '?action=list'});
			InterLocSetTabDs.load({params:{start:0, limit:InterLocSetTabPagingToolbar.pageSize}});
		}
	},
	onTrigger2Click: function() {
		if(this.getValue()) {
			InterLocSetTabDs.proxy = new Ext.data.HttpProxy({
				url: InterLocSetTabUrl + '?action=list&searchField=' + InterLocSetSearchField + '&searchValue=' + this.getValue()});
	        	InterLocSetTabDs.load({params:{start:0, limit:InterLocSetTabPagingToolbar.pageSize}});
	    	}
	}
});
