var personUrl = '../csp/dhc.pa.jxinpersonexe.csp';
var personProxy;
		...
		if(len < 1){
			...
			Ext.MessageBox.confirm('提示','确定要删除选定的行?',
				function(btn) {
					if(btn == 'yes'){
						Ext.Ajax.request({
							url:'../csp/dhc.pa.jxinpersonexe.csp?action=del&rowid='+rowObj[0].get("rowid"),
							waitMsg:'删除中...',
...
var personFilterItem = new Ext.Toolbar.MenuButton({
	...
	menu: {items: [
		...
		new Ext.menu.CheckItem({ text: '名称',value: 'userName',checked: false,group: 'personFilter',checkHandler: onCheck })
	]}
});
	...
	listeners: {
		...
		onTrigger2Click: function() {
			if(this.getValue()) {
				...
				url: personUrl+'?action=personlist&jxUnitDr='+JXUnitGrid.getSelections()[0].get("rowid")+'&searchField='+personSearchField+'&searchValue='+encodeURIComponent(this.getValue())+'&sort=rowid&dir=asc'});
				personDs.load({params:{start:0,limit:personPagingToolbar.pageSize}});
			}
		}
	});
