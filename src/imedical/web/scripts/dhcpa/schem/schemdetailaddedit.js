...
updatedetailaddFun = function(node){
	...
	if(node==null){
		...
		if((node.attributes.method=="扣分法")||(node.attributes.method=="加分法")){
			...
			var BaseVField = new Ext.form.TextField({
				id:'BaseVField',
				fieldLabel: '基础值设置',
				//allowBlank: false,
				width:180,
			...
			formPanel.on('afterlayout', function(panel,layout) {
				...
				BaseVField.setValue(node.attributes.base);
			});	
			...
			editHandler = function(){
				...
				var  baseValue = BaseVField.getValue();
				name = trim(name);	
                changeValue = trim(changeValue);
				base = trim(base);
				baseValue = trim(baseValue);
				method = trim(method);
                var data = method+'^'+name+'^'+changeValue+'^'+base+'^'+baseValue;
