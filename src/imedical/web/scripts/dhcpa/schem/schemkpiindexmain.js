...
var addButton=new Ext.Toolbar.Button({
	...
	iconCls: 'add',
	handler:function(){
		var seleObj=SchemGrid.getSelectionModel().getSelections();
	    if(seleObj==""){
			Ext.Msg.show({title:'提示',msg:'请选择要制定的方案!',buttons:Ext.Msg.OK,icon:Ext.MessageBox.INFO});
			return false;
		}else{
			var isStop=seleObj[0].data.isStop
			if(isStop=='N'){ //停用
				Ext.Msg.show({title:'提示',msg:'该方案已经停用不能添加指标！',buttons:Ext.Msg.OK,icon:Ext.MessageBox.INFO});
				return false;
			}else{
			//	console.log(seleObj[0].data.isStop);
