...
var SchemDs = new Ext.data.Store({
	...
	reader: new Ext.data.JsonReader({
		...
		totalProperty: 'results'
            ...
            'rowid','parRef','code','name','shortcut','appSysDr','frequency','KPIDr','KPIName','desc','level','appSys','quency', 'computeTypeDr','computeTypeName','upschemDr','upschemName','schemFlag','schemType','isStop'
...
var SchemCm = new Ext.grid.ColumnModel([
		...
		renderer : function(v, p, record){
			...
			return (v=='N'?'<span style="color:red;">是</span>':'否');
		}
	}
...
var SchemFilterItem = new Ext.SplitButton({
		...
		menu: {items: [
				...
				new Ext.menu.CheckItem({ text: '停用',value: 'active',checked: false,group: 'SchemFilter',checkHandler: onSchemItemCheck })
		]}
});
...
var SchemSearchBox = new Ext.form.TwinTriggerField({//查找按钮
		...
		onTrigger2Click: function() {
			if(this.getValue()) {
				if(SchemSearchField=="active"){
					var searchValue=this.getValue()=='是'?'Y':'N';
				}else{
					var searchValue=this.getValue();
				}
			
				SchemDs.proxy = new Ext.data.HttpProxy({
				url: SchemUrl + '?action=list&searchField=' + SchemSearchField + '&searchValue=' + encodeURIComponent(searchValue)});
				SchemDs.load({params:{start:0, limit:SchemPagingToolbar.pageSize}});
	    	}
		}
});
...
var stopButton = new Ext.Toolbar.Button({
		...
		//tooltip: '删除选择的方案',
		iconCls: 'remove',
		handler: function(){stopSchemFun(SchemDs,SchemGrid,SchemPagingToolbar,"stop");
		}
});
var actButton = new Ext.Toolbar.Button({
		text: '启用',
		//tooltip: '删除选择的方案',
		iconCls: 'add',
		handler: function(){stopSchemFun(SchemDs,SchemGrid,SchemPagingToolbar,"active");
		}
});
*/
var SchemGrid = new Ext.grid.EditorGridPanel({//表格
		...
		//clicksToEdit: 2,
		stripeRows: true, 
        tbar:[addAdjustButton,'-',editAdjustButton],		
