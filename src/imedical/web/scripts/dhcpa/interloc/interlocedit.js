editInterLocFun = function(dataStore,grid,pagingTool) {
	...
	ds.proxy=new Ext.data.HttpProxy({
		url:InterLocUrl+'?action=getjxunit&searchField=shortcut&searchValue='+encodeURIComponent(jxUnitField.getRawValue()),method:'POST'})
	});
    ...
    buttons: [{
		...
		handler: function() {
      	// check form value
			...
			//var data = code+'^'+name+'^'+order+'^'+jxUnit+'^'+patType+'^'+interLocSetField.getValue()+'^'+remark+'^'+tieOff;
...
//      		};
        	...
        	if (formPanel.form.isValid()) {
						Ext.Ajax.request({
							url: InterLocUrl+'?action=edit&data='+encodeURIComponent(data)+'&rowid='+myRowid,
