/**
  ...
  *author:wang ying
  *Date:2011-11-13
  *绩效考核在系统指标数据excel文件上传功能
 */
var importHZJYY = function(){
	var data2="";
	var freStore="";
	...
	var formPanel = new Ext.form.FormPanel({
		...
		//applyTo:'form',
		items: [cycle,periodType,period]
		//buttons:[{text:'导入海总数据',handler:handler}]
	});
	...
	var handler = function(bt){
		if(bt=="yes"){
			function callback(id){
				if(id=="yes"){
					var cycleName=Ext.getCmp('cycle').getRawValue();
			if(cycleName==""){
		...
		Ext.MessageBox.confirm('提示','确定要导入系统数据吗?',callback);
	} 
