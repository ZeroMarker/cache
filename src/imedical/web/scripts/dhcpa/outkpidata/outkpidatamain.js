...
Ext.onReady(function() {  
    ...
    Ext.Ajax.timeout = 99999999999999999;  
   });
...
var kpiTypeStore = new Ext.data.SimpleStore({
	fields: ['key','keyValue'],
	data:[['1','全部指标'],['2','工作量指标'],['3','抗生素使用指标'],['4','其他指标']]
});
var kpiTypeField = new Ext.form.ComboBox({
	id: 'kpiTypeField',
	fieldLabel: '指标类型',
	selectOnFocus: true,
	width:100,
...
//////////////////////////////////////////////////////////
function callbackImport(cycleDr,periodType,period,kpiType,locSetDr,locSetName){
		
		Ext.MessageBox.confirm('提示','导入前请确认接口导入的各项配置，继续导入该条件的指标数据吗?',
    	    function(btn){
	    	    if(btn == 'yes'){
					...
					Ext.Ajax.request({
						url: OutKPIDataUrl+'?action=import&CycleDr='+cycleDr+'&frequency='+periodType+'&period='+period+'&locSetDr='+locSetDr+'&kpiType='+kpiType,
						waitMsg:'导入中...',
						failure: function(result, request){
							loadMask.hide(); 

							Ext.Msg.show({title:'提示',msg:'数据正在导入，请稍后查看!',buttons: Ext.Msg.OK,icon:Ext.MessageBox.INFO});
						},
						...
						loadMask.hide(); 
							...
							if (jsonData.success=='true'){
								...
								if(jsonData.info=="noData"){
									Ext.Msg.show({title:'提示',msg:'指标数据导入失败请检查!',buttons:Ext.Msg.OK,icon:Ext.MessageBox.ERROR});
									}
...
var importButton = new Ext.Toolbar.Button({
	...
	handler: function(){
		...
		var kpiType =Ext.getCmp('kpiTypeField').getValue();
		if(kpiType ==""){
			Ext.Msg.show({title:'提示',msg:'请选择指标类型!',buttons: Ext.Msg.OK,icon:Ext.MessageBox.INFO});
			return false;
		}
		...
		Ext.Ajax.request({
			url:OutKPIDataUrl+'?action=isExistPeriod&CycleDr='+cycleDr+'&frequency='+periodType+'&period='+period,
			failure: function(result, request){
				Ext.Msg.show({title:'提示',msg:'网络异常，确认导入期间的请求失败!',buttons: Ext.Msg.OK,icon:Ext.MessageBox.INFO});
			},
			success: function(result,request){
				var jsonData = Ext.util.JSON.decode( result.responseText );
				if(jsonData.success =="false"){
					if (jsonData.info=='UnCycleCode'){
						Ext.Msg.show({title:'错误',msg:'该年度未维护，请在考核周期中维护!',buttons: Ext.Msg.OK,icon:Ext.MessageBox.ERROR});
					}else{
						Ext.Msg.show({title:'错误',msg:'请在接口期间界面中维护对应的期间数据!',buttons: Ext.Msg.OK,icon:Ext.MessageBox.ERROR});
					}
				}
				if(jsonData.success =="true"){
					
					callbackImport(cycleDr,periodType,period,kpiType,locSetDr,locSetName);	
				}
				},
			scope: this
		});
...
var refreshButton = new Ext.Toolbar.Button({
	...
	iconCls: 'add',
	handler: function(){
		refreshFun(OutKPIDataDs,OutKPIDataMain,OutKPIDataPagingToolbar);
	}
});
...
var OutKPIDataFilterItem = new Ext.SplitButton({
		...
		menu: {items: [
				...
				new Ext.menu.CheckItem({ text: '外部单位名称',value: 'outUnitName',checked: false,group: 'OutKPIDataFilter',checkHandler: onOutKPIDataItemCheck }),
				new Ext.menu.CheckItem({ text: '绩效单元代码',value: 'outUnitLocCode',checked: false,group: 'OutKPIDataFilter',checkHandler: onOutKPIDataItemCheck }),
				new Ext.menu.CheckItem({ text: '绩效单元名称',value: 'outUnitLocName',checked: false,group: 'OutKPIDataFilter',checkHandler: onOutKPIDataItemCheck }),
				...
				new Ext.menu.CheckItem({ text: '绩效指标代码',value: 'kpiCode',checked: false,group: 'OutKPIDataFilter',checkHandler: onOutKPIDataItemCheck }),
				new Ext.menu.CheckItem({ text: '绩效指标名称',value: 'kpiName',checked: false,group: 'OutKPIDataFilter',checkHandler: onOutKPIDataItemCheck }),
...
var OutKPIDataMain = new Ext.grid.GridPanel({//表格
		...
		clicksToEdit: 2,
		stripeRows: true,  
		tbar: ['接口套','-',locSetField,'-','年度:',cycleField,'-','期间类型:',periodTypeField,'-','期间:',periodField,'-','指标类型:',kpiTypeField,'-',importButton,'-',refreshButton /*,'-',delButton*/],
		bbar: OutKPIDataPagingToolbar

});
locSetField.on('select', function(cmb,rec,id){
	OutKPIDataDs.proxy=new Ext.data.HttpProxy({
	url:OutKPIDataUrl+'?action=list&locSetDr='+locSetField.getValue()+'&CycleDr='+cycleField.getValue()+'&frequency='+periodTypeField.getValue()+'&period='+periodField.getValue(),method:'POST'
	});
	OutKPIDataDs.load({params:{start:0, limit:OutKPIDataPagingToolbar.pageSize}});
});
...
periodField.on('select', function(cmb,rec,id){
	OutKPIDataDs.proxy=new Ext.data.HttpProxy({
	url:OutKPIDataUrl+'?action=list&CycleDr='+cycleField.getValue()+'&frequency='+periodTypeField.getValue()+'&period='+periodField.getValue()+'&locSetDr='+locSetField.getValue(),method:'POST'
	});
	OutKPIDataDs.load({params:{start:0, limit:OutKPIDataPagingToolbar.pageSize}});
});
////
