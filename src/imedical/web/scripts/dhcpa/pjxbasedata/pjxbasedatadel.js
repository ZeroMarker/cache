	...
	else
    {
    	Ext.MessageBox.confirm('提示',
    	    ...
    	    function(btn) {
		         ...
		         {
					          Ext.Ajax.request({
					              url: PJXBaseDataUrl+'?action=del&CycleDr='+cycleField.getValue()+'&frequency='+periodTypeField.getValue()+'&period='+periodField.getValue(),
												waitMsg:'删除中...',
												...
												success: function(result, request) {
						                          ...
						                          if (jsonData.success=='true') {
													...
													if (jsonData.info=='100') {
													       Ext.MessageBox.alert('提示', '没有可删除的数据');
														   dataStore.load({params:{start:pagingTool.cursor, limit:pagingTool.pageSize}});	
														}
									    			else {
