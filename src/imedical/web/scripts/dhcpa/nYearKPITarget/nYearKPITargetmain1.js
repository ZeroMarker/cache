...
var addDimensType = new Ext.Toolbar.Button({
	...
	handler:function(){
		...
		addHandler = function(){
			...
			Ext.Ajax.request({
				url: '../csp/dhc.pa.dimenstypeexe.csp?action=add&code='+code+'&name='+name+'&order='+order+'&appSysDr='+appSysDr+'&desc='+desc,
				waitMsg:'保存中...',
...
var editDimensType = new Ext.Toolbar.Button({
	...
	handler:function(){
		...
		editHandler = function(){
			...
			Ext.Ajax.request({
				url: '../csp/dhc.pa.dimenstypeexe.csp?action=edit&rowid='+rowid+'&code='+code+'&name='+name+'&order='+order+'&appSysDr='+appSysDr+'&desc='+desc+'&isInner='+isInner+'&active='+active,
				waitMsg:'保存中...',
				failure: function(result, request){
					Handler = function(){codeField.focus();}
					Ext.Msg.show({title:'错误',msg:'请检查网络连接!',buttons: Ext.Msg.OK,icon:Ext.MessageBox.ERROR,fn:Handler});
