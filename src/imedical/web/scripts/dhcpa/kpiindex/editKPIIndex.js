	...
	if(node==null){
		...
		if(node.attributes.id=="roo"){
			...
			CalUnitDs.on('beforeload', function(ds, o){
				ds.proxy=new Ext.data.HttpProxy({
					url:'../csp/dhc.pa.kpiindexexe.csp?action=calunit&str='+encodeURIComponent(Ext.getCmp('CalUnitField').getRawValue()),method:'POST'})
			});
...
//=====================================================================
			...
			ColDeptDs.on('beforeload', function(ds, o){
				ds.proxy=new Ext.data.HttpProxy({
					url:'../csp/dhc.pa.kpiindexexe.csp?action=coldept&str='+encodeURIComponent(Ext.getCmp('ColDeptField').getRawValue()),method:'POST'})
			});
			...
			*/	
			...
			var isKPI = new Ext.form.Checkbox({
			 ...
			 if(node.attributes.isEnd=='Y'){
                           ...
                           // DimTypeField.disable();
							ScoreMethodField.disable();
							//alert(ScoreMethodField.getValue());
							//scoreMethodField.setValue("");
							CalUnitField.disable();
							ExtreMumField.disable();
							ColDeptField.disable();
							DataSourceField.disable();
			...
			editHandler = function(){
				...
				if(IsEnd=="Y"){
					...
					if(scoreMethodCode==""){
						Ext.Msg.show({title:'提示',msg:'评分方法为空!',buttons: Ext.Msg.OK,icon:Ext.MessageBox.ERROR,width:200});
						return false;
					};

					calUnitDr = trim(calUnitDr);
					if(calUnitDr==""){
						Ext.Msg.show({title:'提示',msg:'计量单位为空!',buttons: Ext.Msg.OK,icon:Ext.MessageBox.ERROR,width:200});
						return false;
					};

					extreMum = trim(extreMum);
						if(extreMum==""){
							Ext.Msg.show({title:'提示',msg:'极值为空!',buttons: Ext.Msg.OK,icon:Ext.MessageBox.ERROR,width:200});
							return false;
						};
				...
				code = trim(code);
				if(name==""){
					Ext.Msg.show({title:'提示',msg:'指标名称为空!',buttons: Ext.Msg.OK,icon:Ext.MessageBox.ERROR,width:200});
					return false;
				};
				...
				Ext.Ajax.request({
					url: '../csp/dhc.pa.kpiindexexe.csp?action=edit&data='+encodeURIComponent(data)+'&rowid='+rowid,
					waitMsg: '修改中...',
					...
					success: function(result, request) {
						...
						if (jsonData.success=='true') {
							Ext.Msg.show({title:'提示',msg:'修改成功!',buttons: Ext.Msg.OK,icon:Ext.MessageBox.INFO,width:200});
							globalStr3="";
							...
							if(jsonData.info=='RepCode') message='指标代码重复!';
							if(jsonData.info=='dataEmpt') message='空数据!';
							if(jsonData.info=='rowidEmpt') message='数据有错!';
							Ext.Msg.show({title:'错误',msg:message,buttons: Ext.Msg.OK,icon:Ext.MessageBox.ERROR,width:200});
						}
