...
schemDs.on('beforeload', function(ds, o){
		...
		ds.proxy=new Ext.data.HttpProxy({url:'../csp/dhc.pa.paauditexe.csp?action=schem2&str='+Ext.getCmp('schem').getRawValue()+'&active=Y'+'&periodType='+Ext.getCmp('periodTypeField').getValue()+'&userCode='+userCode,method:'POST'})
...
function searchFun(periodType){
	...
	alert("1111");
	schemDs.proxy=new Ext.data.HttpProxy({url:'../csp/dhc.pa.paauditexe.csp?action=schem2&str='+Ext.getCmp('schem').getRawValue()+'&active=Y'+'&periodType='+Ext.getCmp('periodTypeField').getValue()+'&userCode='+userCode,method:'POST'})
	schemDs.load({params:{start:0, limit:schem.pageSize}});
	
	
};
//setAndLoadComboValueFromServer(schemDs,schem);
