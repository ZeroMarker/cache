...
function WrtOtherDOC(){
	var encmeth=DHCWebD_GetObjValue("RepOthEncmeth");
	var RepID=DHCWebD_GetObjValue("RepID");
	var myExpStr="";
	
	if (encmeth!=""){
		var rtnvalue=(cspRunServerMethod(encmeth,RepID, myExpStr)) 
		if (rtnvalue!=""){
			DHCWebD_AutoSetDocValue(rtnvalue,String.fromCharCode(2),"^");
		}
	}
}
...
function bFinRepTotal_OnClick(){
	...
	var NewWin=open(lnk,"DHCOPFin_Reps_BJJSTRepPrint","scrollbars=yes,resizable=yes,top=30,left=30,width=900,height=660");
...
function WrtDoc(rtnary){
	...
	if ((rtnary[33]=="")){rtnary[33]=0;}
	if ((rtnary[34]=="")){rtnary[34]=0}
	...
	////BankInSum
	DHCWebD_SetObjValueB("BankInSum",rtnary[62]);
	
	////BankCardNum
	DHCWebD_SetObjValueB("BankCardNum",rtnary[63]);
	
	///BankCardSum
	DHCWebD_SetObjValueB("BankCardSum",rtnary[64]);
	///PRDBankCardSum
	DHCWebD_SetObjValueB("PRDBankCardSum",rtnary[65]);
}
