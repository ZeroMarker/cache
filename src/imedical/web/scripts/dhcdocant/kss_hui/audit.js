...
$(function(){
	...
	$("#i-audit-condition-btn-undo").on("click", function () {
		...
		for (var i = 0; i < auditSelectedRows.length; i++) {
			...
			if (preAudit) {
				/*
					1、如果后面有审核的，不让其进行撤销。即：哪一步的撤销，必须由哪一步完成。
					2、
	...
	*/
	...
	function SetDefaultAuthType () {
		...
		if (auditType == "U") {
			auditValue = "0,0,1,0";
			$('#i-audit-condition-lastaudit').radio('check');	//8.20HISUI更新不再支持iCheck
		} else if (auditType == "S") {
			auditValue = "0,1,0,0";
	...
	function sendMsgByType(applyType, applyStatus, applyProcess, action) {
		...
		if ( applyType == "KS" ) {
			...
			if ( pS ) {
				...
				resultObj.value = 1;
				resultObj.msg = "流程不包含科室预审，不能进行科室审核！";
		...
		if ( applyType == "LAST" ) {
			if (( applyStatus == "U")||(( applyStatus == "R"))) {
				resultObj.value = 1;
				resultObj.msg = "已经审核完成的，不能"+ acName + "！";
				return resultObj;
			};
			if ( pU ) {
				resultObj.msg = "还没有完成上个流程，不能"+ acName + "！";
				if (pS) {
				...
				resultObj.value = 1;
				resultObj.msg = "流程不包含最终审核，不能进行最终审核和拒绝操作！";
				resultObj.icon = "warning";
			}
			return resultObj;
		};
