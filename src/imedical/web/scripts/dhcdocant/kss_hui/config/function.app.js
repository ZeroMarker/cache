...
$(function(){
	var products = ANT.DHC.getProcessDep(),
		...
		//authTop = 0,//$("#i-auth-container").offset().top,
		//authHeight = bodyHeight - authTop - 19,
		...
		ANT.preventRightClick(".c-book");
		/*if (browserType == "IE") {
			$("#i-auth-container").css("height", authHeight );
			alert(authHeight);
		} else if (browserType == "IE8") {
			$("#i-auth-container").css("height", authHeight - 21 );
		} else {
			$("#i-auth-container").css("height", '300px;' ).css('background','red');
		};*/
	$("#item").on("click",function(){
		layer.alert("暂未开放，请等待下一版本，谢谢...", {title:'提示',icon: 0}); 
		return false;
		...
		if ($("#nav").tree('isLeaf', node.target)) {
			$.messager.alert('提示', '不能添加子分类','info');
			return false;
		};
		if (node&&!$("#nav").tree('isLeaf', node.target)){
			if($('#i-catagory').hasClass("c-hidden")) {
				$('#i-catagory').removeClass("c-hidden");
			};
			$('#i-catagory').window({
				title: '添加分类',
				modal: true,
	...
	$("#item-a").on('click', function(){
		...
		if (!$("#nav").tree('isLeaf', node.target)) {
			$.messager.alert('提示', '不能给大分类添加配置项','info');
			return false;
		};
	...
	$("#i-catagory-btn").on('click', function(){
		var code = $("#i-catagory input[name='code']").val();
		var desc = $("#i-catagory input[name='desc']").val();
		var node = $("#nav").tree('getSelected');
		
	});
	
	$("#i-config-btn").on('click', function(){
		var code = $("#i-config input[name='code']").val();
		var desc = $("#i-config input[name='desc']").val();
		var value = $("#i-config input[name='value']").val();
		var node = $("#nav").tree('getSelected');
	});
	...
	$("#l-panel").panel({
		...
		tools: [{   
			...
			handler:function(){
				$("#i-para-layout").layout('collapse','west');
			}   
		  }]
	...
	$('#i-tab-sys').simpledatagrid({
		...
		columns:[[
			...
			{field:'ParentCode',title:'父级代码',width:100},
			{field:'Code',title:'配置代码',width:100},
			{field:'Mcgdesc',title:'配置描述',width:200},
	...
	$('#i-tab-process').simpledatagrid({
		...
		columns:[[
			{field:'ParentCode',title:'流程类型代码',width:100},
			{field:'ParentDesc',title:'流程类型描述',width:130},
			{field:'MCGType',title:'配置类型代码',width:100},
			...
			{field:'id',title:'id',width:100,hidden:true},
			{field:'action',title:'动作',width:70,align:'center',
				formatter:function(value,row,index){
					var listId = "#i-tab-process";
					if (row.editing){
						var s = '<a href="#" onclick="ANT.saveGridRow(this,' + "'" + listId + "'" + ')">保存</a>&nbsp;|&nbsp;';
						var c = '<a href="#" onclick="ANT.cancelGridRow(this,' + "'" + listId + "'" + ')">取消</a>';
						return s + c;
					} else {
						var e = '<a href="#" onclick="ANT.editGridRow(this,' + "'" + listId + "'" + ')">编辑</a> ';
						return e;
					}
				}
			}
		...
		onAfterEdit:function(index,row){
			...
			if ($.trim(selected.CodeU) == "1") {
				if (($.trim(selected.ULocID) == "")||(!selected.ULocID)) {
					layer.alert("流程中包含最终审核，必须要维护最终审核科室，谢谢！", {title:'提示',icon: 0}); 
			...
			if (rtn == "0") {
				layer.alert("保存成功...", {title:'恭喜',icon: 1}); 
			} else {
