...
function BClear_click() {
	...
	$('#NewAmount,#BackAmount').val("");
	$("#PayedAmount0,#RefundAmount0,#No0,#NewPayMode0,#PayedAmount1,#RefundAmount1,#No1,#NewPayMode1,#Balance,#InvName,#TaxpayerNum").val("");
	$("#PayMode1").combogrid("setValue", "");
	$("#PayMode0").combogrid("setValue", "");
...
function BGREPrint_Click() {
	...
	if (obj && "" != obj.value) {
		...
		var PrintFlag = tkMakeServerCall("web.DHCPE.Cashier", "GetPrintFlag", PEINVDR)
		if (PrintFlag == "1") {
			$.messager.alert("提示", "发票号为虚拟发票号,不能打印!", "info");
			return;
		}
	...
	var TxtInfo = tkMakeServerCall("web.DHCPE.Cashier", "GetInvoiceInfo", peAdmType, PEINVDR, "INV", UserID)
...
function GetPayModeInfo(IsAllRefund) {
	...
	for (i = 0; i < paymodecount; i++) {
		...
		newInvAmount = newInvAmount.toFixed(2);
	...
	if (backtotal != total) {
		$.messager.alert("提示", "请输入正确的退款金额!", "info");
		return -1;
	}
...
function Refund(IsReprint, IsAllRefund) {
	...
	var flag = tkMakeServerCall("web.DHCPE.ItemFeeList", "DrugPermControl", InvPrtId, "1", InvPrtId, locId, userId);
	if (IsReprint == 0) {
		if (flag == "1") {
		...
		var flag = tkMakeServerCall("web.DHCPE.ItemFeeList", "IsExsitItemExcut", InvPrtId);
		if (flag == "1") {
			$.messager.alert("提示", "存在已执行的医嘱,不能退费!", "info");
			return false;
		}
	...
	if (CardFlag == "1") {
		...
		var CardTypeID = $("#CardTypeRowID").val();
		if (CardTypeID == "1") {
			var iCardNo = $("#CardNo").val();

			var myrtn = DHCACC_GetAccInfo(CardTypeID, iCardNo, "");
	...
	if (tmp[0] == "") {
		...
		if (ret != "" && IsAllRefund == "0") {
			...
			var NewInsuID = ""
			if (InsuFlag == "1") {
				var insuRtn = $PEPay.insurancePay(tmp[2], "", userId, AdmSorce, AdmReason);  //DHCPEPayService.js
				...
				var extRtn = $PEPay.extPay(tmp[2], userId, NewInsuID, AdmSorce, AdmReason);
			...
			var newInvNo = tkMakeServerCall("web.DHCPE.DHCPEPAY", "getcurinvno", userId, "N", locId);
			var ret = tkMakeServerCall("web.DHCPE.Cashier", "RealCashier", tmp[2], userId, newInvNo.split("^")[0], locId);
			var tmp = ret.split("^");
			if (tmp[0] != "") {
			...
			var IfPrintMinusInvFlag = tkMakeServerCall("web.DHCPE.HISUICommon", "IfPrintMinusInvFlag", locId);
			if (IfPrintMinusInvFlag == "Y") {
				PrintMinusBill(tmp[1], tmp[2], tmp[3]);
			}
			PrintBill(tmp[1], tmp[2], tmp[3]);
		}
		var rfdRtn = $PEPay.extRefund("", InvPrtId);  //DHCPEPayService.js
...
function PrintBill(invprtid, relInvPrtID, PrintListFlag) {
	...
	var TxtInfo = tkMakeServerCall("web.DHCPE.Cashier", "GetInvoiceInfo", peAdmType, invprtid, "INV", UserID);
...
function PrintMinusBill(invprtid, relInvPrtID, PrintListFlag) {
	...
	var TxtInfo = tkMakeServerCall("web.DHCPE.Cashier", "GetInvoiceInfo", peAdmType, MunusInv, "INV", UserID);
...
function BPrintList_click() {
	...
	var listFlag = GetListFlag(peAdmType);
	var TxtInfo = tkMakeServerCall("web.DHCPE.Cashier", "GetInvoiceInfo", peAdmType, PEINVDR, "List", UserID);
...
function InitInvInfo() {
	...
	var RefundFlag=tkMakeServerCall("web.DHCPE.ItemFeeList","GetApplyRefundFlag",INVID);
	...
	for (var I = 0; I < paymodecount; I++) {
		...
		//退费支付方式-重新加载
	    var NewPayModeUrl=$URL+"?ClassName=web.DHCPE.CT.HISUICommon&QueryName=FindRefundPayMode&ResultSetType=array&LocID="+LocID+"&PayModeID="+paymode[0];
        $('#NewPayMode'+I).combobox('reload',NewPayModeUrl);
        $('#NewPayMode'+I).combobox("setValue",paymode[0]); //退费支付方式赋值
 
		if ((paymode[1] == "体检预交金") || (paymode[1] == "体检代金卡")) {
			$("#NewPayMode0").combobox("disable");
			$("#No0").attr("disabled",true);
		}
		else {
			$("#NewPayMode0").combobox("enable");
			$("#No0").attr("disabled",false);

		}
		$("#PayedAmount" + I).val(paymode[2].split("元")[0]);
		$("#No" + I).val(paymode[3]);

		if(RefundFlag=="0"){ //全退
        	$("#RefundAmount"+I).val(paymode[2].split("元")[0]);
		}
...
function AddNewRow(index) {
	...
	$HUI.combogrid("#PayMode"+index,{
		panelWidth:220,
		url:$URL+"?ClassName=web.DHCPE.CT.HISUICommon&QueryName=FindTJPayMode&LocID="+LocID,
		mode:'remote',
		delay:200,
		idField:'id',
		textField:'desc',
		...
		columns:[[
			...
			{field:'desc',title:'名称',width:110}
		]]
		});
	
	//退费支付方式
	$HUI.combobox("#NewPayMode"+index,{
		url:$URL+"?ClassName=web.DHCPE.CT.HISUICommon&QueryName=FindRefundPayMode&ResultSetType=array&LocID="+LocID,
		valueField:'id',
		textField:'desc'
...
function InitChecked() {
	...
	var RefundFlag=tkMakeServerCall("web.DHCPE.ItemFeeList","GetApplyRefundFlag",invPrtId);
	if(RefundFlag=="0"){ //全退

	    var backAmount=tkMakeServerCall("web.DHCPE.DHCPEBillCharge","GetAmountByInv",invPrtId); //应退合计 
		
	}else if(RefundFlag=="1"){ //部分退费

    	var SelectIds=tkMakeServerCall("web.DHCPE.ItemFeeList","GetApplyRefund",invPrtId,1);
		setValueById("SelectIds",SelectIds);//设置SelectIds中的值
		if(SelectIds!=""){	
...
if(isApply=="2"){var ApplyUserHidden=true;}
if(isApply=="1"){var ApplyUserHidden=false;}
var columns = [
[
	...
	{field:'TApplyUser',title:'退费申请人',hidden:ApplyUserHidden},
...
function InitInvListQueryTabDataGrid() {
	...
	$HUI.datagrid("#InvListQueryTab", {
		...
		onSelect: function (rowIndex, rowData) {
			...
			$("#NewPayMode0,#NewPayMode1").combobox("setValue","");
			$("#PayMode0,#PayMode1").combogrid("setValue",""); 
			$("#PayedAmount0,#RefundAmount0,#No0").val("");
		    $("#PayedAmount1,#RefundAmount1,#No1").val("");
		    $("#Balance,#InvName,#TaxpayerNum").val("");
			$("#AdmReason,#YLLB").combobox("setValue","");
...
function InitCombobox() {
	...
	var PayModeObj = $HUI.combobox("#PayMode", {
		url: $URL + "?ClassName=web.DHCPE.CT.HISUICommon&QueryName=FindTJPayMode&ResultSetType=array&LocID=" + session['LOGON.CTLOCID'],
		valueField: 'id',
		textField: 'desc'
	...
	var PayMode0Obj = $HUI.combogrid("#PayMode0", {
		panelWidth: 220,
		url: $URL + "?ClassName=web.DHCPE.CT.HISUICommon&QueryName=FindTJPayMode&LocID=" + session['LOGON.CTLOCID'],
		mode: 'remote',
		delay: 200,
		idField: 'id',
		textField: 'desc',
