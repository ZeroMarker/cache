...
function reBillClick() {
	...
	function _rebill() {
		...
		if(rtn == 0) {
			...
			$.messager.alert("账单", "重新生成账单失败."+rtn,'info');
			return;
		}
	}
}
...
$('#BtnPrice').bind('click', function () {
	$.m({  
		ClassName: "BILL.PKG.BL.OrderIPPriceDivide",
		MethodName: "DividePrice",
		Adm:GV.ADM,
		User:PUBLIC_CONSTANT.SESSION.USERID,
		HospDr:PUBLIC_CONSTANT.SESSION.HOSPID,
		},function(rtn){
			if(rtn==='0'){
				$.messager.alert('提示','价格确认成功','info',function(){
					initLoadGrid();	
			});
			}else{
				$.messager.alert('提示','价格确认失败:'+rtn,'info');
			}
	})		
});
