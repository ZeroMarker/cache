/**
 * FileName: dhcbill.opbill.emergwarrant.js
 * Anchor: ZhYW
 * Date: 2018-07-11
 * Description: 门急诊担保
 */
	...
	$HUI.combogrid('#admList', {
		...
		columns: [[{field: 'adm', title: '就诊号', width: 60},
				   ...
				   {field: 'admDept', title: '就诊科室', width: 150,
					formatter: function(value, row, index) {
						if(row.admType == 'E') {
							return value + "<span class='icon icon-emergency'></span>";
						}else {
							return value + "<span class='icon icon-outpatient'></span>";
						}
					}
				}
			]],
...
function appListClick() {
	var url = 'dhcem.gua.guarantee.csp?&GrossClass=' + getValueById('GrossClass') + '&PatientID=' + getValueById('PatientID');
	websys_showModal({
		url: url,
		title: '申请列表',
		iconCls: 'icon-w-edit',
...
function selectRowHandler(row) {
	...
	setValueById('warrItem', row.warrItems);
