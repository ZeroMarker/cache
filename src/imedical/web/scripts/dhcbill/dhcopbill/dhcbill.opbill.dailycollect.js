/**
 * FileName: dhcbill.opbill.dailycollect.js
 * Anchor: ZhYW
 * Date: 2018-03-12
...
function initQueryMenu() {
	...
	$HUI.combobox("#userCombo", {
		...
		rowStyle: 'checkbox',
		multiple: true,
		url: $URL + "?ClassName=web.DHCOPBillDailyCollect&QueryName=FindOPCashier&ResultSetType=array",
		valueField: 'id',
		textField: 'text',
		defaultFilter: 4,
...
function initReceTree() {
	GV.ReceTree = $HUI.tree("#rece-tree", {
		fit: true,
		url: $URL + "?ClassName=web.DHCOPBillDailyCollect&MethodName=BuildReceiveTree&ResultSetType=array&guser=" + PUBLIC_CONSTANT.SESSION.USERID,
		animate: true,
		autoNodeHeight: true,
...
function receiveClick() {
	var iframe = $('#tabMain')[0].contentWindow.$('#iframe_opbillCollectList')[0].contentWindow;
	if (!iframe) {
		return;
	}
