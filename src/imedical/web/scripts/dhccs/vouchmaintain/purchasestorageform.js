purchasestorageformFun = function(grid, rowIndex, columnIndex, e ,busiTypeId) {
	
	//var busiTypeId = grid.getColumnModel().getDataIndex(4);
	var busiTypeName = grid.getColumnModel().getDataIndex(4);
	//alert(busiTypeName);
	...
	//var fieldWH = grid.getColumnModel().getDataIndex(5);
	
	var dataNO = selectedRec.get(fieldNO);
	var dataDate = selectedRec.get(fieldDate);
	var busiTypeName = selectedRec.get(busiTypeName);
	//var dataWH = selectedRec.get(fieldWH);
	
	dataNO=(dataNO==undefined)?"":dataNO;
	dataDate=(dataDate==undefined)?"":dataDate;
	busiTypeName=(busiTypeName==undefined)?"":busiTypeName;
	//dataWH=(dataWH==undefined)?"":dataWH;

	//trNo,busiTypeId
	
	var getDhcDataDetailST = new Ext.data.Store({
			proxy: new Ext.data.HttpProxy({url:'dhc.cs.autohisoutexe.csp?action=GetDhcDataDetailJson&trNo='+dataNO+'&busiTypeId='+busiTypeId}),
							...
							items: [
								{
									columnWidth:.9,
									xtype: 'displayfield', 
									value: '<font size="6px"><center>'+busiTypeName+'单</center></font>'
								}
							]
						},
						{
