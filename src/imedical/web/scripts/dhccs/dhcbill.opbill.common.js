...
function chargeErrorTip(flag, errCode, job) {
	...
	case 102:
		...
		break;
	case 103:
		$.messager.alert("提示", "重新生成账单错误：" + errCode, "error");
		break;
	case 104:
		$.messager.alert("提示", "取配置信息有误：" + errCode, "error");
		break;
	case 105:
		$.messager.alert("提示", "支付方式输入为空：" + errCode, "error");
		break;
	case 107:
		$.messager.alert("提示", "院内账户不存在：" + errCode, "error");
		break;
	case 109:
		$.messager.alert("提示", "收费员没有可用票据：" + errCode, "error");
		break;
	case 110:
		$.messager.alert("提示", "发药科室为空，请检查医嘱接收科室或发药窗口：" + errCode, "error");
		break;
	case 112:
		$.messager.alert("提示", "四舍五入舍入金额过大，请分批结算：" + errCode, "error");
		break;
	case 113:
		$.messager.alert("提示", "院内账户与患者身份不符：" + errCode, "error");
		break;
	case 114:
		$.messager.alert("提示", "医嘱被执行，不能退费：" + errCode, "error");
		break;
	case 120:
		$.messager.alert("提示", "有异常收费记录需要处理：" + errCode, "error");
		break;
	case 123:
		$.messager.alert("提示", "欠费结算异常：" + errCode, "error");
		break;
	case 125:
		$.messager.alert("提示", "账户余额不足：" + errCode, "error");
		break;
	case 129:
		$.messager.alert("提示", "减材料库存失败：" + errCode, "error");
		break;
	case 130:
		$.messager.alert("提示", "请选择全部的挂号相关费用进行结算：" + errCode, "error");
		break;
	case 131:
		$.messager.alert("提示", "患者结算需要附加条件：" + errCode, "error");
		break;
	case 132:
		$.messager.alert("提示", "同一个处方被拆分：" + errCode, "error");
		break;
	case 133:
		$.messager.alert("提示", "账单表金额不平：" + errCode, "error");
		break;
	case 134:
		$.messager.alert("提示", "发票表与账单表金额不平：" + errCode, "error");
		break;
	case 135:
		$.messager.alert("提示", "发票表自负金额与支付方式金额不平：" + errCode, "error");
		break;
	case 140:
		$.messager.alert("提示", "传入的支付方式未配置：" + errCode, "error");
		break;
	case 141:
		$.messager.alert("提示", "有收费医嘱数据不完整：" + errCode, "error");
		break;
	case 142:
		$.messager.alert("提示", "药品医嘱结算后药房中间表数据有误：" + errCode, "error");
		break;
	default:
