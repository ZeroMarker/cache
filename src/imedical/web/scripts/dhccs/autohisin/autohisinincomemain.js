...
function checkFun1(){
			...
			if((bdate==="")||(sdate==="")){
				...
				if(bdate>sdate){
					...
					Ext.Ajax.request({
						url:'dhc.cs.autohisoutexe.csp?action=IPIncomeHead&stDate='+bdate+'&endDate='+sdate+'&inOut=DHCOPB0011',
						waitMsg:'...',
						...
						success: function(result, request) {
							...
							var tmpStore = new Ext.data.Store({
								proxy:new Ext.data.HttpProxy({url:'dhc.cs.autohisoutexe.csp?action=IPIncomeJson&stDate='+bdate+'&endDate='+sdate+'&inOut=DHCOPB0011', method:'GET'}),
...
var genButton1 = new Ext.Toolbar.Button({
		...
		handler: function(){
				...
				//checkFun();
				Ext.Ajax.request({
					url: 'dhc.cs.autohisoutexe.csp?action=IOVouch&yearPeriodId=1&stDate='+bdate+'&endDate='+sdate+'&operator='+operator+'&inOut=DHCOPB0011',//////////////////////////////////////////////
					waitMsg:'保存中...',
					failure: function(result, request) {
						Ext.Msg.show({title:'错误',msg:'请检查网络连接!',buttons: Ext.Msg.OK,icon:Ext.MessageBox.ERROR});
					},
