...
function checkFun(){
			...
			if((bdate==="")||(sdate==="")){
				...
				if(bdate>sdate){
					...
					Ext.Ajax.request({
						url:'dhc.cs.autohisoutexe.csp?action=IPReportCatDetailByDateHead&stDate='+bdate+'&endDate='+sdate+'&inOut=DHCOPB001',
						waitMsg:'...',
						...
						success: function(result, request) {
							...
							tmpStore = new Ext.data.Store({
								proxy:new Ext.data.HttpProxy({url:'dhc.cs.autohisoutexe.csp?action=IPReportCatDetailByDateJson&stDate='+bdate+'&endDate='+sdate+'&inOut=DHCOPB001', method:'GET'}),
...
var genButton = new Ext.Toolbar.Button({
		...
		handler: function(){
			...
			//	var busiTypeId = businessTypeCB.getValue();                ////////////////////////////////////////////
				...
				//checkFun();
				Ext.Ajax.request({
					url: 'dhc.cs.autohisoutexe.csp?action=IOVouch&yearPeriodId=1&stDate='+bdate+'&endDate='+sdate+'&operator='+operator+'&inOut=DHCOPB001',//////////////////////////////////////////////
					waitMsg:'保存中...',
					failure: function(result, request) {
						Ext.Msg.show({title:'错误',msg:'请检查网络连接!',buttons: Ext.Msg.OK,icon:Ext.MessageBox.ERROR});
					},
