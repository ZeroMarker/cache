...
var formatItems=document.getElementById('HL7ResultStatus');
...
function Init() {
	...
	var obj=document.getElementById('HL7ResultStatus'); if (obj) obj.onchange=HL7RSExtResStatussChangeHandler;

	var obj=document.getElementById('HL7RSExternalSys'); if (obj) obj.onfocus=HL7RSExternalSysFocusHandler;
	var obj=document.getElementById('HL7RSExtResStatus'); if (obj) obj.onfocus=HL7RSExtResStatusFocusHandler;


    var obj=document.getElementById("HL7RSTrakResStatus");
    if (obj) {
        TRAKRES_CHANGE=obj.onchange;
        obj.onchange=TrakResChangeHandler;
    }

	//var obj=document.getElementById('HL7RSTrakResStatus'); if (obj) obj.onblur=HL7RSTrakResStatusBlurHandler;

	var obj=document.getElementById('RemoveExtSystem'); if (obj) obj.onclick=RemoveExtSystemClickHandler;

	var obj=document.getElementById('AddResultStatus'); if (obj) obj.onclick=AddResultStatusClickHandler;
	var obj=document.getElementById('RemoveResultStatus'); if (obj) obj.onclick=RemoveResultStatusClickHandler;
...
function HL7RSExternalSysLookUp(str) {
	var lu = str.split("^");
    //alert(lu);
	...
	if (lu[2]!="") {
		...
		for(i=0; i<extsysItems.length; i++) {
			if (extsysItems[i].innerText==lu[0]) {
				...
				// Clear LookUp.
				var obj=document.getElementById('HL7RSExternalSys');
				if (obj) obj.value="";
		...
		websys_setfocus("HL7RSExtResStatus");
	}
}
...
function HL7RSExtResStatussChangeHandler() {
	ResetFormatCodeBox();
...
function ResetFormatCodeBox() {
	var obj=document.getElementById('HL7RSExtResStatus');
	if (obj) {
		obj.value="";
		obj.className="";
	}
...
function AddResultStatusClickHandler() {
	var str="";
	var str1=new Array();
	var obj=document.getElementById('HL7RSExtResStatus');
	...
	if ((obj)&&(obj.disabled==false)&&(str!="")) {
		...
		if ((str.length < 4) || ((str.substring(0,2) != "\\X") && (str.substring(0,2) != "\\Z")) || (str.substring(str.length-1,str.length) != "\\")) {
			var cap=document.getElementById("cHL7RSExtResStatus");
		...
		*/
		...
		for(i=0; i<formatItems.length; i++) {
			if (formatItems[i].innerText==str) {
				...
				HL7RSExtResStatussChangeHandler();
				websys_setfocus('HL7ResultStatus');
				return false;
			}
		}
		...
		websys_setfocus("HL7RSTrakResStatus");
	}
}
...
function RemoveExtSystemClickHandler() {
	...
	if (j!=-1) {
		...
		websys_setfocus("HL7RSExternalSys");
	}
...
function RemoveResultStatusClickHandler() {
	var j=extsysItems.selectedIndex;
	var k=formatItems.selectedIndex;
	if ((j!=-1)&&(k!=-1)) {
...
function HL7RSExternalSysFocusHandler() {
	DisableFormatCodeProperties();
	DisableAsciiConProperties();
	ClearFormatCodeList();
...
function HL7RSExtResStatusFocusHandler() {
	DisableAsciiConProperties();
	formatItems.selectedIndex=-1;
}
...
function HL7RSExtResStatusChangeHandler() {
	var str="";
	var str1=new Array();
	var obj=document.getElementById('HL7RSExtResStatus');
	if (obj) str=obj.value();
...
function UpdateClickHandler() {
	...
	for(j=0; j<extsysAry.length; j++) {
		if (extsysformatAry[j].length==0) {
			alert(t['AtLeastOne'] + " '" + t['HL7RSExtResStatus'] + "' " + t['IsRequiredFor'] + " " + t['HL7RSExternalSys'] + " '" + extsysItems[j].innerText + "'");
			return false;
		}
	}
    ...
    for(j=0; j<extsysAry.length; j++) {
		for(k=0; k<extsysformatAry[j].length; k++) {
			if (extsysformatAry[j][k].asciiconv=="") {
				alert("'" + t['HL7RSTrakResStatus'] + "' " + t['IsMandatory'] + " " + t['HL7RSExternalSys'] + " '" + extsysItems[j].innerText + "', " + t['HL7RSExtResStatus'] + " '" + formatItems[k].innerText + "'");
				return false;
			}
		}
	}
...
function DisableFormatCodeProperties() {
	DisableField('HL7RSExtResStatus',1);
}

function EnableFormatCodeProperties() {
	EnableField('HL7RSExtResStatus');
}

function DisableAsciiConProperties() {
	DisableField('HL7RSTrakResStatus',1);
}

function EnableAsciiConProperties() {
	EnableField('HL7RSTrakResStatus');
}
