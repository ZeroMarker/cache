...
function BFoot_Click()
{
	...
	if (""!=UnFootUser)
	{
		...
		alert(alertInfo);
		var listobj=document.getElementById("TTMPJIDz"+2);
		if (!listobj)
		{
			alert(t['NoHandin']);
			return;
...
function BFind_Click()
{
	...
	var lnk = "websys.default.csp?WEBSYS.TCOMPONENT=UDHCJFOP.Footin9&TMPJID="+TMPJID+"&EndDate="+EndDate+"&StDate="+StDate+"&StartTime="+StartTime+"&EndTime="+EndTime+"&StatFlag="+StatFlag+"&BFootFlag="+BFootFlag;
    //alert(lnk)
...
function PrintClickHandler()
{
	try{
		...
		var Template=TemplatePath+"JFOP_BJYKYZLFootRep.xls";
		//alert(Template)
		...
		gridlist(xlsheet,xlsRow+1,xlsRow+i-1,2,Cols);
		
		//Other
		i++;
		xlsheet.cells(xlsRow+i,xlsCol+2)=t["zbr"];
		xlsheet.cells(xlsRow+i,xlsCol+3)=ZBR;
		xlsheet.cells(xlsRow+i,xlsCol+9)=t["shr"];
		xlsheet.cells(xlsRow+i,xlsCol+14)=t["bt"];
		xlsheet.cells(xlsRow+i,xlsCol+16)=" "+CurDate;
