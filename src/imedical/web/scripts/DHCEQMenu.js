...
{
	if (!CheckEquipDR()) return;
	var str='websys.default.hisui.csp?WEBSYS.TCOMPONENT=DHCEQUseRecordFind&EquipDR='+menuequipdr+"&ReadOnly="+GetElementValue("ReadOnly");  //modify by lmm 2018-09-02 hisui改造：修改hisui默认csp
	SetWindowSize(str,1,'890','680','','','',1)
    //window.open(str,'_blank','toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes,copyhistory=yes,width=890,height=680,left=120,top=0')
}
...
{
	if (!CheckEquipDR()) return;
	var str="websys.default.csp?WEBSYS.TCOMPONENT=DHCEQSerContract&EquipDR="+menuequipdr+"&ReadOnly="+GetElementValue("ReadOnly");
    window.open(str,'_blank','toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes,copyhistory=yes,width=890,height=650,left=120,top=0')
}
...
function MenuPayPlan()
{
	...
	var ReadOnly=GetElementValue("ReadOnly");
	if (GetElementValue("Status")!=0) ReadOnly=1;
	var str="websys.default.csp?WEBSYS.TCOMPONENT=DHCEQPayPlan&SourceType="+SourceType+"&SourceID="+SourceID+"&ReadOnly="+ReadOnly;
    window.open(str,'_blank','toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes,copyhistory=yes,width=880,height=580,left=120,top=0')
}
...
function ProductPicture()
{
	...
	var ReadOnly=0
	var Status=0;
	var encmeth=GetElementValue("GetProductID");
	if (encmeth=="") return;
	var result=cspRunServerMethod(encmeth,"1",menuequipdr);
	var list=result.split("^");
	if (list[0]==0)
	{
		var ProductID=list[1]
		if (ProductID=="") return;
		var str='dhceq.process.picturemenu.csp?&CurrentSourceType=96&CurrentSourceID='+ProductID+'&Status='+Status+'&ReadOnly='+ReadOnly;
		SetWindowSize(str,1,'890','500','','','',1)
		//window.open(str,'_blank','left='+ (screen.availWidth - 1150)/2 +',top='+ ((screen.availHeight>750)?(screen.availHeight-750)/2:0) +',width=1150,height=750,toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes,copyhistory=yes')
	}
}