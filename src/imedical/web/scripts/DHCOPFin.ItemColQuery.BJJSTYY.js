////DHCOPFin.ItemColQuery.BJJSTYY.js
...
function ReadFootSum(){
	...
	if (encmeth!=""){
		var myrtn=cspRunServerMethod(encmeth,myBDate, myEDate, myExpStr);
		...
		DHCWebD_SetObjValueB("YHSum",mystr[7])
		DHCWebD_SetObjValueB("YBPaySum",mystr[8])
		DHCWebD_SetObjValueB("RoundSum",mystr[9])
		///alert(mystr[5]);
		///
	}
}
...
function InitialCAT()
{
	...
	var objtbl=document.getElementById("tDHCOPFin_ItemColQuery_BJJSTYY");
...
function DelTabRow(){
	var TabObj=document.getElementById("tDHCOPFin_ItemColQuery_BJJSTYY");
...
function PrintClickHandler(){
	try {
	    ...
	    var Template=TemplatePath+"HospOPFinItemQueryBJJST.xls";
	    ...
	    ///xlsheet.cells(xlsrow,7+myItemLen)=t["YHHJ"];
	    ///xlsheet.cells(xlsrow,8+myItemLen)=t["YBTCSum"];
	    ...
	    for (var Row=1;Row<myRows;Row++){
			...
			for (var itmIdx=1;itmIdx<=myItemLen;itmIdx++){
			    ...
			    xlsheet.Cells(xlsrow,xlsCurcol+2+itmIdx).HorizontalAlignment=1
			    xlsheet.Cells(xlsrow,xlsCurcol+2+itmIdx).NumberFormatLocal = "0.00;0.00"
		    ...
		    xlsheet.Cells(xlsrow,xlsCurcol+3+myItemLen).HorizontalAlignment=1
		    xlsheet.Cells(xlsrow,xlsCurcol+3+myItemLen).NumberFormatLocal = "0.00;0.00"
			...
			///var listobj=document.getElementById("TYHSumz"+Row);
			///var myval=DHCWebD_GetCellValue(listobj);
			///xlsheet.cells(xlsrow,xlsCurcol+6+myItemLen)=myval;
			
			///var listobj=document.getElementById("TYBPaySumz"+Row);
			///var myval=DHCWebD_GetCellValue(listobj);
			///xlsheet.cells(xlsrow,xlsCurcol+7+myItemLen)=myval;
			
	    }
		
		//gridlist(xlsheet,5,xlsrow,2,xlsCurcol+5+myItemLen)
		gridlist(xlsheet,5,xlsrow,2,xlsCurcol+3+myItemLen)
		...
		xlsheet.cells(4,4)=myary[0];
	   	...
	   	xlsheet.cells(xlsrow+2,4)=t["AcCashSum"];
	   	var myval=DHCWebD_GetObjValue("AcCashSum");
	   	xlsheet.cells(xlsrow+2,5)=myval;
	   	...
	   	//xlsheet.cells(xlsrow+3,3)=t["YHHJ"];
	   	var myval=DHCWebD_GetObjValue("YHSum");
	   	//xlsheet.cells(xlsrow+3,4)=myval;
	   	//xlsheet.cells(xlsrow+3,5)=t["YBTCSum"];
	   	var myval=DHCWebD_GetObjValue("YBPaySum");
	   	//xlsheet.cells(xlsrow+3,6)=myval;
