...
function FilterType3_Clicked()
{
	...
	var fileName="DHCEQInventoryLossListExport.raq&DisplayType=0&InventoryPlanDR="+getElementValue("InventoryPlanDR")+"&InventoryDR="+getElementValue("InventoryDR");
	//alert(fileName)
	DHCCPM_RQPrint(fileName);
}
function FilterType4_Clicked()
{
	...
	var fileName="DHCEQInventoryLossListExport.raq&DisplayType=1&InventoryPlanDR="+getElementValue("InventoryPlanDR")+"&InventoryDR="+getElementValue("InventoryDR");
	//alert(fileName)
	DHCCPM_RQPrint(fileName);
}
...
function BLocSubmit_Clicked()
{
	...
	var jsonData=tkMakeServerCall("web.DHCEQ.EM.BUSInventory","LocInventory", getElementValue("InventoryDR"),curLocID, "1");  //Modefied by zc0130 2023-2-17 科室盘点确认
	jsonData=JSON.parse(jsonData)
	if (jsonData.SQLCODE!=0)
	{
...
function initInventoryLastDeal()
{
	...
	$HUI.datagrid("#tDHCEQInventoryLastDeal",{
		...
		toolbar:[{
				...
				,   //Modefied by zc0130 2023-2-17 添加盘点完成按钮 begin
				{
				id:'BAudit',
				iconCls:'icon-accept',
				text:'盘点完成',
				handler:function(){BAudit_Clicked()}
				}   //Modefied by zc0130 2023-2-17 添加盘点完成按钮 end
        ],
	    pagination:true,
		pageSize:15,
		pageNumber:1,
