...
//The following functions are generic functions for manipulating Data in List Boxes (SELECT) used in EPR to accommodate "^" delimeters
...
//Use when code/description delimited by "^" character CDD(comma description delimited)
function callAddItemToListCDD(obj,txtstr,valstr,fcs) {
...
//This function converts delimited strings to an array in preperation for sending to AddItemToList method.
//Use when only description delimited by "^" character CD(comma delimited)
function callAddItemToListCD(obj,txtstr,valstr,fcs) {
	var arytxt=txtstr.split("^");
	var aryval=valstr.split(",");
	var arytxtNew = new Array();
	var aryvalNew = new Array();
...
function TransferToList(obj,val) {
	var found=0
	ary=val.split("^")
	arytxt=ary[0].split("^");
	aryval=ary[1].split("^");
...
function loopArrayForMatches(aryallval,aryalltxt,aryval) {
	...
	var count=0
	for (var h=0;h<aryval.length;h++) {
		for (var i=0;i<aryallval.length;i++) {
			if (aryallval[i]==aryval[h]) {arytxt[count]=aryalltxt[i];count++}
		}
	}
	return arytxt;
}