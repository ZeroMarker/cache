...
function ShowResultHistory(e)
{
	...
	websys_lu(url,false,'width=1020,height=600,hisui=true,title=历次结果') 
...
function SavePEResult(e,CompleteFlag)
{
	...
	if($("#"+HISUIButtonID).text().indexOf("修改结果")!=-1)
	{
		...
		$("#"+HISUIButtonID).linkbutton({text:'保存结果'})
		return false;
	}
	...
	if (RetArr[0]==0){
		...
		$("#"+HISUIButtonID).linkbutton({text:'修改结果'})
		for (var i=0;i<ArrLength;i++)
		{
			if (ObjArr[i].type=='button') continue;
			ObjArr[i].readOnly=true;
