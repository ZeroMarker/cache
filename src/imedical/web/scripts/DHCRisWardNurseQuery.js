...
function BodyLoadHandler()
{
	...
	var GetWardNameFunction=document.getElementById("GetWardName").value;
 	var value=cspRunServerMethod(GetWardNameFunction,gLocID);
 	var Item=value.split("^");
 	
 	document.getElementById("WardName").value=Item[0];
 	document.getElementById("ward").value=Item[1];
	...
	var InRegNoObj=document.getElementById("InRegNo");
	if (InRegNoObj)
	{
		InRegNoObj.onkeydown=RegNo_keyDown;
	}
	
	var InStudyNoObj=document.getElementById("InStudyNo");
	if (InStudyNoObj)
	{
		InStudyNoObj.onkeydown=StudyNo_keyDown;
	}
	...
	var PrintAllobj=document.getElementById("PrintAll");
	if(PrintAllobj)
	{
		PrintAllobj.onclick=PrintClick;
	}
	
	var PrintSelObj=document.getElementById("PrintSelList");
	if(PrintSelObj)
	{
		PrintSelObj.onclick=PrintSelClick;
	}
	var IsAllCheckobj=document.getElementById("IsAllCheck");
	if (IsAllCheckobj)
	{
	   IsAllCheckobj.onclick=IsAllCheckClick;
	}
	
	

    var RisStatusCode=document.getElementById("RisStatusCode").value;
 	var GetStatusListStr=DHCC_GetElementData('GetPatStatusList');
 	
 
	var RisStatusObj=document.getElementById("RisStatus");
	if(RisStatusObj)
	{
		combo_Use=dhtmlXComboFromStr("RisStatus",GetStatusListStr);
		combo_Use.enableFilteringMode(true);
		
		if (RisStatusCode!="")
		{
			combo_Use.setComboValue(RisStatusCode);
		}
...
{
  var obj=combo_Use;
  var RisStatusCode=obj.getActualValue();
  document.getElementById("RisStatusCode").value=RisStatusCode;
...
function GetSelectArcItemInfo(Infos)
{
  Item=Infos.split("^");
  document.getElementById("ArcItemName").value=Item[0];
  document.getElementById("ArcItemRowid").value=Item[1];
}
 	         ...
 	         if(GetParintData!="")
 	         {
		 	         ...
		 	         xlsheet.cells(2,8)=session['LOGON.USERNAME'];
				 	  ...
				 	  if(OeItemID!="" && PrintFlag=="")   
				 	  {
	 	                ...
	 	                ret=cspRunServerMethod(SetPrintFlagFun,OeItemID,BodyRowid);
	 	                var PrintData=SetPrintData(OeItemID,BodyRowid);
                ...
                if ((selectedobj)&&(selectedobj.checked))
		          {
				      ...
				      var OeItemID=document.getElementById("OEOrdItemIDz"+i).value;
				 	  ...
				 	  var BodyRowid=document.getElementById("bodypartIDz"+i).value;
...
function GetSelectRecLocInfo(Infos)
{
  Item=Infos.split("^");
  document.getElementById("RecLocId").value=Item[0];
  document.getElementById("RecLocName").value=Item[1];
}
