...
common.getTokenUrl=function(url){
...
common.easyModal=function(title,url,width,height,onClose){
	...
	var $easyModal=$('#criticalvalue-easyModal');
	if ($easyModal.length==0){
		$easyModal=$('<div id="criticalvalue-easyModal" style="overflow:hidden;" ><iframe name="criticalvalue-easyModal" style="	width: 100%;height: 100%; margin:0; border: 0;" scroll="auto"></iframe></div>').appendTo('body');
	}
...
common.easyOriginWin=function(winname,url,width,height,opWin){
	...
	if (opWin && typeof opWin.open=='function') {
		window.originWins[winname]=opWin.open(url,winname,features);
	}else{
