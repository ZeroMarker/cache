...
GV.util={
	...
	easyModal:function(title,url,width,height){
		...
		url=url+(url.indexOf('?')>-1?'&':'?')+'MWToken='+GV.util.getToken();
...
//	}
	...
	$('#pHospId').combogrid({
		...
		onSelect:function(row){
			setTimeout(function(){
				if($('#pAdmLoc').length>0) $('#pAdmLoc').combogrid('setRemoteValue',{value:'',text:''})	
				
				if($('#pRepLoc').length>0) $('#pRepLoc').combogrid('setRemoteValue',{value:'',text:''})	
				
				if($('#pOrdLoc').length>0) $('#pOrdLoc').combogrid('setRemoteValue',{value:'',text:''})	
	...
	if($('#pExecConResult').length>0) {
		$.q({ClassName:'web.DHCAntCVOptions',QueryName:'Find',OptsType:'ConResultExec',q:''},function(data){
			if (data && data.rows){
				var conResultData=[{value:'',text:$g('全部')}]
				$.each(data.rows,function(){
					conResultData.push({value:this.TCode,text:$g(this.TDesc)})
				})
			}else{
				var conResultData=[{value:'',text:$g('全部')},{value:'F',text:$g('已通知')},{value:'C',text:$g('未联系到')}]	
			}
			
			$('#pExecConResult').combobox({
				data:conResultData,editable:false,panelHeight:'auto',editable:false
			}).combobox('setValue','');
		})
	...
	if ($('#pExecTimeUsed').length>0) {
		...
		$.each(GV.ExecTimeUsedData,function(){
			this.oglValue=this.value;
	...
	$('#pRepLoc').combogrid({
		...
		delay: 500,
		mode: 'remote',
		url:$URL+"?ClassName=web.DHCAntCVComm&QueryName=FindRepLoc",
		onBeforeLoad:function(param){
			param.HospId=getHospId();
			param.desc=param.q;
			return true;
	...
	var search_fun=function(){
		...
		var pExecConResult=$('#pExecConResult').length>0?$('#pExecConResult').combobox('getValue'):'';
		pStatus=pStatus+'^^'+pExecConResult;  //TransStatus^第6位 处理时联系结果
		
		var pAdmDocId=$('#pAdmDocId').length>0?$('#pAdmDocId').combogrid('getValue'):'';
		var pOrdLocId=$('#pOrdLoc').length>0?$('#pOrdLoc').combogrid('getValue'):''; //开单科室
		var pRepLoc=$('#pRepLoc').length>0?$('#pRepLoc').combogrid('getValue'):''; //报告科室
		pReportType=pReportType+"^"+pRepLoc; //报告科室拼在报告类型的^第二位  
...
var init=function(){
	var columns0=[
			...
			{title:'下医嘱科室',field:'ordLoc',width:120},
			{title:'开单医生',field:'ordDoc',width:80},
			{title:'危急值报告结果',field:'repResult',width:300},
			{title:'医嘱报告时间',field:'mtsRepTime',width:160}
		].concat(
			GV.ReceiveMode>0?[{title:'接收状态',field:'recStatusDesc',width:80},
				{title:'接收时间',field:'recDate',width:160,formatter:function(val,row,ind){
			...
			[
				...
				{title:'处理状态',field:'execStatusDesc',width:80},
...
function openTransOrd(ind){
	...
	if ($transOrdWin.length>0){
		...
		var $tb=$('<div id="transOrdExecTable-tb" style="padding:4px 0 4px 10px;line-height:'+tbLineHeight+'px;"><span style="padding-right:10px;">'+$g('执行日期')+'</span><input class="tb-st" /> -- <input class="tb-end" /> <a href="javascript:void(0);" class="tb-search" style="margin-left:10px;" >查询</a></div> ').appendTo('body');;
		//console.log($tb.html());
		...
		$transOrdTable.datagrid({
			...
			border:true,
			url:$URL+'?ClassName=web.DHCAntCVReportLink&QueryName=FindOrd',
...
function openTransEmr(ind){
	...
	if ($transEMRWin.length>0){
		...
		$transEMRTable.datagrid({
			...
			bodyCls:'panel-header-gray',
			border:true,
			url:$URL+'?ClassName=web.DHCAntCVReportLink&QueryName=FindEMR',
			queryParams:{reportID:row.reportID},
			idField:'lkID' ,
			singleSelect:true,
			rownumbers: true,
