...
function InitBillInfoList(){
		...
		,{field: 'ExeDetails', title: '执行情况', width: 80,sortable: false,align:'center',hidden:true,
			formatter:function(value,row,index){
	    		...
	    		var btn = '<a style="color: #ff7f24;text-decoration: underline;" onclick="OpenExeDetails(\''+row.OEItemID +'\',\''+row.Desc+'\')">'+$g("明细")+'</a>';
	    		return btn;
      	}}
		...
		,{field: 'CoverMainInsur', title: '医保', width: 60}
			...
			for (var i=0;i<cm.length;i++){
				...
				if(cm[i]['field']=="ExeDetails"){
					cm[i].formatter = function(value,rec){
			    		...
			    		var btn = '<a style="color: #ff7f24;text-decoration: underline;" onclick="OpenExeDetails(\''+rec.OEItemID +'\',\''+rec.Desc+'\')">'+$g("明细")+'</a>';
			    		return btn;
					}
				}
...
function getRefData(){
   ...
   if (oeoris=="") {
	   if ((ARCOSFormulaExistFlag=="1")&&(NotARCOSFormulaFlag=="1")&&(FormulaCanAppendItem=="0")){
			$.extend(rtnObj, { msg: "协定处方不可与其他医嘱一起复制!",success:false});
	   		return rtnObj;
	   }
	   oeoris=CMoeoris,toTitle=$g("草药录入");
   }
