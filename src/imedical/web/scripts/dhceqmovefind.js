...
function initTopPanel() {
	...
	jQuery("#btnAdd").linkbutton({
		 iconCls: 'icon-add'
	});
	jQuery("#btnQuery").on("click", QueryMoveClick);
	jQuery("#btnAdd").on("click", AddClick);
	initObjType();
	initSourceType();
	initEventType();
	initStatus();
...
function initMoveListPanel() {
	...
	jQuery('#tMoveList').datagrid({
		...
		columns:[[  
			...
			{ title: '设备名称', field: 'equipname',halign:"center", width:160},
			...
			{ title: 'eventypedr', field: 'eventypedr',align: "right",halign:"center", width:50 , hidden:true},
			{ title: '移动缘由', field: 'eventype',align: "right",halign:"center", width:80 },
...
function fomartOperation(value, rowData, rowIndex) {//生成行内的修改和删除按钮和事件
	...
	if(rowData.rowid!="")
	{
		if((rowData.statusdr==0)&&(rowData.invalidflag=="N"))
		{
		...
		if(rowData.statusdr==0)
		{	
			if(rowData.invalidflag=="N")	
			{	
		...
		if((rowData.statusdr!=0)||(rowData.invalidflag=="Y"))
		{
		...
		if(rowData.statusdr==2)
		{
			if(rowData.eventypedr==1)
			{
				str+='    <a onclick="onmake('+rowData.rowid+','+rowData.eventypedr+')" href="#"><img border=0 complete="complete" src="../scripts/dhceq/easyui/themes/icons/reload.png" />返还</a>'; 
			}
			else
			{
				str+='    <a onclick="onmake('+rowData.rowid+','+rowData.eventypedr+')" href="#"><img border=0 complete="complete" src="../scripts/dhceq/easyui/themes/icons/reload.png" />送出</a>'; 
			}
		}
	}
	return str;
...
function onedit(rowid,statusdr) {
	var str="id="+rowid+"&status="+statusdr;
   	//window.open('dhceqmovenew.csp?'+str, '_blank', 'toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=no,resizable=no,copyhistory=no,width=800,height=400,left=100,top=100');
	//Modified by HHM 2016-01-21 修改窗口弹出方式
	openwindow('dhceqmovenew.csp',str,800,400);
}
...
function onupload(rowid) {
	...
	if(InfoStr[10]=="")
	{
		jQuery.messager.alert("提示", "开始日期为空");
		return false;
	}
	if(InfoStr[11]=="")
	{
		jQuery.messager.alert("提示", "开始时间为空");
		return false;
	}
	if(InfoStr[17]=="")
	{
		jQuery.messager.alert("提示", "运送负责人为空");
		return false;
	}
	var str = tkMakeServerCall("web.DHCEQMove", "GetMoveUserInfo",rowid);
	if(str=="")
	{
		jQuery.messager.alert("提示", "运送人员信息为空");
		return false;
	}
    var result = tkMakeServerCall("web.DHCEQMove", "SubitMoveInfo",1,rowid,SessionObj.guser);
    if(result==0) {
...
function onmake(rowid,eventype)
{
	var str = tkMakeServerCall("web.DHCEQMove", "CheckMoveUnqIdx",rowid);
	if(str!="")
	{
		if(eventype==1)
		{
			jQuery.messager.alert("提示", "返还单据已存在");
			return false;
		}
		if(eventype==2)
		{
			jQuery.messager.alert("提示", "送出单据已存在");
			return false;
		}
	}
	var user=SessionObj.guser;
	var result = tkMakeServerCall("web.DHCEQMove", "CreatMoveInfo",rowid,user,"","");
	if(result>0)
	{
		if(eventype==1)
		{
			jQuery.messager.alert("提示", "返还单据生成成功");
			jQuery("#tMoveList").datagrid('reload'); 
			return false;
		}
		if(eventype==2)
		{
			jQuery.messager.alert("提示", "送出单据生成成功");
			jQuery("#tMoveList").datagrid('reload'); 
			return false;
		}
	}
	else
	{
		if(eventype==1)
		{
			jQuery.messager.alert("提示", "返还单据生成失败");
			return false;
		}
		if(eventype==2)
		{
			jQuery.messager.alert("提示", "送出单据生成失败");
			return false;
		}
	}
}
...
function AddClick()
{
	//window.open('dhceqmovenew.csp?id=&status=', '_blank', 'toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=no,resizable=no,copyhistory=no,width=800,height=400,left=50,top=0');	
	//Modified by HHM 2016-01-21 修改窗口弹出方式
	var str='dhceqmovenew.csp';
	openwindow(str,'id=&status=',800,400);
}
