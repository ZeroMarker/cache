...
function StartICUMonitoring(lnk,newwin){
	...
	if (flag==1){
		if ("undefined"==typeof EnableLocalWeb || 0==EnableLocalWeb ){
		     ...
		     var icuMonitoringLink = "../service/dhcclinic/app/icu/Main.application?icuaId="+monId+"&admId=" + EID + "&userId=" + session["LOGON.USERID"] + "&groupId=" + session["LOGON.GROUPID"] + "&locId=" + ctLocId + "&connString=" + ServerNameSpace + "&filePath=" + filePath + "&total=";
...
function StartBPMonitoring(lnk,newwin){
	...
	var flag = isAccessMP("BP-01","BP-01"); //判断血透菜单是否有运行权限
	if (flag==1){
		//未开启使用平台组插件或老项目,仍旧用ClinicOnce方式运行血液净化
		if ("undefined"==typeof EnableLocalWeb || 0==EnableLocalWeb ){
		...
		{
			//测试写死数据库地址
			//var tmpServerNameSpace="cn_iptcp:114.251.235.112[1972]:DHC-APP"		
			//var args=[session["LOGON.USERID"],session["LOGON.GROUPID"],session['LOGON.CTLOCID'],tmpServerNameSpace]
			//自动获取数据库地址
