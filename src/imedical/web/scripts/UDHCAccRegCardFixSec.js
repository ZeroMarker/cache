...
function DocumentLoadHandler() {
	...
	OldObj=document.getElementById('Old');  //lgl+
	if (OldObj){
		OldObj.onkeydown =nextfocus;
		OldObj.onchange=Old_Change;
	}
...
function ReadMagCard_Click(){
	...
	if (obj){
		...
		if (myary[0]=='0'){
			...
			if (m_CardValidateCode.length<6){
				alert("刷卡错误,请重新刷卡!");
				return;
			}
			getpatbycard();
...
function NewCard_click()
{
	...
	var mySecrityNo=m_CardValidateCode;
	...
	if (encmeth!=""){
		...
		////UpdatePatDetail
		var myPatName=NameObj.value;
		myPatName=myPatName.Trim();
		var myPatInfo=PatientID+"^"+myPatName+"^"+Sexid+"^"+BirthObj.value+"^"+TelNoObj.value+"^"+OpMedicareObj.value+"^"+InMedicareObj.value+"^"+PatTypeid+"^"+IDCardNo1Obj.value+"^"+VocationObj.value+"^"+CompanyObj.value+"^"+AddressObj.value;
		////grantcardUpdate
...
{			  			
			...
			if (tmp) tmp.value = "";
			var tmp=document.getElementById('Old');
			if (tmp) tmp.value = "";
...
function Old_Change(){  // lgl+
  var OldObj=document.getElementById('Old');
  if (OldObj.value!=""){
	var old=OldObj.value;
	
	var d,s="";
	d= new Date();
	s+=(d.getYear()-old);
	
	if (d.getMonth()<9){   //月份返回是0-11
      s += "0"+(d.getMonth() + 1);
  }
  else{
	  s += (d.getMonth() + 1);
