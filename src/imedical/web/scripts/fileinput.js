...
(function ($) {
    ...
    var isIE = function (ver) {
        ...
        tActions = '<div class="file-actions">\n' +
            ...
            '        {check}{delete}{other}' +		//Modify DJ 2019-02-19 {check}{upload}{delete}{other}
            '    </div>\n' +
            '    <div class="file-upload-indicator" tabindex="-1" title="{indicatorTitle}">{indicator}</div>\n' +
            '    <div class="clearfix"></div>\n' +
            '</div>',
        tActionCheck = '<input type="checkbox" id="{imgcheckindex}">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</input>\n',	//modify by wl 2019-12-13 WL0024
        ...
        defaultLayoutTemplates = {
            ...
            footer: tFooter,
            actions: tActions,
            actionCheck: tActionCheck,		//Modify DJ 2019-02-16
    ...
    FileInput.prototype = {
        ...
        listen: function () {
            ...
            self.$container.find('.kv-fileinput-upload').off('click').on('click', function (e) {
                ...
                e.preventDefault();
                //增加业务处理	Add By DJ 2019-02-16
                var PicType=getElementValue("PicType")
                var PicName=getElementValue("PicName")
                if ((PicType=="")||(PicType==0))
                {
	                alert("请选择正确的的图片类型!")
	                return
                }
                if (PicName=="")
                {
	                alert("请输入图片名称!")		//批量图片上传
	                return
                }
				/// Modefied by zc0060 20200329  图片上传最大数量限制 begin
				if (self.getFileStack().length>self.maxFilesNum)
            	{
	            	alert("图片数量已超过最大数量"+self.maxFilesNum+"张,请删除多余图片再上传")		
                	return
	        	}
				/// Modefied by zc0060 20200329  图片上传最大数量限制 end
        ...
        upload: function () {
            ...
            //检测是否有选择主图片	//Modify DJ 2019-02-18
            len = self.filestack.length;
            var FindCheckMain=""
            var chkCount=0
            if (getElementValue("PTRowID")=="")
            {
	            for (i = 0; i < len; i +=1)
	            {
	                if (getElementValue("imgchk"+i))
	                {
		                chkCount=chkCount+1
		                FindCheckMain=i
	                }
	            ...
	            //优先上传主图片
	            if (self.filestack[FindCheckMain] !== undefined)
	            {
	                self.uploadSingle(FindCheckMain, self.filestack, true);
	                //上传成功获取主图ID
	                setElement("PTRowID",getElementValue("PTRowID"))
	            }
            }
            self.resetUpload();
            ...
            if (self.uploadAsync && self.showPreview) {
                ...
                //上传剩余图片
                for (i = 0; i < len; i += 1) {
                    if ((self.filestack[i] !== undefined)&&(i!==FindCheckMain))
                    {
        ...
        renderFileActions: function (showUpload, showDelete, disabled, url, key) {
            ...
            var self = this,
                ...
                btnUpload = '',
                btnCheck = self.getLayoutTemplate('actionCheck'),		//Modify DJ 2019-02-16
            ...
            return template
                ...
                .repl('{check}', btnCheck)			//Modify DJ 2019-02-16
        ...
        ajaxSubmit: function (fnBefore, fnSuccess, fnComplete, fnError) {
            ...
            if (PicName=="")
            {
                alert("请输入图片名称!")		//单个图片上传
                return
            }
			//Moidefied by zc0064 2020-4-8  图片名称传入后台乱码问题处理 begin
            //self.uploadUrl='../csp/dhceq.process.pictureaction.csp?&actiontype=UploadByFtpStream&PTRowID='+PTRowID+'&PicSort=1&PicTypeDR='+PicType+'&PicName='+PicName+'&SourceType='+SourceType+'&SourceID='+SourceID
            self.uploadUrl='../csp/dhceq.process.pictureaction.csp?&actiontype=UploadByFtpStream&PTRowID='+PTRowID+'&PicSort=1&PicTypeDR='+PicType+'&PicName='+encodeURIComponent(PicName)+'&SourceType='+SourceType+'&SourceID='+SourceID
			//Moidefied by zc0064 2020-4-8  图片名称传入后台乱码问题处理 end
			
            settings = $.extend({
                xhr: function () {
                    var xhrobj = $.ajaxSettings.xhr();
                    //return self.initXhr(xhrobj, 98);
		    return self.initXhr(xhrobj, 100);
                },
                url: self.uploadUrl,
                type: 'POST',
                dataType: 'json',
                ...
                success: fnSuccess,
                complete: fnComplete,
                async : false,		//Modify DJ 2019-02-18
                error: fnError
            }, self.ajaxSettings);
		...
		pct = Math.ceil(self.uploadCount * 100 / total);
                self.uploadPercent = Math.max(pct, self.uploadPercent);
                self.setProgress(self.uploadPercent);
                self.initPreviewDeletes();
            };
		...
		if (self.abort(params)) {
            ...
            fnSuccess = function (data, textStatus, jqXHR) {
	            setElement("PTRowID",data.PTRowID)		//Modify DJ 2019-02-18
                ...
                setTimeout(function () {
                    if (data.error === undefined) {
                        ...
                        self.showUploadError("["+$("#"+self.previewInitId + "-" + i).find("img").attr("title")+"]  "+data.error, params);
                    }
                }, 100);
            };
            fnComplete = function () {
            ...
            if (chkTypes && chkMimes) {
                if (cat === 'text') {
                    ...
                    content = tmplt.repl('{previewId}', previewId).repl('{caption}', caption)
                        ...
                        .repl('{footer}', footer).repl('{fileindex}', ind)		//Modify DJ 2019-02-18
                        .repl('{footer}', footer).repl('{imgcheckindex}', "imgchk"+ind) + modal;		//Modify DJ 2019-02-18
                } else {
                    content = tmplt.repl('{previewId}', previewId).repl('{caption}', caption)
                        .repl('{frameClass}', '')
                        ...
                        .repl('{footer}', footer).repl('{imgcheckindex}', "imgchk"+ind);	//Modify DJ 2019-02-18
                }
                self.$preview.append("\n" + content);
