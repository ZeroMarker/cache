...
function InitEvent(){
	...
	// 打印卷条码
	$('#btnPrintVolBar').click(function(){
		var rows = $('#gridWorkList').datagrid('getSelections');  		//取得所有选中行数据，返回元素记录的数组数据
		if (rows.length==0) {
			$.messager.popover({msg: '请选择需要打印条码的数据！',type: 'alert',timeout: 1000});
			return;
		}
		var volids = '';
		for (var i=0;i<rows.length ;i++ ){
			var volid = rows[i].VolID
			volids += ',' + volid;
		}
		PrintBarCode('V',volids);
	});	
		
	// 打印病案条码
	$('#btnPrintMrBar').click(function(){
		var rows = $('#gridWorkList').datagrid('getSelections');  		//取得所有选中行数据，返回元素记录的数组数据
		if (rows.length==0) {
			$.messager.popover({msg: '请选择需要打印条码的数据！',type: 'alert',timeout: 1000});
			return;
...
function InitGridWorkList(){
	var columns = [[
		...
		{field:'DocName',title:'主管医师',width:100,align:'left'},
		...
		{field:'HappenDate',title:$g('回收时间'),width:180,align:'left',hidden:true,
			formatter: function(value,row,index){
				return row.HappenDate+ ' ' + row.HappenTime;
			}
		},
		{field:'HappenUserDesc',title:$g('回收人员'),width:100,align:'left',hidden:true},
		{field:'FillNo',title:'归档号',width:100,align:'left',hidden:true},
		{field:'EstimDischDate',title:'医生结算时间',width:180,align:'left',
			formatter: function(value,row,index){
				return row.EstimDischDate+ ' ' + row.EstimDischTime;
			}
		},
		{field:'NurseDischDate',title:'护士结算时间',width:180,align:'left',
			formatter: function(value,row,index){
				return row.NurseDischDate+ ' ' + row.NurseDischTime;
			}
		}
	]];
...
function ReLoadWorkList(){
	if ($('#chkRevoke').checkbox('getValue')) {
		var UserFrom = "";
		var objRetrieveUser = $('#cboRetrieveUser').combogrid('grid').datagrid('getSelected');
		if (objRetrieveUser!==null){
			var UserFrom = objRetrieveUser.ID;
		}
...
// 打印条码
function PrintBarCode(BarType,VolumeIDs){
	$m({
		ClassName:"MA.IPMR.SSService.VolumeSrv",
		MethodName:"GetBarInfo",
		aVolumeIDs:VolumeIDs
	},function(ret){
		if (ret == '') return;
		var arrBarInfo = ret.split(CHR_1);
		...
		for (var indMR = 0; indMR < arrBarInfo.length; indMR++) {
			...
			if (BarType=="M") {
				...
				var printList="";
				// 打印参数
				printParam=printParam+"MrType"+String.fromCharCode(2)+MrType;
				printParam=printParam+"^BarCode"+String.fromCharCode(2)+MBarCode;
				printParam=printParam+"^PatientName"+String.fromCharCode(2)+PatientName;
				printParam=printParam+"^Sex"+String.fromCharCode(2)+Sex;
				printParam=printParam+"^Age"+String.fromCharCode(2)+Age;
				printParam=printParam+"^MrNo"+String.fromCharCode(2)+MrNo;
				
				// 获取XML模板配置
				PisDHCP_GetXMLConfig("InvPrintEncrypt","IPMRPrintVBar");
			...
			if (BarType=='V') {
				...
				for (indVol = 1; indVol < arrPatBarInfo.length; indVol++){
					...
					if (arrVBarInfo.length>3) {
						...
						var FillNo 		= arrVBarInfo[5];
						var printParam="";
						var printList="";
						// 打印参数
						printParam=printParam+"^PatientName"+String.fromCharCode(2)+PatientName;
						printParam=printParam+"^DischLoc"+String.fromCharCode(2)+DischLoc;
						printParam=printParam+"^BarCode"+String.fromCharCode(2)+VBarCode;
						printParam=printParam+"^MrNo"+String.fromCharCode(2)+MrNo;
						printParam=printParam+"^FillNo"+String.fromCharCode(2)+FillNo;
						// 获取XML模板配置
						PisDHCP_GetXMLConfig("InvPrintEncrypt","IPMRPrintVBar");
						DHC_PrintByLodop(getLodop(),printParam,'',{},"");
					}
