 ...
 * quality 病案质控
 * 
 * Copyright (c) 2018-2020 SJZ. All rights reserved.
 * 
 * CREATED BY SJZ 2021-03-25
 * 
 * 注解说明
 * TABLE: 
 */
...
function Init(){
	...
	//$('#cboHospital').next(".combo").hide();	// 隐藏医院下拉框
	//$('#cboMrType').next(".combo").hide();		// 隐藏病案类型下拉框
	InitGridWorkList();
}
	...
	$HUI.checkbox('#chkRevoke',{
		onCheckChange:function(e,value){
			if (value) {
				...
				$('#btnRevokeQry').linkbutton("disable");		// 已质控
				$('#btnTodoQry').linkbutton("enable");			// 待质控
...
function InitGridWorkList(){
	var columns = [[
		...
		{field:'PapmiNo',title:'登记号',width:120,align:'left',sortable:true},
		{field:'MrNo',title:'病案号',width:100,align:'left',sortable:true},
		{field:'Sex',title:'性别',width:80,align:'left',sortable:true},
		{field:'Age',title:'年龄',width:80,align:'left',sortable:true},
		{field:'VolStatusDesc',title:'当前状态',width:100,align:'left',sortable:true},
		{field:'StepDesc',title:'当前步骤',width:80,align:'left',sortable:true},
		{field:'DischLocDesc',title:'出院科室',width:160,align:'left',sortable:true},
		{field:'DischWardDesc',title:'出院病区',width:160,align:'left'},
		{field:'AdmDate',title:'就诊日期',width:120,align:'left',sortable:true},
		{field:'DischCondit',title:'出院情况',width:100,align:'left',sortable:true},
		{field:'DischDate',title:'出院日期',width:120,align:'left',sortable:true},
		{field:'HappenDate',title:$g('质控时间'),width:180,align:'left',hidden:true,
			formatter: function(value,row,index){
				return row.HappenDate+ ' ' + row.HappenTime;
			}
		},
		{field:'HappenUserDesc',title:$g('质控人员'),width:120,align:'left',hidden:true,sortable:true}
	]];
