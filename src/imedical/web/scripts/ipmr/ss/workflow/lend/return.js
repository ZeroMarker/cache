...
function Init(){
	...
	Common_ComboGridToUser("cboUserFrom","");
	InitGridWorkList();
}
...
function InitGridWorkList(){
	var columns = [[
		...
		{field:'HappenDate',title:'借出时间',width:180,align:'left',
			formatter: function(value,row,index){
				return row.HappenDate+ ' ' + row.HappenTime;
			}
		},
		{field:'HappenUserDesc',title:'借出人员',width:100,align:'left'}
	]];
...
function ReLoadWorkList(){
	...
	if (objLendUser!==null){
		var UserTo = objLendUser.ID;
	}
	var UserFrom='';
	var objUserFrom = $('#cboUserFrom').combogrid('grid').datagrid('getSelected');
	if (objUserFrom!==null){
		var UserFrom = objUserFrom.ID;
	}
