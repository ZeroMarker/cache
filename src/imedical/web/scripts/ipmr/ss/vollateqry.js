...
function Init(){
	...
	Common_ComboToDic("cboQryDateType","VolLateQryDateType",1,'');					// 初始化日期类型
	$('#cboQryDateType').combobox('setValue',ServerObj.LateQryDateTypeId);				// 选择默认日期类型
...
function InitgridLateStatis(){
	var QryDateTypeCode = getDateTypeCode();
    if (QryDateTypeCode==1) {
    	var DischCnttitle=$g('出院人数');
    }else if (QryDateTypeCode==2) {
    	var DischCnttitle=$g('回收人数');
    }else{
    	var DischCnttitle='';
    }
	...
	var columns = [[
		...
		{field: 'DischCnt',title: DischCnttitle, width: 60, align: 'left',
		...
		{field: 'Dis1DLateCnt',title: '1日迟归人数', width: 80,  align: 'left',
			formatter:function(value,row,index) {
           		var StatLocID = row.StatLocID;
            	var BackDays  = 1
           	    if(StatLocID=="-") StatLocID=""
		...
		{field: 'Dis2DLateCnt', title: '2日迟归人数', width: 80, align: 'left',
			formatter:function(value,row,index) {
           		var StatLocID = row.StatLocID;
            	var BackDays  = 2
           	    if(StatLocID=="-") StatLocID=""
		...
		{field: 'Dis3DLateCnt',title: '3日迟归人数', width: 80, align: 'left',
			formatter:function(value,row,index) {
           		var StatLocID = row.StatLocID;
            	var BackDays  = 3
           	    if(StatLocID=="-") StatLocID=""
		...
		{field: 'Dis5DLateCnt',title: '5日迟归人数', width: 80, align: 'left',
			formatter:function(value,row,index) {
           		var StatLocID = row.StatLocID;
            	var BackDays  = 5
           	    if(StatLocID=="-") StatLocID=""
		...
		{field: 'Dis7DLateCnt',title: '7日迟归人数', width: 80,  align: 'left',
			formatter:function(value,row,index) {
           		var StatLocID = row.StatLocID;
            	var BackDays  = 7
           	    if(StatLocID=="-") StatLocID=""
		...
		{field: 'Dis1DLateRatio',title: '1日迟归率', width: 70,  align: 'left'},
		{field: 'Dis2DLateRatio',title: '2日迟归率', width: 70,  align: 'left'},
		{field: 'Dis3DLateRatio',title: '3日迟归率', width: 70,  align: 'left'},
		{field: 'Dis5DLateRatio',title: '5日迟归率', width: 70,  align: 'left'},
		{field: 'Dis7DLateRatio',title: '7日迟归率', width: 70,  align: 'left'},
		{field: 'DeathCnt',title: '死亡人数', width: 60,  align: 'left'},
		{field: 'Dth6DLateCnt',title: '6日迟归人数', width: 80,  align: 'left',
			formatter:function(value,row,index) {
           		var StatLocID = row.StatLocID;
            	var BackDays  = 6
           	    if(StatLocID=="-") StatLocID=""
		...
		{field: 'Dth6DLateRatio',title: '6日迟归率', width: 70,  align: 'left'},
		/*
...
function getDateTypeCode(){
	var DateTypeId = $("#cboQryDateType").combobox('getValue');
	if (DateTypeId=="") {
