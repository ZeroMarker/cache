...
function StatusQry(queryflg)
{
	...
	var objOperator = $('#cboOperator').combogrid('grid').datagrid('getSelected');
	var operauser ='';
	if (objOperator!==null){
		operauser = objOperator.ID;
	}
	...
	if( queryflg == "H")
	{
		$('#gridVol').datagrid('options').title="历史操作查询结果-"+wfitemDesc;
		$("#gridVol").datagrid("getPanel").panel("setTitle","历史操作查询结果-"+wfitemDesc)
	}else if(queryflg == "U")
	{
		$('#gridVol').datagrid('options').title="未处理操作查询结果-"+wfitemDesc;
		$("#gridVol").datagrid("getPanel").panel("setTitle","未处理操作查询结果-"+wfitemDesc)
	}else{
		
	}
	
	//重载状态查询表格
	$('#gridVol').datagrid('reload',{
		ClassName	: "MA.IPMR.SSService.VolStatusQry",
		QueryName	: "QryVolOperation",
		aHospID		: hospital,
		aMrTypeID	: mrtype,
...
function Export()
{
	...
	var gridoptions = $('#gridVol').datagrid('options');
