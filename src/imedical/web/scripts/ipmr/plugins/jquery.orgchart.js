...
(function(factory) {
  ...
  function createNode(nodeData, level, opts) {
	...
	// if (nodeData.type=='1')
	// {
	// 	$nodeDiv.children('.content').prepend('<img class="relationand">');
	// }else if (nodeData.type=='2')
	// {
	// 	$nodeDiv.children('.content').prepend('<img class="relationor">');
	// }
	$nodeDiv.hover(
	  function () {
		if (globalObj.m_limitedit=='1') return;
		var html = '<div class="datagrid-toolbar test" style="border:0px;position:absolute">';
  	  	html	+= '<table cellspacing="0" cellpadding="0">'
    		html	+= '<tbody>'
    		html	+= '<tr>'
        if (nodeData.type==1){
            html  += '<td><a href="javascript:addLogicMark(\'' + nodeData.type + '\',\'' + nodeData[opts.nodeId] + '\')" class="l-btn l-btn-small l-btn-plain" group="">'
            html  += '<span class="l-btn-left l-btn-icon-left">'
            html  += '<span class="l-btn-text"></span>'
            html  += '<span class="l-btn-icon icon-add"></a></td>'
            html  += '<td><a href="javascript:addCustomLogic(\'' + nodeData.type + '\',\'' + nodeData[opts.nodeId] + '\')" class="l-btn l-btn-small l-btn-plain" group="">'
        		html	+= '<span class="l-btn-left l-btn-icon-left">'
        		html	+= '<span class="l-btn-text"></span>'
        		html	+= '<span class="l-btn-icon icon-add-report"></a></td>'
        }
    		html	+= '<td><a href="javascript:eidtCustomLogic(\'' + nodeData.type + '\',\'' + nodeData[opts.nodeId] + '\')" class="l-btn l-btn-small l-btn-plain" group="">'
    		html	+= '<span class="l-btn-left l-btn-icon-left">'
    		html	+= '<span class="l-btn-text"></span>'
    		html	+= '<span class="l-btn-icon icon-edit"></a></td>'
        if (nodeData[opts.nodeId]!=''){
      		html	+= '<td><a href="javascript:cancelCustomLogic(\'' + nodeData.type + '\',\'' + nodeData[opts.nodeId] + '\')" class="l-btn l-btn-small l-btn-plain" group="">'
      		html	+= '<span class="l-btn-left l-btn-icon-left">'
      		html	+= '<span class="l-btn-text"></span>'
      		html	+= '<span class="l-btn-icon icon-cancel"></a></td>'
        }
    		html	+= '</tr></tbody></table></div>'
		    $nodeDiv.append(html);
			var name	= nodeData[opts.nodeTitle];
			var content	= nodeData[opts.nodeContent];
			if (nodeData.type==2){
				NodePopover(nodeData[opts.nodeId],content);
			}
	  },
	  function () {
	  	$nodeDiv.children('.test').css('display','none')
	  }
	);
