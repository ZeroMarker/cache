		...
		enableDnd: function(jq, index){
			...
			return jq.each(function(){
				...
				var draggableOptions = {
					...
					proxy: function(source) {
						...
						$.map(rows, function(row,i){
							...
							$('<td><span class="tree-dnd-icon dnd-no-icons" style="position:static">&nbsp;</span></td>').insertBefore(p.find('tr:eq('+i+') td:first'));
						});
				...
				function setProxyFlag(source, allowed){
					...
					icon.removeClass('dnd-yes-icons dnd-no-icons').addClass(allowed ? 'dnd-yes-icons' : 'dnd-no-icons');
				}
				function getRow(tr){
