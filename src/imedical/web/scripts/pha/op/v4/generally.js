 ...
 */
 ...
 $(function () {
     ...
     if (OPIVAS_PARAMS.REQNEEDAUDIT === 'Y'){
         $('#tdReqNeedAudit').show()
     }
 });
 ...
 function InitGridGenerally() {
     var columns = [
         [{
             ...
             title: '置为是否可退信息',
             width: 150,
             hidden: (OPIVAS_PARAMS.REQNEEDAUDIT !== 'Y') ? true : false
         },
          {
             field: "check",
             title: 'check',
 ...
 function SaveRetFlag(Flag) {
     $.messager.confirm('提示', '您确认置为' + (Flag == 'Y' ? '可退药' : '不可退药') + '吗?', function (r) {
         if (r) {
             ...
             var saveRet = tkMakeServerCall('web.DHCSTPIVAS.DataHandler', 'SaveRetFlag', pogIdStr, Flag, session['LOGON.USERID']);
