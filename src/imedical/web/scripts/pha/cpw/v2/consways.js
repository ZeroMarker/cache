...
$(function()
{
	$("#dg").datagrid({
		idField:"rowid",
		url: url+"?action=GetPhConWaysInfo",  
		rownumbers:true,
		striped: true,
...
function deleteRow()
{
	...
	if (rows.length > 0) {
		$.messager.confirm("提示", "您确定要删除这些数据吗？", function (res) {//提示是否删除
			if (res) {
				$.post(url+'?action=DeletePhConWays',{"index":rows[0].rowid}, function(data){
...
function saveRow()
{
	...
	//保存数据
	$.post(url+'?action=UpdatePhConWays',{"dataList":rowstr},function(data){
