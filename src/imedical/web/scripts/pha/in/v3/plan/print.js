var PLAN_PRINT = {
    ...
    Print: function (planID) {
        PHA.Loading('Show');
        var prtData = this.GetData(planID);
        var rows = prtData.List;
        var para = prtData.Para;
        PIVASLODOP = getLodop();
        PIVASLODOP.PRINT_INIT('PHAINPLAN');
        ...
        // 头
        html += '<thead>';
        html += '	<tr><td colspan=8 style="text-align:center;border:none"><p class="pha-print-title">' + para.title + '</p></td></tr>';
        html += '	<tr><td colspan=8 style="border:none">';
        html += '<p style="border:none;">';
        html += '科室：' + para.locDesc;
        html += '　　类型：' + para.operateTypeDesc;
        html += '　　单号：' + para.no;
        html += '　　制单：' + para.createDate + ' ' + para.createTime + ' ' + para.createUserName;
        html += '</p>';
        html += '    </td></tr>';
        html += '	<tr>';
        ...
        html += '       <td class="pha-print-vendorDesc">经营企业</td>';
        ...
        html += '<tfoot>';
        html += '	<tr>';
        html += '		<td colspan=7 style="text-align:left;">总计</td>';
        html += '		<td colspan=1 style="text-align:right;">' + para.totalRpAmt + '</td>';
        html += '	</tr>';
        html += '	<tr>';
        html += '		<td colspan=6 style="text-align:left;border:0;">' + '打印:' + para.printInfo + '</td>';
        ...
        for (var i = 0; i < rows.length; i++) {
            ...
            html += '	<td class="pha-print-vendorDesc"><p>' + rowData.vendorDesc + '</p></td>';
