...
var hospWidth = 360;
var TABLENAME = 'DHC_IncReasonForStockScrap';

var GRIDID = 'gridReason';
var GRIDBARID = '#gridReasonBar';
var APINAME = 'PHA.IN.Scrapreason.Api';
...
function InitGrid(){
	var columns = [
        [
            { field: 'scrapId', 	title: 'scrapId', 		hidden: true },
            { field: 'scrapCode', 	title: '代码',  		align: 'left',		width: 150,		editor: {type: 'validatebox',options: {required: true}}},  
            { field: 'scrapDesc', 	title: '名称',  		align: 'left',		width: 900,		editor: {type: 'validatebox',options: {required: true}}},  
        ]
    ];
    var dataGridOption = {
        url: PHA.$URL,
...
function GetIpblId(){
    ...
    if (gridSelect) return gridSelect.scrapId;
    return '';
}
...
function Delete(){
    ...
    $.messager.confirm('确认对话框', '您确定删除吗？', function (r) {
        if (r) {
            var scrapId = gridSelect.scrapId || '';
            if (scrapId == '') {
