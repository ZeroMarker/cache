...
var PHA_UPLOAD = {
	...
	Upload: function (dataOpts, winOpts) {
		...
		$('#' + this.WinId).window(defWinOpts).window("open");
		...
		PHA_UPLOAD_Loader.on('fileQueued', function (file) {
			...
			PHA_UPLOAD_Loader.makeThumb(file, function (error, src) {
				if (error) {
					// 不可预览
					var unimgHtml = '<div>仅图片支持预览</div>';
					$("#" + file.id).find('.pha-uploader-body-img').removeClass("pha-uploader-body-img").addClass("pha-uploader-body-unimg").append(unimgHtml);
					return;
				} else {
					// 可预览
					var imgHtml = '<img src=' + src + '>';
					$("#" + file.id).find('.pha-uploader-body-img').append(imgHtml);
				}
	...
	Preview: function (dataOpts, winOpts) {
		...
		$('#' + this.WinId).dialog(defWinOpts).window("open"); // diag依赖window
		$.parser.parse($('#' + this.WinId));
		var dataGridOption = {
			pagination: false,
			toolbar: null,
	...
	GetHttpFile: function (_name) {
		...
		var httpSrc = "http://" + PHA_UPLOAD.HttpSrc + _name;
		return httpSrc;
	},
	/**
	 * 
	...
	RefreshSingle: function (_liId, _data,_dataOpts) {
		...
		if (PHA_UPLOAD.CanView(name) == true) {
			...
			var imgHtml = '<img src=' + src + '>';
			$("#" + fileId).find('.pha-uploader-body-img').append(imgHtml);
		} else {
			var unimgHtml = '<div>仅图片支持预览</div>';
			$("#" + fileId).find('.pha-uploader-body-img').removeClass("pha-uploader-body-img").addClass("pha-uploader-body-unimg").append(unimgHtml);
		}
	...
	UploadHtml: function () {
		var winHtml = "" +
			'<div id="' + this.WinId + '" class="hisui-dialog" data-options="modal:true,iconCls:\'icon-w-plus\'" title="文件上传">' +
