	...
	$HUI.combobox('#CookModelist',{      
    	...
    	onSelect:function(record){
			if (record) {
				...
				if (record.RecLoc!=""){
					 CMRecLocStr=CMRecLocStr+"!"+record.RecLoc
				}
		...
		new Promise(function(resolve,rejected){
			...
			return new Promise(function(resolve,rejected){
				...
				if ((PageLogicObj.IsARCOSFormula=="1")||(GlobalObj.CMMedNotOpenARCOS=="1")){
					ARCOSChangePrescType(ARCOSRowid);
...
function CheckProtocolPackSum(GroupIndex,Row,callBackFun) {
	...
	var numStr = $.cm({
		...
		MethodName:"GetDspUomFac",
		dataType:"text",
		arcimID:OrderARCIMRowid, 
		RecLocID:RecLoc
	},false);
	if(numStr!=""){
		if((parseInt(Sum).toString()!=Sum.toString())){   //判断是不是整数
			$.messager.alert("提示","数量 请输入整数!","info",function(){
				callBackFun(false);
			});
			return false;
		}else{
			if(Sum>100){
				$.messager.alert("提示","数量不能大于100!","info",function(){
					callBackFun(false);
				})
				return	false;
			}	
		}
		var Checkflag=CheckSums(numStr,Sum);
		if(Checkflag==0){
			var OrderDes=$("#"+Row+"_OrderName"+GroupIndex+"").val(); 
			var RecLocDesc=$("#RecLoc").combobox('getText');
			var numStr1=numStr.split("^");
			var numStrs=""
			for(var m=0;m<numStr1.length;m++){
				if(numStrs==""){
					numStrs=numStr1[m]+"g"	
				}else{
					numStrs=numStrs+","+numStr1[m]+"g"	
				}
			}
			if (numStrs.split(",").length>1){
                //找最邻近的小数值,用于建议
                var tmpSum=Sum,minNear=0;
                while (minNear==0) {
                    tmpSum--;
                    if (tmpSum<0) {
                        minNear=-1;
                    ...
                    if (CheckSums(numStr, tmpSum)!=0) minNear=tmpSum;
                }
                if (minNear==-1) minNear="";
                //找最邻近的大数值,用于建议
                var tmpSum=Sum,maxNear=0;
                while (maxNear==0) {
                    tmpSum++;
                    if (CheckSums(numStr, tmpSum)!=0) maxNear=tmpSum;
                }
                var confirmStr = OrderDes + RecLocDesc + "目前只能提供" + numStrs + "整包装,请选择小包装组合数.与所开数量最邻近的小值为【"+minNear+"】,大值为【"+maxNear+"】";
            }else{
	            var confirmStr = OrderDes + RecLocDesc + "目前只能提供" + numStrs + "整包装,请选择小包装组合数";
	        }
			$.messager.alert("提示",confirmStr,function(){
...
function SetScreenSum(){
		...
		for (var j=1; j<=GlobalObj.ViewGroupSum; j++){
			...
			if ((OrderARCIMRowid!="")&&(OrderItemRowid=="")){
				...
				var colPromis=new Promise(function(resolve,rejected){
					...
					ColArrIndex++;
					/*var INCIPackCombStrArr=INCIPackCombStr.split("@");
					var OrderDoseQty=0;
					for (var k=0;k<INCIPackCombStrArr.length;k++){
						OrderDoseQty=OrderDoseQty+parseFloat(INCIPackCombStrArr[k].split("!")[1]);
					}*/
					var SumObj={
						amount:parseFloat(OrderItemSum) //,*parseFloat(OrderDoseQty)
						//ScreenQtySum:parseFloat(OrderDoseQty)
					};
					return SumObj;
				})
	...
	.then(function(SumObj){
		...
		for (var i=0;i<SumObj.length;i++) {
			...
			//ScreenQtySum=ScreenQtySum+parseFloat(SumObj[i].ScreenQtySum);
		}
...
function SetPrescAppenItemQty(){
    ...
    for (i=0;i<CookModelist.length;i++){
		if (CookModelist[i].RowID==CookMode){
			if (CookModelist[i].RecLoc!=""){
					 RecLocStr=RecLocStr+"!"+CookModelist[i].RecLoc
				}
			}
    }
	...
	for(var i=1;i<=rows;i++){
		...
		for (var j=1;j<=GlobalObj.ViewGroupSum;j++){
		   ...
		   if (OrderARCIMRowid!=""){
				...
				var OrderPhSpecInstr=$("#"+i+"_OrderPhSpecInstr"+j+"").find("option:selected").text()
			    OrderItem=OrderARCIMRowid+String.fromCharCode(1)+OrderDoseQty+String.fromCharCode(1)+OrderPhSpecInstr
				if (ArcimStr==""){
					ArcimStr=OrderItem
				}else{
...
function HIS_dealwithPASSCheck(result) {
    if (result > 0) {
        if (dhcsys_confirm("合理用药检查有提示，你确认通过吗?",true)) {
...
function UpdateClickHandlerNew() {
...
function addSelectedFav(value,callBackFun) {
	...
	new Promise(function(resolve,rejected){
		...
		if (newselary[0].split(String.fromCharCode(4))[0]=="ARCOS") {
			var osid = newselary[i].split(String.fromCharCode(4))[2];
...
function ARCOSChangePrescType(ARCOSRowid){
    ...
    if (ARCOSPrescTypeStr==""){return true}
    var ARCOSPrescTypeCode=ARCOSPrescTypeStr.split("^")[0];
    if (ARCOSPrescTypeCode=="") {return true}
    var CMPrescTypeKWSel=$("#CMPrescTypeKW").keywords('getSelected');
	var CMPrescTypeDetails=CMPrescTypeKWSel[0].value;
    var DetailsArr=CMPrescTypeDetails.split("#");
    var SelPrescTypeCode=DetailsArr[0];
...
//判断一个数是不是由几个系数组合而来 
function CheckSums(NumStr, Sum) {
    var Nums = NumStr.split("^")
    var Nums1 = new Array(); //去掉0系数
    var flag = 0
    for (var i = 0; i < Nums.length; i++) {
        var num = Nums[i]
        if (parseInt(num) != 0) {
            Nums1[Nums1.length] = parseInt(num)
        }
    }
    if (Nums1.length == 1) {
        if ((Sum % Nums1[Nums1.length - 1]) == 0) {
            flag = 1
        }
    } else {
        flag = CheckSum(Sum, Nums1, 0, "", Sum)
    }
    return flag
}
function CheckSum(Sum, Nums, j, str1, Sum1) {
    var str = ""
    if ((j + 1) >= Nums.length) {
        return 0
    }
    ...
    while (1) {
        ...
        if (n2 == 0) {
            ...
            var flag = CheckSum(n1, Nums, j + 1, str1, Sum1)
        }
        if (flag == 1) {
            return 1
        }
				...
				case "SelectCMDoseQty": //弹出草药组合数界面
					(function(callBackFunExec){
						new Promise(function(resolve,rejected){
							...
							if (OrderDoseQty!="") {
								$.extend(UserOptionObj,{Type:"SelectCMDoseQty",Value:OrderDoseQty});
							}
							callBackFunExec();
						})
...
function xhrRefresh(Args){
	...
	if ((adm==GlobalObj.EpisodeID)||(adm=="")){
		var EpisPatInfo = tkMakeServerCall("web.DHCDocViewDataInit", "InitPatOrderViewGlobalCM", adm);
		var EpisPatObj=eval("("+EpisPatInfo+")");
...
function ReloadFrameData() {
	...
	window.requestAnimationFrame(function(){
		...
		var EpisPatInfo = tkMakeServerCall("web.DHCDocViewDataInit", "InitPatOrderViewGlobalCM", GlobalObj.EpisodeID);
		InitPatOrderViewGlobalCM(EpisPatInfo);
		$('#CMOrdEntry_DataGrid').jqGrid('clearGridData');
		var records=$('#CMOrdEntry_DataGrid').getGridParam("records");
		if(records==0){
