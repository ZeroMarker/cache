...
function insertPisRow(){
	...
	if(editRow>="0"){
		$("#pisdislist").datagrid('endEdit', editRow);//结束编辑，传入之前编辑的行
	}
	 
	$("#pisdislist").datagrid('insertRow', {//在指定行添加数据，appendRow是在最后一行添加数据
		index: 0, // 行数从0开始计算
		row: {APRowID:'',APCode:'',APDesc:'',FlagCode:'Y',APFlag:'Y',HospDr:LgHospID,HospDesc:LgHospID}
	});
    
	$("#pisdislist").datagrid('beginEdit', 0);//开启编辑并传入要编辑的行
	editRow=0;
}
///保存申请状态
function savePisRow(){
	...
	runClassMethod("web.DHCAPPPisInfDis","SavePisDis",{"params":params},function(jsonString){
		...
		$('#pisdislist').datagrid('reload'); //重新加载
	});
}
...
function deletePisRow(){
	...
	var rowsData = $("#pisdislist").datagrid('getSelected'); //选中要删除的行
	if (rowsData != null) {
