...
function InitPageComponent(){
	...
	/*$HUI.combobox('#OperName',{      
    	...
    	onSelect:function(record){
			if (record) {
				$("#OperPart").val(record.BodySiteDesc)
				$HUI.datetimebox("#OperTime").setValue(record.OperDateTime);
			}
		}
	});*/
...
function SavePisNo(){
	...
	*/
	...
	var arOperName =  $HUI.combobox("#OperName").getValue();  //$("#OperName").val(); 	/// 手术名称
	...
	if (FrostFlag == "N"){	/// 冰冻未勾选时,标本时间不能空 qunianpeng 2018/9/3
		if (TmpSepTime=="") {
			$.messager.alert("提示:","请填写标本离体日期!","info",function(){
				$('#SepDate').next('span').find('input').focus();
			});
			//$("#SepDate").val("");
			return false;    ////最好加光标定位;
		}
		if (TmpFixTime=="") {
			$.messager.alert("提示:","请填写标本固定日期!","info",function(){
				$('#FixDate').next('span').find('input').focus();
			});
			//$("#FixDate").val("");
			return false;
		}
...
function InsertDoc(){
	...
	if (FrostFlag != "Y"){	/// 冰冻未勾选时,标本时间不能空 qunianpeng 2018/9/3
		if (TmpSepTime=="") {
			$.messager.alert("提示:","请填写标本离体日期!","info",function(){
				$('#SepDate').next('span').find('input').focus();
			});
...
function InsPisNoObj(itemobj){
	...
	//$HUI.combobox("#OperName").setValue(itemobj.OperName); /// 手术名称
