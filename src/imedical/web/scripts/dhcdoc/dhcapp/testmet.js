...
function initPageDefault(){
	initTestlist();       	/// 初始页面DataGrid申请状态表
...
function initTestlist(){
	...
	var Hospeditor={		//设置其为可编辑
		...
		options: {
			...
			onSelect:function(option){
				///设置类型值
				var ed=$("#testmetlist").datagrid('getEditor',{index:editRow,field:'HospDesc'});
				$(ed.target).combobox('setValue', option.text);
				var ed=$("#testmetlist").datagrid('getEditor',{index:editRow,field:'HospDr'});
	...
	var Flageditor={  //设置其为可编辑
		...
		options: {
			...
			onSelect:function(option){
				///设置类型值
				var ed=$("#testmetlist").datagrid('getEditor',{index:editRow,field:'ATFlag'});
				$(ed.target).combobox('setValue', option.text);  //设置是否可用
				var ed=$("#testmetlist").datagrid('getEditor',{index:editRow,field:'FlagCode'});
				$(ed.target).val(option.value); 
			} 
		}
	...
	var option = {
	    ...
	    onDblClickRow: function (rowIndex, rowData) {//双击选择行编辑
            if (editRow != ""||editRow === 0){ 
                $("#testmetlist").datagrid('endEdit', editRow); 
            } 
            $("#testmetlist").datagrid('beginEdit', rowIndex); 
            
            editRow = rowIndex; 
        }
	};
	var uniturl = LINK_CSP+'?ClassName=web.DHCAPPTesItmMet&MethodName=QueryTestMet&HospID='+LgHospID;
	new ListComponent('testmetlist', columns, uniturl, option).Init(); 
}
...
function insertMetRow(){
	...
	if(editRow>="0"){
		$("#testmetlist").datagrid('endEdit', editRow);//结束编辑，传入之前编辑的行
	}
	 
	$("#testmetlist").datagrid('insertRow', {//在指定行添加数据，appendRow是在最后一行添加数据
		index: 0, // 行数从0开始计算
		row: {ATRowID:'',ATCode:'',ATDesc:'',FlagCode:'Y',ATFlag:'Y',HospDr:LgHospID,HospDesc:LgHospID}
	});
    
	$("#testmetlist").datagrid('beginEdit', 0);//开启编辑并传入要编辑的行
	editRow=0;
}
///保存申请状态
function saveMetRow(){
	...
	var rowsData = $("#testmetlist").datagrid('getChanges');
	...
	//保存数据
	runClassMethod("web.DHCAPPTesItmMet","SaveTestMet",{"params":params},function(jsonString){
		...
		$('#testmetlist').datagrid('reload'); //重新加载
	});
}
...
function deleteMetRow(){
	...
	var rowsData = $("#testmetlist").datagrid('getSelected'); //选中要删除的行
	if (rowsData != null) {
		$.messager.confirm("提示", "您确定要删除这些数据吗？", function (res) {//提示是否删除
			if (res) {
				runClassMethod("web.DHCAPPTesItmMet","DeleteTestMet",{"ATRowID":rowsData.ATRowID},function(jsonString){
					$('#testmetlist').datagrid('reload'); //重新加载
				})
			}
		});
...
function findMetlist()
{
	...
	$('#testmetlist').datagrid('load',{params:params}); 
}	 
/// JQuery 初始化页面
$(function(){ initPageDefault(); })
