...
$(function(){
	...
	var hospComp = GenHospComp("Doc_APP_Itemtemp",hospStr);
...
function InitDetList(){
	...
	var columns=[[
		...
		{field:'aitHospID',title:'aitHospID',width:100,hidden:true}, //editor:textEditor
	...
	var uniturl = LINK_CSP+"?ClassName=web.DHCAPPNotItemTemp&MethodName=QueryAppItemTemp&HospID="+HospID;
...
function saveRow(){
	...
	for(var i=0;i<rowsData.length;i++){
		...
		var tmp=rowsData[i].aitID +"^"+ rowsData[i].aitCode +"^"+ rowsData[i].aitDesc +"^"+ rowsData[i].aitActCode +"^"+ HospID;
		dataList.push(tmp);
	}
