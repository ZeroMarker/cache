...
FileName:   scripts/dhcdoc/interface/JiangXiShengPingTai/ElecHealthCard.js
Description: * 江西省平台电子健康卡统一封装js
             * 本js为接口层js
注意事项: 
    ● 报错弹窗在接口层js(本js)
入参说明:
...
方法说明:
    ...
    ● GetCardEnableFlag: 根据卡类型卡号等判断是否为当前js对应卡类型【其他业务界面,HISUI版本无卡类型】
        入参:   CardTypeDR: 卡类型id,(可能为空)           
                CardNo: 卡号(二维码信息), 
        返回值: 否false, 是true
    ● ReadMagCard: 读卡(或通过二维码等)获取卡号【其他业务界面】
        入参:   medStepCode: 诊疗环节(必须,由各个界面csp传入,应用层js不传入)
...
$(function () {
    ...
    var ElecCardObj = {
        Name: "JiangXiShengPingTai_ElecHealthCard",
        // 业务初始化
        Init: function () {
            return false;
        },
        ...
        GetQRPatInfo: function (medStepCode, CardTypeDR, CardNo) {
            ...
            if (CardNo != "") {
                ...
                var InInfo = CardNo;
                var ReInfo = tkMakeServerCall("DHCDoc.Interface.Outside.JXSElecHealthCard.Methods", "GetEleHeadlthInfo", InInfo);
                if (ReInfo.split("^")[0] == "0") {
	...
	//	PatientID: 病人表id
        ...
        PrintCardInfo: function (PatientID, QRCodeImageData) {
        },
        // 入参：  CardTypeDR  卡类型id, 
        //	CardNo: 卡号, 
        //	PatientID: 病人表id
        ...
        Funcs: {
            QRCodeRegClick: function (InputInfo) {
                ...
                var qCode = tkMakeServerCall("DHCDoc.Interface.Outside.JXSElecHealthCard.Methods", "EleHealthCardRegister", InputInfo);
                ...
                if (success != "0") {
                    $.messager.alert("提示", "电子健康卡注册失败,失败原因:" + qCodeArr[1]);
                    return false;
                }
            },
        }
    ...
    var ElecCardFuncs = {
        ...
        */
        DHCWebCheckCard: function (CardNo, medStepCode) {
            ...
            var ret = tkMakeServerCall("DHCDoc.Interface.Outside.JXSElecHealthCard.Methods", "GetPatCardNoByElecStaticNew", CardNo, "", terminalCode);
            var rtnArr = ret.split("^");
            if ((rtnArr[0] != 0)) {
                alert("获取卡号失败：" + rtnArr[1]);
                return false;
            }
            CardNo = rtnArr[1];
            return CardNo;
        ...
        EmrCard_click: function (PatientID, QRCodeImageData) {
            var Rtn = tkMakeServerCall("DHCDoc.Interface.Outside.JXSElecHealthCard.Methods", "NewJKCardInfo", PatientID);
            ...
            var base64code = ReInfo.split("^")[2];
            var myPAPMIName = ReInfo.split("^")[3];
            var myNO = ReInfo.split("^")[5];
            if (base64code != "") {
