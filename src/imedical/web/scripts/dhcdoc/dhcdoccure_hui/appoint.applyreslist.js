...
function Init(){
	var myAppDate=$('#AppDate').val();
	$('#Apply_AppDate').datebox('setValue',myAppDate);
...
{  
	var cureRBCResSchduleColumns=[[    
					...
					{ field: 'AppedLeftNumber', title: '剩余可预约数', width: 30, align: 'center', sortable: true,resizable: true,
					...
					{ field: 'AvailPatType', title: '可用类型', width: 20, align: 'center', hidden: true,resizable: true
					},
					{ field: 'AutoAvtiveFlag', title: '自动预约启用开关', width: 20, align: 'center', hidden: true
					}
    			 ]];
...
function BtnAppClick(){
    ...
    if (rows.length > 0) {
		...
		for(var j=0;j<DCARowIdArr.length;j++){	
			...
			*/
			var Para=oneDCARowId+"^"+ID+"^"+"M"+"^"+session['LOGON.USERID'];
			//多人预约，需要进行同步处理
			var ObjScope=$.cm({
