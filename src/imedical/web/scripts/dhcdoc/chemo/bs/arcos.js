...
function InitARCOSItemListTabDataGrid(){
	...
	var Columns=[[ 
		...
		/*{field:'ItemZF',title:'组符',width:50,
			formatter: function(value,row,index){
				var ItemOrderZF=row['ItemOrderZF'];
				if (ItemOrderZF!="") {
					return '<font style="color:red"> '+ItemOrderZF+'<font>';
				}
				return "";
			}
...
function AddClickHandle(){
	...
	if (Copyary.length!=0){
		...
		websys_showModal('options').AddCopyItemToList(ServerObj.id,Copyary,ServerObj.GID,ServerObj.GDesc);
