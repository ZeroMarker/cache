...
function InitOrdListDataGrid(){
	...
	var Columns=[[ 
		...
		{field:'OrderMasterSeqNo',title:'关联',width:40,hidden:true},
...
function AddItemToList(rowid,OrdParams,AddMethod,ExpStr) {
	if (PageLogicObj.LinkedMasterOrderRowid==""){
		$.messager.alert("提示","请选择需要补录的医嘱进行补录");
		return RtnObj;
		}
	var RtnObj={
		returnValue:false,
...
function xhrRefresh(obj){
	...
	if (JsonData.length>0){
		$("#selPatKW").html($g("主医嘱:")+JsonData[0].OrdName+$g(",医嘱ID")+JsonData[0].OrderId);
	}
	LoadOrdListData();
	//若"显示已补录"窗口打开,则刷新列表
...
function MulOrdDealWithComDe(OEOrdItem){
	...
	var rytneAry=rtn.split("^")
	if (rytneAry[0]!=0){
		$.messager.alert("处理失败", rytneAry[0]);
	}else{
	...
	function GetOrderItemByItemCongeries(Startid,OrderItemCongeriesJson,Count,AnaesthesiaID){
		...
		*/
		...
		for (var j=0,Length=OrderItemCongeriesObj.length;j<Length;j++) {
			...
			var OrderOperationCode=OrderOperationCode; //GetCellData(Startid, "OrderOperationCode");	//手术列表
	...
	var btntext=[{
		...
		$.ajax('ipdoc.ordtemplatnew.show.hui.csp'+((typeof websys_getMWToken=='function')?("?MWToken="+websys_getMWToken()):""), {
			...
			"success" : function(data, textStatus) {
				createModalDialogParent("TemplateDetail","医嘱模板明细", 280, 700,"icon-w-list",btntext,data,"InitOrdTemplTree()");
