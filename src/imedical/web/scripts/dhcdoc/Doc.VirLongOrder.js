...
function InitOrderTableDataGrid(){
	...
	var OrderTableDataGrid=$("#OrderTable").datagrid({
		...
		onLoadSuccess:function (data){
			...
			//根据就诊号返回病人费用信息 
	    	$(".pricePart").show();
		    if(obj1) obj1.innerHTML="总金额:<font style='color:blue;font-weight:bold;'>"+data.AllPrice+"</font>";
		    if(obj2) obj2.innerHTML="已记账金额:<font style='color:green;font-weight:bold;'>"+data.PayPrice+"</font>";
		    if(obj3) obj3.innerHTML="未记账金额:<font style='color:red;font-weight:bold;'>"+data.NoPrice+"</font>";
		},
		...
		{field:'TExecState',title:'状态',align:'center',width:80,auto:false,
			styler: function(value,row,index){
 				if (row.TExecStateCode){
	 				if( ["未执行","C"].indexOf(row.TExecStateCode) > -1 ){
		 				return "background-color: yellow;"
		 			}
 				}
 			}
