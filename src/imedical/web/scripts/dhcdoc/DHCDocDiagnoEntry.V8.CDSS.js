	...
	function InittabDiagnosEntry(){
        ...
        $("#tabDiagnosEntry").jqGrid({
	        ...
	        url:'oeorder.oplistcustom.new.request.csp?action=GetCDSSDiagList',
			datatype: "json",
	...
	function InittabDiagTempDetail(){ 
		...
		DiagTempDetailDataGrid=$("#tabDiagTemplateDetail").datagrid({  
			...
			onDblClickRow:function(rowIndex, rowData){
				...
				var SDSTermDR=rowData.SDSTermDR;
				var SDSDisplayIDStr  =rowData.SDSDisplayIDStr;
				...
				if (MasterICDRowid!=""){
					...
					AddDiagItemtoList(MasterICDRowid,DHCMRDiaICDICDNotes,"","",SDSTermDR,SDSDisplayIDStr);
	...
	function Add_Diag_row(){
		...
		InitDiagnosICDDescLookUp(rowid+"_DiagnosICDDesc");
	...
	function SaveToPriateTemp(){
		...
		for (var i=0;i<ids.length;i++){
			...
			var DiagnosHiddenPara=GetCellData(ids[i],"DiagnosHiddenPara");
			// 结构化诊断RowId^结构化诊断中心词DR^结构化诊断表达式ID串^是否本次就诊诊断
			var SDSTermDR=DiagnosHiddenPara.split("^")[1];
			if (!SDSTermDR) SDSTermDR="";
			var SDSDisplayIDStr=DiagnosHiddenPara.split("^")[2];
			...
			if (MRDIARowId!=""){ //已保存过的诊断加入模板
				...
				if (DiagnosCat=="证型"){ //找对应主诊断
					...
					if (MRCIDRowId!="") oneDiagStr=MRCIDRowId+String.fromCharCode(1)+DiagnosCatRowId+String.fromCharCode(1)+SDSTermDR+String.fromCharCode(1)+SDSDisplayIDStr;
					else oneDiagStr="$"+DiagnosNotes+String.fromCharCode(1)+DiagnosCatRowId+String.fromCharCode(1)+SDSTermDR+String.fromCharCode(1)+SDSDisplayIDStr;
				}
    ...
    //双击模板或历史记录添加到录入框
	function AddDiagItemtoList(MRCICDRowId,MRCICDNotes,CMFlag,DiagnosPrefix,SDSTermDR,SDSDisplayIDStr){
		var CruRow=GetPreRowId();
	   ...
	   if (SDSTermDR) {
		   if (!SDSDisplayIDStr) SDSDisplayIDStr="";
		   var DiagnosHiddenPara="^"+SDSTermDR+"^"+SDSDisplayIDStr+"^true";
			SetCellData(CruRow,"DiagnosHiddenPara",DiagnosHiddenPara);
	   }
	}
	//检查行是否空白行
 ...
 function getStructDiagnosTarget(){
	 var col=$('#layoutcenter').children().find('tr[datagrid-row-index=0]');
 ...
 /*function InitDiagnosICDDescLookUp(rowid){
	 ...
	 $("#"+rowid+"_DiagnosICDDesc").lookup({
		    ...
		    onSelect:function(index,rec){
		    	...
		    	PageLogicObj.CSDDTargetRowid=rowid;
		    	var target=$("#"+rowid+"_DiagnosICDDesc"); //getStructDiagnosTarget();
		    	CreatPropertyDom(target,rec.MKBTRowId,"",indexTemplate)
		    },
	 ...
	 new Promise(function(resolve,rejected){
			(function(callBackFun){
				function loop(i){
					new Promise(function(resolve,rejected){
						var MRCIDRowId=GetCellData(rowids[i],"MRCIDRowId");
						var DiagnosHiddenPara=GetCellData(rowids[i],"DiagnosHiddenPara");
						var ThisAdmDiagnos=DiagnosHiddenPara.split("^")[3];
						//孕周诊断
					    if ((MRCIDRowId==ServerObj.WeekGestationDia)&&(ThisAdmDiagnos =="true")){
							(function(callBackFun){
								new Promise(function(resolve,rejected){
									var LMPResultStr=tkMakeServerCall("web.DHCDocDiagnosEntryV8","GetPatLMPResultByLMP",ServerObj.PatientID)
									if (LMPResultStr!="") {
						 ...
						 var SDSTermDR=DiagnosHiddenPara.split("^")[1];
						 var SDSDisplayIDStr=DiagnosHiddenPara.split("^")[2];
					     if (((MRCIDRowId!="")||(DiagnosNotes!=""))&&(DiagnosCatRowId!="2")&&(ThisAdmDiagnos=="true")){
						      if ((DiagnosCatRowId=="1")&&(MRDIARowId=="")){ 
							     var SyndromeInfo=GetSyndromeListInfo(i);
							 }else{
								 var SyndromeInfo="";
							 ...
							 var DiagnosBodyPartId=GetCellData(i,"DiagnosBodyPartRowId");							 
						     if(MRDIARowId!=""){ //已保存过的诊断修改
							     var OneDiagItemStr=MRDIARowId+"^"+DiagnosNotes+"^"+DiagnosTypeRowId+"^"+MainDiagFlag+"^"+DiagnosStatusRowId+"^"+DiagnosOnsetDate+"^"+DiagnosBodyPartId+"^"+DiagnosDate+"^"+LongDiagnosFlagRowId+"^"+DiagnosPrefix+"^"+MRCIDRowId+"^"+SDSTermDR+"^"+SDSDisplayIDStr;
							 }else{
								 var OneDiagItemStr=""+"^"+DiagnosNotes+"^"+MRCIDRowId+"^"+DiagnosTypeRowId+"^"+MainDiagFlag
								 OneDiagItemStr=OneDiagItemStr+"^"+DiagnosStatusRowId+"^"+DiagnosOnsetDate+"^"+SyndromeCICDStr+"^"+SyndromeCDescStr+"^"+DiagnosBodyPartId;
								 OneDiagItemStr=OneDiagItemStr+"^"+DiagnosDate+"^"+LongDiagnosFlagRowId+"^"+DiagnosPrefix+"^"+SDSTermDR+"^"+SDSDisplayIDStr;
							 }
							 if (DiagItemStr==""){DiagItemStr=OneDiagItemStr}else{DiagItemStr=DiagItemStr+String.fromCharCode(1)+OneDiagItemStr}
						}
						i++;
...
function CDSSPropertyConfirmCallBack(CDSSStr,DomID){
	var rowid=DomID.split("_")[0];
	var SDSSArr=CDSSStr.split("^");
	var notes=SDSSArr[3];
	var ItemStr=SDSSArr[1]+"^"+SDSSArr[8]+"^"+SDSSArr[6];
	var rtn=DiagItemLookupSelect(ItemStr,rowid);
	if (rtn !==false){
		//结构化诊断RowId^结构化诊断中心词DR^结构化诊断表达式^是否本次就诊诊断
		var DiagnosHiddenPara="^"+SDSSArr[0]+"^"+SDSSArr[2]+"^true";
		SetCellData(rowid,"DiagnosHiddenPara",DiagnosHiddenPara);
		if (notes!="") SetCellData(rowid,"DiagnosNotes",notes);
...
function DiagnosOpertions(cellvalue, options, rowdata){
	if ((rowdata.MRDIARowId)&&(rowdata.DiagnosHiddenPara)) {
		var DiagnosHiddenPara=rowdata.DiagnosHiddenPara;
		var SDSTermDR=DiagnosHiddenPara.split("^")[1];
		var ThisAdmDiagnos=DiagnosHiddenPara.split("^")[3];
		if (SDSTermDR !=""){
			var btn="";
			if (ThisAdmDiagnos =="true") {
				btn = '<a id="'+options.rowId+'_EditPrority" class="editcls" title="修改属性" onclick="ShowCDSSExpProperty(\'' + rowdata['DiagnosHiddenPara'] +'\',\''+options.rowId +'\')"><img src="../scripts_lib/hisui-0.1.0/dist/css/icons/pencil.png"/></a>';
			}else{
				btn = '<a class="editcls" title="置为本次就诊诊断" onclick="AddCDSSToThisAdm(\'' + rowdata['DiagnosHiddenPara'] +'\',\''+options.rowId +'\')"><img src="../scripts_lib/hisui-0.1.0/dist/css/icons/add.png"/></a>';
			}
			btn = btn+'<a class="editcls" title="查看历史" style="margin-left:5px;" onclick="ShowCDSSHistory(\'' + rowdata['DiagnosHiddenPara'] +'\',\''+options.rowId +'\')"><img src="../scripts_lib/hisui-0.1.0/dist/css/icons/search.png"/></a>';
			return btn;
		}else{
			return "";
		}
...
//SDSRowId结构化诊断RowId，SDSTermDR结构化诊断中心词Id,Supplement备注
function ShowCDSSExpProperty(DiagnosHiddenPara,rowid){
	EditRow(rowid);
	var SDSRowId=DiagnosHiddenPara.split("^")[0];
	var SDSTermDR=DiagnosHiddenPara.split("^")[1];
	var Supplement=GetCellData(rowid,"DiagnosNotes");
	var target=$("#"+rowid+"_EditPrority");
	InSDSExpProperty(SDSRowId,SDSTermDR,Supplement,target);
}
function ShowCDSSHistory(DiagnosHiddenPara,rowid){
	var SDSRowId=DiagnosHiddenPara.split("^")[0];
	var url = "dhc.bdp.sds.structdiagnosexplog.csp?SDSRowId="+SDSRowId;
	websys_showModal({
		url:url,
		title:'诊断属性修改历史',
		width:'90%',height:'60%'
...
function AddCDSSToThisAdm(DiagnosHiddenPara,rowid){
	var CDSSArr=new Array();
	var CruRow=GetPreRowId();
	if(!CheckIsClear(CruRow)){
		Add_Diag_row();
		var CruRow=GetPreRowId();
	}
	//var SDSRowId=DiagnosHiddenPara.split("^")[0];
	...
	var DiagnosICDDesc=GetCellData(rowid,"DiagnosICDDesc");
	var DiagnosNotes=GetCellData(rowid,"DiagnosNotes");
	var MRCIDCode=GetCellData(rowid,"MRCIDCode");
	var MRCIDRowId=GetCellData(rowid,"MRCIDRowId");
	CDSSArr[0]=SDSTermDR;
	CDSSArr[1]=DiagnosICDDesc;
	CDSSArr[2]=SDSDisplayIDStr;
	CDSSArr[3]=DiagnosNotes;
	CDSSArr[6]=MRCIDCode;
	CDSSArr[8]=MRCIDRowId;
	CDSSPropertyConfirmCallBack(CDSSArr.join("^"),CruRow);
}
function SynDiagInfoToCDSS(DiagRowIdStr)
{
	if(!dhcsys_getMenuWin) return;
	var menuwin=dhcsys_getMenuWin();
	if(menuwin.CopyDataToCDSS){
		$.cm({
            ClassName:"DHCDoc.Interface.Inside.CDSS",
            MethodName:"GetAllDiagnos", //"GetDiagnosInfo",
            EpisodeID:EpisodeID,
            DiagRowIdStr:DiagRowIdStr
        },function(DiagInfo){
            var CDSSRet=menuwin.CopyDataToCDSS("SYNCHRONOUS_DIAGNOSTIC_INFORMATION",DiagInfo);
        });
	}
}