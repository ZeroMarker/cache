...
function BodyLoadHandler() {
	...
	if(IfOutTime) 
	{
		...
		if((ifDisbled==1)&(appType=="ward")&(opaId=="")&(admType!="E")&(admType!="O"))
		{
			var obj=document.getElementById("btsave");
			if (obj) 
			{
		...
		if((ifDisbled==2)&(appType=="ward")&(opaId=="")&(admType!="E")&(admType!="O"))
		{  
		    var obj=document.getElementById("btsave");
		    obj.style.visibility="hidden";
			alert(t['alert:OutTime']);
...
function LoadData(appList)
{
	...
	var returnOpe=document.getElementById("returnOpe");
	if(returnOpe)
	{
			if(appList[48]=="Y")
			{
				returnOpe.checked=true;
			}
			else
			{
				returnOpe.checked=false;
			}
	}
     ...
     if (anMethod[1])
          {
                ...
                var docamthList=new Array();
                if (objDocMethodList)
                {
                        docamthList=anMethod[1].split("!");
	...
	//display multi-bodysites anaopPreDiagnosis
	...
	if (appList[24]=="") {BloodRowId="";}
	else BloodRowId=appList[24];
	...
	if (appList[48]=="Y")
	{
		btsaveobj=document.getElementById("btsave");
		if (btsaveobj)
		{
...
function CheckData()
{
	...
	else
	{
		...
		{
			var ifDisbled=IfOutTime.value;
			if((ifDisbled==2)&(appType=="ward")&(admType!="O")&(admType!="E"))
			{
					GetCurentDate=document.getElementById("GetCurentDate").value;
					var ret=cspRunServerMethod(GetCurentDate,ifDisbled)
					if(obj.value!=ret)
					{
					alert("急诊手术只能为当前日期")
					return false
    ...
    var obj=document.getElementById("surgeon");
    if (((ifEditDoc!="Y" && docIdList[0]=="")&&(admType!="O"))||((ifEditDoc=="Y" && docDescList[0]=="")&&(admType!="O")))
    {
	    alert(t['alert:selOperDoc']);
	    obj.focus();
	    return false
	}
		obj=document.getElementById("NeedAnaesthetist"); // dyl
    var NeedAnaesthetist    
    if (obj.checked==true) NeedAnaesthetist="Y";
    else  NeedAnaesthetist="N";

    var obj=document.getElementById("anaDoc");
	if ((appType!="op")&&(appType!="ward")&&(obj)&&(obj.value=="")&&(NeedAnaesthetist=="Y")) { //for JST
	...
	if ((appType!="op")&&(appType!="ward")&&(anaMethodList)&&(anaMethodList.length<1)&&(NeedAnaesthetist=="Y")) { //for JST
...
function Save_Click()
{
   ...
   if(obj){ifUserDef=obj.value}
    ...
    //重返手术标记 modefy by whl 20120518
    obj=document.getElementById("returnOpe")
    var returnOpe=""
    if (obj)
    {
	    if(obj.checked==true) var returnOpe="Y";
	    else returnOpe="N";
    }
    str1=str1+"^"+opaMedInfect+"^"+transferMeansCode+"^"+returnOpe; //添加returnOpe 20120518
    ...
    switch(appType)
    {
		...
		case "ward":  //doctor apply: new and update
           ...
           if(obj)selOpType=obj.checked;
			...
			{
				var ifDisbled=IfOutTime.value;
				if((ifDisbled==1)&(opaId=="")&(selOpType==false)&(admType!="E")&(admType!="O"))
				{
					alert(t['alert:OutTime'])
					return ;
				}
...
function GetLabInfo(EpisodeID){
	...
	if(LabInfo[1].length>0){
		for (i=0;i<LabRHBloodType.length;i++) {
			//.substr(LabInfo[1].length-2)
	    	if (LabRHBloodType.options[i].text==LabInfo[1]){
	...
	if(LabInfo[2].length>0){
		if(LabALT) LabALT.value=LabInfo[2];
	}	
	else{
	...
	if(LabInfo[3].length>0){
		for (i=0;i<LabHBSAG.length;i++) {
	    	...
	    	if (LabHBSAG.options[i].text==LabInfo[3])
	    	{   
	...
	if(LabInfo[4].length>0){
		for (i=0;i<LabHCVAB.length;i++) {
	    	//if (LabHCVAB.options[i].text==LabInfo[12].substr(LabInfo[12].length-2)){
		    	if (LabHCVAB.options[i].text==LabInfo[4]){
	...
	if(LabInfo[5].length>0){
		for (i=0;i<LabHIVAB.length;i++) {
	    	//if (LabHIVAB.options[i].text==LabInfo[13].substr(LabInfo[13].length-2)){
		    	if (LabHIVAB.options[i].text==LabInfo[5]){
	...
	if(LabInfo[6].length>0){
		for (i=0;i<Labchmd.length;i++) {
	    	//if (Labchmd.options[i].text==LabInfo[14].substr(LabInfo[14].length-2)){
		    	if (Labchmd.options[i].text==LabInfo[6]){
