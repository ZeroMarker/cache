...
function RefundSaveInfo(RefundFlag){
	...
	if (encmeth!=""){
		...
		if (rtn[0]=='0')
		{
		    ...
		    if (PayMobj) PayMStr=PayMobj.options[PayMobj.selectedIndex].innerText
		
			alert("实际退费金额:"+myRefPaySum+t["ParkYMB"]+",退费方式为"+PayMStr);		/////alert  return Money
...
function QueryYBsysStrik(myINSDivDR){
	...
	*/
	...
	//OutString=InsuDivideStrik(insudHandle,myINSDivDR,myUser,ExpStr,InsuType,CPPFlag);
...
function QueryYBsys(myCID){
	...
	*/
	...
	OutString=InsuOPDivide(insudHandle,myUser,myCID,m_admsource,InsuType,ExpStr,CPPFlag);
...
function RePrt_OnClick(){
	...
	var myAPIRowID=DHCWebD_GetObjValue("OldAccPayINVRowID");
	alert(myAPIRowID)
	if (myAPIRowID=="")  {alert("请输入需要打印的发票号并回车");return;}
