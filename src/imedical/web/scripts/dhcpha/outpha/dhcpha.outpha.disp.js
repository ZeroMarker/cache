...
function InitFYList(){
	...
	var columns=[[
		...
		{field:'TPrescNo',title:'处方号',width:100, //处方预览
			formatter:function(value,row,index){
				return "<a href='#' onclick='showPreviewPrescWindow("+index+")'>"+value+"</a>";
			}},
...
function InitFYListItm()
{
	var columns=[[
		...
		{field:'TRealQty',title:'实发数量',width:80,align:'center',
			editor:{
			type:'numberbox',
				options:{
					precision:2
	...
	$('#fydetailgrid').datagrid({
		...
		onClickCell: function (rowIndex, field, value) {
			...
			var pyflag=selecteddata["TPrintFlag"];
			if (fyflag=="OK"){
				$.messager.alert('提示',"该记录已发药!");
				return
			}
			if (pyflag=="OK"){
				$.messager.alert('提示',"该记录已配药!");
				return
			}
			if (rowIndex != currEditRow) {
	        	if (endEditing()) {
					$("#fydetailgrid").datagrid('beginEdit', rowIndex);
					var realQtyEdt=	$('#fydetailgrid').datagrid('getEditor', { index: rowIndex, field: 'TRealQty' });
				    realQtyEdt.target.focus();
		     	    realQtyEdt.target.select();
					$(realQtyEdt.target).bind("blur",function(){
	                	endEditing();        // 根据 数量或单价变更后计算 金额
	            	});
...
var endEditing = function () {
    ...
    if ($('#fydetailgrid').datagrid('validateRow', currEditRow)) {
        var ed = $('#fydetailgrid').datagrid('getEditor', { index: currEditRow, field: 'TRealQty' });
        var realqty = $(ed.target).numberbox('getValue');
        $('#fydetailgrid').datagrid('updateRow',{
			index: currEditRow,
			row: {TRealQty:realqty}
		});
        $('#fydetailgrid').datagrid('endEdit', currEditRow);
        currEditRow = undefined;
        return true;
    } else {
        return false;
...
function QueryFY(){
	...
	$('#fygrid').datagrid({
		url:commonOutPhaUrl+'?action=QueryDispList',	
		queryParams:{
			params:params}
	});
...
function QueryFYSub(){
	...
	var params=gphl+"^"+prt+"^"+prescno+"^"+phdrow;
	$('#fydetailgrid').datagrid({
...
function DispensingMonitor(dispselectdata,updflag){
	...
	if (updflag==""){ //全发时此标志为U
		...
		for(var rowi=0;rowi<fydetailgridrows;rowi++){
			var oeoriQty=fydetailgridrowsdata[rowi]["TPhQty"]
			var realQty=fydetailgridrowsdata[rowi]["TRealQty"]
			...
			var oeori=fydetailgridrowsdata[rowi]["TOrditm"]
			var unit=fydetailgridrowsdata[rowi]["TUnit"]
