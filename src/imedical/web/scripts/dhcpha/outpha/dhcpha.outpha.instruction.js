...
function btnAddHandler()
{
    ...
    var returnValue= tkMakeServerCall("web.DHCOutPhCode","InsertDHCOutPrintKF",gLocId,instru,instrudesc);
    if(returnValue==-1){
...
function btnDeleteHandler(){
	...
	$.messager.confirm('提示',"确认删除吗？",function(r){
		if(r){
			var retValue=tkMakeServerCall("web.DHCOutPhCode","DeleteDHCOutPrintKF",gLocId,yfcodeid);
			if(retValue==0){
				$('#instructiongrid').datagrid('reload');
			}
			else{
