...
Ext.onReady(function() {
 ...
 items:[   
       ...
       var DaTongAnalyseButton = new Ext.Button({
             ...
             listeners:{
                          "click":function(){   
                              ...
                              //DaTongPrescAnalyse();
                              //MKPrescAnalyse();
							   Ext.Msg.show({title:'注意',msg:'需和第三方合理用药软件接口,暂未开放!',buttons: Ext.Msg.OK,icon:Ext.MessageBox.INFO});
  ...
  var orddetailgridcm = new Ext.grid.ColumnModel([nm,sm,
        ...
        {header:'合理用药',dataIndex:'druguse',width: 80,menuDisabled :'false',renderer:showMKHMTL},
        {header:'审核结果',dataIndex:'result',width:80},
        {header:'登记号',dataIndex:'patid',width: 90,menuDisabled :'false'},
        {header:'姓名',dataIndex:'patname',width: 75,menuDisabled :'false'},
     ...
     var toolform = new Ext.TabPanel({
		...
		items: [orddetailgrid]
					...
					id:'framepaallergy'
		  			...
		  			//src: 'emr.interface.browse.category.csp',  //新版
	 ...
	 var PhaLocSelecter = new Ext.form.ComboBox({
				  ...
				  mode: 'local',
				      listeners:{      
                                ...
                                //alert(Ext.get("DocLocSelecter").dom.value);   //获取id为combo的值
...
function EditReason()
{
       ...
       for (var detaili=0;detaili<detailrow.length;detaili++){
		   	...
		   	if (iaresult=="<font color=red>拒绝</font>"){
				canrefuse="1"
				break;
			}
	   }
	   if (canrefuse==1){
	       Ext.Msg.getDialog().setWidth(500); 
           Ext.MessageBox.alert("提示", "您选择的记录已拒绝!");
           return;
	   }
 ...
 function StopAutoLoadTree()
 {
  ...
  function setDefaultLoc()
   { 
	 ...
	 if (perflag==""){
		Ext.Msg.show({title:'注意',msg:'您所登录的安全组的默认登录科室无此权限 !',buttons: Ext.Msg.OK,icon:Ext.MessageBox.INFO}); 
		return;
	 }
  ...
  function GetListInput()
  {
        ...
        var onlyrefflag=Ext.getCmp('OnlyRefuseBox').getValue();  ///仅拒绝
        if (onlyrefflag){
	    	aduitFlag = "2";
	    	}
