...
function setPatientInfo()
{
	var argParams = {"DiseaseID":"0","PatientID":patientID,"EpisodeID":episodeID,"UserCode":userCode,"SessionID":sessionID,
	               "UserID":userID,"UserName":userName,"SsgroupID":ssgroupID,"UserLocID":userLocID,"IPAddress":ipAddress};
...
function createDocument(tempParam)
{
	...
	if (tempParam.actionType == "CREATEBYTITLE")
	{
		focusDocument("","S002","Last");
	}
	else if (tempParam.actionType == "CREATE")
	{
		focusDocument("","S002","Last");
	}
...
function setDocTempalte(emrDocId,isMutex,isGuideBox)
{
	...
	if (isGuideBox && templateType == "UserTemplate")
	{
...
function signDocument(instanceId,type,signLevel,userId,userName,Image,actionType,description,headerImage,fingerImage)
{
	...
	var argJson = {action:"SIGN_DOCUMENT",args:{"InstanceID":instanceId,"Type":type,"SignatureLevel":signLevel,"actionType":actionType,"AddSignPrefix":signPrefix,"Authenticator":{"Id":userId,"Name":userName,"Image":Image,"Description":description, "HeaderImage":headerImage,"FingerImage":fingerImage},"params":{}}}
...
function eventSaveDocument(commandJson)
{
	...
	else if (commandJson["args"]["result"] == "ERROR")
	{
	    ...
	    {
		    setMessage('病历已存在!','forbid');
		}
		else if ((commandJson["args"]["params"]["logStatus"].indexOf("IdxForLink") >=0))
		{
			setMessage('患者此病历时间在库中已存在,请修改病历时间','warning');
		}
		else
		{
...
function eventInsertSummary(commandJson)
{
	...
	window.showModalDialog("emr.memo.csp?",paramSummary,"dialogHeight:333px;dialogWidth:454px;resizable:yes;status:no");
...
{
    userTemplate = window.showModalDialog("emr.ip.navigation.template.csp?DocID="+param.emrDocId+"&LocID="+userLocID+"&EpisodeID="+episodeID,"","dialogHeight:600px;dialogWidth:500px;status:no");
	...
	if (userTemplate.titleCode != "") 
	{
		var tabParam ={
			...
			"status":"NORMAL",
			"titleCode":userTemplate.titleCode,
			"titleName":userTemplate.titleName,
			"dateTime":userTemplate.dateTime,
			"userTemplateCode":userTemplate.code
		 }; 
...
function changeIMGfreq(returnValue)
{
	jQuery.ajax({
			type : "GET", 
			dataType : "text",
