...
var g_igrid = false;
//创建word编辑器
function wordDoc(tempParam,pluginUrl,argConnect,defFontStyle)
{
	var result = true;
	...
	if (!g_iword)
	{
		...
		var nectresult = cmdSetConnect(argConnect);
...
//创建gird编辑器
function girdDoc(tempParam,pluginUrl,argConnect)
{
	var result = true;
...
//建立数据库连接
function cmdSetConnect(argConnect){
...
//设置只读
function cmdSetReadOnly(instanceIds)
{
	var argJson = "";
	if (instanceIds == "")
	{
