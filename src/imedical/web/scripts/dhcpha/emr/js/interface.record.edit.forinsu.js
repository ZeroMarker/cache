...
function audit(signProperty)
{
	...
	else
	{
		...
		var returnValues = window.showModalDialog("emr.record.sign.forinsu.csp?UserName="+userName+"&UserCode="+userCode+"&PasswordType=INSU",signParam,"dialogHeight:220px;dialogWidth:300px;resizable:yes;status:no");
		returnValues = eval("("+returnValues+")");
		if (returnValues != "" && returnValues != undefined)
		{
			userInfo = returnValues.userInfo;			
...
document.getElementById("spechars").onclick = function(){
	...
	if (returnValues != "")	insertText(returnValues); 
};
