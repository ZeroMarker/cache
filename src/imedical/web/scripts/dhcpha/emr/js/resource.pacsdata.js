...
function initPacsSubTable()
{
	...
	for (var i=1;i<pacsSubItems[0].length;i++)
	{
		...
		{ 
			td= "<th id='" +pacsSubItems[0][i].field+ "' style='display:none;font-size:16px;' ><input type='checkbox' name='SubPacs'/>" + pacsSubItems[0][i].title + "</th>"    
		}
		else
		{
			td= "<th id='" +pacsSubItems[0][i].field+ "' style='border-bottom:#999 1px dotted;font-size:16px;border-right:#999 1px dotted;background-color:#F5F5F5;width:" +pacsSubItems[0][i].width+ ";'><input type='checkbox' name='SubPacs' onclick='checkOnClick(this)' />" + pacsSubItems[0][i].title + "</th>" 			
		}
		$(tr).append(td);
	}
