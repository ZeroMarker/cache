...
$(function () {
    ...
    function KeySign() {
        ...
        if ($('#loginDialog').length === 0) {
            ...
            dialogHtml += '<td><input id="keys" name="keys" class="easyui-combobox"</td>';
            dialogHtml += '</tr>';
            dialogHtml += '<tr>';
            dialogHtml += '<td align="right">密码: </td>';
            dialogHtml += '<td><input id="password" type="password" name="password"';
            dialogHtml += ' onkeypress="if(event.keyCode==13) {handleSign(); return false;}"/></td>';
            dialogHtml += '</tr>';
            dialogHtml += '</table>';
            dialogHtml += '</form>';
            dialogHtml += '</div>';
        ...
        //登录，返回用户名，和签名图片
        function serverLogin(key, operate) {
            if ('' === key) return;
            var loginInfo = '';
            var cert = '';
            var UserSignedData = '';
        ...
        function getSignContent(usrInfo, insID, checkresult) {
            var ret = '';
            var signInfo = iEmrPlugin.SIGN_DOCUMENT({
                    InstanceID: insID,
                    Type: usrInfo.Type,
                    SignatureLevel: usrInfo.Level,
        ...
        this.sign = function (operate, insID, signProperty) {
            ...
            var doSign = function (loginInfo) {
                ...
                var fnSign = function () {
                    try {
                        //文档签名
                        var signInfo = getSignContent(loginInfo, insID, checkresult);
                        ...
                        if (saveRet && 'OK' === saveRet.params.result) {
                            ...
                            privilege.setRevsion(saveRet.params.InstanceID);
                            return true;
                        } else {
                            return false;
                        }
            ...
            var loginInfo = serverLogin(strKey, operate) || '';
            if ('' !== loginInfo) {
                doSign(loginInfo);
            } else {
                ...
                showLoginDialog(function (key) {
                    ...
                    var loginInfo = serverLogin(key, operate) || '';
                    if ('' === loginInfo) {
                        return;
                    }
                    strKey = key;
