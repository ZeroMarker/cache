﻿$(function(){
	...
	if (para)
	{
		var image = para.Gender=="女"?"../scripts/emr/image/icon/women_cq.png":"../scripts/emr/image/icon/men_cq.png";
		$("#photo").append('<img src="'+image+'"/>');
		$(".tdtitle").append('<span>病人ID:</span><span class="tcol1">'+para.PatientNo+'</span>');
...
function modityFavInfo()
{
	$.ajax({ 
        type: "POST", 
        url: "../EMRservice.Ajax.favorites.cls", 
        data: "Action=ModifyInfoMemo&FavInfoID="+ para.ID+"&Memo="+$("#txaMemo").val().replace(/\n|\r\n/g,"\\n"), 
        error: function (XMLHttpRequest, textStatus, errorThrown) { 
            alert(textStatus); 
        }, 
        success: function (data) { 
