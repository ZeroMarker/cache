$(function(){
	...
	$('a').attr('href','#');
	checkWholePrivilege();
	//initQualityListTitle();
	//initQualityList();
});
//初始化工具栏质控消息列表内容
function initQualityList()
{
	$("#qualityMessageList").powerFloat({
	    width: 257,
	    position :"5-7",
	    overflow:scroll,
	    target: function() { 
			...
			var title = "";
		 	if (isArchived == "0")
		 	{
			 	href = '../web.eprajax.qualityreport.cls?EpisodeID='+episodeID+'&RecordType='+isArchived+'&CTLocID='+userLocID+'&SSGroupID='+ssgroupID +'&ResultType=';
			} 
			else
			{
				href = '../web.eprajax.qualityreport.cls?EpisodeID='+episodeID+'&RecordType='+isArchived +'&ResultType='; 
			}
			return href; 
	    },
	    targetMode: "ajax"	
...
//初始化工具栏中的质控消息标题
function initQualityListTitle()
{
	var result = ""
	if (isArchived == "0")
 	{
	 	qualityurl = '../web.eprajax.qualityreport.cls?EpisodeID='+episodeID+'&RecordType='+isArchived+'&CTLocID='+userLocID+'&SSGroupID='+ssgroupID +'&ResultType=Title';
	} 
	else
	{
		qualityurl = '../web.eprajax.qualityreport.cls?EpisodeID='+episodeID+'&RecordType='+isArchived +'&ResultType=Title'; 
	}
	jQuery.ajax({
			type : "GET", 
			dataType : "text",
	...
	if (result != "")
	{
		document.getElementById("qualityMessageList").innerHTML = result;
	}
}
...
window.onbeforeunload = function(){    
	...
	if ($.browser.version == '11.0')
	{
		...
		var c = (document.body.clientWidth-event.clientX) > 700;
		if ((b && window.event.clientY < 0) || ( c && !b && window.event.clientY > 0) || (window.event.screenY > document.body.clientHeight) || window.event.altKey)
		{    
			if (window.frames["framRecord"] && window.frames["framRecord"].length >0)
			{
				var result = window.frames["framRecord"].savePrompt();
