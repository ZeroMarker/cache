//变量声明********************************************
var url = '../EMRservice.Ajax.Appoint.cls';
...
function gridRowClick(g, index, e){
	...
	requestCateCharpterDesc = TrimEnterAndWrite(requestCateCharpterDesc);
...
function commitAppoint(){
	...
	if (selections.length > 0)
	{
		...
		{
			var selectData = selections[k];
			var Index = store.indexOf(selectData);
			expander.toggleRow(Index);
			var expanderGridID = "expanderGrid" + Index;
			...
			expanderGrid.getSelectionModel().selectAll();
		}
	}
	//对每一条记录里面的选中项目进行授权操作
    for (var j = 0; j < total; j++) {
		var expanderGridID = "expanderGrid" + j;
		var expanderGrid = Ext.getCmp(expanderGridID);
		if ((expanderGrid == undefined)||(expanderGrid == "undefined")||(expanderGrid == "null")||(expanderGrid == "NULL")||(expanderGrid == ""))
		{
			continue;
		}
		var selectedRows = expanderGrid.getSelectionModel().getSelections();
		seletedCount = seletedCount + selectedRows.length;
...
function withdrawAppoint(){
	...
	for (var j = 0; j < total; j++) {
		...
		var expandertotal = expanderstore.getCount();

		for (var i = 0; i < expandertotal; i++) {
			if (expanderstore.getAt(i).data['AppointWithdraw'] == true) {
				//ID
				var ID = expanderstore.getAt(i).data['ID'];
...
function doExport(grid, Flag){
    try {
        ...
        for (i = 1, j = temp_obj.length + 1 ; i <= temp_expanderopObj.length; i++, j++) {
            ...
            //设置导出的列格式，解决字符串在导出到excel中变成数值型
			var fld = expanderGrid.getStore().recordType.prototype.fields.get(expanderCM.getDataIndex(i));
            if (fld.type == "auto") {
