...
$(function()
{
	$("#dg").datagrid({
		idField:"rowid",
		url: url+"?action=GetPhConBasInfo",  
		rownumbers:true,
		striped: true,
...
function insertRow()
{
	...
	$("#dg").datagrid('insertRow', {//在指定行添加数据，appendRow是在最后一行添加数据
		index: 0, // 行数从0开始计算
		row: {rowid: '',PhContBCode:'',PhContBDesc: ''}
	});
...
function deleteRow()
{
	...
	if (rows.length > 0) {
		$.messager.confirm("提示", "您确定要删除这些数据吗？", function (res) {//提示是否删除
			if (res) {
				$.post(url+'?action=DeletePhConBas',{"index":rows[0].rowid}, function(data){
...
function saveRow()
{
	...
	{
		if((rows[i].PhContBCode=="")||(rows[i].PhContBDesc=="")){
		...
		for(var j=0;j<allRows.length;j++){
			if((rows[i].PhContBCode==allRows[j].PhContBCode)&&(rows[i].PhContBDesc==allRows[j].PhContBDesc)){
		...
		var tmp=rows[i].rowid+"^"+rows[i].PhContBCode+"^"+rows[i].PhContBDesc;
		dataList.push(tmp);
	} 
	...
	//保存数据
	$.post(url+'?action=UpdatePhConBas',{"dataList":rowstr},function(data){
