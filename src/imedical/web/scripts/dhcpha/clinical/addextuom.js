...
var ajax;
var dg = {
	url: url+'?action=QueryExtUomList',  //csp, 空为null
	columns: columns,  //列信息
	pagesize:150,  //一页显示记录数
	table: '#extuomgrid', //grid ID
	field:'rowid', //记录唯一标识
...
{            
			...
			{
				//更新
				var row = $("#extuomgrid").datagrid("getSelected"); 
				if (!(row))
...
function ReLoad()
{
	...
	$('#extuomgrid').datagrid('load',  {  
				action: 'QueryExtUomList'
	});
