...
$(function(){
	...
	// 定义columns
	var columns=[[
		{field:"monScopeID",title:'monScopeID',width:70,align:'center'},
		{field:"monScopeCode",title:'代码',width:100,editor:texteditor},
		{field:'monScopeDesc',title:'描述',width:600,editor:texteditor},
		...
		{field:'monSubClass',title:'学科分类',width:150,editor:monLocEditor}
	]];
    ...
    $('a:contains("查询")').bind('click',queryMonScope);
...
function insertRow()
{
	...
	$("#dg").datagrid('insertRow', {//在指定行添加数据，appendRow是在最后一行添加数据
		index: 0, // 行数从0开始计算
		row: {monScopeID: '',monScopeCode:'',monScopeDesc: '',monLevID:'',monLevel: '',monSubClassID:'',monSubClass: ''}
	});
...
function deleteRow()
{
	...
	if (rows.length > 0) {
		$.messager.confirm("提示", "您确定要删除这些数据吗？", function (res) {//提示是否删除
			if (res) {
				$.post(url+'?action=DelMonScope',{"params":rows[0].monScopeID}, function(data){
...
function saveRow()
{
	...
	for(var i=0;i<rows.length;i++){
		if((rows[i].monScopeCode=="")||(rows[i].monScopeDesc=="")||(rows[i].monLevID=="")||(rows[i].monSubClassID=="")||(rows[i].monLevel==undefined)||(rows[i].monSubClass==0)){
		...
		for(var j=0;j<allRows.length;j++){
					if((rows[i].monScopeCode==allRows[j].monScopeCode)&&(rows[i].monScopeDesc==allRows[j].monScopeDesc)&&(rows[i].monLevID==allRows[j].monLevID)&&(rows[i].monSubClassID==allRows[j].monSubClassID)){
							$.messager.alert("提示","模板已存在，请重新维护!");
							$('#dg').datagrid('reload');
							return true;
						}
					}
		var tmp=rows[i].monScopeID+"^"+rows[i].monScopeCode+"^"+rows[i].monScopeDesc+"^"+rows[i].monLevID+"^"+rows[i].monSubClassID;
		dataList.push(tmp);
	} 
	var rowstr=dataList.join("||");

	//保存数据
	$.post(url+'?action=SaveMonScope',{"params":rowstr},function(data){
