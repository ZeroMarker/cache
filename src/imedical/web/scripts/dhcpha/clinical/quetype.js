...
$(function()
{
	$("#dg").datagrid({
		idField:"rowid",
		url: url+"?action=GetPhConQueTypeInfo",  
		rownumbers:true,
		striped: true,
...
function insertRow()
{
	...
	$("#dg").datagrid('insertRow', {//在指定行添加数据，appendRow是在最后一行添加数据
		index: 0, // 行数从0开始计算
		row: {rowid: '',PhContQueCode:'',PhContQueDesc: ''}
	});
...
function deleteRow()
{
	//$.messager.alert('提示','<font style="color:red;font-weight:bold;font-size:20px;">如需进行删除操作,请联系信息科！谢谢！</font>','warning');
	//return;
	...
	if (rows.length > 0) {
		$.messager.confirm("提示", "您确定要删除这些数据吗？", function (res) {//提示是否删除
			if (res) {
				$.post(url+'?action=DeletePhConQueTypeInfo',{"index":rows[0].rowid}, function(data){
...
function saveRow()
{
	...
	{
		if((rows[i].PhContQueCode=="")||(rows[i].PhContQueDesc=="")){
			$.messager.alert("提示","类别代码或类别描述不能为空!"); 
			$('#dg').datagrid('reload');
			return false;
		}
		...
		for(var j=0;j<allRows.length;j++){
			if((rows[i].PhContQueCode==allRows[j].PhContQueCode)&&(rows[i].PhContQueDesc==allRows[j].PhContQueDesc)){
							$.messager.alert("提示","模板已存在，请重新维护!");
							$('#dg').datagrid('reload');
							return true;
				}
					}
		var tmp=rows[i].rowid+"^"+rows[i].PhContQueCode+"^"+rows[i].PhContQueDesc;
		dataList.push(tmp);
	} 
	...
	//保存数据
	$.post(url+'?action=UpdatePhConQueType',{"dataList":rowstr},function(data){
