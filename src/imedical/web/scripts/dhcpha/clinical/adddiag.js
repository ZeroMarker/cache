...
var ajax;
var dg = {
	url: url+'?action=QueryDiagFeildList',  //csp, 空为null
	columns: columns,  //列信息
	pagesize:150,  //一页显示记录数
	table: '#diaggrid', //grid ID
	field:'rowid', //记录唯一标识
...
{            
			...
			{
				//更新
				var row = $("#diaggrid").datagrid("getSelected"); 
				if (!(row))
...
﹛﹛﹛﹛﹛﹛var input=code+"^"+desc+"^"+type+"^"+active+"^"+rowid ;
            
            rq.url=url+'?action=AddDiagFeild' ;
            rq.data={"input":input},
...
function ReLoad()
{
	...
	$('#diaggrid').datagrid('load',  {  
				action: 'QueryDiagFeildList'
	});
