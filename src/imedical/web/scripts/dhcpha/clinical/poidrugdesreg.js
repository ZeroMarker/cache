...
var ddrDesWayArr = [{"value":"10","text":'送医疗废物管理办'}];
var ddrDesReaArr = [{"value":"10","text":'质量问题'}, {"value":"20","text":'药品过期'}, {"value":"30","text":'药品召回'}];
$(function(){
...
function InitDefault(){
	 ...
	 * 销毁方式
	 */
	var ddrDesWayCombobox = new ListCombobox("ddrDesWay",'',ddrDesWayArr,{panelHeight:"auto"});
	ddrDesWayCombobox.init();
	
			
	/**
	 * 销毁原因
	 */
	var ddrDesReaCombobox = new ListCombobox("ddrDesRea",'',ddrDesReaArr,{panelHeight:"auto"});
	ddrDesReaCombobox.init();
}
...
function InitDetList(){
	...
	var columns=[[
		...
		{field:'ddrQty',title:'销毁数量',width:120,editor:NumberEditor},
	...
	var option = {
	    ...
	    onDblClickRow: function (rowIndex, rowData) {//双击选择行编辑
	    	...
	    	if ($('#ddrChkFlag').is(':checked')){
				 $.messager.alert('提示','销毁单据已核对完成,不能进行编辑！','warning');
				 return;
			}
	
			if ($('#ddrComFlag').is(':checked')){
				 $.messager.alert('提示','销毁单据已完成,不能进行编辑！','warning');
				 return;
			}
...
function insertRow(){
	...
	if ($('#ddrChkFlag').is(':checked')){
		 $.messager.alert('提示','销毁单据已核对完成,不允许修改！','warning');
		 return;
	}
	
	if ($('#ddrComFlag').is(':checked')){
		 $.messager.alert('提示','销毁单据已完成,请先"取消"后,再操作！','warning');
		 return;
	}
...
function saveRow(){
	...
	///销毁方式
	var ddrDesWay = $("#ddrDesWay").combobox("getValue");
	if (ddrDesWay == ""){
		$.messager.alert("提示:","<font style='font-size:20px;color:red;'>销毁方式不能为空！</font>");
		return;
	}

	///销毁原因
	var ddrDesRea = $("#ddrDesRea").combobox("getValue");
	if (ddrDesRea == ""){
		$.messager.alert("提示:","<font style='font-size:20px;color:red;'>销毁原因不能为空！</font>");
		return;
	}
	var ddrMListData = ddrDeptID +"^"+ ddrUser +"^"+ ddrDesWay +"^"+ ddrDesRea;
	...
	for(var i=0;i<rows.length;i++){
		...
		if (rows[i].ddrInci != ""){
			...
			tmp = tmp +"^"+ rows[i].ddrQty  +"^"+ rows[i].ddrUomID +"^"+ rows[i].ddrBatNo +"^"+ rows[i].ddrExpDate;
			ListData.push(tmp);
		}
	}
	...
	//保存数据
	$.post(url+'?action=saveDesDrgReg',{"ddrID":ddrID, "ddrMListData":ddrMListData, "ddrDListData":ddrDListData},function(jsonString){
...
/// 查找销毁单据
function findTindy(){
	createDesDrugRegWin(setCurrddrNoUI);
}
...
function setCurrddrNoUI(ddrID){
	...
	/// 查询数据
	$.post(url+'?action=GetDesDrgRegMainInfo',{"ddrID":ddrID},function(jsonString){
		...
		$('#ddrDesWay').combobox("setValue",jsonddrObj.ddrDesWayCode); ///销毁方式
		$('#ddrDesRea').combobox("setValue",jsonddrObj.ddrDesReaCode); ///销毁原因
	});
...
function setCurrCompEditable(){
	...
	$('#ddrDesWay').combobox({"disabled":true});
}
...
function clrCurrUI(){
	...
	$('#ddrDesWay').combobox({"disabled":false});
	...
	$("#ddrDept").combobox("setValue",LgCtLocID);
	$("#ddrUser").combobox("setValue",LgUserID);
	$('#ddrDesWay').combobox("setValue",""); ///销毁目的
...
function setComFlag(){
	...
	//保存数据
	$.post(url+'?action=SetDesRegComFlag',{"ddrID":ddrID},function(jsonString){
		var jsonddrObj = jQuery.parseJSON(jsonString);
		if (jsonddrObj.ErrorCode == "0"){
...
function cancelComFlag(){
	...
	//保存数据
	$.post(url+'?action=CancelDesRegComFlag',{"ddrID":ddrID},function(jsonString){
		var jsonddrObj = jQuery.parseJSON(jsonString);
		if (jsonddrObj.ErrorCode == "0"){
...
function DelddrNo(){
	...
	//保存数据
	$.post(url+'?action=DelDesReg',{"ddrID":ddrID},function(jsonString){
		var jsonddrObj = jQuery.parseJSON(jsonString);
		if (jsonddrObj.ErrorCode == "0"){
...
function delddrItm(){
	...
	//保存数据
	$.post(url+'?action=delDdrItm',{"ddrItmID":rowData.ddrItmID},function(jsonString){
		var jsonddrObj = jQuery.parseJSON(jsonString);
		if (jsonddrObj.ErrorCode == "0"){
			$.messager.alert("提示:","删除成功！");
