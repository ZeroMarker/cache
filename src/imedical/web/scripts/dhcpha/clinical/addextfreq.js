...
var ajax;
var dg = {
	url: url+'?action=QueryExtFreqList',  //csp, 空为null
	columns: columns,  //列信息
	pagesize:150,  //一页显示记录数
	table: '#extfreqgrid', //grid ID
	field:'rowid', //记录唯一标识
...
function BodyLoadHandler()
{
 ...
 ﹛﹛﹛extfreqgrid = new DataGrid(dg);
   ﹛﹛extfreqgrid.init();  
       extfreqgrid.clickrow(extfreqgridclick);

	   function extfreqgridclick(rowIndex, rowData)
	   {           
...
{            
			...
			{
				//更新
				var row = $("#extfreqgrid").datagrid("getSelected"); 
				if (!(row))
...
function ReLoad()
{
	...
	$('#extfreqgrid').datagrid('load',  {  
				action: 'QueryExtFreqList'
	});
