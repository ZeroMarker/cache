...
var medsrID="";patientID="";EpisodeID="";editFlag="";adrDataList="";
...
$(function(){
	...
	$('#ApoL').datagrid({
		...
		onSelect: function (rowIndex, rowData) {//单击选择行编辑
	        ...
	        var e = ed[2];//固定 field:'Error' 列存在回车事件
	        $(e.target).bind('keydown', function(event)
	            {
		            if(event.keyCode == "13")
		            {
			            var eds=$("#ApoL").datagrid('getEditor',{index:rowIndex,field:'Error'});
			            var inputCorrect=$(eds.target).val();
			            if(rowData.Code=="2-1")
			            {
				            var QueryDrug=$(e.target);
	        ...
	        $(e.target).bind('keydown', function(event)
	            {
		            if(event.keyCode == "13")
		            {
			            var eds=$("#DispL").datagrid('getEditor',{index:rowIndex,field:'Error'});
			            var inputCorrect=$(eds.target).val();
			            if(rowData.Code=="3-1")
			            {
				            var QueryDrug=$(e.target);
	        ...
	        $(e.target).bind('keydown', function(event)
	            {
		            if(event.keyCode == "13")
		            {
			            var eds=$("#NurL").datagrid('getEditor',{index:rowIndex,field:'Error'});
			            var inputCorrect=$(eds.target).val();
	        ...
	        $(e.target).bind('keydown', function(event)
	            {
		            if(event.keyCode == "13")
		            {
			            var eds=$("#PatL").datagrid('getEditor',{index:rowIndex,field:'Error'});
			            var inputCorrect=$(eds.target).val();
...
function saveMedsReport(flag)
{
	...
	if(flag==1){
		medsrRepAuditList=medsrCurStatusDR+"^"+LgUserID+"^"+LgCtLocID+"^"+LgGroupID+"^"+medadrNextLoc+"^"+medadrLocAdvice+"^"+medadrReceive+"^"+medsrReportType;
	}
	var param="medsrID="+medsrID+"&medsrDataList="+medsrDataList+"&medsrRepAuditList="+medsrRepAuditList; 
	//alert(param);
	//保存
...
function InitPatientInfo(medsrID,adrDataList)
{
   ...
   success: function(val){
		if(val==-1){
			...
			//$('#medsrCreator').attr("disabled","true");
			LocDr=tmp[6];
			$('#medsrRepLocDr').combobox('setValue',tmp[6]);    //科室描述
		...
		for(var i=0;i<Aporows.length;i++){
			...
			if (code=="2-1")
			{
				...
				$('#ApoL').datagrid('updateRow',{	
					index: i,	
					row: {	
					Error: rowData.DrugDesc  //品种错误   药品名称
					}
				});
			}
		}
		...
		for(var i=0;i<Disprows.length;i++){
			...
			if (code=="3-1")
			{
				...
				$('#DispL').datagrid('updateRow',{	
					index: i,	
					row: {	
					Error: rowData.DrugDesc  //配送错误  药品名称
					}
				});
			}
		...
		for(var i=0;i<Nurrows.length;i++){
			...
			if (code=="4-2")
			{
				...
				$('#NurL').datagrid('updateRow',{	
					index: i,	
					row: {	
					Error: rowData.DrugDesc  //药物错误    药品名称
					}
				});
			}
			if (code=="4-4")
			{
				...
				$('#NurL').datagrid('updateRow',{	
					index: i,	
					row: {	
					Error: rowData.PhcInDesc  //给药途径错误  用法
					}
				});
			}
			if (code=="4-5")
			{
				...
				$('#NurL').datagrid('updateRow',{	
					index: i,	
					row: {	
					Error: rowData.PhcFreqDesc  //给药频率错误    频次
					}
				});
			}
		}
