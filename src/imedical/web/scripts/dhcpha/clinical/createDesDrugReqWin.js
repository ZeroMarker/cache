/// 查找点评单据
createDesDrugReqWin = function(FN){
	...
	var newDdrWindowUX = new WindowUX('查找麻精药品申请记录', 'newDdrWin', '1200', '550', option);
	newDdrWindowUX.Init();
	...
	function QueryDdrNoList(){
		...
		var ddrWinDept = $('#ddrWinDept').combobox('getValue');      //申请科室
		var ddrWinUser = $('#ddrWinUser').combobox('getValue');  	 //申请人员

		var ListData = ddrStartDate + "^" + ddrEndDate + "^" + ddrWinDept + "^" + ddrWinUser;

		$('#ddrNoList').datagrid({
			url:url+'?action=QueryReqNoList',
			queryParams:{
				param : ListData}
		});
	}
	...
	function SelDdrNoList(){
		...
		if (rows != null) {
			...
			$.messager.alert('提示','请选择要提取的申请单记录','warning');
			return false; 
		}
	}
	...
	function InitDdrWinList(){
		...
		var columns=[[
			...
			{field:'ddrToLocDesc',title:'接收科室',width:100},
