...
function saveAdrEventReport(flag)
{
	...
	//alert(param);
	//保存
	$.messager.confirm("提示", "是否进行保存数据", function (res) {//提示是否保存
		if (res) {
    		$.ajax({
   	   			type: "POST",
       			url: url,
					...
					if (medrArr[0]=="0") {
						...
						if(flag==1){
							...
							buttondiv.style.display='none';
						        ...
						        submitdiv.style.display='none'; //隐藏提交按钮	
	...
	//获取报表信息
 	$.ajax({
       ...
       success: function(val){
			...
			$('#PatWard').combobox('setValue',tmp[40]);     //病区
			//报告类别
			if (tmp[41]=="med"){
	...
	$.ajax({
		...
		success: function(val){
			...
			$('#PatDiag').attr("disabled","true");
           	$('#PatWard').combobox('setValue',tmp[12]);  //病区
			patIDlog=$('#PatID').val();
		}
	})
}
