...
var ajax;
var dg = {
	url: url+'?action=QueryDisHistoryList',  //csp, 空为null
	columns: columns,  //列信息
	pagesize:150,  //一页显示记录数
	table: '#dishistorygrid', //grid ID
	field:'rowid', //记录唯一标识
...
{            
			...
			{
				//更新
				var row = $("#dishistorygrid").datagrid("getSelected"); 
				if (!(row))
...
﹛﹛﹛﹛﹛﹛var input=code+"^"+desc+"^"+active+"^"+rowid ;
            ...
            rq.url=url+'?action=AddDisHistoryFeild' ;
            rq.data={"input":input},
...
function ReLoad()
{
	...
	$('#dishistorygrid').datagrid('load',  {  
				action: 'QueryDisHistoryList'
	});
