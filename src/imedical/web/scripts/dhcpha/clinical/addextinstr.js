...
var ajax;
var dg = {
	url: url+'?action=QueryExtInstrList',  //csp, 空为null
	columns: columns,  //列信息
	pagesize:150,  //一页显示记录数
	table: '#extinstrgrid', //grid ID
	field:'rowid', //记录唯一标识
...
function BodyLoadHandler()
{
 ...
 ﹛﹛﹛extinstrgrid = new DataGrid(dg);
   ﹛﹛extinstrgrid.init();  
       extinstrgrid.clickrow(extinstrgridclick);

	   function extinstrgridclick(rowIndex, rowData)
	   {           
...
{            
			...
			{
				//更新
				var row = $("#extinstrgrid").datagrid("getSelected"); 
				if (!(row))
...
function ReLoad()
{
	...
	$('#extinstrgrid').datagrid('load',  {  
				action: 'QueryExtInstrList'
	});
