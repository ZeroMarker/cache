...
var init = function(){
	...
	$("#loadgrpData").click(function(){
		var row = icdgrp.getSelected();
		if (row){
			$.messager.confirm("确认","确定要同步 "+ row.ICDCCode + " 分组对照表数据么？",function(r){
				if(r){
					$.messager.progress({
						title: "提示",
						msg: '正在同步数据,请勿关闭浏览器',
			...
			$.messager.confirm("确认","确定要同步所有分组的对照表数据么？",function(r){
				if(r){
					$.cm({ClassName:"web.QueryMrInfoNew",MethodName:"SynchronousDate",type:"all",icdGroup:"",timeout:300000
					},true,function(data){
						$.messager.progress("close");
						if("ok"==data.responseText){
							$.messager.alert("提示","同步请求已发送成功，由于执行时间过长，前端无法监控，请一段时间后后端确认对照表数据");
						}else{
							$.messager.alert("提示","同步失败");
							//console.log("这里有个错误");
						}
