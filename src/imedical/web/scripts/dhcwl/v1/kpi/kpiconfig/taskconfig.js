$(document).ready(function(){
	...
	$("#taskConfigButton").click(function(e){
		$.messager.confirm("提示","您确定操作么？",function(r){
			if (r){
				var initValue,suspendValue;
				initValue = $("#GlobalTaskInitCfg").checkbox("getValue");
				if (initValue){
					initValue = "Y";
				}else{
					initValue = "N";
				}
				suspendValue = $("#GlobalTaskSuspendCfg").checkbox("getValue");
				if (suspendValue){
					suspendValue = "Y";
				}else{
					suspendValue = "N";
				}
				logInfors = "GlobalTaskInitCfg" + "," + "指标任务添加时的默认激活配置" + "," + initValue;
				logInfors = logInfors + "||" + "GlobalTaskSuspendCfg" + "," + "指标任务出错时的挂起配置" + "," + suspendValue;
				//alert(logInfors);
				//return;
				$m({
