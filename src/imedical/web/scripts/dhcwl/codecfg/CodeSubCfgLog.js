...
dhcwl.codecfg.CodeCfgLog=function(kpiId,kpiName){
	...
	var grpCombo=new Ext.form.ComboBox({
		...
		store : new Ext.data.Store({
			proxy:new Ext.data.HttpProxy({url:'dhcwl/codecfg/codecfgservice.csp?action=getSubGrpCombo'}),
    ...
    var columnModel=new Ext.grid.ColumnModel({
        ...
        new Ext.grid.RowNumberer(),{
			...
			renderer : function(value) {
				if (value == '大组明细') {
					return "<span style='color:blue;font-weight:bold;'>大组明细</span><img src='../images/uiimages/createschedule.png' />";
				}else{
					return value;
				}
			},
    ...
    var store = new Ext.data.Store({
        proxy: new Ext.data.HttpProxy({url:serviceUrl+"?action=GetSubLogInfor"}), 
        ...
        bbar:new Ext.PagingToolbar({
		 ...
		 tbar: new Ext.Toolbar([{
			...
			handler:function(){
				...
				//alert(GrplogCombo);
				//return;
				store.proxy.setUrl(encodeURI(serviceUrl+'?action=GetSubLogInfor&start=0&limit=20&searchName='+GrpName+'&modType='+GrpModType+'&operType='+GrpOperType+'&date='+GrpDate+'&grpCode='+GrplogCombo+'&grpFlag=2'+'&onePage=1'));
				store.load();
				codeCfgLogGrid.show();
    	   ...
    	   handler:function(){
			   ...
			   //var value6=Ext.getCmp('grpComboName').setValue('');			   
    	   }
       }]),listeners: { 'beforeedit': function (editor) {
            if (editor.value != '大组明细') return false;
            return true;
        }
	   }
    });
	...
	this.show=function(id,kpiName){
		...
		store.proxy.setUrl(encodeURI(serviceUrl+'?action=GetSubLogInfor&start=0&limit=20&searchName=&modType=&operType=&date=&grpFlag=2&onePage=1'));
		store.load();
		codeCfgLogGrid.show();
		codeCfgLogWin.show();
	}
