// creator by lhh
(function() {
	Ext.ns("dhcwl.docappcfg");
	// 工具函数供导入script文件用
	dhcwl.dhcappcfg.Util = function() {
	};
	dhcwl.dhcappcfg.Util.importScript = function(file) {
		if (!file || file == "")
			return;
	...
	// 工具函数供导入CSS文件用
	dhcwl.dhcappcfg.Util.importCSS = function(file) {
		if (!file || file == "")
			return;
	...
	//判断给定字符串是否只为字母或数字的组合
	dhcwl.dhcappcfg.Util.valideValue=function(code){
	...
	//返回给定年的每月天数
	dhcwl.dhcappcfg.Util.getMonthDays=function(year,month){
	...
	dhcwl.dhcappcfg.Util.nowDate = function() {
		var date = new Date();
	...
	dhcwl.dhcappcfg.Util.yesterday = function() {
		var date = new Date();
		...
		if(day==1){
			...
			day=dhcwl.dhcappcfg.Util.getMonthDays(year,month);
		}else{
			day--;
		}
		date = (year)
	...
	dhcwl.dhcappcfg.Util.nowDateTime = function() {
		var date = new Date();
		date = (date.getFullYear())
				+ '-'
				+ ((date.getMonth() < 9)
	...
	// ajax操作工具方法
	dhcwl.docappcfg.Util.ajaxExc = function(url, para, successFun, successScope,onlyText,timeout) {
		if (!para)
			para = {};
		if (!successFun)
			successFun = null;
	...
	// JS处理本地文件方法
	//浏览本地文件系统目录
	dhcwl.docappcfg.Util.browseFolder = function() {
		var FilePath = "";
	...
	//根据文件路径打开文件
	dhcwl.dhcappcfg.Util.openFile = function openFile(filePath) {
		var fso, f;
	...
	//写内容到指定文件名中
	dhcwl.dhcappcfg.Util.writeFile = function(filename, filecontent) {
		var fso, f, s;
		fso = new ActiveXObject("Scripting.FileSystemObject");
	...
	dhcwl.dhcappcfg.Util.browseFile = function(folderPath) {
		var FilePath = "";
	...
	// 从已打开文件中读取指定字节数的内容
	dhcwl.dhcappcfg.Util.stepReadFile = function(file, stepSize) {
		if (!file)
			return "";
		var content = "", vs = "";
	...
	dhcwl.dhcappcfg.Util.trimLeft = function(s) {
		if (s == null) {
			return "";
		}
