...
dhcwl.mkpi.showkpidata.SetDimFilterRule=function(){
	...
	var kpiTreePanel = new Ext.tree.TreePanel({
		...
		loader:new Ext.tree.TreeLoader({
			dataUrl:'dhcwl/kpi/setfilterservice.csp?action=getDimList&type=root&kpiRuleStr='+kpiRuleStr
		}),
		...
		listeners:{
			...
			'beforeload':function(node){
				...
				type = args[0];
				if ('root'==type) {
					kpiTreeArg = '&dimRuleStr='+kpiRuleStr;
				} else if ('dim'==type) {
					kpiTreeArg = '&dimRuleStr=&dimCode='+node.text;	
				} 
				node.loader = new Ext.tree.TreeLoader({
					dataUrl:'dhcwl/kpi/setfilterservice.csp?action=getDimList&type='+type+kpiTreeArg
				});
			}
		}
	});
				...
				if (btn == 'ok'){
					...
					if (!kpiFilterStr[kpiCode]){
						...
						if (("&&"==logicalOperatorStr)||("||"==logicalOperatorStr)) {
							...
							kpiFilterStr[kpiCode] = tempStr.substring(0,lenOfGet)+' '+logicalOperatorStr+' '+kpiCodeStr.substring(1,kpiCodeLen);
						}
					}
					UpdateTextZoneValue();
			...
			items:[{
				...
				listeners:{
					'click':function(){
						...
						parentWin.setFilterRuleArray(rule,rule);
						setFilterWin.close();
	...
	function setFilter(node){
		...
		var dimCode = "";
		if(node.isLeaf()){
			dimCode = node.parentNode.parentNode.text;
		}else if ('dim'==type){
			dimCode = node.parentNode.text;
		}
		
		if (!!(kpiFilterStr[dimCode])&&("&&"!=logicalOperatorStr)&&("||"!=logicalOperatorStr)){
