...
dhcwl.mkpi.showkpidata.SetKpiRule=function(){
    ...
    var selectedKpiPanel = new Ext.grid.GridPanel({
    	title:'选中的指标',
        id:"selectedKpiPanel",
        width:390,
        height:330,
        ...
        listeners:{
        	"rowdblclick":function(pGrid,pIndex,pEvent){
        		var sm=selectedKpiPanel.getSelectionModel();
        		if(!sm) return;
        		var record = sm.getSelected();
        		if(!record) return;
                var kpiCode=record.get("kpiCode");
                //var flag=dhcwl.mkpi.Util.remove(selectedKpiArr,kpiCode)
                //if(!flag) return;
                if(!mapGet(selectedMap,kpiCode)) return;
                var selStore=kpiListPanel.getStore();
                var recordeData={"kpiCode":kpiCode,"kpiName":record.get("kpiName"),"category":record.get("category"),"section":record.get("section")};
                selStore.add(new kpiRecorde(recordeData));
                selectedKpiPanel.getStore().removeAt(pIndex);
                mapRemove(selectedMap,kpiCode);
                mapRemove(selectedKpiDimMap,kpiCode+"_");
                mapRemove(selectedDimProMap,kpiCode+"_");
                kpiDimListPanel.getStore().removeAll();
                selectedKpiDimListPanel.getStore().removeAll();
        	...
        	"rowclick":function(pGrid,pIndex,pEvent){
        		...
        		if(!record) return;
                var kpiCode=record.get("kpiCode");
        		var store=kpiDimListPanel.getStore();
        		//store.proxy.setUrl(encodeURI(preUrl+'kpidimservice.csp?action=GetKpiDimList&start=0&limit=10&kpiId='+kpiCode));
				store.proxy.setUrl(encodeURI(preUrl+'kpidimservice.csp?action=GetKpiDimList&start=0&limit=0&kpiId='+kpiCode));
				store.load();
				selectedKpi=kpiCode;
				selectedKpiDimListPanel.getStore().removeAll();
				dimProListPanel.getStore().removeAll();
				selectedDimProListPanel.getStore().removeAll();
				var selectedDim=mapGet(selectedKpiDimMap,kpiCode+"_");
				for(var tempPro in selectedDim){
					selectedKpiDimListPanel.getStore().add(new kpiDimRecorde(selectedDim[tempPro]));
				}
        	}
        },
         tbar: [{
            ...
            handler : function(){
            	delete selectedMap;
            	delete selectedKpiDimMap;
            	delete selectedDimProMap;
            	selectedMap={},selectedKpiDimMap={},selectedDimProMap={};
            	selectedKpiPanel.getStore().removeAll();
            	kpiDimListPanel.getStore().removeAll();
            	selectedKpiDimListPanel.getStore().removeAll();
            	dimProListPanel.getStore().removeAll();
				selectedDimProListPanel.getStore().removeAll();
    ...
    var kpiDimListPanel = new Ext.grid.GridPanel({
        ...
        listeners:{
        	"rowdblclick":function(pGrid,pIndex,pEvent){
        		var sm=kpiDimListPanel.getSelectionModel();
        		if(!sm) return;
        		var record = sm.getSelected();
        		if(!record) return;
                var code=record.get("MKPIDimCode");
                if(mapGet(selectedKpiDimMap,code)!=null) return;
                var recordData={"MKPIDimCode":code,"MKPIDimDes":record.get("MKPIDimDes"),"MKPIDimDimDr":record.get("MKPIDimDimDr")};
                selectedKpiDimListPanel.getStore().add(new kpiDimRecorde(recordData));
                kpiDimListPanel.getStore().removeAt(pIndex);
                mapAdd(selectedKpiDimMap,selectedKpi+"_"+code,recordData);
        	}
        }
    });
    ...
    var dimProListPanel = new Ext.grid.GridPanel({
        ...
        listeners:{
        	"rowdblclick":function(pGrid,pIndex,pEvent){
        		var sm=dimProListPanel.getSelectionModel();
        		if(!sm) return;
        		var record = sm.getSelected();
        		if(!record) return;
                var code=record.get("Code");
                if(mapGet(selectedDimProMap,code)!=null) return;
                var recordData={"Code":code,"Name":record.get("Name")};
                selectedDimProListPanel.getStore().add(new dimProRecorde(recordData));
                dimProListPanel.getStore().removeAt(pIndex);
                mapAdd(selectedDimProMap,selectedKpi+"_"+selectedKpiDim+"_"+code,recordData);
        	}
        }
    });
