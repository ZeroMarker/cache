...
(function() {
	Ext.ns("dhcwl.mkpi");
	// 工具函数供导入script文件用
	dhcwl.mkpi.Util = function() {
	};
	dhcwl.mkpi.Util.importScript = function(file) {
		if (!file || file == "")
			return;
	...
	// 工具函数供导入CSS文件用
	dhcwl.mkpi.Util.importCSS = function(file) {
		if (!file || file == "")
			return;
	...
	//判断给定字符串是否只为字母或数字的组合
	dhcwl.mkpi.Util.valideValue=function(code){
	...
	dhcwl.mkpi.Util.getMonthDays=function(year,month){
	...
	dhcwl.mkpi.Util.nowDate = function() {
		var date = new Date();
	...
	dhcwl.mkpi.Util.yesterday = function() {
		var date = new Date();
		...
		if(day==1){
			...
			day=dhcwl.mkpi.Util.getMonthDays(year,month);
		}else{
			day--;
		}
		date = (year)
	...
	dhcwl.mkpi.Util.nowDateTime = function() {
		var date = new Date();
		date = (date.getFullYear())
				+ '-'
				+ ((date.getMonth() < 9)
	...
	// ajax操作工具方法
	dhcwl.mkpi.Util.ajaxExc = function(url, para, successFun, successScope,onlyText,timeout) {
		if (!para)
			para = {};
		if (!successFun)
			successFun = null;
	...
	// JS处理本地文件方法
	//浏览本地文件系统目录
	dhcwl.mkpi.Util.browseFolder = function() {
		var FilePath = "";
	...
	//根据文件路径打开文件
	dhcwl.mkpi.Util.openFile = function openFile(filePath) {
		var fso, f;
	...
	//写内容到指定文件名中
	dhcwl.mkpi.Util.writeFile = function(filename, filecontent) {
		var fso, f, s;
		fso = new ActiveXObject("Scripting.FileSystemObject");
	...
	dhcwl.mkpi.Util.writeFile1 = function(dir,filename, filecontent) {
		var fso, f, s;
		fso = new ActiveXObject("Scripting.FileSystemObject");
		//var dir = dhcwl.mkpi.Util.browseFolder();
		if (dir!="") {
	...
	dhcwl.mkpi.Util.browseFile = function(folderPath) {
		var FilePath = "";
	...
	// 从已打开文件中读取指定字节数的内容
	dhcwl.mkpi.Util.stepReadFile = function(file, stepSize) {
		if (!file)
			return "";
		var content = "", vs = "";
	...
	dhcwl.mkpi.Util.trimLeft = function(s) {
		if (s == null) {
			return "";
		}
	...
	dhcwl.mkpi.Util.add=function(arr,ele){
		var i,len;
	...
	dhcwl.mkpi.Util.remove=function(arr,ele){
		var i,len;
		for(i=0,len=arr.length;i<len;i++){
			if(arr[i]==ele){
	...
	dhcwl.mkpi.Util.mapAdd=function(map,key,value){
		if(!map[key]){
			map[key]=value;
		}
	}
