...
dhcwl.BDQ.SummaryDataQryCfg=function(pObj){
   ...
   // create the itemGrid
		...
		layout:'border',
			...
			items: [
			{
				...
				items:[{
					...
					html: '<iframe id="runqianRpt" width=100% height=100% src="dhccpmrunqianreport.csp?reportName=DTHealth-DHCWL-BaseDataQry.raq"></iframe>'		
				}]				
	...
	function updateGrids(cfgData) {
		...
		comboDateItem.getStore().reload();
		comboDateItem.setValue(dateItemName);
		//Ext.getCmp("summaryComboDateItem").setValue(dateItemName);
