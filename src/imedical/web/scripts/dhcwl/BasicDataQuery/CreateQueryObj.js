    ...
    var QryNameForm = new Ext.FormPanel({
		...
		{       
			...
			store:new Ext.data.Store({
					proxy: new Ext.data.HttpProxy({url:serviceUrl+'?action=searchbasetab'}),
		...
		if (obj.field=="ItemType") {
			...
			if (rec.get("ItemType")=="维度") {
				...
				if (Name=="WorkLoad_Rowid" || Name=="WR_rowid" || Name=="MRIP_rowid" || Name=="WLOP_Rowid" || Name=="DHCMRInfo_RowID" || Name=="DHCMRBase_RowID") {
					...
					obj.grid.getColumnModel().setEditable(4,true);
					obj.grid.getColumnModel().setEditable(5,true);
			}else{
					rec.set("DimType","");
					rec.set("DimCode","");		
					obj.grid.getColumnModel().setEditable(4,false);
					obj.grid.getColumnModel().setEditable(5,false);
			}
		}
	});	
