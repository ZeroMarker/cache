(function(){
	Ext.ns("dhcwl.complexrptsrmx.TestRow");
})();
dhcwl.complexrptsrmx.TestRow=function(rptCode,rptName){
	...
	var preUrl="dhcwl/complexrptsrmx/";

	/// 统计项模型定义:列
	var columnModelItem = new Ext.grid.ColumnModel([
        {header:'名称',dataIndex:'statItemDesc', width: 360, autoWidth:true, sortable: false
        },{header:'编码',dataIndex:'statItemCode', width: 310, sortable: false
        }
    ]);
...
//	});
    ...
    var contextRecorde= Ext.data.Record.create([
        //{name: 'statCode', type: 'string'},statItemCode
...
//        }),
        listeners:{
        	"rowdblclick":function(pGrid,pIndex,pEvent){
                ...
                var selStore=selectedColPanel.getStore();
                if (!checkDataSame(contextCode,selStore)){
                	//var recordeData={"statItemCode":contextCode,"statItemDesc":contextDesc};
                    var recordeData={"statItemCode":contextCode+"."+contextCode,"statItemDesc":contextDesc};
   ...
   var selectedContextPanel = new Ext.grid.GridPanel({
    ...
    var statModeCombo=new Ext.form.ComboBox({
		...
		store : new Ext.data.Store({
			autoLoad : true,
			proxy:new Ext.data.HttpProxy({url:'dhcwl/complexrptsrmx/rptdimservice.csp?action=getStatModeCombo'}),
...
//			},
			'afterrender': function(combo) {
				url=encodeURI('dhcwl/complexrptsrmx/rptdimservice.csp?action=getRptCfgMode&rptCode='+outthis.rptCode);
	...
	rowColBar=new Ext.Toolbar([
           '条件类型:',{text:'      '},{
               ...
               vertical: true,
               items: [
                {boxLabel: '行条件', name: 'rowflag', inputValue: 1,hidden:true},   
                {boxLabel: '列条件', name: 'rowflag',inputValue: 2,checked: true,hidden:true}   
               ]
            }
        ])
...
//		},
    ...
    */
    ...
    var textZone = new Ext.form.TextArea({
		...
		maskRe:/37|38|39|40/,   // 键盘键左右上下11
		listeners :{
			'afterrender': function(textZone) {
				var aa=outthis.rptCode // preUrl+
				//url:preUrl+'rptdimservice.csp?action=getRptCfg&rptCode='+outthis.rptCode+'&codeType=Row'
				url=preUrl+'rptdimservice.csp?action=getRptCfgFilter&rptCode='+outthis.rptCode;
				//url=encodeURI('dhcwl/complexrptsrmx/rptdimservice.csp?action=getRptCfgFilter&rptCode='+outthis.rptCode);
  				dhcwl.mkpi.Util.ajaxExc(url,{},
					function(jsonData){
						if(jsonData.success==true && jsonData.tip=="ok"){
							var statDatas=jsonData.data;
	...
	var outputPanel = new Ext.Panel({
		...
		items:[{region:'center',
			...
			items:[{
				...
				listeners:{
					'click':function(){
						var rptFilterCfg = new dhcwl.complexrptsrmx.RptFilterCfg();
...
//		});
			...
			'dblclick':function(node, e){
           	...
           	'click':function(node, e){
				...
				if((node.attributes.className=="DHCWL.ComplexRpt.StatItem")&&(!node.hasChildNodes())){
					...
					updateTree(treeStatCode,'dimPro','GetKpiDimAndPro');
	...
	function OnConfirm(){
		...
		//paraValues='rptCode='+outthis.rptCode+'&rptMode='+outMode+'&rptContext='+outContextString+'&rptRow='+outRowString+'&rptCol='+outColString+'&text='+text;
   		//dhcwl.mkpi.Util.ajaxExc(preUrl+'rptcfgservice.csp?action=AddRptCfg&'+paraValues);
   		dhcwl.mkpi.Util.ajaxExc(preUrl+'rptcfgservice.csp?action=AddRptCfg&'+paraValues,{
   			rptFilter:outFilter
   		},function(jsonData){
			if(jsonData.success==true && jsonData.tip=="ok"){
		...
		//-------------
		condPara=outMode+"@"+outColString+"@"+outFilter
	    var QueryForm=parentWin.getForm();  // QueryForm
    	QueryForm.getForm().findField('CondStr').setValue(condPara);
    	
    	//var rptCombo=parentWin.getRptCombo();
    	//rptCombo.setValue("");
		//-------------
		rptCfgWin.close();	
	...
	function updateTree(rptCodes,flag,statAction){
		var url=encodeURI('dhcwl/complexrptsrmx/rptdimservice.csp?action='+statAction);
		dhcwl.mkpi.Util.ajaxExc(url,
			{
				statCodes:rptCodes,
				Flag:flag
	...
	function checkNodeSame(nodeData,objPanelStore){
		...
		this.showRptCfgWin=function(parent){
			parentWin=parent;
			rptCfgWin.show();
			//var QueryForm=parentWin.getForm();
			return rptCfgWin;
		}
}