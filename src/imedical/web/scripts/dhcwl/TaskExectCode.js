...
dhcwl.mkpi.TaskExectCode=function(){
	var execCodeType='TAS', execCodeSubType="";
	var parentWin=null;
        ...
        bbar:new Ext.PagingToolbar({
            ...
            listeners :{
	        	'beforechange':function(pt,page){
					store.proxy.setUrl('dhcwl/kpi/executecodeservice.csp?action=SECKPIEXCList&type='+execCodeType+'&subType='+execCodeSubType);
	        	}
            }           
	...
	this.setExecCodeType=function(realType, subType){
		execCodeType=realType;
		execCodeSubType=subType;
	}
    this.showWin=function(){
		var store=Ext.getCmp('executeCodeSelection').getStore();
		store.proxy.setUrl('dhcwl/kpi/executecodeservice.csp?action=SECKPIEXCList&start=0&limit=16&type='+execCodeType+'&subType='+execCodeSubType);
		store.load();
    	exeCodeWin.show();
    }
}