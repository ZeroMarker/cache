...
function rowdblclickFn(grid, rowIndex, e) {
	...
	var lnk = "dhcnuremrcomm.csp?" + "&EmrCode=DHCNURANHUI24" + "&EpisodeID=" + EpisodeID + "&DHCMoudDataRowId=" + RowId;
	//var StartDate = formatDate(Ext.getCmp("mygridstdate").getValue()); 
	//var EndDate = formatDate(Ext.getCmp("mygridenddate").getValue()); 
	//var lnk= "dhcnuremrcomm.csp?"+"&EmrCode=DHCNURANHUI24;
...
function modccevent() {
	...
	if (objRow.length == 0) {
		...
		//alert(EpisodeID)
		var lnk = "DHCNurEmrComm.csp?" + "&EmrCode=DHCNURANHUI24&EpisodeID=" + EpisodeID + "&NurRecId=" + par; //"&DtId="+DtId+"&ExamId="+ExamId
...
function FindSPLIST() {
	...
	var parr = "DHCNURANHUI24^" + adm + "^" + loc + "^" + "^" + "^" + IfCncelRec;
	//var parr = "DHCNURANHUI24^"+adm+"^"+loc+"^"+StDate.value+"^"+Enddate.value;
	//alert(parr)
...
function newEvent() {
	...
	if (logonht == 1) {
		var lnk = "DHCNurEmrComm.csp?" + "&EmrCode=DHCNURANHUI24&EpisodeID=" + EpisodeID; //"&DtId="+DtId+"&ExamId="+ExamId
	}

	if (logonht == 0) {
		var lnk = "DHCNurEmrComm.csp?" + "&EmrCode=DHCNURANHUI24&EpisodeID=" + EpisodeID; //"&DtId="+DtId+"&ExamId="+ExamId
	}
	//var topValue=(window.screen.height-800)/6;
	...
	//window.open(lnk, "htm", 'toolbar=no,location=no,directories=no,resizable=yes,top='+topValue+',left='+leftValue+',width=1000,height=800');
...
function newfourdan() {
	var lnk = "dhcnuremrcomm.csp?" + "&EmrCode=DHCNURANHUI24" + "&EpisodeID=" + EpisodeID
		//var StartDate = formatDate(Ext.getCmp("mygridstdate").getValue()); 
		//var EndDate = formatDate(Ext.getCmp("mygridenddate").getValue()); 
		//var lnk= "dhcnuremrcomm.csp?"+"&EmrCode=DHCNURANHUI24;
	window.open(lnk, "htm", 'left=520,top=55,toolbar=no,location=no,directories=no,resizable=yes,width=860,height=800');


}

function newfourdan111() {
	//alert("11");	
	myId = "";
	var arr = new Array();
	// 录入界面
	var a = cspRunServerMethod(pdata1, "", "DHCNURANHUI24", EpisodeID, "");
	arr = eval(a);
...
function scoreAdd2(obj) {
	...
	if (scorehj2 >= 45) {
		Ext.Msg.alert('提示', "评分>45分,请转至跌倒坠床高危患者追踪记录单,每周定期评估!");
		return;
	}
}
