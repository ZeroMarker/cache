...
var partScoreArray = ["Item17", "Item18", "Item19", "Item20", "Item21", "Item22"];
ScoreItem[0] = "^Item17^Item18^Item19^Item20^Item21^Item22^";
ValueItem[0] = "Item16";
NameItem[0] = "Item23";
DateItem[0] = "Item25";
TimeItem[0] = "Item26";
var prncode = "DHCNURMouldPrn_MDDWJPGLB";
...
function BodyLoadHandler() {
 ...
 /* var but = Ext.getCmp("butPrint");
  ...
  // firstScore.focus();
  //计算各项radio分数	
  calScore("Item7", "Item17");
  calScore("Item8", "Item18");
  calScore("Item9", "Item19");
  calScore("Item10", "Item20");
  calScore("Item11", "Item21");
  calScore("Item12", "Item22");
 /* if(NurRecId==""){
...
function Save() {
	//by lmm  增加年份保存
	var recDate = Ext.getCmp("Item25")
	var recYear="";
	if(recDate.getValue()!=""){
		recYear=formatDate(recDate.getValue()).split("-")[0]
	}
	var recYearObj = Ext.getCmp("Item345")
  ...
  var gform = Ext.getCmp("gform");
  gform.items.each(eachItem1, this);
  var arr = new Array("Item17", "Item18", "Item19", "Item20", "Item21", "Item22");
  var count = 0;
  ...
  var stid = NurRecId;
  if (NurRecId != "") {
    var Id = cspRunServerMethod(SaveRec.value, "^RecLoc|" + session['LOGON.CTLOCID'] +"^EmrUser|" + session['LOGON.USERID'] + "^EmrCode|" + EmrCode + "^" + ret + "^DetLocDr|" + session['LOGON.CTLOCID'] + "^EpisodeId|" + EpisodeID + "&" + checkret + "&" + comboret + "&" + radioret, NurRecId);
    currid = Id.split("||")[0];
  }
  else {
    var Id = cspRunServerMethod(SaveRec.value, "^RecLoc|" + session['LOGON.CTLOCID'] +"^EmrUser|" + session['LOGON.USERID'] + "^EmrCode|" + EmrCode + "^" + ret + "^DetLocDr|" + session['LOGON.CTLOCID'] + "^EpisodeId|" + EpisodeID + "&" + checkret + "&" + comboret + "&" + radioret, "");
    if (Id != "") {
      NurRecId = Id;
      subid=Id;
    }
...
function calScore(partItem, scoreItem) {
  ...
  {	
	...
	case "Item9":
		...
		item.on("check", function (field, checked) {
        if (checked == true) {
          if (field.boxLabel == "依扶家具") {
            partScore = 30;
          } else if (field.boxLabel == "使用拐杖/手杖"){
            partScore = 15;
          }else {
            partScore = 0;
          }
	...
	case "Item11":
		...
		item.on("check", function (field, checked) {
        if (checked == true) {
          if (field.boxLabel == "功能受损") {
            partScore = 20;
          } else if (field.boxLabel == "虚弱无力"){
            partScore = 10;
          }else {
            partScore = 0;
          }
	...
	case "Item12":
		...
		item.on("check", function (field, checked) {
        if (checked == true) {
          if (field.boxLabel == "忘记自己受限制") {
            partScore = 15;
          } else if (field.boxLabel == "高估自己的能力"){
            partScore = 15;
          }else {
            partScore = 0;
          }
