...
function modccevent()
{
	...
	if (objRow.length == 0) {
		...
		if (Status=="已审核") 
		{var Status="A"}
		var lnk= "DHCNurEmrComm.csp?"+"&EmrCode=DHCNURXHSB_geiyao&EpisodeID="+EpisodeID+"&NurRecId="+par+"&Status="+Status  ;//"&DtId="+DtId+"&ExamId="+ExamId
...
function newEvent()
{
    ...
    if (logonht==1)
	{
		var lnk= "DHCNurEmrComm.csp?"+"&EmrCode=DHCNURXHSB_geiyao&EpisodeID="+EpisodeID  ;//"&DtId="+DtId+"&ExamId="+ExamId
	}
...
function find()
{
	...
	var parr = "DHCNURXHSB_geiyao^"+adm+"^"+loc+"^"+StDate.value+"^"+Enddate.value+"^"+IfCancelRec;
  //	alert(parr);
