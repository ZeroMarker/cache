...
function modccevent()
{
	...
	if (objRow.length == 0) {
	  ...
	  //alert(EpisodeID)
		var lnk= "DHCNurEmrComm.csp?"+"&EmrCode=DHCNURPL_PG&EpisodeID="+EpisodeID+"&NurRecId="+par ;//"&DtId="+DtId+"&ExamId="+ExamId
...
function newEvent()
{
    ...
    if (logonht==1)
	{
		var lnk= "DHCNurEmrComm.csp?"+"&EmrCode=DHCNURPL_PG&EpisodeID="+EpisodeID  ;//"&DtId="+DtId+"&ExamId="+ExamId
	}
...
function find()
{
	...
	//var parr = "DHCNURXHFFYC^"+adm+"^"+loc+"^"+StDate.value+"^"+Enddate.value+"^"+IfCncelRec;
		var parr = "DHCNURPL_PG^"+adm+"^"+loc+"^"+"^"+"^"+IfCncelRec;
  //alert(parr);
