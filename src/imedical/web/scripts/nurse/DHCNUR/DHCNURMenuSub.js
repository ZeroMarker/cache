...
function loadgname()
{
	...
	var objloc = Ext.getCmp("locbox").getValue();
	//alert(objloc)
	gbox=new Array()
	tkMakeServerCall("User.DHCNURMenu","getgbox",objloc,"addgbox")	
...
LocBox.on('select', function() {
		...
		{
		  loadgname() //加载根目录
		  var a=tkMakeServerCall("User.DHCNURMenuSub","iflochaveset",objloc.getValue())	
	      if(a=="")  //该科没有保存配置
	      {
...
function BodyLoadHandler(){
   ...
   { 
	...
	//alert(loginloc)
	loadgname() //加载根目录
	//inidata("KNurseName","Desc","User.DHCNURMenuSub:GetRootname","Loc$"+loginloc);
  ...
  if (EpisodeID=="")
  {
	  ...
	  //alert(EpisodeID)
	  var a=tkMakeServerCall("User.DHCNURMenuSub","iflochaveset",EpisodeID)	
	  if(a=="")  //该科没有保存配置
	  {
		  Ext.getCmp("deleteloc").setDisabled(true)
		   Ext.getCmp("add").setDisabled(true)
