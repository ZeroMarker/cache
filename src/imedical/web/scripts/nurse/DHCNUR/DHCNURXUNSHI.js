...
if (GetNurseRecSet){
	var ret=cspRunServerMethod(GetNurseRecSet.value,"DHCNURXUNSHI");
	var hh=ret.split("^");
...
function BodyLoadHandler(){
    ...
    var sm2 = new Ext.grid.RowSelectionModel({
  	...
  	grid.setTitle("<font Size='2'>"+gethead()+"<font/>");
  	var mydate=new Date();
...
grid.on('mouseover',function(e){//添加mouseover事件
	...
	if(index!==false){//当取到了正确的列时，（因为如果传入的target列没有取到的时候会返回false）
		...
		if ((totalRow)&&(totalRow.indexOf("入量")>-1))
		{
...
grid.getStore().on('load',function(s,records){
          var girdcount=0;
          s.each(function(r){
              if(r.get(DisplaySumInName)=='总入量'){
                    grid.getView().getRow(girdcount).style.backgroundColor='#F7FE2E';
              }
              if(r.get(DisplaySumInName)=='入量'){
...
function PrintSet()
{
 ...
 //debugger;
   ...
   // debugger;
    REC=new Array();
 	var a=cspRunServerMethod(GetNurPrnSet.value,"DHCNURXUNSHI",EpisodeID,'addprnset');
...
function find(){
	...
	var parr = adm + "^" + StDate.value + "^" + StTime.value + "^" + Enddate.value + "^" + EndTime.value + "^"+"DHCNURXUNSHI" + "^" + IfCancelRec+"^"+CurrHeadDr;
	// debugger;
...
{  //查询出入液量合计
	...
	//alert(parr);
  	//debugger;
 	 	var a=cspRunServerMethod(GeInOutAmount.value,adm,StDate.value,StTime.value,Enddate.value,EndTime.value,"DHCNURXUNSHI");
	...
	/*
	mygrid.getSelectionModel().getSelections()[0].set(DisplaySumInName,"24H入量总结="); 
	mygrid.getSelectionModel().getSelections()[0].set(DisplaySumInAmount,tt[0]); 
	mygrid.getSelectionModel().getSelections()[0].set(DisplaySumOutName,"24H出量总结="); 
	mygrid.getSelectionModel().getSelections()[0].set(DisplaySumOutAmount,tt[1]);
	mygrid.getSelectionModel().getSelections()[0].set(PartInName,"入量分项="); 
	mygrid.getSelectionModel().getSelections()[0].set(PartInAmount,tt[2]); 
	mygrid.getSelectionModel().getSelections()[0].set(PartOutName,"出量分项="); 
	...
	//mygrid.getSelectionModel().getSelections()[0].set("CaseMeasure","");
	//OutQtAmount:a2,
	*/
	...
	for (var i = 0; i < list.length; i++) {
	  ...
	  var flag="0";
		...
		if ((str!="")&&(flag=="0"))
		{
			...
			var a=cspRunServerMethod(RecSave.value,EpisodeID,str,session['LOGON.USERID'],"DHCNURXUNSHI",session['LOGON.GROUPDESC']);
			if (a!="0")
			{
				alert(a);
				return;
...
function MultiFun()
{
	...
	var ret= cspRunServerMethod(GetMulti.value, "DHCNURXUNSHI");
	...
	var tabstr=cspRunServerMethod(getcheckform.value, "DHCNURXUNSHI",ab);
...
function multiSel(ret)
{
    ...
    var itname=grid1.getSelectionModel().getSelections()[0].get("itm4"); 
	  var getcheckform=document.getElementById('getcheckform');
		alert(itname+"!!"+code)
		var ret=cspRunServerMethod(getcheckform.value, "DHCNURXUNSHI",itname,code,"");	
...
{ //小结
	...
	else
	{
	  ...
	  else
	  {
		...
		//alert(str);
		var a=cspRunServerMethod(SaveOutIn.value,EpisodeID,str,session['LOGON.USERID'],"DHCNURXUNSHI");
		find();
	}
}
function InOutSum()
    ...
    if (selModel.hasSelection()) {   
       	// Ext.Msg.confirm("警告", "确定要删除吗？", function(button) {   
				...
				Ext.each(selections, function(item) {
					...
					//var ifDrug=document.getElementById("ifDrug").value;
					//var drugFlag=cspRunServerMethod(ifDrug,item.data.Oew,item.data.OrdSub)
					//if((drugFlag=="false")||(seqno!=item.data.OrdSub))
					//{
						//des="";
					//}
					...
					for (var i = rowIndex-1; i >=0; i--) {
						...
						if (subseqno==seqno)
						{
							//var drugFlag=cspRunServerMethod(ifDrug,store.getAt(i).data.Oew,store.getAt(i).data.OrdSub)
							//if((drugFlag=="false")||(subseqno!=store.getAt(i).data.OrdSub))
							//{
							//	break;
							//}
							//else
					...
					for (var i = rowIndex+1; i < store.getCount(); i++) {
						...
						{
							//var drugFlag=cspRunServerMethod(ifDrug,store.getAt(i).data.Oew,store.getAt(i).data.OrdSub)
							//if((drugFlag=="false")||(subseqno!=store.getAt(i).OrdSub))
							//{
							//	break;
							//}
							//else
...
{ 
   ...
   // debugger;
	...
	var parr="DHCNURXUNSHI^"+EpisodeID+"^CaseMeasureXml^"+rowid;
	//alert(parr);
	var arr = new Array();
	var a = cspRunServerMethod(pdata1, "", "DHCPatOrdList", EpisodeID, "");
	arr = eval(a);
...
function sureMeasure()
{
   ...
   // var parr="DHCNURXUNSHI^"+EpisodeID+"^CaseMeasureXml";
...
function printNurRec()
{
		...
		var ret=tkMakeServerCall("web.DHCMGNurComm","PatInfoNew",EpisodeID,session['LOGON.CTLOCID']);
		//var ret=cspRunServerMethod(GetHead.value,EpisodeID);
		var hh=ret.split("^");
		//debugger;
		var a=cspRunServerMethod(GetPrnSet.value,"DHCNURXUNSHI",EpisodeID); //page, caredattim, prnpos, adm,Typ,user
		if (a=="") return;
		var GetLableRec=document.getElementById('GetLableRec');
		var LabHead=cspRunServerMethod(GetLableRec.value,"DHCNURXUNSHI^"+session['LOGON.CTLOCID']+"^"+EpisodeID+"^"+session['LOGON.USERID']);
		var tm=a.split("^");
		var stdate="" //tm[2];
		var stim="" //tm[3];
		var edate="" //tm[4];
		...
		PrintComm.ItmName = "DHCNurSSPrnMould"; //338155!2010-07-13!0:00!!
		//debugger;
		var parr=EpisodeID+"!"+stdate+"!"+stim+"!"+edate+"!"+etim+"!DHCNURXUNSHI"+"!"+CurrHeadDr+"!"+session['LOGON.WARDID'];
		PrintComm.ID = "";
		PrintComm.MultID = "";
	   ...
	   alert("234")
		...
		//alert(pages+","+CareDateTim+","+stprintpos+","+EpisodeID+","+"DHCNURXUNSHI"+","+session['LOGON.USERID']+","+PrintComm.PrnFlag);
		//PrnFlag==1说明是打印预览
		if (PrintComm.PrnFlag==1) return;
		//如果原记录保存打印到第8页则当打印第8页之前页时不保存打印记录
		if (pages<aa[0]) return;
		var a=cspRunServerMethod(SavePrnSet.value,pages,CareDateTim,stprintpos,EpisodeID,"DHCNURXUNSHI",session['LOGON.USERID']); //page, caredattim, prnpos, adm,Typ,user
		//find();
}
