...
function gethead()
{
 ...
 function BodyLoadHandler(){
	...
	for (var i = 0; i < len; i++) {
		if ((grid.getColumnModel().getDataIndex(i) == 'par')||(grid.getColumnModel().getDataIndex(i) == 'rw')) {
   ...
   tobar.addButton(
    ...
    {	   
	   ...
	   {
			    text: "生成所有图片",
			   handler:function(){MakePicturePGd(prncode)},
...
function cellclick(obj)
{
   ...
   for (colIndex = 2; colIndex < 10; colIndex++) 
    {
      ...
      if (isNaN(celldata)) celldata=0;
      sum=sum+ parseInt(celldata);     
      //alert(colname+"="+celldata);    
    } 
    mygrid.store.getAt(rowIndex).set(mygrid.getColumnModel().getDataIndex(10),sum);
