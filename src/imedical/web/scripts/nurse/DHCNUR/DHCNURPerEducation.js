...
//	}
	...
	mygrid.store.on("beforeload",function(){   
		  ...
		  var mygrid = Ext.getCmp("mygrid");
	    mygrid.store.baseParams.parr1=PerID;
...
function editAudite()
{
  ...
  var tm=ret.split('^')
	...
	var aa=cspRunServerMethod(pdata1,"","DHCNURPerEducationAdd","","");
	var arr=eval(aa);
	...
	Ext.getCmp("PerTotalHours").setValue(ha1.items("PerTotalHours"));
	Ext.getCmp("PerCredit").setValue(ha1.items("PerCredit"));
	Ext.getCmp("PerCreditType").setValue(ha1.items("PerCreditType"));
	Ext.getCmp("PerTrainContent").setValue(ha1.items("PerTrainContent"));
	Ext.getCmp("PerCourseName").setValue(ha1.items("PerCourseName"));
	Ext.getCmp("PerTrainHabitude").setValue(ha1.items("PerTrainHabitude"));
	Ext.getCmp("PerTrainUnits").setValue(ha1.items("PerTrainUnits"));	
	window.show();
	var btnSave=Ext.getCmp("btnSave");
...
function editAdd()
{
  ...
  var tm=ret.split('^')
	...
	var perTrainHabitude=Ext.getCmp('PerTrainHabitude');
	perTrainHabitude.setEditable(false);
	perTrainHabitude.triggerAction='all';
	var perCreditType=Ext.getCmp('PerCreditType');
	perCreditType.setEditable(false);
	perCreditType.triggerAction='all';
	var perName=Ext.getCmp("PerName");
	perName.disabled=true;
	Ext.getCmp("PerName").setValue(ha1.items("PerName"));		
	var perID=Ext.getCmp("PerID");
...
function addNew()
{
  ...
  var tm=ret.split('^')
	sethashvalue(ha,tm);
	var a = cspRunServerMethod(pdata1, "", "DHCNURPerEducationAdd", "", "");
	arr = eval(a);
	var window= new Ext.Window({
		title : '添加教育培训',
		id : "gform2",
...
function SureCheck(RowId,edit1)
{
	...
	var PerTotalHours=Ext.getCmp("PerTotalHours").getValue();//总学时
	var PerTrainContent=Ext.getCmp("PerTrainContent").getValue();//培训内容
	if(PerTrainContent==""){Ext.Msg.alert('提示',"请输入培训内容！");return;}
	
	var PerCredit=Ext.getCmp("PerCredit").getValue();//学分
	if(PerCredit==""){Ext.Msg.alert('提示',"请填写学分！");return;}
	var PerCreditType=Ext.getCmp("PerCreditType").getValue();//学分类型
	if(PerCreditType==""){Ext.Msg.alert('提示',"请选择学分类型！");return;}
	var PerCourseName=Ext.getCmp("PerCourseName").getValue();//培训班名称
	var PerTrainHabitude=Ext.getCmp("PerTrainHabitude").getValue();//培训性质
	if(PerTrainHabitude==""){Ext.Msg.alert('提示',"请选择培训性质！");return;}
	var PerTrainUnits=Ext.getCmp("PerTrainUnits").getValue();//培训单位
	var parr="raw|"+RowId+"^PerName|"+PerName+"^PerID|"+PerID+"^StaDate|"+StaDate+"^EndDate|"+EndDate+"^PerTotalHours|"+PerTotalHours+"^PerTrainContent|"+PerTrainContent+"^PerCredit|"+PerCredit+"^PerCreditType|"+PerCreditType+"^PerCourseName|"+PerCourseName+"^PerTrainHabitude|"+PerTrainHabitude+"^PerTrainUnits|"+PerTrainUnits;
	//alert(parr);
