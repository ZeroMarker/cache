	...
	grid.on('mouseover', function (e) { //添加mouseover事件
		...
		if (index !== false) { //当取到了正确的列时，（因为如果传入的target列没有取到的时候会返回false）
			...
			if ((store.getCount() > 0)) //续打
			{
				...
				if ((totalRow) && (totalRow.indexOf("小结入量") > -1)) {
					...
					if (typeof (str) == "undefined") {
						var str = "<TABLE borderColor=#000000 width='300px' bgColor=#ffffff border=1 cellSpacing=0 cellPadding=0><TBODY>";
					}
					//alert("middle")
			...
			if ((recpar != "") && (recrw != "")) //图片
			{
				...
				if(TypeStr.split("^").length>1){
					...
					for (var i=0;i<TypeArr.length;i++){
						if(i==0){
							str = str1 +"^Item42|"+TypeArr[i]+"^Item44|"+QuantityArr[i];
							...
							str = str + "^RecLoc|" + patloc + "^RecNurseLoc|" + session['LOGON.CTLOCID'] + "^RecBed|" + bedCode; //patloc是病人当前科室，如果打印时“科室”希望打印的是科室则用该句；
							str = str + "^" + CaseMeasureID;
							str = str +"^Item42|"+TypeArr[i]+"^Item44|"+QuantityArr[i];
					...
					str=str +"^Item42|"+TypeStr+"^Item44|"+QuantityStr;
					var a = tkMakeServerCall("Nur.CommonInterface.Temperature","LinkTemp", EpisodeID, str, session['LOGON.USERID'], EmrCode, session['LOGON.GROUPDESC']);
				}
				if (a != "0") {
					alert(a);
			...
			if((rtn==1)||(recid!="")){
				if((TypeStr.split("^").length>1)||(TypeStrOut.split("^").length>1)){
					...
					for (var i=0;i<moreLenTypeArr.length;i++){
						if(i==0){
							str = str1 +"^Item42|"+TypeArr[i]+"^Item43|"+QuantityArr[i];
							str = str +"^Item44|"+TypeArrOut[i]+"^Item45|"+QuantityArrOut[i];
							...
							{
								str = str +"^Item42|"+TypeArr[i]+"^Item43|"+QuantityArr[i];
							}
				...
				//alert(str);
					...
					//str=str +"^Item42|"+TypeStr+"^Item44|"+QuantityStr+ "^RecNurseLoc|" + session['LOGON.CTLOCID'] ;
					//alert(str);
					str=str +"^Item42|"+TypeStr+"^Item44|"+QuantityStr+"^Item9|"+TypeStrOut+"^Item10|"+QuantityStrOut+ "^RecNurseLoc|" + session['LOGON.CTLOCID'] ;
					if(canSaveFlag=="1") a = cspRunServerMethod(RecSave.value, EpisodeID, str, session['LOGON.USERID'], EmrCode, session['LOGON.GROUPDESC']);
					}
				//var a = cspRunServerMethod(RecSave.value, EpisodeID, str, session['LOGON.USERID'], EmrCode, session['LOGON.GROUPDESC']);
				if (i == (list.length - 1)) {
...
function printNurRecZK() {
	...
	//alert(PrintCommPicPic.selRowId)
	//end
    ...
    //PrintCommPic.CLeft = "Y";      //Y-偶数页左边距是奇数页右边距 N-奇数和偶数页左边距相等           
