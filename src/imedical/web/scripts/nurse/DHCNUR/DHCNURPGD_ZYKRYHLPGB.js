...
var partScoreArray = ["Item30", "Item31", "Item32", "Item33", "Item34"];
ScoreItem[0] = "^Item30^Item31^Item32^Item33^Item34^";
ValueItem[0] = "Item35";
NameItem[0] = "Item79";
DateItem[0] = "Item36";
//TimeItem[0] = "Item91";
var prncode = "DHCNURMouldPrn_XSESSSHRTTGL";  //DHCNURMouldPrn_barthel2
...
function BodyLoadHandler() {
  ...
  // firstScore.focus();
  //计算各项radio分数
  calScore("Item22", "Item30");
  calScore("Item23", "Item31");
  calScore("Item24", "Item32");
  calScore("Item25", "Item33");
  calScore("Item26", "Item34");
...
function Save() {
	//by lmm  增加年份保存
	var recDate = Ext.getCmp("Item36")    //评估日期Item
	var recYear="";
	if(recDate.getValue()!=""){
		recYear=formatDate(recDate.getValue()).split("-")[0]
	}
	var recYearObj = Ext.getCmp("Item38")  //新家的评估年份Item 元素不要超过345
  ...
  comboret = "";
  radioret = "";
  // var SaveTime = document.getElementById('Item92');
   //var Time=new Date();
  // SaveTime.value=Time;
  //alert(Time);
  ...
  //alert(ret);
  // alert(comboret);
  var arr = new Array("Item30","Item31","Item32","Item33","Item34");
  var count = 0;
...
function getPatInfo() {
  ...
  if (PatInfo) {
    ...
    var MedCareNo = Ext.getCmp("Item10");
    MedCareNo.setValue(getValueByCode(tt[9]));
    var diag = Ext.getCmp("Item20");
...
function calScore(partItem, scoreItem) {
  ...
  {	
	case "Item22":
        ...
        if (checked == true) {
          if (field.boxLabel == "无(非高调哭)") {
            partScore = 0;
          } else if (field.boxLabel == "高调哭但可安抚") {
            partScore = 1;
          } else {
            partScore = 2;
          }
	...
	case "Item23":
		...
		item.on("check", function (field, checked) {
        if (checked == true) {
          if (field.boxLabel == "心率和平均血压≤术前值") {
            partScore = 0;
          } else if (field.boxLabel == "心率或平均血压增高但幅度<术前的20%") {
            partScore = 1;
          } else {
            partScore = 2;
          }
		...
		item.on("check", function (field, checked) {
        if (checked == true) {
          if (field.boxLabel == "无痛苦表情") {
            partScore = 0;
          } else if (field.boxLabel == "痛苦表情") {
            partScore = 1;
          } else {
            partScore = 2;
          }
...
function butPrintFn() {
  PrintCommMultiPGD.WebUrl = WebIp + "/imedical/web/DWR.DoctorRound.cls";
