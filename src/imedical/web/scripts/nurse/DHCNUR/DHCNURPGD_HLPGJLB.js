...
var scoreArray1 = ["Item38", "Item40", "Item42", "Item44", "Item46", "Item49"]; //最后三个为总分Item，日期Item，签名Item
var scoreArray2 = ["Item39", "Item41", "Item43", "Item45", "Item47", "Item48", "Item51"];
...
function scoreAdd(field) {
	...
	if (scoreArray1.indexOf(itmid) != -1) {
		for (var i = 0; i < scoreArray1.length - 1; i++) {
			var domobj = Ext.getCmp(scoreArray1[i]);
			if (domobj.getValue() != "") score = score + parseInt(domobj.getValue().split('(')[1].split(')')[0]);
		}
		Ext.getCmp(scoreArray1[scoreArray1.length - 1]).setValue(score);
	} else if (scoreArray2.indexOf(itmid) != -1) {
		for (var i = 0; i < scoreArray2.length - 1; i++) {
			var domobj = Ext.getCmp(scoreArray2[i]);
			if (domobj.getValue() != "") score = score + parseInt(domobj.getValue().split('(')[1].split(')')[0]);
		}
		Ext.getCmp(scoreArray2[scoreArray2.length - 1]).setValue(score);
	}
}
