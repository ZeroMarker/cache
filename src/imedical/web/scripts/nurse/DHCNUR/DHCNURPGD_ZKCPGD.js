...
//DateItem[0] = "Item13";
TimeItem[0] = "Item91";
var prncode = "DHCNURMouldPrn_ZKCHLDFY";
...
function BodyLoadHandler() {
 ...
 // tmpItm.disable();
  ...
  var tmpItm = Ext.getCmp("Item135")
  tmpItm.disable();
  var tmpItm = Ext.getCmp("Item136")
  tmpItm.disable();
  var tmpItm = Ext.getCmp("Item137")
  tmpItm.disable();
  var tmpItm = Ext.getCmp("Item138")
  tmpItm.disable();
...
function Save() {
  ...
  var recYearObj = Ext.getCmp("Item139")  //新加的评估年份Item 元素不要超过345
...
function getPatInfo() {
  ...
  if (PatInfo) {
	...
	var InpatNo=Ext.getCmp("Item3");
	InpatNo.setValue(getValueByCode(tt[28]));
	var signature = Ext.getCmp("Item12");
	...
	var age=Ext.getCmp("Item135");
	age.setValue(getValueByCode(tt[6]));
	var patLoc=Ext.getCmp("Item136");
	 patLoc.setValue(getValueByCode(tt[7]));
	 var bedCode=Ext.getCmp("Item137");
	 bedCode.setValue(getValueByCode(tt[5]));
	 var sex=Ext.getCmp("Item138");
