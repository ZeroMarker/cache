				...
				listeners: {
					click: function() {
						...
						window.buttons[0].on("click", function() {
						    ...
						    var resStr = tkMakeServerCall("web.DHCNurPdaDepModelSet", "setLocComFlag", selId, ComFlag);
							if(resStr!=""){
								alert("保存成功")
							}
							window.close();
			...
			new Ext.Button({
				...
				listeners: {
					click: function() {
						...
						var resStr = tkMakeServerCall("Nur.DHCNurPdaDepModelSet", "Delete", subsselloc);
						if(resStr!="0"){
							alert(resStr)
					    }
						var selobj = Ext.getCmp("locList").getSelectionModel().getSelected();
...
function saveModel()
{
	...
	//if(subselobj){
		...
		//alert(setId)
	//}
	var parr=selId+"^"+modelCode+"^"+menuId+"^"+session['LOGON.USERID']+"^"+setId+"^"+showName+"^"+modelType+"^"+LinkModelName+"^"+modelNum+"^"+getValMth+"^"+getListMth+"^"+saveMth;
	
	var resStr = tkMakeServerCall("Nur.DHCNurPdaDepModelSet", "Save", parr);
	if(resStr==""){
		alert("保存成功")
	}
	AddModelItemwindow.hide();
...
{ 
	...
	var setInfo = tkMakeServerCall("web.DHCNurPdaDepModelSet", "getModelSetInfoById", setId);
	if(setInfo==""){
		alert("没有配置信息");
		return;
	}
