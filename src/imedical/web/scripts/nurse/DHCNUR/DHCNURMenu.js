...
DFGMenu.on('select', function() 
 ...
 LocBox.on('select', function() {
		...
		if (objloc.getValue()!="")
		{
		  var a=tkMakeServerCall("User.DHCNURMenu","iflochaveset",objloc.getValue())
          //alert(a)		  
	      if(a=="")  //该科没有保存配置
	      {
...
{	  
	  var a=tkMakeServerCall("User.DHCNURMenu","iflochaveset",loc)	
	  if((a=="")||(loc==""))  //该科没有保存配置
	  {
		   Ext.getCmp("deleteloc").setDisabled(true)
		   Ext.getCmp("btnadd").setDisabled(true)
...
function deleteloc()
{
	  ...
	  if (locboxobj.getValue()!="")
	  {
	  	var flag = confirm("你确定要删除此条记录吗!")
		    if (flag) 
		    {
		    	      var a=tkMakeServerCall("User.DHCNURMenu","deleteloc",locboxobj.getValue())	
	                  Ext.getCmp("deleteloc").setDisabled(true)
					  Ext.getCmp("btnsave").setDisabled(false)	
                      find();	
	        }
...
function up()
{
  var grid=Ext.getCmp("mygrid");
	...
	if (objRow.length == 0) {
		...
		{ 
	          //alert(rw)
			 var a=tkMakeServerCall("User.DHCNURMenu","upordown",locs,rw,-1)
             find()
		}else{
			alert("请先保存")
		}
...
function loadgname()
{
	var obj = Ext.getCmp("KNurseName");
	gbox=new Array()
	tkMakeServerCall("User.DHCNURMenu","getgbox","","addgbox")	
...
function additm()
{
	...
	var parr =locboxobj.getValue() + "^" + modle.getValue()
	//alert(parr);
	var a=tkMakeServerCall("User.DHCNURMenu","Additm",parr)	
	if (a!=0)
	{
		 alert(a)
		 return;
...
function save()
{
	...
	else 
	{
        ...
        if (rw!="")		
		{
		  var id=rw;
		  //alert(rw)
		  var a = tkMakeServerCall("User.DHCNURMenu","update",id,codename)
		  //alert(a)
	      if(a!=0)
		  {
			  alert(a);
...
{	
	...
	if (seloc=="")
	{
		...
		var a=tkMakeServerCall("User.DHCNURMenu","iflochaveset",seloc)
		if (a!="")
		{
		  Ext.getCmp("deleteloc").setDisabled(false)	
		}else{
	...
	//alert(seloc)
	var a = cspRunServerMethod(GetQueryData.value, "User.DHCNURMenu:CRItem", "parr$"+seloc, "AddRec");
    //alert(a);
   Ext.getCmp("delete").setDisabled(true)
...
{	
	...
	Ext.getCmp("btnsave").setDisabled(true)
	//alert(session['LOGON.CTLOCID']);
	var a = cspRunServerMethod(GetQueryData.value, "User.DHCNURMenu:CRItem", "parr$", "AddRec");
  //alert(a);
  grid.store.loadData(arrgrid);   
  Ext.getCmp("delete").setDisabled(true)
}
	...
	if (len == 0) {
		...
		return;
		 ...
		 if (rw!="")
		 {
		    ...
		    if (flag) 
		    {
			  var a=tkMakeServerCall("User.DHCNURMenu","QtDelete",rw)	
	                  Ext.getCmp("delete").setDisabled(true)
					   Ext.getCmp("xsname").setValue("")
					   Ext.getCmp("KNurseName").setValue("")
					   Ext.getCmp("btnadd").setDisabled(false)
