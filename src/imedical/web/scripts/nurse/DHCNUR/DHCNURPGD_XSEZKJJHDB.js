...
function eachItem1(item, index, length) {
	...
	if (item.xtype=="checkbox") {   
			...
			if (item.checked==true) 	{checkret=checkret+item.id+"|"+item.boxLabel+"|"+item.boxLabel+"^";}
...
function getPatInfo() {
	...
	if (PatInfo) {
		...
		bedCode.setValue(getValueByCode(tt[2]));
		var patLoc = Ext.getCmp("Item10");
		...
		var Loc = Ext.getCmp("Item72");
		Loc.setValue(getValueByCode(tt[1]));
        var ret2 = tkMakeServerCall("web.DHCMGNurComm","getPatAdmInfo", EpisodeID);
		var a=ret2.split('^')
		var OutLoc = Ext.getCmp("Item9");
		OutLoc.setValue(a[1]);
