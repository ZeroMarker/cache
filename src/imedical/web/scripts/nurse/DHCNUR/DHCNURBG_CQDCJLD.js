...
var prnmode ="DHCNURMouldPrn_CQDCJLD"
var prnmcodes = "";
if (prnmode != "") {
			...
			if((rtn==1)||(recid!="")){
				if((TypeStr.split("^").length>1)||(TypeStrOut.split("^").length>1)){
					...
					for (var i=0;i<moreLenTypeArr.length;i++){
						if(i==0){
							str = str1 +"^Item17|"+TypeArr[i]+"^Item18|"+QuantityArr[i];
							str = str +"^Item19|"+TypeArrOut[i]+"^Item20|"+QuantityArrOut[i];
							...
							{
								str = str +"^Item17|"+TypeArr[i]+"^Item18|"+QuantityArr[i];
							}
				...
				//alert(str);
					...
					//alert(str);
					str=str +"^Item17|"+TypeStr+"^Item18|"+QuantityStr+"^Item19|"+TypeStrOut+"^Item20|"+QuantityStrOut+ "^RecNurseLoc|" + session['LOGON.CTLOCID'] ;
					if(canSaveFlag=="1") a = cspRunServerMethod(RecSave.value, EpisodeID, str, session['LOGON.USERID'], EmrCode, session['LOGON.GROUPDESC']);
					}
				//var a = cspRunServerMethod(RecSave.value, EpisodeID, str, session['LOGON.USERID'], EmrCode, session['LOGON.GROUPDESC']);
				if (i == (list.length - 1)) {
