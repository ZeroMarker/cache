...
function checkListener(boxObj, val) {
	if (boxObj.id == "typ") {
		var btnCancelAuditObj = Ext.getCmp("btnCancelAudit");
		if (val) {
			btnCancelAuditObj.disable();
		} else {
			btnCancelAuditObj.enable();
		}
	}
}
function getDisposeStatColor() {
	...
	var pngUrl = "";
	for (var i = 0; i < parameterLabelArray.length; i++) {
		switch (parameterColorArray[i]) {
		case "red":
			pngUrl = "../images/uiimages/label/operation_in.png"
				str += "<TD noWrap><label id='renderclortip" + i + "' style='color:#fff;BACKGROUND-IMAGE: url(" + pngUrl + "); PADDING-LEFT: 16px; WIDTH: 77px; DISPLAY: inline-block; HEIGHT: 19px; FONT-SIZE: 12px; PADDING-TOP: 2px; REPEAT: repeat'>" + parameterLabelArray[i] + "</label></TD>"
				break;
		case "green":
			pngUrl = "../images/uiimages/label/11.png"
				str += "<TD noWrap><label id='renderclortip" + i + "' style='color:#fff;BACKGROUND-IMAGE: url(" + pngUrl + "); PADDING-LEFT: 16px; WIDTH: 77px; DISPLAY: inline-block; HEIGHT: 19px; FONT-SIZE: 12px; PADDING-TOP: 2px; REPEAT: repeat'>" + parameterLabelArray[i] + "</label></TD>"
				break;
		case "blue":
			pngUrl = "../images/uiimages/label/12.png"
				str += "<TD noWrap><label id='renderclortip" + i + "' style='color:#fff;BACKGROUND-IMAGE: url(" + pngUrl + "); PADDING-LEFT: 16px; WIDTH: 77px; DISPLAY: inline-block; HEIGHT: 19px; FONT-SIZE: 12px; PADDING-TOP: 2px; REPEAT: repeat'>" + parameterLabelArray[i] + "</label></TD>"
				break;
		case "yellow":
			pngUrl = "../images/uiimages/label/13.jpg"
				str += "<TD noWrap><label id='renderclortip" + i + "' style='BACKGROUND-IMAGE: url(" + pngUrl + "); PADDING-LEFT: 16px; WIDTH: 77px; DISPLAY: inline-block; HEIGHT: 19px; FONT-SIZE: 12px; PADDING-TOP: 2px; REPEAT: repeat'>" + parameterLabelArray[i] + "</label></TD>"
				break;
		default:
...
function InitializeRight() {
	...
	tbar.addItem(pogFlagLabelall);
	tbar.addItem(pogFlagFieldall);
...
function Click_Button(pid, bid) {
	if (bid == 'btnArrowAudit') {
	...
	} else if (bid == 'btnCancelAudit') {
		try {
			Cancel_Audit(pid, bid);
		} catch (e) {}
	...
	} else if (bid == 'btnStoppog') {
		try {
			btnStoppog_click(pid, bid);
		} catch (e) {}
		...
		grid.collapse();
		panel.expand();
	} else if (bid == 'btnStoppogMX') {
		try {
			btnbtnStoppogMX_click(pid, bid);
		} catch (e) {}
