...
function BodyLoadHandler() {
	...
	grid.on('mouseover', function (e) { //添加mouseover事件
		...
		if (index !== false) { //当取到了正确的列时，（因为如果传入的target列没有取到的时候会返回false）
			...
			if(typeof(store.getAt(index).get("par"))=="undefined") return;
			var totalRow = "";
			if (store.getCount() > 0)
				totalRow = store.getAt(index).get(DisplaySumInName.split("!")[0]);
	...
	grid.getStore().on('load', function (s, records) {
		var girdcount = 0;
			s.each(function (r) {
			if(r.get("PageMark") == 'Y') {
				grid.getView().getRow(girdcount).style.backgroundColor = '#F7EE7E';     //分页标记过颜色
			}
...
/*function SepcialChar() {
	...
	var window = new Ext.Window({
			...
			listeners : {
				'close' : function () {
					if (Ext.getCmp('mygrid').rowIndex > 0)
...
/*function setColor(record, rowIndex, rowParams, store) {
	//禁用数据显示红色
...
/*function save() {
	...
	for (var i = 0; i < list.length; i++) {
		...
		if ((str != "") && (flag == "0")) {
			...
			//alert(str);
			var rtn=tkMakeServerCall("Nur.DHCNurseRecSub","JurgeTimeByCode",EpisodeID,CareDate,CareTime,EmrCode);
			var recid = ""
			if ((recpar != "") && (recrw != "")) //图片
			{
				recid = recpar + "||" + recrw
			...
			if((rtn==1)||(recid!="")){
				if((TypeStr.split("^").length>1)||(TypeStrOut.split("^").length>1)){
					...
					for (var i=0;i<moreLenTypeArr.length;i++){
						if(i==0){
							str = str1 +"^Item24|"+TypeArr[i]+"^Item25|"+QuantityArr[i];
							str = str +"^Item26|"+TypeArrOut[i]+"^Item27|"+QuantityArrOut[i];
							if(canSaveFlag=="1") a = cspRunServerMethod(RecSave.value, EpisodeID, str, session['LOGON.USERID'], EmrCode, session['LOGON.GROUPDESC']);
						}
						else{
							str="CareDate|" + CareDate + "^CareTime|" + CareTime;
							...
							{
								str = str +"^Item24|"+TypeArr[i]+"^Item25|"+QuantityArr[i];
							}
				...
				//alert(str);
					...
					//str=str +"^Item7|"+TypeStr+"^Item8|"+QuantityStr+ "^RecNurseLoc|" + session['LOGON.CTLOCID'] ;
					//alert(str);
					str=str +"^Item24|"+TypeStr+"^Item25|"+QuantityStr+"^Item26|"+TypeStrOut+"^Item27|"+QuantityStrOut+ "^RecNurseLoc|" + session['LOGON.CTLOCID'] ;
					if(canSaveFlag=="1") a = cspRunServerMethod(RecSave.value, EpisodeID, str, session['LOGON.USERID'], EmrCode, session['LOGON.GROUPDESC']);
					}
				//var a = cspRunServerMethod(RecSave.value, EpisodeID, str, session['LOGON.USERID'], EmrCode, session['LOGON.GROUPDESC']);
				if (i == (list.length - 1)) {
...
function InOutSum() {
	...
	if (objRow.length == 0) {
		...
		str = str + "^DiagnosDr|" + DiagnosDr;
		str = str + "^RecLoc|" + patloc + "^RecNurseLoc|" + session['LOGON.CTLOCID'] 
		var a = cspRunServerMethod(SaveOutIn.value, EpisodeID, str, session['LOGON.USERID'], EmrCode);
		find();
	}
}
...
function linkDocBl() {
	...
	var objret = window.showModalDialog(lnk, 'docBl', "dialogWidth:1200px;dialogHeight:800px;center:yes;resizable:no;status:no;scroll:YES;help:no;edge:raised;");
}
