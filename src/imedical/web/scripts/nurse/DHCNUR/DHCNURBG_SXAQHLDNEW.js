...
DateItem[0] = "Item13";
TimeItem[0] = "Item91";
var prncode = "DHCNURMouldPrn_SXAQHLD";
...
function BodyLoadHandler() {
  ...
  /*begin*****南方医院的搞特殊处理方法*********************/
	...
	for(var i=10;i<71;i++){
		...
		if(i>=13&&i<=16){
			SMutualExclusion(13,16,selFlag,date)
		}
		else if(i>=17&&i<=18){
			SMutualExclusion(17,18,selFlag,date)
		}
		
		else if(i>=23&&i<=24){
			SMutualExclusion(23,24,selFlag,date)
		}
		else if(i>=25&&i<=26){
			SMutualExclusion(25,26,selFlag,date)
		}
		else if(i>=33&&i<=34){
			SMutualExclusion(33,34,selFlag,date)
		}
		else if(i>=35&&i<=36){
			SMutualExclusion(35,36,selFlag,date)
		}
		else if(i>=37&&i<=38){
			SMutualExclusion(37,38,selFlag,date)
		}
		else {
...
function Save() {
	...
	var recYearObj = Ext.getCmp("Item77")  //新家的评估年份Item 元素不要超过345
...
function getPatInfo() {
  ...
  if (PatInfo) {
	...
	var signature = Ext.getCmp("Item71");
