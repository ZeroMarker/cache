...
var partScoreArray = ["Item16", "Item17", "Item18", "Item19", "Item20", "Item21", "Item28"];
ScoreItem[0] = "^Item16^Item17^Item18^Item19^Item20^Item21^Item28";
ValueItem[0] = "Item22";
NameItem[0] = "Item23";
DateItem[0] = "Item29";
TimeItem[0] = "Item30";
var prncode = "DHCNURMouldPrn_DVTFXPGLBDY";  //DHCNURMouldPrn_barthel2
...
function BodyLoadHandler() {
 ...
 /* var tmpItm = Ext.getCmp("Item1")
  ...
  // firstScore.focus();
  //计算各项radio分数
  calScore("Item9", "Item16");
  calScore("Item10", "Item17");
  calScore("Item11", "Item18");
  calScore("Item12", "Item19");
  calScore("Item13", "Item20");
  calScore("Item14", "Item21");
  calScore("Item15", "Item28");
}
function sjj() { }
...
function Save() {
	//by lmm  增加年份保存
	var recDate = Ext.getCmp("Item29")    //评估日期Item
	var recYear="";
	if(recDate.getValue()!=""){
		recYear=formatDate(recDate.getValue()).split("-")[0]
	}
	//var recYearObj = Ext.getCmp("Item102")  //新家的评估年份Item 元素不要超过345
  ...
  comboret = "";
  radioret = "";
   var SaveTime = document.getElementById('Item30');
   var Time=new Date();
   SaveTime.value=Time;
  //alert(Time);
  ...
  //alert(ret);
  // alert(comboret);
  var arr = new Array("Item16", "Item17", "Item18", "Item19", "Item20", "Item21", "Item28");
  var count = 0;
...
function calScore(partItem, scoreItem) {
  ...
  {	
	case "Item9":
        ...
        if (checked == true) {
          if (field.boxLabel == "10-30") {
            partScore = 0;
          } else if (field.boxLabel == "31-40") {
            partScore = 1;
          } else if (field.boxLabel == "41-50") {
            partScore = 2;
          }else if (field.boxLabel == "51-60") {
            partScore = 3;
          }else {
            partScore = 4;
          }
	...
	case "Item10":
		...
		item.on("check", function (field, checked) {
       if (checked == true) {
          if (field.boxLabel == "体重不足(16-19)") {
            partScore = 0;
          } else if (field.boxLabel == "体重适中(20-25)") {
            partScore = 1;
          } else if (field.boxLabel == "超重(26-30)") {
            partScore = 2;
          }else if (field.boxLabel == "肥胖(31-40)") {
            partScore = 3;
          }else {
            partScore = 4;
          }
		...
		item.on("check", function (field, checked) {
       if (checked == true) {
          if (field.boxLabel == "20-35岁") {
            partScore = 1;
          } else if (field.boxLabel == ">35岁") {
            partScore = 2;
          }else {
            partScore = 3;
          }
		...
		item.on("check", function (field, checked) {
        if (checked == true) {
          if (field.boxLabel == "能自主活动") {
            partScore = 0;
          } else if (field.boxLabel == "活动受限") {
            partScore = 1;
          } else if (field.boxLabel == "活动严重受限") {
            partScore = 2;
          }else if (field.boxLabel == "轮椅") {
            partScore = 3;
          }else {
            partScore = 4;
          }
		...
		item.on("check", function (field, checked) {
         if (checked == true) {
          if (field.boxLabel == "头部或胸部") {
            partScore = 1;
          } else if (field.boxLabel == "头部及胸部") {
            partScore = 2;
          } else if (field.boxLabel == "脊柱") {
            partScore = 2;
          }else if (field.boxLabel == "骨盆") {
            partScore = 3;
          }else {
            partScore = 4;
          }
		...
		item.on("check", function (field, checked) {
        if (checked == true) {
          if (field.boxLabel == "小手术<30min") {
            partScore = 1;
          } else if (field.boxLabel == "择期大手术") {
            partScore = 2;
          } else if (field.boxLabel == "急诊大手术") {
            partScore = 3;
          }else if (field.boxLabel == "骨盆手术") {
            partScore = 3;
          }else if (field.boxLabel == "胸部手术") {
            partScore = 3;
          }else if (field.boxLabel == "腹部手术") {
            partScore = 3;
          }else if (field.boxLabel == "泌尿系统手术") {
            partScore = 3;
          }else if (field.boxLabel == "神经系统手术") {
            partScore = 3;
          }else if (field.boxLabel == "妇科手术") {
            partScore = 3;
          }else if (field.boxLabel == "骨科手术(腰部以下)") {
            partScore = 4;
          }else {
            partScore = 4;
          }
		...
		item.on("check", function (field, checked) {
         if (checked == true) {
          if (field.boxLabel == "溃疡性结肠炎") {
            partScore = 1;
          } else if (field.boxLabel == "镰状细胞贫血") {
            partScore = 2;
          } else if (field.boxLabel == "红细胞增多症") {
            partScore = 2;
          }else if (field.boxLabel == "溶血性贫血") {
            partScore = 2;
          }else if (field.boxLabel == "慢性心脏病（冠心病）") {
            partScore = 3;
          }else if (field.boxLabel == "心肌梗塞") {
            partScore = 4;
          }else if (field.boxLabel == "恶性肿瘤") {
            partScore = 5;
          }else if (field.boxLabel == "脑血管疾病") {
            partScore = 6;
          }else if (field.boxLabel == "静脉曲张") {
            partScore = 6;
          }else {
            partScore = 7;
          }
