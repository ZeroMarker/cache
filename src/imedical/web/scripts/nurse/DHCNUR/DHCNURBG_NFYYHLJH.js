...
if (GetNurseRecSet){
	var ret=cspRunServerMethod(GetNurseRecSet.value,"DHCNURBG_NFYYHLJH");
	var hh=ret.split("^");
...
function PrintSet()
{
 ...
 //debugger;
   ...
   // debugger;
    REC=new Array();
 	var a=cspRunServerMethod(GetNurPrnSet.value,"DHCNURBG_NFYYHLJH",EpisodeID,'addprnset');
...
function find(){
  ...
  // alert(parent.frames.length);
	...
	var parr = adm + "^" + StDate.value + "^" + StTime.value + "^" + Enddate.value + "^" + EndTime.value + "^"+"DHCNURBG_NFYYHLJH" + "^" + IfCancelRec+"^"+CurrHeadDr;
	// debugger;
	console.log(parr);
...
{  //查询出入液量合计
	...
	var mygrid=Ext.getCmp("mygrid");
  //alert(adm+"|"+StDate.value+"|"+StTime.value+"|"+Enddate.value+"|"+EndTime.value+"|"+"DHCNURBG_NFYYHLJH")
 	var a=cspRunServerMethod(GeInOutAmount.value,adm,StDate.value,StTime.value,Enddate.value,EndTime.value,"DHCNURBG_NFYYHLJH");
	...
	for (var i = 0; i < list.length; i++) {
	  ...
	  var flag="0";
		...
		if ((str!="")&&(flag=="0"))
		{
			...
			//alert(str);
			var a=cspRunServerMethod(RecSave.value,EpisodeID,str,session['LOGON.USERID'],"DHCNURBG_NFYYHLJH",session['LOGON.GROUPDESC']);
			if (a!="0")
			{
				alert(a);
				return;
...
var rightClick = new Ext.menu.Menu(  {
            ...
            },  */
			{
                ...
                handler:function(){
					...
					var CareDate=record.get("CareDate");
					var CareTime=record.get("CareTime");
					OldDate=CareDate;
					OldTime=CareTime;
					PlanModel();
					var fiter = record.get("Item2");
					//Ext.getCmp("PlanModel").getStore().removeAll();
					Ext.getCmp("Label").setText("") ;
					//alert(fiter);
					loadData(Ext.getCmp("planModel"), {
...
function MultiFun()
{
	...
	var ret= cspRunServerMethod(GetMulti.value, "DHCNURBG_NFYYHLJH");
	...
	var tabstr=cspRunServerMethod(getcheckform.value, "DHCNURBG_NFYYHLJH",ab);
...
function multiSel(ret)
{
    ...
    var itname=grid1.getSelectionModel().getSelections()[0].get("itm4"); 
	  var getcheckform=document.getElementById('getcheckform');
		alert(itname+"!!"+code)
		var ret=cspRunServerMethod(getcheckform.value, "DHCNURBG_NFYYHLJH",itname,code,"");	
...
{ //小结
	...
	else
	{
	  ...
	  else
	  {
		...
		str=str+"^DiagnosDr|"+DiagnosDr+"^HeadDR|"+CurrHeadDr;
		//alert(str);
		var a=cspRunServerMethod(SaveOutIn.value,EpisodeID,str,session['LOGON.USERID'],"DHCNURBG_NFYYHLJH");
		find();
	}
}
function InOutSum()
...
function Measure()
{
   ...
   // debugger;
	...
	var parr="DHCNURBG_NFYYHLJH^"+EpisodeID+"^CaseMeasureXml^"+rowid;
	//alert(parr);
	var arr = new Array();
	var a = cspRunServerMethod(pdata1, "", "DHCPatOrdList", EpisodeID, "");
	arr = eval(a);
...
function sureMeasure()
{
   ...
   // var parr="DHCNURBG_NFYYHLJH^"+EpisodeID+"^CaseMeasureXml";
...
function printNurRec() {
	 printNurCareRec("DHCNURMouldPrn_NFYYHLJH")
	 return
	...
	var hh = ret.split("^");
	//debugger;
	var a = cspRunServerMethod(GetPrnSet.value, "DHCNURBG_NFYYHLJH", EpisodeID); //page, caredattim, prnpos, adm,Typ,user
	if (a == "") return;
	var GetLableRec = document.getElementById('GetLableRec');
	var LabHead = cspRunServerMethod(GetLableRec.value, "DHCNURBG_NFYYHLJH" + "^" + session['LOGON.CTLOCID']);
	//alert(GetLableRec);
	var tm = a.split("^");
	var stdate = "" //tm[2];
	var stim = "" //tm[3];
	...
	PrintCommPicIP.EmrCode = "DHCNURBG_NFYYHLJH";
	//alert(PrintComm.Pages);
	//PrintComm.SetConnectStr(CacheDB);
	PrintCommPicIP.ItmName = "DHCNURMouldPrn_NFYYHLJH"; //338155!2010-07-13!0:00!!
	//debugger;
	var parr = EpisodeID + "!" + stdate + "!" + stim + "!" + edate + "!" + etim + "!" + "DHCNURBG_NFYYHLJH" + "!";
	//alert(parr);
	PrintCommPicIP.ID = "";
...
function printNurRecbak()
{
		...
		if (a=="") return;
		var GetLableRec=document.getElementById('GetLableRec');
		var LabHead=cspRunServerMethod(GetLableRec.value,"DHCNURBG_NFYYHLJH^"+session['LOGON.CTLOCID']+"^"+EpisodeID+"^"+session['LOGON.USERID']);
		//alert("DHCNURBG_NFYYHLJH^"+session['LOGON.CTLOCID']+"^"+EpisodeID+"^"+session['LOGON.USERID'])
		var tm=a.split("^");
		var stdate="" //tm[2];
		var stim="" //tm[3];
		var edate="" //tm[4];
		...
		PrintCommPicIP.WebUrl=WebIp+"/dthealth/web/DWR.DoctorRound.cls";
		var aa=tm[1].split("&");
		//PrintCommPicIP.stPage=aa[0];
		//if (aa.length>1) PrintCommPicIP.stRow=aa[1];
		PrintCommPicIP.stPage=0;
		PrintCommPicIP.stRow=0;
		PrintCommPicIP.previewPrint="1"; //是否弹出设置界面
		//PrintCommPicIP.stprintpos=tm[0];
		PrintCommPicIP.stprintpos=0;
		//alert(PrintCommPicIP.Pages);
		PrintCommPicIP.SetConnectStr(CacheDB);
		PrintCommPicIP.ItmName = "DHCNURMouldPrn_NFYYHLJH"; //338155!2010-07-13!0:00!!
		//debugger;
		var parr=EpisodeID+"!"+stdate+"!"+stim+"!"+edate+"!"+etim+"!DHCNURBG_NFYYHLJH!"+CurrHeadDr;
		PrintCommPicIP.ID = "";
		PrintCommPicIP.MultID = "";
		//PrintCommPicIP.MthArr="web.DHCConsult:getConsultInfo&id:"+myid;
		if(LabHead!="")PrintCommPicIP.LabHead=LabHead;
		...
		//debugger;
		var CareDateTim=PrintCommPicIP.CareDateTim;
		if (CareDateTim=="") return ;
		var pages=PrintCommPicIP.pages;
		var stRow=PrintCommPicIP.stRow;
		//debugger;
		var stprintpos=PrintCommPicIP.stPrintPos;
		//alert(pages+","+CareDateTim+","+stprintpos+","+EpisodeID+","+"DHCNURBG_NFYYHLJH"+","+session['LOGON.USERID']+","+PrintCommPicIP.PrnFlag);
		//PrnFlag==1说明是打印预览
		if (PrintCommPicIP.PrnFlag==1) return;
		//如果原记录保存打印到第8页则当打印第8页之前页时不保存打印记录
		if (pages<aa[0]) return;
		var a=cspRunServerMethod(SavePrnSet.value,pages,CareDateTim,stprintpos,EpisodeID,"DHCNURBG_NFYYHLJH",session['LOGON.USERID']); //page, caredattim, prnpos, adm,Typ,user
		//find();
}
...
function PlanModel()
{
	var selectModel = new Ext.grid.CheckboxSelectionModel({});
		var cloumnModel = new Ext.grid.ColumnModel([
		   selectModel,
			{header: "护理诊断",width:90,dataIndex:"planName"},
			{header: "护理措施",width:60,dataIndex:"planMeasure",editor:new Ext.form.Field()},
			{header: "ID",width:20,dataIndex:"planID"},
		]);
		var window =createEidtPanelWin({
			winID:"planModelWin",
			gridID:"planModel",
			cm:cloumnModel,
			fields:["planName","planMeasure","planID"],
			className:"web.DHCNurQMPage",
			methodName:"getMenuNurseJson",
			forceFit:true,
			idField:"menuID"
...
*Note:创建窗口
*/
function createEidtPanelWin(obj,SM)
{
			...
			new Ext.form.TextField({
				...
				listeners:{specialkey:function(field,e){
						if(e.getKey()==Ext.EventObject.ENTER){
							var fiter = Ext.getCmp("fiter").getValue();
							Ext.getCmp(obj.gridID).getStore().removeAll();
							loadData(Ext.getCmp(obj.gridID), {
		...
		buttons:[
			new Ext.Button({
				...
				handler:function(){
						...
						var SelectionModel  = planGrid.getSelectionModel();
						var rowArray = SelectionModel.getSelections();
							for(var i =0;i<rowArray.length;i++)
							{	var count=""
								var rowRecord = rowArray[i];
								var planID = rowRecord.get("planID");
								var planName = rowRecord.get("planName");
								if(i>0){
									var MenuName=TotalName.split(",")
									for (var y=1;y<MenuName.length;y++){
										if(MenuName[y]==planName){
											count=1;
											break;
										}
								...
								if(count==""){
									TotalName=TotalName+","+planName
								}
								var menuId=tkMakeServerCall("web.DHCNurQMPage","SeachMenuID",planID);
								TotalplanId=TotalplanId+"^"+planID;	
								//addPlan(planName,planMeasure);	
							}
						Ext.getCmp("Label").setText(TotalName) ;	
						var columnModel=planGrid.getColumnModel().setColumnHeader(1,"护理措施");
						var columnModel=planGrid.getColumnModel().setColumnHeader(2,"自定义措施");
							loadData(Ext.getCmp(obj.gridID), {
									className:"web.DHCNurQMPage",
									methodName:"getSetByDesc",
									parameter1:TotalplanId
							});
				}
			}),
			new Ext.Button({
...
*保存计划
*/
function Sure(gridID)
{
	var planGrid = Ext.getCmp(gridID);
	var SelectionModel  = planGrid.getSelectionModel();
	var rowArray = SelectionModel.getSelections();
	...
	if(label.text==""){
		for(var i =0;i<rowArray.length;i++)
		{
			...
			var menuId=tkMakeServerCall("web.DHCNurQMPage","SeachMenuID",planID);
			//alert(menuId);
			var planMeasure = rowRecord.get("planMeasure");
			addPlan(planName,planMeasure,menuId);
		...
		{
			var rowRecord = rowArray[i];
			var planName = label.text;
			var planID = rowRecord.get("planID");
			var menuId=tkMakeServerCall("web.DHCNurQMPage","DetailsSearchMenuID",planID);
			//alert(menuId);
			var planMeasure = rowRecord.get("planMeasure");
			addPlan(planName,planMeasure,menuId);
...
function StopPlan()
{
	...
	for(var i =0;i<rowArray.length;i++)
	{
		rowArray[i].set("Item3", new Date());
		rowArray[i].set("Item7", new Date().dateFormat('H:i'));
		rowArray[i].set("Item5", session['LOGON.USERNAME']);
	}
	save();
}
/**
...
{
	var subselobj = Ext.getCmp("mygrid").getSelectionModel().getSelected();
	if(subselobj)
	{
		var menuId=subselobj.get("Item50");
		var RecAuditUser=session['LOGON.USERNAME']
		var resStr = tkMakeServerCall("web.DHCNurQMPage", "getDiagnosis", EpisodeID,menuId,RecAuditUser);
...
function addPlan(planName,planMeasure,menuId)
{ 
  	var Plant = Ext.data.Record.create([
	{name:'CareDate'}, 
	{name:'CareTime'},
    ...
    var count = grid.store.getCount(); 
	if(OldDate==""&&OldTime==""){
		var r = new Plant({CareDate:new Date(),CareTime:new Date().dateFormat('H:i'),Item2:planName,Item1:planMeasure,Item6:session['LOGON.USERNAME'],Item50:menuId}); 
		//alert(new Date());
	}
    else{
		var r = new Plant({CareDate:OldDate,CareTime:OldTime,Item2:planName,Item1:planMeasure,Item6:session['LOGON.USERNAME'],Item50:menuId}); 
	}
	OldDate="";
	OldTime="";
    grid.store.commitChanges(); 
