...
function getPatInfo() {
	...
	if (PatInfo) {
		...
		//alert(ret);
		var tt = ret.split('^');
		var patName = Ext.getCmp("Item74");
		patName.setValue(getValueByCode(tt[4]));
		patName.disable();
		var sex = Ext.getCmp("Item75");
		sex.setValue(getValueByCode(tt[3]));
		sex.disable();
		var age = Ext.getCmp("Item77");
		age.setValue(getValueByCode(tt[6]));
		age.disable();
		var medcare=Ext.getCmp("Item79");
		medcare.setValue(getValueByCode(tt[9]));
		medcare.disable();
		//var diag = Ext.getCmp("Item5");
		//diag.setValue(getValueByCode(tt[8]));
		//diag.disable();
		//var regno = Ext.getCmp("Item10");
		//regno.setValue(getValueByCode(tt[0]));
		//regno.disable();
		var loc = Ext.getCmp("Item76");
		loc.setValue(getValueByCode(tt[1]));
		loc.disable();
		var bedcode = Ext.getCmp("Item78");
