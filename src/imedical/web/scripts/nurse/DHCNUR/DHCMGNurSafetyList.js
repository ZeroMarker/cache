...
function BodyLoadHandler() {
  ...
  grid.on("dblclick",ModCheck);
	...
	but1.on("click",NewCheck);
	but1.hide();
	var but = Ext.getCmp("mygridbut2");
	but.on("click",ModCheck);
	but.hide();
...
tbar2.render(grid.tbar);
	...
	CheckItem.store.on(
    "beforeLoad",function(){
         CheckItem.store.baseParams.par=CheckRoomId;
...
function ModCheck()
{
  var grid = Ext.getCmp("mygrid");
    ...
    var ECode=CheckMould;
    var ward="";
    var link="dhcnuremrcomm.csp?CheckRoomId="+CheckRoomId+"&EmrCode="+ECode+"&CheckTyp="+CheckTyp+"&CheckLoc="+ward+"&CheckCode="+CheckCode+"&CheckRW="+Par;
    //alert(1);
    //alert(link);
    window.showModalDialog(link,"核查","dialogWidth=700px;dialogHeight=700px");
    window.location.reload();
   // window.showModalDialog(link,'核查','dialogWidth=700px;dialogHeight=700px');
   //	window.open (link,'核查','height=700,width=700,top=0,left=300,toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no') 
		return;
...
function NewCheck() 
{
  //Check(NurTyp,"");
  //checkid = Checi;
  ward =  Ext.getCmp("WardLoc").getValue();
  if (ward=="")
  {
	   alert("请选择科室/病区！");
	   return;
 ...
 //alert(CheckCode)
  var ECode=CheckMould;
  var link="dhcnuremrcomm.csp?CheckRoomId="+CheckRoomId+"&EmrCode="+ECode+"&CheckTyp="+CheckTyp+"&CheckLoc="+ward+"&CheckCode="+CheckCode;
  //window.showModalDialog(link,'核查',"dialogWidth=1000px;dialogHeight=700px");
  //window.location.reload();
  window.open (link,'核查','height=700,width=700,top=0,left=300,toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no') 
		return;
}
