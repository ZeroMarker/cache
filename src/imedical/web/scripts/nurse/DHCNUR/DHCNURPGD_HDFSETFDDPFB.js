...
//var partArray = ["Item7", "Item8", "Item9", "Item10", "Item11", "Item12"];
var partScoreArray = ["Item31", "Item32", "Item33", "Item34", "Item35", "Item36","Item37"];
ScoreItem[0] = "^Item31^Item32^Item33^Item34^Item35^Item36^Item37^";
ValueItem[0] = "Item18";
NameItem[0] = "Item19";
DateItem[0] = "Item46";
TimeItem[0] = "Item47";
var prncode = "DHCNURMouldPrn_HDFSETFDDPFBDY";
...
function totelmouth() {
  ...
  zongf = a[0];
  fenj = a[1];
  var zf = Ext.getCmp("Item16");
  zf.setValue(getValueByCode(zongf));
  var fj = Ext.getCmp("Item25");
...
function BodyLoadHandler() {
	var patName = document.getElementById("_Label234");
	//patName.style.color="green";
 ...
 // tmpItm.disable();
	//var tmpItm = Ext.getCmp("Item23")
  //tmpItm.disable();
  // var firstScore=Ext.getCmp("Item96_1")
  // firstScore.focus();
  //计算各项radio分数	
  calScore("Item31", "Item20");
  calScore("Item32", "Item21");
  calScore("Item33", "Item26");
  calScore("Item34", "Item28");
  calScore("Item35", "Item29");
  calScore("Item36", "Item30");
  calScore("Item37", "Item38");
...
function Save() {
	//by lmm  增加年份保存
	var recDate = Ext.getCmp("Item46")
	var recYear="";
	if(recDate.getValue()!=""){
		recYear=formatDate(recDate.getValue()).split("-")[0]
	}
	var recYearObj = Ext.getCmp("Item42")
  ...
  var gform = Ext.getCmp("gform");
  gform.items.each(eachItem1, this);
  var arr = new Array("Item20", "Item21", "Item26", "Item28", "Item29", "Item30", "Item38");
  var count = 0;
...
function getPatInfo() {
  ...
  if (PatInfo) {
    ...
    var  regno= Ext.getCmp("Item1");
    regno.setValue(getValueByCode(tt[0]));
    var age = Ext.getCmp("Item10");
...
function calScore(partItem, scoreItem) {
  ...
  {	
	case "Item31":
        ...
        if (checked == true) {
          if (field.boxLabel == "小于3岁") {
		  partScore = 4;
		  }else if (field.boxLabel == "3-7岁") {
            partScore = 3;
          } else if (field.boxLabel == "7-13岁") {
            partScore = 2;
          }else {
            partScore = 1;
          }
	...
	case "Item33":
		...
		item.on("check", function (field, checked) {
        if (checked == true) {
          if (field.boxLabel == "神经病学诊断") {
            partScore = 4;
          } else if (field.boxLabel == "含氧量的变化（呼吸诊断、脱水、贫血、厌食、晕厥、头晕等）"){
            partScore = 3;
          } else if (field.boxLabel == "心理/行为障碍"){
            partScore = 2;
          }else {
            partScore = 1;
          }
	...
	case "Item34":
		...
		item.on("check", function (field, checked) {
        if (checked == true) {
          if (field.boxLabel == "没有意识到不能自我行动") {
            partScore = 3;
          } else if (field.boxLabel == "记有行动限制"){
            partScore = 2;
          }else {
            partScore = 1;
          }
	...
	case "Item35":
		...
		item.on("check", function (field, checked) {
        if (checked == true) {
          if (field.boxLabel == "有跌倒史，或有婴儿安置在成人床") {
            partScore = 4;
          } else if (field.boxLabel == "患者要使用辅助装置或在婴儿床内的婴儿，或三人房"){
            partScore = 3;
          }else if (field.boxLabel == "患儿安置在床上"){
            partScore = 2;
          }else {
            partScore = 1;
          }
	...
	case "Item36":
		...
		item.on("check", function (field, checked) {
        if (checked == true) {
          if (field.boxLabel == "24小时内") {
            partScore = 3;
          } else if (field.boxLabel == "48小时内"){
            partScore = 2;
          }else {
            partScore = 1;
          }
		...
		item.on("check", function (field, checked) {
        if (checked == true) {
          if (field.boxLabel == "镇静剂、安眠药、巴比妥类药物、噻嗪类、抗抑郁药、泻药、利尿剂、麻醉剂、散瞳剂") {
            partScore = 3;
          } else if (field.boxLabel == "上面列出的任一种"){
            partScore = 2;
          }else {
            partScore = 1;
          }
