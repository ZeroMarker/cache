...
function BodyLoadHandler() {
	...
	for(var i = 0 ;i < len;i++){
		if((grid.getColumnModel().getDataIndex(i) == 'rw')||(grid.getColumnModel().getDataIndex(i) == 'par')){
			...
			if((rtn==1)||(recid!="")){
				if((TypeStr.split("^").length>1)||(TypeStrOut.split("^").length>1)){
					...
					for (var i=0;i<moreLenTypeArr.length;i++){
						if(i==0){
							str = str1 +"^Item6|"+TypeArr[i]+"^Item7|"+QuantityArr[i];
							str = str +"^Item8|"+TypeArrOut[i]+"^Item9|"+QuantityArrOut[i];
							...
							{
								str = str +"^Item6|"+TypeArr[i]+"^Item7|"+QuantityArr[i];
							}
				...
				//alert(str);
					...
					//alert(str);
					str=str +"^Item6|"+TypeStr+"^Item7|"+QuantityStr+"^Item8|"+TypeStrOut+"^Item9|"+QuantityStrOut+ "^RecNurseLoc|" + session['LOGON.CTLOCID'] ;
					if(canSaveFlag=="1") a = cspRunServerMethod(RecSave.value, EpisodeID, str, session['LOGON.USERID'], EmrCode, session['LOGON.GROUPDESC']);
					}
				//var a = cspRunServerMethod(RecSave.value, EpisodeID, str, session['LOGON.USERID'], EmrCode, session['LOGON.GROUPDESC']);
				if (i == (list.length - 1)) {
