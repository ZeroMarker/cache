...
var combo = new Ext.form.ComboBox({
 ...
 store:new Ext.data.Store({
	...
	tabIndex:'0',listWidth:'200',height:18,width:180,xtype : 'combo',
	...
	listeners:{
		select:function(ts,tr,e){
			...
			if(bedflag==0){
				Ext.getCmp('mygrid').getColumnModel().setColumnHeader(5,"床位/备注");
			}else{
				Ext.getCmp('mygrid').getColumnModel().setColumnHeader(5,"床位/备注");
			}
			FindArgJobData();
...
function BodyLoadHandler()
{
	...
	window.onresize=function(){
		setTimeout(function(){
			Ext.getCmp('gform').setSize(document.body.clientWidth,document.body.clientHeight);
			Ext.getCmp('gform').items.each(function(item,index,length){
				if(item.getXType()=='panel'){
					item.setSize(document.body.clientWidth,document.body.clientHeight);
				}
			})
			//Ext.getCmp('gridpanel').setSize(document.body.clientWidth,document.body.clientHeight);
			Ext.getCmp('mainpanel').setSize(document.body.clientWidth,document.body.clientHeight);
			Ext.getCmp('mygrid').setSize(document.body.clientWidth,document.body.clientHeight);	
		},1000)	
	}
...
function createGrid()
{
	...
	addToolbar();
	setTimeout(function(){
			panel.setSize(document.body.clientWidth,document.body.clientHeight);
			Ext.getCmp('mainpanel').setSize(document.body.clientWidth,document.body.clientHeight);
			Ext.getCmp('mygrid').setSize(document.body.clientWidth,document.body.clientHeight-160);	
	},500)
	Ext.getCmp('mygrid').getBottomToolbar().hide();
}
function addToolbar()
{
//	//判断安全组
//	var secGrpFlag=toMgSecGrp(session['LOGON.GROUPID']);
	...
	tobar.addItem("-");
	tobar.addButton({id:'allfind',icon:'../images/uiimages/search.png',text:'查询',handler:findRec});
	if((secGrpFlag=="nurhead")||(secGrpFlag=="demo")){
		tobar.addItem("-");
		tobar.addButton({id:'sendbtn',icon:'../images/uiimages/blue_arrow.png',text:'发布',handler:sendRec});
		tobar.addItem("-");
		tobar.addButton({id:'unsendbtn',icon:'../images/uiimages/undo.png',text:'撤销发布',handler:unsendRec});
	...
	tobar2.addButton({
		id:'nowWeek',
		icon:'../images/uiimages/TMrowid.png',
		text:'本周',
		handler:nowWeekRec
	});
	tobar2.addItem("-");
...
function setStatus()
{
	...
	if(a=="Y"){
		...
		if(b==1) Ext.Msg.alert('提示',"提交成功!");
		else Ext.Msg.alert('提示',"提交失败!");
	}else {
		Ext.Msg.alert('提示','还有未完成的排班，不可提交!');
	}
}
...
function createTable()
{
	...
	if(bedflag==0){
		colModelStr.push({header: '床位/备注',align:'center', width: 80,sortable: false,dataIndex: 'JobAssign',editor:new Ext.grid.GridEditor(new Ext.form.TextField({readOnly:false,listeners:{blur:function(){saveRec('mygrid')}}}))});
	}else{
		colModelStr.push({header: '床位/备注',align:'center', width: 80,sortable: false,dataIndex: 'JobAssign',editor:new Ext.grid.GridEditor(new Ext.form.TextField({readOnly:false,listeners:{blur:function(){saveRec('mygrid')}}}))});
	}
	...
	for(AdmDate=new Date(StDateValue.format('m/d/Y'));AdmDate<=new Date(EndDateValue.format('m/d/Y'));AdmDate=AdmDate.add(Date.DAY,1)){
		if(flagNum==3){
			...
			colModelStr .push({header:(AdmDate.format('d/m')+'<br>'+getWeek(AdmDate.getDay())),align:'center', width: 95, sortable: false,dataIndex: 'Date'+AdmDate.format('Y-m-d')});
		}
	...
	var myview=new Ext.grid.GridView({ //var myview=new Ext.ux.grid.LockingGridView
		syncHeights:true,
		onLoad : Ext.emptyFn, 
    listeners : { 
...
function createASMPanel()
{
	var panel = new Ext.Panel({
		...
		layout:'absolute',
		frame:true,
		tbar:['-',new Ext.Button({id:'asgnmtfindbtn',icon:'../images/uiimages/search.png',text:'查询',handler:getArgJob}),
					'-',new Ext.Button({id:'loadlastasgnmt',icon:'../images/uiimages/saveapplycard.png',text:'加载上周',handler:getlastArgJob}),
					'-',new Ext.Button({id:'asgnmtbtn',icon:'../images/uiimages/filesave.png',text:'保存',handler:function(){
						var wardid=combo.getValue();
						var EndDateVal=EndDate.getValue().format('Y/m/d');
						var daynum=EndDate.getValue().format('N')
						var firstDay=((new Date(EndDateVal)).add(Date.DAY,1-daynum)).format('Y-m-d');
					...
					'-',new Ext.Button({id:'amcanclebtn',icon:'../images/uiimages/clearscreen.png',text:'清屏',handler:function(){
						var Assignment=Ext.getCmp("Assignment");
						Assignment.setValue("");
					}})
		],
...
function createControlTable()
{
	var colModel = new Ext.grid.ColumnModel([
		{ header: "时段", width: 120, sortable: true,dataIndex: 'ControlDate'},
		{ header: "名称", width: 100, sortable: true,dataIndex: 'ControlName'},
    { header: "值", width: 80, sortable: true,dataIndex: 'ControlValue'}
 	]);
...
function createPostTable()
{
    ...
    { header: "PostID", width: 80, sortable: true, dataIndex: 'PostID',hidden:true}
 	]);
	var store = new Ext.data.Store({
 		proxy:new Ext.data.HttpProxy({url:"../csp/dhc.nurse.ext.common.getdata.csp"}),
   	reader:new Ext.data.JsonReader({root:'rows',totalProperty:'results',
...
function ChoosePostData(postType,gridStr)
{
	...
	tobar.addItem("-");
	tobar.addItem(new Ext.Button({id:'checkbtn',icon:'../images/uiimages/ok.png',text:'选择',handler:function(){if(WritePostData(postType,gridStr)) {window.close();FindArgJobData();}}}));
	//tobar2.addItem("-");
	//tobar2.addItem(new Ext.form.Label({id:'remarklabel',icon:'../images/uiimages/bluepaper.png',text:'备注：'}));
...
function getControlData()
{
	...
	if(selections.length==1){
		...
		var firstDay1="",lastDay1="";
		if(flagNum==3){
			firstDay1=((new Date(EndDateVal)).add(Date.DAY,1-daynum)).format('Y-m-d');
			lastDay1=((new Date(EndDateVal)).add(Date.DAY,7-daynum)).format('Y-m-d');
		}else if(flagNum==4){
			firstDay1=((new Date(EndDateVal)).add(Date.DAY,1-daynum)).format('d/m/Y');
			lastDay1=((new Date(EndDateVal)).add(Date.DAY,7-daynum)).format('d/m/Y');
		}
		for(var i=0;i<array.length;i++){
			REC.push({
				ControlDate:firstDay1+"至"+lastDay1,
		...
		var yearMon="";
		if(flagNum==3){
			yearMon=StDate.getValue().format('Y-m');
		}else if(flagNum==4){
			yearMon=StDate.getValue().format('m/Y');
		}
		for(var i=0;i<array.length;i++){
			REC.push({
...
function ChooseHolidayData(gridStr)
{
	...
	tobar.addItem("-");
	tobar.addItem(new Ext.Button({id:'checkbtn',icon:'../images/uiimages/ok.png',text:'选择',handler:function(){if(WriteHolidayData(gridStr)) window.close();}}));
	tobar.addItem("-");
	tobar.doLayout();
...
function CreateOverTime(gridStr)
{
	...
	var window = new Ext.Window({
			...
			width:490,height:210,x:250,y:120,
			renderTo: Ext.getBody(),
			modal:true,layout: 'absolute',
			items:[
				new Ext.form.Label({id:'StDate2Label',text:'开始日期',width:60,x:10,y:10}),
				new Ext.form.DateField({id:'StDate2',x:80,y:10,width:150,value:nowDate}),
				...
				new Ext.form.DateField({id:'EndDate2',x:80,y:40,width:150,value:nowDate}),
				...
				new Ext.Button({id:'overtimebtn',text:'确定',icon:'../images/uiimages/ok.png',width:100,x:360,y:150,
...
function WritePostData(postType,gridStr)
{
	...
	if(PostHalfDay1==true&&PostHalfDay2==true){
		alert("上下午不可以同时选择,请重新选择!");
		return;
	}
	var selections=mygrid.getSelectionModel().selections;
	...
	for(var ii=0;ii<selections.length;ii++){
		...
		if(postType=="P"){
			var bbb=WardId+"^"+NurseId+"^"+nowDate+"^"+NurseType;
			var aaa=tkMakeServerCall("DHCMGNUR.MgNurArrangeJob","FindNurRosterData",bbb);
			if(aaa!=1){
				alert("该护士当天已排班,若要排班请选择追加排班!");
				return;
			}
		}	
...
function SaveArgJobData(gridStr,parr)
{
	...
	var firstDay=((new Date(EndDateVal)).add(Date.DAY,1-daynum)).format('Y-m-d');
	var lastDay=((new Date(EndDateVal)).add(Date.DAY,7-daynum)).format('Y-m-d');
	var par=WardId+"^"+firstDay+"^"+lastDay;
	var aa=tkMakeServerCall("DHCMGNUR.MgNurArgJob","GetStatus2",par)
	if (aa=="S"){
		alert("排班已发布,请撤销发布后再修改!");
		return;
	}
...
function ExportAJMData()
{
	...
	var stdate = StDate.getValue().format('Y-m-d');
	var endate = EndDate.getValue().format('Y-m-d');
	var stdate1 = StDate.getValue().format('d/m/Y');
	var endate1 = EndDate.getValue().format('d/m/Y');
  ...
  xlSheet.PageSetup.BottomMargin = 1/0.035;   //页边距 下5厘米   
	...
	xlSheet.Cells(1,1).Value=combo.getRawValue()+"工作安排表";//.split('-')[1]
	xlSheet.Cells(1,1).Font.Size = 16; 
	xlSheet.Cells(1,1).Font.Bold = true;
	xlSheet.rows(1).rowHeight=30;
	xlSheet.rows(1).HorizontalAlignment = 3;
	xlSheet.Cells(2,1).Value="制表时间："+new Date().format('d/m/Y')+"   "+(new Date()).format('H:i:s');
	xlSheet.Cells(2,5).Value="时间段："+stdate1+" 至 "+endate1;
...
function PrintAJMData()
{
  ...
  xlSheet.PageSetup.BottomMargin = 1/0.035;   //页边距 下5厘米   
	...
	for(var col=1;col<cm.getColumnCount()-3;col++){
		if(cm.getColumnWidth(col)!=0){
			...
			if((collen>3)&&(collen<11)) xlSheet.Columns(collen).ColumnWidth=9.5;
	...
	xlSheet.rows(1).HorizontalAlignment = 3;
	xlSheet.Cells(2,1).Value="制表时间："+new Date().format('d/m/Y')+"   "+(new Date()).format('H:i:s');
	xlSheet.Cells(2,6).Value="时间段："+stdate1+" 至 "+endate1;
  ...
  var array=SplitStr(ret);
	...
	xlSheet.Rows(5+rowlen+spacelen).RowHeight =50;
	xlSheet.Cells(5+rowlen+spacelen,1).VerticalAlignment=1
	xlSheet.Cells(5+rowlen+spacelen,1).Value="通知或备注："+Assignment;
	//xlSheet.PrintOut(1,1,1,false,"",false,false);
	//xlSheet.PageSetup.BottomMargin = 3/0.035;
	xlSheet.PrintOut();
	xlBook.Close (savechanges=false);
...
function connectBed(gridStr)
{
    ...
    { header: "BedId", width: 0,  dataIndex: 'BedId',hidden:true}
 	]);
...
//    })
    ...
    { header: "GroupId", width: 0,  dataIndex: 'GroupId',hidden:true}
 	]);
	...
	var grouptable=new Ext.grid.GridPanel({
		...
		tbar:['-',new Ext.Button({id:'connbtn',text:'关联',icon:'../images/uiimages/link-big.png',handler:function(){saveConnectBed(gridStr);Win.close();}})],
//		bbar:new Ext.PagingToolbar({ 
//        pageSize: 1000, 
//        store: groupstore, 
//        displayInfo: true, 
...
function saveConnectBed(gridStr)
{
	...
	for(var i=0;i<selections.length;i++){
		...
		if(cellValue!=""){
			...
			var nowDate=new Date(cm.getDataIndex(cellIndexNum).substring(4).replace(/\-/g,'/')).format('Y-m-d');
			var NurseType = mygrid.getStore().getAt(rowIndexNum).get("NurseType")
			var nuritem=WardId+"^"+NurseId+"^"+nowDate+"^"+NurseType;
			var ret=tkMakeServerCall("DHCMGNUR.MgNurArrangeJob","FindNurRosterData",nuritem);
			var posttype=ret.split("^");
			if (posttype[0].indexOf("Holiday")>=0){
				alert("已休班的人员不可关联床位,请重新选择!");
				return;
			}
			...
			Ext.Msg.alert('提示','存在未排班的记录,不可关联床位');
			return;
		}
	}
...
function sendRec()
{
	...
	var lastDay=((new Date(EndDateVal)).add(Date.DAY,7-daynum)).format('Y-m-d');
	var parr=WardId+"^"+firstDay+"^"+lastDay;
	var aa=tkMakeServerCall("DHCMGNUR.MgNurArgJob","GetStatus2",parr)
	if (aa=="S"){
		alert("已发布!");
		return;
	}
...
function unsendRec()
{
	...
	if(ret==""){Ext.Msg.alert('提示',"不是发布状态，不可以撤销！"); return;}
	var array=SplitStr(ret);
	if(array["Status"]=="S"){
		var SetStatus=document.getElementById('SetStatus').value;
		var b=cspRunServerMethod(SetStatus,parr+"^N");
		if(b==1) Ext.Msg.alert('提示',"撤销发布成功!");
		else Ext.Msg.alert('提示',"撤销发布失败!");
	}else {
		Ext.Msg.alert('提示',"不是发布状态，不可以撤销！");
	}
	FindArgJobData();
}
