...
function gethead() {
	...
	var hh = ret.split("^");
	//return hh[0] + "  " + hh[1];
	return "<font size=3>"+hh[0] + "  " + hh[1]+"</font>";
	//debugger;
}
...
function BodyLoadHandler() {
	...
	if((session['LOGON.GROUPDESC'].indexOf("医生")!="-1")||(session['LOGON.GROUPDESC'].indexOf("医师")!="-1")){
		var objItem=colModel.config;
		for(var i=0;i<objItem.length;i++){
			if(objItem[i].dataIndex=="Item7"){
				colModel.setColumnWidth(objItem[i].id,350)
			}	
		}
	}
	...
	if (UserType == "NURSE") {
		//竖方向录入start
		document.querySelector("#ext-gen4").style.height = (document.body.clientHeight - 500) + "px";
		document.querySelector("#ext-gen5").style.height = (document.body.clientHeight - 517) + "px";
		document.querySelector("#ext-gen8").style.height = (document.body.clientHeight - 517) + "px";
		document.querySelector("#ext-gen13").style.height = (document.body.clientHeight - 517) + "px"; 
	...
	if (StartLock == "1") {
		if ((Startloc.indexOf(session['LOGON.CTLOCID']) > -1) || (Startloc == "")) {
			...
			*/
			tobar.addButton({
				text: "预览打印",
				handler: printNurRecZK, //这个个调用原来的方法,在原来基础上加一句：PrintComm.Xuflag=0
				id: 'PrintButZK'
			});
		} else {
			tobar.addButton({
	...
	*/
	...
	grid.getStore().on('load', function (s, records) {
		...
		s.each(function (r) {
			if(r.get("PageMark") == 'Y') {
				grid.getView().getRow(girdcount).style.backgroundColor = '#F7EE7E';     //术后标记过的显示红色
			}

			if ((r.get(DisplaySumInName.split("!")[0]) == '总入液量=')||(r.get(DisplaySumInName.split("!")[0]) == '总结入量')){
				grid.getView().getRow(girdcount).style.backgroundColor = '#F7FE2E';
			}
			if ((r.get(DisplaySumInName.split("!")[0]) == '入液量=')||(r.get(DisplaySumInName.split("!")[0]) == '小结入量')) {
...
function SepcialChar() {
	...
	var a = cspRunServerMethod(GetQueryData.value, "User.DHCTEMPSPECIALCHAR:CRItemNew", "", "AddSpecChar");	
	var table = new Ext.grid.EditorGridPanel({
		...
		listeners:{
			...
			render: function (grid){   
                ...
                listeners: {              // Change content dynamically depending on which element triggered the show.  
                    beforeshow: function updateTipBody(tip) {  
                        ...
                        if(cellIndex < 3 || cellIndex >colCount-6) return false;
                        var WardId=combo.getValue();
												...
												var nowDate = date.add(Date.DAY,cellIndex-3).format('Y-m-d'); 
			...
			celldblclick:function(grid,rowIndex,colIndex ,e)
			{
				...
				sepRowIndex=rowIndex;
				sepColIndex=colIndex;
				insertSpecChar();
			}, 
	...
	table.store.loadData(storespechar);
...
function SpecCharFn(flag) {
	...
	if (selModel.hasSelection()) {
		...
		*/
		var specchardesc="";
		var cell=grid.getView().getCell(sepRowIndex,sepColIndex);
		if(cell){
			specchardesc=cell.innerText;
		}
		var mygrid = Ext.getCmp('mygrid');
				...
				if(window.frames[0]){
					...
					if(flag=="I"){
						...
						window.frames[0].InserTextCommReplace(specchardesc);
					}
					return;
				}
...
function save() {
	...
	for (var i = 0; i < list.length; i++) {
		...
		for (var p in obj) {
			...
			if (aa == "") {
				if(p=='Item7')  {
					...
					TypeStrOut=DBC2SBC(obj[p]);
					//return;
				}
				else if(p=='Item10')  {
					QuantityStrOut=DBC2SBC(obj[p]);
					//return;
				}
				else {
					str = str + p + "|" + DBC2SBC(obj[p]) + '^';
				...
				if((p!="CareDate")&&(p!="CareTime")&&(p!="par")&&(p!="rw")){
					if(obj[p]!=""){
						canSaveFlag="1";
					} 
				}
		...
		if ((str != "") && (flag == "0")) {
			...
			if((rtn==1)||(recid!="")){
				if((TypeStr.split("^").length>1)||(TypeStrOut.split("^").length>1)){
					//alert(1);
					var moreLenTypeStr=""
					var moreQuantityStrOut=""
					if((TypeStr.split("^").length>TypeStrOut.split("^").length)){
						moreLenTypeStr=TypeStr;
						moreQuantityStrOut=QuantityStr;	
					}else{
						moreLenTypeStr=TypeStrOut;
						moreQuantityStrOut=QuantityStrOut;	
					}
					/*var TypeArr=TypeStr.split("^")
					var QuantityArr=QuantityStr.split("^")
					var str1=str;*/
					var moreLenTypeArr=moreLenTypeStr.split("^")
					var moreLenQuantityArr=moreQuantityStrOut.split("^")
					var TypeArr=TypeStr.split("^");
					var QuantityArr=QuantityStr.split("^");
					var TypeArrOut=TypeStrOut.split("^");
					var QuantityArrOut=QuantityStrOut.split("^");
					var str1=str;
					
					
					
					for (var i=0;i<moreLenTypeArr.length;i++){
						if(i==0){
							str = str1 +"^Item7|"+TypeArr[i]+"^Item8|"+QuantityArr[i];
							str = str +"^Item9|"+TypeArrOut[i]+"^Item10|"+QuantityArrOut[i];
							if(canSaveFlag=="1") a = cspRunServerMethod(RecSave.value, EpisodeID, str, session['LOGON.USERID'], EmrCode, session['LOGON.GROUPDESC']);
						}
						else{
							str="CareDate|" + CareDate + "^CareTime|" + CareTime;
							str = str + "^DiagnosDr|" + DiagnosDr;
							str = str + "^RecLoc|" + patloc + "^RecNurseLoc|" + session['LOGON.CTLOCID'] + "^RecBed|" + bedCode;   //patloc是病人当前科室，如果打印时“科室”希望打印的是科室则用该句；//patloc
							str = str + "^" + CaseMeasureID;
							//str = str +"^Item7|"+TypeArr[i]+"^Item8|"+QuantityArr[i];
							if(i<TypeStr.split("^").length)
							{
								str = str +"^Item7|"+TypeArr[i]+"^Item8|"+QuantityArr[i];
							}
							if(i<TypeStrOut.split("^").length){
								str = str +"^Item9|"+TypeArrOut[i]+"^Item10|"+QuantityArrOut[i];
							}
							if(canSaveFlag=="1") a = cspRunServerMethod(RecSave.value, EpisodeID, str, session['LOGON.USERID'], EmrCode, session['LOGON.GROUPDESC']);
						}
				...
				//alert(str);
					...
					//str=str +"^Item7|"+TypeStr+"^Item8|"+QuantityStr+ "^RecNurseLoc|" + session['LOGON.CTLOCID'] ;
					//alert(str);
					str=str +"^Item7|"+TypeStr+"^Item8|"+QuantityStr+"^Item9|"+TypeStrOut+"^Item10|"+QuantityStrOut+ "^RecNurseLoc|" + session['LOGON.CTLOCID'] ;
					if(canSaveFlag=="1") a = cspRunServerMethod(RecSave.value, EpisodeID, str, session['LOGON.USERID'], EmrCode, session['LOGON.GROUPDESC']);
					}
				//var a = cspRunServerMethod(RecSave.value, EpisodeID, str, session['LOGON.USERID'], EmrCode, session['LOGON.GROUPDESC']);
				if (i == (list.length - 1)) {
	...
	//if() 
	window.frames[0].self.location.reload();
	//setTimeout("find()", 0);
	//setcolor(grid); //续打设置颜色
}
function saveOld() {
...
function printNurRecXuTest() {
	...
	var edate = "" //tm[4];
	var etim = "" //tm[5];
	PrintCommPicIP.RHeadCaption = hh[1];
	PrintCommPicIP.LHeadCaption = hh[0];
	PrintCommPicIP.TitleStr = ret;
	PrintCommPicIP.LFootCaption = hh[2];
	PrintCommPicIP.SetPreView("1"); //打印时是否预览：1预览，0不预览
	PrintCommPicIP.PrnLoc = session['LOGON.CTLOCID']; //如果没更新转科这句放开
	//PrintCommPicIP.PrnLoc=patloc ;  //转科打印时设置床号 如果没更新转科这句屏蔽
	PrintCommPicIP.PrnBed = bedCode; //转科打印时设置床号  如果没更新转科这句屏蔽
	PrintCommPicIP.tranflag = "1" //转科打印设置:0不启用 1启用转科打印
	PrintCommPicIP.SplitPage = "1" //转科是否换页
	PrintCommPicIP.dxflag = 0 //分割线设置：1一条记录打印一条线 0 一行打印一条线		
	PrintCommPicIP.xuflag = 1; //续打配置：1启用续打 0 普通全部打印	
	PrintCommPicIP.previewPrint = "0"; //是否弹出设置界面:0不弹出，1弹出
	PrintCommPicIP.ItmName = prncode; //打印模板名称取的是配置
	PrintCommPicIP.WebUrl = WebIp + "/dthealth/web/DWR.DoctorRound.cls";
	PrintCommPicIP.stPage = 0;
	PrintCommPicIP.stRow = 0;
	PrintCommPicIP.stprintpos = 0;
	var parr = EpisodeID + "!" + stdate + "!" + stim + "!" + edate + "!" + etim + "!" + EmrCode + "!"; //注意不要少了最后一个！
	PrintCommPicIP.ID = "";
	PrintCommPicIP.MultID = "";
	//PrintCommPicIP.MthArr="web.DHCConsult:getConsultInfo&id:"+myid;	//混合模板打印	
	if (LabHead != "") PrintCommPicIP.LabHead = LabHead;
	PrintCommPicIP.SetParrm(parr); // ="EpisodeId" ; //"p1:0^p2:"
...
function printNurRecXu() {
	...
	PrintCommPicIP.stPage = 0;
	PrintCommPicIP.stRow = 0;
	PrintCommPicIP.NurseLocHuanYe="N";
	PrintCommPicIP.stprintpos = 0;
	var parr = EpisodeID + "!" + stdate + "!" + stim + "!" + edate + "!" + etim + "!" + EmrCode + "!"; //注意不要少了最后一个！
	PrintCommPicIP.ID = "";
	PrintCommPicIP.MultID = "";
		...
		s.each(function (r) {
			...
			if (printinfoST != "") {
				...
				if (rowid != printarrST[0]) {
					...
					} else if ((r.get(DisplaySumInName.split("!")[0]) == '总入液量=')||(r.get(DisplaySumInName.split("!")[0]) == '总结入量')) {
						grid.getView().getRow(girdcount).style.backgroundColor = '#F7FE2E';
					} else if ((r.get(DisplaySumInName.split("!")[0]) == '入液量=')||(r.get(DisplaySumInName.split("!")[0]) == '小结入量')){
						grid.getView().getRow(girdcount).style.backgroundColor = '#A7FE2E';
...
function printNurRec() {
	...
	PrintComm.ItmName = "DHCNurPrnMouldXH2"; //338155!2010-07-13!0:00!!
	PrintComm.NurseLocHuanYe="N";
	//debugger;
	...
	//alert(pages+","+CareDateTim+","+stprintpos+","+EpisodeID+","+"EmrCode"+","+session['LOGON.USERID']+","+PrintComm.PrnFlag);
	//PrnFlag==1说明是打印预览
...
function printNurRecnew() {
	...
	//alert(PrintComm.Pages);
	//PrintComm.SetConnectStr(CacheDB);
	PrintComm.ItmName = "DHCNurPrnMouldXH2new"; //338155!2010-07-13!0:00!!
	//debugger;
	var parr = EpisodeID + "!" + stdate + "!" + stim + "!" + edate + "!" + etim + "!" + EmrCode;
	//alert(parr);
	PrintComm.ID = "";
...
function printNurRecZK() {
	...
	var stim = "" //tm[3];
	var edate = "" //tm[4];
	//alert(PrintCommPicIP)
	var etim = "" //tm[5];
	PrintCommPicIPDBGJ.RHeadCaption = hh[1];
	PrintCommPicIPDBGJ.LHeadCaption = hh[0];
	//PrintComm.RFootCaption="第";
	//PrintComm.LFootCaption="页";
	//alert(ret)
	PrintCommPicIPDBGJ.TitleStr = ret;
	PrintCommPicIPDBGJ.LFootCaption = hh[2];
	PrintCommPicIPDBGJ.SetPreView("1");
	//PrintCommPicIP.PrnLoc=session['LOGON.CTLOCID'];
	PrintCommPicIPDBGJ.PrnLoc = patloc;
	PrintCommPicIPDBGJ.PrnBed = bedCode; //转科打印时设置床号
	PrintCommPicIPDBGJ.xuflag = 0; //0 不启用续打，1 启用续打
	PrintCommPicIPDBGJ.dxflag = 0;
	PrintCommPicIPDBGJ.SplitPage = "1" //转科是否换页1 换页，0不换页
	PrintCommPicIPDBGJ.WebUrl = WebIp + "/dthealth/web/DWR.DoctorRound.cls";
	//alert(PrintComm.WebUrl);
	var aa = tm[1].split("&");
	//PrintComm.stPage=aa[0];
	//if (aa.length>1) PrintComm.stRow=aa[1];
	PrintCommPicIPDBGJ.stPage = 0;
	PrintCommPicIPDBGJ.stRow = 0;
	PrintCommPicIPDBGJ.previewPrint = "0"; //是否弹出设置界面
	PrintCommPicIPDBGJ.NurseLocHuanYe="Y";
	//PrintComm.stprintpos=tm[0];
	PrintCommPicIPDBGJ.stprintpos = 0;
	PrintCommPicIPDBGJ.tranflag = 1; //是否启用转科
	PrintCommPicIPDBGJ.EmrCode = EmrCode;
	//alert(PrintComm.Pages);
	//PrintComm.SetConnectStr(CacheDB);
	PrintCommPicIPDBGJ.ItmName = prncode; //338155!2010-07-13!0:00!!
	//debugger;
	var parr = EpisodeID + "!" + stdate + "!" + stim + "!" + edate + "!" + etim + "!" + EmrCode + "!";
	//alert(parr);
	PrintCommPicIPDBGJ.ID = "";
	PrintCommPicIPDBGJ.MultID = "";
	//PrintCommPicIP.MthArr="web.DHCConsult:getConsultInfo&id:"+myid;
	if (LabHead != "") PrintCommPicIPDBGJ.LabHead = LabHead;
	//alert(parr);
	PrintCommPicIPDBGJ.SetParrm(parr); // ="EpisodeId" ; //"p1:0^p2:"
	
	//by lmm 定位预览
	var selobj = grid.getSelectionModel().getSelected();
	if(selobj){
		PrintCommPicIPDBGJ.selRowId=selobj.get("par")+"||"+selobj.get("rw");
	}
	//alert(PrintCommPic.selRowId)
	//end
	//PrintCommPicIPDBGJ.MakeInfo = "Y";    // by lmm 奇偶页打印生成打印记录 --增加
	PrintCommPicIPDBGJ.firstShowFlag = 0; 
	PrintCommPicIPDBGJ.PrintOut();
	//by lmm 奇偶页打印 --增加
    //PrintCommPicIPDBGJ.SetPreView("1");  //1 预览 0 不预览
    //PrintCommPicIPDBGJ.xuflag = "0";     //1-启用续打 ；0-全部打印   
    //PrintComm.CLeft = "Y";      //Y-偶数页左边距是奇数页右边距 N-奇数和偶数页左边距相等           
    //PrintCommPicIPDBGJ.PrintOutAll();       //生成打印记录    
...
function printNurRecZKBak() {
	...
	PrintCommPicIP.TitleStr = ret;
	PrintCommPicIP.LFootCaption = hh[2];
	PrintCommPicIP.SetPreView("1");
	//PrintCommPicIP.PrnLoc=session['LOGON.CTLOCID'];
	PrintCommPicIP.PrnLoc = patloc;
	PrintCommPicIP.PrnBed = bedCode; //转科打印时设置床号
	PrintCommPicIP.xuflag = 0; //0 不启用续打，1 启用续打
	PrintCommPicIP.dxflag = 0;
	PrintCommPicIP.SplitPage = "1" //转科是否换页1 换页，0不换页
	PrintCommPicIP.WebUrl = WebIp + "/dthealth/web/DWR.DoctorRound.cls";
	//alert(PrintComm.WebUrl);
	var aa = tm[1].split("&");
	//PrintComm.stPage=aa[0];
	...
	PrintCommPicIP.stPage = 0;
	PrintCommPicIP.stRow = 0;
	PrintCommPicIP.previewPrint = "1"; //是否弹出设置界面
	PrintCommPicIP.NurseLocHuanYe="N";
	//PrintComm.stprintpos=tm[0];
	PrintCommPicIP.stprintpos = 0;
	PrintCommPicIP.tranflag = 1; //是否启用转科
	PrintCommPicIP.EmrCode = EmrCode;
	//alert(PrintComm.Pages);
	//PrintComm.SetConnectStr(CacheDB);
	PrintCommPicIP.ItmName = prncode; //338155!2010-07-13!0:00!!
	//debugger;
	var parr = EpisodeID + "!" + stdate + "!" + stim + "!" + edate + "!" + etim + "!" + EmrCode + "!";
	//alert(parr);
	PrintCommPicIP.ID = "";
	...
	//alert(parr);
	PrintCommPicIP.SetParrm(parr); // ="EpisodeId" ; //"p1:0^p2:"
	//PrintCommPicIP.MakeInfo = "Y";    // by lmm 奇偶页打印生成打印记录 --增加
	PrintCommPicIP.PrintOut();
	//by lmm 奇偶页打印 --增加
    //PrintCommPicIP.SetPreView("1");  //1 预览 0 不预览
    //PrintCommPicIP.xuflag = "0";     //1-启用续打 ；0-全部打印   
    //PrintComm.CLeft = "Y";      //Y-偶数页左边距是奇数页右边距 N-奇数和偶数页左边距相等           
    //PrintCommPicIP.PrintOutAll();       //生成打印记录    
	//window.location.reload();
	//end
}
...
function MakePicture(page) {
	...
	PrintCommPicIP.stRow = 0;
	PrintCommPicIP.TitleStr = ret;
	PrintCommPicIP.previewPrint = "0"; //是否弹出设置界面
	PrintCommPicIP.stPrintPos = 0;
	PrintCommPicIP.PrnLoc = session['LOGON.CTLOCID'];
	PrintCommPicIP.Patcurloc = patloc; //病人当前科室id
	PrintCommPicIP.NurseLocHuanYe = "Y"; //Y:转病区换页(按绑定的护士科室id) N:按病人科室换页
	PrintCommPicIP.PrnBed = bedCode; //病人当前床号
	PrintCommPicIP.tranflag = "1"; //转科换页
	PrintCommPicIP.dxflag = 0; //1：一条记录打印一条横线；0：一条记录的每一行文字下面都打印一条横线
	PrintCommPicIP.xuflag = "0"; //0:不启用续打印 ；1 启用续打
	PrintCommPicIP.SplitPage = "1"; //1:有转科或转病区时新起一页打印
	PrintCommPicIP.ItmName = prncode; //打印模板名 请参考printNurRec
	PrintCommPicIP.EpisodeID = EpisodeID; //图片
	PrintCommPicIP.EmrCode = EmrCode; //图片
	PrintCommPicIP.MakeTemp = "Y"; //图片  Y:生成图片  ；N:不生成
	if (WillUpload == "Y") {
		PrintCommPicIP.IfUpload = "Y"; //图片是否上传ftp服务器：Y -上传，N-不上传
	} else {
		PrintCommPicIP.IfUpload = "N";
	}
	PrintCommPicIP.MakeAllPages = "N"; //图片  Y:生成该病人该模板的所有图片; N:生成最近一页图片
	PrintCommPicIP.curPages = page; //图片  开始页码
	PrintCommPicIP.filepath = WebIp + "/DHCMG/HLBLMakePictureSet.xml"; //图片 ftp服务器配置
	var parr = EpisodeID + "!" + "!" + "!" + "!" + "!" + EmrCode + "!";
	PrintCommPicIP.ID = "";
	PrintCommPicIP.MultID = "";
	PrintCommPicIP.SetParrm(parr);
	PrintCommPicIP.PrintOut();
...
function printNurRecZKTest() {
	...
	PrintCommPicIP.RHeadCaption = hh[1];
	PrintCommPicIP.LHeadCaption = hh[0];
	//PrintCommPicIP.RFootCaption="第";
	//PrintCommPicIP.LFootCaption="页";
	//alert(ret)
	PrintCommPicIP.TitleStr = ret;
	PrintCommPicIP.LFootCaption = hh[2];
	PrintCommPicIP.SetPreView("1");
	...
	//alert(PrintComm.Pages);
	//PrintComm.SetConnectStr(CacheDB);
	PrintCommPicIP.ItmName = "DHCNurPrnMouldXH2newzk"; //338155!2010-07-13!0:00!!
	//debugger;
	var parr = EpisodeID + "!" + stdate + "!" + stim + "!" + edate + "!" + etim + "!" + EmrCode + "!";
	//alert(parr);
	PrintCommPicIP.ID = "";
...
function linkDocBl() {
	...
	var lnk = "emr.record.browse.category.csp?" + "&PatientID=&EpisodeID=" + EpisodeID  //""+"&Status="+""  ;//"&DtId="+DtId+"&ExamId="+ExamId
	// window.open(lnk, "title3444", ',top=10,left=360,toolbar=no,location=no,directories=no,resizable=yes,width=800,height=500');
	//var objret = window.showModalDialog(lnk, 'docBl', "dialogWidth:1200px;dialogHeight:800px;center:yes;status:no;scroll:YES;help:no;edge:raised;");
	var objret = window.open(lnk, 'docBl', "dialogWidth:1200px;dialogHeight:800px;center:yes;status:no;scroll:YES;help:no;edge:raised;");
}
function setPageMark(){
		...
		if (objRow.length == 0) {
			...
			var rw = objRow[0].get("rw");
			var PageMark= "Y";
			var a=tkMakeServerCall("Nur.DHCNurseRecSub","SetPageMark", par, rw, session['LOGON.USERID'], session['LOGON.GROUPDESC'],PageMark)
			if(a==0){
				find();
			}else{
				alert(a);
	...
	function cancelPageMark(){
		...
		if (objRow.length == 0) {
			...
			var rw = objRow[0].get("rw");
			var PageMark= "N";
			var a=tkMakeServerCall("Nur.DHCNurseRecSub","CancelPageMark", par, rw, session['LOGON.USERID'], session['LOGON.GROUPDESC'],PageMark)
			if(a==0){
				find();
			}else{
				alert(a);
