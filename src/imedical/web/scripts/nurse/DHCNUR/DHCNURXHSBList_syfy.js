...
{ 
  ...
  if (getuser) {
   ...
   //alert(getuser)
    if (getuser!=""){Ext.Msg.alert('提示:以下人员需填压疮追踪表', getuser)}		
	}
...
function qddelete()
{
	...
	var objRow=grid.getSelectionModel().getSelections();
	var Status = objRow[0].get("Status");
		if ((objRow.length != 0)&&(Status.indexOf("已作废")>-1)) { Ext.Msg.alert('提示', "已经作废过了!"); return;}
...
function modccevent()
{
	...
	if (objRow.length == 0) {
		...
		if (Status=="已审核") 
		{var Status="A"}
		var lnk= "DHCNurEmrComm.csp?"+"&EmrCode=DHCNURXHSB_ssfy&EpisodeID="+EpisodeID+"&NurRecId="+par+"&Status="+Status  ;//"&DtId="+DtId+"&ExamId="+ExamId
...
function newEvent()
{
    ...
    if (logonht==1)
	{
		var lnk= "DHCNurEmrComm.csp?"+"&EmrCode=DHCNURXHSB_ssfy&EpisodeID="+EpisodeID  ;//"&DtId="+DtId+"&ExamId="+ExamId
	}
	...
	//alert(lnk)
	 wind22=window.open(lnk,"htmi",'left=10,toolbar=no,location=no,directories=no,resizable=yes,width=800,height=700');
	 //alert(wind22)
...
function find()
{
  ...
  var IfCancelRec=Ext.getCmp("IfCancelRec").getValue();
	//alert(StDate.vlaue);
	var parr = "DHCNURXHSB_ssfy^"+adm+"^"+loc+"^"+StDate.value+"^"+Enddate.value+"^"+IfCancelRec;
  //	alert(parr);
   ...
   var store = mygrid.store;
    ...
    { 
    	...
    	{  if (Value.indexOf("B")==-1)
    	   {
    	    mygrid.getView().getCell(i,9).style.backgroundColor="#FF0000"	    	
    	   }
    	   if (Value.indexOf("B")!=-1)
    	   {
    	    mygrid.getView().getCell(i,12).style.backgroundColor="#CCFFFF"
    	    //break;	    	
    	   }
    	   if (Value.indexOf("K")!=-1)
    	   {
    	    mygrid.getView().getCell(i,12).style.backgroundColor="#66CCFF"	    	
    	   }
    	   if ((Value.indexOf("B")!=-1)&&(Value.indexOf("K")==-1))
    	   {
    	     mygrid.getView().getCell(i,12).style.backgroundColor="#6699CC"	    	
    	   }
    	    if ((Value.indexOf("B")!=-1)&&(Value.indexOf("K")==-1)&&(Value.indexOf("H")==-1))
    	   {
    	     mygrid.getView().getCell(i,12).style.backgroundColor="#6600FF"	    	
    	   }
     }
    	if (usertype=="科护士长")
    	{   if ((Value.indexOf("B")==-1)||(Value.indexOf("K")==-1))
    		{
    			mygrid.getView().getCell(i,9).style.backgroundColor="ff4500"
    		}
    	}
      ...
      if (flag==1)
       {
		...
		if(Status=="V")
		{ var mygrid=Ext.getCmp('mygrid');
			mygrid.getView().getCell(i,11).style.backgroundColor="#FFCCFF"
		}
		if(Status=="V2")
		{ var mygrid=Ext.getCmp('mygrid');
			mygrid.getView().getCell(i,11).style.backgroundColor="#FFCCFF"
	    ...
	    if(Status=="S1")
		{ var mygrid=Ext.getCmp('mygrid');
			mygrid.getView().getCell(i,11).style.backgroundColor="FFCC66"
		}
		 if(Status=="S2")
		{ var mygrid=Ext.getCmp('mygrid');
		 ...
		 if(Status=="A")
		{ var mygrid=Ext.getCmp('mygrid');
			mygrid.getView().getCell(i,11).style.backgroundColor="1e90ff"
		}
