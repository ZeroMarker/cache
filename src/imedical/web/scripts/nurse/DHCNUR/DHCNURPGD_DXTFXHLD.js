...
//DateItem[0] = "Item13";
//TimeItem[0] = "Item91";
var prncode = "DHCNURMouldPrn_DXTFXHLD";
		...
		if(Ext.getCmp('Item'+i)){
			Ext.getCmp('Item'+i).on('focus',function(e){
				...
				if(itm.value==''){
					...
					itm.blur();
					if(selFlag==0){
						Ext.getCmp('Item41').setValue(date.dateFormat('Y-m-d'));
						Ext.getCmp('Item42').setValue(date.dateFormat('H:i'));
						selFlag=1;
					}
				}else{
					itm.setValue('');
...
function Save() {
  ///by lmm  增加年份保存
  var recDate = Ext.getCmp("Item41")    //括号item数值要改成评估日期的item数值
  var recYear="";
  if(recDate.getValue()!=""){
  recYear=formatDate(recDate.getValue()).split("-")[0]
  }
  var recYearObj = Ext.getCmp("Item98")  //新加的评估年份Item 元素不要超过345
