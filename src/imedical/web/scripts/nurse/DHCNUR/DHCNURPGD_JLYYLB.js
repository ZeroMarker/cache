...
var partArray = ["Item11", "Item12", "Item13", "Item14", "Item15", "Item16", "Item17", "Item18", "Item19", "Item20", "Item21", "Item22", "Item23", "Item24"];   //项目
var partScoreArray = ["Item29", "Item30", "Item31", "Item32", "Item33", "Item34", "Item35", "Item36", "Item37", "Item38", "Item39", "Item40", "Item41", "Item42"];  //分数
ScoreItem[0] = "^Item29^Item30^Item31^Item32^Item33^Item34^Item35^Item36^Item37^Item38^Item39^Item40^Item41^Item42^";  //分数值Item
ValueItem[0] = "Item26";       //总分
NameItem[0] = "Item27";        //评估护士签名(默认)
DateItem[0] = "Item8";			//日期、时间
TimeItem[0] = "Item9";
var prncode = "DHCNURMouldPrn_JLYYLB";  //打印模板
...
function BodyLoadHandler() {
  ...
  but.on('click', Save);
  var but = Ext.getCmp("btnPrintNone");
 ...
 // tmpItm.disable();
  ...
  calScore("Item11", "Item29");
  calScore("Item12", "Item30");
  calScore("Item13", "Item31");
  calScore("Item14", "Item32");
  calScore("Item15", "Item33");
  calScore("Item16", "Item34");
  calScore("Item17", "Item35");
  calScore("Item18", "Item36");
  calScore("Item19", "Item37");
  calScore("Item20", "Item38");
  calScore("Item21", "Item39");
  calScore("Item22", "Item40");
  calScore("Item23", "Item41");
  calScore("Item24", "Item42");
}
function sjj() { }
...
function Save() {
	...
	var recYearObj = Ext.getCmp("Item43")  //新家的评估年份Item 元素不要超过345    
  ...
  comboret = "";
  radioret = "";
   //var SaveTime = document.getElementById('Item9');
   //var Time=new Date();
   //SaveTime.value=Time;
  //alert(Time);
  ...
  //alert(ret);
  // alert(comboret);
  var arr = new Array("Item29", "Item30", "Item31", "Item32", "Item33", "Item34", "Item35", "Item36", "Item37", "Item38", "Item39", "Item40", "Item41", "Item42");       //分数值
  var count = 0;
  //用于判断分数值填写是否完整
...
function getPatInfo() {
	...
	//	paersonLX.setValue(getValueByCode(tt[20]) + getValueByCode(tt[10]));
...
function calScore(partItem, scoreItem) {
  ...
  {	
	case "Item11":
        ...
        if (checked == true) {
          if (field.boxLabel == "3 几乎所有时候") {
            partScore = 3;
          } else if (field.boxLabel == "2 大多数时候") {
            partScore = 2;
          }else if (field.boxLabel == "1 有时") {
            partScore = 1;
          } else {
            partScore = 0;
          }
	...
	case "Item12":
		...
		item.on("check", function (field, checked) {
        if (checked == true) {
          if (field.boxLabel == "3 基本上没有了") {
            partScore = 3;
          } else if (field.boxLabel == "2 只有一点儿") {
            partScore = 2;
          }else if (field.boxLabel == "1 不像以前那样多") {
            partScore = 1;
          } else {
            partScore = 0;
          }
	...
	case "Item13":
		...
		item.on("check", function (field, checked) {
        if (checked == true) {
          if (field.boxLabel == "3 非常肯定和十分严重") {
            partScore = 3;
          } else if (field.boxLabel == "2 有，不太严重") {
            partScore = 2;
          }else if (field.boxLabel == "1 有一点，但并不使我苦恼") {
            partScore = 1;
          } else {
            partScore = 0;
          }
	...
	case "Item14":
		...
		item.on("check", function (field, checked) {
        if (checked == true) {
          if (field.boxLabel == "3 根本没有") {
            partScore = 3;
          } else if (field.boxLabel == "2 现在肯定是不太多了") {
            partScore = 2;
          }else if (field.boxLabel == "1 现在已经不太这样了") {
            partScore = 1;
          } else {
            partScore = 0;
          }
	...
	case "Item15":
		...
		item.on("check", function (field, checked) {
        if (checked == true) {
          if (field.boxLabel == "3 大多数时间") {
            partScore = 3;
          } else if (field.boxLabel == "2 常常如此") {
            partScore = 2;
          }else if (field.boxLabel == "1 时时，但并不经常") {
            partScore = 1;
          } else {
            partScore = 0;
          }
	...
	case "Item18":
		...
		item.on("check", function (field, checked) {
        if (checked == true) {
          if (field.boxLabel == "3 肯定") {
            partScore = 3;
          } else if (field.boxLabel == "2 并不像我应该做到的那样关心") {
            partScore = 2;
          }else if (field.boxLabel == "1 我可能不是非常关心") {
            partScore = 1;
          } else {
            partScore = 0;
          }
	...
	case "Item19":
		...
		item.on("check", function (field, checked) {
        if (checked == true) {
          if (field.boxLabel == "3 确实非常多") {
            partScore = 3;
          } else if (field.boxLabel == "2 是不少") {
            partScore = 2;
          }else if (field.boxLabel == "1 并不很少") {
            partScore = 1;
          } else {
            partScore = 0;
          }
	...
	case "Item20":
		...
		item.on("check", function (field, checked) {
        if (checked == true) {
          if (field.boxLabel == "3 几乎从来不这样做") {
            partScore = 3;
          } else if (field.boxLabel == "2 很少这样做") {
            partScore = 2;
          }else if (field.boxLabel == "1 并不完全是这样做的") {
            partScore = 1;
          } else {
            partScore = 0;
          }
	...
	case "Item21":
		...
		item.on("check", function (field, checked) {
        if (checked == true) {
          if (field.boxLabel == "3 确实很经常") {
            partScore = 3;
          } else if (field.boxLabel == "2 时常") {
            partScore = 2;
          }else if (field.boxLabel == "1 并非经常") {
            partScore = 1;
          } else {
            partScore = 0;
          }
	...
	case "Item22":
		...
		item.on("check", function (field, checked) {
        if (checked == true) {
          if (field.boxLabel == "3 几乎所有的时间") {
            partScore = 3;
          } else if (field.boxLabel == "2 很经常") {
            partScore = 2;
          }else if (field.boxLabel == "1 有时") {
            partScore = 1;
          } else {
	...
	case "Item24":
		...
		item.on("check", function (field, checked) {
        if (checked == true) {
          if (field.boxLabel == "3 很少") {
            partScore = 3;
          } else if (field.boxLabel == "2 并非经常") {
            partScore = 2;
          }else if (field.boxLabel == "1 有时  ") {
            partScore = 1;
          } else {
...
function butPrintFn() {
	...
	var getid = document.getElementById('GetId');
	var id = cspRunServerMethod(getid.value, EmrCode, EpisodeID);
	PrintCommPicIP.ItmName = "DHCNURMouldPrn_JLYYLB";
	PrintCommPicIP.MthArr = "Nur.DHCMoudData:getVal2&id:" +"" + "!";
  ...
  PrintCommMultiPGD.PrintRecId="";  
 
  PrintCommMultiPGD.MthArr= "Nur.DHCMoudDataSub:getVal1&parr:" + "" + "!flag:";
