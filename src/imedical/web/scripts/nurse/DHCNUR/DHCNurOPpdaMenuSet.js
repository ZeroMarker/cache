...
function onReady() {
	...
	var modelList = new Ext.grid.GridPanel({
		...
		cm: new Ext.grid.ColumnModel([{
			...
			header: "目录显示",
			dataIndex: "menuPdaShow",
			width: 300
		}, {
			header: "目录fragement",
			dataIndex: "menuPdaFragement",
			width: 300
		}, {
			header: "ID",
			dataIndex: "ID",
				...
				listeners: {
					click: function() {
						...
						loadData(Ext.getCmp("modelList"), {
							className: "Nur.DHCNurOPpdaMenuSet",
		...
		store: createStore({
			...
			methodName: "getSetByloc"
		}, ["menuPdaName", "menuPdaId", "menuPdaShow", "menuPdaFragement","ID"]),
		viewConfig: {
			forceFit: true
		}
	});
 ...
 var WinForm = new Ext.FormPanel({
				...
				items: [
					{ 
								id: "menuPdaId", 
								xtype: "textfield",
								fieldLabel:'<font size=3>目录id</font>',
								width: 250,
								value: "",
								labelStyle: "font-size:18px"
					},
					{ 
								id: "menuPdaName", 
								xtype: "textfield",
								fieldLabel:'<font size=3>目录名称</font>',
								width: 250,
								value: "",
								labelStyle: "font-size:18px"
					},
					{ 
								id: "menuPdaShow", 
								xtype: "textfield",
								fieldLabel:'<font size=3>目录显示</font>',
								width: 250,
								value: "",
								labelStyle: "font-size:18px"
					},
					{ 
								id: "menuPdaFragement", 
								xtype: "textfield",
								fieldLabel:'<font size=3>目录fragement</font>',
								width: 250,
								value: "",
								labelStyle: "font-size:18px"
					}
					,{
								id: "menuPdaSheet",
								xtype: "combo",
								fieldLabel:'<font size=3>是否启用</font>',
								width: 250,
								mode : 'local',
								store: [
								["1", "Y"],
...
function saveModel()
{
	var menuPdaId = Ext.getCmp("menuPdaId").getValue();
	var menuPdaName = Ext.getCmp("menuPdaName").getValue();
	var menuPdaShow = Ext.getCmp("menuPdaShow").getValue();
	var menuPdaFragement = Ext.getCmp("menuPdaFragement").getValue();
	var menuPdaSheet = Ext.getCmp("menuPdaSheet").getValue();
	
	if((menuPdaId=="")||(menuPdaName=="")||(menuPdaShow=="")||(menuPdaFragement=="")||(menuPdaSheet==""))
	{
		alert("信息不完整!!!")
		return;
	}
	...
	//if(subselobj){
		...
		//alert(setId)
	//}
	var parr=menuPdaId+"^"+menuPdaName+"^"+menuPdaShow+"^"+menuPdaFragement+"^"+menuPdaSheet;
	
	var resStr = tkMakeServerCall("Nur.DHCNurOPpdaMenuSet", "Save", parr);
	if(resStr==""){
		alert("保存成功")
	}
	AddModelItemwindow.hide();
...
{ 
	...
	var setInfo = tkMakeServerCall("Nur.DHCNurOPpdaMenuSet", "getModelSetInfoById", setId);
	var setArr=setInfo.split("^");
	Ext.getCmp("menuPdaName").setValue(setArr[0]);
	Ext.getCmp("menuPdaId").setValue(setArr[1]);
	Ext.getCmp("menuPdaShow").setValue(setArr[2]);
	Ext.getCmp("menuPdaFragement").setValue(setArr[3]);
