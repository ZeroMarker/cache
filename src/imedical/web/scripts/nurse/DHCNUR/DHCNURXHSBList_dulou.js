...
function modccevent()
{
	...
	if (objRow.length == 0) {
		...
		if (Status=="已审核") 
		{var Status="A"}
		var lnk= "DHCNurEmrComm.csp?"+"&EmrCode=DHCNURXHSB_dulou&EpisodeID="+EpisodeID+"&NurRecId="+par+"&Status="+Status  ;//"&DtId="+DtId+"&ExamId="+ExamId
...
function newEvent()
{
    ...
    if (logonht==1)
	{
		var lnk= "DHCNurEmrComm.csp?"+"&EmrCode=DHCNURXHSB_dulou&EpisodeID="+EpisodeID  ;//"&DtId="+DtId+"&ExamId="+ExamId
	}
...
function find()
{
   ...
   var IfCancelRec=Ext.getCmp("IfCancelRec").getValue();
	//alert(StDate.vlaue);
	var parr = "DHCNURXHSB_dulou^"+adm+"^"+loc+"^"+StDate.value+"^"+Enddate.value+"^"+IfCancelRec;
  //	alert(parr);
    ...
    var Value=store.getAt(i).get("value");	
   	...
   	if (Status=="未上报") 
		...
		{var Status="A"}
		 if ((date>6)&&(Status=="S2"))
    { 
    	if ((usertype=="")||(usertype=="科护士长")||(usertype=="护士长"))
      ...
      if (flag==1)
       {
		...
		if(Status=="V")
		{ var mygrid=Ext.getCmp('mygrid');
			mygrid.getView().getCell(i,12).style.backgroundColor="#FFCCFF"
		}
		if(Status=="V2")
		{ var mygrid=Ext.getCmp('mygrid');
			mygrid.getView().getCell(i,12).style.backgroundColor="#FFCCFF"
	 ...
	 if(Status=="S1")
		{ var mygrid=Ext.getCmp('mygrid');
			mygrid.getView().getCell(i,12).style.backgroundColor="FFCC66"
		}
		 if(Status=="S2")
		{ var mygrid=Ext.getCmp('mygrid');
			mygrid.getView().getCell(i,12).style.backgroundColor="ff4500"
		 ...
		 if(Status=="A")
		{ var mygrid=Ext.getCmp('mygrid');
			mygrid.getView().getCell(i,12).style.backgroundColor="1e90ff"
		}
