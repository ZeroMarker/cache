...
function qdtj()
{
	...
	if (((objRow.length != 0)&&(Status!="未上报"))||((objRow.length != 0)&&(Status!="已退回"))) { alert("已经上报了!"); return;}
...
function modccevent()
{
	...
	if (objRow.length == 0) {
		...
		if (Status=="已审核") 
		{var Status="A"}
		var lnk= "DHCNurEmrComm.csp?"+"&EmrCode=DHCNURXHBJ_YYCC2015&EpisodeID="+EpisodeID+"&NurRecId="+par+"&Status="+Status  ;//"&DtId="+DtId+"&ExamId="+ExamId
...
function newEvent()
{
    ...
    if (logonht==1)
	{
		var lnk= "DHCNurEmrComm.csp?"+"&EmrCode=DHCNURXHBJ_YYCC2015&EpisodeID="+EpisodeID  ;//"&DtId="+DtId+"&ExamId="+ExamId
	}
	...
	if (logonht==0)
	{
	 ...
	 wind22=window.open(lnk,"htm",'left=10,toolbar=no,location=no,directories=no,resizable=yes,width=870,height=700');
...
function find()
{
   ...
   var IfCancelRec=Ext.getCmp("IfCancelRec").getValue();
	//alert(StDate.vlaue);
	var parr = "DHCNURXHBJ_YYCC2015^"+adm+"^"+loc+"^"+StDate.value+"^"+Enddate.value+"^"+IfCancelRec;
  //	alert(parr);
  ...
  mygrid.store.loadData(arrgrid);   
   var store = mygrid.store;
    ...
    { 
		...
		if(Status=="V")
		{ var mygrid=Ext.getCmp('mygrid');
			mygrid.getView().getCell(i,9).style.backgroundColor="#FFCCFF"
		}
		if(Status=="V2")
		{ var mygrid=Ext.getCmp('mygrid');
			mygrid.getView().getCell(i,9).style.backgroundColor="#FFCCFF"
	 ...
	 if(Status=="S1")
		{ var mygrid=Ext.getCmp('mygrid');
			mygrid.getView().getCell(i,9).style.backgroundColor="FFCC66"
		}
		 if(Status=="S2")
		{ var mygrid=Ext.getCmp('mygrid');
		 ...
		 if(Status=="A")
		{ var mygrid=Ext.getCmp('mygrid');
			mygrid.getView().getCell(i,9).style.backgroundColor="1e90ff"
		}
