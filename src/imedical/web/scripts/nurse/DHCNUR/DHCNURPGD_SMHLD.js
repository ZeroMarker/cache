...
var partArray = ["Item28", "Item29", "Item30", "Item31", "Item32", "Item33", "Item34", "Item35"];
var partScoreArray = ["Item7", "Item8", "Item9", "Item10", "Item11", "Item12", "Item13", "Item14"];
ScoreItem[0] = "^Item7^Item8^Item9^Item10^Item11^Item12^Item13^Item14^";
ValueItem[0] = "Item15";
NameItem[0] = "Item24";
DateItem[0] = "Item26";
TimeItem[0] = "Item27";
var prncode = "DHCNURMouldPrn_SMHLD";
...
function totelmouth() {
  ...
  zongf = a[0];
  fenj = a[1];
  var zf = Ext.getCmp("Item15");
  zf.setValue(getValueByCode(zongf));
  var fj = Ext.getCmp("Item26");
...
function BodyLoadHandler() {
  ...
  // firstScore.focus();
  //计算各项radio分数
  calScore("Item28", "Item7");
  calScore("Item29", "Item8");
  calScore("Item30", "Item9");
  calScore("Item31", "Item10");
  calScore("Item32", "Item11");
  calScore("Item33", "Item12");
  calScore("Item34", "Item13");
  calScore("Item35", "Item14");
		...
		if(Ext.getCmp('Item'+i)){
			Ext.getCmp('Item'+i).on('focus',function(e){
				...
				if(itm.value==''){
					...
					itm.blur();
					if(selFlag==0){
						Ext.getCmp('Item26').setValue(date.dateFormat('Y-m-d'));
						Ext.getCmp('Item27').setValue(date.dateFormat('H:i'));
						selFlag=1;
					}
				}else{
					itm.setValue('');
...
function Save() {
	//by lmm  增加年份保存
	var recDate = Ext.getCmp("Item26")    //评估日期Item
	var recYear="";
	if(recDate.getValue()!=""){
		recYear=formatDate(recDate.getValue()).split("-")[0]
	}
	var recYearObj = Ext.getCmp("Item36")  //新家的评估年份Item 元素不要超过345
...
function getPatInfo() {
  ...
  if (PatInfo) {
	...
	var signature = Ext.getCmp("Item24");
  ...
  var partGroup = Ext.getCmp(partItem).items.items;
    partGroup.forEach(function (item, index, array) {
      item.on("check", function (field, checked) {
        if (checked == true) {
          if (field.boxLabel == "不会瞌睡") {
            partScore = 0;
          } else if (field.boxLabel == "很少打瞌睡") {
            partScore = 1;
          } else if (field.boxLabel == "有时打瞌睡") {
            partScore = 2;
          } else {
            partScore = 3;
          }
