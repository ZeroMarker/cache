...
function editAudite()
{
  ...
  var tm=ret.split('^')
	...
	var aa=cspRunServerMethod(pdata1,"","DHCNURPerPersonalBookAdd","","");
	var arr=eval(aa);
	...
	Ext.getCmp("PerBookTitle").setValue(ha1.items("PerBookTitle"));
	Ext.getCmp("PerBookNumber").setValue(ha1.items("PerBookNumber"));
	Ext.getCmp("PerPubAgency").setValue(ha1.items("PerPubAgency"));
	Ext.getCmp("PerPubDate").setValue(ha1.items("PerPubDate"));
	Ext.getCmp("PerCpyEtTyp").setValue(ha1.items("PerCpyEtTyp"));
...
function editAdd()
{
  ...
  var tm=ret.split('^')
	...
	Ext.getCmp("PerPubAgency").setValue(ha1.items("PerPubAgency"));
	Ext.getCmp("PerPubDate").setValue(ha1.items("PerPubDate"));
	var PerCpyEtTyp=Ext.getCmp("PerCpyEtTyp");
	PerCpyEtTyp.triggerAction='all';
	PerCpyEtTyp.setEditable(false);
	PerCpyEtTyp.setValue(ha1.items("PerCpyEtTyp"))
	window.show();
	var btnSave=Ext.getCmp("btnSave");
...
function addNew()
{
  ...
  var tm=ret.split('^')
	sethashvalue(ha,tm);
	var a = cspRunServerMethod(pdata1, "", "DHCNURPerPersonalBookAdd", "", "");
	arr = eval(a);
	var window= new Ext.Window({
		title : '添加个人著作信息',
		id : "gform2",
...
function SureCheck(RowId,edit1)
{
	...
	var PerBookTitle=Ext.getCmp("PerBookTitle").getValue();
	if(PerBookTitle==""){Ext.Msg.alert('提示',"请填写书名！");return;}
	var PerBookNumber=Ext.getCmp("PerBookNumber").getValue();
	if(PerBookNumber==""){Ext.Msg.alert('提示',"请填写出版书号！");return;}
	var PerPubAgency=Ext.getCmp("PerPubAgency").getValue();
	if(PerPubAgency==""){Ext.Msg.alert('提示',"请填写出版社！");return;}	
	var PerPubDate=Ext.getCmp("PerPubDate").getValue();
	if(PerPubDate==""){
		Ext.Msg.alert('提示',"请填写出版时间！");
		return;
	}else{
	...
	var PerCpyEtTyp=Ext.getCmp("PerCpyEtTyp").getValue();

	var parr="raw|"+RowId+"^PerName|"+PerName+"^PerID|"+PerID+"^PerBookTitle|"+PerBookTitle+"^PerBookNumber|"+PerBookNumber+"^PerPubAgency|"+PerPubAgency+"^PerPubDate|"+PerPubDate+"^PerCpyEtTyp|"+PerCpyEtTyp;
//alert(parr);
