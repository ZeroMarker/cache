...
DateItem[0] = "Item13";
TimeItem[0] = "Item91";
var prncode = "DHCNURMouldPrn_LYHLD";
...
function BodyLoadHandler() {
  ...
  // firstScore.focus();
  //计算各项radio分数
  calScore("Item11", "Item35");
  calScore("Item12", "Item36");
		...
		if(Ext.getCmp('Item'+i)){
			Ext.getCmp('Item'+i).on('focus',function(e){
				...
				if(itm.value==''){
					...
					itm.blur();
					if(selFlag==0){
						Ext.getCmp('Item9').setValue(date.dateFormat('Y-m-d'));
						Ext.getCmp('Item10').setValue(date.dateFormat('H:i'));
						selFlag=1;
					}
				}else{
					itm.setValue('');
...
function Save() {
  ///by lmm  增加年份保存
  var recDate = Ext.getCmp("Item9")    //括号item数值要改成评估日期的item数值
  var recYear="";
  if(recDate.getValue()!=""){
  recYear=formatDate(recDate.getValue()).split("-")[0]
  }
  var recYearObj = Ext.getCmp("Item37")  //新加的评估年份Item 元素不要超过345
...
function getPatInfo() {
  ...
  if (PatInfo) {
    ...
    var InpatNo = Ext.getCmp("Item7");
    InpatNo.setValue(getValueByCode(tt[28]));
	var signature = Ext.getCmp("Item33");
  ...
  if (partItem == "Item11") {
    partGroup.forEach(function (item, index, array) {
      item.on("check", function (field, checked) {
        if (checked == true) {
          if (field.boxLabel == "独立安全的行走50m(6分)") {
            partScore = 6;
          } else if (field.boxLabel == "独立进出、独立驱动轮椅行50m(5分)") {
            partScore = 5;
          } else if (field.boxLabel == "进出时需要他人辅助、但进入轮椅后能独立驱动行进50m(4分)") {
            partScore = 4;
          } 
		  else if (field.boxLabel == "需用安全带防护或需监视或只能行17m(3分)") {
            partScore = 3;
          } 
		  else if (field.boxLabel == "有部分时间需由他人协助推动轮椅(2分)") {
            partScore = 2;
          } else {
            partScore = 1;
          }
    ...
    partGroup.forEach(function (item, index, array) {
      item.on("check", function (field, checked) {
        if (checked == true) {
          if (field.boxLabel == "不需要使用轮椅(评分6分)") {
            partScore = 6;
          } else if (field.boxLabel == "监护患者自行使用轮椅(评分5分)") {
            partScore = 5;
          } else if (field.boxLabel == "使用轮椅需要部分协助(评分4分)") {
            partScore = 4;
          } 
		  else if (field.boxLabel == "使用轮椅需要部分协助(评分3分)") {
            partScore = 3;
          } 
		  else if (field.boxLabel == "使用轮椅需要部分协助(评分2分)") {
            partScore = 2;
          } else {
            partScore = 1;
          }
