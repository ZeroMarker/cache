...
if (GetNurseRecSet){
	var ret=cspRunServerMethod(GetNurseRecSet.value,"DHCNURBURN");
	var hh=ret.split("^");
...
function getBurnDate()
{
	...
	var GetHead=document.getElementById('GetPatInfoBurn');
	var ret=cspRunServerMethod(GetPatInfoBurn.value,EpisodeID);
	var hh=ret.split("@");
	StartTime=hh[1];
	BurnDate=hh[5];
	BurnTime=hh[6];
...
function saveBurnDate()
{
	 var SaveBurnDate=document.getElementById("SaveBurnDate").value
	 var burnDate = Ext.getCmp("burnDate");
	 var burnTime = Ext.getCmp("burnTime");
	 var weight = Ext.getCmp("weight").getValue();
	 var totalArea = Ext.getCmp("totalArea").getValue();
	 var areaOne = Ext.getCmp("areaOne").getValue();
	 var areaTwo = Ext.getCmp("areaTwo").getValue();
	 var areaThree = Ext.getCmp("areaThree").getValue();
	 //alert(burnTime.value)
	 var a=cspRunServerMethod(SaveBurnDate,EpisodeID,burnDate.value,burnTime.value,weight,totalArea,areaOne,areaTwo,areaThree);
	 
	 alert("保存成功!")
}
function saveBurnDateNoAlert()
{
	 var SaveBurnDate=document.getElementById("SaveBurnDate").value
	 var burnDate = Ext.getCmp("burnDate");
	 var burnTime = Ext.getCmp("burnTime");
...
function PrintSet()
{
 ...
 //debugger;
   ...
   // debugger;
    REC=new Array();
 	var a=cspRunServerMethod(GetNurPrnSet.value,"DHCNURBURN",EpisodeID,'addprnset');
...
function find(){
	...
	var parr = adm + "^" + StDate.value + "^" + StTime.value + "^" + Enddate.value + "^" + EndTime.value + "^"+"DHCNURBURN" + "^" + IfCancelRec+"^"+CurrHeadDr;
	// debugger;
...
{  //查询出入液量合计
	...
	//alert(parr);
  	//debugger;
 	 	var a=cspRunServerMethod(GeInOutAmount.value,adm,StDate.value,StTime.value,Enddate.value,EndTime.value,"DHCNURBURN");
...
{ 
	...
	for (var i = 0; i < list.length; i++) {
	  ...
	  var flag="0";
		for (var p in obj) {
	   if(p!="CareDate"&&p!="CareTime")
		{
			obj[p]=obj[p].replace(/\s/g,"");
				if(obj[p].match(re))
				{
					alert("包含错误的字符哦！请检查删除后再保存 ")
					return
				}
			}		
			var aa = formatDate(obj[p]);				
		...
		if ((str!="")&&(flag=="0"))
		{
			...
			var a=cspRunServerMethod(RecSave.value,EpisodeID,str,session['LOGON.USERID'],"DHCNURBURN",session['LOGON.GROUPDESC']);
			if (a!="0")
			{
				alert(a);
				return;
...
function MultiFun()
{
	...
	var ret= cspRunServerMethod(GetMulti.value, "DHCNURBURN");
	...
	var tabstr=cspRunServerMethod(getcheckform.value, "DHCNURBURN",ab);
...
function multiSel(ret)
{
    ...
    var itname=grid1.getSelectionModel().getSelections()[0].get("itm4"); 
	  var getcheckform=document.getElementById('getcheckform');
		alert(itname+"!!"+code)
		var ret=cspRunServerMethod(getcheckform.value, "DHCNURBURN",itname,code,"");	
...
{ //小结
	...
	else
	{
	  ...
	  else
	  {
		...
		//alert(str);
		var a=cspRunServerMethod(SaveOutIn.value,EpisodeID,str,session['LOGON.USERID'],"DHCNURBURN");
		find();
	}
}
function InOutSum()
...
{ 
   ...
   // debugger;
	...
	var parr="DHCNURBURN^"+EpisodeID+"^CaseMeasureXml^"+rowid;
	//alert(parr);
	var arr = new Array();
	var a = cspRunServerMethod(pdata1, "", "DHCPatOrdList", EpisodeID, "");
	arr = eval(a);
...
function sureMeasure()
{
   ...
   // var parr="DHCNURBURN^"+EpisodeID+"^CaseMeasureXml";
