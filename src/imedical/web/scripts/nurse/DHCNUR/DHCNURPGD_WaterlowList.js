...
function butPrintFn()
{
  var mygrid = Ext.getCmp("mygrid");
	...
	if (len < 1) {
		Ext.Msg.show({title:'注意',msg:'请选择需要打印的数据!',buttons:
...
function CancelRecord()
{
	...
	{
		var flag = confirm("你确定要作废这条记录吗？")
		if (flag) {
                    var par=objRow[0].get("par");
					//var rw=objRow[0].get("rw");
					//alert(par+","+rw+","+session['LOGON.USERID']+","+session['LOGON.GROUPDESC']);
					var a = tkMakeServerCall("Nur.DHCMoudData","CancelMoudRecord", par, session['LOGON.USERID'], session['LOGON.GROUPDESC']);
					if (a!=0){
						alert(a);
						return;
					}else{
...
var logonht=1;
function newEventold()
{
     //alert(NewPGD)
	  NewPGD.Adm=EpisodeID;
	  NewPGD.ConnectStr="";
    NewPGD.weburl=WebIp+"/imedical/web/DWR.DHCNurNewPGD.cls";
    NewPGD.wardId=session['LOGON.CTLOCID'];
	  NewPGD.UserDesc=session['LOGON.USERNAME'];
    NewPGD.UserId=session['LOGON.USERID'];
    NewPGD.UserCode=session['LOGON.USERCODE'];
	  NewPGD.UserGrp=session['LOGON.GROUPDESC']
    NewPGD.UserGrpId=session['LOGON.GROUPID']
    NewPGD.EmrCode="DHCNURPGD_WATERLOW";
    NewPGD.NurRecId="";
	if (websys_DateFormat=='d/m/Y')
	{
	  NewPGD.format="dd/MM/yyyy"
	}else{
		NewPGD.format="yyyy-MM-dd"
	}
	//alert(NewPGD.format)
    NewPGD.FilePath=WebIp+"/dhcmg/DHCNURPGD_WATERLOW.xml";
    NewPGD.ShowWindow();
...
function newEvent()
{
	...
	var weburl=WebIp+"/imedical/web/DWR.DHCNurNewPGD.cls";
	...
	if(fso.FileExists("c:\\newpgd.txt"))
	{
		var f2 = fso.GetFile("c:\\newpgd.txt");
		if(f2) {f2.Delete();}
	}
	*/
	var killstr=tkMakeServerCall("NurEmr.DHCNurEmrPrintComm","KillTmpNewPGDGlobal","");
	//alert(NewPGD.format)
    var FilePath=WebIp+"/dhcmg/DHCNURPGD_WATERLOW.xml";
   
	var link=WebIp+"/dhcmg/NewPGD/NewPGD.application?method=ShowWindow&EpisodeID="+EpisodeID+"&EmrCode="+"DHCNURPGD_WATERLOW"+"&ConnectStr="+"&weburl="+weburl+"&wardId="+session['LOGON.CTLOCID']+"&UserDesc="+session['LOGON.USERNAME']+"&UserId="+session['LOGON.USERID']+"&UserCode="+session['LOGON.USERCODE']+"&UserGrp="+session['LOGON.GROUPDESC']+"&UserGrpId="+session['LOGON.GROUPID']+"&NurRecId="+"&format="+format+"&FilePath="+FilePath;
...
function modccevent()
{
	...
	*/
	...
	var link=WebIp+"/dhcmg/NewPGD/NewPGD.application?method=ShowWindow&EpisodeID="+EpisodeID+"&EmrCode="+"DHCNURPGD_WATERLOW"+"&ConnectStr="+"&weburl="+weburl+"&wardId="+session['LOGON.CTLOCID']+"&UserDesc="+session['LOGON.USERNAME']+"&UserId="+session['LOGON.USERID']+"&UserCode="+session['LOGON.USERCODE']+"&UserGrp="+"护士长"+"&UserGrpId="+session['LOGON.GROUPID']+"&NurRecId="+par+"&format="+format+"&FilePath="+FilePath;
	window.location.href=link;
	}
	setTimeout("find()",10000);
	//setTimeout("searchlist()",5000);
...
function searchlist()
{
	...
	*/
	...
	while(getstr=="")
	{
		getstr=tkMakeServerCall("NurEmr.DHCNurEmrPrintComm","GetTmpNewPGDGlobal","");
		if(getstr=="-1")
		{
			find();
			var killstr=tkMakeServerCall("NurEmr.DHCNurEmrPrintComm","KillTmpNewPGDGlobal","");
...
function find()
{
	...
	//var parr = "DHCNURXHFFYC^"+adm+"^"+loc+"^"+StDate.value+"^"+Enddate.value+"^"+IfCncelRec;
		var parr = "DHCNURPGD_WATERLOW^"+adm+"^"+loc+"^"+"^"+"^"+IfCncelRec;
		//alert(parr)
		  var mygrid = Ext.getCmp("mygrid");
  ...
  //alert(parr);
  /*
	var a = cspRunServerMethod(GetQueryData.value, "Nur.DHCMoudData:MoudDatabyadm", "parr$" +parr,"AddRec");
	//alert(a);
	
  mygrid.store.loadData(arrgrid);   
  //arr = eval(a);
  //if(arr[0].items[0]) arr[0].items[0].getBottomToolbar().hide();
  */
