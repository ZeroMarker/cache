...
function editAudite()
{
  ...
  var tm=ret.split('^')
	...
	var aa=cspRunServerMethod(pdata1,"","DHCNURPerRewardAdd","","");
	var arr=eval(aa);
	...
	Ext.getCmp("PerRewPunDate").setValue(ha1.items("PerRewPunDate"));
	Ext.getCmp("PerRewPunName").setValue(ha1.items("PerRewPunName"));
	Ext.getCmp("PerRewPunReason").setValue(ha1.items("PerRewPunReason"));
	Ext.getCmp("PerRewPunUnit").setValue(ha1.items("PerRewPunUnit"));
	window.show();
	var btnSave=Ext.getCmp("btnSave");
...
function addNew()
{
  ...
  var tm=ret.split('^')
	sethashvalue(ha,tm);
	var a = cspRunServerMethod(pdata1, "", "DHCNURPerRewardAdd", "", "");
	arr = eval(a);
	var window= new Ext.Window({
		title : '添加奖惩信息',
		id : "gform2",
...
function SureCheck(RowId,edit1)
{
	...
	var PerRewPunDate=Ext.getCmp("PerRewPunDate").getValue();//奖惩日期
	if(PerRewPunDate==""){
		Ext.Msg.alert('提示',"请选择奖惩日期！");
		return;
	}else{
		if(PerRewPunDate instanceof Date){
			PerRewPunDate=PerRewPunDate.format('Y-m-d');
		}
	}
	var PerRewPunName=Ext.getCmp("PerRewPunName").getValue();//奖惩名称
	if(PerRewPunName==""){Ext.Msg.alert('提示',"请输入奖惩名称！");return;}
	var PerRewPunReason=Ext.getCmp("PerRewPunReason").getValue();//奖惩原因
	var PerRewPunUnit=Ext.getCmp("PerRewPunUnit").getValue();//奖惩批准单位	
	if(PerRewPunUnit==""){Ext.Msg.alert('提示',"请输入奖惩单位！");return;}
	var parr="raw|"+RowId+"^PerName|"+PerName+"^PerID|"+PerID+"^PerRewPunDate|"+PerRewPunDate+"^PerRewPunName|"+PerRewPunName+"^PerRewPunReason|"+PerRewPunReason+"^PerRewPunUnit|"+PerRewPunUnit;
	//alert(parr)
