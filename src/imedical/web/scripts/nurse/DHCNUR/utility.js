			...
			beforequery: function (e) {
				...
				if (!e.forceAll) {
					...
					combo.store.filterBy(function (record, id) {
						var text = getPinyin(record.get(displayField));
...
function CreateComboBox(id, displayField, valueField, width, className, methodName, queryParam) {
	...
	combox.on('expand', function(comboBox) {
		comboBox.list.setWidth(width*2.5);
		comboBox.innerList.setWidth(width*2.5);
	}, this, {
		single: true
	});
	return combox;
...
function CreateTitle(id, value, type) {
	...
	var title = '<span id=\'' + id + 'Title\' onclick=\'\' onmousedown=\'function() { this.blur(); }\' oncontextmenu=\'function() { return false; }\' >'+value+'</span>';
	return title;
}
