...
function editAudite()
{
  ...
  var tm=ret.split('^')
	...
	var aa=cspRunServerMethod(pdata1,"","DHCNURPerChiefPositionAdd","","");
	var arr=eval(aa);
	...
	var dutyNameStore=Ext.getCmp("PerDutyName").getStore();
	dutyNameStore.load({params:{start:0,limit:100}});
	dutyNameStore.on('load',function(dutyNameStore,record,options){
		Ext.getCmp("PerDutyName").selectText();
		Ext.getCmp("PerDutyName").setValue(ha1.items("PerDutyName").replace("__","||"));	
	});
	Ext.getCmp("PerPromoteDate").setValue(ha1.items("PerPromoteDate"));
	Ext.getCmp("PerWorkUnit").setValue(ha1.items("PerWorkUnit"));
	window.show();
	var btnSave=Ext.getCmp("btnSave");
...
function editAdd()
{
  ...
  var tm=ret.split('^')
	...
	var perDutyName=Ext.getCmp('PerDutyName');
	perDutyName.setEditable(false);
	var perName=Ext.getCmp("PerName");
	perName.disabled=true;
	Ext.getCmp("PerName").setValue(ha1.items("PerName"));		
	var perID=Ext.getCmp("PerID");
...
function addNew()
{
  ...
  var tm=ret.split('^')
	sethashvalue(ha,tm);
	var a = cspRunServerMethod(pdata1, "", "DHCNURPerChiefPositionAdd", "", "");
	arr = eval(a);
	var window= new Ext.Window({
		title : '添加行政职务信息',
		id : "gform2",
...
function SureCheck(RowId,edit1)
{
	...
	var PerDutyName=Ext.getCmp("PerDutyName").getValue();//职务名称
	if(PerDutyName==""){Ext.Msg.alert('提示',"请选择职务名称！");return;}
	if(PerDutyName){
		PerDutyName=PerDutyName.replace("||","__");
		}
	var PerPromoteDate=Ext.getCmp("PerPromoteDate").getValue();//晋升日期	
	if(PerPromoteDate==""){
		Ext.Msg.alert('提示',"请选择晋升日期！");
		return;
	}else{
		if(PerPromoteDate instanceof Date){
			PerPromoteDate=PerPromoteDate.format('Y-m-d');
		}
	}
	var PerWorkUnit=Ext.getCmp("PerWorkUnit").getValue();//任职单位
	if(PerWorkUnit==""){Ext.Msg.alert('提示',"请填写任职单位！");return;}
	
	var parr="raw|"+RowId+"^PerName|"+PerName+"^PerID|"+PerID+"^PerDutyName|"+PerDutyName+"^PerPromoteDate|"+PerPromoteDate+"^PerWorkUnit|"+PerWorkUnit;
	//alert(parr);
