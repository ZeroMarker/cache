...
DateItem[0] = "Item13";
TimeItem[0] = "Item91";
var prncode = "DHCNURPGD_TNBBRWJZHLCS";

function formatDate(value) { alert(88);
  try {
		...
		if(Ext.getCmp('Item'+i)){
			Ext.getCmp('Item'+i).on('focus',function(e){
				...
				if(itm.value==''){
					...
					itm.blur();
					if(selFlag==0){
						Ext.getCmp('Item12').setValue(date.dateFormat('Y-m-d'));
						Ext.getCmp('Item13').setValue(date.dateFormat('H:i'));
						selFlag=1;
					}
				}else{
					itm.setValue('');
...
function Save() {
	//by lmm  增加年份保存
	var recDate = Ext.getCmp("Item12")    //评估日期Item
	var recYear="";
	if(recDate.getValue()!=""){
		recYear=formatDate(recDate.getValue()).split("-")[0]
	}
	var recYearObj = Ext.getCmp("Item41")  //新家的评估年份Item 元素不要超过345
...
function getPatInfo() {
  ...
  if (PatInfo) {
	...
	var signature = Ext.getCmp("Item38");
