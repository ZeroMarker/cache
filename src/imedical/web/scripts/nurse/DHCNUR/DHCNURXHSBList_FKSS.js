...
function modccevent()
{
	...
	if (objRow.length == 0) {
	  ...
	  //alert(EpisodeID)
		var lnk= "DHCNurEmrComm.csp?"+"&EmrCode=DHCNURXH_FKSS2&EpisodeID="+EpisodeID+"&NurRecId="+par ;//"&DtId="+DtId+"&ExamId="+ExamId
	    window.open(lnk,"htm",'left=50,toolbar=no,location=no,directories=no,resizable=yes,width=1250,height=800');
...
function newEvent()
{
    ...
    if (logonht==1)
	{
		var lnk= "DHCNurEmrComm.csp?"+"&EmrCode=DHCNURXH_FKSS2&EpisodeID="+EpisodeID  ;//"&DtId="+DtId+"&ExamId="+ExamId
	}
	
	if (logonht==0)
	{
		//var lnk= "DHCNurEmrComm.csp?"+"&EmrCode=DHCNURXH_FKSS&EpisodeID="+EpisodeID  ;//"&DtId="+DtId+"&ExamId="+ExamId
	}
	window.open(lnk,"htm",'left=50,toolbar=no,location=no,directories=no,resizable=yes,width=1250,height=1000');
...
function find()
{
	...
	//alert(StDate.vlaue);
	var parr = "DHCNURXH_FKSS2^"+adm+"^"+loc+"^"+StDate.value+"^"+Enddate.value;
  //alert(parr);
