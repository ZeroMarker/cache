...
function modccevent()
{
	...
	if (objRow.length == 0) {
		...
		if (Status=="已审核") 
		{var Status="A"}
		var lnk= "DHCNurEmrComm.csp?"+"&EmrCode=DHCNURXHSB_glhuatuo&EpisodeID="+EpisodeID+"&NurRecId="+par+"&Status="+Status  ;//"&DtId="+DtId+"&ExamId="+ExamId
...
function newEvent()
{
    ...
    if (logonht==1)
	{
		var lnk= "DHCNurEmrComm.csp?"+"&EmrCode=DHCNURXHSB_glhuatuo&EpisodeID="+EpisodeID  ;//"&DtId="+DtId+"&ExamId="+ExamId
	}
...
function find()
{
   ...
   var IfCancelRec=Ext.getCmp("IfCancelRec").getValue();
	//alert(StDate.vlaue);
	var parr = "DHCNURXHSB_glhuatuo^"+adm+"^"+loc+"^"+StDate.value+"^"+Enddate.value+"^"+IfCancelRec;
  //	alert(parr);
