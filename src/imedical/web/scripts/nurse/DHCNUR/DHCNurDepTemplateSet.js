...
var gitemData = new Array();

var loc = session['LOGON.CTLOCID'];
cspRunServerMethod(getloc, 'addloc');
var objGetRootName = document.getElementById('GetRootName');
if (objGetRootName) {
	cspRunServerMethod(objGetRootName.value, '', 'addGitem');
}

//根下拉
var GArr
function getgname(seloc) {
	GArr = tkMakeServerCall("NurMp.TemplateMenu", "GetGName", seloc)
	// alert(GArr)
	if (GArr == "") {
		GArr = []
	}
...
var storeGitem = new Ext.data.JsonStore({
	data: gitemData,
	fields: ['id', 'desc']
});
...
var modelgrid = new Ext.grid.EditorGridPanel({
	...
	store: new Ext.data.JsonStore({
		data: [],
		fields: ['GUID', 'ModelKey', 'ModelDesc', { name: 'IfShow', type: 'bool' }, { name: 'IfAllShow', type: 'bool' }, { name: 'IfSelect', type: 'bool' }, 'ModelSort', 'ModelName', 'GItem']
	}),
			...
			editor: new Ext.grid.GridEditor(
				new Ext.form.ComboBox({
					...
					listeners: {
						...
						'beforequery': function (e) {
							var combo = e.combo;
							combo.store.baseParams.loc = LOC;
						}
					},
					// mode: 'local'
				}))
...
//弹出根目录   
function MainMenu() {
	var lnk = "DHCNurEmrComm.csp?" + "&EmrCode=DHCNURTemplateMenu&EpisodeID=" + LOC;//"&DtId="+DtId+"&ExamId="+ExamId
	//alert(lnk)
...
function MainMenuSub() {
	var lnk = "DHCNurEmrComm.csp?" + "&EmrCode=DHCNURTemplateMenuSub&EpisodeID=" + LOC;//"&DtId="+DtId+"&ExamId="+ExamId
	//alert(lnk)
...
function BodyLoadHandler() {
   ...
   var len = grid1.getColumnModel().getColumnCount();
	for(var i = 0 ;i < len;i++){
		if(grid1.getColumnModel().getDataIndex(i) == 'GUID'){
...
function waterafterEditFn(e) {
	...
	if (curfield == "GItem") {
		//alert(curvalue)	
		var code = select.get("GUID")
		var name = select.get("ModelDesc")
		var gname = tkMakeServerCall("NurMp.TemplateMenu", "getnamebyid", curvalue)  //根目录名称
		var parr = curvalue + "^" + code + "^" + LOC + "^" + name + "^" + gname;

		var a = tkMakeServerCall("NurMp.TemplateMenuSub", "Additm", parr) //保存
		//alert(a)
		if (LOC == "") {
...
function loaddepmodelset(locId) {
	...
	if (GetDepModelSet) {
		tkMakeServerCall("NurMp.TemplateMenuSub", "GetDepModelSet2014", locId, "adddepmodel")
	}
	...
	mygrid1.getStore().on('load', function (s, records) {
		...
		s.each(function (r) {
			...
			if (r.get("IfAllShow")) {
				...
				grid1.getView().getCell(girdcount, 4).style.backgroundColor = "#FF0000"
			}

			if (r.get("IfShow")) {
				//grid1.getView().getRow(girdcount).style.backgroundColor='#F7FE2E';
				grid1.getView().getCell(girdcount, 3).style.backgroundColor = "#B0C4DE"
			}
			if (r.get("IfSelect")) {
				//grid1.getView().getRow(girdcount).style.backgroundColor='#F7FE2E';
				grid1.getView().getCell(girdcount, 5).style.backgroundColor = "#66CD00"
			}
			else {
...
//普通
function adddepmodel(a1, a2, a3, a4, a5, a6, a7, a8, a9) {
	REC.push({
		ModelKey: a1,
		ModelDesc: a2,
		IfShow: a3,
...
function save() {
	...
	for (var i = 0; i < store.getCount(); i++) {
		...
		if (IfShow == 1) {
			var ModelKey = store.getAt(i).get("GUID");
