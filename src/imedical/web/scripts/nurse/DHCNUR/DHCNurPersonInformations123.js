	...
	var winUpload = new Ext.Window({
		...
		buttons : [{
			...
			handler : function() {
				...
				form.getForm().submit({
						success:function(form,action){setTimeout(function(){Ext.MessageBox.hide();winUpload.close();},5000);winUpload.close();}
				})	
				*/		
			//
				...
				var filepath=newSrc;
				//alert(webIP.substring(7,webIP.length))
				var tmpArr=findFtpServer();
				//var ftpServerIP=parseXML()+PersonID+"/";
				var ftpServerIP="ftp://"+tmpArr["server"]+":"+tmpArr["portID"]+"/ftploadimg/"+PersonID+"/";
				/*
					arr["server"]=server;
					arr["userName"]=userName;
					arr["password"]=password;
				...
				*/
				...
				var loginname=tmpArr["userName"]
				//var loginpassword=getFtpPassWord();
				var loginpassword=tmpArr["password"]
				//alert("filepath="+filepath+",ftpServerIP="+ftpServerIP+",savename="+savename+",loginname="+loginname+",loginpassword="+loginpassword)
				var mn=FtpLoadimg(filepath,ftpServerIP,savename,loginname,loginpassword);
				alert(mn)
				//var mn=FtpLoadimg(filepath,ftpServerIP,savename);
				alert(mn);
				if(mn=="图片上传成功！"){
					var box1=Ext.getCmp("imgBox");					
					//var imgsrc=savepath+savename+".jpg";
					//var imgsrc=ftpServerIP+savename;
					var imgsrc="ftp://"+loginname+":"+loginpassword+"@"+tmpArr["server"]+":"+tmpArr["portID"]+"/ftploadimg/"+PersonID+"/"+savename;
					//alert(imgsrc);
					box1.getEl().dom.src=imgsrc;
					//alert(imgsrc);
					//alert(box1.getEl().dom.src);
					winUpload.close();
					Ext.getCmp("UpLoadImg").disable();
				}else if(mn=="图片上传失败！"){
					winUpload.close();
				}
				}
