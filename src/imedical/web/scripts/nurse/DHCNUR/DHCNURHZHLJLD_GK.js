...
if (GetNurseRecSet){
	var ret=cspRunServerMethod(GetNurseRecSet.value,"DHCNURHZHLJLD_GK");
	var hh=ret.split("^");
   	...
   	tobar.addButton(
		  {
			...
			text: "打印",
			handler:function(){
				printNurCareRec("DHCNurPrnMouldXH1")
...
function PrintSet()
{
 ...
 //debugger;
   ...
   // debugger;
    REC=new Array();
 	var a=cspRunServerMethod(GetNurPrnSet.value,"DHCNURHZHLJLD_GK",EpisodeID,'addprnset');
...
function find(){
  ...
  // alert(parent.frames.length);
	...
	var parr = adm + "^" + StDate.value + "^" + StTime.value + "^" + Enddate.value + "^" + EndTime.value + "^"+"DHCNURHZHLJLD_GK" + "^" + IfCancelRec+"^"+CurrHeadDr;
	// debugger;
...
{  //查询出入液量合计
	...
	var mygrid=Ext.getCmp("mygrid");
  //alert(adm+"|"+StDate.value+"|"+StTime.value+"|"+Enddate.value+"|"+EndTime.value+"|"+"DHCNURHZHLJLD_GK")
 	var a=cspRunServerMethod(GeInOutAmount.value,adm,StDate.value,StTime.value,Enddate.value,EndTime.value,"DHCNURHZHLJLD_GK");
	...
	for (var i = 0; i < list.length; i++) {
	  ...
	  var flag="0";
		...
		if ((str!="")&&(flag=="0"))
		{
			...
			//alert(str);
			var a=cspRunServerMethod(RecSave.value,EpisodeID,str,session['LOGON.USERID'],"DHCNURHZHLJLD_GK",session['LOGON.GROUPDESC']);
			if (a!="0")
			{
				alert(a);
				return;
...
function MultiFun()
{
	...
	var ret= cspRunServerMethod(GetMulti.value, "DHCNURHZHLJLD_GK");
	...
	var tabstr=cspRunServerMethod(getcheckform.value, "DHCNURHZHLJLD_GK",ab);
...
function multiSel(ret)
{
    ...
    var itname=grid1.getSelectionModel().getSelections()[0].get("itm4"); 
	  var getcheckform=document.getElementById('getcheckform');
		alert(itname+"!!"+code)
		var ret=cspRunServerMethod(getcheckform.value, "DHCNURHZHLJLD_GK",itname,code,"");	
...
{ //小结
	...
	else
	{
	  ...
	  else
	  {
		...
		str=str+"^DiagnosDr|"+DiagnosDr+"^HeadDR|"+CurrHeadDr;
		//alert(str);
		var a=cspRunServerMethod(SaveOutIn.value,EpisodeID,str,session['LOGON.USERID'],"DHCNURHZHLJLD_GK");
		find();
	}
}
function InOutSum()
...
function Measure()
{
   ...
   // debugger;
	...
	var parr="DHCNURHZHLJLD_GK^"+EpisodeID+"^CaseMeasureXml^"+rowid;
	//alert(parr);
	var arr = new Array();
	var a = cspRunServerMethod(pdata1, "", "DHCPatOrdList", EpisodeID, "");
	arr = eval(a);
...
function sureMeasure()
{
   ...
   // var parr="DHCNURHZHLJLD_GK^"+EpisodeID+"^CaseMeasureXml";
...
function printNurRec()
{
		...
		//alert("ddd");
		//debugger;
		var a=cspRunServerMethod(GetPrnSet.value,"DHCNURHZHLJLD_GK",EpisodeID); //page, caredattim, prnpos, adm,Typ,user
		if (a=="") return;
		var GetLableRec=document.getElementById('GetLableRec');
		var LabHead=cspRunServerMethod(GetLableRec.value,"DHCNURHZHLJLD_GK^"+session['LOGON.CTLOCID']+"^"+EpisodeID+"^"+session['LOGON.USERID']);
		//alert("DHCNURHZHLJLD_GK^"+session['LOGON.CTLOCID']+"^"+EpisodeID+"^"+session['LOGON.USERID'])
		var tm=a.split("^");
		var stdate="" //tm[2];
		var stim="" //tm[3];
		var edate="" //tm[4];
		...
		//debugger;
		var parr=EpisodeID+"!"+stdate+"!"+stim+"!"+edate+"!"+etim+"!DHCNURHZHLJLD_GK!"+CurrHeadDr;
		PrintComm.ID = "";
		PrintComm.MultID = "";
		...
		//alert(pages+","+CareDateTim+","+stprintpos+","+EpisodeID+","+"DHCNURHZHLJLD_GK"+","+session['LOGON.USERID']+","+PrintComm.PrnFlag);
		//PrnFlag==1说明是打印预览
		if (PrintComm.PrnFlag==1) return;
		//如果原记录保存打印到第8页则当打印第8页之前页时不保存打印记录
		if (pages<aa[0]) return;
		var a=cspRunServerMethod(SavePrnSet.value,pages,CareDateTim,stprintpos,EpisodeID,"DHCNURHZHLJLD_GK",session['LOGON.USERID']); //page, caredattim, prnpos, adm,Typ,user
		//find();
}
