...
function BodyLoadHandler() {
	...
	but.setText("保存");
	but.on('click', save);
	/*Ext.getCmp("mygrid").addListener("rowclick", function(grid, num, e) {
		curSelectRow=num;
	});
	Ext.getCmp("mygrid").on('bodyscroll',function(){
		//this.store.commitChanges();
		var rowObj=this.getSelectionModel().getSelections();
		if(rowObj.length==0)return;
		var storobj=rowObj[rowObj.length-1];
		this.store.removeAt(curSelectRow);
		this.store.insert(curSelectRow,storobj);
		this.store.commitChanges();
	});*/
...
function SureIn() {
	...
	if (selModel.hasSelection()) {
		...
		Ext.each(selections, function (item) {
			...
			var userDefineml=item.data.exeuse;
			var unit = item.data.DoseUnit;
			if ((unit != "ml") && (unit != "ML")&&(userDefineml!=""))
				ml = userDefineml;
...
function printNurRecZK() {
	...
	PrintCommPic.TitleStr = ret;
	PrintCommPic.LFootCaption = hh[2];
	PrintCommPic.SetPreView("1"); //打印预览
	PrintCommPic.dxflag = 2;
