...
{
	var mygridpl=Ext.getCmp('mygrid3pl');
...
function selectDate(df,date)
{
	if(df.id=="GRPersonDob")
	{
		...
		var stdate=Ext.getCmp('GRPersonStDate').getValue();
		var enddate=Ext.getCmp('GRPersonEndDate').getValue()
		if(enddate!=""&&stdate!=""&&stdate>enddate)
		{		
			alert("开始日期不能小于结束日期！")
			Ext.getCmp('GRPersonEndDate').setValue(stdate)
		}	
	}
}
function CreateGrid()
{
	...
	tobar.addItem('-','开始日期:',new Ext.form.DateField({id:'GRStDate',value:new Date(),format:'Y-m-d'}));
	tobar.addItem('-','结束日期:',new Ext.form.DateField({id:'GREndDate',value:new Date(),format:'Y-m-d'}));
	tobar.addItem('-');tobar.addButton({id:'grfindBtn',icon:'../Image/icons/find.png',text:'查询',handler:findRec});
	tobar.addItem('-');tobar.addButton({id:'graddBtn',icon:'../Image/icons/add.png',text:'新建',handler:addRec});
	tobar.addItem('-');tobar.addButton({id:'grupdateBtn',icon:'../Image/icons/cog_edit.png',text:'修改',handler:updateRec});
	//tobar.addItem('-');tobar.addButton({id:'grprintBtn',icon:'../Image/icons/printer.png',text:'打印',handler:printRec});
	tobar.addItem('-');tobar.addButton({id:'grstatusBtn',icon:'../Image/icons/delete.png',text:'作废',handler:statusRec});
	tobar.doLayout();
...
{
	var a=cspRunServerMethod(pdata1,"","DHCNURGRPERSON","","");
	var arr = eval(a);
...
function findRec()
{
	...
	var StDate=Ext.getCmp('GRStDate').getValue().format('Y-m-d');
	var EndDate=Ext.getCmp('GREndDate').getValue().format('Y-m-d');
	var parr=StDate+'^'+EndDate+'^'+"W";
...
function addRec()
{
	...
	win.show();
	Ext.getCmp('GRPersonDob').on('select',selectDate);
	Ext.getCmp('GRPersonStDate').on('select',selectDate);
	Ext.getCmp('GRPersonEndDate').on('select',selectDate);
	if(session['LOGON.GROUPDESC']=='住院护士长'||session['LOGON.GROUPDESC'].indexOf('护士长')>0)
	{
		var PersonWard=Ext.getCmp("GRPersonWard");
		PersonWard.store.load({params:{start:0,limit:1000},callback:function(){PersonWard.setValue(session['LOGON.CTLOCID']);}});
		PersonWard.disable();
	}
	var closeBtn=Ext.getCmp('grcloseBtn');
	if(closeBtn!=null)
	{
		closeBtn.on('click',function(){win.close();});
	}
	var clearBtn=Ext.getCmp('grclearBtn');
	if(clearBtn!=null)
	{
		clearBtn.on('click',clearRec);
	}
	var saveBtn=Ext.getCmp('grsaveBtn');
	if(saveBtn!=null)
	{
		saveBtn.on('click',function()
		{
			...
			if(parr!=false)
			{
				var PersonID=Ext.getCmp('GRPersonID').getValue();
...
function setValue(ID)
{
	...
	var PersonStDate=Ext.getCmp('GRPersonStDate');
	if(PersonStDate!=null) PersonStDate.setValue(array['PersonStDate']);
	var PersonEndDate=Ext.getCmp('GRPersonEndDate');
	if(PersonEndDate!=null) PersonEndDate.setValue(array['PersonEndDate']);
	var PersonSex=Ext.getCmp('GRPersonSex');
	if(PersonSex!=null) PersonSex.setValue(array['PersonSex']);
	var PersonComeDate=Ext.getCmp('GRPersonComeDate');
	if(PersonComeDate!=null) PersonComeDate.setValue(array['PersonComeDate']);
	var PersonWorkDate=Ext.getCmp('GRPersonWorkDate');
	if(PersonWorkDate!=null) PersonWorkDate.setValue(array['PersonWorkDate']);
	var PersonType2=Ext.getCmp('GRPersonType');
	if(PersonType2!=null) PersonType2.setValue(array['PersonType2']);
}
function statusRec()
{
	var mygrid=Ext.getCmp('mygrid3');
...
function clearRec()
{
	...
	var PersonType2=Ext.getCmp('GRPersonType');
	if(PersonType2!=null) PersonType2.setValue("");
}
