...
var prnmode ="DHCNURMouldPrn_YBHLJLDDY"
var prnmcodes = ""
if (prnmode != "") {
...
function BodyLoadHandler() {
	...
	if (UserType == "NURSE") {
		...
		document.querySelector("#ext-gen4").style.height = (document.body.clientHeight - 300) + "px";
		//alert(document.querySelector("#ext-gen4").style.height)
		document.querySelector("#ext-gen5").style.height = (document.body.clientHeight - 317) + "px";
		//alert(document.querySelector("#ext-gen5").style.height)
		document.querySelector("#ext-gen8").style.height = (document.body.clientHeight - 317) + "px";
		//alert(document.querySelector("#ext-gen8").style.height)
		document.querySelector("#ext-gen13").style.height = (document.body.clientHeight - 317) + "px"; 
		//alert(document.querySelector("#ext-gen13").style.height)
		document.querySelector("#ext-gen14").style.font = "15px";
		}
...
function SepcialChar() {
	...
	var a = tkMakeServerCall("web.DHCNurseRecordComm","GetQueryData1", "User.DHCTEMPSPECIALCHAR:CRItemNew", "", "AddSpecChar")
...
function SpecCharFn20190128(flag) {
	var grid = Ext.getCmp('mygridspecchar');
	//弹出界面中Grid
	var selModel = grid.getSelectionModel();
	if (selModel.hasSelection()) {
...
function printNurRecZK() {
	...
	if(selobj){
		PrintCommPic.selRowId=selobj.get("par")+"||"+selobj.get("rw");
	}
	//alert(PrintCommPic.selRowId)
	//end
	//PrintCommPic.MakeInfo = "Y";    // by lmm 奇偶页打印生成打印记录 --增加
	PrintCommPic.firstShowFlag = 0; 
	PrintCommPic.PrintOut();
	//by lmm 奇偶页打印 --增加
    //PrintCommPic.SetPreView("1");  //1 预览 0 不预览
    //PrintCommPic.xuflag = "0";     //1-启用续打 ；0-全部打印   
    //PrintComm.CLeft = "Y";      //Y-偶数页左边距是奇数页右边距 N-奇数和偶数页左边距相等           
    //PrintCommPic.PrintOutAll();       //生成打印记录    
