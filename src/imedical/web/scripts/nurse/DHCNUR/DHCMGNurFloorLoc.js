...
//",Ward,Execute,Drug Injection,Dispensing,Cashier,Other,Operating Theatre,Emergency,Day Surgery,Medical Records,OutPatient Consulting Room,Clinic,Admission Point",
//MAXLEN = 26, TRUNCATE = 1, VALUELIST = ",W,E,DI,D,C,O,OP,EM,DS,MR,OR,CL,ADM") [ SqlColumnNumber = 27, SqlFieldName = CTLOC_Type ];
var loctyp2 = new Ext.form.ComboBox({
	name : 'LocTyp2',
	id : 'LocTyp2',
	tabIndex : '0',
	x : 0,
...
function BodyLoadHandler() {
	...
	myGridPl.setPosition(480,0);
	myGridPl.setHeight(Height);
	myGridPl.setWidth(480);
...
//	but.on('click', menudelPos);
	tobar.addButton({
 	...
 	var cmb1=Ext.getCmp("LocTyp2");
 	cmb1.on("select",setgrid2);
 	//cmb.setValue("ZYL");
	// tobar.render(grid.tbar);
	var mygrid=Ext.getCmp('mygrid');
	var bbar1 = mygrid.getBottomToolbar ();
	bbar1.hide();
	var bmbar= new Ext.PagingToolbar({
		pageSize:30,
		store:mygrid.store,
		displayInfo:true,
	...
	for(var i = 0 ;i < len;i++){
		if(mygrid.getColumnModel().getDataIndex(i) == 'LocDr'){
...
function setgrid2() 
{
	var mygrid = Ext.getCmp("mygrid");
	mygrid.store.on("beforeload",function(){
  	var loctyp=Ext.getCmp("LocTyp2").getValue();
    var hsdr=1;
    if (loctyp=="") return;
    mygrid.store.baseParams.HsDr=hsdr;
    mygrid.store.baseParams.typ=loctyp;
...
function QualSubWin() 
{
	...
	stdata.on("beforeLoad",function(){
		stdata.baseParams.Par=curmenuid;
	})
