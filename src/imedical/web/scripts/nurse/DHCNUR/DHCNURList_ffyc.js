...
//debugger;
 ...
 //生成图片
	...
	if(id)
	{
			...
			var prncode="DHCNURPRNFFYC";
			var MthArr="Nur.DHCMoudData:getVal$parr:"+id+"!";	
			//是否生成图片 Y 生成，N 不生成
			tkMakeServerCall("NurEmr.DHCNurEmrPrintComm","SetEmrTempGlobal","MakeTemp","DHCNURXHFFYC","Y");
			//是否生成图片 Y 生成，N 不生成
			tkMakeServerCall("NurEmr.DHCNurEmrPrintComm","SetEmrTempGlobal","MakePic","DHCNURXHFFYC","Y");
			//是否开始生成图片
			tkMakeServerCall("NurEmr.DHCNurEmrPrintComm","SetEmrTempGlobal","StartMakePic","DHCNURXHFFYC","Y");
			//生成的图片是否上传ftp
			tkMakeServerCall("NurEmr.DHCNurEmrPrintComm","SetEmrTempGlobal","IfUpload","DHCNURXHFFYC","Y");
			//是否生成所有页 ，默认是否(从原来最后一页开始生成)；"Y"--从第一页开始生成
			tkMakeServerCall("NurEmr.DHCNurEmrPrintComm","SetEmrTempGlobal","MakeAllPages","DHCNURXHFFYC","Y");
			var EmrType=3; 
			var link=WebIp+"/dhcmg/PrintComm/PrintCommPic.application?method=PrintOut&EpisodeID="+EpisodeID+"&EmrCode="+"DHCNURXHFFYC"+"&EmrType="+EmrType+"&ItmName="+prncode+"&MthArr="+MthArr+"&WebUrl="+WebUrl;
			window.location.href=link;
		
			var link=WebIp+"/dhcmg/PrintComm/PrintCommPic.application?method=MakePicture&EpisodeID="+EpisodeID+"&EmrCode="+"DHCNURXHFFYC"+"&EmrType="+EmrType+"&ItmName="+prncode+"&MthArr="+MthArr+"&WebUrl="+WebUrl;
			window.location.href=link;
		   alert("生成成功！")
...
function butPrintFn()
{
  var mygrid = Ext.getCmp("mygrid");
	...
	if(id)
	{
		...
		//是否生成图片 Y 生成，N 不生成
		tkMakeServerCall("NurEmr.DHCNurEmrPrintComm","SetEmrTempGlobal","MakeTemp","DHCNURXHFFYC","N");
		//是否生成图片 Y 生成，N 不生成
		tkMakeServerCall("NurEmr.DHCNurEmrPrintComm","SetEmrTempGlobal","MakePic","DHCNURXHFFYC","N");
		//是否开始生成图片
		tkMakeServerCall("NurEmr.DHCNurEmrPrintComm","SetEmrTempGlobal","StartMakePic","DHCNURXHFFYC","N");
		//生成的图片是否上传ftp
		tkMakeServerCall("NurEmr.DHCNurEmrPrintComm","SetEmrTempGlobal","IfUpload","DHCNURXHFFYC","N");
		//是否生成所有页 ，默认是否(从原来最后一页开始生成)；"Y"--从第一页开始生成
		tkMakeServerCall("NurEmr.DHCNurEmrPrintComm","SetEmrTempGlobal","MakeAllPages","DHCNURXHFFYC","N");
	
		var GetPGDId=tkMakeServerCall("Nur.DHCMoudData","GetId","DHCNURXHFFYC",EpisodeID);
		if(NurRecId!=="") GetPGDId=NurRecId;
		var WebUrl=WebIp+"/imedical/web/DWR.DoctorRound.cls";
		var parr="@"+EpisodeID+"@"+"DHCNURXHFFYC";
		var EmrType=3;  //1:混合单 2：记录单 3：评估单
		//var MthArr="web.DHCNurMouldDataComm:GetPrnValComm&parr:"+parr;
		var MthArr="Nur.DHCMoudData:getVal$parr:"+id+"!";	
		var link=WebIp+"/dhcmg/PrintComm/PrintCommPic.application?method=PrintOut&EpisodeID="+EpisodeID+"&EmrCode="+"DHCNURXHFFYC"+"&EmrType="+EmrType+"&ItmName="+"DHCNURPRNFFYC"+"&MthArr="+MthArr+"&WebUrl="+WebUrl;
		window.location.href=link;
		}
}
function qdtj()
...
function modccevent()
{
	...
	if (objRow.length == 0) {
	  ...
	  //alert(EpisodeID)
		var lnk= "DHCNurEmrComm.csp?"+"&EmrCode=DHCNURXHFFYC&EpisodeID="+EpisodeID+"&NurRecId="+par ;//"&DtId="+DtId+"&ExamId="+ExamId
...
function newEvent()
{
    ...
    if (logonht==1)
	{
		var lnk= "DHCNurEmrComm.csp?"+"&EmrCode=DHCNURXHFFYC&EpisodeID="+EpisodeID  ;//"&DtId="+DtId+"&ExamId="+ExamId
	}
	...
	window.open(lnk,"htm",'left=460,top=10,toolbar=no,location=no,directories=no,resizable=yes,width=1000,height=700');
...
function find()
{
	...
	//alert(StDate.vlaue);
	//var parr = "DHCNURXHFFYC^"+adm+"^"+loc+"^"+StDate.value+"^"+Enddate.value+"^"+IfCncelRec;
		var parr = "DHCNURXHFFYC^"+adm+"^"+loc+"^"+"^"+"^"+IfCncelRec;
  //alert(parr);
