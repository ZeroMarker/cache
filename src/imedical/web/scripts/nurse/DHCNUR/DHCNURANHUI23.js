...
var ITypItm="Item10"; //后取数据字典型
var ITypItm1="Item22"; //后取数据字典型
var ITypItm2="Item26"; //后取数据字典型
var ITypItm3="Item34"; //后取数据字典型
...
{	        
			...
			PrintComm.ItmName = "DHCNurPrnMouldANHUI24";
			//debugger;
			//var parr="@"+EpisodeID+"@DHCNURHYD_Assessment@DHCNURHYD_DisSummary";
			var parr="@"+EpisodeID+"@DHCNURANHUI23";
			//alert(parr)
...
function printNurRec(){		
	...
	var id=cspRunServerMethod(getid.value,EmrCode,EpisodeID);		
	//debugger;		
	var a=cspRunServerMethod(GetPrnSet.value,"DHCNURANHUI23",EpisodeID); 
...
function BodyLoadHandler(){
  ...
  but.on('click',function(){butPGDPrintFn("DHCNurPrnMouldANHUI24")}); 
	
	var Item2 = Ext.getCmp("Item10");
	Item2.on('specialkey',cmbkey);
	
	var Item3 = Ext.getCmp("Item22");
	Item3.on('specialkey',cmbkey);
	
	var Item4 = Ext.getCmp("Item26");
	Item4.on('specialkey',cmbkey);
	
	var Item5 = Ext.getCmp("Item34");
	Item5.on('specialkey',cmbkey);

	var score1 = Ext.getCmp("Item11_1");
  score1.on('check',scoreAdd,this);
  var score1 = Ext.getCmp("Item11_2");
  score1.on('check',scoreAdd,this); 
  var score1 = Ext.getCmp("Item11_3");
  score1.on('check',scoreAdd,this);
  var score1 = Ext.getCmp("Item11_4");
  score1.on('check',scoreAdd,this);

  
  var score1 = Ext.getCmp("Item12_1");
  score1.on('check',scoreAdd,this);
  var score1 = Ext.getCmp("Item12_2");
  score1.on('check',scoreAdd,this); 
  var score1 = Ext.getCmp("Item12_3");
  score1.on('check',scoreAdd,this);
  var score1 = Ext.getCmp("Item12_4");
  score1.on('check',scoreAdd,this);
  ...
  score1.on('check',scoreAdd,this);
  var score1 = Ext.getCmp("Item14_5");
  score1.on('check',scoreAdd,this);
  var score1 = Ext.getCmp("Item14_6");
  score1.on('check',scoreAdd,this);  
  ...
  score1.on('check',scoreAdd,this); 
  var score1 = Ext.getCmp("Item16_5");
  score1.on('check',scoreAdd,this);
  var score1 = Ext.getCmp("Item16_6");
  score1.on('check',scoreAdd,this);
  var score1 = Ext.getCmp("Item16_7");
  score1.on('check',scoreAdd,this);
...
function scoreAdd(obj)
{
	...
	{
		 Item11Value=scoreFormat(scorefen);
	}
	if (scoreid.indexOf("Item12")!="-1")
	{
		Item12Value=scoreFormat(scorefen);
	}
	if (scoreid.indexOf("Item13")!="-1")
	{
		Item13Value=scoreFormat(scorefen);
	}
	if (scoreid.indexOf("Item14")!="-1")
	{
		Item14Value=scoreFormat(scorefen);
	}
	if (scoreid.indexOf("Item15")!="-1")
	{
		Item15Value=scoreFormat(scorefen);
	}
	if (scoreid.indexOf("Item16")!="-1")
	{
		Item16Value=scoreFormat(scorefen);
	}
	if (scoreid.indexOf("Item17")!="-1")
	{
		Item17Value=scoreFormat(scorefen);
	}
  if (scoreid.indexOf("Item18")!="-1")
	{
  ...
  var HJScore = Ext.getCmp("Item19");
  
  HJScore.setValue(parseInt(Item11Value)+parseInt(Item12Value)+parseInt(Item13Value)+parseInt(Item14Value)+parseInt(Item15Value)+parseInt(Item16Value)+parseInt(Item17Value)+parseInt(Item18Value));
}
   ...
   gform.items.each(eachItem, this);  
	...
	for (var i=0 ; i<ht.keys().length;i++)//for...in statement get all of Array's index
	{
		...
		if (key.indexOf("_") == -1) 
		{
			...
			var flag2=ifflag2(key);
			var flag3=ifflag3(key);
...
function getPatInfo()
{
    ...
    var ReportDate = Ext.getCmp("Item24");  
	 	...
	 	var TBDate = Ext.getCmp("Item8");
	 	TBDate.setValue(nowDate);  //填报日期
	  
	  var PJDate = Ext.getCmp("Item33");
	 	PJDate.setValue(nowDate);  //评价日期
	 	
	  /* var ReportTime = Ext.getCmp("Item25");
