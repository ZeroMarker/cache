...
var linkCode = "DHCNURPGD_DNGBGZZ";
var prncode="DHCNURMouldPrn_DNGBGZZMRPGB";
...
function butPrintFn() {
  ...
  var recId=tkMakeServerCall("Nur.DHCMoudData","getLinkSubIdToZY",subId,EpisodeID,linkCode);     
...
function CancelRecord() {
	...
	if (objRow.length == 0) {
		...
		if (flag) {
		    ...
		    var a=tkMakeServerCall("Nur.DHCMoudDataSub","CancelRecord", par, rw, session['LOGON.USERID'], session['LOGON.GROUPDESC']);
			if (a != 0) {
					alert(a);
					return;
				} else {
...
function Cancelzf() {
	...
	if (objRow.length == 0) {
		...
		if (flag) {
			...
			var rw = objRow[0].get("chl");
			//alert(par+","+rw+","+session['LOGON.USERID']+","+session['LOGON.GROUPDESC']);
			var a=tkMakeServerCall("Nur.DHCMoudDataSub","Cancelzf", par, rw, session['LOGON.USERID'], session['LOGON.GROUPDESC']);
			if (a != 0) {
					alert(a);
					return;
				} else {
...
function find() {
	...
	//alert(EpisodeID);
	//alert(parr);
	var a = cspRunServerMethod(GetQueryData.value, "web.DHCNurMouldDataComm:MoudDataSort", "parr$" + parr, "AddRec");
	//alert(a);
...
function newrec() {
	...
	//window.open(lnk, "htm", 'toolbar=no,location=no,directories=no,resizable=yes,top='+topValue+',left='+leftValue+',top=50,width=820,height=820');
