...
var linkCode = "DHCNURPGD_SSHZZYJJJLD0705NEW";
var prncode="DHCNURMouldPrn_SSHZJJJLDDYNEW";
...
function modccevent()
{
	...
	if (objRow.length == 0) {
	  ...
	  //alert(EpisodeID)
		...
		window.open(lnk, "htm", 'toolbar=no,location=no,directories=no,resizable=yes,top=50,width=820,height=820');
		return;
		var lnk= "DHCNurEmrComm.csp?"+"&EmrCode=DHCNURBAOAN_ZKBLJJBNEW&EpisodeID="+EpisodeID+"&NurRecId="+par ;//"&DtId="+DtId+"&ExamId="+ExamId
	    window.open(lnk,"htm",'toolbar=no,location=no,directories=no,resizable=yes,width=1000,height=600');
...
function newEvent()
{
	...
	window.open(lnk, "htm", 'toolbar=no,location=no,directories=no,resizable=yes,top='+topValue+',left='+leftValue+',width=820,height=720');
	//alert(1);
	//alert(logonht)
	return;
	...
	if (logonht==1)
	{	
	var lnk= "DHCNurEmrComm.csp?"+"&EmrCode=DHCNURBAOAN_ZKBLJJBNEW&EpisodeID="+EpisodeID+"&TransFlag="+TransFlag  ;//"&DtId="+DtId+"&ExamId="+ExamId
	}
	if (logonht==0)
	{
		var lnk= "DHCNurEmrComm.csp?"+"&EmrCode=DHCNURBAOAN_ZKBLJJBNEW&EpisodeID="+EpisodeID+"&TransFlag="+TransFlag   ;//"&DtId="+DtId+"&ExamId="+ExamId
     ...
     alert(2)	
    var topValue=(window.screen.height-width)/6;
	var leftValue=(window.screen.width-height)/2;
window.open(lnk,"htm","toolbar=no,location=no,directories=no,resizable=yes,scrollbars=yes,scrolling=yes,top="+topValue+",left="+leftValue+",width="+width+",height="+height);
...
function find()
{
 ...
 var IfCncelRec=Ext.getCmp("IfCancelRec").getValue();
	...
	//alert(StDate.vlaue);
	var parr = "DHCNURPGD_SSHZZYJJJLD0705NEW^"+adm+"^"+loc+"^"+"^"+"^"+IfCncelRec;
