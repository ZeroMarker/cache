...
function BodyLoadHandler() {
  ...
  //initEvents();    //加载总分功能
  setvalue();
  var NurseSing = Ext.getCmp("Item30")
  NurseSing.disable();
		...
		if(Ext.getCmp('Item'+i)){
			Ext.getCmp('Item'+i).on('focus',function(e){
				...
				if(itm.value==''){
					...
					itm.blur();
					if(selFlag==0){
						Ext.getCmp('Item31').setValue(date.dateFormat('Y-m-d'));
						Ext.getCmp('Item32').setValue(date.dateFormat('H:i'));
						selFlag=1;
					}
				}else{
					itm.setValue('');
...
function Save() {
	//by lmm  增加年份保存
	var recDate = Ext.getCmp("Item31")    //评估日期Item
	var recYear="";
	if(recDate.getValue()!=""){
  ...
  comboret = "";
  radioret = "";
   var SaveTime = document.getElementById('Item32');
   var Time=new Date();
   SaveTime.value=Time;
