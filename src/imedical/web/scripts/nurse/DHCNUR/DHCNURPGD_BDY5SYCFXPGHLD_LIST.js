...
var linkCode = "DHCNURPGD_BDY5SYCFXPGHLD";
var prncode="DHCNURMouldPrn_BDY5SYCFXPGHLD";
function gethead() {
...
function butPrintFn() {
  ...
  //alert(subId)
  var recId=tkMakeServerCall("Nur.DHCMoudData","getLinkSubIdToZYPaging",subId,EpisodeID,linkCode);  
...
function BodyLoadHandler() {
	...
	tobar.addButton({
		...
		handler: AddPagingFlag,
		id: 'PrintAddPagingFlag'
	});
	tobar.addButton({
		className: 'new-topic-button',
		text: "删除分页标记",
		icon: '../images/uiimages/edit_remove.png',
		handler: CancelPagingFlag,
		id: 'PrintCancelPagingFlag'
	});
	/*var but2 = Ext.getCmp("mygridbut3");
	but2.setText("增加分页标记");
	but2.on('click', AddPadingFlag);
	var but3 = Ext.getCmp("mygridbut4");
	but3.setText("删除分页标记");
	but3.on('click', CancelPadingFlag);
	/*but.setText("增加分页标记");
	but.on('click', AddPadingFlag);

	but.setText("删除分页标记");
	but.on('click', CancelPadingFlag);*/
...
function AddPagingFlag() {
	...
	if (objRow.length == 0) {
		...
		var flag = confirm("你确定要由此条记录添加标记吗？")
		if (flag) {
			var par = objRow[0].get("rw");
			var rw = objRow[0].get("chl");
			console.log(par+"^"+rw+"^"+session['LOGON.USERID']);
			var a = tkMakeServerCall("Nur.DHCNurFormPaging","SaveFormPagingFlag", par, rw, session['LOGON.USERID']);
			//alert(a);
			if (a!= 0) {
					alert(a);
					return;
...
function CancelPagingFlag() {
	...
	if (objRow.length == 0) {
		...
		var flag = confirm("你确定要删除此条记录标记吗？")
		if (flag) {
			var par = objRow[0].get("rw");
			var rw = objRow[0].get("chl");
			console.log(par+"^"+rw+"^"+session['LOGON.USERID']);
			var a = tkMakeServerCall("Nur.DHCNurFormPaging","ChacelFormPagingFlag", par, rw, session['LOGON.USERID']);
			//alert(a);
			if (a!= 0) {
					alert(a);
					return;
...
function newrec() {
	...
	//var topValue=(window.screen.height-820)/6;
	//var leftValue=(window.screen.width-820)/2;
	var widthValue=820;
	var heightValue=820;
	...
	//window.open(lnk, "htm", 'toolbar=no,location=no,directories=no,resizable=yes,top='+topValue+',left='+leftValue+',width=820,height=820');
