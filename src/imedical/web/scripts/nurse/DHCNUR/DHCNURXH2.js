...
function gethead()
{
  ...
  var StartLock="1" //续打全院开关：1:启用续打，0:不启用续打
 ...
 function BodyLoadHandler(){
    ...
    var sm2 = new Ext.grid.RowSelectionModel({   
  	...
  	grid.on('rowdblclick',MeasureNew);
...
{
	var retval=""
  var obj=self.parent.frames["eastbl"].window.frames["frameBrowseCategory"]
  if (obj)
  {
	  retval=obj.selectedContent()
  }
  //alert(retval)
	if (retval!="")
	{
		InserTextComm(retval);
//		var selgrid=Ext.getCmp("mygrid")
//		//alert(selgrid.store)
//		var seletval=selgrid.store.getAt(grid.rowIndex).get(grid.getColumnModel().getDataIndex(grid.columnIndex));
//		//alert(seletval)
//		if (seletval==undefined)
//		{
//			seletval=""
//	  }
//		if(seletval!="")
//		{
//			retval=seletval+retval
//		}
//		selgrid.store.getAt(grid.rowIndex).set(grid.getColumnModel().getDataIndex(grid.columnIndex),retval);
	}
	else
	{
	  alert("没有选中电子病历内容!");
...
function beforeEditFn(e){
	...
	selobj=textarea1;
	textarea.addListener('blur',function(e){
		//e.complete()
		//alert("textarea:"+textarea1.getValue())
		//textarea1.completeEdit()
		//debugger;
	})
   //textarea.on('blur',getRangeData2) 
   var el = Ext.get(e.grid.getView().getCell(e.row, e.column)); //获取列元素 
   var ed = e.grid.colModel.getCellEditor(e.column, e.row); //获取编辑器
   //alert(textarea.growMin+"^"+el.getHeight())   
   //ed.setSize(el.getWidth(),el.getHeight()); //设置编辑器大小 
   textarea.setSize(el.getWidth(),el.getHeight()); //设置编辑器大小 
}
function getRangeData2()
{
	alert(selobj.getValue())
			if (selobj)
				{
					//alert(grid.activeEditor.id)
					selobj.completeEdit();
				}
	...
	for (var i = 0; i < list.length; i++) {
      ...
      var isModifiedflag=""
		for (var p in obj) {
			...
			{
			   var rw=obj["rw"];
			   if ((store.getAt(i).isModified(p))||(rw==undefined))
	           {
	      	      isModifiedflag="Y";
	           }
	        }else
...
function SetXu()
{
  if ((StartLock!="1")||(prnmode=="")) //没有开启续打
  {
	...
	select.setValue(formatDate1(CareDate,format)+"/"+(CareTime)+"/"+User)
...
{  
    ...
    var emrknowurl="http://127.0.0.1/dhcmg/KnowLedge/DesignForm.application?method=GetXmlData&EpisodeID="+EpisodeID+"&EmrCode="+"DHCNurKnowldge"+"&Parr="+parr+"&LogLoc="+session['LOGON.CTLOCID']+"&Ward="+session['LOGON.CTLOCID']+"&DepId="+session['LOGON.CTLOCID']+"&webIp="+"http://127.0.0.1";	
...
function MeasureNew()
{
    ...
    //var emrknowurl="dhcnuremrknow.csp?EmrCode=DHCNurKnowldge&Parr="+parr+"&EpisodeID="+EpisodeID; 
    var emrknowurl="http://127.0.0.1/dhcmg/KnowLedge/DesignForm.application?method=GetXmlData&EpisodeID="+EpisodeID+"&EmrCode="+"DHCNurKnowldge"+"&Parr="+parr+"&LogLoc="+session['LOGON.CTLOCID']+"&Ward="+session['LOGON.CTLOCID']+"&DepId="+session['LOGON.CTLOCID']+"&webIp="+"http://127.0.0.1"+"&logUser="+session['LOGON.USERID'];	
		...
		for (var i = 0; i < str.length; i++) {
			...
			if (code >= 65281 && code <= 65373) {
				...
				if (code == 12288) {
					...
					if (code==94) //^
					{
						val=String.fromCharCode(65342); //转成全角"^"
