...
function BodyLoadHandler() {
  ...
  tmpItm.disable();
  var tmpItm = Ext.getCmp("Item59")
  tmpItm.disable();
  var tmpItm = Ext.getCmp("Item73")
  tmpItm.disable();
  var tmpItm = Ext.getCmp("Item74")
  tmpItm.disable();
  var tmpItm = Ext.getCmp("Item75")
  tmpItm.disable();
  var tmpItm = Ext.getCmp("Item76")
  tmpItm.disable();
...
function Save() {
  ///by lmm  增加年份保存
  var recDate = Ext.getCmp("Item10")    //括号item数值要改成评估日期的item数值
  var recYear="";
  if(recDate.getValue()!=""){
  recYear=formatDate(recDate.getValue()).split("-")[0]
  }
  var recYearObj = Ext.getCmp("Item77")   //新加的评估年份Item 元素不要超过345
...
function getPatInfo() {
  ...
  if (PatInfo) {
	...
	var Nursesignature = Ext.getCmp("Item59");
