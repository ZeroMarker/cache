...
//debugger;
 ...
 //生成图片
	...
	if(id)
	{
		var prncode="DHCNURPRNDDZC";
		var GetPGDId=tkMakeServerCall("Nur.DHCMoudData","GetId","DHCNURXHFFDD",EpisodeID);
		if(id)
		{   
			var WebUrl=WebIp+"/dthealth/web/DWR.DoctorRound.cls";
			var MthArr="Nur.DHCMoudData:getVal$parr:"+id+"!";	
			//是否生成图片 Y 生成，N 不生成
			tkMakeServerCall("NurEmr.DHCNurEmrPrintComm","SetEmrTempGlobal","MakeTemp","DHCNURXHFFDD","Y");
			//是否生成图片 Y 生成，N 不生成
			tkMakeServerCall("NurEmr.DHCNurEmrPrintComm","SetEmrTempGlobal","MakePic","DHCNURXHFFDD","Y");
			//是否开始生成图片
			tkMakeServerCall("NurEmr.DHCNurEmrPrintComm","SetEmrTempGlobal","StartMakePic","DHCNURXHFFDD","Y");
			//生成的图片是否上传ftp
			tkMakeServerCall("NurEmr.DHCNurEmrPrintComm","SetEmrTempGlobal","IfUpload","DHCNURXHFFDD","Y");
			//是否生成所有页 ，默认是否(从原来最后一页开始生成)；"Y"--从第一页开始生成
			tkMakeServerCall("NurEmr.DHCNurEmrPrintComm","SetEmrTempGlobal","MakeAllPages","DHCNURXHFFDD","Y");
			var EmrType=3; 
			var link=WebIp+"/dhcmg/PrintComm/PrintCommPic.application?method=PrintOut&EpisodeID="+EpisodeID+"&EmrCode="+"DHCNURXHFFDD"+"&EmrType="+EmrType+"&ItmName="+prncode+"&MthArr="+MthArr;
			window.location.href=link;
		
			var link=WebIp+"/dhcmg/PrintComm/PrintCommPic.application?method=MakePicture&EpisodeID="+EpisodeID+"&EmrCode="+"DHCNURXHFFDD"+"&EmrType="+EmrType+"&ItmName="+prncode+"&MthArr="+MthArr+"&WebUrl="+WebUrl;
			window.location.href=link;
	}	
		 // alert("成功")
		}
...
function butPrintFn()
{
  var mygrid = Ext.getCmp("mygrid");
	...
	if(id)
	{
		...
		//是否生成图片 Y 生成，N 不生成
		tkMakeServerCall("NurEmr.DHCNurEmrPrintComm","SetEmrTempGlobal","MakeTemp","DHCNURXHFFDD","N");
		//是否生成图片 Y 生成，N 不生成
		tkMakeServerCall("NurEmr.DHCNurEmrPrintComm","SetEmrTempGlobal","MakePic","DHCNURXHFFDD","N");
		//是否开始生成图片
		tkMakeServerCall("NurEmr.DHCNurEmrPrintComm","SetEmrTempGlobal","StartMakePic","DHCNURXHFFDD","N");
		//生成的图片是否上传ftp
		tkMakeServerCall("NurEmr.DHCNurEmrPrintComm","SetEmrTempGlobal","IfUpload","DHCNURXHFFDD","N");
		//是否生成所有页 ，默认是否(从原来最后一页开始生成)；"Y"--从第一页开始生成
		tkMakeServerCall("NurEmr.DHCNurEmrPrintComm","SetEmrTempGlobal","MakeAllPages","DHCNURXHFFDD","N");
		var WebUrl=WebIp+"/dthealth/web/DWR.DoctorRound.cls";
		var GetPGDId=tkMakeServerCall("Nur.DHCMoudData","GetId","DHCNURXHFFDD",EpisodeID);
		if(NurRecId!=="") GetPGDId=NurRecId;
		var parr="@"+EpisodeID+"@"+"DHCNURXHFFDD";
		var EmrType=3;  //1:混合单 2：记录单 3：评估单
		//var MthArr="web.DHCNurMouldDataComm:GetPrnValComm&parr:"+parr;
		var MthArr="Nur.DHCMoudData:getVal$parr:"+id+"!";	
		var link=WebIp+"/dhcmg/PrintComm/PrintCommPic.application?method=PrintOut&EpisodeID="+EpisodeID+"&EmrCode="+"DHCNURXHFFDD"+"&EmrType="+EmrType+"&ItmName="+"DHCNURPRNDDZC"+"&MthArr="+MthArr+"&WebUrl="+WebUrl;
		window.location.href=link;
...
function modccevent()
{
	...
	if (objRow.length == 0) {
	  ...
	  //alert(EpisodeID)
		var lnk= "DHCNurEmrComm.csp?"+"&EmrCode=DHCNURXHFFDD&EpisodeID="+EpisodeID+"&NurRecId="+par ;//"&DtId="+DtId+"&ExamId="+ExamId
	    window.open(lnk,"htm",'toolbar=no,location=no,directories=no,resizable=yes,left=200,width=1000,height=800');
...
function newEvent()
{
    ...
    if (logonht==1)
	{
		var lnk= "DHCNurEmrComm.csp?"+"&EmrCode=DHCNURXHFFDD&EpisodeID="+EpisodeID  ;//"&DtId="+DtId+"&ExamId="+ExamId
	}
...
function find()
{
 ...
 var IfCncelRec=Ext.getCmp("IfCancelRec").getValue();
	...
	//alert(StDate.vlaue);
	var parr = "DHCNURXHFFDD^"+adm+"^"+loc+"^"+"^"+"^"+IfCncelRec;
