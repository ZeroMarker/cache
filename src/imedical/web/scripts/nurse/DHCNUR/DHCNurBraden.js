 ...
 if(EpisodeID!="")
  ...
  // var id=cspRunServerMethod(getid.value,EmrCode,EpisodeID);
   ...
   if (id!= "") {
   	...
   	var ret = cspRunServerMethod(getVal.value, EpisodeID);
   	var tm = ret.split('^')
		sethashvalue(ha, tm);
			getPatInfo();	 
			}
...
function Save()
{
 ...
 //alert(NurRecId)
 var Id=cspRunServerMethod(SaveRec.value,"^EmrUser|"+session['LOGON.USERID']+"^EmrCode|"+EmrCode+"^"+ret+"^DetLocDr|"+session['LOGON.CTLOCID']+"^AnaesId|"+NurRecId+"^EpisodeId|"+EpisodeID+"&"+checkret+"&"+comboret);
 
 var flag9=1;
 //alert("保存成功");
 //window.opener.find();
 window.close();
 window.opener.Braden();
