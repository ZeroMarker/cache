...
if (GetNurseRecSet){
	var ret=cspRunServerMethod(GetNurseRecSet.value,"DHCNURXH12");
	var hh=ret.split("^");
   	...
   	tobar.addButton(
		  {
			//className: 'new-topic-button',
			text: "打印",
			handler:function(){printNurCareRec("DHCNurPrnMouldXH12")},
			id:'PrintBut'
		  }
	  );
	  	...
	  	tobar.addButton(
		  {
			//className: 'new-topic-button',
			text: "生成所有图片",
			handler:function(){MakeRecPicture("DHCNurPrnMouldXH12")},
			id:'MakePicture1'
		  }
	  );
 	//tobar.addButton(
...
{
		PrintCommPic.StartMakePic="Y"
		printNurRec()
		PrintCommPic.EmrCode=EmrCode
		PrintCommPic.EpisodeID=EpisodeID
		PrintCommPic.curPages=0;
		PrintCommPic.MakeTemp="Y";
		PrintCommPic.filepath=WebIp+"/DHCMG/HLBLMakePictureSet.xml"		
		//PrintCommPic.MakeAllPages="Y";
		PrintCommPic.MakePicture();
		alert("成功")
...
function PrintSet()
{
 ...
 //debugger;
   ...
   // debugger;
    REC=new Array();
 	var a=cspRunServerMethod(GetNurPrnSet.value,"DHCNURXH12",EpisodeID,'addprnset');
...
{	
	...
	//alert(Timepoint24)
  var aa=tkMakeServerCall("NurEmr.webheadchange","GetInOutAmountCommAndInsert","Nod",EpisodeID,StDate,StTime2,Enddate,EndTime2,EmrCode,session['LOGON.USERID'],session['LOGON.CTLOCID'],CurrHeadDr,"false")
  find()
}
...
{	
	...
	//alert(Timepoint24)
  var aa=tkMakeServerCall("NurEmr.webheadchange","GetInOutAmountCommAndInsert","Nod",EpisodeID,StDate,StTime,Enddate,EndTime,EmrCode,session['LOGON.USERID'],session['LOGON.CTLOCID'],CurrHeadDr,"false")
  find()
}
...
{	
  ...
  var aa=tkMakeServerCall("NurEmr.webheadchange","GetInOutAmountCommAndInsert","Sum",EpisodeID,StDate,StTime,Enddate,EndTime,EmrCode,session['LOGON.USERID'],session['LOGON.CTLOCID'],CurrHeadDr,"false")
  find()
  //alert(aa)
  return
       ...
       {   var alertstr="体温单中"+EndTime+"点以下项目值有修改："+flag+"你确定要更新这些值吗"
           Ext.Msg.show({    
	              ...
	              fn:  function(btn, text){    
	                 if (btn == 'ok'){   	                 	 	                    
	                    //alert(parr)              
	                    var id=tkMakeServerCall("NurEmr.webheadchange","hlblSave",EpisodeID,aa,session['LOGON.USERID'],"DHCNUR6",session['LOGON.GROUPDESC'],Enddate,EndTime);
                      alert("完成")
	                 }
					        else
	                {   
...
function find(){
  ...
  // alert(parent.frames.length);
	...
	var parr = adm + "^" + StDate.value + "^" + StTime.value + "^" + Enddate.value + "^" + EndTime.value + "^"+"DHCNURXH12" + "^" + IfCancelRec+"^"+CurrHeadDr;
	// debugger;
...
{  //查询出入液量合计
	...
	var mygrid=Ext.getCmp("mygrid");
  //alert(adm+"|"+StDate.value+"|"+StTime.value+"|"+Enddate.value+"|"+EndTime.value+"|"+"DHCNURXH12")
 	var a=cspRunServerMethod(GeInOutAmount.value,adm,StDate.value,StTime.value,Enddate.value,EndTime.value,"DHCNURXH12");
	...
	for (var i = 0; i < list.length; i++) {
	  ...
	  var flag="0";
		...
		if ((str!="")&&(flag=="0"))
		{
			...
			//alert(str);
			var a=cspRunServerMethod(RecSave.value,EpisodeID,str,session['LOGON.USERID'],"DHCNURXH12",session['LOGON.GROUPDESC']);
			if (a!="0")
			{
				alert(a);
				return;
...
function MultiFun()
{
	...
	var ret= cspRunServerMethod(GetMulti.value, "DHCNURXH12");
	...
	var tabstr=cspRunServerMethod(getcheckform.value, "DHCNURXH12",ab);
...
function multiSel(ret)
{
    ...
    var itname=grid1.getSelectionModel().getSelections()[0].get("itm4"); 
	  var getcheckform=document.getElementById('getcheckform');
		alert(itname+"!!"+code)
		var ret=cspRunServerMethod(getcheckform.value, "DHCNURXH12",itname,code,"");	
...
{ //小结
	...
	else
	{
	  ...
	  else
	  {
		...
		str=str+"^DiagnosDr|"+DiagnosDr+"^HeadDR|"+CurrHeadDr;
		//alert(str);
		var a=cspRunServerMethod(SaveOutIn.value,EpisodeID,str,session['LOGON.USERID'],"DHCNURXH12");
		find();
	}
}
function InOutSum()
...
function Measure()
{
   ...
   // debugger;
	...
	var parr="DHCNURXH12^"+EpisodeID+"^CaseMeasureXml^"+rowid;
	//alert(parr);
	var arr = new Array();
	var a = cspRunServerMethod(pdata1, "", "DHCPatOrdList", EpisodeID, "");
	arr = eval(a);
...
function sureMeasure()
{
   ...
   // var parr="DHCNURXH12^"+EpisodeID+"^CaseMeasureXml";
...
function printNurRec()
{
		...
		//alert("ddd");
		//debugger;
		var a=cspRunServerMethod(GetPrnSet.value,"DHCNURXH12",EpisodeID); //page, caredattim, prnpos, adm,Typ,user
		if (a=="") return;
		var GetLableRec=document.getElementById('GetLableRec');
		var LabHead=cspRunServerMethod(GetLableRec.value,"DHCNURXH12^"+session['LOGON.CTLOCID']+"^"+EpisodeID+"^"+session['LOGON.USERID']);
		//alert("DHCNURXH12^"+session['LOGON.CTLOCID']+"^"+EpisodeID+"^"+session['LOGON.USERID'])
		var tm=a.split("^");
		var stdate="" //tm[2];
		var stim="" //tm[3];
		var edate="" //tm[4];
    ...
    PrintCommPic.RFootCaption = "411111";
		...
		//debugger;
		var parr=EpisodeID+"!"+stdate+"!"+stim+"!"+edate+"!"+etim+"!DHCNURXH12!"+CurrHeadDr;
		//alert(parr)
		PrintCommPic.ID = "";
		PrintCommPic.MultID = "";
