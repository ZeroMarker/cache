...
//	grid.colModel.setHidden(13,true);
	//增加颜色控制
	grid.getColumnModel( ).setRenderer(11,function(value){
			if(value=="待处理"){
	...
	for(var i=0;i<len;i++){
		if(grid.getColumnModel().getDataIndex(i)=="Opinion"){
...
//	tobar.addItem("-","检查病区",WardLoc);
  ...
  //if((session['LOGON.GROUPDESC']!="护理部")&&(session['LOGON.GROUPDESC']!="护理部主任")&&(session['LOGON.GROUPDESC']!="Demo Group")){
	if((secGrpFlag!="hlb")&&(secGrpFlag!="hlbzr")&&(secGrpFlag!="demo")){
		comboload1(WardLoc,session['LOGON.CTLOCID']);
		WardLoc.disable()
	}
  var mygrid = Ext.getCmp("mygrid");
	mygrid.store.on("beforeload",function(){
  	...
  	if(!eddate){
 	 	...
 	 	//if((session['LOGON.GROUPDESC']!="护理部")&&(session['LOGON.GROUPDESC']!="护理部主任")&&(session['LOGON.GROUPDESC']!="Demo Group")){
		if((secGrpFlag!="hlb")&&(secGrpFlag!="hlbzr")&&(secGrpFlag!="demo")){
			var ward=session['LOGON.CTLOCID'];
	 	}else{
	...
	//setsize("mygridpl", "gform", "mygrid",20);
	SchQual();
	var len = mygrid.getColumnModel().getColumnCount();
	for(var i = 0 ;i < len;i++){
		if(mygrid.getColumnModel().getDataIndex(i) == 'CheckContent'){
...
function Check(Qreason,Par,CheckTyp,stateTyp){
	...
	but1.on('click',function(){Save(Par,CheckTyp),window1.close(),SchQual()});
  ...
  if(ret!="")
  {
    	...
    	//Ext.getCmp("statType").setValue(arr[11])
    }
...
function Save(checkid,CheckTyp)
{
	//alert(Ext.getCmp("Question").getValue())
	...
	//var parr=Reasona+"^"+Methoda+"^"+Goala+"^"+Praisea+"^"+Opiniona
