(function($) {
		...
		$.fn.extend({
			...
			getExcelColumNum: function() {
				var headTds = $(this).find(".thead td");
				var num = headTds.length - 1;
				if (num < 0) {
					num = 0
				}
				return num;
			},
			getExcelRowNum: function() {
				var headTds = $(this).find("tr");
				var num = headTds.length - 1;
				if (num < 0) {
					num = 0
				}
		...
		function initTable(t, setting) {
			...
			//生成table
				...
				table = $("<table border='1'></table>").appendTo(t);
		...
		function valueChange(e) {
			...
			if (selectTd) {
				selectTd.html(val);
				var isAllLetter = /^[0-9a-zA-Z]+$/.test(selectTd.text());
				var isLetterAndNum = /^[a-z0-9]+$/i.test(selectTd.text());
				var isLetterFirst = /^[a-z]+$/i.test(selectTd.text().substr(0, 1));
				if (isAllLetter && isLetterAndNum && isLetterFirst) {
					selectTd.attr('data-id', selectTd.text());
				} else {
					selectTd.removeAttr("data-id");
					if (!selectTd.hasClass('itemName')) {
						selectTd.html('');
					}
				}
			}
		...
		function tdMousedown(e, table) {
			...
			$(e).addClass('selectTd');
			$('.cell-width').val(selectTd.width());
			$('.cell-height').val(selectTd.height());
		}
		...
		function getChosenList(table, begPosi, endPosi) {
			if (begPosi != undefined && endPosi != undefined) {
				for (var i = (begPosi.row > endPosi.row ? endPosi.row : begPosi.row); i <= (begPosi.row > endPosi.row ? begPosi.row :
						endPosi.row); i++) {
					var tr = table.find("tr:eq(" + i + ")");
					for (var j = (begPosi.col > endPosi.col ? endPosi.col : begPosi.col); j <= (begPosi.col > endPosi.col ? begPosi.col :
							endPosi.col); j++) {
					...
					coll.each(function() {
						...
						if (!$(this).is(fir)) {
							...
							if ($(this).attr("rowspan") != undefined && $(this).attr("colspan") != undefined) {
								r = (p.row + (Number($(this).attr("rowspan")) - 1) - posi.row) > r ? (p.row + (Number($(this).attr("rowspan")) -
									1) - posi.row) : r;
								c = (p.col + (Number($(this).attr("colspan")) - 1) - posi.col) > c ? (p.col + (Number($(this).attr("colspan")) -
									1) - posi.col) : c
							}
						}
					});
		...
		function drugCell(table, t) {
			...
			table.find("tr").first().find("td").each(function() {
				...
				colWidthPanelItem.attr("draggable", true).mousedown(function(e) {
						...
						clearDurgEle(table, t)
					}).css("transform", colTransform).css("left", left + this.offsetWidth - 4).css("height", firstTr[0].offsetHeight)
					.appendTo(colWidthPanel)
			});
			table.find("tr").each(function() {
				...
				rowHeightPanelItem.attr("draggable", true).mousedown(function(e) {
					...
					clearDurgEle(table, t);
				}).css("top", top + this.offsetHeight - 4).css("width", firstTr.find("td")[0].offsetWidth).appendTo(
					rowHeightPanel)
			});
		...
		function addRowCol(table, type, t) {//type:0上插一行;1下插一行;2左插一列;3右插一列;4删除行;5删除列
		...
		function addRowColSpan(list, ty) {
			...
			if (ty == 0 || ty == 1 || ty == 4) {//type:0上插一行;1下插一行;4删除行
				var tr = list;
				var tyIndex = 0;
				var tr0MulColRowAdd1 = 0;
				tr.find("td").each(function() {
					ty1RowSpanArr[tyIndex] = 0;
					if ($(this).is(":hidden")) {
						...
						if (con && p != null) {
							...
							if(Number(p.attr("colspan")) > 1){
								tr0MulColRowAdd1 = 1	
							}
						}
						if(ty == 1 && p != null && Number(p.attr("rowspan")) > 1){//下插一行
							ty1RowSpanArr[tyIndex] = 1;
						}else if(ty == 0 && p != null && Number(p.attr("rowspan")) > 1){//上插一行
							var pColSpan = Number(p.attr("colspan"))
							if(pColSpan <= 1){//多行单列合并
								ty1RowSpanArr[tyIndex] = 1;
							}else if(tr0MulColRowAdd1){//多行多列合并
								ty1RowSpanArr[tyIndex] = 1;
							}
						}
					} else {
						...
						if ($(this).attr("rowspan") && $(this).attr("colspan")) {
							...
							if (ty == 4) {//删除行
								...
								if (nextTr.length == 1 && Number($(this).attr("rowspan")) > 1) {
									...
									nextTr.find("td:eq(" + ind + ")").attr("rowspan", spanNum($(this).attr("rowspan"), -1)).attr("colspan", $(
										this).attr("colspan")).css("display", "").addClass('itemName');//增加itemName
								}
							} else if (ty == 0) {//上插一行
								$(this).attr("rowspan", Number($(this).attr("rowspan")))
							} else if (ty == 1) {//下插一行
								if(Number($(this).attr("rowspan")) > 1){
						...
						if ($(this).attr("rowspan") && $(this).attr("colspan")) {
							...
							if (ty == 5) {
								...
								if (nextTd.length == 1 && Number($(this).attr("colspan")) > 1) {
									nextTd.width($(this).width()).attr("rowspan", $(this).attr("rowspan")).attr("colspan", spanNum($(this).attr(
										"colspan"), -1)).css("display", "")
								}
							} else {
		...
		function setSelectBorder(table, width, height, top, left) {
			...
			$("<div class='chosen-area-p'></div>").width(1).height(height + 1).css("margin-top", top - 1).css("margin-left",
				left - 1).insertBefore(table);
			$("<div class='chosen-area-p'></div>").width(width + 1).height(1).css("margin-top", top - 1).css("margin-left", left -
				1).insertBefore(table);
			$("<div class='chosen-area-p'></div>").width(1).height(height).css("margin-top", top - 1).css("margin-left", left +
				width - 1).insertBefore(table);
			$("<div class='chosen-area-p'></div>").width(width).height(1).css("margin-top", top + height - 1).css("margin-left",
				left - 1).insertBefore(table);
			$("<div class='chosen-area-p chosen-area-p-drug'></div>").mousedown(function() {
				...
				}).width(3).height(3).css("padding", "2px").css("margin-top", top + height - 4).css("margin-left", left + width - 4)
				.insertBefore(table)
		}
		...
		function drawDrugArea(table) {
			...
			table.find("tr").find("td:eq(0)").each(function() {
				$(this).width(60).addClass("drug-ele-td drug-ele-td-vertical").css("text-align", "center").html(ind === 0 ? "" :
					ind);
				ind++
			});
		}
		...
		function showRightPanel(table, t, e) {
			...
			var posX = e.clientX;
			var posY = e.clientY;
			// 500是大约的 大菜单的 高度 修改这个数可以提高菜单的最低top
			if (e.clientY + 500 > $(window).height()) {
				var overHeight = $(window).height() - e.clientY - 500;
				posY = e.clientY + overHeight;
			}
			// 350是左边项目表格宽度 修改这个数可以提高菜单的最低left
			if (e.clientX + 350 > $(window).width()) {
				var overWidth = $(window).width() - e.clientX - 350;
				posX = e.clientX + overWidth;
			}
			var rightMousePanel = $("<div class='rightmouse-panel-div'></div>").css("left", posX + 15).css("top", posY + 10).insertBefore(
				table);
			...
			// luzhiqiang
			$("<div class='wb setItem'></div>").html(
				"<span class='excel-rightmomuse-icon-css'><i class='fa fa-check'></i></span><span class='excel-rightmomuse-text-css'>设置为项目单元格</span><span class='excel-rightmomuse-icon-css excel-rightmomuse-icon-next-css'></span>"
			).appendTo(leftPanel);
			$("<div class='wb setFixed'></div>").html(
				"<span class='excel-rightmomuse-icon-css'><i class='fa fa-pencil-square-o'></i></span><span class='excel-rightmomuse-text-css'>设为固定</span><span class='excel-rightmomuse-icon-css excel-rightmomuse-icon-next-css'></span>"
			).appendTo(leftPanel);
			$("<div class='hr'></div>").appendTo(leftPanel);
			$("<div class='wb removeItem'></div>").html(
				"<span class='excel-rightmomuse-icon-css'><i class='fa fa-remove'></i></span><span class='excel-rightmomuse-text-css'>取消项目单元格</span><span class='excel-rightmomuse-icon-css excel-rightmomuse-icon-next-css'></span>"
			).appendTo(leftPanel);
			$("<div class='wb removeFixed'></div>").html(
				"<span class='excel-rightmomuse-icon-css'><i class='fa fa-remove'></i></span><span class='excel-rightmomuse-text-css'>取消固定</span><span class='excel-rightmomuse-icon-css excel-rightmomuse-icon-next-css'></span>"
			).appendTo(leftPanel);
			$("<div class='hr'></div>").appendTo(leftPanel);
			...
			$("<div class='wb fuzhidanyuange'></div>").html(
				"<span class='excel-rightmomuse-icon-css'><i class='fa fa-columns'></i></span><span class='excel-rightmomuse-text-css'>复制单元格样式</span>"
			).appendTo(leftPanel);
			$("<div class='wb zhantiedanyuange'></div>").html(
				"<span class='excel-rightmomuse-icon-css'><i class='fa fa-columns'></i></span><span class='excel-rightmomuse-text-css'>粘贴单元格样式</span>"
			).appendTo(leftPanel);
			$("<div class='hr'></div>").appendTo(leftPanel);
			$("<div class='wb clearFontSize'></div>").html(
				"<span class='excel-rightmomuse-icon-css'><i class='fa fa-trash-o'></i></span><span class='excel-rightmomuse-text-css'>初始化字体大小</span>"
			).appendTo(leftPanel);
			$("<div class='wb clearColor'></div>").html(
				"<span class='excel-rightmomuse-icon-css'><i class='fa fa-trash-o'></i></span><span class='excel-rightmomuse-text-css'>初始化字体颜色</span>"
			).appendTo(leftPanel);
			$("<div class='wb clearbackground'></div>").html(
				"<span class='excel-rightmomuse-icon-css'><i class='fa fa-trash-o'></i></span><span class='excel-rightmomuse-text-css'>初始化背景颜色</span>"
			).appendTo(leftPanel);
			$("<div class='hr'></div>").appendTo(leftPanel);
			...
			var setting = $("<div class='wb setting'></div>").html(
				"<div class='setting-item'><span class='excel-rightmomuse-icon-css'><i class='fa fa-pencil-square'></i></span><span class='excel-rightmomuse-text-css'>单元格宽度</span><span class='setting-input'><input type='number' name='width' title='单元格宽度'/></span></div><div class='setting-item'><span class='excel-rightmomuse-icon-css'><i class='fa fa-vimeo-square'></i></span><span class='excel-rightmomuse-text-css'>设置行</span><span class='setting-input'><input type='number' max='30' min='1' name='row'/></span></div><div class='setting-item'><span class='excel-rightmomuse-icon-css'><i class='fa fa-h-square'></i></span><span class='excel-rightmomuse-text-css'>设置列</span><span class='setting-input'><input type='number' max='50' min='1' name='col'/></span></div>"
			).appendTo(leftPanel);
			...
			rightMousePanel.find(".wb").click(function() {
				...
				if (!obj.hasClass("duiqifangsi") && !obj.hasClass("setting")) {
					...
					if (obj.hasClass("setItem")) {
						if (!coll.hasClass('itemName')) {
							coll.addClass('itemName') //luzhiqiang
						} else {
							alert('已经是项目单元格 (蓝色背景的单元格)');
						}
					}
					if (obj.hasClass("removeItem")) {
						if (coll.hasClass('itemName')) {
							coll.removeClass('itemName') //luzhiqiang
							if (coll.hasClass('fixed')) {
								coll.removeClass('fixed')
								coll.removeClass('iconfont');
								coll.removeClass('iconpin');
							}
						} else {
							alert('此单元格不是项目格，无法取消')
						}
					}
					if (obj.hasClass("setFixed")) {
						if (!coll.hasClass('itemName')) {
							alert('非项目单元格不能固定（请先设置成项目单元格)');
						} else {
							coll.addClass('fixed');
							coll.addClass('iconfont');
							coll.addClass('iconpin');

						}
					}
					if (obj.hasClass("removeFixed")) {
						if (coll.hasClass('fixed')) {
							coll.removeClass('iconfont');
							coll.removeClass('iconpin');
							coll.removeClass('fixed') //luzhiqiang
						} else {
							alert('未固定，无法取消')
						}
					}
					...
					if (obj.hasClass("clearColor")) {
						clearColor(table)
					}
					if(obj.hasClass("clearFontSize")){
						clearFontSize(table)
					}
					if (obj.hasClass("clearbackground")) {
					...
					//当前选中的单元格
					var rowSpan = $(selectTd).attr("rowspan")
					if (obj.hasClass("xiachayihang")) {
						if(rowSpan > 1){
							$.messager.popover({msg: '请选择没有行合并的单元格！',type:'alert'});
							return;	
						}
						addRowCol(table, 1, t)
					}
					...
					if (obj.hasClass("shangchayihang") || obj.hasClass("xiachayihang") || obj.hasClass("zuochayilie") || obj.hasClass(
							"youchayilie") || obj.hasClass("shanchuhang") || obj.hasClass("shanchulie")) {
						drugCell(table, t)
					}
      ...
      var classStr = $(selectTd).attr('class')
			var styleStr = $(selectTd).attr('style')
			selectTdStyle['class'] = classStr
			selectTdStyle['style'] = styleStr
      return
		...
		function pastedanyuange() {
			$(selectTd).attr('class',selectTdStyle['class'])
			$(selectTd).attr('style',selectTdStyle['style'])
			return
		...
		function clearFontSize(){
			if (selectTd) {
				selectTd.css('font-size', '');
			}
		}
		
		function clearBackground() {
			if (selectTd) {
				selectTd.css('background-color', '');
			}
		}
		...
		function setFontFamily() {
			...
			var selectList = document.querySelectorAll('.td-chosen-css')
			for(var i = 0; i < selectList.length;i++){
				selectList[i].style.fontFamily = fontFamily
			}
		}
      ...
      //selectTd.css('font-size', fontSize);
			var selectList = document.querySelectorAll('.td-chosen-css')
			for(var i = 0; i < selectList.length;i++){
				selectList[i].style.fontSize = fontSize
			}
		}
		...
		function setFontBold() {
			...
			var selectList = document.querySelectorAll('.td-chosen-css')
			for(var i = 0; i < selectList.length;i++){
				selectList[i].style.fontWeight = destWeight
			}
		}
		...
		function setFontItalic() {
			...
			var selectList = document.querySelectorAll('.td-chosen-css')
			for(var i = 0; i < selectList.length;i++){
				selectList[i].style.fontStyle = destStyle
			}
		}
		...
		function setValign(event) {
			...
			$('.btn-av').removeClass('buttonBgColor');
			$(this).addClass("buttonBgColor");
			selectTd.css("vertical-align", event.data);
		}
		...
		function setBorderLeft() {
			...
			let rowSpanNum = 0;
			selectTd.parent().each(function(index) {
		...
		function setBorderTop() {
			...
			//let coll = $('table').first().find('.td-chosen-css');
			selectTd.parent().first().children('.td-chosen-css:visible').css('border-top', getBorderCssStr())
		}
		...
		function setBorderBottom() {
			...
			//let coll = $('table').first().find('.td-chosen-css');
			let i = 0;
			selectTd.parent().last().children('.td-chosen-css').each(function() {
		...
		function setBorderAll() {
			...
			let style = $('.borderStyleOption option:selected').val();
			if (style === 'none') {
				selectTd.css('border', '#ccc 1px solid')
			} else {
				selectTd.css('border', getBorderCssStr())
			}
		}
