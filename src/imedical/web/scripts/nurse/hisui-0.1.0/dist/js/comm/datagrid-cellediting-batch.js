(function($){
	$.extend($.fn.datagrid.defaults, {
		...
		onCellDirectHandler: function(index, field, dir){} //滁州第二：方向键新增
	});

	function navHandler(e, dir){//滁州第二：方向键新增
		if (dir == "left" || dir == "right" || dir == "up" || dir == "down") {
			...
			if (input){
				...
				{
					var opts = $(this).datagrid('options');
					opts.onCellDirectHandler.call(this, cell.index, cell.field, dir);
				}
			} else {
		...
		if (col && col.editor){
			...
			if (input){
				...
				if (ed.type == 'combobox') {
					...
					if (editor) {
		       			var target2 = editor.target.combobox('textbox');
		       			if (target2) {
							target2.unbind("keydown");
				...
				//input.combobox('showPanel') //展开列表
				//input.combobox('hidePanel') //隐藏列表
				dg.datagrid('gotoCell', param);
				setTimeout(function(){
