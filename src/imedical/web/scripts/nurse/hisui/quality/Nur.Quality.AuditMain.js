...
var GV = {
    ...
    showStatusList: {all: $g('全部'), S: $g('待质控员审核'), ZKYSH: $g('待护士长审核'), HSZ: $g('待护理部审核'), HLBSH1: $g('审核通过')}, // xfc：增加
    buttonList: [], // 按钮
    statusConfigList: [],   // 状态
...
function initGrid() {
    ...
    var frozenColumns = [
        ...
        {
            field: 'status', title: $g('状态'), width: 110, align: 'left',
    ...
    columns = columns.concat([{ field: 'recDesc', title: $g('文书名称'), width: 240, align: 'left', formatter: showRecDesc },
...
function initShowInputEmrNote() {
    ...
    $('#inputEmrNoteTable').datagrid({
        ...
        nowrap: false,
        columns: [[
            { field: 'content', title: '备注内容', width: 150, editor: { type: 'textarea' } },
            { field: 'addUser', title: '备注人', width: 70, align: 'left' },
            { field: 'addDateTime', title: '备注日期', width: 160, align: 'left' },
            {
                field: 'responsible', title: '责任人', width: 70, align: 'left',
            ...
            {
                field: 'ifAdajust', title: '是否调整', width: 70, align: 'left', editor: 'text',
            ...
            { field: 'adajustUser', title: '调整人', width: 70, align: 'left' },
            { field: 'adajustDateTime', title: '调整日期', width: 150, align: 'left' },
    ...
    runClassMethod("Nur.Quality.Service.Audit", "getAuditMainDetails",
        {
            ...
            for (var item in details) {
                ...
                var item = new Object();
                item['title'] = status;
                item['context'] = '<label>' + $g('操作人：') + '</label><div class="cntt"><div>' + auditUser + '</div><div>' + auditDateTime + '</div></div>';
                items.push(item);
            }
...
function getAppraiseList(auditMainID) {
    $('#appraiseListTable').datagrid({
        ...
        idField: 'appraiseID',
        columns: [[
            { field: 'appraiseScore', title: $g('评价结果'), width: 150, align: 'left' },
            { field: 'appraiseUser', title: $g('评价人'), width: 150, align: 'left' },
            { field: 'appraiseDateTime', title: $g('评价日期'), width: 230, align: 'left' },
...
function emrNotesBtn() {
    ...
    if (selectedNote.type == 'root') {
        $.messager.popover({ msg: $g('请选择病历模板！'), type: 'error', timeout: 1000 });
        return;
    }
...
function showEmrScoreTable(data, divID) {
    ...
    var html = '<div style="width: 90%; margin: 0px auto;">' +
        ...
        $g("患者") + '<input value="' + appraiseInfo.patName + '" class="textbox" name="patName" disabled="true" style="width:120px;height:26px;margin:0 12px 0 10px;" >' +
        '&nbsp&nbsp' + $g("登记号") + '<input value="' + appraiseInfo.regNo + '" class="textbox" name="RegNo" disabled="true" style="width:120px;height:26px;margin:0 12px 0 10px;" >' +
        '&nbsp&nbsp' + $g("评价日期") + '<input value="' + appraiseInfo.appDateTime + '" class="textbox" name="apoDateTime" disabled="true" style="width:140px;height:26px;margin:0 12px 0 10px;" >' +
        '&nbsp&nbsp' + $g("成绩") + '<input id="appScore" value="' + appraiseInfo.score + '" class="textbox" name="appScore" disabled="true" style="width:50px;height:26px;margin:0 12px 0 10px;" >' +
        '&nbsp&nbsp' + $g("状态") + '<input value="' + $g(appraiseInfo.status) + '" class="textbox" name="appStatus" disabled="true" style="width:50px;height:26px;margin-left:10px;" >' +
        '</div>' +
