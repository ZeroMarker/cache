(function($){
	function getVisibleFields(dg)
	{
		var fields=dg.datagrid('getColumnFields',true).concat(dg.datagrid('getColumnFields',false));
		var newFields=[]
		for(var i=0; i<fields.length; i++){
            var col = dg.datagrid('getColumnOption', fields[i]);
            if ((col["checkbox"]!=true)&&(col["hidden"]!=true))
            {
    ...
    function getRows(target,parseDataFun){
        ...
        if (state.filterSource){
	        ...
	        {
		        return parseDataFun(state.data.rows)
		    }else{
    ...
    function toHtml(target, rows, footer, caption, parseDataFun){
	    if (parseDataFun==null) parseDataFun=""
        rows = rows || getRows(target,parseDataFun);
        rows = rows.concat(footer||getFooterRows(target));
        var dg = $(target);
        var data = ['<table border="1" rull="all" style="border-collapse:collapse">'];
        //var fields = dg.datagrid('getColumnFields',true).concat(dg.datagrid('getColumnFields',false));
        var fields = getVisibleFields(dg)
        ...
        for(var i=0; i<fields.length; i++){
            ...
            data.push('<td style="vnd.ms-excel.numberformat:@;'+tdStyle+'">'+col.title+'</td>');
        }
        data.push('</tr>');
        $.map(rows, function(row){
            ...
            for(var i=0; i<fields.length; i++){
                ...
                data.push(
                    '<td  style="vnd.ms-excel.numberformat:@;'+tdStyle+'">'+ value +'</td>'
                );
            }
            data.push('</tr>');
        });
    ...
    function toExcel(target, param){
        ...
        if (typeof param['name'] == 'string'){
            filename = param['name'];
            parseDataFun = param['parseDataFun'];
        } else {
        ...
        var table = toHtml(target, rows, footer, caption, parseDataFun);
