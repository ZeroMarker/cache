...
(function($, window, document, undefined) {
	...
	var Scroll = $.Class.extend({
		...
		_nearestSnap: function(x) {
			...
			if (this.element.id == 'slider' && this.direction!= undefined) {
				var currnum = this.slideNumber
				var moveX=0  //相对滑动距离
				var oldX = this.pages[this.slideNumber][0].leftX
				var startSpace = this.pages[0][0].leftX
				if (this.direction === 'left') {
					currnum += 1
					moveX = oldX-x-startSpace
				}else if (this.direction === 'right') {
					currnum -= 1
					moveX = x-oldX+startSpace
				}
				if (currnum < 0) {
					currnum = 0
				}else if (currnum >= length) {
					currnum = length-1
				}
				if (moveX > 50) { //相对滑动距离超过50才生效
					return this.pages[currnum][0];
				}else{
					return this.pages[this.slideNumber][0];
				}
...
(function($, window, document, name) {
	...
	var backdrop = (function() {
		...
		element.addEventListener('tap', function(e) {
			...
			if (popover) {
				if(popover.getAttribute('data-disable-auto-close')){
					return;
				} 
