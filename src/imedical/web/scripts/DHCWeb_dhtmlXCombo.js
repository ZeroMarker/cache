...
////DHCWeb_dhtmlXCombo.js
*/ 

var m_dhtmlXListSplitStr="$c(1)";
///var m_dhtmlXValueSplitStr="^";
...
function dhtmlXComboFromStr(parent,str){
 ...
 //w.tagName=parent.tagName;
 //w.readOnly=parent.readOnly;
 //w.id=parent.id
...
for(var myIdx=0;myIdx<options.length;myIdx++){
	///alert(options[myIdx].text);
}
 this.render(false);
 ...
 dhtmlXCombo.prototype._createSelf = function(selParent,name,width)
{
 ...
 ///Add Array()
	this._WBCodeAry=Array();
	this._PYCodeAry=Array();
	this.TxtAry=Array();
 ...
 dhtmlXCombo.prototype._positList = function()
{
 ...
 if(ev.keyCode=="13")
 {
 	if(typeof that.selectHandle == "function"){
		that.selectHandle();
	}
	...
	if (typeof that.selectlistchange == "function")
	{
		that.selectlistchange();
	}
...
function dhtmlXComboChangleHandler(obj){
}
...
if(ev==118||ev==119||ev==120||ev==121||ev==122||ev==123)
{
  return;
}
...
// dhtmlXRange(this.DOMelem_input,text.length+1,data[1].length);修改text Range
//dhtmlXRange(this.DOMelem_input,data[1].toUpperCase().indexOf(text.toUpperCase())+text.length,data[1].length);
}
}
 else
 this.setComboText(data[1]);
 ...
 dhtmlXCombo.prototype.closeAll = function()
{
 ...
 if(_isIE)dhx_glbSelectAr[i]._IEFix(false);
	dhtmlXComboChangleHandler(this);
}
}
