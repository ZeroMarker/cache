...
Ext.onReady(function() {
	...
	ShowPage();
	Ext.getCmp("grdDataType").loadData();
});

//显示页面
function ShowPage() {
    var allItems = InitControl();
    var frmDataType = new Ext.form.FormPanel({
        id: "frmDataType",
        //layout: "absolute",
...
function InitControl() {
    ...
    var grdDataType = new FW.Ctrl.ListGrid({
        id: "grdDataType",
...
function EditDataType() {
    var rows = Ext.getCmp("grdDataType").getSelectionModel().getSelections();
    ...
    Ext.getCmp("pnlDataType").getForm().findField("txtCode").setValue(rows[0].get("Code"));
    Ext.getCmp("pnlDataType").getForm().findField("txtDescription").setValue(rows[0].get("Description"));
}
...
function DeleteData(btn) {
    if (btn == "ok") {
        ...
        var actionParams = { 'AcessType': 'ClassMethod', 'MethodSignature': 'icare.DataTypeConfig:DeleteDataType', 'Arg1': deleteId };
        FW.CommonMethod.SubmitForm("frmDataType", actionParams, "grdDataType");
    }
}
...
function InitPanel(actionDesc) {
    var controls = [{ xtype: "FWTextBox", fieldLabel: "代码", vtype: 'number', id: "txtCode", maxLength: 20, allowBlank: false,  height: 20, width: 150 ,labelSeparator:""},
         { xtype: "FWTextBox", fieldLabel: "描述", id: "txtDescription", maxLength: 20, allowBlank: false, height: 20, width: 150 ,labelSeparator:""}
         ];
...
//保存信息
function SaveInfor() {
    var code = Ext.getCmp("pnlDataType").getForm().findField("txtCode").getValue();
    var desc = Ext.getCmp("pnlDataType").getForm().findField("txtDescription").getValue();
    var actionParams = "";
    if (actionDesc == "Add") {
        actionParams = { 'AcessType': 'ClassMethod', 'MethodSignature': 'icare.DataTypeConfig:AddDataType', 'Arg1': code, 'Arg2': desc };
    }
    else if (actionDesc == "Edit") {
    actionParams = { 'AcessType': 'ClassMethod', 'MethodSignature': 'icare.DataTypeConfig:UpdateDataType', 'Arg1': selectCode, 'Arg2': code, 'Arg3': desc };
    }
    SubmitForm("pnlDataType", actionParams, "grdDataType", "winDataType");
}

//取消保存信息
function CancelSave() {
    Ext.getCmp("winDataType").close();
}
