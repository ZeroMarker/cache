...
function InitLookup(){
	$("#ClassName").lookup({
			...
			queryParams:{ClassName: 'web.DHCBL.Patient.DHCPatientUpdateLog',QueryName: 'FindClass'},
	...
	$("#ClassProperty").lookup({
            ...
            textField:'propertyName',
            columns:[[  
                {field:'propertyName',title:'属性名',width:250}
            ]],
			pagination:true,
			panelWidth:300,
			panelHeight:300,
			...
			delay:'500',
			queryOnSameQueryString:true,
			queryParams:{ClassName: 'web.DHCBL.Patient.DHCPatientUpdateLog',QueryName: 'SelectPropertyNew'},
			onBeforeLoad:function(param){
				var desc=param['q'];
				//if (desc=="") return false;
	...
	$("#PropertyLinkName").lookup({
			...
			onBeforeLoad:function(param){
				var desc=param['q'];
				//if (desc=="") return false;
				var ClsName=tkMakeServerCall("web.DHCBL.Patient.DHCPatientUpdateLog","PropertyLinkClassNameNew",$("#ClassNameId").val(),$("#ClassPropertyId").val())
				param = $.extend(param,{ClsName:ClsName,Desc:desc});
			},
            pagination:true,
            onSelect:function(index,rowData){
