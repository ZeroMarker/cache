...
//搜索指标列
var PartColumns = [[
	    {field:'Synonym',title:'代码',width:100},
...
function DrugInfo(){
	...
	$('#meduselinkdg').datagrid({
			...
			fitColumns:true,  //自动初始化宽度，不要与frozenColumns（冻结列）一起使用
...
function IndexInfo(phdcDr){
	$('#meduselinkitmdg').datagrid({
			...
			columns:[[		  //定义接收的数据格式，其中的field要和后台传递过来的相对应
				{field:'Synonym',title:'代码',width:100,editor:'text'},
				{field:'TestCodeName',title:'描述',width:150,editor:'text'},
...
function savmuliRow()
{
	...
	{
		if((rows[i].Synonym=="")||(rows[i].TestCodeName=="")){
			$.messager.alert("提示","有必填项未填写，请核实!"); 
			return false;
		}
		var tabId=rows[i].Id; //指标列表里指标id,如果id为空则新增数据，有值则更新数据
		//指标对应的id
		runClassMethod("web.DHCSTPHCMedIndexMonitoringInquiry","GetIndexId",{"TestCodeName":rows[i].TestCodeName},function(id){
...
function dataIndexGridBindEnterEvent(editmuliRow){
	...
	function setCurrIndexEditRowCellVal(rowObj){
		var ed=$("#meduselinkitmdg").datagrid('getEditor',{index:editmuliRow, field:'Synonym'});
		$(ed.target).val(rowObj.Synonym);
		var ed=$("#meduselinkitmdg").datagrid('getEditor',{index:editmuliRow, field:'TestCodeName'});		
