...
function BodyLoadHandler()
{
	...
	//PatientID=document.getElementById("PatientID").value
		var ReceipNO1=document.getElementById("ReceipNO1").value
	var obj=document.getElementById("ReceipNO1");
	if (obj){
		//alert(obj.value+"HHHHHH")
		ReceipNO_OnKeyPress();
	}
}
...
function ReceipNO_OnKeyPress()
{
	...
	var myReceipNO=DHCWebD_GetObjValue("ReceipNO1");
//	alert(myReceipNO+"@zn%myReceipNO")
...
//		alert(flag+"zb")
		...
		//alert(myrtn.split("^")[15]+"$$"+myrtn.split("^")[20]+"$$"+myrtn.split("^")[6])
		if (rtn!="0")
		{
			alert(t['06']);
		...
		else
		{
			...
			//alert(myAPIRowID+"***zn***myAPIRowID")
...
function WrtRefundMain(AccINVInfo)
{
	...
	myBtnFlag="S"  //忽略未审核 add by  2012-12-25 merry christmas
...
{
	var InvDided=tkMakeServerCall("web.UDHCJFBaseCommon","JudgeAccInvDided",InvNo)
//	alert(InvDided+"123")
	if(InvDided=="-1"){
		alert("此发票已经做过门诊医保结算，不允许转入")
		return 0;
	}
