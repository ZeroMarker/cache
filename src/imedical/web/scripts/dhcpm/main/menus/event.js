	...
	obj.btnDelete_click = function()
	{
		...
		if (selectObj){
			ExtTool.confirm('选择框','确定删除?',function(btn){
				if(btn=="no") return;
				var objMainService = ExtTool.StaticServerObject("DHCPM.SSService.MenusMain");
...
function InitwinScreenEvent(obj) {
	...
	obj.LoadEvent = function(){
		var data = arguments[0][0];
  		if (data){
  			var objMenus = ExtTool.StaticServerObject("DHCPM.SSService.MenusMain");
	...
	{
		var objMenus = ExtTool.StaticServerObject("DHCPM.SS.Menus");
	...
	{
		var objProduct = ExtTool.StaticServerObject("DHCPM.SS.MenuOperation");
