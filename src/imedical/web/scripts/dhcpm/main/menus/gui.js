function InitViewport1(){
	...
	obj.tbbutton = new Ext.Toolbar({
			enableOverflow : true,
			items : [obj.btnNew, '-', obj.btnEdit,'-',obj.btnDelete]
		});
	/** 搜索工具条 */
	obj.tb = new Ext.Toolbar({
	            //enableOverflow : true,
				id : 'tb',
				items : [new Ext.Toolbar.TextItem('菜单代码：'),obj.txtMenuCode,'-',new Ext.Toolbar.TextItem('菜单名称：'),obj.txtMenuName,'-',new Ext.Toolbar.TextItem('所属产品：'),obj.txtProName,'-',new Ext.Toolbar.TextItem('父菜单：'),obj.txtMenuCaption,'-',obj.btnQuery],
				listeners : {
					render : function() {
						obj.tbbutton.render(obj.gridList.tbar);
					}
				}
			});
	...
	obj.txtProNameStoreProxy.on('beforeload', function(objProxy, param){
			param.ClassName = 'DHCPM.SSService.ProductsMain';
	...
	obj.txtMenuCaptionStoreProxy.on('beforeload', function(objProxy, param){
			param.ClassName = 'DHCPM.SSService.MenusMain';
