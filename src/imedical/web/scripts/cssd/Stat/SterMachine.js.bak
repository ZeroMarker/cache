...
var init = function() {
	...
	var ReqLocBox = $HUI.combobox('#PackName',{
		url: $URL + '?ClassName=web.CSSDHUI.Common.Dicts&QueryName=GetPackage&ResultSetType=array&typeDetial='+typeDetial+'&packageClassDr='+packageClassDr,
		valueField: 'RowId',
	...
	function CheckedUrl(Checked,Params,Conditions){
		//入库单列表
		if('FlagStatSterMachine'==Checked){
			p_URL = PmRunQianUrl+'?reportName=CSSD_HUI_StatSterMachine.raq&Params='+Params+'&Conditions='+Conditions;
		}else if('FlagStatSterMachineDetail'==Checked){
			p_URL = PmRunQianUrl+'?reportName=CSSD_HUI_StatSterMachineDetail.raq&Params='+Params+'&Conditions='+Conditions;
		}
	...
	function GetConditions(ParamsObj){
		...
		if(ParamsObj.StartDate!=""){
			Conditions=" 统计时间: "+ParamsObj.StartDate+" "+ParamsObj.StartTime;
		}
		if(ParamsObj.EndDate!=""){
			Conditions=Conditions+" 截止时间："+ParamsObj.EndDate+" "+ParamsObj.EndTime
		}
		return Conditions;
	}
