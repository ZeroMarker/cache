...
var init = function() {
	...
	function CheckedUrl(Checked, Params, Conditions) {
		var ReportUrl = '';
		if ('ExtStatFlag' === Checked) {
			ReportUrl = PmRunQianUrl + '?reportName=CSSD_HUI_ExtRegStat.raq&Params=' + Params + '&Conditions=' + Conditions;
		}
		return ReportUrl;
	}
	// 组织查询条件
	function GetConditions(ParamsObj) {
		...
		if (ParamsObj.ExtFirmId !== '') {
			Conditions = Conditions + ' 厂商: ' + $('#ExtFirmId').combobox('getText');
		}
		return Conditions;
	}
