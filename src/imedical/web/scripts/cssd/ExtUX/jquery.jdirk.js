...
(function(window, $, undefined) {
	...
	coreUtil.getRootPath = function() {
		...
		if (pathname.indexOf('/') > -1) {
			...
			for (i = 0; i < coreUtil.rootDegree && i < temps.length; i++) { result += '/' + temps[i]; }
		}
		return result;
	};
	...
	coreString.toCase = function(str) {
		...
		for (var i = 0; i < str.length; i++) {
			if (str.charCodeAt(i) > 0 && str.charCodeAt(i) < 255) { tmp += String.fromCharCode(str.charCodeAt(i) + 65248); } else { tmp += String.fromCharCode(str.charCodeAt(i)); }
		}
		return tmp;
	};
	...
	coreString.toDate = function(str) {
		str = coreString.isNullOrEmpty(str) ? '' : String(str);
		try { return new Date(str.replace(/-/g, '\/')); } catch (e) { return undefined; }
	};
