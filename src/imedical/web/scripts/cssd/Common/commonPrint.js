...
function PrintDocument(PrintNum, IndirPrint, TaskName, Title, Head, Foot, Cols, DetailData) {
	...
	for (i = 0; i < DetailData.length; i++) {
		...
		td = "<tr style='height: " + DataTrHeight + "'>";
	...
	for (j = 0; j < colslen; j++) { // 循环列
		...
		} else if (j == 1) {	// format='#' 医为浏览器不能转换格式，注释掉
			strTableFootP += '<th ' + nowrap + " style='border-left:1px solid gray;border-top:1px solid gray;font-size:" + FootFontsize + ";' align='right' tdata='SubCount' >###</th>";
			strTableFootT += '<th ' + nowrap + " style='border-left:1px solid gray;border-top:1px solid gray;font-size:" + FootFontsize + ";' align='right' tdata='AllCount' >###</th>";
		} else if (Cols[j].sum == 'Y') {
			strTableFootP += '<th ' + nowrap + " style='border-left:1px solid gray;border-top:1px solid gray;font-size:" + FootFontsize + ";' align='" + Cols[j].align + "' tdata='SubSum' >###</th>";
			strTableFootT += '<th ' + nowrap + " style='border-left:1px solid gray;border-top:1px solid gray;font-size:" + FootFontsize + ";' align='" + Cols[j].align + "' tdata='AllSum' >###</th>";
		} else {
