...
//          }
        ...
        gridObj.enter=function(){
            ...
            for (var i = 0, Len = ed.length; i < Len; i++){
                var e = ed[i];
                $(e.target).bind('keyup', function(ev){
                    if (window.event.keyCode == 13){
                        var Field = $(this).parents('td[field]').attr('field');
                        
                        var NoEnterIndex;
                        var NoEnterFieldArr = [].concat(gridObj.CombogridNoEnterField);
                        $.each(NoEnterFieldArr, function(index, value){
                            if(NoEnterFieldArr[index] == Field){
                                NoEnterIndex = index;
                                return false;
                            }
                        });
                        if(NoEnterIndex === 0){
                            //不处理
                        }else if(NoEnterIndex === 1){
                            //第二个不跳转时,释放
                            gridObj.CombogridNoEnterField = null;
                        }else{
                ...
                $(e.target).next().children().bind('keydown', function(ev){
                    if (window.event.keyCode == 13){
                        ...
                        if(EditorObj.hasClass('combogrid-f')){
                            ...
                            //combogrid回车时,设置一个datagrid属性(不使用跳转的field),用于控制是否表格跳转
                            gridObj.CombogridNoEnterField = [Field, NextField];
	...
	// 停止回车跳转的方法,通过控制enter方法用到的CombogridNoEnterField实现
	gridObj.stopJump = function(editor){
		gridObj.CombogridNoEnterField = [ editor.field ];
	}
	...
	gridObj.getNextFocusField = function(Field){
		...
		gridObj.getChangesData=function(key){
			var arrData=[];  //处理后的数据
			if(!isEmpty(key)){
				if((this.editIndex!=undefined)&&(!isEmpty(this.getRows()[this.editIndex][key]))){
					if(!this.endEditing()){
						return false;
					}
				}
