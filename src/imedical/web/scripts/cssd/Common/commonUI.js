...
function CommonUI() {
	...
	*/
	this.msg = function(type, msg, timeout, showSpeed, showType, style) {
		...
		showType = showType || 'slide', style = style || { top: document.body.scrollTop + document.documentElement.scrollTop + 10, left: '' };
	...
	*/
	...
	this.datagrid = function(id, options) {
		...
		if (ToolBarType === 'object') {
			...
			} else if (options.showDelSaveItems) {
				options.toolbar.unshift(deleteRow, saveRow);
			} else {
				if (options.showSaveItems) {
		...
		gridObj.endEditing = function(newEditIndex) {
			...
			if (!isEmpty(checkField)) {
				...
				if (isEmpty(CheckFieldValue)) {
					ed = gridObj.getEditor({ index: this.editIndex, field: checkField });
					if (!isEmpty(ed)) {
		...
		gridObj.commonDeleteRow = function(ConfirmFlag, SelIndex) {
			...
			var len = '';
			if (!isEmpty(SelIndex)) {
				gridObj.selectRow(SelIndex);
				row = gridObj.getRows()[SelIndex];
				rowsData.push(row);
				...
				if (len <= 0) {
					...
					if (cell) {
						row = gridObj.getRows()[cell.index];
						rowsData.push(row);
					}
				}
			}
			...
			if (arrData.length <= 0) {
				...
				if (options.deleteRowParams && !isEmpty(options.deleteRowParams.ClassName) && !isEmpty(options.deleteRowParams.MethodName)) {
					if (ConfirmFlag !== false) {
						$UI.confirm('确定要删除选中行吗?', '', '', deleterows, '', CancelDelete);
					} else {
						deleterows();
					}
				}
				/* else{//部分界面的删除操作写在了beforeDelFn，导致跟此处提示冲突，暂且注释
		...
		gridObj.getFirstFocusField = function() {
			...
			var firstFocusField = gridObj.jdata.options.firstFocusField;
			if (!isEmpty(firstFocusField)) {
				return firstFocusField;
			}
			$.each(colSort, function(index, value) {
				if (isEmpty(firstFocusField) && !isEmpty(value.editor) && value.hidden !== true) {
					firstFocusField = value.field;
					return false;
				}
			});
			return firstFocusField;
			...
			if (options.showBar) {
				...
				if (SaveModInfo != '') {
					...
					if (!isEmpty(newcm) && newcm.length > 0) {
						...
						var SortLen = SortColumns.length;
						for (i = SortLen - 1; i >= 0; i--) {
	...
	this.loopBlock = function(container, skipNames, skipHidden, skipDisabled) {
			...
			ipts.each(function() {
				for (var i = 0; i < cbxCls.length; i++) {
					...
					if (jQuery(this).hasClass(type + '-f')) {
						...
						if (options['multiple']) {
							val = jQuery(this)[type]('getValues');
		...
		// radio&checkbox
		ipts = jQuery('input[name][type=radio], input[name][type=checkbox]', target);
		...
		if (ipts.length) {
			ipts.each(function() {
				...
				if (jQuery(this).lookup('options')['valueType'] === 'text') {
					...
					val = jQuery(this).lookup('getText');
					if (!isEmpty(RowId)) {
						val = '';
					}
				} else {
					var Text = jQuery(this).lookup('getText');
					val = jQuery(this).lookup('getValue');
					if (isEmpty(Text)) {
						val = '';
					}
				}
	...
	this.fillBlock = function(container, data) {
		...
		function setNameVal(name, val) {
			...
			// radio checkbox
			opts = $('input[name="' + name + '"][type=radio]', target);
			if (opts.length) {
				opts.prop('checked', false);
				opts.each(function() {
					var f = $(this);
