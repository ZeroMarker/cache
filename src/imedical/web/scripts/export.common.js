...
function GetNum(TempNode,job)
{
	...
	num=cspRunServerMethod(encmeth,TempNode,"",job);		// MZY0149	3076883		2023-01-09
}
...
function  PrintDHCEQEquipChrome(TableName,SaveOrPrint,job,vData,TempNode,GetRowsPerTime)
{
    ...
    for (Row=1;Row<=num;Row++)
    {
		...
		if (endRow>num) endRow=num;
		var rowsCount=endRow-Row+1;
		// MZY0143	3070631		2022-11-16
	 	if (!GetRowsPerTime)
	 	{
		 	var str=cspRunServerMethod(encmeth,TempNode,job,Row)   //Modefied by zc0126 2022-12-26 修正导出问题
	 	}
	 	else
	 	{
			var str=cspRunServerMethod(encmeth,TempNode,job,Row,rowsCount)   //Modefied by zc0126 2022-12-26 修正导出问题
	 	}
	 	str=filepath(str,",","、");
    ...
    OneBatchInfoStr=OneBatchInfoStr.replace(/\t/g," ");		//czf 2021-01-26 1750644
	OneBatchInfoStr = OneBatchInfoStr.replace(/\n/g,"");
	OneBatchInfoStr=OneBatchInfoStr.replace(/\r\n/g," ");
...
///判断TableName是否有列设置 有true 无false
function CheckColset(TableName)
{
