...
function SelectRowHandler()	{
	...
	var objtbl=document.getElementById('tDHCEQMaintPlanItem'); //得到表格   t+组件名称
	...
	var obj1=document.getElementById('MaintPlanDR');
...
function FillData(selectrow)
{
	...
	var sort=10;
	SetElement("MaintPlanDR",list[0]);
	SetElement("MaintPlan",list[sort+0]);
...
{	
	...
	var InspectPlanDR=document.getElementById('MaintPlanDR').value;
	if (InspectPlanDR==""){
		alertShow(t["02"]);
		return;}
	...
	if (RowID!=""){
		alertShow(t["01"]);
		return;}
	var InspectPartDR=document.getElementById('MaintPartDR').value;
	var InspectItemDR=document.getElementById('MaintItemDR').value;
	if (CheckNull()) return;
	...
	var sqlco=cspRunServerMethod(encmeth,"","",InspectPlanDR,InspectPartDR,InspectItemDR,Remark)
...
function BUpdate_click()
{
	...
	var sqlco=cspRunServerMethod(encmeth,"","",RowID,InspectPlanDR,InspectPartDR,InspectItemDR,Remark)
	if (sqlco!='0') {
		alertShow(t["04"]);
	return;	}
    try {		   
