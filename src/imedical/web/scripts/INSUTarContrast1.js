...
function BodyLoadHandler() {
	...
	var obj=document.getElementById("UpLoadConAudit");
	if(obj){obj.onclick=UpLoadConAudit_onclick;}
	var obj=document.getElementById("DownLoadConAudit");
	if(obj){obj.onclick=UpLoadConAudit_onclick;}
	ini();
	IPAddress=GetLocalIPAddress() 
	}
	
function UpLoadConAudit_onclick() {
	var obj=document.getElementById("Type");
	if (obj){var Type=obj.value}
	alert(Type)
	var flag=INSUUpLoadConAudit(Type)
	if (flag=="0"){alert("对照审批上报失败")}
	else {alert("对照审批上报成功")}
	}
function DownLoadConAudit_onclick() {
	var obj=document.getElementById("Type");
	if (obj){var Type=obj.value}
	var flag=INSUDownLoadConAudit(Type)
	if (flag=="0"){alert("对照审批下载失败")}
	else {alert("对照审批下载成功")}
	}
function KeyDown(){
...
function SelectRowHandler()	{
	...
	if (iSeldRow==selectrow){
		...
		obj.value="^^^"     //add by lilizhi 2013-03-29 
...
function ExportXLS_onclick()
{
	try
	{
		...
		if((repidsub==0)||(PrtRowCnt==0)){alert("没有数据,请重新查询!");return;}
		if(PrtRowCnt>500){
			ExportCSV();
			return;
		}
		...
		//清理垃圾
		tkMakeServerCall("web.INSUTarContrastCom","ClearTmpGlobal",repidsub);
		alert("导出成功!")
	} catch(e) {
		alert(e.message);
	};
...
function ExportCSV()
{
	try
	{
		...
		if((repidsub==0)||(PrtRowCnt==0)){alert("没有数据,请重新查询!");return;}
		var file_name=window.prompt("请指定输出文件名称(.csv)","C://Export.csv");
		var file_name=file_name.split("//").join("////");
