...
$(function(){
	...
	var columns=[[
		...
		{field:'Hosp',title:'医院',width:200,editor:hospEditor}, //hxy 2019-07-01
		{field:'HospID',title:'医院ID',width:80,editor:'text',hidden:true} //hxy 2019-07-01
	]];
...
{
	if(editRow>="0"){
		$("#advimp").datagrid('endEdit', editRow);//结束编辑，传入之前编辑的行
	}
	...
	$("#advimp").datagrid('insertRow', {//在指定行添加数据，appendRow是在最后一行添加数据
		index: 0, // 行数从0开始计算
		row: {ID: '',Code:'',Desc:'',Active:'Y',Hosp:LgHospID,HospID:LgHospID} //hxy 2019-07-01 LgHospID
	});
	$("#advimp").datagrid('beginEdit', 0);//开启编辑并传入要编辑的行
	editRow=0;
}
...
function deleteRow()
{
	var rows = $("#advimp").datagrid('getSelections'); //选中要删除的行
	if (rows.length > 0) {
