...
var selItems = $('#maindg').datagrid('getSelections');
	...
	var Shareparams=LgUserID+"^"+LgCtLocID+"^"+LgGroupID+"^"+typecode;
	//获取重点关注权限
	var ShareAuthority="";
	 $.ajax({  
		url:'dhcapp.broker.csp?ClassName=web.DHCADVCOMMON&MethodName=GetShareAuthority'+'&params='+Shareparams, 	        
	...
	if(ShareAuthority=="N"){
		$.messager.alert("提示:","无"+Sharemessage+"操作权限！");
		return;
	}
...
function RepImpFlag()
{
	...
	var Focusparams=LgUserID+"^"+LgCtLocID+"^"+LgGroupID+"^"+typecode;
	//获取重点关注权限
	var FocusAuthority="";
	 $.ajax({  
...
function setCellEditSymbol(value, rowData, rowIndex)
{
		...
		var Medadrreceivedr=rowData.Medadrreceivedr;
...
{          
		...
		var Medadrreceivedr=rowData.Medadrreceivedr //接收id 2为驳回
		var StaFistAuditUser=rowData.StaFistAuditUser //被驳回人
		var BackAuditUser=rowData.BackAuditUser //驳回人
		//return "<a href='#' onclick=\"showAssessWin('"+ID+"','"+typecode+"')\"><img src='../scripts/dhcadvEvt/images/viewlist.png' border=0/></a>";
		var html = "";
		if ((AssessRowID != "")&&(StaFistAuditUser!=LgUserName)&&(BackAuditUser!=LgUserName)){
		...
		}else if ((AssessRowID != "")&&((StaFistAuditUser==LgUserName)||(BackAuditUser==LgUserName))){
			html = "<a href='#' onclick=\"showAssessWin('"+ID+"','"+typecode+"','"+TypeID+"','"+AssessRowID+"')\" style='margin:0px 5px;font-weight:bold;text-decoration:none;'>已评估</a>";
		}
		else{
			  html = "未评估";
			}
