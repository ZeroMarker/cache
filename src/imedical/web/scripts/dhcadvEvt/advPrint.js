...
function printDrugErrData(data){
	...
	objSheet.Cells(3,2).value=$getValue(data["DisMedThingPatName"]); //患者姓名
	objSheet.Cells(3,4).value=$getValue(data["PatMedicalNo"]); //病案号
	objSheet.Cells(3,6).value=$getValue(data["OccuLoc"]); //发生科室
	
	objSheet.Cells(4,2).value=$getValue(data["PatSexinput"]); //性别
	objSheet.Cells(4,5).value="登记号："; //登记号
	objSheet.Cells(4,6).value="'"+$getValue(data["PatID"]); //登记号
	objSheet.Cells(5,2).value=$getValue(data["PatAge"]); //年龄
	objSheet.Cells(6,2).value=$getValue(data["PatDiag"]); //第一诊断
	//var PatOrign=$getValue(data["PatOrigin-label-94335"])
	...
	objSheet.Cells(8,2).value=$getValue(data["PatAdmDate"]); //入院日期
	...
	objSheet.Cells(12,1).value="给药发生日期："+$getValue(data["GiveDrugHappenTime"])+" "+$getValue(data["OccurTime"]); //给药发生日期
	...
	objSheet.Cells(15,1).value="当事人工作年限："+$getValue(data["DrugUsePartyWorkYears"]); //当事人工作年限
	
	objSheet.Cells(16,2).value=$getValue(data["PartyName"]); //当事人姓名
	objSheet.Cells(16,4).value=$getValue(data["DrugUseErrLevel"]); //能级
	objSheet.Cells(16,6).value=$getValue(data["Shift"]);  //班次
	...
	objSheet.Cells(18,2).value=$getValue(data["WallDiscover"]); //发现人姓名
	objSheet.Cells(18,4).value="";///$getValue(data["DisMedThingPatName"]); //职称  
	objSheet.Cells(18,6).value=$getValue(data["WLManWorkLife"]); //工作年限
	
	var list="";
	var dosetargeterr=$getValue(data["DrugUseErrType-94616"]); //给药对象错误
	if (dosetargeterr!=""){
		list=list+dosetargeterr+"；";
	}

	var dosetimeerr=$getValue(data["DrugUseErrType-94617"]); //给药时间错误
	var dosetime=$getValue(data["DrugUseErrType-94617-94204"]);//医嘱给药时间
	var errdosetime=$getValue(data["DrugUseErrType-94617-94205"]);//错误给药时间
	if(dosetime!=""){
	...
	var dosewayerr=$getValue(data["DrugUseErrType-94618"]); //给药途径错误
	var doseway=$getValue(data["DrugUseErrType-94618-94208"]);//医嘱给药途径
	var errdoseway=$getValue(data["DrugUseErrType-94618-94209"]);//错误给药途径
	if(doseway!=""){
	...
	var missdose=$getValue(data["DrugUseErrType-94619"]); //遗漏给药
	var missnum=$getValue(data["DrugUseErrType-94619-94212"]);//遗漏次数
	var missdosetime=$getValue(data["DrugUseErrType-94619-94213"]);//医嘱给药时间
	if(missnum!=""){
	...
	var infusionspeed=$getValue(data["DrugUseErrType-94620"]); //输液速度错误
	var infudrugname=$getValue(data["DrugUseErrType-94620-94215"]);//药物名称
	var errdoseinfusion=$getValue(data["DrugUseErrType-94620-94216"]);//错误给药速度
	if(infudrugname!=""){
	...
	var dosageerr=$getValue(data["DrugUseErrType-94621"]); //剂量错误
	var dosedosage=$getValue(data["DrugUseErrType-94621-94219"]);//医嘱给药剂量
	var errdosage=$getValue(data["DrugUseErrType-94621-94220"]);//错误给药剂量 
	if(dosedosage!=""){
	...
	var dosageformerr=$getValue(data["DrugUseErrType-94622"]); //剂型错误
	var dosedosageform=$getValue(data["DrugUseErrType-94622-94223"]);//医嘱给药剂型
	var errdosageform=$getValue(data["DrugUseErrType-94622-94224"]);//错误给药剂型 
	if(dosedosageform!=""){
	...
	var drugerr=$getValue(data["DrugUseErrType-94623"]); //药物错误
	var dosedrugname=$getValue(data["DrugUseErrType-94622-94223"]);//医嘱给药名称
	var errdrugname=$getValue(data["DrugUseErrType-94622-94224"]);//错误给药名称 
	if(dosedrugname!=""){
	...
	var drugvalidityerr=$getValue(data["DrugUseErrType-94624"]); //药物效期错误
	if (drugvalidityerr!=""){
		list=list+drugvalidityerr+"；";
	}

	var drugoth=$getValue(data["DrugUseErrType-94625"]); //其他
	if (drugoth!=""){
		list=list+drugoth+"；";
	}
	...
	objSheet.Cells(23,2).value=$getValue(data["RepHospType"]); //报告单位
	objSheet.Cells(23,6).value=$getValue(data["HospPhone"]); //联系电话
	objSheet.Cells(24,2).value=$getValue(data["ReportDate"]); //报告日期
	
	objSheet.Cells(25,2).value=$getValue(data["RepUserName"]); //填报人姓名
	objSheet.Cells(25,4).value=$getValue(data["RepUserTitle"]); //职称
	objSheet.Cells(25,6).value=$getValue(data["RepUserWorkYears"]); //工作年限
	
	var WLErow=parseInt($getValue(data["WLEventProcess"]).length/45);
	var WLEheightlen=(WLErow+1)*14;
	objSheet.Rows(27+":"+27).RowHeight = WLEheightlen; 
	objSheet.Cells(27,1).value=$getValue(data["WLEventProcess"]); //事件经过
	var dealWayrow=parseInt($getValue(data["dealWay"]).length/45);
	var dealWayheightlen=(dealWayrow+1)*14;
	objSheet.Rows(29+":"+29).RowHeight = dealWayheightlen;
	objSheet.Cells(29,1).value=$getValue(data["dealWay"]); //处理措施
...
function printAccidentData(data){
	...
	objSheet.Cells(8,2).value=$getValue(data["PatAdmDate"]); //入院日期  
	objSheet.Cells(9,1).value="入院时ADL得分："+$getValue(data["PatAdmADLScore"]); //入院时ADL得分
	...
	objSheet.Cells(15,2).value=$getValue(data["HappenTime"])+" "+$getValue(data["OccurTime"]); //给药发生日期
	...
	objSheet.Cells(19,2).value=$getValue(data["WLManWorkLife"]); //工作年限
	...
	if (patHOD!=""){
		eventresult=eventresult+"； "+$getValue(data["AFResult-94566"])+"（"+patHOD+"）";
	}	
	
	objSheet.Cells(21,1).value=eventresult; //事件造成的后果  
	
	objSheet.Cells(22,2).value=$getValue(data["RepHospType"]); //报告单位
	objSheet.Cells(22,6).value=$getValue(data["HospPhone"]); //联系电话
	objSheet.Cells(23,2).value=$getValue(data["ReportDate"]); //报告日期
	
	var row=parseInt($getValue(data["WLEventProcess"]).length/45);
	var heightlen=row*14;
	objSheet.Rows(25+":"+25).RowHeight = heightlen; 
	objSheet.Cells(25,1).value=$getValue(data["WLEventProcess"]); //事件经过
	...
	if(notice!=""){
		deal=$getValue(data["PatEventProcess-95021"])+"（"+notice+"）； "+deal
	}
	if($getValue(data["PatEventProcess-95033"])!=""){
		deal=deal+"；医疗或护理措施（"+$getValue(data["PatEventProcess-95033"])+"）"
	}
	if($getValue(data["PatEventProcess-95034"])!=""){
		deal=deal+$getValue(data["PatEventProcess-95034"])
	}
...
function printWallLData(data){
	...
	objSheet.Cells(3,2).value=$getValue(data["AFLoc"]); //发生科室
	objSheet.Cells(3,4).value=$getValue(data["EveType"]); //事件类别
	objSheet.Cells(3,6).value=$getValue(data["FindDate"]); //发现日期
	
	var Prow=parseInt($getValue(data["WLEventProcess"]).length/45);
	var Pheightlen=(Prow+1)*14;
	objSheet.Rows(5+":"+5).RowHeight = Pheightlen; 
	objSheet.Cells(5,1).value=$getValue(data["WLEventProcess"]); //事件经过
	
	var CArow=parseInt($getValue(data["WLCauseAnalysis"]).length/45);
	var CAheightlen=(CArow+1)*14;
	objSheet.Rows(7+":"+7).RowHeight = CAheightlen; 
	objSheet.Cells(7,1).value=$getValue(data["WLCauseAnalysis"]); //原因分析
	
	var IMrow=parseInt($getValue(data["WLImprovedMethod"]).length/45);
	var IMheightlen=(IMrow+1)*14;
	objSheet.Rows(9+":"+9).RowHeight = IMheightlen; 
	objSheet.Cells(9,1).value=$getValue(data["WLImprovedMethod"]); //改进办法
	
	objSheet.Cells(11,2).value=$getValue(data["WallDiscover"]); //发现人
	objSheet.Cells(11,4).value=$getValue(data["JobTitle"]); //职称
	objSheet.Cells(11,6).value=$getValue(data["Duty"]); //职务
	objSheet.Cells(12,2).value=$getValue(data["WallWorkYears"]); //工作年限
	...
	objSheet.Cells(16,2).value=$getValue(data["WLMan"]); //堵漏人
	objSheet.Cells(16,4).value=$getValue(data["WLManWorkLife"]); //工作年限
	objSheet.Cells(16,6).value=$getValue(data["WallLoc"]); //堵漏人科室
	
	var Brow=parseInt($getValue(data["WLBehavior"]).length/45);
	var Bheightlen=(Brow+1)*14;
	objSheet.Rows(17+":"+18).RowHeight = Bheightlen; 
	objSheet.Cells(17,1).value="堵漏行为："+$getValue(data["WLBehavior"]); //堵漏行为
	
	var QCrow=parseInt($getValue(data["QualityCommitteeOpinion"]).length/45);
	var QCheightlen=(QCrow+1)*14;
	objSheet.Rows(20+":"+20).RowHeight = QCheightlen; 
	objSheet.Cells(20,1).value=$getValue(data["QualityCommitteeOpinion"]); //质量委员会意见
...
function printDisMedData(data){
	...
	objSheet.Cells(3,2).value=$getValue(data["DisMedThingPatName"]); //患者姓名
	objSheet.Cells(3,3).value="病案号："+$getValue(data["PatMedicalNo"])+" "+"性别："+$getValue(data["PatSexinput"])+" "+"年龄："+$getValue(data["PatAge"])+" "+"登记号："+$getValue(data["PatID"]);  //病案号
	//objSheet.Cells(3,5).value="性别："+$getValue(data["PatSexinput"]); //性别
	//objSheet.Cells(3,6).value="年龄："+$getValue(data["PatAge"]); //年龄
	
	objSheet.Cells(5,1).value="事件发生日期："+$getValue(data["DisMedThingHappenDate"]); //事件发生日期
	objSheet.Cells(5,5).value="发生科室："+$getValue(data["OccuLoc"]); //发生科室
	var SArow=parseInt($getValue(data["DisMedThingSpecArea"]).length/45);
	var SAheightlen=(SArow+1)*14;
	objSheet.Rows(7+":"+7).RowHeight = SAheightlen; 
	objSheet.Cells(7,1).value="    "+$getValue(data["WLEventProcess"]); //事件陈述
	objSheet.Cells(9,1).value="医疗器械分类名称："+$getValue(data["DisMedThingMdType"]); //医疗器械分类名称
	
	objSheet.Cells(10,2).value=$getValue(data["DisMedThingGoodName"]); //商品名称
	objSheet.Cells(10,6).value=$getValue(data["DisMedThingRegNumber"]); //注册证号
	objSheet.Cells(11,1).value="生产企业名称："+$getValue(data["DisMedThingREnterpriseName"]); //生产企业名称
	objSheet.Cells(11,6).value=$getValue(data["HospPhone"]); //联系电话
	objSheet.Cells(12,1).value="生产企业地址："+$getValue(data["DisMedThingAddress"]); //生产企业地址
	objSheet.Cells(13,2).value=$getValue(data["DisMedThingModelSpecification"]); //型号规格
	objSheet.Cells(13,4).value=$getValue(data["DisMedThingCode"]); //产品编号
	objSheet.Cells(13,6).value=$getValue(data["DisMedThingProductNummber"]); //产品批号
	
	objSheet.Cells(14,2).value=$getValue(data["DisMedThingPeriodValidity"]); //有效期至
	objSheet.Cells(15,2).value=$getValue(data["DisMedThingDeactivateDate"]); //停用日期
	objSheet.Cells(16,1).value="植入日期（若植入）："+$getValue(data["DisMedThingImlantDate"]); //植入日期（若植入）
	objSheet.Cells(17,1).value="事件发生原因分析："+$getValue(data["DisMedThingEventCauseAnaly"]); //事件发生原因分析
	objSheet.Cells(18,1).value="事件处理情况："+$getValue(data["WLEventProcess"]); //事件处理情况
	...
	var Carow=parseInt($getValue(data["DisMedThingCaution"]).length/45);
	var Caheightlen=(Carow+1)*14;
	objSheet.Rows(21+":"+21).RowHeight = Caheightlen; 
	objSheet.Cells(21,1).value=$getValue(data["DisMedThingCaution"]); //警示
	
	
	objSheet.Cells(22,1).value="质量管理委员会："+$getValue(data["DisMedThingQMC"]); //质量管理委员会
	objSheet.Cells(23,1).value="上报人（签名）："+$getValue(data["DisMedThingEventUserOfReport"]); //上报人（签名）
	objSheet.Cells(23,3).value="报告日期："+$getValue(data["ReportDate"]); //报告日期
	
	objSheet.Cells(23,5).value="报告科室："+$getValue(data["DisMedThingDept"]); //报告科室
	
	var RRrow=parseInt($getValue(data["DisMedThingRepRemark"]).length/45);
	var RRheightlen=(RRrow+1)*14;
	objSheet.Rows(25+":"+25).RowHeight = RRheightlen; 
	objSheet.Cells(25,1).value=$getValue(data["DisMedThingRepRemark"]); //备注
...
function printPipeOffData(data){
	...
	objSheet.Cells(9,1).value="入院时ADL得分："+$getValue(data["PatAdmADLScore"]); //入院时ADL得分
	objSheet.Cells(9,5).value="患者自我照顾能力："+radioValue("PatSelfCareAbility-94345,PatSelfCareAbility-94346,PatSelfCareAbility-94347",data); //患者自我照顾能力
	objSheet.Cells(10,1).value="发生前ADL得分："+$getValue(data["OccurADLScore"]); //发生前ADL得分
	...
	objSheet.Cells(15,1).value="脱管发现时间："+$getValue(data["PipeFindDate"])+" "+$getValue(data["PipeFindTime"]); //脱管发现时间
	objSheet.Cells(16,2).value=$getValue(data["TubeDate"]); //置管日期  
	...
	objSheet.Cells(18,3).value=radioValue("PipeDutyNurTitle-94468,PipeDutyNurTitle-94469,PipeDutyNurTitle-94470,PipeDutyNurTitle-94471",data);//事件发生当班护士职称
	objSheet.Cells(19,2).value=$getValue(data["WallWorkYears"]); //工作年限
	...
	var pipetakename=$getValue(data["PipeTakeSteps-94536-94258"]); //诊断性检查 具体名称
	if (pipetakename!=""){
	...
	var ifblood=$getValue(data["PipeComplication-94540-94541"]); //并发症出血
	if (ifblood!=""){
	...
	objSheet.Cells(40,2).value=$getValue(data["RepHospType"]); //报告单位
	objSheet.Cells(40,6).value=$getValue(data["HospPhone"]); //联系电话
	objSheet.Cells(41,2).value=$getValue(data["ReportDate"]); //报告日期
	
	objSheet.Cells(42,2).value=$getValue(data["RepUserName"]); //填报人姓名
	objSheet.Cells(42,4).value=$getValue(data["RepUserTitle"]); //职称
	objSheet.Cells(42,6).value=$getValue(data["RepUserWorkYears"]); //工作年限
	
	var WLErow=parseInt($getValue(data["WLEventProcess"]).length/45);
	var WLEheightlen=(WLErow+1)*14;
	objSheet.Rows(44+":"+44).RowHeight = WLEheightlen; 
	objSheet.Cells(44,1).value=$getValue(data["WLEventProcess"]); //事件经过
	var dealWayrow=parseInt($getValue(data["dealWay"]).length/45);
	var dealWayheightlen=(dealWayrow+1)*14;
	objSheet.Rows(46+":"+46).RowHeight = dealWayheightlen; 
	objSheet.Cells(46,1).value=$getValue(data["dealWay"]); //处理措施
...
function printSkinUlcerData(data){
	...
	objSheet.Cells(12,1).value="入院压疮风险评分："+$getValue(data["HospUlcerRiskScore"]); //入院压疮风险评分
	...
	objSheet.Cells(13,1).value="发生压疮时风险评分："+$getValue(data["OccurUlcerRiskScore"]); //发生压疮时风险评分
	...
	objSheet.Cells(16,2).value=$getValue(data["OpeDuration"]); //手术（小时）
	
	
	var UlcerPartlist=$getValue(data["UlcerPart"]);//压疮部位
	...
	for(var k=0;k<Ulcerlen;k++){
		...
		objSheet.Cells(17+5*k,2).value="发现日期："+$getValue(UlcerPartlist[k]["UlcerPart-95158-95162-95192"]); //发现日期
		...
		var cmlist=""
		if(($getValue(UlcerPartlist[k]["UlcerPart-95158-95189-94247"])!="")){
			cmlist=$getValue(UlcerPartlist[k]["UlcerPart-95158-95189-94247"]);
			if(($getValue(UlcerPartlist[k]["UlcerPart-95158-95189-94249"])!="")){
				cmlist=$getValue(UlcerPartlist[k]["UlcerPart-95158-95189-94247"])+"×"+$getValue(UlcerPartlist[k]["UlcerPart-95158-95189-94249"]);
				if(($getValue(UlcerPartlist[k]["UlcerPart-95158-95189-94251"])!="")){
					cmlist=$getValue(UlcerPartlist[k]["UlcerPart-95158-95189-94247"])+"×"+$getValue(UlcerPartlist[k]["UlcerPart-95158-95189-94249"])+"×"+$getValue(UlcerPartlist[k]["UlcerPart-95158-95189-94251"]);
				}
			}
		}
	...
	/* objSheet.Cells(17+length,3).value="发现日期："+$getValue(data["PipeFindDate"])+" "+$getValue(data["PipeFindTime"]); //发现日期
	objSheet.Cells(18+length,2).value="来源："+$getValue(data["PatAdmDate"]); //来源
	objSheet.Cells(19+length,2).value="部位："+$getValue(data["PatAdmDate"]); //部位
	objSheet.Cells(20+length,2).value="分期："+$getValue(data["PatAdmDate"]); //分期
	objSheet.Cells(21+length,1).value="面积："+$getValue(data["PatAdmDate"]); //面积
	 */
	...
	objSheet.Cells(31+length,1).value="报告单位："+$getValue(data["RepHospType"]); //报告单位
	...
	objSheet.Cells(31+length,5).value="联系电话："+$getValue(data["HospPhone"]); //联系电话
	...
	objSheet.Cells(32+length,1).value="报告日期："+$getValue(data["ReportDate"]); //报告日期
	...
	objSheet.Cells(33+length,1).value="填报人姓名："+$getValue(data["RepUserName"]); //填报人姓名
	xlApp.Range(xlApp.Cells(33+length,3),xlApp.Cells(33+length,4)).MergeCells = true; //合并单元格
	objSheet.Cells(33+length,3).value="职称："+$getValue(data["RepUserTitle"]); //职称
	...
	objSheet.Cells(33+length,5).value="工作年限："+$getValue(data["RepUserWorkYears"]); //工作年限
	...
	objSheet.Cells(35+length,1).value=$getValue(data["WLEventProcess"]); //事件经过
	...
	objSheet.Cells(39+length,1).value=$getValue(data["dealWay"]); //处理措施
...
function printFallDownData(data){
	...
	var xlBook = xlApp.Workbooks.Add(Template);
	var objSheet = xlBook.ActiveSheet;
	objSheet.Cells(4,3).value=$getValue(data["DisMedThingPatName"]); //姓名
	objSheet.Cells(4,6).value=$getValue(data["PatMedicalNo"]); //病案号
	objSheet.Cells(4,8).value=$getValue(data["OccuLoc"]); 		//发生科室
	objSheet.Cells(5,3).value=$getValue(data["PatSexinput"]);  //患者性别 
	objSheet.Cells(5,6).value="登记号："; //登记号
	objSheet.Cells(5,7).value="'"+$getValue(data["PatID"]); //登记号
	objSheet.Cells(6,3).value=$getValue(data["PatAge"]); 		//年龄
	objSheet.Cells(7,3).value=$getValue(data["PatDiag"]); 		//第一诊断
	...
	objSheet.Cells(9,3).value=$getValue(data["PatAdmDate"]); //入院日期
	objSheet.Cells(10,4).value=$getValue(data["PatAdmADLScore"]); //ADL得分
	...
	objSheet.Cells(14,4).value=$getValue(data["HappenTime"])+" "+$getValue(data["OccurTime"]); //发生时间
	...
	objSheet.Cells(17,3).value=$getValue(data["JuredPart"]); //受伤部位
	objSheet.Cells(18,3).value=$getValue(data["PatMedicalNo"]); //发生原因(可多选) 待做
	...
	objSheet.Cells(22,3).value=$getValue(data["OccurReason-95071"]); //其他因素
	...
	objSheet.Cells(25,4).value=$getValue(data["WLManWorkLife"]); //工作年限(年)
	...
	//objSheet.Cells(28,4).value=$getValue(data["FDResult-95134"])==""?"无":"有";
	...
	if (patHOD!=""){
		eventresult=$getValue(data["FDResult-95132"])+"（"+patHOD+"）"+"； "+eventresult;
	}	
	...
	objSheet.Cells(30,3).value=$getValue(data["RepHospType"]); //填报单位
	objSheet.Cells(30,8).value=$getValue(data["HospPhone"]); //联系电话
	objSheet.Cells(31,3).value=$getValue(data["ReportDate"]); //报告日期
	
	objSheet.Cells(32,3).value=$getValue(data["RepUserName"]); //填报人
	objSheet.Cells(32,6).value=$getValue(data["RepUserTitle"]); //职称
	objSheet.Cells(32,8).value=$getValue(data["RepUserWorkYears"]); //工作年限
	
	var WLErow=parseInt($getValue(data["WLEventProcess"]).length/40);
	var WLEheightlen=(WLErow+1)*14;
	objSheet.Rows(34+":"+34).RowHeight = WLEheightlen; 
	objSheet.Cells(34,2).value=$getValue(data["WLEventProcess"]); //事件经过
	var notice=radioValue("PatEventProcess-95021-95024,PatEventProcess-95021-95025,PatEventProcess-95021-95026,PatEventProcess-95021-95028,PatEventProcess-95021-95029,PatEventProcess-95021-95030,PatEventProcess-95021-95031",data); //立即通知
