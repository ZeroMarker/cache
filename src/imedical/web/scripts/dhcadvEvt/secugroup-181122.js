...
function addRow(){
	commonAddRow({'datagrid':'#datagrid',value:{'SECUActiveFlag':'Y','SECUHospDr':'2'}})
}
//双击编辑
		...
		{'GrpDr':Id},
		 function(data){
			 ...
			 for (var i=0;i<strArray.length-1;i++)
			 {
				...
				html+='<tr>';
				html+='<td style="width:30px;"><a id="'+trArray[0]+'" class="img icon-cancel" onclick="deleteUser(this.id)"></a></td>';
			    html+='<td style="width:50px;">'+trArray[1]+'</td>'
				...
				if(trArray[4]==""){
					...
					for (var j=0;j<GUWArray.length-1;j++)
					{
			        	...
			        	wardHtml+='	<span id="'+tdArray[0]+'" class="img icon-no" onclick="deleteWard(this.id)" style="float:left"></span><span class="warditm">'+tdArray[1]+'</span>';				
						wardHtml+='</div>';	
					}
				}
				html+=wardHtml
				...
				html+='<td style="width:30px;"><a id="'+trArray[3]+'" class="img icon-add" onclick="addWard(this.id)"></a></td>';
				...
				if(trArray[5]==""){
					...
					for (var j=0;j<GULArray.length-1;j++)
					{
			        	...
			        	LocHtml+='	<span id="'+tdArray[0]+'" class="img icon-no" onclick="deleteLoc(this.id)" style="float:left"></span><span class="Locitm">'+tdArray[1]+'</span>';				
				...
				html+=LocHtml
				html+='</td>'
				html+='<td style="width:30px;"><a id="'+trArray[3]+'" class="img icon-add" onclick="addLoc(this.id)"></a></td>';
				if(trArray[2]=="N"){
			        html+='<td style="width:30px;"><input id="'+trArray[0]+'" class="checkbox" type="checkbox" name="" value="Y" onclick="updateLeadFlag(this)"></input></td>';	 //hxy 2018-02-07
			    }else{
					html+='<td style="width:30px;"><input id="'+trArray[0]+'" class="checkbox" type="checkbox" name="" value="N" checked onclick="updateLeadFlag(this)"></input></td>';	 //hxy 2018-02-07
				}  
				html+='</tr>';
			 }//第一个for循环结束
	 	    // html+='</table>';
