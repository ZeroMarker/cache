...
function exportAccidentData(data,filePath,NameList){
	...
	for (i=0;i<strjLen;i++)
  	{
		objSheet.Cells(2+i,1).value=$getValue(data[i]["RepHospType"]); //填报单位
		objSheet.Cells(2+i,2).value="'"+"003903110105210111"; //机构登记号
		objSheet.Cells(2+i,3).value="'"+$getValue(data[i]["DeptLocOne"]); //填报科室1		
		objSheet.Cells(2+i,4).value="'"+$getValue(data[i]["DeptLocTwo"]); //填报科室2
		objSheet.Cells(2+i,5).value="'"+$getValue(data[i]["ReportDate"]); //报告日期	
		objSheet.Cells(2+i,6).value=$getValue(data[i]["DisMedThingPatName"]); //患者姓名
		objSheet.Cells(2+i,7).value=$getValue(data[i]["PatMedicalNo"]); //病案号	
		objSheet.Cells(2+i,8).value=$getValue(data[i]["OccuLoc"]); //发生科室		
		...
		objSheet.Cells(2+i,10).value=$getValue(data[i]["PatSexinput"]); //性别
		objSheet.Cells(2+i,11).value=$getValue(data[i]["PatAge"]); //年龄
		...
		objSheet.Cells(2+i,13).value="'"+$getValue(data[i]["PatDiag"]); //第一诊断		
		objSheet.Cells(2+i,14).value="'"+$getValue(data[i]["PatAdmDate"]); //入院日期  
		objSheet.Cells(2+i,15).value="'"+$getValue(data[i]["PatAdmADLScore"]); //入院时ADL得分
		...
		objSheet.Cells(2+i,20).value="'"+$getValue(data[i]["HappenTime"])+" "+$getValue(data[i]["OccurTime"]).split(":")[0]+"时"; //给药发生日期
		...
		if($getValue(data[i]["WLManWorkLife"])!=""){
			objSheet.Cells(2+i,25).value="'"+$getValue(data[i]["WLManWorkLife"])+"年"; //工作年限
		}	
		objSheet.Cells(2+i,26).value="'"+radioValue("AFResult-94565",data[i]); //事件造成的后果
		objSheet.Cells(2+i,27).value="'"+$getValue(data[i]["AFResult-94567"]); //事件造成的后果		
		if($getValue(data[i]["AFResult-94567"])!=""){
			objSheet.Cells(2+i,26).value="其他"
		}
		objSheet.Cells(2+i,28).value="'"+$getValue(data[i]["WLEventProcess"]); //事件经过
	}
...
function exportDrugErrData(data,filePath,NameList)
{
	...
	for (i=0;i<strjLen;i++)
	{
		objSheet.Cells(2+i,1).value=$getValue(data[i]["RepHospType"]); //填报单位
		objSheet.Cells(2+i,2).value="'"+"003903110105210111"; //机构登记号
		objSheet.Cells(2+i,3).value="'"+$getValue(data[i]["ReportDate"]); //报告日期
		objSheet.Cells(2+i,4).value="'"+$getValue(data[i]["DeptLocOne"]); //填报科室1		
		objSheet.Cells(2+i,5).value="'"+$getValue(data[i]["DeptLocTwo"]); //填报科室2
			
		objSheet.Cells(2+i,6).value=$getValue(data[i]["DisMedThingPatName"]); //患者姓名
		objSheet.Cells(2+i,7).value=$getValue(data[i]["PatMedicalNo"]); //病案号	
		objSheet.Cells(2+i,8).value=$getValue(data[i]["OccuLoc"]); //发生科室		
		objSheet.Cells(2+i,9).value=$getValue(data[i]["PatSexinput"]); //性别
		objSheet.Cells(2+i,10).value=$getValue(data[i]["PatAge"]); //年龄
		...
		objSheet.Cells(2+i,16).value="'"+$getValue(data[i]["GiveDrugHappenTime"]); //给药发生日期
		if($getValue(data[i]["OccurTime"])!=""){
			objSheet.Cells(2+i,16).value="'"+$getValue(data[i]["GiveDrugHappenTime"])+" "+$getValue(data["OccurTime"]).split(":")[0]+"时"; //给药发生日期
		}
		...
		if($getValue(data[i]["DrugUsePartyWorkYears"])!=""){
			objSheet.Cells(2+i,25).value="'"+$getValue(data[i]["DrugUsePartyWorkYears"])+"年"; //当事人工作年限
		}
		objSheet.Cells(2+i,19).value=$getValue(data[i]["Shift"]);  //班次
		...
		objSheet.Cells(2+i,22).value="'"+$getValue(data[i]["DrugUseErrType-94616"]); //给药对象错误	
		objSheet.Cells(2+i,23).value="'"+$getValue(data[i]["DrugUseErrType-94617"]); //给药时间错误
		objSheet.Cells(2+i,24).value="'"+$getValue(data[i]["DrugUseErrType-94618"]); //给药途径错误
		objSheet.Cells(2+i,25).value="'"+$getValue(data[i]["DrugUseErrType-94619"]); //遗漏给药	
		objSheet.Cells(2+i,26).value="'"+$getValue(data[i]["DrugUseErrType-94620"]); //输液速度错误
		objSheet.Cells(2+i,27).value="'"+$getValue(data[i]["DrugUseErrType-94621"]); //剂量错误		
		objSheet.Cells(2+i,28).value="'"+$getValue(data[i]["DrugUseErrType-94622"]); //剂型错误
		objSheet.Cells(2+i,29).value="'"+$getValue(data[i]["DrugUseErrType-94623"]); //药物错误
		objSheet.Cells(2+i,30).value="'"+$getValue(data[i]["DrugUseErrType-94624"]); //药物效期错误
		objSheet.Cells(2+i,31).value="'"+$getValue(data[i]["DrugUseErrType-94625"]); //其他
		...
		objSheet.Cells(2+i,35).value="'"+$getValue(data[i]["WLEventProcess"]); // 事件经过
...
function exportFallDownData(data,filePath,NameList)
{
	...
	for (i=0;i<strjLen;i++)
	{
		...
		objSheet.Cells(2+i,10).value=$getValue(data[i]["PatAge"]); //年龄
		objSheet.Cells(2+i,11).value="'"+radioValue("PatOrigin-label-94335,PatOrigin-label-94336,PatOrigin-label-94337,PatOrigin-label-94338,PatOrigin-label-94339",data[i]); //患者来源
		objSheet.Cells(2+i,12).value="'"+$getValue(data[i]["PatDiag"]); //第一诊断
		
		objSheet.Cells(2+i,13).value="'"+$getValue(data[i]["PatAdmDate"]); //入院日期	
		objSheet.Cells(2+i,14).value="'"+$getValue(data[i]["PatAdmADLScore"]); //ADL得分
		objSheet.Cells(2+i,15).value="'"+radioValue("NursingLev-94352,NursingLev-94353,NursingLev-94354,NursingLev-94355,NursingLev-94367",data[i]); //护理级别
		...
		objSheet.Cells(2+i,17).value="'"+$getValue(data[i]["HappenTime"]); //给药发生日期
		if($getValue(data[i]["OccurTime"])!=""){
			objSheet.Cells(2+i,17).value="'"+$getValue(data[i]["HappenTime"])+" "+$getValue(data[i]["OccurTime"]).split(":")[0]+"时"; //给药发生日期
		}
		...
		objSheet.Cells(2+i,21).value="'"+($getValue(data[i]["OccurReason-95068"])==""?"":"是"); //患者因素
		objSheet.Cells(2+i,22).value="'"+($getValue(data[i]["OccurReason-95068-95072"])==""?"":"是"); //意识障碍	
		objSheet.Cells(2+i,23).value="'"+($getValue(data[i]["OccurReason-95068-95073"])==""?"":"是"); //视力听力、障碍
		objSheet.Cells(2+i,24).value="'"+($getValue(data[i]["OccurReason-95068-95074"])==""?"":"是"); //活动障碍
		objSheet.Cells(2+i,25).value="'"+($getValue(data[i]["OccurReason-95068-95085"])==""?"":"是"); //有跌倒史	
		objSheet.Cells(2+i,26).value="'"+($getValue(data[i]["OccurReason-95068-95088"])==""?"":"是"); //疾病
		...
		objSheet.Cells(2+i,29).value="'"+($getValue(data[i]["OccurReason-95069"])==""?"":"是"); //药物因素
		objSheet.Cells(2+i,30).value="'"+($getValue(data[i]["OccurReason-95069-95094"])==""?"":"是"); //散瞳剂	
		objSheet.Cells(2+i,31).value="'"+($getValue(data[i]["OccurReason-95069-95095"])==""?"":"是"); //镇静安眠剂
		objSheet.Cells(2+i,32).value="'"+($getValue(data[i]["OccurReason-95069-95098"])==""?"":"是"); //降压利尿剂
		objSheet.Cells(2+i,33).value="'"+($getValue(data[i]["OccurReason-95069-95099"])==""?"":"是"); //降糖剂	
		objSheet.Cells(2+i,34).value="'"+($getValue(data[i]["OccurReason-95069-95102"])==""?"":"是"); //镇挛抗癫剂
		objSheet.Cells(2+i,35).value="'"+($getValue(data[i]["OccurReason-95069-95103"])==""?"":"是"); //麻醉止痛剂	
		objSheet.Cells(2+i,36).value="'"+($getValue(data[i]["OccurReason-95069-95104"])==""?"":"是"); //泻药
		...
		objSheet.Cells(2+i,39).value="'"+($getValue(data[i]["OccurReason-95070"])==""?"":"是"); //管理因素
		objSheet.Cells(2+i,40).value="'"+($getValue(data[i]["OccurReason-95070-95106"])==""?"":"是"); //环境因素	
		objSheet.Cells(2+i,41).value="'"+($getValue(data[i]["OccurReason-95070-95112"])==""?"":"是"); //设备设施缺陷或故障
		objSheet.Cells(2+i,42).value="'"+($getValue(data[i]["OccurReason-95070-95113"])==""?"":"是"); //宣教不到位
		objSheet.Cells(2+i,43).value="'"+($getValue(data[i]["OccurReason-95070-95114"])==""?"":"是"); //管理不到位	
		objSheet.Cells(2+i,44).value="'"+($getValue(data[i]["OccurReason-95070-95121"])==""?"":"是"); //培训不到位
		...
		objSheet.Cells(2+i,47).value="'"+$getValue(data[i]["OccurReason-95071"]); //其他因素	
		...
		if($getValue(data[i]["WLManWorkLife"])!=""){
			objSheet.Cells(2+i,51).value="'"+$getValue(data[i]["WLManWorkLife"])+"年"; //工作年限
		}
		...
		objSheet.Cells(2+i,54).value="'"+$getValue(data[i]["JuredPart"]); //受伤部位		
		...
		objSheet.Cells(2+i,57).value="'"+$getValue(data[i]["WLEventProcess"]); //事件经过
...
function exportPipeOffData(data,filePath,NameList)
{
	...
	for (i=0;i<strjLen;i++)
	{
		...
		objSheet.Cells(2+i,14).value="'"+$getValue(data[i]["PatAdmDate"]); //入院日期	
		objSheet.Cells(2+i,15).value="'"+$getValue(data[i]["PatAdmADLScore"]); //ADL得分
		objSheet.Cells(2+i,16).value="'"+radioValue("NursingLev-94352,NursingLev-94353,NursingLev-94354,NursingLev-94355,NursingLev-94367",data[i]); //护理级别
		objSheet.Cells(2+i,17).value=radioValue("PatSelfCareAbility-94345,PatSelfCareAbility-94346,PatSelfCareAbility-94347",data[i]); //患者自我照顾能力
		
		objSheet.Cells(2+i,18).value="'"+($getValue(data[i]["PipeType-94449"])==""?"":"是"); //胃管
		objSheet.Cells(2+i,19).value="'"+($getValue(data[i]["PipeType-94450"])==""?"":"是"); //尿管	
		objSheet.Cells(2+i,20).value="'"+($getValue(data[i]["PipeType-94451"])==""?"":"是"); //透析管路
		objSheet.Cells(2+i,21).value="'"+($getValue(data[i]["PipeType-94452"])==""?"":"是"); //气管插管
		objSheet.Cells(2+i,22).value="'"+($getValue(data[i]["PipeType-94453"])==""?"":"是"); //气管切开套管	
		objSheet.Cells(2+i,23).value="'"+($getValue(data[i]["PipeType-94454"])==""?"":"是"); //鼻饲管
		objSheet.Cells(2+i,24).value="'"+($getValue(data[i]["PipeType-94455"])==""?"":"是"); //动脉置管
		objSheet.Cells(2+i,25).value="'"+($getValue(data[i]["PipeType-94456"])==""?"":"是"); //深静脉置管	
		objSheet.Cells(2+i,26).value="'"+($getValue(data[i]["PipeType-94457"])==""?"":"是"); //PICC
		objSheet.Cells(2+i,27).value="'"+($getValue(data[i]["PipeType-94458"])==""?"":"是"); //胸腔闭式引流管
		objSheet.Cells(2+i,28).value="'"+($getValue(data[i]["PipeType-94459"])==""?"":"是"); //腹腔引流管
		objSheet.Cells(2+i,29).value="'"+($getValue(data[i]["PipeType-94460"])==""?"":"是"); //伤口引流管	
		objSheet.Cells(2+i,30).value="'"+($getValue(data[i]["PipeType-94461"])==""?"":"是"); //心包引流管
		objSheet.Cells(2+i,31).value="'"+($getValue(data[i]["PipeType-94462"])==""?"":"是"); //脑室引流管
		...
		objSheet.Cells(2+i,34).value="'"+$getValue(data[i]["PipeFindDate"]); //脱管发生时间
		if($getValue(data[i]["OccurTime"])!=""){
			objSheet.Cells(2+i,17).value="'"+$getValue(data[i]["PipeFindDate"])+" "+$getValue(data[i]["OccurTime"]).split(":")[0]+"时"; //脱管发生时间
		}
		
		objSheet.Cells(2+i,35).value="'"+$getValue(data[i]["TubeDate"]); //置管日期	
		...
		objSheet.Cells(2+i,39).value="'"+$getValue(data[i]["WallWorkYears"]); //工作年限
		if($getValue(data[i]["WallWorkYears"])!=""){
			objSheet.Cells(2+i,51).value="'"+$getValue(data[i]["WallWorkYears"])+"年"; //工作年限
		}
		...
		objSheet.Cells(2+i,43).value="'"+$getValue(data[i]["PipePS-94475-94490"]); //其他活动能力
		...
		objSheet.Cells(2+i,57).value="'"+($getValue(data[i]["PipeComplication-94540-94542"])==""?"":"是"); //气栓
		objSheet.Cells(2+i,58).value="'"+($getValue(data[i]["PipeComplication-94540-94543"])==""?"":"是"); //血栓
		objSheet.Cells(2+i,59).value="'"+($getValue(data[i]["PipeComplication-94540-94544"])==""?"":"是"); //窒息	
		objSheet.Cells(2+i,60).value="'"+($getValue(data[i]["PipeComplication-94540-94545"])==""?"":"是"); //感染
		objSheet.Cells(2+i,61).value="'"+($getValue(data[i]["PipeComplication-94540-94546"])==""?"":"是"); //气胸
		objSheet.Cells(2+i,62).value="'"+($getValue(data[i]["PipeComplication-94540-94547"])==""?"":"是"); //吻合口瘘
		...
		objSheet.Cells(2+i,65).value="'"+($getValue(data[i]["PipeTakeSteps-94533"])==""?"":"是"); //重新置管  
		objSheet.Cells(2+i,66).value="'"+($getValue(data[i]["PipeTakeSteps-94534"])==""?"":"是"); //脱管部位处理
		objSheet.Cells(2+i,67).value="'"+($getValue(data[i]["PipeTakeSteps-94536"])==""?"":"是"); //诊断性检查
		...
		objSheet.Cells(2+i,70).value="'"+$getValue(data[i]["WLEventProcess"]); //事件经过
...
function exportSkinUlcerData(data,filePath,NameList)
{
	...
	for (i=0;i<strjLen;i++)
	{
		objSheet.Cells(3+i,1).value=$getValue(data[i]["RepHospType"]); //填报单位
		objSheet.Cells(3+i,2).value="'"+"003903110105210111"; //机构登记号
		objSheet.Cells(3+i,3).value="'"+$getValue(data[i]["ReportDate"]); //报告日期
		objSheet.Cells(3+i,4).value="'"+$getValue(data[i]["DeptLocOne"]); //填报科室1		
		objSheet.Cells(3+i,5).value="'"+$getValue(data[i]["DeptLocTwo"]); //填报科室2
			
		objSheet.Cells(3+i,6).value=$getValue(data[i]["DisMedThingPatName"]); //患者姓名
		objSheet.Cells(3+i,7).value=$getValue(data[i]["PatMedicalNo"]); //病案号	
		objSheet.Cells(3+i,8).value=$getValue(data[i]["OccuLoc"]); //发生科室		
		objSheet.Cells(3+i,9).value=$getValue(data[i]["PatSexinput"]); //性别
		objSheet.Cells(3+i,10).value=$getValue(data[i]["PatAge"]); //年龄
		...
		objSheet.Cells(3+i,12).value="'"+$getValue(data[i]["PatDiag"]).split(",")[0]; //第一诊断
		
		objSheet.Cells(3+i,13).value="'"+$getValue(data[i]["PatAdmDate"]); //入院日期	
		objSheet.Cells(3+i,14).value="'"+$getValue(data[i]["PatAdmADLScore"]); //ADL得分
		...
		objSheet.Cells(3+i,19).value="'"+$getValue(data[i]["OccurUlcerRiskScore"]); //压疮风险评分
		...
		var UlcerPartlist=$getValue(data[i]["UlcerPart"]);//压疮部位
		...
		for(var k=0;k<Ulcerlen;k++){
			...
			objSheet.Cells(3+i,23+32*k).value="'"+$getValue(UlcerPartlist[k]["UlcerPart-95158-95162-95192"]); //压疮发现日期
			objSheet.Cells(3+i,24+32*k).value="'"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95172"])==""?"":"是"); //部位_枕部
			objSheet.Cells(3+i,25+32*k).value="'"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95178"])==""?"":"是"); //部位_骶尾部	
			objSheet.Cells(3+i,26+32*k).value="'"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95173"])==""?"":"是"); //部位_耳廓
			objSheet.Cells(3+i,27+32*k).value="'"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95173-95196"])==""?"":"是"); //耳廓_左
			objSheet.Cells(3+i,28+32*k).value="'"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95173-95197"])==""?"":"是"); //耳廓_右
			objSheet.Cells(3+i,29+32*k).value="'"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95179"])==""?"":"是"); //部位_膝部	
			objSheet.Cells(3+i,30+32*k).value="'"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95179-94189"])==""?"":"是"); //膝部_左
			objSheet.Cells(3+i,31+32*k).value="'"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95179-94190"])==""?"":"是"); //膝部_右
			objSheet.Cells(3+i,32+32*k).value="'"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95174"])==""?"":"是"); //部位_肩胛部
			objSheet.Cells(3+i,33+32*k).value="'"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95174-94173"])==""?"":"是"); //肩胛部_左
			objSheet.Cells(3+i,34+32*k).value="'"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95174-94174"])==""?"":"是"); //肩胛部_右
			objSheet.Cells(3+i,35+32*k).value="'"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95180"])==""?"":"是"); //部位_踝部
			objSheet.Cells(3+i,36+32*k).value="'"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95180-94193"])==""?"":"是"); //踝部_左
			objSheet.Cells(3+i,37+32*k).value="'"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95180-94194"])==""?"":"是"); //踝部_右
			objSheet.Cells(3+i,38+32*k).value="'"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95175"])==""?"":"是"); //部位_肘部
			objSheet.Cells(3+i,39+32*k).value="'"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95175-94177"])==""?"":"是"); //肘部_左
			objSheet.Cells(3+i,40+32*k).value="'"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95175-94178"])==""?"":"是"); //肘部_右
			objSheet.Cells(3+i,41+32*k).value="'"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95181"])==""?"":"是"); //部位_足跟部
			objSheet.Cells(3+i,42+32*k).value="'"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95181-94197"])==""?"":"是"); //足跟部_左
			objSheet.Cells(3+i,43+32*k).value="'"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95181-94198"])==""?"":"是"); //足跟部_右
			objSheet.Cells(3+i,44+32*k).value="'"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95176"])==""?"":"是"); //部位_髂前上棘
			objSheet.Cells(3+i,45+32*k).value="'"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95176-94181"])==""?"":"是"); //髂前上棘_左
			objSheet.Cells(3+i,46+32*k).value="'"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95176-94182"])==""?"":"是"); //髂前上棘_右
			objSheet.Cells(3+i,47+32*k).value="'"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95177"])==""?"":"是"); //部位_髋部
			objSheet.Cells(3+i,48+32*k).value="'"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95177-94185"])==""?"":"是"); //髋部_左
			objSheet.Cells(3+i,49+32*k).value="'"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95177-94186"])==""?"":"是"); //髋部_右
		...
		objSheet.Cells(3+i,151).value="'"+($getValue(data[i]["UlcerOccurReason-94948-94952"])==""?"":"是"); //卧床
		objSheet.Cells(3+i,152).value="'"+($getValue(data[i]["UlcerOccurReason-94948-94953"])==""?"":"是"); //制动
		objSheet.Cells(3+i,153).value="'"+($getValue(data[i]["UlcerOccurReason-94948-94954"])==""?"":"是"); //强迫体位	
		objSheet.Cells(3+i,154).value="'"+($getValue(data[i]["UlcerOccurReason-94948-94955"])==""?"":"是"); //肥胖
		objSheet.Cells(3+i,155).value="'"+($getValue(data[i]["UlcerOccurReason-94948-94956"])==""?"":"是"); //大小便失禁
		objSheet.Cells(3+i,156).value="'"+($getValue(data[i]["UlcerOccurReason-94948-94957"])==""?"":"是"); //浮肿
		objSheet.Cells(3+i,157).value="'"+($getValue(data[i]["UlcerOccurReason-94948-94958"])==""?"":"是"); //消瘦	
		...
		objSheet.Cells(3+i,161).value="'"+($getValue(data[i]["UlcerOccurReason-94949-94960"])==""?"":"是"); //低蛋白血症
		objSheet.Cells(3+i,162).value="'"+($getValue(data[i]["UlcerOccurReason-94949-94961"])==""?"":"是"); //贫血
		objSheet.Cells(3+i,163).value="'"+($getValue(data[i]["UlcerOccurReason-94949-94962"])==""?"":"是"); //昏迷	
		objSheet.Cells(3+i,164).value="'"+($getValue(data[i]["UlcerOccurReason-94949-94963"])==""?"":"是"); //感觉受损
		...
		objSheet.Cells(3+i,168).value="'"+($getValue(data[i]["UlcerOccurReason-94950-94966"])==""?"":"是"); //未按时翻身
		objSheet.Cells(3+i,169).value="'"+($getValue(data[i]["UlcerOccurReason-94950-94967"])==""?"":"是"); //未及时清洁、擦洗皮肤
		objSheet.Cells(3+i,170).value="'"+($getValue(data[i]["UlcerOccurReason-94950-94968"])==""?"":"是"); //床单潮湿、不洁、褶皱	
		objSheet.Cells(3+i,171).value="'"+($getValue(data[i]["UlcerOccurReason-94950-94969"])==""?"":"是"); //管路较长时间受压而未发现
		objSheet.Cells(3+i,172).value="'"+($getValue(data[i]["UlcerOccurReason-94950-94970"])==""?"":"是"); //管路固定不当
		objSheet.Cells(3+i,173).value="'"+($getValue(data[i]["UlcerOccurReason-94950-94971"])==""?"":"是"); //护理操作不当，拖、拉、扯、拽等
		objSheet.Cells(3+i,174).value="'"+($getValue(data[i]["UlcerOccurReason-94950-94972"])==""?"":"是"); //护理人员评估不当	
		objSheet.Cells(3+i,175).value="'"+($getValue(data[i]["UlcerOccurReason-94950-94973"])==""?"":"是"); //器具使用不当
		...
		objSheet.Cells(3+i,179).value="'"+($getValue(data[i]["UlcerOccurReason-94951-94975"])==""?"":"是"); //护理人员配备不足
		...
		objSheet.Cells(3+i,182).value="'"+($getValue(data[i]["AdoptNursMeasure-95006"])==""?"":"是"); //增加翻身频次
		objSheet.Cells(3+i,183).value="'"+($getValue(data[i]["AdoptNursMeasure-95007"])==""?"":"是"); //保持皮肤清洁
		objSheet.Cells(3+i,184).value="'"+($getValue(data[i]["AdoptNursMeasure-95008"])==""?"":"是"); //保持床单清洁干燥平整	
		objSheet.Cells(3+i,185).value="'"+($getValue(data[i]["AdoptNursMeasure-95009"])==""?"":"是"); //使用防压疮气垫
		objSheet.Cells(3+i,186).value="'"+($getValue(data[i]["AdoptNursMeasure-95010"])==""?"":"是"); //使用软垫垫于骨隆突部位
		objSheet.Cells(3+i,187).value="'"+($getValue(data[i]["AdoptNursMeasure-95011"])==""?"":"是"); //应用医疗仪器治疗创面
		objSheet.Cells(3+i,188).value="'"+($getValue(data[i]["AdoptNursMeasure-95012"])==""?"":"是"); //贴膜保护受压部位皮肤	
		objSheet.Cells(3+i,189).value="'"+($getValue(data[i]["AdoptNursMeasure-95013"])==""?"":"是"); //伤口换药	
		...
		for(var k=4;k<UlcerPartlist.length;k++){
			...
			var ekpart=checksubValue($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95173"]),"UlcerPart-95158-95166-95173-95196,UlcerPart-95158-95166-95173-95197",UlcerPartlist[k]); //耳廓
			if (ekpart!=""){part=ekpart+"； "+part; }
			var jjpart=checksubValue($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95174"]),"UlcerPart-95158-95166-95174-94173,UlcerPart-95158-95166-95174-94174",UlcerPartlist[k]); //肩胛部
			if (jjpart!=""){part=jjpart+"； "+part; }
			var zbpart=checksubValue($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95175"]),"UlcerPart-95158-95166-95175-94177,UlcerPart-95158-95166-95175-94178",UlcerPartlist[k]); //肘部
			if (zbpart!=""){part=zbpart+"； "+part; }
			var qqsjpart=checksubValue($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95176"]),"UlcerPart-95158-95166-95175-94177,UlcerPart-95158-95166-95175-94178",UlcerPartlist[k]); //髂前上棘
			if (qqsjpart!=""){part=qqsjpart+"； "+part; }
			var kbpart=checksubValue($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95177"]),"UlcerPart-95158-95166-95177-94185,UlcerPart-95158-95166-95177-94186",UlcerPartlist[k]); //髋部
			if (kbpart!=""){part=kbpart+"； "+part; }
			var xbpart=checksubValue($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95179"]),"UlcerPart-95158-95166-95179-94189,UlcerPart-95158-95166-95179-94190",UlcerPartlist[k]); //膝部
			if (xbpart!=""){part=xbpart+"； "+part; }
			var hbpart=checksubValue($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95180"]),"UlcerPart-95158-95166-95180-94193,UlcerPart-95158-95166-95180-94194",UlcerPartlist[k]); //踝部
			if (hbpart!=""){part=hbpart+"； "+part; }
			var zgbpart=checksubValue($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95181"]),"UlcerPart-95158-95166-95181-94197,UlcerPart-95158-95166-95181-94198",UlcerPartlist[k]); //足跟部
			if (zgbpart!=""){part=zgbpart+"； "+part; }
			if (UlcerOthlist=""){
				UlcerOthlist="部位"+(k+1)+"（发现日期："+$getValue(UlcerPartlist[k]["UlcerPart-95158-95162-95192"])+"来源："+orign+"部位："+part+"分期："+radioValue("UlcerPart-95158-95169-95183,UlcerPart-95158-95169-95184,UlcerPart-95158-95169-95185,UlcerPart-95158-95169-95186,UlcerPart-95158-95169-95187,UlcerPart-95158-95169-95188",UlcerPartlist[k])+"面积(长宽深 cm×cm×cm)："+$getValue(UlcerPartlist[k]["UlcerPart-95158-95189-94247"])+"×"+$getValue(UlcerPartlist[k]["UlcerPart-95158-95189-94249"])+"×"+$getValue(UlcerPartlist[k]["UlcerPart-95158-95189-94251"])+"）" ;
			}else{
				UlcerOthlist=UlcerOthlist+"；"+"部位"+(k+1)+"（发现日期："+$getValue(UlcerPartlist[k]["UlcerPart-95158-95162-95192"])+"来源："+orign+"部位："+part+"分期："+radioValue("UlcerPart-95158-95169-95183,UlcerPart-95158-95169-95184,UlcerPart-95158-95169-95185,UlcerPart-95158-95169-95186,UlcerPart-95158-95169-95187,UlcerPart-95158-95169-95188",UlcerPartlist[k])+"面积(长宽深 cm×cm×cm)："+$getValue(UlcerPartlist[k]["UlcerPart-95158-95189-94247"])+"×"+$getValue(UlcerPartlist[k]["UlcerPart-95158-95189-94249"])+"×"+$getValue(UlcerPartlist[k]["UlcerPart-95158-95189-94251"])+"）" ;
			}
		}
		objSheet.Cells(3+i,192).value="'"+$getValue(data[i]["WLEventProcess"])+UlcerOthlist; //事件经过(详细描述)	
	...
	for (i=1;i<strjLen;i++)
  	{
		...
		for (j=0;j<titlelen;j++){
			Str = Str +"xlApp.Range(xlApp.Cells(1,1),xlApp.Cells(1,"+titlelen+")).MergeCells = true;" ; //合并单元格
			Str = Str +"objSheet.Cells(1,1).value='报告信息';";
		...
		if(((RepType.indexOf("压力性损伤")>0)||(RepType.indexOf("压力性损伤")==0))&&(tbstrjLen>0)) 
	  	{ 
			...
			for (j=0;j<tbtitlelen;j++){ 
				...
				Str = Str +"objSheet.Cells(2,"+(1+titlelen+3)+").value='部位_骶尾部';";
				Str = Str +"objSheet.Cells(2,"+(1+titlelen+4)+").value='部位_耳廓_左';";
				Str = Str +"objSheet.Cells(2,"+(1+titlelen+5)+").value='部位_耳廓_右';";
				Str = Str +"objSheet.Cells(2,"+(1+titlelen+6)+").value='部位_膝部_左';";
				Str = Str +"objSheet.Cells(2,"+(1+titlelen+7)+").value='部位_膝部_右';";
				Str = Str +"objSheet.Cells(2,"+(1+titlelen+8)+").value='部位_肩胛部_左';";
				Str = Str +"objSheet.Cells(2,"+(1+titlelen+9)+").value='部位_肩胛部_右';";
				Str = Str +"objSheet.Cells(2,"+(1+titlelen+10)+").value='部位_踝部_左';";
				Str = Str +"objSheet.Cells(2,"+(1+titlelen+11)+").value='部位_踝部_右';";
				Str = Str +"objSheet.Cells(2,"+(1+titlelen+12)+").value='部位_肘部_左';";
				Str = Str +"objSheet.Cells(2,"+(1+titlelen+13)+").value='部位_肘部_右';";
				Str = Str +"objSheet.Cells(2,"+(1+titlelen+14)+").value='部位_足跟部_左';";
				Str = Str +"objSheet.Cells(2,"+(1+titlelen+15)+").value='部位_足跟部_右';";
				Str = Str +"objSheet.Cells(2,"+(1+titlelen+16)+").value='部位_髂前上棘_左';";
				Str = Str +"objSheet.Cells(2,"+(1+titlelen+17)+").value='部位_髂前上棘_右';";
				Str = Str +"objSheet.Cells(2,"+(1+titlelen+18)+").value='部位_髋部_左';";
				Str = Str +"objSheet.Cells(2,"+(1+titlelen+19)+").value='部位_髋部_右';";
				Str = Str +"objSheet.Cells(2,"+(1+titlelen+20)+").value='其它_内容';"; 
				Str = Str +"objSheet.Cells(2,"+(1+titlelen+21)+").value='Ⅰ期';";
				Str = Str +"objSheet.Cells(2,"+(1+titlelen+22)+").value='Ⅱ期';";
				Str = Str +"objSheet.Cells(2,"+(1+titlelen+23)+").value='Ⅲ期';";
				Str = Str +"objSheet.Cells(2,"+(1+titlelen+24)+").value='Ⅳ期';";
				Str = Str +"objSheet.Cells(2,"+(1+titlelen+25)+").value='可疑深度组织损伤';";
				Str = Str +"objSheet.Cells(2,"+(1+titlelen+26)+").value='难以分期';";
				Str = Str +"objSheet.Cells(2,"+(1+titlelen+27)+").value='压疮面积长';";
				Str = Str +"objSheet.Cells(2,"+(1+titlelen+28)+").value='压疮面积宽';";
				Str = Str +"objSheet.Cells(2,"+(1+titlelen+29)+").value='压疮面积深';";
				
				for(var k=0;k<Ulcerlen;k++){
					Str = Str +"objSheet.Cells("+(3+len+k)+","+(1+titlelen+0)+").value='"+radioValue("UlcerPart-95158-95163-95170,UlcerPart-95158-95163-95171",UlcerPartlist[k])+"';" ;//压疮来源	
					Str = Str +"objSheet.Cells("+(3+len+k)+","+(1+titlelen+1)+").value='"+$getValue(UlcerPartlist[k]["UlcerPart-95158-95162-95192"])+"';" ; //压疮发现日期
					Str = Str +"objSheet.Cells("+(3+len+k)+","+(1+titlelen+2)+").value='"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95172"])==""?"0":"1")+"';"; //部位_枕部
					Str = Str +"objSheet.Cells("+(3+len+k)+","+(1+titlelen+3)+").value='"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95178"])==""?"0":"1")+"';"; //部位_骶尾部	
					Str = Str +"objSheet.Cells("+(3+len+k)+","+(1+titlelen+4)+").value='"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95173-95196"])==""?"0":"1")+"';"; //耳廓_左
					Str = Str +"objSheet.Cells("+(3+len+k)+","+(1+titlelen+5)+").value='"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95173-95197"])==""?"0":"1")+"';"; //耳廓_右
					Str = Str +"objSheet.Cells("+(3+len+k)+","+(1+titlelen+6)+").value='"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95179-94189"])==""?"0":"1")+"';"; //膝部_左
					Str = Str +"objSheet.Cells("+(3+len+k)+","+(1+titlelen+7)+").value='"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95179-94190"])==""?"0":"1")+"';"; //膝部_右
					Str = Str +"objSheet.Cells("+(3+len+k)+","+(1+titlelen+8)+").value='"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95174-94173"])==""?"0":"1")+"';"; //肩胛部_左
					Str = Str +"objSheet.Cells("+(3+len+k)+","+(1+titlelen+9)+").value='"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95174-94174"])==""?"0":"1")+"';"; //肩胛部_右
					Str = Str +"objSheet.Cells("+(3+len+k)+","+(1+titlelen+10)+").value='"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95180-94193"])==""?"0":"1")+"';"; //踝部_左
					Str = Str +"objSheet.Cells("+(3+len+k)+","+(1+titlelen+11)+").value='"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95180-94194"])==""?"0":"1")+"';"; //踝部_右
					Str = Str +"objSheet.Cells("+(3+len+k)+","+(1+titlelen+12)+").value='"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95175-94177"])==""?"0":"1")+"';"; //肘部_左
					Str = Str +"objSheet.Cells("+(3+len+k)+","+(1+titlelen+13)+").value='"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95175-94178"])==""?"0":"1")+"';"; //肘部_右
					Str = Str +"objSheet.Cells("+(3+len+k)+","+(1+titlelen+14)+").value='"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95181-94197"])==""?"0":"1")+"';"; //足跟部_左
					Str = Str +"objSheet.Cells("+(3+len+k)+","+(1+titlelen+15)+").value='"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95181-94198"])==""?"0":"1")+"';"; //足跟部_右
					Str = Str +"objSheet.Cells("+(3+len+k)+","+(1+titlelen+16)+").value='"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95176-94181"])==""?"0":"1")+"';"; //髂前上棘_左
					Str = Str +"objSheet.Cells("+(3+len+k)+","+(1+titlelen+17)+").value='"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95176-94182"])==""?"0":"1")+"';"; //髂前上棘_右
					Str = Str +"objSheet.Cells("+(3+len+k)+","+(1+titlelen+18)+").value='"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95177-94185"])==""?"0":"1")+"';"; //髋部_左
					Str = Str +"objSheet.Cells("+(3+len+k)+","+(1+titlelen+19)+").value='"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95177-94186"])==""?"0":"1")+"';"; //髋部_右
					Str = Str +"objSheet.Cells("+(3+len+k)+","+(1+titlelen+20)+").value='"+$getValue(UlcerPartlist[k]["UlcerPart-95158-95166-95182"])+"';"; //其它_内容
					
					Str = Str +"objSheet.Cells("+(3+len+k)+","+(1+titlelen+21)+").value='"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95169-95183"])==""?"0":"1")+"';"; //Ⅰ期
					Str = Str +"objSheet.Cells("+(3+len+k)+","+(1+titlelen+22)+").value='"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95169-95184"])==""?"0":"1")+"';"; //Ⅱ期
					Str = Str +"objSheet.Cells("+(3+len+k)+","+(1+titlelen+23)+").value='"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95169-95185"])==""?"0":"1")+"';"; //Ⅲ期
					Str = Str +"objSheet.Cells("+(3+len+k)+","+(1+titlelen+24)+").value='"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95169-95186"])==""?"0":"1")+"';"; //Ⅳ期
					Str = Str +"objSheet.Cells("+(3+len+k)+","+(1+titlelen+25)+").value='"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95169-95187"])==""?"0":"1")+"';"; //可疑深度组织损伤
					Str = Str +"objSheet.Cells("+(3+len+k)+","+(1+titlelen+26)+").value='"+($getValue(UlcerPartlist[k]["UlcerPart-95158-95169-95188"])==""?"0":"1")+"';"; //难以分期
					Str = Str +"objSheet.Cells("+(3+len+k)+","+(1+titlelen+27)+").value='"+$getValue(UlcerPartlist[k]["UlcerPart-95158-95189-94247"])+"';";  //压疮面积长 
					Str = Str +"objSheet.Cells("+(3+len+k)+","+(1+titlelen+28)+").value='"+$getValue(UlcerPartlist[k]["UlcerPart-95158-95189-94249"])+"';";  //压疮面积宽
					Str = Str +"objSheet.Cells("+(3+len+k)+","+(1+titlelen+29)+").value='"+$getValue(UlcerPartlist[k]["UlcerPart-95158-95189-94251"])+"';";  //压疮面积深
...
{ 
	...
	Str=Str+"objSheet.cells(1,1).value='不良事件统计';" ;
	Str=Str+"objSheet.cells(2,1).value='患者姓名';" ;
	Str=Str+"objSheet.cells(2,2).value='患者性别';" ;
   	Str=Str+"objSheet.cells(2,3).value='患者年龄';" ;
    Str=Str+"objSheet.cells(2,4).value='病案号';" ;
    Str=Str+"objSheet.cells(2,5).value='登记号';" ;
    Str=Str+"objSheet.cells(2,6).value='第一诊断';" ;
    Str=Str+"objSheet.cells(2,7).value='入院日期';" ;
    Str=Str+"objSheet.cells(2,8).value='出院日期';" ;
    Str=Str+"objSheet.cells(2,9).value='护理级别';" ;
    Str=Str+"objSheet.cells(2,10).value='文化程度';" ;
    Str=Str+"objSheet.cells(2,11).value='科室';" ;
    Str=Str+"objSheet.cells(2,12).value='报告日期';" ;
    Str=Str+"objSheet.cells(2,13).value='发生时间';" ;
    Str=Str+"objSheet.cells(2,14).value='报告状态';" ;
    Str=Str+"objSheet.cells(2,15).value='病房';" ;
    Str=Str+"objSheet.cells(2,16).value='报告类型';" ;
    Str=Str+"xlApp.Range(xlApp.Cells(2,1),xlApp.Cells("+strjLen+2+",16)).Borders.LineStyle=1;" ; 	//设置边框
	Str=Str+"xlApp.Range(xlApp.Cells(2,1),xlApp.Cells("+strjLen+2+",16)).WrapText=true;";	 //自动换行
	...
	{		   
	    ...
	    "objSheet.cells("+(i+2)+",7).value='"+strjData[i-1].PatAdmDate+"';"+ 		//入院日期
	    "objSheet.cells("+(i+2)+",8).value='"+strjData[i-1].DischgDate+"';"+  		//出院日期
	    "objSheet.cells("+(i+2)+",9).value='"+strjData[i-1].NurLev+"';"+  			//护理级别
	    "objSheet.cells("+(i+2)+",10).value='"+strjData[i-1].DegreeEducate+"';"+  	//文化程度
	    "objSheet.cells("+(i+2)+",11).value='"+strjData[i-1].RepLoc+"';"+ 			//报告科室
	    //objSheet.cells(i+2,12)="'"+strjData[i-1].LocDep;  					//报告大科室
	    "objSheet.cells("+(i+2)+",12).value='"+strjData[i-1].RepDate+"';"+  		//报告日期
	    "objSheet.cells("+(i+2)+",13).value='"+strjData[i-1].OccurDate+"';"+		//发生日期
	    "objSheet.cells("+(i+2)+",14).value='"+strjData[i-1].RepStaus+"';"+			//报告状态
	    "objSheet.cells("+(i+2)+",15).value='"+strjData[i-1].OccurLoc+"';"+		//发生科室
	    //objSheet.cells(i+2,16)="'"; //发布共享
