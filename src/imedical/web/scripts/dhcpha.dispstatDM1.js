...
function PrnHX()
{
    ...
    {   
		...
		xlsheet.Cells(2,14)=t['PRINTUSER']+gusername
		...
		for (var row=1;row<=cnt;row++)
		{
			...
			xlsheet.Cells(startrow-1+row,3)=""
			xlsheet.Cells(startrow-1+row,4)=regno
			xlsheet.Cells(startrow-1+row,5)=name
			xlsheet.Cells(startrow-1+row,6)=sex
			xlsheet.Cells(startrow-1+row,7)=age
			xlsheet.Cells(startrow-1+row,8)=personid
			xlsheet.Cells(startrow-1+row,9)=desc
			xlsheet.Cells(startrow-1+row,10)=barcode
			xlsheet.Cells(startrow-1+row,11)=uom
			xlsheet.Cells(startrow-1+row,12)=qty
			xlsheet.Cells(startrow-1+row,13)=doctor
			xlsheet.Cells(startrow-1+row,14)=disp //调剂人与发药人一致
			xlsheet.Cells(startrow-1+row,15)=disp //发药人
