//$.haiExcel 在Base.js中定义

$.haiExcel.DataToExcelTool = function(){
	this.ExcelApp=$.haiExcel.ExcelApp;
...
function ExprotGrid(argGrid,argFileName,argIsChecked)
{
	...
	this.BuildHeader(argGrid.headers,1,1);
...
function ExprotGridNew(grid,argFileName) {
	...
	var objExcelTool=$.haiExcel.DataToExcelTool;
...
function BuildHeader(headers,row,col)
{
	var n = 0;
    for(var i=0;i<headers.length;++i)
    {
        ...
        }else if (cfg.id=='expander') //Add By LiYang 2011-12-08 将监控结果导出
        {
        	...
        	cfg.header = ReplaceText(cfg.header, "<br/>", "\n");
        	cfg.header = ReplaceText(cfg.header, "<br>", "\n");
        	if ((typeof(IsSecret)!="undefined")&&(IsSecret!=1))     //不涉密（csp文件中定义）
        	{
        		if ((cfg.id=="Secret1")||(cfg.id=="Secret2"))  //【病人密级】和【病人级别】两列不输出
        		{
	        		n++;
	        		continue; 
        		}
...
DataToExcelTool=$.haiExcel.DataToExcelTool;