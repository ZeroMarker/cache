function InitLab(obj){
	...
	function INFLabAdd(d,r){
		...
		var LABIsEPDID = $.form.GetValue("cboLABIsEPD");
		if (LABIsEPDID==''){
			var LABIsEPD = '';
		}else{
			var LABIsEPD = $.form.GetText("cboLABIsEPD");
		}
	...
	function InitINFLabEditData(d,r){
		...
		if (d){
			...
			$.form.SetValue("cboLABIsEPD",d.LABIsEPDID,d.LABIsEPD);
		
		}else{
			$.form.DateRender('txtSubmissDate','');
			$.form.SetValue("cboTestSet",'','');
			$.form.SetValue("cboLABIsEPD",'','');
		}
	}
...
//			});
	...
	function OpenINFLabSync(){
		var INFCount = obj.gridINFDiagnos.rows().count();
		var INFSelectCount = obj.gridINFDiagnos.rows({selected: true}).count();
		if ((INFCount>1)&&(INFSelectCount!=1)) {
			layer.msg('请选中一条感染诊断后填写!',{icon: 2});
			return ;
		}
		...
		obj.LayerOpenINFLabSync = layer.open({
				...
				zIndex: 100,
				maxmin: false,
				title: "病原学检验信息-提取<span class='text-red'>[双击数据进行编辑]<span>", 
				area: '75%',
	...
	obj.LAB_Save = function(){
    	...
    	for (var i=0;i<obj.gridINFLab.data().length;i++){
    		...
    		Input = Input + CHR_1 + obj.gridINFLab.data()[i].LABIsEPDID;
    		// 药敏
    		var SenDatas='';
