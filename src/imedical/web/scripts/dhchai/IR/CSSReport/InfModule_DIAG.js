function InitDiag(obj){
	...
	function InitINFDiagnosEidtData(d){
		// 渲染控件
		$.form.SelectRender("#cboInfPos1");
		$.form.SelectRender("#cboInfPos");
		$("#cboInfSub").data("param","^");	
		$.form.SelectRender("#cboInfSub");
		$.form.SelectRender('cboInfEffect');
		var InfCategoryDesc = $.form.GetText("chkInfCategory");
		if (InfCategoryDesc=="社区感染"){
			$("#cboInfLoc").data("param","^");
		}else{
			$("#cboInfLoc").data("param",EpisodeID+"^E");
		}	
		$.form.SelectRender('cboInfLoc');
		// 感染部位和感染诊断进行关联
		$("#cboInfPos1").change(function(){
			var InfPos1ID = $.form.GetValue("cboInfPos1");
			$("#cboInfPos").data("param",InfPos1ID);	
			$.form.SelectRender("#cboInfPos");
		});
		// 感染诊断和分类进行关联
		...
		// 编辑框赋值
		if (d){
			var PosInfo=$.Tool.RunServerMethod("DHCHAI.BTS.InfPosSrv","GetPosByInfDr",d.InfPosID);
			var PosDr="",PosDesc="";
			if (PosInfo!=""){
				PosDr=PosInfo.split("^")[0];
				PosDesc=PosInfo.split("^")[1];
			}
			$.form.SetValue("cboInfPos1",PosDr,PosDesc);
	...
	function INFDiagnosAdd(d,r){
    	...
    	if (InfXDate!=''){
			...
			//感染结束日期需要在住院期间或超出当前日期
			if (!obj.checkDate(InfXDate)){
				layer.msg('感染结束日期应在住院期间或不应超出当前日期!',{icon: 2});
				return;
			}
    		if (InfEffectID==''){
	...
	var DiagBasisList="";
    $('#gridDiagBasis tbody').on('change', '.icheckbox_square-blue input[type=checkbox]', function(){
    	if(this.checked){
	    	// 选中
	    	var BTDesc = $(this).parent().parent().next().text(); 	
	...
	var DiagCourseList="";
    $('#gridDiagCourse tbody').on('change', '.icheckbox_square-blue input[type=checkbox]', function(){
    	if(this.checked){
	    	// 选中
	    	var DicDesc = $(this).parent().parent().next().text(); 	
			$(this).parent().addClass("checked");
