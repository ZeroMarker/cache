...
function initTopPanelEvent() {
	...
	jQuery("#patDetBtn").off().on("click", function() {
		...
		var lnk = 'websys.default.csp?WEBSYS.TCOMPONENT=DHCNurPatDetail&EpisodeID=' + tepisodeId + '&AccessFlag=1';
		var iHeight = 450;
		var iWidth = 700;
		...
		window.open(lnk, '_blank', 'toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes,copyhistory=yes,width='+iWidth+',height='+iHeight+',left='+iLeft+',top='+iTop+'');
	});
...
function initPatListPanel() {
	jQuery('#tPatList').datagrid({
		...
		columns:[[  
			...
			{ title: '医保报销', field: 'ybfee',align: "right" ,halign:"center", width:100,hidden:true},	//先隐藏，具体项目再放开
			...
			{ title: '审核原因',  field: 'CodingReason',halign:"center", width:160 },
			...
			{ title: '就诊状态', field: 'visitstatus',halign:"center", hidden:true,width:1 },
			{ title: '账单ID', field: 'pbrowid',halign:"center", hidden:false,width:80},
			{ title: '账单状态',  field: 'pbflag',halign:"center", hidden:true,width:1 },
			{ title: '费别ID', field: 'instypedr',halign:"center", hidden:true,width:1},
			{ title: 'PAPMIRowID', field: 'patientid',halign:"center", hidden:true,width:1 },
			{ title: '发票ID',  field: 'prtrowid' ,halign:"center", hidden:true,width:1},
			{ title: '发票状态', field: 'prtflag',halign:"center", width:80},
			{ title: '就诊号', field: 'episodeid',halign:"center", hidden:false,width:80}
		]],
		rowStyler: function(index,row){
			//if (row.dischargestatus == "最终结算"){
		...
		onLoadError: function() {
			jQuery.messager.alert('错误', '加载病人列表失败.');
		},
		onSelect: function(rowIndex, rowData) {
			DepositSelectObj.depositArray = []; //清空押金数组
			DepositSelectObj.loadFlag = false;
			/*
			//反选，这个有问题，当有别的页签过来的时候正好反选，不能默认到原来的账单了
			if(rowData.pbrowid == GlobalObj.billId && rowData.episodeid == GlobalObj.episodeId) {
				jQuery(this).datagrid("unselectRow",rowIndex);
				GlobalObj.clearGlobal();
				clearData();
				log(DepositSelectObj.depositArray + "---")
				return false;
			}
			*/
...
function initPaymListPanel() {
	jQuery('#tPaymList').datagrid({ 
		...
		columns:[[  
						...
						onSelect: function(record, row) {
							...
							if(editIndex != undefined) {
								...
								if(ed){
									...
									//alert(productname + ":::" + jQuery(ed.target).combobox('getValue'))
			...
			{ title: '支行',  field: 'CTPMBankSub',width:1,hidden:true
				/*添加支行时使用
			...
			{ title: '支付方式CODE', field: 'CTPMCode',hidden:true,width:100 },
			{ title: '转账', field: 'pmck',width:40,checkbox:true,hidden:true},
			{ title: '支付方式ID', field: 'CTPMRowID',hidden:true,width:51},
			{title: 'ybFlag', field: 'ybFlag',hidden:true,width:1 },
			{title: 'fixedValue', field: 'fixedValue',hidden:true,width:1 }
		]],
		onBeforeLoad: function(param) {
		...
		onLoadSuccess: function(data) {
			...
			onLoadSuccessPaym();
			//Lid 2015-11-02 此处有问题，先特殊处理。
			//添加一个空行，不添加时有bug，问题还行再查，先规避一下？？？
			var maxLen111 = jQuery("#tPaymList").datagrid("getData").rows.length;
		...
		onClickCell: function(index, field,value) {
			...
			if(index == maxIndex && field == "CTPMAmt") {
				return false;
			}
		...
		onSelect:function(index,rowData){
			// var objGrid = jQuery("#tPaymList");
            // var amtEdt = objGrid.datagrid('getEditor', {index:index,field:'CTPMAmt'});
			// if(amtEdt){
				// jQuery(amtEdt.target).bind("blur",function(){
					// log("blur");
				// });
			// }
		},
...
function initLedgerPanel() {
	jQuery("#tLedger").datagrid({
		...
		columns:[[  
			...
			{ title: '分类', field: 'TarDesc2' ,hidden:true,width:60},
...
function initDepositPanel() {
	jQuery("#tDepositList").datagrid({
		...
		url: null,
		columns:[[  
			{ title: 'ck', field: 'ck',checkbox:true,width:10},
			...
			{ title: '结算标志',  field: 'TBillFlag' ,width:1,hidden: true,
				formatter:function(value,row,index){
					return +value;
				}
			},
			{ title: '押金状态', field: 'TPrtStatus',width:60},
			{ title: 'TPayMRowid', field: 'TPayMRowid',hidden:true,width:1},
			{ title: 'TRcptRowid', field: 'TRcptRowid',hidden:true,width:1 },
			{ title: 'TBankBackFlag', field: 'TBankBackFlag',hidden:true,width:1},
			{ title: 'TRefundNo',  field: 'TRefundNo',hidden:true,width:1 },
			{ title: 'TPrtStatusFlag', field: 'TPrtStatusFlag',hidden:true,width:1},
			{ title: 'PAPMIRowID', field: 'patientid',hidden:true,width:1 },
			{ title: 'TARRCPTDR',  field: 'TARRCPTDR' ,hidden:true,width:1}
		]],
		onBeforeLoad: function(param) {
			//目的是为了防止loadData时，设置url加载两次
			if(param.ArgCnt == undefined) {
...
function addGridInsuInfo(ybInfo) {
	...
	//ybInfo = "" //"0^1^1^1" + CH2 + "28^22" + CH2 + "29^33.2"  + CH2 + "30^83.1"
	//1,首先判断页面上是否已经有了医保要添加的支付方式
	//2,如果有改变原来的值，没有的话再添加
	var insuPmLen = ybInfo.split(String.fromCharCode(2)).length;
	var myIndex = 0;
	var myInsuAmt = 0;
	var insuChargeFlag = true, insuPmErrStr = "";
	for(var i = 1; i <= insuPmLen - 1; i++) {
		var pmId = ybInfo.split(String.fromCharCode(2))[i].split("^")[0];
		var pmAmt = ybInfo.split(String.fromCharCode(2))[i].split("^")[1];
		var checkInfo = checkPmExist(pmId);
		var checkFlag = checkInfo.split("^")[0];
		var checkAmt = checkInfo.split("^")[1];
		var checkIndex = checkInfo.split("^")[2];
		var checkYbFlag = checkInfo.split("^")[3];
...
function setCurrentAdm() {
	...
	//patientId, episodeId, billId, prtId)
...
function getStAmt() {
	...
	//alert(totAmt + ":" + ckDeposit + ":" + disAmt + ":" + payorAmt)
...
function createToolMenu(target, toolbar, menuBtnArr) {
	var toolbarDiv = "";
	//console.log(jQuery("#" + toolbar).length)
	if(!jQuery("#" + toolbar).length) {
		toolbarDiv = '<div id=\"' + toolbar + '\" style=\"height:auto;\"><div>';
		jQuery('#' + target).append(toolbarDiv);
	}
	...
	//console.log(jQuery("#panelPatList").width());
...
function initBillDetailTab() {
	...
	addOneTab("chargeTabs", "billDetailTab", "病人费用明细", url);
}
...
function initAdmOrderFeeTab() {
	var url = 'dhcipbillpatordfeecheck.csp?EpisodeID=' + GlobalObj.episodeId;
