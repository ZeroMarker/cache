var init = function () {
	var Clear = function () {
		...
		var Dafult = {
			StartDate: DateAdd(new Date(), 'd', -1),
			EndDate: new Date(),
			excludeInci:"Y"
		};
		$("#BarCode").val("");
	...
	$UI.linkbutton('#CancelAuditBT', {
		onClick: function () {
			...
			if (!isEmpty(SelectedRow.CancelDate)) {
				$UI.msg('alert', '您已取消过一次，请删除重新增加!')
				return;
			}
	...
	$('#wardno').bind('keypress', function (event) {
		if (event.keyCode == "13") {
			...
			var newWardno = NumZeroPadding(wardno, PatNoLen);
			$("#wardno").val(newWardno);
		}
	});
	$('#BarCode').bind('keypress', function (event) {
		if (event.keyCode == "13") {
			...
			if (BarCodeData.Inclb == "") {
				...
				$UI.msg('alert', "高值材料有未审核的" + BarCodeData.lastDetailOperNo + ",请核实!");
				return;
