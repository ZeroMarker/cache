...
var init = function () {
	...
	$UI.linkbutton('#FindTemBT', {
		onClick: function () {
			findReqTemplateWin('O', Select, TemSelect);
		}
	});
	...
	var HandlerParams = function () {
		...
		var QtyFlag=(GetAppPropValue('DHCSTINREQM',LocDr).QtyFlag && GetAppPropValue('DHCSTINREQM',LocDr).QtyFlag == 'Y') ? '1' : '0';  /// 根据库房判断
		...
		var Obj = {
			...
			NoLocReq: "N",
			HV: HV,
			RequestNoStock: InRequestParamObj.IsRequestNoStock
		};
		return Obj;
	}
		...
		onEndEdit: function (index, row, changes) {
			...
			for (var i = 0; i < Editors.length; i++) {
				...
				if (Editor.field == 'Qty') {
					...
					if ((InRequestParamObj.IfRequestMoreThanStock != "Y") && ((Qty - ProLocAllAvaQty) > 0)) {
