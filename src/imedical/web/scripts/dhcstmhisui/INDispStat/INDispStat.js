var init = function () {
	...
	//发放科室
	var DispLocParams = JSON.stringify(addSessionParams({ Type: "All" }));
	...
	$HUI.tabs("#tabs", {
		onSelect: function (title) {
			...
			} else if (title == "发放公支明细") {
				Params = encodeUrlStr(Params);
				var url = PmRunQianUrl + '?reportName=DHCSTM_HUI_SubLocAllotDetail.raq&Params=' + Params;
				var currTab = $('#tabs').tabs('getSelected');
				$('#tabs').tabs('update', {
					tab: currTab,
					options: {
