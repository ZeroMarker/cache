...
var init = function() {
	...
	function Create() {
		...
		$.cm({
			...
			if (jsonData.success == 0) {
				...
				if (CURRENT_INGR != '') { $('#MatOeriTab').tabs('select', '入库信息'); }
				Query();
			} else {
	...
	var RecLocParams = JSON.stringify(addSessionParams({ Type: 'Login', Element: 'RecLocId' }));
	...
	var OeriRecLocIdParams = JSON.stringify(addSessionParams({ Type: 'All', Element: 'OeriRecLocId' }));
