(function() {
	...
	var Export = {
		 ...
		 */
		toExcel: function(datas, datasArr, fName, XLSXJson) {
			...
			if (isId || datas instanceof Array) {
				if (useIE) {
					...
					// 推荐使用XLSX插件功能--2022-10改为使用json_to_sheet(原为dom)
					DownLoadExcelByXLSX(XLSXJson, fName);
 ...
 * @param {} FileName
 */
function DownLoadExcelByXLSX(XLSXJson, FileName) {
	var ExcelSheet = XLSX.utils.json_to_sheet(XLSXJson);
