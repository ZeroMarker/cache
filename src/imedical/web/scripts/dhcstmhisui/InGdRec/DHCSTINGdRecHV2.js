...
function SaveIngrInfo(index){
	...
	if (FindIndex>=0){
		...
		if (tmpIngrid!=""){
			...
			var MainData=JSON.stringify(addSessionParams({RecLoc:LocId,ApcvmDr:VendId,GiftFlag:GiftFlag,AdjCheque:AdjCheque,IngrTypeId:IngrTypeId,PurchaseUser:PurchaseUser,SourceOfFund:SourceOfFund,InGrRemarks:InGrRemarks}));
			var SaveData = $.cm({
				ClassName: 'web.DHCSTMHUI.DHCINGdRec',
