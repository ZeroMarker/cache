...
var init = function() {
	...
	$HUI.combobox('#ReqLocId', {
		url: $URL
			...
			+ JSON.stringify(addSessionParams({ Type: 'All', Element: 'ReqLocId' })),
		valueField: 'RowId',
	...
	$HUI.combobox('#RecLocId', {
		url: $URL
			+ '?ClassName=web.DHCSTMHUI.Common.Dicts&QueryName=GetCTLoc&ResultSetType=Array&Params='
			+ JSON.stringify(addSessionParams({ Type: 'All', Element: 'RecLocId' })),
		valueField: 'RowId',
		textField: 'Description'
	});
	$HUI.combobox('#RecLoc', {
	...
	function GetMatDetail(InciId, RowIndex) {
		...
		$('#BarCodeGrid').datagrid('refreshRow', RowIndex);
		BarCodeGrid.commonAddRow();
	}
