...
function InitDetails(){
	...
	var UserName=document.getElementById("UserName").value ;
	var lnk="websys.default.csp?WEBSYS.TCOMPONENT=INSUAuditReport&StartDate="+StartDate+"&EndDate="+EndDate+"&UserName="+UserName;
	parent.frames['INSUAuditReport'].location.href=lnk;
	var lnk="websys.default.csp?WEBSYS.TCOMPONENT=INSUAuditRepDetails&UserDr="+""+"&StartDate="+""+"&EndDate="+""+"&Job="+"";
...
function SelectRowHandler()	{
	...
	else
	{
		...
		var SelRowObj
		var obj
		SelRowObj=document.getElementById('TUserDrz'+selectrow);
		if (SelRowObj){UserRowid=SelRowObj.value;}
		else{UserRowid="";}
		SelRowObj=document.getElementById('Jobz'+selectrow);
		if (SelRowObj){Job=SelRowObj.value;}
		//alert(Job)
		var StartDate=document.getElementById("StartDate").value ;
		var EndDate=document.getElementById("EndDate").value ;
	}
	var lnk="websys.default.csp?WEBSYS.TCOMPONENT=INSUAuditRepDetails&UserDr="+UserRowid+"&StartDate="+StartDate+"&EndDate="+EndDate+"&Job="+Job;
	parent.frames['INSUAuditRepDetails'].location.href=lnk;
}
...
function Export_Click() //增加打印功能 Lou 2011-10-22
{
	...
	*/
	...
	xlApp=null;
	xlsheet=null;
	alert("导出完成,导出到"+"D:\\"+UserDate[0]+"医保审核统计.xls");
	}catch(e){
	     alert(e.message);
	     return
	}finally{
