...
function InitDataGrid(){
	...
	var Columns=[[    
		...
		{ field: 'SearchMasFlag', title: '读卡是否读取PA_PatMas信息', width: 205,
		...
		{ field : 'SetFocusElement',title : '自动聚焦功能',width : 110 },
        { field : 'PatMasFlag',title : '写入患者基本信息标志',width : 165 ,
			formatter:function(value,row,index){
				if (value == "Y") {
					return "<span class='c-ok'>是</span>"
				} else {
        ...
        { field : 'CardRefFlag',title : '写入卡信息标志',width : 120,
			formatter:function(value,row,index){
				if (value == "Y") {
					return "<span class='c-ok'>是</span>"
				} else {
        ...
        { field : 'AccManageFlag',title : '写入患者账户信息标志',width : 160,
			formatter:function(value,row,index){
				if (value == "Y") {
					return "<span class='c-ok'>是</span>"
				} else {
        ...
        { field : 'ParseTag',title : '分析信息维护界面与 Cache解耦合标志',width : 260},
        ...
        { field : 'CPRIsNotStructAddressFLag',title : '是否结构化地址',width : 120,
			formatter:function(value,row,index){
				if (value == "Y") {
					return "<span class='c-ok'>是</span>"
				} else {
...
function LoadAddrDefType(){
	...
	$.cm({
		...
		//$HUI.combobox("#AddrDefType", {
			...
			multiple:true,
			blurValidValue:true,
			data: JSON.parse(ret), //[{"id":"出生","text":"出生"},{"id":"现住","text":"现住"}],
		})
	})
