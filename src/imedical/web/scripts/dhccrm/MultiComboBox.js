Ext.form.MultiComboBox = Ext.extend(Ext.form.TriggerField, {
	...
	initList : function() {
		if (!this.list) {
			...
			if (!this.tpl) {
				this.tpl = '<tpl for="."><div class="' + cls+ '-item"><span class="unchecked" id="'+this.hiddenName+'checkBox_{'+this.displayField+'}"+ width="20">&nbsp;&nbsp;&nbsp;&nbsp;</span>{'+ this.displayField+'}</div></tpl>';
			}
	...
	onSelect : function(record, index) {
		if (this.fireEvent('beforeselect', this, record, index) !== false) {
			...
			var newValue = r.data[this.displayField];
			var check = document.getElementById(this.hiddenName+"checkBox_" + newValue);
			...
			for (var i = 0; i < this.store.data.length; i++) {
				...
				check = document.getElementById(this.hiddenName+"checkBox_" + newValue);
				if (check.className == "checked") {
	...
	expand : function() {
		...
		for (var i = 0; i < this.store.data.length; i++) {
			...
			for (var j = 0; j < hvalueArray.length; j++) {
				if (hvalueArray[j] == v) {
					document.getElementById(this.hiddenName+"checkBox_" + newValue).className = "checked";
				}
			}
