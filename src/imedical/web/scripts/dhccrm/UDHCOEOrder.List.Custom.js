...
function CheckBoreChangePrescriptType(tabidx) {
	...
	if ((NewRows>0)&&(CurrPrescTypeTab!=tabidx)) {
		var DelCheck=confirm((t['NO_SAVE']),false);
...
function CheckPrescItemAndCount(ItemDesc,ArcimRowid,ARCIMDetail){
	...
	if (PrescCheck==false){
		var PrescCheck=confirm((ItemDesc+t['SELFPAY_ALERT']),true);
		if (PrescCheck==false){
       		return false;
		}
	}
	...
	*/
	...
	if (PrescCheck==true){
		...
		if ((OrderMaxDoseQtyPerDay!="")&&(OrderMaxDoseQtyPerDay!=0)){
			alert(ItemDesc+t['NOT_ALLOW_SAME_ORDERITEM']);
			return false;
		}else{
			var PrescCheck=confirm((ItemDesc+t['SAME_ORDERITEM']),true);
    ...
    */
	...
	if (ItemDiagnosCheck==false){
		alert(ItemDesc+t['LIMIT_DIAGNOSE']);
    return false;
	}
	return true;
}
...
function CheckPrescriptType(PHPrescType){
	...
	for (var i=1; i<rows; i++){
		...
		if ((i!=FocusRowIndex)&&(OrderARCIMRowid!="")&&(OrderItemRowid=="")&&(OrderType=="R")){
			...
			//alert(OrderItemRowid+"^"+OrderARCIMRowid+"^"+OrderType+"^"+PHPrescType+"^"+OrderPHPrescType);
...
function CheckPrescriptForm(FormDesc){
	...
	for (var i=1; i<rows; i++){
		...
		if ((OrderARCIMRowid!="")&&(OrderItemRowid=="")&&(OrderType=="R")&&(OrderPHPrescType=="3")){
			...
			//alert(OrderItemRowid+"^"+OrderARCIMRowid+"^"+OrderType+"^"+FormDesc+"^"+OrderPHForm);
...
function CheckPrescriptCount(PHPrescType){
	...
	//alert("PHPrescType:"+PHPrescType+"  PrescLimitCount:"+PrescLimitCount+"   PrescCount:"+PrescCount);
...
function CheckPrescriptItem(ArcimRowid){
  ...
  var objPrescItemCats=document.getElementById("PrescItemCats");
	if ((objPrescItemCats)&&(objPrescItemCats.value!="")) PrescItemCats=objPrescItemCats.value;
        //alert("PrescItemCats:"+PrescItemCats+"ArcimRowid:"+ArcimRowid);
	if (PrescItemCats!="") {
...
function CheckDupOrderItemCat(ItemCatRowId){
	...
	if ((PAAdmType=="O")&&(OPUnRepeatItemCat!=="")) {
		...
		if (OPUnRepeatItemCats.indexOf(SubCatID)>-1) {
			...
			for (var i=1; i<rows; i++){
				...
				if ((OrderARCIMRowid!="")&&(OrderItemRowid=="")&&(OrderType!="R")){
					if ((OrderItemCatRowid==ItemCatRowId)&&(i!=FocusRowIndex)) {
						//alert(t['ItemCatRepeat']+OrderName);
						return true;
					}
				}
			}
		    ...
		    //alert("PAADMRowid"+PAADMRowid+"ARCIMRowid:"+ARCIMRowid);
...
function CheckDupLabSpecItem(LabExCode,LabSpecStr){
	...
	if ((CheckRepeatLabSpecMethod!="")&&(OrderLabExCodeStr!="")&&(CFNotCheckSameSpecItem!=1)){
		...
		if (retArr[0]==1){
			...
			var Row=RowArr[i];
			var OrderName=GetColumnData("OrderName",Row);
			alert(t['SameLabSpecItem']+":"+OrderName);
			*/
			//alert(t['SameLabSpecItem']);
			//return true;
			if (HospitalCode=="HF"){
				alert(t['SameLabSpecItem']);
				return true;
			...
			var PrescCheck=confirm(t['SameLabSpecItem'],true);
...
function CheckItemAllergy(iallergy){
	...
	if (allergycode=="01"){
  	var ret=confirm(t['Have_Allergy'],true);
...
function CheckItemPoison(ItemDesc,PHPoisonCode){
		...
		if (obj) IDCardNo=obj.value;
	  if (PAAdmType=="I") {
	  	if (HospitalCode=='JSXZZXYY'){
	  		var PatientID=document.getElementById('PatientID').value;
	  		if (((PHPoisonCode=='J1')||(PHPoisonCode=='DX')||(PHPoisonCode=='MZ'))&&(IDCardNo=='')){
					alert(ItemDesc+t['NeedIDCardNo']);
	  ...
	  /*
    if (GetPHCDPoisonMethod!=""){
			var retDetail=cspRunServerMethod(GetPHCDPoisonMethod,PHPoisonCode);
			if (retDetail!=''){
				var PoisonCode=cspRunServerMethod(GetPHCDPoisonCodeMethod,retDetail);
				if ((PoisonCode=='YB')||(PoisonCode=='GZ')){
					return true;
				}else{
					return false;
				}
  ...
  if ((HospitalCode=='SHSDFYY')||(HospitalCode=='JSXZZXYY')) {
		...
		if ((PHPoisonCode=='J2')||(PHPoisonCode=='J1')||(PHPoisonCode=='DX')||(PHPoisonCode=='MZ')){
			alert(ItemDesc+t['POISON_ALERT'])
			return true;
		}
		
		if ((PHPoisonCode=='DBTHZJ')||(PHPoisonCode=='TLJS')){
			alert(ItemDesc+t['POISON_SPORT_ALERT'])
			return true;
		}
	}else{
		if ((PHPoisonCode!="")&&(PHPoisonCode.indexOf("KSS")==-1)){
			alert(ItemDesc+t['POISON_ALERT'])
			return true;
		}
	}
	...
	if (CheckItemDiagnos) {
		...
		//alert("encmeth:"+encmeth);
			...
			var PatType=GetPatType();
	                //alert("PatType:"+PatType+"   ARCIMRowid:"+ARCIMRowid+"  mradmid"+mradmid);
...
function CheckDurationPackQty(Row) {
  ...
  if ((OrderMaxQty>0)&&(parseFloat(OrderPackQty)>parseFloat(OrderMaxQty))) {
    alert(OrderName+t['MedMaxQty']+OrderMaxQty);
    return false;
  }
  ...
  if (OrderPackQty>CheckPackQty){
    alert(OrderName+t['ExceedDurationQty']);
    return false;
  }
	return true;
}
	...
	if (PAAdmType=="I"){
	  ...
	  if (PrescLimitSum==0) return true;
	 	if ((eval(ScreenBillSum)+eval(AddSum))>eval(PrescLimitSum)){
			alert(t['EXCEED_PRESCAMOUNT']+PrescLimitSum);
			return false;
		}
		*/
	}
...
function CheckDeposit(amount,arcim){
	...
	if (CheckIPDepositMethod!=""){
		...
		if (retDetail!=0) {
			...
			if (retArray[4]=='C'){
				if (retArray[5]=='N'){
					alert(t['ExceedDeposit']+retArray[0]);
					return false;
				}else{
					if (arcim!=""){
...
function SetPrescBillTypeID(BillTypeRowid) {
	//alert("BillTypeID:"+BillTypeRowid);
...
function SetTotalToBillSum(TotalToBillSum) {
        //alert("ToBillSum:"+TotalToBillSum);
...
function SetPCSPatType(PCSPatTypeID) {
        //alert("PCSPatTypeID:"+PCSPatTypeID);
...
function CheckConflict(ARCIMRowid){
			...
			for (var i=1; i<rows; i++){
				...
				if ((OrderARCIMRowid!="")&&(OrderItemRowid=="")&&(FocusRowIndex!=i)){
					if (TempConflictItems.indexOf(TempOrderARCIMRowid)!=-1) {
						alert(t['ItemConflict']+OrderName);
						return false;
					}
				}
			}	
    ...
    if (CheckConflictMethod){
	    ...
	    if (CheckConflict=="Y")  {
	    	alert(t['ItemConflict']+ConflictItemDesc+t['ItemConflictAutoStop']+ConflictItemDesc);
	    }
  	}
...
function InitDHCC(){
	...
	if (objtbl){
		...
		var OrderItemRowid=GetColumnData("OrderItemRowid",Row);
		var OrderARCIMRowid=GetColumnData("OrderARCIMRowid",Row);
		//alert(OrderItemRowid+"^"+OrderARCIMRowid);
	...
	*/
	...
	var retval=cspRunServerMethod(GetOEPrefMethod,session["LOGON.USERID"]);
	 if(retval=="0"){ 
			var obj=document.getElementById("OEPrefUser");
...
function EQASInfoClickHandler(){
	...
	//alert(OrderARCIMRowid+"^"+OrderRecDepRowid);
...
function GetTableSequence(rowobj){
		...
		//alert(OrderPackQtyColumnIndex+"^"+OrderDoseQtyColumnIndex+"^"+OrderPriorColumnIndex+"^"+OrderNameColumnIndex);
}
...
function UpdateClickHandler() {
	...
	if (DHCDTInterface==1){
		...
		if (XHZYRetCode!=0){
    	var PrescCheck=confirm(t['DTQuestion'],true);
      if (PrescCheck==false){
				obj.disabled=false;
				obj.onclick=UpdateClickHandler;
      	return false;
	...
	if (DirectSave!="1"){
		if (OrderItemStr!=""){
			var OEOrdItemIDs=InsertOrderItem(OrderItemStr);
			//alert(OEOrdItemIDs);
			if (OEOrdItemIDs=="0") {
		...
		var OEOrdItemIDs=SaveOrderItems(OrderItemStr,XHZYRetCode);
		//alert(OEOrdItemIDs);
		if (OEOrdItemIDs=="100") {
			alert(t['Fail_SaveOrder']);
			return websys_cancel();
	...
	UpdateFlag=true;
	//自动弹出检查申请单
	var OrderARCIMRowidStr=GetOrderARCIMRowidStrMethod()
	var SGFlag=cspRunServerMethod(GetSGFlagMethod,OrderARCIMRowidStr);
	//alert("SGFlag="+SGFlag)
	if(SGFlag=="1"){
		if(confirm("是否立即填写申请单",true)){
			var Eposide=document.getElementById("EpisodeID").value;
			var mradm=document.getElementById("EpisodeID").value;
			var PatientID=document.getElementById("PatientID").value;
	...
	if ((DHCDTInterface==1)&&(DHCDTUploadFlag==1)){
		...
		//if (myrtn==2)alert(t['DTQuestion']);
	}

	//alert("OEOrdItemIDsobj:"+OEOrdItemIDsobj.value+"  summflag:"+summflag.value);
	/*
...
function CheckBeforeUpdate(){
  ...
  try{
		...
		for (var i=1; i<rows; i++){
			...
			//if (OrderFirstDayTimes=="0") {alert(OrderName+t['FirstTimesNotZero']); return false}
			if ((OrderFreqFactor!="")&&(OrderFirstDayTimes!="")&&(Number(OrderFirstDayTimes)>Number(OrderFreqFactor))) {alert(OrderName+t['FirstMaxNum']); return false}
			...
			if ((CFIPDispensingRecLoc!='')&&(PAAdmType=="I")&&(OrderType=='R')){
				...
				if ((DispensingRecLocStr.indexOf(OrderRecDepStr)!=-1)&&(OrderPackQty!="")){
					alert(OrderName+t['PackQtyNotAccept']);
					return false;
				}
			}
			...
			if ((OrderARCIMRowid!="")&&(OrderItemRowid=="")){
				...
				if (!(CheckOrderStartDate(OrderStartDate,CurrDate))) {
					alert(OrderName+t['STARTDATE_EXCEEDTODAY']);
				...
				if (OrderType=="R"){
					...
					if ((OrderFreqRowid=='')||(OrderFreq=="")) {
						alert(OrderName+t['NO_Frequence']);
					...
					if (((OrderDoseUOMRowid=='')||(OrderDoseUOM==""))&&(!CheckWYInstr)) {
						alert(OrderName+t['NO_DoseUOM']);
						SetFocusColumn("OrderDoseUOM",Row);
						FocusRowIndex=i;
						return false;
					}
					if (((OrderDurRowid=='')||(OrderDur==''))&&(!CheckWYInstr)) {
						alert(OrderName+t['NO_Duration']);
						SetFocusColumn("OrderDur",Row);
						FocusRowIndex=i;
						return false;
					}
					if ((OrderInstrRowid=='')||(OrderInstrRowid=='')) {
						alert(OrderName+t['NO_Instr']);
					...
					if ((OrderPriorRowid!=OMOrderPriorRowid)&&(OrderPriorRowid!=OMSOrderPriorRowid)){
						...
						if (Check=='0') {
				  		...
				  		var OrderRecDepDesc=CellObj.options[CellObj.selectedIndex].text;
							alert(OrderName+OrderRecDepDesc+t['QTY_INCItemLocked']);
							return false;
						}
					}
					...
					if ((PHPrescType=="3")&&(HospitalCode=="HF")&&(OrderARCIMRowid=="5612||1")){
						...
						if ((OrderPackQty!="")&&(OrderPackQty>2)){
							alert(OrderName+t['Exceed_CNDurCL']);
					    SetFocusColumn("OrderDur",Row);
					    FocusRowIndex=i;
					    return false;
						}
					...
					if ((OrderPHForm==t['CNMedForm'])&&(PHPrescType=="3")&&(((HospitalCode=="HF")&&(OrderARCIMRowid!="5612||1"))||(HospitalCode!="HF"))){
						if ((OrderDurFactor!="")&&(OrderDurFactor>14)){
							alert(OrderName+t['Exceed_KLCNDur']);
							SetFocusColumn("OrderDur",Row);
							FocusRowIndex=i;
							return false;
						}
	        ...
	        if ((OrderPHForm==t['CNMedForm'])&&(PHPrescType=="3")&&(((HospitalCode=="HF")&&(OrderARCIMRowid!="5612||1"))||(HospitalCode!="HF"))){
	        	if ((OrderDurFactor!="")&&(OrderDurFactor>7)){
	        		  alert(OrderName+t['Exceed_CNDur']);
						    SetFocusColumn("OrderDur",Row);
						    FocusRowIndex=i;
						    return false;
	        	}
	        ...
	        if (PAAdmType!="I") {
						//alert(XYPrescCount+"^"+ZCYPrescCount);
						if (HospitalCode=="NB"){
					  	...
					  	if (HospitalCode=="ZGYKDFSYY"){
					  		...
					  		if ((IPDosingRecLoc!="")&&(OrderType=="R")&&(OrderPriorRowid==LongOrderPriorRowid)&&(OrderDoseQty>=100)&&(OrderDoseUOMRowid==MLUOMRowId)&&(OrderInstrRowid==IVInstrRowId)){
					  			...
					  			if (!ret){
					  				...
					  				if (ret){
						  				alert(OrderName+t['NotDosingRecLoc']);
						  				return false;
						  			}
					  			}
					  		}
					...
					if ((OrderPriorRowid==OMOrderPriorRowid)||(OrderPriorRowid==OMSOrderPriorRowid)){
						alert(OrderName+t['OMOrderPriorNotAllow']);
						return false;
					}
					//自定价医嘱
					if (OrderType=="P"){
					...
					if (OrderType=="L"){
						...
						if (HospitalCode!="ZGYKDFSYY"){
							...
							if (PrescCheck==true){
								alert(OrderName+t['SAME_LABORDERITEM']);
							  return false;
						  }
						}
						var OrderLabSpecRowid=GetColumnData("OrderLabSpecRowid",Row);
						if (OrderLabSpecRowid==''){
							alert(OrderName+t['NeedLabSpec']);
							return false;
						}
					}
				}
			  ...
			  if (PriorCheck==false) {
			  	alert(OrderName+t['PriorNoAllow']);
			  	return false;
			  }
    ...
    if (!CheckBillTypeSum(Amount)){
    	var PrescCheck=confirm(t['EXCEED_ADMAMOUNT'],true);
      if (PrescCheck==false){return false;}
    }
			
    if (!CheckOutOrderSum(Amount)){
    	var OutOrderCheck=alert(t['EXCEED_OutOrderAMOUNT']);
      return false;
    }
			
		if ((CYPrescCount>0)&&(!FindCNMedCl)&&(HospitalCode=="HF")){
      var PrescCheck=confirm(t['No_AddCNMedCL'],true);
      if (PrescCheck==false){return false;}
      //alert(t['No_AddCNMedCL']);
			//return false;
		}
		if (NewRows==0){
			alert(t['NO_NewOrders']);
			return false;
		}
		...
		if(OrderMasterSeqNo!=""){
			...
			if(MastOrderPriorRowid!=OrderPriorRowid ){
				alert("关联医嘱类型不同，不给予审核！")
				return false;
				}
			}
		return true;
	...
	try{
		...
		for (var i=1; i<rows; i++){
  	    ...
  	    if (OrderCoverMainIns==true){OrderCoverMainIns="Y"}else{OrderCoverMainIns="N"}
				...
				if (((OrderPriorRowid==LongOrderPriorRowid)||(OrderPriorRowid==OMSOrderPriorRowid))&&(PAAdmType=="I")) OrderFirstDayTimes=GetColumnData("OrderFirstDayTimes",Row);
...
function InsertOrderItem(OrderItemStr){
	var UserAddRowid="";
	var UserAddDepRowid="";
	var DoctorRowid="";
  ...
  var StopConflictFlag="";
  if (ConflictItemsInfo!=""){
  	var StopConflictItems=confirm("存在以下互斥医嘱："+ConflictItemsInfo+" "+"请确认是否自动停止互斥医嘱");
  	if (StopConflictItems) StopConflictFlag="1";
  }
  //alert(OrderItemStr+"----"+UserAddRowid+"^"+UserAddDepRowid+"^"+DoctorRowid);
...
function SaveOrderItems(OrderItemStr,XHZYRetCode){
	var UserAddRowid="";
	var UserAddDepRowid="";
	var DoctorRowid="";
  UserAddRowid=session['LOGON.USERID'];
  ...
  //alert(OrderItemStr+"----"+UserAddRowid+"^"+UserAddDepRowid+"^"+DoctorRowid);
	var ret=cspRunServerMethod(SaveOrderItemsMethod,EpisodeID,OrderItemStr,UserAddRowid,UserAddDepRowid,DoctorRowid,XHZYRetCode)
	return ret;
         ...
         if (RowObj.cells[j].firstChild.tagName!="LABEL"){continue;}
          ...
          ClearAllList(RowObj.cells[j].firstChild);
			...
			if (arrId[0]=="OrderFirstDayTimes"){
					var str="<input type=text id=\""+Id+"\" name=\""+Id+ "\" style=\"WIDTH:" + objwidth + " ;HEIGHT:" + objheight +"\" value=\"\" onkeypress=\"OrderFirstDayTimeskeypresshandler()\" onchange=\"OrderFirstDayTimeschangehandler()\" onkeydown=\"OrderFirstDayTimeskeydownhandler()\">";
          RowObj.cells[j].innerHTML=str;
			}
...
function OrderMultiDate_lookupSelect(dateval) {
	...
	//alert(GetColumnData("OrderMultiDate",Row));
...
function OrderPriorchangeCommon(OldPriorRowid,PriorRowid,Row){
	...
	//把从医院代码设定改为从设置去判断
	if ((IPShortOrderPriorST=="1")&&((PriorRowid==ShortOrderPriorRowid)||(PriorRowid==OMOrderPriorRowid))&&(OrderFreqRowid!=STFreqRowid)&&(PAAdmType=="I")){
	...
	if ((OrderARCIMRowid!="")&&(OrderItemRowid=="")){
			...
			if (obj){
				...
				if (cellobj){
					...
					if ((PriorRowid==LongOrderPriorRowid)||(PriorRowid==OMSOrderPriorRowid)){
						var str="<input type=text id=\""+Id+"\" name=\""+Id+ "\" style=\"WIDTH:" + objwidth + " ;HEIGHT:" + objheight +"\" value=\"\" onkeypress=\"OrderFirstDayTimeskeypresshandler()\" onkeydown=\"OrderFirstDayTimeskeydownhandler()\" onchange=\"OrderFirstDayTimeschangehandler()\">";
						cellobj.innerHTML=str;
					}else{
						var str="<label id=\""+Id+"\" name=\""+Id+ "\" style=\"WIDTH:" + objwidth + " ;HEIGHT:" + objheight +"\" value=\"\">";
						cellobj.innerHTML=str;
		...
		if (cellobj){
			...
			var IMGId="ldi"+Id;
			var OrderPriorRemarks=GetColumnData("OrderPriorRemarks",Row);
			if (((PriorRowid==LongOrderPriorRowid)||(PriorRowid==OMSOrderPriorRowid))&&(OrderPriorRemarks!="OUT")){
				var str="<label id=\""+Id+"\" name=\""+Id+ "\" style=\"WIDTH:" + objwidth + " ;HEIGHT:" + objheight +"\" value=\"\">";
				cellobj.innerHTML=str;
	    	...
	    	var OrderDur=GetColumnData("OrderDur",Row);
				objwidth=AdjustWidth(objwidth);
				var str="<input type=text id=\""+Id+"\" name=\""+Id+ "\" style=\"WIDTH:" + objwidth + " ;HEIGHT:" + objheight +"\" value=\""+OrderDur+"\" onkeydown=\"DurationChangeHandler()\" onchange=\"DurationChangeHandler()\">";
				str=str+"<IMG id=\""+IMGId+"\" name=\""+IMGId+ "\" src=\"../images/websys/lookup.gif\" onclick=\"PHCDUDesc_lookuphandler()\">";
				cellobj.innerHTML=str;
...
function OrderRecDepchangehandler(e){
	...
	var OrderStartDate="",OrderStartTime="";
	var OrderType=GetColumnData("OrderType",Row);
	if (((OrderPriorRowid==LongOrderPriorRowid)||(PriorRowid==OMSOrderPriorRowid))&&(OrderType=="R")){
		var ret=CheckDosingRecLoc(RecDepRowid);
		if (ret){
			var OrderHiddenPara=GetColumnData("OrderHiddenPara",Row);
			var OrderItemCatRowid=mPiece(OrderHiddenPara,String.fromCharCode(1),2);
   		...
   		var OrderPriorRowid=GetColumnData("OrderPriorRowid",Row);
 			if ((OrderPriorRowid!=OMOrderPriorRowid)&&(OrderPriorRowid!=OMSOrderPriorRowid)){
	  		...
	  		var OrderName=GetColumnData("OrderName",Row);
				...
				if (Check=='0'){
					alert(OrderName+OrderRecDepDesc+t['QTY_NOTENOUGH']);
					if (HospitalCode=='BJDTYY'){
				    DeleteTabRow(objtbl,FocusRowIndex);
				    return;
				  }
					...
					//不是通过用法改变事件来改变接收科室的而是通过序号关联事件来 重新组织接收科室串
					//另建一方法根据EpisodeID,InstrRowid,LogonDep,ARCIMRowId统一取接收科室的方法
					var OrderHiddenPara=GetColumnData("OrderHiddenPara",Row);
					var OrderItemCatRowid=mPiece(OrderHiddenPara,String.fromCharCode(1),2);
					var PreInstrRecLocStr=GetColumnData("OrderRecLocStr",Row);
					if ((ReclocByInstr=="1")&&(GetInstrReclocMethod!="")){
			    	var LogonDep=GetLogonLocByFlag();;
			    	//alert("EpisodeID:"+EpisodeID+"MasterOrderInstrRowid:"+MasterOrderInstrRowid+"^"+"OrdDept:"+OrdDept)
			    	var InstrReclocStr=cspRunServerMethod(GetInstrReclocMethod,EpisodeID,MasterOrderInstrRowid,LogonDep,OrderItemCatRowid,OrderPriorRowid)
			    	var OrderPriorRowid=GetColumnData("OrderPriorRowid",Row);
			    	CurrentInstrReclocStr=GetRecLocStrByPrior(InstrReclocStr,OrderPriorRowid);
			    	SetColumnData("OrderRecLocStr",Row,InstrReclocStr);
			    	SetColumnList("OrderRecDep",Row,CurrentInstrReclocStr);
			    }
			    //End
					...
					if (FindSubRecDep==false){
						alert(t['SubOrderRecDepNotDefine']);
						return;
					}
...
function OrderDoseQtykeydownhandler(e)
{
	...
	try {
		...
		if ((keycode==13)||(keycode==9)){
		  ...
		  if (OrderARCIMRowid!=""){
		  	//alert(OrderType+"^"+OrderDoseQty);
			  if (OrderType=="R"){
			  	//(OrderDoseQty!="")
			  	//alert("OrderMasterSeqNoz"+Row);
			  	XHZY_Click();
			  	websys_setfocus("OrderMasterSeqNoz"+Row);
			  }else{
			  	if (OrderPHPrescType!="4"){
...
function DurationLookUpSelect(value) {
    ...
    var OrderType=GetColumnData("OrderType",Row);
    if ((OrderType=="R")&&(OrderPackQtyColumnIndex>OrderDoseQtyColumnIndex)) {
        if (ShowDurLookup==1){websys_setfocus("OrderPackQtyz"+Row);}
    }else{			  	
			if ((PAAdmType=="I")||((PAAdmType!="I")&&(OrderPackQty!=""))){
	  			window.setTimeout("AddClickHandler()",200);
	  	}
...
function InstrLookUpSelect(value) {
    ...
    var OrderHiddenPara=GetColumnData("OrderHiddenPara",Row);
		...
		var PreInstrRecLocStr=GetColumnData("OrderRecLocStr",Row);
		//alert("PreInstrRecLocStr"+PreInstrRecLocStr)
		
    if ((InstrOrderType=="R")&&(ReclocByInstr=="1")&&(GetInstrReclocMethod!="")){
    	var OrdDept=session['LOGON.CTLOCID'];
    	//alert("EpisodeID:"+EpisodeID)
    	//alert("OrderInstrRowid"+":"+OrderInstrRowid+"^"+"OrdDept"+":"+OrdDept+"^"+"PreInstrRecLocStr"+":"+PreInstrRecLocStr)
    	...
    	//alert("InstrReclocStr"+":"+InstrReclocStr)
    	var OrderPriorRowid=GetColumnData("OrderPriorRowid",Row);
    	CurrentInstrReclocStr=GetRecLocStrByPrior(InstrReclocStr,OrderPriorRowid);
    	//alert("CurrentInstrReclocStr"+":"+CurrentInstrReclocStr)
    	SetColumnData("OrderRecLocStr",Row,InstrReclocStr);
    	SetColumnList("OrderRecDep",Row,CurrentInstrReclocStr);  	
    }
    //End
...
///将用法关联的医嘱增加到"医嘱录入"界面上
function AddLinkItemToList(ParaArr,MasterOrderItemRowId,MasterOrderSeqNo,MasterOrderPriorRowid,MasterOrderFreqRowid,MasterOrderStartDate){
	//VerifiedOrderMasterRowid=MasterOrderItemRowId;
	//VerifiedOrderMasterSeqNo=MasterOrderSeqNo;
	//alert("MasterOrderItemRowId=="+MasterOrderItemRowId+"    MasterOrderSeqNo=="+MasterOrderSeqNo+"    MasterOrderPriorRowid=="+MasterOrderPriorRowid);
	//alert("MasterOrderFreqRowid=="+MasterOrderFreqRowid+"    MasterOrderStartDate=="+MasterOrderStartDate);
	var MasterOrderStartDate=cspRunServerMethod(ConverDateMethod,MasterOrderStartDate,1);
	var MasterOrderStartDate=cspRunServerMethod(ConverDateMethod,MasterOrderStartDate,4);
	//var MasterOrderFreq=cspRunServerMethod(GetFreqDescMethod,MasterOrderFreqRowid);
	//MasterOrderSeqNo="!"+MasterOrderSeqNo;
	  ...
	  var CopyBillTypeRowId="";
	    ...
	    if (ret==true){
		  	...
		  	SetColumnData("OrderPrior",Row,MasterOrderPriorRowid);
				...
				SetColumnData("OrderPackQty",Row,linkqty);
				SetColumnData("OrderDepProcNote",Row,LinkNotes);
			...
			if (ArrData1[2]!="Y") {
				...
				if ((Check!='0')&&(Check!='-1')) {
					...
					if ((HospitalCode=='YKYZLYY')||((HospitalCode=='SDWFYY')&(PAAdmType=="I"))){
						var PrescCheck=confirm((OrderName+t['Change_Stock_Recloc']),true);
						if (PrescCheck==false){return '';}
					}else{
						//由设置决定是否切换药房
						if (CFNotAutoChangeRecloc=='1'){
							if (Check=='0')	alert(OrderName+OrderRecLocDesc+t['QTY_NOTENOUGH']);
							if (Check=='-1') alert(OrderName+OrderRecDepDesc+t['QTY_INCItemLocked']);
							return '';
						}
...
function AddItemToList(RowIndex,icode,Para,CopyBillTypeRowId,ItemDefaultRowId) {	  //Add an item to a listbox
 ...
 if (!CopyBillTypeRowId){CopyBillTypeRowId=""}
  ...
  if (AlertAuditSpecialItem!=0){
  	var NeedInsuAudit=confirm(idesc+t['NeedInsuAudit']);
  ...
  var OrderPoisonCode=mPiece(iformstr,String.fromCharCode(1),4);
	...
	if (OrderLabItemActive!="Y"){
		alert(idesc+t['LabItemUnavailable']);
		return false;
	}
  ...
  if (OrderType=="R"){
		...
		//检查医生对管制分类药品的控制级别
  		var PrescCheck=CheckDoctorTypePoison(OrderPoisonRowid,icode,Row);
		//if (PrescCheck==false){return false;}
  ...
  if ((HospitalCode=='SHHSPD')&&(PAAdmType=="I")&&(needskintest=="Y")){
	  ...
	  var DelCheck=confirm((t['IPNeedSkinTest']),false);
	  if (DelCheck==false) {return false;}
	}
  ...
  //如果是复制医嘱或开医嘱套的就优先取传入的医嘱类型
  if ((Para!="")&&(SpecOrderPriorRowid!="")){OrderPriorRowid=SpecOrderPriorRowid}
  /*
	//如果是st医嘱就改为临时医嘱
  if ((OrderPriorRowid==LongOrderPriorRowid)&&(OrderFreqRowid==STFreqRowid)){OrderPriorRowid=ShortOrderPriorRowid}
  */
  ///此处处理有问题需改正
  ...
  if ((PAAdmType=="I")&&(OrderType=="R")){
		if ((OrderPriorRowid==OutOrderPriorRowid)&&(ioutpriorreclocstr!="")){
			...
			CurrentRecLocStr=GetRecLocStrByPrior(ireclocstr,OrderPriorRowid);
		}
	}else{
		//var CurrentRecLocStr=ireclocstr;
		CurrentRecLocStr=GetRecLocStrByPrior(ireclocstr,OrderPriorRowid)
  ...
  //医生自行设置是否弹出提示
	...
	if (objmsgflag){
		//alert(objmsgflag.checked+","+iordermsg);
		if (!objmsgflag.checked){OrderMsg=""}
	}
	
  if ((OrderMsg!="")&&(((Para=="")&&(HospitalCode=="HF"))||(HospitalCode!="HF"))){alert(idesc+OrderMsg);}
  ...
  if (HospitalCode=="ZJQZRMYY") InsurCheck=true;
	...
	if ((InsurFlag!='0')||(CFAllConfirmInsurSYMM==1)){
		...
		//if ((limitFlag)&&(InsurFlag!="0")&&(HospitalCode=="ZGYKDFSYY")){
			var InsurCheck=window.confirm(t['InsurSYMMMessage1']+InsurAdmAllowFlag+t['InsurSYMMMessage2']);
		}
	}
	...
	if ((HospitalCode=="SDWFYY")&(PAAdmType=="I")){
		...
		if ((LabDescFlag!="-1")&(OrderBillTypeRowid=="1")&(OrderType=="L")){
			var LabDescCheck=confirm(t['OrderOnlyForInsurAdm'],false);
	...
	if ((HospitalCode=='YY')||(HospitalCode=='BJZYY')||(HospitalCode=='BJDTYY')||(HospitalCode=='BJFCYY')||(HospitalCode=='YKYZLYY')){
		...
		if (InsurAlertStr==""){
			...
			if (Itemtemp.length==1){
				...
				//if (HospitalCode!='BJZYY') InsurCheck=window.confirm(t['Insu_Ind1']+bz+t['Insu_Ind2']);
				if (HospitalCode!='BJZYY') InsurCheck=window.confirm(t['InsurSYMMMessage1']+bz+t['InsurSYMMMessage2']);
			}
			if (Itemtemp.length>1){
				var obj = new Object();
        obj.name="Para";
  ...
  //alert(OrderFreqFactor+"^"+OrderFreqInterval+"^"+OrderDurFactor+"^"+OrderConFac+"^"+OrderDrugFormRowid);
	SetColumnData("OrderFreq",Row,OrderFreq);
	SetColumnData("OrderFreqRowid",Row,OrderFreqRowid);
	SetColumnData("OrderInstr",Row,OrderInstr);
	SetColumnData("OrderInstrRowid",Row,OrderInstrRowid);
  ...
  if (needskintest=="Y"){
  	...
  	if ((HospitalCode=='SHSDFYY')||(HospitalCode=='JSXZZXYY')){        //东方医院需要医生确认是否置皮试
  		var DelCheck=confirm("该药品需要皮试,是否做皮试?",false);
			var OrderPriorArray=OrderActionStr.split("^");
			for (var i=0;i<OrderPriorArray.length;i++) {
			 var OrderPrior=OrderPriorArray[i].split(String.fromCharCode(1));
			...
			//还需要增加不是青霉素类药品的判断
	  	if (DelCheck==true) {              //皮试备注标记原液;点击取消后,皮试不打勾,皮试备注标记为免试液
	  		SetColumnData("OrderSkinTest",Row,true);
	  		if (skintestyy=='1'){
...
function CheckOrderDoseLimit(Row) {
	...
	//alert(OrderHiddenPara);
	//判断没有设置参数就不用走后面的处理
	if ((OrderMaxDoseQty=='')&&(OrderMaxDoseQtyPerDay='')&&(OrderLimitDays=='')&&(OrderMaxDoseQtyPerOrder='')) return false;
	...
	if (CalEqDoseMethod!=''){
		...
		//alert("OrderDoseUOMRowid:"+OrderDoseUOMRowid+" DrugFormRowid:"+DrugFormRowid+" OrderDoseQty:"+OrderDoseQty);
		//alert("BaseDoseQty:"+BaseDoseQty+" OrderMaxDoseQty:"+OrderMaxDoseQty+" OrderMaxDoseQtyPerDay:"+OrderMaxDoseQtyPerDay+" OrderMaxDoseQtyPerOrder:"+OrderMaxDoseQtyPerOrder);
		if ((BaseDoseQty!='')&&(BaseDoseQty!=0)){
			if ((OrderMaxDoseQty!='')&&(OrderMaxDoseQty!=0)){
				if (parseFloat(BaseDoseQty)>parseFloat(OrderMaxDoseQty)){
					alert(t['ExceedPHDoseQtyLimit']);
					SetFocusColumn("OrderDoseQty",Row);
					return false;
				}
			}
			if ((OrderMaxDoseQtyPerDay!='')&&(OrderMaxDoseQtyPerDay!=0)){
				...
				if ((parseFloat(BaseDoseQtyPerDay)>parseFloat(OrderMaxDoseQtyPerDay))){
					alert(t['ExceedPHDoseQtyPerDayLimit']);
					SetFocusColumn("OrderFreq",Row);
					return false;
				}
			}
			if ((OrderMaxDoseQtyPerOrder!='')&&(OrderMaxDoseQtyPerOrder!=0)){
				...
				if ((parseFloat(BaseDoseQtyPerOrder)>parseFloat(OrderMaxDoseQtyPerOrder))){
					alert(t['ExceedPHDoseQtyPerOrderLimit']);
					SetFocusColumn("OrderDur",Row);
					return false;
				}
			}
...
function SetPackQty(Row) {
	...
	//alert(OMOrderPriorRowid+"^"+OMSOrderPriorRowid+"^"+OrderPriorRowid);
	if (PAAdmType=="I") {
		//中医院取药医嘱和出院带药医嘱需要整包装
		if (((OrderPriorRowid==OutOrderPriorRowid)||(OrderPriorRowid==OneOrderPriorRowid))&&(CFOutAndOneDefaultPackQty==1)) {
...
function SetOrderFirstDayTimes(Row){
	...
	//if ((OrderPriorRowid==LongOrderPriorRowid)&&(PAAdmType=="O")&&(OrderPHPrescType=='4')&&(GetTodayDispTimesMethod!='')){
	if ((OrderType=='R')||(OrderPHPrescType=='4')){
		if (GetTodayDispTimesMethod!=''){
			var OrderFreqRowid=GetColumnData("OrderFreqRowid",Row);
			var OrderStartTime=GetColumnData("OrderStartTime",Row);
			var OrderFirstDayTimes=cspRunServerMethod(GetTodayDispTimesMethod,OrderFreqRowid,OrderStartTime);
			SetColumnData("OrderFirstDayTimes",Row,OrderFirstDayTimes);
...
function HiddenDeleteOrder_changehandler(encmeth) {
	//alert(" "+encmeth+" "+Gparam1+" "+Gparam2+" "+Gparam3+" "+Gparam4);
...
function OrderDetailsShowing(f,ShowAutoOnly) {
		...
		//alert("selectedIndex="+lstOrders.selectedIndex+" OrderDetailsOpenCount="+OrderDetailsOpenCount);
		if (lstOrders.selectedIndex == -1) {
			//alert("Please select an item");
			return;
		}
		//alert(lstOrders.selectedIndex+"***"+OrderDetailsOpenCount);
		if (lstOrders.options[lstOrders.selectedIndex+OrderDetailsOpenCount] && lstOrders.options[lstOrders.selectedIndex+OrderDetailsOpenCount].selected) {
			...
			if (newOrderValue.substring(0,5)=="     ") {
				...
				//alert("LinkedItmID1="+LinkedItmID1);
			}
			...
			if (mPiece(itype,String.fromCharCode(4),0)=="ARCOS") {
				...
				if (EpisObj) EpisID=EpisObj.value;
				//alert("ItemDesc/itemdata/itemvalue/OSRowid/EpisID="+ItemDesc+"/"+itemdata+"/"+itemvalue+"/"+OSRowid+"/"+EpisID);
				//alert("matchcategory "+CatID+"##"+SubCatID+"##"+itemvalue);
...
function OrderDetailsOpen(sItemDesc,sitemdata,sitemvalue,sOSRowid,sEpisodeID,ORIRowID,LinkedItmID1) {
		...
		//alert(sItemDesc+"*"+sitemdata+"*"+sitemvalue+"*"+sOSRowid+"*"+ORIRowID);
		var OkToOpen=1;
...
function AddToPrefTabClickHandlerNew(evt) {
	...
	if (lstidx==""){
		alert(t['NoListSelected']);
		return false;
	}
	var other=userid+"^"+ctlocid+"^"+groupid+"^"+currTab+"^"+lstidx+"^"+id;  
  var ret=cspRunServerMethod(AddToFavMethod,other);
  if (ret=="0") {alert(t['Success_AddToFavPref']);}else{alert(t['Fail_AddToFavPref']);}
...
function Init() {
	...
	//if (flagOBJ) alert("DisplayQuestionFlag "+flagOBJ.value);

	//flagOBJ=document.getElementById("NewOrders");
	//if (flagOBJ) alert("NewOrders "+flagOBJ.value);
  ...
  obj=document.getElementById("Add");
	...
	//alert(IspayorObj.value);
	//alert(IsplanObj.value);
...
function PopUpExec(ordItmMastId,ordExecId,ordId) {
	//alert("Come in to PopUpExec "+ordItmMastId+"*"+ordExecId);
	var valPatient;
	var patobj=document.getElementById("PatientID");
...
function PopUpExecWithoutExecTime(ordId) {
	//alert("Come in to PopUpExecWithoutExecTime "+ordId);
	var valPatient;
	var patobj=document.getElementById("PatientID");
	if (patobj) valPatient=patobj.value;
...
function DisableUpdateButton(Disable){
	...
	//alert("in DisableUpdateButton "+Disable);
...
function StartDateInRange(DoNotClickDeleteWhenNotContinue) {   // *****  Log# 30710; AmiN ; 17/Dec/2002 Start Date must be within Episode Admin Date and Discharge date *****
	...
	//alert("73 xx OEORISttDat ^ DischDate ^ AdmDate = "+sobj.value+"^"+dobj.value+"^"+aobj.value);
...
function matchCategory(Type,orderCat,orderSubCat,orderCode) {
	...
	//alert(Type+"*"+orderCat+"*"+orderSubCat+"*"+orderCode);
	if (orderCat=="" || orderSubCat=="") {
		var NewOrders=cNewOrders.split("^");
		for (var b1=0;b1<NewOrders.length-1;b1++) {
			//alert("order is "+mPiece(mPiece(cNewOrders,"^",b1),"*",0));
			//alert("order is:"+cNewOrders);
	...
	if  (Type=="OS") {
		...
		//alert(objOSCat.value+"*"+objOSSubCat.value);
		if (objOSCat && objOSSubCat) {
			...
			for (i=0;i<grpOSCatArray.length;i++) {
				//alert("Cat:"+orderCat+","+grpOSCatArray[i]);
				if ((orderCat==grpOSCatArray[i])&&(orderCat!="")) {
					return true;
				}
			}
			for (i=0;i<grpOSSubCatArray.length;i++) {
				//alert("Sub-Cat:"+orderSubCat+","+grpOSSubCatArray[i]);
		...
		//alert(objIMCat.value+"*"+objIMSubCat.value);
...
function AddInput(selList,value){
	...
	//alert("newelement value on update="+NewElement.value);
		...
		if (obj.options[i].selected==true) {
			...
			if (doAdd) {
				...
				if(mPiece(obj.options[i].value,String.fromCharCode(4),0)=="ARCOS") {
					// BM  for orderset
					OSItemsWithoutDesc="";
					//alert("Place 0:"+obj.options[i].value);
...
function PricesClickHandler(evt) { 	//form string of items to check prices first
	...
	for (var i=0; i<lstOrders.options.length; i++) {
		...
		//alert("lstOrders.options[i].id="+mPiece(lstOrders.options[i].id,String.fromCharCode(4),2));
		...
		//alert("lstOrders.options[i].value="+lstOrders.options[i].value);
		//alert("lstOrders.options[i].itype="+lstOrders.options[i].itype);
...
function NonFormularyClickHandler() {
	//alert("NonFormulary:"+window.currTab);
	var CurrTab="";
...
function CreateXHZYSave(){   //上传保存
	...
	if ((EpisodeID!="")){
		...
		//alert(EpisodeID+"&"+session['LOGON.USERCODE']+"&"+Orders+"&"+escapeDHCDTPrescXML+"&"+UploadFlag);
  ...
  try{
  	var Str="";
			...
			for (var i=1; i<rows; i++){
			    ...
			    var OrderFlowRateUnit=GetColumnData("OrderFlowRateUnit",Row);
				  ...
				  var iother=;
					...
					Str=Str+"^"+OrderFlowRateUnit+"^"+OrderDate+"^"+OrderTime+"^"+OrderNeedPIVAFlag+"^"+OrderAntibApplyRowid+"^"+AntUseReason+"^"+OrderPriorRemarks;
																																																																																																																																		 										
					if (UnsaveData==""){UnsaveData=Str}else{UnsaveData=UnsaveData+String.fromCharCode(2)+Str;}
				}
				if ((UnsaveData!="")&&(HospitalCode=="ZHSRMYY")) alert(t['OrderIsNotSave']);
			}
		}
		/*
		if (UserUnSaveData!=''){
			var RetCheck=confirm((t['NO_SAVE']),false);
		...
		*/
		...
		//alert("Unload"+SessionFieldName+":"+UnsaveData);
	} catch(e) {alert(e.message);}
...
function AddUnsaveDataToList(UnsaveData){
	...
	var objtbl=document.getElementById('tUDHCOEOrder_List_Custom');
	var UnsaveArr=UnsaveData.split(String.fromCharCode(2));
	for (var j=0;j<UnsaveArr.length;j++) {
		var ret=CanAddRow(objtbl);
		if (ret){AddRowToList(objtbl);}
	  	var RowIndex=FocusRowIndex;
			var icode=mPiece(UnsaveArr[j],"^",0);
			var packqtystr=mPiece(UnsaveArr[j],"^",1);
			var doseqtystr=mPiece(UnsaveArr[j],"^",2);
			var reclocstr=mPiece(UnsaveArr[j],"^",3);
			var frequencestr=mPiece(UnsaveArr[j],"^",4);
			var instructionstr=mPiece(UnsaveArr[j],"^",5);
			var durationstr=mPiece(UnsaveArr[j],"^",6);
			var defpriorstr=mPiece(UnsaveArr[j],"^",7);
			var lab=mPiece(UnsaveArr[j],"^",8);
		  var other=mPiece(UnsaveArr[j],"^",9);
		  var OrderHiddenPara=mPiece(UnsaveArr[j],"^",10);
		  var OrderMultiDate=mPiece(UnsaveArr[j],"^",11);
			var Arr=OrderMultiDate.split(String.fromCharCode(1));
			OrderMultiDate=Arr.join(String.fromCharCode(2));

			var OrderSeqNo=mPiece(UnsaveArr[j],"^",12);
			var OrderMasterSeqNo=mPiece(UnsaveArr[j],"^",13);
			var OrderBillTypeRowid=mPiece(UnsaveArr[j],"^",14);
			var OrderDIACatRowId=mPiece(UnsaveArr[j],"^",15);
			var OrderFirstDayTimes=mPiece(UnsaveArr[j],"^",16);
			var OrderSpeedFlowRate=mPiece(UnsaveArr[j],"^",17);
			var OrderStartDate=mPiece(UnsaveArr[j],"^",18);
			var OrderStartTime=mPiece(UnsaveArr[j],"^",19);
			//可用接收科室串
			var OrderRecLocStr=mPiece(UnsaveArr[j],"^",20);
			var OrderFlowRateUnit=mPiece(UnsaveArr[j],"^",21);
			var OrderDate=mPiece(UnsaveArr[j],"^",22);
			var OrderTime=mPiece(UnsaveArr[j],"^",23);
			var OrderNeedPIVAFlag=mPiece(UnsaveArr[j],"^",24);
			var OrderAntibApplyRowid=mPiece(UnsaveArr[j],"^",25);
			//抗生素使用原因
			var AntUseReason=mPiece(UnsaveArr[j],"^",26);
			//附加说明
			var OrderPriorRemarks=mPiece(UnsaveArr[j],"^",27);
			if (OrderRecLocStr!=''){
				var OrderRecLocArr=OrderRecLocStr.split(String.fromCharCode(3));
				OrderRecLocStr=OrderRecLocArr.join(String.fromCharCode(2));
			}
			
			//跨院标志
			var OrderOpenForAllHosp=mPiece(reclocstr,String.fromCharCode(1),1);
  		...
  		//var OrderPackQty=mPiece(ipackqtystr,String.fromCharCode(1),0);
  		var OrderPackQty=mPiece(packqtystr,String.fromCharCode(1),0);
  		var OrderPackUOM=mPiece(ipackqtystr,String.fromCharCode(1),1);
  		var OrderPackUOMRowid=mPiece(ipackqtystr,String.fromCharCode(1),2);
  		/*
  		var OrderFreq=mPiece(ifrequencestr,String.fromCharCode(1),0);
		  ...
		  var OrderDurFactor=mPiece(idurationstr,String.fromCharCode(1),2); 
		  */
  		var OrderFreq=mPiece(frequencestr,String.fromCharCode(1),0);
		  var OrderFreqRowid=mPiece(frequencestr,String.fromCharCode(1),1);
		  var OrderFreqFactor=mPiece(frequencestr,String.fromCharCode(1),2);
		  var OrderFreqInterval=mPiece(frequencestr,String.fromCharCode(1),3);
		  var OrderInstr=mPiece(instructionstr,String.fromCharCode(1),0);
		  var OrderInstrRowid=mPiece(instructionstr,String.fromCharCode(1),1);
		  var OrderDur=mPiece(durationstr,String.fromCharCode(1),0);
		  var OrderDurRowid=mPiece(durationstr,String.fromCharCode(1),1);
		  var OrderDurFactor=mPiece(durationstr,String.fromCharCode(1),2); 
		  var EmergencyFlag=mPiece(iskintest,String.fromCharCode(1),3); 

			/*
		  var OrderDefPriorRowid= mPiece(idefpriorstr,String.fromCharCode(1),1); 
		  */
		  var OrderPriorRowid= mPiece(defpriorstr,String.fromCharCode(1),0); 
	  	...
	  	var OrderFile2=mPiece(iordermsg,String.fromCharCode(1),2);
	  	var OrderFile3=mPiece(iordermsg,String.fromCharCode(1),3);
  		var OrderARCOSRowid=mPiece(other,String.fromCharCode(1),0);
  		var OrderSkinTest=mPiece(other,String.fromCharCode(1),1);
  		var OrderCoverMainIns=mPiece(other,String.fromCharCode(1),2);
  		var OrderActionRowid=mPiece(other,String.fromCharCode(1),3);
  		var OrderNotifyClinician=mPiece(other,String.fromCharCode(1),4);
  		var OrderDepProcNote=mPiece(other,String.fromCharCode(1),5);
  		var OrderInsurCatRowId=mPiece(other,String.fromCharCode(1),6);
  		var OrderInsurCat=mPiece(other,String.fromCharCode(1),7);
  		var OrderInsurSignSymptomCode=mPiece(other,String.fromCharCode(1),8);
  		var OrderInsurSignSymptom=mPiece(other,String.fromCharCode(1),9);
  		//因为在关闭界面的医嘱拼串分隔符和适应症串分隔符有冲突??换了分隔符??所以在重新载入时需要恢复原来的分隔符号$C(2)
  		if (OrderInsurSignSymptomCode!=""){
	  		var DSYMArr=OrderInsurSignSymptomCode.split(String.fromCharCode(3));
				OrderInsurSignSymptomCode=DSYMArr.join(String.fromCharCode(2));
			}
			//身体部位
  		var OrderBodyPart=mPiece(other,String.fromCharCode(1),10);
  		//医嘱阶段
  		var OrderStageCode=mPiece(other,String.fromCharCode(1),11);
  		//药品通用名
  		var OrderGenericName=mPiece(other,String.fromCharCode(1),12);
			//审批类型
			var OrderInsurApproveTyp=mPiece(other,String.fromCharCode(1),13);
			//临床路径步骤
			var OrderCPWStepItemRowId=mPiece(other,String.fromCharCode(1),14);
			//高值材料条码
			var OrderMaterialBarCode=mPiece(other,String.fromCharCode(1),15);

			var OrderRecDepRowid=mPiece(reclocstr,String.fromCharCode(1),0);
	 	  var OrderDoseQty=mPiece(doseqtystr,String.fromCharCode(1),0);
	 	  var OrderDoseUOMRowid=mPiece(doseqtystr,String.fromCharCode(1),2);
		  ...
		  SetColumnData("OrderAction",Row,OrderActionRowid);  
			...
			SetColumnData("OrderSpeedFlowRate",Row,OrderSpeedFlowRate);
			SetColumnData("OrderGenericName",Row,OrderGenericName);
			SetColumnData("OrderInsurApproveTyp",Row,OrderInsurApproveTyp);
  		...
  		if (OrderCoverMainIns==false) SetColumnData("OrderCoverMainIns",Row,false);
...
function InsertMRDiagnos(){
	if (document.getElementById("MRDiagnoseCount").value!=0){alert("诊断已存在!");return;};
	...
	var yMRDIADesc="";
	//alert(yLogDepRowid+"^"+yMRAdmRowid+"^"+yICDCodeRowid+"^"+yUserRowid+"^"+yMRDIADesc);
	...
	var obj=MRDiagnosRowid.split("^")[0]
		if (obj) {
			...
			if (encmeth!="") {
				...
				//alert(encmeth);
				//alert(EpisodeID+"^"+Doc+"^"+session['LOGON.CTLOCID']+"^"+uuser);
...
function AddRemarkClickhandler(rowindex){
	...
	var remark=GetColumnData("OrderDepProcNote",focusrowid)
	var rtnvalue=window.showModalDialog("websys.default.csp?WEBSYS.TCOMPONENT=DHCOPDoc.Remark.Edit&frowid=&remark="+remark,"","dialogwidth:500px;dialogheight:300px;status:no;center:1;resizable:yes")
	if (rtnvalue)	{
		SetColumnData("OrderDepProcNote",focusrowid,rtnvalue);
	}
}
...
function BtnPrtGuidPatHandler(){
  ...
  var obj=document.getElementById('EpisodeID');
	if (obj){EpisodeID=obj.value;}
	if (EpisodeID!=""){
		window.open("websys.default.csp?WEBSYS.TCOMPONENT=DHCDocPatGuideDocumentsPrt&EpisodeID="+EpisodeID,"","scrollbars=no,resizable=no,top=6,left=6,width=1000,height=680,status=yes");
		//window.showModalDialog("websys.default.csp?WEBSYS.TCOMPONENT=DHCDocPatGuideDocumentsPrt&EpisodeID="+EpisodeID,"","dialogwidth:1050px;dialogheight:600px;status:yes;center:1;resizable:yes");
	}
}
...
function CheckPriorAllowRange(OrderPriorRowid,OrderARCIMRowid){
	if (CheckPriorAllowRangeMethod!=""){
		//alert(OrderPriorRowid+","+OrderARCIMRowid)
		...
		*/
		...
		if ((OrderARCIMRowid!="")&&(OrderItemRowid=="")&&(OrderSeqNo==CurrentOrderMasterSeqNo)){
			...
			if ((OrderRecDepRowid!=CurrentOrderRecDepRowid)&&(CFSameRecDepForGroup==1)){
				alert(OrderName+t['SubOrderRecDepNotMatch']);
				return false;
			}
			
			if (OrderFreqRowid!=CurrentOrderFreqRowid){
				alert(OrderName+t['SubOrderFreqNotMatch']);
				return false;
			}
			if (OrderStartTime!=CurrentOrderStartTime){
				alert(OrderName+t['SubOrderStartTimeNotMatch']);
				return false;
			}
			/*
...
function SetSaveForUserClickHandler(){
	var eSrc = window.event.srcElement;
	if (eSrc) {
		if (FocusRowIndex < 1) {alert("请选择行!");return websys_cancel();}
		//添加到常用医嘱
		var Row=GetRow(FocusRowIndex);
		var OrderType=GetColumnData("OrderType",Row);
	  if (OrderType!="R") {alert("只允许添加药品!");return websys_cancel();}
...
function GetContralStr(ContralType,ContralKey,Row){
	...
	var ContralStr=OrderARCIMRowid+"^"+ContralType+"^"+ContralKey+"^"+OrderPriorRowid+"^"+OrderDoseQty+"^"+OrderDoseUOMRowid;
	    ...
	    ContralStr=ContralStr+"^"+OrderActionRowid+"^"+OrderMasterSeqNo+"^"+OrderSeqNo+"^"+Notes;
...
function GetItemDefaultRowId(icode) {
	...
	if (GetUserItemDefaultSingleMethod!="") {
		var ItemDefaultRowIds=cspRunServerMethod(GetUserItemDefaultSingleMethod,icode,userid);
...
function BtnViewArcItemInfoHandler(){
	...
	var lnk="websys.default.csp?WEBSYS.TCOMPONENT=UDHCJFArcimPrice";
	var NewWin=open(lnk,"UDHCJFArcimPrice","scrollbars=yes,resizable=yes,top=6,left=6,width=1000,height=680");
}
...
function OrderPriorRemarkschangehandler(e){
	...
	if (OrderPriorRemarks=="ONE"){
		...
		var OrderMasterSeqNo=GetColumnData("OrderMasterSeqNo",Row);
	 	...
	 	var OrderSeqNo=GetColumnData("OrderSeqNo",Row);
	 	for (i=1;i<=rows;i++){
	 		var MasterOrderSeqNo=GetColumnData("OrderMasterSeqNo",i);
...
function CheckDoctorTypePoison(PoisonRowId,ArcimRowid,Row){
	...
	for (j=0;j<Arr.length;j++) {
		...
		if (PoisonRowId==TypePoisonRowId){
			if (TypeControl=='F'){
				alert(t['PoisonClassIsFobidden']);
				return false;
			}
			if (TypeControl=='A'){
				var ret=confirm((t['PoisonClassNeedConfirm']),false);
  			return ret;
  		}
  		if (TypeControl=='P'){
  			...
  			//var ret=window.open("websys.default.csp?WEBSYS.TCOMPONENT=DHCDocAntibioticApply&ArcimRowid="+ArcimRowid+"&EpisodeID="+PAADMRowid,"","top:50;left:200;status:yes;center:1;width:900;height:600;resizable:yes");
				if ((ret=="")||(ret=="undefined")||(ret==null)) {
					//return false;
					var infoArr=false+"||"+ret
					return infoArr;
				}else {
					var OrderAntibApplyInfoArr=ret.split("^");
					var OrderAntibApplyRowid=OrderAntibApplyInfoArr[0];
					var AntUseReason=OrderAntibApplyInfoArr[1];
					var OrderInstrRowid=OrderAntibApplyInfoArr[2];
					var OrderInstr=OrderAntibApplyInfoArr[3];
					SetAntibioticRowid(OrderAntibApplyRowid);
					SetColumnData("AntUseReason",Row,AntUseReason);
					SetColumnData("OrderInstrRowid",Row,OrderInstrRowid);
					SetColumnData("OrderInstr",Row,OrderInstr);
					//return true;
					var infoArr=true+"||"+ret
					return infoArr;
				}	
  		}
  		if (TypeControl=='S'){
...
function CPWCheck(){
	...
	var ret=tkMakeServerCall("web.DHCCPW.MR.ClinPathWaysVariance","CheckOutARCIM",EpisodeID,ArcimIDs);
	if (!ret) return true;
...
//得到ARCIMrowid串
function GetOrderARCIMRowidStrMethod(){
	var objtbl=document.getElementById('tUDHCOEOrder_List_Custom');
	var rows=objtbl.rows.length;
	var OrderARCIMRowidStr=""
	for (var i=1; i<rows; i++){
		var Row=GetRow(i);
		var OrderARCIMRowid=GetColumnData("OrderARCIMRowid",Row);
		OrderARCIMRowidStr=OrderARCIMRowid+"^"+OrderARCIMRowidStr 
	}
	return OrderARCIMRowidStr;
}
