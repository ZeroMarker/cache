...
function BodyLoadHandler()
 {	
    ...
    document.getElementById("POrditemDR").value=POrderValue
   	...
   	if (ComName=="") ComName="DHCPISAppSZBD"
 ...
 function GetWomanTumourInfo()
 {
      ...
      if (GetTumourInfo)
      {
	     ...
	     if(bNewPatient==1)
	     {
			...
			document.getElementById('OperationDoctor').disabled=false
			document.getElementById('OperationStartDate').disabled=false
			document.getElementById('OperationStartTime').disabled=false
			document.getElementById('OperationRoom').disabled=false
			document.getElementById('YJSJSJ').disabled=false
			...
			document.getElementById('YWPathDiagnosis').disabled=false
	     }
         else
	     {
			...
			document.getElementById('OperationDoctor').disabled=true
			document.getElementById('OperationStartDate').disabled=true
			document.getElementById('OperationStartTime').disabled=true
			document.getElementById('OperationRoom').disabled=true
			document.getElementById('YWPathDiagnosis').disabled=true
			document.getElementById('YJSJSJ').disabled=true
 ...
 { 
    ...
    if ((SpeNum=="")||(SpePos.replace(/[ ]/g,"")=="")||(TMrowid=="")||(SpeMemo.replace(/[ ]/g,"")=="")||(isNaN(SpeMemo.replace(/[ ]/g,""))))
    {
	   alert(t['NOTNULL']);
	   return;
    }
 ...
 { 
    ...
    var SpeMemo=document.getElementById("SpeMemo").value;
    var SpeRowId=document.getElementById("SpeRowId").value;
    if ((SpeNum=="")||(SpePos.replace(/[ ]/g,"")=="")||(SpeMemo.replace(/[ ]/g,"")=="")||(isNaN(SpeMemo.replace(/[ ]/g,""))))
    {
	   alert(t['NOTNULL']);
	   return;
    }
 ...
 { 
 	//validate specimens
 	var Objtbl=document.getElementById('tDHCPISAppSZBD')
	...
	if(Rows<1)
	{
 	...
 	var on=Trim(document.getElementById('OperationName').value)
	var od=Trim(document.getElementById('OperationDoctor').value)
	var osd=Trim(document.getElementById('OperationStartDate').value)
	var ost=Trim(document.getElementById('OperationStartTime').value)
	var or=Trim(document.getElementById('OperationRoom').value)
	var sj=Trim(document.getElementById('YJSJSJ').value)
	var yw=Trim(document.getElementById('YWPathDiagnosis').value)
	if((on=="")||(od=="")||(osd=="")||(ost=="")||(sj=="")||(or=="")||(yw==""))
 	{
 		alert(t['SSNotNull'])
 		return;
 	}
 ...
 function Refresh()
 {
    ...
    var ClinicRecord=document.getElementById("ClinicRecord").value;
    var on=document.getElementById('OperationName').value
	var od=document.getElementById('OperationDoctor').value
	var osd=document.getElementById('OperationStartDate').value
	var ost=document.getElementById('OperationStartTime').value
	//var sjrq=document.getElementById('YJSJRQ').value
	var sjsj=document.getElementById('YJSJSJ').value
    var OperationSee=document.getElementById("OperationSee").value;
    var ClinicDiag=document.getElementById("ClinicDiag").value;
    ...
    lnk+="&OperationName="+on;
    lnk+="&OperationDoctor="+od;
    lnk+="&OperationStartDate="+osd;
    lnk+="&OperationStartTime="+ost;
    //lnk+="&YJSJRQ="+sjrq;
    lnk+="&YJSJSJ="+sjsj;
...
{	
	var myobj=document.getElementById("PrintSheet")
	if(myobj.disabled)
	{
		return;	
	}
...
//alert("1")
	...
	if(document.getElementById("Frost").checked==true)
      ...
      else
      {
		...
		xlsheet.cells(10,4)=document.getElementById("OperationName").value;        //手术名称
	    xlsheet.cells(10,21)=document.getElementById("OperationDoctor").value;      //主术者
		xlsheet.cells(11,21)=document.getElementById("OperationRoom").value;      //手术间
	    var osd=document.getElementById("OperationStartDate").value
	    if(osd!="")
      	{
	  		var item=osd.split("/")
      		osd=item[2]+"-"+item[1]+"-"+item[0];   //发现日期
      	}
	    xlsheet.cells(11,5)=osd+" "+document.getElementById("OperationStartTime").value;      //开始日期
		xlsheet.cells(11,15)=document.getElementById("YJSJSJ").value;      //预计送检日期
		
        var GetSpecimensFunc=document.getElementById("autoNumAdd").value
	    var info=cspRunServerMethod(GetSpecimensFunc,ttmrowid)
	    xlsheet.cells(13,1)=info                                                //标本信息
	    xlsheet.cells(15,1)=document.getElementById("ClinicRecord").value;        //临床病历
	    xlsheet.cells(17,1)=document.getElementById("OperationSee").value;      //手术所见
	    xlsheet.cells(19,1)=document.getElementById("YWPathDiagnosis").value;
    ...
    for (j=0;j<tmpList.length-1;j++)
                {
                    ...
                    Bar.PatLoc=PatLoc;
                    //Bar.PatWard=RoomNo;
                    Bar.OrdName=OrderName+"--"+specinfo;
                    Bar.PatName=PName;
...
function GettclinicRecord()
{
	var on=document.getElementById('OperationName').value
	var od=document.getElementById('OperationDoctor').value
	var osd=document.getElementById('OperationStartDate').value
	var ost=document.getElementById('OperationStartTime').value
	var sj=document.getElementById('YJSJSJ').value
	var cr=document.getElementById('ClinicRecord').value
	var or=document.getElementById('OperationRoom').value
	var yw=document.getElementById('YWPathDiagnosis').value
	tclinicRecord=t['SHMC']+on+";"+t['ZSZ']+od+";"+t['SSKSRQ']+osd+" "+ost+";"+t['YJSJSJ']+sj+";"+t['LCBL']+cr+";"+t['OR']+or+";"+t['YW']+yw
}

function SplittclinicRecord()
{
	
	var item=tclinicRecord.split(";")
  if(item[0].split(t['SHMC'])[1]==undefined)
  {
  	document.getElementById('OperationName').value=""
  }
  else
  {
  	document.getElementById('OperationName').value=item[0].split(t['SHMC'])[1]
  }
  
  if(item[1]==undefined)
  {
  	document.getElementById('OperationDoctor').value=""
  }
  else
  {
  	document.getElementById('OperationDoctor').value=item[1].split(t['ZSZ'])[1]
  }
	//document.getElementById('OperationDoctor').value=item[1].split(t['ZSZ'])[1]
	if(item[2]==undefined)
	{
		document.getElementById('OperationStartDate').value=""
		document.getElementById('OperationStartTime').value=""
	}
	else
	{
			var ssks=item[2].split(t['SSKSRQ'])[1]
			document.getElementById('OperationStartDate').value=ssks.split(" ")[0]
	    document.getElementById('OperationStartTime').value=ssks.split(" ")[1]
	}
	if(item[3]==undefined)
	{
		//alert(document.getElementById('YJSJSJ').value)
		document.getElementById('YJSJSJ').value=""
	}
	else
	{
		document.getElementById('YJSJSJ').value=item[3].split(t['YJSJSJ'])[1]
	//document.getElementById('YJSJRQ').value=yj.split(" ")[0]
	//document.getElementById('YJSJSJ').value=yj.split(" ")[1]
	}
	//alert(item[4])
  if(item[4]==undefined)
  {
  	document.getElementById('ClinicRecord').value=""
  }
  else
  {
    document.getElementById('ClinicRecord').value=item[4].split(t['LCBL'])[1]
  }
	if(item[5]==undefined)
  {
  	document.getElementById('OperationRoom').value=""
  }
  else
  {
    document.getElementById('OperationRoom').value=item[5].split(t['OR'])[1]
  }
  if(item[6]==undefined)
  {
  	document.getElementById('YWPathDiagnosis').value=""
  	document.getElementById('OperationSee').value=""   //手术所见
  }
  else
  {
    document.getElementById('YWPathDiagnosis').value=item[6].split(t['YW'])[1]
  }
}
...
function womenInfo()
{
	...
	}else if((sex==t['SEXNV'])&&(bNewPatient!=1))
	{
		document.getElementById("LastCata").disabled=true
	...
	{
		document.getElementById("LastCata").style.display ="none"
 		document.getElementById("EndCata").style.display ="none"
		document.getElementById("Tai").style.display ="none"
		document.getElementById("Chan").style.display ="none"
		document.getElementById("cLastCata").style.display ="none"
 		document.getElementById("cEndCata").style.display ="none"
		document.getElementById("cTai").style.display ="none"
		document.getElementById("cChan").style.display ="none"
		document.getElementById("cscHistory").style.display ="none"
		document.getElementById("ld51940iLastCata").style.display ="none"
	}	
}
