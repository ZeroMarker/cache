...
function BDelete_Clicked()
{
	...
	else
	{
		...
		var url="dhceq.plat.invoicebindinstocklist.csp?&RowID="+"&BussType="+getElementValue("BussType");
		if ('function'==typeof websys_getMWToken){		//czf 2023-02-14 token启用参数传递
			url += "&MWToken="+websys_getMWToken()
		}
	    window.location.href= url	// Mozy003005	1248904		2020-4-1
	}
...
function RemoveGridData()
{
	...
	if (combindata=="")
	{
		$.messager.popover({msg: '请先选择需要取消关联的记录',type:'error',timeout: 1000});		
		return;
	}
...
function BSave_Clicked()
{
	...
	var url="dhceq.plat.invoicebindinstocklist.csp?&RowID="+Rtn[1]+"&BussType="+getElementValue("BussType");
	if ('function'==typeof websys_getMWToken){		//czf 2023-02-14 token启用参数传递
		url += "&MWToken="+websys_getMWToken()
	}
    window.location.href= url;	// Mozy003005	1248904		2020-4-1
