...
//名称       dhcpe/ct/dhcpe.ct.ordersets.js
//功能       体检套餐维护（多院区）
//创建日期  
//创建人     zhongricheng
...
function InitARCOSDataGrid() {
	...
	$HUI.datagrid("#OrderSetsList", {
		...
		columns:[[
			...
			{field:'BreakDesc', title:'拆分', hidden:true},  // 拆分
			{field:'SexDesc', title:'对应性别', align:'center'},  // 性别
			{field:'VIPDesc',title:'套餐等级'},
			{field:'DeitDesc',title:'早餐', hidden:true},
			{field:'DateBeg',title:'开始日期'},
			{field:'DateEnd',title:'截止日期'},
			{field:'PGDesc',title:'可用团体'},
			{field:'Sort',title:'顺序'}, 
			{field:'Frequency',title:'使用频次', hidden:true}, 
			...
			*/
			{field:'Disable', title:'Disable', hidden:true},  // 启用/禁用 标识
			{field:'DisableDesc', title:'状态', align:'center'}  // 启用/禁用 描述															 
		]],
		...
		*/
		{
			...
			handler: function(){
				...
				$("#SortWin").numberbox("setValue", SelRowData.Sort);  // 显示顺序
				$("#EffDateBeginWin").datebox("setValue", SelRowData.DateBeg);  // 生效日期
				$("#EffDateEndWin").datebox("setValue", SelRowData.DateEnd);  // 截止日期
		...
		*/
		...
		onClickRow: function (rowIndex, rowData) {  // 选择行事件 	
			...
			$('#BDisabled').linkbutton(rowData.Disable=="Y"?{text:'启用',iconCls:'icon-accept'}:{text:'禁用',iconCls:'icon-forbid'});
		},
...
function BAdd_click(type){
	...
	if (LocId == "") {
		$.messager.alert("提示","无法获取当前科室！", "info");
		return false; 
	}
	...
	*/
	...
	if((EffDateBeg!="")&&(EffDateEnd!="")){ 
		...
		if(iEffDateBeg>=iEffDateEnd) {
        	$.messager.alert("提示","生效日期不能大于截止日期！","info");
        	return false;
    	}
	}
	...
	if (type == "Add") {
		...
		$.m({
			...
			$("#OrderSetsList").datagrid("insertRow", {
				...
				row:{
					...
					DateBeg:OSExData.DateBeg,  // 开始日期
					DateEnd:OSExData.DateEnd,  // 结束日期
					Sort:OSExData.Sort,  // 顺序
					Frequency:OSExData.Frequency,  // 使用频次
			...
			BClear_click("1");
			//$.cm({ClassName:"web.DHCPE.CT.OrderSets", MethodName:"GetARCItemCatID"}, function(Data){ $("#CategoryWin").val("医嘱套"); $("#csubCatIDWin").val(Data.ARCItemCatDesc); $("#CategoryIDWin").val(Data.Category); $("#subCatIDWin").val(Data.ARCItemCatID);});
		...
		if (upddata.code == "1") {
			$.messager.alert("提示", upddata.msg, "info");
			return false;
		}
...
function BDisable_click() {
	var SelRowData = $("#OrderSetsList").datagrid("getSelected");
	var SelRowIndex = $("#OrderSetsList").datagrid("getRowIndex", SelRowData);
	if ( !SelRowData ) { $.messager.alert("提示", "请选择医嘱套进行操作！", "info"); return false; }
	var UserID = session["LOGON.USERID"];
	var FavRowid = SelRowData.FavRowid;
	var ARCOSRowid = SelRowData.ARCOSRowid;
	var Disabled = SelRowData.Disable;
	var ADisable = (Disabled=="Y"?"启用":"禁用");
	
	$.messager.confirm("确认", "确定" + ADisable + "该套餐吗？", function(r) {
		if (r){
			$.m({
				ClassName:"web.DHCPE.CT.OrderSets",
				...
				if (ReturnValue != "0") {
					$.messager.alert("提示", ReturnValue + ADisable + "失败。", "info");
				}else{
					$.messager.alert("提示", "医嘱套" + ADisable + "成功。", "info");
					$("#OrderSetsList").datagrid("updateRow", {
								index: SelRowIndex,
								row:{
										Disabled:Disabled,				// 启用/禁用 标识
										DisableDesc:ADisable			// 启用/禁用 描述
								}
					});
					// 需要清空查询条件
					BClear_click("0");
...
function BClear_click(Type) {
	...
	$("#SortWin").numberbox("setValue",""); //显示顺序
	$("#EffDateBeginWin").datebox("setValue","");//生效日期
	$("#EffDateEndWin").datebox("setValue",""); //日期日期
	BSearch_click();
}
		...
		**/
		{
			...
			handler: function(){
				...
				if (ItemPriceEnable == "Y") {
					...
					websys_showModal({title:'套餐定价-' + ARCOSDesc, url:PEURLAddToken(lnk), width:'700',height:'600',iconCls:'icon-w-edit',onClose:function(){
						...
						$("#OrderSetsList").datagrid("updateRow", {
							index: SelRowIndex,
							row:{
								OSExPrice:OSData.ARCOSPrice+"元", // OSExData.OSExPrice,  // 价格
							}
						});
						$("#OrderSetsDetailList").datagrid("reload");
						InitToolbarForARCOSPrice(ARCOSRowid);
					}});
					//websys_lu(lnk,"aaaaaaa","iconCls=icon-w-edit,width=700,height=600,hisui=true,title=套餐定价-" + ARCOSDesc);
					return false;
				} else {
		...
		$.m({
			...
			if ( ret == 0 ) {
				...
				if (ItemPriceEnable == "Y") {
					...
					if (preturn != "0") {
						$.messager.alert("提示", "明细删除成功，套餐价格更新失败，请重新维护价格！" + preturn, "info");
					} else {
						$.messager.alert("提示","删除成功！", "info");
					}
					var iPrice = $.cm({ClassName:"web.DHCPE.CT.OrderSets", MethodName:"UpdateOSExPriceNew", ARCOSRowid:ARCOSRowid}, false);
...
function IAdd(AddType,Id,AddAmount,Index,tablename){
	...
	$.cm({
		...
		if (json.code == 1) {
			...
			$.messager.confirm("确认", json.msg, function(r){
				if (r) {
...
function IAddItem(SelOSRowData, ARCOSRowid, Id, SampleId, DHCDocOrdRecLoc) {
	...
	if (itemInfo.ItemOrderType == "R") {
		...
		GetDrugByArcIM(Id); //根据医嘱项初始化药品附加信息	
					...
					var data = $.m({
						...
						if (ret.indexOf("||") > 0) {
							if (ItemPriceEnable == "Y") {
								...
								if (preturn != "0") {
									$.messager.alert("提示", "明细增加成功，套餐价格更新失败，请重新维护价格！" + preturn, "info");
								}
								$("#OrderSetsDetailList").datagrid("reload");
				...
				$.messager.confirm("确认", "该套餐已定价，若再添加项目则会恢复原价，确认添加？", function(r){
					if (r){
						...
						if (rtn == "1") {
							...
							$.messager.alert("提示", "项目复制失败！", "info");
						} else if (rtn == "-2") {
							$.messager.alert("提示", "项目价格更新失败！", "info");
						} else if (rtn == "-3") {
							$.messager.alert("提示", "套餐价格更新失败！", "info");
						} else if (rtn == "-101") {
							$.messager.alert("错误", "程序报错", "error");
						}
					}
				});	
			} else if (amtflag == "-1") {
...
function InitNetSetsWinNew(NetOSInfo) {
	...
	$("#Win_NetDesc").val(NetOSInfo.Desc).validatebox("validate");  // 描述
	$("#Win_NetSort").numberbox("setValue",NetOSInfo.Sort).validatebox("validate");  // 序号
				
	
	$("#Win_NetSex").combobox("setValue", NetOSInfo.Sex);  // 性别
	$("#Win_NetRemark").val(NetOSInfo.Remark);  // 备注
				
	if (NetOSInfo.GIFlag == "I") $("#Win_NetGroup").checkbox("setValue", false);  // 团体
	else $("#Win_NetGroup").checkbox("setValue", true);
	if (NetOSInfo.ActiveFlag == "Y") $("#Win_NetActive").checkbox("setValue", true);  // 激活
...
function update_netsets(){
	...
	if (NOSHisSetsID == "") { $.messager.alert("提示", "未选择医嘱套!", "info"); return false; }
		
	NOSDesc = $("#Win_NetDesc").val();  // 描述
	if (NOSDesc == "") { $.messager.alert("提示", "描述不能为空!", "info"); return false; }
		
	NOSSort = $("#Win_NetSort").numberbox("getValue");  // 序号
	if (NOSSort == "") { $.messager.alert("提示", "序号不能为空!", "info"); return false; }
		
	NOSVIPLevel = ""; //$("#Win_NetLevel").combobox("getValue");  // 套餐等级
	//if (NOSVIPLevel == "") { $.messager.alert("提示", "套餐等级不能为空!", "info"); return false; }
		
	NOSSex = $("#Win_NetSex").combobox("getValue");  // 性别
	if (NOSSex == "") { $.messager.alert("提示", "性别不能为空!", "info"); return false; }
	NOSRemak = $("#Win_NetRemark").val();  // 备注
	NOSLocID = session["LOGON.CTLOCID"];  // 科室
		
	var cIsIG = $("#Win_NetGroup").checkbox("getValue");  // 团体
	if (cIsIG) { NOSGIFlag = "G"; }
	else { NOSGIFlag = "I"; }
			    	
	var cIsActive = $("#Win_NetActive").checkbox("getValue");  // 激活
	if (cIsActive) { NOSActive = "Y"; }
	else { NOSActive = "N"; }
	...
	$.m({ClassName:"web.DHCPE.NetPre.OrdSetsInfo", MethodName:"SaveOrdSets", ID:ID, StrInfo:StrInfo},
		function(ret) {
			if (ret.split("^")[0] == "-1") { $.messager.alert("提示", "保存失败!" + ret.split("^")[1], "info"); return false; }
			else { $.messager.alert("提示", "保存成功!", "info",cancel_netsets);  }
		}
	);
}
