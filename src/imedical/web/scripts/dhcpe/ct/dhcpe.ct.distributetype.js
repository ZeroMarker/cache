/**
 * 医生分配类型  dhcpe.ct.distributetype.js
 * @Author   wangguoying
 * @DateTime 2021-02-01
 */
...
var _tableNme = "DHC_PE_WorkDistributeType";
...
function initSetp(){
	...
	$.cm({
	    wantreturnval: 1,
	    ClassName: 'web.DHCPE.CT.WorkDistribution',
...
function setActiveFLow(){
	...
	var activeFlow=tkMakeServerCall("web.DHCPE.CT.WorkDistribution","GetActiveType",locId);
...
function switch_change(event,obj){
    ...
    var ret = tkMakeServerCall("web.DHCPE.CT.WorkDistribution","UpdateTypeByCode",flowObj.code,active,session["LOGON.CTLOCID"]);
    if(ret != ""){
        $.messager.alert("提示",msg+"失败："+ret,"error");
        return false;
    }
