...
function initDocument()
{
    ...
    defindTitleStyle();
    initButton(); 				//按钮初始化 
    //initButtonWidth();        //modified by txr 3163823 按钮宽度过宽
    InitEvent()
    setRequiredElements("CTContractName^CTProviderDR_VDesc^CTSignDate^CTHold1"); //必填项 MZY0040	1407717		2020-7-21
...
function BSave_Clicked()
{
	...
	for (var i = 0; i < rows.length; i++) 
	{
		...
		// MZY0040	1407717		2020-7-21
		if ((ItemReqFlag==0)&&(oneRow.CTLItemDR=="")&&(getElementValue("CTHold1")!=4))
		{
...
function BPicture_Clicked()
{
	...
	if (value==1)
	{
		...
		var str='dhceq.plat.picturemenu.csp?&CurrentSourceType=95&CurrentSourceID='+CTRowID+'&Status='+Status+'&ReadOnly=';
	}
	else
	{
		//modified by czf 20190305
		var str='dhceq.plat.picturemenu.csp?&CurrentSourceType=94&CurrentSourceID='+CTRowID+'&Status='+Status+'&ReadOnly=';
	}
...
function BAppendFile_Clicked()
{
	...
	if (value==0)
	{
		...
		var str='dhceq.plat.appendfile.csp?&CurrentSourceType=94&CurrentSourceID='+CTRowID+'&Status='+CTStatus+'&ReadOnly=';
	...
	else
	{
		...
		var str='dhceq.plat.appendfile.csp?&CurrentSourceType=95&CurrentSourceID='+CTRowID+'&Status='+CTStatus+'&ReadOnly=';
		//Modefied by zc0060 20200329 文件上传改造  end
	}
