...
ExtTool.RunServerMethod = function(ClassName,MethodName) {
    ...
    /*if(objMethodSign == null)
    {
        	Ext.Msg.show({
					   title: '程序运行错误',
					   msg: "<div style='overflow:scroll;height=300px;width:500px;text-align:left'>请在CSP文件中增加'<input type=\"hidden\" id=\"MethodRunClassMethod\" value=\"<%=##Class(%CSP.Page).Encrypt($LB(\"web.DHCClinic.ClassMethodService.RunClassMethod\"))%>\"/>\').RunClassMethod</div>",
...
ExtTool.StaticServerObject = function(ClassName) {
	var strResult = ExtTool.RunServerMethod("web.DHCClinic.ClassMethodService", "StaticObject", ClassName);
	window.eval(strResult);
	return objTmp;
...
ExtTool.CreateDataStoreByQuery = function (ClassName, QueryName, fillParamFn) {
    ...
    var strColLst = ExtTool.RunServerMethod("web.DHCClinic.QueryService", "GetQueryStore", ClassName, QueryName);
...
{
  		var strColModal = ExtTool.RunServerMethod("web.DHCClinic.QueryService", "GetQueryGridHeaderDeclare",  ClassName, QueryName);
