...
function BodyLoadHandler()
{
	...
	//BY:GBX GBX0026 2014-8-21 18:00:38
	//退货隐藏减少类型
	//减少隐藏供应商
	var ReturnTypeDR=GetElementValue("ReturnTypeDR")
	if (ReturnTypeDR == 1)	//Modify DJ 2014-09-11
	{
...
function FillData()
{
	...
	SetElement("ReduceLocDR",list[3])		//需求序号:	417182		Mozy	20170801	修正减少库房赋值
	...
	SetElement("MakeDate",list[sort+7]);	//Add By HZY 2013-7-1
...
function GetTableInfo()
{
	...
	for(var i=1;i<rows;i++)
	{
		if (tableList[i]=="0")
		{
			...
			var TSerialFlag=GetElementValue('TSerialFlagz'+i); //QW0006-2014-08-21 增加序列号显示标志
			if (TAccessory=="")
			{
				//continue;
			...
			var TManuFactoryDR=GetElementValue('TManuFactoryDRz'+i); ///生产厂商 Modify BY:GBX 2014-8-5 14:43:15
...
function ChangeRowStyle(RowObj)
{
   ...
   for (var j=0;j<RowObj.cells.length;j++)
   {
	...
	else if (colName=="TSerialFlag")//QW0006-2014-08-21
	{
		value=GetCElementValue(Id);
...
function CheckNull()
{
	var ReturnTypeDR=GetElementValue("ReturnTypeDR")	//Add By DJ 2014-09-12
...
function TotalFee_Change()
{
	...
	if((TReturnQtyNum>TReturnNum)||(TReturnQtyNum<1)||(isNaN(TReturnQtyNum)))
	{
		...
		{
			alertShow("修改数量超出了可用数量范围!"); //Modify DJ 2014-09-12
...
function DeleteClickHandler() {
	...
	try
	{
		...
		if (((TotalFlag==0)&&(rows<=2))||((rows<=3)&&((TotalFlag==1)||(TotalFlag==2))))
		{
			...
			ClearValue()
			//SetElement("Job","")  //modify BY:GBX 2014-9-13 14:57:01
		} 
		else
		{
...
function AddClickHandler() {
	try 
	{
		...
		if (ret)
		{
	    	...
	    	//add by GBX 2014-8-25 15:06:06 新增时增加序列号点击事件
   ...
   if (event.keyCode==13||event.keyCode==0||vClickEventFlag==1)
	{
		if (CheckNull()) return;
		selectrow=GetTableCurRow();
		var changeType=GetElementValue("ChangeType"); //modify by GBX 2014-8-31 13:26:46
...
function BodyUnloadHandler()	//Add By DJ 2014-09-12
{
