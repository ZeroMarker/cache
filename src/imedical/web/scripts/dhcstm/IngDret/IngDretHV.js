	...
	listeners : {
		specialkey : function(field, e) {
			...
			if (Barcode!="" && e.getKey() == Ext.EventObject.ENTER){
				...
				var RowRecord = IngDretDetailGrid.store.getAt(row);
					...
					success : function(result, request){
						...
						if(jsonData.success == 'true'){
							...
							if(status!="Enable"){
								...
								return;
							}else if(!CheckScgRelation(groupField,scgID)){
...
var completeBT = new Ext.Toolbar.Button({
	...
	height : 30,
	handler:function(){
		if(Ext.isEmpty(gIngrt)){
			return false;
		}
		var CheckInfo = tkMakeServerCall('web.DHCSTM.DHCINGdRet', 'CheckItmLoc', gIngrt);
		if(!Ext.isEmpty(CheckInfo) && confirm(CheckInfo + '\r\n是否继续?')==false){
			return false;
		}
		Complete();
	}
	...
	handler:function(){
		...
		for(var index=0;index<count;index++){
			...
			if(row.data.newRecord || row.dirty){	
				...
				if((reason==null || reason=="")&&(gParam[8]!="Y")){
				...
				var data =  ingrti+"^"+ingri+"^"+qty+"^"+uomId+"^"+rp
					+"^"+sp+"^"+invNo+"^"+invDate+"^"+invAmt+"^"+sxNo
					+"^"+reason+"^"+HVBarCode+"^"+Remark+"^"+SpecDesc+"^"+inclb;
				if(rows!=""){
...
var IngDretDetailGridCm = new Ext.grid.ColumnModel([
	new Ext.grid.RowNumberer(),
	{
		...
		renderer:Ext.util.Format.comboRenderer(Cause2,"reasonId","retReason")
	},{
		header:"退货进价",
		dataIndex:'rp',
		width:100,
...
var IngDretDetailGrid = new Ext.grid.EditorGridPanel({
	...
	listeners:{
		'beforeedit':function(e){
			...
			if(e.field=='qty'){
				if(UseItmTrack && e.record.get('HVFlag')=='Y'&& e.record.get("OriginalStatus")!="NotUnique"){
					e.cancel=true;
				}
			}
...
var formPanel = new Ext.ux.FormPanel({
	title: '退货制单',
	tbar : [newBT,'-',findIngDret,'-',saveIngDret,'-',completeBT,'-',cancelCompleteBT,'-',deleteIngDret,'-',clearBT,'-',printBT,'-',PrintHVCol],
	items:[{
		xtype:'fieldset',
		title:'退货单信息',
...
//加入退货列表
//2017-05-22 添加rowData参数,传入选择的record
function AddList(row, rowData){
	if(Ext.isEmpty(row) || Ext.isEmpty(rowData)){
		return;
	}
	var rowIndex = IngDretDetailGridDs.indexOf(rowData);
	...
	Ext.Ajax.request({
		...
		success: function(result, request) {
			...
			if (jsonData.success=='true') {
				...
				var VirtualFlag = tkMakeServerCall('web.DHCSTM.Common.UtilCommon','GetMainLocInfo',arrData[7]);
