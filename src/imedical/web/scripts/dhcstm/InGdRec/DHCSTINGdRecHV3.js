...
Ext.onReady(function() {
	...
	function CheckBeforeClear(){
		var IngrCount = GrMasterInfoStore.getCount();
		for(var i = 0; i < IngrCount; i++){
			var record = GrMasterInfoStore.getAt(i);
			var CompFlag = record.get('Complete');
			if(CompFlag != 'Y'){
				var IngrNo = record.get('IngrNo');
				Msg.info('warning', IngrNo + '尚未完成,不可清空!');
				return false;
			}
		}
		return true;
	...
	// 增加按钮
	var UPBT = new Ext.Toolbar.Button({
		id : "UPBT",
		text : '更新发票信息',
		tooltip : '点击增加',
		width : 70,
	...
	function addNewRow() {
		...
		var lastInvNo=Ext.getCmp("invno").getValue();
		var lastInvDate=Ext.getCmp("invdate").getValue();
...
//		}
	...
	function saveOrder(rowIndex) {
		...
		var Grindex=Grstore.findBy(function(r){
			var ven=r.get("Vendor")
			var reqloc=r.get("ReqLoc")
			if(ven==VenId&&reqloc==ReqLoc){return true}
			else {return false}
		})
		if (Grindex!=-1){
			...
			var MainInfo = VenId + "^" + LocId + "^" + CreateUser+ "^^^^^^^^^" +ReqLoc
		}else{
			var IngrNo =""
			var LocId = Ext.getCmp("PhaLoc").getValue();
			var CreateUser = gUserId;
			...
			var MainInfo = VenId + "^" + LocId + "^" + CreateUser + "^" + PresentFlag + "^"
					+ ExchangeFlag + "^" + IngrTypeId + "^" + PurUserId + "^"+StkGrpId+"^"+""+"^"+InGrRemarks+"^"+SourceOfFund+"^"+ReqLoc;
			...
			var ingrno=mytrnarr[0],vendor=mytrnarr[1],vendordesc=mytrnarr[2],complete=mytrnarr[9],reqloc=mytrnarr[21],reqlocdesc=mytrnarr[22];
			var defaData = {Ingr:ingr,IngrNo:ingrno,Vendor:vendor,VendorDesc:vendordesc,Complete:complete,ReqLoc:reqloc,ReqLocDesc:reqlocdesc};
							...
							Ext.Ajax.request({
								...
								success : function(result, request){
									...
									if(jsonData.success == 'true'){
										...
										var poi=itmArr[15],batchno=itmArr[16],expdate=itmArr[17],reqloc=itmArr[20],reqlocdesc=itmArr[21]
...
//									}
	...
	var DetailGrid = new Ext.grid.EditorGridPanel({
		...
		tbar:{items:[AddBT,'-',{height:30,width:70,text:'列设置',iconCls:'page_gear',handler:function(){GridColSet(DetailGrid,"DHCSTIMPORTM");}},'-',UPBT,'-','默认发票号:','-',invno,'-','默认发票日期:',invdate]},
		clicksToEdit : 1,
...
//						return;
					...
					var ReqLoc = e.record.get("ReqLoc");
					var Ingr = '';
					if(!Ext.isEmpty(VenId) && !Ext.isEmpty(ReqLoc)){
						var Grstore=GrMasterInfoGrid.getStore();
						var Grindex=Grstore.findBy(function(r){
							var ven=r.get("Vendor")
							var reqloc=r.get("ReqLoc")
	...
	*/
	function getDrugList(record) {
		...
		rowData.set("ReqLocDesc",reqlocdesc);
		var lastInvNo=Ext.getCmp("invno").getValue();
		var lastInvDate=Ext.getCmp("invdate").getValue();
	...
	// 指定列参数
	var fields = ["Ingr","IngrNo","Vendor","VendorDesc","ReqLoc","ReqLocDesc","CreateUser", "Complete"];
	// 支持分页显示的读取方式
	var reader = new Ext.data.JsonReader({
			root : 'rows',
			totalProperty : "results",
