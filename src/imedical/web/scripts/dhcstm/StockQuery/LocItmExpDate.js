...
Ext.onReady(function() {
	...
	function searchData() {
		...
		var RestMon=null;
		var ManfId=Ext.getCmp("INFOPbManf").getValue();
		...
		+"^"+UseFlag+"^"+StkGrpRowId+"^"+ManfId+"^"+InciDesc;
	...
	function clearData() {
		...
		Ext.getCmp("INFOPbManf").setValue("");
	...
	var fields = ["inclb", "inci", "code", "desc","spec", "manf", "incscDesc", "batNo",
		"expDate", "stkQty", "stkUom", "sbDesc","lastvendorName", "warnDays","month","SpecDesc"];
	// 支持分页显示的读取方式
	...
	function InclbHVBarcodeShow(){
		...
		if(rows.length==0){
			Ext.Msg.show({title:'错误',msg:'请选择要查看的物资！',buttons: Ext.Msg.OK,icon:Ext.MessageBox.ERROR});
		}else{
			var selectedRow = rows[0];
			var IncDesc=selectedRow.get("desc");
			var Inclb = StockQtyGrid.getSelectionModel().getSelected().get("inclb");
	...
	var HisListTab = new Ext.form.FormPanel({
		...
		frame : true,
		autoScroll : true,
		bodyStyle : 'padding:5px 1px 1px 5px;',
		tbar : [SearchBT, '-', RefreshBT],
		items : [PhaLoc,DateTime,StkGrpType,DHCStkCatGroup,InciDesc,INFOPbManf,ZeroFlag,NotUseFlag,
