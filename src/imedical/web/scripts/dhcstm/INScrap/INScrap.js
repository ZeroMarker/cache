					...
					success : function(result, request){
						...
						if(jsonData.success == 'true'){
							...
							if(Ext.getCmp("groupField").getValue()!=scgID){
								...
								INScrapMGrid.store.getAt(row).set("HVBarCode","");
								return;
							}else if(inclb==""){
...
var INScrapMGridDs = new Ext.data.Store({
    ...
    reader:new Ext.data.JsonReader({
		...
		"HVFlag","HVBarCode",
		{name:'AvaQty'},
		{name:'DirtyQty',convert:function(value,rec){return rec['qty']}}
	]),
    remoteSort:false
});
			...
			listeners:{
				specialKey:function(field, e) {
					if (e.getKey() == Ext.EventObject.ENTER) {
						...
						if(field.getValue()>Number(rowData.get('AvaQty'))){
	...
	listeners:{
		...
		afteredit:function(e){
			if(e.field=='qty'){
				...
				if(e.value - DirtyQty - AvaQty > 0){
					Msg.info("error","报损数量不能大于批次可用库存!");
...
function returnInfo(records) {
	...
	Ext.each(records,function(record,index,allItems){
		...
		if(!UseItmTrack || HVFlag!='Y'){
			var findIndex=INScrapMGridDs.findExact('inclb',inclb,0);
		...
		if(!(ItmTrack==gHVInScrap) && HVFlag=='Y'){
			Msg.info("warning",record.get("InciDesc")+"是高值材料,请根据条码录入!");
			return;
		}
		var rowData = INScrapMGrid.getStore().getAt(selectRow);
		...
		var lastIndex = INScrapMGridDs.getCount()-1;
		if(INScrapMGridDs.getAt(lastIndex).get('inclb')!=""){
			addNewRow();
		}else{
			var col=GetColIndex(INScrapMGrid,'desc')
			INScrapMGrid.startEditing(lastIndex,col);
		}
	});
}
