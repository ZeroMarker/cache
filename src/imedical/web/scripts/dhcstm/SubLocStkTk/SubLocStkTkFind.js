SubLocStkTkFind = function(Fn){
	var FindPhaLoc = new Ext.ux.LocComboBox({
		id : 'FindPhaLoc',
		fieldLabel : '库房',
		anchor : '90%',
		emptyText : '',
	...
	var FindQuery = new Ext.ux.Button({
		...
		iconCls : 'page_find',
		handler : function(){
			var FindPhaLoc = Ext.getCmp('FindPhaLoc').getValue();
			if(Ext.isEmpty(FindPhaLoc)){
				Msg.info('warning', '库房不可为空!');
				return false;
			}
			...
			var StrParam = FindPhaLoc + '^' + FindStkTkLoc + '^' + StartDate + '^' + EndDate;
