...
// /编写者：zhangdongmei
// /编写日期: 2012.12.27
var ARG_DATEFORMAT = 'Y-m-d';
//2016-12-26 重写ComboBox控件的宽度, 'auto'时会引起错位(width属性设置过大亦不可)
Ext.form.ComboBox.prototype.width = 80;
Ext.form.Checkbox.prototype.height = 20;
Ext.form.Checkbox.prototype.boxMinWidth = 50;
//IE11下的特殊处理
function websys_css_Func(){
	//2017-03-16 引用web\skin\default\css\websys.ext.3.3.0.css后,界面上的特殊处理
	Ext.form.Field.prototype.labelSeparator = '';
	//Ext.form.Field.prototype.boxMinWidth = 50;
	Ext.form.Radio.prototype.hideLabel=true;
	Ext.form.RadioGroup.prototype.hideLabel=true;
...
/**
 * 根据标题获取新的列宽,避免标题显示不完整的情况
 * @param {} Header
 * @param {} Width
 * @return {}
 */
function GridColumn_NewWidth(Header, Width){
	if(!Ext.isEmpty(Header)){
		Header = Ext.util.Format.stripTags(Header);				//考虑html
		Header = Header.replace(/&#160;/g,'');					//考虑liveGrid
		var NewHeaderLen = 0;
		for(var i = 0, HeaderLen = Header.length; i < HeaderLen; i++){
			var AsciiCode = Header.charAt(i).charCodeAt();
			if(AsciiCode <= 126 && AsciiCode >= 33){
				//英文等字符长度不一,暂按半个汉字宽度处理
				NewHeaderLen = NewHeaderLen + 0.5;
			}else if(AsciiCode >= 19968){
				NewHeaderLen = NewHeaderLen + 1;
			}
		}
		var HeaderFontSize = 14;
		var RightBtn = 15, LeftMargin = 5, RightMargin = 3;		//左右空白
		var NewWidth = NewHeaderLen * HeaderFontSize + RightBtn + LeftMargin + RightMargin;
		if(NewWidth > Width){
			Width = NewWidth;
		}
	}
...
/**
 * 2017-12-01 重写ColumnModel的获取列宽方法getColumnWidth
 * @param {} col
 * @return {计算后的新宽度}
 */
Ext.grid.ColumnModel.prototype.getColumnWidth = function(col) {
	var width = this.config[col].width;
	if(typeof width != 'number'){
		width = this.defaultWidth;
	}else{
		var columnHeader = this.config[col].header;
		width = GridColumn_NewWidth(columnHeader, width);
	}
	return width;
};
...
Ext.ux.TreeCombo = Ext.extend(Ext.form.ComboBox, {
	...
	onViewClick: function (doFocus) {
		//2017-04-06 覆盖为空,使用this.tree.on('click',...
	},
	/**
	...
	setValue : function(v, text){
		...
		}else if(objReg.test(v)){
			//2016-03-22 components.js上方的代码,曾重写了Combo的setValue方法,为使TreeCombo沿用之前的赋值方式,这里做此改动
			var valueObj = eval("("+v+")");
			if(valueObj && valueObj['RowId'] != null && valueObj["Description"] != null){
				v = valueObj['RowId'], text = valueObj['Description'];
			}
		}
	...
	onSelect : function (obj, node) {
		...
		if ((obj.selectMode == 'leaf' && node.leaf == true) || obj.selectMode == 'all') {
			if (obj.fireEvent('beforeselect', obj, node)) {
				...
				obj.collapse();
				this.fireEvent('select', obj, node);
			}
		}
	},
...
Ext.ux.LocComboBox = Ext.extend(Ext.ux.ComboBox,{
	...
	protype:'',			//科室类型, L:表示关联科室, 结合linkloc属性使用
	linkloc:null,       //新增属性,默认为登陆科室,根据此科室获取关联科室
	...
	initComponent:function(){
		var myUrl="";
		if (this.protype == 'L' && !Ext.isEmpty(this.linkloc)){
			myUrl=DictUrl+ 'orgutil.csp?actiontype=GetLinkLocations';  //获取关联科室
		...
		this.store=new Ext.data.JsonStore({
			...
			listeners : {
				load : function(store, records, options){
					if(_this.protype == 'L'){
						if(records.length > 0){
							_this.setValue(records[0].get(_this.valueField));
							_this.fireEvent('select', _this, records[0], 0);
						}
					}
				}
			}
		...
		if(this.protype != 'L'){
			...
			this.setValue(rowId);
		}else if(this.protype == 'L' && Ext.isEmpty(this.defaultLoc)){
			this.store.load();
		}
 ...
 * 显示库存项图片的Resizable
 */
//2017-03-16 gCustom后来标准版不再使用,为减少不必要的报错(csr缺少img定义),这里暂时注释
/*
...
function ShowInciInfo(inci){
	...
	var picTpl = new Ext.XTemplate(
		'<tpl for=".">',
			'<div style="padding:5px; height:230px; width:210px; float:left;" class="select_pic" >',
				...
				'<p><font color=blue size="2">{type}</font></p>',
			'</div>',
		'</tpl>'
	);
	...
	var ChainInfoStrParam = groupId + '^' + locId;
	var ItmCertInfoPanel = new Ext.grid.GridPanel({
		region:'center',
		store : new Ext.data.JsonStore({
			autoLoad : true,
			url : 'dhcstm.dicgroupaction.csp?actiontype=GetItmChainInfo&inci=' + inci + '&StrParam=' + ChainInfoStrParam,
			totalProperty : 'results',
			root : 'rows',
			fields : ['MainInfo','ItemName','ItemValue']
		}),
		cm : new Ext.grid.ColumnModel([
			{header : '', dataIndex : 'MainInfo', align : 'left', width : 120},
			{header : '', dataIndex : 'ItemName', align : 'left', width : 120},
			{header : '', dataIndex : 'ItemValue', align : 'left', width : 200}
		]),
		viewConfig: {
			forceFit: true
		},
	...
	var InciInfoWin = new Ext.Window({
		...
		modal : true,
		layout : 'border',
		items : [ItmCertInfoPanel,detailPanel]
	});
	InciInfoWin.show();
}
 ...
 */
var DateFormat=typeof(websys_DateFormat)=='undefined'? ARG_DATEFORMAT : websys_DateFormat;
Ext.ux.DateField=Ext.extend(Ext.form.DateField,{
	...
	initComponent:function(){
		// Y-m-d
		var altFormats_YMD = 'j|d|md|ymd|Ymd'+'|Y-m|Y-n|y-m|y-n'+'|Y-m-d|Y-m-j|Y-n-d|Y-n-j|y-m-d|y-m-j|y-n-d|y-n-j';
		var regex_YMD = /^(t|(t[\+\-]\d*)|\d{1,2}|\d{4}|\d{6}|\d{8}|((\d{4}|\d{2})\-\d{1,2}\-\d{1,2}))$/;
		// m/d/Y
		var altFormats_MDY = 'j|d|md|mdy|mdY'+'|n/j|n/d|m/j|m/d'+'|n/j/y|n/j/Y|n/d/y|n/d/Y|m/j/y|m/j/Y|m/d/y|m/d/Y';
		var regex_MDY = /^(t|(t[\+\-]\d*)|\d{1,2}|\d{4}|\d{6}|\d{8}|(\d{1,2}\/\d{1,2}\/(\d{4}|\d{2})))$/;
		// d/m/Y
		var altFormats_DMY = 'j|d|dm|dmy|dmY'+'|j/n|j/m|d/n|d/m'+'|j/n/y|j/n/Y|j/m/y|j/m/Y|d/n/y|d/n/Y|d/m/y|d/m/Y';
		var regex_DMY = /^(t|(t[\+\-]\d*)|\d{1,2}|\d{4}|\d{6}|\d{8}|(\d{1,2}\/\d{1,2}\/(\d{4}|\d{2})))$/;
		
		var altFormats = altFormats_YMD;
		var regex = regex_YMD;
		if(DateFormat == 'Y-m-d'){
			altFormats = altFormats_YMD;
		}else if(DateFormat == 'm/d/Y'){
			altFormats = altFormats_MDY;
		}else if(DateFormat == 'd/m/Y'){
			altFormats = altFormats_DMY;
		}
		
		if(!Ext.isEmpty(CommParObj.DateAltFormat)){
			var DateAltFormat = CommParObj.DateAltFormat;
			if(DateAltFormat == 'Y-m-d'){
				altFormats = altFormats_YMD + '|' + altFormats;
			}else if(DateAltFormat == 'm/d/Y'){
				altFormats = altFormats_MDY + '|' + altFormats;
			}else if(DateAltFormat == 'd/m/Y'){
				altFormats = altFormats_DMY + '|' + altFormats;
			}
		}
		
		this.altFormats = altFormats;
		//this.regex = regex;		//2017-12-17 不再使用regex属性
...
Ext.ux.NumberField = Ext.extend(Ext.form.NumberField,{
	anchor : '90%',
	formatType : null,		//FmtRP,FmtPP,FmtSP,FmtRA,FmtPA,FmtSA中某一个(于参数同名)
	initComponent : function() {
		var myUrl = "";
...
// GridPanel封装中调用
function CommonGridColSet(gridId){
...
// 在GridPanel标题上扩展"列设置","导出Excel"等按钮
function CreateTitleButtons(grid){
	var gridId = grid.getId();
	if(gridId.indexOf('ext-comp')>=0){
		return false;
	}
	if(!Ext.isEmpty(grid.tbar)){
		grid.tbar = [].concat(grid.tbar);
		grid.tbar.push('->', {xtype:'button',tooltip:'列设置',iconCls:'page_gear',handler:function(){CommonGridColSet(gridId);}},
			'-', {xtype:'button',tooltip:'导出当前页',iconCls:'page_excel',handler:function(b,e){gridSaveAsExcel(grid);}},
			'-', {xtype:'button',tooltip:'导出所有',iconCls:'page_excel',handler:function(b,e){ExportAllToExcel(grid);}});
	}else{
		grid.title = (grid.title || '')
			+ "&nbsp" + "<button type='button' style='margin-bottom:-5px;width:22px;height:22px;' class='x-btn-text page_gear' title='列设置' onclick='CommonGridColSet(\""+gridId+"\");'></button>"
			+ "&nbsp" + "<button type='button' style='margin-bottom:-5px;width:22px;height:22px;' class='x-btn-text page_excel' title='导出当前页' onclick='gridSaveAsExcel(\""+gridId+"\");'></button>"
			+ "&nbsp" + "<button type='button' style='margin-bottom:-5px;width:22px;height:22px;' class='x-btn-text page_excel' title='导出所有' onclick='ExportAllToExcel(\""+gridId+"\");'></button>";
	}
}
	...
	*/
	...
	GetTypeByColumn : function(col){
		...
		if(colXType=='booleancolumn'){
			...
			col.dateFormat = DateFormat;	//datecolumn对应store定义用到
			type='date';
		}else{
			type='string';
		}
			...
			for(var j=0,arrLen=this.saveColArray.length; j<arrLen; j++){
				if(this.saveColArray[j]){
					...
					if(saveColValue!="" && col.xtype=='datecolumn'){
						saveColValue = Ext.util.Format.date(saveColValue,ARG_DATEFORMAT);
					}
