...
Ext.ux.form.LovCombo = Ext.extend(Ext.form.ComboBox, {
     ...
     */
    ,displaySeparator:','
	,filterName : ''		//2015-06-13 store定义的根据录入内容过滤数据的参数名称
	,params : {}
	,pageSize : 9999
	,forceSelection : true
	,triggerAction : 'all'
    ...
    ,onBeforeQuery:function(qe) {
		...
		var RawValue = this.getRawValue(), Separator = this.separator;
		if(RawValue.indexOf(Separator) != -1 && this.store.getCount() > 0){
			return;
		}
		...
		if(this.filterName!=""){
			this.store.setBaseParam(this.filterName, this.getRawValue());
		}
		if(this.pageSize == 0){
