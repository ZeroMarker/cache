...
Ext.ux.UploadDialog.Dialog = function(config) {
	var default_config = {
		...
		url : "",
		base_params : {},
		permitted_extensions : [],		//为便于控制, 使用小写字母
		reset_on_hide : true,
...
Ext.extend(Ext.ux.UploadDialog.Dialog, Ext.Window, {
	...
	createGrid : function() {
		...
		var cm = new Ext.grid.ColumnModel([{
					...
					header : this.i18n.extension_col_title,
					width : this.i18n.extension_col_width,
					dataIndex : "extension",
					sortable : true,
					renderer : this.renderExtensionCell.createDelegate(this)
				}]);
	...
	renderExtensionCell : function(data, cell, record, row_index, column_index,
			store) {
		var view = this.grid_panel.getView();
		var f = function() {
			try {
	...
	addFileToUploadQueue : function(browse_btn) {
		...
		store.add(new Ext.ux.UploadDialog.FileRecord({
					...
					extension:this.getFileExtension(input_file.dom.value)
				}));
...
//									});
	...
	uploadFile : function(record) {
		// this.form.submit();
		this.base_params.extension=record.data.extension
		Ext.Ajax.request({
					url : this.url,
...
p.i18n = {
	...
	note_col_title : "备注",
	note_col_width : 150,
	extension_col_title : "扩展名",
	extension_col_width : 100,	
	add_btn_text : "添加",
	...
	upload_btn_start_tip : "上传文件队列",
	upload_btn_stop_tip : "停止上传",
	close_btn_text : "关闭",
	close_btn_tip : "关闭上传对话框",
	progress_waiting_text : "等待...",
	progress_uploading_text : "上传中:{0}个文件（共{1}个文件）上传成功",
