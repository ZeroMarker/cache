...
Ext.onReady(function() {
	...
	function ChartInfoAddFun() {
		//被服类转入, 被服管理人员制作转入单据, "科室"为接收部门(出库制单的"请求部门")
		...
		// 原出库制单的"供给部门"
		var FromPhaLoc = new Ext.ux.LocComboBox({
					fieldLabel : '来源部门',
					id : 'FromPhaLoc',
					name : 'FromPhaLoc',
					anchor:'90%',
		...
		var SearchInItBT = new Ext.Toolbar.Button({
					...
					handler : function() {
//						var frLoc="";
//						var toLoc=Ext.getCmp("PhaLoc").getValue();
//						if(toLoc==""){
//							Msg.info("warning","科室不可为空!");
//							return;
//						}
						BCInIsTrfFind(1,Query);
					}
				});
		...
		function clearData() {
			...
			Ext.getCmp("FromPhaLoc").setValue("");
			Ext.getCmp("FromPhaLoc").setDisabled(0);
			Ext.getCmp("ReqNo").setValue("");
			Ext.getCmp("ReqNoID").setValue("");
			gInitId="";
			gInclbFlag=true;
			Ext.getCmp('FromPhaLoc').setDisabled(false);
		...
		var AddBT = new Ext.Toolbar.Button({
					...
					handler : function() {
						...
						var FromPhaLoc = Ext.getCmp("FromPhaLoc").getValue();
						if (FromPhaLoc == null || FromPhaLoc.length <= 0) {
							Msg.info("warning", "请选择来源部门!");
							return;
						}
						if (PhaLoc == FromPhaLoc) {
							Msg.info("warning", "来源部门和供应部门不能相同!");
							return;
						}
		...
		function CheckDataBeforeSave() {
			...
			var requestphaLoc = Ext.getCmp("PhaLoc").getValue();
			if (requestphaLoc == null || requestphaLoc.length <= 0) {
				Msg.info("warning", "请选择来源部门!");
				return false;
			}
			var supplyphaLoc = Ext.getCmp("FromPhaLoc")
					.getValue();
		...
		function saveOrder() {
			...
			var MainInfo = FromPhaLoc + "^" + PhaLoc + "^" + reqid + "^" + operatetype + "^"
		...
		function Complete() {
			...
			var url = DictUrl
					+ 'dhcinistrfaction.csp?actiontype=MakeComplete';

			Ext.Ajax.request({
						url : url,
						params : {Rowid:gInitId,Status:11,Complete:"Y",BCFlag:"Y"},
						method : 'POST',
						waitMsg : '查询中...',
						success : function(result, request) {
							var jsonData = Ext.util.JSON
		...
		var SelTransBT = new Ext.Toolbar.Button({
				...
				iconCls : 'page_find',
				handler : function() {
					var FrLoc=Ext.getCmp("FromPhaLoc").getValue();
					if(FrLoc==null){
						Msg.info("warning","请选择来源部门!")
						return;
					}
						...
						success : function(result, request) {
							...
							if (jsonData.success == 'true') {
								...
								if (list.length > 0) {
									...
									addComboData(FromPhaLoc.getStore(),list[6],list[26]);
									Ext.getCmp("FromPhaLoc").setValue(list[6]);
									addComboData(PhaLoc.getStore(),list[12],list[27]);
									Ext.getCmp("PhaLoc").setValue(list[12]);
		...
		function getDetail(InitRowid) {
			...
			DetailStore.load({
				...
				callback : function(r,options,success){
					gInclbFlag = false;
					Ext.getCmp('FromPhaLoc').setDisabled(true);
		...
		var fields = ["initi", "inrqi", "inci","inciCode",
				...
				"pp", "spec","newSp", "spAmt", "rp","rpAmt", "ConFac", {name:"BUomId",mapping:"BuomId"}, "inclbDirtyQty",
		...
		**/		
		...
		function GetPhaOrderInfo(item, stkgrp) {
			...
			if (item != null && item.length > 0) {
				IncItmBatWindow(item, stkgrp, App_StkTypeCode, FromPhaLoc, "N", "1", gHospId, PhaLoc, returnInfo);
			}
		}
		...
		var HisListTab = new Ext.ux.FormPanel({
			title:'被服类转入',
			tbar : [SearchInItBT, '-',  ClearBT, '-', SaveBT, '-', DeleteBT, '-', CheckBT,'-',PrintBT,'-',SelTransBT],
			items:[{
				xtype : 'fieldset',
				title : '转移单信息',
				layout : 'column',	
				style : 'padding:5px 0px 0px 5px',
				defaults:{border:false,columnWidth: 0.33,xtype:'fieldset'},
				items : [{
					items: [PhaLoc,FromPhaLoc,StkGrpType]
				},{
					items: [InItNo,InItDate,OperateOutType]
				},{
					items: [Remark,InItFlag,ReqNoID]	//ReqNo,
				}]
			}]
		});
		...
		function QueryInclbInfo(){
			...
			DetailStore.removeAll();
			var PhaLoc = Ext.getCmp("FromPhaLoc").getValue();
			if(PhaLoc==""){
				return;
			}
