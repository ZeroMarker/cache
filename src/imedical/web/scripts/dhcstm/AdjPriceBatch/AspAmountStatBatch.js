...
Ext.onReady(function(){
	...
	function ShowReport()
	{
		...
		if(StatFlag==1){
			...
			var p_URL = PmRunQianUrl+'?reportName=DHCSTM_aspamountstat-incbatch.raq&StartDate='+
				StartDate +'&EndDate=' +EndDate +'&Others='+Others;
			var NewWin=(window.open(p_URL,"调价损益单品汇总","top=20,left=20,width=930,height=660,scrollbars=1"));
		} 
		//单品科室汇总
		else if(StatFlag==2){
			var Others=LocId+"^"+AspNo+"^"+AspReasonId+"^"+OptType+"^"+InciId+"^^"+InciDesc;
			var p_URL = PmRunQianUrl+'?reportName=DHCSTM_aspamountstat-inclocbatch.raq&StartDate='+
				StartDate +'&EndDate=' +EndDate +'&Others='+Others;
			var NewWin=open(p_URL,"调价损益单品科室汇总","top=20,left=20,width=930,height=660,scrollbars=1");
		}
		//供应商汇总
		else if(StatFlag==3){
			var Others=LocId+"^"+AspNo+"^"+AspReasonId+"^"+OptType+"^"+InciId+"^"+VenId+"^"+InciDesc;
			var p_URL = PmRunQianUrl+'?reportName=DHCSTM_aspamountstat-vendorbatch.raq&StartDate='+
				StartDate +'&EndDate=' +EndDate +'&Others='+Others;
			var NewWin=open(p_URL,"调价损益供应商汇总","top=20,left=20,width=930,height=660,scrollbars=1");
		}
	}
