...
Ext.onReady(function () {
	...
	function ChartInfoAddFun() {
											...
											Ext.Ajax.request({
												...
												success: function (result, request) {
													...
													if (jsonData.success == 'true') {
														...
														var status = itmArr[0],
														type = itmArr[1],
														lastDetailAudit = itmArr[2],
														lastDetailOperNo = itmArr[3];
														var inclb = itmArr[4],
														inciDr = itmArr[5],
														inciCode = itmArr[6],
														inciDesc = itmArr[7],
														...
														if (Ext.getCmp("StkGrpType").getValue() != scgID) {
															...
															DetailGridR.store.getAt(row).set("HVBarCode", "");
															return;
														} else if (inclb == "") {
		...
		var DetailGridR = new Ext.grid.EditorGridPanel({
				...
				title: '退库单',
				tbar: {
					items: [AddBT, '-', DeleteDetailBT, '-', {
							height: 30,
							width: 70,
							text: '列设置',
							iconCls: 'page_gear',
							handler: function () {
								GridColSet(DetailGridR, "DHCSTTRANSFERM");
							}
						}
					]
				},
		...
		function returnInfo(records) {
			...
			Ext.each(records, function (record, index, allItems) {
				...
				if (!(ItmTrack == gHVInIsTrf) && HVFlag == 'Y') {
					...
					var index = GetColIndex(DetailGridR, "HVBarCode");
					rowData.set("InciDesc", "");
					DetailGridR.startEditing(cell[0], index);
					return;
				}
				...
				if (DetailStore.getAt(lastIndex).get('inclb') != "") {
					...
					var col = GetColIndex(DetailGridR, 'inciDesc')
						DetailGridR.startEditing(lastIndex, col);
				}
			});
		}
		...
		var HisListTab = new Ext.ux.FormPanel({
				...
				height: 200,
				region: 'north',
				title: gHVInIsTrf ? '库存转移制单(高值)' : '退库制单',
