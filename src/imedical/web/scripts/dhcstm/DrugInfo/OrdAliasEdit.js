 ...
 * 查询界面
 */
function OrdAliasEdit(dataStore, ArcRowid,objAlias) {
	Ext.QuickTips.init();
	...
	function SaveAlias() {
		...
		for (var i = 0; i < rowCount; i++) {				
			...
			{
				var AliasRowid = rowData.get("ALIASRowid");
				var AliasText = rowData.get("ALIASText");	
				if(StrData=="")
				{
					StrData=AliasRowid+"^"+AliasText;
				}
				else
				{
					StrData=StrData+xRowDelim()+AliasRowid+"^"+AliasText;
				}
			}
		}
		if(StrData==""){
		...
		var url = DictUrl
				+ "druginfomaintainaction.csp?actiontype=SaveArcAlias";
		Ext.Ajax.request({
			url : url,
	...
	function deleteData() {
		...
		if(cell==null){
			...
			var AliasRowid = record.get("ALIASRowid");
			if (AliasRowid == null || AliasRowid.length <= 0) {
	...
	function showResult(btn) {
		if (btn == "yes") {
			...
			// 删除该行数据
			var url = DictUrl
					+ "druginfomaintainaction.csp?actiontype=DeleteArcAlias&AliasRowid="
					+ AliasRowid;
	...
	// 显示医嘱项别名
	function getArcAlias(ArcRowid) {
		if (ArcRowid == null || ArcRowid=="") {
			return;
		}
		var url = DictUrl
				+ "druginfomaintainaction.csp?actiontype=GetArcAlias&ArcRowid="
				+ ArcRowid;
	...
	// 指定列参数
	var fields = ["ALIASRowid", "ALIASText"];
	// 支持分页显示的读取方式
	...
	var window = new Ext.Window({
		...
		listeners:{
			'close':function(){
				var cls='web.DHCSTM.ARCALIAS';
				var mthd='GetArcAlias';
				var alias=tkMakeServerCall(cls,mthd,ArcRowid);
				if (objAlias){
					objAlias.setValue(alias);
				}
			}	
		}
	});
