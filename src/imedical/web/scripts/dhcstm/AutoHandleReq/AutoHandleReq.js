...
Ext.onReady(function () {
	...
	function addNewRow() {
		var rec = CreateRecordInstance(ReqStore.fields);
		ReqStore.add(rec);
		var col = GetColIndex(ReqGrid, 'UserId');
		ReqGrid.startEditing(ReqStore.getCount() - 1, col);
	}
	// 查询按钮
	...
	function Query() {
		...
		ReqStore.setBaseParam('Params', phaLoc);
		ReqStore.removeAll();
		ReqStore.load();
	}
	...
	function save() {
		var ListDetail = "";
		var mr = ReqStore.getModifiedRecords();
		var phaLoc = Ext.getCmp("PhaLoc").getValue();
		...
		var url = DictUrl + "autohandlereqaction.csp?actiontype=Save";
		Ext.Ajax.request({
			url: url,
			params: {
	...
	var ReqCm = new Ext.grid.ColumnModel([nm, {
					header: "Rowid",
					dataIndex: 'Rowid',
					width: 80,
					align: 'left',
		...
		var DspPhaUrl = DictUrl
		 + 'autohandlereqaction.csp?actiontype=Query';
	// 通过AJAX方式调用后台数据
	...
	// 数据集
	var ReqStore = new Ext.data.Store({
			proxy: proxy,
	...
	var ReqGrid = new Ext.ux.EditorGridPanel({
			title: '生成采购单任务配置',
			id: 'ReqGrid',
			cm: ReqCm,
			store: ReqStore,
