...
var ShowRetWin=function(Status,HVFlag,Paramstr,DeskStore) {
	...
	function Complete(){
		...
		Ext.Ajax.request({
			url : 'dhcstm.ingdretaction.csp?actiontype=complet&ret='+RowId,
			method : 'POST',
			waitMsg : '查询中...',
	...
	function Audit(){
		...
		var RowId = selectRows[0].get("RowId");
		Ext.Ajax.request({
			url:"dhcstm.ingdretaction.csp?actiontype=audit&ret="
				+ RowId + "&userId=" + gUserId,
			success:function(result, request){
				var jsonData = Ext.util.JSON.decode(result.responseText);
				if (jsonData.success=='true'){
	...
	// 访问路径
	var MasterUrl = 'dhcstm.deskaction.csp?actiontype=QueryRet';
	// 通过AJAX方式调用后台数据
	...
	// 指定列参数
	var fields = ["RowId","No","LocDesc","UserName","Date","Time","CompFlag","AuditFlag","ScgDesc","VendorDesc"];
	// 支持分页显示的读取方式
