...
var ShowAdjWin=function(Status,HVFlag,Paramstr,DeskStore) {
	...
	function Complete(){
		...
		Ext.Ajax.request({
			url : 'dhcstm.inadjaction.csp?actiontype=setComplete&adj='+RowId,
			method : 'POST',
			waitMsg : '查询中...',
	...
	function Audit(){
		...
		var RowId = selectRows[0].get("RowId");
		Ext.Ajax.request({
			url:"dhcstm.inadjaction.csp?actiontype=audit&adj="
	...
	// 访问路径
	var MasterUrl = 'dhcstm.deskaction.csp?actiontype=QueryAdj';
	// 通过AJAX方式调用后台数据
