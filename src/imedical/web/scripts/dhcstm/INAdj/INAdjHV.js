	...
	listeners : {
		specialkey : function(field, e) {
			...
			if (Barcode!="" && e.getKey() == Ext.EventObject.ENTER){
				...
				var RowRecord = INAdjMGrid.store.getAt(row);
					...
					success : function(result, request){
						...
						if(jsonData.success == 'true'){
							...
							var url = "dhcstm.drugutil.csp?actiontype=GetDrugBatInfo&IncId="+inciDr
								+"&ProLocId="+phaLoc+"&ReqLocId="+phaLocRQ+"&QtyFlag="+QtyFlag+"&StkType="+App_StkTypeCode+"&Inclb="+inclb+"&start=0&limit=1";
							var LBResult=ExecuteDBSynAccess(url);
							var info=Ext.util.JSON.decode(LBResult);
							if(info.results=='0'&& OriginalStatus!="NotUnique"){
								Msg.info("warning","该高值材料没有相应库存记录,不能制单!");
								RowRecord.set("HVBarCode","");
								return;
							}
			...
			listeners:{
				specialKey:function(field, e) {
					if (e.getKey() == Ext.EventObject.ENTER) {
						...
						if(Number(field.getValue())+Number(rowData.get('Inclbqty'))<0){
...
var INAdjMGrid = new Ext.grid.EditorGridPanel({
	...
	listeners:{
		...
		afteredit:function(e){
			if(e.field=='qty'){
				if(e.value<0 && Math.abs(e.value)>Number(e.record.get('Inclbqty'))){
	...
	Ext.Ajax.request({
		...
		success:function(result,request){
			...
			if (jsonData.results>0) {
				...
				if (completeFlag=='Y'&& auditFlag!='Y') 
				{//saveINAdjM.disable();
				changeButtonEnable("1^1^0^0^1^0^1^1")
				}
				else if( completeFlag!='Y'&& auditFlag!='Y') 
				{//saveINAdjM.enable();
				changeButtonEnable("1^1^1^1^1^1^0^1")
				}
				else {
					changeButtonEnable("1^1^0^0^1^0^0^0")
					}
...
function returnInfo(record, rowData) {
	...
	//20190905 
	var OriginalStatus=rowData.get("OriginalStatus")
	if(record.get("InclbQty")<1 && OriginalStatus!="NotUnique"){
		Msg.info("warning","可用库存不足!");
		return;
	}else{
		rowData.set("qty", -1);
		rowData.set("rpAmt", accMul(-1,record.get("Rp")));
		rowData.set("spAmt", accMul(-1,record.get("Sp")));
		addNewRow();
	}
}
...
function addNewRow() {
	...
	setEditDisable();
	changeButtonEnable("1^1^1^0^1^0^0^0")
}
...
function changeButtonEnable(str) {
	...
	//新建^查询^保存^删除^清空^单据完成^取消完成^打印
	addINAdjM.setDisabled(list[0]);
	findINAdjM.setDisabled(list[1]);
	saveINAdjM.setDisabled(list[2]);
	deleteINAdjM.setDisabled(list[3]);
	clearINAdjM.setDisabled(list[4]);
	adjComplete.setDisabled(list[5]);
	cancelAdjComplete.setDisabled(list[6]);
