...
var StkDecimalDesc="";
//配置数据源
var StkSysAppParameGridUrl = 'dhcstm.stksysgridsetaction.csp';
...
var alignStore = new Ext.data.SimpleStore({
	fields:['key', 'keyValue'],
	data:[["LeftTop",'LeftTop'], ["LeftCenter",'LeftCenter'], ["LeftBottom",'LeftBottom'], ["CenterTop",'CenterTop'], ["CenterCenter",'CenterCenter'], ["CenterBottom",'CenterBottom'], ["RightTop",'RightTop'], ["RightCenter",'RightCenter'], ["RightBottom",'RightBottom'], ["General",'General']]
});
...
//配置数据源
var gridUrl = 'dhcstm.stksysgridsetaction.csp';
	...
	new Ext.grid.RowNumberer(),{
		...
		editor: new Ext.form.ComboBox({
			...
			listeners:{
				specialKey:function(field, e) {
					if (e.getKey() == Ext.EventObject.ENTER) {
						grid.startEditing(gridDs.getCount() - 1, 10);
					}
				}
			}
        })
...
StkSysAppParameGrid.on('rowclick',function(grid,rowIndex,e){
	...
	ModValue = selectedRow.data["SaveValue"];
...
function DeleteGridSet(){
	...
	var myModValue = selectedRow.get("SaveValue");
