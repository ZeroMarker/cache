...
function BarCodePackItm(dhcit, InfoStr) {
	...
	var PackItmGrid = new Ext.ux.EditorGridPanel({
		...
		listeners : {
			rowcontextmenu : function(grid,rowindex,e){
				...
				PackItmRightClick.showAt(e.getXY());
			}
		}
	});
	
	var PackItmRightClick = new Ext.menu.Menu({
		items: [
			{
				text: '删除新行',
				handler: PackItmDeleteDetail
			}, {
				text: '拆分明细',
				handler: PackItmSplitDetail
			}
		]
	});
	var PackItmRowsAdd = 1000;
	function PackItmSplitDetail(item,e){
		var cell = PackItmGrid.getSelectionModel().getSelectedCell();
		if (cell == null) {
			Msg.info("warning", "没有选中行!");
			return;
		}
		var row = cell[0];
		var record = PackItmStore.getAt(row);
		var newRecord = new PackItmStore.recordType(record.copy().data,++PackItmRowsAdd);
		newRecord.set('RowId', '');
		PackItmStore.insert(row+1,newRecord);
		PackItmGrid.view.refresh();
	}
	function PackItmDeleteDetail(item,e){
		var cell = PackItmGrid.getSelectionModel().getSelectedCell();
		var record = PackItmStore.getAt(cell[0]);
		if(record.get('RowId') != ''){
			Msg.info('warning', '已保存明细不可删除!');
			return;
		}
		PackItmStore.remove(record);
		PackItmGrid.view.refresh();
	}
	...
	var PackItmWin = new Ext.Window({
		...
		layout : 'fit',
		items : [PackItmGrid],
		tbar : [PackItmSaveBT, '-', PackItmCloseBT]
	});
	PackItmWin.show();
