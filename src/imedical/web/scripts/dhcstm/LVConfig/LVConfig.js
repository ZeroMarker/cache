               ...
               var RowId = record.get("LvRowId");
	            ...
	            function(btn){
		               if(btn=="yes"){
			               ...
			               var url = "dhcstm.LVConfigaction.csp?actiontype=Delete&lvrowid="+RowId;
...
function updateOrder(){
	...
	for (var i = 0; i < rowCount; i++) {
		...
		if(rowData.data.newRecord || rowData.dirty){
			...
			var str= RowId + "^"+ InciId+"^"+LocCode+"^"+LVNumber
			if(ListDetail==""){
				ListDetail=str;
			}else{
	...
	if(ListDetail==""){
		...
		//alert(ListDetail);
		var url ="dhcstm.lvconfigaction.csp?actiontype=updateLvConfig";
		var mask=ShowLoadMask(Ext.getBody(),"处理中请稍候...");
		Ext.Ajax.request({
			url : url,
			method : 'POST',
...
var fields = ["RowId", "InciCode","InciDesc","Spec", "LocDescId","LocDesc","LVNumber"];
...
function saveOrder(){
	...
	for (var i = 0; i < rowCount; i++) {
		...
		if(rowData.data.newRecord || rowData.dirty){
			...
			var str= InciId+"^"+LocCode+"^"+LVNumber
			if(ListDetail==""){
				ListDetail=str;
			}else{
				ListDetail=ListDetail+RowDelim+str;
	...
	if(ListDetail==""){
		...
		//alert(ListDetail);
		var url ="dhcstm.lvconfigaction.csp?actiontype=savelocqty";
		var mask=ShowLoadMask(Ext.getBody(),"处理中请稍候...");
		Ext.Ajax.request({
			url : url,
			method : 'POST',
...
// 访问路径
var LvDetailUrl ='dhcstm.lvconfigaction.csp?actiontype=query';
// 通过AJAX方式调用后台数据
var proxy = new Ext.data.HttpProxy({
			url : LvDetailUrl,
			method : "POST"
...
// 指定列参数
		//RowId InciId  InciCode InciDesc Spec LocDescId  LVNumber
var fields = ["LvRowId", "LvInciId", "LvInciCode","LvInciDesc","LvSpec", "LvLocDescId","LvLocDesc","LvLVNumber"];
