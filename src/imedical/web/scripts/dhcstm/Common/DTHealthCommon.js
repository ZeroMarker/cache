...
SetLogInDept = function(store, CmpName) {
	...
	var RowIndex = store.findExact(Ext.getCmp(CmpName).valueField, rowId);
	var Record = store.getAt(RowIndex);
	Ext.getCmp(CmpName).fireEvent('select', Ext.getCmp(CmpName), Record, RowIndex);
};
...
function gridSaveAsExcel(grid,name){
	...
	if(!name){
		...
		var gridPanelTitle=gridPanelTitle.replace(/<\/?.+?>/g,"");
 		var gridPanelTitle=gridPanelTitle.replace(/ /g,"");
...
function StoreSaveAsExcel(store,cm,name)
{
	...
	if(Ext.isEmpty(name)){
		name = new Date().format('Y-m-d-His');
	}
	...
	for (var n=0;n<temp_obj.length;n++){
		xlSheet.Cells(1,n+1).Value=Ext.util.Format.stripTags(temp_obj[n].header);
	}
...
function ExportAllToExcel(gridPanel,name) {
	...
	if (gridPanel) {
		...
		if(tmpParam==null){
			gridSaveAsExcel(gridPanel,name);
			return;
		}
 ...
 * 侧菜单模式下,界面跳转后的回置
 */
function Common_PageBack(){
	var src = window.frameElement && window.frameElement.src || '';
	//仅考虑侧菜单模式
	//ps: 头菜单模式,只能通过全局变量来处理了吧???
	var pathname = location.pathname;
	var websysflag = pathname.indexOf('websys.csp') != -1? true : false;
	if(!websysflag && !Ext.isEmpty(src)){
		var OriginalMenu = src.split('/web/csp/')[1];
		window.location.href = OriginalMenu;
	}
}
 ...
 */
function Common_PrintLog(Type, Pointer, AutoFlag){
	var UserId = session['LOGON.USERID'];
	var Ret = tkMakeServerCall('web.DHCSTM.Common.UtilCommon', 'BillPrintLog', Type, Pointer, AutoFlag, UserId);
	if(Ret !== '0'){
 ...
 * @param {User名称,如User.INCItm} TableName
 * @param {相应数据rowid} RowId
 * @return {Boolean}
 */
function Common_GetLog(TableName, RowId){
	if(Ext.isEmpty(TableName)){
		Msg.info('warning', '日志检索参数有误,请核实是否!');
		return false;
	}
	var BDPLogUrl = "dhc.bdp.bdp.datalogforother.csp?OBJDESC="+RowId+"&UserClass="+TableName;
	new Ext.Window({
		width : gWinWidth,
		height : gWinHeight,
		title : '查看数据日志',
		...
		constrain : true,
		closeAction : 'close',
		html : '<iframe id="iframlog" src="'+ BDPLogUrl +'" width="100%" height="100%"></iframe>'
	}).show();
}
/**
 * 判断业务单据是否高值单
...
function GetCertDocHVFlag(Pointer, Type){
	var Ret = tkMakeServerCall('web.DHCSTM.Common.UtilCommon', 'GetCertDocHVFlag',Pointer, Type);
	return Ret;
}
 ...
 * @param {工具条名称} paging
 */
function Common_ClearPaging(paging){
	if(!paging.rendered){
		return;
	}
	if(!Ext.isEmpty(paging.store)){
		paging.store.removeAll();
	}
	paging.cursor = 0;
	var ap = 1, ps = 1;
	
	paging.afterTextItem.setText(String.format(paging.afterPageText, ps));
	paging.inputItem.setValue(ap);
	paging.first.setDisabled(ap == 1);
	paging.prev.setDisabled(ap == 1);
	paging.next.setDisabled(ap == ps);
	paging.last.setDisabled(ap == ps);
	paging.updateInfo();
	paging.refresh.disable();
}
...
function GetBarCodeSplitInfo(code){
	...
	if(code.length <= 22){
		...
		if(Start2 == '01'){
			...
			var result = tkMakeServerCall('web.DHCSTM.DHCBarCodeSplitInfo','LocateSplitInfo',code);
			var SplitInfoArr = result.split('^');
			if(SplitInfoArr[0] > 0){
				barcode = SplitInfoArr[8];
			}
		}
	}
	if(Ext.isEmpty(barcode)){
		Msg.info('warning', '无法获取物品码!');
		return {Code : ''};
	}
	...
	var tmpBarcode = barcode.replace(/\(/g,'').replace(/\)/g,'');		//临时inci_barcode,不计括号
	
	if(!Ext.isEmpty(barcode)){
		var BarcodeSplitInfo = tkMakeServerCall('web.DHCSTM.DHCBarCodeSplitInfo','GetSplitInfo',barcode);
		...
		var SplitInfoArr = BarcodeSplitInfo.split('^');
		if(SplitInfoArr[0] > 0){
			InciId = SplitInfoArr[1];
			ExpStartPosition = SplitInfoArr[2];
			ExpLength = SplitInfoArr[3];
			BatStartPosition = SplitInfoArr[4];
			BatLength = SplitInfoArr[5];
			ProStartPosition = SplitInfoArr[6];
			ProLength = SplitInfoArr[7];
		}
	}
	
	//<1>先计算效期起始位置
	if(!Ext.isEmpty(ExpStartPosition)){
		ExpStartPosition = Number(ExpStartPosition) - 1;	// 客户维护数据,从1开始数,所以-1
	}else{
		ExpStartPosition = tmpBarcode.length + 2;			// +2 表示标识符
	}
	//<2>计算效期截取长度
	if(!Ext.isEmpty(ExpLength)){
	...
	//<3>计算批号起始位置
	if(!Ext.isEmpty(BatStartPosition)){
	...
	//<4>计算批号长度
	if(!Ext.isEmpty(BatLength)){
		BatLength = Number(BatLength);
	}else{
		if(!Ext.isEmpty(BatStartPosition)){
			//这个位置,如果能获取BatStartPosition,默认到最后一位
			BatLength = tmpCode.length - BatStartPosition + 1;
		}else if(ExpLength === 0){
	...
	if(!Ext.isEmpty(ProStartPosition)){
		ProStartPosition = Number(ProStartPosition) - 1;	// 客户维护数据,从1开始数,所以-1
	}
	//<6>生产日期长度
	if(!Ext.isEmpty(ProLength)){
		ProLength = Number(ProLength);
	}else if(!Ext.isEmpty(ProStartPosition)){
		//缺省为6(ymd格式长度)
		ProLength = 6;
	}
	...
	try{
		...
		if(!Ext.isEmpty(ExpDateStr)){
			ExpDate = Date.parseDate(ExpDateStr,'ymd');
		}
		//生产日期
		if(!Ext.isEmpty(ProStartPosition)){
			ProduceDateStr = tmpCode.substring(ProStartPosition, ProStartPosition + ProLength);
			if(!Ext.isEmpty(ProduceDateStr) && (ProduceDateStr.length == ProLength)){
				ProduceDate = Date.parseDate(ProduceDateStr,'ymd');
			}else{
				ProduceDate = '';
			}
		}
