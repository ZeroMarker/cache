...
function SelectRowHandler()	{
    ...
    var SelRowObjAuditFlag=document.getElementById('TAuditFlagz'+selectrow);
    var ObjAuditFlag=document.getElementById('AuditFlag');
    if (ObjAutoPy){
	    if (SelRowObjAuditFlag){
   	  if (SelRowObjAuditFlag.innerText=="是"){ObjAuditFlag.checked=true} else {ObjAuditFlag.checked=false}
	    }
    }
        var SelRowObjOthLocRet=document.getElementById('TOthLocRetz'+selectrow);
    var ObjOthLocRet=document.getElementById('OthLocRet');
    if (ObjAutoPy){
	    if (SelRowObjOthLocRet){
   	  if (SelRowObjOthLocRet.innerText=="是"){ObjOthLocRet.checked=true} else {ObjOthLocRet.checked=false}
	    }
    }
...
function Bupdate_click()	
{
    ...
    var auditflag="N"
    var auditflagobj =document.getElementById('AuditFlag')
    if (auditflagobj)
    {
	    if (auditflagobj.checked)
	    {
		    auditflag="Y"
     ...
     var OthLocRetflag="N"
    var OthLocRetobj =document.getElementById('OthLocRet')
    if (OthLocRetobj)
    {
	    if (OthLocRetobj.checked)
	    {
		    OthLocRetflag="Y"
	...
	var updateret=cspRunServerMethod(encmeth,phlid,py,fy,math,tjfs,cyflag,userid,sendflag,printflag,wintypeflag,machine,screen,scpath,pysure,autopy,auditflag,OthLocRetflag)
	if (updateret=="0"){alert("保存成功!")}
	else if (updateret==""){alert("当前用户非本药房用户,请先在药房人员维护中添加,再进行相关操作!");return}
	else{alert("保存失败!"+updateret);return;}
    try {		   
...
function Badd_click()	
{
   ...
   var sendflag=""
	...
	if (cspRunServerMethod(encmeth,p1,p2,p3,p4,p5,cyflag,sendflag,printflag,wintypeflag,machine,screen,scpath,pysure,autopy,auditflag,OthLocRetflag)!='0')
	{
		alert(t['08']);
		return;
	}
