...
$(function(){ 
  InitBloodFlag("Combo_BloodFlag",SPECCode);
  InitSpecList();
  InitItemList();
...
{
	if(SPECCode==""){
		$.messager.alert('提示',"请先选择检验标本");
		return false;
	};
	var BloodFlag=$("#Combo_BloodFlag").combobox("getValue");
	var value=$.m({
		 ClassName:"DHCDoc.DHCDocConfig.Specimen",
		 MethodName:"updateSpecimenBloodFlag",
		 rowid:SPECCode,
		 BloodFlag:BloodFlag
	},false);
	if(value=="0"){
		SpecListDataGrid.datagrid('load');
   		SpecListDataGrid.datagrid('unselectAll');
	}else{
		$.messager.alert('提示',"保存失败:"+value);
	}
};
function InitBloodFlag(param1,param2)
{
	$("#"+param1+"").combobox({      
    	...
    	onBeforeLoad:function(param){
			param.ClassName = 'DHCDoc.DHCDocConfig.Specimen';
...
{
	SpecListColumns=[[    
                    { field: 'SPECCode', title: 'ID', width: 1,hidden:true},
        			{ field: 'SPECDesc', title: '名称', width: 50},
					{ field: 'BloodFlag', title: '名称', width: 50,hidden:true},
					{ field: 'BloodFlagDesc', title: '取血类型', width: 50}
    			 ]];
	SpecListDataGrid=$('#tabSpecList').datagrid({  
		fit : true,
		width : 'auto',
		border : false,
		striped : true,
		...
		fitColumns : true,
		autoRowHeight : false,
		url:$URL+"?ClassName=DHCDoc.DHCDocConfig.Specimen&QueryName=GetSpecList",
		loadMsg : '加载中..',  
...
function InitItemList()
{
	...
	ItemListColumns=[[    
                    { field: 'SPECIARCIMDR', title: 'ID', width: 1,hidden:true},
					{ field: 'rowid', title: 'ID', width: 1,hidden:true},
        			{ field: 'SPECIARCIMDesc', title: '名称', width: 120,editor:{
		                         type:'combogrid',
		                         options:{
		                             required: true,
		                             panelWidth:450,
	...
	ItemListDataGrid=$('#tabItemList').datagrid({  
		...
		fitColumns : true,
		autoRowHeight : false,
		url:$URL+"?ClassName=DHCDoc.DHCDocConfig.Specimen&QueryName=GetItemBySpecCode&SPECCode="+SPECCode,
		loadMsg : '加载中..',  
		pagination : true,  //是否分页
		rownumbers : true,  //
		idField:"rowid",
...
function LoadItemListDataGrid()
{
	...
	queryParams.ClassName ='DHCDoc.DHCDocConfig.Specimen';
	queryParams.QueryName ='GetItemBySpecCode';
	queryParams.SPECCode =SPECCode;
