...
function SaveCNMedCode()
{
   ...
   if(DataList=="") DataList="OPLimitType"+String.fromCharCode(1)+OPLimitType;
   else  DataList=DataList+String.fromCharCode(2)+"OPLimitType"+String.fromCharCode(1)+OPLimitType;
   var EPLimitType="";
   ...
   if(DataList=="") DataList="EPLimitType"+String.fromCharCode(1)+EPLimitType;
   else  DataList=DataList+String.fromCharCode(2)+"EPLimitType"+String.fromCharCode(1)+EPLimitType;
   ...
   /*
   //保存病情诊断
   /*var Diagnose=$('#SSDBCombo_Diagnose').combogrid('getValue');
   if(Diagnose!="") Diagnose=$('#SSDBCombo_Diagnose').combogrid("options").value
   if(DataList=="") DataList="DiagnoseRemarkICDDx"+String.fromCharCode(1)+Diagnose;
   else  DataList=DataList+String.fromCharCode(2)+"DiagnoseRemarkICDDx"+String.fromCharCode(1)+Diagnose;*/
	//保存需要审批的收费类别 
	var Str="";
	var size = $("#List_AdmReason"+ " option").size();
	   if(size>0){
			...
			if(DataList=="") DataList="NeedCheckBillType"+String.fromCharCode(1)+Str;
	        else  DataList=DataList+String.fromCharCode(2)+"NeedCheckBillType"+String.fromCharCode(1)+Str;
	   }
	   
   //datagrid的保存
	//var rows = BillTypeDurationDataGrid.datagrid("getRows");
	if (editRow != undefined) {
		var rows=BillTypeDurationDataGrid.datagrid("selectRow",editRow).datagrid("getSelected");  
		//var BillTypeRowid=rows.SelBillTypeRowid;
		var BillTypeRowid=rows.BillTypeRowid
		var editors = BillTypeDurationDataGrid.datagrid('getEditors', editRow); 
		var BillTypeDuration =  editors[0].target.combobox('getValue');
		var DiagnosCatArcimAllow =editors[1].target.is(':checked');
		if(DiagnosCatArcimAllow){
			  DiagnosCatArcimAllow=1;
		}else{
			  DiagnosCatArcimAllow=0;
		}
		var NotAllowItemInDuration=editors[2].target.is(':checked');
		if(NotAllowItemInDuration){
			  NotAllowItemInDuration=1;
		}else{
			  NotAllowItemInDuration=0;
		}
		...
		$.m({
			 ...
			 Node:"BillTypeDuration",
			 Node1:BillTypeRowid,
			 NodeValue:BillTypeDuration,
			 HospId:HospID
		},false);
		$.m({
			 ClassName:"web.DHCDocConfig",
			 MethodName:"SaveConfig1",
			 Node:"DiagnosCatArcimAllow",
			 Node1:BillTypeRowid,
			 NodeValue:DiagnosCatArcimAllow,
			 HospId:HospID
		},false);
		$.m({
			 ClassName:"web.DHCDocConfig",
			 MethodName:"SaveConfig1",
			 Node:"NotAllowItemInDuration",
			 Node1:BillTypeRowid,
			 NodeValue:NotAllowItemInDuration,
			 HospId:HospID
		},false);
		/*$.dhc.util.runServerMethod("web.DHCDocConfig","SaveConfig1","false",function(objScope,value,extraArg){	
		
	    },"","","BillTypeDuration",BillTypeRowid,BillTypeDuration);
		
		$.dhc.util.runServerMethod("web.DHCDocConfig","SaveConfig1","false",function(objScope,value,extraArg){	
		
	    },"","","DiagnosCatArcimAllow",BillTypeRowid,DiagnosCatArcimAllow);
		
		$.dhc.util.runServerMethod("web.DHCDocConfig","SaveConfig1","false",function(objScope,value,extraArg){	
		 
	    },"","","NotAllowItemInDuration",BillTypeRowid,NotAllowItemInDuration);*/
		BillTypeDurationDataGrid.datagrid("endEdit", editRow);
		editRow = undefined;
		BillTypeDurationDataGrid.datagrid('load');
		BillTypeDurationDataGrid.datagrid('unselectAll');
	}
	//保存便捷门诊信息
	/*var AdmReason=$("#SSDBCombo_AdmReason").combobox("getValue");
	if(DataList=="") DataList="QuickAdmBillType"+String.fromCharCode(1)+AdmReason;
	else  DataList=DataList+String.fromCharCode(2)+"QuickAdmBillType"+String.fromCharCode(1)+AdmReason;*/
	/*var size = $("#List_QuickAdmDep"+ " option").size();
	var Str="";
	if(size>0){
			$.each($("#List_QuickAdmDep"+" option:selected"), function(i,own){
              var svalue = $(own).val();
			  if (Str=="") Str=svalue;
			  else Str=Str+"^"+svalue;
			})
			if(DataList=="") DataList="QuickAdmDep"+String.fromCharCode(1)+Str;
	        else  DataList=DataList+String.fromCharCode(2)+"QuickAdmDep"+String.fromCharCode(1)+Str;
	}*/
	var HospID=$HUI.combogrid('#_HospList').getValue();
	if ((!HospID)||(HospID=="")) {
		HospID=session['LOGON.HOSPID'];
	...
	/*$.dhc.util.runServerMethod("web.DHCDocConfig","SaveConfig","false",function(value){	
       	if(value=="0"){
