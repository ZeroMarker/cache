...
//	websys_setTitleBar("OutPatient  Registeration");
   ...
   var obj=document.getElementById('guser');
	...
	if (obj1) obj1.onclick = add_Click;
...
{   type=document.getElementById('type').value
	...
	if (cspRunServerMethod(encmeth,'SetNo','',guser,type)=='0') {
			}	
}
...
{    judgepass1=0
     ...
     if (truthBeTold) {
      ...
      var str=startno+"^"+endno+"^"+startno+"^"+userid1+"^"+""+"^"+userid+"^^"+kyendno+"^"+kyrowid+"^"+type
      var err="";
 		p1=str
 		var getadd=document.getElementById('getadd');
		if (getadd) {var encmeth=getadd.value} else {var encmeth=''};
		if (cspRunServerMethod(encmeth,'addok','',p1)=='0'){
		};
     }
	 }
