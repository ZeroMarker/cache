...
function initDocument()
{
	...
	//刷新数据
	fillData()
	if(getElementValue("MRHold17")=="") getIP();
	totalFee_Change()
	//按钮控制
	setEnabled()
	...
	if (MaintType==1)
	{
		if ((Status=="")||(Status==0))
		{
			...
			singlelookup("MRAssignDR_UserName","PLAT.L.EQUser","","")		
		}
...
//modified by QW20220913 需求号:2787676 获取客户端ip 
function getIP(){
	setElement("MRHold17",ClientIPAddress)
}
...
function initPage()
{
	...
	initButton();
	jQuery("#BSave").linkbutton({iconCls:''});  ///Modefidy by txr 2022-12-30 2807805
	jQuery("#BSubmit").linkbutton({iconCls:''});  ///Modefidy by txr 2022-12-30 2807805
	jQuery("#BDelete").linkbutton({iconCls:''});  ///Modefidy by txr 2022-12-30 2807805
	jQuery("#BPicture").linkbutton({iconCls:''});  ///Modefidy by txr 2022-12-30 2807805
	...
	//jQuery("#BSelfCancelSubmit").linkbutton({iconCls: 'icon-w-back'}); //Add By QW20210916 BUG:QW0150 增加图标  ///Modefidy by txr 2022-12-30 2807805
	...
	setRequiredElements("MRRequestUserDR_UserName^MRRequestLocDR_LocDesc^MRRequestDate^MRExObjDR_ExObj^MRFaultCaseRemark^MRRequestTel^MRStartDate^MRFaultCaseDR_FCDesc")
    ...
    var ObjLocDR=getElementValue("MRMaintType")
}
...
function setSelectValue(vElementID,rowData)
{
	...
	else if (vElementID=="MRAssignDR_UserName"){setElement("MRAssignDR",rowData.TRowID)}
	...
	if ((EquipDR!="")&&(RowID==""))
	{
		$cm({
			...
			if (jsonData.SQLCODE<0) {$.messager.alert(jsonData.Data);return;}
				...
				//$("#MRSourceType").keywords("select","1");
...
function bcancelsubmit()
{
    ...
    {
	    messageShow("","","",t[0])
	    var url="dhceq.em.problem.csp?&RowID="+RtnObj.Data+"&CurRole="+getElementValue("CurRole");
		if ('function'==typeof websys_getMWToken){		//czf 2023-02-14 token启用参数传递
			url += "&MWToken="+websys_getMWToken()
		}
	    window.setTimeout(function(){window.location.href=url},50); 
...
function onBeforeShowPanel(vElementID)
{
	if (vElementID=="MRExObjDR_ExObj")
	{
		...
		if (getElementValue("MRObjLocDR")=="")
		{
			...
			messageShow("","","",Info)
			return false*/ //modify by zyq 2023-03-16 3278828 某些数据不存在使用科室
...
function bcanceltrue()
{
	...
	{	
		...
		{
			 url="dhceq.em.problemsimple.csp?&RowID=&Status=0"
		}
		else
		{
			url= "dhceq.em.problem.csp?&RowID=&Status=0&MaintType=1"
		}
		if ('function'==typeof websys_getMWToken){		//czf 2023-02-14 token启用参数传递
			url += "&MWToken="+websys_getMWToken()
		}
...
function BSelfCancelSubmit_Clicked()
{
    ...
    if (RtnObj.SQLCODE==0)
    {
		...
		{
			url="dhceq.em.problemsimple.csp?"+val;	
			}
		else
		{
			url="dhceq.em.problem.csp?"+val;	
		}
		if ('function'==typeof websys_getMWToken){		//czf 2023-02-14 token启用参数传递
			url += "&MWToken="+websys_getMWToken()
		}
