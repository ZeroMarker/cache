...
function initDocument()
{
	...
	//modified by zy 20191025 ZY0194
	setRequiredElements("BRRequestDate^BRRequestLocDR_CTLOCDesc^BRPrjName^BRLName^BRManageLocDR_CTLOCDesc^BRLRequestNum^BRLHold10^BRUseLocDR_CTLOCDesc^BRLHold1^BRLHold2^BRLHold3^BRLHold4^BRLRequestReason^BREquipTypeDR_ETDesc"); //必填项
...
function BSave_Clicked()
{
	...
	if (jsonData.SQLCODE==0)
	{
		...
		url="dhceq.em.buyrequest.csp?"+val
...
function GetAuditData()
{
	...
	ValueList=ValueList+"^"+getElementValue("BuyUserDR");
	
	ValueList=ValueList+"^"+getElementValue("BRAIRowID");	//7
	ValueList=ValueList+"^"+getElementValue("BRLRowID");	//BRLRowID
	ValueList=ValueList+"^"+getElementValue("RoleStep");	//RoleStep
	ValueList=ValueList+"^"+getElementValue("ActionID");		//Action
...
function fillData()
{
	...
	for (var i=1;i<=step;i++)
	{
		...
		if (i==step)
		{
			...
			$.parser.parse(); //add by zx 2019-09-11
...
function setArgumentation()
{
	...
	{
		argValue=0;
		setRequiredElements("ECWorkLoadNum^ECServiceIncome^ECMaintFee^ECFeeOfEmployee"); //必填项
	}
	...
	//modofied by zy ZY0206 begin
	var showFlag=0;
	if(argValue<=BRLHold10) showFlag=1
...
function uniteRequest()
{
	...
	if (getElementValue("BRLRowID")=="")
	{
		messageShow("","","","请先保存申请单!")
		return
	}
	//add by zx 2019-09-16 增加readonly参数
	var val="BRRowID="+getElementValue("BRRowID")+"&ApproveType=1&ReadOnly="+getElementValue("ReadOnly");
...
function ifbListForLoc()
{
	...
	var val="&SourceType=1&SourceID="+getElementValue("BRLRowID")+"&ReadOnly="+getElementValue("ReadOnly");
	url="dhceq.em.ifblistforloc.csp?"+val;
	//modified by zy ZY0222 2020-04-16
	showWindow(url,"采购申请论证-推荐产品信息","","","icon-w-paper","modal","","","middle",setIfbListForLocNum);   //modify by lmm 2020-06-04
}
...
function researchList(id)
{
	...
	{
		//add by ZY0224 2020-04-24
		var val="&BussType=1&SourceType=1&SourceID="+getElementValue("BRLRowID")+"&ReadOnly="+getElementValue("ReadOnly")+"&RFunProFlag=0";
		url="dhceq.em.research.csp?"+val;
		///modified ZY0201 
		showWindow(url,"采购申请论证-科研项目/论文","","","icon-w-paper","modal","","","middle",setResearchList);   //modify by lmm 2020-06-04
	}
	else
	{
		//add by ZY0224 2020-04-24
		var val="&BussType=1&SourceType=1&SourceID="+getElementValue("BRLRowID")+"&ReadOnly="+getElementValue("ReadOnly")+"&RFunProFlag=1";
		url="dhceq.em.research.csp?"+val;
		//modified by zy ZY0222 2020-04-16
		showWindow(url,"采购申请论证-功能项目","","","icon-w-paper","modal","","","small",setResearchList);
	}
}
//add by zy ZY0206 begin
//经济预测链接按钮
function econcalcList(CalcFlag)
{
	...
	var val="&SourceType=1&SourceID="+getElementValue("BRLRowID")+"&CalcFlag="+CalcFlag+"&ReadOnly="+getElementValue("ReadOnly");
	url="dhceq.em.econcalc.csp?"+val;
	//modified by zy ZY0222 2020-04-16
	showWindow(url,"采购申请论证-经济信息","","","icon-w-paper","modal","","","middle",setEconcalcList);
}
function reloadGrid()
{
	var WaitAD=getElementValue("WaitAD"); 
...
function setResearchList(RFunProFlag)
{
	...
	if (RFunProFlag=="0")
	{
		$("#cResearchLWNum").html("开展科研及论文【"+num+"】");
	}
	else
	{
...
function equipServiceList()
{
	...
	var val="&BussType=1&SourceType=3&SourceID="+getElementValue("BRLRowID")+"&ReadOnly="+getElementValue("ReadOnly");
...
function setServiceConsumableList()
{
	...
	jsonData=jsonData.Data;
	var num=jsonData.count;
	$("#cResearchNum").html("开展科研项目及论文【"+num+"】");
}
//add by zy ZY0224 end