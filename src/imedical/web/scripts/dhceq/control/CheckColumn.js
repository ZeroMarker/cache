//GR0033 图片上传用
//参照/scripts/dhcstm/ExtUx/CheckColumn.js
/*!
 ...
 * a column.
 */
Ext.dhceq.grid.CheckColumn = function(config){
...
Ext.dhceq.grid.CheckColumn.prototype ={
    init : function(grid){
        this.grid = grid;
    ...
    renderer : function(v, p, record){
        ...
        if(v=="Y"){
        	return String.format('<div class="x-grid3-check-col{0} {1}">&#160;</div>', '-on', this.createId());
        }else{
        	return String.format('<div class="x-grid3-check-col{0} {1}">&#160;</div>', '', this.createId());
        }
    }, 
...
// register ptype
Ext.preg('checkcolumn', Ext.dhceq.grid.CheckColumn);

// backwards compat
Ext.grid.CheckColumn = Ext.dhceq.grid.CheckColumn;