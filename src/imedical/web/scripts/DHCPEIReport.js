...
function BodyLoadHandler() {
	...
	obj=document.getElementById("BExportED");
	if (obj){ obj.onclick=BExportED_click; }	
	...
	if (obj){ obj.onclick = BPrintView; }
	//导出PDF
	obj=document.getElementById("BExportPDF");
	if (obj){ obj.onclick = BPrintView; }
	//发送按钮
	...
	if(obj&&(session['LOGON.CTLOCID']==572)) {obj.checked=true;}
...
{  
	...
	for (var i=1;i<rows;i++)
	{
		...
		if ((chkobj)&&(chkobj.checked)){
			var obj=document.getElementById("RPT_IADM_DRz"+i);
	...
	for (var i=1;i<rows;i++)
	{
		...
		if ((chkobj)&&(chkobj.checked)){
			...
			{	
				...
				xlsheet.cells(49,1).Value=xlsheet.cells(49,1).Value+Str[1]+XBExpStr; // 姓名
				xlsheet.cells(51,1).Value=xlsheet.cells(51,1).Value+Str[10]; // 套餐
				xlsheet.cells(52,1).Value=xlsheet.cells(52,1).Value+Str[11]; // 加项
				xlsheet.cells(5,8).Value=xlsheet.cells(5,8).Value+Str[12]; // 病历号
				//xlsheet.Range(xlsheet.Cells(1,2), xlsheet.Cells(4,2)).Font.Name = "黑体" 
			}
			xlsheet.printout();
			xlBook.Close (savechanges=false);
...
function SendEMail(infor, subject)
{
    ...
    jMail.Silent = true;
    jMail.Charset = "utf-8";
    jMail.FromName = "廊坊管道局医院体检中心"; //发件人
    jMail.From = "1964540152@qq.com"; //发送人的邮件地址
    jMail.AddRecipient("9739348@qq.com"); //收件人的邮件地址
    jMail.Subject = "ceshi";
    jMail.Body = "asdf"; //Email 内容
    jMail.MailServerUserName="1964540152@qq.com";//username
    jMail.MailServerPassWord="qwerty"; //password
    var ret = jMail.Send("smtp.qq.com");
    jMail.Close();
    return false;
...
function iniForm() {
	...
	if(IsSecret=="Y"){
		...
		if (obj) obj.innerText="保密报告";
		var obj=document.getElementById("cTTitle");
		if (obj) obj.innerText="保密报告列表";
	}
...
function BQuery_click() {
	...
	var IsSecret="N";
	var obj;
	var DocDR="",DepartName="";
	var iAuditTimeFrom="",iAuditTimeEnd="";
  	obj=document.getElementById("RegNo");
	...
	obj=document.getElementById("HadCompete");
	if ((obj)&&(obj.checked==true)) { iSTatus=iSTatus+"^"+"COMPLETE"+"^"+"C"; }
	...
	obj=document.getElementById("AuditTimeFrom");
	if (obj){ iAuditTimeFrom=obj.value; }

	obj=document.getElementById("AuditTimeEnd");
	if (obj){ iAuditTimeEnd=obj.value; }
	...
	var lnk="websys.default.csp?WEBSYS.TCOMPONENT="+"DHCPEIReport"
			...
			+"&AuditTimeFrom="+iAuditTimeFrom
			+"&AuditTimeEnd="+iAuditTimeEnd
			;
...
function BExportED_click()
{
	...
	var encmeth="",iPAADMDR="",RegNo="",Name="";
	var obj=document.getElementById("GetEDInfo");
	...
	{	
		...
		if ((chkobj)&&(chkobj.checked))
		{
			...
			obj=document.getElementById("RPT_PAADM_DRz"+i);
			if (obj){ iPAADMDR=obj.value; }
			var Info=cspRunServerMethod(encmeth,iPAADMDR)
			myTable.Rows.add();//新增行
			Row=Row+1;
			myTable.Cell(Row,3).Range.Text=Info;
			var obj=document.getElementById("RPT_RegNoz"+i);
			if (obj){ RegNo=obj.innerText; }
			myTable.Cell(Row,1).Range.Text=RegNo;
			var obj=document.getElementById("RPT_IADM_DR_Namez"+i);
...
function BPrintView(){
	...
	/*---新版C/S体检报告---	如使用标准功能，请将此段内代码注释  add by wgy*/
	if(eSrc.id=="BExport"||eSrc.id=="BPrint"||eSrc.id=="BExportPDF"||eSrc.id=="NoCoverPrint"||eSrc.id=="BExportHtml"||eSrc.id=="BPrintView"){
		if (jarPAADM==""){
			alert("请选择待打印报告");
		}else if(statusStr.indexOf("未")>0){
			alert("未初检，请核实报告状态");
		}else if(eSrc.id=="BPrintView"&&jarPAADM.split("#").length>1){
			alert("不可批量预览，请重新选择！");
		}else{
			calPEReportProtocol(eSrc.id,jarPAADM);
		}		
		return false;
	}
	/*---end---- */
...
function ShowPrintWindows(iRowID,iPAADMStr) {
	...
	if (Type=="Print"){
		...
		{
			var OneID=Arr[i]
			var InString=OneID+"^P^^";
			var ret=tkMakeServerCall("web.DHCPE.Report","SetReportStatus","","",InString)
		}
	}
	*/
