...
{   
	...
	var Instring=""
	var crmOrder=parent.frames["DHCPEIAdmItemStatusDetail"].GetId();
	for (var iLLoop=0;iLLoop<Datas.length-1;iLLoop++){
		FData=Datas[iLLoop].split("^");
		if (""!=FData) {
			var IADM=FData[2];
			PrintLisRequestApp(IADM,crmOrder,"IADM")
		}
...
{   
	...
	var value=cspRunServerMethod(encmeth,Instring,"","","N");
	if (value=="") return;
	PrintRis(value,Templatefilepath,Title)
}
...
{    
	...
	for (var iLLoop=0;iLLoop<Datas.length-1;iLLoop++){
		...
		if (""!=FData) {
			var IADM=FData[2];
			PrintRisRequestApp(IADM,crmOrder,"IADM")
		}
...
{   
	...
	//PrintRis(Instring,Templatefilepath,Title)
}

function PrintRisRequestAppOld(Instring,Type,BDFlag)
{   
	var targetFrame=GetCtlValueById("txtTargetFrame");
	...
	{    
		...
		{ 
			...
			Pages=Pages+1;
			var Rows=Pages*20
			xlsheet.Range(xlsheet.Cells(Rows+1,1),xlsheet.Cells(Rows+2,5)).mergecells=true; //合并单元格
			xlsheet.Range(xlsheet.Cells(Rows+1,6),xlsheet.Cells(Rows+2,8)).mergecells=true; //合并单元格
			xlsheet.Range(xlsheet.Cells(Rows+1,1),xlsheet.Cells(Rows+2,5)).HorizontalAlignment =-4108;//居中
		     xlsheet.Range(xlsheet.Cells(Rows+1,6),xlsheet.Cells(Rows+2,8)).HorizontalAlignment =-4108;//居中
		     xlsheet.Range(xlsheet.Cells(Rows+1,6),xlsheet.Cells(Rows+2,8)).VerticalAlignment=Excel.XlVAlign.xlVAlignUpper
		    xlsheet.cells(Rows+1,6).Value="*"+OnePatInfo[3]+"*";  //登记号条码 
		    xlsheet.cells(Rows+1,6).Font.Name="C39P36DmTt"; 
		    xlsheet.cells(Rows+1,6).RowHeight = 25;
		     //xlsheet.cells(Rows+1,6).ColumnWidth = 35;
		    xlsheet.cells(Rows+1,6).Font.Size=25;
		    xlsheet.cells(Rows+1,1).Font.Size=25;
			xlsheet.cells(Rows+2,1).Value=OnePatInfo[7]; //顺序号  Add 209-02-22
			xlsheet.cells(Rows+1,1).Value=Title;
			xlsheet.cells(Rows+4,1).Value="姓名:";
			xlsheet.cells(Rows+4,2).Value=OnePatInfo[0]; //姓名
			xlsheet.cells(Rows+4,3).Value="年龄:";
			xlsheet.cells(Rows+4,4).Value=OnePatInfo[1]; //年龄 
	...
	function PrintYGGZD()
{
	...
	for (var iLLoop=0;iLLoop<Datas.length-1;iLLoop++){
		...
		if (""!=FData) {
			var IADM=FData[2];
			var InfoStr=cspRunServerMethod(encmeth,IADM,"");
			if (InfoStr=="") continue;
			var InfoStrArr=InfoStr.split("$$");
			var BaseInfo=InfoStrArr[0];
			var BaseArr=BaseInfo.split("^");
			...
			{  
				...
				var Templatefilepath=prnpath+Temp[0]+".xls";
				var OrderName=Temp[1];
				xlApp = new ActiveXObject("Excel.Application");
				xlBook = xlApp.Workbooks.Add(Templatefilepath);
				xlsheet = xlBook.WorkSheets("Sheet1");
