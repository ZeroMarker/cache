...
function initSelect() {
    ...
    initCombobox("IsShowCALogonLinks",ret,false,false);
    initCombobox("IsNeedQrCode",ret,false,false);
    initCombobox("IsSupportPDFSign",ret,false,false);
    initCombobox("AllPatCAOn",ret,false,false);
}
...
function initVenderCode() {
    ...
    ajaxPOSTSync(data, function (ret) {
        if (ret != "")
        {
            ...
            initCombobox("DefaultPATPDFVenderCode",ret,false,true);
        }
        else
        {
...
function saveCommon() {
	if ((selHospID == "")&&(sysOption.IsOnMulityHospMgr == "1"))
	{
    ...
    var Arr = [];
    pushArr(Arr,"IsCAOn","是否开启医护签名",$("#IsCAOn").combobox('getValue'));
    pushArr(Arr,"AllCAOn","全院开启医护签名",$("#AllCAOn").combobox('getValue'));
    ...
    pushArr(Arr,"DefaultPATPDFVenderCode","患者签名厂商代码",$("#DefaultPATPDFVenderCode").combobox('getValue'));
    ...
    pushArr(Arr,"IsShowCALogonLinks","登录页面是否显示CA登录按钮",$("#IsShowCALogonLinks").combobox('getValue'));
    pushArr(Arr,"IsSupportPDFSign","是否支持PDF签署",$("#IsSupportPDFSign").combobox('getValue'));
    pushArr(Arr,"IsNeedQrCode","签署时是否需要展示二维码",$("#IsNeedQrCode").combobox('getValue'));
    
    pushArr(Arr,"AllPatCAOn","全院开启患者签名",$("#AllPatCAOn").combobox('getValue'));
...
function setCommon(data) {
    ...
    $("#DefaultPATPDFVenderCode").combobox('select',data["DefaultPATPDFVenderCode"]);
    ...
    $("#IsShowCALogonLinks").combobox('select',data["IsShowCALogonLinks"]);
    $("#IsNeedQrCode").combobox('select',data["IsNeedQrCode"]);
    $("#IsSupportPDFSign").combobox('select',data["IsSupportPDFSign"]);
    $("#AllPatCAOn").combobox('select',data["AllPatCAOn"]);
