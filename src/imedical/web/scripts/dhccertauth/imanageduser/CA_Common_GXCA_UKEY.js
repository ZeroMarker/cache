    ...
    (function() {
        function initXTXAppCOM(xtxsync) {
            ...
            if (xtxsync.XTXAppCOM == null) {
                alert("加载XTXAppCOM控件失败，请确认已正确安装GXCA证书应用环境!");
                return false;
            }
        ...
        function initXTXAppWebSocket(xtxsync) {
            ...
            if (xtxsync.XTXAppWebSocket == null) {
                alert("连接XTXAppCOM服务失败，请确认已正确安装GXCA证书应用环境!");
                return false;
            }
            return true;
        }
        ...
        function initXTXAppObject(xtxsync) {
            ...
            if (util.checkBrowserISIE()) { // IE
                ...
                getImplmentFunction = function(methodInfo) {
                    if (methodInfo.inParams == '') { // 0 input param
                        gxca_method[methodInfo.funcName] = new Function('return xtxsync.XTXAppCOM.' + methodInfo.funcName + '();');
                    } else {
                        gxca_method[methodInfo.funcName] = new Function(methodInfo.inParams,
                ...
                getImplmentFunction = function(methodInfo) {
                    if (methodInfo.inParams == '') { // 0 input param
                        gxca_method[methodInfo.funcName] = new Function(
            ...
            for (var i = 0; i < export_functions.length; i++) {
                ...
                xtxsync[export_functions[i].funcName] = gxca_method[export_functions[i].funcName];
                if (export_functions[i].aliasName) {
                    xtxsync[export_functions[i].aliasName] = gxca_method[export_functions[i].funcName];
                }
            }
            return true;
        }
