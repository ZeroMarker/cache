...
AntMain=function(ShowTabStr,EpisodeID,ArcimRowid,AntibApplyRowid,UserReasonId,OrderPoisonCode){
		...
		if (wantAudit){this.DoctorInfo+="&nbsp;&nbsp;&nbsp;&nbsp;&nbsp"+"<font color='blue'><B>需要审核</B></font>";}
		if (wantConsultation){this.DoctorInfo+="&nbsp;&nbsp;&nbsp;&nbsp;&nbsp"+"<font color='blue'><B>需要会诊</B></font>";}
...
AntReasonPan=function(EpisodeID,ArcimRowid,UserReasonId,OrderPoisonCode){
		...
		this.AntIndications = new Ext.form.ComboBox({
			...
			,valueField : 'Rowid'
			,fieldLabel : "指征<font color='red'>*</font>",
			allowBlank:false,
			emptyText:"请选择指征"
			,displayField : 'desc'
			,triggerAction : 'all',
			...
			listeners:{
						select:function(x,R,curindex){
							...
							if(Ext.getCmp('UserReason').getRawValue().indexOf("清洁手术")>=0){
								alert(1)
								if(R.data.desc.indexOf("手术>3小时")>=0){
	  ...
	  this.AntOperationStoreProxy = new Ext.data.HttpProxy(new Ext.data.Connection({
		...
		this.AntOperationStore=new Ext.data.Store({
				...
				reader:new Ext.data.JsonReader({
						...
						[
								...
								{name:'OperationStartDate',mapping:'StartDateStr'},
								{name:'OperationEndDate',mapping:'EndDateStr'},
		...
		this.AntOperation=new Ext.grid.GridPanel({
				...
				columns:[
						...
						{header:'手术开始时间',dataIndex:'OperationStartDate',sortable: true},
						{header:'手术结束时间',dataIndex:'OperationEndDate',sortable: true},
    ...
    var AnditArcArr=""
		...
		var btnSave = new Ext.Button({
				...
				handler: function(){
					...
					var ret=cspRunServerMethod(AddAuditItemToListMethod,'AddCopyItemToList','',EpisodeID,AnditAARowidArr);
	  ...
	  ///
		...
		this.auditApplyInfo=new Ext.grid.GridPanel({
			...
			/*
			*/
		var winAuditInfo=new Ext.Window({
				//id:'AuditAnt',
				height:400,
				width:600,
				title:'以下抗菌药物申请已经审核，是否下医嘱？',
				layout:'fit',
				closeAction:'close',
				resizable:false,
				closable:true,
