...
Ext.onReady(function(){
	//创建window窗体
	function createWindow(episodeID, patientID, tree)
	{
		var singleWindowID = 'frmSingle';
		var arrSelectNode = tree.getChecked();
		var frmurlSingle = '';
		for (var i = 0; i < arrSelectNode.length; i++)
		{
			var node = arrSelectNode[i];
			//创建唯一
			if (node.id.substring(0, 2) == 'TS' ||node.id.substring(0, 2) == 'TM')
			{
				var templateDocID = node.id.substring(2, node.id.length);
				var printTemplateDocID = node.attributes.printTemplateDocID;
				var frameID = 'frame' + episodeID + printTemplateDocID + templateDocID;
				frmurlSingle += '<iframe id=' + frameID + ' scroll=false style="border-style: none; width:100%; height:100%" src = "epr.newfw.viewerSingle.csp?EpisodeID=' + episodeID + '&PatientID=' + patientID + '&PrintTemplateDocID=' + printTemplateDocID + '&TemplateDocID=' + templateDocID + '"</iframe>';
				var win = Ext.getCmp(singleWindowID);
				if(!win){
					win = new Ext.Window({
						id: singleWindowID,
						footer : false,
						...
						html: '<div style="width:100%;height:100%;OVERFLOW-y:auto;" id = "divSingle"></div>'
					});
					LocationX += addX;
					LocationY += addY;
				...
				divSingle.innerHTML = '';
				divSingle.innerHTML = frmurlSingle;
				win.show();
			}
			/*
			//创建历次
			if (node.id.substring(0, 2) == 'TM')
			{
				var templateDocID = node.id.substring(2, node.id.length);
				var printTemplateDocID = node.attributes.printTemplateDocID;
				var mutilWindowID = 'frm' + episodeID + printTemplateDocID + templateDocID;
				var frameID = 'frame' + episodeID + printTemplateDocID + templateDocID;
				...
				var win = Ext.getCmp(mutilWindowID);
				if(!win)
				{
					win = new Ext.Window({
						id: mutilWindowID,
	...
	function getBorwserTree(episodeID)
	{
		...
		var treeTbar = [
		{
			...
			pressed: false,
			cls: 'x-btn-text-icon',
			icon:'../scripts/epr/viewer/window/Pics/btnConfirm.gif',
			handler: function(){
				//alert(tree.getChecked('id'));
				
				createWindow(episodeID, patientID, tree);
			}
		}];
	...
	var panel = new Ext.Panel(
	{
            ...
            items : [
                        ...
                        },
                        {
                            ...
                            border : false,
                            buttons: [
								{ id : "btnOrdLong", text: "长期医嘱", handler: tt},
