...
function preview() {
	...
	eprform.PreviewClick(bindPrnTemplateID, printTemplateDocId,privacyLevel);
}
...
function exportRecord() {
	...
	eprform.PrnTemplateDocID = printTemplateDocId;
	eprform.TemplateDocID = templateDocId;
	eprform.ExportRecordClick(bindPrnTemplateID, printTemplateDocId,privacyLevel);
}

// 删除病历 Add by LiangWeixi
function delChart(){
    //alert("foo function")
	Ext.Ajax.request({
			...
			success : function(response, opts) {
				...
				//refresh(episodeID,patientID,templateDocId,printTemplateDocId)
				else {
					alert("删除失败!")
				}
			},
...
function save(isConfirm) {
	...
	if (Ext.getCmp('btnSave').disabled == false) {
		...
		if (EPRNum == '1') {
		    ...
		    eprform.InvokePreviewService(pateintID, episodeID, logID, bindPrnTemplateID, printType, "", userID, printTemplateDocId);
...
function print() {
	...
	if (Ext.getCmp('btnPrint').disabled == false) {
		...
		var printState = eprform.PrintClick(bindPrnTemplateID,
				printTemplateDocId,privacyLevel);
		...
		if (templateID == SendMsgTemplateID)
		{
			GetEpsisodeIDToENC();
		}	
...
function refresh(episodeID, patientID, templateDocID, prtTemplateDocID) {
	...
	Ext.Ajax.request({
		...
		success : function(response, opts) {
	        ...
	        */
	        var tmpRevisionStatus = getRevisionStatus(revision, revisionActiveStatus, obj[0].currState);
	        eprEditDll.Revisionable = tmpRevisionStatus;
	    },
	    failure: function(response, opts) {
...
//时间：2014-8-22
//功能：病案首页打印成功就诊号推送
function GetEpsisodeIDToENC(){
			Ext.Ajax.request({
			url: '../web.eprajax.Observer.GetEpsisodeIDToENC.cls',
...
function handWrittenSign()
{
	...
	*/
    var eprform = frames['epreditordll'].document.getElementById('eprform');
    //debugger; 
    var succ = eprform.HandwrittenSingle(pateintID, episodeID, EPRNum, eprLogsID, userID, printTemplateDocId, bindPrnTemplateID);
    if (succ)
    {
