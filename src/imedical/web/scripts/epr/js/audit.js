...
function eprAudit(userLevel) {
	...
	function ajaxLogin() {
		...
		if (!parent.parent.AdAuthForXieHe(userName, pwd)) {
			return;
		}
		...
		Ext.Ajax.request({
			...
			success : function(response, opts) {
				...
				if (obj == "LoginValidFail") {
					...
					win.close();
					getPower();
					//生成病历图片 -- add on 2012-12-27 by houj
					invokeSinglePreviewBySltTpl(bindPrnTemplateID, printTemplateDocId);
				} else if (obj == "sessionTimedOut") {
					alert("登陆超时,会话已经中断,请重新登陆在进行操作!");
				} else {
					alert("操作失败!错误原因:" + obj);
					// bbar.innerHTML = '<div
					// style="font-weight:bold;">日志记录失败!</div>';
				}
			},
			failure : function(response, opts) {
				Ext.get("winLogin").unmask();
				var obj = "操作失败,错误代码:" + response.status + "," + "错误信息:"
						+ response.statusText;
				alert(obj);
			}
		});
