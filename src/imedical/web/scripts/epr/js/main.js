...
var menu1 = new Ext.menu.Menu({
		items:[{
			...
			id : 'btnBatchOperation',
			cls : 'x-btn-text-icon',
			text : '批量提交',
			height : 20,
			pressed : false,
			disabled : false,
			icon : '../scripts/epr/Pics/btnConfirm.ico',
			handler : function() {
				window.showModalDialog(
						"../csp/epr.newfw.batchoperation.csp",
						self,
						"dialogWidth:800px;dialogHeight:500px;toolbar=no;menubar:no;scrollbars:no;resizable:no;location:no;status:no;help:no;fullscreen=no");			
			}
		}]
});
...
if (IsRecyclerOn())
{
	menu1.addMenuItem({
			...
			pressed : false,
			disabled : false,
			icon : '../scripts/epr/Pics/Recycler.png',
			handler : function() {
				setDllVisibility("hidden");
				var win = new Ext.Window({
					id : 'restoreWin',
					...
					height : 500,
					title : '病历恢复',
					animateTarget : 'btnRestore',
					constrainHeader: true,
					// raggable: true, //不可拖动
					modal : true, // 遮挡后面的页面
					resizable : true, // 重置窗口大小
					html : '<iframe id="frmRestore" style="width:100%; height:100%" src="epr.newfw.restorechart.csp?EpisodeID='
							+ episodeID
							+ '&PatientID='
							+ patientID
							+ '"></iframe>'
...
function IsRecyclerOn() {
	return "Y" == RecyclerStatus;
}
