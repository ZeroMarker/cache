...
(function($) {
	$(function() {
		...
		$('#addApplyBtn').on('click', function() {
			...
			$('#patientListTable').datagrid({
				...
				columns: [[
					...
					{ field: 'CheckNoNeedApply', title: '是否可无需申请', width: 80 },
			...
			*/
			...
			//IE10及以下不支持readAsBinaryString兼容
			...
			document.querySelector("#file").addEventListener("change", function () {
				...
				reader.onload = function(e){
					...
					var zzexcel = window.XLS.read(data,{type:'binary'})
					if (JSON.stringify(zzexcel.Sheets)=="{}")
					{
						alert("请确保Excel格式版本是否正确后再操作!")
						return;
					}
					/*//可以多个sheet导入
					...
					//默认导入sheet1中的内容
					var result = window.XLS.utils.sheet_to_json(zzexcel.Sheets[zzexcel.SheetNames[0]]);
					var cnt = 0;
					var str = "";
					var len = result.length
					console.log(data,zzexcel)
					for (var i = 0; i < len; i++)
					{
						var flg = patchSearchPatient(result[i]["MedRecordNo"])
						if (flg)
						{
							cnt+=1;
						}
						...
						{
							str+=result[i]["MedRecordNo"]+","
						}
					}
					if (len != cnt)
					{
						alert("导入共计"+len+"条记录,查询成功"+cnt+"条记录,其中查询失败的病案号有:" + str)
					}
				}
			});
			//批量导入患者查询
			$('#patchPatSearchBtn').on('click', function() {
		...
		//批量查询就诊
		function patchSearchPatient(MedRecordNo) {
			if (MedRecordNo == null || MedRecordNo=="") return false;
			var obj = $.ajax({
				url: '../DHCEPRRBAC.web.eprajax.ApplyPermission.cls',
				data: {
					Action: 'episodelist',
					...
					PatientID: '',
					RegNo: '',
					MedRecordID: MedRecordNo
				},
				type: 'post',
				async: false
			});
			obj = JSON.parse(obj.responseText)
			if (obj.total<1) return false;
			var row = obj.rows[0] || {}
			$('#patientListTable').datagrid('appendRow',row);
			return true;
		}
		...
		//默认角色浏览列表
		$('#applyDefaultListTable').datagrid({
			//url: '../DHCEPRRBAC.web.eprajax.ApplyPermission.cls',
			queryParams: {
		...
		$('#applyListTable').datagrid({
			...
			columns: [[
				...
				{ field: 'AppointComment', title: '被拒绝或取消原因', width: 150 },
		...
		//收藏分类列表
		$('#categoryListSelectTable').datagrid({
			//url: '../DHCEPRRBAC.web.eprajax.FavoritesCategory.cls',
			queryParams: {
				Action: 'getcategorylist',
				UserID: appointUserID
		...
		function refleshCategory(id) {
			...
			queryParams.Action = 'getcategorylist';
			queryParams.UserID = appointUserID;
