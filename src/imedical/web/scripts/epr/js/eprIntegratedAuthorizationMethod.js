...
//右键菜单*********************************************
//Desc: 弹出病历浏览
function goEPRBrowse(grid, rowindex, e) {
    //debugger;
...
//“是否已打印”、“是否发送至病案室”Y/N为是/否
function getIsPrintedAndIsCommited(val){
    var retStr = "";
    switch (val) {
        case 'Y':
            retStr = '是';
...
function getCateCharpterDesc(val) {
	...
	for (i=0;i<ccDescStr.length ;i++ )
	{
		...
		if (resultStr == "")
		{
			resultStr = ccDescSub[1];
		}
		else
		{
			resultStr = resultStr + "," + ccDescSub[1];
		}
	}
	return resultStr;
}
...
function changeRowBackgroundColor(grid){
    ...
    for (var i = 0; i < total; i++) {
        ...
        if (isAppointed) {
            ...
            if (store.getAt(i).data['CanAppoint'] == '0') {
...
function gridRowClick(g, index, e){
    ...
    var isAppointed = CheckAppointed(record.data['IsAppointed']);
    var canAppoint = record.data['CanAppoint'];
    var appointCateCharpter = record.data['AppointCateCharpter'];
	
	//增加“是否已打印”、“是否已送至病案室”信息  by niucaicai 2016-3-15
	//var IsPrinted = getIsPrintedAndIsCommited(record.data['IsPrinted']);
	var IsCommited = getIsPrintedAndIsCommited(record.data['IsCommited']);
	//Ext.getCmp('lblIsPrinted').setValue(IsPrinted);
	Ext.getCmp('lblIsCommited').setValue(IsCommited);
    
    //授权申请信息(申请相关)
    Ext.getCmp('lblRequestDate').setValue(record.data['RequestDate']);
    Ext.getCmp('lblRequestTime').setValue(record.data['RequestTime']);
    Ext.getCmp('lblRequestUser').setValue(record.data['RequestUser']);
    Ext.getCmp('lblRequestDept').setValue(record.data['RequestDept']);
    Ext.getCmp('lblActionType').setValue(getAction(record.data['EPRAction']));
    
    var requestReason = record.data['RequestReason'];
	while (requestReason.indexOf("@@@")!= -1)
	{
		requestReason = requestReason.replace("@@@","'");
	}
    Ext.getCmp('taRequestReason').setValue(requestReason);
    var beforeRequestContent = record.data['BeforeRequestContent'];
    while (beforeRequestContent.indexOf("@@@")!= -1)
	{
		beforeRequestContent = beforeRequestContent.replace("@@@","'");
	}
    Ext.getCmp('taBeforeRequestContent').setValue(beforeRequestContent);
    var afterRequestContent = record.data['AfterRequestContent'];
    while (afterRequestContent.indexOf("@@@")!= -1)
	{
		afterRequestContent = afterRequestContent.replace("@@@","'");
	}
    ...
    if (isAppointed) {
        //已审核
        Ext.getCmp('lblAppointDate').setValue(record.data['AppointDate']);
        Ext.getCmp('lblAppointTime').setValue(record.data['AppointTime']);
        Ext.getCmp('lblAppointUser').setValue(record.data['AppointUser']);
        Ext.getCmp('lblAppointType').setValue(getAppointType(record.data['AppointType']));
        Ext.getCmp('lblAppointSpan').setValue(record.data['AppointSpan'] + '小时');
        //病历范围
        setCateChapterInfo(requestCateCharpter, requestCateCharpterDesc, appointCateCharpter, true, ID, index, canAppoint);
    }
    else {
        //未审核
        ...
        Ext.getCmp('lblAppointType').reset();
        Ext.getCmp('lblAppointSpan').setValue("");
        //病历范围
        setCateChapterInfo(requestCateCharpter, requestCateCharpterDesc, appointCateCharpter, false, ID, index, canAppoint);
    }
}
...
function setCateChapterInfo(requestCC, requestCCDesc, appointCC, isAppointed, ID, index, canAppoint){
    ...
    if (isAppointed) {
        ...
        if (canAppoint == '0') {
			...
			var btnSelectAllFsRange = new Ext.Button({
                id: 'btnSelectAllFsRange',
                name: 'btnSelectAllFsRange',
                text: '全选',
                handler: function(){
					for (var i = 0; i < fsRange.items.items.length; i++) {
						if (fsRange.items.items[i].checked == false) {
							fsRange.items.items[i].setValue(true);
						}
					}
				}
			...
			var btnUnSelectAllFsRange = new Ext.Button({
                id: 'btnUnSelectAllFsRange',
                name: 'btnUnSelectAllFsRange',
                text: '反全选',
                handler: function(){
					for (var i = 0; i < fsRange.items.items.length; i++) {
						if (fsRange.items.items[i].checked) {
							fsRange.items.items[i].setValue(false);
						}
					}
				}
			});
            //fsRange.add(btnUpdateFsRange);
			fsRange.buttons = [btnSelectAllFsRange,btnUnSelectAllFsRange,btnUpdateFsRange];
        }
    }
...
function getUrl(){
    ...
    //拼url
    var PAADM = '&PAAdmType=' + pAAdmType + '&PAStatus=' + pAStatus + '&PapmiNo=' + PapmiNo + '&MedicareNo=' + MedicareNo; //modify by niucaicai 2013-03-13
...
function commitAppoint(){
    ...
    if (selectedRows.length > 0) {
        for (var i = 0; i < selectedRows.length; i++) {
            ...
            //操作类型
            var action = row.get('ActionType');
            if (action == "") {
                //操作类型没填，则同申请类型
                action = row.get('EPRAction');
            }
            ...
            //授权范围
            var appointCateCharpter = "";
            var isAppointed = CheckAppointed(row.get('IsAppointed'));
            //若未授权而授权范围为空，则默认为审批时没有更改授权范围，授权范围为申请的范围
            //郑大五附院row.get['AppointCateCharpter'] == null这个判断条件会导致表达式一直为真，导致修改申请范围功能失效
            if (((row.get('AppointCateCharpter') == "") || (row.get('AppointCateCharpter') == null)) && (!isAppointed)) {
                appointCateCharpter = row.get('RequestCateCharpter');
            }
            else {
                appointCateCharpter = row.get('AppointCateCharpter');
            }
            //ajax
            Ext.Ajax.request({
                url: url,
    ...
    gstore.load({
        ...
        callback: function(r, options, success){
            ...
            if (recordCount == 0) {
                ...
                var view = grid.getView();
                for (i = 1 + k * pageSize, n = 1; i <= k * pageSize + recordCount, n <= recordCount; i++, n++) {
                    for (j = 1; j <= temp_obj.length; j++) {
