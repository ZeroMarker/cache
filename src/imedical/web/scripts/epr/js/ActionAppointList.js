...
//是否可以审核指定授权级别的记录
function CheckAuthLevel(authLevel,mainDocID,deptID) {
    if (authLevelTop == "100"){
		//患者主管医师的授权级别，只能审核“本人主管患者” + “患者主管医师级别”的授权
		return (authLevel <= "100" && mainDocID == appointUserID);
	} else if (authLevelTop == "200") {
		//科主任的授权级别，只能审核“本科室患者” + “科主任级别”的授权
		return (authLevel <= "200" && deptID == appointUserLoc);
	} else if (authLevelTop == "300") {
		//医务处的授权级别，可以审核“医务处级别”及以下级别的授权
...
function rightClickFn(grid, rowindex, e) {
    ...
    var menus = new Ext.menu.Menu({
        ...
        items: [
        {
            ...
            handler: function() { 
				parent.goEPRBrowse(grid, rowindex, e); 
			}
        }]
    });
...
function getGridPanel(){
    ...
    var sm = new Ext.grid.CheckboxSelectionModel({
		...
		renderer: function(v, c, r){
			...
			if (r.get('CanAppoint') == '0' || CheckAppointed(r.get('IsAppointed')) || CheckAuthLevel(r.get('AuthLevel'),r.get('MainDocID'),r.get('CurDeptID')) == false) {
				return " ";
			}
			//原本行头的复选框
			else {
...
function getDetailForm(){
    ...
    var fsBasicInfo = {
        ...
        items: [{
            ...
            store: new Ext.data.SimpleStore({
                fields: ['value', 'text'],
                data: [['view', '界面模板浏览'], ['save', '保存'], ['print', '打印'], ['commit', '提交'], ['switch', '切换模板'], ['switchtemplate', '更新模板'], ['chiefcheck', '主任医师签名'], ['attendingcheck', '主治医生签名'], ['browse', '病历浏览']]
            }),
            valueField: 'value',
            displayField: 'text',
            typeAhead: true,
    ...
    var fsReason = new Ext.form.FieldSet({
        id: 'fsReason',
        labelWidth: 0,
        title: '申请原因',
        collapsible: true,
    ...
    var detailForm = new Ext.FormPanel({
        ...
        //width: 500,
        //layout:'fit',
        items: [fsBasicInfo, fsReason, fsRange],
        tbar: ['->', '-', {
            id: 'btnRefuse',
            name: 'btnRefuse',
            text: '拒绝',
    ...
    Ext.Ajax.request({
        ...
        success: function(response, opts){
            ...
            if (response.responseText == "1") {
                ...
                Ext.getCmp('taRequestReason').setValue("");
                Ext.getCmp('taBeforeRequestContent').setValue("");
                Ext.getCmp('taAfterRequestContent').setValue("");
                //重新初始化fsRange
...
function commitAppoint(){
    ...
    var selectedRows = Ext.getCmp('disAppointGrid').getSelectionModel().getSelections();
    var row = selectedRows[0]; //已保证是单选
    if (action != row.get('EPRAction')) {
        Ext.MessageBox.confirm("提示", "授予的操作权限与申请的权限不同，确定要进行授权操作？", function(btn){
            if (btn == 'yes') {
                ajaxCommit(params);
            }
            else {
...
grid.on('rowclick', function(g, index, e){
   ...
   var requestReason = record.data['RequestReason'];
    ...
    //授权申请信息(审核相关)
    if (IsAppointed(appointDate, appointTime) || CheckAuthLevel(record.data['AuthLevel'],record.data['MainDocID'],record.data['CurDeptID']) == false) {
        //已审核
		...
		Ext.getCmp('btnRefuse').setDisabled(true);
    }
    else {
        //未审核
        Ext.getCmp('appointDate').setValue("");
        ...
        Ext.getCmp('appointUser').setValue("");
        Ext.getCmp('appointType').reset();
        Ext.getCmp('appointSpan').setValue(defaultAppointSpan);
        //病历范围
		...
		Ext.getCmp('btnRefuse').setDisabled(false);
    }
...
function IsAppointed(appointDate, appointTime){
    if (appointDate == "1840-12-31" || appointDate == "") 
        return false;
    if (appointTime == "00:00:00" || appointTime == "") 
        return false;
    return true;
}
