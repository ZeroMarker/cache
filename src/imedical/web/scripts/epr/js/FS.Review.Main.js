...
(function (win) {
	$(function () {
		...
		$('#resetBtn').on('click', function () {
			...
			$('#inputType').combobox('select','MR||U');
		});
		...
		$('#passBtn').on('click', function () {
			...
			if (result !== "-1") {
				var obj = $.ajax({
					url: "../DHCEPRFS.web.eprajax.AjaxReview.cls?Action=reviewpass&ActionCode=1&EpisodeID=" + FSReviewMain.SelectEpisode + "&UserID=" + userID,
					type: 'post',
					async: false
				});
				...
				if (ret == "-10") {
					$.messager.alert('提示', '质控科复核未通过，不能进行操作！', 'info');
					return;
				}
				else if (ret != "1") {
		...
		var patientDG = $('#episodeListTable').datagrid({
			...
			columns: [[
				...
				{ field: 'ReviewStatus', title: '病案室审核', width: 80, styler: cellStyler },
				{ field: 'QCDeptStatus', title: '质控科审核', width: 80, styler: cellStyler, hidden: true },
				...
				{ field: 'MedRecordNo', title: '病案号', width: 80, sortable: true, formatter: medRecordNoFormatter },
		...
		$('#inputType').combobox({
			...
			onSelect: function(rec) {
				FSReviewMain.TypeCode = rec.code;
				if ((FSReviewMain.TypeCode == 'QC||B')||(FSReviewMain.TypeCode == 'MR||B')||(FSReviewMain.TypeCode == 'A')) {
		...
		function getSelectEpisode() {
			...
			if (rows.length !== 1) {
				$.messager.alert(FSReviewMain.Config.ERROR_INFO, FSReviewMain.Config.ERROR_INFO_NOEPISODESELECTED, 'error');
				return "-1";
			}
			else {
				var row = rows[0];
		...
		function qcback(reason) {
			var obj = $.ajax({
				url: "../DHCEPRFS.web.eprajax.AjaxReview.cls?Action=qcback&ActionCode=1&EpisodeID=" + FSReviewMain.SelectEpisode + "&UserID=" + userID + "&Reason=" + encodeURI(reason) + "&Back2Nur=" + FSReviewMain.Back2Nur + "&Back2Doc=" + FSReviewMain.Back2Doc,
				type: 'post',
				async: false
			});
			FSReviewMain.Back2Doc = "0";
		...
		function superURLFormatter(value, row, index) {
			...
			return '<a href="javascript:void(0)" onclick="window.showModalDialog(\''+url+'\',{ funObj: FSReviewMain.SearchEpisode,ActionCode:1 },\''+style+'\');">三单一致</a>';
		}
		...
		function openWin(episodeID) {
			...
			var iLeft = 0;
			var url = 'dhc.epr.fs.review.view.csp?EpisodeID=' + episodeID + '&UserID=' + userID;
			window.showModalDialog(url, { funObj: FSReviewMain.SearchEpisode, ActionCode:'1' }, 'dialogHeight=' + iHeight + 'px;dialogWidth=' + iWidth + 'px;dialogTop=' + iTop + 'px;dialogLeft=' + iLeft + 'px;center=yes;help=no;resizable=yes;scroll=no;status=no;edge=sunken');
		}
		...
		function eprURLFormatter(value, row, index) {
			...
			var style = 'dialogHeight=' + iHeight + 'px;dialogWidth=' + iWidth + 'px;dialogTop=' + iTop + 'px;dialogLeft=' + iLeft + 'px;center=yes;help=no;resizable=yes;scroll=no;status=no;edge=sunken';
			return '<a href="javascript:void(0)" onclick="window.showModalDialog(\''+url+'\',{ funObj: FSReviewMain.SearchEpisode,ActionCode:1 },\''+style+'\');">电子病历</a>';
		}
