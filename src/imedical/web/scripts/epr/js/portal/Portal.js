...
Ext.ux.Portal = Ext.extend(Ext.Panel, {
	...
	getAllCol : function() {
		...
		for (var i = 0; i < this.items.length; i++) {
			arrCol.push(this.items.itemAt(i));
		}
		return arrCol;
	},
	...
	/*
	  * 返回列内所有的panel,用到这个
	  */
	getColAllPanel : function(col) {
		var arrColAllPanel = new Array();
		for (var i = 0; i < col.items.length; i++) {
			arrColAllPanel.push(col.items.itemAt(i));
		}
		return arrColAllPanel;
	},
    
    /**
	 * 添加Portlet,用到这个
	 */
    addPorlet : function(porlet, colIndex)
    {
		var c = this.items.itemAt(colIndex);		
		c.items.add(porlet);
    },
		...
		for (var i = 0; i < this.items.length; i++) {
			...
			if (c.items)
				for (var j = c.items.length - 1; j >= 0; j--) {
					if(c.items.itemAt(j).id == id)
					{
						c.items.itemAt(j).setVisible(false);
	 ...
	 * 关闭所有Portlet
	 */
	removeAllPortlets : function() {debugger
		for (var i = 0; i < this.items.length; i++) {
			var c = this.items.itemAt(i);
			if (c.items)
				for (var j = c.items.length - 1; j >= 0; j--) {
