	...
	$(function () {
		...
		$('#checkTable').datagrid({
			...
			onLoadSuccess:function(data){
				$('#checkTable').parent().find("div.datagrid-header-check").children("input[type=\"checkbox\"]").eq(0).attr("style", "display:none;");
				if(data){
					$.each(data.rows, function(index, item){
						if(item.CheckResult == "1"){
							$('#checkTable').datagrid('checkRow', index);
						}
						else{
							if(item.IsNeeded == "Y"){
								$('#checkCommitBtn').hide();
			...
			onExpandRow:function(index,row){
				...
				detailTable.datagrid({
					...
					onResize:function(){
						$('#checkTable').datagrid('fixDetailRowHeight',index);
					},
					rowStyler: function(index,row){
						if (row.CheckResult == "1"){
		...
		//提交
		$('#checkCommitBtn').on('click', function () {
