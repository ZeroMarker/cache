    ...
    $.ajax({ 
        ...
        success: function (data) { 
            if (data == "1") 
            {
                ...
                var titlediv = "<img class='favIcon icon-star-light-yellow' onclick='doFavorite("+'"'+ instanceId +'","'+episodeId+'","'+favInfoId +'","'+pluginType+'","'+chartItemType+'","'+emrDocId+'","'+episodeDate+'","'+episodeType+'","'+episodeLoc+'"'+")' title='添加收藏'/>";
    ...
    $.ajax({ 
        ...
        success: function (data) { 
            if (data == "1") 
            {
                ...
                $(tmpArray).find(".titlediv>img").remove();
                var titlediv = "<img class='favIcon icon-star-yellow' onclick='cancelFavRecrod("+'"'+ instanceId +'","'+episodeId+'","'+favInfoId +'","'+pluginType+'","'+chartItemType+'","'+emrDocId+'","'+episodeDate+'","'+episodeType+'","'+episodeLoc+'"'+")' title='取消收藏'/>";
                $(tmpArray).find(".titlediv").append(titlediv);
                
                var datediv = "<img class='favIcon icon-add-note' onclick='remarkingFav("+'"'+ instanceId +'","'+ favInfoId +'","'+titleText+'","'+episodeId+'","'+pluginType+'","'+chartItemType+'","'+emrDocId+'"'+")' title='病历评价'/>";
...
function setRecords(data,type,favInfoId,episodeId,episodeDate,episodeType,episodeLoc)
{
    ...
    for (var i=0;i<data.length;i++)
    {
        ...
        {
            titlediv = titlediv + "<img class='favIcon icon-star-yellow' onclick='cancelFavRecrod("+'"'+ data[i].id +'","'+episodeId+'","'+favInfoId +'","'+data[i].documentType+'","'+data[i].chartItemType+'","'+data[i].emrDocId+'","'+episodeDate+'","'+episodeType+'","'+episodeLoc+'"'+")' title='取消收藏'/>";
            
            datediv = datediv + "<img class='favIcon icon-add-note' onclick='remarkingFav("+'"'+ data[i].id +'","'+ favInfoId +'","'+data[i].text+'","'+episodeId+'","'+data[i].documentType+'","'+data[i].chartItemType+'","'+data[i].emrDocId+'"'+")' title='病历评价'/>";
        } 
        else
        {
            titlediv = titlediv + "<img class='favIcon icon-star-light-yellow' onclick='doFavorite("+'"'+ data[i].id +'","'+episodeId+'","'+favInfoId+'","'+data[i].documentType+'","'+data[i].chartItemType+'","'+data[i].emrDocId+'","'+episodeDate+'","'+episodeType+'","'+episodeLoc+'"'+")' title='添加收藏'/>";
        }
