...
(function($) {
	$(function() {
		...
		//授权列表
		$HUI.datagrid('#authorizationListTable',{
			fit: true,
			border: false,
			...
			queryParams: {
				...
				QueryName: 'GetApplyListByStatus',
				AStatus: 'N',
				AStartDate: '',
				AEndDate: ''
			},
			...
			onExpandRow: function(index,row) {
				...
				$('#ddv-' + index).datagrid({
					...
					queryParams: {
						...
						AAuthorizationID: row.AuthorizationID
					},
					rownumbers: true,
					singleSelect: true,
					columns:[[
		...
		function refreshGrid() {
			...
			queryParams.AStatus = FSAuthorization.typeCode;
			queryParams.AStartDate = FSAuthorization.dateStart;
			queryParams.AEndDate = FSAuthorization.dateEnd;
		...
		$('#assignRoleBtn').on('click', function() {
			...
			if (defaultRoleID == '') {
				...
				$.messager.popover({msg:'请先选择一个授权！',type:'alert',timeout:1000});
				return;
			}
			else {
