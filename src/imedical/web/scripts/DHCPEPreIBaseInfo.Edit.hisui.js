...
//名称  DHCPEPreIBaseInfo.Edit.hisui.js
//功能	个人基本信息维护
//创建	2020.03.16
//创建人  xy
...
$("#CardNo").keydown(function(e) {
	   if (e.keyCode == "13") {
			$(this).unbind("change");//取消绑定元素指定事件的具体函数
			//CardNo_Change();
			var ClearFlag=0
		...
		if (myary[0]=='0'){
			...
			if(myary[0]=='-340'){
			$.messager.alert('提示',"此卡没有对应的病人信息!","info");
			return false;
		}else if(myary[0]=='-350'){
				$.messager.alert('提示',"此卡已经使用,不能重复发卡!","info");
			return false;
		}else if(myary[0]=='-351'){
			$.messager.alert('提示',"此卡已经被挂失,不能使用","info");
			return false;
		}else if(myary[0]=='-352'){
			$.messager.alert('提示',"此卡已经被作废,不能使用!","info");
			return false;
		}else{
			$.messager.alert('提示',"Error Code:" +myary[0],"info");
			return false;
		}
		if (myary[0]!='0'){
...
function CardNo_Change()
{
	...
	CardNoChangeAppHISUI("PAPMINo","CardNo","RegNoChange()","Clear_click()","0");
...
function CardNoChangeAppHISUI(RegNoElement,CardElement,AppFunction,AppFunctionClear,ClearFlag)
{
	...
	if (myary[0]=='0'){
		$("#Update").linkbutton('enable');
		var CardNo=CardNo+"$"+SelectCardTypeRowID;	
...
function CardNoChangeAppHISUI(RegNoElement,CardElement,AppFunction,AppFunctionClear,ClearFlag)
{
	...
	if (myary[0]=='0'){
		...
		case "-352":
			...
			break;
		case "-356":
			$.messager.alert('提示',"发卡时,配置要求新增卡记录,但是此卡数据被预先生成错误!","info");
			break;
		case "-357":
			$.messager.alert('提示',"发卡时,配置要求更新卡记录,但是此卡数据没有预先生成!","info");
			break;
		case "-358":
			$.messager.alert('提示',"发卡时,此卡已经有对应的登记号了,不能再新增!","info");
			break;
		default:
			$.messager.alert('提示',"Error Code:" +myary[0],"info");
			break;
