...
function InitMonitorViewport(){
	...
	obj.cboLoc = new Ext.form.ComboBox({
		//id : 'cboLoc',  modify by liyi form.js中有个同id的ComboBox
		width : 100
	...
	obj.ResultGridPanelStore = new Ext.data.Store({
		...
		[
			...
			,{name: 'CostRatio', mapping: 'CostRatio'}
			,{name: 'DaysRatio', mapping: 'DaysRatio'}
			,{name: 'CostFlag', mapping: 'CostFlag'}
			,{name: 'DaysFlag', mapping: 'DaysFlag'}
		])
	});
	...
	obj.ResultGridPanel = new Ext.grid.GridPanel({
		...
		,columns: [
			...
			,{header: '费用增减率', width: 60, dataIndex: 'CostRatio', sortable: true}    //增加费用增减率
			,{header: '费用增减', width: 60, dataIndex: 'CostFlag', sortable: true}       //去掉颜色表示,采用符号标识
			...
			,{header: '天数增减率', width: 60, dataIndex: 'DaysRatio', sortable: true}    //增加天数增减率
			,{header: '天数增减', width: 60, dataIndex: 'DaysFlag', sortable: true}       //去掉颜色表示,采用符号标识
			,{header: '变异次数', width: 60, dataIndex: 'VarCount', sortable: false}
			,{header: '出径原因', width: 150, dataIndex: 'OutReasonDesc', sortable: true}
			/*,{
				header: '出径原因'
				,width: 150
				,renderer : function(v, m, rd, r, c, s){
	...
	obj.ResultGridPanelStoreProxy.on('beforeload', function(objProxy, param){
			...
			if (QueryType=='ConditionTab1') {
				param.QueryName = 'QryCPWByInDate';
			}
			if (QueryType=='ConditionTab2') {
				param.QueryName = 'QryCPWByOutDate';
			}
			if (QueryType=='ConditionTab3') {
				param.QueryName = 'QryCPWByAdmDate';
			}
