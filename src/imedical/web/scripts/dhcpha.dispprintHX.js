...
function PrintPJ(phac,phatype)
{
    ...
    xlsheet.Cells(row,11).value=t['REC_USER']
            ...
            if((tmpno!=prescno)&&(tmpid!=relateid))
            {
	           ...
	           if (priority!="出院带药")
	           {
	              ...
	              for (var n=1;n<=parseInt(facotor)*2;n++)
	              {
        		     ...
        		     xlsheet.Cells(startrow+7, startcol1).Value = "日期"+" "+orddate    //     getRelaDate2(orddate,h)
          		     //if ((n%2==1)&&(n!=1)) {h=h+1}
               ...
               if (priority=="出院带药"){var nextrow=startrow+4}
            }
	    }
