	...
	if (obj)
	{
		if (value==true)
		{
			...
			*/
			{
				obj.disabled=value;
				obj.className="EQDisabledBUTTON";
...
function SetCElement(ename,evalue)
{
	var obj;
	obj=document.getElementById(ename);
	//begin modify by jyp 20170407
	if(obj)           //modify by jyp 20170411
	{
	    for(i=0;i<obj.childNodes.length;i++)
	    {
		    if(obj.childNodes[i].nodeType==3)
		    {
			    obj.childNodes[i].nodeValue=evalue;
		    }
	    }
    }
	//end modify by jyp 20170407
}
...
function KeyUp(Value,showmsg)
{
	...
	for (i=0;i<value.length;i++)
	{
		...
		if (obj) {obj.onchange=Standard_KeyUp;}
			...
			if (showmsg!="N") alertShow(value[i])}
	}
}
...
function GetEquipOBJ(rowid,methodElement)
{
	...
	///alertShow(this.MaintUserDR+" "+this.MaintUser);
}
...
function SetItemStyle(ElementType,ClassName)
{
	...
	for(var I = 0; I < Length; I++)
	{
		//alertShow(All[I].style.fontSize);
...
///Date:2009-08-14
///根据元素名字设置元素值
///modified by czf 增加原值保存,保存至Hold2中
function GetModelRowID(type,i)
{
	var ModelName="Model";
	var ModelDRName="ModelDR"
	var ItemDRName="ItemDR"
	var OriginalFee="OriginalFee"
	if ((i)&&(i!=""))
	{
		ModelName="T"+ModelName+"z"+i;
		ModelDRName="T"+ModelDRName+"z"+i;
		ItemDRName="T"+ItemDRName+"z"+i;
		OriginalFee="TPriceFee"+"z"+i;
	}
	//add by kdf 2017-11-03 begin
	//解决 点击更新 规格型号不能正常保存的问题
	if ((GetElementValue(ModelDRName)!="")&&((type==0)||(type=="")))    
	{
		return GetElementValue(ModelDRName)
	}
	//add by kdf 2017-11-03 end						
	...
	var val=Model+"^"+OriginalFee;
	var rtn=cspRunServerMethod(encmeth,val,ItemDR);
	return rtn
}
...
{	
	...
	for(var I = 0; I < Length; I++)
	{
		if (All[I].disabled)
		{
			//alertShow(All[I].tagName);
			//alertShow(All[I].type);
...
function GetTableCurRow()
{
	...
	if (!eSrc) return "";
	//add begin by jyp 20170705  jyp0004
	if (eSrc.id!="")
	{
	...
	else
	{
		...
		else if (eSrc.tagName=="TD")
		{
			var TDesc=eSrc.childNodes[0].id.split("z");
		}
		else
		{
			var TDesc=eSrc.id.split("z");
	...
	//add end by jyp 20170629  jyp0004
	/*
	...
	//add begin by jyp 20170629
	else if((eSrc.tagName=="TD")&&(eSrc.id==""))
	{
		var TDesc=eSrc.childNodes[0].id.split("z");
	}
	//add end by jyp 20170629
	else
	{
		var TDesc=eSrc.id.split("z");		
	}
...
function GetColCaption(colName)
{
	...
	//alertShow(Caption+"&"+ComponentID+"&"+colName);
	return Caption;
}
...
function ReplaceTitle(FromStr,ToStr)
{
	...
	for(var K = 0; K < Length; K++)
	{
		...
		for(var I = 0; I < Length; I++)
		{
			///alertShow(head.children[I].innerText);
...
function ChineseNum(num) 
{
	...
	for (var i=0; i < num.length; i++)
	{
		...
		///alertShow(str+"&"+num.charAt(i)+"&"+unit.charAt(i))
	}
