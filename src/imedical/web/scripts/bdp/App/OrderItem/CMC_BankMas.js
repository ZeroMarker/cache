...
Ext.onReady(function() {
	...
	var ACTION_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.CT.CMCBankMas&pClassQuery=GetList";
	var SAVE_ACTION_URL_New = "../csp/dhc.bdp.ext.entitydatatrans.csp?pClassName=web.DHCBL.CT.CMCBankMas&pClassMethod=SaveEntity&pEntityName=web.Entity.CT.CMCBankMas";
	var DELETE_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.CMCBankMas&pClassMethod=DeleteData";
	var OPEN_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.CMCBankMas&pClassMethod=OpenData";	
  
     //初始化“别名”维护面板
     var AliasGrid = new Ext.BDP.grid.AliasEditorGridPanel({
      TableN : "CMC_BankMas"
  });
     Ext.BDP.FunLib.TableName="CMC_BankMas";
     /// 获取查询配置按钮 
	...
	/*****************排序*******************/
    Ext.BDP.FunLib.SortTableName = "User.CMCBankMas";
   ...
   btnhislog.on('click', function(btn,e){    
	   ...
	   if (grid.selModel.hasSelection()) {
	       ...
	       RowID=rows[0].get('CMCBMRowId');
	       Desc=rows[0].get('CMCBMDesc');
 ...
 /************************************ 删除功能*************************************************/
	var btnDel = new Ext.Toolbar.Button({
		...
		var records =  grid.selModel.getSelections();
				...
				if(recordsLen == 0){
					...
					if (btn == 'yes') {
                        AliasGrid.DataRefer=records[0].get('CMCBMRowId');
   ...
   var CodeText=new Ext.BDP.FunLib.Component.TextField({
       ...
       labelSeparator:"",
       allowBlank : false,
       readOnly : Ext.BDP.FunLib.Component.DisableFlag('CMCBMCodeF'),
       style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('CMCBMCodeF')),
       ...
       validator : function(thisText){
          ...
          var className =  "web.DHCBL.CT.CMCBankMas";   
          var classMethod = "FormValidate"; //数据重复验证后台函数名                             
          var id="";
          if(win.title=='修改'){  
            var _record = grid.getSelectionModel().getSelected();
            var id = _record.get('CMCBMRowId');  
          }
            var flag = "";
  ...
  var DescText=new Ext.BDP.FunLib.Component.TextField({
       ...
       name : 'CMCBMDesc',
       id:'CMCBMDescF',
       readOnly : Ext.BDP.FunLib.Component.DisableFlag('CMCBMDescF'),
       style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('CMCBMDescF')),
       allowBlank : false,
       enableKeyEvents : true,
       validationEvent : 'blur',
       validator : function(thisText){
       ...
       var className =  "web.DHCBL.CT.CMCBankMas"; //后台类名称
       var classMethod = "FormValidate"; //数据重复验证后台函数名
       var id="";
       if(win.title=='修改'){ 
          var _record = grid.getSelectionModel().getSelected();
  ...
  var DateFromText=new Ext.BDP.FunLib.Component.DateField({
       ...
       name :'CMCBMDateFrom',
       id:'CMCBMDateFromF',
       readOnly : Ext.BDP.FunLib.Component.DisableFlag('CMCBMDateFromF'),
       style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('CMCBMDateFromF')),
  ...
  var DateToText=new Ext.BDP.FunLib.Component.DateField({
       ...
       name : 'CMCBMDateTo',
       id:'CMCBMDateToF',
       readOnly : Ext.BDP.FunLib.Component.DisableFlag('CMCBMDateToF'),
       style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('CMCBMDateToF')),
       format : BDPDateFormat,
       enableKeyEvents : true,
       listeners : {   
        'keyup' : function(field, e)
...
/************************************增加修改时弹出窗口************************************/
	var win = new Ext.Window({
		...
		buttons : [{
			...
			handler : function() {
			var tempCode = Ext.getCmp("form-save").getForm().findField("CMCBMCode").getValue();
			var tempDesc = Ext.getCmp("form-save").getForm().findField("CMCBMDesc").getValue();
			var startDate = Ext.getCmp("form-save").getForm().findField("CMCBMDateFrom").getValue();
			var endDate = Ext.getCmp("form-save").getForm().findField("CMCBMDateTo").getValue();
			if (tempCode=="") {
				Ext.Msg.show({ 
								title : '提示', 
								msg : '代码不能为空!',
								...
								fn:function()
								{
									Ext.getCmp("form-save").getForm().findField("CMCBMCode").focus(true,true);
								}
							});
      			return;
			}
			if (tempDesc=="") {
				Ext.Msg.show({ 
								...
								fn:function()
								{
									Ext.getCmp("form-save").getForm().findField("CMCBMDesc").focus(true,true);
								}
							});
      				return;
			}
			if (startDate=="") {
				Ext.Msg.show({ 
								...
								fn:function()
								{
									Ext.getCmp("form-save").getForm().findField("CMCBMDateFrom").focus(true,true);
								}
							});
      			return;
			}
		...
		listeners : {
			"show" : function() {
					Ext.getCmp("form-save").getForm().findField("CMCBMCode").focus(true,300);
			},
			"hide" : function(){
...
/************************************create the Grid**********************************/ 
	...
	var GridCM=[new Ext.grid.CheckboxSelectionModel(), {
							...
							dataIndex :'CMCBMDateFrom'
						}, {
							header : '结束日期',
							width : 120,
							sortable : true,
							renderer : Ext.util.Format.dateRenderer(BDPDateFormat),
							dataIndex : 'CMCBMDateTo'
						}];
...
btnTrans.on("click",function(){
	...
	{
	  var _record=grid.getSelectionModel().getSelected();
      var selectrow=_record.get('CMCBMRowId');
	 }
	else
	{
	  var selectrow=""
	  ...
	  var loadFormData = function(grid){
	        ...
	        if (!_record) {
	            Ext.Msg.alert('修改操作', '请选择要修改的一项!');
	        		...
	        		WinForm.form.load( {
	                url : OPEN_ACTION_URL + '&id='+ _record.get('CMCBMRowId'),  //id 对应OPEN里的入参
	                success : function(form,action) {
                    },
	                failure : function(form,action) {
	                }
               ...
               tabs.setActiveTab(0);
               //加载别名面板
               AliasGrid.DataRefer = _record.get('CMCBMRowId');
               AliasGrid.loadGrid();
	        }
	    };
