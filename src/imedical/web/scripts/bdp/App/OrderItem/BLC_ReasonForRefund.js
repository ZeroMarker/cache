...
Ext.onReady(function() {
  ...
  ///   调用 退款原因维护后台类方法
	var ACTION_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.CT.BLCReasonForRefund&pClassQuery=GetList";
	var SAVE_ACTION_URL_New = "../csp/dhc.bdp.ext.entitydatatrans.csp?pClassName=web.DHCBL.CT.BLCReasonForRefund&pClassMethod=SaveEntity&pEntityName=web.Entity.CT.BLCReasonForRefund";
	var DELETE_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.BLCReasonForRefund&pClassMethod=DeleteData";
	var OPEN_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.BLCReasonForRefund&pClassMethod=OpenData";	
 
    /*****************排序*******************/
    Ext.BDP.FunLib.SortTableName = "User.BLCReasonForRefund";
    var btnSort = Ext.BDP.FunLib.SortBtn;
     //初始化“别名”维护面板
     var AliasGrid = new Ext.BDP.grid.AliasEditorGridPanel({
      TableN : "BLC_ReasonForRefund"
  });
     Ext.BDP.FunLib.TableName="BLC_ReasonForRefund";
     // 获取查询配置按钮 
    ...
    HospWinButton.on("click" , function(){
           if (grid.selModel.hasSelection()) { //选中一条数据数据时，弹出 数据与医院关联窗口
				var rowid=grid.getSelectionModel().getSelections()[0].get("RFRRowId");
   ...
   btnhislog.on('click', function(btn,e){    
	   ...
	   if (grid.selModel.hasSelection()) {
	       ...
	       RowID=rows[0].get('RFRRowId');
	       Desc=rows[0].get('RFRDesc');
	...
	var btnDel = new Ext.Toolbar.Button({
		...
		var records =  grid.selModel.getSelections();
				...
				if(recordsLen == 0){
					...
					if (btn == 'yes') {
                        AliasGrid.DataRefer=records[0].get('RFRRowId');
 ...
 /// RowId  RowIdText,CodeText,RFRDescText
 ...
 /// CODE
  var CodeText=new Ext.BDP.FunLib.Component.TextField({
        ...
        name :'RFRCode',
        id:'RFRCodeF',
        readOnly : Ext.BDP.FunLib.Component.DisableFlag('RFRCodeF'),
        style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('RFRCodeF')),
        ...
        validator : function(thisText){
         ...
         var className =  "web.DHCBL.CT.BLCReasonForRefund";   
         var classMethod = "FormValidate"; //数据重复验证后台函数名                             
         var id="";
         if(win.title=='修改'){  
            var _record = grid.getSelectionModel().getSelected();
            var id = _record.get('RFRRowId');  
          }
           var flag = "";
  ...
  var RFRDescText=new Ext.BDP.FunLib.Component.TextField({
         ...
         name : 'RFRDesc',
         id:'RFRDescF',
         readOnly : Ext.BDP.FunLib.Component.DisableFlag('RFRDescF'),
         style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('RFRDescF')),
         minLength :1 ,                 
         ...
         validator : function(thisText){
           ...
           var className =  "web.DHCBL.CT.BLCReasonForRefund"; //后台类名称
           var classMethod = "FormValidate"; //数据重复验证后台函数名
           var id="";
           if(win.title=='修改'){ 
             var _record = grid.getSelectionModel().getSelected();
...
/************************************增加修改的Form************************************/
	var WinForm = new Ext.form.FormPanel({
				...
				reader:JsonReaderE,
				items : [RowIdText,CodeText,RFRDescText]
			});
...
/************************************增加修改时弹出窗口***********************************/
	var win = new Ext.Window({
		...
		buttons : [{
			...
			handler : function() {
			var tempCode = Ext.getCmp("form-save").getForm().findField("RFRCode").getValue();
			var tempDesc = Ext.getCmp("form-save").getForm().findField("RFRDesc").getValue();
			...
			if (tempCode=="") {
				Ext.Msg.show({ 
								...
								fn:function()
								{
									Ext.getCmp("form-save").getForm().findField("RFRCode").focus(true,true);
								}
							});
      				return;
				}
			if (tempDesc=="") {
				Ext.Msg.show({ 
								...
								fn:function()
								{
									Ext.getCmp("form-save").getForm().findField("RFRDesc").focus(true,true);
								}
							 });
      					return;
					}
		...
		listeners : {
			"show" : function() {
				Ext.getCmp("form-save").getForm().findField("RFRCode").focus(true,300);
			},
			"hide" : function(){
...
/************************************create the Grid ************************************/
	var GridCM=[new Ext.grid.CheckboxSelectionModel(), 
		{header : 'RFRRowId',width : 160,sortable : true,dataIndex : 'RFRRowId',hidden : true},
				{header : '代码',width : 160,sortable : true,dataIndex : 'RFRCode'},
				 {header : '描述',width : 160,sortable : true,dataIndex : 'RFRDesc'}];
...
btnTrans.on("click",function(){
	...
	{
	  var _record=grid.getSelectionModel().getSelected();
      var selectrow=_record.get('RFRRowId');
	 }
	else
	{
	  var selectrow=""
	  ...
	  var loadFormData = function(grid){
	        ...
	        if (!_record) {
	            Ext.Msg.alert('修改操作', '请选择要修改的一项!');
	        		 ...
	        		 WinForm.form.load( {
	                 url : OPEN_ACTION_URL + '&id='+ _record.get('RFRRowId'),  //id 对应OPEN里的入参
	                 success : function(form,action) {
                    },
	                failure : function(form,action) {
	                }
               ...
               tabs.setActiveTab(0);
               //加载别名面板
               AliasGrid.DataRefer = _record.get('RFRRowId');
               AliasGrid.loadGrid();
	        }
	    };
