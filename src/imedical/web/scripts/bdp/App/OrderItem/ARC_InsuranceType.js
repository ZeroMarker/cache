 ...
 */
 ...
 /// 调用 别名维护 的后台类方法
    ...
    /// 调用付款人维护的后台类方法
	var ACTION_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.CT.ARCInsuranceType&pClassQuery=GetList";
    var SAVE_ACTION_URL = "../csp/dhc.bdp.ext.entitydatatrans.csp?pClassName=web.DHCBL.CT.ARCInsuranceType&pClassMethod=SaveEntity&pEntityName=web.Entity.CT.ARCInsuranceType";
	var DELETE_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.ARCInsuranceType&pClassMethod=DeleteData";
	var OPEN_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.ARCInsuranceType&pClassMethod=OpenData";	
    
      //初始化“别名”维护面板
     var AliasGrid = new Ext.BDP.grid.AliasEditorGridPanel({
      TableN : "ARC_InsuranceType"
   });
     Ext.BDP.FunLib.TableName="ARC_InsuranceType";
     /// 获取查询配置按钮 
	 ...
	 /*****************排序*******************/
    Ext.BDP.FunLib.SortTableName = "User.ARCInsuranceType";
   ...
   if (grid.selModel.hasSelection()) {
       ...
       RowID=rows[0].get('INSTRowId');
       Desc=rows[0].get('INSTDesc');
   ...
   //多院区医院下拉框
    ...
    HospWinButton.on("click" , function(){
           if (grid.selModel.hasSelection()) { //选中一条数据数据时，弹出 数据与医院关联窗口
				var rowid=grid.getSelectionModel().getSelections()[0].get("INSTRowId");
			  ...
			  var records =  grid.selModel.getSelections();
				...
				if(recordsLen == 0){
					...
					Ext.MessageBox.confirm('<font color=blue>提示</font>','<font color=red>确定要删除所选的数据吗?</font>', function(btn) {
                      if (btn == 'yes') { 
						AliasGrid.DataRefer=records[0].get('INSTRowId');
    ...
    /// RowId
     var RowIdText=new Ext.BDP.FunLib.Component.TextField({
        fieldLabel : 'INSTRowId',
        hideLabel : 'True',
        hidden : true,
        name : 'INSTRowId'
     });
    // code RowIdText,CodeText,DescText
    var CodeText=new Ext.BDP.FunLib.Component.TextField({
        ...
        regexText:"代码不能为空",
        id:'INSTCodeF',
        readOnly : Ext.BDP.FunLib.Component.DisableFlag('INSTCodeF'),
        style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('INSTCodeF')),
        ...
        validator : function(thisText){
           if(thisText==""){  //当文本框里的内容为空的时候不用此验证
            ...
            var className =  "web.DHCBL.CT.ARCInsuranceType";   
            var classMethod = "FormValidate"; //数据重复验证后台函数名                             
            var id="";
            if(win.title=='修改'){  
               var _record = grid.getSelectionModel().getSelected();
               var id = _record.get('INSTRowId');  
            }
           var flag = "";
    ...
    var DescText=new Ext.BDP.FunLib.Component.TextField({
       ...
       regexText:"描述不能为空",
       id:'INSTDescF',
       readOnly : Ext.BDP.FunLib.Component.DisableFlag('INSTDescF'),
       style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('INSTDescF')),
       minLength :1 ,                 
       minLengthText : "描述最少1个字符",                  
       maxLength : 30 ,                 
       maxLengthText :"描述至多30个字符",                
       enableKeyEvents : true,
       validationEvent : 'blur',
       validator : function(thisText){
         if(thisText==""){  
         ...
         var className =  "web.DHCBL.CT.ARCInsuranceType"; //后台类名称
         var classMethod = "FormValidate"; //数据重复验证后台函数名
         var id="";
         if(win.title=='修改'){ 
           var _record = grid.getSelectionModel().getSelected();
   ...
   var DateFromText=new Ext.BDP.FunLib.Component.DateField({
       ...
       id:'INSTDateFromF',
       readOnly : Ext.BDP.FunLib.Component.DisableFlag('INSTDateFromF'),
       style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('INSTDateFromF')),
       allowBlank:false,
       regexText:"开始日期不能为空",
       editable:true,
       enableKeyEvents : true,
   ...
   var DateToText=new Ext.BDP.FunLib.Component.DateField({
       ...
       name : 'INSTDateTo',
       id:'INSTDateToF',
       readOnly : Ext.BDP.FunLib.Component.DisableFlag('INSTDateToF'),
       style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('INSTDateToF')),
       format : BDPDateFormat,
       enableKeyEvents : true,
       listeners : {   
        'keyup' : function(field, e)
 ...
 Ext.BDP.AddTabpanelFun(tabs,Ext.BDP.FunLib.TableName);
	...
	var win = new Ext.Window({
		...
		buttons : [{
			...
			handler : function() {
			var tempCode = Ext.getCmp("form-save").getForm().findField("INSTCode").getValue();
			var tempDesc = Ext.getCmp("form-save").getForm().findField("INSTDesc").getValue();
			var startDate = Ext.getCmp("form-save").getForm().findField("INSTDateFrom").getValue();
			var endDate = Ext.getCmp("form-save").getForm().findField("INSTDateTo").getValue();
			if (tempCode=="") {
				Ext.Msg.show({ 
								title : '<font color=blue>提示</font>',
								msg : '代码不能为空!', 
								...
								fn:function()
								{
									Ext.getCmp("form-save").getForm().findField("INSTCode").focus(true,true);
								}
							});
      				return;
				}
			if (tempDesc=="") {
				Ext.Msg.show({ 
								...
								fn:function()
								{
									Ext.getCmp("form-save").getForm().findField("INSTDesc").focus(true,true);
								}
							 });
      					return;
					}
			if (startDate=="") {
				Ext.Msg.show({ 
								...
								fn:function()
								{
									Ext.getCmp("form-save").getForm().findField("INSTDateFrom").focus(true,true);
								}
							 });
      				return;
			}
		...
		listeners : {
			"show" : function() {
               Ext.getCmp("form-save").getForm().findField("INSTCode").focus(true,300);
			},
			"hide" : function(){
	...
	var GridCM= [new Ext.grid.CheckboxSelectionModel(), {
				...
				dataIndex : 'INSTDateFrom'
			}, {
				header : '结束日期',
				width : 85,
				sortable : true,
				renderer : Ext.util.Format.dateRenderer(BDPDateFormat),
				dataIndex : 'INSTDateTo'
			}];
...
btnTrans.on("click",function(){
	...
	{
	  var _record=grid.getSelectionModel().getSelected();
      var selectrow=_record.get('INSTRowId');
	 }
	else
	{
	  var selectrow=""
	  ...
	  var loadFormData = function(grid){
	        ...
	        if (!_record) {
	            Ext.Msg.alert('修改操作', '请选择要修改的一项!');
	        		...
	        		WinForm.form.load( {
	                url : OPEN_ACTION_URL + '&id='+ _record.get('INSTRowId'),  //id 对应OPEN里的入参
	                success : function(form,action) {
                       },
	                failure : function(form,action) {
	                }
	              ...
	              //激活基本信息面板
	               tabs.setActiveTab(0);
	               //加载别名面板
	               AliasGrid.DataRefer = _record.get('INSTRowId');
	               AliasGrid.loadGrid();
	        }
	    };
