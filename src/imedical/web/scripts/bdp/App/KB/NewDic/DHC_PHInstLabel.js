...
* @Last Modified time: 2018-11-05 10:55:56
* @描述:知识库目录字典
*/
...
var init = function(){
 var columns =[[
        ...
        {field:'PINLGenFlag',title:'在药品通用名目录显示',sortable:true,width:100,
              formatter:ReturnFlagIcon
    	},
        {field:'PINLProFlag',title:'在药品商品名目录显示',sortable:true,width:100,
              formatter:ReturnFlagIcon
    	},    	    	
    ...
    SearchFunLib=function()
    {
    	...
    	var knoflag=$('#TextLib').combobox('getValue');
    ...
    {
    	//singleselect=true;
    	//$('#PINLCode').attr("disabled",false);//代码不可修改
		$('#PINLCode').attr("readonly",false);
        $('#PINLCode')[0].readonly=false;
        $('#PINLCode').css({'background-color':'#ffffff'});
 		$("#myWin").show();
		...
		$HUI.checkbox("#PINLGenFlag").setValue(true);
		$HUI.checkbox("#PINLProFlag").setValue(true);	
    } 
    updateData=function()
	{
		...
		if(record)
		{
			...
			$.cm({
				...
				if (jsonData.PINLGenFlag=="Y"){
					$HUI.checkbox("#PINLGenFlag").setValue(true);		
				}else{
					$HUI.checkbox("#PINLGenFlag").setValue(false);
				}
				if (jsonData.PINLProFlag=="Y"){
					$HUI.checkbox("#PINLProFlag").setValue(true);		
				}else{
					$HUI.checkbox("#PINLProFlag").setValue(false);
				}								
				$('#form-save').form("load",jsonData);	
			});	
			//$('#PINLCode').attr("disabled",true);//代码不可修改
			$('#PINLCode').attr("readonly",true);
	        $('#PINLCode')[0].readonly=true;
	        $('#PINLCode').css({'background-color':'#EBEBE4'});
			$("#myWin").show();
			var myWin = $HUI.dialog("#myWin",{
				iconCls:'icon-w-edit',
				resizable:true,
