...
var init=function()
{
   var columns =[[
    ...
    $('#BatchContrast').click(function(e){
        var result= tkMakeServerCall("web.DHCBL.KB.DHCGenItmContrast","AutoContrastCheckValues");
    ...
    var hisgengrid = $HUI.datagrid("#hisgengrid",{
        ...
        queryParams:{
            ClassName:"web.DHCBL.KB.DHCGenItmContrast",
            QueryName:"GetDataForCheckValues"
        },
    ...
    conMethod=function()
    {
 		setTimeout(function(){
			...
			if(!record)
			{
				$.messager.alert('错误提示','请选择需要对照的检查测值项目！',"error");
            	return;
			}
			var ids=record.PHEGRowId+'^'+hisRecord.RowId+'^'+hisRecord.Code+'^'+hisRecord.Desc;
			var data=tkMakeServerCall("web.DHCBL.KB.DHCGenItmContrast","SaveCheckValues",ids);
			var data=eval('('+data+')');
