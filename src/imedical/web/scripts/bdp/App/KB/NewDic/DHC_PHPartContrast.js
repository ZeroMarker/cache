...
* @Last Modified time: 2018-10-24 09:41:33
* @描述:检查部位与his部位对照
*/
var DELETE_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.KB.DHCPHPartContrast&pClassMethod=DeleteData";
var init=function()
{
   var columns =[[
        {field:'PHEPCode',title:'代码',sortable:true,width:100},
        {field:'PHEPDesc',title:'描述',sortable:true,width:100},
        {field:'PHEPAcitveFlag',title:'是否可用',sortable:true,width:100,
              formatter:ReturnFlagIcon
    	},
        {field:'PHEPSysFlag',title:'系统标识',sortable:true,width:100,
              formatter:ReturnFlagIcon
    	},
        {field:'PHEPRowId',title:'rowid',sortable:true,width:100,hidden:true}
    ]];
    var partgrid = $HUI.datagrid("#partgrid",{
        url:$URL,
        queryParams:{
            ClassName:"web.DHCBL.KB.DHCPHExtPart",
            QueryName:"GetList"
        },
        columns: columns,  //列信息
        ...
        onClickRow:function(index,row)
        {
	        $('#contrastgrid').datagrid('reload',  {
            	ClassName:"web.DHCBL.KB.DHCPHPartContrast",
            	QueryName:"GetList",
            	id:row.PHEPRowId
    		});        	
        }		
    ...
    $('#BatchContrast').click(function(e){
        var result= tkMakeServerCall("web.DHCBL.KB.DHCPHPartContrast","AutoContrastData");
    ...
    var hiscolumns =[[
	  ...
	  {field:'Code',title:'代码',width:300,sortable:true},
    ...
    conMethod=function()
    {
 		setTimeout(function(){
			...
			if(!record)
			{
				$.messager.alert('错误提示','请选择需要对照的部位！',"error");
            	return;
			}
			var ids=record.PHEPRowId+'^'+hisRecord.RowId;
			var data=tkMakeServerCall("web.DHCBL.KB.DHCPHPartContrast","SaveData",ids);
			var data=eval('('+data+')');
    ...
    //对照表
    var columns =[[
        {field:'PHEPRowId',title:'rowid',width:100,hidden:true,sortable:true},
        {field:'PHEPPartCode',title:'代码',width:100,sortable:true},
        {field:'PHEPPartDesc',title:'描述',width:100,sortable:true},
        {field:'PHEPHisPartCode',title:'his代码',width:100,sortable:true},
        {field:'PHEPHisPartDesc',title:'his描述',width:100,sortable:true},
        {field:'opt',title:'操作',width:50,align:'center',
			formatter:function(){ 
	...
	delContrast=function()
	{
		setTimeout(function(){
			...
			if(record)
			{
				/*tkMakeServerCall("web.DHCBL.KB.DHCPHPartContrast","DeleteData",record.PHEPRowId);
				var recordMKB=$('#partgrid').datagrid('getSelected');
				$('#contrastgrid').datagrid('load',  {
	            	ClassName:"web.DHCBL.KB.DHCPHPartContrast",
	            	QueryName:"GetList",
	            	id:recordMKB.PHEPRowId
        		});*/
        		var partrecord=$('#partgrid').datagrid('getSelected');
      			var rowid=record.PHEPRowId;
