...
var init = function(){
	...
	{		
		...
		{
			$.messager.alert('错误提示','检查项目不能为空，请选择下拉列表里的值!',"error");
			return;
		}
		if (($('#GlPPointerF').combobox('getValue')==undefined)||($('#GlPPointerF').combobox('getValue')==""))
		{
			$.messager.alert('错误提示','检查部位不能为空，请选择下拉列表里的值!',"error");
			return;
		}
	...
	var columns =[[  
				  ...
				  {field:'GlPGenDr',title:'检查项目ID',width:150,sortable:true,hidden:true},
				  {field:'PHEPDesc',title:'检查部位',width:150,sortable:true},
				  {field:'GlPPointer',title:'检查部位ID',width:150,sortable:true,hidden:true},
