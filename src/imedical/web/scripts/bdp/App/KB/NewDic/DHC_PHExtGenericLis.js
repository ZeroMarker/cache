...
var init = function(){
	...
	//检验标本查询下拉框
	$('#GlPPointer').combobox({ 
		url:$URL+"?ClassName=web.DHCBL.KB.DHCPHExtGeneric&QueryName=GetDataForCmb1&ResultSetType=array&code=SPEC",
		valueField:'PHEGRowId',
		textField:'PHEGDesc'
	});
	 //查询方法
	SearchFunLib=function (){
		var pointer=$.trim($('#GlPPointer').combobox('getText'));
		$('#mygrid').datagrid('load',  { 
			ClassName:"web.DHCBL.KB.DHCGenLinkPointer",
			QueryName:"GetNewLisList",
			'gen':selectrow,
			'pointer':pointer
		});
	...
	{		
		...
		{
			$.messager.alert('错误提示','检验标本不能为空!',"error");
			return;
		}
	...
	*/
	...
	var columns =[[  
				  ...
				  {field:'PHEGDesc',title:'检验通用名',width:150,sortable:true,
				  		formatter:function(value,row,index)
				  		{
				  			return selectrowDesc;
				  		}
				  },
				  {field:'GIPGenDr',title:'通用名ID',width:150,sortable:true,editor:'validatebox',hidden:true},
				  {field:'GlPPointer',title:'检验标本',width:150,sortable:true,editor:{
					  	type:'combobox',
					  	options:{
							url:$URL+"?ClassName=web.DHCBL.KB.DHCPHExtGeneric&QueryName=GetDataForCmb1&ResultSetType=array&code=SPEC",
							//multiple:true,
				  ...
				  {field:'GlPPointerF',title:'检验标本ID',width:150,sortable:true,hidden:true,editor:'validatebox'},
				  {field:'GlPActiveFlagF',title:'是否可用',width:150,sortable:true,align:'center',editor:'validatebox',hidden:true},
				  ...
				  {field:'GlPSysFlagF',title:'是否系统标识',width:150,sortable:true,align:'center',editor:'validatebox',hidden:true},
   ...
   $('#add_btn').click(function(e){
	...
	function DblClickFun (index,row,field){   //双击激活可编辑   （可精简）
		row.GlPRowId=row.GlPRowId.replace(/,/g, "")    //row中的GlPRowId会在数字后面自动加逗号
		if(index==editIndex){
			return
		}
