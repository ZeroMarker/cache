...
var init = function(){
	...
	//检查部位查询下拉框
	$('#GlPPointer').combobox({ 
		url:$URL+"?ClassName=web.DHCBL.KB.DHCPHExtPart&QueryName=GetDataForCmb1&ResultSetType=array",
		valueField:'PHEPRowId',
		textField:'PHEPDesc'
	});
	 //查询方法
	...
	{		
		...
		{
			$.messager.alert('错误提示','检查部位不能为空!',"error");
			return;
		}
	...
	*/
	...
	var columns =[[  
				  {field:'GlPRowId',title:'GlPRowId',width:80,sortable:true,hidden:true,editor:'validatebox'},
				  {field:'PHEGDesc',title:'检查项目',width:150,sortable:true,
				  		formatter:function(value,row,index)
				  		{
				  			return selectrowDesc;
				  		}
				  },
				  {field:'GlPGenDr',title:'检查项目ID',width:150,sortable:true,hidden:true,editor:'validatebox'},
				  {field:'GlPPointer',title:'检查部位',width:150,sortable:true,editor:{
					  	type:'combobox',
					  	options:{
							url:$URL+"?ClassName=web.DHCBL.KB.DHCPHExtPart&QueryName=GetDataForCmb1&ResultSetType=array",
							//multiple:true,
				  ...
				  {field:'GlPPointerF',title:'检查部位F',width:150,sortable:true,hidden:true,editor:'validatebox'},
				  ...
				  {field:'GlPActiveFlagF',title:'是否可用F',width:150,sortable:true,align:'center',hidden:true,editor:'validatebox'},
				  ...
				  {field:'GlPSysFlagF',title:'是否系统标识F',width:150,sortable:true,align:'center',hidden:true,editor:'validatebox'}
	 ]];
   ...
   $('#add_btn').click(function(e){
		...
		if (endEditing()){
			if(rowsvalue!= undefined){
				if((rowsvalue.GlPPointer!="")){
					...
					$.messager.alert('错误提示','部位不能为空！','error')
	...
	function AddData(){
		...
		//ClickFun('AddData')
		if (endEditing()){
			$('#mygrid').datagrid('insertRow',{index:0,row:{GlPGenDr:selectrow,GlPActiveFlag:'Y'}});
