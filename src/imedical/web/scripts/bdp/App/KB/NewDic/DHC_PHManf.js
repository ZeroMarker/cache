...
* @Last Modified time: 2018-11-07 15:42:17
* @描述:厂家字典
*/
...
{
    var columns =[[
        {field:'PHMACode',title:'代码',sortable:true,width:100},
        {field:'PHMADesc',title:'描述',sortable:true,width:100},
        {field:'PHMAActiveFlag',title:'是否可用',sortable:true,width:100,
              formatter:ReturnFlagIcon
    	},
        {field:'PHMARowId',title:'rowid',sortable:true,width:100,hidden:true}
    ]];
    var mygrid = $HUI.datagrid("#mygrid",{
        url:$URL,
        queryParams:{
            ClassName:"web.DHCBL.KB.DHCPHManf",
            QueryName:"GetList"
        },
        columns: columns,  //列信息
    ...
    AddData=function(){ 
		...
		//默认选中
		$HUI.checkbox("#PHMAActiveFlag").setValue(true);
	...
	SaveFunLib=function(id,flagT)
	{
		...
		//alert(flag)
		var code=$.trim($("#PHMACode").val());
		var desc=$.trim($("#PHMADesc").val());	
		if (code=="")
		{
		...
		var active="N";
		if ($('#PHMAActiveFlag').attr('checked')) 
		{
			var active="Y" ;
		}	
		...
		success: function (data) { 
		  	...
		  	if (data.success == 'true') {
				...
				for(i=0;i<otherdata.rows.length;i++)
				{
					...
					var dataRow=otherdata.rows[i].AliasRowId+"^"+"DHC_PHManf"+"^"+otherdata.rows[i].DataAlias+"^"+DataRefer;
					if(dataforSave=="")
					{
						dataforSave=dataRow;
					}
	...
	updateData=function()
	{
		...
		if(record)
		{
			...
			$.cm({
				...
				if (jsonData.PHMAActiveFlag=="Y"){
					$HUI.checkbox("#PHMAActiveFlag").setValue(true);		
				}else{
					$HUI.checkbox("#PHMAActiveFlag").setValue(false);
				}
			...
			if (r){
				$.ajax({
					...
					success: function(data){
							  var data=eval('('+data+')'); 
							  if (data.success == 'true') {
							  	var datad=tkMakeServerCall("web.DHCBL.BDP.BDPAlias","DeleteAll","DHC_PHManf",data.id);
								/*$.messager.show({ 
								  title: '提示消息', 
								  msg: '删除成功', 
								  showType: 'show', 
			...
			if(rowsvalue!= undefined){
				if((rowsvalue.DataAlias!="")){
				    ...
				    for (var i = 0; i < rows.length; i++) {  
				        ...
				        if (valuerow == rowsvalue.DataAlias && i != editIndex)
				        {
