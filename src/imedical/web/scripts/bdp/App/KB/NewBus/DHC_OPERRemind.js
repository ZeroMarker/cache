...
function InitMyList()
{
	...
	$('#mygrid').datagrid({ 
		...
		url:$URL,
		queryParams:{
			ClassName:"web.DHCBL.KB.DHCOPERRemind",
			QueryName:"GetList",
			TypeDr : "",
			GenDr: GenDr,
			PointerType:PointerType,
...
function InitPopuList()
{
	...
	var Popucolumns=[[
				  ...
				  {field:'SPEDesc',title:'描述',width:220,sortable:true},
				  {field:'SPECode',title:'代码',width:200,sortable:true}, 
...
function InitUnPopuList()
{
	...
	var unPopuColumns=[[ 
				  ...
				  {field:'SPEDesc',title:'特殊人群描述',width:250,sortable:true}, 
...
function RemovePopu(index)
{
	...
	if (record){
		...
		if(SPEPIRowId==""){  //如果是新加的数据，还没保存，则只是移除
			...
			if($('#unPopuWindow').is(":visible"))  //如果新增检查窗口打开了，则点删除后刷新未选检查列表
			{
				PopuReset();
			}			
		}else{   //如果是已保存的数据直接执行删除操作
