...
var init = function(){
	...
	var preeditIndex="";
	var columns =[[  
	  {field:'PHEINDescF',title:'描述多选框F',width:200,hidden:true,editor:'validatebox'},
	  {field:'PHEINDesc',title:'描述',width:200,sortable:true,editor:{
		  	type:'combobox',
		  	options:{
				...
				onHidePanel:function(){
		        	...
		        	var ed = $('#mygrid').datagrid('getEditor', {index:editIndex,field:'PHEINDescF'});
					$(ed.target).val(val)
		        }
			}
		}},
	  ...
	  {field:'PDIExcludeFlagF',title:'禁忌下拉框F',width:100,hidden:true,editor:'validatebox'},
	  {field:'PDIExcludeFlag',title:'禁忌',width:100,sortable:true,align:'center',formatter:ReturnFlagIcon,editor:{
		  	type:'combobox',
			options:{
				panelHeight: 200,
				valueField:'id',
	            ...
	            onHidePanel:function(){
		        	var val=$(this).combobox('getText');
		        	var ed = $('#mygrid').datagrid('getEditor', {index:editIndex,field:'PDIExcludeFlagF'});
					$(ed.target).val(val)
		        }	
				
			}
		}},
	  {field:'PDIAlertMsg',title:'提示消息',width:200,
		  	formatter:function(value,row,index){
			  	return '<span class="mytooltip" title="'+value+'">'+value+'</span>'
			},
		  	editor:{type:'textarea'}
		  },
	{field:'PDIRowId',title:'PDIRowId',width:80,sortable:true,hidden:true,editor:'validatebox'},
	{field:'PDIInstDr',title:'PDIInstDr',width:80,sortable:true,hidden:true,editor:'validatebox'},
	{field:'PDIUseDR',title:'PDIUseDR',width:80,sortable:true,hidden:true,editor:'validatebox'}
	 ]];
	...
	var mygrid = $HUI.datagrid("#mygrid",{
		url:$URL,
		queryParams:{
			ClassName:"web.DHCBL.KB.DHCPHDiseaseUse",         ///调用Query时
	...
	function UpdataRow(row,Index){
		...
		temp=row.PDIExcludeFlag
		row.PDIExcludeFlag=row.PDIExcludeFlagF
		row.PDIExcludeFlagF=temp
	...
	function DblClickFun (index,row,field){   //双击激活可编辑   （可精简）
		...
		if((row!=undefined)&&(row.PDIRowId!=undefined)){
			UpdataRow(row,index)
		}
		preeditIndex=editIndex
		if (editIndex != index){
	...
	function DelData(){
		...
		if((record.PDIRowId==undefined)||(record.PDIRowId=="")){
	...
	function SaveData (record,type){
		...
		var ExcludeFlag=record.PDIExcludeFlag
		if (ExcludeFlag!="Y")
		{
			ExcludeFlag="N"
		}
		...
		var AlertMsg=$.trim(record.PDIAlertMsg);
		var InstDr=$.trim(record.PDIInstDr);
		
		/*if (InstDr!=""){
			var saveURL="../csp/dhc.bdp.ext.entitydatatrans.csp?pClassName=web.DHCBL.KB.DHCPHDiseaseUse&pClassMethod=UpdateData&pEntityName=web.Entity.KB.DHCPHDiseaseUse"
		}
		else
		{
			var saveURL="../csp/dhc.bdp.ext.entitydatatrans.csp?pClassName=web.DHCBL.KB.DHCPHDiseaseUse&pClassMethod=SaveData&pEntityName=web.Entity.KB.DHCPHDiseaseUse"
		}*/
		if (InstDr!=""){
		...
		//获取选中的数据
		var PDIUserDRs=record.PHEINDesc
		var rows = PDIUserDRs.split(",");;	
		...
		for(var i=0; i<rows.length; i++){
			...
			var PDIUseDR=rows[i]; //频率id
			useStr = useStr+PDIUseDR+"^"+Mode+"^"+Type+"^"+OrderNum+"^"+GenDr+"^"+PointerDr+"^"+PointerType+"^"+Lib+"^"+ActiveFlag+"^"+SysFlag+"^"+ExcludeFlag+"^"+Age+"^"+MinVal+"^"+MaxVal+"^"+UomDr+"^"+AlertMsg+"^"+InstDr;
			var repeatstr=repeatstr+"<"+PDIUseDR+"&"+Age+">"
		}
		
		//遍历已选列表，判断是否有重复数据
		var centerrows = $('#mygrid').datagrid('getRows');	
		for(var j=0; j<centerrows.length; j++){
			if ((centerrows[j].PDIRowId!="")&&(centerrows[j].PDIRowId!=record.PDIRowId)){
				var FreqDR=centerrows[j].PDIUseDR; //频率id
...
*/		   var data = tkMakeServerCall("web.DHCBL.KB.DHCPHDiseaseUse", saveURL,useStr)    
			var data=eval('('+data+')');
		        if(data.success=='true'){
	...
	function SearchFunLib(){
		...
		$('#mygrid').datagrid('load',{
			ClassName:"web.DHCBL.KB.DHCPHDiseaseUse",
			QueryName:"GetListNew",
			'desc': desc,
			TypeDr : "",
			GenDr: GenDr,
