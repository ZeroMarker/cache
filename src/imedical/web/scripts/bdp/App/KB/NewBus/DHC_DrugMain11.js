...
function InitMainList()
{
	...
	var mainclumns=[[ 
				  {field:'PHINSTDesc',title:'',width:650,
					formatter: function(value,row,index){
						if (row._parentId=="")
						{
							return "<a id="+row.PHINSTDesc+">"+ReturnFlagIcon(row.PHINSTGroupDesc)+"</a>"
						}else{
							//return '<div style="word-break:break-all;word-wrap:break-word;white-space:pre-wrap;overflow:visible;nowrap:false;">'+value+'</div>';
							return value
						}
					},
	...
	var leftclumns=[[ 
				  {field:'text',title:'',width:'120',
					formatter: function(value,row,index){
						if (row.icon!=""){
							if (row.count!="0"){
								return "<a href=#"+row.text+"><img src='"+row.icon+"' style='border: 0px'><span>"+value+"</span><span class='badgeDiv'>"+row.count+"</span></a>";
							}else{
								return "<a href=#"+row.text+"><img src='"+row.icon+"' style='border: 0px'><span>"+value+"</a>";
							}
						}else{
							return value
						}
...
ReturnFlagIcon = function(value)
{
    ...
    {	
		...
		var mywin = document.getElementById("myWindow");
		if (mywin)
		{
