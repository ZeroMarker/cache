...
Ext.onReady(function() {
	var QUERY_ACTION_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.KB.DHCPHProcessingAct&pClassQuery=GetList";
  	var OPEN_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.KB.DHCPHProcessingAct&pClassMethod=OpenData";
	...
	var formSearch = new Ext.form.FormPanel({
				...
				reader: new Ext.data.JsonReader({root:'list'},
		                             [
                                        {name: 'PACTRowId',mapping:'PACTRowId',type:'string'},
                                        {name: 'PACTInstDr',mapping:'PACTInstDr',type:'string'},
                                        /*{name: 'PACTCode',mapping:'PACTCode',type:'string'},
                                        {name: 'PACTText',mapping:'PACTText',type:'string'},
                                        {name: 'PACTOrdNum',mapping:'PACTOrdNum',type:'string'},*/                                      
				...
				items:[{
							fieldLabel : 'PACTRowId',
							hideLabel : 'True',
							hidden : true,
							name : 'PACTRowId'
						},{
							fieldLabel : 'PACTInstDr',
							hideLabel : 'True',
							hidden : true,
							name : 'PACTInstDr'
						},/*{
							...
							name : 'PACTCode',
							id:'PACTCodeF',
							readOnly : Ext.BDP.FunLib.Component.DisableFlag('PACTCodeF'),
   							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('PACTCodeF')),
							allowBlank : false,
							...
							name : 'PACTText',
							id:'PACTTextF',
							readOnly : Ext.BDP.FunLib.Component.DisableFlag('PACTTextF'),
   							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('PACTTextF')),
							editable:false,
							allowBlank : false,
							enableKeyEvents : true,
							validationEvent : 'blur',
                            ...
                            listeners : {
								...
								'blur' : function(){
									if(Ext.getCmp("PHINSTTextF").getValue()==""){
										Ext.getCmp("PHINSTTextF").setValue(Ext.getCmp("PACTTextF").getValue());
									}		
								}
							}	
						},{
							...
							name : 'PACTOrdNum',
							id:'PACTOrdNumF',
							readOnly : Ext.BDP.FunLib.Component.DisableFlag('PACTOrdNumF'),
   							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('PACTOrdNumF')),
							allowBlank : false,
   ...
   /** grid数据存储 */
	var ds = new Ext.data.Store({
				...
				reader : new Ext.data.JsonReader({
						...
						sortInfo: {field: 'PACTOrdNum', direction: 'asc'}
			});
	/** grid数据加载遮罩 */
			        ...
			        'afterrender' : function() {		        		
			            var ddrow = new Ext.dd.DropTarget(grid.container, {  
			                ...
			                notifyDrop : function(dd, e, data) {		                
			                    ...
			                    var rowid =rows[0].get('PACTRowId')
		 ...
		 var record = grid.getSelectionModel().getSelected();	
	        if (record) {
	          ...
	          // alert("index"+index+"^"+"PINLOrderNum"+record.get('PINLOrderNum')+"^PINLRowID"+record.get('PINLRowID'))
	            if (index > 0) {
	            	var checkId=record.get('PACTRowId')
	            	var upId=grid.getStore().getAt(index-1).get('PACTRowId')
	            	Ext.Ajax.request({											
		...
		if(downbtn){
	        ...
	        if (record) {
	            ...
	            if (index < grid.store.getCount() - 1) {
	            	var checkId=record.get('PACTRowId')
	            	var downId=grid.getStore().getAt(index+1).get('PACTRowId')
	            	Ext.Ajax.request({											
							url:ORDER_ACTION_URL,
							method:'post', 
							params:{
