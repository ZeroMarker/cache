...
var QUERY_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.KB.DHCPHDiseaseFreq&pClassQuery=GetFreqList";
var SAVE_ACTION_URL = "../csp/dhc.bdp.ext.entitydatatrans.csp?pClassName=web.DHCBL.KB.DHCPHDiseaseFreq&pClassMethod=SaveData&pEntityName=web.Entity.KB.DHCPHDiseaseFreq";
var QUERY_ACTION_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.KB.DHCPHDiseaseFreq&pClassQuery=GetList";
var DELETE_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.KB.DHCPHDiseaseFreq&pClassMethod=DeleteData";
var SAVE_MODE_URL = "../csp/dhc.bdp.ext.entitydatatrans.csp?pClassName=web.DHCBL.KB.DHCPHDiseaseFreq&pClassMethod=SaveMode&pEntityName=web.Entity.KB.DHCPHDiseaseFreq";
					...
					handler : function SaveData() {
						...
						if(rs.length==0){
							...
							Ext.each(rs,function(){
								...
								var PDFFreqDR=this.get('PHEFRowId');//频率id
								...
								if (MinVal != "" && MaxVal != "") {
									...
									var FreqDR = record.get('PDFFreqDR');
								...
								freqStr = freqStr+PDFFreqDR+"^"+Mode+"^"+Type+"^"+OrderNum+"^"+Gen+"^"+Pointer+"^"+PointerType+"^"+Lib+"^"+ActiveFlag+"^"+SysFlag+"^"+ExcludeFlag+"^"+Age+"^"+MinVal+"^"+MaxVal+"^"+UomDr+"^"+AlertMsg;
							})
							if (flagRet == 0){
								Ext.Ajax.request({
									url : SAVE_ACTION_URL , 		
...
/*	 gridWest.on("rowdblclick", function(grid, rowIndex, e){
		var PDFFreqDR=gridWest.getSelectionModel().getSelected().get('PHEFRowId');//频率id
		var Mode=Ext.getCmp("Mode").getValue(); //管制级别
		var Type="FREQ";//知识库目录
		var OrderNum=0;//顺序
		var Gen=GenDr;//通用名  传
		...
		var MaxVal = Ext.getCmp("PDAMaxVal").getValue();
		var UomDr = Ext.getCmp("PDAUomDrF").getValue();
		var freqStr = PDFFreqDR+"^"+Mode+"^"+Type+"^"+OrderNum+"^"+Gen+"^"+Pointer+"^"+PointerType+"^"+Lib+"^"+ActiveFlag+"^"+SysFlag+"^"+ExcludeFlag+"^"+Age+"^"+MinVal+"^"+MaxVal+"^"+UomDr;
	...
	var gridCenter = new Ext.grid.EditorGridPanel({
		...
		columns : [new Ext.grid.CheckboxSelectionModel(), // 单选列
				{
					...
					header : '禁忌',
					sortable : true,
					dataIndex : 'PDFExcludeFlag',
					renderer : Ext.BDP.FunLib.Component.ReturnFlagIcon,
					editor:new Ext.form.ComboBox({
						name : 'PDFExcludeFlag',
						hiddenName : 'PDFExcludeFlag',
						editable:false,
						allowBlank : false,
						width : 230,
						labelWidth:30,
		...
		listeners:{
			'afteredit':function(e){
				...
				var flag=record.get('PDFExcludeFlag');
				var msg=record.get('PDFAlertMsg');
				if (minVal != "" && maxVal != "") {
        			if (parseInt(minVal)>parseInt(maxVal)) {
        				Ext.Msg.show({
			        					title : '提示',
	...
	/**已选列表双击进行已选列表中删除操作*/
	 gridCenter.on("rowdblclick", function(grid, rowIndex, e){
		Ext.Ajax.request({
			...
			method : 'POST',
			params : {
				'id' : gridCenter.getSelectionModel().getSelected().get('PDFRowId')//频率id
			},
			callback : function(options, success, response) {
				if (success) {
					var jsonData = Ext.util.JSON.decode(response.responseText);
					if (jsonData.success == 'true') {
						...
						if(gridCenter.getSelectionModel().getCount()!='0'){
							var id = gridCenter.getSelectionModel().getSelections()[0].get('PDFRowId');
						}
						gridCenter.getStore().load({
							params : {
								'TypeDr':'',
