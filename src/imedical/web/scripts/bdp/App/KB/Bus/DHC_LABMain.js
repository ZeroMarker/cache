 ...
 */
 ...
 //全局变量
	var GlPGenDr ="" //检验项目通用名ID
	var GlPPointer =""  //关联剂型ID
	var GenDesc ="" //检验项目名加剂型名
	
Ext.onReady(function(){

	
	/* 检验项目通用名列表 */var DrugGeneric_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.KB.DHCBusMain&pClassQuery=GetInstrList"; 
	/* 检验项目通用名列表 */var GetDrugBookList = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.KB.DHCBusMain&pClassQuery=GetDrugBookList1";
	
	/* 检验项目说明书明细 *///var GetDrugBookList = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.KB.DHCBusMain&pClassMethod=GetDrugBookList";
 ...
 /** ----------------------------------------------------- ↑↑↑↑↑ 3.说明书编辑部分（弹出框）  -------------------------------------------------------------**/
	...
	ReturnFlagIcon = function(value)
	{
		...
		switch(value)
		{
		  ...
		  break;
		  case "临床意义":
		  returnValue = "<img src='"+Ext.BDP.FunLib.Path.URL_Img +"KB/lcyy32.png' style='border: 0px'><span style='font-size:24px'>"+value+"</span>";
		  break;
		  case "不良反应":
		  ...
		  break;
		  case "检验结果辅助诊断":
		  returnValue = "<img src='"+Ext.BDP.FunLib.Path.URL_Img +"KB/fzzd32.png' style='border: 0px'><span style='font-size:24px'>"+value+"</span>";
		  break;
...
//	    		obj.update(html);
	 ...
	 grid.on("rowdblclick",function(grid,rowIndex,e){  //双击功能
	 	...
	 	var countLabIndic=0,countLabContr=0,countLabClinical=0,countLabInterEach=0,countLabMatNeAt=0,countLabResultDiag=0
	 	var arrLabIndic=[],arrLabContr=[],arrLabClinical=[],arrLabInterEach=[],arrLabMatNeAt=[],arrLabResultDiag=[]
		ds.each(function(record) { 
			if (record.get('PHINSTGroupDesc')=='适应证' ){
				arrLabIndic.push(record.get('PHINSTCount'));
			}
			if (record.get('PHINSTGroupDesc')=='禁忌证' ){
				arrLabContr.push(record.get('PHINSTCount'));
			}	
			if (record.get('PHINSTGroupDesc')=='临床意义' ){
				arrLabClinical.push(record.get('PHINSTCount'));
			}	
			if (record.get('PHINSTGroupDesc')=='相互作用' ){
				arrLabInterEach.push(record.get('PHINSTCount'));
			}	
			if (record.get('PHINSTGroupDesc')=='注意事项' ){
				arrLabMatNeAt.push(record.get('PHINSTCount'));
			}
			if (record.get('PHINSTGroupDesc')=='检验结果辅助诊断' ){
				arrLabResultDiag.push(record.get('PHINSTCount'));
			}
		})
		countLabIndic=arrLabIndic.length==0?0:Math.max.apply(null,arrLabIndic)
		countLabContr=arrLabContr.length==0?0:Math.max.apply(null,arrLabContr)
		countLabClinical=arrLabClinical.length==0?0:Math.max.apply(null,arrLabClinical)
		countLabInterEach=arrLabInterEach.length==0?0:Math.max.apply(null,arrLabInterEach)
		countLabMatNeAt=arrLabMatNeAt.length==0?0:Math.max.apply(null,arrLabMatNeAt)
		countLabResultDiag=arrLabResultDiag.length==0?0:Math.max.apply(null,arrLabResultDiag)
		countInfo="适应证#"+countLabIndic+"^禁忌证#"+countLabContr+"^临床意义#"+countLabClinical+"^相互作用#"+countLabInterEach+"^注意事项#"+countLabMatNeAt+"^检验结果辅助诊断#"+countLabResultDiag
		loadMenu(menuArray,westpanel);
