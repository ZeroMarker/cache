...
Ext.onReady(function(){
	...
	/* 心电项目通用名列表 */var DrugGeneric_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.KB.DHCBusMain&pClassQuery=GetInstrList"; 
	/* 心电项目通用名列表 */var GetDrugBookList = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.KB.DHCBusMain&pClassQuery=GetDrugBookList1";
	
	/* 心电项目说明书明细 *///var GetDrugBookList = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.KB.DHCBusMain&pClassMethod=GetDrugBookList";
...
//	    		obj.update(html);
	 ...
	 grid.on("rowdblclick",function(grid,rowIndex,e){  //双击功能
	 	...
	 	var countELECTIndic=0,countELECTContr=0,countELECTInterEach=0,countELECTAdvRea=0,countELECTMatNeAt=0
	 	var arrELECTIndic=[],arrELECTContr=[],arrELECTInterEach=[],arrELECTAdvRea=[],arrELECTMatNeAt=[]
		ds.each(function(record) { 
			if (record.get('PHINSTGroupDesc')=='适应证' ){
				arrELECTIndic.push(record.get('PHINSTCount'));
			}
			if (record.get('PHINSTGroupDesc')=='禁忌证' ){
				arrELECTContr.push(record.get('PHINSTCount'));
			}
			if (record.get('PHINSTGroupDesc')=='相互作用' ){
				arrELECTInterEach.push(record.get('PHINSTCount'));
			}
			if (record.get('PHINSTGroupDesc')=='不良反应' ){
				arrELECTAdvRea.push(record.get('PHINSTCount'));
			}
			if (record.get('PHINSTGroupDesc')=='注意事项' ){
				arrELECTMatNeAt.push(record.get('PHINSTCount'));
			}
		})
		countELECTIndic=arrELECTIndic.length==0?0:Math.max.apply(null,arrELECTIndic)
		countELECTContr=arrELECTContr.length==0?0:Math.max.apply(null,arrELECTContr)
		countELECTInterEach=arrELECTInterEach.length==0?0:Math.max.apply(null,arrELECTInterEach)
		countELECTAdvRea=arrELECTAdvRea.length==0?0:Math.max.apply(null,arrELECTAdvRea)
		countELECTMatNeAt=arrELECTMatNeAt.length==0?0:Math.max.apply(null,arrELECTMatNeAt)
		countInfo="适应证#"+countELECTIndic+"^禁忌证#"+countELECTContr+"^相互作用#"+countELECTInterEach+"^不良反应#"+countELECTAdvRea+"^注意事项#"+countELECTMatNeAt
		loadMenu(menuArray,westpanel);
