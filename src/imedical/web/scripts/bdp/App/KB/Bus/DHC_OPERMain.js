...
Ext.onReady(function(){
	...
	/* 手术项目通用名列表 */var DrugGeneric_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.KB.DHCBusMain&pClassQuery=GetInstrList"; 
	/* 手术项目通用名列表 */var GetDrugBookList = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.KB.DHCBusMain&pClassQuery=GetDrugBookList1";
	
	/* 手术项目说明书明细 *///var GetDrugBookList = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.KB.DHCBusMain&pClassMethod=GetDrugBookList";
...
//	    		obj.update(html);
 ...
 /** ----------------------------------------------------- ↑↑↑↑↑ 3.说明书编辑部分（弹出框）  -------------------------------------------------------------**/
	...
	ReturnFlagIcon = function(value)
	{
		...
		{
		  case "高危提醒":
		  returnValue = "<img src='"+Ext.BDP.FunLib.Path.URL_Img +"KB/gwtx32.png' style='border: 0px'><span style='font-size:24px'>"+value+"</span>";
		  break;
...
//	    		obj.update(html);
	 ...
	 grid.on("rowdblclick",function(grid,rowIndex,e){  //双击功能
	 	...
	 	var countOPERRemind=0
	 	var arrOPERRemind=[]
		ds.each(function(record) { 
			if (record.get('PHINSTGroupDesc')=='高危提醒' ){
				arrOPERRemind.push(record.get('PHINSTCount'));
			}
		})
		countOPERRemind=arrOPERRemind.length==0?0:Math.max.apply(null,arrOPERRemind)
		countInfo="高危提醒#"+countOPERRemind
		loadMenu(menuArray,westpanel);
