...
var QUERY_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.KB.DHCPHExtGeneric&pClassQuery=GetDataForCmb1";
					...
					handler : function SaveData() {
						...
						if(rs.length==0){
							...
							Ext.each(rs,function(){
								...
								var fieldDR=this.get('PHEGRowId');//证型id
	  ...
	  if(btn){
	 	 ...
	 	 if(button=="yes"){
			Ext.Ajax.request({
				...
				method : 'POST',
				params : {
					'id' : gridCenter.getSelectionModel().getSelected().get('PHSGRowId')//频率id
				},
				callback : function(options, success, response) {
					if (success) {
						var jsonData = Ext.util.JSON.decode(response.responseText);
