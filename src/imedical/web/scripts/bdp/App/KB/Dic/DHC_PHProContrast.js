...
var QUERY_HIS_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.KB.DHCPHProContrast&pClassQuery=GetDataForHis";
var QUERY_CONTRAST_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.KB.DHCPHProContrast&pClassQuery=GetList";
...
/**********************************商品名字典表***********************************/
	...
	dsWestS.on('beforeload',function(){ 
		if(dsWestS.getCount()!='0'){
			if(gridWestN.getSelectionModel().getCount()!=0){
				Ext.apply(this.baseParams,{id:gridWestN.getSelectionModel().getSelections()[0].get('PHNRowId')})
			}
		}
	});  
	/** 已对照grid加载数据 */
		...
		columns : [new Ext.grid.CheckboxSelectionModel(), // 单选列
				{
					...
					header : 'his代码',
					sortable : true,
					dataIndex : 'PHPCHisCode'
				}, {
					header : 'his描述',
					sortable : true,
					dataIndex : 'PHPCHisDesc'
				}, {
					header : '操作',
					dataIndex : 'PHPCRowId',
					align:'center',
	...
	/**根据对照查询已对照*/
	 gridWestN.on("rowclick", function(grid, rowIndex, e){
	 	var id = gridWestN.getSelectionModel().getSelections()[0].get('PHNRowId');
...
/**********************************Center his剂型表*************************************/
	...
	/**对照**/
	 gridCenter.on('cellclick', function (grid, rowIndex, columnIndex, e) {  
	 	...
	 	if(btn){
		 	if(gridWestN.getSelectionModel().getCount()!=0){
		 		var rowid = gridWestN.getSelectionModel().getSelections()[0].get('PHNRowId');
		 		var hisrowid = gridCenter.getSelectionModel().getSelections()[0].get('PHCDRowId');
