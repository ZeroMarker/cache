...
var QUERY_ACTION_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.KB.DHCGenItmContrast&pClassQuery=GetLABList";//默认标识为检验项目的通用名
var QUERY_HIS_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.CT.CTTestCode&pClassQuery=GetList";//检验项目
var QUERY_CONTRAST_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.KB.DHCGenItmContrast&pClassQuery=GetList";
var SAVE_ACTION_URL = "../csp/dhc.bdp.ext.entitydatatrans.csp?pClassName=web.DHCBL.KB.DHCGenItmContrast&pClassMethod=SaveLisData&pEntityName=web.Entity.KB.DHCGenItmContrast";
var DELETE_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.KB.DHCGenItmContrast&pClassMethod=DeleteData";
var SAVE_COEFF_URL = "../csp/dhc.bdp.ext.entitydatatrans.csp?pClassName=web.DHCBL.KB.DHCGenItmContrast&pClassMethod=SaveCoeff&pEntityName=web.Entity.KB.DHCGenItmContrast";
var pagesize_main = Ext.BDP.FunLib.PageSize.Main;

	/** ****************************清空已对照数据 *************************/
	function clearSubWin() {
			gridWestS.getStore().removeAll()
			document.getElementById('ext-comp-1032').innerHTML="没有记录",
			document.getElementById('ext-comp-1028').innerHTML="页,共 1 页",
			document.getElementById('ext-comp-1027').value="1"
	}
/**********************************通用名表***********************************/
	/** grid数据存储 */
	...
	var tbWestN = new Ext.Toolbar({
		id : 'tbWestN',
		items : [
				'<h3 style="color:blue;">检验通用名切换</h3>',{
					xtype:'combo',
					id:'lis',
					width:70,
					...
					listeners:{
			        'select': function(){
			                if(Ext.getCmp('lis').getValue() == 'LAB')
			                {
			                    var QUERY_ACTION_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.KB.DHCGenItmContrast&pClassQuery=GetLABList";//标识为检验项目的通用名
			                    dsWestN.proxy=new Ext.data.HttpProxy({url:QUERY_ACTION_URL});
			                    dsWestN.reload({
			                    	params : {
										start : 0,
										limit : pagesize_main
			                    ...
			                    dsWestS.removeAll();
			                    var QUERY_HIS_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.CT.CTTestCode&pClassQuery=GetList";//检验项目
								dsCenter.proxy=new Ext.data.HttpProxy({url:QUERY_HIS_URL});
			                    dsCenter.reload({
			                    	params : {
										start : 0,
										limit : pagesize_main
			                    ...
			                    gridWestN.getColumnModel().setHidden(5,false);
			                    gridWestS.getColumnModel().setHidden(5,false);
			                }
			                else
			                {
			                    var QUERY_ACTION_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.KB.DHCGenItmContrast&pClassQuery=GetSPECList";//标识为检验标本的通用名
			                    dsWestN.proxy=new Ext.data.HttpProxy({url:QUERY_ACTION_URL});
			                    dsWestN.reload({
			                    	params : {
										start : 0,
			                    ...
			                    dsWestS.removeAll();
			                    var QUERY_HIS_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.CT.CTSpecimen&pClassQuery=GetList";//检验标本
			                	dsCenter.proxy=new Ext.data.HttpProxy({url:QUERY_HIS_URL});
			                    dsCenter.reload({
			                    	params : {
										start : 0,
			                    ...
			                    gridWestN.getColumnModel().setHidden(5,true);
			                    gridWestS.getColumnModel().setHidden(5,true);
			                }
		...
		columns : [new Ext.grid.CheckboxSelectionModel(), // 单选列
				{
					...
					renderer:function(value){
						if(value=="TS"){return "检验套"}
						if(value=="TC"){return "检验项"}
					}
				}],
		stripeRows : true,
		viewConfig : {
		...
		listeners:{
			'afteredit':function(e){
				var record = e.record; //得到当前行所有数据
       			var coeff = e.value; //得到修改列修改后值
               Ext.Ajax.request({
                   url: SAVE_COEFF_URL,
                   method: "POST",
                   params: {
                       coeff: coeff,
                       id: record.get('GICTRowId')
                   },
                   success: function(r) {
	...
	/**根据对照查询已对照;根据医嘱标识TC->TestCode,TS->TestSet查询his数据*/
	 gridWestN.on("rowclick", function(grid, rowIndex, e){
		...
		if(Ext.getCmp('lis').getValue() == 'LAB'){
			var OTFlag=gridWestN.getSelectionModel().getSelections()[0].get('OTFlag');
	 		if(OTFlag=="TS"){
	 			var QUERY_HIS_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.CT.CTTestSet&pClassQuery=GetList";//检验项目
				dsCenter.proxy=new Ext.data.HttpProxy({url:QUERY_HIS_URL});
                dsCenter.reload();
	 		}else{
	 			var QUERY_HIS_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.CT.CTTestCode&pClassQuery=GetList";//检验项目
	 ...
	 gridCenter.on('cellclick', function (grid, rowIndex, columnIndex, e) {  
	 	...
	 	if(btn){
		 	if(gridWestN.getSelectionModel().getCount()!=0){
		 		...
		 		var flag = Ext.getCmp('lis').getValue() 
		 		var otflag=gridWestN.getSelectionModel().getSelections()[0].get('OTFlag');
		    	var ids=rowid+"^"+hisrowid+"^"+flag+"^"+otflag;
