...
var QUERY_ACTION_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.KB.DHCGenItmContrast&pClassQuery=GetGenList";
	...
	var btnCenterSearch = new Ext.Button({
		...
		handler : function() {
			gridCenter.getStore().baseParams={			
					name : Ext.getCmp("textHisDesc").getValue()
			};
	 ...
	 gridCenter.on('cellclick', function (grid, rowIndex, columnIndex, e) {  
	 	...
	 	if(btn){
		 	if(gridWestN.getSelectionModel().getCount()!=0){
		 		...
		 		var hisrowid = gridCenter.getSelectionModel().getSelections()[0].get('PHCGERowId');
