...
Ext.onReady(function() {
	...
	var QUERY_ACTION_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.KB.DHCGenLinkPointer&pClassQuery=GetPartList";
	...
	var BindingPart="../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.KB.DHCPHExtPart&pClassQuery=GetDataForCmb1";
	...
	var WinFormAdd = new Ext.form.FormPanel({
				...
				items : [{
							...
							xtype : 'combo',
							fieldLabel : '<font color=red>*</font>检查项目',
							name : 'GlPGenDr',
							id:'GlPGenDrF',
							hiddenName:'GlPGenDr',
							...
							xtype : 'multiSelect',
							fieldLabel : '<font color=red>*</font>检查部位',
							name : 'GlPPointer',
							id:'GlPPointerF',
							...
							store : new Ext.data.JsonStore({
								...
								root : 'data',
								totalProperty : 'total',
								idProperty : 'PHEPRowId',
								fields : ['PHEPRowId', 'PHEPDesc'],
								listeners:{
									'load':function(store){
	...
	function saveData(){
				var tempCode = Ext.getCmp("GlPGenDrF").getValue();
    			...
    			if (tempDesc=="") {
    				Ext.Msg.show({ title : '提示', msg : '检查部位不能为空!', minWidth : 200, icon : Ext.Msg.ERROR, buttons : Ext.Msg.OK });
          			return;
    			}
	...
	var winUpdate = new Ext.Window({
		...
		buttons : [{
			...
			handler : function() {
				var tempCode = Ext.getCmp("GlPGenDrFU").getValue();
				...
				if (winUpdate.title == "添加") {
					WinFormUpdate.formUpdate.submit({
						clientValidation : true, // 进行客户端验证
						waitTitle : '提示',
						url : SAVE_ACTION_URL,
						method : 'POST',
	...
	var btnSearch = new Ext.Button({
				...
				handler : function() {
					grid.getStore().baseParams={			
							gen : Ext.getCmp("GlPGenDr").getValue(),
	...
	var tb = new Ext.Toolbar({
				...
				items : [
						...
						'检查项目', {
							xtype : 'combo',
							id : 'GlPGenDr',
							disabled : Ext.BDP.FunLib.Component.DisableFlag('GlPGenDr'),
							triggerAction : 'all',// query
							queryParam : "desc",
							forceSelection : true,
							selectOnFocus : false,
