...
var QUERY_CONTRAST_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.KB.DHCPHHospLocCon&pClassQuery=GetList";
...
/**********************************科室字典表***********************************/
	...
	var dsWestS = new Ext.data.Store({
		...
		reader : new Ext.data.JsonReader({
					...
					successProperty : 'success'
							...
							name : 'HOSPLHisCode',
							mapping : 'HOSPLHisCode',
							type : 'string'
						}, {
							name : 'HOSPLHisDesc',
							mapping : 'HOSPLHisDesc',
							type : 'string'
						},{
							name : 'HOSPLRowId',
							mapping : 'HOSPLRowId',
	...
	dsWestS.on('beforeload',function(){ 
		if(dsWestS.getCount()!='0'){
			if(gridWestN.getSelectionModel().getCount()!=0){
				Ext.apply(this.baseParams,{id:gridWestN.getSelectionModel().getSelections()[0].get('HOSPLRowId')})
			}
		}
	});  
	/** 已对照grid加载数据 */
		...
		columns : [new Ext.grid.CheckboxSelectionModel(), // 单选列
				{
					...
					header : 'his代码',
					sortable : true,
					dataIndex : 'HOSPLHisCode'
				}, {
					header : 'his描述',
					sortable : true,
					dataIndex : 'HOSPLHisDesc'
				}, {
					header : '操作',
					dataIndex : 'HOSPLRowId',
					align:'center',
	...
	/**根据对照查询已对照*/
	 gridWestN.on("rowclick", function(grid, rowIndex, e){
	 	var id = gridWestN.getSelectionModel().getSelections()[0].get('HOSPLRowId');
...
/**********************************Center his剂型表*************************************/
	...
	/**对照**/
	 gridCenter.on('cellclick', function (grid, rowIndex, columnIndex, e) {  
	 	...
	 	if(btn){
		 	if(gridWestN.getSelectionModel().getCount()!=0){
		 		var rowid = gridWestN.getSelectionModel().getSelections()[0].get('HOSPLRowId');
