...
var QUERY_HIS_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.CT.CTUOM&pClassQuery=GetList";
var QUERY_CONTRAST_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.KB.DHCPHUomContrast&pClassQuery=GetList";
var SAVE_ACTION_URL = "../csp/dhc.bdp.ext.entitydatatrans.csp?pClassName=web.DHCBL.KB.DHCPHUomContrast&pClassMethod=SaveData&pEntityName=web.Entity.KB.DHCPHUomContrast";
...
/**********************************剂型表***********************************/
	...
	var dsWestS = new Ext.data.Store({
		...
		reader : new Ext.data.JsonReader({
					...
					successProperty : 'success'
							...
							name : 'PHUCTUomCode',
							mapping : 'PHUCTUomCode',
							type : 'string'
						}, {
							name : 'PHUCTUomDesc',
							mapping : 'PHUCTUomDesc',
							type : 'string'
						}, {
							name : 'PHUCTHisUomCode',
							mapping : 'PHUCTHisUomCode',
							type : 'string'
						}, {
							name : 'PHUCTHisUomDesc',
							mapping : 'PHUCTHisUomDesc',
							type : 'string'
						},{
							name : 'PHUCTRowId',
							mapping : 'PHUCTRowId',
	...
	dsWestS.on('beforeload',function(){ 
		if(dsWestS.getCount()!='0'){
			if(gridWestN.getSelectionModel().getCount()!=0){
				Ext.apply(this.baseParams,{id:gridWestN.getSelectionModel().getSelections()[0].get('PHEURowId')})
			}
		}
	});  
	/** 已对照grid加载数据 */
		...
		columns : [new Ext.grid.CheckboxSelectionModel(), // 单选列
				{
					header : '代码',
					sortable : true,
					dataIndex : 'PHUCTUomCode'
				}, {
					header : '描述',
					sortable : true,
					dataIndex : 'PHUCTUomDesc'
				}, {
					header : 'his代码',
					sortable : true,
					dataIndex : 'PHUCTHisUomCode'
				}, {
					header : 'his描述',
					sortable : true,
					dataIndex : 'PHUCTHisUomDesc'
				}, {
					header : '操作',
					dataIndex : 'PHUCTRowId',
					align:'center',
					renderer:function (){    
	...
	/**根据对照查询已对照*/
	 gridWestN.on("rowclick", function(grid, rowIndex, e){
	 	var id = gridWestN.getSelectionModel().getSelections()[0].get('PHEURowId');
...
/**********************************Center his剂型表*************************************/
	...
	/**对照**/
	 gridCenter.on('cellclick', function (grid, rowIndex, columnIndex, e) {  
	 	...
	 	if(btn){
		 	if(gridWestN.getSelectionModel().getCount()!=0){
		 		var rowid = gridWestN.getSelectionModel().getSelections()[0].get('PHEURowId');
		 		var hisrowid = gridCenter.getSelectionModel().getSelections()[0].get('CTUOMRowId');
