/// 名称: 症状字典对照
/// 描述: 包含症状字典表与his症状对照功能，与已对照删除功能
/// 编写者: 基础数据平台组-谷雪萍
/// 编写日期: 2015-1-28
Ext.onReady(function(){
var QUERY_ACTION_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.KB.DHCPHSymptom&pClassQuery=GetList";
var QUERY_HIS_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.KB.DHCSymptomFeild&pClassQuery=GetList";
var QUERY_CONTRAST_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.KB.DHCPHSymptomCon&pClassQuery=GetList";
...
/**********************************症状字典表***********************************/
	...
	dsWestS.on('beforeload',function(){ 
		if(dsWestS.getCount()!='0'){
			if(gridWestN.getSelectionModel().getCount()!=0){
				Ext.apply(this.baseParams,{id:gridWestN.getSelectionModel().getSelections()[0].get('SYMRowId')})
			}
		}
	});  
	/** 已对照grid加载数据 */
		...
		columns : [new Ext.grid.CheckboxSelectionModel(), // 单选列
				{
					...
					header : 'his代码',
					sortable : true,
					dataIndex : 'SYMHisCode'
				}, {
					header : 'his描述',
					sortable : true,
					dataIndex : 'SYMHisDesc'
				}, {
					header : '操作',
					dataIndex : 'SYMRowId',
					align:'center',
	...
	/**根据对照查询已对照*/
	 gridWestN.on("rowclick", function(grid, rowIndex, e){
	 	var id = gridWestN.getSelectionModel().getSelections()[0].get('SYMRowId');
...
/**********************************Center his剂型表*************************************/
	...
	/**对照**/
	 gridCenter.on('cellclick', function (grid, rowIndex, columnIndex, e) {  
	 	...
	 	if(btn){
		 	if(gridWestN.getSelectionModel().getCount()!=0){
		 		var rowid = gridWestN.getSelectionModel().getSelections()[0].get('SYMRowId');
		 		var hisrowid = gridCenter.getSelectionModel().getSelections()[0].get('SYFRowId');
