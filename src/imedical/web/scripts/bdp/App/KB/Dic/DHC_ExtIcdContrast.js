/// 名称: 诊断对照
/// 描述: 包含诊断表与his诊断表对照功能，与已对照诊断删除功能
/// 编写者: 基础数据平台组-谷雪萍
/// 编写日期: 2014-11-11
Ext.onReady(function(){
var QUERY_ACTION_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.KB.DHCExtIcdFeild&pClassQuery=GetList";
var QUERY_HIS_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.CT.MRCICDDx&pClassQuery=GetDataForCmb1";
var QUERY_CONTRAST_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.KB.DHCExtIcdContrast&pClassQuery=GetList";
...
/**********************************诊断表***********************************/
	...
	dsWestS.on('beforeload',function(){ 
		if(dsWestS.getCount()!='0'){
			if(gridWestN.getSelectionModel().getCount()!=0){
				Ext.apply(this.baseParams,{id:gridWestN.getSelectionModel().getSelections()[0].get('ICDRowId')})
			}
		}
	});  
	/** 已对照grid加载数据 */
		...
		columns : [new Ext.grid.CheckboxSelectionModel(), // 单选列
				{
					...
					header : 'his代码',
					sortable : true,
					dataIndex : 'ICONHisCode'
				}, {
					header : 'his描述',
					sortable : true,
					dataIndex : 'ICONHisDesc'
				}, {
					header : '操作',
					dataIndex : 'ICONRowId',
					align:'center',
	...
	/**根据对照查询已对照*/
	 gridWestN.on("rowclick", function(grid, rowIndex, e){
	 	var id = gridWestN.getSelectionModel().getSelections()[0].get('ICDRowId');
	...
	gridWestS.on('cellclick', function (grid, rowIndex, columnIndex, e) {  
	 	...
	 	if(btn){
	 		Ext.MessageBox.confirm('提示','确定撤销此条对照吗?', function(btn) {
	 			if (btn == 'yes') {
...
/**********************************Center his诊断表*************************************/
	...
	/**对照**/
	 gridCenter.on('cellclick', function (grid, rowIndex, columnIndex, e) {  
	 	...
	 	if(btn){
		 	if(gridWestN.getSelectionModel().getCount()!=0){
		 		var rowid = gridWestN.getSelectionModel().getSelections()[0].get('ICDRowId');
		 		var hisrowid = gridCenter.getSelectionModel().getSelections()[0].get('MRCIDRowId');
