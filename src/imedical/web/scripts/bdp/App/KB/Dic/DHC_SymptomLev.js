...
Ext.onReady(function() {
	...
	var SAVE_ACTION_URL = "../csp/dhc.bdp.ext.entitydatatrans.csp?pClassName=web.DHCBL.KB.DHCSymptomLev&pClassMethod=SaveData&pEntityName=web.Entity.KB.DHCSymptomLev";
	var OPEN_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.KB.DHCSymptomLev&pClassMethod=OpenData";
	
	var TREE_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.KB.DHCSymptomLev&pClassMethod=GetTreeJson";
	var TREE_COMBO_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.KB.DHCSymptomLev&pClassMethod=GetTreeComboJson";
	var DRAG_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.KB.DHCSymptomLev&pClassMethod=DragNode";
	//var SAVE_DROP_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.KB.DHCSymptomLev&pClassMethod=SaveDropData";
			...
			disabled : Ext.BDP.FunLib.Component.DisableFlag('btnFeild'),
	      	handler: ConLab=function () {			        ;
				if(selectNode == ""){
				 	...
				 	//alert(selectNode)
	                	var link="dhc.bdp.ext.default.csp?extfilename=App/KB/Dic/DHC_SymptomCon&selectNode="+selectNode; 
	                	/** 症状分级与症状关联窗口 */
				...
				Ext.MessageBox.confirm('提示', '确定要删除所选的数据吗?', function(btn) {
					if (btn == 'yes') {
						...
						if(ids.indexOf("Con")>-1){
							ids=ids.replace("Con","");
							var DELETE_ACTION_URL="../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.KB.DHCSymptomCon&pClassMethod=DeleteData";
						}
						else{
							var DELETE_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.KB.DHCSymptomLev&pClassMethod=DeleteData";
						}
						Ext.Ajax.request({
	...
	var WinForm = new Ext.form.FormPanel({
				...
				items : [{
                            ...
                            validator : function(thisText){
	                            ...
	                            var className =  "web.DHCBL.KB.DHCSymptomLev";  //后台类名称
							...
							if (action.result.success == 'true') {
							   ...
							   //添加之后 刷新
								...
								if (treePanel.getSelectionModel().getSelectedNode()) {
									...
									var level =tkMakeServerCall("web.DHCBL.KB.DHCSymptomLev","GetLevel",_record.id);
				...
				handler : AddData=function () {
					...
					if(_record){
						...
						if(ids.indexOf("Con")>-1){
							...
							level=parseInt(level)+1
							Ext.getCmp("SYLLevelF").setValue(level);
							var seq =tkMakeServerCall("web.DHCBL.KB.DHCSymptomLev","GetSequence",_record.id);
							seq=parseInt(seq)
	...
	function AddSame(){
			...
			if(_record){
				...
				if(ids.indexOf("Con")>-1){
					...
					Ext.getCmp("SYLLevelF").setValue(level);
					var seq =tkMakeServerCall("web.DHCBL.KB.DHCSymptomLev","GetSequence",_record.parentNode.id);
					seq=parseInt(seq)
					Ext.getCmp("SYLSequenceF").setValue(seq)
	...
	var panel = new Ext.Panel({
		...
		tbar:[
			...
			}, '-',btnAddwin, '-', btnEditwin, '-', btnDel, '-',btnFeild
		]
	});
	/** 布局 */
