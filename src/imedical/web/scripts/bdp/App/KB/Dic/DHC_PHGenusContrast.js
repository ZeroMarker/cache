/// 名称: 菌属对照
/// 描述: 包含菌属字典表与dbo.BT_Genus对照功能，与已对照删除功能
/// 编写者: 基础数据平台组-谷雪萍
/// 编写日期: 2017-4-10
Ext.onReady(function(){
var QUERY_ACTION_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.KB.DHCPHBtGenus&pClassQuery=GetList";
var QUERY_HIS_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.KB.DHCPHGenusContrast&pClassQuery=GetHisList";
var QUERY_CONTRAST_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.KB.DHCPHGenusContrast&pClassQuery=GetList";
...
/**********************************知识库标识表***********************************/
	...
	var dsWestS = new Ext.data.Store({
		...
		reader : new Ext.data.JsonReader({
					...
					successProperty : 'success'
							...
							name : 'PHGGenusCode',
							mapping : 'PHGGenusCode',
							type : 'string'
						}, {
							name : 'PHGGenusDesc',
							mapping : 'PHGGenusDesc',
							type : 'string'
						}, {
							name : 'PHGHisGenusCode',
							mapping : 'PHGHisGenusCode',
							type : 'string'
						}, {
							name : 'PHGHisGenusDesc',
							mapping : 'PHGHisGenusDesc',
							type : 'string'
						},{
							name : 'PHGRowId',
							mapping : 'PHGRowId',
	...
	dsWestS.on('beforeload',function(){ 
		if(dsWestS.getCount()!='0'){
			if(gridWestN.getSelectionModel().getCount()!=0){
				Ext.apply(this.baseParams,{id:gridWestN.getSelectionModel().getSelections()[0].get('BTGERowId')})
			}
		}
	});  
	/** 已对照grid加载数据 */
		...
		columns : [new Ext.grid.CheckboxSelectionModel(), // 单选列
				{
					header : '代码',
					sortable : true,
					dataIndex : 'PHGGenusCode'
				}, {
					header : '描述',
					sortable : true,
					dataIndex : 'PHGGenusDesc'
				}, {
					header : 'his代码',
					sortable : true,
					dataIndex : 'PHGHisGenusCode'
				}, {
					header : 'his描述',
					sortable : true,
					dataIndex : 'PHGHisGenusDesc'
				}, {
					header : '操作',
					dataIndex : 'PHGRowId',
					align:'center',
	...
	/**根据对照查询已对照*/
	 gridWestN.on("rowclick", function(grid, rowIndex, e){
	 	var id = gridWestN.getSelectionModel().getSelections()[0].get('BTGERowId');
...
/**********************************Center his剂型表*************************************/
	...
	/**对照**/
	 gridCenter.on('cellclick', function (grid, rowIndex, columnIndex, e) {  
	 	...
	 	if(btn){
		 	if(gridWestN.getSelectionModel().getCount()!=0){
		 		var rowid = gridWestN.getSelectionModel().getSelections()[0].get('BTGERowId');
