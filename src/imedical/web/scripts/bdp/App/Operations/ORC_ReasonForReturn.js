...
Ext.onReady(function() {
	...
	var AliasGrid = new Ext.BDP.grid.AliasEditorGridPanel({
		TableN : "ORC_ReasonForReturn"
	});
	Ext.BLANK_IMAGE_URL = '../scripts_lib/ext3.2.1/resources/images/default/s.gif';
	
	var ACTION_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.CT.ORCReasonForReturn&pClassQuery=GetList";
	var OPEN_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.ORCReasonForReturn&pClassMethod=OpenData";
	var SAVE_ACTION_URL = "../csp/dhc.bdp.ext.entitydatatrans.csp?pClassName=web.DHCBL.CT.ORCReasonForReturn&pClassMethod=SaveEntity&pEntityName=web.Entity.CT.ORCReasonForReturn";
	var DELETE_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.ORCReasonForReturn&pClassMethod=DeleteData";
	
	var pagesize = Ext.BDP.FunLib.PageSize.Main;
	/********排序*******/
    Ext.BDP.FunLib.SortTableName = "User.ORCReasonForReturn";
    var btnSort = Ext.BDP.FunLib.SortBtn;
    //-----------------翻译开始--------//	
	Ext.BDP.FunLib.TableName="ORC_ReasonForReturn"
			...
			if (grid.selModel.hasSelection()) {
				...
				Ext.MessageBox.confirm('提示', '确定要删除所选的数据吗?', function(btn) {
					if (btn == 'yes') {
						...
						//删除所有别名
						AliasGrid.DataRefer = rows[0].get('RFRRowId');
	...
	var WinForm = new Ext.FormPanel({
				...
				waitMsgTarget : true,
       			reader: new Ext.data.JsonReader({root:'list'},
                                        [{name: 'RFRCode',mapping:'RFRCode'},
                                         {name: 'RFRDesc',mapping:'RFRDesc'},
                                         {name: 'RFRDateFrom',mapping:'RFRDateFrom'},
                                         {name: 'RFRDateTo',mapping:'RFRDateTo'},
                                         {name: 'RFRRowId',mapping:'RFRRowId'}
                                        ]),
				defaults : {
					anchor : '85%',
					bosrder : false
				...
				items : [{
							...
							id:'RFRCode',
							maxLength:30,
							//disabled : Ext.BDP.FunLib.Component.DisableFlag('RFRCode'),
							readOnly : Ext.BDP.FunLib.Component.DisableFlag('RFRCode'),
							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('RFRCode')),
							name : 'RFRCode',
							allowBlank:false,
                            ...
                            validator : function(thisText){
	                            ...
	                            var className =  "web.DHCBL.CT.ORCReasonForReturn";
							...
							id:'RFRDesc',
							maxLength:30,
							//disabled : Ext.BDP.FunLib.Component.DisableFlag('RFRDesc'),
							readOnly : Ext.BDP.FunLib.Component.DisableFlag('RFRDesc'),
							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('RFRDesc')),
							name : 'RFRDesc',
							allowBlank:false,
							validationEvent : 'blur',
							enableKeyEvents:true, 
	...
	var ds = new Ext.data.Store({
				...
				//remoteSort : true
				//sortInfo: {field : "RFRRowId",direction : "ASC"}
	});
	//ds.sort("RFRCode","DESC");
   ...
   // 载入被选择的数据行的表单数据
    var loadFormData = function(grid) {
        ...
        if (!_record) {
            ...
            WinForm.form.load( {
                url : OPEN_ACTION_URL + '&id='+ _record.get('RFRRowId'),
                //waitMsg : '正在载入数据...',
                success : function(form,action) {
