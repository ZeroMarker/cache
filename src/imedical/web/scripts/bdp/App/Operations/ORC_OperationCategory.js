...
Ext.onReady(function() {
	...
	var AliasGrid = new Ext.BDP.grid.AliasEditorGridPanel({
		TableN : "ORC_OperationCategory"
	});
	Ext.BLANK_IMAGE_URL = '../scripts_lib/ext3.2.1/resources/images/default/s.gif';
	
	var ACTION_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.CT.ORCOperationCategory&pClassQuery=GetList";
	var OPEN_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.ORCOperationCategory&pClassMethod=OpenData";
	var SAVE_ACTION_URL = "../csp/dhc.bdp.ext.entitydatatrans.csp?pClassName=web.DHCBL.CT.ORCOperationCategory&pClassMethod=SaveEntity&pEntityName=web.Entity.CT.ORCOperationCategory";
	var DELETE_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.ORCOperationCategory&pClassMethod=DeleteData";
	
	var pagesize = Ext.BDP.FunLib.PageSize.Main;
	/********排序*******/
    Ext.BDP.FunLib.SortTableName = "User.ORCOperationCategory";
    var btnSort = Ext.BDP.FunLib.SortBtn;
    //-----------------翻译开始--------//	
	Ext.BDP.FunLib.TableName="ORC_OperationCategory"
	...
	btnhislog.on('click', function(btn,e){    
		...
		if (grid.selModel.hasSelection()) {
			...
			RowID=rows[0].get('CATEGRowId');
			Desc=rows[0].get('CATEGDesc');
			...
			if (grid.selModel.hasSelection()) {
				...
				Ext.MessageBox.confirm('提示', '确定要删除所选的数据吗?', function(btn) {
					if (btn == 'yes') {
						...
						//删除所有别名
						AliasGrid.DataRefer = rows[0].get('CATEGRowId');
	...
	var WinForm = new Ext.FormPanel({
				...
				waitMsgTarget : true,
       			reader: new Ext.data.JsonReader({root:'list'},
                                        [{name: 'CATEGCode',mapping:'CATEGCode'},
                                         {name: 'CATEGDesc',mapping:'CATEGDesc'},
                                         {name: 'CATEGDateFrom',mapping:'CATEGDateFrom'},
                                         {name: 'CATEGDateTo',mapping:'CATEGDateTo'},
				...
				items : [{
							...
							id:'CATEGCode',
							maxLength:15,
							//disabled : Ext.BDP.FunLib.Component.DisableFlag('CATEGCode'),
							readOnly : Ext.BDP.FunLib.Component.DisableFlag('CATEGCode'),
							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('CATEGCode')),
							name : 'CATEGCode',
							allowBlank:false,
                            ...
                            validator : function(thisText){
	                            ...
	                            var className =  "web.DHCBL.CT.ORCOperationCategory";
	                            var classMethod = "FormValidate";
	                            var id="";
	                            if(win.title=='修改'){
	                            	var _record = grid.getSelectionModel().getSelected();
	                            	var id = _record.get('CATEGRowId');
	                            }
	                            var flag = "";
							...
							id:'CATEGDesc',
							maxLength:220,
							//disabled : Ext.BDP.FunLib.Component.DisableFlag('CATEGDesc'),
							readOnly : Ext.BDP.FunLib.Component.DisableFlag('CATEGDesc'),
							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('CATEGDesc')),
							name : 'CATEGDesc',
							allowBlank:false,
							validationEvent : 'blur',
							enableKeyEvents:true, 
	...
	var win = new Ext.Window({
		...
		listeners : {
			"show" : function() {
				Ext.getCmp("form-save").getForm().findField("CATEGCode").focus(true,300);
			},
			"hide" : function() {
	...
	var btnEditwin = new Ext.Toolbar.Button({
				...
				handler : UpdateData=function() {
					if (grid.selModel.hasSelection()) {
				        ...
				        //加载别名面板
			            var _record = grid.getSelectionModel().getSelected();
			            AliasGrid.DataRefer = _record.get('CATEGRowId');
				        AliasGrid.loadGrid();
	...
	var ds = new Ext.data.Store({
				...
				//remoteSort : true
				//sortInfo: {field : "CATEGRowId",direction : "ASC"}
	});
	//ds.sort("CATEGCode","DESC");
	...
	var GridCM=[sm, {
							...
							dataIndex : 'CATEGDateFrom'
						}, {
							header : '结束日期',
							width : 80,
							sortable : true,
							renderer : Ext.util.Format.dateRenderer(BDPDateFormat),
							dataIndex : 'CATEGDateTo'
						}]
	// 创建Grid
   ...
   // 载入被选择的数据行的表单数据
    var loadFormData = function(grid) {
        ...
        if (!_record) {
            ...
            WinForm.form.load( {
                url : OPEN_ACTION_URL + '&id='+ _record.get('CATEGRowId'),
                //waitMsg : '正在载入数据...',
                success : function(form,action) {
	...
	btnTrans.on("click",function(){
		if (grid.selModel.hasSelection()) {		
	        var _record = grid.getSelectionModel().getSelected();
	        var selectrow = _record.get('CATEGRowId');	           
		 } else {
		 	var selectrow="";
		 }
