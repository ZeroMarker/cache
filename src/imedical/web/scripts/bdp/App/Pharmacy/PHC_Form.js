...
Ext.onReady(function() {
	...
	var ACTION_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.CT.PHCForm&pClassQuery=GetList";
	var OPEN_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.PHCForm&pClassMethod=OpenData";
	var SAVE_ACTION_URL_New = "../csp/dhc.bdp.ext.entitydatatrans.csp?pClassName=web.DHCBL.CT.PHCForm&pClassMethod=SaveEntity&pEntityName=web.Entity.CT.PHCForm";
	var DELETE_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.PHCForm&pClassMethod=DeleteData";
	...
	/********排序*******/
    Ext.BDP.FunLib.SortTableName = "User.PHCForm";
    var btnSort = Ext.BDP.FunLib.SortBtn;
    //-----------------翻译开始--------//	
	Ext.BDP.FunLib.TableName="PHC_Form"
	...
	btnhislog.on('click', function(btn,e){    
		...
		if (grid.selModel.hasSelection()) {
			...
			RowID=rows[0].get('PHCFRowId');
			Desc=rows[0].get('PHCFDesc');
			...
			if (grid.selModel.hasSelection()) {                           //-------如果选中某一行则继续执行删除操作
				...
				Ext.MessageBox.confirm('提示', '确定要删除所选的数据吗?', function(btn) {
					if (btn == 'yes') {                                   //-------点击确定按钮后继续执行删除操作
						...
						//删除所有别名
						AliasGrid.DataRefer = rows[0].get('PHCFRowId');
						...
						Ext.Ajax.request({	                              	
							...
							params : {									  //-------请求带的参数
								'id' : rows[0].get('PHCFRowId')          //-------通过RowId来删除数据
							},
	...
	var WinForm = new Ext.form.FormPanel({
				...
				reader: new Ext.data.JsonReader({root:'list'},           //-------使用open方法要在form中加上数据的映射
                                        [{name: 'PHCFRowId',mapping:'PHCFRowId',type:'string'},
                                         {name: 'PHCFCode',mapping:'PHCFCode',type:'string'},
                                         {name: 'PHCFDesc',mapping:'PHCFDesc',type:'string'}
                                        ]),
				defaultType : 'textfield',								 //--------统一设定items类型为textfield
				items : [{
							id:'PHCFRowId',
							fieldLabel : 'PHCFRowId',
							hideLabel : 'True',
							...
							id:'PHCFCodeF',
							maxLength:15,
   							//disabled : Ext.BDP.FunLib.Component.DisableFlag('PHCFCodeF'),
   							readOnly : Ext.BDP.FunLib.Component.DisableFlag('PHCFCodeF'),
							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('PHCFCodeF')),
                            ...
                            validator : function(thisText){
	                            ...
	                            var className =  "web.DHCBL.CT.PHCForm";
	                            var classMethod = "FormValidate";                     
	                            var id="";
	                            if(win.title=='修改'){
	                            	var _record = grid.getSelectionModel().getSelected();
	                            	var id = _record.get('PHCFRowId');
	                            }
	                            var flag = "";
							...
							name : 'PHCFDesc',
							//disabled : Ext.BDP.FunLib.Component.DisableFlag('PHCFDescF'),
							readOnly : Ext.BDP.FunLib.Component.DisableFlag('PHCFDescF'),
							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('PHCFDescF')),
							allowBlank: false,
						    enableKeyEvents:true, 
						    validationEvent : 'blur',  
						    validator : function(thisText){
	...
	var win = new Ext.Window({
		...
		listeners : {
			"show" : function() {
				Ext.getCmp("form-save").getForm().findField("PHCFCode").focus(true,300);
	...
	var btnEditwin = new Ext.Toolbar.Button({
				...
				handler : UpdateData=function () {						        //-----------显示修改的窗口和form	
					...
					loadFormData(grid);
				        ...
				        //加载别名面板
			            var _record = grid.getSelectionModel().getSelected();
			            AliasGrid.DataRefer = _record.get('PHCFRowId');
				        AliasGrid.loadGrid();
					}
					else {
						Ext.Msg.show({
	...
	var grid = new Ext.grid.GridPanel({
				...
				//autoExpandColum:'PHCFForeignDesc',
				bbar : paging,                                    //-----------------底部状态栏
				tbar : tb,                                        //-----------------顶部状态栏
				stateId : 'grid'
			});
   ...
   /**---------双击事件-----------*/
	...
	btnTrans.on("click",function(){
		if (grid.selModel.hasSelection()) {		
	        var _record = grid.getSelectionModel().getSelected();
	        var selectrow = _record.get('PHCFRowId');	           
		 } else {
		 	var selectrow="";
		 }
    ...
    var loadFormData = function(grid) {
        ...
        if (!_record) {
            ...
            win.show();
            WinForm.form.load( {
                url : OPEN_ACTION_URL + '&PHCFRowId='+ _record.get('PHCFRowId'),
                waitMsg : '正在载入数据...',
                success : function(form,action) {
                },
                failure : function(form,action) {
