...
Ext.onReady(function() {
	...
	var ACTION_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.CT.PHCGeneric&pClassQuery=GetList";
	var OPEN_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.PHCGeneric&pClassMethod=OpenData";
	var SAVE_ACTION_URL_New = "../csp/dhc.bdp.ext.entitydatatrans.csp?pClassName=web.DHCBL.CT.PHCGeneric&pClassMethod=SaveEntity&pEntityName=web.Entity.CT.PHCGeneric";
	var DELETE_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.PHCGeneric&pClassMethod=DeleteData";
	
	var pagesize = Ext.BDP.FunLib.PageSize.Main;
	/********排序*******/
    Ext.BDP.FunLib.SortTableName = "User.PHCGeneric";
    var btnSort = Ext.BDP.FunLib.SortBtn;
    //-----------------翻译开始--------//	
	Ext.BDP.FunLib.TableName="PHC_Generic"
	...
	btnhislog.on('click', function(btn,e){    
		...
		if (grid.selModel.hasSelection()) {
			...
			RowID=rows[0].get('PHCGERowId');
			Desc=rows[0].get('PHCGEName');
	...
	*/
	Ext.apply(Ext.form.VTypes, {										   
		cKDate:function(val, field){
			var v1 = Ext.getCmp("startdt").getValue();
    		var v2 = Ext.getCmp ("enddt").getValue();
			...
			if (grid.selModel.hasSelection()) {                           //-------如果选中某一行则继续执行删除操作
				...
				Ext.MessageBox.confirm('提示', '确定要删除所选的数据吗?', function(btn) {
					if (btn == 'yes') {                                   //-------点击确定按钮后继续执行删除操作
						...
						//删除所有别名
						AliasGrid.DataRefer = rows[0].get('PHCGERowId');
						...
						Ext.Ajax.request({	                              	
							...
							params : {									  //-------请求带的参数
								'id' : rows[0].get('PHCGERowId')          //-------通过RowId来删除数据
							},
	...
	var WinForm = new Ext.form.FormPanel({
				...
				reader: new Ext.data.JsonReader({root:'list'},
                                        [{name: 'PHCGERowId',mapping:'PHCGERowId',type:'string'},
                                         {name: 'PHCGECode',mapping:'PHCGECode',type:'string'},
                                         {name: 'PHCGEName',mapping:'PHCGEName',type:'string'},
                                         {name: 'PHCGEDateFrom',mapping:'PHCGEDateFrom',type:'string'},
                                         {name: 'PHCGEDateTo',mapping:'PHCGEDateTo',type:'string'}
                                        ]),
				defaultType : 'textfield',								 //--------统一设定items类型为textfield
				items : [{
							id:'PHCGERowId',
							fieldLabel : 'PHCGERowId',
							hideLabel : 'True',
							...
							id:'PHCGECodeF',
							maxLength:100,
							//disabled : Ext.BDP.FunLib.Component.DisableFlag('PHCGECodeF'),
							readOnly : Ext.BDP.FunLib.Component.DisableFlag('PHCGECodeF'),
							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('PHCGECodeF')),
                            ...
                            validator : function(thisText){
	                            ...
	                            var className =  "web.DHCBL.CT.PHCGeneric";
	                            var classMethod = "FormValidate";                     
	                            var id="";
	                            if(win.title=='修改'){
	                            	var _record = grid.getSelectionModel().getSelected();
	                            	var id = _record.get('PHCGERowId');
	                            }
	                            var flag = "";
							...
							name : 'PHCGEName',
							id:'PHCGENameF',
							//disabled : Ext.BDP.FunLib.Component.DisableFlag('PHCGENameF'),
							readOnly : Ext.BDP.FunLib.Component.DisableFlag('PHCGENameF'),
							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('PHCGENameF')),
							allowBlank: false,
							...
							//disabled : Ext.BDP.FunLib.Component.DisableFlag('startdt'),
							readOnly : Ext.BDP.FunLib.Component.DisableFlag('startdt'),
							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('startdt')),
       					    vtype: 'cKDate',
       					    allowBlank: false
						}, {
							xtype : 'datefield',
							...
							listeners : {   'keyup' : function(field, e){	 Ext.BDP.FunLib.Component.GetCurrentDate(field,e);  }},
						    id: 'enddt',
						   //disabled : Ext.BDP.FunLib.Component.DisableFlag('enddt'),
						    readOnly : Ext.BDP.FunLib.Component.DisableFlag('enddt'),
							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('enddt')),
       					    vtype: 'cKDate'
						}
						]
			});
	...
	var win = new Ext.Window({
		...
		listeners : {
			"show" : function() {
				Ext.getCmp("form-save").getForm().findField("PHCGECode").focus(true,300);
	...
	var btnEditwin = new Ext.Toolbar.Button({
				...
				handler : UpdateData=function () {
					...
					loadFormData(grid);
				        ...
				        //加载别名面板
			            var _record = grid.getSelectionModel().getSelected();
			            AliasGrid.DataRefer = _record.get('PHCGERowId');
				        AliasGrid.loadGrid();
					}
					else {
							Ext.Msg.show({
	...
	var btnRefresh = new Ext.Button({
				...
				handler : function() {
					...
					Ext.getCmp("TextName").reset();                     //-----------将输入框清空
	...
	var grid = new Ext.grid.GridPanel({
				...
				//autoExpandColum:'PHCGEForeignName',
				bbar : paging,                                    //-----------------底部状态栏
				tbar : tb,                                        //-----------------顶部状态栏
				stateId : 'grid'
			});
	...
	btnTrans.on("click",function(){
		if (grid.selModel.hasSelection()) {		
	        var _record = grid.getSelectionModel().getSelected();
	        var selectrow = _record.get('PHCGERowId');	           
		 } else {
		 	var selectrow="";
		 }
    ...
    var loadFormData = function(grid) {
        ...
        if (!_record) {
            ...
            win.show();
            WinForm.form.load( {
                url : OPEN_ACTION_URL + '&PHCGERowId='+ _record.get('PHCGERowId'),
                waitMsg : '正在载入数据...',
                success : function(form,action) {
                },
                failure : function(form,action) {
