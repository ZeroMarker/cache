...
Ext.onReady(function() {
	...
	var ACTION_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.CT.PHManufacturer&pClassQuery=GetList";
	var OPEN_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.PHManufacturer&pClassMethod=OpenData";
	var SAVE_ACTION_URL_New = "../csp/dhc.bdp.ext.entitydatatrans.csp?pClassName=web.DHCBL.CT.PHManufacturer&pClassMethod=SaveEntity&pEntityName=web.Entity.CT.PHManufacturer";
	var DELETE_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.PHManufacturer&pClassMethod=DeleteData";
				...
				Ext.MessageBox.confirm('提示', '确定要删除所选的数据吗?', function(btn) {
					if (btn == 'yes') {                                   //-------点击确定按钮后继续执行删除操作
						...
						Ext.Ajax.request({	                              	
							...
							params : {									  //-------请求带的参数
								'id' : rows[0].get('PHMNFRowId')          //-------通过RowId来删除数据
							},
	...
	var WinForm = new Ext.form.FormPanel({
				...
				reader: new Ext.data.JsonReader({root:'list'},
                                        [{name: 'PHMNFRowId',mapping:'PHMNFRowId',type:'string'},
                                         {name: 'PHMNFCode',mapping:'PHMNFCode',type:'string'},
                                         {name: 'PHMNFName',mapping:'PHMNFName',type:'string'},
                                         {name: 'PHMNFAddress',mapping:'PHMNFAddress',type:'string'},
                                         {name: 'PHMNFTel',mapping:'PHMNFTel',type:'string'}
                                        ]),
				defaultType : 'textfield',								 //--------统一设定items类型为textfield
				items : [{
							id:'PHMNFRowId',
							fieldLabel : 'PHMNFRowId',
							hideLabel : 'True',
						...
						},
						{
							...
							id:'PHMNFCodeF',
							maxLength:12,
							//disabled : Ext.BDP.FunLib.Component.DisableFlag('PHMNFCodeF'),
							readOnly : Ext.BDP.FunLib.Component.DisableFlag('PHMNFCodeF'),
							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('PHMNFCodeF')),
							allowBlank:false,
                            ...
                            validator : function(thisText){
	                            ...
	                            var className =  "web.DHCBL.CT.PHManufacturer";
	                            var classMethod = "FormValidate";                     
	                            var id="";
	                            if(win.title=='修改'){
	                            	var _record = grid.getSelectionModel().getSelected();
	                            	var id = _record.get('PHMNFRowId');
	                            }
	                            var flag = "";
						...
						},
						{
							...
							id:'PHMNFNameF',
							maxLength:220,
							//disabled : Ext.BDP.FunLib.Component.DisableFlag('PHMNFNameF'),
							readOnly : Ext.BDP.FunLib.Component.DisableFlag('PHMNFNameF'),
							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('PHMNFNameF')),
							allowBlank:false,
							enableKeyEvents:true,                       //--------启用事件
						...
						},
						{
							...
							name : 'PHMNFAddress',
							id:'PHMNFAddressF',
							readOnly : Ext.BDP.FunLib.Component.DisableFlag('PHMNFAddressF'),
							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('PHMNFAddressF'))
							//disabled : Ext.BDP.FunLib.Component.DisableFlag('PHMNFAddressF')
						},
						{
							fieldLabel : "电话",
							name : 'PHMNFTel',
							...
							id:'PHMNFTelF',
							readOnly : Ext.BDP.FunLib.Component.DisableFlag('PHMNFTelF'),
							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('PHMNFTelF'))
							//disabled : Ext.BDP.FunLib.Component.DisableFlag('PHMNFTelF')
						}
						]
			});
	...
	var win = new Ext.Window({
		...
		listeners : {
			"show" : function() {
				Ext.getCmp("form-save").getForm().findField("PHMNFCode").focus(true,300);
	...
	var btnSearch = new Ext.Button({
				...
				grid.getStore().baseParams = { // -----------模糊查询
						...
						name : Ext.getCmp("TextName").getRawValue()
    ...
    var loadFormData = function(grid) {
        ...
        if (!_record) {
            ...
            win.show();
            WinForm.form.load( {
                url : OPEN_ACTION_URL + '&PHMNFRowId='+ _record.get('PHMNFRowId'),
                waitMsg : '正在载入数据...',
                success : function(form,action) {
