...
Ext.onReady(function() {
	...
	var ACTION_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.CT.PHCDosage&pClassQuery=GetList";
	var OPEN_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.PHCDosage&pClassMethod=OpenData";
	var SAVE_ACTION_URL_New = "../csp/dhc.bdp.ext.entitydatatrans.csp?pClassName=web.DHCBL.CT.PHCDosage&pClassMethod=SaveEntity&pEntityName=web.Entity.CT.PHCDosage";
	var DELETE_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.PHCDosage&pClassMethod=DeleteData";
	...
	/********排序*******/
    Ext.BDP.FunLib.SortTableName = "User.PHCDosage";
    var btnSort = Ext.BDP.FunLib.SortBtn;
    //-----------------翻译开始--------//	
	Ext.BDP.FunLib.TableName="PHC_Dosage"
	...
	btnhislog.on('click', function(btn,e){    
		...
		if (grid.selModel.hasSelection()) {
			...
			RowID=rows[0].get('PHCDORowId');
			Desc=rows[0].get('PHCDODesc1');
			...
			if (grid.selModel.hasSelection()) {                           //-------如果选中某一行则继续执行删除操作
				...
				Ext.MessageBox.confirm('提示', '确定要删除所选的数据吗?', function(btn) {
					if (btn == 'yes') {                                   //-------点击确定按钮后继续执行删除操作
						...
						//删除所有别名
						AliasGrid.DataRefer = rows[0].get('PHCDORowId');
						...
						Ext.Ajax.request({	                              	
							...
							params : {									  //-------请求带的参数
								'id' : rows[0].get('PHCDORowId')          //-------通过RowId来删除数据
							},
	...
	var WinForm = new Ext.form.FormPanel({
				...
				reader: new Ext.data.JsonReader({root:'list'},
                                        [{name: 'PHCDORowId',mapping:'PHCDORowId',type:'string'},
                                         {name: 'PHCDOCode',mapping:'PHCDOCode',type:'string'},
                                         {name: 'PHCDOFactor',mapping:'PHCDOFactor',type:'string'},
                                         {name: 'PHCDODesc1',mapping:'PHCDODesc1',type:'string'},
                                         {name: 'PHCDODesc2',mapping:'PHCDODesc2',type:'string'},
                                         {name: 'PHCDOCTUOMDR',mapping:'PHCDOCTUOMDR',type:'string'},
                                         {name: 'PHCDOQty',mapping:'PHCDOQty',type:'string'}
                                        ]),
				defaultType : 'textfield',								 //--------统一设定items类型为textfield
				items : [{
							id:'PHCDORowId',
							fieldLabel : 'PHCDORowId',
							hideLabel : 'True',
						...
						},
						{
							...
							id:'PHCDOCodeF',
							maxLength:15,
   							//disabled : Ext.BDP.FunLib.Component.DisableFlag('PHCDOCodeF'),
   							readOnly : Ext.BDP.FunLib.Component.DisableFlag('PHCDOCodeF'),
							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('PHCDOCodeF')),
                            ...
                            validator : function(thisText){
	                            ...
	                            var className =  "web.DHCBL.CT.PHCDosage";
	                            var classMethod = "FormValidate";                     
	                            var id="";
	                            if(win.title=='修改'){
	                            	var _record = grid.getSelectionModel().getSelected();
	                            	var id = _record.get('PHCDORowId');
	                            }
	                            var flag = "";
						...
						{
							fieldLabel : "<span style='color:red;'>*</span>中文描述",
							name : 'PHCDODesc1',
							id:'PHCDODesc1F',
							maxLength:220,
   							//disabled : Ext.BDP.FunLib.Component.DisableFlag('PHCDODesc1F'),
   							readOnly : Ext.BDP.FunLib.Component.DisableFlag('PHCDODesc1F'),
							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('PHCDODesc1F')),
							allowBlank:false,
							enableKeyEvents:true,
							validationEvent : 'blur',  
						    validator : function(thisText){
						...
						},
						{
							...
							id:'PHCDODesc2F',
							maxLength:220,
   							//disabled : Ext.BDP.FunLib.Component.DisableFlag('PHCDODesc2F')
   							readOnly : Ext.BDP.FunLib.Component.DisableFlag('PHCDODesc2F'),
							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('PHCDODesc2F'))
						},{
							fieldLabel : "<span style='color:red;'>*</span>系数(数字)",
							xtype:'numberfield',
							name : 'PHCDOFactor',
							id:'PHCDOFactorF',
							//maxLength:8,
							allowBlank:false,  ///表结构限制
							maxValue : 99999999,
   							//disabled : Ext.BDP.FunLib.Component.DisableFlag('PHCDOFactorF'),
   							readOnly : Ext.BDP.FunLib.Component.DisableFlag('PHCDOFactorF'),
							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('PHCDOFactorF')),
							...
							//maxLength:8,
							maxValue : 99999999,
   							//disabled : Ext.BDP.FunLib.Component.DisableFlag('PHCDOQtyF'),
   							readOnly : Ext.BDP.FunLib.Component.DisableFlag('PHCDOQtyF'),
							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('PHCDOQtyF')),
							allowDecimals : false, //不允许输入小数
							minValue : 0,
							minText : '数量不能小于0'
						},{
							...
							listWidth:250,
                   			//emptyText:'请选择',
                  		    fieldLabel: "单位",
   							//disabled : Ext.BDP.FunLib.Component.DisableFlag('PHCDOCTUOMDR'),
   							readOnly : Ext.BDP.FunLib.Component.DisableFlag('PHCDOCTUOMDR'),
							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('PHCDOCTUOMDR')),
                            hiddenName: 'PHCDOCTUOMDR',//真正提交时此combo的name，这个一定要是hiddenName不是name，不然提交的数据为文本框中内容  
	...
	var win = new Ext.Window({
		...
		listeners : {
			"show" : function() {
				Ext.getCmp("form-save").getForm().findField("PHCDOCode").focus(true,300);
	...
	var btnEditwin = new Ext.Toolbar.Button({
				...
				handler : UpdateData=function (){						        //-----------显示修改的窗口和form	
				        ...
				        //加载别名面板
			            var _record = grid.getSelectionModel().getSelected();
			            AliasGrid.DataRefer = _record.get('PHCDORowId');
				        AliasGrid.loadGrid();
						}
						else
						{
	...
	var GridCM= [sm, {									//----------------定义列
							...
							width : 100,
							sortable : true,
							dataIndex : 'PHCDODesc1'
						}, {
							header : '英文描述',
							width : 100,
							sortable : true,
							dataIndex : 'PHCDODesc2'
						}, {
							header : '系数',
							width : 100,
							sortable : true,
							dataIndex : 'PHCDOFactor'
	...
	var grid = new Ext.grid.GridPanel({
				...
				//autoExpandColum:'PHCDOForeignDesc',
				bbar : paging,                                    //-----------------底部状态栏
				tbar : tb,                                        //-----------------顶部状态栏
				stateId : 'grid'
			});
	...
	btnTrans.on("click",function(){
		if (grid.selModel.hasSelection()) {		
	        var _record = grid.getSelectionModel().getSelected();
	        var selectrow = _record.get('PHCDORowId');	           
		 } else {
		 	var selectrow="";
		 }
    ...
    var loadFormData = function(grid) {
        ...
        if (!_record) {
            ...
            win.show();
            WinForm.form.load( {
                url : OPEN_ACTION_URL + '&PHCDORowId='+ _record.get('PHCDORowId'),
                waitMsg : '正在载入数据...',
                success : function(form,action) {
