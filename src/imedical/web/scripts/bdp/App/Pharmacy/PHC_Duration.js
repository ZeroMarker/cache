...
Ext.onReady(function() {
	...
	var ACTION_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.CT.PHCDuration&pClassQuery=GetList";
	var OPEN_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.PHCDuration&pClassMethod=OpenData";
	var SAVE_ACTION_URL_New = "../csp/dhc.bdp.ext.entitydatatrans.csp?pClassName=web.DHCBL.CT.PHCDuration&pClassMethod=SaveEntity&pEntityName=web.Entity.CT.PHCDuration";
	var DELETE_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.PHCDuration&pClassMethod=DeleteData";
	
	var pagesize = Ext.BDP.FunLib.PageSize.Main;
	/********排序*******/
    Ext.BDP.FunLib.SortTableName = "User.PHCDuration";
    var btnSort = Ext.BDP.FunLib.SortBtn;
    //-----------------翻译开始--------//	
	Ext.BDP.FunLib.TableName="PHC_Duration"
	...
	btnhislog.on('click', function(btn,e){    
		...
		if (grid.selModel.hasSelection()) {
			...
			RowID=rows[0].get('PHCDURowId');
			Desc=rows[0].get('PHCDUDesc1');
			...
			if (grid.selModel.hasSelection()) {                           //-------如果选中某一行则继续执行删除操作
				...
				Ext.MessageBox.confirm('提示', '确定要删除所选的数据吗?', function(btn) {
					if (btn == 'yes') {                                   //-------点击确定按钮后继续执行删除操作
						...
						//删除所有别名
						AliasGrid.DataRefer = rows[0].get('PHCDURowId');
						...
						Ext.Ajax.request({	                              	
							...
							params : {									  //-------请求带的参数
								'id' : rows[0].get('PHCDURowId')          //-------通过RowId来删除数据
							},
	...
	var WinForm = new Ext.form.FormPanel({
				...
				reader: new Ext.data.JsonReader({root:'list'},
                                        [{name: 'PHCDURowId',mapping:'PHCDURowId',type:'string'},
                                         {name: 'PHCDUCode',mapping:'PHCDUCode',type:'string'},
                                         {name: 'PHCDUFactor',mapping:'PHCDUFactor',type:'string'},
                                         {name: 'PHCDUDesc1',mapping:'PHCDUDesc1',type:'string'},
                                         {name: 'PHCDUDesc2',mapping:'PHCDUDesc2',type:'string'},
                                         {name: 'PHCDUDateFrom',mapping:'PHCDUDateFrom',type:'string'},
                                         {name: 'PHCDUDateTo',mapping:'PHCDUDateTo',type:'string'}
                                        ]),
				defaultType : 'textfield',								 //--------统一设定items类型为textfield
				items : [{
							id:'PHCDURowId',
							fieldLabel : 'PHCDURowId',
							hideLabel : 'True',
						...
						},
						{
							...
							id:'PHCDUCodeF',
							maxLength:15,
   							//disabled : Ext.BDP.FunLib.Component.DisableFlag('PHCDUCodeF'),
   							readOnly : Ext.BDP.FunLib.Component.DisableFlag('PHCDUCodeF'),
							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('PHCDUCodeF')),
                            ...
                            validator : function(thisText){
	                            ...
	                            var className =  "web.DHCBL.CT.PHCDuration";
	                            var classMethod = "FormValidate";                     
	                            var id="";
	                            if(win.title=='修改'){
	                            	var _record = grid.getSelectionModel().getSelected();
	                            	var id = _record.get('PHCDURowId');
	                            }
	                            var flag = "";
						...
						
						{
							...
							id:'PHCDUDesc1F',
							maxLength:220,
   							//disabled : Ext.BDP.FunLib.Component.DisableFlag('PHCDUDesc1F'),
   							readOnly : Ext.BDP.FunLib.Component.DisableFlag('PHCDUDesc1F'),
							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('PHCDUDesc1F')),
							allowBlank:false,
							enableKeyEvents:true,
							validationEvent : 'blur',  
						    validator : function(thisText){
						...
						},
						{
							...
							id:'PHCDUDesc2F',
							maxLength:220,
   							//disabled : Ext.BDP.FunLib.Component.DisableFlag('PHCDUDesc2F')
   							readOnly : Ext.BDP.FunLib.Component.DisableFlag('PHCDUDesc2F'),
							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('PHCDUDesc2F'))
						},
						{
							...
							//maxLength:8,
							maxValue : 99999999,
   							//disabled : Ext.BDP.FunLib.Component.DisableFlag('PHCDUFactorF'),
   							readOnly : Ext.BDP.FunLib.Component.DisableFlag('PHCDUFactorF'),
							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('PHCDUFactorF')),
							allowBlank:false,
	...
	var win = new Ext.Window({
		...
		listeners : {
			"show" : function() {
				Ext.getCmp("form-save").getForm().findField("PHCDUCode").focus(true,300);
	...
	var btnEditwin = new Ext.Toolbar.Button({
				...
				handler : UpdateData=function (){						        //-----------显示修改的窗口和form	
				        ...
				        //加载别名面板
			            var _record = grid.getSelectionModel().getSelected();
			            AliasGrid.DataRefer = _record.get('PHCDURowId');
				        AliasGrid.loadGrid();
						}
						else
						{
	...
	var GridCM= [sm, {									//----------------定义列
							...
							width : 100,
							sortable : true,
							dataIndex : 'PHCDUDesc1'
						}, {
							header : '英文描述',
							width : 100,
							sortable : true,
							dataIndex : 'PHCDUDesc2'
						}, {
							header : '系数',
							width : 100,
							sortable : true,
							dataIndex : 'PHCDUFactor'
						}, {
							header : '开始日期',
							width : 80,
							sortable : true,
							renderer : Ext.util.Format.dateRenderer(BDPDateFormat),
							dataIndex : 'PHCDUDateFrom'
						}, {
							header : '结束日期',
							width : 80,
							sortable : true,
							renderer : Ext.util.Format.dateRenderer(BDPDateFormat),
							dataIndex : 'PHCDUDateTo'
						}]
	/**---------创建grid-----------*/
	var grid = new Ext.grid.GridPanel({
				...
				//autoExpandColum:'PHCDUForeignDesc',
				bbar : paging,                                    //-----------------底部状态栏
				tbar : tb,                                        //-----------------顶部状态栏
				stateId : 'grid'
			});
	...
	btnTrans.on("click",function(){
		if (grid.selModel.hasSelection()) {		
	        var _record = grid.getSelectionModel().getSelected();
	        var selectrow = _record.get('PHCDURowId');	           
		 } else {
		 	var selectrow="";
		 }
    ...
    var loadFormData = function(grid) {
        ...
        if (!_record) {
            ...
            win.show();
            WinForm.form.load( {
                url : OPEN_ACTION_URL + '&PHCDURowId='+ _record.get('PHCDURowId'),
                waitMsg : '正在载入数据...',
                success : function(form,action) {
