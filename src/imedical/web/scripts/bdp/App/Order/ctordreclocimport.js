	...
	/// 编写者:基础数据平台组 - 钟荣枫
	/// 编写日期:2019-10-17
document.write('<script type="text/javascript" src="../scripts/bdp/App/Order/RecLocSelect.js"> </script>');
Ext.onReady(function() {
	...
	var ReadExcel=function (efilepath,sheet_id,row_from,importclassname,importmethod)
		...
		*/
		...
		var importrowtotal=rowcount-row_from+1
		...
		Ext.TaskMgr.start({  
		  run:function(){
		  	...
		  	if(row>rowcount) //当到达最后一行退出
		  	{
				...
				win2.close();
				window.parent.window.winimp.close();
		  	...
		  	else
		  	{
				...
				tempStr=""
			    progressText = "正在导入"+oSheet.name+"表的第"+handlerow+"条记录,总共"+importrowtotal+"条记录!";  
  ...
  var ExcelImport=function (){
		...
		var importclassname="web.DHCBL.CT.RecLocSelect",importmethod="ImportExcel";  //导入地址数据
	...
	var formSearch = new Ext.form.FormPanel({
		...
		},
		{
                	title:"备注说明",
                	width:700,
					html : "<ui><li>1.导入时需选择正确的xls/xlsx格式的文件。</li></ui> <ui><li>2.需在IE下执行操作,并在[internet选项]-[安全]-[受信任的站点]-[站点]中添加开始界面到可信任的站点，然后在[自定义级别]中对[没有标记为安全的ActiveX控件进行初始化和脚本运行]这一项设置为启用。</li></ui> <ui><li>3.导入的为User.ARCItmRecLoc（医嘱项接收科室）、User.OECOrdCatRecLoc（医嘱大类接收科室）、User.ARCItemCatRecLoc（医嘱子类接收科室）这3个表的数据。Excel文件导入原则：code相同的数据，如果与原数据库完全相同则跳过，有变动则修改，code不同的数据新增，方便更新数据</li></ui> "
