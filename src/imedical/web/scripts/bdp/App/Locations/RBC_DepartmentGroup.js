...
Ext.onReady(function() {
	...
	var QUERY_ACTION_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.CT.RBCDepartmentGroup&pClassQuery=GetList";//ds里调用数据
	var OPEN_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.RBCDepartmentGroup&pClassMethod=OpenData";//修改时取数据
	var SAVE_ACTION_URL = "../csp/dhc.bdp.ext.entitydatatrans.csp?pClassName=web.DHCBL.CT.RBCDepartmentGroup&pClassMethod=SaveEntity&pEntityName=web.Entity.CT.RBCDepartmentGroup";
	var DELETE_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.RBCDepartmentGroup&pClassMethod=DeleteData";
	
	var ARC_ItmMast_DR_QUERY_ACTION_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.CT.RBCDepartmentGroup&pClassQuery=GetData1";
	...
	//Ext.state.Manager.setProvider(new Ext.state.CookieProvider());
		...
		//-----------------翻译开始--------//	
	Ext.BDP.FunLib.TableName="RBC_DepartmentGroup"
	...
	var AliasGrid = new Ext.BDP.grid.AliasEditorGridPanel({
		TableN : "RBC_DepartmentGroup"
	});
	
	/*****************排序*******************/
    Ext.BDP.FunLib.SortTableName = "User.RBCDepartmentGroup";
   ...
   if (grid.selModel.hasSelection()) {
       ...
       RowID=rows[0].get('DEPRowId');
       Desc=rows[0].get('DEPDesc');
    ...
    HospWinButton.on("click" , function(){
           if (grid.selModel.hasSelection()) { //选中一条数据数据时，弹出 数据与医院关联窗口
				var rowid=grid.getSelectionModel().getSelections()[0].get("DEPRowId");
			...
			if (grid.selModel.hasSelection()) {
				...
				Ext.MessageBox.confirm('提示', '确定要删除所选的数据吗?', function(btn) {
					if (btn == 'yes') {//点击确定按钮后执行删除操作
						...
						AliasGrid.DataRefer = rows[0].get('DEPRowId');
						...
						Ext.Ajax.request({
							...
							method : 'POST',
							params : {
								'id' : rows[0].get('DEPRowId')//通过RowId来删除数据
							},
				...
				waitMsgTarget : true,//这个属性决定了load和submit中对数据的处理,list必须是一个集合类型,json格式应该是[]包含的一个数组
       			reader: new Ext.data.JsonReader({root:'list'},
                                        [{name: 'DEPCode',mapping:'DEPCode'},
                                         ...
                                         {name: 'DEPDateFrom',mapping:'DEPDateFrom'},
                                         {name: 'DEPDateTo',mapping:'DEPDateTo'},
                                         {name: 'DEPRowId',mapping:'DEPRowId'}
                                        ]),
				items : [{
							xtype:'textfield',
							fieldLabel : 'DEPRowId',
							...
							id:'DEPCode',
							name : 'DEPCode',
							readOnly : Ext.BDP.FunLib.Component.DisableFlag('DEPCode'),
							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('DEPCode')),
   							//disabled : Ext.BDP.FunLib.Component.DisableFlag('DEPCode'),
							allowBlank:false,
                            ...
                            validator : function(thisText){
	                            ...
	                            var className =  "web.DHCBL.CT.RBCDepartmentGroup";  //后台类名称
	                            var classMethod = "FormValidate"; //数据重复验证后台函数名	                            
	                            var id="";
	                            if(win.title=='修改'){ // 如果窗口标题为“修改”，则获取rowid
	                            	var _record = grid.getSelectionModel().getSelected();
	                            	var id = _record.get('DEPRowId'); //此条数据的rowid
	                            }
	                            var flag = "";
							...
							id:'DEPDesc',
							name : 'DEPDesc',
							readOnly : Ext.BDP.FunLib.Component.DisableFlag('DEPDesc'),
							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('DEPDesc')),
							//disabled : Ext.BDP.FunLib.Component.DisableFlag('DEPDesc'),
							allowBlank:false,
							validationEvent : 'blur',
                            validator : function(thisText){
                            	//Ext.Msg.alert(thisText);
	...
	var win = new Ext.Window({
		...
		listeners : {
			"show" : function() {
				Ext.getCmp("form-save").getForm().findField("DEPCode").focus(true,50);
			},
			"hide" : function() {
	...
	var btnEditwin = new Ext.Toolbar.Button({
				...
				handler : UpdateData=function() {
					if (grid.selModel.hasSelection()) {
			            ...
			            tabs.setActiveTab(0);
				        //加载别名面板
			            AliasGrid.DataRefer = record.get('DEPRowId');
				        AliasGrid.loadGrid();
					} else {//对应if (grid.selModel.hasSelection()) {}
						Ext.Msg.show({
									title : '提示',
   ...
   // 载入被选择的数据行的表单数据
    var loadFormData = function(grid) {
        ...
        if (!_record) {
            ...
            WinForm.form.load( {
                url : OPEN_ACTION_URL + '&id='+ _record.get('DEPRowId'),
                success : function(form,action) {
                },
                failure : function(form,action) {
    ...
    grid.on("rowdblclick", function(grid, rowIndex, e) {
        	...
        	var _record = grid.getSelectionModel().getSelected();//获取当前行的记录
        	//加载别名面板
            AliasGrid.DataRefer = _record.get('DEPRowId');
	        AliasGrid.loadGrid();
    });	
	...
	btnTrans.on("click",function(){
		if (grid.selModel.hasSelection()) {		
	        var _record = grid.getSelectionModel().getSelected();
	        var selectrow = _record.get('DEPRowId');	           
		 } else {
		 	var selectrow="";
		 }
