//version 3.0
//2022-01-07 引用拼音转换，在筛选的时候 根据首拼筛选
...
Ext.ux.ItemSelector = Ext.extend(Ext.form.Field,  {
	...
	hideNavIcons:false,
	imagePath:"",
	iconUp:"../scripts/bdp/App/BDPSystem/BDPItemAuthorize/up2.gif",
	iconDown:"../scripts/bdp/App/BDPSystem/BDPItemAuthorize/down2.gif",
	//--------------------------------20131025 by lisen
	iconAllLeft:"../scripts/bdp/App/BDPSystem/BDPItemAuthorize/left.gif",
	iconAllRight:"../scripts/bdp/App/BDPSystem/BDPItemAuthorize/right.gif",
	//----------------------------------------------------
	iconLeft:"../scripts/bdp/App/BDPSystem/BDPItemAuthorize/left2.gif",
	iconRight:"../scripts/bdp/App/BDPSystem/BDPItemAuthorize/right2.gif",
	iconTop:"../scripts/bdp/App/BDPSystem/BDPItemAuthorize/top2.gif",
	iconBottom:"../scripts/bdp/App/BDPSystem/BDPItemAuthorize/bottom2.gif",
	drawUpIcon:true,
	drawDownIcon:true,
	drawLeftIcon:true,
	drawRightIcon:true,
				...
				items : ['查找', {
							...
							listeners : {
								'keyup':function(field, e){
									var textF = field.getValue();
									//Ext.getCmp('toText').setValue(textF);
									this.searchFn(textF,'from');
								},
								scope:this
							}
						},new Ext.Button({
							...
							text : '清空',
							handler : function() {
								Ext.getCmp("fromText").reset();
								elment.fromStore.reload();
								//Ext.getCmp("toText").reset();
							}
						})
					]
			});
		this.toTBar = new Ext.Toolbar({
				items : ['查找', {
							xtype : 'textfield',
							id : 'toText',
							enableKeyEvents : true,
							listeners : {
								'keyup':function(field, e){
									var textT = field.getValue();
									//Ext.getCmp('fromText').setValue(textT);
									this.searchFn(textT,'to');
								},
								scope:this
							}
						},new Ext.Button({
							...
							handler : function() {
								//Ext.getCmp("fromText").reset();
								Ext.getCmp("toText").reset();
								elment.toStore.reload();
							}
						})
					]
			});
		this.fromStore.on('load',function(store, rec, obj){
						Ext.getCmp('fromText').setValue('');
						Ext.getCmp('toText').setValue('');
					});
		//---------------------------------------------------------------
    ...
    onRender: function(ct, position){
		...
		if (!this.readOnly) {
			...
			//------------------------20131025 by lisen
			this.addAllIcon.on('click', this.fromToAll, this);
			this.removeAllIcon.on('click', this.toFromAll, this);
			//--------------------------------------------
		}
	...
	//----------------20140224 by lisen
	searchFn : function(text,type){
		if(type=='from'){
			var fromArr = [];
			this.fromStore.each(function (rec){
				var str = rec.get('Name').toUpperCase();
				var strPY=Pinyin.GetJPU(str);
				var textU=text.toUpperCase();
				if(((text!='') && ((str.indexOf(textU)>-1)||(strPY.indexOf(textU)>-1)))){
					fromArr.push(rec);
				}
			},this);
			this.fromMultiselect.view.select(fromArr);
			this.toTop_from();
		}
		else{
			var toArr = [];
			this.toStore.each(function (rec){
				var str = rec.get('Name').toUpperCase();
				var strPY=Pinyin.GetJPU(str);
				var textU=text.toUpperCase();
				if(((text!='') && ((str.indexOf(textU)>-1)||(strPY.indexOf(textU)>-1)))){
			...
			this.toMultiselect.view.select(toArr);
			this.toTop();
		}	
	},
	...
	fromToAll : function() {
		var records = [];
		for (var i=0; i<this.fromStore.getCount(); i++) {
			record = this.fromStore.getAt(i);
			records.push(record);
		}
		this.fromStore.removeAll();
		for (var j=0; j<records.length; j++) {
			this.toStore.add(records[j]);
		}
	},
	
	toFromAll : function() {
		var records = [];
		for (var i=0; i<this.toStore.getCount(); i++) {
			record = this.toStore.getAt(i);
			records.push(record);
		}
		this.toStore.removeAll();
		for (var j=0; j<records.length; j++) {
			this.fromStore.add(records[j]);
		}
	},
