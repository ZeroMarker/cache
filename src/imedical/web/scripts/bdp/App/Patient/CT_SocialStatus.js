...
Ext.onReady(function() {
	...
	var QUERY_ACTION_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.CT.CTSocialStatus&pClassQuery=GetList";
	var SAVE_ACTION_URL = "../csp/dhc.bdp.ext.entitydatatrans.csp?pClassName=web.DHCBL.CT.CTSocialStatus&pClassMethod=SaveEntity&pEntityName=web.Entity.CT.CTSocialStatus";
	var OPEN_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.CTSocialStatus&pClassMethod=OpenData";
	var DELETE_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.CTSocialStatus&pClassMethod=DeleteData";
	var pagesize_main = Ext.BDP.FunLib.PageSize.Main;
	
	/*************************************排序*********************************/
    Ext.BDP.FunLib.SortTableName = "User.CTSocialStatus";
    var btnSort = Ext.BDP.FunLib.SortBtn;
    
	//初始化“别名”维护面板
	var AliasGrid = new Ext.BDP.grid.AliasEditorGridPanel({
		TableN : "CT_SocialStatus"
	});
	Ext.BDP.FunLib.TableName="CT_SocialStatus";
    /// 获取查询配置按钮 
   ...
   if (grid.selModel.hasSelection()) {
       ...
       RowID=rows[0].get('SSRowId');
       Desc=rows[0].get('SSDesc');
   ...
   //多院区医院下拉框
    ...
    HospWinButton.on("click" , function(){
           if (grid.selModel.hasSelection()) { //选中一条数据数据时，弹出 数据与医院关联窗口
				var rowid=grid.getSelectionModel().getSelections()[0].get("SSRowId");
			...
			if (grid.selModel.hasSelection()) {
				Ext.MessageBox.confirm('提示', '确定要删除所选的数据吗?', function(btn) {
					if (btn == 'yes') {
						...
						//删除所有别名
						AliasGrid.DataRefer = rows[0].get('SSRowId');
	...
	var WinForm = new Ext.form.FormPanel({
        		...
        		reader: new Ext.data.JsonReader({root:'list'},
                                        [{name: 'SSRowId',mapping:'SSRowId',type:'int'},
                                         {name: 'SSCode',mapping:'SSCode',type:'string'},
                                         {name: 'SSDesc',mapping:'SSDesc',type:'string'}
                                   ]),
				defaults : {
					anchor : '92%',
					border : false
				...
				items : [{
							...
							name : 'SSCode',
							id:'SSCodeF',
   							readOnly : Ext.BDP.FunLib.Component.DisableFlag('SSCodeF'),
							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('SSCodeF')),
							allowBlank : false,
                            ...
                            validator : function(thisText){
	                            ...
	                            var className =  "web.DHCBL.CT.CTSocialStatus";  //后台类名称
	                            var classMethod = "FormValidate"; //数据重复验证后台函数名	                            
	                            var id="";
	                            if(win.title=='修改'){ // 如果窗口标题为"修改",则获取rowid
	                            	var _record = grid.getSelectionModel().getSelected();
	                            	var id = _record.get('SSRowId'); //此条数据的rowid
	                            }
	                            var flag = "";
							...
							name : 'SSDesc',
							id:'SSDescF',
   							readOnly : Ext.BDP.FunLib.Component.DisableFlag('SSDescF'),
							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('SSDescF')),
							allowBlank : false,
							validationEvent : 'blur',
                            validator : function(thisText){
	                            if(thisText==""){ //当文本框里的内容为空的时候不用此验证
	...
	var win = new Ext.Window({
		...
		buttons : [{
			...
			handler : function() {
				var tempCode = Ext.getCmp("form-save").getForm().findField("SSCode").getValue();
				var tempDesc = Ext.getCmp("form-save").getForm().findField("SSDesc").getValue();
				if (tempCode=="") {
		...
		listeners : {
			"show" : function() {
				Ext.getCmp("form-save").getForm().findField("SSCode").focus(true,800);
			},
				...
				handler : UpdateData=function() {
					...
					if (!_record) {
			            ...
			            Ext.getCmp("form-save").getForm().load({
			                url : OPEN_ACTION_URL + '&id=' + _record.get('SSRowId'),
			                success : function(form,action) {
			                },
			                failure : function(form,action) {
			            ...
			            tabs.setActiveTab(0);
				        //加载别名面板
			            AliasGrid.DataRefer = _record.get('SSRowId');
				        AliasGrid.loadGrid();
			        }
	...
	btnTrans.on("click",function(){
		...
		{
		  var _record=grid.getSelectionModel().getSelected();
          var selectrow=_record.get('SSRowId');
		 }
		else
		{
		  var selectrow=""
