...
Ext.onReady(function() {
	...
	var AliasGrid = new Ext.BDP.grid.AliasEditorGridPanel({
		TableN : "PAC_BroadPatientGroup"
	});
	/********排序*******/
    Ext.BDP.FunLib.SortTableName = "User.PACBroadPatientGroup";
    var btnSort = Ext.BDP.FunLib.SortBtn;
    
	//翻译
	Ext.BDP.FunLib.TableName="PAC_BroadPatientGroup";
	...
	btnhislog.on('click', function(btn,e){    
		...
		if (grid.selModel.hasSelection()) {
			...
			RowID=rows[0].get('BPGRowId');
			Desc=rows[0].get('BPGDesc');
	...
	var ACTION_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.CT.PACBroadPatientGroup&pClassQuery=GetList";
	var OPEN_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.PACBroadPatientGroup&pClassMethod=OpenData";
	var SAVE_ACTION_URL = "../csp/dhc.bdp.ext.entitydatatrans.csp?pClassName=web.DHCBL.CT.PACBroadPatientGroup&pClassMethod=SaveEntity&pEntityName=web.Entity.CT.PACBroadPatientGroup";
	var DELETE_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.PACBroadPatientGroup&pClassMethod=DeleteData";
			...
			if (grid.selModel.hasSelection()) {
				...
				Ext.MessageBox.confirm('提示', '确定要删除所选的数据吗?', function(btn) {
					if (btn == 'yes') {
						...
						//删除所有别名
						AliasGrid.DataRefer = rows[0].get('BPGRowId');
	...
	var WinForm = new Ext.FormPanel({
				...
				waitMsgTarget : true,
       			reader: new Ext.data.JsonReader({root:'list'},
                                        [{name: 'BPGCode',mapping:'BPGCode'},
                                         {name: 'BPGDesc',mapping:'BPGDesc'},
                                         {name: 'BPGDateFrom',mapping:'BPGDateFrom'},
                                         {name: 'BPGDateTo',mapping:'BPGDateTo'},
                                         {name: 'BPGRowId',mapping:'BPGRowId'}
                                        ]),
				defaults : {
					anchor : '85%',
					bosrder : false
				...
				items : [{
							...
							id:'BPGCode',
							maxLength:30,
							//disabled : Ext.BDP.FunLib.Component.DisableFlag('BPGCode'),
							readOnly : Ext.BDP.FunLib.Component.DisableFlag('BPGCode'),
							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('BPGCode')),
							name : 'BPGCode',
							allowBlank:false,
                            ...
                            validator : function(thisText){
	                            ...
	                            var className =  "web.DHCBL.CT.PACBroadPatientGroup";
	                            var classMethod = "FormValidate";                    
	                            var id="";
	                            if(win.title=='修改'){
	                            	var _record = grid.getSelectionModel().getSelected();
	                            	var id = _record.get('BPGRowId');
	                            }
	                            var flag = "";
							...
							id:'BPGDesc',
							maxLength:30,
							//disabled : Ext.BDP.FunLib.Component.DisableFlag('BPGDesc'),
							readOnly : Ext.BDP.FunLib.Component.DisableFlag('BPGDesc'),
							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('BPGDesc')),
							name : 'BPGDesc',
							allowBlank:false,
							validationEvent : 'blur',
							enableKeyEvents:true, 
	...
	var win = new Ext.Window({
		...
		listeners : {
			"show" : function() {
				Ext.getCmp("form-save").getForm().findField("BPGCode").focus(true,300);
			},
			"hide" : function() {
	...
	var btnEditwin = new Ext.Toolbar.Button({
				...
				handler : UpdateData=function() {
					if (grid.selModel.hasSelection()) {
				        ...
				        //加载别名面板
			            var _record = grid.getSelectionModel().getSelected();
			            AliasGrid.DataRefer = _record.get('BPGRowId');
				        AliasGrid.loadGrid();
	...
	var ds = new Ext.data.Store({
				...
				//remoteSort : true
				//sortInfo: {field : "BPGRowId",direction : "ASC"}
	});
	//ds.sort("BPGCode","DESC");
   ...
   // 载入被选择的数据行的表单数据
    var loadFormData = function(grid) {
        ...
        if (!_record) {
            ...
            WinForm.form.load( {
                url : OPEN_ACTION_URL + '&id='+ _record.get('BPGRowId'),
                //waitMsg : '正在载入数据...',
                success : function(form,action) {
    ...
    //翻译
		btnTrans.on("click",function(){
			if (grid.selModel.hasSelection()) {		
	      			var _record = grid.getSelectionModel().getSelected();
	       			 var selectrow = _record.get('BPGRowId');	           
		 	} else {
		 		var selectrow="";
			 }
