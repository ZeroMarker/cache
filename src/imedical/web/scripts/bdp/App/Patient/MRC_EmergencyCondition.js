...
Ext.onReady(function() {
	...
	var QUERY_ACTION_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.CT.MRCEmergencyCondition&pClassQuery=GetList";
	var SAVE_ACTION_URL = "../csp/dhc.bdp.ext.entitydatatrans.csp?pClassName=web.DHCBL.CT.MRCEmergencyCondition&pClassMethod=SaveEntity&pEntityName=web.Entity.CT.MRCEmergencyCondition";
	var OPEN_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.MRCEmergencyCondition&pClassMethod=OpenData";
	var DELETE_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.MRCEmergencyCondition&pClassMethod=DeleteData";
	var pagesize_main = Ext.BDP.FunLib.PageSize.Main;
 
	/** ***********************************排序******************************** */
	Ext.BDP.FunLib.SortTableName = "User.MRCEmergencyCondition";
	var btnSort = Ext.BDP.FunLib.SortBtn;

	// 初始化“别名”维护面板
	var AliasGrid = new Ext.BDP.grid.AliasEditorGridPanel({
				TableN : "MRC_EmergencyCondition"
			});
	Ext.BDP.FunLib.TableName = "MRC_EmergencyCondition";
    /// 获取查询配置按钮 
	...
	btnhislog.on('click', function(btn, e) {
				...
				if (grid.selModel.hasSelection()) {
					...
					RowID = rows[0].get('EMCRowId');
					Desc = rows[0].get('EMCDesc');
			...
			if (grid.selModel.hasSelection()) {
				Ext.MessageBox.confirm('提示', '确定要删除所选的数据吗?', function(btn) {
					if (btn == 'yes') {
						...
						// 删除所有别名
						AliasGrid.DataRefer = rows[0].get('EMCRowId');
	...
	var WinForm = new Ext.form.FormPanel({
		...
		items : [{
					...
					name : 'EMCCode',
					id : 'EMCCodeF',
					readOnly : Ext.BDP.FunLib.Component.DisableFlag('EMCCodeF'),
					style : Ext.BDP.FunLib
							.ReadonlyStyle(Ext.BDP.FunLib.Component
									.DisableFlag('EMCCodeF')),
					allowBlank : false,
					...
					validator : function(thisText) {
						...
						var className = "web.DHCBL.CT.MRCEmergencyCondition"; // 后台类名称
						var classMethod = "FormValidate"; // 数据重复验证后台函数名
						var id = "";
						if (win.title == '修改') { // 如果窗口标题为"修改",则获取rowid
							var _record = grid.getSelectionModel()
									.getSelected();
							var id = _record.get('EMCRowId'); // 此条数据的rowid
						}
						var flag = "";
					...
					name : 'EMCDesc',
					id : 'EMCDescF',
					readOnly : Ext.BDP.FunLib.Component.DisableFlag('EMCDescF'),
					style : Ext.BDP.FunLib
							.ReadonlyStyle(Ext.BDP.FunLib.Component
									.DisableFlag('EMCDescF')),
					allowBlank : false,
					validationEvent : 'blur',
					validator : function(thisText) {
						if (thisText == "") { // 当文本框里的内容为空的时候不用此验证
	...
	var win = new Ext.Window({
		...
		listeners : {
			"show" : function() {
				Ext.getCmp("form-save").getForm().findField("EMCCode").focus(
						true, 800);
			},
	...
	var btnEditwin = new Ext.Toolbar.Button({
				...
				handler : UpdateData = function() {
					...
					if (!_record) {
						...
						tabs.setActiveTab(0);
						// 加载别名面板
						AliasGrid.DataRefer = _record.get('EMCRowId');
						AliasGrid.loadGrid();
					}
	...
	btnTrans.on("click", function() {
				if (grid.selModel.hasSelection()) {
					var _record = grid.getSelectionModel().getSelected();
					var selectrow = _record.get('EMCRowId');
				} else {
					var selectrow = ""
				}
