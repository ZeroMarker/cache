...
Ext.onReady(function() {
	...
	var QUERY_ACTION_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.CT.PACAdmReason&pClassQuery=GetList";
	var SAVE_ACTION_URL = "../csp/dhc.bdp.ext.entitydatatrans.csp?pClassName=web.DHCBL.CT.PACAdmReason&pClassMethod=SaveEntity&pEntityName=web.Entity.CT.PACAdmReason";
	var OPEN_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.PACAdmReason&pClassMethod=OpenData";
	var DELETE_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.PACAdmReason&pClassMethod=DeleteData";
	var pagesize_main = Ext.BDP.FunLib.PageSize.Main;
	
	
	/** ***********************************排序******************************** */
	Ext.BDP.FunLib.SortTableName = "User.PACAdmReason";
	var btnSort = Ext.BDP.FunLib.SortBtn;

	// 初始化“别名”维护面板
	var AliasGrid = new Ext.BDP.grid.AliasEditorGridPanel({
		TableN : "PAC_AdmReason"
	});
	Ext.BDP.FunLib.TableName = "PAC_AdmReason";
    /// 获取查询配置按钮 
	...
	btnhislog.on('click', function(btn, e) {
				...
				if (grid.selModel.hasSelection()) {
					...
					RowID = rows[0].get('REARowId');
					Desc = rows[0].get('READesc');
    ...
    HospWinButton.on("click" , function(){
           if (grid.selModel.hasSelection()) { //选中一条数据数据时，弹出 数据与医院关联窗口
				var rowid=grid.getSelectionModel().getSelections()[0].get("REARowId");
			...
			if (grid.selModel.hasSelection()) {
				Ext.MessageBox.confirm('提示', '确定要删除所选的数据吗?', function(btn) {
					if (btn == 'yes') {
						...
						// 删除所有别名
						AliasGrid.DataRefer = rows[0].get('REARowId');
	...
	var WinForm = new Ext.form.FormPanel({
		...
		reader : new Ext.data.JsonReader({
					root : 'list'
							...
							name : 'REANationalCode',
							mapping : 'REANationalCode',
							type : 'string'
						}, {
							name : 'REAAdmSource',
							mapping : 'REAAdmSource',
							type : 'string'
						}]),
		defaults : {
			anchor : '92%',
		...
		items : [{
					...
					name : 'REACode',
					id : 'REACodeF',
					readOnly : Ext.BDP.FunLib.Component.DisableFlag('REACodeF'),
					style : Ext.BDP.FunLib
							.ReadonlyStyle(Ext.BDP.FunLib.Component
									.DisableFlag('REACodeF')),
					allowBlank : false,
					...
					validator : function(thisText) {
						...
						var className = "web.DHCBL.CT.PACAdmReason"; // 后台类名称
						var classMethod = "FormValidate"; // 数据重复验证后台函数名
						var id = "";
						if (win.title == '修改') { // 如果窗口标题为"修改",则获取rowid
							var _record = grid.getSelectionModel()
									.getSelected();
							var id = _record.get('REARowId'); // 此条数据的rowid
						}
						var flag = "";
					...
					name : 'READesc',
					id : 'READescF',
					readOnly : Ext.BDP.FunLib.Component.DisableFlag('READescF'),
					style : Ext.BDP.FunLib
							.ReadonlyStyle(Ext.BDP.FunLib.Component
									.DisableFlag('READescF')),
					allowBlank : false,
					validationEvent : 'blur',
					validator : function(thisText) {
						if (thisText == "") { // 当文本框里的内容为空的时候不用此验证
					...
					fieldLabel : '住院NationalCode',
					labelSeparator : "",
					name : 'REANationalCode',
					id : 'REANationalCodeF',
					readOnly : Ext.BDP.FunLib.Component
							.DisableFlag('REANationalCodeF'),
					style : Ext.BDP.FunLib
							.ReadonlyStyle(Ext.BDP.FunLib.Component
									.DisableFlag('REANationalCodeF'))
				}, {
					fieldLabel : '门诊AdmSource',
					labelSeparator : "",
					name : 'REAAdmSource',
					id : 'REAAdmSourceF',
					readOnly : Ext.BDP.FunLib.Component
							.DisableFlag('REAAdmSourceF'),
					style : Ext.BDP.FunLib
							.ReadonlyStyle(Ext.BDP.FunLib.Component
									.DisableFlag('REAAdmSourceF'))
				}, {
	...
	var win = new Ext.Window({
		...
		listeners : {
			"show" : function() {
				Ext.getCmp("form-save").getForm().findField("REACode").focus(
						true, 800);
			},
	...
	var btnEditwin = new Ext.Toolbar.Button({
				...
				handler : UpdateData = function() {
					...
					if (!_record) {
						...
						tabs.setActiveTab(0);
						// 加载别名面板
						AliasGrid.DataRefer = _record.get('REARowId');
						AliasGrid.loadGrid();
					}
	...
	var tbbutton = new Ext.Toolbar({
				enableOverflow : true,
				items : [btnAddwin, '-', btnEditwin, '-', btnDel, '-', HospWinButton ,'-', btnSort,'-',btnTrans, '->', btnlog, '-', btnhislog]   ///多院区医院
			});
	/** 搜索按钮 */
	...
	var GridCM = [
			new Ext.grid.CheckboxSelectionModel(), // 单选列
			{
				...
				header : '住院NationalCode',
				sortable : true,
				dataIndex : 'REANationalCode'
			}, {
				header : '门诊AdmSource',
				sortable : true,
				dataIndex : 'REAAdmSource'
			}, {
				header : '开始日期',
				sortable : true,
				renderer : Ext.util.Format.dateRenderer(BDPDateFormat),
				dataIndex : 'READateFrom'
			}, {
				header : '结束日期',
				sortable : true,
				renderer : Ext.util.Format.dateRenderer(BDPDateFormat),
	...
	btnTrans.on("click", function() {
				if (grid.selModel.hasSelection()) {
					var _record = grid.getSelectionModel().getSelected();
					var selectrow = _record.get('REARowId');
				} else {
					var selectrow = ""
				}
