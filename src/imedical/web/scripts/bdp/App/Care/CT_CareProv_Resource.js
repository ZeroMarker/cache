...
/// 编写日期: 2012-11-19
	...
	var BindingLoc1="../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.CT.CTLoc&pClassQuery=GetDataForCmb1";
	var OPEN_Resource_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.RBResource&pClassMethod=OpenData";
	 
	/**----------------------------------根据医护人员判断其医护人员类型，再控制科室列表-------------------------------------**/	
    ...
    var RBCTLOCStore=new Ext.data.JsonStore({
			url:BindingLoc1,
			autoLoad: true,
			root: 'data',
			totalProperty: 'total',
        	 	...
        	 	Ext.Msg.show({
					...
					Ext.Ajax.request({
						...
						params:{
							...
							'RESCTPCPDR':Ext.getCmp("hidden_RESCTPCPDR").getValue(),
							'RESDesc':Ext.getCmp("hidden_RESDesc").getValue(),
							'RESCTLOCDR':Ext.getCmp("comboxResource").getValue()
						},
							...
							if(success){
								...
								if(jsonData.success == 'true'){
									Ext.Msg.show({
										...
										buttons:Ext.Msg.OK,
										fn:function(btn){
											gridResource.getStore().load({params:{start:0, limit:pagesize_Resource,Code:Ext.getCmp("hidden_RESCode").getValue()}});
										}
									});
		...
		handler : function() {
			winResource.setTitle('添加');
 			winResource.setIconClass('icon-add');
   			...
   			Ext.getCmp("RESCTPCPDR").setValue(Ext.getCmp("hidden_RESCTPCPDR").getValue());;
						...
						if(success){
							...
							if(jsonData.success == 'true'){
								Ext.Msg.show({
									...
									buttons:Ext.Msg.OK,
									fn : function(btn){
										Ext.BDP.FunLib.DelForTruePage(gridResource,pagesize_Resource);
									}
								});
							}
							else{
    ...
    var tbResource=new Ext.Toolbar({
        ...
        items:[
            ...
            ResourceDel,
            {
				xtype: 'textfield',
				id: 'hidden_RESCode',
				disabled : Ext.BDP.FunLib.Component.DisableFlag('hidden_RESCode'),
				hidden : true
			},
			{
				xtype: 'textfield',
				id: 'hidden_RESDesc',
				disabled : Ext.BDP.FunLib.Component.DisableFlag('hidden_RESDesc'),
				hidden : true
			},
			{
				xtype: 'textfield',
	...
	var RESCTPCPDR=new Ext.BDP.FunLib.Component.TextField({ 
	    fieldLabel: 'RESCTPCPDR',
	    id:'RESCTPCPDR',
	...
	var RESCTLOCDR = new Ext.BDP.Component.form.ComboBox({            //----------CTCPTDesc   科室类型  combo		
    ...
    columns: [
		...
		{ header: '描述', width: 100, sortable: true, dataIndex: 'RESDesc' },
...
Ext.BDP.FunLib.ShowUserHabit(gridResource,"User.RBResourcesCareProv");
