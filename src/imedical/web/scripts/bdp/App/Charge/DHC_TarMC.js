...
Ext.onReady(function() {
    ...
    var ACTION_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.CT.DHCTarMC&pClassQuery=GetList";
    var SAVE_ACTION_URL_New ="../csp/dhc.bdp.ext.entitydatatrans.csp?pClassName=web.DHCBL.CT.DHCTarMC&pClassMethod=SaveEntity&pEntityName=web.Entity.CT.DHCTarMC";
    var DELETE_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.DHCTarMC&pClassMethod=DeleteData";
    var OPEN_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.DHCTarMC&pClassMethod=OpenData";
   
    /*************************************排序*********************************/
    Ext.BDP.FunLib.SortTableName = "User.DHCTarMC";
    ...
    //初始化“别名”维护面板
     ...
     Ext.BDP.FunLib.TableName="DHC_TarMC";
     /// 获取查询配置按钮 
   ...
   btnhislog.on('click', function(btn,e){    
       ...
       if (grid.selModel.hasSelection()) {
           ...
           RowID=rows[0].get('TARTMCRowId');
           Desc=rows[0].get('TARTMCDesc');
    ...
    HospWinButton.on("click" , function(){
           if (grid.selModel.hasSelection()) { //选中一条数据数据时，弹出 数据与医院关联窗口
				var rowid=grid.getSelectionModel().getSelections()[0].get("TARTMCRowId");
 ...
 /************************************删除功能************************************/
    var btnDel = new Ext.Toolbar.Button({
        ...
        var records =  grid.selModel.getSelections();
                ...
                if(recordsLen == 0){
                    ...
                    if (btn == 'yes') {
                        //删除所有别名
                        AliasGrid.DataRefer=records[0].get('TARTMCRowId') ;
  ...
  /***********************JSON 解析**********************************************/
   ...
   var CodeText=new Ext.BDP.FunLib.Component.TextField({
        ...
        anchor: '85%',
        id:'TARTMCCodeF',
        readOnly : Ext.BDP.FunLib.Component.DisableFlag('TARTMCCodeF'),
        style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('TARTMCCodeF')),
        allowBlank : false,
        ...
        validator : function(thisText){
         ...
         var className =  "web.DHCBL.CT.DHCTarMC";   
         var classMethod = "FormValidate"; //数据重复验证后台函数名                             
         var id="";
         if(win.title=='修改'){  
           var _record = grid.getSelectionModel().getSelected();
           var id = _record.get('TARTMCRowId');  
         }
        var flag = "";
   ...
   var DescText= new Ext.BDP.FunLib.Component.TextField({
          ...
          anchor: '85%',
          id:'TARTMCDescF',
          readOnly : Ext.BDP.FunLib.Component.DisableFlag('TARTMCDescF'),
          style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('TARTMCDescF')),
          allowBlank : false,
          enableKeyEvents : true,
          validationEvent : 'blur',
          validator : function(thisText){
        ...
        var className =  "web.DHCBL.CT.DHCTarMC"; //后台类名称
        var classMethod = "FormValidate"; //数据重复验证后台函数名
        var id="";
        if(win.title=='修改'){ 
           var _record = grid.getSelectionModel().getSelected();
...
/************************************增加修改时弹出窗口***********************************/
    var win = new Ext.Window({
        ...
        buttons : [{
            ...
            handler : function() {
            var tempCode = Ext.getCmp("TARTMCCodeF").getValue();
            var tempDesc = Ext.getCmp("TARTMCDescF").getValue();
            if (tempCode=="") {
                Ext.Msg.show({
                                title : '提示',
                                msg : '项目代码不能为空!', 
                                ...
                                fn:function()
                                {
                                    Ext.getCmp("TARTMCCodeF").focus(true,true);
                                }
                            });
                return;
            }
            if (tempDesc=="") {
                Ext.Msg.show({ 
                                ...
                                fn:function()
                                {
                                    Ext.getCmp("TARTMCDescF").focus(true,true);
                                }
                            });
                    return;
            }
        ...
        listeners : {
            "show" : function() {
                Ext.getCmp("TARTMCCodeF").focus(true,300);
            },
            "hide" :  function(){
...
/************************************create the Grid**********************************/
    ...
    var grid = new Ext.grid.GridPanel({
        ...
        btnTrans.on("click",function(){
            if (grid.selModel.hasSelection()) {     
                var _record = grid.getSelectionModel().getSelected();
                var selectrow = _record.get('TARTMCRowId');              
             } else {
                var selectrow="";
             }
      ...
      var loadFormData = function(grid){
            ...
            if (!_record) {
                Ext.Msg.alert('修改操作', '请选择要修改的一项!');
                    ...
                    WinForm.form.load( {
                    url : OPEN_ACTION_URL + '&id='+ _record.get('TARTMCRowId') 
                });
             //激活基本信息面板
               tabs.setActiveTab(0);
               //加载别名面板
               AliasGrid.DataRefer = _record.get('TARTMCRowId');
               AliasGrid.loadGrid();
            }
        };
