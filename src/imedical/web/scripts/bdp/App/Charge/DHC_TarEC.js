...
Ext.onReady(function() {
    ...
    var ACTION_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.CT.DHCTarEC&pClassQuery=GetList";
    var SAVE_ACTION_URL_New ="../csp/dhc.bdp.ext.entitydatatrans.csp?pClassName=web.DHCBL.CT.DHCTarEC&pClassMethod=SaveEntity&pEntityName=web.Entity.CT.DHCTarEC";
    var DELETE_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.DHCTarEC&pClassMethod=DeleteData";
    var OPEN_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.DHCTarEC&pClassMethod=OpenData";
   
    /*************************************排序*********************************/
    Ext.BDP.FunLib.SortTableName = "User.DHCTarEC";
    var btnSort = Ext.BDP.FunLib.SortBtn;
    //初始化“别名”维护面板
     var AliasGrid = new Ext.BDP.grid.AliasEditorGridPanel({
      TableN : "DHC_TarEC"
   });
     Ext.BDP.FunLib.TableName="DHC_TarEC";
     /// 获取查询配置按钮 
   ...
   btnhislog.on('click', function(btn,e){    
       ...
       if (grid.selModel.hasSelection()) {
           ...
           RowID=rows[0].get('TARTECRowId');
           Desc=rows[0].get('TARTECDesc');
   ...
   //多院区医院下拉框
    ...
    HospWinButton.on("click" , function(){
           if (grid.selModel.hasSelection()) { //选中一条数据数据时，弹出 数据与医院关联窗口
				var rowid=grid.getSelectionModel().getSelections()[0].get("TARTECRowId");
 ...
 /************************************删除功能************************************/
    var btnDel = new Ext.Toolbar.Button({
        ...
        var records =  grid.selModel.getSelections();
                ...
                if(recordsLen == 0){
                    ...
                    if (btn == 'yes') {
                        //删除所有别名
                        AliasGrid.DataRefer=records[0].get('TARTECRowId') ;
  ...
  /***********************JSON 解析**********************************************/
   ...
   var CodeText=new Ext.BDP.FunLib.Component.TextField({
        ...
        anchor: '85%',
        id:'TARTECCodeF',
        readOnly : Ext.BDP.FunLib.Component.DisableFlag('TARTECCodeF'),
        style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('TARTECCodeF')),
        allowBlank : false,
        ...
        validator : function(thisText){
         ...
         var className =  "web.DHCBL.CT.DHCTarEC";   
         var classMethod = "FormValidate"; //数据重复验证后台函数名                             
         var id="";
         if(win.title=='修改'){  
           var _record = grid.getSelectionModel().getSelected();
           var id = _record.get('TARTECRowId');  
         }
        var flag = "";
   ...
   var DescText= new Ext.BDP.FunLib.Component.TextField({
        ...
        var className =  "web.DHCBL.CT.DHCTarEC"; //后台类名称
        var classMethod = "FormValidate"; //数据重复验证后台函数名
        var id="";
        if(win.title=='修改'){ 
           var _record = grid.getSelectionModel().getSelected();
...
/************************************增加修改时弹出窗口***********************************/
    var win = new Ext.Window({
        ...
        buttons : [{
            ...
            handler : function() {
            var tempCode = Ext.getCmp("TARTECCodeF").getValue();
            ...
            if (tempCode=="") {
                Ext.Msg.show({
                                ...
                                fn:function()
                                {
                                    Ext.getCmp("TARTECCodeF").focus(true,true);
                                }
                            });
                return;
            }
        ...
        listeners : {
            "show" : function() {
                Ext.getCmp("TARTECCodeF").focus(true,300);
            },
            "hide" :  function(){
...
/************************************create the Grid**********************************/
    ...
    var grid = new Ext.grid.GridPanel({
        ...
        btnTrans.on("click",function(){
            if (grid.selModel.hasSelection()) {     
                var _record = grid.getSelectionModel().getSelected();
                var selectrow = _record.get('TARTECRowId');              
             } else {
                var selectrow="";
             }
      ...
      var loadFormData = function(grid){
            ...
            if (!_record) {
                Ext.Msg.alert('修改操作', '请选择要修改的一项!');
                    ...
                    WinForm.form.load( {
                    url : OPEN_ACTION_URL + '&id='+ _record.get('TARTECRowId') 
                });
             //激活基本信息面板
               tabs.setActiveTab(0);
               //加载别名面板
               AliasGrid.DataRefer = _record.get('TARTECRowId');
               AliasGrid.loadGrid();
            }
        };
