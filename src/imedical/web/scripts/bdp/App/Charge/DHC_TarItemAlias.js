...
Ext.BDP.grid.AliasEditorGridPanel = Ext.extend(Ext.grid.EditorGridPanel,{
	...
	ALIAS_QUERY_ACTION_URL : "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.CT.DHCTarItemAlias&pClassQuery=GetList",
	ALIAS_DELETE_ACTION_URL : "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.DHCTarItemAlias&pClassMethod=DeleteData",
	ALIAS_SAVE_ACTION_URL : "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.DHCTarItemAlias&pClassMethod=SaveAll",
	//ALIAS_SAVE_ACTION_URL :"../csp/dhc.bdp.ext.entitydatatrans.csp?pClassName=web.DHCBL.CT.DHCTarItemAlias&pClassMethod=SaveEntity&pEntityName=web.Entity.CT.DHCTarItemAlias",
	TableN : "DHC_TarItem",
	title : "收费项别名",
	ALIASParRef : "", //表中数据ID
	height : 200,
	...
	addNewRow : function() {
		...
		if (cnt>0){  	 
			var value=this.store.data.items[0].data.TIAAlias;   
			if ((value=="")||(value==null)){
				Ext.Msg.show({
					title:'提示',
					minWidth:240,
	...
	loadGrid : function(){
		if(this.TableN){
			if(this.TIATARIDR){
				this.store.load({
					params : {
						TARIRowId : this.TIATARIDR
					}
				});
				
			    /// 加载国家标准编码数据
			    Ext.BDP.OpenNationalCodeFun(this.TableN,this.TIATARIDR);
			}else{
				Ext.Msg.show({
					title:'提示',
					minWidth:240,
					msg:'AliasEditorGridPanel控件的TIATARIDR属性为空,请查看原因!',
					icon:Ext.Msg.ERROR,
					buttons:Ext.Msg.OK
				});
				return;
			...
			Ext.Msg.show({
					title:'提示',
					minWidth:240,
					msg:'AliasEditorGridPanel控件的TableN属性为空,请设置后加载Grid!',
					icon:Ext.Msg.ERROR,
					buttons:Ext.Msg.OK
				});
				return;
	...
	saveAlias : function(){
		...
		//// 保存国家标准编码
		Ext.BDP.NationalCodeModFun(this.TableN,this.TIATARIDR)
		
		var modiRec = this.store.getModifiedRecords();
		var data = "";
		for(var i=0; i<modiRec.length; i++){
			var RowId = modiRec[i].data['TIARowId'].trim();
			//var TIADesc = modiRec[i].data['TIADesc'].trim();
			var Alias = modiRec[i].data["TIAAlias"].trim();
			var TIATARIDR = this.TIATARIDR;
			if(Alias!=""){
				var dataRow = RowId+"^"+Alias+"^"+TIATARIDR;
				if(data==""){
					data = dataRow;
				}else{
					data += "&#"+dataRow;
