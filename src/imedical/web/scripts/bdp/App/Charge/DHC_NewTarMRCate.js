...
Ext.onReady(function() {
    ...
    var ACTION_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.CT.DHCNewTarMRCate&pClassQuery=GetList";
    var SAVE_ACTION_URL_New ="../csp/dhc.bdp.ext.entitydatatrans.csp?pClassName=web.DHCBL.CT.DHCNewTarMRCate&pClassMethod=SaveEntity&pEntityName=web.Entity.CT.DHCNewTarMRCate";
    var DELETE_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.DHCNewTarMRCate&pClassMethod=DeleteData";
    var OPEN_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.DHCNewTarMRCate&pClassMethod=OpenData";
   
    /*************************************排序*********************************/
    Ext.BDP.FunLib.SortTableName = 'User.DHCTarMCCateNew';
    var btnSort = Ext.BDP.FunLib.SortBtn;
    Ext.BDP.FunLib.TableName="DHC_TarMCCateNew"; 
    //初始化“别名”维护面板
     var AliasGrid = new Ext.BDP.grid.AliasEditorGridPanel({
        TableN : "DHC_TarMCCateNew"
    });
    ...
    /// 获取查询配置按钮 
    var LookUpConfigureBtn = Ext.BDP.FunLib.GetLookUpBtnFun("DHC_TarMCCateNew");  /// 表名
     //////////////////////////////日志查看 ///////////////////////////////////////////////////////////
   ...
   btnhislog.on('click', function(btn,e){    
       ...
       if (grid.selModel.hasSelection()) {
           ...
           RowID=rows[0].get('NTARMCRowId');
           Desc=rows[0].get('NTARMCDesc');
           var classdesc="新病案首页子类";
           var result = tkMakeServerCall("web.DHCBL.BDP.BDPDataChangeLog","SaveLogParams",RowID,Desc,classdesc);        
        }
        else
        {
    ...
    HospWinButton.on("click" , function(){
           if (grid.selModel.hasSelection()) { //选中一条数据数据时，弹出 数据与医院关联窗口
				var rowid=grid.getSelectionModel().getSelections()[0].get("NTARMCRowId");
 ...
 /************************************删除功能************************************/
    var btnDel = new Ext.Toolbar.Button({
        ...
        var records =  grid.selModel.getSelections();
                ...
                if(recordsLen == 0){
                    ...
                    if (btn == 'yes') {
                        //删除所有别名
                        AliasGrid.DataRefer=records[0].get('NTARMCRowId') ;
  ...
  /***********************JSON 解析**********************************************/
   ...
   var CodeText=new Ext.BDP.FunLib.Component.TextField({
        ...
        name : 'NTARMCCode',
        id:'NTARMCCodeF',
        readOnly : Ext.BDP.FunLib.Component.DisableFlag('NTARMCCodeF'),
        style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('NTARMCCodeF')),
        allowBlank : false,
        ...
        validator : function(thisText){
         ...
         var className =  "web.DHCBL.CT.DHCNewTarMRCate";   
         var classMethod = "FormValidate"; //数据重复验证后台函数名                             
         var id="";
         if(win.title=='修改'){  
           var _record = grid.getSelectionModel().getSelected();
           var id = _record.get('NTARMCRowId');  
         }
        var flag = "";
   ...
   var DescText= new Ext.BDP.FunLib.Component.TextField({
        ...
        var className =  "web.DHCBL.CT.DHCNewTarMRCate"; //后台类名称
        var classMethod = "FormValidate"; //数据重复验证后台函数名
        var id="";
        if(win.title=='修改'){ 
           var _record = grid.getSelectionModel().getSelected();
   ...
   //   新病案首页大类 combo  
     var BindingLoc="../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.CT.DHCNewTarMC&pClassQuery=GetDataForCmb1";
     var TAROCTARTOCDRCombo =new Ext.BDP.Component.form.ComboBox({    
        fieldLabel: "<font color=red>*</font>新病案首页大类",
        name: 'NTARMCDR',
        id:'NTARMCDRF',
        disabled : Ext.BDP.FunLib.Component.DisableFlag('NTARMCDRF'),
        hiddenName:'NTARMCDR',//不能与id相同
        forceSelection: true,
        queryParam:"desc",
        triggerAction : 'all',
        selectOnFocus:false,
        ...
        listWidth:250,
        valueField:'NTARTMCRowId',
        displayField:'NTARTMCDesc',
        store:new Ext.data.JsonStore({
            url:BindingLoc,
            root: 'data',
            totalProperty: 'total',
            idProperty: 'NTARTMCRowId',
            fields:['NTARTMCRowId','NTARTMCDesc'],
            remoteSort: true,
            sortInfo: {field: 'NTARTMCRowId', direction: 'ASC'}
        }),
		listeners:{
...
/************************************增加修改时弹出窗口***********************************/
    var win = new Ext.Window({
        ...
        buttons : [{
            ...
            handler : function() {
            var tempCode = Ext.getCmp("NTARMCCodeF").getValue();
            var tempDesc = Ext.getCmp("TARTECDescF").getValue();
            var tempCate = Ext.getCmp('NTARMCDRF').getValue();
            if (tempCode=="") {
                Ext.Msg.show({
                                title : '提示',
                                msg : '项目代码不能为空!', 
                                ...
                                fn:function()
                                {
                                    Ext.getCmp("NTARMCCodeF").focus(true,true);
                                }
                            });
                return;
            }
        ...
        listeners : {
            "show" : function() {
                Ext.getCmp("NTARMCCodeF").focus(true,300);
            },
            "hide" :  function(){
...
/************************************ 将工具条放到一起***********************************/
    var tb = new Ext.Toolbar({
                ...
                items : ['项目代码', {
                    ...
                    } ,'新病案首页大类',TextResource,'-',LookUpConfigureBtn,'-', btnSearch, '-', btnRefresh, '->'
                ],
                listeners : {
                    render : function() {
    ...
    var grid = new Ext.grid.GridPanel({
       ...
       //单击事件（翻译按钮要用到）
        btnTrans.on("click",function(){
            if (grid.selModel.hasSelection()) {     
                var _record = grid.getSelectionModel().getSelected();
                var selectrow = _record.get('NTARMCRowId');              
             } else {
                var selectrow="";
             }
      ...
      var loadFormData = function(grid){
            ...
            if (!_record) {
                Ext.Msg.alert('修改操作', '请选择要修改的一项!');
                    ...
                    WinForm.form.load( {
                    url : OPEN_ACTION_URL + '&id='+ _record.get('NTARMCRowId') 
                });
             //激活基本信息面板
               tabs.setActiveTab(0);
               //加载别名面板
               AliasGrid.DataRefer = _record.get('NTARMCRowId');
               AliasGrid.loadGrid();
            }
        };
