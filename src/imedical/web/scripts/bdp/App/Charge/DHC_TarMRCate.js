...
Ext.onReady(function() {
    ...
    var ACTION_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.CT.DHCTarMRCate&pClassQuery=GetList";
    var SAVE_ACTION_URL_New ="../csp/dhc.bdp.ext.entitydatatrans.csp?pClassName=web.DHCBL.CT.DHCTarMRCate&pClassMethod=SaveEntity&pEntityName=web.Entity.CT.DHCTarMRCate";
    var DELETE_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.DHCTarMRCate&pClassMethod=DeleteData";
    var OPEN_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.DHCTarMRCate&pClassMethod=OpenData";
   
    /*************************************排序*********************************/
    Ext.BDP.FunLib.SortTableName = "User.DHCTarMRCate";
    var btnSort = Ext.BDP.FunLib.SortBtn;
 
    //初始化“别名”维护面板
     var AliasGrid = new Ext.BDP.grid.AliasEditorGridPanel({
      TableN : "DHC_TarMRCate"
     });
     Ext.BDP.FunLib.TableName="DHC_TarMRCate";
     /// 获取查询配置按钮 
   ...
   btnhislog.on('click', function(btn,e){    
       ...
       if (grid.selModel.hasSelection()) {
           ...
           RowID=rows[0].get('TARMCRowId');
           Desc=rows[0].get('TARMCDesc');
    ...
    HospWinButton.on("click" , function(){
           if (grid.selModel.hasSelection()) { //选中一条数据数据时，弹出 数据与医院关联窗口
				var rowid=grid.getSelectionModel().getSelections()[0].get("TARMCRowId");
 ...
 /************************************删除功能************************************/
    var btnDel = new Ext.Toolbar.Button({
        ...
        var records =  grid.selModel.getSelections();
                ...
                if(recordsLen == 0){
                    ...
                    if (btn == 'yes') {
                        //删除所有别名
                        AliasGrid.DataRefer=records[0].get('TARMCRowId') ;
  ...
  /***********************JSON 解析**********************************************/
   ...
   var CodeText=new Ext.BDP.FunLib.Component.TextField({
        ...
        name : 'TARMCCode',
        id:'TARMCCodeF',
        readOnly : Ext.BDP.FunLib.Component.DisableFlag('TARMCCodeF'),
        style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('TARMCCodeF')),
        allowBlank : false,
        ...
        validator : function(thisText){
         ...
         var className =  "web.DHCBL.CT.DHCTarMRCate";   
         var classMethod = "FormValidate"; //数据重复验证后台函数名                             
         var id="";
         if(win.title=='修改'){  
           var _record = grid.getSelectionModel().getSelected();
           var id = _record.get('TARMCRowId');  
         }
        var flag = "";
   ...
   var DescText= new Ext.BDP.FunLib.Component.TextField({
        ...
        var className =  "web.DHCBL.CT.DHCTarMRCate"; //后台类名称
        var classMethod = "FormValidate"; //数据重复验证后台函数名
        var id="";
        if(win.title=='修改'){ 
           var _record = grid.getSelectionModel().getSelected();
   ...
   //   病案首页大类 combo  
     var BindingLoc="../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.CT.DHCTarMC&pClassQuery=GetDataForCmb1";
     var TAROCTARTOCDRCombo =new Ext.BDP.Component.form.ComboBox({    
        fieldLabel: "<font color=red>*</font>病案首页大类",
        name: 'TARMCTARTMCDR',
        id:'TARMCTARTMCDRF',
        disabled : Ext.BDP.FunLib.Component.DisableFlag('TARMCTARTMCDRF'),
        hiddenName:'TARMCTARTMCDR',//不能与id相同
        forceSelection: true,
        queryParam:"desc",
        //triggerAction : 'all',
        selectOnFocus:false,
        ...
        listWidth:250,
        valueField:'TARTMCRowId',
        displayField:'TARTMCDesc',
        store:new Ext.data.JsonStore({
            url:BindingLoc,
            root: 'data',
            totalProperty: 'total',
            idProperty: 'TARTMCRowId',
            fields:['TARTMCRowId','TARTMCDesc'],
            remoteSort: true,
            sortInfo: {field: 'TARTMCRowId', direction: 'ASC'}
        }),
		 listeners:{
...
/************************************增加修改时弹出窗口***********************************/
    var win = new Ext.Window({
        ...
        buttons : [{
            ...
            handler : function() {
            var tempCode = Ext.getCmp("TARMCCodeF").getValue();
            var tempDesc = Ext.getCmp("TARTECDescF").getValue();
            var tempCate= Ext.getCmp('TARMCTARTMCDRF').getValue();
            if (tempCode=="") {
                Ext.Msg.show({
                                title : '提示',
                                msg : '项目代码不能为空!', 
                                ...
                                fn:function()
                                {
                                    Ext.getCmp("TARMCCodeF").focus(true,true);
                                }
                            });
                return;
            }
        ...
        listeners : {
            "show" : function() {
                Ext.getCmp("TARMCCodeF").focus(true,300);
            },
            "hide" :  function(){
...
/************************************ 将工具条放到一起***********************************/
    var tb = new Ext.Toolbar({
                ...
                items : ['项目代码', {
                    ...
                    } ,'病案首页大类',TextResource,'-',LookUpConfigureBtn,'-', btnSearch, '-', btnRefresh, '->'
                ],
                listeners : {
                    render : function() {
    ...
    var grid = new Ext.grid.GridPanel({
       ...
       //单击事件（翻译按钮要用到）
        btnTrans.on("click",function(){
            if (grid.selModel.hasSelection()) {     
                var _record = grid.getSelectionModel().getSelected();
                var selectrow = _record.get('TARMCRowId');              
             } else {
                var selectrow="";
             }
      ...
      var loadFormData = function(grid){
            ...
            if (!_record) {
                Ext.Msg.alert('修改操作', '请选择要修改的一项!');
                    ...
                    WinForm.form.load( {
                    url : OPEN_ACTION_URL + '&id='+ _record.get('TARMCRowId') 
                });
             //激活基本信息面板
               tabs.setActiveTab(0);
               //加载别名面板
               AliasGrid.DataRefer = _record.get('TARMCRowId');
               AliasGrid.loadGrid();
            }
        };
