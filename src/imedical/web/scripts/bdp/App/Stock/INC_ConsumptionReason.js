...
Ext.onReady(function() {
	...
	var ACTION_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.CT.INCConsumptionReason&pClassQuery=GetList";
	var OPEN_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.INCConsumptionReason&pClassMethod=OpenData";
	var SAVE_ACTION_URL_New = "../csp/dhc.bdp.ext.entitydatatrans.csp?pClassName=web.DHCBL.CT.INCConsumptionReason&pClassMethod=SaveEntity&pEntityName=web.Entity.CT.INCConsumptionReason";
	var DELETE_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.INCConsumptionReason&pClassMethod=DeleteData";
				...
				Ext.MessageBox.confirm('提示', '确定要删除所选的数据吗?', function(btn) {
					if (btn == 'yes') {                                   //-------点击确定按钮后继续执行删除操作
						...
						Ext.Ajax.request({	                              	
							...
							params : {									  //-------请求带的参数
								'id' : rows[0].get('CONRRowId')          //-------通过RowId来删除数据
							},
	...
	var WinForm = new Ext.form.FormPanel({
				...
				reader: new Ext.data.JsonReader({root:'list'},
                                        [{name: 'CONRRowId',mapping:'CONRRowId',type:'string'},
                                         {name: 'CONRCode',mapping:'CONRCode',type:'string'},
                                         {name: 'CONRDesc',mapping:'CONRDesc',type:'string'}
                                        ]),
				defaultType : 'textfield',								 //--------统一设定items类型为textfield
				items : [{
							id:'CONRRowId',
							fieldLabel : 'CONRRowId',
							hideLabel : 'True',
							...
							fieldLabel : '<font color=red>*</font>代码',
   							//disabled : Ext.BDP.FunLib.Component.DisableFlag('CONRCodeF'),
   							readOnly : Ext.BDP.FunLib.Component.DisableFlag('CONRCodeF'),
							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('CONRCodeF')),
							name : 'CONRCode',
							allowBlank: false,
                            ...
                            validator : function(thisText){
	                            ...
	                            var className =  "web.DHCBL.CT.INCConsumptionReason";
	                            var classMethod = "FormValidate";                     
	                            var id="";
	                            if(win.title=='修改'){
	                            	var _record = grid.getSelectionModel().getSelected();
	                            	var id = _record.get('CONRRowId');
	                            }
	                            var flag = "";
							...
							id:'CONRDescF',
							maxLength:220,
   							//disabled : Ext.BDP.FunLib.Component.DisableFlag('CONRDescF'),
   							readOnly : Ext.BDP.FunLib.Component.DisableFlag('CONRDescF'),
							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('CONRDescF')),
	...
	var win = new Ext.Window({
		...
		listeners : {
			"show" : function() {
				Ext.getCmp("form-save").getForm().findField("CONRCode").focus(true,300);
	...
	var grid = new Ext.grid.GridPanel({
				...
				//autoExpandColum:'CONRForeignDesc',
				bbar : paging,                                    //-----------------底部状态栏
				tbar : tb,                                        //-----------------顶部状态栏
				stateId : 'grid'
			});
    ...
    var loadFormData = function(grid) {
        ...
        if (!_record) {
            ...
            win.show();
            WinForm.form.load( {
                url : OPEN_ACTION_URL + '&CONRRowId='+ _record.get('CONRRowId'),
                waitMsg : '正在载入数据...',
                success : function(form,action) {
                },
                failure : function(form,action) {
