...
Ext.onReady(function() {
	...
	var ACTION_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.CT.INCSterileCategory&pClassQuery=GetList";
	var OPEN_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.INCSterileCategory&pClassMethod=OpenData";
	var SAVE_ACTION_URL_New = "../csp/dhc.bdp.ext.entitydatatrans.csp?pClassName=web.DHCBL.CT.INCSterileCategory&pClassMethod=SaveEntity&pEntityName=web.Entity.CT.INCSterileCategory";
	var DELETE_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.INCSterileCategory&pClassMethod=DeleteData";
				...
				Ext.MessageBox.confirm('提示', '确定要删除所选的数据吗?', function(btn) {
					if (btn == 'yes') {                                   //-------点击确定按钮后继续执行删除操作
						...
						Ext.Ajax.request({	                              	
							...
							params : {									  //-------请求带的参数
								'id' : rows[0].get('SCATRowId')          //-------通过RowId来删除数据
							},
	...
	var WinForm = new Ext.form.FormPanel({
				...
				reader: new Ext.data.JsonReader({root:'list'},
                                        [{name: 'SCATRowId',mapping:'SCATRowId',type:'string'},
                                         {name: 'SCATDesc',mapping:'SCATDesc',type:'string'}
                                        ]),
				defaultType : 'textfield',								 //--------统一设定items类型为textfield
				items : [{
							id:'SCATRowId',
							fieldLabel : 'SCATRowId',
							hideLabel : 'True',
							...
							id:'SCATDescF',
							maxLength:220,
  						 	//disabled : Ext.BDP.FunLib.Component.DisableFlag('SCATDescF'),
  						 	readOnly : Ext.BDP.FunLib.Component.DisableFlag('SCATDescF'),
							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('SCATDescF')),
						    ...
						    validator : function(thisText){
	                            ...
	                            var className =  "web.DHCBL.CT.INCSterileCategory";
	                            var classMethod = "FormValidate";
	                            var id="";
	                            if(win.title=='修改'){
	                            	var _record = grid.getSelectionModel().getSelected();
	                            	var id = _record.get('SCATRowId');
	                            }
	                            var flag = "";
	...
	var win = new Ext.Window({
		...
		listeners : {
			"show" : function() {
				Ext.getCmp("form-save").getForm().findField("SCATDesc").focus(true,300);
	...
	var grid = new Ext.grid.GridPanel({
				...
				//autoExpandColum:'SCATForeignDesc',
				bbar : paging,                                    //-----------------底部状态栏
				tbar : tb,                                        //-----------------顶部状态栏
				stateId : 'grid'
			});
    ...
    var loadFormData = function(grid) {
        ...
        if (!_record) {
            ...
            win.show();
            WinForm.form.load( {
                url : OPEN_ACTION_URL + '&SCATRowId='+ _record.get('SCATRowId'),
                waitMsg : '正在载入数据...',
                success : function(form,action) {
