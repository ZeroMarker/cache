...
Ext.onReady(function() {
	...
	var ACTION_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.CT.INCReasonForAdjustment&pClassQuery=GetList";
	var OPEN_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.INCReasonForAdjustment&pClassMethod=OpenData";
	var SAVE_ACTION_URL_New = "../csp/dhc.bdp.ext.entitydatatrans.csp?pClassName=web.DHCBL.CT.INCReasonForAdjustment&pClassMethod=SaveEntity&pEntityName=web.Entity.CT.INCReasonForAdjustment";
	var DELETE_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.INCReasonForAdjustment&pClassMethod=DeleteData";
				...
				Ext.MessageBox.confirm('提示', '确定要删除所选的数据吗?', function(btn) {
					if (btn == 'yes') {                                   //-------点击确定按钮后继续执行删除操作
						...
						Ext.Ajax.request({	                              	
							...
							params : {									  //-------请求带的参数
								'id' : rows[0].get('ADJRowId')          //-------通过RowId来删除数据
							},
	...
	var WinForm = new Ext.form.FormPanel({
				...
				reader: new Ext.data.JsonReader({root:'list'},           //-------使用open方法要在form中加上数据的映射
                                        [{name: 'ADJRowId',mapping:'ADJRowId',type:'string'},
                                         {name: 'ADJCode',mapping:'ADJCode',type:'string'},
                                         {name: 'ADJDesc',mapping:'ADJDesc',type:'string'}
                                        ]),
				...
				items : [{
							...
							name : 'ADJCode',
							maxLength:15,
   							//disabled : Ext.BDP.FunLib.Component.DisableFlag('ADJCodeF'),
   							readOnly : Ext.BDP.FunLib.Component.DisableFlag('ADJCodeF'),
							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('ADJCodeF')),
							allowBlank: false,
                            ...
                            validator : function(thisText){
	                            ...
	                            var className =  "web.DHCBL.CT.INCReasonForAdjustment";
	                            var classMethod = "FormValidate";                     
	                            var id="";
	                            if(win.title=='修改'){
	                            	var _record = grid.getSelectionModel().getSelected();
	                            	var id = _record.get('ADJRowId');
	                            }
	                            var flag = "";
							...
							id:'ADJDescF',
							maxLength:220,
   							//disabled : Ext.BDP.FunLib.Component.DisableFlag('ADJDescF'),
   							readOnly : Ext.BDP.FunLib.Component.DisableFlag('ADJDescF'),
							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('ADJDescF')),
	...
	var win = new Ext.Window({
		...
		listeners : {
			"show" : function() {
				Ext.getCmp("form-save").getForm().findField("ADJCode").focus(true,300);
	...
	var grid = new Ext.grid.GridPanel({
				...
				//autoExpandColum:'ADJForeignDesc',
				bbar : paging,                                    //-----------------底部状态栏
				tbar : tb,                                        //-----------------顶部状态栏
				stateId : 'grid'
			});
    ...
    var loadFormData = function(grid) {
        ...
        if (!_record) {
            ...
            win.show();
            WinForm.form.load( {
                url : OPEN_ACTION_URL + '&ADJRowId='+ _record.get('ADJRowId'),
                waitMsg : '正在载入数据...',
                success : function(form,action) {
                },
                failure : function(form,action) {
