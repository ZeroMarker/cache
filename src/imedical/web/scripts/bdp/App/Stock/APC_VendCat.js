...
Ext.onReady(function() {
	...
	var ACTION_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.CT.APCVendCat&pClassQuery=GetList";
	var OPEN_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.APCVendCat&pClassMethod=OpenData";
	var SAVE_ACTION_URL_New = "../csp/dhc.bdp.ext.entitydatatrans.csp?pClassName=web.DHCBL.CT.APCVendCat&pClassMethod=SaveEntity&pEntityName=web.Entity.CT.APCVendCat";
	var DELETE_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.APCVendCat&pClassMethod=DeleteData";
	
	var pagesize = Ext.BDP.FunLib.PageSize.Main;

	var APCVC_CoCode_DR_QUERY_ACTION_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.CT.APCVendCat&pClassQuery=GetCTCODR";
				...
				Ext.MessageBox.confirm('提示', '确定要删除所选的数据吗?', function(btn) {
					if (btn == 'yes') {                                   //-------点击确定按钮后继续执行删除操作
						...
						Ext.Ajax.request({	                              	
							...
							params : {									  //-------请求带的参数
								'id' : rows[0].get('APCVCRowId')          //-------通过RowId来删除数据
							},
	...
	var WinForm = new Ext.form.FormPanel({
				...
				reader: new Ext.data.JsonReader({root:'list'},
                                        [{name: 'APCVCRowId',mapping:'APCVCRowId',type:'string'},
                                         {name: 'APCVCCoCodeDR',mapping:'APCVCCoCodeDR',type:'string'},
                                         {name: 'APCVCCode',mapping:'APCVCCode',type:'string'},
                                         {name: 'APCVCDesc',mapping:'APCVCDesc',type:'string'}
                                        ]),
				defaultType : 'textfield',								 //--------统一设定items类型为textfield
				items : [{
							id:'APCVCRowId',
							fieldLabel : 'APCVCRowId',
							hideLabel : 'True',
							...
							id:'APCVCCodeF',
							maxLength:15,
   							//disabled : Ext.BDP.FunLib.Component.DisableFlag('APCVCCodeF'),
   							readOnly : Ext.BDP.FunLib.Component.DisableFlag('APCVCCodeF'),
							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('APCVCCodeF')),
                            ...
                            validator : function(thisText){
	                            ...
	                            var className =  "web.DHCBL.CT.APCVendCat";
	                            var classMethod = "FormValidate";                     
	                            var id="";
	                            if(win.title=='修改'){
	                            	var _record = grid.getSelectionModel().getSelected();
	                            	var id = _record.get('APCVCRowId');
	                            }
	                            var flag = "";
							...
							id:'APCVCDescF',
							maxLength:220,
							//disabled : Ext.BDP.FunLib.Component.DisableFlag('APCVCDescF'),
							readOnly : Ext.BDP.FunLib.Component.DisableFlag('APCVCDescF'),
							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('APCVCDescF')),
							...
							hiddenName : 'APCVCCoCodeDR',
							//disabled : Ext.BDP.FunLib.Component.DisableFlag('APCVCCoCodeDR'),
							readOnly : Ext.BDP.FunLib.Component.DisableFlag('APCVCCoCodeDR'),
							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('APCVCCoCodeDR')),
							//id :'APCVCCoCodeDR',
							store : new Ext.data.Store({
										autoLoad: true,
										proxy : new Ext.data.HttpProxy({ url : APCVC_CoCode_DR_QUERY_ACTION_URL }),
										reader : new Ext.data.JsonReader({
										totalProperty : 'total',
										root : 'data',
										successProperty : 'success'
									}, [{ name:'CTCORowId',mapping:'CTCORowId'},
										{name:'CTCODesc',mapping:'CTCODesc'} ])
								}),
							mode : 'local',
							shadow:false,
							//queryParam : 'desc',
							...
							triggerAction : 'all',
							//hideTrigger: false,
							displayField : 'CTCODesc',
							valueField : 'CTCORowId'
							//typeAhead : true,
							//minChars : 1,
							//listWidth : 150,
							//editable: false
	...
	var win = new Ext.Window({
		...
		listeners : {
			"show" : function() {
				Ext.getCmp("form-save").getForm().findField("APCVCCode").focus(true,300);
	...
	var grid = new Ext.grid.GridPanel({
				...
				//autoExpandColum:'APCVCForeignDesc',
				bbar : paging,                                    //-----------------底部状态栏
				tbar : tb,                                        //-----------------顶部状态栏
				stateId : 'grid'
			});
    ...
    var loadFormData = function(grid) {
        ...
        if (!_record) {
            ...
            win.show();
            WinForm.form.load( {
                url : OPEN_ACTION_URL + '&APCVCRowId='+ _record.get('APCVCRowId'),
                waitMsg : '正在载入数据...',
                success : function(form,action) {
                },
                failure : function(form,action) {
