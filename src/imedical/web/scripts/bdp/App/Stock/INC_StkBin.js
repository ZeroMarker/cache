...
Ext.onReady(function() {
	...
	var ACTION_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.CT.INCStkBin&pClassQuery=GetList";
	var OPEN_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.INCStkBin&pClassMethod=OpenData";
	var SAVE_ACTION_URL_New = "../csp/dhc.bdp.ext.entitydatatrans.csp?pClassName=web.DHCBL.CT.INCStkBin&pClassMethod=SaveEntity&pEntityName=web.Entity.CT.INCStkBin";
	var DELETE_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.INCStkBin&pClassMethod=DeleteData";
				...
				Ext.MessageBox.confirm('提示', '确定要删除所选的数据吗?', function(btn) {
					if (btn == 'yes') {                                   //-------点击确定按钮后继续执行删除操作
						...
						Ext.Ajax.request({	                              	
							...
							params : {									  //-------请求带的参数
								'id' : rows[0].get('INCSBRowId')          //-------通过RowId来删除数据
							},
	...
	var WinForm = new Ext.form.FormPanel({
				...
				reader: new Ext.data.JsonReader({root:'list'},
                                        [{name: 'INCSBRowId',mapping:'INCSBRowId',type:'string'},
                                         {name: 'INCSBCode',mapping:'INCSBCode',type:'string'},
                                         {name: 'INCSBDesc',mapping:'INCSBDesc',type:'string'}
                                        ]),
				defaultType : 'textfield',								 //--------统一设定items类型为textfield
				items : [{
							id:'INCSBRowId',
							fieldLabel : 'INCSBRowId',
							hideLabel : 'True',
							...
							name : 'INCSBCode',
							id:'INCSBCodeF',
   							//disabled : Ext.BDP.FunLib.Component.DisableFlag('INCSBCodeF'),
   							readOnly : Ext.BDP.FunLib.Component.DisableFlag('INCSBCodeF'),
							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('INCSBCodeF')),
							allowBlank: false,
                            ...
                            validator : function(thisText){
	                            ...
	                            var className =  "web.DHCBL.CT.INCStkBin";
	                            var classMethod = "FormValidate";                     
	                            var id="";
	                            if(win.title=='修改'){
	                            	var _record = grid.getSelectionModel().getSelected();
	                            	var id = _record.get('INCSBRowId');
	                            }
	                            var flag = "";
							...
							id:'INCSBDescF',
							maxLength:220,
   							//disabled : Ext.BDP.FunLib.Component.DisableFlag('INCSBDescF'),
   							readOnly : Ext.BDP.FunLib.Component.DisableFlag('INCSBDescF'),
							style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('INCSBDescF')),
							allowBlank: false,
							enableKeyEvents:true, 
						    validationEvent : 'blur',  
						    validator : function(thisText){
	...
	var win = new Ext.Window({
		...
		listeners : {
			"show" : function() {
				Ext.getCmp("form-save").getForm().findField("INCSBCode").focus(true,300);
	...
	var grid = new Ext.grid.GridPanel({
				...
				//autoExpandColum:'INCSBForeignDesc',
				bbar : paging,                                    //-----------------底部状态栏
				tbar : tb,                                        //-----------------顶部状态栏
				stateId : 'grid'
			});
    ...
    var loadFormData = function(grid) {
        ...
        if (!_record) {
            ...
            win.show();
            WinForm.form.load( {
                url : OPEN_ACTION_URL + '&INCSBRowId='+ _record.get('INCSBRowId'),
                waitMsg : '正在载入数据...',
                success : function(form,action) {
                },
                failure : function(form,action) {
