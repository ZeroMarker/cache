...
Ext.onReady(function() {
	...
	var ACTION_URL = "../csp/dhc.bdp.ext.querydatatrans.csp?pClassName=web.DHCBL.CT.RBCEquipmentGroup&pClassQuery=GetList";
	var SAVE_ACTION_URL_New = "../csp/dhc.bdp.ext.entitydatatrans.csp?pClassName=web.DHCBL.CT.RBCEquipmentGroup&pClassMethod=SaveEntity&pEntityName=web.Entity.CT.RBCEquipmentGroup";
	var DELETE_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.RBCEquipmentGroup&pClassMethod=DeleteData";
	var OPEN_ACTION_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.CT.RBCEquipmentGroup&pClassMethod=OpenData";

	/*************************************排序*********************************/
    Ext.BDP.FunLib.SortTableName = "User.RBCEquipmentGroup";
    var btnSort = Ext.BDP.FunLib.SortBtn;
    
    //初始化“别名”维护面板
     var AliasGrid = new Ext.BDP.grid.AliasEditorGridPanel({
       TableN : "RBC_EquipmentGroup"
   }); 
    Ext.BDP.FunLib.TableName="RBC_EquipmentGroup";
    /// 获取查询配置按钮 
   ...
   if (grid.selModel.hasSelection()) {
       ...
       RowID=rows[0].get('GRPRowId');
       Desc=rows[0].get('GRPDesc');
   ...
   //多院区医院下拉框
    ...
    HospWinButton.on("click" , function(){
           if (grid.selModel.hasSelection()) { //选中一条数据数据时，弹出 数据与医院关联窗口
				var rowid=grid.getSelectionModel().getSelections()[0].get("GRPRowId");
 ...
 /*************************** 删除功能*************************************************/
		...
		handler : DelData=function() {
			var records =  grid.selModel.getSelections();
				...
				if(recordsLen == 0){
					...
					if (btn == 'yes') {
                         //删除所有别名
                        AliasGrid.DataRefer=records[0].get('GRPRowId') ;
    ...
    var jsonReaderE=new Ext.data.JsonReader({root:'list'},
                        [{name: 'GRPRowId',mapping:'GRPRowId',type:'int'},
                         {name: 'GRPCode',mapping:'GRPCode',type:'string'},
                         {name: 'GRPDesc',mapping:'GRPDesc',type:'string'},
                         {name: 'GRPDateFrom',mapping:'GRPDateFrom',type:'string'},
                         {name: 'GRPDateTo',mapping:'GRPDateTo',type:'string'}                 
                ]);
    ...
    var CodeText=new Ext.BDP.FunLib.Component.TextField({
	       ...
	       name :'GRPCode',
	       id:'GRPCodeF',
	       readOnly : Ext.BDP.FunLib.Component.DisableFlag('GRPCodeF'),
	       style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('GRPCodeF')),
           ...
           validator : function(thisText){
              ...
              var className =  "web.DHCBL.CT.RBCEquipmentGroup";   
              var classMethod = "FormValidate"; //数据重复验证后台函数名                             
              var id="";
              if(win.title=='修改'){  
                 var _record = grid.getSelectionModel().getSelected();
                 var id = _record.get('GRPRowId');  
               }
             var flag = "";
    ...
    var DescText=new Ext.BDP.FunLib.Component.TextField({
       ...
       name : 'GRPDesc',
       id:'GRPDescF',
       readOnly : Ext.BDP.FunLib.Component.DisableFlag('GRPDescF'),
       style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('GRPDescF')),
       enableKeyEvents : true,
       validationEvent : 'blur',
       validator : function(thisText){
       if(thisText==""){  
       ...
       var className =  "web.DHCBL.CT.RBCEquipmentGroup"; //后台类名称
       var classMethod = "FormValidate"; //数据重复验证后台函数名
       var id="";
       if(win.title=='修改'){ 
        var _record = grid.getSelectionModel().getSelected();
    ...
    var DateFromText=new Ext.BDP.FunLib.Component.DateField({
       ...
       name :'GRPDateFrom',
       id:'GRPDateFromF',
       readOnly : Ext.BDP.FunLib.Component.DisableFlag('GRPDateFromF'),
       style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('GRPDateFromF')),
    ...
    var DateToText=new Ext.BDP.FunLib.Component.DateField({
       ...
       name : 'GRPDateTo',
       id:'GRPDateToF',
       readOnly : Ext.BDP.FunLib.Component.DisableFlag('GRPDateToF'),
       style:Ext.BDP.FunLib.ReadonlyStyle(Ext.BDP.FunLib.Component.DisableFlag('GRPDateToF')),
       format : BDPDateFormat,
       enableKeyEvents : true,
       listeners : {   
        'keyup' : function(field, e)
...
Ext.BDP.AddTabpanelFun(tabs,Ext.BDP.FunLib.TableName);
	...
	var win = new Ext.Window({
		...
		buttons : [{
			...
			handler : function() {
			var tempCode = Ext.getCmp("form-save").getForm().findField("GRPCode").getValue();
			var tempDesc = Ext.getCmp("form-save").getForm().findField("GRPDesc").getValue();
			var startDate = Ext.getCmp("form-save").getForm().findField("GRPDateFrom").getValue();
			var endDate = Ext.getCmp("form-save").getForm().findField("GRPDateTo").getValue();
			if (tempCode=="") {
				Ext.Msg.show({
								title : '<font color=blue>提示</font>', 
								...
								fn:function()
								{
									Ext.getCmp("form-save").getForm().findField("GRPCode").focus(true,true);
								}
							 });
      			return;
			}
			if (tempDesc=="") {
				Ext.Msg.show({ 
								...
								fn:function()
								{
									Ext.getCmp("form-save").getForm().findField("GRPDesc").focus(true,true);
								}
						   });
      			return;
			}
			if (startDate=="") {
				Ext.Msg.show({ 
								...
								fn:function()
								{
									Ext.getCmp("form-save").getForm().findField("GRPDateFrom").focus(true,true);
								}
							});
      			return;
			}
		...
		listeners : {
			"show" : function() {
			Ext.getCmp("form-save").getForm().findField("GRPCode").focus(true,300);
			},
			"hide" : function(){
...
/************************************create the Grid *************************************/
 ...
 /****************************************右键菜单的关键代码，右键可以选中一行***************************************************/
  ...
  btnTrans.on("click",function(){
	...
	{
	  var _record=grid.getSelectionModel().getSelected();
          var selectrow=_record.get('GRPRowId');
	 }
	else
	{
	  var selectrow=""
	  ...
	  var loadFormData = function(grid){
	        ...
	        if (!_record) {
	            Ext.Msg.alert('修改操作', '请选择要修改的一项!');
	        		...
	        		WinForm.form.load( {
	                url : OPEN_ACTION_URL + '&sid='+ _record.get('GRPRowId'),  //id 对应OPEN里的入参
	                success : function(form,action) {
               ...
               tabs.setActiveTab(0);
               //加载别名面板
               AliasGrid.DataRefer = _record.get('GRPRowId');
               AliasGrid.loadGrid();
	        }
	    };
