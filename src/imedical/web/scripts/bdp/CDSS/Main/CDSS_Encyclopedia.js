...
$(function () {
	...
	var MInterface = "http://192.144.152.252/imedical/web";
	var termdrDesc = CDSSMakeServerCall("web.DHCBL.MKB.MKBTerm[A]GetDesc[A]"+termdr);
	String.addSpace = function (str) {
			 return str.split("").join(" ");
	};
	termdrDesc = String.addSpace(termdrDesc);

	var textData = CDSSMakeServerCall("web.DHCBL.MKB.MKBEncyclopedia[A]GetTermInfo[A]" + termdr);
	if (textData) { 
	...
	setTimeout(function () {  //加延迟解决首页属性标题栏出不来的问题
		// $.ajax({
        //     url:MInterface+"/csp/dhc.bdp.cdss.queryforcdss.csp?CacheNoRedirect=1&CacheUserName=dhsyslogin&CachePassword=1q2w3e4r%T6y7u8i9o0p&pClassName=web.DHCBL.MKB.MKBEncyclopedia&pClassQuery=GetList&termdr="+encodeURIComponent(termdr),
        //     type: "GET",
        //     //dataType: "json",
		// 	success: function (data) {   
				var data = CDSSMakeServerCall("web.CDSS.Public.MethodForEncyclopedia[A]GetList[A]"+termdr);
				...
				for (var j = 0, len = jsonData.rows.length; j < len; j++) {
					...
					var extendInfo = CDSSMakeServerCall("web.DHCBL.MKB.MKBTermProDetail[A]getExtendInfo[A]"+propertyid );
					...
					if (type == "L") {
						...
						//var QUERY_ACTION_URL = MInterface+"/csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.MKB.MKBTermProDetail&pClassMethod=GetMyList";
						//列表datagrid
						...
						var dataInfo = CDSSMakeServerCall("web.CDSS.Public.MethodForEncyclopedia[A]GetMyList[A]" + propertyid);
						if (dataInfo) { 
						...
						$("#div" + j).append(iframhtml);
						//var QUERY_ACTION_URL = MInterface+"/csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.MKB.MKBEncyclopedia&pClassMethod=GetJsonList";
						//列表datagrid
						...
						var dataInfo = CDSSMakeServerCall("web.CDSS.Public.MethodForEncyclopedia[A]GetJsonList[A][A]" + propertyid);
						if (dataInfo) { 
						...
						var tmpdesc = CDSSMakeServerCall("web.DHCBL.MKB.MKBEncyclopedia[A]GetDetailDesc[A]"+propertyid+"[A]"+type );
							...
							},
							{
								...
								formatter: function (v, row, index) {
									if (v) {
										...
										var showvalue = CDSSMakeServerCall("web.DHCBL.MKB.MKBTermProDetail[A]GetDesc[A]"+v );
										return showvalue
									} else {
										return "暂无";
									}
						...
						var plistdata = CDSSMakeServerCall("web.CDSS.Public.MethodForEncyclopedia[A]GetSelPropertyList[A]"+propertyid);
						var plistdata = eval('(' + plistdata + ')');
						$("#plist" + j).datagrid('loadData',plistdata)
						/*$.ajax({
							url: MInterface + "/csp/dhc.bdp.cdss.queryforcdss.csp?CacheNoRedirect=1&CacheUserName=dhsyslogin&CachePassword=1q2w3e4r%T6y7u8i9o0p&pClassName=web.DHCBL.MKB.MKBTermProDetail&pClassQuery=GetSelPropertyList&property="+ propertyid,
							type: "GET",
							//dataType: "json",
							success: function (data) {
								var dd = eval('(' + data + ')');
						...
						var configListOrTree = CDSSMakeServerCall("web.DHCBL.MKB.MKBTermProDetail[A]GetProListOrTree[A]"+propertyid );
						if (configListOrTree == "T") {
							iframhtml = '<table id="treeterm' + j + '" data-options="fit:false,border:false"></table>';
							$("#div" + j).append(iframhtml);
							$HUI.tree('#treeterm' + j, {
								//url: MInterface+"/csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.MKB.MKBTermProDetail&pClassMethod=GetDocSourseTreeJson&property=" + propertyid,
								data:[],
							...
							var dataInfo = CDSSMakeServerCall("web.CDSS.Public.MethodForEncyclopedia[A]]GetDocSourseTreeJson[A]"+ propertyid);
							if (dataInfo) { 
							...
							var listdata = CDSSMakeServerCall("web.CDSS.Public.MethodForEncyclopedia[A]GetSellistTermList[A]"+propertyid);
							var listdata = eval('(' + listdata + ')');
							$("#listterm" + j).datagrid('loadData',listdata)
	
							/*$.ajax({
								url: MInterface + "/csp/dhc.bdp.cdss.queryforcdss.csp?CacheNoRedirect=1&CacheUserName=dhsyslogin&CachePassword=1q2w3e4r%T6y7u8i9o0p&pClassName=web.DHCBL.MKB.MKBTermProDetail&pClassQuery=GetSellistTermList&property="+ propertyid,
								type: "GET",
								//dataType: "json",
								success: function (data) {
									var dd = eval('(' + data + ')');
						...
						$HUI.tree('#streeterm' + j, {
							//url: MInterface+"/csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.MKB.MKBTermProDetail&pClassMethod=GetSourseSingleTermJson&property=" + propertyid,
							data:[],
							lines: true,  //树节点之间显示线条
						...
						var dataInfo = CDSSMakeServerCall("web.CDSS.Public.MethodForEncyclopedia[A]GetSourseSingleTermJson[A]" + propertyid);
						if (dataInfo) { 
	 ...
	 */
	 ...
	 DirectoryNav.prototype = {
		 ...
		 make:function(){
			 ...
			 $hs.each(function(index){
				 ...
				 if (this.tagName.toLowerCase() == 'h2') {
					 ...
					 if(index1>18) 
					 {
						 ...
						 $("#directoryNav").append('<li class="l1"><span class="c-dot" id="moreendc"></span><a id="moreend">'+"更多目录"+'</a></li>');
						 return false;
			 ...
			 $("#moreend,#moreendc").click(function (e) {
		 ...
		 ifPos: function (st) {
			 //console.log(This)
			//alert(this.$pageNavListLis.filter(".cur").index())
			...
			for(var i=0;i<offArr.length;i++){
				if((offArr[i] - windowHeight) < st) {
					...
					//console.log(tagTop,this.$pageNavListLiH*0.5)
					//var topC = tagTop + this.$pageNavListLiH * 0.5
					//alert($('#directoryNav')[0].offsetTop)
					//var topC = ($('#directoryNav')[0].offsetTop) + (24*i-0)
					if (jumpIndex > 18) jumpIndex = 18
					var topC = parseInt(25) + parseInt(24 * jumpIndex)
					//var topC = 60 + (this.$pageNavListLis.length-i)*24
					this.posTag(topC);
					//console.log(11111,this.$pageNavListLis.filter(".cur").index())
	...
	//点击下载按钮
	 function DownLoadFile(fileName)
	 {
		 if(fileName)
		 {
			 var isExists = CDSSMakeServerCall("web.DHCBL.BDP.BDPUploadFile[A]IsExistsFile[A]scripts\\bdp\\MKB\\Doc\\Doc\\"+fileName);

			 var filepath = MInterface+"/scripts/bdp/MKB/Doc/Doc/" + fileName;
	 ...
	 function previewFile(fileName)
	 {
		 ...
		 if(fileName)
		 {
			 ...
			 var PDFisExists = CDSSMakeServerCall("web.DHCBL.BDP.BDPUploadFile[A]IsExistsFile[A]scripts\\bdp\\MKB\\Doc\\Doc\\" + fileName.replace(fileType, "pdf") );
			 if(PDFisExists==1)
			 {
				 var filepath =MInterface+"/scripts/bdp/MKB/Doc/Doc/"+fileName.replace(fileType,"pdf");
