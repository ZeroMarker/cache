var init = function(){
	...
	var SDSADMNOID="",SDSPMINOID=""; //全局变量就诊号、患者登记号
		...
		$HUI.datagrid("#Form_DiagPropertySelectedGrid",{
			...
			onClickCell:function(rowIndex, field, value){
				if (field=="title"){
					...
					setTimeout(function(){
						var desc=$('#SearchProperty').searchbox('textbox').val() 
						LoadPropertyData($("#SelMKBRowId").val(),SDSRowId,indexTemplate);
						//DiagPropertySearch(desc,indexTemplate)
					},100)
				}else{
					if (value=="") return;
	...
	//诊断查找选择后初始化诊断属性列表页
	LoadPropertyData=function(SDSTermDR,SDSRowId,indexTemplate,desc){
		...
		if (RetStr!=""){
			...
			if ((desc==undefined)||(desc=="undefined")) desc=""
				...
				$(modeJsonInfo).each(function(index,item){
					...
					if (childType=="L"){
						if (isTOrP=="P"){
							var ListRetStr = tkMakeServerCall("web.DHCBL.MKB.MKBTermProDetail","GetListDetailInfo", desc, childId, "0", "1000", SearchMode);
							ListRetStr=eval("("+ListRetStr+")");
							var C_valueField='MKBTPDRowId';  
							var C_textField='comDesc'; //展示名
						}else{ //术语标识
							var ListRetStr = tkMakeServerCall("web.DHCBL.MKB.MKBTermProDetail","GetListTermJsonForDoc", "", childId, desc, "1000", "1", SearchMode);
							ListRetStr=eval("("+ListRetStr+")");
					...
					if (childType=="S"){
						if ((showType=="C")||(showType=="CB")||(showType=="CG")||(showType=="G")) { //引用术语 展示为下拉框
							var ListRetStr = tkMakeServerCall("web.DHCBL.MKB.MKBTermProDetail","GetDocSourseList", childId, showType, desc, SearchMode); 
							ListRetStr=eval("("+ListRetStr+")");
							var C_valueField='MKBTRowId';  
					...
					if (childType=="P"){
						var ListRetStr = tkMakeServerCall("web.DHCBL.MKB.MKBTermProDetail","GetDocPropertyList",childId, desc, SearchMode); 
						ListRetStr=eval("("+ListRetStr+")")
						if (showType=="C") {
					...
					if (showType=="CB"){
						...
						if ((desc!="")&&(CBRetStr.data.length==0)){
							$(tool).css("display","none") //检索没有属性内容时隐藏整行
						}
						var TA_tool="";
					...
					if (showType=="CG"){
						...
						if ((desc!="")&&(CGRetStr.data.length==0)){
							$(tool).css("display","none") //检索没有属性内容时隐藏整行
						}
						var TA_tool="";
					...
					if (showType=="C"){
						...
						if ((desc!="")&&(ListRetStr.data.length==0)){
							$(tool).css("display","none") //检索没有属性内容时隐藏整行
						}
							...
							if (($("#SelSDSSupplement").val()!="")&&(($("#SelSDSSupplement").val()!=undefined))){
								...
								var tree=$HUI.tree("#"+childId+"_T"+isTOrP+"_"+treeNode+"",{
												...
												if(WikiPermission=="Y"){ //是否有权限
													$('#NodeMenu').menu('appendItem', {
														...
														onclick:function(){
															var height=parseInt(window.screen.height)-300;
															var width=parseInt(window.screen.width)-300;
															var repUrl="dhc.bdp.mkb.mkbencyclopedia.csp?base=&id="+SDSTermDR;
															if ("undefined"!==typeof websys_getMWToken){
																repUrl += "&MWToken="+websys_getMWToken()
															}
															$("#myWinMenuWiki").show();  
															var myWinMenuWiki = $HUI.dialog("#myWinMenuWiki",{
																resizable:true,
																title:'医为百科',
																width:width,
																height:height,
												...
												onclick:function(){
					                            	...
					                            	if ((assStr!="")&(assStr!=null)){
														...
														for (var i=0;i<ass.length;i++){
															...
															var id=assId;
															var assTool='<li class="tip-li"><div class="tip-mid"><img src="../scripts/bdp/Framework/icons/mkb/sds-rightarrow.png"><a id="Ass'+id+'" class="tip-title" href="#" onclick="viewAss('+id+","+assId+')">'+$g(assDesc)+'</a></div><p class="tip-note" style="display:none" id="PTAssScore'+id+'">'+$g("总分")+'：<span id="AssScore'+id+'"></span></p><p class="tip-note" style="display:none" id="PTAssRank'+id+'">'+$g("风险评估")+'：<span id="AssRank'+id+'"></span></p><p class="tip-note" style="display:none" id="PTAssDesc'+id+'">'+$g("结论")+'：<span id="AssDesc'+id+'"></span></p><span style="display:none" id="AssValue'+id+'"></span></li>';
															...
															if (SDSRowId!=""){
																...
																if (assInfo!=""){
																	$("#AssScore"+id).text($g(assInfo.split("&%")[0]));
																	$("#AssDesc"+id).text($g(assInfo.split("&%")[1]));
																	$("#AssRank"+id).text($g(assInfo.split("&%")[2]));
																	$("#AssValue"+id).text($g(assInfo.split("&%")[3]));
												...
												onclick:function(){
													...
													if ((tydiagStr!="")&(tydiagStr!=null)){
														...
														for (var i=0;i<tydiag.length;i++){
															...
															var tydiagTool='<li class="tip-li"><div class="tip-mid"><img src="../scripts/bdp/Framework/icons/mkb/sds-rightarrow.png"><span id="synony'+id+'" class="tip-title" href="#" >'+$g(tydiagDesc)+'</span><span id="synonyid'+id+'" style="display:none">'+tydiagId+'</span></div></li>';
												...
												onclick:function(){
													...
													if ((jbdiagStr!="")&(jbdiagStr!=null)){
														...
														for (var i=0;i<jbdiag.length;i++){
															...
															var jbdiagTool='<li class="tip-li"><div class="tip-mid"><img src="../scripts/bdp/Framework/icons/mkb/sds-rightarrow.png"><span>'+$g(jbdiagDesc)+'</span></div><p style="margin-left:12px" class="tip-note">'+$g(jbdiagNote)+'</p></li>';
									...
									onLoadSuccess:function(node, data){
										...
										if (desc!=undefined){ //树形前台检索，初次加载及分词点击不调用
											$(this).tree("search",desc+"^"+SearchMode)
											
											if (desc!=""){
												var hideNum=0
												for (var j=0;j<Roots.length;j++){
													//if (!$(this).tree("isLeaf",Roots[j].target)) $(this).tree("collapseAll",Roots[j].target);
													 if ($(Roots[j].target).is(":hidden")) { 
													 	hideNum++;
													 }
												}
												if (hideNum==Roots.length){
													$(tool).css("display","none") //检索没有属性内容时隐藏整行
												}
											}
										}
	...
	/***************************右侧属性列表检索重置功能开始**********************************************************************/
	$('#SearchProperty').searchbox('textbox').bind('click',function(){ 
		$('#SearchProperty').searchbox('textbox').select()    //重新点击时 默认之前输入的值为选中状态，方便删除     
	});
	//右侧属性列表检索功能
	$('#SearchProperty').searchbox({
	   searcher : function (value, name) { // 在用户按下搜索按钮或回车键的时候调用 searcher 函数
			$("#DiagForm").empty();
			LoadPropertyData($("#SelMKBRowId").val(),$("#SelSDSRowId").val(),indexTemplate,value);
			/*setTimeout(function(){
				DiagPropertySearch(value,indexTemplate)
			},80)*/
			
	   }
	})
	//右侧属性列表重置功能
	$("#btnResetProperty").click(function (e) { 
		indexTemplate=undefined;
		$HUI.radio("#SearchModeA").uncheck(); 
		$HUI.radio("#SearchModeL").uncheck();
		$("#DiagForm").empty();
		LoadPropertyData($("#SelMKBRowId").val(),$("#SelSDSRowId").val(),indexTemplate,"");
		/*setTimeout(function(){
			DiagPropertySearch("",indexTemplate)
		},50)*/
		$('#SearchProperty').searchbox('setValue','')
	})
	//切换检索模式后实时检索
	$HUI.radio("#SearchModeA,#SearchModeL",{
	 	onCheckChange:function(){
	 		var desc=$('#SearchProperty').searchbox('textbox').val() 
	 		if (desc!=""){
				$("#DiagForm").empty();
				LoadPropertyData($("#SelMKBRowId").val(),$("#SelSDSRowId").val(),indexTemplate,desc);
			}
	 	}
	 }); 
	/***************************右侧属性列表检索重置功能结束**********************************************************************/
	...
	GetPropertyValue=function(flagFrequency){
		...
		if ($("#SelMKBRowId").val()!=""){
			var TermVal=tkMakeServerCall("web.DHCBL.MKB.MKBStructuredData","StandardizedStructStr",$("#SelMKBRowId").val()+"-"+DataStr)
			var ValueStr=""
			if (TermVal.indexOf("-")>-1){
				ValueStr=TermVal.split("-")[1]
			}
			var DiagnosPropertyStr=$("#SelMKBRowId").val()+"#"+ValueStr+"#"+Supplement+"#"+resultRequired; //+"#"+diagNote //安贞
		}else{
			...
			for (var j=1;j<tds.length;j++){
				...
				if (showtype=="T"){
					if (childDesc==$g("文本")){
						...
						$("#"+detailId+"").tree("search",SearchText+"^"+SearchMode)
...
/** 
 ...
 (function($) {    
     ...
     $.extend($.fn.tree.methods, {  
          ...
          */  
         search: function(jqTree, searchParam) { 
         	 var searchText = searchParam.split("^")[0]
         	 var searchMode = searchParam.split("^")[1]
     ...
     function isMatch(searchText, targetText, searchMode) {  
		...
		if (searchMode=="L"){ //左匹配检索
			var searchTextLen=searchText.length
			var textSub=text.substr(0,searchTextLen)
			if(aliasflag==1){
				searchCodeSub=searchCode.substr(0,searchTextLen); //检索码
				othernameSub=searchCode.substr(0,searchTextLen); //别名
				displaynameSub=searchCode.substr(0,searchTextLen); //展示名
				return ($.trim(textSub)!="") && ((textSub.toUpperCase().indexOf(searchText)!=-1) || (searchCodeSub.toUpperCase().indexOf(searchText)!=-1) || (othernameSub.toUpperCase().indexOf(searchText)!=-1) || (displaynameSub.toUpperCase().indexOf(searchText)!=-1))  
			}else{
				return ($.trim(textSub)!="") && (textSub.toUpperCase().indexOf(searchText)!=-1);  
			}
		}
     }  
	...
	/******************************属性列表功能结束**************************************************************/
	/***************************医为智能提示功能开始***************************************************************************/
	InitSDSDrg=function(EpisodeID,PatientID){
		if(!ServerObj.SDSDiagEntry) return; //调用医生站结构化诊断科室授权，未授权结构化诊断，不显示DRG分组
		SDSADMNOID=EpisodeID,SDSPMINOID=PatientID //全局变量赋值，获取就诊号、患者登记号
		var repUrl="../csp/dhc.bdp.sds.structdiagnosdrg.csp?EpisodeID="+EpisodeID+"&PatientID="+PatientID+""
		if ("undefined"!==typeof websys_getMWToken){
			repUrl += "&MWToken="+websys_getMWToken()
		}
	    //DRG预分组悬浮窗
    	$('body').append('<iframe id="SDSDrgIframe" src='+repUrl+' name= "ifd"  width="330px" height="150px" scrolling="no" frameborder="0" >')
    	$('#SDSDrgIframe').css({
        	position: 'absolute',
    ...
    ReLoadSDSDrg=function(EpisodeID,PatientID){
    	if ((SDSADMNOID=="")&&(SDSPMINOID=="")) return; //未调DRG项目，不走此方法
    	if (ifFirstLoadPropertyData==true) return;
    	var DiagInfo=""
		var rowids=$('#tabDiagnosEntry').getDataIDs();
		for(var i=0;i<rowids.length;i++){
			var ICDDiagnosDesc=GetCellData(rowids[i],"DiagnosICDDesc");
			var MRCIDCode=GetCellData(rowids[i],"MRCIDCode");
			var MainDiagFlag=GetCellData(rowids[i],"MainDiagFlag"); //是/否
			if (ICDDiagnosDesc=="") continue;
			var EditFlag=""
			if (DiagEditId==(+i+1)){ //DiagEditId从1开始累加，i从0开始累加
				EditFlag="是"
			}
			if (DiagInfo==""){
    	...
    	$m({
			...
			$("#SDSDrgIframe").contents().find("#DrgDiv").html(data.split("&^")[0])
			$("#SDSDrgIframe").contents().find("#DrgUrl").html(data.split("&^")[1])
		});
    }
    ShowDrgDetail=function(url){
		...
		var myWinRelatedGroup = $HUI.dialog("#myWinRelatedGroup",{
			...
			modal:true,
			content:'<iframe id="myDRGDetail" frameborder="0" src="'+url+'" width="99%" height="98%"></iframe>'
		});
    }
	...
	InitSmartTip=function(termid,flag,linkicdinfo,value){
		...
		ReLoadSDSDrg(SDSADMNOID,SDSPMINOID); //调用DRG智能分组
