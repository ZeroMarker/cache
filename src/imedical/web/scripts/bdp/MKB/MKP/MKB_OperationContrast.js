...
{
	//开始时将右侧面板折叠
    var SelMRCICDRowid=""; //手术查找下拉框的手术id
    var findCondition="";  //手术描述
    var CacheDiagPropertyICD={}; //快速检索中属性列表获取
    var CacheDiagPropertyICDNote={}; //快速检索中属性列表备注获取
    var PreSearchText="";
    var cbg=null;//手术combogrid 
    //var base=tkMakeServerCall("web.DHCBL.MKB.MKBTermBase","GetIdByFlag","Diagnose");//根据标识获取知识库id 获取临床实用诊断
    var base=tkMakeServerCall("web.DHCBL.MKB.MKBTermBase","GetIdByFlag","Operation");
    //var base=tkMakeServerCall("web.DHCBL.MKB.MKBTermBase","GetIdByDesc","治疗手术");//根据标识获取知识库id
	...
	var defaultflag = tkMakeServerCall("web.DHCBL.MKB.MKBOperationContrast","getYesOrNot")//获取默认版本号选项
	//设置加载第几项
	var indexArri = 0;
    ...
    //var jumpflag=""
    //var defaultflag="D" //默认来源国家临床2.0-手术操作分类代码
    /********************************************父表************************************************************************/
       var columns =[[
        {field:'ck',checkbox:true},
		{field:'Rowid',title:'rowid',sortable:true,width:100,hidden:true},
		{field:'MKBOperationConCode',title:'代码',sortable:true,width:150,hidden:true},
		//{field:'MKBICDTermDr',title:'国际码ID',sortable:true,width:150,hidden:true},
        {field:'MKBOperationConDesc',title:'手术描述',sortable:true,width:150,
			formatter:function(value,row,index){
				if(row.MKBOperationInitialICD == "Y"){
					var content = '</span><span class="besticd"><font color="#FFFFFF" style="padding:0px 2px;">优</font></span><span href="#" title="' + value + '" class="mytooltip">' + value + '</span>'; //8A2BE2
				}else{
		...
		formatter:function(value,row,index){
			...
			if(row.MKBOperationResultRelation == "AND"){

				//var content = '<span class="andor"><font color="#FFFFFF">AND</font></span><span href="#" title="' + value + '" class="myresult">' + value + '</span>';
				var content = '<span class="andor"><font color="#FFFFFF" style="padding:0px 2px;">AND</font></span><span  id="Arr1"  class="myresult">';//title="' + value + '"
			...
			}else if(row.MKBOperationResultRelation == "OR"){
				//var content = '<span class="andor"><font color="#FFFFFF">OR</font></span><span title="' + value + '" class="myresult">' + value + '</span>';
				var content = '<span class="andor"><font color="#FFFFFF" style="padding:0px 2px;">OR</font></span><span  class="myresult">'; //title="' + value + '"
				var brArr =value.split('<br/>');
				for(var i = 0; i < brArr.length; i++){
					//content = content + '<a href = "#" onclick="changeIndex('+i+')">'+brArr[i]+'</a><br/><br/>'
				    if(i==0)
				...
				var content = '<span href="#"  class="myresult">';//title="' + value + '"
				var brArr =value.split('<br/>');
				for(var i = 0; i < brArr.length; i++){
					//content = content + '<a href = "#" onclick="changeIndex('+i+')">'+brArr[i]+'</a><br/><br/>'
				    if(i==0)
		...
		{field:'MKBOperationConNumber',title:'主要编码',sortable:true,width:80,
			formatter:function(value,row,index){
				var content = '<span href="#" title="' + value + '" class="mytooltip">' + value + '</span>';
		...
		{field:'MKBOperationConExtendNumber',title:'扩展编码',sortable:true,width:150,hidden:true},
		{field:'MKBOperationConOption',title:'录入选项',sortable:true,width:150,hidden:true},
		{field:'MKBOperationConType',title:'类别',sortable:true,width:150,hidden:true},
		{field:'MKBOperationDiaSource',title:'编码来源标识',sortable:true,width:150,hidden:true,
			formatter:function(value,row,index){
        ...
        {field:'MKBOperationConStatus',title:'处理状态',sortable:true,width:40,
		...
		{field:'MKBOperationRemark',title:'标记',sortable:true,width:150,hidden:true,
			formatter:function(value,row,index){
				if(value=="Y")
				{
					return "可匹配";
        ...
        {field:'MKBOperationConCenterWordID',title:'参考中心词ID',sortable:true,width:100,hidden:true},
        {field:'MKBOpeConCenterWord',title:'参考中心词',sortable:true,width:100,
            formatter:function(value,row,index){
        ...
        {field:'MKBOperationConSegmentation',title:'分词',sortable:true,width:100,
            formatter:function(value,row,index){
                //var values = value.substr(1);
                var content = '<span href="#" title="' + value + '" class="mytooltip">' + value + '</span>';
                return content;
        ...
        {field:'MKBOperationConDate',title:'操作时间',sortable:true,width:50,hidden:true},
		{field:'MKBOperationConUpdateUser',title:'操作人',sortable:true,width:50,hidden:true},
		{field:'MKBOperationInitialICD',title:'最优匹配',sortable:true,width:50,hidden:true},
        {field:'MKBOperationResultRelation',title:'结构化结果关系',sortable:true,width:150,hidden:true}
    ]];
    var leftgrid = $HUI.datagrid("#leftgrid",{
        /*url:$URL,
        queryParams:{
            ClassName:"web.DHCBL.MKB.MKBOperationContrast",
        ...
        remoteSort:false,
        ClassTableName:'User.MKBOperationContrast',
        SQLTableName:'MKB_OperationContrast',
        idField:'Rowid',
		...
		onClickCell:function(index, field, value){ //单击单元格触发
			//copyToCommand(value)
		},
		onDblClickCell:function(rowIndex,field,value){
            var record=$('#leftgrid').datagrid('getSelected')
            var col=$('.westgrid').children().find('tr[datagrid-row-index='+rowIndex+']');
            if(field == "MKBOperationConNumber" || field == "MKBOperationConDesc"){//ICD
                var text = (record.MKBOperationConNumber).slice(0,4);//只要数点前边的数字
                $('#TextInterDesc').val('');
                ...
                var target=$(col).find('td[field=MKBOperationConNumber]')
                var t = $("#icdDiv")
                //$("#otherDiv").hide();
                showICDDiv(target,t);   
                firstflag = "first"//默认选中子表第一条      
        ...
        onClickRow:function(rowIndex,row) //单击行触发
        { 
			  //可编辑表格部分
			...
			//查询国际码
			/*var desc = (row.MKBOperationConNumber).slice(0,3)
        ...
        onDblClickRow:function(rowIndex, field, value) //双击行触发
        {
		...
		onRowContextMenu:function (e, rowIndex,row) { //右键时触发事件
            ...
            $(
				...
				'<div onclick="copyResultBtn()" iconCls="icon-copyorder" data-options="">复制结构化手术</div>' +
	...
	refreshResultcolumn=function(){
		...
		//setTimeout(function(){
			//前台获取第一条数据
			var record = $("#leftgrid").datagrid('getSelected')
			var resultDesc = tkMakeServerCall("web.DHCBL.MKB.MKBOperationContrast","GetSingleData",record.Rowid)
			var ANDOR = record.MKBOperationResultRelation
			//自动更新ANDOR字段
			if(resultDesc.split("<br/>").length>1 && record.MKBOperationResultRelation == ""){
				//如果结构化结果多余两条，并且没有维护ANDOR，则自动设置为AND
				var result=tkMakeServerCall("web.DHCBL.MKB.MKBOperationContrast","SaveResultRelation",record.Rowid,"AND");
				var data=eval('('+result+')');
				if(data.success == 'true'){
					ANDOR = "AND"
				}
			}else if(resultDesc.split("<br/>").length<2 && record.MKBOperationResultRelation != ""){
				//如果结构化结果少于两条，并且维护了ADNOR，则自动清空关系
				var result=tkMakeServerCall("web.DHCBL.MKB.MKBOperationContrast","SaveResultRelation",record.Rowid,"");
				var data=eval('('+result+')');
				if(data.success == 'true'){
					ANDOR = ""
				}
			...
			$('#leftgrid').datagrid('getRows')[resultIndex].MKBOperationResultRelation = ANDOR;
	...
	matchSign = function(flagT)
	{
		...
		var record = $('#leftgrid').datagrid('getSelected');
		if(record){
				var MKBOperationRemark = flag;
				var resultIndex = $('#leftgrid').datagrid('getRowIndex',record)
				var result=tkMakeServerCall("web.DHCBL.MKB.MKBOperationContrast","EditUpdate",record.Rowid,"",MKBOperationRemark,"","");
				$('#leftgrid').datagrid('getRows')[resultIndex].MKBOperationRemark = MKBOperationRemark;
				$('#leftgrid').datagrid('refreshRow',resultIndex)
...
/***************************************国际码开始********************************************** */
	...
	var interColumns=[[ 	
		...
		{field:'MKBTPDDesc',title:'手术编码',width:100},
		{field:'MKBTDesc',title:'中文描述',width:200,
			formatter: function(value, row, index) {
	...
	/*interRest = function(){
		...
		if(record){
			...
			if(interRecord){
				...
				$('#leftgrid').datagrid('getRows')[resultIndex].MKBOperationRemark = "F";
				$('#leftgrid').datagrid('refreshRow',resultIndex)
	...
	/*$('#Initial_ICD').checkbox({
		onCheckChange:function(e,value){
            setTimeout(function(){
					...
					var record = $('#leftgrid').datagrid('getSelected');
					if(record){
							var MKBOperationInitialICD = flagt;
							var resultIndex = $('#leftgrid').datagrid('getRowIndex',record)
							var result=tkMakeServerCall("web.DHCBL.MKB.MKBICDContrast","EditUpdate",record.Rowid,"","","",MKBOperationInitialICD);
							$('#leftgrid').datagrid('getRows')[resultIndex].MKBOperationInitialICD = MKBOperationInitialICD;
							$('#leftgrid').datagrid('refreshRow',resultIndex)
							$("#update_sign").popover('hide');
							//loadViewTr(record);
	...
	conInitIcd = function(flag){
		...
		if(record){
            ...
            var icdFlag = tkMakeServerCall("web.DHCBL.MKB.MKBOperationContrast","GetDescAndParid",record.Rowid);
            var icdflagdesc = icdFlag.split("*")[0] //之前最优匹配的描述
            var icdflagid = icdFlag.split("*")[1] //最优匹配在对照表中的id
            var MKBOperationDiaSource = icdFlag.split("*")[2] //数据来源

            var sourceFlag=-1
            var source = $('#TextEdition').combobox('getValue');
            if (icdFlag!="")
            {
                var sourceFlag=MKBOperationDiaSource.indexOf(source);
            }
            if(flagt=="Y"&&sourceFlag!=-1){                   
                $.messager.confirm('提示', '已有诊断短语<font color=red>'+icdflagdesc+'</font>设置为最优匹配,是否重新设置？', function(r){
                    if (r){
                        var result=tkMakeServerCall("web.DHCBL.MKB.MKBOperationContrast","InitialICDUpdate",record.Rowid,MKBICDInitialICD);
                        var result2=tkMakeServerCall("web.DHCBL.MKB.MKBOperationContrast","InitialICDUpdate",icdflagid,"N");
                        var data=eval('('+result+')');
                        if(data.success=='true'){
                            $.messager.popover({msg: '设置成功！',type:'success',timeout: 1000}); 
                            //searchFunLibData("");
                            var resultIndex = $('#leftgrid').datagrid('getRowIndex',record)
                            $('#leftgrid').datagrid('getRows')[resultIndex].MKBOperationInitialICD = MKBICDInitialICD;
	...
	/*******************************初始icd勾选结束******************************** */
	//单机参考中心词、手术名、icd描述将内容复制到剪贴板
	copyToCommand = function(a){//a 内容标志
		//3 参考中心词  2icd描述  1 icd编码
		var record = $('#leftgrid').datagrid('getSelected')
		var text = ""
		if(a == "3"){
			text = record.MKBOpeConCenterWord
		}else if(a == "2"){
			text = record.MKBOperationConDesc
		}else if(a == "1"){
			text = record.MKBOperationConNumber
		}else{
			text = textForCopy
		}
	...
	loadViewTr = function(row)
	{
		...
		//标记
		/*$(".sign_class").remove();
		var remark = row.MKBOperationRemark
		if(row.MKBOperationRemark == "Y"){
			var reDesc = "可匹配"
		}else if(row.MKBOperationRemark == "N"){
			var reDesc = "无匹配"
		}else if(row.MKBOperationRemark == "F"){
			var reDesc = "不匹配"
		}else if(row.MKBOperationRemark == "W"){
			var reDesc = "待匹配"
		}
		...
		//手术编码
		$('.codeICD').remove();
		var codeP = '<a class="codeICD" href="#" onClick="copyToCommand(1)"><font color=#000 >&nbsp&nbsp'+row.MKBOperationConNumber+'</font></a>' //red
		$('#codeId').append(codeP);

		//手术描述
		/*$('.view2').remove();
		//$('.trView').remove();
		if(row.MKBOperationConDesc != "" && (row.MKBOperationConDesc).length>6){
			var p2 = '<a class="view2" href="#" onClick="copyToCommand(2)"><font color=red >&nbsp&nbsp'+(row.MKBOperationConDesc).substring(0,5)+'...</font></a>'
		}else{
			var p2 = '<a class="view2" href="#" onClick="copyToCommand(2)"><font color=red >&nbsp&nbsp'+row.MKBOperationConDesc+'</font></a>'
		}
		$('#termViewId').append(p2);*/
		
		//参考中心词显示
		$('.view3').remove();
		if(row.MKBOpeConCenterWord != "" && (row.MKBOpeConCenterWord).length>6){
			var p3 = '<a class="view3" href="#" onClick="copyToCommand(3)"><font color=#000>&nbsp&nbsp'+(row.MKBOpeConCenterWord).substring(0,5)+'...</font></a>' //black
		}else{
			var p3 = '<a class="view3" href="#" onClick="copyToCommand(3)"><font color=#000>&nbsp&nbsp'+row.MKBOpeConCenterWord+'</font></a>' //black
		}
		$('#termWordId').append(p3); 
					
		//分词显示
		$('#contentId').html("")
		var indexFlag = (row.MKBOperationConSegmentation).indexOf("(")
		if(indexFlag >= 0)
		{
			//中心词
			var segContent = (row.MKBOperationConSegmentation).split('(')[0];
			...
			if(flagCon >= 0)
			{
				...
				for(var i = 0; i<conArr.length; i++)
				{
					...
					{
						$('#contentId').append('&nbsp&nbsp<a class="trView" href="#" onClick="putTextIntoCom(this)"><font color=#017bec >'+conArr[i]+'</font></a>') //redgreen
					}
					else
					{
						$('#contentId').append('&nbsp&nbsp<a class="trView" href="#" onClick="putTextIntoCom(this)"><font color=#017bec >'+conArr[i]+'</font></a><font color=#017bec class="trView">&nbsp&nbsp(</font>') //redgreen
					}
				}
			}
			else
			{
				$('#contentId').append('&nbsp&nbsp<a class="trView" href="#" onClick="putTextIntoCom(this)"><font color=#017bec >'+segContent+'</font></a><font color=#017bec class="trView">&nbsp&nbsp(</font>')	//redgreen			
			}

			//属性
			var segPro = ((row.MKBOperationConSegmentation).split('(')[1]).substring(0,15);
			//去掉右括号
			var segPro1= segPro
			...
			for(var i = 0; i<proArr.length; i++)
			{
				...
				{
					$('#contentId').append('&nbsp&nbsp<a class="trView" href="#" onClick="putTextIntoCom(this)"><font color=#017bec >'+proArr[i]+'</font></a>') //redgreen
				}
				else
				{
					if(segPro1.indexOf(')')>-1){
						$('#contentId').append('&nbsp&nbsp<a class="trView" href="#" onClick="putTextIntoCom(this)"><font color=#017bec >'+proArr[i]+'</font></a><font color=#017bec class="trView">&nbsp&nbsp)</font>') //redgreen
		...
		{
			var conArr = (row.MKBOperationConSegmentation).split("，");
			for(var i = 0; i<conArr.length; i++)
			{
				$('#contentId').append('&nbsp&nbsp<a class="trView" href="#" onClick="putTextIntoCom(this)"><font color=#017bec >'+conArr[i]+'</font></a>') //redgreen
			}				
    ...
    updataStatic = function(flag){
		...
		if(records != ""){
			...
			for(var i = 0; i < records.length; i++){
				...
				var resultIndex = $('#leftgrid').datagrid('getRowIndex',record)
				//$('#leftgrid').datagrid('getRows')[resultIndex].MKBOperationConStatus = flag;
				//$('#leftgrid').datagrid('refreshRow',resultIndex)	
				if(resultIndex > -1){
					var result=tkMakeServerCall("web.DHCBL.MKB.MKBOperationContrast","EditUpdate",record.Rowid,flag,"","","");
					var data=eval('('+result+')');
					if(data.success=='true'){   
						$('#leftgrid').datagrid('getRows')[resultIndex].MKBOperationConStatus = flag;
						$('#leftgrid').datagrid('refreshRow',resultIndex)
	...
	refreshRelation = function(flagt){
		...
		if(record){
			if($('#mygrid').datagrid('getData').rows.length<2){
				$.messager.alert('错误提示','仅一条结构化手术，请查看!',"error");
				return;	
			}
			var result=tkMakeServerCall("web.DHCBL.MKB.MKBOperationContrast","SaveResultRelation",record.Rowid,flag);
			var data=eval('('+result+')');
			if(data.success=='true'){   
				$.messager.popover({msg: '设置成功!',type:'success',timeout: 1000});
				var resultIndex = $('#leftgrid').datagrid('getRowIndex',record)
				$('#leftgrid').datagrid('getRows')[resultIndex].MKBOperationResultRelation = flag;
				$('#leftgrid').datagrid('refreshRow',resultIndex);	
			} 	
		}else{
    ...
    loadRightResult = function(row){
        ...
        /*if($('#mygrid').datagrid('getData').total > 0){
            ...
            LoadDiagnosData(termDesc,ccc);//加载手术下拉框
		...
		//只按照匹配到的中心词展示
		if(row.MKBOpeConCenterWord != ""  && row.Result == ""){//&& ($('#mygrid').datagrid('getData').total == 0)){//否则按照匹配的参考中心新词来加载
			var flag = tkMakeServerCall("web.DHCBL.MKB.MKBOperationContrast","JustTermClose",row.MKBOperationConCenterWordID)
			if(flag != "N"){
				var termDesc = (row.MKBOpeConCenterWord).split(",")[0];
				var termid = (row.MKBOperationConCenterWordID).split(",")[0];
				SelMRCICDRowid = termid
            ...
            //$('.view1').remove();
		}else if(row.MKBOpeConCenterWord == "" && row.Result ==""){//&& ($('#mygrid').datagrid('getData').total == 0)){//都没有就不加载
			SelPropertyData=""
			LoadDiagnosData("",ccc);
			$('#combogrid').combogrid('setValue',''); 
            ...
            if(copyFlag == 1){
                for(var i = 0;i < dataContainer.rows.length; i++){
                    for(var j = 0; j < records.length; j++){
						...
						var existStr = tkMakeServerCall("web.DHCBL.MKB.MKBOperationContrast","justHasExist",parId,MKBICDConTermDr);
                        if(existStr != ""){
							//如果存在同名手术则覆盖
							tkMakeServerCall("web.DHCBL.MKB.MKBOperationContrast","SaveData",parId,existStr.split("^")[0],MKBICDConResultID,MKBICDConResult,MKBICDConTermDr,MKBICDConSupplement);
                        }else{
                            tkMakeServerCall("web.DHCBL.MKB.MKBOperationContrast","SaveData",parId,"",MKBICDConResultID,MKBICDConResult,MKBICDConTermDr,MKBICDConSupplement);                       
						}
				...
				for(var i = 0;i < records.length;i++){
					...
					tkMakeServerCall("web.DHCBL.MKB.MKBOperationContrast","EditUpdate",Rowid,"","F",MKBICDTermDr,"");
	...
	pasteResultBtn = function(){//1是复制多条，2是复制右侧单条
        ...
        if(copyFlag == 1){
            for(var i = 0;i < dataContainer.rows.length; i++){
                ...
                var MKBICDConTermDr = dataContainer.rows[i].MKBICDConTermDr
                var MKBICDConSupplement = dataContainer.rows[i].MKBICDConSupplement
                //判断是否存在同名（手术模板id相同）的数据...结果传空或者是有的话传id和他的描述
                var existStr = tkMakeServerCall("web.DHCBL.MKB.MKBOperationContrast","justHasExist",parId,MKBICDConTermDr);
                if(existStr != ""){
					//如果存在同名手术则覆盖
					var result = tkMakeServerCall("web.DHCBL.MKB.MKBOperationContrast","SaveData",parId,existStr.split("^")[0],MKBICDConResultID,MKBICDConResult,MKBICDConTermDr,MKBICDConSupplement);
					var data=eval('('+result+')');
					if(data.success == 'true'){
					...
					var result = tkMakeServerCall("web.DHCBL.MKB.MKBOperationContrast","SaveData",parId,"",MKBICDConResultID,MKBICDConResult,MKBICDConTermDr,MKBICDConSupplement);
					var data=eval('('+result+')');
					if(data.success == 'true'){
						refreshResultcolumn();//刷新左侧结构化字段
					}
			...
			var resultIndex = $('#leftgrid').datagrid('getRowIndex',record)
			var result=tkMakeServerCall("web.DHCBL.MKB.MKBOperationContrast","EditUpdate",Rowid,"","F",MKBICDTermDr,"");
			$('#leftgrid').datagrid('getRows')[resultIndex].MKBICDTermDr = MKBICDTermDr;
			$('#leftgrid').datagrid('getRows')[resultIndex].MKBTDesc = MKBTDesc;
			$('#leftgrid').datagrid('getRows')[resultIndex].MKBTPDDesc = MKBTPDDesc;
			$('#leftgrid').datagrid('getRows')[resultIndex].MKBOperationRemark = "F";
        ...
        {field:'MKBICDConResult',title:'结构化结果',sortable:true,width:100,
			formatter:function(value,row,index){
				if(row.MKBTActiveFlag=="N"){
					var content = '<font color=#666>手术'+(index+1)+':</font>&nbsp&nbsp'+'<font color=red>'+value+'</font>'; //green-666
				}else{
					var content = '<font color=#666>手术'+(index+1)+':</font>&nbsp&nbsp'+value; //green-666
				}
				return content;
			}
		},
        {field:'MKBICDConTermDr',title:'术语ID',sortable:true,width:100,hidden:true},
		{field:'MKBICDConSupplement',title:'补充手术',sortable:true,width:100,hidden:true},
    ...
    var mygrid = $HUI.datagrid("#mygrid",{
        ...
        onLoadSuccess:function(data){
			...
			setTimeout(function(){
				if(data.rows.length!=0 && firstflag=="first"){
					...
					$(this).prev().find('div.datagrid-body').prop('scrollTop',0);
					$('#mygrid').datagrid('selectRow',0);
					var flag = tkMakeServerCall("web.DHCBL.MKB.MKBOperationContrast","JustTermClose",data.rows[0].MKBICDConTermDr)
					if(flag != "N"){
						SelPropertyData="";
						refreshRightPanel();
		...
		onRowContextMenu:function (e, rowIndex,row) { //右键时触发事件
            ...
            $(
				...
				'<div onclick="copyResultCBtn()" iconCls="icon-copyorder" data-options="">复制结构化手术</div>'+
   ...
   /* $("#TextGen").keyup(function(){
	...
	$("#TextEdition").combobox({
		url:$URL+"?ClassName=web.DHCBL.MKB.MKBOperationContrast&QueryName=GetOpeEdition&ResultSetType=array",
		valueField:'id',
		textField:'text',
		panelWidth:260,
		/*data:[
			...
			{id:'A',text:'北京临床-ICD9手术分类临床版'},
			{id:'B',text:'国家标准版-ICD9字典库2011'},
			{id:'C',text:'国家临床1.1-手术操作分类代码'},
			//{id:'B',text:'疾病分类与ICD代码(2015国家修订版)'},
			//{id:'C',text:'疾病分类与代码国家临床版1.1'},
			//{id:'D',text:'疾病分类与代码国家临床版2.0'}
			{id:'D',text:'国家临床2.0-手术操作分类代码'}
		],*/
		value:defaultflag,
		onSelect: function(record){
			searchFunLibData("");
    ...
    $('#hispartRefresh').click(function(e){
		...
		//if((arrayRecord && arrayRecord.MKBOpeConCenterWord == "") || ){
        	//$('#Form_DiagPropertySearchGrid').datagrid('loadData',newarray);
			//$('#Form_DiagPropertySelectedGrid').datagrid('loadData',newarray);  
		//}
		$("#div-img").show();	     
    ...
    ///删除
    var DELETE_Result_URL = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.MKB.MKBOperationContrast&pClassMethod=DeleteData";
    DelDataM=function()
    {                  
...
/**********************************************可编辑表格开始************************************************/
    ...
    {
        var temp;
        temp=row.MKBOperationConStatus;
        row.MKBOperationConStatus=row.MKBOperationConStatus;
        row.MKBOperationConStatus=temp;
	...
	function ClickFun(type){   //单击执行保存可编辑行
        if (endEditing()){
            if(rowsvalue!= undefined){
                if((rowsvalue.MKBOperationConStatus!="")){
                    var differentflag="";
    ...
    function SaveData (record,type){
        //alert(record.MKBOperationConStatus)
        var result=tkMakeServerCall("web.DHCBL.MKB.MKBOperationContrast","EditUpdate",record.Rowid,record.MKBOperationConStatus,"","","");
        var data=eval('('+result+')');
        if(data.success=='true'){   
...
/**********************************************可编辑表格结束************************************************/
    ...
    $('#combogrid').combogrid({
        ...
        textField: 'MKBTDesc',    
        columns: [[    
            {field:'MKBTDesc',title:'手术名称',width:200,sortable:true,
            ...
            {field:'MKBTCode',title:'手术代码',width:120,sortable:true,hidden:true},
        ...
        keyHandler:{
            ...
            enter: function () { 
                ...
                if (selected) { 
                  ...
                  SelMRCICDRowid=selected.MKBTRowId; //手术id
 ...
 */ 
    ...
    updataResult=function(flag){		
        ...
        if(record)
        {
            ...
            //alert(MKBICDConTermDr)
            var MKBICDConResult = ""//.replace(/\(.*?\)/g,'');
			var MKBICDConSupplement = propertyValue.split('#')[2]
			var termidforjust=$('#combogrid').combogrid('getValue')
			if (termidforjust==""||termidforjust==undefined){
				$.messager.alert('错误提示','手术不能为空!',"error");
				return;
			}
			//判断获取到的下拉框的值是否正确，防止保存上一条记录的数据(TermId)
			var IdExsit=tkMakeServerCall("web.DHCBL.MKB.MKBStructuredData","justIdExist",termidforjust);
			if(termidforjust != "" && IdExsit==""){
				$.messager.alert('错误提示','请在下拉框选择一条正确的手术!',"error");
				return;
			}			
            if(flag == 1) {
                var cord=$("#mygrid").datagrid("getSelected");
                if(cord){
					var result = tkMakeServerCall("web.DHCBL.MKB.MKBOperationContrast","SaveData",rowid,cord.Rowid,MKBICDConResultID,MKBICDConResult,MKBICDConTermDr,MKBICDConSupplement);
					var data=eval('('+result+')');
					...
					if(data.success == 'true'){
                        ...
                        $.messager.alert('错误提示','请先选中一条手术!',"error");
				} 
            } else if(flag == 2){
                //如果是新增要判断有没有同名手术
                var existStr = tkMakeServerCall("web.DHCBL.MKB.MKBOperationContrast","justHasExist",rowid,MKBICDConTermDr);
                if(existStr != ""){ 
					$("#xWin").show();
					$("#xWin").html("")
					$("#xWin").append('已经存在同名手术<font color=red>'+ existStr.split('^')[1]+'</font>确定要替换吗?')
					var myWin = $HUI.dialog("#xWin",{
						...
						buttons:[{
							...
							handler:function(){
								...
								var result = tkMakeServerCall("web.DHCBL.MKB.MKBOperationContrast","SaveData",rowid,childId,MKBICDConResultID,MKBICDConResult,MKBICDConTermDr,MKBICDConSupplement);
								var data=eval('('+result+')');
								firstflag = "up";//判断是否是默认加载第一项的标志
								firstid = data.id;//保存操作项id							
								if(data.success == 'true'){
							...
							text:'新增',
							handler:function(){
								var result = tkMakeServerCall("web.DHCBL.MKB.MKBOperationContrast","SaveData",rowid,"",MKBICDConResultID,MKBICDConResult,MKBICDConTermDr,MKBICDConSupplement);
								var data=eval('('+result+')');
								firstflag = "up";//判断是否是默认加载第一项的标志
								firstid = data.id;//保存操作项id							
								if(data.success == 'true'){
    ...
    refreshRightPanel=function(){
        ...
        if(record.MKBICDConTermDr != ""){
            ...
            /*if(record.MKBICDConTermDr.indexOf("And")>0){
				...
				var termDesc =""
				var desc2 = tkMakeServerCall("web.DHCBL.MKB.MKBOperationContrast","findTermDescById",termid); 
				if(desc2 != ""){
					termDesc = desc2
				}else{
        ...
        }/*else if(record.MKBOpeConCenterWord != "" && record.MKBICDConTermDr == ""){
            var termDesc = (record.MKBOpeConCenterWord).split(",")[0];
            var termid = (record.MKBOperationConCenterWordID).split(",")[0];
            SelMRCICDRowid = termid
            CreatPropertyDom("","",termid,"","");  
            LoadDiagnosData(termDesc,ccc);
            $('#combogrid').combogrid('setValue',termid); 
        ...
        /*$('.view2').remove();
        var p2 = '<font color=red class="view2">&nbsp&nbsp'+record.MKBOperationConDesc+'</font>'
        $('#termViewId').append(p2);
        //分词显示
        $('.trView').remove();
        var trViewArr = (record.MKBOperationConSegmentation).split(';')
    ...
    $("#btnTerm").click(function (e) { 
		...
		var menuid=tkMakeServerCall("web.DHCBL.BDP.BDPMenuDefine","GetID","dhc.bdp.mkb.mtm.SY000057");//dhc.bdp.mkb.mtm.SY000057
		...
		if(!Sys.ie){
			...
			window.parent.showNavTab(menuid,"江苏省中医治疗手术",'dhc.bdp.ext.sys.csp?BDPMENU='+menuid+"&TermID="+termid+"&ProId=",parentid,menuimg)
		}else{
			parent.PopToTab(menuid,"江苏省中医治疗手术",'dhc.bdp.ext.sys.csp?BDPMENU='+menuid+"&TermID="+termid+"&ProId=",menuimg);
		}
	});
    ...
    var reviewedcolumns =[[ 
		...
		{field:'MKBDRDiagDr',title:'手术dr',hidden:true,sortable:true,width:100},
		{field:'MKBDRDiagDesc',title:'原手术名',sortable:true,width:100,
			formatter:function(value,row,index){
				var content = '<span href="#" title="' + value + '" class="mytooltip">' + value + '</span>';
				return content;
			}
		},
		{field:'MKBDRStructChild',title:'结构化手术第一条',sortable:true,width:100,hidden:true},
		{field:'MKBDRDiag',title:'上报手术名',sortable:true,width:100,
			formatter:function(value,row,index){
				var content = '<span href="#" title="' + value + '" class="mytooltip">' + value + '</span>';
				return content;
			}
		...
		{field:'MKBDRSupplement',title:'补充手术',sortable:true,width:100,hidden:true},
		...
		onExpandRow:function(rowIndex,rowData){
			...
			//mradm^短语dr^短语描述(或简拼)^Termdr^ResultID^补充手术^ICDDr^ICD代码^ICD描述^结构化手术描述
    ...
    previewStruct=function()
    {
        ...
        if(record)
        {
            ...
            if(record1)
            {
                ...
                var url="../csp/dhc.bdp.mkb.mkboperationpreview.csp?termid="+termid+"&parid="+parid+"&rowid="+rowid
                ...
                var myWinPreStruct=$HUI.window("#myWinPre",{
                    ...
                    closable:true,
                    resizable:true,
                    width:$(window).width()*6/10, //不含滚动条，浏览器的宽度
                    height:$(window).height()*9/10,
                    collapsible:false,
                    minimizable:false,
                    title:'预览',
                    modal:true,   
                    //content:'<iframe frameborder="0" src="../csp/dhc.bdp.mkb.mkbopeconpre.csp?termid='+termid+'&parid='+parid+'&rowid='+rowid+'" width="100%" height="99%" ></iframe>'
    ...
    JumpToSecond = function(){
        ...
        var record = $('#leftgrid').datagrid('getSelected')
        var Data = tkMakeServerCall("web.DHCBL.MKB.MKBOperationContrast","GetRowData",record.Rowid)
        Data = eval('('+Data+')'); 
        var jumpidsS = Data.MKBOpeConResultID
        var jumptermdrS = Data.MKBOpeConTermDr
        var jumpSupplementS = Data.MKBOpeConSupplement
        var jumpflagS = 1

        var menuid=tkMakeServerCall("web.DHCBL.BDP.BDPMenuDefine","GetID","dhc.bdp.mkb.mkbopeconseconddata");
        var parentid="" //tkMakeServerCall("web.DHCBL.BDP.BDPMenuDefine","GetID","dhc.bdp.mkb.mtm");
        var menuimg="" //tkMakeServerCall("web.DHCBL.BDP.BDPMenuDefine","GetIconByID",menuid);
        //判断浏览器版本
        var Sys = {};
        ...
        window.parent.showNavTab(menuid,"手术对照二厂",'dhc.bdp.ext.sys.csp?BDPMENU='+menuid+"&jumpidsS="+jumpidsS+"&jumptermdrS="+jumptermdrS+"&jumpSupplementS="+jumpSupplementS+"&jumpflagS="+jumpflagS+"&ICDSource="+ICDSource+"&jumpflag2="+1,parentid,menuimg)
