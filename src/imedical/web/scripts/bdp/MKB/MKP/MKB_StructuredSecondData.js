...
var init=function()
{
	var StructDesc = tkMakeServerCall("web.DHCBL.MKB.MKBStructuredSecondData", "getSecondStruDesc",STBBase)
    ...
    var columns =[[  
        ...
        {field:'MKBTDesc',title:'中心词',width:270,sortable:true,
			formatter: function(value,row,index){
				if(row.MKBSDInitialICD != "Y"){ //不是最优
                    if(row.MKBTActiveFlag=="N") //封闭
                    {
                        return "<font color='white'>"+value+"</font>"+"<span class='badgeDiv'>"+parseInt(row.MKBTDetailCount)+"</span>";
                    }
                    else
                    {
                    ...
                    {
                        return value+"<span class='badgeDiv'>"+parseInt(row.MKBTDetailCount)+"</span><span class='besticd'><font color='#FFFFFF'>优</font></span>"; //8A2BE2
                    }
				}
			...
			styler: function (value, row, index) {
				...
				{
				return 'background-color:#F16E57;'; //red ee4f38
				} 
			}
		},
		...
		{field:'MKBSDInitialICD',title:'最优匹配',width:80,hidden:true}
        ]];
    var leftgrid = $HUI.datagrid("#leftgrid",{
        //url:"../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.MKB.MKBStructuredSecondData&pClassMethod=GetMyList&base="+baseId+"&sortway="+orign+"&closeflag="+seeAll,
        /*url:$URL,
        queryParams:{
        ...
        onClickRow: function (rowIndex,rowData) {
			...
			alloptions={title:'完全匹配<font color=#21ba45 style="padding-left:10px">'+rowData.MKBTDesc+'</font>'};
			alloptions.url=$URL;
           ...
           // $("#allgrid").datagrid('unselectAll');
			...
			partoptions={title:'包含查询信息<font color=#21ba45 style="padding-left:10px">'+rowData.MKBTDesc+'[...]</font>'};
			partoptions.url=$URL;
		...
		onBeforeLoad:function(data){
			//动态显示标题名
			$("#leftgrid").datagrid("getPanel").panel("setTitle",StructDesc);
		},
		toolbar:'#leftbar'
	});
	//切换排序方式
    ...
    TextSearchFun = function() {
        ...
        $('#leftgrid').datagrid('options').url = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.MKB.MKBStructuredSecondData&pClassMethod=GetMyList&base="+baseId+"&sortway="+orign

        $('#leftgrid').datagrid('options').queryParams={'base':baseId,'desc':desc,'closeflag':seeAll,'stbbase':STBBase}
    ...
    ClearFunLib=function (){
        ...
        $('#leftgrid').datagrid('options').url = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.MKB.MKBStructuredSecondData&pClassMethod=GetMyList" //&base="+base+"&sortway="+sortway, 
        $('#leftgrid').datagrid('load',  {
            'base':baseId,
            'desc':"",
            'sortway':orign,
	 ...
	 var ifLoadPropertySearchGrid;
		...
		$HUI.datagrid("#Form_DiagPropertySelectedGrid",{
			columns: [[    
				...
				{field:'title',title:'属性标题',width:130,sortable:true,
					formatter:function(value,rec){ 
				   	  //鼠标悬浮显示全部
						return "<span class='hisui-tooltip' title='"+value+"'>"+value+"</span>" 
	                } 
				},
				{field:'text',title:'已选属性',width:260,sortable:true,
					formatter:function(value,rec){ 
						var showValue=value
				...
				$(modeJsonInfo).each(function(index,item){
					...
					if (showType=="T"){
						if (childDesc=="文本"){
							...
							var TA_tool="<td><textarea style='margin:2px;width:400px;height:80px;padding:2px' id='"+childId+"_T"+isTOrP+"_"+treeNode+"'></textarea></td>"  
							tool.append(TA_tool);
							/*if (Rowid!=""){
 ...
 */ 
    ...
    SearchDataFromStrData = function(){
        ...
        //$("#allgrid").datagrid('setTitle','完全匹配<font color=green style="padding-left:10px">'+resultDesc+'</font>');
        //$("#allgrid").datagrid({title:'完全匹配<font color=green style="padding-left:10px">'+resultDesc+'</font>'})
    ...
    var allcolumns =[[
        ...
        {field:'MKBSDDiag',title:'诊断名',sortable:true,width:100,
            formatter:function(value,row,index){
                if(row.MKBSDInitialICD == "Y"){
                    var content = '<span class="matchBesticd"><font color="#FFFFFF" style="padding:0px 2px;">优</font></span><span href="#" title="' + value + '" class="mytooltip" style="margin-left:2px">' + value + '</span>';
                }else{
            ...
            /*formatter: function(value,row,index){
                if(row.MKBSDInitialICD != "Y"){
                    return value+"<span class='badgeDiv'>""</span>";
                }else{
                    return value+"<font color='#FFFFFF'>优</font></span>"; //8A2BE2
                }
                
            },*/
        },
        {field:'MKBSDICD',title:'ICD编码',sortable:true,width:80,
            formatter:function(value,row,index){
                var content = '<span href="#" title="' + value + '" class="mytooltip">' + value + '</span>';
                return content;
            }
        },
        {field:'MKBSDMrc',title:'ICD描述',sortable:true,width:100,
            formatter:function(value,row,index){
                var content = '<span href="#" title="' + value + '" class="mytooltip">' + value + '</span>';
                return content;
            }    
		...
		{field:'MKBSDInitialICD',title:'最优匹配',sortable:true,width:100,hidden:true,sortable:true}
	...
	updateICDFlag = function(flag){
		...
		if(icd == "Y" && record.MKBSDICD == ""){
			$.messager.alert('提示','请先在数据处理工厂关联一条ICD后再进行设置!',"error");
			return;		
		}
		...
		if(icd == "Y" && icdFlag!=""){
			...
			$.messager.confirm('提示', '已有诊断短语<font color=red>'+icdflagdesc+'</font>设置为最优匹配,是否重新设置？', function(r){
				if (r){
					var result=tkMakeServerCall("web.DHCBL.MKB.MKBStructuredSecondData","InitialICDUpdate",record.Rowid,icd);
    ...
    var partcolumns =[[
		...
		{field:'MKBSDInitialICD',title:'最优匹配',sortable:true,width:100,hidden:true}
    ]];
	...
	jumpToStData = function(flag){//1 完全 2不完全
		...
		var menuid=tkMakeServerCall("web.DHCBL.BDP.BDPMenuDefine","GetID","dhc.bdp.mkb.stb."+STBBase);
		var parentid="" //tkMakeServerCall("web.DHCBL.BDP.BDPMenuDefine","GetID","dhc.bdp.mkb.mtm");
		var menuimg="" //tkMakeServerCall("web.DHCBL.BDP.BDPMenuDefine","GetIconByID",menuid);
		//判断浏览器版本
		var Sys = {};
		...
		window.parent.showNavTab(menuid,Caption,'dhc.bdp.ext.sys.csp?BDPMENU='+menuid+"&jumpids="+MKBSDStructResultID+"&jumptermdr="+MKBSDTermDR+"&jumpSupplement="+MKBSDSupplement+"&jumpflag="+flag,parentid,menuimg)
