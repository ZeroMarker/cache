...
var init=function()
{
    ...
    var base=tkMakeServerCall("web.DHCBL.MKB.MKBTermBase","GetIdByFlag","Operation");//根据标识获取知识库id 获取临床实用诊断
    var baseId=tkMakeServerCall("web.DHCBL.MKB.MKBTermBase","GetIdByFlag","Operation");//根据标识获取知识库id 获取临床实用诊断
    //var STBBase=""
    //var jumpflagS=""
    //var defaultICDSource="D" //默认来源
    var defaultflag = tkMakeServerCall("web.DHCBL.MKB.MKBOperationContrast","getYesOrNot")//获取默认版本号选项
    if(ICDSource=="")
    {
        ICDSource=defaultflag;
    }
    ...
    var columns =[[  
		...
		{field:'MKBTActiveFlag',title:'未封闭',width:80,hidden:true},
		{field:'MKBOperationInitialICD',title:'最优匹配',width:80,hidden:true}
        ]];
    var leftgrid = $HUI.datagrid("#leftgrid",{
        ...
        onClickRow: function (rowIndex,rowData) {
			...
			alloptions.queryParams={
				ClassName:"web.DHCBL.MKB.MKBOpeConSecondData",
    ...
    ClearFunLib=function (){
        jumpflagS = ""//清空跳转信息
        ICDSource=defaultflag //默认来源
        $("#con_left").panel('setTitle','完全匹配')
        $("#con_right").panel('setTitle','包含查询信息')
        $("#TextSearch").combobox('setValue', '');
        $('#leftgrid').datagrid('options').url = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.MKB.MKBOpeConSecondData&pClassMethod=GetMyList" //&base="+base+"&sortway="+sortway, 
    ...
    TextSearchFun = function() {
        ...
        //alert(ICDSource);
        $('#leftgrid').datagrid('options').url = "../csp/dhc.bdp.ext.datatrans.csp?pClassName=web.DHCBL.MKB.MKBOpeConSecondData&pClassMethod=GetMyList&base="+baseId+"&sortway="+orign
 ...
 */ 
    ...
    var allcolumns =[[
        ...
        {field:'MKBOperationConDesc',title:'手术描述',sortable:true,width:100,
            formatter:function(value,row,index){
                var content = '<span href="#" title="' + value + '" class="mytooltip">' + value + '</span>';
                return content;
            }    
        },
        {field:'MKBOperationConNumber',title:'手术主要编码',sortable:true,width:80,
            formatter:function(value,row,index){
                var content = '<span href="#" title="' + value + '" class="mytooltip">' + value + '</span>';
                return content;
            }
		...
		{field:'MKBOperationInitialICD',title:'最优匹配',sortable:true,width:100,hidden:true,sortable:true},
        {field:'MKBOperationDiaSource',title:'手术来源标识',sortable:true,width:200,hidden:true,
            formatter:function(value,row,index){
    ...
    var allgrid = $HUI.datagrid("#allgrid",{
		...
		remoteSort:false,
		sortOrder:'desc',
		sortName : 'MKBOperationInitialICD',
	...
	updateICDFlag = function(flag){
		...
		if(icd == "Y" && record.MKBOperationConNumber == ""){
        ...
        if(icd=="Y"&&sourceFlag!=-1){
            ...
            $.messager.confirm('提示', '已有诊断短语<font color=red>'+icdflagdesc+'</font>设置为最优匹配,是否重新设置？', function(r){
                if (r){
                    var result=tkMakeServerCall("web.DHCBL.MKB.MKBOpeConSecondData","InitialICDUpdate",record.Rowid,icd);
                    ...
                    if(data.success == "true"){
                        ...
                        $('#leftgrid').datagrid('getRows')[resultIndex].MKBOperationInitialICD = icd;
    ...
    var partcolumns =[[
		...
		{field:'MKBOperationInitialICD',title:'最优匹配',sortable:true,width:100,hidden:true},
        {field:'MKBOperationDiaSource',title:'手术来源标识',sortable:true,width:200,hidden:true,
            formatter:function(value,row,index){
                var str=value.replace(/&/g,"\n")
                var content = '<span href="#" title="' + str + '" class="mytooltip">' + value + '</span>';
	...
	jumpToOpeConData = function(flag){//1 完全 2不完全
		var propertyValue = GetPropertyValue("1");
        //UpdateName(propertyValue);
        var record = $("#leftgrid").datagrid("getSelected");		
        //var MKBSDTermDR=propertyValue.split("#")[0];
        var MKBOpeConTermDR = record.MKBTRowId;
        var MKBOpeConResultID=propertyValue.split("#")[1];
        var MKBOpeConSupplement=propertyValue.split("#")[2];
        var resultRequired=propertyValue.split("#")[3]; //必填项未维护结果集 


		var menuid=tkMakeServerCall("web.DHCBL.BDP.BDPMenuDefine","GetID","dhc.bdp.mkb.mkboperationcontrast");
		var parentid="" //tkMakeServerCall("web.DHCBL.BDP.BDPMenuDefine","GetID","dhc.bdp.mkb.mtm");
		var menuimg="" //tkMakeServerCall("web.DHCBL.BDP.BDPMenuDefine","GetIconByID",menuid);
		//判断浏览器版本
		var Sys = {};
		...
		//if(!Sys.ie){
		window.parent.closeNavTab(menuid);
		window.parent.showNavTab(menuid,"各版本手术对照",'dhc.bdp.ext.sys.csp?BDPMENU='+menuid+"&jumpids="+MKBOpeConResultID+"&jumptermdr="+MKBOpeConTermDR+"&jumpSupplement="+MKBOpeConSupplement+"&jumpflag="+flag+"&ICDSource="+ICDSource,parentid,menuimg)
    ...
    $("#btnOthers").popover({
        ...
        content:
                ...
                '<a href="#" onclick="selectICDEdition(1)" >北京临床-ICD9手术分类临床版</a><br/><br/>'+
                '<img  style="padding-left:0px" src="../scripts_lib/hisui-0.1.0/dist/css/icons/select_grant.png">'+
                '<a href="#" onclick="selectICDEdition(2)" >国家标准版-ICD9字典库2011</a><br/><br/>'+
                '<img  style="padding-left:0px" src="../scripts_lib/hisui-0.1.0/dist/css/icons/select_grant.png">'+
                '<a href="#" onclick="selectICDEdition(3)" >国家临床1.1-手术操作分类代码</a><br/><br/>'+
                '<img  style="padding-left:0px" src="../scripts_lib/hisui-0.1.0/dist/css/icons/select_grant.png">'+
                '<a href="#" onclick="selectICDEdition(4)" >国家临床2.0-手术操作分类代码</a>'
    });
