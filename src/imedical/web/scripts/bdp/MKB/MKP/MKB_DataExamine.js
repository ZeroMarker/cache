...
var init=function()
{
	...
	var pageType = ""; //判断审核界面是icd对照还是结构化诊断
	//判断字符串是否为数字
				...
				$(modeJsonInfo).each(function(index,item){
					...
					if (showType=="T"){
						if (childDesc=="文本"){
							...
							var TA_tool="<td><textarea style='width:350px;height:82px;margin:2px' id='"+childId+"_T"+isTOrP+"_"+treeNode+"'></textarea></td>"  
							tool.append(TA_tool);
							/*if (Rowid!=""){
														...
														$('#NodeMenu').menu('appendItem', {
															...
															onclick:function(){
																...
																//if(!Sys.ie){
																	...
																	if(isTOrP=="P"){
																		window.parent.parent.showNavTab(menuid,"临床实用诊断",'dhc.bdp.ext.sys.csp?BDPMENU='+menuid+"&TermID="+MRCICDRowid+"&ProId="+childId+"&detailId="+node.id,parentid,menuimg)
																	}else{
																		var termmenuid=tkMakeServerCall("web.DHCBL.MKB.MKBTermBase","GetMenuId",childDesc); 
																		window.parent.parent.showNavTab(menuid,"临床实用诊断",'dhc.bdp.ext.sys.csp?BDPMENU='+termmenuid+"&TermID="+node.id+"&ProId=",parentid,menuimg)
																	}
 ...
 */ 
    ...
    setTimeout(function(){
        if(SDSStr.split("^")[1]!=""){
			if(pageType == "SD"){
				var initData = tkMakeServerCall("web.DHCBL.MKB.MKBStructuredData","GetSingleData",SDSStr.split("^")[1])
				//console.log(initData)
				var initdata = eval('('+initData+')');
				var SDMKBSDDiag = initdata.MKBSDDiag
				var SDMKBSDMrc = initdata.MKBSDMrc
				var SDMKBSDICD = initdata.MKBSDICD
				var SDResult = initdata.Result
	
			}
			else{
				var initData = tkMakeServerCall("web.DHCBL.MKB.MKBICDContrast","GetSingleData",SDSStr.split("^")[1])
				var SDMKBSDDiag = SDSStr.split("^")[2]
				var SDMKBSDMrc = SDSStr.split("^")[8]
				var SDMKBSDICD = SDSStr.split("^")[7]
				var SDResult = initData.split("<br/>")[0]
			}
        }
        else{
            var SDMKBSDDiag = ""
        ...
        $('#note_str').val(SDnote)
        if(SDreason.indexOf("A")>-1){
            //$HUI.checkbox("#reason_A").setValue(true);
            $("#reason_A").attr("checked", 'checked');
        }
        if(SDreason.indexOf("B")>-1){
            //$HUI.checkbox("#reason_B").setValue(true);
            $("#reason_B").attr("checked", 'checked');
        }
        if(SDreason.indexOf("C")>-1){
            //$HUI.checkbox("#reason_C").setValue(true);
            $("#reason_C").attr("checked", 'checked');
        }
        if(SDreason.indexOf("D")>-1){
            //$HUI.checkbox("#reason_D").setValue(true);
            $("#reason_D").attr("checked", 'checked');
        }
        if(SDreason.indexOf("E")>-1){
            //$HUI.checkbox("#reason_E").setValue(true);
            $("#reason_E").attr("checked", 'checked');
        }
        if(SDreason.indexOf("F")>-1){
            //$HUI.checkbox("#reason_F").setValue(true);
            $("#reason_F").attr("checked", 'checked');
        }
		...
		$('#textDia').text(SDMKBSDDiag);
		$('#textICDDesc').text(SDMKBSDMrc);
		$('#textICDCode').text(SDMKBSDICD);
		$('#textResult').text(SDResult);
        /*if(SDSStr.split("^")[1] == ""){
        ...
        $('#push_icd_id').val(SDSStr.split("^")[6])
		$('#result_icd_id').val(SDSStr.split("^")[6])

		//LoadICDData(SDSStr.split("^")[8],$('#textICDDr'));//加载下拉框
        $('#textICDDr').val(SDSStr.split("^")[8])

		//$('#textICDDesc').text(SDMKBSDMrc)

        //$('#textICDCode').text(SDMKBSDICD)
        $('#textICDCode2').val(SDSStr.split("^")[7])

        
		$('#textResult2').val(SDSStr.split("^")[9])
		
		$('#textOtherDesc').val(SDSStr.split("^")[10])
		
		pageType = SDSStr.split("^")[11]
		...
		if(pageType == "SD"){
			$('#same_grid').datagrid('load',  { 
				...
				diag:SDSStr.split("^")[2],
				icd:SDSStr.split("^")[8],
				result:SDSStr.split("^")[9],
				termid:SDSStr.split("^")[3],
				icdcode:SDSStr.split("^")[7]
			}); 
			...
			$('#textOtherDesc').attr('readonly',true);
			$('#textOtherDesc').css("background","#E0E0E0")
       ...
       // var reson = $('#justreason_id').text();
		...
		if(SDstatus=="N"){//未审核数据
			if(pageType == "SD"){//数据处理工厂
				...
				var diadr = SDSStr.split("^")[1]
				if(SDSStr.split("^")[1] != ""){//修改
					var PYCode=Pinyin.GetJPU(dia);
					var result1=tkMakeServerCall("web.DHCBL.MKB.MKBDataReviewed","UpdateDiag",SDSStr.split("^")[1],dia,"Y",icddr,childflag,PYCode,icddesc);
					var data1=eval('('+result1+')');
					if(data1.success=='true'){
						flagupdata="Y"
						if(termdr != ""){
							tkMakeServerCall("web.DHCBL.MKB.MKBStructuredData","SaveData",SDSStr.split("^")[1],SDchild,result,"",termdr,Supplement); 
						}
				...
				}else if(SDSStr.split("^")[1] == ""){//新增
					var PYCode=Pinyin.GetJPU(dia);
					var result3=tkMakeServerCall("web.DHCBL.MKB.MKBDataReviewed","SaveNewParDia",parent.STBBase,dia,"Y",PYCode,icddr,termdr,Supplement,result,icddesc); 
					var data3=eval('('+result3+')');
					if(data3.success=='true'){
						flagupdata="Y"
						diadr = data3.id
						$.messager.popover({msg: '审核成功!',type:'success',timeout: 1000});
						parent.$('#reviewed_grid').datagrid('reload');
					}else{
						var errorMsg ="审核失败！"
						if (data3.errorinfo) {
							if((data3.errorinfo).indexOf("键不唯一")){
								...
								diadr = id
								var PYCode=Pinyin.GetJPU(dia);
								var result4=tkMakeServerCall("web.DHCBL.MKB.MKBDataReviewed","UpdateDiag",id,dia,"Y",icddr,childflag,PYCode);	
								var data4=eval('('+result4+')');
								if(data4.success=='true'){
									flagupdata="Y"
									if(termdr != ""){
										tkMakeServerCall("web.DHCBL.MKB.MKBStructuredData","SaveData",id,SDchild,result,"",termdr,Supplement); 
									}
									$.messager.popover({msg: '审核成功!',type:'success',timeout: 1000});
								}
				...
				if(flagupdata=="Y"){				
					var alldata = diadr +"^"+ dia +"^"+ adm +"^"+ termdr +"^"+ result +"^"+ Supplement +"^"+ icddr +"^"+ icddesc +"^"+ reson +"^"+ note +"^Y"+"^"+otherdesc+"^"+parent.$('#reviewed_grid').datagrid('getSelected').MKBDRType
					var result2 = tkMakeServerCall("web.DHCBL.MKB.MKBDataReviewed","SaveReviewedDia",SDid,alldata);

					//添加其他描述
					var pycode = Pinyin.GetJPU(otherdesc)
					var od=tkMakeServerCall("web.DHCBL.MKB.MKBStructuredData","OtherUpdate",diadr,"",otherdesc,"",pycode,"OD");
					//var result2 = tkMakeServerCall("web.DHCBL.MKB.MKBDataReviewed","uptateStatus",SDid,"Y",diadr);
				...
				if(termdr != ""){
					var res=tkMakeServerCall("web.DHCBL.MKB.MKBICDContrast","SaveData",SDSStr.split("^")[1],SDchild,result,"",termdr,Supplement); 
					var res=eval('('+res+')');
					if(res.success=='true'){	
						var id = parent.$('#reviewed_grid').datagrid('getSelected').MKBDRRowId
						var result4=tkMakeServerCall("web.DHCBL.MKB.MKBDataReviewed","uptateStatus",id,"Y");	
						var data4=eval('('+result4+')');
						if(data4.success=='true'){
	...
	//修改目标状态
	$('#updata_target').click(function(e){
		if(parent.$('#reviewed_grid').datagrid('getSelected').MKBDRStatus=="N"){
			var diadr = $('#result_dia_id').val();
			var dia =   $('#textDiaDr').val();
			var adm =  $('#push_adm_id').val();
			var termdr = $('#result_term_id').val();
			var result = $('#result_Ids').val();
		   ...
		   // var reson = $('#justreason_id').text();
			...
			var adm =  $('#push_adm_id').val();
			var icddesc = $('#textICDDr').val();	
			var status = parent.$('#reviewed_grid').datagrid('getSelected').MKBDRStatus
			if($('#push_dia_id').val() != "" && diadr == ""){
				diadr = $('#push_dia_id').val()
			}
			var alldata = diadr +"^"+ dia +"^"+ adm +"^"+ termdr +"^"+ result +"^"+ Supplement +"^"+ icddr +"^"+ icddesc +"^"+ reson +"^"+ note +"^"+status+"^"+otherdesc+"^"+parent.$('#reviewed_grid').datagrid('getSelected').MKBDRType
			var result = tkMakeServerCall("web.DHCBL.MKB.MKBDataReviewed","SaveReviewedDia",SDid,alldata);
			var data=eval('('+result+')');
			if(data.success == 'true'){
				$.messager.popover({msg: '保存目标状态成功!',type:'success',timeout: 1000});
			}else{
				errorMsg =errorMsg+ '<br/>错误信息:' + data3.errorinfo
				$.messager.alert('操作提示',errorMsg,"error");
			}
    ...
    var table = $HUI.datagrid("#reviewed_grid",{     
        data:[
            {no:'原始',Diag:'<font color=red id="textDia"></font>',ICDDesc:'<font color=red id="textICDDesc"></font>',ICDNumber:'<font color=red id="textICDCode"></font>',DiagExp:'<font color=red id="textResult"></font>',OtherDesc:''},
    ...
    var diagrid = $HUI.datagrid("#same_grid",{
		...
		onDblClickCell:function(rowIndex,field,row){
			...
			if(field == "MKBSDICD" || field == "MKBSDMrc"){//icd
				...
				$('#result_dia_id').val(rowData.Rowid);
	...
	//跳转到主界面
	$('#jump_main').click(function(e){
		if(pageType == "ICD"){
			$.messager.alert('提示','该界面不支持跳转!',"error");
			return;

		}
		parent.$('#reviewed_Data_Div').dialog('close');
		var diag = $('#textDiaDr').val();
		var icd = $('#textICDDr').val();
		var result = $('#textResult2').val();
		/*$('#same_grid').datagrid('load',  { 
		...
		parent.$('#leftgrid').datagrid(options);
		parent.$("#leftgrid").datagrid('unselectAll'); 
	})
}
$(init);