...
Ext.BDP.Component.tree.TreePanel = Ext.extend(Ext.tree.TreePanel, {
    ...
    saveAuthorizeData : function(){
		...
		//获取授权JSON
		AutJson = tkMakeServerCall(this.getAutClass,this.getAutMethod,this.ObjectType,this.ObjectReference);
		...
		//保存改动后的数据
		rtn = tkMakeServerCall(this.saveAutClass,this.saveAutMethod,this.ObjectType,this.ObjectReference,strAut);
		var rtn = rtn.replace(/\r\n/g,"");
 ...
 * 		limited  0:不受限  1:自定义  2:登陆角色医院 
 */
Ext.BDP.Component.DataAutPanel = Ext.extend(Ext.Panel, {
	layout:'border',
	//title : Ext.BDP.FunLib.getParam('ObjectType')==null?'':(Ext.BDP.FunLib.getParam('ObjectType')=='G'?'安全组：'+$URL.decode(Ext.BDP.FunLib.getParam('ObjectReferenceText')):(Ext.BDP.FunLib.getParam('ObjectType')=='L'?'科室：'+$URL.decode(Ext.BDP.FunLib.getParam('ObjectReferenceText')):'用户：'+$URL.decode(Ext.BDP.FunLib.getParam('ObjectReferenceText')))),
	//iconCls : Ext.BDP.FunLib.getParam('ObjectType')==null?'':(Ext.BDP.FunLib.getParam('ObjectType')=='G'?'icon-book':(Ext.BDP.FunLib.getParam('ObjectType')=='L'?'icon-LoginLoc':'icon-user')),
		...
		this.myTPanel = new Ext.Panel({
				...
				items : [{
					...
					items : [{
			            ...
			            listeners : {
			            	'afterrender' : function(com){
			            		//Ext.get('radio4').dom.checked = true;
			            		var domRadioLable1 = Ext.getCmp('radio1').getEl().dom.parentNode.lastChild;
			            		var domRadioLable2 = Ext.getCmp('radio2').getEl().dom.parentNode.lastChild;
			            		var domRadioLable3 = Ext.getCmp('radio3').getEl().dom.parentNode.lastChild;
                      var domRadioLable4 = Ext.getCmp('radio4').getEl().dom.parentNode.lastChild;     //update2019-12-23 zrf 增加状态——未授权
			            		domRadioLable1.setAttribute('ext:qtip','不受任何限制,可以查看所有医院数据');
			            		domRadioLable2.setAttribute('ext:qtip','只可以查看所勾选医院数据');
			            		domRadioLable3.setAttribute('ext:qtip','只可以查看该角色登录科室指向的医院数据');
			            		domRadioLable4.setAttribute('ext:qtip','没有对应的授权数据，默认取登录角色医院的权限');
					...
					html : '<div style="text-align:right;">'
							+ '<img style="width:12px;height:12px;" src="'+ Ext.BDP.FunLib.Path.URL_Icon +'no1.png" />'
							+ '<font style="color:red;">没有对应的授权数据，默认取登录角色医院的权限。</font></div>'
				}]
			})
		this.add(this.myTPanel);
    ...
    radioOnCheck : function(com, checked){
		...
		if(checked){
            ...
            if(com.id=='radio4'){           //update2019-12-23 增加状态——未授权 钟荣枫
                var lmd = "";
                this.myTree.setDisabled(true);
                this.setShowState('3');
                //this.myTree
            ...
            if (flagLimited!=lmd)   //radio发生改变
            {
                ...
                if(changedflag==1)  
                {
                    if(com.id!='radio4'){   //非未授权状态
                        ...
                        //未授权时，删除对应授权数据
                        var rowid= tkMakeServerCall("web.DHCBL.Authorize.BDPHospitalAut","DHCGetRowID",this.ObjectType,this.ObjectReference);
    ...
    setShowState : function(v){
    	...
    	if(v=='0'){
			var html = '<div style="text-align:right;">'
    						...
    						+ '<font style="color:green;">不受任何限制，可以查看所有医院数据。</font></div>';
    		comShowState.getEl().update(html);
    	}
		if(v=='1'){
    		var html = '<div style="text-align:right;">'
							+ '<img style="width:12px;height:12px;" src="'+ Ext.BDP.FunLib.Path.URL_Icon +'stop.png" />'
    						+ '<font style="color:red;">只可以查看所勾选医院数据。</font></div>';
    		comShowState.getEl().update(html);
    	}
    	if(v=='2'){
    		var html = '<div style="text-align:right;">'
							+ '<img style="width:12px;height:12px;" src="'+ Ext.BDP.FunLib.Path.URL_Icon +'bckdelete.gif" />'
    						+ '<font style="color:red;">只可以查看该角色登录科室指向的医院数据。</font></div>';
    		comShowState.getEl().update(html);
    	}
        if(v=='3'){
            var html = '<div style="text-align:right;">'
                            + '<img style="width:12px;height:12px;" src="'+ Ext.BDP.FunLib.Path.URL_Icon +'no1.png" />'
                            + '<font style="color:green;">没有对应的授权数据，默认取登录角色医院的权限。</font></div>';
            comShowState.getEl().update(html);
        }
    }
});