...
var init = function(){
	...
	var SAVE_ACTION_URL = "../csp/dhc.bdp.ext.entitydatatrans.csp?pClassName=web.DHCBL.LAB.BTDepartment&pClassMethod=SaveEntity&pEntityName=web.Entity.LAB.BTDepartment";
	...
	SearchFunLib=function(){
		...
		$('#mygrid').datagrid('load',  { 
			'ClassName':"web.DHCBL.LAB.BTDepartment",
			'QueryName':"GetList",
			'code': code,
			'desc': name
		});
	...
	{	
		...
		//var RowID=$("#RowID").val()
		var result= tkMakeServerCall("web.DHCBL.LAB.BTDepartment","FormValidate",id,Code,HospitalDR);
		if(result==0){
	...
	DelData=function()
	{
		...
		$.messager.confirm('提示', '确定要删除所选的数据吗?', function(r){
			if (r){
				var datas = tkMakeServerCall("web.DHCBL.LAB.BTDepartment","DeleteData",rowid);
				var data = eval('('+datas+')');
	...
	var mygrid = $HUI.datagrid("#mygrid",{
		url:$URL,
		queryParams:{
			ClassName:"web.DHCBL.LAB.BTDepartment",         ///调用Query时
			QueryName:"GetList"
		},
		ClassTableName:'dbo.BT_Department',
		SQLTableName:'dbo.BT_Department',
		idField:'RowID',
		columns: columns,  //列信息
