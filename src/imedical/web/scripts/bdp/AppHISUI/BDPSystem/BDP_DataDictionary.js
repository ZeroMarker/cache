...
$.extend($.fn.menu.methods, {
 ...
 $.extend($.fn.datagrid.methods, {
    ...
    function DownloadTemplate(){
	   ...
	   var excelinfostr=tkMakeServerCall("web.DHCBL.BDP.BDPDictionaryConfig","ProductTemplate",treeid);
	   if (excelinfostr=="") {alert("请先配置通用字典"); return;}
	...
	function uploadExcel(){   
		var files=$('#ExcelImportPath').filebox('files')
		if (!files.length) {
		...
		reader.onload = function(event) {
			...
			var myVar = setInterval(function () {
			  	...
			  	if(row>taskcount) //当到达最后一行退出
			  	{
			  		$('#prowin').dialog('close') 
                    clearInterval(myVar)
					$.messager.alert('提示',"导入完成，共计"+taskcount+"条数据。",'warning')					
			  	}
			  	else
			  	{
					...
					{ 
						allDataArray.push(datas[row-1][ColumnsCode[j]])
					} 
					/// console.log(allDataArray)
	 			   var allDataStr=allDataArray.join("&%"); 
				   var Flag =tkMakeServerCall(kbclassname,"ImportExcel",treeid,allDataStr); 	 
				}   
			},20)  
		};
...
AddTJProperty =function (){ 
        ...
        $('#BDPMenu').combobox('setValue',""); /// 扩展字段来源    
        $("#dictab").tabs('enableTab', 1); 
		$('#BDPMenu').combobox('disable');  //不可用（有效）  
		$HUI.checkbox("#BDPConfigFlag").setValue(false);
        $('#BDPConfigFlag').checkbox({
            onChecked: function(){
                 $("#dictab").tabs('disableTab', 1); // 禁用第二个选项卡面板
...
///新增下级节点属性
 function ToolbarAddProperty(){ 
        ...
        var existflag=tkMakeServerCall("web.DHCBL.BDP.BDPDataDictionary","IfNodeExist",treeid); 
        //alert(treeid+"^"+existflag)  
        if (existflag==0){
             $('#BDPParentMenuDR').combotree('setValue',"")
        }
        else{
