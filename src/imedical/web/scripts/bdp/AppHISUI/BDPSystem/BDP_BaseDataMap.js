...
AddTJProperty =function (){  
    ...
    {            
        ...
        if (id!==""){
            var MapDataStr=code+"^"+desc+"^"+type+"^"+linktable ;
            var OldMapDataStr=tkMakeServerCall("web.DHCBL.CT.BDPBaseDataMap","GetMapData",id);
            if (MapDataStr!==OldMapDataStr){  
                var modifyflag=tkMakeServerCall("web.DHCBL.CT.BDPBaseDataMap","GetIfDataMap",id); 
                if (modifyflag==1){
                    $.messager.alert('错误提示','该字典已被对照,不能再修改!',"error");
                    return false;
                }
            }
        }
 ...
 /// 字典数据对照  新增
     var AddDicMapFun=function(){ 
        ...
        $HUI.checkbox("#BDPMapDHisFlag").setValue(false); 
        $HUI.checkbox("#BDPMapDActive").setValue(true);   //默认选中 
 ...
 var ModifyDicMapFun=function(){
        ...
        if (record){             
            ...
            $.cm({
                ...
                $('#dicmap-form-save').form("load",jsonData); 
                $('#BDPMapDHisDicDesc').combobox("setValue",record.BDPMapDHisDicDesc);   
                if (jsonData.BDPMapDHisFlag=="Y"){  //给 外部到HIS对照 单独赋值  
                    $HUI.checkbox("#BDPMapDHisFlag").setValue(true);        
                }else{
                    $HUI.checkbox("#BDPMapDHisFlag").setValue(false);
                } 
                if (jsonData.BDPMapDActive=="Y"){ //给 有效标志 单独赋值 
                    $HUI.checkbox("#BDPMapDActive").setValue(true);        
                }else{
                    $HUI.checkbox("#BDPMapDActive").setValue(false);
                }  
            });  
