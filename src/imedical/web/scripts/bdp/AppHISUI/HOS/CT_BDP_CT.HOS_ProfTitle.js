...
var init = function(){
    var columns =[[ 
                  ...
                  {field:'PROFTDeptCode',title:'所属部门',width:180,sortable:true},
                  {field:'PROFTPROFTCCode',title:'职务字典编码',width:180,sortable:true},
                  {field:'PROFTLimitedNum',title:'额定人数',width:180,sortable:true},
   ...
   //搜索回车事件
    ...
    $('#PROFTPROFTCCode').combobox({ 
		...
		textField:'PROFTCDesc'
	        ...
	        //失焦事件
     $('#PROFTDesc').bind('blur',function(){
          var PROFTDesc=$("#PROFTDesc").val()  
          var PYCode=tkMakeServerCall("web.DHCBL.BDP.FunLib","GetPYCODE",PROFTDesc) 
          var WBCode=tkMakeServerCall("web.DHCBL.BDP.FunLib","GetSWBCODE",PROFTDesc,1) 
          $("#PROFTPYCode").val(PYCode)
          $("#PROFTWBCode").val(WBCode)                                           
      });	
    ...
    function AddData() { 
        $("#myWin").show();
 		var url=$URL+"?ClassName=web.DHCBL.CT.HOSDepartment&QueryName=GetDataForCmb1&ResultSetType=array";
	    $('#PROFTDeptCode').combobox('reload',url);       
		var url=$URL+"?ClassName=web.DHCBL.CT.HOSProfTitleDict&QueryName=GetDataForCmb1&ResultSetType=array";
		$('#PROFTPROFTCCode').combobox('reload',url);     				  
    ...
    {            
        ...
        var proftdeptcode=$("#PROFTDeptCode").combobox('getValue');
        var proftproftccode=$("#PROFTPROFTCCode").combobox('getValue');
		...
		if (proftdeptcode=="")
        {
            $.messager.alert('错误提示','所属部门不能为空!',"error");
            return;
        } 
		if (proftproftccode=="")
        {
            $.messager.alert('错误提示','职务字典不能为空!',"error");
            return;
        }                
		if (datefrom != "" && dateto != "") {   
        	if (datefrom >dateto) {
