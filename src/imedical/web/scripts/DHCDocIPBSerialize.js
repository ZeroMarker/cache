...
function BuildPatient(str)
{
	...
	obj.RowID = arry[21];
	obj.PoliticalLevel=arry[25];
	obj.SecretLevel=arry[26];
	return obj;
}
...
function BuildMedIPBKTempItem(str)
{
	if((str == "") || (str == null))
		return null;
	var obj = new DHCDocIPBKTempItem();
	var arry = str.split(CHR_Up);
	obj.RowID = arry[0];
	obj.ItemCode = arry[1];
	obj.ItemDesc = arry[2];
...
function BuildDHCDocIPBKTemplate(str) {

    if ((str == "") || (str == null))
        return null;
    var obj = new DHCDocIPBKTemplate();
    var arry = str.split("^");
    obj.RowID = arry[0];
    obj.TempCode = arry[1];
    obj.TempDesc = arry[2];
...
function SerializeDHCDocIPBKTempDtlList(obj)
{
	if(obj == null)
	{
		return "";
...
//add by lxf 2008-10-23
//Get the array that contained the MedDictionary objects.
function GetDHCDocIPBKTempItemArray(str)
{
	var objDicArray = new Array();
	var objArray = str.split(CHR_1);
	var objDic = null;
...
function BuildDHCDocIPBooking(str) {

    if ((str == "") || (str == null))
        return;
    var obj = new DHCDocIPBooking();
    var arry = str.split("^");
    obj.RowID = arry[0];
    ...
    obj.MRCCondtion= arry[16];
    obj.IPDeposit=arry[17];
    obj.IPBookNo=arry[20];
    return obj;

}


function SerializeDHCDocIPBooking(obj) {
    ...
    str +=""+CHR_Up;
    str+=obj.IPDeposit+CHR_Up;
	//str+=obj.AdmCondition
	str+=obj.AdmCondition+CHR_Up;
	str+=obj.IPBookNo
    return str;
...
function BuildDHCDocIPBKState(str) {
...
function SerializeDHCDocIPBKState(obj) {
...
function BuildDHCDocIPBKDetail(str) {
...
function SerializeDHCDocIPBKDetail(obj) {
