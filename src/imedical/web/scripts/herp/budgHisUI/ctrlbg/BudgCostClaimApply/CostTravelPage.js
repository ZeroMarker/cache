...
CostTravelPage = function (maingrid, detailgridid, isAudit, isEdit) {
    ...
    $winTravel = $('#AddTravel').window({
        ...
        onBeforeOpen: function () {
	        //console.log(isEdit);
            $("#TrPayCode").triggerbox("disable");
            $("#TrPrePayCode").triggerbox("disable");
            ...
            if(isAudit!=100){
	            ...
	            $("#TrPrePayCode").triggerbox("setValue",maingrid.PreBillCode);
	            ...
	            $("#TrPayCode").triggerbox("setValue",maingrid.applycode);
	            $("#TrDescbox").val(maingrid.applydecl);		       
	            }
        },
        onClose: function () { //关闭窗口后触发
        	$("#TrBalancebox").val("");
            $("#TrPayCode").triggerbox("setValue","");
            $("#TrPrePayCode").triggerbox("setValue","");
            $("#TrBankbox").val("");
            $("#TrNumberbox").val("");
            $("#TrApplyCbox").val("");
            $("#TrDescbox").val("");
    ...
    $("#TrPayCode").triggerbox("textbox").focus(function(){
		msgFn()
	})
    ...
    $("#ChargePrePay").checkbox({
        onCheckChange: function (event, value) {
            if (!value) {
                ...
                $("#TrPrePayCode").triggerbox("enable");
            }
        },
        onLoadSuccess: function () {
    ...
    $("#TrPrePayCode").triggerbox("textbox").focus(function(){
		msgChargePrePay()
	})
    ////////////////////////////////////分割线----travelgrid表格//////////////////////////////
    ...
    function addRow(ind, row) {
        var rowHtm = "";
        rowHtm += "<tr id='row" + ind + "' style='border-left:0'>";
        rowHtm += '<td style="border-left:0"><button name = "rowselected" id="check' + ind + '" class="icon-cancel" style="border:0;color:#ff6356; width:25px" onclick="delBtn(this.id)">x</button></td>'
        rowHtm += '<td style="display:none"><input class="text" id="rowid' + ind + '"  style="width:175px" value="' + row.rowid + '"/ ></td>'
        rowHtm += '<td><input type="text" class="partdata1" id="arrdate' + ind + '" style="width:175px" value="' + row.arrdate + '" /></td>';
        rowHtm += '<td><input class="partdata2 textbox" id="arrlocation' + ind + '" style="width:73px; height:30px" value="' + row.arrlocation + '" onblur="inputBlur(this.id)"/></td>';
        rowHtm += '<td><input type="text" class="partdata3" id="leadate' + ind + '" style="width:177px" value="' + row.leadate + '"/></td>';
        rowHtm += '<td><input class="partdata4 textbox" id="lealocation' + ind + '" style="width:73px; height:30px" value="' + row.lealocation + '" onblur="inputBlur(this.id)"/></td>';
        rowHtm += '<td><input type="text" class="partdata5" id="transport' + ind + '" style="width:132px" value="' + row.transport + '"/></td>';
        ...
        rowHtm += '<td><select class="partdata9" id="itemname' + ind + '" style="width:80px;height:30px" disabled><option value=' + row.item + '>' + row.itemname + '</select></td>';
    ...
    function addblankRow(ind) {
        ...
        rowHtm += '<td rowspan=1><input class="partdata2 textbox" id="arrlocation' + ind + '"  style="width:73px; height:30px" onblur="inputBlur(this.id)"/></td>';
        rowHtm += '<td rowspan=1><input type="text" class="partdata3" id="leadate' + ind + '" style="width:177px"/></td>';
        rowHtm += '<td rowspan=1><input class="partdata4 textbox" id="lealocation' + ind + '" style="width:73px; height:30px" onblur="inputBlur(this.id)"/></td>';
        rowHtm += '<td rowspan=1><input type="text" class="partdata5" id="transport' + ind + '" style="width:132px"/></td>';
        ...
        rowHtm += '<td rowspan=1><input type="text" class="partdata9" id="itemname' + ind + '" style="width:80px;height:30px" disabled/></td>';
    ...
    function initRow(rownum) {
        ...
        //地点组件的加载
        var test = new Vcity.CitySelector({input:'arrlocation' + rownum});
        var test = new Vcity.CitySelector({input:'lealocation' + rownum});
    ...
    inputPayCode = function () {
        $.m({
                ...
                MethodName: 'Genfee',
                applycode: $("#TrPayCode").triggerbox("getValue"),
    ...
    $("#TravelSave").unbind("click").click(function () {
        ...
        if ($("#ChargePay").checkbox("getValue") == 1 && ($("#TrPayCode").triggerbox("getValue") == "")) {
            var message = "请填写借款单号!";
            $.messager.popover({
                msg: message,
                type: 'info',
        ...
        $.messager.confirm('确定', '确定要保存选定的数据吗？', function (t) {
            if (t) {
                ...
                var mainData = maingridid 
                    ...
                    + "|" + $("#TrPayCode").triggerbox("getValue") //借款单号
                    + "|" + "" //预报销单号
