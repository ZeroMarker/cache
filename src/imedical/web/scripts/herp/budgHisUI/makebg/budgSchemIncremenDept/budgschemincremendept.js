...
function Init(){
    ...
    var Schembox = $HUI.combobox("#Schembox",{
        ...
        onBeforeLoad:function(param){
	        ...
	        if(year==""){
		        $.messager.popover({msg: '请先选择年度！',type:'info',timeout: 2000,showType: 'show',
		                      style:{
					            "position":"absolute",
					            "z-index":"9999",
    ...
    var FindBtn= function()
    {
        ...
        var deptId 		= $('#Deptbox').combobox('getValue'); //科室
        //console.log(JSON.stringify(deptId));
        if(Schem==""||(deptId=="")){
	        $.messager.popover({
						    msg:'请根据方案和科室查询！',timeout: 3000,type:'info',showType: 'show',
				            style:{
					            "position":"absolute",
					            "z-index":"9999",
					            left:document.body.scrollLeft/2 +document.documentElement.scrollLeft/2,
