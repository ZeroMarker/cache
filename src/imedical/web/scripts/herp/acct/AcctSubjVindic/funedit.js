editFun = function(rowid,SubjCode,SubjName,SubjNature,SubjType,SubjLevel,BalanceDirc,SubjNameAll,SuperCode,Spell,SelfCode,IsLast,Iscrash,Unit,ischeck) {
              
		var userdr = session['LOGON.USERID'];	
		var username = session['LOGON.USERNAME'];
               ///因为分级显示从主界面传过来的科目名称显示为（&nbsp;&nbsp;&nbsp;银行本票存款 ）此处用于去掉“&nbsp;”
		var SubjNames=SubjName.split(";");
	...
	var uNameField = new Ext.form.TextField({
				...
				anchor: '95%',
                                value:SubjName,
				selectOnFocus:'true',
                               // regex :/(^[A-Z][a-z0-9_]+[a-z0-9]+$|^[A-Z][a-z0-9])/,
				listeners : {
...
())+'&level='+encodeURIComponent(Ext.getCmp('Phone').getRawValue()),method:'POST'});
			});
...
CountUnitField 
			...
			addHandler = function(){
				...
				var cash = validField.getValue(); //现金标志
                                var Unit = CountUnitField.getValue(); //计量单位
                                
				var data = name + "|" + code + "|" + level + "|" + nameall + "|" + supers + "|" + spell + "|" + selcode + "|" + subje + "|" + nature + "|" +islast +"|" +couflag +"|" +direct +"|" +cash+"|" +Unit

				
				if(formPanel.form.isValid()){
			Ext.Ajax.request({
						url: '../csp/herp.acct.acctsubjvindicmainexe.csp?action=edit&data='+encodeURIComponent(data) + '&userid=' + userid+ '&rowid='+rowid,
					waitMsg:'保存中...',
					...
					success: function(result, request){
						...
						if (jsonData.success=='true'){
							...
							Ext.Msg.show({title:'注意',msg:'修改成功，请稍作等候!',icon:Ext.MessageBox.INFO,buttons:Ext.Msg.OK});
