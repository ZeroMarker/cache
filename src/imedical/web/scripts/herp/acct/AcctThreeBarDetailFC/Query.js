var projUrl = '../csp/herp.acct.acctThreeBarDetailFCexe.csp';
...
function Query() {
	...
	SchemGrid.addButton('-');
	//SchemGrid.addButton('<div style="color:red">说明：科目明细账-外币金额式的方案编号请以ZB0303开头。</div>');
	...
	//==外币金额式==//
	var SubjCodeZB0303Ds = new Ext.data.Store({
			autoLoad: true,
			proxy: "",
	...
	SubjCodeZB0303Ds.on('beforeload', function (ds, o) {
		ds.proxy = new Ext.data.HttpProxy({
				url: projUrl + '?action=GetFCAcctSubjZB0303&str=' + Ext.getCmp('AcctFCSubjCodeZB0303').getRawValue() + '&bookID=' + acctbookid,
				method: 'POST'
			});
	});

	var AcctFCSubjCodeZB0303 = new Ext.form.ComboBox({
			id: 'AcctFCSubjCodeZB0303',
			fieldLabel: '会计科目',
			labelSeparator:'',
			store: SubjCodeZB0303Ds,
			valueField: 'SubjCode',
	...
	var frm = new Ext.form.FormPanel({
			...
			items: [{
					xtype: 'fieldset',
					items: [AcctFCSubjCodeZB0303, AcctYearZB0301, AcctMonthZB0301Strat, AcctMonthZB0301End, StateZB0301Field]
				}, {
					xtype: 'button',
	...
	SchemGrid.on('rowclick', function (grid, rowIndex, e) {
		...
		if (rowObjID == "" || rowObjID == null || rowObjID == undefined) {
			...
			AcctFCSubjCodeZB0303.setValue(SubjCode);
	...
	function FindQuery() {
		var sjcode = Ext.getCmp('AcctFCSubjCodeZB0303').getValue();
		if (sjcode != "") {
		...
		p_URL = 'dhccpmrunqianreport.csp?reportName=herp.acct.ThreeBarDetailCashFC.raq&&sjcode=' + sjcode +
