...
dhc.herp.Gridwolf = Ext.extend(Ext.grid.EditorGridPanel, {
	edit : true,
	trackMouseOver : true,
	stripeRows : true,
	cm : colModel,
	...
	initComponent : function() {
		...
		dhc.herp.Gridwolf.superclass.initComponent.call(this);
	},
	createPlugins : function() {
		...
		plugins.push(dhc.herp.GridwolfTip);
	...
	createBbar : function() {
		this.bbar = new Ext.PagingToolbar({// 分页工具栏
			...
			store : this.store,
			displayInfo : true,
			plugins : new dhc.herp.PageSizePluginof(),
	...
	add : function() {
	  ...
	  var TempletID = selectedRow[0].data['rowid'];
	 // alert(TempletID);
		if (len < 1) {
			Ext.Msg.show({
						title : '注意',
		...
		if (store.recordType) {
			...
			rec.fields.each(function(f) {
						if (f.defaultValue != null) {
							...
							if(f.name=="AllocRate"){
								f.defaultValue="是";
							}else if(f.name=="TempletID"){
								//alert(rec.data[f.name]);
								f.defaultValue =TempletID
							}
							//alert(f.name)
	...
	del : function() {
		...
		var rowObjwolf=itemDetail.getSelectionModel().getSelections();
		var len = rowObjwolf.length;
		    //判断是否选择了要修改的数据	   
		    if(len < 1){
				Ext.Msg.show({
					title: '注意',
				...
				var projadjdr=rowObjwolf[0].get("projadjdr")
				var bpbudpaydr=rowObjwolf[0].get("bpbudpaydr")
			}	 */
...
//		var bpbudpaydr=rowObjwolf[0].get("bpbudpaydr")
				...
				Ext.each(records, function(record) {
							...
							Ext.Ajax.request({
										url : tmpUrl + '?action=del&rowid='
												+ record.get("rowid"),	//+'&projadjdr='+projadjdr+'&bpbudpaydr='+bpbudpaydr,
										waitMsg : '删除中...',
	...
	save : function() {
		...
		*/
		...
		Ext.each(records, function(r) {
			...
			//var rowObjwolf=itemMain.getSelectionModel().getSelections();
			//var projadjdr=rowObjwolf[0].get("projadjdr")
			//alert(projadjdr);
			// 数据完整性验证Beging
			for (var i = 0; i < this.fields.length; i++) {
			...
			if (r.data.newRecord) {
			   //var rowObjwolf=itemMain.getSelectionModel().getSelections();
			   //var projrowid=rowObjwolf[0].get("rowid");
			  
				recordType = "add";
				o[this.idName] = r.get(this.idName);
			    data.push(o);
				var selectedRow = itemMain.getSelectionModel().getSelections();
				var TempletID = "&TempletID=" + r.data['TempletID'];
				//alert(TempletID);
			    var saveUrl = this.url + '?action=' + recordType + stro.toString()
					+ "&" + tmpstro.toString() + "&"
					+ Ext.urlEncode(this.urlParam)+"&" + TempletID.toString();	//+"&projrowid="+projrowid;
			} else {
				//var rowObjwolf1=itemDetail.getSelectionModel().getSelections();
			    //var projadjdr1=rowObjwolf1[0].get("projadjdr")
				recordType1 = "edit";
				o[this.idName] = r.get(this.idName);
			    data.push(o);
				var TempletID = "&TempletID=" + r.data['TempletID'];
			    var saveUrl = this.url + '?action=' + recordType1 + stro.toString()
					+ "&" + tmpstro.toString() + "&"
					+ Ext.urlEncode(this.urlParam)+"&" + TempletID.toString();	//+"&projadjdr="+projadjdr1;
			}
			//o[this.idName] = r.get(this.idName);
			//data.push(o);
			//var saveUrl = this.url + '?action=' + recordType + stro.toString()
			...
			p = {
				...
				success : function(result, request) {
					...
					if (jsonData.success == 'true') {
						...
						if (jsonData.refresh == 'true') {
							tmpStore.load({
								params : {
									...
									limit : this.pageSize
									// parowid:parowid,projrowid:projrowid
								}
							});
						  // itemMain.load({params:{start:0, limit:25}});		
						}
						...
						// this.store.rejectChanges(); //报错后不清空数据
					}
				},
				scope : this
			};
	...
	load : function(param) {
		var limits=Ext.getCmp("PageSizePluginof").getValue();
			 //alert(limits);
	         if(!limits){limits=25};
		if (!Ext.isEmpty(param))
...
dhc.herp.PageSizePluginof = function() {
	dhc.herp.PageSizePluginof.superclass.constructor.call(this, {
...
Ext.extend(dhc.herp.PageSizePluginof, Ext.form.ComboBox, {
