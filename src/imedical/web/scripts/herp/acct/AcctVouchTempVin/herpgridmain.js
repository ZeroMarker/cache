...
dhc.herp.Grid = Ext.extend(Ext.grid.EditorGridPanel, {
			...
			if (store.recordType) {
				...
				rec.fields.each(function (f) {
					if (f.defaultValue != null) {
						...
						//做一个显示的假象
						if (f.name == "IsShelf") { //是否私有
							f.defaultValue = "否";
						}
						//rowid_"^"_TempNo_"^"_TempName_"^"_VouchType_"^"_Tempdesc_"^"_IsShelf_"^"_UserID_"^"_UserName
		...
		del: function () {
			...
			var not_userid_create = [],
			is_userid_create = [],
			temp_99 = [];
			var del_ajax_url = tmpUrl + '?action=del&rowid=';
			Ext.each(records, function (record) {
				var rowid = record.get("rowid");
				var temp_no_2 = record.get("TempNo").substr(0, 2);
				var createuserdr = record.get("UserID");
				if (!rowid) {
					// record.reject(true);
					...
					if (temp_no_2 == "99") {
						...
						if (createuserdr == UserID) {
							is_userid_create.push(rowid);
						} else {
							not_userid_create.push(rowid);
						}

					}
				}
			});
			var not_userid_rowid = not_userid_create.join();
			var is_userid_rowid = is_userid_create.join();
			if (temp_99.length > 0) {
				Ext.Msg.show({
					title: '错误',
					msg: '以99开头的模板不能删除！ ',
			...
			if (not_userid_rowid.length > 0) {
				Ext.MessageBox.confirm('提示', '所删模板中包含非本人录制的模板,<br>确定要删除该模板及其明细吗？', function (btn) {
					if (btn == "yes") {
						// 将两个数组合并
						var userid_rowid = not_userid_create.concat(is_userid_create).join();
						del_ajax_url = del_ajax_url + userid_rowid;
						// alert(del_ajax_url)
						ajax_cx.url = del_ajax_url;
						Ext.Ajax.request(ajax_cx);
					} else {
						return;
					}

				});
			} else if(not_userid_rowid.length ==0&&is_userid_rowid.length > 0){
				Ext.MessageBox.confirm('提示', '确定要删除选定的模板及其明细吗？', function (btn) {
					if (btn == 'yes') {
						del_ajax_url = del_ajax_url + is_userid_rowid;
						// alert(del_ajax_url)
						ajax_cx.url = del_ajax_url;
						Ext.Ajax.request(ajax_cx);
					} else {
			...
			/* var createuserdr = records[0].get("UserID");
			if (createuserdr != UserID) {
			Ext.MessageBox.confirm('提示', '非本人录制模板,确定要删除该模板及其明细吗？', function (btn) {
			...
			if(record.get("TempNo").substr(0,2)=="99") return;
			...
			Ext.each(records, function (r) {
				...
				for (var i = 0; i < this.fields.length; i++) {
					var indx = this.getColumnModel().getColumnId(i + 1)
						...
						// alert(indx+"+"+r.data[indx].toString());
						if ((indx == "TempNo") && (r.data[indx].toString().substr(0, 2) == "99")) {
							Ext.Msg.show({
								title: '警告',
								msg: '新增凭证模板序号不能以99开头，且以99开头的凭证模板主表不能修改！ ',
								icon: Ext.MessageBox.WARNING,
								buttons: Ext.Msg.OK
							});
							return;
					...
					for (var f in r.getChanges()) {
						...
						if (r.data[f] != null) {
							if (tmpDate.indexOf(f) >= 0) { // field.type=='date'
								...
								var chk = r.data[f];
								if ((chk == "") && (f == "Tempdesc")) { //说明该列被清空了,否则说明该列没有动
									var chk = r.data[f] + "^Tempdesc";
								}
				...
				data.push(o);
				/*
				if((createuserdr!="")&&(createuserdr!=UserID)){


				Ext.MessageBox.confirm('提示', '非本人录制模板,确定要更改该模板信息吗？', function(btn) {
				if (btn == 'yes') {
					...
					success: function (result, request) {
						...
						if (jsonData.success == 'true') {
							...
							if (jsonData.info == 'RecordExist')
								message = '输入的记录已经存在!';
							if (jsonData.info == 'RepTempNo')
								message = '当前模板序号已被使用,请更改模板序号! ';
							if (jsonData.info == 'RepTempName')
								message = '当前模板名称已被使用,请更改模板名称! ';
							Ext.Msg.show({
								title: '错误',
								msg: message,
								buttons: Ext.Msg.OK,
