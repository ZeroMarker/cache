...
dhc.herp.Grid = Ext.extend(Ext.grid.EditorGridPanel, {
		...
		createStore: function () {
			...
			// 远程
				...
				this.store = new Ext.data.Store({
						proxy: new Ext.data.HttpProxy({
							url: this.url + '?action=listnum&userdr=' + userdr
						}),
						reader: new Ext.data.JsonReader({
							root: 'rows',
							totalProperty: 'results'
		...
		save: function () {
			...
			var month = Ext.getCmp('AcctMonthField').getValue();
			var vouchstate = Ext.getCmp('VouchStateField').getValue();
			if (vouchstate == true) {
				vouchstate = 1;
			} else {
				vouchstate = 0;
			}
			var endsumflag = Ext.getCmp('EndSumFlagField').getValue();
			if (endsumflag == true) {
				endsumflag = 1;
			} else {
				endsumflag = 0;
			...
			var newdesc = SubjCode1 + ';' + SubjCode2 + ';' + subjlevel + ';' + year + ';' + month + ';' + vouchstate + ';' + endsumflag;
			...
			var rtn = 0
				if (records.length > 0) {
						...
						p = {
							...
							success: function (result, request) {
								...
								if (jsonData.success == 'true') {
									...
									var message = "";
									message = recordType == 'addnum' ? '添加成功!' : '保存成功!'
										Ext.Msg.show({
											title: '注意',
											msg: message,
											buttons: Ext.Msg.OK,
					...
					var saveUrl = this.url + '?action=editnum' + '&rowid=' + rowObj[0].get("rowid") + '&descs=' + encodeURIComponent(newdesc);
					// alert(saveUrl)
					var m = {
						url: saveUrl,
						method: 'GET',
		...
		submitRecord: function () {
			...
			this.getStore().proxy = new Ext.data.HttpProxy({
					url: this.url + '?action=listnum' + stro
				});
			var data = [];
