...
function Query() {
	...
	// SchemGrid.addButton('-');
	// SchemGrid.addButton('<div style="color:red">说明：科目总账-数量金额式的方案编号以ZB0102开头。</div>');
	...
	var SubjCodeNameProxy = new Ext.data.HttpProxy({
			url: 'herp.acct.acctsubjledgreportexe.csp?action=GetSubjCodeNameNum&bookID=' + bookID, //&subjstr='+SubjCodeNameField1.getValue(),	//Ext.getCmp('SubjCodeNameField1').getRawValue(),
			method: 'POST'
		});
	...
	function FindQuery() {
		...
		p_URL = 'dhccpmrunqianreport.csp?reportName=herp.acct.AcctSubjNumAmountForm.raq&AcctSubjCode1=' + AcctSubjCode1 + '&AcctSubjCode2=' + AcctSubjCode2
