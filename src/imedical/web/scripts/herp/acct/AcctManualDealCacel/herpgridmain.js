...
dhc.herp.GridMain = Ext.extend(Ext.grid.EditorGridPanel, {
	...
	createStore : function() {
		...
		// 远程
			...
			this.store = new Ext.data.Store({
						proxy : new Ext.data.HttpProxy({
									url : this.url + '?action=list&userdr='+userdr
								}),
						reader : new Ext.data.JsonReader({
									root : 'rows',
									totalProperty : 'results'
	...
	createBbar : function() {
		this.bbar = new Ext.PagingToolbar({// 分页工具栏
			...
			store : this.store,
			displayInfo : true,
			plugins : new dhc.herp.PageSizePluginmain(),
	...
	load : function(param) {
		var limits=Ext.getCmp("PageSizePluginmain").getValue();
			 //alert(limits);
	         if(!limits){limits=25};
		if (!Ext.isEmpty(param))
...
dhc.herp.PageSizePluginmain = function() {
	dhc.herp.PageSizePluginmain.superclass.constructor.call(this, {
...
Ext.extend(dhc.herp.PageSizePluginmain, Ext.form.ComboBox, {
