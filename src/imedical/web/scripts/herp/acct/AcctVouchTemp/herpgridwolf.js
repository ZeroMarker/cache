...
dhc.herp.Gridwolf = Ext.extend(Ext.grid.EditorGridPanel, {
	...
	btnAddHide : function() {
		//alert(111)
		var tbar = this.getTopToolbar();
		var tbutton = tbar.get('herpAddsId');
		tbutton.hide();
		},
	//隐藏保存按钮
	btnSaveHide : function(id) {
		var tbar = this.getTopToolbar();
		var tbutton = tbar.get('herpSavesId');
		tbutton.hide();
		},
		//隐藏重置按钮
	...
	btnDeleteHide : function() {
		...
		var tbar = this.getTopToolbar();
		var tbutton = tbar.get('herpDeletesId');
		tbutton.hide();
		},
		//隐藏打印按钮
			...
			if (r.data.newRecord) {
			   ...
			   var projrowid=rowObjwolf[0].get("rowid");
			    ...
			    var saveUrl = this.url + '?action=' + recordType + stro.toString()
					+ "&" + tmpstro.toString() + "&"
					+ Ext.urlEncode(this.urlParam)+"&projrowid="+projrowid;
			} else {
			    ...
			    var saveUrl = this.url + '?action=' + recordType1 + stro.toString()
					+ "&" + tmpstro.toString() + "&"
					+ Ext.urlEncode(this.urlParam)+"&projadjdr="+projadjdr1;
			}
			//o[this.idName] = r.get(this.idName);
			//data.push(o);
			//var saveUrl = this.url + '?action=' + recordType + stro.toString()
