			...
			for (var i = 0; i < this.fields.length; i++) {
				...
				if (tmobj != null) {
					...
					if (tmobj.allowBlank == false) {
						var title = tmobj.editor.fieldLabel;
				...
				success : function(result, request) {
					...
					if (jsonData.success == 'true') {
						...
						if (jsonData.info == 'RecordExist')
							message = '输入的记录已经存在!';
     	     			if (jsonData.info == 'RecordEndday')
							message = '会计账套的结账日为空，请先添加结账日';
						if (jsonData.info == 'ErrYear')
							message = '会计年度有误，请重新输入！';
						if (jsonData.info == 'OutRange')
							message = '本期结束日期不能超出下月结束日期！ ';
						if (jsonData.info == 'ErrMonth')
							message = '会计月份有误，请重新输入！ ';
			...
			success : function(result, request) {
				...
				if (jsonData.success == 'true') {
					...
					if (jsonData.info == 'RecordExist')
						message = '输入的记录已经存在!';
					if (jsonData.info == 'RecordEndday')
						message = '会计账套的结账日不能为空!';
					if (jsonData.info == 'ErrYear')
							message = '会计年度有误，请重新输入';
					if (jsonData.info == 'ErrMonth')
							message = '会计月份有误，请重新输入！ ';
					if (jsonData.info == 'OutRange')
							message = '本期结束日期不能超出下月结速日期,请输入小于次月结束日期或更改次月结束日期';
					Ext.Msg.show({
								title : '错误',
								msg : message,
								buttons : Ext.Msg.OK,
