 ...
 * 	
 	save() 			//保存grid中修改的数据，增加数据时调用action=add；修改数据时调用action=edit；
 					成功后返回json格式："{success:'true',refresh:'true',info:'保存成功！'}
 	add()			//在grid上增加一行
...
Ext.ns('Ext.ux.gridMain');

var LODOP
Ext.ux.gridMain.CheckColumn = function(config) {
...
Ext.ux.gridMain.CheckColumn.prototype = {
	init : function(grid) {
		this.grid = grid;
...
// register ptype
Ext.preg('checkcolumn', Ext.ux.gridMain.CheckColumn);

// backwards compat
Ext.grid.CheckColumn = Ext.ux.gridMain.CheckColumn;
...
Ext.grid.Column.types = {
	...
	checkcolumn : Ext.ux.gridMain.CheckColumn
};
