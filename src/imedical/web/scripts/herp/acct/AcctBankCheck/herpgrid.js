 ...
 * 
 ...
 ---------------------------------------------------- 	
	...
	3、增加saveurl(url)方法,可以根据需要自定义url，保存提交数据，如：url='herp.budg.budgschemmainexe.csp?action=copysheme&rowid='+schmDr
	4、增加了删除标识，删除某行数据后增加标识，只有保存提交数据后才改数据库。
	5、增加查询按钮，去掉原来的页面自动加载（可以在扩展里重写）。
...
dhc.herp.Grid = Ext.extend(Ext.grid.EditorGridPanel, {
				...
				if (tmpObj.type instanceof Object) {
					...
					cols[i + 1].renderer = function(v, p, r, i) {
						...
						if (v instanceof Date) {
							...
							return new Date(v).format(websys_DateFormat); //edit 2017-03-03
						} else {
							return v;
	...
	createStore : function() {
		...
		// 远程
			...
			this.store = new Ext.data.Store({
						proxy : new Ext.data.HttpProxy({
									url : this.url + '?action=list&userdr='+userdr+'&acctbookid='+acctbookid
								}),
						reader : new Ext.data.JsonReader({
									root : 'rows',
									totalProperty : 'results'
		...
		Ext.MessageBox.confirm('提示', '确定要删除选定的行吗？ ', function(btn) {
			if (btn == 'yes') {
				Ext.each(records, function(record) {
							...
							if((IsChecked=="1")||(ConfirmState=="已审核")||(ConfirmState=="1")){
							    Ext.Msg.show({title:'注意',msg:'数据已对账或已审核,不能删除！',buttons: Ext.Msg.OK,icon:Ext.MessageBox.WARNING});
			                      return;
	...
	save : function() {
		...
		Ext.each(topbar.items, function(i) {
					//alert(encodeURIComponent(i));
					//alert(i.getXType()+'111');
					if (i.getXType() === 'combo') {
		...
		Ext.each(records, function(r) {
			...
			for (var i = 0; i < this.fields.length; i++) {
				...
				if (tmobj != null) {
					...
					if (tmobj.allowBlank == false) {
						...
						// (r.data[indx].toString() == "")|| (parseInt(r.data[indx].toString()) == 0) 不能为空或0
						//不能为空
						if (((r.data[indx].toString() == "")
								|| (parseInt(r.data[indx].toString()) == 0))&&((indx!="AmtDebit")&&(indx!="AmtCredit"))) {
			...
			var saveUrl = this.url + '?action=' + recordType + stro.toString()
					+ "&" + tmpstro.toString() + "&"
					+ Ext.urlEncode(this.urlParam)+"&userdr="+userdr+'&acctbookid='+acctbookid;
			 //alert(saveUrl);
			p = {
				url : saveUrl,
				method : 'GET',
