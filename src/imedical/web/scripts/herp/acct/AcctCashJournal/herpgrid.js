...
dhc.herp.Grid = Ext.extend(Ext.grid.EditorGridPanel, {
	...
	del : function() {
		...
		for(var i=0;i<len;i++){
		var state=records[i].get("JournalState");
		if(state=="提交"){
			Ext.Msg.show({
					title : '错误',
					msg : '数据已提交，不可删除! ',
				...
				Ext.each(records, function(record) {
							...
							Ext.Ajax.request({
										...
										success : function(result, request) {
											...
											}else if(jsonData.info=="deleteNO"){
												Ext.Msg.show({
															title : '提示',
															msg : '数据不可删除！ ',
															buttons : Ext.Msg.OK,
		  ...
		  var EndDateF=EndDate.format('Y-m-d');
			...
			var EndDateF="";
						...
						if (jsonData.info == 'RepOrder')
							message = '输入的序号已经存在!';
						if (jsonData.info == 'NoEdit')
							message = '日期不可修改! ';
						if (result.split("*")[0] == 'NOsubmit')
							message = result.split("*")[1]+'之前有未提交的数据,请提交后再录入! ';
						if (result.split("*")[0] == 'hasnext')
							message = result.split("*")[1]+'已经录入数据,请先删除! ';
						Ext.Msg.show({
									title : '错误',
									msg : message,
									buttons : Ext.Msg.OK,
