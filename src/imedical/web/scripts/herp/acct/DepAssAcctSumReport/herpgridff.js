...
dhc.herp.Grid = Ext.extend(Ext.grid.EditorGridPanel, {
	...
	save : function() {
        ...
        var start=Ext.getCmp('startYearMonth').getValue();
		if(start!="")
	    {
			start=start.format('Y-m')
		};
		var end=Ext.getCmp('endYearMonth').getValue();
		if(end!="")
	    {
			end=end.format('Y-m')
		};      
		...
		var amtdebit=Ext.getCmp('AmtDebitField').getValue();
		    if(amtdebit==1){
			    amtdebit=1;
		    }else{
			    amtdebit=0;
		    }
		 var outsubjcode=Ext.getCmp('OutSubjCodeField').getValue();
		    if(outsubjcode==1){
			    outsubjcode=1;
		    }else{
			    outsubjcode=0;
		    }
		var isStop=Ext.getCmp('IsStopField').getValue();
		    if(isStop==1){
			    isStop=1;
		    }else{
			    isStop=0;
		    }
		var newdesc=start+';'+end+';'+Itemcode1+';'+Itemcode2+';'+subjcode+';'+subjlevel+';'
		            +vouchstate+';'+amtdebit+';'+outsubjcode+';'+isStop;
