...
var projUrl = "herp.acct.AcctBankJournalexe.csp";
...
//银行科目
var AcctBankDs = new Ext.data.Store({
	autoLoad: true,
	proxy: "",
	reader: new Ext.data.JsonReader({
		totalProperty: 'results',
		root: 'rows'
	}, ['subjId', 'subjCode', 'subjCodeName'])
});
AcctBankDs.on('beforeload', function(ds, o) {
	ds.proxy = new Ext.data.HttpProxy({
		url: 'herp.acct.AcctBankJournalexe.csp?action=GetBank&str=' + Ext.getCmp('AcctBank').getRawValue() + '&AcctBookID=' + AcctBookID,
		method: 'POST'
	});

});

var AcctBank = new Ext.form.ComboBox({
	id: 'AcctBank',
	fieldLabel: '银行科目',
	store: AcctBankDs,
	valueField: 'subjCode',
...
//银行科目
var AcctBankDs1 = new Ext.data.Store({
	autoLoad: true,
	proxy: "",
	reader: new Ext.data.JsonReader({
		totalProperty: 'results',
...
AcctBankDs1.on('beforeload', function(ds, o) {
	ds.proxy = new Ext.data.HttpProxy({
		url: 'herp.acct.AcctBankJournalexe.csp?action=GetBank&str=' + Ext.getCmp('AcctBank1').getRawValue() + '&AcctBookID=' + AcctBookID,
		method: 'POST'
	});

});

var AcctBank1 = new Ext.form.ComboBox({
	id: 'AcctBank1',
	fieldLabel: '银行科目',
	store: AcctBankDs1,
	valueField: 'subjCode',
...
var find = function() {
	...
	var BankCode = AcctBank1.getValue();
	if (StartDate != "") {
...
var submitData = function() {
	...
	Ext.MessageBox.confirm('提示', '确定提交数据? ', function(btn) {
		if (btn == "yes") {
			Ext.Ajax.request({
				url: 'herp.acct.AcctBankJournalexe.csp?action=Sbumitdata&ID=' + ID,
				method: 'POST',
...
var CanSubmit = function() {
	...
	Ext.MessageBox.confirm('提示', '确定撤销提交数据? ', function(btn) {
		if (btn == "yes") {
			Ext.Ajax.request({
				url: 'herp.acct.AcctBankJournalexe.csp?action=CanSubmit&ID=' + ID,
				method: 'POST',
				failure: function(result, request) {
					Ext.Msg.show({
						title: '错误',
...
var SaveSum = function() {
	...
	Ext.MessageBox.confirm('提示', '确定保存摘要? ', function(btn) {
		if (btn == "yes") {
			Ext.Ajax.request({
				url: 'herp.acct.AcctBankJournalexe.csp?action=SaveSum&Summary=' + encodeURIComponent(summary),
				method: 'POST',
				failure: function(result, request) {
					Ext.Msg.show({
						title: '错误',
...
//从凭证引入银行日记账
    var VouchIncome = function() {
		...
		Ext.Ajax.request({
			url: projUrl + "?action=getvbStartDate&AcctBookID=" + AcctBookID,
			method: 'GET',
			success: function(result, request) {
				// alert('success');
				var jsonData = Ext.util.JSON.decode(result.responseText);
            ...
            buttons: [{
                ...
                handler: function() {
                    ...
                    function callback(btnid){
                        if(btnid=='yes'){
                            Ext.Ajax.request({
                                url: projUrl+'?action=bankVouchIncome&AcctBookID=' + AcctBookID+
                                ...
                                success: function(result, request) {
                                    ...
                                    if (jsonData.success == 'true') {
                                    	/*if(typeof(jsonData.info)=='number'&&jsonData.info>0){
                                    		Ext.Msg.show({
	                                            title: '提示',
	                                            msg: '引入成功! ',
	                                            buttons: Ext.Msg.OK,
...
var itemGrid = new dhc.herp.Grid({
	width: 400,
	region: 'center',
	url: 'herp.acct.AcctBankJournalexe.csp',
...

		{
			...
			renderer: function(value, cellmeta, record, rowIndex, columnIndex, store) {
				...
				if (value == '0') {
					...
					return '<span style="color:red;cursor:hand;TEXT-DECORATION:underline">未添加辅助核算</span>';
				} else if (value == '110') {
