...
dhc.herp.Grid = Ext.extend(Ext.grid.EditorGridPanel, {
	...
	createStore : function() {
		...
		// 远程
			...
			this.store = new Ext.data.Store({
						proxy : new Ext.data.HttpProxy({
									url : this.url + '?action=listcur&userdr='+userdr
								}),
						reader : new Ext.data.JsonReader({
									root : 'rows',
									totalProperty : 'results'
	...
	save : function() {
			...
			p = {
				...
				success : function(result, request) {
					...
					if (jsonData.success == 'true') {
						...
						var message = "";
						message = recordType == 'addcur' ? '添加成功!' : '保存成功!'
						Ext.Msg.show({
									title : '注意',
									msg : message,
									buttons : Ext.Msg.OK,
		...
		var saveUrl = this.url + '?action=editcur'+'&rowid='+rowObj[0].get("rowid") +'&descs='+encodeURIComponent(newdesc);
			// alert(saveUrl)
        var m = {
				url : saveUrl,
				method : 'GET',
	...
	submitRecord : function() {
		...
		this.getStore().proxy = new Ext.data.HttpProxy({
					url : this.url + '?action=listcur' + stro
				});
		var data = [];
