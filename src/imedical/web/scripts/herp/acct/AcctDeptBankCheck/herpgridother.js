...
dhc.herp.Gridother = Ext.extend(Ext.grid.EditorGridPanel, {
	edit : true,
	trackMouseOver : true,
	stripeRows : true,
	cm : colModel,
	...
	initComponent : function() {
		...
		dhc.herp.Gridother.superclass.initComponent.call(this);
	},
	createPlugins : function() {
		...
		plugins.push(dhc.herp.GridotherTip);
	...
	createStore : function() {
		...
		// 远程
			...
			this.store = new Ext.data.Store({
						proxy : new Ext.data.HttpProxy({
									url : this.url + '?action=list&bookID='+bookID
								}),
						reader : new Ext.data.JsonReader({
									root : 'rows',
									totalProperty : 'results'
	...
	btnAddHide : function() {
		//alert(111)
		var tbar = this.getTopToolbar();
		var tbutton = tbar.get('herpAddIdId');
		tbutton.hide();
		},
	//隐藏保存按钮
	btnSaveHide : function(id) {
		var tbar = this.getTopToolbar();
		var tbutton = tbar.get('herpSaveIdId');
		tbutton.hide();
		},
		//隐藏重置按钮
	...
	btnDeleteHide : function() {
		...
		var tbar = this.getTopToolbar();
		var tbutton = tbar.get('herpDeleteIdId');
		tbutton.hide();
		},
		//隐藏打印按钮
...
//////*******************************************************************
		...
		if(rowObjDetail[0].get("midcheckFlag")=="已提交"){
			Ext.Msg.show({title:'注意',msg:'已提交申请数据不能再删除!',buttons: Ext.Msg.OK,icon:Ext.MessageBox.WARNING});
			return;
	    }
...
//////*******************************************************************
		...
		Ext.each(records, function(r) {
			...
			if (r.data.newRecord) {
				...
				if(rowObj[0].get("midcheckFlag")=="已提交"){
					Ext.Msg.show({title:'注意',msg:'已提交申请数据不能再修改!',buttons: Ext.Msg.OK,icon:Ext.MessageBox.WARNING});
					return;
			    }
			...
			data.push(o);
				var saveUrl = this.url + '?action=' + recordType + stro.toString()
						+ "&" + tmpstro.toString() + "&"
						+ Ext.urlEncode(this.urlParam)+'&RowID='+RowID;
			...
			p = {
				...
				success : function(result, request) {
				   ...
				   //itemGrid.getSelectionModel().getSelections();
	...
	load : function(param) {
		...
		var limits=Ext.getCmp("PageSizePluginws").getValue();
			 //alert(limits);
	         if(!limits){limits=25};
