...
dhc.herp.Gridvouchprogress = Ext.extend(Ext.grid.EditorGridPanel, {
	...
	createBbar : function() {
		this.bbar = new Ext.PagingToolbar({// 分页工具栏
			...
			store : this.store,
			displayInfo : true,
			plugins : new dhc.herp.PageSizePluginess(),
	...
	load : function(param) {
		var limits=Ext.getCmp("PageSizePluginess").getValue();
			 //alert(limits);
	         if(!limits){limits=25};
		if (!Ext.isEmpty(param))
...
dhc.herp.PageSizePluginess = function() {
	dhc.herp.PageSizePluginess.superclass.constructor.call(this, {
...
Ext.extend(dhc.herp.PageSizePluginess, Ext.form.ComboBox, {
