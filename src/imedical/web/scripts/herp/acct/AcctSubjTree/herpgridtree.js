...
dhc.herp.Grid = Ext.extend(Ext.grid.EditorGridPanel, {
		...
		Ext.MessageBox.confirm('提示', '确定要删除选定的行吗', function(btn) {
			if (btn == 'yes') {
				Ext.each(records, function(record) {
							...
							if (Ext.isEmpty(record.get("CheckMaprowid"))) {
								// record.reject(true);
							...
							Ext.Ajax.request({
										url : tmpUrl + '?action=del&MainRowid='
												+ record.get("CheckMaprowid"),
										waitMsg : '删除中...',
	...
	save : function() {
		...
		Ext.each(records, function(r) {
			//alert(tmpNode+'tmpNode');
		// var rowObjlyf=
		// var rowObjlyf=itemGrid.getSelectionModel().getSelections();
		// var MainRowid=rowObjlyf[0].get("rowid");
		var MainRowid=tmpNode.attributes["rowid"];
			var o = {};
			var tmpstro = "&CheckMaprowid=" + r.data['CheckMaprowid']+'&MainRowid='+MainRowid;
				...
				success : function(result, request) {
					...
					if (jsonData.success == 'true') {
						...
						if (jsonData.info == 'RecordExist')
							message = '输入的记录已经存在!';
						if (jsonData.info == 'SubjNull')
							message = '输入的记录已经存在!';
						if (jsonData.info == 'OnWork')
							message = '科目已启用,无法删除!';
						if (jsonData.info == 'RepRecorde')
							message = '项目已存在,请勿重复添加!';
	...
	submitRecord : function() {
		...
		this.getStore().proxy = new Ext.data.HttpProxy({
					url : this.url + '?action=listcheck' + stro
				});
		var data = [];
