		...
		Ext.MessageBox.confirm('提示', '确定要删除选定的行吗', function(btn) {
			if (btn == 'yes') {
				Ext.each(records, function(record) {
							...
							/* if (Ext.isEmpty(record.get("AcctBusiSupplierMapID"))) {
								// record.reject(true);
 ...
 */
							...
							Ext.Ajax.request({
										url : tmpUrl + '?action=del&AcctBusiSupplierMapID='
												+ record.get("AcctBusiSupplierMapID"),
										waitMsg : '删除中...',
	...
	save : function() {
		...
		Ext.each(records, function(r) {
			...
			var o = {};
			var tmpstro = "&AcctBusiSupplierMapID=" + r.data['AcctBusiSupplierMapID'];
			...
			for (var i = 0; i < this.fields.length; i++) {
				...
				if (tmobj != null) {
					...
					if (tmobj.allowBlank == false) {
						...
						//title 中加了css样式，截取名称
						var arr=title.split(">",2)
							title=arr[1].split("<",1)
						if ((r.data[indx]== "")
								|| (parseInt(r.data[indx]) == 0)) {
				...
				success : function(result, request) {
					...
					if (jsonData.success == 'true') {
						...
						if (jsonData.refresh == 'true') {
							 //刷新后停留在当前页
							var tbarnum = this.getBottomToolbar();  
							tbarnum.doLoad(tbarnum.cursor);
							/*console.log(tbarnum,tbarnum.cursor);
							alert(tbarnum.cursor)
							tmpStore.load({
								params : {
									start : Ext
