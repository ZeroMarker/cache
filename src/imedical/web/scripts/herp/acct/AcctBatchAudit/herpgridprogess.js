...
dhc.herp.Gridvouchprogress = Ext.extend(Ext.grid.EditorGridPanel, {
	edit : true,
	trackMouseOver : true,
	stripeRows : true,
	cm : colModel,
	...
	initComponent : function() {
		...
		dhc.herp.Gridvouchprogress.superclass.initComponent.call(this);
	},
	createPlugins : function() {
		...
		plugins.push(dhc.herp.GridvouchprogressTip);
	...
	createBbar : function() {
		this.bbar = new Ext.PagingToolbar({// 分页工具栏
			...
			store : this.store,
			displayInfo : true,
			plugins : new dhc.herp.PageSizePluginOR(),
				...
				success : function(result, request) {
					...
					if (jsonData.success == 'true') {
						...
						if (jsonData.refresh == 'true') {
							...
							addmainGrid.load({params:{start:0, limit:25}});
						}
					} else {
						if (jsonData.refresh == 'true') {
							tmpStore.load({
	...
	submitRecord : function() {
		...
		this.getStore().proxy = new Ext.data.HttpProxy({
					url : this.url + '?action=listd' + stro
				});
		var data = [];
	...
	load : function(param) {
		var limits=Ext.getCmp('PageSizePluginOR').getValue();
			 //alert(limits);
	    if(!limits){limits=25};
		if (!Ext.isEmpty(param))
		...
		//console.log(this.getBottomToolbar);
		this.getStore().load({
			params : {
				start : Ext.isEmpty(this.getBottomToolbar().cursor) ? 0 : this
...
dhc.herp.PageSizePluginOR = function() {
	dhc.herp.PageSizePluginOR.superclass.constructor.call(this, {
...
Ext.extend(dhc.herp.PageSizePluginOR, Ext.form.ComboBox, {
