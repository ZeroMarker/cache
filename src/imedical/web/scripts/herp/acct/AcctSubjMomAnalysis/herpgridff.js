...
dhc.herp.Grid = Ext.extend(Ext.grid.EditorGridPanel, {
	...
	createStore : function() {
		...
		// 远程
			...
			this.store = new Ext.data.Store({
						proxy : new Ext.data.HttpProxy({
									url : this.url + '?action=ListSubjMom&userdr='+UserID
								}),
						reader : new Ext.data.JsonReader({
									root : 'rows',
									totalProperty : 'results'
	...
	save : function() {
		...
		Ext.each(records, function(r) {
			...
			var recordType;
			if (r.data.newRecord) {
				recordType = "addSubjMom";
				
			} else {
				recordType = "editSubjMom";
				//alert(tmpstro);
			...
			p = {
				...
				success : function(result, request) {
					...
					if (jsonData.success == 'true') {
						...
						var message = "";
						message = recordType == 'addSubjMom' ? '添加成功!' : '保存成功!'
						Ext.Msg.show({
									title : '注意',
									msg : message,
									buttons : Ext.Msg.OK,
		...
		var saveUrl = this.url + '?action=editSubjMom'+'&rowid='+rowObj[0].get("rowid") +'&descs='+encodeURIComponent(newdesc);
			// alert(saveUrl)
        var m = {
				url : saveUrl,
				method : 'GET',
