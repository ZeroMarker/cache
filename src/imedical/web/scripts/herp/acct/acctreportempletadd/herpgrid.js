...
dhc.herp.Grid = Ext.extend(Ext.grid.EditorGridPanel, {
	...
	del : function() {
		...
		var CheckState=records[0].data['CheckState'];
		var IsEarly=records[0].data['IsEarly'];
				...
				Ext.each(records, function(record) {
							...
							Ext.Ajax.request({
										...
										success : function(result, request) {
											...
											itemDetail.load({params:{start:0, limit:25,MainRowid:''}});	
											} else {
												var message = "SQLErr: "
														+ jsonData.info;
												Ext.Msg.show({
	...
	save : function() {
		...
		Ext.each(records, function(r) {
			...
			// if(indx=="ReportExplain"){
						
			// var file=reValue;
			// var uploadUrl="http://192.186.5.4:8080/FileUploadFtp/UploadServlet";
			// var upUrl=uploadUrl+"?file="+file
			// alert(upUrl);
