...
dhc.herp.Gridlyf = Ext.extend(Ext.grid.EditorGridPanel, {
	...
	createBbar : function() {
		this.bbar = new Ext.PagingToolbar({// 分页工具栏
			...
			store : this.store,
			displayInfo : true,
			plugins : new dhc.herp.PageSizePluginhss(),
	...
	//alert(CheckState);
		if ((store.recordType)&&(CheckState=="新建")) {
	...
	del : function() {
	    ...
	    var rowObj = itemMain.getSelectionModel().getSelections();
	    var rowObjdetil = this.getSelectionModel().getSelections();
	    var len = rowObjdetil.length;
	    if (len < 1) {
		    Ext.Msg.show({
					title : '注意',
					msg : '请单击选择对应的行记录!  ',
		...
		var CheckState=rowObj[0].data['CheckState'];
		var IsEarly=rowObj[0].data['IsEarly'];
		if(CheckState=="审核"){
		    Ext.Msg.show({
						title : '警告',
						msg : '方案已审核，不能删除!  ',
		...
		Ext.MessageBox.confirm('提示', '确定要删除选定的行吗  ', function(btn) {
			var rowObjlyf=itemMain.getSelectionModel().getSelections();
		    var projadjdr=rowObjlyf[0].get("parowid")
			if (btn == 'yes') {
				Ext.each(records, function(record) {
							...
							Ext.Ajax.request({
										url : tmpUrl + '?action=del&rowid='
												+ record.get("rowid")+'&parowid='+projadjdr+'&userId='+userid+'&uname='+encodeURIComponent(uname),
										waitMsg : '删除中...',
	...
	save : function() {
		...
		Ext.each(records, function(r) {
			...
			var tmpstro = "&rowid=" + r.data['rowid']+"&RowNumber=" + r.data['RowNumber'];
			...
			for (var i = 0; i < this.fields.length; i++) {
                ...
                // alert(tmpstro);
					if (tmobj.allowBlank == false) {
						...
						var stitle=title.split(">")[1];
						//alert(stitle);
						title=stitle.split("<")[0];
			...
			//alert(aaa)
		    var rowObjlyf=itemMain.getSelectionModel().getSelections();
			var MainRowid=rowObjlyf[0].get("rowid");
			if (r.data.newRecord) {
				recordType = "add";
			    ...
			    var saveUrl = this.url + '?action=' + recordType + stro.toString()
					+ "&" + tmpstro.toString() + "&"
					+ Ext.urlEncode(this.urlParam);  //+'&MainRowid='+MainRowid
			} else {
				recordType1 = "edit";
				o[this.idName] = r.get(this.idName);
			    data.push(o);
			...
			p = {
				...
				success : function(result, request) {
					...
					if (jsonData.success == 'true') {
						...
						var tbarnum = itemDetail.getBottomToolbar();  
...
dhc.herp.PageSizePluginhss = function() {
	dhc.herp.PageSizePluginhss.superclass.constructor.call(this, {
...
Ext.extend(dhc.herp.PageSizePluginhss, Ext.form.ComboBox, {
