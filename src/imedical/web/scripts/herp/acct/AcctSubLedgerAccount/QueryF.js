...
function Query() {
	...
	var SchemGrid = new dhc.herp.Grid({
			...
			iconCls:'maintain',
			region: 'center',
			url: 'herp.acct.AcctSubLedgerAccountFexe.csp',
			split: true,
			viewConfig: {
				forceFit: true
			},
	...
	//SchemGrid.addButton('-');
	//SchemGrid.addButton('<div style="color:red">说明：辅助账明细账-外币金额式的方案编号以ZB1203开头。</div>');
	SchemGrid.load({
		params: {
			start: 0,
			limit: 10,
	...
	AcctSubjDs.on('beforeload', function (ds, o) {
		ds.proxy = new Ext.data.HttpProxy({
				url: '../csp/herp.acct.AcctSubLedgerAccountFexe.csp?action=GetAcctSubjF&bookId=' + bookID,
				method: 'POST'
			});
	});
	//辅助核算科目ComboBox
	...
	function FindQuery() {
		...
		p_URL = 'dhccpmrunqianreport.csp?reportName=herp.acct.AcctSubLedgerAccountF.raq&SubjName=' + AcctSubj + '&StartDate=' + StartDate +
