...
dhc.herp.Grid = Ext.extend(Ext.grid.EditorGridPanel, {
	...
	createStore : function() {
		...
		// 远程
			...
			this.store = new Ext.data.Store({
						proxy : new Ext.data.HttpProxy({
									url : this.url + '?action=ListSubjFixBase&bookID='+AcctBookID	//userdr='+UserID
								}),
						reader : new Ext.data.JsonReader({
									root : 'rows',
									totalProperty : 'results'
	...
	save : function() {
        ...
        var SubjCode1=Ext.getCmp('SubjCodeNameField1').getRawValue();
		var SubjCode2=Ext.getCmp('SubjCodeNameField2').getRawValue();
		var AcctYearMonth1=Ext.getCmp('AcctYearMonthField1').getRawValue();	
		var AcctYearMonth2=Ext.getCmp('AcctYearMonthField2').getRawValue();	
		var subjlevel=Ext.getCmp('SubjLevelField').getValue();
		var AnalysisType=Ext.getCmp('AnalysisTypeField').getValue();

		var newdesc=SubjCode1+';'+SubjCode2+';'+AcctYearMonth1+';'+AcctYearMonth2+';'+subjlevel+';'+AnalysisType;
		...
		Ext.each(records, function(r) {
			...
			var recordType;
			if (r.data.newRecord) {
				recordType = "addSubjFixBase";
				
			} else {
				recordType = "editSubjFixBase";
				//alert(tmpstro);
			...
			p = {
				...
				success : function(result, request) {
					...
					if (jsonData.success == 'true') {
						...
						var message = "";
						message = recordType == 'addSubjFixBase' ? '添加成功!' : '保存成功!'
						Ext.Msg.show({
									title : '注意',
									msg : message,
									buttons : Ext.Msg.OK,
		...
		var saveUrl = this.url + '?action=editSubjFixBase'+'&rowid='+rowObj[0].get("rowid") +'&descs='+encodeURIComponent(newdesc);
			// alert(saveUrl)
        var m = {
				url : saveUrl,
				method : 'GET',
	...
	submitRecord : function() {
		...
		this.getStore().proxy = new Ext.data.HttpProxy({
					url : this.url + '?action=list1' + stro
				});
		var data = [];
