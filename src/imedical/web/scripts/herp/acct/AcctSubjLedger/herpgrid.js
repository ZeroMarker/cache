...
dhc.herp.Grid = Ext.extend(Ext.grid.EditorGridPanel, {
	...
	del : function() {
		...
		if(this.edit==false)
		{
			...
			Ext.Msg.show({title:'注意',msg:'初始余额录入已完成，不可删除!',buttons: Ext.Msg.OK,icon:Ext.MessageBox.WARNING});
			return;
		}
					...
					Ext.each(records, function(record) {
						...
						Ext.Ajax.request({
									url : tmpUrl + '?action=del&rowid='
											+ record.get("rowid")+'&LedgerID='+record.get("AcctLedgerID"),
									waitMsg : '删除中...',
	...
	save : function() {
		...
		Ext.each(records, function(r) {
			...
			for (var i = 0; i < this.fields.length; i++) {
				...
				var indx = this.getColumnModel().getColumnId(i)
				//alert(indx);
			...
			if (r.data.newRecord) {
				...
				//this.url='herp.acct.acctsubjledgerexe.csp';
				if(subjnumfcCombo.getValue()==1){
					this.url='herp.acct.acctsubjledgerexe.csp';
				}
				if(subjnumfcCombo.getValue()==2){
					this.url='herp.acct.acctsubjledgernumexe.csp';
				}
				if(subjnumfcCombo.getValue()==3){
					this.url='herp.acct.acctsubjledgerfcexe.csp';
				}
			}
			o[this.idName] = r.get(this.idName);
			data.push(o);
			//var records = itemGrid.getSelectionModel().getSelections();
			//var SubjID=records[0].get('AcctSubjID')
			var SubjID = "&SubjID=" + r.data['AcctSubjID'];
			var SubjCode = "&SubjCode=" + r.data['AcctSubjCode'];
			var LedgerID = "&LedgerID=" + r.data['AcctLedgerID'];
			
			//alert(SubjID);
			var saveUrl = this.url + '?action=' + recordType + stro.toString()
					+ "&" + tmpstro.toString() + "&"
					+ Ext.urlEncode(this.urlParam)+"&" + SubjID.toString()+"&" + SubjCode.toString() 
					+ "&" + LedgerID.toString()
					+ "&" + "bookID="+bookID;
			//alert(saveUrl);
