...
dhc.herp.Gridp = Ext.extend(Ext.grid.EditorGridPanel, {
	...
	createStore : function() {
		...
		// 远程
			...
			this.store = new Ext.data.Store({
						proxy : new Ext.data.HttpProxy({
									url : this.url + '?action=ListTemplete'
								}),
						reader : new Ext.data.JsonReader({
									root : 'rows',
									totalProperty : 'results'
	...
	createBbar : function() {
		this.bbar = new Ext.PagingToolbar({// 分页工具栏
			...
			store : this.store,
			displayInfo : true,
			plugins : new dhc.herp.PageSizePluginp(),
	...
	submitRecord : function() {
		...
		this.getStore().proxy = new Ext.data.HttpProxy({
					url : this.url + '?action=ListTemplete' + stro
				});
		var data = [];
...
dhc.herp.PageSizePluginp = function() {
	dhc.herp.PageSizePluginp.superclass.constructor.call(this, {
...
Ext.extend(dhc.herp.PageSizePluginp, Ext.form.ComboBox, {
