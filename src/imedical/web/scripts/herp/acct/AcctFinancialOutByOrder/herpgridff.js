...
dhc.herp.Gridff = Ext.extend(Ext.grid.EditorGridPanel, {
	...
	createBbar : function() {
		this.bbar = new Ext.PagingToolbar({// 分页工具栏
			...
			displayInfo : true,
			atLoad : true,
			plugins : new dhc.herp.PageSizePluginff(),
...
/*
	...
	load : function(param) {
		var limits=Ext.getCmp("PageSizePluginff").getValue();
			 //alert(limits);
	         if(!limits){limits=25};
		if (!Ext.isEmpty(param))
...
dhc.herp.PageSizePluginff = function() {
	dhc.herp.PageSizePluginff.superclass.constructor.call(this, {
...
Ext.extend(dhc.herp.PageSizePluginff, Ext.form.ComboBox, {
