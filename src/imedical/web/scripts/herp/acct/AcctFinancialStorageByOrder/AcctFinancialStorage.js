...
var projUrl = "herp.acct.acctfinancialstoragByOrderexe.csp";
var acctbookid = "";
var userid = "";
var vouchNO = "";
...
var itemGrid = new dhc.herp.Grid({
		...
		fields: [
			new Ext.grid.CheckboxSelectionModel({
				...
				id: 'BillNO',
				header: '<div style="text-align:center">发票号</div>',
				width: 90,
				align: 'center',
				editable: false,
				allowBlank: true,
...
var selected = itemGrid.getSelectionModel().getSelections();
	...
	Ext.Ajax.request({
		url: '../csp/herp.acct.acctfinancialstoragByOrderexe.csp?action=CreateVouch&rowid=' + FinancialReviewDR + '&vouchdate=' + VouchDate + '&AcctBookID=' + acctbookid + "&userid=" + userid,
		method: 'POST',
		success: function (result, request) {
			var jsonData = Ext.util.JSON.decode(result.responseText);
			if (jsonData.success == 'true') {
...
var createVouch = function () {
	...
	Ext.Ajax.request({
		url: 'herp.acct.acctfinancialstoragByOrderexe.csp?action=ifNotConfigured&rowid=' + rowid + '&AcctBookID=' + acctbookid + "&userid=" + userid,
		method: 'POST',
		success: function (result, request) {
			var jsonData = Ext.util.JSON.decode(result.responseText);
			if (jsonData.success == 'true') {
				...
				if (information == "NoSubj") {
					...
					var Loc = information.split("*")[1]
						Ext.Msg.show({
							title: '提示',
							width: 300,
							msg: '生成凭证失败！未找到[<span style="color:blue">' + Loc + '</span><br/>]对应的供应商类别,',
							buttons: Ext.Msg.OK,
							icon: Ext.MessageBox.INFO
