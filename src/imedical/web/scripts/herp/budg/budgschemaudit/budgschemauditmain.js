...
var schemeUrl = 'herp.budg.budgschemauditexe.csp';
...
var YearDs = new Ext.data.Store({
	...
	YearDs.on('beforeload', function(ds, o){
		...
		ds.proxy=new Ext.data.HttpProxy({url:commonboxURL+'?action=year&flag=&str='+encodeURIComponent(Ext.getCmp('YearCombo').getRawValue()),method:'POST'});
...
var searchbotton = new Ext.Toolbar.Button({
	...
	handler: function(){
		...
		budgschemauditDs.proxy = new Ext.data.HttpProxy({
						...
						+'&type='+schemeTypeCombo.getValue()+'&name='+encodeURIComponent(Ext.getCmp('nameField').getValue())
						+'&start=0&limit=25',
						method : 'GET'
					});
