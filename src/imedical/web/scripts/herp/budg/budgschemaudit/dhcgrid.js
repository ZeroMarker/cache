...
Dhc.ca.Grid = Ext.extend(Ext.grid.EditorGridPanel, {
	...
	createColumns : function() {
		...
		// Ext.apply(this.cm,new Ext.grid.ColumnModel(cols))
		/*
		 * tmobj=this.cm.getColumnById("InvCode") if (tmobj!=null){
		 * alert(tmobj.header+":"+tmobj.allowBlank) }
		 */
		/*
		 * ID=this.cm.getIndexById("InvName") alert("ID="+ID)
		 * 
		 * alert("DataIndex="+this.cm.getDataIndex(3))
		 */
			...
			for (var i = 0; i < this.fields.length; i++) {
				...
				if (tmobj != null) {
					if (tmobj.allowBlank == false) {
						...
						//alert(r.data[indx]);
						//alert(title + ":" + r.data[indx].toString() + ":"+ tmobj.allowBlank)
...
/*
					var tmobj = this.getColumnModel().getColumnById(f)
					if (tmobj != null) {
						...
						// alert(tmobj.header+":"+f+":"+tmobj.allowBlank)
					}
					*/
					o[f] = r.data[f];
					if (r.data[f] != null) {
