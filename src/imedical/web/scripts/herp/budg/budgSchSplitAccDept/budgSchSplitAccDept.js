...
var budgetDs = new Ext.data.Store({
	...
	budgetDs.on('beforeload', function(ds, o){		
		ds.proxy=new Ext.data.HttpProxy({url:'herp.budg.budgcommoncomboxexe.csp?action=itemtype&flag=1',method:'POST'});		
	});		
...
var findButton = new Ext.Toolbar.Button({
	...
	handler:function(){
		...
		//alert(itemGrid.pageSize)
		itemGrid.load(({params:{start : Ext.isEmpty(itemGrid.getTopToolbar().cursor)
										? 0: itemGrid.getTopToolbar().cursor,
							    limit : itemGrid.pageSize,
							    
							  	year:year,type:btype,bname:bname,bcode:bcode}}));
	}
});	
            ...
            'cellclick' : function(grid, rowIndex, columnIndex, e) {
                var record = grid.getStore().getAt(rowIndex);
                 ...
                 //alert(record.get('isLast'))
                    if ((record.get('isLast') == '0')&& ((columnIndex == 11)||(columnIndex == 10))) {
                   // alert(222)
                         return false;
                     } else {return true;}
               },
