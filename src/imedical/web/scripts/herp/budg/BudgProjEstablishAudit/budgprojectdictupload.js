//创建图片浏览窗口(弹出)
projuploadFun = function CreatePicWin(rowid)
{
    //初始化取消按钮
    var delButton = new Ext.Toolbar.Button({
        ...
        handler:function()
        {
            ...
            {Msg.info("error","请选择要删除的图片!");return}
             ...
             Ext.MessageBox.confirm('提示','确定要删除选定图片?',
                    function(btn) {
                        if(btn == 'yes'){
                            ...
                            Ext.Ajax.request({
                                url:'herp.budg.budgprojectdictexe.csp?action=DeletePic&rowid='+rowid+'&picsrc='+picsrc,
                                waitMsg:'删除中...',
    ...
    var PicUrl = 'herp.budg.budgprojectdictexe.csp?action=GetPic&rowid='+rowid;
    // 通过AJAX方式调用后台数据
    ...
    var  src="http://172.16.100.50/ftpdir/BUDGUPLOAD/"    //图片路径
