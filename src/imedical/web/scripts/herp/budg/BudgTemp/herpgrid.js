...
dhc.herp.Grid = Ext.extend(Ext.grid.EditorGridPanel, {
	...
	createStore : function() {
		...
		// 远程
			...
			this.store = new Ext.data.Store({
						proxy : new Ext.data.HttpProxy({
									url : this.url + '?action=list&userid='+userid+'&userdept='+userdept
								}),
						reader : new Ext.data.JsonReader({
									root : 'rows',
									totalProperty : 'results'
	...
	save : function() {
		...
		Ext.each(records, function(r) {
			...
			var saveUrl = this.url + '?action=' + recordType + stro.toString()
					+ "&" + tmpstro.toString() +'&userid='+userid+'&userdept='+userdept+ "&"
	...
	submitRecord : function() {
		...
		this.getStore().proxy = new Ext.data.HttpProxy({
					url : this.url + '?action=list' + stro+'&userid='+userid+'&userdept='+userdept
				});
		var data = [];
