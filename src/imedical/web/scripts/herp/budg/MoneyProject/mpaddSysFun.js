...
var findButton = new Ext.Toolbar.Button({
	...
	handler:function(){
		...
		AddSysGrid.load(({params:{start:0, limit:25,sname:sysname,lcode:""}}));
	}
});		
...
AddSysGrid.load(({params:{start:0, limit:25,sname:"",lcode:""}}));	
//定义添加按钮响应函数
addHandler = function(){
        var rowObj=AddSysGrid.getSelectionModel().getSelections();
	    var len = rowObj.length;
	    if(len < 1){
		    Ext.Msg.show({title:'注意',msg:'请至少选择一个系统！',buttons: Ext.Msg.OK,icon:Ext.MessageBox.WARNING});
		    return;
	       }
	    var codes="";
	    for(var i = 0; i < len; i++){
	    	var Code = rowObj[i].get("rowid");
			codes=codes+"^"+Code;
			sysField.setValue(codes);
			}	 
		  window.close();
