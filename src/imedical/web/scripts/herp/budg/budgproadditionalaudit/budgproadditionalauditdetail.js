...
var BudgProAdditionalAuditDetailUrl = '../csp/herp.budg.budgproadditionalauditdetailexe.csp';
...
var saveButton = new Ext.Toolbar.Button({
	...
	handler:function(){
		AddFun1();
		itemDetail.store.on("load",function(){  
        itemDetail.getSelectionModel().selectRow(row,true);  
    	});
...
var itemDetail = new dhc.herp.Gridlyf({
    title: '项目预算审核明细',
    region : 'center',
    url: 'herp.budg.budgproadditionalauditdetailexe.csp',
    tbar:[saveButton],
    listeners:{
...
AddFun1=function() {
		...
		Ext.each(records, function(r) {
			...
			//alert(r.data['PerOrigin']);
			datad=datad+'|'+r.data['itemcode']+'|'+r.data['budgnum']+'|'+r.data['budgprice'].replace(/,/g,'')+'|'+encodeURIComponent(r.data['budgdesc'])
			+'|'+r.data['FeeScale']+'|'+r.data['AnnBenefit']+'|'+r.data['MatCharge']+'|'+r.data['SupCondit']+'|'+r.data['Remarks']+'|'+r.data['budgprice']+'|'
			+r.data['brand1']+'|'+r.data['spec1']+'|'+r.data['brand2']+'|'+r.data['spec2']+'|'+r.data['brand3']+'|'+r.data['spec3']+'|'+encodeURIComponent(r.data['isaddedit'])+"|"+r.data['PerOrigin']
			+"|"+userid+'|'+uname;
			
			var saveUrl = 'herp.budg.budgproadditionaldetailexe.csp?action='+recordType+tmpstro+'&data='+datad+Ext.urlEncode(this.urlParam);
