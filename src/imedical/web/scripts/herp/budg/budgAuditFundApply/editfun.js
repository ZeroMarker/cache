...
var dname=selectedRow[0].data['Dept'];
var uname=selectedRow[0].data['User'];
var tmpUserDR=selectedRow[0].data['UserDR'];
...
var tmpCheckid=selectedRow[0].data['Checkdr'];
...
var printButton = new Ext.Toolbar.Button({
	...
	//alert(billcode);
	/*fileName="{herp.budg.report.budgfundpaybillprint.raq(billcode1="+billcode+")}";
	//alert("ss");
	DHCCPM_RQDirectPrint(fileName);*/
	
	Ext.Ajax.request({
				url:'herp.budg.budgctrlfundbillmngexe.csp?action=print&billcode='+billcode,
				waitMsg:'打印中...',
				failure: function(result, request) {
					Ext.Msg.show({title:'错误',msg:'请检查网络连接!',buttons: Ext.Msg.OK,icon:Ext.MessageBox.ERROR});
				},
				success: function(result, request) {
					...
					if (jsonData.success=='true') {
						...
						printApply(listm,listd,rows,billcode)			
					}else{
...
var editmainGrid = new dhc.herp.Grid({
				...
				listeners : {
		            'cellclick' : function(grid, rowIndex, columnIndex, e) {
		                var record = grid.getStore().getAt(rowIndex);
		                  // 根据条件设置单元格点击编辑是否可用
		                  //alert(columnIndex);
		                  if ((columnIndex == 5)&&(record.get('rowid')=="")&&(record.get('ItemName')!="")) {
			                  var yearmonth = yearmonField.getValue();
			                  var dirty = false;
			                  var itemcode="";
		            ...
		            'celldblclick' : function(grid, rowIndex, columnIndex, e) {
						...
						if ((columnIndex == 5)&&(record.get('rowid')=="")&&(record.get('ItemName')!="")) {
							...
							var dirty = false;
					          ...
					          var DeptDR = audnamefield.getValue();
							  var arr = DeptDR.split("_");DeptDR=arr[0];
							  var data=DeptDR+"^"+yearmonth+"^"+itemcode+"^"+record.get('State')+"^"+record.get('rowid')+"^Req^^^";
					        	Ext.Ajax.request({
								url: '../csp/herp.budg.budgctrlfundbillmngexe.csp?action=getcurbalance&data='+encodeURIComponent(data),				
								success: function(result, request){
									var jsonData = Ext.util.JSON.decode( result.responseText );
	...
	Add1Fun=function() {
	    ...
	    editmainGrid.getSelectionModel().selectRow(0);
	    var getSelection = editmainGrid.getSelectionModel().getSelections();
	    //alert(getSelection);
...
editmainGrid.addButton(printButton);
