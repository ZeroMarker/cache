...
dhc.herp.Grid = Ext.extend(Ext.grid.EditorGridPanel, {
	...
	del : function() {
		...
		if (len>0){			
								...
								Ext.Ajax.request({
											...
											success : function(result, request) {
												...
												if (jsonData.success == 'true') {
													...
													tmpStore.load({
																params : {
																	start : Ext.isEmpty(tmpG.getBottomToolbar().cursor) ? 0: tmpG.getBottomToolbar().cursor,
									                                limit : tmpG.getBottomToolbar().pageSize
																}
															});
												} else {
													var message = "SQLErr: "
			...
			Ext.Msg.show({title:'提示',msg:'请选择要删除的行!',buttons: Ext.Msg.OK,icon:Ext.MessageBox.WARNING});
			return false;
		}		
	},
	createTbar : function() {
				...
				success : function(result, request) {
					...
					if (jsonData.success == 'true') {
						...
						if (jsonData.refresh == 'true') {
							/*alert("1111"+this.getBottomToolbar().getPageData().activePage );
							alert(this.getBottomToolbar().cursor);
							alert(Ext.isEmpty(this.getBottomToolbar().cursor) ? 0 : this.getBottomToolbar().cursor);*/
							tmpStore.load({
								params : {
									start : Ext
											.isEmpty(this.getBottomToolbar().cursor)
											? 0
