...
dhc.herp.Gridapplyadddetail = Ext.extend(Ext.grid.EditorGridPanel, {
	edit : true,
	trackMouseOver : true,
	stripeRows : true,
	cm : colModel,
	...
	initComponent : function() {
		...
		dhc.herp.Gridapplyadddetail.superclass.initComponent.call(this);
	},
	createPlugins : function() {
		...
		plugins.push(dhc.herp.GridapplyadddetailTip);
	...
	save : function() {
		...
		Ext.each(records, function(r) {
			...
			var saveUrl = this.url + '?action=' + recordType + stro.toString()
					+ "&" + tmpstro.toString() + "&"
					+'&mdesc='+Descfield.getValue()+'&fundbilldr='+FundBillDR				
					+'&deptdr='+deptcode+'&userdr='+userid+'&projdr='+projDr+
				...
				success : function(result, request) {
					...
					if (jsonData.success == 'true') {
						...
						if (jsonData.refresh == 'true') {
							...
							//adddetailGrid.load({params:{start:0, limit:25}});
						}
					} else {
						if (jsonData.refresh == 'true') {
							tmpStore.load({
	...
	submitRecord : function() {
		...
		this.getStore().proxy = new Ext.data.HttpProxy({
					url : this.url + '?action=list2' + stro
				});
		var data = [];
...
dhc.herp.GridapplyadddetailTip = {
	init : function(g) {
		var message = "";
