...
ScenarioNameDs.on('beforeload', function(ds, o){
	ds.proxy=new Ext.data.HttpProxy({	
		url: projUrl+'?action=scenarioname'+'&UserID='+UserID+'&adjustno='+AdjustNo.getValue(),method:'POST'}) // +'&adjustNo='+AdjustNo.getValue()
		});
...
var calculateButton = new Ext.Toolbar.Button({
			...
			myMask.show();
			Ext.Ajax.request({
			url : budgDataInitUrl + '?action=yearbudgetingdeptsplit&year='+ syear+'&schemdr='+Schemname,
...
var itemMain = new dhc.herp.Grid({
	...
	tbar:['会计年度:',yearField,'-','调整序号:',AdjustNo,'-','方案名称:',ScenarioName,'-',searchButton],
...
itemMain.on('cellclick', function(g, rowIndex, columnIndex, e) {
	...
	itemDetail.load({params:{start:0, limit:12,SchemDr:SchemDr,objdeptdr:objdeptdr,bsmyear:bsmyear}});
