...
DeptNameDs.on('beforeload', function(ds, o){
	ds.proxy=new Ext.data.HttpProxy({url:commonboxUrl+'?action=dept&flag=1&str='+encodeURIComponent(Ext.getCmp('DeptNameFieldCom').getRawValue()),method:'POST'})
});
var DeptNameFieldCom  = new Ext.form.ComboBox({
	id: 'DeptNameFieldCom',
	fieldLabel: '科室',
	width : 100,
	listWidth : 220,
...
var printBT = new Ext.Toolbar.Button({
	...
	var DeptDR=DeptNameFieldCom.getValue();
	if(year==""){
						Ext.Msg.show({title:'提示',msg:'年月不能为空!',buttons:Ext.Msg.OK,icon:Ext.MessageBox.INFO,width:160});
						return false;
	}

	fileName="{HERPBUDGDepBudgAdjustInfoReqPrint.raq(year="+year+";itemInfo="+itemInfo+";DeptDR="+DeptDR+";HOSPID="+HOSPID+";userdr="+userdr+")}";
...
var searchPanel = new Ext.form.FormPanel({
			...
			items : [{
						...
						items: [{
								xtype:'displayfield',
								value:'<center><p style="font-weight:bold;font-size:180%">'+'科室预算调整查询'+'</p></center>',
								columnWidth:1,
								height:'40'
								}]
	   				},{
...
function ShowReport()
{
	...
	//if (year=="") { year=0;}	
	//alert(year)
	var p_URL = 'dhccpmrunqianreport.csp?reportName=HERPBUDGDepBudgAdjustInfoReq.raq&year='+year+'&itemInfo='+itemInfo+'&DeptDR='+DeptDR+'&HOSPID='+HOSPID+'&userdr='+userdr+''; 
	//alert(itemInfo)
