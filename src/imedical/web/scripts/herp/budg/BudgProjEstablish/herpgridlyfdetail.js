...
dhc.herp.Gridlyf = Ext.extend(Ext.grid.EditorGridPanel, {
			...
			if (this.edit) {
				if (tmpObj.type instanceof Object) {
					...
					cols[i + 1].editor = new Ext.form.TextField({
								...
								width : tmpObj.width,
								// anchor: '90%',
								maxLength:tmpObj.maxLength,
								maxLengthText:tmpObj.maxLengthText,
								hidden : tmpObj.hidden,
								id : tmpObj.dataIndex,
								allowBlank : tmpObj.allowBlank
							})
	...
	if (len < 1) {
		...
		chkstate=rowObj[0].data['chkstate'];
		if(chkstate!=="0"){
			Ext.Msg.show({
					title : '注意',
					msg : '提交的单据不能删除!',
				...
				Ext.each(records, function(record) {
							...
							Ext.Ajax.request({
										url : tmpUrl + '?action=del&rowid='
												+ record.get("rowid")+'&projadjdr='+projadjdr+'&userId='+userid+'&uname='+encodeURIComponent(uname),
										waitMsg : '删除中...',
	...
	save : function() {
		...
		Ext.each(records, function(r) {
			...
			//alert(aaa)
			if (r.data.newRecord) {
			   var rowObjlyf=itemGrid.getSelectionModel().getSelections();
			   var projadjdr=rowObjlyf[0].get("projadjdr");
