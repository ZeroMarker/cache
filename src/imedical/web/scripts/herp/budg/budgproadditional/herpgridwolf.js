...
dhc.herp.Gridlyf = Ext.extend(Ext.grid.EditorGridPanel, {
	...
	if (len < 1 ) {
		...
		if ((store.recordType)&&(chkstate=="0")) {
	...
	if (len < 1) {
		...
		chkstate=rowObj[0].data['pachkstate'];
		if(chkstate!=="0"){
			Ext.Msg.show({
					title : '注意',
					msg : '提交的单据不能删除!',
	...
	save : function() {
		...
		Ext.each(records, function(r) {
			...
			//alert(aaa)
			if (r.data.newRecord) {
			   var rowObjlyf=itemDetail.getSelectionModel().getSelections();
			   var projadjdr=rowObjlyf[0].get("projadjdr");
			   var year=rowObjlyf[0].get("year");
				recordType = "add";
				o[this.idName] = r.get(this.idName);
			    data.push(o);
			    var saveUrl = this.url + '?action=' + recordType + stro.toString()
					+ "&" + tmpstro.toString() + "&"
					+ Ext.urlEncode(this.urlParam)+"&projadjdr="+projadjdr+'&year='+year;
		//alert(saveUrl);
			} else {
				//alert('sss')
				var rowObjlyf1=itemDetail.getSelectionModel().getSelections();
			    var projadjdr1=rowObjlyf1[0].get("projadjdr");
			    var year=rowObjlyf[0].get("year");
				recordType1 = "edit";
				o[this.idName] = r.get(this.idName);
			    data.push(o);
			    var saveUrl = this.url + '?action=' + recordType1 + stro.toString()
					+ "&" + tmpstro.toString() + "&"
					+ Ext.urlEncode(this.urlParam)+"&projadjdr="+projadjdr1+'&year='+year;
			}
			//o[this.idName] = r.get(this.idName);
			//data.push(o);
			//var saveUrl = this.url + '?action=' + recordType + stro.toString()
