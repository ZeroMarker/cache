...
var URL='herp.budg.budgschemauditdeptexe.csp';
...
var YearCombo = new Ext.form.ComboBox({
			...
			listeners:{
	            "select":function(combo,record,index){
		            schemDs.removeAll();     
					schemCombo.setValue('');
					schemDs.proxy = new Ext.data.HttpProxy({url: commonboxURL+'?action=bsm&flag=2&year='+combo.value,method:'POST'})  
...
schemDs.on('beforeload', function(ds, o) {  
		...
		ds.proxy = new Ext.data.HttpProxy({
				url : commonboxURL+'?action=bsm&flag=2&year='+year,
				method : 'POST'
		})
	});
