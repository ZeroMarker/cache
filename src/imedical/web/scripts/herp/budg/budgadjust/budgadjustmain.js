...
var startButton = new Ext.Toolbar.Button({
		...
		handler: function(){
    	    ...
    	    function(btn){
				...
				//for(var i=0;i<len;i++){
				Ext.Ajax.request({
				url:adjustURL+'?action=start&Year='+year+'&AdjustNo='+adjno,
				waitMsg:'启用中...',
...
/*****************
//alert(hospid)
var adjustUrl = adjustURL;
var addProxy = new Ext.data.HttpProxy({url: adjustUrl+'?action=list&hospid=1'});
...
//itemGrid.load({params:{start:0, limit:12, Year:'',AdjustNo:'', startdate:'', enddate:'', AdjustFile:'', IsApprove:'',hospid:hospid}});
...
itemGrid.on('cellclick', function(g, rowIndex, columnIndex, e) {
	...
	if (columnIndex == 6) { 
		...
		var CompName  = records[0].get("CompName");
		...
		//alert(CompDR)
		editFun(rowid,CompDR,CompName,Year, AdjustNo, AdjustDate, AdjustFile,Memo,IsApprove, IsElast,ElastMonth);
	    //alert("哈哈");
	}
