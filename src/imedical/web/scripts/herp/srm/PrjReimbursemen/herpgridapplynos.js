...
dhc.herp.Gridapplynos = Ext.extend(Ext.grid.EditorGridPanel, {
	edit : true,
	trackMouseOver : true,
	stripeRows : true,
	cm : colModel,
	...
	initComponent : function() {
		...
		dhc.herp.Gridapplynos.superclass.initComponent.call(this);
	},
	createPlugins : function() {
		...
		plugins.push(dhc.herp.GridapplynosTip);
	...
	save : function() {
		...
		var billstate=rowObj[0].get("BillState")
		//alert(billstate);
		/*   20161204开发 在点击修改按钮时就已经判断单据状态故此在这去掉次判断libairu
		if(billstate!="新建") {
			Ext.Msg.show({
				title : '注意',
				msg : '不是“新建”状态，不能进行“修改保存”操作!',
				buttons : Ext.Msg.OK,
		...
		var selectidDetail=this.getStore().getModifiedRecords();
		...
		Ext.each(records, function(r) {
			...
			var saveUrl = this.url + '?action=' + recordType + stro.toString()
				...
				//	+"&mdesc="+Descfield1.getValue()+"&fundbilldr"+applyCombo1.getValue();
			// alert(saveUrl)
			p = {
				url : saveUrl,
				method : 'GET',
