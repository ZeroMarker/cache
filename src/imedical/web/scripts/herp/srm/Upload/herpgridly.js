 ...
 */
 ...
 //科研上传URL
//var srmuploadurl="http://172.19.19.58:8080/FileUpload/UploadTestServlet";
//var srmuploadurl="ftp://ftpuser:Ftp!@#@172.19.19.58:1885";

//科研下载URL
//var user="dthealth"
//var pwd="DT_2016"
//var srmdownloadurl="ftp://"+user+":"+pwd+"@172.19.19.58:1885";
//var srmdownloadurl="ftp://172.19.19.58";

//科研绩效计算URL
//var srmpointcalcurl="http://172.19.19.58:8080";


//科研所有数据查询安全组
//var srmfindgroupdesc="科研管理系统(信息查询)";
//var srmpointcalcurl="http://172.19.19.58:8080";

//科研所有数据修改安全组
//var srmeditgroupdesc="科研管理系统(信息修改)";
 
//科研处安全组
//var srmrdgroupdesc="Demo Group";
...
dhc.herp.Gridly = Ext.extend(Ext.grid.EditorGridPanel, {
	edit : true,
	trackMouseOver : true,
	stripeRows : true,
	cm : colModel,
	...
	initComponent : function() {
		...
		dhc.herp.Gridly.superclass.initComponent.call(this);
	},
	createPlugins : function() {
		...
		plugins.push(dhc.herp.GridlyTip);
	...
	add : function() {
  //alert("先选择项目添加项目明细");----可以在响应函数中添加一些限定条件
  ///*********************************************************************
	...
	var state = rowObj[0].get("DataStatuslist");
				//alert(state);			
	if(state == "已提交" ){
     Ext.Msg.show({title:'警告',msg:'数据已提交，不可再添加明细信息！',buttons: Ext.Msg.OK,icon:Ext.MessageBox.WARNING});
     return;
     }
   ///********************************************************************  	
...
/////********************************************************************
	...
	//var rowObjDetail = itemGrid.getSelectionModel().getSelections();
	var len = rowObj.length; 
	if (len < 1) {
		Ext.Msg.show({
					title : '注意',
	...
	/* var state = rowObjDetail[0].get("midcheckFlag");
	//alert(state);			
	if(state == "已提交" ){
		
     Ext.Msg.show({title:'警告',msg:'数据已提交，不可再删除明细信息！',buttons: Ext.Msg.OK,icon:Ext.MessageBox.WARNING});
     return;
     } */
//////*******************************************************************
		var tmpUrl = this.url;
		...
		Ext.MessageBox.confirm('提示', '确定要删除选定的行吗', function(btn) {
			if (btn == 'yes') {
				Ext.each(records, function(record) {
							...
							Ext.Ajax.request({
										url : tmpUrl + '?action=del&fileUrl='
												+ record.get("FileURL")+"&Delrowid="+record.get("rowid"),
										waitMsg : '删除中...',
                                     ...
                                     //itemGrid.load({params:{start:0, limit:25,rowid:rowid}});
											} else {
												var message = "SQLErr: "
														+ jsonData.info;
												Ext.Msg.show({
...
///**********************************************************
//如果应用过程中tbar没有定义，那么会默认显示这里push的buttons,这里push的buttons可以减少或增加；但是相应的，隐藏button的函数是根据push进去的button的id来识别的，所以，没有push的button不能再使用隐藏函数，否则会报错tbutton没有定义
//如果自己定义了tbar,那么tbar中的button也需要自己定义
///**********************************************************
		buttons.push([{
					text : '增加',
					id : 'herpAddId',
...
/////********************************************************************
	...
	if(state == "已提交" ){
     ...
     Ext.Msg.show({title:'警告',msg:'数据已提交，不可再修改明细信息！',buttons: Ext.Msg.OK,icon:Ext.MessageBox.WARNING});
     return;
     }
...
//////*******************************************************************
		...
		Ext.each(records, function(r) {
			...
			/////herpgrid.js中原来的程序   var tmpstro = "&rowid=" + r.data['rowid'];
			/////在科研中，因为类似科研项目中页面的下半部分的itemGrid是根据主页面的mainrowid进行修改的，因此把这里的itemGrid的明细表的rowid去掉了
			var tmpstro="";
		 ...
		 ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
				...
				if(PrjCN!=""){
		    var rowidmain = rowObj[0].get("rowid");
     Ext.Msg.show({title:'警告',msg:'已添加项目合同明细信息，不能再添加！',buttons: Ext.Msg.OK,icon:Ext.MessageBox.WARNING});
     itemGrid.load({params:{start:0, limit:12,rowid:rowid}});	
     return;
     }
     ////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			} else {
	...
	if(state == "已提交" ){
     ...
     Ext.Msg.show({title:'警告',msg:'数据已提交，不可修改明细信息！',buttons: Ext.Msg.OK,icon:Ext.MessageBox.WARNING});
	 itemGrid.load({params:{start:0, limit:12,rowid:rowid}});	
     return;
     }
			}
			...
			var rowObj1 = itemGrid.getSelectionModel().getSelections();
			 var rowidmain = rowObj[0].get("rowid");
			  var rowiddetail = rowObj1[0].get("RowID");
			//alert(stro);
			//alert(tmpstro);
			//alert(urlParam);
			var saveUrl = this.url + '?action=' + recordType + stro.toString()
					+ "&" + tmpstro.toString() + "&"
					+ Ext.urlEncode(this.urlParam)+ "&usercode="+ usercode+"&rowid2="+rowiddetail;
			//alert(saveUrl);		
				//&str='+encodeURIComponent(Ext.getCmp('deptCombo').getRawValue()),
			 //alert(saveUrl)
			p = {
				url : saveUrl,
				method : 'GET',
				...
				success : function(result, request) {
				   ...
				   /**添加明细信息成果之后，刷新页面上面Grid**/
					 //itemGrid.load({params:{start:0, limit:25,rowid:rowidmain}});
