...
//						.child(tb.x_buttons.upload.buttonSelector).dom[tb.x_buttons.upload.tooltipType] = this.i18n.upload_btn_start_tip;
	...
	uploadFile : function(record) {
		// this.form.submit();
		//var rowObj = this.grid_panel.getSelectionModel().getSelections();  
	    //var filename=rowObj[0].get("fileName");
	    
	    //alert(record.get("fileName"));
		Ext.Ajax.request({//this.grid_panel.getSelectionModel().getSelections()[0].get("fileName")
					url : this.url+"&FileURL="+record.get("fileName"),
