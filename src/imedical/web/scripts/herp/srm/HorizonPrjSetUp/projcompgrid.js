...
dhc.herp.CompGrid = Ext.extend(Ext.grid.EditorGridPanel, {
	edit : true,
	trackMouseOver : true,
	stripeRows : true,
	cm : colModel,
	...
	initComponent : function() {
		...
		dhc.herp.CompGrid.superclass.initComponent.call(this);
	},
	createPlugins : function() {
		...
		plugins.push(dhc.herp.CompGridTip);
	...
	var state = rowObj[0].get("ProjStatus");
				//alert(state);			
	 //if(state == "在研" ){
     //Ext.Msg.show({title:'警告',msg:'数据已立项申请，不可再添加明细信息！',buttons: Ext.Msg.OK,icon:Ext.MessageBox.WARNING});
     //return;
     //}
	...
	var state = rowObj[0].get("ProjStatus");
				//alert(state);			
	//if(state == "在研" ){
    // Ext.Msg.show({title:'警告',msg:'数据已立项申请，不可再删除明细信息！',buttons: Ext.Msg.OK,icon:Ext.MessageBox.WARNING});
    // return;
    // }
		var tmpUrl = this.url;
		var tmpG = this;
	...
	var state = rowObj[0].get("ProjStatus");
				//alert(state);			
	//if(state == "在研" ){
    // Ext.Msg.show({title:'警告',msg:'数据已立项申请，不可再修改明细信息！',buttons: Ext.Msg.OK,icon:Ext.MessageBox.WARNING});
    // return;
    // }
