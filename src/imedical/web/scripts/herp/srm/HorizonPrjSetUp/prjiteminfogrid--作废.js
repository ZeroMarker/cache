 ...
 * 	
	...
	3、增加savel(
	url(url)方法,可以更具需要自定义url，保存提交数据，如：url='herp.budg.budgschemmainexe.csp?action=copysheme&rowid='+schmDr
...
dhc.herp.Gridhs = Ext.extend(Ext.grid.EditorGridPanel, {
	edit : true,
	trackMouseOver : true,
	stripeRows : true,
	cm : colModel,
	...
	initComponent : function() {
		...
		dhc.herp.Gridhs.superclass.initComponent.call(this);
	},
	createPlugins : function() {
		...
		plugins.push(dhc.herp.GridhsTip);
...
//		},
  ...
  ///*********************************************************************
	...
	var state = rowObj[0].get("DataStatuslist");
				//alert(state);			
	var projstatus = rowObj[0].get("ProjStatus");
	var ChkResult = rowObj[0].get("ChkResult");
	//alert(state);	
    if((state == "已提交")&(projstatus=="在研"&(ChkResult=="0")) ){
		 //alert("state:"+state+"projstatus:"+projstatus+"ChkResult:"+ChkResult);
