...
var InventorsGrid = new Ext.grid.GridPanel({
    ...
    colModel: new Ext.grid.ColumnModel({
        ...
        columns: [
            ...
            {id: 'isthehosrowid', header: '申请专利时身份ID', width: 129, sortable: true, dataIndex: 'isthehosrowid',hidden:true},
            {header: '申请专利时身份', dataIndex: 'isthehos',align:'center',width: 80}
        ]
    }),
			...
			if(total>0){	
				...
				if(id==""){
					...
					if(InventorsRange=="")
							...
							if(IsTheHos=="")
							{
								Ext.Msg.show({title:'错误',msg:'请选择申请专利时身份!',buttons: Ext.Msg.OK,icon:Ext.MessageBox.ERROR});
							  return;
							}
      		...
      		if(InventorsGrid.getStore().getCount()<1)
      {
      	...
      	return;
        	...
        	if (addFormPanel.form.isValid()) {
						Ext.Ajax.request({
							url: projUrl+'?action=add&Name='+encodeURIComponent(Name)+'&Inventors='+Inventors+'&CertificateNo='+CertificateNo+'&AnnDate='+AnnDate.format("Y-m-d")+'&YearDr='+YearDr+'&Patentee='+Patentee+'&PatentNum='+PatentNum+'&AppDate='+AppDate.format("Y-m-d")+'&userdr='+userdr+'&DeptDr='+DeptDr+'&PatentType='+PatentType+'&AnnUnit='+AnnUnit+'&CompleteUnit='+CompleteUnit+'&VCAmount='+VCAmount+'&InvoiceCode='+InvoiceCode+'&InvoiceNo='+InvoiceNo+'&unitMoneys='+unitMoney,
							waitMsg:'保存中...',
