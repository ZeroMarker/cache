noauditfun = function() {
		...
		var projUrl = 'herp.srm.auditprjachievementexe.csp';
	...
	//////////////////////////  增加按钮响应函数   //////////////////////////////
		addHandler = function(){      			
		   ...
		   if(formPanel.form.isValid()){
		       for(var i = 0; i < len; i++){
			    Ext.Ajax.request({
					url:projUrl+'?action=noaudit'+'&rowid='+rowObj[i].get("rowid")+'&view='+view+'&checkerdr='+userdr,
					waitMsg:'保存中...',
					...
					success: function(result, request){
						var jsonData = Ext.util.JSON.decode( result.responseText );
							if (jsonData.success=='true'){
								...
								Ext.Msg.show({title:'注意',msg:'操作成功!',icon:Ext.MessageBox.INFO,buttons:Ext.Msg.OK});								 var inputdata="|||||"+userdr;				
