...
var itemGridUrl = '../csp/herp.srm.horizonendprjexe.csp';
//配件数据源
...
PrjSrcDs.on('beforeload', function(ds, o){
	...
	url:itemGridUrl+'?action=GetPrjType&str='+encodeURIComponent(Ext.getCmp('PrjSrcMField').getRawValue()),method:'POST'});
});
var PrjSrcMField = new Ext.form.ComboBox({
	id: 'PrjSrcMField',
	fieldLabel: '课题来源',
	width:150,
	listWidth :225,
...
PrjHeaderDs.on('beforeload', function(ds, o){
	ds.proxy=new Ext.data.HttpProxy({
	url:itemGridUrl+'?action=GetPrjHeader&str='+encodeURIComponent(Ext.getCmp('PrjHeaderMField').getRawValue()),method:'POST'});
});
var PrjHeaderMField = new Ext.form.ComboBox({
	id: 'PrjHeaderMField',
	fieldLabel: '课题负责人',
	width:150,
	listWidth :225,
...
CalDeptDs.on('beforeload', function(ds, o) {
			...
			ds.proxy = new Ext.data.HttpProxy({
		  url:itemGridUrl+'?action=GetDepts&str='+encodeURIComponent(Ext.getCmp('CalDeptMField').getRawValue()),method:'POST'});
		});
var CalDeptMField = new Ext.form.ComboBox({
	id: 'CalDeptMField',
	fieldLabel: '科室',
	width:100,
	listWidth : 225,
...
function srmFundSearch(){
    ...
    //alert(endtime);
    var deptdr  = CalDeptMField.getValue();
    var source = PrjSrcMField.getValue(); 
    var headdr = PrjHeaderMField.getValue();
...
var queryPanel = new Ext.FormPanel({
		...
		defaults: {bodyStyle:'padding:5px'},
			...
			items: [
				{   
					xtype:'displayfield',
					value:'<center><p style="font-weight:bold;font-size:150%">项目结题材料上报</p></center>',
					columnWidth:1,
					height:'50'
				}]
		    },{
...
var itemGrid = new Ext.grid.GridPanel({
		    ...
		    width:400,
		    readerModel:'local',
		    url: 'herp.srm.horizonendprjexe.csp',
		    atLoad : true, // 是否自动刷新
			store: itemGridDs,
			cm: itemGridCm,
			trackMouseOver: true,
  ...
  var srmdeptuserDs=function(){	
	    ...
	    itemGridDs.proxy = new Ext.data.HttpProxy({
								...
								url : 'herp.srm.horizonendprjexe.csp?action=list&code='+ encodeURIComponent(uCodeField.getValue())+ 
...
//		 }
	...
	function handler(id){
		if(id=="yes"){
			...
			for(var i = 0; i < len; i++){
				    Ext.Ajax.request({
					url:'herp.srm.horizonendprjexe.csp?action=monographAsk&&rowid='+rowObj[i].get("rowid")+'&sysno='+rowObj[i].get("SysNo")+'&editor='+rowObj[i].get("EditorDr")+'&deptdr='+rowObj[i].get("DeptDr"),
					waitMsg:'审核中...',
					failure: function(result, request){
