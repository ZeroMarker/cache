...
dhc.herp.Gridhss = Ext.extend(Ext.grid.EditorGridPanel, {
	edit : true,
	trackMouseOver : true,
	stripeRows : true,
	cm : colModel,
	...
	initComponent : function() {
		...
		dhc.herp.Gridhss.superclass.initComponent.call(this);
	},
	createPlugins : function() {
		...
		plugins.push(dhc.herp.GridhssTip);
...
/////********************************************************************
	var rowObj = itemGrid.getSelectionModel().getSelections();
	var rowObjDetail = DetailGrid.getSelectionModel().getSelections();
	var len = rowObj.length; 
	if (len < 1) {
		Ext.Msg.show({
					title : '注意',
...
///**********************************************************
//如果应用过程中tbar没有定义，那么会默认显示这里push的buttons,这里push的buttons可以减少或增加；
//但是相应的，隐藏button的函数是根据push进去的button的id来识别的，
//所以，没有push的button不能再使用隐藏函数，否则会报错tbutton没有定义
...
//////*******************************************************************
		...
		Ext.each(records, function(r) {
			...
			/////在科研中，因为类似科研项目中页面的下半部分的detailGrid是根据主页面的mainrowid进行修改的，因此把这里的DetailGrid的明细表的rowid去掉了
			var tmpstro="";
     ...
     ////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			var saveUrl = this.url + '?action=' + recordType + stro.toString()
					+ "&" + tmpstro.toString() + "&"
					+ Ext.urlEncode(this.urlParam)+ "&usercode="+ usercode;
			//alert(saveUrl);		
			...
			p = {
				...
				success : function(result, request) {
				   ...
				   /**添加明细信息成果之后，刷新页面上面Grid**/
					 //DetailGrid.load({params:{start:0, limit:25,rowid:rowidmain}});
