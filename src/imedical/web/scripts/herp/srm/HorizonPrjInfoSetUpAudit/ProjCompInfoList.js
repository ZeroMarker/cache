...
var FlagField = new Ext.form.ComboBox({
	      ...
	      //CompCombox.removeAll();     
				CompCombox.setValue('');
				CompCombox.proxy = new Ext.data.HttpProxy({url:ProjCompUrl+'?action=relyuintslist1&prjrowid='+projectrowid,method:'POST'})  
				//CompCombox.load({params:{start:0,limit:10,prjrowid:projectrowid}});   
			}   		
			else
			{
				//CompCombox.removeAll();     
				CompCombox.setValue('');
				CompCombox.proxy = new Ext.data.HttpProxy({url:ProjCompUrl+'?action=relyuintslist2&str='+encodeURIComponent(Ext.getCmp('CompCombox').getRawValue()),method:'POST'})  
				//CompCombox.load({params:{start:0,limit:10}});  
			}			
			}
		}
		});
...
ProjCompGrid.btnAddHide();  //隐藏增加按钮
ProjCompGrid.btnSaveHide();  //隐藏保存按钮
ProjCompGrid.btnDeleteHide(); //隐藏删除按钮
