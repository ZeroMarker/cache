noauditfun = function() {
		...
		var projUrl = 'herp.srm.srmPartTimeInfoAuditexe.csp';
	...
	//////////////////////////  增加按钮响应函数   //////////////////////////////
		addHandler = function(){      							
		   ...
		   if(formPanel.form.isValid()){
		       for(var i = 0; i < len; i++){
			    Ext.Ajax.request({
					url:projUrl+'?action=noaudit'+'&rowid='+rowObj[i].get("rowid")+'&view='+view+'&usercode='+checkerdr,
					waitMsg:'保存中...',
					...
					success: function(result, request){
						var jsonData = Ext.util.JSON.decode( result.responseText );
							...
							itemGridDs.load({params:{start:0, limit:12,usercode:checkerdr}});							
							}
							else
							{
