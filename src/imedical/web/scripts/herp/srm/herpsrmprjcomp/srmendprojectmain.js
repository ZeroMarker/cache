...
var itemGridUrl = '../csp/herp.srm.endprojectexe.csp';
//配件数据源
...
var itemGrid = new Ext.grid.GridPanel({
		    ...
		    width:400,
		    readerModel:'local',
		    url: 'herp.srm.endprojectexe.csp',
		    atLoad : true, // 是否自动刷新
			store: itemGridDs,
			cm: itemGridCm,
			trackMouseOver: true,
  ...
  var srmdeptuserDs=function(){	
	    ...
	    itemGridDs.proxy = new Ext.data.HttpProxy({
								...
								url : 'herp.srm.endprojectexe.csp?action=list&code='+ encodeURIComponent(uCodeField.getValue())+ 
...
//		 }
	...
	function handler(id){
		if(id=="yes"){
			...
			for(var i = 0; i < len; i++){
				    Ext.Ajax.request({
					url:'herp.srm.endprojectexe.csp?action=monographAsk&&rowid='+rowObj[i].get("rowid")+'&sysno='+rowObj[i].get("SysNo")+'&editor='+rowObj[i].get("EditorDr")+'&deptdr='+rowObj[i].get("DeptDr"),
					waitMsg:'审核中...',
					failure: function(result, request){
