...
eYearDs.on('beforeload', function(ds, o){
	...
	url:'herp.srm.srmprojectsinfoexe.csp'+'?action=GetYear&str='+encodeURIComponent(Ext.getCmp('eYearField').getRawValue()),
	method:'POST'});
});
...
var RelyUnitsGrid = new Ext.grid.GridPanel({
		...
		url:'herp.srm.srmprojectsinfoexe.csp?action=listrelyunits&start='+0+'&limit='+25+'&RelyUnitsIDs='+RelyUnitsIDs,
		method:'POST'}),
...
var eaddRelyUnits  = new Ext.Button({
		...
		handler: function(){
			...
			var unittypeid = Ext.getCmp('aUnitTypeCombox').getValue();
			var RelyUnitName = Ext.getCmp('eRelyUnits').getRawValue();
			var UnitTypeName = Ext.getCmp('aUnitTypeCombox').getRawValue();
...
var ParticipantsGrid = new Ext.grid.GridPanel({
        ...
        autoLoad:true,
		proxy: new Ext.data.HttpProxy({
		url:'herp.srm.srmprojectsinfoexe.csp'+'?action=listparticipantsInfo&start='+0+'&limit='+25+'&ParticipantsIDs='+encodeURIComponent(ParticipantsIDs),
		method:'POST'}),
    ...
    buttons: [{
      ...
      var DeptDr = DeptFields.getValue();
			...
			if(eYearField.getRawValue()=="")
      		...
      		if(ProjectsNameFields.getValue()=="")
      		{
