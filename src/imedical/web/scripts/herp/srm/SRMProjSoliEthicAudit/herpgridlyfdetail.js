...
dhc.herp.Gridlyf = Ext.extend(Ext.grid.EditorGridPanel, {
	...
	var preaudit= rowObj[0].get("PreAuditState")
	if (preaudit!="预审-审核通过")
	{
		Ext.Msg.show({
					title : '注意',
	...
	del : function() {
		...
		var selectedRow1 = EthicResultGrid.getSelectionModel().getSelections();
		var len = MainRowObj.length; 
		if (len < 1) {
			Ext.Msg.show({
					title : '注意',
		...
		var isfeedback = selectedRow1[0].get("IsFeedBack");
		if(datastatus=="是")
		{
			Ext.Msg.show({
			title : '错误',
		...
		Ext.MessageBox.confirm('提示', '确定要删除选定的行吗', function(btn) {
			var rowObjlyf=EthicResultGrid.getSelectionModel().getSelections();
			if (btn == 'yes') {
	...
	save : function() {
		...
		var prjdr = rowObj[0].data['prjrowid'];

		var DetailRowObj = EthicResultGrid.getSelectionModel().getSelections();
		...
		Ext.each(records, function(r) {
			...
			for (var i = 0; i < this.fields.length; i++) {
				...
				if (tmobj != null) {
					...
					if ((typeof(reValue) != "undefined") && (tmobj.update)) {
						tmpstro += "&" + indx + "=" + encodeURIComponent(r.data[indx].toString());
					}
			...
			if (r.data.newRecord) {
			    ...
			    var saveUrl = this.url + '?action=' + recordType + stro.toString()
					+ "&" + tmpstro.toString() + "&"
					+ Ext.urlEncode(this.urlParam)+'&usercode='+session['LOGON.USERCODE'];
				//alert(saveUrl);
			} else {
				var rowObjlyf1=EthicResultGrid.getSelectionModel().getSelections();
				var len=rowObjlyf1.length;
				for (var i = 0; i < len; i++){
				
				var rowid =r.get("rowid");
				var prjrowid=r.get("prjrowid");
				//r.get("itemdr");
				var EthicChkResult=r.get("EthicChkResult");  
				var EthicAuditDesc=r.get("EthicAuditDesc");  
				var datastatus = r.get("DataStatus"); 
				var isfeedback = r.get("IsFeedBack"); 
				...
				success : function(result, request) {
					...
					if (jsonData.success == 'true') {
						...
						if (jsonData.refresh == 'true') {
						   ...
						   //EthicResultGrid.load({params:{start:0, limit:25,prjdr:prjdr}});		
						}
					} else {
						if (jsonData.refresh == 'true') {
							tmpStore.load({
						...
						if (jsonData.info == 'RecordExist')
							message = '输入的记录已经存在!';
						if (jsonData.info == 'RepSubmit')
							message = '已存在审核通过且提交的伦理审核结果，不可再次添加其他伦理审核结果!';
