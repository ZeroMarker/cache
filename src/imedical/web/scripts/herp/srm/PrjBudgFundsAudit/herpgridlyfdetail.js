...
dhc.herp.Gridlyf = Ext.extend(Ext.grid.EditorGridPanel, {
	...
	del : function() {
		...
		var selectedRow1 = prjbudgfundsGrid.getSelectionModel().getSelections();
	    var prjdr=selectedRow1[0].data['rowid'];
	...
	save : function() {
		...
		Ext.each(records, function(r) {
			...
			if (r.data.newRecord) {
			   var rowObjlyf=prjbudgfundsDetail.getSelectionModel().getSelections();
			   var prjdr=rowObjlyf[0].get("prjrowid");
		...
		//alert(saveUrl);
				...
				var itemname=rowObjlyf1[0].data['itemdr'];
				var fundsource=rowObjlyf1[0].get("fundsourcedr");
				var budgvalue=rowObjlyf1[0].get("budgvalue");
				var budgdesc=rowObjlyf1[0].get("budgdesc");
				recordType1 = "edit";
				o[this.idName] = r.get(this.idName);
			    data.push(o);
			    var saveUrl = this.url + '?action=' + recordType1 + stro.toString()
					+ "&" + tmpstro.toString() + "&"
					+ Ext.urlEncode(this.urlParam)+'&itemname='+itemname+'&fundsource='+fundsource
