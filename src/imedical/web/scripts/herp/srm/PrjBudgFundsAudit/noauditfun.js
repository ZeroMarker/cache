noauditfun = function() {
		...
		var projUrl = 'herp.srm.prjbudgfundsauditexe.csp';
...
/////////// 多文本域
	...
	//////////////////////////  增加按钮响应函数   //////////////////////////////
		addHandler = function(){      			
			...
			var str=Ext.getCmp('textArea').getRawValue()
			view=str.replace(/[\r\n]/g,","); 
		   var view= encodeURIComponent(view) 
			 
			
		   if(formPanel.form.isValid()){
		       for(var i = 0; i < len; i++){
			    Ext.Ajax.request({
					url:projUrl+'?action=noaudit'+'&rowid='+rowObj[i].get("rowid")+'&view='+view+'&userdr='+userdr,
					waitMsg:'保存中...',
					...
					success: function(result, request){
						var jsonData = Ext.util.JSON.decode( result.responseText );
							if (jsonData.success=='true'){
								...
								prjbudgfundsDetail.load({params:{start:0, limit:12,usercode:userdr}});	
							}
							else
							{
