...
var HrzPrjSetUpUrl='herp.srm.horizonprjsetupexe.csp';
...
//获取科室名称
	var deptnameDs = new Ext.data.Store({
		autoLoad:true,
		proxy:"",
		reader:new Ext.data.JsonReader({totalProperty:'results',root:'rows'},['rowid','deptname'])
	});
    deptnameDs.on('beforeload', function(ds, o){
		ds.proxy=new Ext.data.HttpProxy({
		url:HrzPrjSetUpUrl+'?action=deptList&str='+encodeURIComponent(Ext.getCmp('deptnameField').getRawValue()),method:'POST'});
	});
...
sourceDs.on('beforeload',function(ds,o){
	ds.proxy= new Ext.data.HttpProxy({
		url:HrzPrjSetUpUrl+'?action=sourceList&str='+encodeURIComponent(Ext.getCmp('sourceField').getRawValue()),
		method:'POST'
	});
});
...
headerDs.on('beforeload',function(ds,o){
	ds.proxy= new Ext.data.HttpProxy({
		url:HrzPrjSetUpUrl+'?action=headerList&str='+encodeURIComponent(Ext.getCmp('headerField').getRawValue()),
		method:'POST'
	});
});
...
var queryPanel = new Ext.FormPanel({
			...
			defaults: {bodyStyle:'padding:5px'},
				...
				items: [
					     {   
						xtype:'displayfield',
						value:'<center><p style="font-weight:bold;font-size:120%">项目立项</p></center>',
						columnWidth:1,
						height:'50'
					 }]
			    },{
		...
		function handler(id){
			if(id=="yes"){
				for(var i = 0; i < len; i++){
						...
						var grafunds=rowObj[i].get("grafunds");
					    Ext.Ajax.request({
						url:HrzPrjSetUpUrl+'?action=setup&rowid='+rowid+'&subno='+subno+'&issuedDate='+issuedDate+'&grafunds='+grafunds,
						waitMsg:'提交中...',
		...
		function handler(id){
			if(id=="yes"){
				for(var i = 0; i < len; i++){
					    var rowid=rowObj[i].get("rowid");
					    Ext.Ajax.request({
						url:'herp.srm.horizonprjsetupexe.csp?action=refuse&rowid='+rowid,
						waitMsg:'提交中...',
						failure: function(result, request){
							Ext.Msg.show({title:'错误',msg:'请检查网络连接!',buttons: Ext.Msg.OK,icon:Ext.MessageBox.ERROR});
						},
