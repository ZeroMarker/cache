...
subFun = function(){
 var rowObj = itemGrid.getSelectionModel().getSelections();     // get the selected items
    ...
    if(len > 0)
    {
	    	//////////////////////////判断是否有附件上传记录///////////////////////////
				Ext.Ajax.request({
					url:'herp.srm.uploadexe.csp?action=GetUpLoadInfo&RecDr='+rowObj[0].get("rowid")+'&SysNo='+'SRMpatentapply001,SRMpatentapply002,SRMpatentapply003,SRMpatentapply004,SRMpatentapply005',
    	    ...
    	    function(btn) {
		         ...
		         {	
				 	if(rowObj[0].get("DataStatus")!="已提交"){
				        for(var i = 0; i < len; i++){     		
					          Ext.Ajax.request({
					                url: 'herp.srm.srmpatentcertificateapplyexe.csp?action=sub&rowid='+rowObj[i].get("rowid")+'&userdr='+userdr,
								    waitMsg:'提交中...',
