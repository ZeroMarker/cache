...
dhc.herp.Gridlyf = Ext.extend(Ext.grid.EditorGridPanel, {
	edit : true,
	trackMouseOver : true,
	stripeRows : true,
	cm : colModel,
	...
	initComponent : function() {
		...
		dhc.herp.Gridlyf.superclass.initComponent.call(this);
	},
	createPlugins : function() {
		...
		plugins.push(dhc.herp.GridlyfTip);
	...
	createStore : function() {
		...
		// 远程
			...
			this.store = new Ext.data.Store({
						proxy : new Ext.data.HttpProxy({
									url : this.url + '?action=listdetail'
								}),
						reader : new Ext.data.JsonReader({
									root : 'rows',
									totalProperty : 'results'
	...
	var rowObj = prjbudgfundsGrid.getSelectionModel().getSelections();
	...
	var year = rowObj[0].data['yeardr'];

	BIDNameField.getStore().load({params:{year:year,start:0,limit:10}}); 
	...
	del : function() {
		...
		var MainRowObj = prjbudgfundsGrid.getSelectionModel().getSelections();
		var len = MainRowObj.length; 
		if (len < 1) {
			Ext.Msg.show({
					title : '注意',
					msg : '请单击选择对应的项目!',
		...
		var selectedRow1 = prjbudgfundsDetail.getSelectionModel().getSelections();
		var len = MainRowObj.length; 
		if (len < 1) {
			Ext.Msg.show({
					title : '注意',
		...
		var datastatus = selectedRow1[0].get("datastatus");
		if(datastatus=="已提交")
		{
			Ext.Msg.show({
			title : '错误',
	    ...
	    var prjdr=selectedRow1[0].data['prjrowid'];

		Ext.MessageBox.confirm('提示', '确定要删除选定的行吗', function(btn) {
			var rowObjlyf=prjbudgfundsDetail.getSelectionModel().getSelections();
			if (btn == 'yes') {
							...
							Ext.Ajax.request({
										...
										success : function(result, request) {
											...
											if (jsonData.success == 'true') {
												...
												//prjbudgfundsGrid.load({params:{start:0, limit:25}});	
											} else {
												var message = "SQLErr: "
														+ jsonData.info;
												Ext.Msg.show({
	...
	save : function() {
		...
		var prjdr = rowObj[0].data['rowid'];
		var year1 = rowObj[0].data['yeardr'];
		BIDNameField.getStore().load({params:{year:year1,start:0,limit:10}}); 
		
		var DetailRowObj = prjbudgfundsDetail.getSelectionModel().getSelections();
		...
		//alert(saveUrl);
				...
				var rowObjlyf1=prjbudgfundsDetail.getSelectionModel().getSelections();
				var len=rowObjlyf1.length;
				for (var i = 0; i < len; i++){
				/* var rowid =rowObjlyf1[i].data['rowid'];
				var itemname=rowObjlyf1[i].data['itemdr'];
				var fundsource=rowObjlyf1[i].get("fundsourcedr");
				var budgvalue=rowObjlyf1[i].get("budgvalue");
				var budgdesc=rowObjlyf1[i].get("budgdesc");
				var prjdr = rowObjlyf1[i].get("prjrowid");
				var datastatus = rowObjlyf1[i].get("datastatus"); */
				
				var rowid =r.get("rowid");
				var itemname=r.get("itemdr");
				//r.get("itemdr");
				var fundsource=r.get("fundsourcedr");  
				var budgvalue=r.get("budgvalue");  
				var budgdesc=r.get("budgdesc"); 
				var prjdr = r.get("prjrowid"); 
				var datastatus = r.get("datastatus"); 
			    ...
			    var saveUrl = this.url + '?action=' + recordType1 + stro.toString()
					+ "&" + tmpstro.toString() + "&"
					//+"&"+
					+ Ext.urlEncode(this.urlParam);
					/* +'&rowid='+rowid+'&prjdr='+prjdr+'&itemname='+itemname+'&fundsource='+fundsource
					+'&budgdesc='+encodeURIComponent(budgdesc)+'&budgvalue='+budgvalue */
			}
			}
			//o[this.idName] = r.get(this.idName);
			//data.push(o);
			//var saveUrl = this.url + '?action=' + recordType + stro.toString()
					//+ "&" + tmpstro.toString() + "&"
					//+ Ext.urlEncode(this.urlParam)+"&projadjdr="+projadjdr;
			p = {
				url : saveUrl,
				method : 'GET',
				waitMsg : '保存中...',
				...
				success : function(result, request) {
					...
					if (jsonData.success == 'true') {
						...
						if (jsonData.refresh == 'true') {
						   ...
						   //prjbudgfundsGrid.load({params:{start:0, limit:25,prjdr:prjdr}});		
						}
					} else {
						if (jsonData.refresh == 'true') {
							tmpStore.load({
						...
						if (jsonData.info == 'RecordExist')
							message = '输入的记录已经存在!';
						if (jsonData.info == 'RepItem')
							message = '输入的科目已经存在!';
						if (jsonData.info == 'OutNumber')
							message = '编制经费已超过批准经费!';
						if (jsonData.info == 'OutToppercent')
							message = '编制经费已超过该科目最大占比!';
						if (jsonData.info == 'LowerBottomLine')
							message = '编制经费低于该科目最小占比!';
