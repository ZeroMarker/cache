...
//Modified by HHM 2016-01-21 去掉多余变量
var GlobalObj = {
	objId : "",
...
function initDocument() {
	...
	initPanel();
	FillDataInfo();
	if((jQuery("#Rowid").val()!="")&&(jQuery("#Status").val()==0))
	{
...
function initDeptIDData(comboid,comboxid)
{
	...
	jQuery("#"+comboxid).combobox('clear');
	jQuery("#"+comboxid).combobox({
...
function initSourceID() {
	jQuery("#SourceID").combogrid({
	    ...
	    columns:[[    
	        ...
	        {field:'equip',title:'设备',width:140},    //add by HHM 2016-01-21 添加设备
...
function SaveClick(){
	if(jQuery("#SourceType").combobox('getValue')==""){
           jQuery.messager.alert("提示", "对应事件不能为空！")
           return false;
    }
    if(jQuery("#SourceID").combogrid('getValue')==""){
           jQuery.messager.alert("提示", "对应业务不能为空！")
           return false;
    }
    if(jQuery("#FromDeptType").combobox('getValue')==""){
           jQuery.messager.alert("提示", "初始单位类型不能为空！")
           return false;
    }
    if(jQuery("#FromDeptID").combobox('getValue')==""){
           jQuery.messager.alert("提示", "初始单位不能为空！")
           return false;
    }
    /*if(jQuery("#FromLocationDR").combobox('getValue')==""){
           jQuery.messager.alert("提示", "初始位置不能为空！")
           return false;
    }*/
    if(jQuery("#StartDate").datebox('getValue')==""){
     ...
     if(jQuery("#SendUserDR").combobox('getValue')==""){
           jQuery.messager.alert("提示", "送出负责人不能为空！")
           return false;
    }
	...
	if((jQuery("#EndDate").datebox('getValue')=="")&&(jQuery("#EndTime").val()!=""))
	{
		jQuery.messager.alert("提示", "结束日期不能为空");
				return false;  
	}
	if(jQuery("#EndDate").datebox('getValue')!="")
    {
	    ...
	    //modified by HHM 2016-01-18 修改日期比较
...
function DeleteClick(){
	...
	alertShow("frowid="+frowid+";erowid="+erowid)
	var result = tkMakeServerCall("web.DHCEQMove", "DeleteMoveInfo",rowid,user,frowid,erowid,invalidreason);
	if(result==0) {
...
function initEventType() {
	jQuery("#EventType").combobox({
		...
		//add by HHM 2016-01-22 添加根据送出/返还类型，清空控件
		onSelect: function(record) {
			jQuery('#SourceType').combobox('clear');
			jQuery("#SourceID").combogrid('clear');
			jQuery("#ObjID").combogrid('clear');
			jQuery("#FromDeptType").combobox('clear');
			jQuery("#FromDeptID").combobox('clear');
