...
NAME: DHC.WMR.CommonFunction.js
	...
	function getElementValue(controlID, objDoc, getCaption)
	{
		...
		switch(obj.nodeName)
		{
			...
			case "SELECT":
				...
				{
					objOption = obj.options.item(i);
					if(objOption.selected)
					{
						strValue += (strValue == "" ? "" : "^") + (getCaption ? objOption.innerText :objOption.value);    //by wuqk 2008-2-28  by liyang 2008-5-22
					}
				}
				break;
			case "LABEL":
...
//						    obj.selectedIndex = i;
	...
	function ValidateUser(title)
	{
		var objReturn = window.showModalDialog(
			"websys.default.csp?WEBSYS.TCOMPONENT=DHC.WMR.ValidateUser" + "&Title=" + title ,
			"",
...
function CreateICDDicQueryComboBox(DicType, Label)
{
    var ds = new Ext.data.Store({
        proxy: new Ext.data.HttpProxy(new Ext.data.Connection({
            url: 'dhc.wmr.dictionary.csp?DicName=' + DicType
        })),
    ...
    ds.on('beforeload', function(ds, o){
        ...
        ds.proxy = new Ext.data.HttpProxy({url: 'dhc.wmr.dictionary.csp?DicName='+DicType+'&query1=' + strTmp});
       });  
   //window.alert('./dhc.wmr.dictionary.csp?DicName=' + DicType); 
   return search; 
}
...
function CreateDicQueryComboBox(DicType, Label)
{
    ...
    var search = new Ext.form.ComboBox({
       ...
       ds.on('beforeload', function(ds, o){
        ...
        ds.baseParams["query"]=strTmp;
        ds.proxy = new Ext.data.HttpProxy({url: '../csp/dhc.wmr.dictionary.csp?DicName='+DicType+'&query1=' + strTmp});
       });  
...
function InitComboBox(Label, DicName)
{
   ...
   var objArry = GetDHCWMRDictionaryArryByTypeFlag("MethodGetDHCWMRDictionaryArryByTypeFlag", DicName, "Y");
   var objRec = null;
   var objItem = null;
   for(var i = 0; i < objArry.length; i++)
   {
...
function SetComboValue(objCombo, ValueFieldName, FieldName, FieldValue) {
    ...
    //Add By LiYang 2009-07-30 get combo's current record object
...
Author:liuxuefeng
Date: 2009-08-19
Comment: Format MrNo According to the Config
s val=##Class(%CSP.Page).Encrypt($LB("web.DHCWMRMedBaseCtl.FormatMrNo"))
============================================*/
 function FormatMrNo(methodControl, MrType, MrNo)
{
 ...
 /*===========================================
Name:FormatInputMrNo
Author:liuxuefeng
Date: 2009-08-19
Comment: Call FormatMrNo(methodControl, MrType, MrNo) method
Modify Component:
Add Hidden Item:MethodFormatMrNo£ºs val=##Class(%CSP.Page).Encrypt($LB("web.DHCWMRMedBaseCtl.FormatMrNo"))
Add Hidden Item:FormatMrNoFlag ValueGet:s val="Y"
Add Hidden tem:MrType s val=%request.Get("MrType")
============================================*/
function FormatInputMrNo(MrTypeField,MrNoField)
{
	var FormatMrNoFlag=getElementValue("FormatMrNoFlag");
	if(FormatMrNoFlag != "Y")
		return;
	var MrNo=getElementValue(MrNoField);
	MrNo=Trim(MrNo);
	var MrType=getElementValue(MrTypeField,null);
	if((MrNo=="")||(MrType==""))
		return;
	var MrNoFormated=FormatMrNo("MethodFormatMrNo", MrType, MrNo);
	if(MrNoFormated!=""){
  	setElementValue(MrNoField,MrNoFormated,null);
	}
}
