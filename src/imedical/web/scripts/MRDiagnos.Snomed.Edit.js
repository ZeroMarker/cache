...
function QualifierClickHandler() {
	q2obj.options.length=0;
	dobj.options.length=0;
	if(qobj.value!="") {
		var q2List=document.getElementById("GENQualz"+qobj.value);
		if(q2List) {
			if(q2List.value!="") {
				var q2Entry=q2List.value.split("^");
				var q2;
				for (i=0;i<q2Entry.length;i++) {
					q2=q2Entry[i].split("*");
					q2obj[q2obj.length] = new Option(q2[1],q2[0]);
				}
			}
		}
	}
...
function Qual2ClickHandler() {
	dobj.options.length=0;
	if(q2obj.value!="" && qobj.value!="") {
		var dList=document.getElementById("GENSubQualz"+qobj.value+"_"+q2obj.value);
		if(dList) {
			if(dList.value!="") {
...
function AddClickHandler() {
	...
	if(qlobj) {
		if(qobj && q2obj) {
			if((qobj.selectedIndex!=-1) && (q2obj.selectedIndex!=-1)) {
				idary = qobj.options[qobj.selectedIndex].value;
				descary = qobj.options[qobj.selectedIndex].text;
				if(idary!="") idary+="^";
				idary += q2obj.options[q2obj.selectedIndex].value;
				if(descary!="") descary+=" - ";
				descary += q2obj.options[q2obj.selectedIndex].text;
			}
		}
		if(dobj) {
...
function Update1ClickHandler() {
	...
	var objd=document.getElementById("DateDiagnosisIdentif");
	if (objd) objd.className="";
	var objt=document.getElementById("TimeDiagnosisIdentif");
	...
	if(objd && objd.value!="") {
		if(objt && objt.value!="") {
			if (DateTimeStringCompareToday(objd.value,objt.value)==1) {
				alert(t['DateDiagnosisIdentif']+" & "+t['TimeDiagnosisIdentif']+" "+t['NOT_IN_FUTURE']);
		...
		if (DateStringCompareToday(objd.value)==1) {
			alert(t['DateDiagnosisIdentif']+" "+t['NOT_IN_FUTURE']);
			objd.className="clsInvalid";
			return false;
		}
	}
	...
	if (objd && ODobj && objd.value!="" && ODobj.value!="") {
		if(objt && OTobj && objt.value!="" && OTobj.value!="") {
			if(DateTimeStringCompare(ODobj.value,OTobj.value,objd.value,objt.value)==1) {
				...
				alert(t['OnsetDate']+" & "+t['OnsetTime']+" "+t['DT_BEFORE']+" "+t['DateDiagnosisIdentif']+" & "+t['TimeDiagnosisIdentif']);
				ODobj.className="clsInvalid";
				objd.className="clsInvalid";			
				OTobj.className="clsInvalid";
				objt.className="clsInvalid";	
			...
			alert(t['OnsetDate']+" "+t['DT_BEFORE']+" "+t['DateDiagnosisIdentif']);
			ODobj.className="clsInvalid";
			objd.className="clsInvalid";
			return false;
		}
