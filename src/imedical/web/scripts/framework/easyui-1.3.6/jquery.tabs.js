...
//		setScrollers(container);
		...
		/* 信息总览-[知情告知-入院记录-手术相关] wanghc
		if (hisTitle){
			selectTab(container, hisTitle);
		} else if (tabs.length){
			selectTab(container, 0);
		}*/
		/*wanghc 20190704 select pretab*/
		var preIndex = index -1;
		if (preIndex>-1){
			selectTab(container, preIndex);
		} else if (tabs.length){
			selectTab(container, 0);
		}
	}
	...
	*/
	...
	function getTab(container, which, removeit){
		...
		which = $.trim(which.split("(")[0]);
		for(var i=0; i<tabs.length; i++){
			var tab = tabs[i];
			/*add wanghc getTab by id */
			...
			var tabTitle = tab.panel('options').title.split("(")[0];
			if ($.trim(tabTitle) == which){
				if (removeit){
					tabs.splice(i, 1);
				}
				return tab;
			}
			// $("#tabsReg").tabs("exists",code,'itarget'); wanghc
			if (tab.panel('options')[removeit] == which){
				return tab;
			}
		}
		return null;
...
//		}
	...
	function selectTab(container, which){
		...
		if (opts.onBeforeSelect){
			if (false==opts.onBeforeSelect.call(container,panel,selected)){
				return false;
			}
		}
		if (selected){
			if (panel[0] == selected[0]){
				setSelectedSize(container);
				//return;  //wanghc 2017-11-29  就算选中,也进入事件
			}
