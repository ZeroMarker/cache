	...
	dhcc.icare.lu.createLookup = function(url,srcElement){
		...
		var GridRowSelect = function(firstCol, allCol) {
			try {
				...
				if (typeof window[srcElement.id + "_lookupsel"]=="function") window[srcElement.id + "_lookupsel"](firstCol);
				...
				if (tmpArr && tmpArr.length>1){
					try{
						...
						if (typeof window[tmpArr[1] + "_lookupsel"]=="function") window[tmpArr[1] + "_lookupsel"](firstCol);
					}catch(err){}
				}
			}
			if (typeof myjsf !="undefined" && myjsf != "") {
				try {
					//eval(myjsf + "(\"" + allCol + "\")");
					if (typeof window[myjsf]=="function") window[myjsf](allCol);
				} catch (err) {}
			}
			isSelected = 1;
			grid.destroy();			
		...
		var findByParam = function(e){
			...
			grid.store.load({params: {start: 0, limit: dhcc.icare.lu.pageSize}});
						...
						listeners:{
							"keydown":function(t,e){
								...
								if (e.getKey()==Ext.EventObject.DOWN){
									...
									var paName = t.id.split("TTF")[0];
									var basePa = grid.store.baseParams[paName];
									var currVal = Ext.getCmp(t.id).getValue(); 
									if (basePa==currVal){ //已查询过,回车是选中行
										selectRowAuto();	
									}else{
										dqTask.delay(300);
									}
