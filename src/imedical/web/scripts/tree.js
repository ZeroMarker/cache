...
function createDiagnosStore() {
    var diagnosStore = new Ext.data.SimpleStore({
        id: 'diagnosStore',
        fields: ["retrunValue", "displayText", "show"],
        proxy: new Ext.data.HttpProxy({ url: '../web.eprajax.amddiagnosmanager.cls?EpisodeID=' + episodeID + '&CurAction=List' })
    })
    diagnosStore.load();
    diagnosStore.on('load', function(store, record) {
        for (var i = 0; i < store.data.length; i++) {
            if (record[i].data.show == "1") {
                Ext.getCmp("cmbKBDiagnos").setValue(record[i].data.retrunValue);
                break;
            }
        }
    });
...
var diagnosStore = createDiagnosStore();
            ...
            listeners:
		    {
		        'select': function(combobox, record, index) {
		            Ext.Ajax.request({
		                ...
		                timeout: timedOut,
		                params: { EpisodeID: episodeID, CurAction: 'Select', KBDiagnos: record.data.retrunValue },
		                success: function(response, opts) {
		                    var obj = eval(response.responseText);
		                    if (obj != true) {
		                        alert('保存病人类型失败');
