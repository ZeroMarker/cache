...
function initDocument(){
	...
	$('#btnDivBalMonQuery').off().on("click", DivBalMonQuery_click);
	$('#btnDivCenterDL').off().on("click", DivCenterDL_click);
	$('#btnDivBalMonSubmit').off().on("click",DivBalMonSubmit_click);
}
...
function QryBallistDg()
{
	...
	$HUI.datagrid('#ballist',{
		url:$URL+"?ClassName="+'web.DHCINSUBalanceMonCtl'+"&QueryName="+'BalanceMonInfo'+"&StDate="+StDate+"&EnDate="+EndDate+"&AdmType="+''+"&InsuType="+InsuType+"&HospId="+HospitalId,
...
function DivBalMonSubmit_click(){
	//DivSubmit();
}
