 ...
 * @DateTime 2019-09-04
 */
function setLayoutSize(flag,HWFlag) {
 //if($("#DocPanel", window.parent.document).length>0){
if(HWFlag=="1"){
    var dWidth = $("#DiagnosisPanel", window.parent.document).width()-21;
    ...
    $("#BodyContent").height($("#DiagPanel", window.parent.document).height()-76);
...
function BodyLoadHandler() {
    setLayoutSize(0,HWFlag);
    SelectAdvice();
    var obj = document.getElementById("EpisodeID");
    if (obj) var EpisodeID = obj.value;

    obj = document.getElementById("AppointMUser");
    if (obj) {
        $.cm({
            wantreturnval: 1,
            ...
            ExcludeUser: session["LOGON.USERID"]
        }, function (data) {
            if (data.length > 0) {
                $HUI.combogrid("#AppointMUser", {
                    panelWidth: 320,
                    idField: "id",
                    textField: "text",
                    columns: [[
                        { field: "id", width: 50, title: "ID" },
                        { field: "text", title: "医生", width: 100 }
                    ]],
                    data: data,
                    onLoadSuccess: function () {
                        var lastUser = tkMakeServerCall("web.DHCPE.WorkDistribution", "GetLastAppointMUser", session["LOGON.USERID"]);
                        if (lastUser != "") {
                            $("#AppointMUser").combogrid("setValue", lastUser);
                        }
...
function HighRiskReport() {
    ...
    websys_lu(url, false, 'width=900,height=600,hisui=true,title='+$g("高危信息"))
...
function PreviewAllReport() {
    ...
    if (NewVerReportFlag == "Word") {
        ...
        websocoket_report("BPrintView", iEpisodeID);
...
function ShowResult_click(SaveFlag) {
    var obj, GSID = "", MainDoctor = "", AppointMUser = "";
    var UserID = session['LOGON.USERID'];
    ...
    obj = document.getElementById("AppointMUser");
    if (obj) AppointMUser = $("#AppointMUser").combogrid("getValue");

    var url = "dhcpenewgssdetail.hisui.csp?GSID=" + GSID + "&ButtonFlag=" + SaveFlag + "&MainDoctor=" + MainDoctor+"&ExpStr="+ExpStr;
    var content = '<iframe id="gssdetailframe" scrolling="auto" frameborder="0"  src="' + PEURLAddToken(url) + '" style="width:100%;height:99%;"></iframe>';
    ...
    $HUI.window("#GSSDetailWin", {
        ...
        onClose: function () {
            ...
            if (ret > 0) {
                ...
                var SignType = MainDoctor == "Y" ? "3" : "2";
                $PECA.CASign(SignType, EpisodeID, "", CAAuditWinClose, { EpisodeID: EpisodeID, MainDoctor: MainDoctor, AppointMUser: AppointMUser });
            }
            else {
                return false;
            }
...
function CAAuditWinClose(pObj) {
    var flag = tkMakeServerCall("web.DHCPE.ResultDiagnosis", "AuditStationS", pObj.EpisodeID, "Submit", "0", pObj.MainDoctor, pObj.AppointMUser);
    if (flag == "0") {
        window.location.reload();
        return false;
    }
...
function ModifyAdviceApp(StationID) {
    ...
    var GSID=$("#SSID").val();
    var MainDoctor=$("#MainDoctor").val();
    var url = "dhcpeneweditadvice.hisui.csp?GSID=" + GSID + "&MainDoctor=" + MainDoctor + "&StationID=" + StationID;
    ...
    var content = '<iframe scrolling="auto" frameborder="0" src="' + PEURLAddToken(url) + '" style="width:100%;height:100%;"></iframe>';


    websys_lu(url,false,'width=850,height=600,hisui=true,title=合并建议')
	*/
        ...
        if ((SaveInfo == "") && (ArrLength == "0")) {
            ...
            $.messager.confirm("提示", "没有建议,是否继续提交?", function (r) {
                if (r) {
                    ...
                    //判断是否存在与性别不一致的关键字
                    var SelectMark = ""
                    var SelectMark = tkMakeServerCall("web.DHCPE.ReportGetInfor", "SelectMarkDesc", PAADM,1);
                    if (SelectMark != "") {
                    ...
                    if (ret != "") {
                        ...
                        $.messager.confirm("提示", "存在未完成的项目'" + NoResultDesc + "'是否把该项目放弃？", function (r) {
                            if (r) {
                                ...
                                var ret = tkMakeServerCall("web.DHCPE.ResultDiagnosisNew", "RefuseItems", NoResultIDs, UserID);
                                ...
                                var ret = tkMakeServerCall("web.DHCPE.ResultDiagnosisNew", "CreateGSSDetail", GSID, MainDoctor, CurLocID);
...
function StatusChange(Type, QXType) {
    ...
    var SignType = MainDoctor == "Y" ? "3" : "2";
    $PECA.CASign(SignType, PAADM, "", CAStatusChangeCallback, { Type: Type, QXType: QXType });
}


function CAStatusChangeCallback(pObj) {
    var encmeth = "", PAADM = "", MainDoctor = "", Type = pObj.Type, QXType = pObj.QXType;
    var obj = document.getElementById("AuditBox");
    if (obj) encmeth = obj.value;
    obj = document.getElementById("EpisodeID");
    if (obj) var PAADM = obj.value;
    obj = document.getElementById("MainDoctor");
    ...
    if (flag == "0") {
        ...
        //刷新患者列表
         ...
         parent.$("#CanDiagnosisList").datagrid('load',{
        ...
        if (flag == "ReportStatusSendErr") { flag = "报告已取，不能取消"; }
        $.messager.alert("提示", flag, "info");
...
function ShowEDInfo(e) {
    ...
    var OEORDItemIDStr = e.id;
    var OEORDItemIDArr = OEORDItemIDStr.split("^");
    var OEORDItemID = OEORDItemIDArr[0];
    CurID = OEORDItemID;
    if (OEORDItemIDArr[1].indexOf("放弃") != -1) {
        $.messager.alert("提示", "该站点已放弃不可再添加建议", 'info');
        return
    }
...
function CreateDiv(obj, Info) {
    ...
    var winobj = document.getElementById("DiagnosisEDDiv");
    var winop = getoffset(winobj);

    var DivTop = winop[0] + 100 //150;
    var Divleft = winop[1] + 50 //560; 
...
//stid：站点ID,adm：就诊ID,stlocid：站点分类ID
function QueryED_KeyDown(e, stid, adm, stlocid) {
    var Key = websys_getKey(e);
    if ((13 == Key)) {

        var ret = tkMakeServerCall("web.DHCPE.DocPatientFind", "OutEDInfobyPrefix", stid, stlocid, adm, "0", e.value);
...
function EDDblClick(e) {
    ...
    // var EDID=EDID.split("^")[1]+"^"+EDID.split("^")[2];
    var EDID = EDID.split("^")[1] + "^" + EDID.split("^")[2] + "^" + EDID.split("^")[3];
...
function AddDiagnosis(EDID) {
    //alert(EDID+"EDID")
    //var ID=EDID.split("^")[0]+"^"+EDID.split("^")[1];
    var ID = EDID.split("^")[0] + "^" + EDID.split("^")[1] + "^" + EDID.split("^")[2];
    var SSIDObj = document.getElementById("SSID");
    if (SSIDObj) var SSID = SSIDObj.value;
    var obj = document.getElementById("AddEDClass");
    if (obj) var encmeth = obj.value;
    if (SSID == "") {
        $.messager.alert("提示", "还没有小结", "info");
        return false;
    }
    var MainDoctor = "";
    var obj = document.getElementById("MainDoctor");
    ...
    if (Arr[0] == 0) {
        ...
        $.parser.parse($("a[name='YGFlag']").parent());
        return false;
    }
...
function GSSDetail() {
    ...
    websys_lu(url, false, 'width=1150,height=660,hisui=true,title='+$g("总检日志"))
...
function Conclusion() {
    ...
    websys_lu(url, false, 'width=583,height=430,hisui=true,title='+$g("检查结论"))
...
function ConclusionOld() {
    ...
    websys_lu(url, false, 'width=670,height=430,hisui=true,title=检查结论')
...
function QMManager() {
    ...
    var url = "dhcpequalitymanager.eddit.hisui.csp?PAADM=" + iEpisodeID;
    websys_lu(url, false, 'width=670,height=600,hisui=true,title='+$g("质量上报"));
    return false;
}
/*
...
function SaveOneAdvice(e, ConfirmFlag) {
    var ID = e.id;
    var UserID = session['LOGON.USERID'];
    var LocID = session['LOGON.CTLOCID'];
    $.messager.confirm("提示", "确实保存到建议库吗？",
        function (r) {
            if (r) {
                var obj = document.getElementById(ID + "^JY");
                if (obj) Advice = obj.value;
                var obj = document.getElementById(ID + "^JL");
                if (obj) DisplayDesc = obj.value;
                var flag = tkMakeServerCall("web.DHCPE.ResultDiagnosisNew", "SaveOneED", ID, Advice, DisplayDesc, UserID, LocID);
                if (flag == "0") {
                    $.messager.popover({ msg: '保存成功！', type: 'success', timeout: 1000 });
                } else if (flag == "1") {
                    $.messager.alert("提示", "建议已存在，不允许再次添加!", "info");
                }
            }
            else {
                return false;
