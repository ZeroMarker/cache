...
$(function(){
    ...
    if(EpisodeID!="")
    {
        ...
        {
	       $.messager.alert("提示","该日间申请已确认，如需修改请选择修改手术申请！","error");
	      	$("#btnSave").linkbutton("disable");
        }
        var needAuditStr=$.m({
            ClassName:"web.DHCANAdaptor",
            MethodName:"CheckDayOperAnAudit",
            AdmId:EpisodeID
        },false);
         var auditstr=needAuditStr.split("^");
         if((auditstr[1]=="Y")&&(auditstr[0]!="Y"))
         {
	          $.messager.alert("提示","该日间申请尚未进行麻醉术前评估,不能正式申请手术！","error");
	          $("#btnSave").linkbutton("disable");
         }
         else if((auditstr[1]=="Y")&&(auditstr[0]=="Y")&&(auditstr[2]!="Y"))
         {
	          $.messager.alert("提示","该日间申请麻醉术前评估未通过！","error");
	          //$("#btnSave").linkbutton("disable");
         }
    }
...
function loadDatas(){
    ...
    if((opaId==null)||(opaId==""))
    {
        ...
        if(checkinbedflag=="N")
		 {
			// $.messager.alert("提示","病人尚未安排床位","info");
            // $("#btnSave").linkbutton('disable');
		 }
        var checkDisflag=$.m({
            ClassName:"web.DHCANOPArrange",
...
function editAnaestOperation(){
    ...
    if(selectRow)
    {
        ...
        $.messager.alert("提示", "请先选择要修改的手术！", 'error');
        return;
    }
}
function removeAnaestOperation(){
    ...
    if(selectRow)
    {
        ...
        $.messager.alert("提示", "请先选择要删除的手术！", 'error');
        return;
    }
}
...
function saveOperation()
{
    ...
    if(surgeonId=="")
    {
       // $.messager.alert("提示","手术医生不能为空！","info");
        //return;
    }
...
function CheckData(){
    ...
    {
        $.messager.alert("提示","请选择HbsAg!","error");
		return false;
    }
    if($HUI.combobox("#HcvAb").getValue()=="")
    {
        $.messager.alert("提示","请选择HcvAb!","error");
		return false;
    }
    if($HUI.combobox("#HivAb").getValue()=="")
    {
        $.messager.alert("提示","请选择HivAb!","error");
		return false;
    }
    if($HUI.combobox("#Syphilis").getValue()=="")
    {
        $.messager.alert("提示","请选择梅毒!","error");
		return false;
    }
