...
{
	initUserInfo(); //add by csj 2020-11-18
	...
	var SourceType=getElementValue("SourceType");
  	...
  	///Modefied by zc0098 2021-1-29 按钮控制 begin
  	if (getElementValue("SourceName")=="PMTemplate")
  	{
	  	if (getElementValue("EquipRangeDR")=="")
	  	{
	  		initDataGrid();
...
function hiddenRadioBySourName()
{
	...
	switch(SourceName){
		case "MeteragePlan":   //modify by mwz 2019-12-16 MWZ0024
			keyrows=[
			//{text:'指定科室',id:'LimitLoc'},     //modify by lmm 2021-06-07 1963932
		...
		case "MaintPlan":   //modify by mwz 2019-12-16  MWZ0024
			keyrows=[{text:'指定科室',id:'LimitLoc'},         
							...
							{text:'指定科室及设备项',id:'LimitLocItem'}]  
		break;
		case "PMTemplate":   //Modefied by zc0098 2021-1-29 begin
			keyrows=[{text:'指定设备',id:'LimitEquip'},         
						{text:'指定设备项',id:'LimitItem'}]  
		break;				 //Modefied by zc0098 2021-1-29 end
		default :
		break;
	}
}
...
function FillEquipRangeData()
{
	...
	$HUI.keywords("#redkw").select("LimitItem");  //关键字添加默认 指定设备项
  	...
  	if ((getElementValue("SourceType")=="3")&&(getElementValue("EquipRangeDR")=="")) return;  	  //Modefied by zc0098 2021-1-29 PM模板类型过滤
...
function EquipRangeEquipDataGrid(EquipRangeListID,Type,title)
{
		...
		{	//add by csj 2020-11-17 添加打印 需求号：1605288
			...
			iconCls:'icon-print', 
	        text:'导出',      
	        handler: function(){BPrint_Clicked($('#EquipRangeDR').val(),Type);}
		 }] , 
		//modify by lmm 2019-06-06 begin	    
...
{
	var keyObj=$HUI.keywords("#redkw").getSelected();		//czf 1776711 2021-03-03
	
	if (keyObj.length==0)
	{
		alertShow("未选中关键字,请先选中关键字再添加设备范围!")
		return
	}
	...
	{
		//add by lmm 2020-08-10
		if (getElementValue("ComputerFlag")=="Y")
		{EquipAttributeString="id1"}
		else {EquipAttributeString=""}  //add by lmm 2020-09-07 1485346
		url="dhceq.em.maintmasteritemlimitlist.csp?"+"&SourceType="+type+"&Planstatus="+planstatus+"&EquipAttributeString="+EquipAttributeString; //modify by lmm 2020-08-10
	}
...
function GetEquipRangeval()
{
	...
	combindata=combindata+"^"+getElementValue("SourceID") ;    ///Modefied by zc0098 2021-1-29 错误修正
...
function hiddenTDeleteListColumn()
{
	if(getElementValue("SourceType")==2)
	{
		...
		list=list.replace(/[\r\n]/g,"")	//去掉回车换行
		list=list.split("^");
		// MZY0112	2434030		2022-01-21	审核或作废
		if ((list[25]==2)||(getElementValue("CancelFlag")=="Y"))
		{
...
///Add by CSJ 2020-11-18 添加润乾导出打印
function BPrint_Clicked(EquipRangeDR,Type)
{
	var PreviewRptFlag=1	//getElementValue("PreviewRptFlag");
	var fileName=""	;
	// MZY0072	1777017		2021-04-19	增加Excel导出
	var GetCurDate=tkMakeServerCall("web.DHCEQCommon","GetCurDate");
	var PrintFlag=tkMakeServerCall("web.DHCEQCommon","GetSysInfo","990062");	//打印单据采用方式(0:Excel		1:润乾)
	if (PrintFlag==0)
	{
		// MZY0082		1954590		2021-07-14	调整导出
		var gbldata=tkMakeServerCall("web.DHCEQ.EM.BUSEquipRange","GetList",EquipRangeDR,Type);
		var list=gbldata.split(tkMakeServerCall("web.DHCEQCommon","GetSysInfo","990026"));
		var Listall=list[0];
		var rows=list[1];
		if (rows<1)
		{
		...
		var Lists=Listall.split(tkMakeServerCall("web.DHCEQCommon","GetSysInfo","990025"));
		var TemplatePath=tkMakeServerCall("web.DHCEQStoreMoveSP","GetPath");
	    var Template=TemplatePath+"DHCEQMaintEquipExport.xls";
		if (getElementValue("ChromeFlag")=="1")
		{
			...
			Str +="var firstRow=1;"
			Str +="var Col;"
			Str +="for (var j=1;j<="+rows+";j++){"
			Str +="var OneInfo=Lists[j];"
			Str +="var List=OneInfo.split('^');"
			Str +="Col=1;"
			Str +="xlsheet.cells(j+firstRow,Col++)=List[1];" // 设备名称
			Str +="xlsheet.cells(j+firstRow,Col++)=List[1];" // 设备编号
			Str +="xlsheet.cells(j+firstRow,Col++)=List[2];" // 设备类组
			Str +="xlsheet.cells(j+firstRow,Col++)=List[3];" // 科室
			Str +="xlsheet.cells(j+firstRow,Col++)=List[4];" // 机型
			Str +="xlsheet.cells(j+firstRow,Col++)=List[5];" // 出厂编号
			Str +="}"
			Str +="xlsheet.cells(j+firstRow,1)=制单人:'"+curUserName+"';"
			Str +="xlsheet.cells(j+firstRow,6)=打印日期:'"+GetCurDate+"';"
		...
		else
		{
	    	...
	    	for (var j=1;j<=rows;j++)
			{
				...
				xlsheet.cells(Row,1)=List[0];//设备名称
				xlsheet.cells(Row,2)=List[1];//设备编号
				xlsheet.cells(Row,3)=List[2];//设备类组
				xlsheet.cells(Row,4)=List[3];//科室
				xlsheet.cells(Row,5)=List[4];//机型
				xlsheet.cells(Row,6)=List[5];//出厂编号
	    	}
	    	xlsheet.cells(Row+1,1)="制单人:"+curUserName  //username; //制单人
	    	xlsheet.cells(Row+1,6)="打印日期:"+GetCurDate;
    ...
    if(PreviewRptFlag==0)
    {
	    fileName="{DHCEQMaintEquipExport.raq(EquipRangeDR="+EquipRangeDR+";USERNAME="+curUserName+";HOSPDESC="+curSSHospitalName+";Type="+Type+")}";
...
///Modefied by zc0098 2021-1-29 保存方法
function BSave_Click()
{
	var EquipRangeval=GetEquipRangeval();
	var EquipRangevalList=GetEquipRangevalList();
	var Rtn = tkMakeServerCall("web.DHCEQ.EM.BUSEquipRange", "SaveEquipRange",EquipRangeval,EquipRangevalList);
	var Rtn=Rtn.split("^");
	if (Rtn[0]<0)
	{
		messageShow("","","保存失败！")
	...
	if (getElementValue("EquipRangeDR")=="")
	{
		setElement("EquipRangeDR",Rtn[1]);
		websys_showModal("options").mth();
	}
	var url="dhceq.plat.equiprange.csp?&SourceType=3&SourceName=PMTemplate&SourceID="+getElementValue("SourceID")+"&EquipRangeDR="+getElementValue("EquipRangeDR");
	if ('function'==typeof websys_getMWToken){		//czf 2023-02-14 token启用参数传递
		url += "&MWToken="+websys_getMWToken()
	}
	window.location.href=url;	
}
///Modefied by zc0098 2021-1-29 删除方法
function BDelete_Click()
{
	if (getElementValue("EquipRangeDR")=="") return;
	var Rtn = tkMakeServerCall("web.DHCEQ.EM.BUSEquipRange", "DeleteEquipRange",getElementValue("EquipRangeDR"));
	var Rtn=Rtn.split("^");
	if (Rtn[0]<0)
	{
		messageShow("","","保存失败！")
	...
	// MZY0119	2574406		2022-04-07注释父窗口调用
	//setElement("EquipRangeDR","");
	//websys_showModal("options").mth();
	var url="dhceq.plat.equiprange.csp?&SourceType=3&SourceName=PMTemplate&SourceID="+getElementValue("SourceID")+"&EquipRangeDR=";
	if ('function'==typeof websys_getMWToken){		//czf 2023-02-14 token启用参数传递
		url += "&MWToken="+websys_getMWToken()
	}
	window.location.href=url;
