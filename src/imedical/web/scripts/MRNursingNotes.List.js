...
function NOTDSReportFlagLinkDisable(evt) {
	return false;
}
...
function DisableLink(fld) {
	if (fld) {
		fld.value = "";
		fld.disabled = true;
		fld.onclick=NOTDSReportFlagLinkDisable
...
function DisableReportFlagLinks() {
	...
	for (var curr_fld=0; curr_fld<ary.length; curr_fld++) {
		...
		if (obj) {
			if (obj.id.substring(0,16) == "NOTDSReportFlagz") {
				DisableLink(obj);
			}
		}
	}
...
// seanl
function NOTIncDischSummFlagLinkDisable(evt) {
	return false;
}

function DisableDischSummLink(fld) {
	if (fld) {
		fld.value = "";
		fld.disabled = true;
		fld.onclick=NOTIncDischSummFlagLinkDisable
		fld.className = "disabledLink";
	}
}

function DisableIncDischSummLinks() {
	var ary=ltbl.getElementsByTagName("A")
	for (var curr_fld=0; curr_fld<ary.length; curr_fld++) {
		var obj=ary[curr_fld];
		if (obj) {
			if (obj.id.substring(0,22) == "NOTIncludedForDischSum") {
				DisableDischSummLink(obj);
			}
		}
	}
...
function MRNursingNotesList_PassSelected(lnk,newwin) {
	var f,aryfound;
