...
function BSearch_click() {
    ...
    if ($("#NoFetchReport").checkbox('getValue')) {
        //$('#BFetch').linkbutton('enable');
        $('#BFetch').linkbutton({ text:$g('撤销领取')});
        ButtonType = "Cancel";
    } else {
        //$('#BFetch').linkbutton('disable');
        $('#BFetch').linkbutton({ text:$g('领取报告')});
        ButtonType = "";
    }
...
function BClear_click(Type) {
    ...
    if (Type == "1") {
        ...
        $("#NameWin,#IDCardWin,#TelWin,#ExpressNoWin,#AddressWin").val("")
    ...
    if (OneOrMore == "ONE") {
        if (Type == "SELF") {
            ...
            if (Arr[0] != 0) {
				...
				$.messager.popover({msg: Type + "成功！",type:'success',timeout: 1000});
        ...
        if (Err == "")  {
			$.messager.popover({msg: '批量领取成功！',type:'success',timeout: 1000});
            $("#FetchReport").datagrid("reload");
            $("#FetchInfoWin").window("close");
            return false;
        } else {
...
function ShowFetchInfo(Type, Data) {
    ...
    var selectrow = $("#FetchReport").datagrid("getChecked");
    for(var i=0;i<selectrow.length;i++){	
     var SendUser=selectrow[i].TSendUser; 
    }
    if (($("#NoFetchReport").checkbox('getValue'))&&(SendUser!="")) {
        //$('#BFetch').linkbutton('enable');
	...
	if(Data.RepoerSend=="KD"){
	    $('#ExpressNoWin').attr('disabled',false);
	    $('#AddressWin').attr('disabled',false);
     	$('#AddressWin').val(Data.Address);
     }else{
      	$('#ExpressNoWin').attr('disabled',true);
      	$('#AddressWin').attr('disabled',true);
      	$('#AddressWin').val(""); 
   	 }
        ...
        if (Type == "FetchByIndex") {   
            ...
            $("#ExpressNoWin").val(ret.ExpressNo);;
            ButtonText = $g("修改");
        }
        ...
        var myWin = $HUI.dialog("#FetchInfoWin",{
            ...
            buttons:[{
                ...
                handler:function(){
                    ...
                    var ExpressNo=$("#ExpressNoWin").val();

					  var Address=$('#AddressWin').val();
                    var Instring = Name + "^" + IDCard + "^" + Tel + "^" + ExpressNo+"^"+Address;
                        //alert(Instring)
        ...
        if (ButtonType == "Cancel") {
            $.messager.confirm("撤销领取", "确定要撤销领取吗?", function (r) {
        	if (r) {
				var Err = "";
                ...
                var myWin = $HUI.dialog("#FetchInfoWin",{
                    ...
                    buttons:[{
                        ...
                        handler:function(){
                            ...
                            var ExpressNo=$("#ExpressNoWin").val();
                            var Instring = Name + "^" + IDCard + "^" + Tel + "^" +  ExpressNo;
