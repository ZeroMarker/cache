	...
	InvDataGrid = $HUI.datagrid("#InvListQueryTab", {
...

			{
				...
				formatter: function (value, rowData, rowIndex) {
					if ((rowData.TOPInfo != "") && (rowData.TOPInfo.split("^")[0] == "SE")) {
						...
						return "<span style='cursor:pointer;' class='icon-cancel' title="+$g('撤销预结算')+" onclick='cancel_cashier(\""+rowData.TRowId+"\")'>&nbsp;&nbsp;&nbsp;&nbsp;</span>&nbsp;&nbsp;&nbsp;&nbsp;"+
					"<span style='cursor:pointer;' class='icon-init' title="+$g('确认完成')+" onclick='confirm_cashier(\""+rowData.TRowId+"\")'>&nbsp;&nbsp;&nbsp;&nbsp;</span>";
					 ...
					 <img src='../scripts_lib/hisui-0.1.0/dist/css/icons/select_grant.png' border=0/></a>";
						...
						return "<span style='cursor:pointer;' class='icon-init' title='补交易' onclick='re_refund(\"" + rowData.TOPInfo.split("^")[1] + "\")'>&nbsp;&nbsp;&nbsp;&nbsp;</span>"
						
						return "<a href='#' onclick='re_refund(\"" + rowData.TOPInfo.split("^")[1] + "\")' >\
					 <img title="+$g('补交易')+"  src='../scripts_lib/hisui-0.1.0/dist/css/icons/paper_money.png' border=0/></a>";
		...
		onLoadSuccess: function (data) {
			...
			for (var i = 1; i < data.rows.length; i++) {
				if ((data.rows[i]['TOPInfo'] != "") && (data.rows[i]['TOPInfo'] == data.rows[i - 1]['TOPInfo'])) {
					mark += 1;
...
function cancel_cashier(inprtId) {
	var insuInfo = tkMakeServerCall("web.DHCPE.CashierEx", "GetInsuInfo", inprtId);
	if (insuInfo != "") {
		var insuStr = "";
		var insuArr = insuInfo.split("^");
		try {
			var insuRet = InsuPEDivideStrike("0", userId, insuArr[0], insuArr[2], insuArr[1], insuStr, "N");
			if (insuRet != "0") {
				$.messager.alert("提示", "医保退费失败", "info");
				return false;
			}
		} catch (e) {
			$.messager.alert("提示", $g("医保退费失败")+":" + e.message, "info");
			return false;
		}
	}
	var Return = tkMakeServerCall("web.DHCPE.DHCPEBillCharge", "CancelCashier", inprtId, userId);
	if (Return != "") {
		$.messager.alert("提示", $g("撤销预结算失败") + Return, "info");
		return false;
	} else {
		$.messager.alert("提示", $g("撤销预结算成功")  + Return, "success");
		InvDataGrid.reload();
	}
}
...
function confirm_cashier(inprtId) {
	...
	var Return = tkMakeServerCall("web.DHCPE.Cashier", "RealCashier", inprtId, userId, "^^1", locId);
	if (Return.split("^")[0] != "") {
		$.messager.alert("提示", $g("确认失败") + Return, "error");
		return false;
	} else {
	...
	$.messager.prompt("提示", "请输入订单ID：", function (r) {
		if (r) {
			...
			if (ReFlag.ResultCode != "0") {
				$.messager.alert("提示", $g("计费关联失败，请联系信息科")+"！\n" + ReFlag.ResultMsg, "info");
			}
		} else {
		}
	});
...
function re_refund(invprt) {
	...
	var rfdRtn = $PEPay.extRefund(dropInvprt,oriInvprt);  //DHCPEPayService
