...
function fillData()
{
	...
	{
		var jsonObj=tkMakeServerCall("web.DHCEQ.EM.BUSMMaintRequest", "GetOneMaintRequest",mrRowID,"",actionCode,"");
	}
	...
	if(actionCode==""){
		...
		if(isPc){
			...
			setElement("MROtherFee","配件费："+jsonData["MROtherFee"]);
			setElement("MRMaintFee","维修费："+jsonData["MRMaintFee"]);
	...
	if(isPc){
		...
		setElement("MREquipDR_EQNameRe",jsonObj.Data["MREquipDR_EQName"]);
...
function doDelete()
{
	...
	if(isPc){
		...
		if(qxType==0)
		{
			if((maintType==1)) src="dhceq.em.hardwaremaintsimple.csp?";  //工程师
			else src="dhceq.em.mainttabsnew.csp?";  //值班员
		}else{
			src="dhceq.em.mainttabsnew.csp?"; //临床科室
		}
...
function doApprove()
{
	...
	if(toAction!=""){
		...
		var rtn=tkMakeServerCall("web.DHCEQ.EM.BUSMMaintRequest","AuditStepsByActions",fromActinCode,toAction,combindata,editFieldsInfo,approveOpinion);
	}else{
