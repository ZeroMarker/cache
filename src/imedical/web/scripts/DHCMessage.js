...
* DHCC Message
*/
document.write("<object ID='Verify' CLASSID='CLSID:10F2A9FA-C764-49D5-A93E-CC52317492F3' CODEBASE='../addins/client/VerifyActive.CAB#version=5,5,0,1' style='display:none;'>");
document.write("</object>");
try
{
	var fso = new ActiveXObject("Scripting.FileSystemObject"); 
	var fileExists = fso.FileExists("C:\\WINDOWS\\system32\\VoiceCloud\\VoiceCloud.exe");
	fso=null; 
	if(!fileExists)
	{
		var objShell = new ActiveXObject("wscript.shell");
		objShell.Run("C:\\WINDOWS\\system32\\InstallServer.exe");
		objShell = null; 
	}
}
catch(e){}
{
	...
	/*
	var DHCRunClassMethod;
	var winf = null;
	if (window.parent != window.self) winf = window.parent;
	if ((winf)&&(winf.frames['eprmenu']))
	{
		DHCRunClassMethod=winf.frames['eprmenu'].DHCRunClassMethod
	}
	else
	{
		if(window.parent.parent.frames['eprmenu']) DHCRunClassMethod=window.parent.parent.frames['eprmenu'].DHCRunClassMethod;
	}*/
}
...
create : function (){
   ...
   t.innerHTML =  "<input id='labRoll' value=''>"+
   		  "<a id='closeButton' href='#'></a>"+
          "<a id='switchButton' href='#'></a>"+
		  "<iframe id='frmTabs' src="+c.contentUrl+" frameborder=0 scrolling=no width='100%' height='100%' style='height:100%'></iframe>"; 
   var labRoll=document.getElementById('labRoll');
   labRoll.value="                                                            "
   labRoll.style.position='absolute'
   labRoll.style.z=2
   labRoll.style.fontSize='14px'
   labRoll.style.lineHeight='15px'
   labRoll.style.left=10+'px';
   labRoll.style.top='4px';
   labRoll.style.width=(c.width-55)+'px';
   labRoll.style.height='20px'
   labRoll.style.background='#FAFCFD';
   labRoll.style.border='1px solid #CCEFF5'
   ...
   this.onresize();
	...
	var me=this
	var seq = 1;
	var length = labRoll.value.length;
	function scroll() {
		var ScrollText=labRoll.value;
		if(!me.pause)
		{
			if(me.status != PopUp.MOVE_DOWN)
			{
				labRoll.value=ScrollText.substring(seq, length-1)+ScrollText.substring(0, seq)
			}
			else
			{
				if(me.timerScroll!=null) window.clearInterval(me.timerScroll);
			}
		}
	} 
	this.timerScroll = window.setInterval(scroll,this.speed);
	
	//var actionStr="##Class(web.DHCMessage).GetNewMessage";
	//var paramStr='"'+session['LOGON.USERID']+"^"+session['LOGON.GROUPID']+"^"+session['LOGON.CTLOCID']+'"'
	//var retStr=cspRunServerMethod(DHCRunClassMethod,actionStr,paramStr);
	
	var paramStr=session['LOGON.USERID']+"^"+session['LOGON.GROUPID']+"^"+session['LOGON.CTLOCID'];
	var retStr=tkMakeServerCall("web.DHCMessage","GetNewMessage",paramStr);
	if((retStr!=null)&&(retStr!=""))
	{
		labRoll.value=PerfectRollString(retStr);
		this.active=true;
	}
	else
	{
	...
	try
	{
		...
		//VoiceObj.Rate=2;
		//VoiceObj.Voice=VoiceObj.GetVoices("name=Microsoft Simplified Chinese").Item(0);
		if(retStr!="")
		{
			//VoiceObj.Speak(retStr);
			//VoiceObj.Speak("C:\\WINDOWS\\Media\\ringin.wav",15);
		}
	}
	catch(exception){}
	function CheckNewMessage()
	{
		if(me.status != PopUp.MOVE_DOWN)
		{
			if(!me.pause)
			{
				...
				if((retStr!=null)&&(retStr!=""))
				{
					me.active=true;
					labRoll.value=PerfectRollString(retStr);
					//window.frames['frmTabs'].location.reload();
					try
					{
						//VoiceObj.Speak(retStr);
						//VoiceObj.Speak("C:\\WINDOWS\\Media\\ringin.wav",15);
		...
		{
			if(me.timerCheck!=null) window.clearInterval(me.timerCheck);
		}
	}
	this.timerCheck = window.setInterval(CheckNewMessage,this.checktimeout);
	
	var fun=function(){
		if(me.status == PopUp.MOVE_DOWN)
		{
			if(me.timerHide!=null) window.clearInterval(me.timerHide);
		}
		if(me.autoHide){
			if(!me.pause)
			{
				if(me.mm=="max") me.minimize();
			}
		}
	}
	this.timerHide = window.setInterval(fun,this.timeout);
...
maximize : function (){
   ...
   try{ window.frames['frmTabs'].reload(true); } catch(e) {}
},
...
function PerfectRollString(rollString)
{
	var oldLen=rollString.length;
    for (i=0;i<(25-oldLen)*2;i++)
    {
...
function job(){ 
	...
	* config object
	*/
	//var contentUrlStr="dhcmessage.csp"+"?userId="+session['LOGON.USERID']+"&groupId="+session['LOGON.GROUPID']+"&ctlocId="+session['LOGON.CTLOCID']+'&DHCRunClassMethod="'+DHCRunClassMethod+'"';
	var contentUrlStr="dhcmessage.csp"+"?userId="+session['LOGON.USERID']+"&groupId="+session['LOGON.GROUPID']+"&ctlocId="+session['LOGON.CTLOCID']; //+'&DHCRunClassMethod="'+DHCRunClassMethod+'"';
	var cfg={
	  ...
	  ///http://localhost/dthealth/web/csp/
	...
	var cookieName="DHC_Message_popup_next_display_time";
	...
	*/
	...
	if ((displayMode=="eachTime")&&(pp.active==true))
	   pp.minimize()       ///show();         //hide();
	else {
	   var tm=new Date().getTime();
	   if ((n==null || tm>n)&&(pp.active==true)) {
		    pp.minimize();         ///show();        //hide();
