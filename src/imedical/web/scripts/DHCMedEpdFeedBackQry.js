...
var tmpList=GetChinese("MethodGetChinese","DHCMedEpdFeedBackQry");
var objLogUser;
...
function Query_click() {
    ...
    //alert(cDateTo);
    //lnk="websys.default.csp?WEBSYS.TCOMPONENT=DHCMedEpdFeedBackList" + "&cDateFrom=" + cDateFrom + "&cDateTo=" +cDateTo
	lnk="websys.default.csp?WEBSYS.TCOMPONENT=DHCMedEpdFeedBackList" + "&cDateFrom=" + cDateFrom + "&cDateTo=" +cDateTo+ "&cDep=" +cDep+ "&cLoc=" +cLoc+ "&cStatus=" +cStatus+ "&cPlace=" +cPlace;
...
function AddStatus(){
	...
	var objDic=BASE_GetDictionary(s,"EpidemicEvaluationType","Y");
	//alert(objDic.Count);
	var a=(new VBArray(objDic.Keys()));   
    for (i=0;i<objDic.Count;i++){
...
function AddPlace(){
	...
	s=gGetObjValue("tmpDictionary");
	var objDic=BASE_GetDictionary(s,"EpidemicPatientType","Y");
	var a=(new VBArray(objDic.Keys()));
    for (i=0;i<objDic.Count;i++){
