...
//配置数据源
var StkSysCounteGridUrl = 'dhcst.stksyscounteaction.csp';
...
var saveStkSysCounte = new Ext.Toolbar.Button({
	...
	handler:function(){
		...
		for(var i=0;i<mr.length;i++){ 
			...
			var dataRow = rowId+"^"+appId+"^"+hosp+"^"+loc+"^"+catGrp+"^"+prefix+"^"+year+"^"+month+"^"+day+"^"+length;
			if(data==""){
				data = dataRow;
			}else{
...
StkSysCounteGrid = new Ext.grid.EditorGridPanel({
	...
	loadMask:true,
    tbar:[saveStkSysCounte],
	clicksToEdit:1
});

StkSysCounteGridDs.load();
//=========================单据号自动生成规则===========================
var HospPanel = InitHospCombo('DHC_StkSysCounter',function(combo, record, index){
	HospId = this.value; 
...
Ext.onReady(function(){
	...
	var mainPanel = new Ext.Viewport({
		layout:'border',
		items:[HospPanel,StkSysCountePanel],
		renderTo:'mainPanel'
	});
});
//===========模块主页面=================================================