function CatAdd(Catid,Catdesc,Fn) {
	Ext.QuickTips.init();
	...
	function InsertData() {
		...
		var url = "dhcst.phccatmaintainaction.csp?action=Save";
			var loadMask=ShowLoadMask(Ext.getBody(),"处理中...");
			Ext.Ajax.request({
						url : url,
						method : 'POST',
						params:{ParCat:Catid,CatCode:catcode,CatDesc:catdesc},
						waitMsg : '处理中...',
	...
	function GetCurMaxCode(){
		var url="dhcst.phccatmaintainaction.csp?action=GetCurCode&CatId="+Catid+"&Hospital="+session['LOGON.HOSPID'];
		var CurMaxCode=ExecuteDBSynAccess(url);
		Ext.getCmp('CatCode').setValue(CurMaxCode);
		}
