...
function GetStoreCon(){
	var url = 'dhcst.druginfomaintainaction.csp?actiontype=GetStoreCon&IncRowid=' + drugRowid;					
	Ext.Ajax.request({
		url : url,
		method : 'POST',
		waitMsg : '查询中...',
			...
			var StoreConFormPanel = new Ext.form.FormPanel({
				...
				items : [{
					...
					items : [{
						columnWidth : .5,
						xtype:'fieldset',
						items : [RoomTemperature,Airtight,Ventilation,MeltSeal,ColdDark,Refrigeration,LowHumidity,HighHumidity]
					}, {
						columnWidth : .5,
						xtype:'fieldset',
						items : [Dry,Dark,Radiation,Cool,Seal,LowestTemperature,MaximumTemperature]
					}]
				}]
			});
			...
			var win = new Ext.Window({
				...
				buttons:[{
					...
					handler: function() {
						...
						//格式：常温+"^"+干燥+"^"+密闭+"^"+避光+"^"+通风+"^"+防辐射+"^"+熔封+"^"+阴凉+"^"+凉暗+"^"+密封+"^"+冷藏+"^"+最低温+"^"+最高温+"^"+最低湿度+"^"+最高湿度
						storeStr = roomTValue+"^"+dryValue+"^"+airtightValue+"^"+darkValue+"^"+venTValue+"^"+radiationValue+"^"+meltSealValue+"^"+coolValue+"^"+coldDarkValue+"^"+sealValue+"^"+refrigerationValue+"^"+lowestTemperatureValue+"^"+maxValue+"^"+lowHumidityValue+"^"+highHumidityValue;
						//存储条件保存成功后,在库存项附加表中保留dr(若之前没有)
						Ext.Ajax.request({
							url: 'dhcst.druginfomaintainaction.csp?actiontype=saveStoreCon&storeStr='+storeStr+'&rowid='+storeConRowId+'&IncRowid=' + drugRowid,
