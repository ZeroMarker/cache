...
var DelDetailBT=new Ext.Button({
	...
	function CheckRpEqualSp(rownum)
	{
		...
		var fields = ["AspBatId","AspBatNo","BatExp","Incib", "StkCatDesc", "InciId", "InciCode","InciDesc", "AspUomId","AspUomDesc",
				...
				,"AdjReasonId", "AdjReason","Remark", "AdjUserName","BUomId","ConFacPur"];
		// 支持分页显示的读取方式
					...
					editor : new Ext.grid.GridEditor(new Ext.form.NumberField({
						...
						listeners : {
							'change' : function(field, newValue,oldValue) {
									...
									var record = DetailGrid.getStore().getAt(cell[0]);
									var PriorSp= record.get("PriorSpUom");
									record.set("DiffSpUom",Math.round((resultSpNew-PriorSp)*10000)/10000);									
									
									//DetailGrid.startEditing(cell[0], 14); */
							},
							'blur':function(field,e){
									var resultSpNew = field.getValue();
									if (resultSpNew == null || resultSpNew.length <= 0) {
			...
			Ext.Ajax.request({
						...
						success : function(result, request) {
							...
							if (jsonData.success == 'true') {
								...
								//var ss2=new Date().add('d',10000).format('Y-m-d')
								
								//rowData.set("PreExecuteDate", Date.parseDate(ss2,'Y-m-d') ); 
