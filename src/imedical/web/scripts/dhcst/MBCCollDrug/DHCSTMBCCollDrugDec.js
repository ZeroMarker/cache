					...
					if (jsonData.success=='true') {
						Msg.info("success", "保存成功!");
					     ...
					     Msg.info("error", "非代煎处方!");
					      ...
					      Msg.info("warning", "该处方的下一状态不是煎药状态!"+"状态应为:"+nextstate+"!")
					}
					 ...
					 if (jsonData.success == 'true') {
						retflag='true';
					     ...
					     //var node=treepanel.getSelectionModel().getSelectedNode();  //获取选中节点	
	                                   ...
	                                   var locid=Gnode
	                                    //alert(locid+","+gUserId+","+Ddate+","+Dtime+","+Ddate+","+Dtime)
	                                    PrintJYReg(locid,gUserId,Ddate,Dtime,Ddate,Dtime,"","40",gUserName)
				               ChildGridDs.removeAll();
                                           ChildGridDs.setBaseParam("locId",'0')
                                           ChildGridDs.setBaseParam("Number",NumberI)
                                           ChildGridDs.setBaseParam("User",gUserId)
			               ...
			               else {retflag='false';Msg.info("success", "更新失败!");}
