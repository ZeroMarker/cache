...
function GridColSet(gridSelected, AppName) {
	...
	// 指定列参数
	var fields = ["rowid", "id", "header", "width", "align", "name", "sortable", "hidden", {
			name: 'seqno',
			type: 'float'
		}, "format", "entersort"];
	// 支持分页显示的读取方式
...
function RefreshGridColSet(Grid, AppName) {
	...
	*/
	...
	var OldConfig = columnModel.config.slice(0); //拷贝数组;
	var colCount = -1;
	var NewConfig = [];
	for (var nc = 0; nc < OldConfig.length; nc++) {
		if (OldConfig[nc]) {
			if (OldConfig[nc].id == 'numberer' || OldConfig[nc].selections) {
				colCount++;
				NewConfig[colCount] = OldConfig[nc]; // 序号与勾选框. Huxt 2022-05-07
			}
		}
	}
	//var NewConfig=columnModel.config.slice(0,1);  //拷贝数组;后面会获取系统保存的列设置,“序号”列未在列设置中保存，直接取出来。
	//var NewConfig=columnModel.config.clone();
	//NewConfig=OldConfig;
	var ColDataIndexArr = []; //存放dataIndex 与 colindex 的键值对
	for (var i = 1; i < OldConfig.length; i++) {
		var column = OldConfig[i];
		var dataIndex = column.dataIndex;
		ColDataIndexArr[dataIndex] = i
	}
	var selColArr = []; //存放列编辑保存的列,dataIndex做下标
	for (var i = 0; i < ColNum; i++) {
		var ColInfo = ListColArr[i];
		var ColInfoArr = ColInfo.split("^");
		var ColKey = ColInfoArr[0];
		var dataIndex = ColInfoArr[1];
		...
		selColArr[dataIndex] = SeqNo;
		var ColIndex = ColDataIndexArr[dataIndex] || -1;
		var column = columnModel.config[ColIndex];
		if (column) {
			column.width = parseFloat(Width);
			column.header = $g(Header);
			column.align = Align;
			if (Sort == "Y") {
			...
			colCount++;
			NewConfig[colCount] = column;
			//NewConfig[SeqNo] = column;  // 后台已排序,可以不用SeqNo
		}
	}
	var NewConfig2 = []; // 列编辑里未保存的列，也就是新增加的列
	var NewConfig2Ind = 0;
	for (var i = 0; i < OldConfig.length; i++) {
		var column = OldConfig[i];
		if (column.id == 'numberer' || column.selections) {
			continue; // 序号与勾选框不考虑. Huxt 2022-05-07
		}
		var dataIndex = column.dataIndex;
		if (selColArr[dataIndex] == undefined) {
			column.hidden = true;  // 新增的列默认隐藏
			NewConfig2[NewConfig2Ind++] = column;
		}
	}
	var NewConfig3 = NewConfig.concat(NewConfig2); // 列设置保存的列与新增的列合并

	Grid.reconfigure(Grid.getStore(), new Ext.grid.ColumnModel(NewConfig3));
