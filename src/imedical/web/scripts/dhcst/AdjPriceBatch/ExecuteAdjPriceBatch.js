...
Ext.onReady(function(){
	...
	function CheckIfHavenAdjDay()
	{
		...
		for (var i = 0; i < rowCount; i++) {				
			...
			if(sm.isSelected(i)){
					...
					var ret=tkMakeServerCall("web.DHCST.INAdjPriceBatch","CheckIfHavenAdjDay",detailId)
					if  (ret=="Y") 
					{
						Msg.info("warning",incidesc+$g(" 药品今日存在已经生效的记录，请确认是否继续生效!"));
						record.set("Status",$g("今日已有生效记录"));
						sureExeflag=1;
						record.dirty = false;  
		...
		Ext.Ajax.request({
			...
			success : function(response) {
				...
				if(jsonDataArr[0]==0){
					...
					Msg.info("warning", $g("错误：")+jsonDataArr[1]+$g("存在今日已生效的调价单,今日不允许再生效!"));
					return;
				}else{
	...
	// 指定列参数
	var fields = ["AspBatId", "AspBatNo","Incib","StkCatDesc", "InciId", "InciCode","InciDesc", "AspUomId",
