...
Ext.onReady(function(){
	...
	function CheckIfHavenAdjDay()
	{
		...
		if(sureExeflag==1) 
		{
			    Ext.Msg.confirm($g('提示'),$g('有药品存在今日已生效的调价批次，是否继续调价？')+"<br>"+"<font color='red'>"+$g("注意：多次生效会导致一个完整的业务流程周期中出现多个价格；如发药前后价格不一致。如医保上传价格后调价导致调后价格和医保上传不一致。请谨慎操作！")+"</font>",
							      function(btn){
								if(btn=='yes'){
								  ExecuteAsp();					
								}else{
