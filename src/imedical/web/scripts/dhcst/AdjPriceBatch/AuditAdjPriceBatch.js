...
Ext.onReady(function() {
	...
	Ext.BLANK_IMAGE_URL = Ext.BLANK_IMAGE_URL;
		...
		function CheckPreDate()
		{
			...
			//alert(DateFlag+"  "+StrAspId)
			//return;
			if(DateFlag==-1) 
			{
				    Ext.Msg.confirm($g('提示:计划生效日期校验','计划生效日期 <font color="yellow"> (黄色日期)</font>小于或者等于今日，审核后任务无法生效，是否修改计划日期为明天再审核？'),
					      function(btn){
						if(btn=='yes'){
						  var ret=tkMakeServerCall("web.DHCST.INAdjPriceBatch","UpDateAspPreDate",StrAspId)
						  if(ret==0) auditOrder();	
						  else {
									Msg.info("warning", $g("更新计划生效日期失败!"));
									return;
								}				
						}else{
						  return;
		...
		function auditOrder() {
			...
			for (var i = 0; i < rowCount; i++) {				
				...
				if(sm.isSelected(i)){
					var record = store.getAt(i);
					var detailId = record.get('AspBatId');	
					if (record.get('Status')==$g("已审核"))
					{
						Msg.info("warning",$g("当前为已审核状态!"));
						return
					}
					if(StrAspId==""){
						StrAspId=detailId;
			...
			Ext.Ajax.request({
				...
				success : function(response) {
					...
					if(jsonData.info==0){
						...
						Msg.info("warning", $g("错误：存在已审核或已生效的调价记录!"));
						return;
					}else if(jsonData.info==-2){
						Msg.info("warning", $g("错误：计划生效日期不能为空!"));
						return;
					}else if(jsonData.info==-3){
						Msg.info("warning", $g("错误：存在已经过期调价单!"));
						return;
					}else if(jsonData.info==-4){
						Msg.info("error", $g("错误：更新调价单状态失败!"));
						return;
					}else if(jsonData.info==-5){
						Msg.info("error", $g("错误：调价单生效失败!"));
						return;
					}else{
				...
				failure : function(response){
					Msg.info("error", $g("审核发生错误：")+response.responseText);					
					return;
				}
			});		
		}
		...
		function cancelAuditOrder(){
			...
			for (var i = 0; i < rowCount; i++) {				
				...
				if(sm.isSelected(i)){
					var record = store.getAt(i);
					var detailId = record.get('AspBatId');
					if (record.get('Status')==$g("未审核"))
					{
						Msg.info("warning",$g("当前为未审核状态!"));
						return
					}	
					if(StrAspId==""){
						StrAspId=detailId;
			...
			if(StrAspId==""){
				Msg.info("warning",$g("未勾选需要取消审核的记录!"));
				return;
			}		
			var mask=ShowLoadMask(Ext.getBody(),$g("处理中..."));
			//提交数据库执行审核
			Ext.Ajax.request({
				...
				success : function(response) {
					...
					if(jsonData.info==0){
						...
						Msg.info("warning", $g("错误：存在不是审核状态的调价记录!"));
						return;
					}else if(jsonData.info==-2){
						Msg.info("error",$g( "错误：更新调价单状态失败!"));
						return;
				...
				failure : function(response){
					Msg.info("error", $g("取消审核发生错误：")+response.responseText);					
					return;
				}
			});
		}
		...
		*/
		...
		DetailPageToolBar.addListener('beforechange',function(ptbar,params){
			...
			if(selrows>0){
				Msg.info("warning",$g("当前页选择了要审核的记录，请先审核后再翻页！"));
				return false;
			}
		})
