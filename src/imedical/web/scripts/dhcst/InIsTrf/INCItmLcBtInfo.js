...
INCItmLcBtInfo = function(InciDr, PhaLoc, PhaLocRQ, Fn) {
	...
	function returnData() {
		...
		if (rowsModified.length==0){
			...
			for (rowslen=0;rowslen<modifylen;rowslen++)
			{
				 ...
				 if (inclbwarnflag=="1"){
					 if (confirm(expbatinfo+$g("为过期药品,是否继续?"))){continue;}
					 else{return}
				 }
				 else if (inclbwarnflag=="2"){
					 if (confirm(expbatinfo+$g("为批次不可用,是否继续?"))){continue}
					 else{return}
				 }
			}
			flg = true;
	...
	ItmLcBtGrid.getView().on('refresh',function(Grid){
		 if (ItmLcBtGrid.getStore().getCount()>0)
		 {
			...
			var IncInfoMain=$g('药品代码:')+IncCode+$g('　　药品名称:')+IncDesc;
			var IncInfoOther=$g("规格:")+IncSpec+$g("　　入库单位:")+IncPuomDesc+$g("　　基本单位:")+IncBuomDesc;
	   	 	var IncInfoStr=IncInfoMain+'　　　'+IncInfoOther;
	   	 	
		 }
		 else
		 {
			var IncInfoStr=$g("该药品没有有效批次信息!")
		 }
