	   ...
	   var GridColSetBT = new Ext.Toolbar.Button({
        ...
        function save(){
	     ...
	     if((CompleteStr!="")&(gParam[0]=='Y')){
		  ...
		  return;
            ...
            Ext.Ajax.request({
                url:url,
                params:{actiontype:'Create',Params:params}, 
        ...
        function Query(inst){
            ...
            Ext.Ajax.request({
                ...
                params:{actiontype:'Select',Rowid:inst},
            ...
            StockQtyStore.load({params:{start:0,limit:size,sort:'sbDesc',dir:'ASC',Parref:inst}});
        ...
        function InstComplete(){
            ...
            Ext.Ajax.request({
                url:url,
                params:{actiontype:'Complete',Rowid:gRowid},
                method:'post',
                waitMsg:'处理中...',
                success:function(response,opt){
                    var jsonData=Ext.util.JSON.decode(response.responseText);
        ...
        function Delete(){
            ...
            Ext.Ajax.request({
                url:url,
                params:{actiontype:'Delete',Rowid:gRowid},
                method:'post',
                waitMsg:'处理中...',
                success:function(response,opt){
                    var jsonData=Ext.util.JSON.decode(response.responseText);
							...
							Ext.MessageBox.confirm('提示','盘点单为账盘完成状态,是否确定删除?',
								function(btn) {
									if(btn == 'yes'){
									 	Ext.Ajax.request({
							                url:url,
							                params:{actiontype:'Delete',Rowid:gRowid,Allow:1},
							                method:'post',
							                waitMsg:'处理中...',
							                success:function(response,opt){
							                    var jsonData=Ext.util.JSON.decode(response.responseText);
...
function LoadLocManGrp(){
            ...
            LocManGrpStore.load({params:{LocId:locId,UserId:UserId}});
        }
        ...
        var DspPhaUrl = DictUrl
                    + 'instktkaction.csp?actiontype=QueryDetail&start=&limit=&sort=sbDesc&dir=ASC';
        // 通过AJAX方式调用后台数据
        var proxy = new Ext.data.HttpProxy({
                    url : DspPhaUrl,
                    method : "POST"
