PayQuery = function(ds,inscrapNumField,finshCK,auditCK,mainRwoId,locField,dateField,reasonId,addB){
	...
	var InscrapPagingToolbar = new Ext.PagingToolbar({
		...
		doLoad:function(C){
			...
			B['sort']='NO';
			B['dir']='desc';
			B['strPar']=Ext.getCmp('startDate').getValue().format('Y-m-d')+"^"+Ext.getCmp('endDate').getValue().format('Y-m-d')+"^"+Ext.getCmp('Loc').getValue()+"^"+(Ext.getCmp('finsh').getValue()==true?'Y':'N')+"^"+(Ext.getCmp('audit').getValue()==true?'Y':'N');
