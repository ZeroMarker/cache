		...
		Ext.Ajax.request({
			...
			success : function(result, request) {
				...
				if (jsonData.success == 'false') {
					ret=jsonData.info;	
					if (ret=="-111"){
						Msg.info("warning","除本次业务外,已存在相同批次信息,不予许修改!");
					}else if (ret=="-61"){
						Msg.info("warning","已发生转移业务,不允许修改进价!");
					}else if (ret=="-62"){
						Msg.info("warning","已生成月报,不允许修改进价!");
					}else if (ret=="-63"){
						Msg.info("warning","已发生退货业务,不允许修改进价!");
					}else if (ret=="-64"){
						Msg.info("warning","已发生付款业务,不允许修改进价!");
					}else if (ret=="-52"){
						Msg.info("warning","已发生付款业务,不允许修改发票信息!");
					}else{
						Msg.info("error","更新某条明细失败!"+ret);
					}
									
				}
				else{
					Msg.info("success","明细更新成功!");
					Query(gIngrRowid)
				}
				mask.hide();
			},
		...
		success : function(result, request) {
			...
			if (jsonData.success == 'true') {
				...
				if(ret==-7){
					...
					Msg.info("error", "该入库单已经生成月报!");
				}else if(ret==-8){
					Msg.info("error", "该入库单有明细已退货制单!");
				}else if(ret==-9){
					Msg.info("error", "该入库单已经生成付款单!");
				}else if(ret==-11){
					Msg.info("error", "该入库单有药品处理库存失败，请核实是否已发药!");
				}else{	
...
var GrDetailInfoGrid = new Ext.grid.EditorGridPanel({
	...
	var InfoForm= new Ext.form.FormPanel({
			...
			labelAlign : 'right',
			id : "InfoForm",			
			tbar : [searchBT, '-', acceptBT, '-', cancelBT,'-',clearBT],
			items : [{
				xtype:'fieldset',
				title:'查询条件',
