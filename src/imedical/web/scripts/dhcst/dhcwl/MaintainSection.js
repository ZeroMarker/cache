dhcwl.mkpi.MaintainSection=function(){
	...
	var columnModel = new Ext.grid.ColumnModel([
        ...
        },{header:'区间编码',dataIndex:'SecCode',sortable:true, width: 30, sortable: true
        },{header:'区间名称',dataIndex:'SecName', width: 100, sortable: true
        },{header:'区间描述',dataIndex:'SecDesc', width: 160, sortable: true
        },{header:'创建者',dataIndex:'SecUser', width: 160, sortable: true
        },{header:'创建/更新日期',dataIndex:'SecUpdateDate', width: 80, sortable: true
        },{header:'备注',dataIndex:'SecRemark', width: 80, sortable: true
        },{header:'直接父区间',dataIndex:'SecDirectParent', width: 80, sortable: true
        }
    ]);
    ...
    var sectionForm = new Ext.FormPanel({
        ...
        tbar:new Ext.Toolbar([enforceCheck,'-',{
                    ...
                    if (btn == 'yes') {
                		...
                		,null,function(jsonData){
                			...
                			if(jsonData.tip!="ok"){
                				...
                				store.remove(record);
                				dhcwl.mkpi.Util.ajaxExc(serviceUrl+'?action=deleteKpiSection&SecCode='+SecCode+'&flag=1');
                				outThis.refresh();
                			}
                		},outThis);
                    }
            ...
            handler: function() {
            	var form=sectionForm.getForm();
    			form.setValues({ID:'',SecCode:'',SecName:'',SecDesc:'',SecUser:'',SecUpdateDate:'',SecRemark:''});
        	}
        },'-',{text   : '查找',
            handler: function() {
                var SecCode=Ext.get('SecCode').getValue();
    ...
    var sectionPanel =new Ext.Panel({
    	...
    	listeners:{
    		"resize":function(win,width,height){
    			sectionGrid.setHeight(height-150);
