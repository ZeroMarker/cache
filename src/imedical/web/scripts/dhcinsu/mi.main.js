...
function wdg_SaveMultRow(){
	...
	for (var rowIndex = 0; rowIndex < dgRows.length; rowIndex++) {
		...
		if(dgRow){
			...
			// 2022-12-12 + tangzf 多行数据不支持NONE模式
            if((editorRow.NODETYPE=="M")&&(editorRow.NODECODE.indexOf("%NONE%")>-1)){
                INSUMIAlert("节点代码："+editorRow.NODECODE + "，NONE模式不支持多行数据");  
                return;
            }
...
function init_edg() { 
	var colums = [[
		{align:'left',field:'SEQ',title:'顺序号',width:50,editor:{
				type: 'numberbox',
				options: {
					precision:0,
					min:0,
		...
		{align:'left',field:'ARGCODE',title:'参数代码',width:100,editor:{
				type: 'validatebox',
				options: {
					required:true	
				}
			}},
		{align:'left',field:'ARGNAME',title:'参数名称',width:100,editor:{
				type: 'validatebox',
				options: {
					required:true	
				}
			}},
		{align:'left',field:'ARGTYPE',title:'参数类型',width:100,editor:{
				type: 'combobox',
				options: {
					defaultFilter: 4,
					valueField: 'cCode',
		...
		{align:'left',field:'CONTYPE',title:'对照类型',width:100,editor:{
				type: 'combobox',
				options: {
					defaultFilter: 4,
					valueField: 'cCode',
		...
		{align:'left',field:'CONINFO',title:'数据源代码',width:94,editor:{
				type: 'combobox',
				options: {
					defaultFilter: 4,
					valueField: 'ARGCODE',
		...
		{align:'left',field:'CONINFODESC',title:'数据源名称',width:94,editor:{
				type: 'text'
			}},	
		{align:'left',field:'CONINFOSOURCE',title:'数据源来源',width:94,editor:{
				type: 'text'
			}},	
		{align:'left',field:'CONINFODEMO',title:'数据源描述',width:94,editor:{
				type: 'text'
			}},
		{align:'left',field:'DEFVALUE',title:'默认值',width:94,editor:{
				type: 'text'
			}},
		{align:'left',field:'LOCALPARCODE',title:'本地参数代码',width:124,editor:{
				type: 'text'
			}},
		{align:'left',field:'EFFTFLAG',title:'生效标志',width:94,editor:{
				type: 'combobox',
				options: {
					defaultFilter: 4,
					valueField: 'cCode',
		...
		{align:'left',field:'MUSTLFLAG',title:'必填标志',width:60,editor:{
				type: 'combobox',
				options: {
					defaultFilter: 4,
					valueField: 'cCode',
		...
		{align:'left',field:'CODEFLAG',title:'代码标识',width:80,editor:{
				type: 'combobox',
				options: {
					valueField: 'cCode',
					textField: 'cDesc',
		...
		{align:'left',field:'DICCODE',title:'代码字典',width:80,editor:{
			type: 'text'
		}
		},
		{align:'left',field:'MAXLENG',title:'最大长度',width:50,editor:{
				type: 'text'
			}},
		{align:'left',field:'SUBNODE',title:'子节点代码',width:48,editor:{
				type: 'text'
			}},
		{align:'left',field:'SUBNAME',title:'子节点名称',width:100,editor:{
				type: 'text'
			}},
	...
	for (var rowIndex = 0; rowIndex < dgRows.length; rowIndex++) {
		...
		if(dgRow){
			...
			if(editorRow['CONTYPE']=="P" &&editorRow['CONINFO']==""){
				ErrorNum++;
				$.messager.alert('提示','对照类型为[方法返回值]时数据源代码不能为空','error')
				break;	
			}
...
function init_insuType(){
	...
	$('#InsuType').combogrid({  
		...
		onSelect:function(index,data){
			...
			if(rtn!=0){
				$.messager.confirm('确认','检测到该医保类型存在新的接口，是否进行添加',function(r){
					if(r){
...
function init_hosp(){
	$('#Hospital').combogrid({  
		...
		textField: 'HOSPDesc',
	    columns:[[   
	        {field:'HOSPRowId',title:'数据ID',width:60},  
	        {field:'HOSPDesc',title:'描述',width:100}
	    ]],
	    fitColumns: true,
	    onSelect:function(index,data){
