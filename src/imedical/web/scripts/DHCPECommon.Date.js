...
/// 主要功能		自定义日期函数
/// 对应表		
/// 使用模块		DHCPEPosStandard.csp
/// 使用要求		日期元素图标得名称(id)="ld"+日期元素id
/// 最后修改时间	
/// 最后修改人	
/// 完成
...
var focusat=null;
var CurDate; //当前日期元素
function SetDate(DateName,ldDateName) {

	var obj=document.getElementById(DateName);
	if (obj && ""!=DateName) { obj.onkeydown=PEDate_lookuphandler;	}
	
	var obj=document.getElementById(ldDateName);
	if (obj && ""!=ldDateName) { obj.onclick=PEDate_lookuphandler; }
	
	var obj=document.getElementById(DateName);
	if (obj && ""!=DateName) { obj.onchange=PEDate_changehandler; }
	
}
function PEDate_lookuphandler(e) {
	...
	CurDate=obj;
	var url='websys.lookupdate.csp?ID='+"PEDate"+'&STARTVAL=';
...
function PEDate_lookupSelect(dateval) {
	CurDate.value=dateval;
	websys_nextfocusElement(CurDate);
}

function PEDate_changehandler(e) {
	var src=window.event.srcElement;
...
function IsValidDate(fld) {
 ...
 if(dtformat=="DMY"){dt=dt.replace(re,'/');}
 if(dtformat=="YMD"){dt=dt.replace(re,'-');}
 //dt=dt.replace(re,'/');

 for (var i=0;i<dt.length;i++) {
    var type=dt.substring(i,i+1).toUpperCase();
    if (type=='T'||type=='W'||type=='M'||type=='Y') {
       if (type=='T') {return ConvertTDate(fld);} 
