	...
	obj.btnDelete_click = function()
	{
		...
		if (selectObj){
			ExtTool.confirm('选择框','确定删除?',function(btn){
				if(btn=="no") return;
				var objMainService = ExtTool.StaticServerObject("DHCWMR.MainService.MenusSrv");
...
function InitwinScreenEvent(obj) {
	...
	obj.LoadEvent = function(){
		...
		if (data){
			var objMenus = ExtTool.StaticServerObject("DHCWMR.MainService.MenusSrv");
			...
			if(Str[8]>0) ExtTool.AddComboItem(obj.winfPParentMenuDr,Str[9],Str[8]);
	...
	obj.winfPSave_click = function()
	{
		...
		if((obj.winfPMenuCode.getValue()=="")||(obj.winfPMenuCaption.getValue()==""))
		{
			ExtTool.alert("提示","菜单代码、名称都不能为空!")
			return;
		}
		...
		try
		{
			var NewID = objMenus.UpdateMenus(tmp);
			if (NewID>0){
	...
	obj.winfPPPDelete_click = function()
	{
		...
		var ret = objMenus.DeleteMenuOperationById(cMOId);
		if(ret==0)
		{
			//alert("删除成功!!");
			obj.winfGPanelStore.removeAll();
	...
	obj.winfPPPUpdate_click = function()
	{
		...
		try
		{
			var NewID = objMenus.UpdateMenuOperation(tmp);
			//window.alert(NewID);
