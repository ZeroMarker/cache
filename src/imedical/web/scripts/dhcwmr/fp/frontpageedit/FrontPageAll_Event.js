function InitFrontPageEvent(obj){
    ...
    obj.LoadEvent = function(){
		...
		obj.pnCost.on("activate", obj.pnCost_activate, obj);
	...
	obj.SaveResult = function(isFinish){
		...
		var strPatInfo = obj.FPM_GetEditData(obj.FPM_Editor.Items);
		
		var flg = ExtTool.RunServerMethod("DHCWMR.FPService.FrontPageSrv","SaveResult",strFPInfo,strFPDInfo,strFPOInfo,strFPEInfo,strPathRepInfo,strPatInfo,LogonHospID);
		if (parseInt(flg) <= 0) {
			return false;
		} else {
	...
	obj.pnEpr_activate = function()
	{
		...
		var strTemplate = "./emr.record.browse.browseform.csp?EpisodeID={0}&PatientID={1}";     //update by jiangpengpeng 2015-07-13
		var strURL = String.format(strTemplate, obj.FrontPage.VolPaadm, obj.FrontPage.PatientID);
		document.getElementById("ifEpr").src = strURL;
	}
	
	obj.pnCost_activate = function()
	{
		Ext.ComponentMgr.all.each(function(cmp){
			var objTD = document.getElementById('TC-' + cmp.id);
			if (objTD) {
				cmp.setWidth(objTD.offsetWidth);
				cmp.render('TC-' + cmp.id);
			}
		});
	}
