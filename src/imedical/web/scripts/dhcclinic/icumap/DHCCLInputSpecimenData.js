$(document).ready(function () {	
	...
	$("#specimendatagrid").datagrid({
		height:480,
		url:'dhcclinic.jquery.csp', 
        queryParams:{
            ClassName:"web.DHCCLDataCom",
            QueryName:"FindLabSpecimenData",
            Arg1:"",
            Arg2:"",
            ArgCnt:2
        },
        ...
        singleSelect:true,
        columns:[[
            {field:'CLCLSTRowId',title:'RowId',width:60}, 
            {field:'CLCLSTCode',title:'代码',width:150},    
            {field:'CLCLSTDesc',title:'描述',width:150}
        ]],
        pagination:true,
        pageSize:15,
        pageNumber:1,
	...
	$("#BtnFind").click(function(){	
    	var queryParams = $("#specimendatagrid").datagrid('options').queryParams;
        queryParams.Arg1 = $("#Code").textbox('getValue');
        queryParams.Arg2 = $("#Desc").textbox('getValue');
        $("#specimendatagrid").datagrid('reload');
     })
     $("#BtnClear").click(function(){
	     $.messager.confirm('请确认', '您确定要清空检验标本类型表相关数据？', function (b) { 
         if (b==false)
         {
             return;
         }
     ...
     for(var i=2;i<(rows+1);i++)
     {
		 ...
		 $.ajax({
            ...
            if (data==0) {
	            ...
	            failStr=failStr+"第"+i+"行代码为"+oSheet.Cells(i,1).value+",描述为"+oSheet.Cells(i,2).value+"的检验标本类型数据插入失败，原因："+data+'\n';
                failnum=failnum+1;
            }
           }
