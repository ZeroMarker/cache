$(document).ready(function () {	
	...
	$("#ctprvdatagrid").datagrid({
        ...
        singleSelect:true,
        columns:[[                
            {field:'CTCPTRowId',title:'RowId',width:60}, 
            {field:'CTCPTCode',title:'代码',width:150},    
            {field:'CTCPTDesc',title:'描述',width:150},
            {field:'CTCPTInternalType',title:'类型',width:150},
            {field:'CTCPTDateFrom',title:'生效日期',width:150}, 
            {field:'CTCPTDateTo',title:'截止日期',width:150} 
        ]],
        pagination:true,
        pageSize:15,
        pageNumber:1,
	...
	$("#BtnFind").click(function(){		
    	var queryParams = $("#ctprvdatagrid").datagrid('options').queryParams;
        queryParams.Arg1 = $("#Code").textbox('getValue');
        queryParams.Arg2 = $("#Desc").textbox('getValue');
        $("#ctprvdatagrid").datagrid('reload');
     })
     ...
     for(var i=2;i<(rows+1);i++)
     {
		 ...
		 $.ajax({
            ...
            if (data==0) {
	            ...
	            failStr=failStr+"第"+i+"行代码为"+oSheet.Cells(i,1).value+",描述为"+oSheet.Cells(i,2).value+"的医护人员类型数据插入失败，原因："+data+'\n';
                failnum=failnum+1;
            }
           }
