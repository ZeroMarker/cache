$(document).ready(function () {	
	...
	$("#ctuomdatagrid").datagrid({
        ...
        singleSelect:true,
        columns:[[                
            {field:'CTUOMRowId',title:'RowId',width:60},
            {field:'CTUOMCode',title:'代码',width:150},    
            {field:'CTUOMDesc',title:'描述',width:150},
            {field:'CTUOMForeignDesc',title:'国外描述',width:150}  
        ]],
        pagination:true,
        pageSize:15,
        pageNumber:1,
	...
	$("#BtnFind").click(function(){
    	var queryParams = $("#ctuomdatagrid").datagrid('options').queryParams;
        queryParams.Arg1 = $("#Code").textbox('getValue');
        queryParams.Arg2 = $("#Desc").textbox('getValue');
        $("#ctuomdatagrid").datagrid('reload');
     })
     ...
     for(var i=2;i<(rows+1);i++)
     {
		 ...
		 $.ajax({
            ...
            if (data==0) {
	            ...
	            failStr=failStr+"第"+i+"行代码为"+oSheet.Cells(i,1).value+",描述为"+oSheet.Cells(i,2).value+"的基本单位表数据插入失败，原因："+data+'\n';
                failnum=failnum+1;
            }
           }
