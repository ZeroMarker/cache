...
Ext.ux.form.LovCombo = Ext.extend(Ext.form.ComboBox, {
    ...
    ,initComponent:function() {
		...
		this.on({
			 scope:this
			,beforequery:this.onBeforeQuery   //可查看源代码，主要作用是把输入的值处理过后传给doQuery,重写以后作用不大
	...
	// {{{
	  /*
	   true时 在多字段中查询(doQuery).false则在displayField中查询, 数据是本地时才有效  
    */    
    ,queryInFields : false    
    /**  
     多列查询的字段名,在queryInFields为true下，默认在全部列中查询,数据是本地时才有效 
    */    
  ,queryFields : []
	...
	,onSelect:function(record, index) {
        if(this.fireEvent('beforeselect', this, record, index) !== false){
			...
			if(this.store.isFiltered()) {
				this.doQuery(this.allQuery); //?????屏蔽掉后变化不大
