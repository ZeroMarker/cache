...
function Query(){
	...
	selTarData=""
	if($('#insuType').combobox('getValue')==""){trigthgrid.datagrid("loadData",{"total":"0",rows:[]});return;}
...
function QueryOEORD(bsyval){
	...
	$('#trightdg').datagrid('options').url=tmpARGUS;
	$('#trightdg').datagrid('options').queryParams.qid="1"
	$('#trightdg').datagrid('reload');
	$('#trightdg').datagrid('getPanel').focus();
}
//查询对照数据
...
function Save(){
	...
	//if(confirm('  吗?')){
		...
		var rtn=tkMakeServerCall("web.INSUINFOTemplate","SaveConInfo",UpdateStr,selHosDr);
		if(rtn.split("^")[0] > 0){
			//$.messager.alert('提示','保存成功!'); 
			editrow['Rowid']=rtn.split("^")[0];
			Query()
	...
	$.messager.confirm('提示','你确定要把 '+selTarData.tplDesc+' 对照成 '+seloeData.OEDesc+' 吗?',function(r){
			if(r){
				...
				if(rtn.split("^")[0] > 0){
					...
					$.messager.alert('提示','对照失败!'+tn.split("^")[1],'info');   
				}
				seloeData=""
				QueryCon(selTarData)
			}else{
	...
	$.messager.confirm('提示','你确定要修改成:'+OEORDCode+' 吗?',function(r){
		if(r){
			...
			if(rtn.split("^")[0]>0){
				...
				$.messager.alert('提示','对照失败!'+rtn.split("^")[1],'info');   
			}
			seloeData=""
			QueryCon(selTarData)
		}else{
...
function DelCon(){
	...
	*/
	if(!selcondata){selcondata =oecongrid.datagrid('getSelected');} //+DingSH 20201105
...
function init_oecdg(){
	oecongrid=$('#oecdg').datagrid({
		...
		columns:[[
			...
			{field:'OEORDCode',title:'对照表达式',width:300,editor:{
				type:'text'	
...
function Exportxls(){
	...
	ExportTitle="序号(非空)^RowID(新增时为空)^模版字段代码^模版字段名称^对照代码(Base64加密)^对照名称^模版字段类型^医保类型^对照标志^父节点Rowid^父节点序号^状态^业务类型^默认设定^预留2^预留3^预留4^预留5^HospitalDr^^^^"
