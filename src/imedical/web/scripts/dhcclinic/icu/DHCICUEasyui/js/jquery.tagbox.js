 ...
 */
/**
 * Modified by yongyang 20180912
 * 
 * reset styles while it's rendered
 */
(function($) {
    function _1(_2) {
    ...
    function resetStyle(_2) {
        var combo = $(_2).data('combo').combo;
        var combo_arrow = $(combo).find('.combo-arrow');
        var combo_text = $(combo).find('input.combo-text');

        $(combo).css({ height: 'auto', 'min-height': 28, position: 'relative' });
        $(combo_arrow).css({ position: 'absolute', right: 0 });
        $(combo_text).css({ color: '#fff' });
    };
