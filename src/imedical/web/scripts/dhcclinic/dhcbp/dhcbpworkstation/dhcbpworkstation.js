...
function initMenuGroup() {
        ...
        if (menuItemsGroups && menuItemsGroups.length > 0) {
            ...
            for (var i = 0; i < menuItemsGroups.length; i++) {
                ...
                for (var j = 0; j < menuItemsGroup.data.length; j++) {
                    ...
                    htmlArr.push("<li><a class='navi-tab' id='" + menuItem.RowId + "' href='#' data-title='" + menuItem.DisplayName + "' data-url='" + menuItem.Url + "' data-modulecode='" + menuItem.MenuItemCode + "' data-exp='" + menuItem.Exp + "'>" + menuItem.DisplayName + "</a></li>");
                }
                htmlArr.push("</ul></div>");
            }
    ...
    var bpprId = dhccl.getQueryString("bpprId");
	var userId = dhccl.getQueryString("userId");
    var bpaId = dhccl.getQueryString("bpaId");
    $("ul.i-menugroup>li>a").each(function(index, item) {
        ...
        var dataUrl = $(item).attr("data-url") + "?bpprId=" + bpprId + "&bpaId=" + bpaId + "&userId=" + userId + "&moduleId=" + moduleId+ "&moduleCode=" + moduleCode;
    ...
    $(".open-all").click(function() {
        ...
        $(selector).each(function(index, item) {
            if ($(item).hasClass("open-all") || $(item).hasClass("close-all")) return;
            var dataUrl = $(item).attr("data-url") + "?bpprId=" + bpprId + "&bpaId=" + bpaId;
            var dataTitle = $(item).attr("data-title");
            var menuIndex = parseInt($(item).attr("data-menuindex"));
            if (dataUrl && dataUrl !== "" && dataTitle && dataTitle !== "") {
