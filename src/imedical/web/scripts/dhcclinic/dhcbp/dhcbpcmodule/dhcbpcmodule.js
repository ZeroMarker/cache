...
function initQueryOptions(){
    $("#filterParent,#ParentModule").combobox({
        ...
        onBeforeLoad: function(param) {
            param.ClassName = "web.DHCBPCModule";
    ...
    $("#btnAdd").linkbutton({
        onClick: function() {
            if ($("#dataForm").form("validate")){
                ...
                if(formData){
                    formData.RowId="";
                    formData.ClassName="User.DHCBPCModule";
	...
	$("#btnPrintTemplate").linkbutton({
        onClick: function() {
            if(dhccl.hasRowSelected($("#dataBox"),true)){
                ...
                var url = "dhcbp.printtemplate.csp?moduleId=" + rowData.RowId + "&moduleCode=" + rowData.Code;
                window.open(url, "_blank",'top=0,left=10,width=1400,height=700')
            }
        }
    });
}
function DeleteRecord(dataForm,index) {
    ...
    if (result) {
             var msg = dhccl.removeData("User.DHCBPCModule", index);
