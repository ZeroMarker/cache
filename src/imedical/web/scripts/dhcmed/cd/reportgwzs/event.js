function InitViewPortEvent(obj) {
	...
	obj.ReportGWZSCls=ExtTool.StaticServerObject("DHCMed.CD.CRReportGWZS");
	...
	obj.DisplayRepInfo = function(){
		if(obj.ReportID==""){
			...
			var objGWZS=obj.ReportGWZSCls.GetStringByParRef(obj.ReportID);
			...
			Common_SetValue("txtCRKPBH",arrGWZS[0]);
			...
			Common_SetValue("cbgZSXZ",arrGWZS[1].split(CHR_1)[0],arrGWZS[1].split(CHR_1)[1]);
			Common_SetValue("txtZSDD",arrGWZS[2]);
			Common_SetValue("txtZSLCBX",arrGWZS[3]);
			Common_SetValue("cbgZSZD",arrGWZS[4].split(CHR_1)[0],arrGWZS[4].split(CHR_1)[1]);
			Common_SetValue("txtZLGY",arrGWZS[5]);
			Common_SetValue("cbgZSZG",arrGWZS[6].split(CHR_1)[0],arrGWZS[6].split(CHR_1)[1]);
			
			Common_SetValue("txtZDQR",arrGWZS[7]);
			Common_SetValue("txtSWRQ",arrGWZS[8]);
	...
	obj.GetGWZSData = function () {
		...
		InputStr=InputStr+"^"+Common_GetValue("cbgZSXZ");
		InputStr=InputStr+"^"+Common_GetValue("txtZSDD");
		InputStr=InputStr+"^"+Common_GetValue("txtZSLCBX");
		InputStr=InputStr+"^"+Common_GetValue("cbgZSZD");
		InputStr=InputStr+"^"+Common_GetValue("txtZLGY");
		InputStr=InputStr+"^"+Common_GetValue("cbgZSZG");
	...
	obj.btnSave_click = function(){
		...
		var GWZSData=obj.GetGWZSData();
		var PatData=obj.GetPatData();
		var ret=obj.RepUpdateCls.SaveRepData(RepData,GWZSData,PatData);
		if(parseInt(ret)<=0){
	...
	obj.btnExport_click = function(){
		...
		if(parseInt(ret)<=0){
			...
			var flg=ExportDataToExcel("","","高温中暑报告卡("+Common_GetValue("txtPatName")+")",cArguments);
		}
	};
	...
	obj.CheckReport = function(){
		...
		errStr += obj.ValidateControl(obj.cbgZSXZ);	//中暑性质
		errStr += obj.ValidateControl(obj.txtZSDD);	//中暑地点
		errStr += obj.ValidateControl(obj.txtZSLCBX);	//临床表现
		errStr += obj.ValidateControl(obj.cbgZSZD);	//中暑诊断
		errStr += obj.ValidateControl(obj.txtZLGY);	//治疗概要
		errStr += obj.ValidateControl(obj.cbgZSZG);	//转归
		...
		if ((Common_GetText("cbgZSZG")!='死亡')&&(obj.txtSWRQ.getRawValue()!="")){
			errStr += '填写了死亡日期，请确认病人转归!<br>';
		}
		if (Common_GetText("cbgZSZG")=='死亡') {
			errStr += obj.ValidateControl(obj.txtSWRQ);	//死亡日期
