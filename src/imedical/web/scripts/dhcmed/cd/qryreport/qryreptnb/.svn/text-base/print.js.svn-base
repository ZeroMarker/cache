...
{
	var objCaseSrv = ExtTool.StaticServerObject("DHCMed.CDService.Interface");
	var ServerInfo=objCaseSrv.GetServerInfo();
	var TemplatePath=GetWebConfig(ServerInfo).Path;
	var FileName=TemplatePath+"\\\\"+"DHCMed.cd.ExportRepInfo.xls";
	try {
	...
	var flg=objCaseSrv.ExportTNBRepInfo("fillxlSheet",strArguments);
	
	xlSheet=xlBook.Worksheets.Item(4);
	var flg=objCaseSrv.ExportTNBPatInfo("fillxlSheet",strArguments);
...
{
	var objCaseSrv = ExtTool.StaticServerObject("DHCMed.CDService.PrintService");
	var ServerInfo=objCaseSrv.GetServerInfo();
	var TemplatePath=GetWebConfig(ServerInfo).Path;
	var FileName=TemplatePath+"\\\\"+"DHCmed.cd.CRReportZLK.xls";
	try {	
		xls = new ActiveXObject ("Excel.Application");
	}catch(e) {
		alert("创建Excel应用对象失败!");
