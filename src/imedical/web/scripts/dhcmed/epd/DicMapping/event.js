...
function InitviewScreenEvent(obj) {
	...
	obj.cboCategory_onSelect = function()
	{
		...
		if (SelectIndex < 0) {
			...
			obj.gridResultStore.removeAll();
			/*
			var flg = ExtTool.RunServerMethod("DHCMed.EPD.IODictionary", "GetMapSrcData", CategoryID);
			if (flg*1<0){
	...
	obj.btnDelete_onClick = function()
	{
		if (obj.RecRowID)
		{
			Ext.MessageBox.confirm('提示', '是否删除当前记录？', function(btn,text){
				if (btn == 'yes') {
					var flg = ExtTool.RunServerMethod("DHCMed.EPD.IODictionary", "DeleteById", obj.RecRowID);
					if (parseInt(flg) <= 0) {
