...
function ExportDataToExcel(strMethodGetServer,strMethodGetData,strTemplateName,strArguments)
{
		...
		var FileName=TemplatePath+"\\\\"+"DHCMed.NINF.InfReport.xls";
		try {
			xls = new ActiveXObject ("Excel.Application");
		}catch(e) {
			alert("创建Excel应用对象失败!");
		...
		xlBook=xls.Workbooks.Add(FileName);
		xlSheet=xlBook.Worksheets.Item(1);
		var obj = ExtTool.StaticServerObject("DHCMed.NINFService.Aim.InfReportSrv");
		var flg=obj.ExportInfReports("fillxlSheet",strArguments);
		//var flg=cspRunServerMethod(strMethod,"fillxlSheet",strArguments);
