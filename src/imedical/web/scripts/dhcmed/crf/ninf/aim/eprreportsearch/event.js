...
{	
	...
	{
		//不是超级权限或不是调查当天，隐藏初始化数据按钮
	 if((tDHCMedMenuOper['Admin']!=1)||(CurDate!=HDMResDate))
	 {
	 	obj.BtnInitData.setVisible(false);
	 }
	...
	obj.BtnInitData_click = function()
	{
	 	var objHDM=ExtTool.StaticServerObject("DHCMed.NINF.Aim.PatientAdmToHDM");
		var ret=objHDM.InsertPatAdmListToHDM();
		if(ret>0)
		{
			alert("初始化成功，共初始化 " + ret + " 条数据，每次调查只需要初始化一次数据!!");
			return;
		}
		else{
			alert("初始化数据失败，请联系信息科!");	
			return;
		}
	};
	...
	obj.EPRReportList_rowdblclick = function()
	{
	  ...
	  var ReportStatus = rc.get("ReportStatus");
		...
		var url=WebServerAppURL+"/csp/dhcmed.ninf.aim.centertablistdetail.csp?PatientID="+"&EpisodeID="+EpisodeID+"&PrintDocID="+PrintDocID+"&TemplateDocID="+TemplateDocID+"&ChartItemID="+ChartItemID+"&InstanceDataID="+InstanceID+"&EPRNum="+EPRNum+"&RepStatus="+ReportStatus+"&ReportID="+ReportID;
