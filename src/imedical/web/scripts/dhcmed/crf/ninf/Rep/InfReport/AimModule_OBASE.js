...
function InitOBASE(obj)
{
	...
	obj.OBASE_cboTransLoc = Common_ComboToTransLoc("OBASE_cboTransLoc","调查科室",obj.CurrReport.EpisodeID,"E");
		...
		if (objPaadm) {
			...
			if (objPatient) {
				...
				if (parseInt(Age) > 0) {
					...
					Common_SetValue('OBASE_txtPatAge',parseInt(Month) + '月' + parseInt(Day) + '天');
				}
