...
function InitICUStrAdmSubWindow(SurveryDate,LocID){
	...
	obj.ICUStrAdmSubGridPanel = new Ext.grid.GridPanel({
		...
		,columns: [
			...
			,{header: '属性', width: 150, dataIndex: 'PatType', sortable: true, menuDisabled:true, hidden:true, align: 'center'}
		...
		,view: new Ext.grid.GroupingView({
			forceFit:true,
			groupTextTpl: '{[values.rs[0].get("PatType")]}：{[values.rs.length]}条记录'
			,groupByText:'依本列分组'
		})
	});
	...
	obj.ICUStrAdmSubGridPanelStoreProxy.on('beforeload', function(objProxy, param){
			param.ClassName = 'DHCMed.NINFService.Rep.AimICUSurveryAdm';
			param.QueryName = 'QryICUAdmStr';
