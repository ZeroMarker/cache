...
function InitWinControl()
{
	...
	obj.RowTemplate = new Ext.XTemplate(
		'<div>',
			'<table border=0 cellpadding=0 cellspacing=0 style="border-collapse:collapse;color:#457294;text-align:center;background-color:#afd3e8;width:100%;">',
				'<tr style="font-size:18px;height:30px;">',
					...
					'<td align="center" style="border:1 solid #FFFFFF;" width="8%">项目</td>',
					'<td align="center" style="border:1 solid #FFFFFF;" width="5%">级别</td>',
					'<td align="center" style="border:1 solid #FFFFFF;" width="30%">摘要</td>',
					'<td align="center" style="border:1 solid #FFFFFF;" width="8%">送检日期</td>',
					'<td align="center" style="border:1 solid #FFFFFF;" width="6%">送检医生</td>',
					'<td align="center" style="border:1 solid #FFFFFF;" width="15%">日期</td>',
					'<td align="center" style="border:1 solid #FFFFFF;" width="5%">操作员</td>',
					'<td align="center" style="border:1 solid #FFFFFF;" width="8%">科室</td>',
					'<td align="center" style="border:1 solid #FFFFFF;" width="8%">病区</td>',
				'</tr>',
				'<tbody>',
					'<tpl for=".">',
						'<tr  class="{[ (xindex % 2 === 1 ? \"RowEven\" : \"RowOdd\")]}" style="border-bottom:1px #BDBDBD solid;">',
							...
							'<td align="left" style="border:1 solid #FFFFFF;">{[ this.LnkMRBReport(values) ]}{Summary}</td>',
							'<td align="center" style="border:1 solid #FFFFFF;">{SubmisDate}</td>',
							'<td align="center" style="border:1 solid #FFFFFF;">{SubmisDoc}</td>',
		...
		'</div>',
		{
			LnkMRBReport : function(values){
				var str = '';
				var EpisodeID = values.EpisodeID;
				var ResultID = values.ResultID;
				var MRBRepFlag = values.MRBRepFlag;
				var MRBRepID = values.MRBRepID;
				if ((MRBRepFlag == 'MRB')&&(MRBRepID == '')) {
					var str = "<A id='lnkMRBReport' HREF='#' onClick='objScreen.MRBReportHeader(\""+ EpisodeID +"\",\"" + ResultID + "\")'><font color='red'>【标记】</font>" + "</A>";
				} else if ((MRBRepFlag == 'MRB')&&(MRBRepID != '')) {
					var str = "<A id='lnkMRBReport' HREF='#' onClick='objScreen.MRBReportHeader(\""+ EpisodeID +"\",\"" + ResultID + "\")'><font color='red'>【查看】</font></A>";
				} else {
					var MRBRepFlag = '';
				}
				return str;
