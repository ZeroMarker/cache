...
FormExport.LoadDataToArray = function(Class,objStore,callback,scope)
{
	...
	var objRec = null;
	for(var i = 0; i < objStore.length;i++) //getCount(); i ++)
	{
		objRec =objStore[i]; //objStore.getAt(i);
		Ext.Ajax.request({
			url: ExtToolSetting.RunQueryPageURL,
			params : {
				ClassName : Class,
				QueryName : 'QryByID',
				Arg1 : objRec.DataId, //objRec.get("DataId"),
				ArgCnt : 1,
				DataIndex : i
			},
			success: function(response, opts) {
			  ...
			  Ext.MessageBox.updateProgress(
				FormExport.tmpArry.length / objStore.length,   //getCount(),
				FormExport.tmpArry.length + "/" + objStore.length, //getCount(),
				"正在读取数据，请稍后..."
			  );
			  if(FormExport.tmpArry.length  == objStore.length) //getCount())
			  {
				Ext.MessageBox.hide();
				if(callback)
					callback.call(scope);
