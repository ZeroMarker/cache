...
function InitCasesXCtlEvent(obj){
	obj.LoadEvent = function(args)
	{
		...
		obj.txtDateTo.setValue(new Date().add(Date.DAY, -1));
	...
	{
		var DateFrom = Common_GetValue(aDateFrom);
		var DateTo = Common_GetValue(aDateTo);
		if ((DateFrom == '')||(DateTo == '')) {
		...
		var Days = Common_ComputeDays(aDateFrom,aDateTo);
		if (Days < 0) {
	...
	obj.btnQryScreenCases_onClick = function()
	{
		...
		var flg = obj.CheckDays('txtDateFrom','txtDateTo',3);
		if (flg != true) return;
	...
	obj.btnQryHandleCases_onClick = function()
	{
		...
		var flg = obj.CheckDays('txtDateFromX','txtDateToX',10);
		if (flg != true) return;
