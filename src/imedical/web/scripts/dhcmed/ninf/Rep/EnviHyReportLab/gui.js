...
function InitViewport1(){
	...
	obj.txtDateFrom = new Ext.form.DateField({
		...
		,width : 10
		,anchor : '100%'
		,value : new Date(curDate.getFullYear(),curDate.getMonth(),1)
	});
	...
	obj.gridEnviHyReport = new Ext.grid.GridPanel({
		...
		,loadMask : true
		,tbar : [
			{text:'申请',style:'background:red;margin-right:5px;padding:0 5px;',xtype:'label'},{text:'材料已发放',style:'background:#FFFF00;margin-right:5px;padding:0 5px;',xtype:'label'},{text:'标本已接收',style:'background:#00FFFF;margin-right:5px;padding:0 5px;',xtype:'label'},{text:'结果已录入',style:'background:#00FF00;margin-right:5px;padding:0 5px;',xtype:'label'},{text:'审核',style:'background:#FF00FF;margin-right:5px;padding:0 5px;',xtype:'label'}
		]
		,columns: [
			...
			,{header: '状态', width: 80, dataIndex: 'RepStatusDesc', sortable: false, menuDisabled:true, align: 'center',
				renderer : function(v, m, rd, r, c, s) {
					...
					if (str == "申请"){
						m.attr = 'style="background:red"';
					}else if(str=="材料发放"){
						m.attr = 'style="background:#FFFF00"';
					}else if(str=='标本接收'){
						m.attr = 'style="background:#00FFFF"';
					}else if(str=='录入结果'){
						m.attr = 'style="background:#00FF00"';
					}else if(str=='审核'){
						m.attr = 'style="background:#FF00FF"';
					}
					return str;
				}
			}
