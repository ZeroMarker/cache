...
function InitViewportEvent(obj) {
	...
	//Add By LiYang 2013-04-28 导出民科接口按钮事件
	obj.btnExportInterface_click = function(objBtn, objEvent, skipMapping)
	{
		...
		ExtTool.RunQuery(
			{
				ClassName : 'DHCMed.NINFService.Srv.ExportMinkeICUSrv',
			...
			function(arryResult, skipMapping){
				if ((arryResult.length > 0) && (!skipMapping)) {
					var objFrm = new InitwinProblem(strRowIDList, "^", obj, true);
						...
						function(e, text){
							if (e == "ok") {
								...
								for(var indRec = 0; indRec < obj.gridInfReportStore.getCount(); indRec++)
								{
									...
									objExportMinke.ExportMinkeDataICU(objRec.get("ReportID"), strPath);
								}
