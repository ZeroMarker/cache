...
function InitViewport()
{
    ...
    obj.cboRepLoc = Common_ComboToLoc("cboRepLoc","报告科室","E","","","cboSSHosp");	//add by yanjf 20140417
	obj.cboDateType = Common_ComboToDateType1("cboDateType","日期类型","报告日期","NCOM");
	...
	obj.txtRegNo = new Ext.form.TextField({
		...
		,labelSeparator :''
		,anchor : '100%'
		,listeners : {  //fix bug 118200 新生儿感染报告查询登记号回车后不能自动补零
	...
	obj.gridInfReport = new Ext.grid.GridPanel({
		...
		,columnLines : true
		,loadMask : true
		,tbar : [{id:'msgInfRepGrid',text: RepTypeDesc + '查询列表',style:'font-weight:bold;font-size:14px;',xtype:'label'},
		...
		//	,{header : '报告类型', width : 90, dataIndex : 'ReportTypeDesc', sortable: false, menuDisabled:true, align:'center' }
			...
			//update by pylian 2015-09-21 新生儿感染报告感染诊断对应的是InfDiagCatDesc
			,{header : '感染诊断', width : 60, dataIndex : 'InfDiagCatDesc', sortable: false, menuDisabled:true, align:'center' , //modified By LiYang 2013-05-18 增加链接到【感染截止日期的录入界面】
			...
			,{
				...
				renderer : function(v, m, rd, r, c, s){
					if (v=='Y') {
						...
						//return '';
						return " <a href='#' onclick='HistoryReportLookUpHeader(\"" + EpisodeID + "\",\"" + RepTypeCode + "\");'>&nbsp;查看&nbsp; </a>";
					} else {
						return '';
					}
				}
