...
{
	obj.OOPR_CurrOprRec = "";
	obj.OOPR_cboOperation = Common_ComboToORCOperation("OOPR_cboOperation","手术名称");
	obj.OOPR_txtStartDateTime = Common_DateFieldToDateTime("OOPR_txtStartDateTime","开始时间");
	obj.OOPR_txtEndDateTime = Common_DateFieldToDateTime("OOPR_txtEndDateTime","结束时间");
	obj.OOPR_cboOperDoc = Common_ComboToSSUser("OOPR_cboOperDoc","手术医生");
	obj.OOPR_cboOperationType = Common_ComboToDic("OOPR_cboOperationType","手术类型","NINFInfOperationType");
	...
	obj.OOPR_cboCuteType = Common_ComboToDic("OOPR_cboCuteType","切口等级","NINFInfCuteType");
	...
	obj.OOPR_cboCuteHealing = Common_ComboToDic("OOPR_cboCuteHealing","愈合情况","NINFInfCuteHealing");
	obj.OOPR_cboAnesthesia = Common_ComboToDic("OOPR_cboAnesthesia","麻醉方式","NINFInfAnesthesia");
	...
	obj.OOPR_SaveData = function(){
		...
		if (!objOpr.OperationDesc) {
			errinfo = errinfo + '手术名称&nbsp;'
		}
		if (!objOpr.OperStartDate) {
			errinfo = errinfo + '开始时间&nbsp;'
		}
		if (!objOpr.OperEndDate) {
			errinfo = errinfo + '结束时间&nbsp;'
		}
		if ((objOpr.OperStartDate!="")&&(objOpr.OperEndDate!="")&&(objOpr.OperEndDate<objOpr.OperStartDate)) {
			errinfo = errinfo + '开始时间不能大于结束时间;'
		}
		
		if (!objOpr.OperDoc) {
			errinfo = errinfo + '手术医生&nbsp;'
		}
		if (!objOpr.OperationType) {
			errinfo = errinfo + '手术类型&nbsp;'
		}
		if (!objOpr.Anesthesia) {
			errinfo = errinfo + '麻醉方式&nbsp;'
		}
		if (!objOpr.CuteType) {
			errinfo = errinfo + '切口类型&nbsp;'
		}
		if (!objOpr.CuteHealing) {
			errinfo = errinfo + '愈合情况&nbsp;'
		}
		if (objOpr.CuteInfFlag) {
			if ((objOpr.CuteInfFlag.Code == 'Y')&&(!objOpr.OperInfType)) {
				errinfo = errinfo + '感染类型&nbsp;'
			}
		}
