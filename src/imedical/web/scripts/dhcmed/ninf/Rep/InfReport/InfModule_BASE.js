...
function InitBASE(obj)
{
	...
	obj.BASE_txtEncryptLevel = Common_TextField("BASE_txtEncryptLevel","病人密级");
	obj.BASE_txtPatLevel = Common_TextField("BASE_txtPatLevel","病人级别");
	...
	Common_SetDisabled("BASE_txtEncryptLevel",true);
	Common_SetDisabled("BASE_txtPatLevel",true);
	//疾病转归
	obj.BASE_cbgDiseasePrognosis = Common_RadioGroupToDic("BASE_cbgDiseasePrognosis","<span><b><font color='red'>*</font>疾病转归</b></span>","NINFInfDiseasePrognosis",5);
	//与死亡关系
	...
	//ICU
	obj.BASE_cbgIsICU = Common_RadioGroupToDic("BASE_cbgIsICU","<span><b><font color='red'>*</font>入住ICU</b></span>","NINFInfICUBoolean",2);
	//ICU科别
	obj.BASE_cbgICULocation = Common_RadioGroupToDic("BASE_cbgICULocation","ICU科别","NINFInfICULocation",2);
	//感染类型(医院感染/社区感染)
	obj.BASE_cbgInfectionType = Common_RadioGroupToDic("BASE_cbgInfectionType","<span><b><font color='red'>*</font>感染类型</b></span>","NINFInfectionType",2);
	...
	obj.BASE_gridDiag_btnGet = new Ext.Button({
		...
		,iconCls : 'icon-update x-btn-over'
		,width: 80
		,text : "提取数据"
		,listeners : {
			'click' :  function(){
								...
								items : [
									{
										...
										labelAlign : 'right',
										labelWidth : 80,
										items : [obj.BASE_txtEncryptLevel]
									},{
										columnWidth:.23,
										layout : 'form',
										labelAlign : 'right',
										labelWidth : 80,
										items : [obj.BASE_txtPatLevel]
									}
								]
							},{
								layout : 'column',
						...
						items : [
							{
								...
								buttons : [obj.BASE_gridDiag_btnGet,obj.BASE_gridDiag_btnAdd,obj.BASE_gridDiag_btnDel,'->','…']
							},{
								region: 'west',
								layout : 'fit',
								width : 68,
		...
		if (objPaadm) {
			...
			if (objPatient) {
				...
				if (parseInt(Age) > 0) {
					...
					Common_SetValue('BASE_txtPatAge',parseInt(Month) + '月'); //fix bug 8000 by pylian 2015-03-24 患者年龄显示为如0月45天
				}else{
					Common_SetValue('BASE_txtPatAge', parseInt(Day) + '天');
				}*/
	            ...
	            Common_SetValue('BASE_txtPatAge',Age)
			...
			Common_SetValue('BASE_txtEncryptLevel',SecretStr.split('^')[0]);
			Common_SetValue('BASE_txtPatLevel',SecretStr.split('^')[1]);
