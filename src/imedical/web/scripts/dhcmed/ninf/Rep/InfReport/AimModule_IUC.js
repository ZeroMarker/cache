...
function InitIUC(obj)
{
	...
	obj.IUC_GridRowDataCheck = function(objRec){
		...
		if (IUC_txtIntubateDateTime != '') {
			...
			var IntubateTime = IUC_txtIntubateDateTime.split(" ")[1];
			...
			var InfDate = IUC_txtInfDate;
			if ((InfDate)&&(IUC_txtExtubateDateTime != '')) {
		...
		if (IUC_txtIntubateDateTime != '') {
			var IntubateDate = Common_DateParse(IUC_txtIntubateDateTime.split(" ")[0]);
			//var IntubateTime = IUC_txtIntubateDateTime.split(" ")[1];
			
			var dt3 = Common_DateParse(IUC_txtIntubateDateTime);
			if (dt3<dt1) errInfo = errInfo + '插管日期小于入院日期!<br>'
			if ((objPaadm.DisDate!="")&&(dt3>dt2)) errInfo = errInfo + '插管日期大于出院日期!<br>'		
						
			if (IUC_txtInfDate) {
				var InfDate = Common_DateParse(IUC_txtInfDate);
				if (IntubateDate>InfDate) errInfo = errInfo + '插管日期大于感染日期!<br>'
			}
			
			if (IUC_txtExtubateDateTime != '') {
				var ExtubateDate = Common_DateParse(IUC_txtExtubateDateTime.split(" ")[0]);
				//var ExtubateTime = IUC_txtExtubateDateTime.split(" ")[1];
				var dt4 = Common_DateParse(IUC_txtExtubateDateTime);
				...
				if (Common_ComputeDays("IUC_txtExtubateDateTime")<0) errInfo = errInfo + '拔管日期大于当前日期!<br>'
								
				if (IUC_txtInfDate) {
					var InfDate = Common_DateParse(IUC_txtInfDate);
					if (InfDate>ExtubateDate) errInfo = errInfo + '感染日期大于拔管日期!<br>'
					if (Common_ComputeDays("IUC_txtInfDate")<0) errInfo = errInfo + '感染日期大于当前日期!<br>'
				}							
			}
		}
	...
	obj.IUC_ViewPort = {
		...
		height : 200,
		anchor : '-20',
		tbar : ['<img SRC="../scripts/dhcmed/img/vnt.png"><b style="font-size:16px;">泌尿道插管</b>'],
		items : [
			obj.IUC_gridUC
		]
	}
