...
function InitIPICC(obj)
{
	...
	obj.IPICC_GridRowDataCheck = function(objRec){
		...
		/*if (IPICC_txtIntubateDateTime != '') {
			...
			if (IPICC_txtExtubateDateTime != '') {
				...
				var ExtubateTime = IPICC_txtExtubateDateTime.split(" ")[1];
				...
				if (objPaadm.DisDate) {
					...
					if ((objPaadm.DisDate==ExtubateDate)&&(objPaadm.DisTime<ExtubateTime))
						errInfo = errInfo + '拔管时间大于出院时间!<br>'
					}
				}
				//update by likai for bug:2142 拔管时间为空，感染日期可以大于当前日期
				var InfDate = IPICC_txtInfDate;
				if ((InfDate)&&(IPICC_txtExtubateDateTime != '')) {
		...
		if (IPICC_txtIntubateDateTime != '') {
			var IntubateDate = Common_DateParse(IPICC_txtIntubateDateTime.split(" ")[0]);
			//var IntubateTime = IPICC_txtIntubateDateTime.split(" ")[1];
			
			var dt3 = Common_DateParse(IPICC_txtIntubateDateTime);
			if (dt3<dt1) errInfo = errInfo + '插管日期小于入院日期!<br>'
			if ((objPaadm.DisDate!="")&&(dt3>dt2)) errInfo = errInfo + '插管日期大于出院日期!<br>'		
						
			if (IPICC_txtInfDate) {
				var InfDate = Common_DateParse(IPICC_txtInfDate);
				if (IntubateDate>InfDate) errInfo = errInfo + '插管日期大于感染日期!<br>'
			}
			
			if (IPICC_txtExtubateDateTime != '') {
				var ExtubateDate = Common_DateParse(IPICC_txtExtubateDateTime.split(" ")[0]);
				//var ExtubateTime = IUC_txtExtubateDateTime.split(" ")[1];
				var dt4 = Common_DateParse(IPICC_txtExtubateDateTime);
				if (dt4<dt1) errInfo = errInfo + '拔管日期小于入院日期!<br>'
				if (dt4<dt3) errInfo = errInfo + '拔管日期小于插管日期!<br>'
				if ((objPaadm.DisDate!="")&&(dt4>dt2)) errInfo = errInfo + '拔管日期大于出院日期!<br>'
				
				if (Common_ComputeDays("IPICC_txtExtubateDateTime")<0) errInfo = errInfo + '拔管日期大于当前日期!<br>'
								
				if (IPICC_txtInfDate) {
					var InfDate = Common_DateParse(IPICC_txtInfDate);
					if (InfDate>ExtubateDate) errInfo = errInfo + '感染日期大于拔管日期!<br>'
					if (Common_ComputeDays("IPICC_txtInfDate")<0) errInfo = errInfo + '感染日期大于当前日期!<br>'
				}							
			}
		}
	...
	obj.IPICC_ViewPort = {
		...
		height : 200,
		anchor : '-20',
		tbar : ['<img SRC="../scripts/dhcmed/img/picc.png"><b style="font-size:16px;">中心静脉置管</b>'],
		items : [
			obj.IPICC_gridPICC
		]
	}
