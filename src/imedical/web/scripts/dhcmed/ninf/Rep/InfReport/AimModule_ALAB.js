...
function InitALAB(obj)
{
	...
	obj.ALAB_GridRowEditer_gridPyAnti_btnAdd = new Ext.Button({
		...
		,listeners : {
			'click' :  function(){
				...
				*/
				var tDate = Common_DateParse(obj.ALAB_GridRowEditer_txtSubmissionDate.getRawValue());
				var tAdmDate = Common_DateParse(obj.CurrPaadm.AdmitDate);
				if(tDate < tAdmDate)
					errInfo = errInfo + '病原学检验送检日期应大于入院日期!<br>';
		...
		items : [
			{
				...
				buttons : [obj.ALAB_GridRowEditer_gridPyAnti_btnAdd,obj.ALAB_GridRowEditer_gridPyAnti_btnDel,'->','…']
			}
		]
	}
	obj.ALAB_GridRowDataCheck = function(objRec){
		...
		//Add By Jeff 2014-12-20 FixBug:2001 公共卫生事件-医院感染报告-ICU感染监测，病原学检验送检日期小于入院日期，提交成功
		/*var tDate = Date.parseDate(obj.ALAB_GridRowEditer_txtSubmissionDate.getRawValue(), "Y-m-d");
		var tAdmDate = Date.parseDate(obj.CurrPaadm.AdmitDate, "Y-m-d");
		if(tDate < tAdmDate) {
			errInfo = errInfo + '病原学检验送检日期应大于入院日期!<br>';
	...
	obj.ALAB_ViewPort = {
		...
		height : 300,
		anchor : '-20',
		tbar : ['<img SRC="../scripts/dhcmed/img/epidemic.gif"><b style="font-size:16px;">病原学检验</b>'],
		items : [
			obj.ALAB_gridLab
		]
	}
