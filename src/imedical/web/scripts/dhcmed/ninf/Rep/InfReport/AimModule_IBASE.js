...
function InitIBASE(obj)
{
	...
	obj.IBASE_cboTransLoc = Common_ComboToTransLoc("IBASE_cboTransLoc","<font color='red'>*</font>调查科室",obj.CurrReport.EpisodeID,"W");
	//Add By LiYang 2014-04-09 增加ApacheII评分
	obj.IBASE_txtApacheII = Common_TextField("IBASE_txtApacheII","ApacheII评分");
	//obj.IBASE_txtEncryptLevel = Common_TextField("IBASE_txtEncryptLevel","病人密级");
	//obj.IBASE_txtPatLevel = Common_TextField("IBASE_txtPatLevel","病人级别");
	...
	//Common_SetDisabled("IBASE_txtEncryptLevel",false);
	//Common_SetDisabled("IBASE_txtPatLevel",false);
						...
						items : [
							{
								...
								labelAlign : 'right',
								labelWidth : 100,
								items : [obj.IBASE_txtEncryptLevel]
							},{
								columnWidth:.23,
								layout : 'form',
								labelAlign : 'right',
								labelWidth : 80,
								items : [obj.IBASE_txtPatLevel]
							}
						]*/
					},{
						layout : 'form',
		...
		if (objPaadm) {
			...
			if (objPatient) {
				...
				if (parseInt(Age) > 0) {
					...
					Common_SetValue('IBASE_txtPatAge',parseInt(Month) + '月'); //fix bug 8000 by pylian 2015-03-24 患者年龄显示为如0月45天
				}else{
					Common_SetValue('IBASE_txtPatAge', parseInt(Day) + '天');
				}
				*/
	            ...
	            Common_SetValue('IBASE_txtPatAge',Age)
			...
			Common_SetValue('IBASE_txtEncryptLevel',SecretStr.split('^')[0]);
			Common_SetValue('IBASE_txtPatLevel',SecretStr.split('^')[1]);
			//报告信息
	...
	obj.IBASE_SaveData = function(){
		...
		objSumm.TransLoc = Common_GetValue('IBASE_cboTransLoc');
		//if (!objSumm.TransLoc.Descs) { //update cpj 2015-02-10 修改调查科室未填可以提交
		if (objSumm.TransLoc == "") { //Modified By LiYang 2015-03-30 调查科室仅为字段值，不是字典对象
