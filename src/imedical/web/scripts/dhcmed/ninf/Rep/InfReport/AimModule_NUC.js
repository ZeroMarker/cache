...
function InitNUC(obj)
{
	...
	obj.NUC_GridRowDataCheck = function(objRec){
		...
		if (NUC_txtIntubateDateTime != '') {
			...
			if ((DisDate!="")&&(dt3>dt2)) errInfo = errInfo + '插管日期大于出院日期!<br>'		
			...
			if (NUC_txtExtubateDateTime != '') {
				...
				if (Common_ComputeDays("NUC_txtExtubateDateTime")<0) errInfo = errInfo + '拔管日期大于当前日期!<br>'
				
				if (NUC_txtInfDate!="") {
					var InfDate = Common_DateParse(NUC_txtInfDate);
					if (InfDate>ExtubateDate) errInfo = errInfo + '感染日期大于拔管日期!<br>'
					if (Common_ComputeDays("NUC_txtInfDate")<0) errInfo = errInfo + '感染日期大于当前日期!<br>'
				}							
			}
		}
	...
	obj.NUC_ViewPort = {
		...
		height : 200,
		anchor : '-20',
		tbar : ['<img SRC="../scripts/dhcmed/img/UC.png"><b style="font-size:16px;">NICU-脐静脉</b>'],
		items : [
			obj.NUC_gridNUC
		]
	}
