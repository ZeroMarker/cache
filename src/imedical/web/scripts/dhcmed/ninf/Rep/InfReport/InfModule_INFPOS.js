...
{
	//易感因素
	obj.INFPOS_cbgInfFactors = Common_CheckboxGroupToDic("INFPOS_cbgInfFactors","<span><b><font color='red'>*</font>易感因素</b></span>","NINFInfInfFactors",6);
	//侵害性操作
	obj.INFPOS_cbgInvasiveOper = Common_CheckboxGroupToDic("INFPOS_cbgInvasiveOper","<span><b><font color='red'>*</font>侵害性操作</b></span>","NINFInfInvasiveOper",6);
	...
	obj.INFPOS_gridInfPos_RowEditer = function(objRec) {
		...
		if (!winGridRowEditer)
		{
			obj.INFPOS_gridInfPos_RowEditer_cboInfPos = Common_ComboToInfPos("INFPOS_gridInfPos_RowEditer_cboInfPos","<span><b><font color='red'>*</font>感染部位</b></span>","INFPOS_gridInfPos_RowEditer_cboInfDiag","","95%");
			obj.INFPOS_gridInfPos_RowEditer_cboInfDate = Common_DateFieldToDate("INFPOS_gridInfPos_RowEditer_cboInfDate","<span><b><font color='red'>*</font>感染日期</b></span>","95%");
			...
			obj.INFPOS_gridInfPos_RowEditer_cboInfDiag = Common_ComboToInfDia("INFPOS_gridInfPos_RowEditer_cboInfDiag","<span><b><font color='red'>*</font>感染诊断</b></span>","INFPOS_gridInfPos_RowEditer_cboInfPos","95%");
			obj.INFPOS_gridInfPos_RowEditer_cboInfDiagCat = Common_ComboToInfDiagCat("INFPOS_gridInfPos_RowEditer_cboInfDiagCat","<span><b><font color='red'>*</font>诊断分类</b></span>","INFPOS_gridInfPos_RowEditer_cboInfDiag","95%");
						...
						listeners : {
							'click' : function(){
								...
								if (InfDate == '') {
									...
									*/
									var objPaadm = obj.CurrPaadm;
									if (Common_ComputeDays("INFPOS_gridInfPos_RowEditer_cboInfDate")<0) errInfo = errInfo + '感染日期大于当前日期!<br>'
									if (Common_DateParse(InfDate)<Common_DateParse(objPaadm.AdmitDate)) errInfo = errInfo + '感染日期小于入院日期!<br>'
									if (objPaadm.DisDate) {
										if (Common_DateParse(InfDate)>Common_DateParse(objPaadm.DisDate)) errInfo = errInfo + '感染日期大于出院日期!<br>'
									}
								}
								...
								if (InfEndDate != '')
								{
									...
									*/
									if (Common_ComputeDays("INFPOS_gridInfPos_RowEditer_cboInfEndDate")<0) errInfo = errInfo + '感染日期大于当前日期!<br>'
									if (Common_DateParse(InfDate)>Common_DateParse(InfEndDate)) errInfo = errInfo + '感染日期大于感染结束日期!<br>'
	...
	obj.INFPOS_gridInfPos_btnAdd = new Ext.Button({
		...
		,listeners : {
			'click' :  function(){
				var objCmp=Ext.getCmp('INFPOS_gridInfPos_RowEditer_cboInfPos');
				if (objCmp){
					objCmp.setValue('');
					objCmp.setRawValue('');
				}
				var objCmp=Ext.getCmp('INFPOS_gridInfPos_RowEditer_cboInfDiag');
				if (objCmp){
					objCmp.getStore().removeAll();
					objCmp.getStore().load({});
				}
			...
			if(obj.CurrReport)
				{
					...
					if((obj.CurrReport.ReportStatus)&&(obj.CurrReport.RowID != "")){
						if((obj.CurrReport.ReportStatus.Code == "3")||
							(obj.CurrReport.ReportStatus.Code == "2")|| //Modified By LiYang 2015-03-27 提交状态的报告也不再次可以修改
	...
	obj.INFPOS_ViewPort = {
		...
		height : 500,
		anchor : '-20',
		tbar : ['<img SRC="../scripts/dhcmed/img/DiseaseInfo.png"><b style="font-size:16px;">感染相关</b>'],
		items : [
			{
				layout : 'border',
				frame : true,
						...
						items : [
							{
								...
								buttons : [obj.INFPOS_gridInfPos_btnAdd,obj.INFPOS_gridInfPos_btnDel,'->','…']
							},{
								region: 'west',
								layout : 'fit',
								width : 68,
								html: '<table border="0" width="100%" height="30px"><tr><td><font color="red">*</font>感染信息</td></tr></table>'
							}
						]
					}
				]
	...
	obj.INFPOS_InitView = function(){
		...
		//**************************************Stt**************************************
		//add by zf 2017-07-25 易感因素和侵害性操作的其他选项后边增加文本框
		var InfFactorstxt = obj.CurrReport.ChildSumm.InfFactorsTxt;
		var objLabelTxt = document.getElementById('INFPOS_cbgInfFactors-txt');
		if (objLabelTxt){
			objLabelTxt.value = InfFactorstxt;
		} else {
			var objDic = ExtTool.RunServerMethod("DHCMed.SSService.DictionarySrv","GetObjByTypeDesc","NINFInfInfFactors","其他");
			if (objDic) InfFactorsID = objDic.RowID;
			var objLabel=document.getElementById('INFPOS_cbgInfFactors-' + InfFactorsID);
			if (objLabel){
				var objLabelTmp = objLabel.nextElementSibling||objLabel.nextSibling;  //兼容低版本问题
				var sHTML='<input type="text" id="INFPOS_cbgInfFactors-txt" disabled value="' + InfFactorstxt + '" size="20" />'
				objLabelTmp.insertAdjacentHTML("afterEnd",sHTML);
			}
		}
		var InvasiveOpertxt = obj.CurrReport.ChildSumm.InvasiveOperTxt;
		var objLabelTxt = document.getElementById('INFPOS_cbgInvasiveOper-txt');
		if (objLabelTxt){
			objLabelTxt.value = InvasiveOpertxt;
		} else {
			var objDic = ExtTool.RunServerMethod("DHCMed.SSService.DictionarySrv","GetObjByTypeDesc","NINFInfInvasiveOper","其他");
			if (objDic) InvasiveOperID = objDic.RowID;
			var objLabel=document.getElementById('INFPOS_cbgInvasiveOper-' + InvasiveOperID);
			if (objLabel){
				var objLabelTmp = objLabel.nextElementSibling||objLabel.nextSibling;  //兼容低版本问题
				var sHTML='<input type="text" id="INFPOS_cbgInvasiveOper-txt" disabled value="' + InvasiveOpertxt + '" size="20" />'
				objLabelTmp.insertAdjacentHTML("afterEnd",sHTML);
			}
		}
		if (Common_GetText('INFPOS_cbgInfFactors').indexOf('其他')>-1) {
			document.all("INFPOS_cbgInfFactors-txt").disabled=false;
		}
		
		if (Common_GetText('INFPOS_cbgInvasiveOper').indexOf('其他')>-1) {
			document.all("INFPOS_cbgInvasiveOper-txt").disabled='';
		}
		
		obj.INFPOS_cbgInfFactors.on('change',function(){
			var objCmp = document.all('INFPOS_cbgInfFactors-txt');
			if (objCmp){
				if (Common_GetText('INFPOS_cbgInfFactors').indexOf('其他')>-1) {
					document.all("INFPOS_cbgInfFactors-txt").disabled=false;
				}else{
					document.all("INFPOS_cbgInfFactors-txt").value='';
					document.all("INFPOS_cbgInfFactors-txt").disabled=true;
				}
			}
		});
		obj.INFPOS_cbgInvasiveOper.on('change',function(){
			var objCmp = document.all('INFPOS_cbgInvasiveOper-txt');
			if (objCmp){
				if (Common_GetText('INFPOS_cbgInvasiveOper').indexOf('其他')>-1) {
					document.all("INFPOS_cbgInvasiveOper-txt").disabled=false;
				}else{
					document.all("INFPOS_cbgInvasiveOper-txt").value='';
					document.all("INFPOS_cbgInvasiveOper-txt").disabled=true;
				}
			}
		});
	...
	obj.INFPOS_SaveData = function(){
		...
		//**************************************Stt**************************************
		//add by zf 2017-07-21 其他后边增加文本框
		var objLabelTxt = document.getElementById("INFPOS_cbgInfFactors-txt");
		if (objLabelTxt){
			obj.CurrReport.ChildSumm.InfFactorsTxt = objLabelTxt.value;
		}
		var objLabelTxt = document.getElementById("INFPOS_cbgInvasiveOper-txt");
		if (objLabelTxt){
			obj.CurrReport.ChildSumm.InvasiveOperTxt = objLabelTxt.value;
		}
		//**************************************End**************************************
