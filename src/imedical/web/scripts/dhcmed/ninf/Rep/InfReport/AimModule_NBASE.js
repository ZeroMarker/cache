...
function InitNBASE(obj)
{
	...
	obj.NBASE_cboTransLoc = Common_ComboToTransLoc("NBASE_cboTransLoc","<font color='red'>*</font>调查科室",obj.CurrReport.EpisodeID,"W");
	...
	//出生体重
	obj.NBASE_txtBornWeight = Common_NumberField("NBASE_txtBornWeight","<font color='red'>*</font>出生体重(g)");
		...
		if (objPaadm) {
			...
			if (objPatient) {
				...
				if (parseInt(Age) > 0) {
					...
					Common_SetValue('NBASE_txtPatAge',parseInt(Month) + '月'); //fix bug 8000 by pylian 2015-03-24 患者年龄显示为如0月45天
				}else{
					Common_SetValue('NBASE_txtPatAge', parseInt(Day) + '天');
				}*/
				//update by pylian 2015-07-17 年龄由接口来获取,解决已上报的报告打开时年龄变化的问题
				var Age = ExtTool.RunServerMethod("DHCMed.SSIO.FromHisSrv","GetPapmiAge",objPaadm.PatientID,"", obj.CurrPaadm.AdmitDate,obj.CurrPaadm.AdmitTime);
	            Common_SetValue('NBASE_txtPatAge',Age)
	...
	obj.NBASE_SaveData = function(){
		...
		objSumm.TransLoc = Common_GetValue('NBASE_cboTransLoc');
		if (!objSumm.TransLoc) {
