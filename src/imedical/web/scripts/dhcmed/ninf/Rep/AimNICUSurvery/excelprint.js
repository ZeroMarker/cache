...
{
		var obj = ExtTool.StaticServerObject("DHCMed.NINFService.Rep.AimNICUSurvery");
		var ServerInfo=obj.GetServerInfo();
		var TemplatePath=GetWebConfig(ServerInfo).Path;
		var FileName=TemplatePath+"\\\\"+"DHCMed.NINF.NICUMonth.xls";
		try {
			xls = new ActiveXObject ("Excel.Application");
		}catch(e) {
			alert("创建Excel应用对象失败!");
		...
		xlSheet=xlBook.Worksheets.Item(1);
		var obj = ExtTool.StaticServerObject("DHCMed.NINFService.Rep.AimNICUSurvery");
		var flg=obj.ExportANISByMonth("fillxlSheet",strArguments);
		//var flg=cspRunServerMethod(strMethod,"fillxlSheet",strArguments);
