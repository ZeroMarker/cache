...
{
		var obj = ExtTool.StaticServerObject("DHCMed.NINF.Aim.PatientAdmToHDM");
		var ServerInfo=obj.GetServerInfo();
		var TemplatePath=GetWebConfig(ServerInfo).Path;
		//var TemplatePath="http://127.0.0.1/trakcarelive/trak/med/results/template/"
		var FileName=TemplatePath+"\\\\"+"横断面床旁调查登记表.xls";
		try {
			xls = new ActiveXObject ("Excel.Application");
		}catch(e) {
			alert("创建Excel应用对象失败!");
		...
		xlBook=xls.Workbooks.Add(FileName);
		xlSheet=xlBook.Worksheets.Item(1);
		var obj = ExtTool.StaticServerObject("DHCMed.NINFService.Aim.ICUSrv");
		var flg=obj.ExportHDMResPat("fillxlSheet",strArguments);
		//var flg=cspRunServerMethod(strMethod,"fillxlSheet",strArguments);
