...
{	obj.LoadEvent = function(args)
	{
		...
		var IsAimLoc = objAimLoc.IsAimLoc("MULTIDRUG",session['LOGON.CTLOCID']);
		if(tDHCMedMenuOper['Admin']==0)
		{
			if(IsAimLoc<0)
			{
				ExtTool.alert("提示","本科室未开展多重耐药菌目标性监测,如需开展,请联系感染办!");
	...
	{
		var objWinEdit = new InitwinScreen("MULTIDRUG");
	...
	obj.grdPatAdmRepList_rowdblclick = function () {
	  ...
	  if((EpisodeID=="")||(PatientID=="")||(PrintDocID=="")||(TemplateDocID=="")||(InstanceID=="")||(EPRNum=="")) return;
		...
		var url="./dhcmed.ninf.aim.centertablistdetail.csp?PatientID="+PatientID+"&EpisodeID="+EpisodeID+"&PrintDocID="+PrintDocID+"&TemplateDocID="+TemplateDocID+"&ChartItemID="+ChartItemID+"&InstanceDataID="+InstanceID+"&EPRNum="+EPRNum+"&RepStatus="+RepStatus+"&ReportID="+ReportID;;
...
function ReportClickHeader(PrintDocID,TemplateDocID,ChartItemID,Description)
{
	...
	var url="./dhcmed.ninf.aim.centertablistdetail.csp?PatientID="+PatientID+"&EpisodeID="+EpisodeID+"&PrintDocID="+PrintDocID+"&TemplateDocID="+TemplateDocID+"&ChartItemID="+ChartItemID+"&InstanceDataID=&EPRNum=&CtlocId="+CtlocId+"&UserId="+UserId+"&GroupId="+GroupId;
	var objWin = new Ext.Window(
		{
			title:Description,
			html:'<iframe width=990 height=620 scrollbars=no src='+ url + '></iframe>',
