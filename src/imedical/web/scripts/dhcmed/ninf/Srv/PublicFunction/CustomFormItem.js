...
//增加按科室过滤主管医生
function Common_ComboToSSUserLoc()
{
		...
		,store : new Ext.data.Store({
			proxy: new Ext.data.HttpProxy({
				...
				listeners : {
					'beforeload' :  function(objProxy, param){
						param.ClassName = 'DHCMed.NINFService.Srv.CommonCls';
						param.QueryName = 'QryUserByAliasLoc';
						param.Arg1      = tmpCombo.getRawValue();
						param.Arg2      = Common_GetValue(linkLoc);
						param.ArgCnt    = 2;
					}
				}
			}),
...
function Common_ComboToDateType1()
{
	...
	var tmpRepType = arguments[3];
	if (typeof tmpRepType == 'undefined') tmpRepType='';
	var tmpCombo = new Ext.form.ComboBox({
		id : arguments[0]
		,fieldLabel : arguments[1]
		,labelSeparator :''
		,store : new Ext.data.Store({
			proxy: new Ext.data.HttpProxy({
				url : ExtToolSetting.RunQueryPageURL + '?1=1&ClassName=DHCMed.NINFService.Rep.InfReport&QueryName=QryDateType&Arg1=' + tmpRepType + '&ArgCnt=1&2=2'
				,failure : function() {
					Ext.Msg.alert("数据加载失败！请检查");
				}
			}),
