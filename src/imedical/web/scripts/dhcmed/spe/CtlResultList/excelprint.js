...
{
		var obj = ExtTool.StaticServerObject("DHCMed.SPEService.PatientsQry");
		var objTmp = ExtTool.StaticServerObject("DHCMed.Service");
		var TemplatePath=objTmp.GetTemplatePath();
		var FileName=TemplatePath+"\\\\"+"DHCMed.SPE.CtlList.xls";
		try {
			xls = new ActiveXObject("Excel.Application");
		}catch(e) {
			alert("创建Excel应用对象失败!");
		...
		xlBook=xls.Workbooks.Add(FileName);
		xlSheet=xlBook.Worksheets.Item(1);
		var flg=obj.QryCtlPaadmToExcel("fillxlSheet",strArguments);
...
function ExportDataToExcelNP(strMethodGetServer,strMethodGetData,strTemplateName,strArguments)
{
		var obj = ExtTool.StaticServerObject("DHCMed.DTHService.ReportInterface");
		var objTmp = ExtTool.StaticServerObject("DHCMed.Service");
		var TemplatePath=objTmp.GetTemplatePath();
		...
		xlBook=xls.Workbooks.Add(FileName);
		xlSheet=xlBook.Worksheets.Item(1);
		var flg=obj.QryReportInfoToPrintNP("fillxlSheet",strArguments);
		var fname = xls.Application.GetSaveAsFilename(strTemplateName,"Excel Spreadsheets (*.xls), *.xls");
		//目录中存在重名的文件，在打开调试时选择“否”、“取消”会有报错
		//不知对选择“否”的处理如何写，暂时不处理
		try {
