...
function InitwinAreaEvent(obj){
	...
	obj.btnSave_click = function(){
		var objAreaDicUpdate = ExtTool.StaticServerObject("DHCMed.SS.AreaDic");
		if ((obj.txtCode.getValue()=="")||(obj.txtShortDesc.getValue()=="")||(obj.txtLongDesc.getValue()=="")) {
			ExtTool.alert("提示","代码、名称和全名不能为空,请认真填写!");
			return;	
		}
		var patrn=/^\d{8}$/;
		if(!patrn.exec(obj.txtCode.getValue())){
			ExtTool.alert("提示","代码只能数8位数字,请重新填写!");
		...
		if (obj.Record) {
			...
			var RowID = '';
			var ret=objAreaDicUpdate.CheckCode(obj.txtCode.getValue());
			if (ret==1) {
				ExtTool.alert("提示","代码与已有数据重复,请重新填写!");
				obj.txtCode.setValue("");
				return;	
			}
		}
