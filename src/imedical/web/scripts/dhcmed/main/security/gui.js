function Init() {
	...
	obj.treePMsTreeLoader = new Ext.tree.TreeLoader({
			nodeParameter : 'Arg1',
			dataUrl : "dhcmed.main.loadpmo.csp",
			baseParams : {
				groupId:"0"
			}			
		});
	...
	obj.treePPorsTreeLoader = new Ext.tree.TreeLoader({
		nodeParameter : 'Arg1',
		dataUrl : "dhcmed.main.loadportlets.csp",
		baseParams : {
			groupId:"0"
		}			
	});
	
	obj.treePPors = new Ext.tree.TreePanel({
		//buttonAlign : 'center'
		title:'页签'
		,region : 'center'
		,width:350
		,rootVisible:false
		,autoScroll:true
		,loader : null //obj.treePPorsTreeLoader
		,root : new Ext.tree.AsyncTreeNode({id:'root',text:'root'})
	});
	
	obj.chkSelectAllPors = new Ext.form.Checkbox({
		fieldLabel : '全选/全不选'
	});
	
	obj.treePorsPanel = new Ext.Panel({
		//buttonAlign : 'center'
		region : 'center'
		,layout : 'border'
		,items:[
