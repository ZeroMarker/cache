//03门诊报告卡
function ExportXMLType3(arryRep){
	var obj = objScreen;
	...
	obj.DSType03StoreProxy = new Ext.data.HttpProxy(objConn);
	obj.DSType03Store = new Ext.data.Store({
		proxy: obj.DSType03StoreProxy,
	...
	obj.ExportXMLFile = function(ReportIDList, isMapping, filePath, HospInsCode){
		obj.DSType03Store.load({
			params : {
				ClassName : 'DHCMed.SMDService.ExportXml03',
				QueryName : 'QryForXML',
				Arg1 : ReportIDList,
				ArgCnt : 1,
				filePath : filePath,
			...
			callback : function(arryResult, param){
				if ((arryResult.length > 0) && (param.params.isMapping != true)) {
					...
					try {
						var objDoc = XMLTool.CreateXML("PSYHOSPITALCARDLISTMZ");
						var objRec = null;
