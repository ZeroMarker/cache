function InitwinProblem(ReportIDList){
	var obj = new Object();
	...
	obj.btnSkip = new Ext.Button({
		id : 'btnSkip'
		//,tooltip : '跳过当前步骤,会导致部分数据导入不到精神卫生信息系统!'
		,iconCls : 'icon-delete'
		,width : 80
		,text : '跳过'
	});
	...
	obj.gridProblemStoreProxy.on('beforeload', function(objProxy, param){
		param.ClassName = 'DHCMed.SMDService.ExportXmlSrv';
	...
	obj.cboTargetDescStoreProxy.on('beforeload', function(objProxy, param){
		param.ClassName = 'DHCMed.SMD.IODictionary';
