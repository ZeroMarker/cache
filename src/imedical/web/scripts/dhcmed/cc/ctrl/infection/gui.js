function InitWinControl(SubjectID)
{
	...
	obj.radioVisitI = new Ext.form.Radio({
		id : 'radioVisitI'
		,name : 'radioVisit'
		,boxLabel : '在院'
		,inputValue : 'I'
	});
	obj.radioVisitO = new Ext.form.Radio({
		id : 'radioVisitO'
		,name : 'radioVisit'
		,boxLabel : '出院'
		,inputValue : 'O'
	...
	obj.DataGridPanelStore = new Ext.data.Store({
		...
		[
			...
			,{name: 'InfTypes', mapping: 'InfTypes'}
			...
			,{
				...
				,renderer: function(v, m, rd, r, c, s){
					...
					for (var iV=0;iV<arrRet.length;iV++){
						if(arrRet[iV].indexOf("医院感染报告") > -1){
							strRet += "<img src='../images/webemr/ill.jpg' alt='" + arrRet[iV] + "' />";
						}
						if(arrRet[iV].indexOf("ICU监测报告") > -1){
							strRet += "<img src='../images/webemr/ill.jpg' alt='" + arrRet[iV] + "' />";
						}
						if(arrRet[iV].indexOf("手术切口报告") > -1){
							strRet += "<img src='../images/webemr/ill.jpg' alt='" + arrRet[iV] + "' />";
						}
					}
					return strRet;
	...
	obj.DataGridPanelStoreProxy.on('beforeload', function(objProxy, param){
			param.ClassName = 'DHCMed.CCService.Ctrl.Infection';
