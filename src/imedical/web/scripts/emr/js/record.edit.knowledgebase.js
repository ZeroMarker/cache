...
function toolButtonClick(type)
{
	...
	else if (type == "toolspechars")
	{

		var returnValues = window.showModalDialog("emr.spechars.csp","","dialogHeight:420px;dialogWidth:490px;resizable:no;center:yes;status:no");	
		if (returnValues != "")	
		{ 
...
function saveNodePropty()
{
	...
	jQuery.ajax({
		...
		async: false,
		data: {"ACTION":"UpdateKBNode","ID":node.id,"Description":Description,"Name":Name,"IsCommon":IsCommon,"IsInUse":IsInUse,"TextData":TextData,"userID":userID,"userName":userName,"ipAddress":ipAddress,"actionType":"EMR.Knowledge.Update"},
		success: function(d) {
            if(d == "1")
            {
...
/// 停用知识库
$("#nodeStatus").live("click",function(){
