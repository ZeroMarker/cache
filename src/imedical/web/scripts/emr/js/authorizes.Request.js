...
function isselectall(){
	...
	Ext.getCmp('save').checked&&Ext.getCmp('print').checked&&Ext.getCmp('delete').checked&&Ext.getCmp('new').checked&&Ext.getCmp('view').checked;
}
	...
	Ext.Ajax.request({
		...
		success: function(response, opts){
			if (response.responseText != "") {
				var authPrivilege = response.responseText.replace(/\\n/g,"<br/>");
				Ext.MessageBox.alert('操作提示', authPrivilege)
			}
			else
			{
		...
		failure: function(response, opts){
			Ext.MessageBox.alert('提示', "AuthPrivilege error");
		}
	});
