...
var emrEditor = {
    ...
    saveDoc: function (documentContext, callback) {
        ...
        if ('False' === modifyResult.Modified) {
            $.messager.popover({msg:'文档没有发生改变！', type:'info', style:{top:10,right:5}});
		...
		{
			flag = this.doRevokeSignedDocument(modifyResult);
		}
        if (!flag) {
    ...
    printDoc: function (printArg, documentContext) {
        ...
        if (id == '1') {
            ...
            var src = "emr.opdoc.printPrompt.csp?prompt="+text+"&dialogId="+dialogId+"&printArg="+printArg;
    ...
    doRevokeSignedDocument: function (modifyResult) {
        ...
        for (var i = 0; i < modifyResult.InstanceID.length; i++) {
            ...
            if (revokeResult.result === 'ERROR') {
                $.messager.popover({msg:'撤销文档签名失败！', type:'error', style:{top:10,right:5}});
                ...
                $.messager.popover({msg:'文档签名已经被撤销！', type:'info', style:{top:10,right:5}});
                ...
                $.messager.popover({msg:'数据保存成功,签名已失效！', type:'success', style:{top:10,right:5}});
    ...
    setPlugin: function (chartItemType) {
        if (sysOption.pluginType === 'DOC') {
            iEmrPlugin.showWord();
            var isSetPlugin = iEmrPlugin.attachWord(sysOption.pluginUrl, sysOption.pluginType, sysOption.argConnect, editorEvt.eventDispatch);
            ...
            iEmrPlugin.showGrid();
            var isSetPlugin = iEmrPlugin.attachGrid(sysOption.pluginUrl, sysOption.pluginType, sysOption.argConnect, editorEvt.eventDispatch);
            if(!isSetPlugin) return isSetPlugin;
        }
    },
    /// 对外接口 异步执行
    ...
    createDocFromInstance: function (insData) {
        ...
        if (ret === '0')
        {
            //$.messager.alert('警示', '已创建同类型模板，不允许继续创建！', 'warning');
    ...
    savePrompt: function (isSync, documentContext) {
        ...
        var fnSavePrompt = function (documentContext) {
            ...
            //returnValues = window.showModalDialog('emr.opdoc.prompt.csp', msgText, 'dialogHeight:110px;dialogWidth:350px;resizable:no;status:no;scroll:yes;');
...
var editorEvt = {
    ...
    eventSaveDocument: function (commandJson) {
        ...
        if (commandJson.args.result === 'OK') {
            if (commandJson.args.params.result === 'OK') {
                $.messager.popover({msg:'数据保存成功！', type:'success', style:{top:10,right:5}});
    ...
    eventDeleteDocument: function(commandJson) {
        if (commandJson.args.result === 'OK') {
            try {
                ...
                $.messager.popover({msg:'病历删除成功！',type:'success', style:{top:10,right:5}});
    ...
    eventReplaceComposite: function (commandJson) {
        if (commandJson.args.result === 'OK') {
            $.messager.popover({msg:'知识库替换成功！', type:'success', style:{top:10,right:5}});
            eventSave("replaceComposite");
            //showEditorMsg('知识库替换成功!', 'alert');
        } else {
            $.messager.popover({msg:'知识库替换失败！', type:'error', style:{top:10,right:5}});
        ...
        var fnAfterSave = function (commandJson) {
            ...
            if ('PATIENT' === signProperty.OriSignatureLevel.toUpperCase()) { //手写签名
                ...
                if ('Y' !== sysOption.isDirectSignOP)
                {
                    ...
                    var src = "emr.opdoc.sign.csp?UserName="+patInfo.UserName+"&UserCode="+patInfo.UserCode+"&UserLocID="+patInfo.UserLocID+"&dialogId="+dialogId+"&signParam="+signParam;
                    var iframeCotent = "<iframe id='"+iframeId+"' width='100%' height='100%' scrolling='auto' frameborder='0' src='"+src+"'></iframe>";
    ...
    eventDiffentReferenceData: function(commandJson) {
        if (commandJson.args.result != 'OK') return;
		//如果插件返回的数据所有新值是空，或者没有数据，默认不弹出；手动可弹出，参数dataObjStr记录了是否手动，false是手动
		var newValueFlag = 0;
		...
		if ((newValueFlag == 0)&&(dataObjStr == true)) return;
		
		dataObjStr = commandJson.args;
		//dataObjStr = base64encode(utf16to8(escape(JSON.stringify(commandJson.args))));
		var tempFrame = "<iframe id='iframeSynData' scrolling='auto' frameborder='0' src='emr.ip.datasyn.csp' style='width:100%; height:100%; display:block;'></iframe>";
		createModalDialog("dialogSynData","数据同步","620","450","iframeSynData",tempFrame,synDataCallback,"");
    },
    ...
    eventInsertTeethImage: function(commandJson) {
        if (commandJson.args.result == 'OK') {
            ...
            $.messager.popover({msg:'插入成功！', type:'success', style:{top:10,right:5}});
        }
    },
...
function showEditorMsg(msg, msgType) {
    if (isExistVar(msg)) {
        ...
        $('#msgtd').append(msg).show();
        intervalidHideMsg = setInterval("$('#msgtd').hide();", millisec);
    }
}*/
...
function openTooth(commandJson)
{
    ...
    var src = "emr.ip.tool.tooth.csp?dialogId="+dialogId+"&selectedToothObjStr="+selectedTeeth;
    var iframeCotent = "<iframe id='"+iframeId+"' width='100%' height='100%' scrolling='auto' frameborder='0' src='"+src+"'></iframe>";
...
function signDocumentCheck(userInfo, arg)
{
    ...
    if (('' === signlevel) && (signProperty.OriSignatureLevel !== "All")) {
        $.messager.popover({msg:'用户签名级别为空！', type:'alert', style:{top:10,right:5}});
    ...
    if (signInfo.result == 'OK') {
        ...
        $.messager.popover({msg:'签名失败！', type:'error', style:{top:10,right:5}});
...
function printDocumentCheck(printArg)
{
    ...
    var documentContext = documentContext || emrEditor.getDocContext();
    emrEditor.saveDoc(documentContext, fnAfterSave);
}
//插入文本数据到病历
//引入惠每数据到病历中的方法
...
function isConsistent(documentContext)
{
	...
	if (documentContext.result === "ERROR") {
		$.messager.popover({msg:'请注意：当前保存病历可能不属于当前患者，保存失败，请重新打开病历页面进行编辑保存！', type:'error', style:{top:10,right:5}});
		return false;
	}
