...
function pluginAdd() {
	...
	addEvent(plugin(), 'onExecute', function (command) {
		...
		if (commandJson.action == 'eventSetNetConnect') {
		...
		} else if (commandJson.action == "eventSaveDocumentAs") {
			eventSaveDocumentAs(commandJson);
...
function init() {   ///iEditor/iGridEditor
	//debugger;
...
function loadModelInstance() {
	var strJson = {
		...
		"args" : {
            "LoadSection" : true,
            "UserTemplateCode" : "LoadExample^"+model.id/*,
            "params" : {
                "actionType" : "LoadExampleSection",
                "ExampleInstanceID" : model.id,
                "SectionList":[]
            }*/
		}
	};
	/*getExampleSections(model.id,function(data){
		$.each(data,function(index,item){
            strJson.args.params.SectionList.push({"Code":item.attributes.sectionCode});
        })
	});*/
    ...
    var strJson = {
        ...
        "args" : {
            "params" : {
                ...
                "TemplateVersionId" : model.attributes.templateId,
                //"UserTemplateId" : "",
                "actionType" : "LoadExample",
                "ExampleInstanceID" : model.id/*,
                "SectionList":[]*/
            }
        }
...
/*
//用范例创建病历
function getExampleSections(exampleId,callback)
{
	jQuery.ajax({
		type: "post",
		dataType: "json",
...
function appendElement() {
	...
	$("#propertySave").live("click", function () {
...
//文档保存事件监听
function eventSaveDocumentAs(commandJson) {
...
function initValue() {
	//最小值是不否有效
	$("#HasMinVal").live("change", function () {
		var flag = true;
	...
	//最大值是不否有效
	$("#HasMaxVal").live("change", function () {
		var flag = true;
	...
	//是否包含日期
	$("#IncludeDate").live("change", function () {
		var flag = true;
	...
	//是否包含时间
	$("#IncludeTime").live("change", function () {
		var flag = true;
	...
	$("#DictionaryType").live("change", function () {
