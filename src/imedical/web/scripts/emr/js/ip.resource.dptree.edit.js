$(function(){
    ...
    initElement();
    //保存xml
    $("#publicSave").on("click", function(){
        var strJson = {"action":"SAVE_DOCUMENT","args":{"params":{"action":"SAVE_COMPOSITE","KBNodeID":DPNode.id,"Type":"DP"}}};
        cmdDoExecute(strJson);
    });
    
    // 停用
    $("#nodeStatus").on("click", function(){
        var IsValid = ($("#nodeStatus").val() == "停用")?"N":"Y";
        jQuery.ajax({
            type: "GET",
            dataType: "text",
            ...
            success: function(d) {
                var btText = (IsValid == "N")?"启用":"停用";
                $("#nodeStatus").text(emrTrans(btText));
                returnValues.IsValid = IsValid;
                if (userLocID != ""){
                    window.returnValue = returnValues;
                }else{
                    if (refreshDPTreeNode){
                        refreshDPTreeNode(returnValues);
                    }
                }
            },
            error: function(d){
...
function pluginAdd() {
    ...
    addEvent(plugin(), 'onExecute', function(command){
       ...
       if (commandJson.action == "eventCreateComposite")
       {
           loadFlag = false;
       }
...
function init()
{
    ...
    cmdDoExecute(strJson);
    strJson = {action:"SET_DEFAULT_FONTSTYLE",args:defaultFontStyle};
    cmdDoExecute(strJson);
    setNodeInfo();
    //加载HIS绑定字段
    getDataBaseCatalog();
    ...
    setFontData();
    setFontSizeData();
    maxElementObj = {"MIString":"","MINumber":"","MIDateTime":"","MIMonoChoice":"","MIMultiChoice":"","MIDictionary":""};
}
...
function setNodeInfo()
{
    jQuery.ajax({
        ...
        data: {
            "OutputType":"Stream",
            "Class":"EMRservice.BL.BLDPCategory",
            "Method":"getDPNodeData",
            "p1":nodeID
        },
        success: function (d) {
            if (d != ""){
                DPNode = d[0];
                if (DPNode.isValid.toUpperCase() == "N")
                {
                ...
                $("#nodeText").val(DPNode.text);
                $("#nodeDesc").val(DPNode.desc);
                if (DPNode.isEmpty == "Y"){
                    createComposite();
                }else{
                    loadComposite();
...
{
	//var strJson = {"action":"CREATE_COMPOSITE","args":{"Code":DPNode.id,"DisplayName":DPNode.text,"DPNodeID":DPNode.id,"Type":"DP","Instances":"0..1"}};     
	var strJson = {"action":"CREATE_COMPOSITE","args":{"Code":DPNode.id,"DisplayName":DPNode.text,"KBNodeID":DPNode.id,"Type":"DP","Instances":"0..1"}};                 
	cmdDoExecute(strJson);
}
//加载知识库
function loadComposite()
{
	//var	strJson = {"action":"LOAD_DOCUMENT","args":{"params":{"action":"LOAD_COMPOSITE","DPNodeID":DPNode.id,"Type":"DP"}}}; //KBNodeID
	var	strJson = {"action":"LOAD_DOCUMENT","args":{"params":{"action":"LOAD_COMPOSITE","KBNodeID":DPNode.id,"Type":"DP"}}};
	cmdDoExecute(strJson);
}
...
function setFontSizeData()
{
    ...
    $("#fontSize").combobox("textbox").keyup(function(event){
        if(event.keyCode == 13){
            var text = $("#fontSize").combobox("getText");
            if (text == ""){
                $("#fontSize").combobox("setValue",defaultFontStyle.fontSize);
                text = defaultFontStyle.fontSize.replace('pt','');
            }
            var strJson = {action:"FONT_SIZE",args:text};
            cmdDoExecute(strJson);
...
//		var returnValues = window.showModalDialog("emr.ip.tool.spechars.csp","","dialogHeight:420px;dialogWidth:490px;resizable:no;center:yes;status:no");
		...
		var callback = function(rtn,arr){
			strJson={action:"INSERT_STYLE_TEXT_BLOCK",args:rtn};
...
{
    /*var data = $('#fontSize').combobox("getData");
...
function eventSaveDocument(commandJson)
{
	...
	if (commandJson.args.result == "OK")
	{
		if (commandJson.args.params.result != "OK")
		{
            if (userLocID != ""){
                ...
                top.$.messager.alert('提示',emrTrans('保存失败'));
            }
		}
		else
		{
			if (flag == "1")
			{
                if (userLocID != ""){
                    ...
                    top.$.messager.alert('提示',emrTrans('保存成功'));
                }
			}
			else
			{
	...
	else if (commandJson.args.result == "INVALID")
    {
        if (userLocID != ""){
            ...
            top.$.messager.alert("提示", emrTrans("病历存在非法字符，不能保存"), 'info');
        }
    }
...
function saveNodePropty()
{
    ...
    if (!$("#nodeStatus").val() == "停用") IsValid = "N";
    ...
    jQuery.ajax({
        ...
        success: function(d) {
            if(d == "1")
            {
                ...
                result = "1";
                //DPNode.text = nodeText;
                //DPNode.desc = Description;
                //DPNode.textData = TextData;
                DPNode.isEmpty = "N";
            }
        },
        error: function(d){
            alert("UpdateDPNodeData error");
        }
    });
    return result;
}
