...
var handSignInterface = {
    ...
    getAnySignLocation: function (parEditor) {
        var ret = parEditor.getAnySignLocation();
        if (ret.Err || false) {
            throw {
...
var hsPluginData = {
    sign: function(parEditor){
        if (!handSignInterface && !handSignInterface.checkStatus()) {
            $.messager.alert("提示", "请配置手写签名设备！", "info");
            return;
        }
        var oriSignatureLevel = parEditor.oriSignatureLevel;
        try {
            var patientSignInfo = parEditor.getPatientSignInfo(oriSignatureLevel);
            if ("" == patientSignInfo){
                $.messager.alert("提示", "患者证件信息不能为空！", "info");
                return;
            }
            // 获取图片
            if (oriSignatureLevel.toUpperCase() == "PATIENT"){
                var evidenceData = handSignInterface.getEvidenceData();
                if (typeof evidenceData === 'undefined'){
                    return;
                }
                patientSignInfo.id = parEditor.userID || handSignInterface.getUsrID(evidenceData);
                patientSignInfo.code = parEditor.code;
                patientSignInfo.textSrc = "";
                patientSignInfo.signSrc = handSignInterface.getSignScript(evidenceData);
                patientSignInfo.headerSrc = handSignInterface.getSignPhoto(evidenceData);
                patientSignInfo.fingerSrc = handSignInterface.getSignFingerprint(evidenceData);
            }else{
                var noteImg = this.getnotation(parEditor.descContent);
                if (noteImg == "") {
                    $.messager.alert("提示", "获取批注图片为空，请检查环境是否正常", "info");
                    return;
                }
                patientSignInfo.id = parEditor.userID;
                patientSignInfo.code = parEditor.code;
                patientSignInfo.textSrc = noteImg;
                patientSignInfo.signSrc = "";
                patientSignInfo.headerSrc = "";
                patientSignInfo.fingerSrc = "";
            }
            var callBack = {
                response: function(signInfo){
                    var params = {
                        id: patientSignInfo.id,
                        elementCode: signInfo.elementCode,
                        code: signInfo.code,
                        invalidCode: signInfo.invalidCode,
                        levelName: patientSignInfo.levelName,
                        roleLevel: patientSignInfo.roleLevel,
                        signLevel: patientSignInfo.signLevel,
                        hashData: signInfo.digest,
                        documentID: parEditor.documentID,
                        time: signInfo.time,
                        type: "AnySign",
                        signScript: patientSignInfo.signSrc || patientSignInfo.textSrc,
                        headerImage: patientSignInfo.headerSrc,
                        fingerPrint: patientSignInfo.fingerSrc,
                    ...
                    if (oriSignatureLevel.toUpperCase() == "PATIENT"){
                        ...
                        params.algorithm = handSignInterface.getAlgorithm(signValue);
                        params.bioFeature = handSignInterface.getBioFeature(signValue);
                        params.eventCert = handSignInterface.getEventCert(signValue);
                        params.signValue = handSignInterface.getSigValue(signValue);
                        params.tsValue = handSignInterface.getTSValue(signValue);
                        params.version = handSignInterface.getVersion(signValue);
                        params.signData = JSON.stringify(signValue);
                    }
                    var saveResult = parEditor.savePatSignDocument(params,signInfo.value);
                    if ("fail" != saveResult.result){
                        showEditorMsg({msg:'数据签名成功!',type:'success'});
                        //hisLog.operate("EMR.OP.Sign.OK",parEditor.documentID);
                        privilege.setRevsion(parEditor.documentID);
                    }else{
            ...
            parEditor.signDocument(patientSignInfo, callBack);
        } catch (err) {
    ...
    getnotation: function(content) {
        ...
        try {
            ...
            $.messager.alert("提示", "患者意见获取失败："+e.message, "info");
            return "";
        }
        return notation;
    }
...
var hsPluginPDF = (function(){
	function signPDF(parEditor) {
		...
		var documentID = parEditor.signProps.documentID;
		var episodeID = parEditor.signProps.episodeID
		var canDo = parEditor.canDoPDFSign(documentID);
		if (!canDo)  {
			handSign.closeDlg();
			return;
		}
		
		var allKeyWord = parEditor.getPatSignKeyWord(documentID);
		if (allKeyWord == "") {
		...
		var signedPdfBase64 = parEditor.getSignedPDF({episodeID:episodeID, documentID:documentID});
		if (signedPdfBase64 == "") {
            var callBack = {
                response: function(pdfBase64){
                    if (pdfBase64 == "") {
                        handSign.showTip("生成待签名PDF失败！");
                        return;
                    }
                    try {
                        var ret = parEditor.pushToSignPDF({
                            documentID: documentID,
                            keyWord: allKeyWord,
                            userID: parEditor.userID,
                            signType: "",
                            pdfBase64: pdfBase64
                        });
                        if ("fail" != ret.result){
                            if (ret.params.split('^')[0] == 1){
                                if (ret.params.split('^')[1] != "") {
                                    handSign.showTip("推送待签名PDF成功...");
                                    setTimeout("handSign.closeDlg()", 2000 )
                                }
                            }else{
                                throw { message : "pushToSignPDF 失败！" + ret.params};
                            }
                        }else{
                            throw { message : "pushToSignPDF error:" + ret.error };
                        }
                    } catch (e) {
            ...
            parEditor.createToSignPDFBase64(["Patient", "Notation"], callBack);
        }
	}
...
var handSign = (function(){
	...
	function getHandSignType(parEditor) {
        ...
        var arr = parEditor.getHandSignType({
            episodeID: parEditor.signProps.episodeID,
            documentID : parEditor.signProps.documentID,
