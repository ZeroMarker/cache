...
$("#delTag").click(function(){
    ...
    if (returnData){
        text='该关键字包含收藏的病历,是否确定删除关键字【' + node.name + '】?';
    }else{
        text='是否确定删除关键字【' + node.name + '】?';
    }
...
function delTag(node, flag){
    ...
    $.ajax({ 
        ...
        success: function (data) {
            if (data != "0") {
                ...
                if (flag) {
                    $("#favInfoList #tags .tag[id="+node.id+"]").each(function(i) {
                        $(this).remove();
                    });
                }
            }
    ...
    //记录用户(整理收藏.我的收藏.新建关键字)行为
    AddActionLog(userId,userLocId,"FavoritesView.MyTreeNode."+node.name+".Del","");
}
