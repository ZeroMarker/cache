...
(function($) {
    $(function() {
        ...
        var patientDG = $('#patientListTable').datagrid({
            url: '../EMRservice.Ajax.DirectAppoint.cls',
            queryParams: {
                PatientName: '',
                PatientLocID: '',
                PatientWardID: '',
        ...
        //选择患者就诊科室
		/*  默认值查询当前登录科室的患者  add by niucaicai 2016-8-5  去掉科室下拉框 1
        $('#inputPatientLoc').combogrid({
			...
			onShowPanel:function(){
				$('#inputPatientLoc').combogrid('grid').datagrid('load');
			}
        });
		默认值查询当前登录科室的患者  add by niucaicai 2016-8-5  去掉科室下拉框2  */
        //选择患者就诊病区
		/*  默认值查询当前登录科室的患者  add by niucaicai 2016-8-5  去掉患者病区下拉框  1
        $('#inputPatientWard').combogrid({
			...
			*/
			/* 去掉患者病区下拉框  2  默认值查询当前登录科室的患者  add by niucaicai 2016-8-5
            onSelect: function() {
                eprDirectAuthorization.selectPatientWardID = "";
                var rows = $('#inputPatientWard').combogrid('grid').datagrid('getSelections');
                var row = rows[0];
                eprDirectAuthorization.selectPatientWardID = row["ID"];
                searchPatient();
            },
			onBeforeLoad:function(param){
				param = $.extend(param,{Filter:param.q});
			...
			onShowPanel:function(){
				$('#inputPatientWard').combogrid('grid').datagrid('load');
			}
        });
		默认值查询当前登录科室的患者  add by niucaicai 2016-8-5  去掉患者病区下拉框  2  */
        //查询患者
        ...
        function expandEMRRangeTree(episodeID) {
            ...
            $('#emrTree').tree({
				...
				url: '../EMRservice.Ajax.DirectAppoint.cls?Action=gettree&EpisodeID=' + episodeID + '&selectedDocID=' + selectedDocID + '&selecteddocCTLocID=' + selecteddocCTLocID + '&selecteddocSSGroupID=' + selecteddocSSGroupID,
                method: 'get',
                animate: true,
                checkbox: true,
		checked: false,
        ...
        $('#inputGivePowerType').combogrid({
            ...
            $('#treeGivePowerBtn').on('click', function() {
				...
				if ((appointCateCharpter.indexOf("||") != -1) && (givePowerTypes.indexOf("new") != -1)) {
                ...
                $.messager.confirm('确认给予权限', msg, function(r) {
                    if (r) {
                        var ret = ""						
						//liuzhongwan如果选择了模板并且选择了new，要为这些模板补上save的申请自动发送
					if ((appointCateCharpter.indexOf("||") == -1) && (givePowerTypes.indexOf("new") != -1) && (givePowerTypes.indexOf("save") == -1)) {
							givePowerTypes = givePowerTypes + "^save";
						}
						//end liuzhongwan
                        var obj = $.ajax({
                            url: "../EMRservice.Ajax.DirectAppoint.cls?Action=givepower&AppointSpan=" + givePowerSpan + "&AppointCateCharpter=" + appointCateCharpter + "&AppointUserID=" + userID + "&EPRAcitons=" + givePowerTypes + "&EpisodeID=" + episodeID + "&UserID=" + docUserID + "&DocLocID=" + docCTLocID,
                            type: 'post',
                            async: false
                        });
					...
					if (appointIDs == "") {
						...
						$.messager.confirm('确认撤销权限', '确实要收回选中的 生效中 授权？', function(r) {
							if (r) {
								var ret = ""
								var obj = $.ajax({
									url: "../EMRservice.Ajax.DirectAppoint.cls?Action=withdraw&AppointIDs=" + appointIDs,
									type: 'post',
									async: false
								});
								var ret = obj.responseText;
