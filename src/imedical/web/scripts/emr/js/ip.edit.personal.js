...
function init() {
	...
	setFontSizeData();
	getDocOutline();
	$("#nodeText").val(model.name);
	var modelInfo = model.documentType;
	if ('DOC'!==modelInfo) {
		cmdDoExecute({
			"action" : "SET_WORKSPACE_CONTEXT",
			"args" : "Single"
...
function eventSaveSection(commandJson) {
	if (commandJson.args.result == "OK") {
		$.messager.popover({msg:'保存成功！', type:'success', style:{top:10,left:400}});
		window.returnValue = "ok";
		RenameIt();
		//加载个人模板
		loadPersonalTemplate();
	} else {
		$.messager.popover({msg:'个人模板保存失败！', type:'error', style:{top:10,left:400}});
...
function checkNewname(){
	...
	if(patrn.test(newName)){
		//提示不允许输入特殊字符
		$.messager.alert('提示','不允许输入特殊字符', 'info');
		for(var i = 0;i<newName.length;i++){
			newName = newName.replace(patrn,"");
		}
		$('#nodeText').val(newName);
		return false;
	}else{
		return true;
	}
...
{
	parent.closeDialog("HisUIEditPersonal");
}