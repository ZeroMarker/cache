...
function setMathSelect(){
	var mathOptions = [
	{value:'toothMath',text:'牙位图'},
	{value:'mensesMath',text:'月经生育史'},
	{value:'ACDMath',text:'前房深度公式'},
	]
	$HUI.combobox('#math-combox',{
		...
		onSelect:function (record) {
			switch(record.value){
				...
				case 'mensesMath':{
					var tempFrame = "<iframe id='iframeMenses' scrolling='auto' frameborder='0' src='emr.ip.menses.math.csp' style='width:100%;height:100%'></iframe>"
...
document.getElementById("favoritesPlus").onclick = function(){
	...
	var tempFrame = "<iframe id='iframeFavAdd' scrolling='auto' frameborder='0' src='emr.fav.add.csp?EpisodeID="+episodeID+"&InstanceID="+ param.id+"&categoryId="+categoryId+"&templateId="+templateId+ "' style='width:450px; height:450px; display:block;'></iframe>";
...
document.getElementById("favorites").onclick = function(){
	...
	var tempFrame = "<iframe id='iframeFavorite' scrolling='auto' frameborder='0' src='emr.fav.favorite.csp' style='width:100%; height:100%; display:block;'></iframe>";
...
document.getElementById("batchprint").onclick = function(){
    ...
    var xpheight = window.screen.height-340;
    createModalDialog("batchPrintDialog","批量打印",xpwidth,xpheight,"iframeBatchPrint","<iframe id='iframeBatchPrint' scrolling='auto' frameborder='0' src='emr.ip.batchprint.csp?EpisodeID=" + episodeID + "&PatientID=" + patientID + "&IPAddress=" + ipAddress + "' style='width:100%;height:100%;display:block;'></iframe>","","");
};
...
function changeRecordFinishedStatus()
{
	var xpwidth = 1000;
	var xpheight = 340;
	var tempFrame = "<iframe id='iframeConfirm' scrolling='auto' frameborder='0' src='emr.ip.confirmadmrecord.csp?EpisodeID="+episodeID+"' style='width:"+xpwidth+"px; height:"+xpheight+"px; display:block;'></iframe>";
...
function confirmRecordFinished()
{
	...
	$.messager.confirm("提示",tipMsg, function (r) {
		...
		{
			top.$.messager.alert("提示信息", "提交病历成功", 'info');
		}
		else 
		{
			top.$.messager.alert("提示信息", "提交病历失败");
			return;
		}
		getNav();
	});
	...
	jQuery.ajax({
			...
			success: function (data){
				...
				if (result == "1") 
				{
					$("#confirmRecordCompleted span .l-btn-icon").removeClass("icon-big-book-arrow-ok");
					$("#confirmRecordCompleted span .l-btn-icon").addClass("icon-big-book-arrow-rt");
					top.$.messager.alert("提示信息", "撤销提交成功", 'info');
					getNav();
				}
				else
				{
					...
					{
						top.$.messager.alert("提示信息", emrTrans(result.substring(2)));	
					}
					else
					{
						top.$.messager.alert("提示信息", "撤销提交失败");
					}
				}
			}
		});	
...
function qualityConfirmDocument()
{
	...
	if ((confirmAlertType == "2")||(confirmAlertType == "3"))
	{
		...
		{
			result = true;
			var quality = "<iframe id='framquality' src='finishdocumentunsignlist.csp?EpisodeID="+episodeID+"' style='width:98%; height:98%;border:0;margin:0px;padding:5px;overflow:hidden;' scrolling=no></iframe>"			
	...
	else
	{
		...
		if (qualityData.total > 0)
		{
			...
			var quality = "<iframe id='framquality' src='dhc.emr.quality.runtimequalitylist.csp?EpisodeID="+episodeID+"&EventType="+eventType+"&TemplateID=&key="+qualityData.key+"' style='width:98%; height:98%;border:0;margin:0px;padding:5px;overflow:hidden;' scrolling=no></iframe>"			
			addTabs("quality","质控提示",quality,true); 
			if (controlType == "0") 
			{
				result = true;
...
function setPatDiseases()
{
	...
	var tempFrame = "<iframe id='iframeAdmPatType' scrolling='auto' frameborder='0' src='emr.ip.admpattype.csp?LocID="+userLocID+"&EpisodeID="+episodeID + "' style='width:100%; height:100%; display:block;'></iframe>";
	createModalDialog("admPatType","设置患者病种","410","410","iframeAdmPatType",tempFrame,"","",true,false);
...
function managePersonalTemplate()
{
    ...
    var content = '<iframe id="managePersonalWinFrame" scrolling="auto" frameborder="0" src="emr.ip.managepersonal.csp?IstanceID=' + insID + '&UserID=' + userID + '&UserLocID=' + userLocID +'" style="width:100%;height:100%;display:block;"></iframe>';
