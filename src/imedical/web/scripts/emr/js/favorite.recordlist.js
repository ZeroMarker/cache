...
// 取消收藏病历
function cancelFavRecrod(instanceId,episodeId,favInfoId,pluginType,chartItemType,emrDocId)
{
	...
	$.ajax({ 
        ...
        success: function (data) { 
        	if (data == "1") 
        	{
	        	...
	        	var tool = "<div class='tooldiv'><a href='javascript:void(0);' style='color:blue;' onclick='doFavorite("+'"'+ instanceId +'","'+episodeId+'","'+favInfoId +'","'+pluginType+'","'+chartItemType+'","'+emrDocId+'"'+")'>添加收藏</a></div>";
		        ...
		        }else
		        {
			        ...
			        $(box).append('<div class="img_l" style="background:url(../scripts/emr/image/icon/arrowscq.png) 0px 0px no-repeat;" onclick="imgleft('+"'"+param+"'"+')"></div>');
			        $(box).append($('<div class="apply_nav"></div>').append($('<div class="apply_w"></div>').append(tmpArray)));
			        $(box).append('<div class="img_r" style="background:url(../scripts/emr/image/icon/arrowscq.png) -36px 0px no-repeat;" onclick="imgright('+"'"+param+"'"+')"></div>');
			        $(apply).append(box);
...
function remarkingFav(instanceId,favInfoId,instanceName,episodeId,pluginType,chartItemType,emrDocId)
{
	...
	var content = '<iframe id="framremark'+instanceId+'" frameborder="0" src="emr.favorite.remarking.csp?FavInfoID='+favInfoId+'&InstanceID='+instanceId+'&InstanceName='+instanceName+'&EpisodeID='+episodeId+'&UserID='+userId+'&UserLocID='+userLocId+'&pluginType='+pluginType+'&chartItemType='+chartItemType+'&emrDocId='+emrDocId+'" style="width:100%;height:100%;scrolling:no;"></iframe>'
	parent.addTab("tabFavorite","remark"+instanceId,"对 "+instanceName+" 评价",content,true);
	
}

//添加收藏
function doFavorite(instanceId,episodeId,favInfoId,pluginType,chartItemType,emrDocId)
{
	...
	$.ajax({ 
        ...
        success: function (data) { 
        	if (data == "1") 
        	{
	        	...
	        	var tool = "<div class='tooldiv'><a href='javascript:void(0);' style='color:blue;' onclick='cancelFavRecrod("+'"'+ instanceId +'","'+episodeId+'","'+favInfoId +'","'+pluginType+'","'+chartItemType+'","'+emrDocId+'"'+")'>取消收藏</a>&nbsp;|&nbsp;";
				tool = tool + "<a href='javascript:void(0);' style='color:blue;' onclick='remarkingFav("+'"'+ instanceId +'","'+ favInfoId +'","'+title+'","'+episodeId+'","'+pluginType+'","'+chartItemType+'","'+emrDocId+'"'+")'>病历评价</a></div>";
	        	$(tmpArray).append(tool);
	...
	$.ajax({ 
        ...
        success: function (data) { 
	        ...
	        if(array.length == "")
	        {
				...
				var box = $('<div class="box"></div>');
				$(box).append('<div class="img_l" style="background:url(../scripts/emr/image/icon/arrowscq.png) 0px 0px no-repeat;"></div>');
				var applyw = $('<div class="apply_w"></div>');
				var empty = $('<div class="empty"><div class="p"></div>没有？快点击列表<br />添加收藏</div>');
	    		$(applyw).append(empty);
				var applynav = $('<div class="apply_nav"></div>').append(applyw);
				$(box).append(applynav);
				$(box).append('<div class="img_r" style="background:url(../scripts/emr/image/icon/arrowscq.png) -36px 0px no-repeat;"></div>');
				$(apply).append(box);
		        $("#favoriteRecordList").append(apply);    
		    }
        } 
...
function setRecords(data,type,favInfoId,episodeId,episodeDate,episodeType,episodeLoc)
{
	...
	for (var i=0;i<data.length;i++)
	{
		...
		var status = "", content = "";
		if((data[i].status=="完成")&&(data[i].isWaitsign != "1")&&(isShowToBeSignedSymbol != "N"))
		{
			status = "待签";
		}
        ...
        if (type == "FavoriteRecord")
        {
	        if (isShare.toUpperCase() != "TRUE")
	        {
	        	tool = tool + "<a href='javascript:void(0);' onclick='cancelFavRecrod("+'"'+ data[i].id +'","'+episodeId+'","'+favInfoId +'","'+data[i].documentType+'","'+data[i].chartItemType+'","'+data[i].emrDocId+'"'+")'>取消收藏</a>&nbsp;|&nbsp;";
	        }
	    ...
	    {
		    tool = tool + "<a href='javascript:void(0);' onclick='doFavorite("+'"'+ data[i].id +'","'+episodeId+'","'+favInfoId+'","'+data[i].documentType+'","'+data[i].chartItemType+'","'+data[i].emrDocId+'"'+")'>添加收藏</a>"
		}
		tool = tool + "</div>"
		$(array).append(tool);
		$(applyw).append(array);
...
$(".apply .content").live('click',function(){
	...
	window.open("emr.record.browse.browsform.editor.csp?patientId="+patientId+"&id="+id+"&pluginType="+pluginType+"&chartItemType="+chartItemType+"&emrDocId="+emrDocId+"&Action=reference","");
