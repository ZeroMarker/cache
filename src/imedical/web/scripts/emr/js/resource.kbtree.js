...
$(function(){
	...
	var displayConfig = getUserConfigData(userID,userLocID,"KBTREETYPE");
	$("input:radio[value="+displayConfig+"]").attr('checked','true');
	$('#kbTree').tree({
	...
	document.getElementById("deleteKBNode").onclick = function(){ 
        ...
        //增加undefined的判断是因为新建的个人知识库目录获取为undefined
		if ((nodeparent.attributes.type == "PersonalKBTree")&&(nodeparent.attributes.personalKBUserID != userID)&&(typeof(nodeparent.attributes.personalKBUserID)!="undefined"))
		{
			alert("不能删除他人创建的个人知识库");
			return;
		}
		jQuery.ajax({
			type : "GET", 
		...
		jQuery.ajax({
			...
			success : function(d) {
	           if ( d == 1) 
			   {
					...
					var type = $("input[name='KbnodeType']:checked").val();
					GetKbnode(type);
			   }
			   else if( d == -1) 
			   {
...
function GetKbnode(type) 
{
	...
	addUserConfigData(userID,userLocID,"KBTREETYPE",type)
}
