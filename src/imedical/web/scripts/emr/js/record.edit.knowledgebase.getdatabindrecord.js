...
function getXml(RecordId)
{
	...
	jQuery.ajax({
		...
		error : function(d) { alert("getXml error");}
	});	
}
...
function readXml(xmlstring)
{
	...
	$('#dataEMRDeital').tree({ 
		...
		onSelect: function(){
				...
				if(rectype == "Section")
				{
					...
					$('#valueType').combobox({
           				 valueField:'value',
          				 textField:'label',
           				 data:[{label: '文本',value: 'T'},{label: '样式',value: 'S'}],
				...
				else
				{
					...
					else if(rectype=="MIMonoChoice")//单选
					{
						$('#valueType').combobox({
           				 valueField:'value',
          				 textField:'label',
           				 data:[{label: '值',value: 'V'},{label: '代码',value: 'C'},{label: '值^代码',value: 'VANDC'},{label: '单选',value: 'Mono'}],
        				 onLoadSuccess: function (data) {
            			 		if (data) {
               			 		$('#valueType').combobox('setValue',data[0].value);
            					}
					...
					else if(rectype=="MIMultiChoice")//多选
					{
						$('#valueType').combobox({
           				 valueField:'value',
          				 textField:'label',
           				 data:[{label: '值',value: 'V'},{label: '代码',value: 'C'},{label: '值^代码',value: 'VANDC'},{label: '多选',value: 'Mult'}],
        				 onLoadSuccess: function (data) {
            			 		if (data) {
               			 		$('#valueType').combobox('setValue',data[0].value);
            					}
					...
					else if(rectype=="MIDateTime")//时间
					{
						$('#valueType').combobox({
           				 valueField:'value',
          				 textField:'label',
           				 data:[{label: '值',value: 'V'},{label: '代码',value: 'C'},{label: '值^代码',value: 'VANDC'},{label: '年',value: 'YEAR'},{label: '月',value: 'MONTH'},{label: '日',value: 'DAY'},{label: '小时',value: 'HOUR'},{label: '分',value: 'MINUTE'},{label: '秒',value: 'SECOND'}],
        				 onLoadSuccess: function (data) {
            			 		if (data) {
               			 		$('#valueType').combobox('setValue',data[0].value);
            					}
					...
					else if(rectype=="MIDictionary")//字典
					{
						$('#valueType').combobox({
           				 valueField:'value',
          				 textField:'label',
           				 data:[{label: '值',value: 'V'},{label: '代码',value: 'C'},{label: '值和代码',value: 'VC'},{label: '代码和值',value: 'VC'},{label: '值^代码',value: 'VANDC'},{label: '字典',value: 'Dict'}],
        				 onLoadSuccess: function (data) {
            			 		if (data) {
               			 		$('#valueType').combobox('setValue',data[0].value);
            					}
		...
		onLoadError: function(){ alert("dataEMRDeitalonLoadError");}
		});
}
...
function GetSample(Sample)
{
	for (var i=0;i<Sample.length;i++) 
	{
		...
		//增加容错性，当元素名称没有值时，一般默认为新建xx单元,防止某些项目上为空值
