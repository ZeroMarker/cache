...
$(function(){
	...
	$HUI.datagrid('#dgresult',{
		...
		lineHeight:12,
		columns:[[
			{field:'qrnumber',title:'输出结果序号',width:40},
			{field:'key',title:'输出字段',width:40},
			{field:'value',title:'输出值',width:200,fontSize:10,lineHeight:14}  /*列上设置字体大小*/
		]],
		rowStyler:function(rowIndex,rowData){
				//奇偶结果数据项展示不同背景色
				//
				if (rowData.qrnumber%2==0){
						return 'background:#f0f0f4';
				}
		}
	})
...
function load()
{
	$.ajax({
		...
		error : function(d) { 
			...
			$.messager.popover({msg: "GetConfigData"+d ,type:'alert'});
		}
	});	
}
...
{	
	...
	$.ajax({
		...
		error : function(d) { 
			//alert("Debugger GetBindQueryData error");
			$.messager.popover({msg: "GetResultData"+d ,type:'alert'});
		}
	});	
}
