...
function LoadPersonTmpl(emrTmplCateid, modelId) {
    ...
    var DocID = common.getEmrDocId(tmpret);
    if (DocID === '0') {
        alert('未创建此病历模板权限，请检查模板权限配置！'+ tmpret);
        return;
    }
...
function LoadPersonRecord(recordParam) {
    ...
    var DocID = common.getEmrDocId(recordParam.emrDocId);
    if (DocID === '0') {
        alert('未创建此病历模板权限，请检查模板权限配置！'+ recordParam.emrDocId);
        return;
    }
