...
//        var returnValues = window.showModalDialog("emr.record.edit.knowledgebase.csp",array,"dialogHeight:"+xpheight+";dialogWidth:"+xpwidth+";resizable:yes;center:yes;minimize:yes;maximize:yes;");
        ...
        var iframeContent = '<iframe id="Knowledgebase" scrolling="no" frameborder="0" src="emr.knowledgebase.csp?NodeStr='+nodeStr+'&VisitTypeStr='+visitTypeStr+'&MWToken='+getMWToken()+'" style="width:100%;height:99%;"></iframe>'
...
function GetKBNodeTextByKBNodeID(kbnodeID){
    jQuery.ajax({
        ...
        success : function(d) {
	        //$('#kbTreeView')[0].style.display = "block";
			//$('#kbCategoryView')[0].style.display = "none";
            var data = d;
            if (data != ""){
                $("#kbNodeText").css("overflow-y","auto");
	...
	$('#cbCategory').combobox({  
	    ...
	    onLoadSuccess : function(){  
	    	...
	    	if (data.length > 0)
		  	...
		  	document.getElementById('kbNodeText').innerHTML="";  
		  	initcbLoc();
		},
...
{
	txt = document.getElementById('kbNodeText').innerHTML;
...
function setContent(categoryId)
{
	jQuery.ajax({
		...
		success: function(d) {
			//$("#kbNodeText").text(d);
			//$('#kbTreeView')[0].style.display = "none";
			//$('#kbCategoryView')[0].style.display = "block";
			document.getElementById('kbNodeText').innerHTML=d; 
		},
