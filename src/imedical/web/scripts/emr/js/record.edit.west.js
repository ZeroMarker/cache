...
function setRecordList(data,instanceId)
{
	...
	for (var i=0;i<data.length;i++)
	{
		...
		if (data[i].statusCode != "finished") {
			statusClass = "blue";
		}
		if ((data[i].modifyTimeFlag == 1)&&(isModifyColorChangeColor == "Y"))
		{
			var datetime = '<h3>'+data[i].happendate+'<span>'+data[i].happentime+'</span><span class="overflow" style="color:#000;">作者:'+data[i].creator+'</span></h3>';
			var content = '<a href="#">'
								+'<DIV><H3 style="color:red;">'+data[i].text+'</H3>'
								+'<P class=con><span id="operator">'+data[i].operator+'</span>&nbsp|&nbsp<span id="status" class="'+statusClass+'">'+data[i].status+'</span></P></DIV>';
						 +'</a>';
		}
		else
		{
...
function modifyListRecord(commandJson)
{
	...
	var datetime = '<h3>'+happendate+'<span>'+happentime+'</span><span class="overflow" style="color:#000;">作者:'+creator+'</span></h3>';
	var statusClass = "green";
	...
	var content = '<a href="#">'
						+'<DIV><H3>'+text+'</H3>'
						+'<P class=con><span id="operator">'+operator+'</span>&nbsp|&nbsp<span id="status" class="'+statusClass+'">'+status+'</span></P></DIV>';
				 +'</a>' 
...
function modifyZTreeRecord(commandJson)
{
	...
	if (name == undefined) name = commandJson["Title"]["DisplayName"];
	var treeObj = $.fn.zTree.getZTreeObj('InstanceTree');
	var node = treeObj.getNodeByParam("id",instanceId, null);
...
function modifyWestListFlag()
{
	...
	{
		var treeObj = $.fn.zTree.getZTreeObj('InstanceTree');
		var node = treeObj.getNodeByParam("id",param["id"], null);
		var text = "<span style='background-color:#E0ECFF;'>" +node.name+"</span>";
		if (node != null) 
		{
			node.name = text;
			treeObj.updateNode(node);	
...
function deleteZTreeRecord(instanceId)
{
	...
	var datetime = '<h3>'+node.attributes.happendate+'<span>'+node.attributes.happentime+'</span><span class="overflow" style="color:#000;">作者:'+node.attributes.creator+'</span></h3>';
	var content = '<a href="#">'
...
//显示已删除病历的操作记录明细
$("#deleteTree li #Logflag").live('click',function()
{
	var docId = $(this).attr("emrDocId");
	var Num = $(this).attr("emrNum");
	window.showModalDialog("emr.logdetailrecord.csp?EpisodeID="+episodeID+"&EMRDocId="+docId+"&EMRNum="+Num,"","dialogWidth:801px;dialogHeight:550px;resizable:yes;");
