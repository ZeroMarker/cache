...
function setDataGrid(interface)
{
	...
	$('#bloodtrans').datagrid({
	    ...
	    loadMsg:'数据装载中......',
	    autoRowHeight: true,
	    url:'../EMRservice.Ajax.bloodtrans.cls?InterFace='+encodeURI(encodeURI(interface)), 
	    rownumbers:true,
	    pagination:true,
	    singleSelect:false,
	    queryParams:param,
...
{
	var param = getParam();
	$('#bloodtrans').datagrid('load',param);
}
...
function getData()
{
	...
	var checkedItems = $('#bloodtrans').datagrid('getChecked');
...
function UnCheckAll()
{
	$("#bloodtrans").datagrid("uncheckAll");
}