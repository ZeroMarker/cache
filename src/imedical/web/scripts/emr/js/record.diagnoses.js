...
$("#addDiagnose").click(function()
{
	...
	if ($('#diagnoscat option:selected').val()==0)
	{
		...
		{
			addSubDiagnose(objDiagnos,position);
		}
		else
		{
			addDiagnose(objDiagnos,position);
		}
	}
	else
	{
		...
		if ((objDiagnos != null)&&(objDiagnos != undefined))
		{
			...
			{
				var tmp = addDiagnose(objDiagnos,position);
				tmp = tmp.split("&");
				if (tmp[0]!="0")
				{
					var tmp = tmp[0].split("||");
...
///添加诊断
function addDiagnose(objDiagnos,position)
{
	var result = "";
	//var objDiagnos = $('#icdlist').combogrid('grid').datagrid('getSelected');
	var icdId = "";
	...
	var parentId = "";
	result = addDiagnoses(icdId,icdDesc,memo,parentId,"1",position);
	return result;
}

//子诊断
function addSubDiagnose(objDiagnos,position)
{
	//var objDiagnos = $('#icdlist').combogrid('grid').datagrid('getSelected');
	var memo = $("#memo").val();
	var icdId = "";
	...
	addDiagnoses(icdId,icdDesc,memo,parentId,level,position);		
}
...
///保存增加诊断信息
function addDiagnoses(icdId,icdDesc,memo,parentId,level,position)
{
	...
	jQuery.ajax({
		...
		success: function(d) {
			...
			if ((tmpd[2] != "")&&(g_infectionsource == "his"))
			{
				/*
				...
				*/
				var arrId = tmpd[1].split("||")
				var tmpList = tmpd[2].split("^");
				var strUrl = "./dhcmed.epd.report.csp?PatientID="+patientID+"&EpisodeID="+episodeID+"&IFRowID="+tmpList[1]+"&LocFlag=0";
				if (tmpList[0]=='0')
				{
					//提示报卡
					if (window.confirm(tmpList[2]))
					{
						var retValue = window.showModalDialog(strUrl,"","dialogHeight:620px; dialogWidth:1200px");
						if (!retValue)
						{
							deleteMRDiagnos(arrId[0],arrId[1]);
						}
					}
				}
				else if (tmpList[0]=='1')
...
$("#btQuote").click(function(){	
	...
	{
		if (typeof(obj.Url.level != "undefined")&& obj.Url.level == "Attending")
		{
			return $.inArray(item.UserLevel, ["Attending","Chief"])>-1
		}
		if (g_hasCheck == "1")
		{
...
$("#delete").click(function(){
	...
	{
		var result = deleteMRDiagnos(node.PID,node.SubID);
		if (result == "0")
		{
			alert("删除失败");
		}
...
function deleteMRDiagnos(pId,subId)
{
	var result = "";
	jQuery.ajax({
		type: "get",
