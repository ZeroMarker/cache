...
$(function() {
    $('#dgChapter').datagrid({
        view: detailview,
        url: textCollector.urlTextCollector + '?action=GetChapterForGrd&usrID=' + userID + '&ctloc=' + userLocID,
        fitColumns: true,
        singleSelect: true,
        height: 'auto',
        columns: [
        ...
        onExpandRow: function(index, row) {
            ...
            var urlGetText = textCollector.urlTextCollector + '?action=GetText&usrID=' + userID + '&ctloc=' + userLocID + '&DiseaseID=' + diseaseID + '&CategoryID=' + categoryID + '&ChapterID=' + row.id;
            //alert(urlGetText);
            ddv.datagrid({
                url: urlGetText,
                height: 'auto',
                ...
                onResize: function() {
                    $('#dgChapter').datagrid('fixDetailRowHeight', index);
                },
                ...
                onSelect: function(idxText, rowText) {
                    ...
                    $('#dgChapter').datagrid('selectRow', parentRowIdx);
    ...
    $('#cboDisease').combobox({
        url: '../EMRservice.Ajax.textCollector.cls?action=GetDiseaseCategoryForCombo&ctloc=' + userLocID,
        method: 'get',
        valueField: 'value',
        textField: 'text',
        groupField: 'group',
        onSelect: function(record) {
            $('#dgChapter').datagrid('reload');
        }
    });
    ...
    $('#textAdd').bind('click', function(event) {
        //debugger;
        var row = $('#dgChapter').datagrid('getSelected');
        if (!row) {
            return;
        };
    ...
    $('#textEdit').bind('click', function(event) {
        //debugger;
        var rowChapter = $('#dgChapter').datagrid('getSelected');
        if (!rowChapter) return;
        var idx = $('#dgChapter').datagrid('getRowIndex', rowChapter);
