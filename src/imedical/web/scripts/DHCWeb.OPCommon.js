...
function DHCWeb_EStopSpaceKey()
{
	if (event.keyCode == 8) {
	   ...
	   event.preventDefault();   //+2019-05-13 ZhYW 
...
function DHCWeb_IsDate(DateString,Dilimeter) 
{
	...
	tempm=eval(tempm)-1
	tempd=eval(tempd)
	//此处不可使用getUTCFullYear获取年份，在每年的12.31和1.1号返回值和getFullYear有可能会不同。比如2019-01-01会返回是2018
	//alert(tempDate.getUTCFullYear().toString()+"^"+tempy+"  "+tempDate.getMonth()+"^"+tempm+"  "+tempDate.getDate()+"^"+tempd)
	if (((tempDate.getFullYear()).toString()==tempy) && (tempDate.getMonth()==tempm) && (tempDate.getDate()==parseInt(tempd))) 
	{ 
		return true; 
	} 
	else 
...
function chargeFail_alert(flag, errCode, job) {
	...
	switch (+errCode) {
		case 101:
			...
			if (myrtn) {
				var url = 'dhcbill.opbill.chargeexception.csp?job='+job;
				var openObj = window;   //2018-05-29 ZhYW 解决在使用Modal Dialog的时候，弹出多个页面会导致页面新页面session丢失，需要重新login
				if (typeof(window.dialogArguments) == "object") {
					openObj = window.dialogArguments;
				}
				openObj.open(url, '_blank', 'toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes,copyhistory=yes,width=1300,height=750,left=0,top=0');
			}
			break;
		case 102:
		...
		case 129:
			...
			break;
		case 130:
			alert(errMsg + '请选择全部的挂号相关费用进行结算' + ': ' + errCode);
			break;
		case 131:
		...
		case 132:
			...
			break;
		case 133:
			alert(errMsg + '门诊收费账单表金额不平' + ': ' + errCode);
			break;
		case 134:
			alert(errMsg + '门诊收费发票表与账单表金额不平' + ': ' + errCode);
			break;
		case 135:
			alert(errMsg + '门诊收费发票表自负金额与支付方式金额不平' + ': ' + errCode);
			break;
		default:
...
//字符转ASCII码,以冒号(:)拼接返回
function DHCWeb_CharTransAsc(Str) {
...
//ASCII码转字符,以冒号(:)拼接传入
function DHCWeb_AscTransChar(AscStr) {
