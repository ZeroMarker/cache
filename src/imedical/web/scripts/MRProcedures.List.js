...
function PROCDSReportFlagDisable(evt) {
	return false;
}
...
function DisableLink(fld) {
	if (fld) {
		fld.value = "";
		fld.disabled = true;
		fld.onclick=PROCDSReportFlagDisable
...
function DisableReportFlagLinks() {
	...
	for (var curr_fld=0; curr_fld<ary.length; curr_fld++) {
		...
		if (obj) {
			if (obj.id.substring(0,17) == "PROCDSReportFlagz") {
				DisableLink(obj);
			}
		}
	}
...
function MRProceduresList_PassSelected(lnk,newwin) {
	var f,aryfound;
