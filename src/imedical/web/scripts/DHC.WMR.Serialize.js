...
var CHR_4=String.fromCharCode(4); //Add By LiYang 2008-11-12
var CHR_Up="^";
var CHR_Tilted="/";
...
function buildDHCWMRHistory(str)
{
	...
	obj.PatientNo = arry[26];      //add by zf 20090903
	return obj;
}
...
function BuildDHCWMRExamResult(str)
{
    ...
    obj.RuleDr = arry[2];//	
    obj.Veto = (arry[3] == "Yes");//	Modified By LiYang 2008-12-9
    obj.Score = arry[4];//	
    obj.ActualScore = arry[5];//	
    obj.IsPrimary = (arry[6] == "Yes");//		Modified By LiYang 2008-12-9
    obj.GradeDr = arry[7];//	
    obj.Remind = arry[8];//	
    obj.Money = arry[9];//	
    ...
    obj.IsActive = (arry[13] == "Yes");//		Modified By LiYang 2008-12-9
...
function BuildDHCWMRExamRDtl(str)
{
    ...
    obj.Parref = GetCode(arry[0], "||");//	DHC_WMR_ExamResult
    obj.ChildSub = GetDesc(arry[0], "||");//
    obj.RowID = arry[0];//		
    obj.EntryDr = arry[1];//	
    obj.Number = arry[2];//	
    obj.Score = arry[3];//	
    obj.Money = arry[4];//	
    obj.TriggerDate = arry[5];//	
    obj.ErrType = arry[6];//	
    obj.IsActive = (arry[7] == "Y");//	
    obj.ResumeText = arry[8];//	
	return obj;
}
...
function SerializeExamEntryForApplet(objCurrRule)
{
    ...
    for(var i = 0; i < objCurrRule.SectionList.length; i ++)
    {
    	...
    	for(var j = 0; j < objSection.EntryList.length; j ++)
    	{
		    ...
		    str += (objEntry.Veto ? "Y" : "N") + appletFieldSplit;
...
function SerializeDHCWMRFPICD(obj)
{
    ...
    str += obj.FPICDType+CHR_Up;
    str +=obj.SrcType+CHR_Up;
    str+=obj.OperationRank;
    return str;
}
...
function BuildDHCWMRFPICD(str)
{
	...
	//update by zf BJWJ
    var arryOpePeople = arry[12].split("#");
    if(arryOpePeople.length != 4)
    {
    	arryOpePeople = ["","","",""]; //Default Value
    }
    ...
    else
    {
    	...
    	obj.OperatorObj = {RowID:"", Code:"", UserName:arryOpePeople[0]};
    }
    if(arry[5] != "")
    {
    	arryTmp = arry[5].split(CHR_Tilted);	
    ...
    else
    {
    	...
    	obj.AssistantDr1Obj = {RowID:"", Code:"", UserName:arryOpePeople[2]};
    }
   	if(arry[6] != "")
    {
  	...
  	else
  	{
		//update by zf BJWJ
  		...
  		obj.AssistantDr2Obj = {RowID:"", Code:"", UserName:arryOpePeople[3]};
  	}
    ...
    if(arry[8] != "")
    {
  	...
  	else
  	{
		...
		//obj.NarcosisDoctorDrObj = {RowID:"", Code:"", UserName:""};
  		obj.NarcosisDoctorDrObj = {RowID:"", Code:"", UserName:arryOpePeople[1]};
  	}
    obj.CloseUp = arry[9];
...
function BuildAdmitOperation(str)
{
	...
	//alert("sarry.length:"+arry.length);
	switch(arry.length)
	{
		case 4:
		...
		case 16:
			...
			obj.CutType=arry[13];     //add by zf 2008-09-23
			obj.CloseType=arry[14];   //add by zf 2008-09-23
			obj.OperationRank=arry[15]; //add by wangcs 2012-01-12
			break;	
	}
...
function BuildDHCMedAdmitDiagnose(str)
{
	...
	arryTmp = arryFields[4].split("%%");  //²¡ÀíÕï¶ÏÓÐ"/"
	if(arryTmp.length == 3)
	{
	...
	if(arryFields.length > 6)//Modified by LiYang 2008-09-16
	{
...
//Build Entire Exam result detail list
function BuildDHCWMRExamResultDetailList(str)
{
	var arryDetail = new Array();
	var arryPeople = null;
	if ((str == "")||(str == null))
		return arryDetail;
	var arryTmp = str.split(CHR_1);
	var strTmpDetail = "";
	var objDetail = null;
	var strTmp = "";
	var strTmpPeople = "";
	...
	{
		strTmpDetail = arryTmp[i];
		strTmp = GetArryItem(strTmpDetail, CHR_2, 0);
		objDetail = BuildDHCWMRExamRDtl(strTmp);
		if (objDetail != null)
		{
			strTmp = GetArryItem(strTmpDetail, CHR_2, 1);
			arryPeople = strTmp.split(CHR_3)
			for(var j = 0; j < arryPeople.length; j ++)
			{
				strTmpPeople = arryPeople[j];
				if(strTmpPeople == "")
					continue;
				objPeople = BuildDHCWMRExamRDtlPeople(GetArryItem(strTmpPeople, CHR_4, 0));
				strUser = GetArryItem(strTmpPeople, CHR_4, 1);
				objUser = {
						RowID:GetArryItem(strUser, CHR_Tilted, 0),
						Code:GetArryItem(strUser, CHR_Tilted, 1),
						UserName:GetArryItem(strUser, CHR_Tilted, 2)
					
					};
				strLoc = GetArryItem(strTmpPeople, CHR_4, 2);
				objLoc = {
					RowID:GetArryItem(strLoc, CHR_Tilted, 0),
					Name:GetArryItem(strLoc, CHR_Tilted, 1)
					};
				objPeople.User = objUser;
				objPeople.Department = objLoc;
				objDetail.DoctorList.push(objPeople);
			}
		}
		arryDetail.push(objDetail);
	}
	return arryDetail;
}
