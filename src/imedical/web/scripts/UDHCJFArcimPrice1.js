...
function BodyLoadHandler() {
	...
	if(obj)obj.onclick=AddAlias;
	var obj=document.getElementById("OrderStatus")
	if(obj){
		obj.size=1
		obj.multiple=false;
		obj.onchange=OrderStatusChange;
		obj.options[obj.length]=new Option("全部",0)
		obj.options[obj.length]=new Option("可用",1)
		obj.options[obj.length]=new Option("不可用",2)
		
		var orderStatusObj=document.getElementById("OrderStatusId")
		if(orderStatusObj){
			if(orderStatusObj.value!=""){
				obj.value=orderStatusObj.value
			}else{
				obj.value=1
				OrderStatusChange()	
			}
...
function SelectRowHandler()	{
   ...
   if (!selectrow) return;
   if(selectrow!=SelectedRow){SelectedRow=selectrow;}
   else {SelectedRow=0;}	
}
function AddAlias(){
	if (!SelectedRow) {alert("请选中要添加别名的医嘱项");return;}
	var Arcim="",Alias="";
	var SelRowObj=document.getElementById('TArcimz'+SelectedRow);
	if(SelRowObj) Arcim=SelRowObj.value;
	var obj=document.getElementById('OrdAlias');
	if(obj) Alias=obj.value;
	if((Alias=="")||(Arcim=="")){alert("医嘱别名不能为空");return;}
	var Ret=tkMakeServerCall("web.DHCBL.CT.ARCAlias","AddOrderAlias",Arcim+"^"+Alias);
	eval("var Ret=" + Ret);
	var rtnObj=Ret;			//JSON.parse(Ret);
...
function OrderStatusChange(){
	...
	var statusidObj=document.getElementById("OrderStatusId")
	if(statusidObj) statusidObj.value=status
}
