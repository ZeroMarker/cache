	      ...
	      if ((tmpno!=prescno)&&(tmpid!=relateid)&&(i!=1))
          {
	       ...
	       if (priority!="出院带药"){
	        ...
	        for (var n=1;n<=parseInt(facotor)*2;n++)
	            {
            setBottomLine(xlsheet,startrow-1,1,12);
		    xlsheet.Cells(startrow, startcol1).Value = "床号"+" "+bed

		    xlsheet.Cells(startrow+1, startcol1).Value = "登记号"+" "+pano
	
		    xlsheet.Cells(startrow+2, startcol1).Value = "姓名"+" "+paname
	
		    xlsheet.Cells(startrow+3, startcol1).Value = "科室"+" "+admloc
		
		    xlsheet.Cells(startrow+4, startcol1).Value = "病区"+" "+getDesc(ward)
	
		    xlsheet.Cells(startrow+5, startcol1).Value = "剂量"+" "+facotor
