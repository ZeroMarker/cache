...
function initQueryMenu() {
	...
	var Flag=url.split("Flag=")[1]
		...
		for (var i = 0;i<dp.length;i++){
			...
			if(DomId){
				...
				if(inJson.input.setlinfo[DomId]){					
					var data = {};
					data["Code"] = dp[i].id;
					data["Name"] = dp[i].title;
					data["value"] = inJson.input.setlinfo[DomId];
					datas.push(data);
					$(DomSelect).val(inJson.input.setlinfo[DomId]);
					///$(DomSelect).validatebox({required:true});
				}else{
					var data = {};
					data["Code"] = dp[i].id;
					data["Name"] = dp[i].title;
		...
		var datagridData = {total:datas.length,rows:datas};
		if(datagridData.rows){$("#setlinfo").datagrid({loadFilter:pagerFilter}).datagrid('loadData',datagridData)};
