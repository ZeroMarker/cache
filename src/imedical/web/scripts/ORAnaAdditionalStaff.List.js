...
var frm = document.forms["fORAnaAdditionalStaff_List"];

function DocumentLoadHandler(e) {
	var objTable=document.getElementById("tORAnaAdditionalStaff_List"); 
	var eSrc;
...
function makeReadOnly() {
	var el=document.forms["fORAnaAdditionalStaff_List"].elements;  
	if(!el) {return;}
...
function TableClickHandler(e) {
	...
	var rowid=document.getElementById("ANAASRowId");
	...
	if (parent.frames["frm_edit"]) {
		...
		if (eSrc.tagName=="A") {
			if (eSrc.id.indexOf("ANAASCareProvTypez")==0) { //if the field id is "CTPCP_Descz" ...
...
function NewClickHander(e) {
	if ((parent)&&(parent.frames)&&(parent.frames["frm_edit"])) {
		...
		var lnk="websys.default.csp?WEBSYS.TCOMPONENT=ORAnaAdditionalStaff.Edit&" +"&PARREF="+parref+"&PatientID="+patid+"&EpisodeID="+epid;
