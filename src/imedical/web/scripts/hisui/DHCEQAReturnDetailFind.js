...
function GetReportData(num)
{
	...
	//HISUI改造-修改TJob取值获取不到的问题 测试组需求修改 modify by jyp 20200212    JYP0020  需求号：1188971
...
function RowDblClick()
{
	...
	//alertShow(selectrow)
	var str="websys.default.hisui.csp?WEBSYS.TCOMPONENT=DHCEQAReturn&RowID="+GetElementValue("TRowIDz"+selectrow)+"&CurRole="+GetElementValue("ApproveRole")+"&QXType="+GetElementValue("QXType")+"&WaitAD="+GetElementValue("WaitAD")+"&Status="+GetElementValue("Status")+"&ReturnTypeDR="+GetElementValue("ReduceType");
...
function DblClickRowHandler(rowIndex,rowData)
{
	if (rowData.TRowID>0)
	{
		...
		var str="websys.default.csp?WEBSYS.TCOMPONENT=DHCEQAReturn&RowID="+rowData.TRowID;
