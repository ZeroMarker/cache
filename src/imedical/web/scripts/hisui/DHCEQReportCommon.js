...
function PrintEQReport(templateName,isSave,savefilename,colset)
{
	...
	{
		PrintEQReport_Chrome(templateName,isSave,savefilename,colset)
	}
	else
	{
		PrintEQReport_IE(templateName,isSave,savefilename,colset)
	}
}
function PrintEQReport_Chrome(templateName,isSave,savefilename,colset)
{
	//获取数据
	var HadData=true;
	var num=1;
	...
	var NewFileName=filepath(savefilename,"\\","\\\\")
	...
	Str +="xlsheet.cells(2,1)='开始日期:"+GetElementValue("StartDate")+"';";
	Str +="xlsheet.cells(2,3)='结束日期:"+GetElementValue("EndDate")+"';";
	Str +="var DataCount='"+DataInfo.length+"';"
	Str +="var DataInfoStr='"+DataInfoStr+"';"
	Str +="var DataInfo=DataInfoStr.split(',');"
	Str +="var colset='"+colset+"';"
	Str +="var colsetList=colset.split('^');"
	Str +="var Length=colsetList.length;"
	Str +="for (var CurRow=0; CurRow<DataCount; CurRow++){"
	Str +="var CurDataStr=DataInfo[CurRow];"
	Str +="var dataList=CurDataStr.split('^');"
	Str +="for (var I=0; I<Length; I++){"
	Str +="var colinfo=colsetList[I].split(':');"
	Str +="var col=colinfo[0];"
	Str +="var content=colinfo[1];"
	Str +="xlsheet.cells(CurRow+4,col)=dataList[content];"
	Str +="if ((col==1)&&(dataList[content]=='')) xlsheet.cells(CurRow+4,1)='合计';"
	Str +="for (var J = 1; J < 5; J++){"
	Str +="xlsheet.cells(CurRow+4,col).Borders(J).LineStyle=1;"
	Str +="xlsheet.cells(CurRow+4,col).Borders(J).Weight=2;"
	Str +="}}}"
	Str +="if ("+isSave+"==1){xlBook.SaveAs('"+NewFileName+"');}else{xlsheet.printout;}"
	...
	if ((isSave=="1")&&(rtn.rtn==1))
	{
	    alertShow("导出完成!");
	}
}
function PrintEQReport_IE(templateName,isSave,savefilename,colset)
{
	var TemplatePath=GetElementValue("GetRepPath");
	try {
...
function  PrintDHCEQEquipNew(TableName,SaveOrPrint,job,vData,TempNode,GetRowsPerTime)
{
	...
	if (num<=0) {alertShow("表没值")}  //modified by sjh 2019-12-06 BUG00019 修改只有一条数据时不能导出
	else
	{
     try {
	     if (SaveOrPrint=="1")
