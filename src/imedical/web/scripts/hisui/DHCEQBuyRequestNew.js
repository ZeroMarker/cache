...
{
	KeyUp("BRLName^RequestLoc^PurchaseType^PurposeType^BRLModel^UseLoc^BRLManuFactory^BRLCurrency^EquipType","N");
	InitUserInfo();
	InitPage();
	FillData();
	IsYear();
	...
	SetTableMarginbottom();	//HISUI改造 add by csj 20190201 调整表单间距
	///ReplaceTitle("设备","家具");
}
function InitPage()
{
	...
	///modified by zy 20181217 ZY0181  审核按钮代码处已经定义事件
	...
	//2019-05-29 ZY0187
	var obj=document.getElementById("BRLQuantityNum");
	if (obj) obj.onchange=function(){
		var BRLQuantityNum=+GetElementValue("BRLQuantityNum");
		if (IsValidateNumber(BRLQuantityNum,0,0,0,0)==0)
		{
			alertShow("审批数量数据异常,请修正.");
...
function FillData()
{
	...
	//SetChkElement("YearFlag",valueRequest[2]);  //modify by wl 2019-9-4  1014272
	...
	//messageShow("","","",valueRequest[sortRequest+20])
	...
	//messageShow("","","",valueRequest[sortRequest+16])
	...
	//messageShow("","","",valueRequestList[sortRequestList+1])
	...
	*/
	...
	//add by zy ZY0174 同一个元素在界面两个位置只有一个地方赋值有效
	SetElement("Unit1",valueRequestList[sortRequestList+7]);
	SetElement("CommonName",valueRequestList[2]); //2013-06-24 DJ0118
	
	var valueRequestDetail=list[2].split("^");
	var sortRequestDetail=29;
	//messageShow("","","",valueRequestList[sortRequestDetailt+1])
	...
	//messageShow("","","",valueArgumentation[4])
...
function SetEnabled()
{
	...
	{
		//Modefied BY QW20190826 需求号:976170 只在审核前设置默认意见
		if (Status==1) 
		{
			if (GetElementValue("EditOpinion")=="") SetElement("EditOpinion","同意");	//2013-06-24 DJ0118
		}
		//End BY QW20190826 需求号:976170 
...
function SetDisplay()
{
	...
	//add by zy ZY0174 同一个元素在界面两个位置只有一个地方赋值有效
	ReadOnlyElements("RequestNo^StatCat^BRLEquipCat^BRLTotalFee^Unit^AuditOpinion^BRDUseYearsNum^BRDFeeOfDepre^BRDCostFee^Unit1",true)
...
function BUpdate_Clicked()
{
	...
	else
	{
		if (list[0]>0)
		{
			...
			ShowMessage();
			//***************************************
			window.location.href= 'websys.default.hisui.csp?WEBSYS.TCOMPONENT=DHCEQBuyRequestNew&RowID='+list[0]+"&WaitAD=off"+"&YearFlag="+GetElementValue("YearFlag"); //hisui改造 modify by lmm 2018-08-18
...
function BDelete_Clicked()
{
	...
	messageShow("confirm","","",t["02"],"",confirmFun,"")
}
///modified by ZY0215 2020-04-02
function confirmFun()
{
    ...
    {
	    window.location.href= 'websys.default.hisui.csp?WEBSYS.TCOMPONENT=DHCEQBuyRequestNew'+"&YearFlag="+GetElementValue("YearFlag"); //hisui改造 modify by lmm 2018-08-18
		websys_showModal("options").mth();  //modify by lmm 2019-02-20
	}
}
...
function BSubmit_Clicked()
{
	...
	if (rtn>0)
    {
	    window.location.href='websys.default.hisui.csp?WEBSYS.TCOMPONENT=DHCEQBuyRequestNew&RowID='+rtn+"&WaitAD="+GetElementValue("WaitAD")+"&YearFlag="+GetElementValue("YearFlag"); //hisui改造 modify by lmm 2018-08-18
		websys_showModal("options").mth();  //modify by lmm 2019-02-20
	}
    else
    {
	    messageShow("","","",t[rtn]+"   "+t["01"]);
    }
}
...
function BApprove_Clicked()
{
	...
	if(GetElementValue("BRLQuantityNum")=="") var BRLQuantityNum=""   //add by jyp 20190307
	var BRLRequestNum=parseInt(GetElementValue("BRLRequestNum"))
	if ((BRLQuantityNum!="")&&(!(BRLQuantityNum>0&&BRLQuantityNum<=BRLRequestNum))) {messageShow("","","",t[-1002]);return;}		//Modify DJ 2015-09-14 DJ0164	modified by czf 20190220  modify by jyp 20190307
	...
	//messageShow("","","",combindata+","+CurRole+","+RoleStep+","+EditFieldsInfo+","+rtn)
    if (rtn>0)
    {
...
function BArgumentation_Clicked()
{
	...
	showWindow(str,"采购论证","","","icon-w-paper","modal");   //modify by lmm 2019-02-16
...
function SetTotal()
{
	...
	if (IsValidateNumber(QuantityNum,0,0,0,0)==0)
	{
...
function BALter_click()
{
	...
	var str='websys.default.hisui.csp?WEBSYS.TCOMPONENT=DHCEQBuyReqListAlter&BuyReqListDR='+BRLRowID+'&ReadOnly='+GetElementValue("ReadOnly");  //hisui改造 modify by lmm 2018-08-18
...
function BClear_Clicked()
{
	...
	window.location.href= 'websys.default.hisui.csp?WEBSYS.TCOMPONENT=DHCEQBuyRequestNew'+lnkvar; //hisui改造 modify by lmm 2018-08-18
...
function SetReclaimYearsNum()
{
	...
	///modified by zy 20181102  ZY0176 修改界面逻辑问题
