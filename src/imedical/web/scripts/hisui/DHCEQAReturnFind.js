...
{
	var objtbl=document.getElementById("tDHCEQAReturnFind");
...
function RowDblClick()
{
	...
	var str="websys.default.hisui.csp?WEBSYS.TCOMPONENT=DHCEQAReturn&RowID="+GetElementValue("TRowIDz"+selectrow)+"&CurRole="+GetElementValue("ApproveRole")+"&QXType="+GetElementValue("QXType")+"&WaitAD="+GetElementValue("WaitAD")+"&Status="+GetElementValue("Status")+"&ReturnTypeDR="+GetElementValue("ReturnTypeDR");
...
/*/ // Mozy0236	2019-11-29	配件退货业务(HisUi)		注释双击事件
function DblClickRowHandler(rowIndex,rowData)
{
	if (rowData.TRowID>0)
	{
		//Mozy	2019-5-28	914623,914675	取消Hisui风格
		var str="websys.default.csp?WEBSYS.TCOMPONENT=DHCEQAReturn&RowID="+rowData.TRowID+"&CurRole="+GetElementValue("ApproveRole")+"&QXType="+GetElementValue("QXType")+"&WaitAD="+GetElementValue("WaitAD")+"&Status="+GetElementValue("Status")+"&ReturnTypeDR="+GetElementValue("ReturnTypeDR");
...
{
	var url="dhceq.mp.return.csp?"+"&Type="+GetElementValue("Type")+"&QXType="+GetElementValue("QXType")+"&WaitAD="+GetElementValue("WaitAD");
	showWindow(url,"配件退货单","","","icon-w-paper","modal","","","large"); //modify by lmm 2020-06-05 UI
}
