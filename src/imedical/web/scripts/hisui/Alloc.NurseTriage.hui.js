...
function InitAllocListTabDataGrid(){
	...
	var Columns=[[ 
		...
		{field:'PatientNo',title:'登记号',width:100,sortable:true},
		...
		{field:'AsStatusDesc',title:'排班状态',width:140,sortable:true},
		{field:'RegFeeDateTime',title:'挂号时间',width:160,sortable:true},
		{field:'AdmDate',title:'就诊日期',width:100,sortable:true},
		{field:'AdmWeek',title:'就诊星期',width:100,sortable:true},
		{field:'AdivseAdmTime',title:'建议就诊时间',width:200,sortable:true},
		{field:'PAAdmReMark',title:'患者备注',width:100,sortable:true,
		...
		{field:'ASNoLimitLoadFlag',hidden:true,title:''},
		...
		{field:'DeptDr',title:'',hidden:true}
    ]]
...
function LoadDocList(QueID){
	$.cm({
		...
		for (var i=0;i<data.length;i++){
			...
			$($("#acc ul")[2]).append('<li><a onClick="ZMarkDocClick(\''+id+'\')" id="MarkDoc_'+id+'">'+name+'</a></li>');
		}
	})
}
function LoadPatQueue(MarkID){
	if ($("#allocroomview")[0].contentWindow.GetSelDivId) {
		MarkID=$("#allocroomview")[0].contentWindow.GetSelDivId();
	}
	if (!MarkID) MarkID="";
