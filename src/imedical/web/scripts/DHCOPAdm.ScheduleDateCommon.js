...
document.writeln('<div id=meizzDateLayer style="position: absolute; width: 200; height: 166; z-index: 9998; display: none">');
...
document.writeln('             onfocus="this.blur()" title="显示当前时间" style="cursor: hand;BACKGROUND-COLOR: #808080;BORDER-BOTTOM: #808080 1px outset; BORDER-LEFT: #808080 1px outset; BORDER-RIGHT: #808080 1px outset; BORDER-TOP: #808080 1px outset;font-size: 12px; height: 20px;color: #FF00FF; font-weight: bold"></td><td ');
...
var meizzTheYear=DateObj.getFullYear(); //定义年的变量的初始值
var meizzTheMonth=DateObj.getMonth()+1; //定义月的变量的初始值
...
var m = (strYear) ? strYear : DateObj.getFullYear();
...
var m = (strMonth) ? strMonth : DateObj.getMonth() + 1;
...
{
    meizzTheYear = DateObj.getFullYear();
    meizzTheMonth = DateObj.getMonth()+1;
    meizzSetDay(meizzTheYear,meizzTheMonth);
}
...
var obj=new Date(yy,mm-1,1)
var day1 = 1,firstday = obj.getDay(); //某月第一天的星期几
...
{ var da = eval("document.all.meizzDay"+i)     //书写新的一个月的日期星期排列
    if (meizzWDay[i]!="")
      { da.innerHTML = "<b>" + meizzWDay[i] + "</b>";
        da.style.backgroundColor = (yy == DateObj.getFullYear() &&
        mm == DateObj.getMonth()+1 && meizzWDay[i] == DateObj.getDate()) ? "#FFD700" : "#73a6de";
...
if (outObject)
{
    ...
    if (DateFormat==3){
	    outObject.value= yy + "-" + mm + "-" + n ; //注在这里你可以输出改成你想要的格式
	}
	if (DateFormat==4){
    ...
    closeLayer();
    WeekDate_change(outObject.value);
}
